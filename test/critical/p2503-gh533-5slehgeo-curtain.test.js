...
describe('P2503 GH#533 Moes 5slehgeo curtain sacred couple', () => {
  ...
  it('current-fps maps 5slehgeo to curtain_motor not climate', () => {
    const fps = JSON.parse(fs.readFileSync(
      path.join(ROOT, 'scripts/data/current-fps.json'),
      'utf8',
    ));
    const map = fps.manufacturerToDrivers || fps.mfrDrivers || fps;
    // structure: often { drivers: { climate_sensor: { manufacturerName: [] } }, reverse: { mfr: [drivers] } }
    const reverse = fps.byManufacturer || fps.mfrIndex || null;
    if (reverse && reverse._TZE204_5slehgeo) {
      assert.deepEqual(reverse._TZE204_5slehgeo, ['curtain_motor']);
    }
    const blob = JSON.stringify(fps);
    // Contre quoi: reverse map must not say climate-only for this couple
