...
class UniversalFlowCardManager {
    ...
    async init() {
        ...
        this.homey.flow.getActionCard('force_refresh_pids').registerRunListener(async (args) => {
        ...
        this.homey.flow.getActionCard('set_sensing_threshold').registerRunListener(async (args) => {
            if (args.device.id !== this.device.id) return false;
        ...
        this.homey.flow.getActionCard('calibrate_virtual_energy').registerRunListener(async (args) => {
             if (args.device.id !== this.device.id) return false;
