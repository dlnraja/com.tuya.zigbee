  ...
  _getFlowCard(id, type = 'trigger') {
    try {
      ...
      if (!this.homey || !this.homey.flow) {
        this.error(`[FLOW-SAFE] ❌ Homey.flow not available yet for card ${id}`);
        return null;
      }

      // v7.2.4: Unified type mapping (Fixes Issue #170)
      const typeMap = {
      ...
      const method = typeMap[type] || 'getTriggerCard';
      
      if (typeof this.homey.flow[method] !== 'function') {
        this.error(`[FLOW-SAFE] ❌ Method homey.flow.${method} does not exist`);
        return null;
      }
