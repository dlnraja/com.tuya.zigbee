...
0. **Cross-App Prompt Mandate**: Every AI prompt and automation result must benefit both app tracks by default: `master` (9.0.x preview) and `stable-v5` (5.12.x LTS). **Both share App ID `com.dlnraja.tuya.zigbee`** — Publish Stable→Test overwrites master Test. Before answering or patching, classify whether the discovery is master-only, stable-only, or a safe cross-app candidate. Never copy App IDs, versions, publish metadata, or branch-specific store links between tracks.
