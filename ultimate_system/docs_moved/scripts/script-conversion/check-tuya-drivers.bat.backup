...
:: Configuration
set "REPORT_FILE=tuya-drivers-report.txt"
...
for /f "delims=" %%i in ('dir /b /ad "%DRIVERS_DIR%\*"') do (
    ...
    if exist "%DRIVERS_DIR%\%%i\driver.compose.json" (
        set /a HAS_CONFIG=1
        set /a VALID_DRIVERS+=1
        echo !DRIVER_NAME! - CONFIG: OK >> "%REPORT_FILE%"
        
        :: Vérifier les icônes
        dir /b "%DRIVERS_DIR%\%%i\*.png" >nul 2>&1
        if !ERRORLEVEL! EQU 0 (
            set /a HAS_ICONS+=1
