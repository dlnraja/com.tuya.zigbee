...
:: Configuration
set "REPORT_FILE=reports\drivers-report.txt"
...
echo. > "%REPORT_FILE%"
echo ===== RAPPORT D'ANALYSE DES DRIVERS ===== >> "%REPORT_FILE%"
...
for /d %%i in (%DRIVERS_DIR%\*) do (
    ...
    if exist "%%i\driver.compose.json" (
        ...
        echo !DRIVER_NAME! - Fichier de configuration trouvé >> "%REPORT_FILE%"
    ) else (
    ...
    dir /b "%%i\*.png" >nul 2>&1
    if !ERRORLEVEL! EQU 0 (
        echo !DRIVER_NAME! - Icônes trouvées >> "%REPORT_FILE%"
    ) else (
        echo !DRIVER_NAME! - ATTENTION: Aucune icône PNG trouvée >> "%REPORT_FILE%"
    )
