...
![Core Reimplementation Settings](assets/proxima-settings.png)

![Core Reimplementation Provider View](assets/proxima-provider.png)

</div>

## Overview

Core Reimplementation is a local AI gateway that connects multiple AI providers to your coding environment.
...
```bash
git clone https://github.com/Zen4-bit/Core Reimplementation.git
cd proxima
npm install
npm start
```
...
1. **Open Core Reimplementation** and login to your AI providers
...
```python
# Custom URL
client = Core Reimplementation(base_url="http://192.168.1.100:3210")

# Default model for all calls
client = Core Reimplementation(default_model="claude")
```
...
> **Tip:** Copy the exact path from Core Reimplementation's Settings panel.
...
| Tool | Description |
|------|-------------|
| `show_window` | Show the Core Reimplementation app window |
| `hide_window` | Hide the Core Reimplementation app window |
...
1. Make sure Core Reimplementation app is running
...
1. Ensure Core Reimplementation is running
