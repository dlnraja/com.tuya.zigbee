...
/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║      3-GANG SWITCH - v9.7.2 UNIVERSAL                                        ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 ...
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
class Switch3GangDevice extends PhysicalButtonMixin(BatteryMixin(VirtualButtonMixin(UnifiedSwitchBase))) {
  ...
  async onNodeInit({ zclNode }) {
    await this._safeInvoke(async () => {
      ...
      this.log('[SWITCH-3G] ✅ Universal initialization complete');
    }, 'onNodeInit');
  }
  ...
  async _cleanupOrphanCapabilities() {
    ...
    for (const cap of currentCaps) {
      if (!validCaps.includes(cap) && !cap.startsWith('button')) {
        this.log(`[SWITCH-3G] ⚠️ Removing orphan capability: ${cap}`);
