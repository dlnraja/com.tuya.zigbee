...
class LedBulbDevice extends TuyaDevice {
    async onInit() {
        // OPTIMIZED VERSION 3.5.4
        this.log('led-bulb device initializing (optimized)...');
        ...
        this.setupPolling();
        this.log('LedBulb device is initializing...');
    ...
    async initializeCapabilities() {
        ...
        this.log('Initializing capabilities for led-bulb');
    }
    ...
    async pollDevice() {
        try {
            ...
            this.log('Polling led-bulb device...');
        } catch (error) {
...
module.exports = LedBulbDevice;
    // TUYA-LIGHT OPTIMIZATIONS
    async initializeTuyaLightFeatures() {
        this.log('Initializing Tuya-Light features for led-bulb');
    ...
    async enhancedPollDevice() {
        try {
            this.log('Enhanced polling led-bulb...');
    ...
    setupAutoFingerprint() {
        ...
        this.log('Auto-fingerprint enabled for led-bulb');
    }
    
    setupFallbackParsing() {
        // Enhanced fallback parsing for tuya-light
        this.fallbackParsing = true;
        this.log('Fallback parsing enabled for led-bulb');
    }
    
    fallbackPolling() {
        // Basic fallback polling
        this.log('Using fallback polling for led-bulb');
    }
