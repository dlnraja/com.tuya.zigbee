...
class TouchSwitch3gangDevice extends ZigBeeDevice {

    async onNodeInit() {
        this.log('touch_switch_3gang device initialized');
...
// TODO: Consider debouncing capability updates for better performance
    ...
    async onDeleted() {
        this.log('touch_switch_3gang device deleted');
    }
  ...
  async registerFlowCardHandlers() {
    ...
    // Condition: OnOff
    try {
      const isOnCard = this.homey.flow.getDeviceConditionCard('touch_switch_3gang_is_on');
      if (isOnCard) {
    ...
    alarmCaps.forEach(alarmCap => {
      try {
        const conditionCard = this.homey.flow.getDeviceConditionCard(`touch_switch_3gang_${alarmCap}_is_active`);
        if (conditionCard) {
    ...
    measureCaps.forEach(measureCap => {
      try {
        // Greater than
        const gtCard = this.homey.flow.getDeviceConditionCard(`touch_switch_3gang_${measureCap}_greater_than`);
        if (gtCard) {
        ...
        // Less than
        const ltCard = this.homey.flow.getDeviceConditionCard(`touch_switch_3gang_${measureCap}_less_than`);
        if (ltCard) {
    ...
    // Action: Turn On
    try {
      const turnOnCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_turn_on');
      if (turnOnCard) {
    ...
    // Action: Turn Off
    try {
      const turnOffCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_turn_off');
      if (turnOffCard) {
    ...
    // Action: Toggle
    try {
      const toggleCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_toggle');
      if (toggleCard) {
    ...
    // Action: Set Dim
    try {
      const setDimCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_set_dim');
      if (setDimCard) {
    ...
    try {
      const setTempCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_set_temperature');
      if (setTempCard) {
    ...
    try {
      const openCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_open');
      if (openCard) {
      ...
      const closeCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_close');
      if (closeCard) {
      ...
      const setPosCard = this.homey.flow.getDeviceActionCard('touch_switch_3gang_set_position');
      if (setPosCard) {
