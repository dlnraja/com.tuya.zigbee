...
class SmartKnobRotaryDevice extends ZigBeeDevice {
  ...
  async _triggerRotateLeft() {
    ...
    // Trigger flow card
      const rotateLeftTrigger = (() => { try { return this.homey.flow.getDeviceTriggerCard('remote_button_wireless_hybrid_rotate_left'); } catch(e) { return null; } })();
  ...
  async _triggerRotateRight() {
    ...
    // Trigger flow card
      const rotateRightTrigger = (() => { try { return this.homey.flow.getDeviceTriggerCard('remote_button_wireless_hybrid_rotate_right'); } catch(e) { return null; } })();
  ...
  async _triggerButtonPress(action) {
    ...
    try {
      await this.homey.flow.getDeviceTriggerCard().trigger(this, { action }).catch(() => {});
    } catch (e) { /* ignore */ }
    ...
    if (action === 'single' || action === 'on' || action === 'off' || action === 'toggle') {
      specificCardId = 'remote_button_wireless_hybrid_single_press';
    } else if (action === 'double') {
      specificCardId = 'remote_button_wireless_hybrid_double_press';
    } else if (action === 'hold' || action === 'long') {
      specificCardId = 'remote_button_wireless_hybrid_long_press';
    }
