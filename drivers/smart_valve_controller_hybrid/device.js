...
class SmartValveControllerHybridDevice extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('smart_valve_controller_hybrid initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('smart_valve_controller_hybrid');
  ...
  async onDeleted() {
    this.log('smart_valve_controller_hybrid deleted');
  }

}

module.exports = SmartValveControllerHybridDevice;
