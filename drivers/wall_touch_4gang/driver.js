...
class WallTouch3GangHybridDriver extends Driver {
 ...
 // this.registerFlowCards();
  ...
  async registerFlowCards() {
    ...
    this.buttonPressedTrigger = this.homey.flow.getDeviceTriggerCard('wall_touch_4gang_button_pressed');
    this.buttonPressedTrigger.registerRunListener(async (args, state) => {
      return args.button === state.button;
    });
    
    // Button 1 pressed
    this.button1PressedTrigger = this.homey.flow.getDeviceTriggerCard('wall_touch_4gang_button1_pressed');
    
    // Button 2 pressed  
    this.button2PressedTrigger = this.homey.flow.getDeviceTriggerCard('wall_touch_4gang_button2_pressed');
    
    // Button 3 pressed
    this.button4PressedTrigger = this.homey.flow.getDeviceTriggerCard('wall_touch_4gang_button4_pressed');
    
    // Button combination
    this.buttonCombinationTrigger = this.homey.flow.getDeviceTriggerCard('wall_touch_4gang_button_combination');
    
    // Power source changed
    this.powerSourceChangedTrigger = this.homey.flow.getDeviceTriggerCard('wall_touch_4gang_power_source_changed');
    
    // Tamper alarm
    this.tamperAlarmTrigger = this.homey.flow.getDeviceTriggerCard('wall_touch_4gang_tamper_alarm');
    
    // CONDITIONS
    
    // Is on battery
    this.homey.flow.getConditionCard('wall_touch_4gang_is_on_battery')
      .registerRunListener(async (args) => {
        return args.device.powerType === 'BATTERY';
      });
    
    // Is on mains
    this.homey.flow.getConditionCard('wall_touch_4gang_is_on_mains')
      .registerRunListener(async (args) => {
        return args.device.powerType === 'AC' || args.device.powerType === 'DC';
      });
    
    // Battery level below
    this.homey.flow.getConditionCard('wall_touch_4gang_battery_below')
      .registerRunListener(async (args) => {
    ...
    // Toggle button
    this.homey.flow.getActionCard('wall_touch_4gang_toggle_button')
      .registerRunListener(async (args) => {
    ...
    // Set button state
    this.homey.flow.getActionCard('wall_touch_4gang_set_button')
      .registerRunListener(async (args) => {
        const capabilityId = `onoff.button${args.button}`;
    ...
    // Refresh device
    this.homey.flow.getActionCard('wall_touch_4gang_refresh')
      .registerRunListener(async (args) => {
  ...
  async onPair(session) {
    ...
    session.setHandler('list_devices', async () => {
      ...
      return [{
        ...
        data: {
          id: 'wall_touch_4gang_' + Math.random().toString(36).substr(2, 9)
        },
        capabilities: [
          'onoff.button1',
          'onoff.button2',
