...
class HvacDehumidifierDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // ACTION: Turn on
    try {
      this.homey.flow.getDeviceActionCard('hvac_dehumidifier_dehumidifier_hybrid_turn_on')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Turn off
    try {
      this.homey.flow.getDeviceActionCard('hvac_dehumidifier_dehumidifier_hybrid_turn_off')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Toggle
    try {
      this.homey.flow.getDeviceActionCard('hvac_dehumidifier_dehumidifier_hybrid_toggle')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
