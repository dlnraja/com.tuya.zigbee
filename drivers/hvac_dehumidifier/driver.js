...
class HvacDehumidifierDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    // CONDITION: Is on
    try {
      this.homey.flow.getConditionCard('hvac_dehumidifier_dehumidifier_is_on')
    ...
    // ACTION: Turn on
    try {
      this.homey.flow.getActionCard('hvac_dehumidifier_dehumidifier_turn_on')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Turn off
    try {
      this.homey.flow.getActionCard('hvac_dehumidifier_dehumidifier_turn_off')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Toggle
    try {
      this.homey.flow.getActionCard('hvac_dehumidifier_dehumidifier_toggle')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
