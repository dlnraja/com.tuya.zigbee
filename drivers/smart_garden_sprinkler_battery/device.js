...
class SmartGardenSprinklerBatteryDevice extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('smart_garden_sprinkler_battery initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('smart_garden_sprinkler_battery');
  ...
  async onDeleted() {
    this.log('smart_garden_sprinkler_battery deleted');
  }

}

module.exports = SmartGardenSprinklerBatteryDevice;
