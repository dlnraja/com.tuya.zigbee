...
class SmokeTempHumidSensorBatteryDevice extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('smoke_temp_humid_sensor_battery initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('smoke_temp_humid_sensor_battery');
  ...
  async onDeleted() {
    this.log('smoke_temp_humid_sensor_battery deleted');
  }

}

module.exports = SmokeTempHumidSensorBatteryDevice;
