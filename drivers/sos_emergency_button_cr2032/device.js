...
class TuyaZigbeeDevice extends ZigBeeDevice {
  ...
  async registerFlowCardHandlers() {
    ...
    // Condition: OnOff
    try {
      const isOnCard = this.homey.flow.getDeviceConditionCard('sos_emergency_button_is_on');
      if (isOnCard) {
    ...
    alarmCaps.forEach(alarmCap => {
      try {
        const conditionCard = this.homey.flow.getDeviceConditionCard(`sos_emergency_button_${alarmCap}_is_active`);
        if (conditionCard) {
    ...
    measureCaps.forEach(measureCap => {
      try {
        // Greater than
        const gtCard = this.homey.flow.getDeviceConditionCard(`sos_emergency_button_${measureCap}_greater_than`);
        if (gtCard) {
        ...
        // Less than
        const ltCard = this.homey.flow.getDeviceConditionCard(`sos_emergency_button_${measureCap}_less_than`);
        if (ltCard) {
    ...
    // Action: Turn On
    try {
      const turnOnCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_turn_on');
      if (turnOnCard) {
    ...
    // Action: Turn Off
    try {
      const turnOffCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_turn_off');
      if (turnOffCard) {
    ...
    // Action: Toggle
    try {
      const toggleCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_toggle');
      if (toggleCard) {
    ...
    // Action: Set Dim
    try {
      const setDimCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_set_dim');
      if (setDimCard) {
    ...
    try {
      const setTempCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_set_temperature');
      if (setTempCard) {
    ...
    try {
      const openCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_open');
      if (openCard) {
      ...
      const closeCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_close');
      if (closeCard) {
      ...
      const setPosCard = this.homey.flow.getDeviceActionCard('sos_emergency_button_set_position');
      if (setPosCard) {
