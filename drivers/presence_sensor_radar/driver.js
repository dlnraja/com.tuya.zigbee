...
class PresenceSensorRadarDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this._getFlowCard('presence_sensor_radar_is_present', 'condition');
      if (card) {
    ...
    try {
      const card = this._getFlowCard('presence_sensor_radar_illuminance_above', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('alarm_motion') === true;
    ...
    try {
      const card = this._getFlowCard('presence_sensor_radar_distance_within', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('alarm_motion') === true;
    ...
    try {
      const card = this._getFlowCard('presence_sensor_radar_motion_active', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('alarm_motion') === true;
