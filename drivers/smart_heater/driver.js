...
async onInit() {
    ...
    try {
      const __card = this.homey.flow.getConditionCard('smart_heater_is_on');
      if (__card) {
    ...
    } catch (e) { this.error('[FLOW] smart_heater_is_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('smart_heater_turn_on');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] smart_heater_turn_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('smart_heater_turn_off');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.safeSetCapabilityValue === 'function') {
    ...
    } catch (e) { this.error('[FLOW] smart_heater_turn_off:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('smart_heater_toggle');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] smart_heater_toggle:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('smart_heater_set_temperature');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] smart_heater_set_temperature:', e.message); }
