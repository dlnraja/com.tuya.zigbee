 ...
 */
class UsbDongleTripleDevice extends VirtualButtonMixin(PhysicalButtonMixin(ZigBeeDevice)) {
  ...
  _bindOnOffChannel(zclNode, endpointId, capabilityId) {
    ...
    onOff.readAttributes(['onOff']).then(data => {
      if (data?.onOff != null) {
        ...
        await this.setCapabilityValue(capabilityId, !!data.onOff).catch(this.error);
      }
    }).catch(() => {});
  ...
  async _configureEnergyReporting(zclNode) {
    ...
    if (electrical) {
      electrical.on('attr.activePower', v => {
        if (this.hasCapability('measure_power')) await this.setCapabilityValue('measure_power', v / 10).catch(this.error);
      });
      electrical.on('attr.rmsVoltage', v => {
        if (this.hasCapability('measure_voltage')) await this.setCapabilityValue('measure_voltage', v / 10).catch(this.error);
      });
      electrical.on('attr.rmsCurrent', v => {
        if (this.hasCapability('measure_current')) await this.setCapabilityValue('measure_current', v / 1000).catch(this.error);
      });
      ...
      electrical.readAttributes(['activePower', 'rmsVoltage', 'rmsCurrent']).then(d => {
        if (d?.activePower != null && this.hasCapability('measure_power')) await this.setCapabilityValue('measure_power', d.activePower / 10).catch(this.error);
        if (d?.rmsVoltage != null && this.hasCapability('measure_voltage')) await this.setCapabilityValue('measure_voltage', d.rmsVoltage / 10).catch(this.error);
        if (d?.rmsCurrent != null && this.hasCapability('measure_current')) await this.setCapabilityValue('measure_current', d.rmsCurrent / 1000).catch(this.error);
      }).catch(() => {});
    }

    if (metering) {
      metering.on('attr.currentSummationDelivered', v => {
        if (this.hasCapability('meter_power')) await this.setCapabilityValue('meter_power', v / 1000).catch(this.error);
      });
      ...
      metering.readAttributes(['currentSummationDelivered']).then(d => {
        if (d?.currentSummationDelivered != null && this.hasCapability('meter_power')) await this.setCapabilityValue('meter_power', d.currentSummationDelivered / 1000).catch(this.error);
      }).catch(() => {});
    }
  }
