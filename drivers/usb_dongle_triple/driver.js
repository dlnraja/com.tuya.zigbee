...
class UsbDongleTripleDriver extends ZigBeeDriver {
  async onInit() {
    ...
    this.homey.flow.getConditionCard('usb_dongle_triple_is_on').registerRunListener(async (args) => {
      return args.device.getCapabilityValue('onoff') === true;
    });
    this.homey.flow.getConditionCard('usb_dongle_triple_power_above').registerRunListener(async (args) => {
      return (args.device.getCapabilityValue('measure_power') || 0) > args.power;
    });
    this.homey.flow.getActionCard('usb_dongle_triple_all_on').registerRunListener(async (args) => {
      const d = args.device;
      for (const cap of ['onoff', 'onoff.usb2', 'onoff.usb3']) {
        if (d.hasCapability(cap)) await d.triggerCapabilityListener(cap, true);
      }
    });
    this.homey.flow.getActionCard('usb_dongle_triple_all_off').registerRunListener(async (args) => {
      const d = args.device;
      for (const cap of ['onoff', 'onoff.usb2', 'onoff.usb3']) {
