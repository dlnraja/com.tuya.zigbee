...
class PlugSmartDriver extends ZigBeeDriver {
  ...
  async onInit() {
    ...
    try {
      const __card = this.homey.flow.getActionCard('contact_sensor_plug_smart_toggle');
      if (__card) {
    ...
    } catch (e) { this.error('[FLOW] contact_sensor_plug_smart_toggle:', e.message); }
  ...
  _registerFlowCards() {
    ...
    } catch (err) { if (this.developerDebugMode) { this.error(`Condition contact_sensor_plug_plug_smart_is_on: ${err.message}`); } }
    ...
    } catch (err) { if (this.developerDebugMode) { this.error(`Action contact_sensor_plug_plug_smart_set_power_on: ${err.message}`); } }
