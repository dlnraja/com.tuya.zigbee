...
class PlugEnergyMonitorDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    // TRIGGERS
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_turned_on'); } catch (e) {}
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_turned_off'); } catch (e) {}
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_measure_power_changed'); } catch (e) {}
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_measure_voltage_changed'); } catch (e) {}
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_measure_current_changed'); } catch (e) {}
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_meter_power_changed'); } catch (e) {}
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_power_changed'); } catch (e) {}
    try { this.homey.flow.getTriggerCard('plug_energy_monitor_battery_low'); } catch (e) {}

    // CONDITIONS
    try {
      const card = this.homey.flow.getConditionCard('plug_energy_monitor_is_on');
      if (card) {
    ...
    // ACTIONS
    try {
      const card = this.homey.flow.getActionCard('plug_energy_monitor_turn_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
