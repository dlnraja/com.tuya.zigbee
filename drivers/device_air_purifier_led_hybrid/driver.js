...
class LedStripDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_led_strip_is_on', 'condition');
      if (card) {
    ...
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    // ACTIONS
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_led_strip_turn_on', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_led_strip_turn_off', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_led_strip_toggle', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_turn_on', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_turn_off', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_toggle', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('device_air_purifier_led_hybrid_led_strip_set_brightness', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
