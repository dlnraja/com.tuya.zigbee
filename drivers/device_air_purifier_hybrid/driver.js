...
class ThermostatTuyaDpDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    try {
      const card = this.homey.flow.getActionCard('thermostat_tuya_dp_temperature_below_target');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('measure_temperature') || 0;
    ...
    // CONDITION: Mode is
    try {
      const card = this.homey.flow.getActionCard('thermostat_tuya_dp_mode_is');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
