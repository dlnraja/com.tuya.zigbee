...
class RadarMotionSensorMmwaveBatteryDevice extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('radar_motion_sensor_mmwave_battery initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('radar_motion_sensor_mmwave_battery');
  ...
  async onDeleted() {
    this.log('radar_motion_sensor_mmwave_battery deleted');
  }

}

module.exports = RadarMotionSensorMmwaveBatteryDevice;
