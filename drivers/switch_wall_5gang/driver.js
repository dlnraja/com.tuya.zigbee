...
class WallSwitch5gangDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    gangs.forEach(gang => {
      try {
      (() => { try { return this.homey.flow.getActionCard('switch_wall_5gang_set_backlight'); } catch(e) { return null; } })()
    ...
    gangs.forEach(gang => {
      try {
      (() => { try { return this.homey.flow.getConditionCard('switch_wall_5gang_turn_on_gang${gang}'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
            if (!args.device) return false;
      ...
      try {
      (() => { try { return this.homey.flow.getConditionCard('switch_wall_5gang_turn_off_gang${gang}'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
            if (!args.device) return false;
      ...
      try {
      (() => { try { return this.homey.flow.getConditionCard('switch_wall_5gang_toggle_gang${gang}'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
            if (!args.device) return false;
    ...
    try {
      ...
      this.log('[FLOW] ✅ Registered: switch_wall_5gang_set_backlight');
    } catch (err) { this.log(`[FLOW] ⚠️ set_backlight: ${err.message}`); }

    try {
      (() => { try { return this.homey.flow.getActionCard('switch_wall_5gang_set_backlight_color'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
          if (!args.device || !args.state || !args.color) return false;
          await args.device.setBacklightColor(args.state, args.color);
          return true;
        });
      this.log('[FLOW] ✅ Registered: switch_wall_5gang_set_backlight_color');
    } catch (err) { this.log(`[FLOW] ⚠️ set_backlight_color: ${err.message}`); }

    try {
      (() => { try { return this.homey.flow.getActionCard('switch_wall_5gang_set_backlight_brightness'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
          if (!args.device || args.brightness === undefined) return false;
          await args.device.setBacklightBrightness(args.brightness);
          return true;
        });
      this.log('[FLOW] ✅ Registered: switch_wall_5gang_set_backlight_brightness');
    } catch (err) { this.log(`[FLOW] ⚠️ set_backlight_brightness: ${err.message}`); }

    // v5.5.930: All on/off actions
    try {
      (() => { try { return this.homey.flow.getConditionCard('switch_wall_5gang_turn_on_all'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
          if (!args.device) return false;
      ...
      this.log('[FLOW] ✅ Registered: switch_wall_5gang_turn_on_all');
    } catch (err) { this.log(`[FLOW] ⚠️ turn_on_all: ${err.message}`); }

    try {
      (() => { try { return this.homey.flow.getConditionCard('switch_wall_5gang_turn_off_all'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
          if (!args.device) return false;
          for (const cap of Object.values(capMap)) {
            if (args.device.hasCapability(cap)) {
      ...
      this.log('[FLOW] ✅ Registered: switch_wall_5gang_turn_off_all');
