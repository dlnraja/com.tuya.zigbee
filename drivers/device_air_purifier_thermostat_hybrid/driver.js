...
class ThermostatTuyaDpDriver extends ZigBeeDriver {
  ...
  const card = this.homey.flow.getConditionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_is_heating');
    ...
    try {
      const card = const card = this.homey.flow.getConditionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_temperature_above_target');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getConditionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_temperature_below_target');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = const card = this.homey.flow.getConditionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_mode_is');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = const card = this.homey.flow.getConditionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_child_lock_is');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = const card = this.homey.flow.getConditionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_is_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    // ACTIONS
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_set_target_temperature');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_set_mode');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_increase_temperature');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_decrease_temperature');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_set_child_lock');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_set_comfort_preset');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_schedule_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_schedule_off');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_turn_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_turn_off');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_toggle');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('device_air_purifier_thermostat_hybrid_thermostat_tuya_dp_set_temperature');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('target_temperature', args.temperature || args.value).catch(() => {});
