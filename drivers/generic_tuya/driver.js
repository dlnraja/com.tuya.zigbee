...
class GenericTuyaDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    try {
      (() => { try { return this.homey.flow.getConditionCard('generic_tuya_battery_above'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
          if (!args.device) return false;
          const battery = args.device.getCapabilityValue('measure_battery') || 0;
          return battery > (args.level || 20);
        });
      this.log('[FLOW] ✅ Registered: generic_tuya_battery_above');
    } catch (err) { this.log(`[FLOW] ⚠️ ${err.message}`); }

    // ACTION: Request DP
    try {
      (() => { try { return this.homey.flow.getConditionCard('generic_tuya_request_dp'); } catch(e) { return null; } })()
        .registerRunListener(async (args) => {
          if (!args.device) return false;
          try {
      ...
      this.log('[FLOW] ✅ Registered: generic_tuya_request_dp');
    } catch (err) { this.log(`[FLOW] ⚠️ ${err.message}`); }
