...
class SoilMoistureSensorBatteryDevice extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('soil_moisture_sensor_battery initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('soil_moisture_sensor_battery');
  ...
  async onDeleted() {
    this.log('soil_moisture_sensor_battery deleted');
  }

}

module.exports = SoilMoistureSensorBatteryDevice;
