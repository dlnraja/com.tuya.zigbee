...
class TemperatureSensorAdvancedBatteryDevice extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('temperature_sensor_advanced_battery initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('temperature_sensor_advanced_battery');
  ...
  async onDeleted() {
    this.log('temperature_sensor_advanced_battery deleted');
  }

}

module.exports = TemperatureSensorAdvancedBatteryDevice;
