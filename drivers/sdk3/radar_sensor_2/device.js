...
const { v3_RADAR_SENSOR_DATA_POINTS } = require('../../lib/TuyaDataPoints');
...
/*     debug(true);
  ...
  async processDatapoint(data) {
    ...
    switch (dp) {
      case v3_RADAR_SENSOR_DATA_POINTS.presenceState:
  ...
  async onSettings({ newSettings, changedKeys }) {
    try {
      if (changedKeys.includes('radar_sensitivity')) {
        await this.writeData32(v3_RADAR_SENSOR_DATA_POINTS.radarSensitivity, newSettings['radar_sensitivity']);
      }
      if (changedKeys.includes('minimum_range')) {
        await this.writeData32(v3_RADAR_SENSOR_DATA_POINTS.minimumRange, newSettings['minimum_range'] * 100); // convert to centimeters
      }
      if (changedKeys.includes('maximum_range')) {
        await this.writeData32(v3_RADAR_SENSOR_DATA_POINTS.maximumRange, newSettings['maximum_range'] * 100); // convert to centimeters
      }
      if (changedKeys.includes('detection_delay')) {
        await this.writeData32(v3_RADAR_SENSOR_DATA_POINTS.detectionDelay, newSettings['detection_delay']);
      }
      if (changedKeys.includes('fading_time')) {
        await this.writeData32(v3_RADAR_SENSOR_DATA_POINTS.fadingTime, newSettings['fading_time']);
      }
    } catch (error) {
