...
class Dimmer010vDriver extends ZigBeeDriver {
  async onInit() {
    ...
    try {
      const __card = this.homey.flow.getConditionCard('dimmer_0_10v_is_on');
      if (__card) {
    ...
    } catch (e) { this.error('[FLOW] dimmer_0_10v_is_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('dimmer_0_10v_turn_on');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] dimmer_0_10v_turn_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('dimmer_0_10v_turn_off');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.safeSetCapabilityValue === 'function') {
    ...
    } catch (e) { this.error('[FLOW] dimmer_0_10v_turn_off:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('dimmer_0_10v_toggle');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] dimmer_0_10v_toggle:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('dimmer_0_10v_set_dim');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] dimmer_0_10v_set_dim:', e.message); }
