...
class SceneController4buttonDevice extends ZigBeeDevice {
    ...
    async onNodeInit() {
        this.log('scene_controller_4button device initialized');
    ...
    async onDeleted() {
        this.log('scene_controller_4button device deleted');
    }
  ...
  async registerFlowCardHandlers() {
    ...
    // Condition: OnOff
    try {
      const isOnCard = this.homey.flow.getDeviceConditionCard('scene_controller_4button_is_on');
      if (isOnCard) {
    ...
    alarmCaps.forEach(alarmCap => {
      try {
        const conditionCard = this.homey.flow.getDeviceConditionCard(`scene_controller_4button_${alarmCap}_is_active`);
        if (conditionCard) {
    ...
    measureCaps.forEach(measureCap => {
      try {
        // Greater than
        const gtCard = this.homey.flow.getDeviceConditionCard(`scene_controller_4button_${measureCap}_greater_than`);
        if (gtCard) {
        ...
        // Less than
        const ltCard = this.homey.flow.getDeviceConditionCard(`scene_controller_4button_${measureCap}_less_than`);
        if (ltCard) {
    ...
    // Action: Turn On
    try {
      const turnOnCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_turn_on');
      if (turnOnCard) {
    ...
    // Action: Turn Off
    try {
      const turnOffCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_turn_off');
      if (turnOffCard) {
    ...
    // Action: Toggle
    try {
      const toggleCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_toggle');
      if (toggleCard) {
    ...
    // Action: Set Dim
    try {
      const setDimCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_set_dim');
      if (setDimCard) {
    ...
    try {
      const setTempCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_set_temperature');
      if (setTempCard) {
    ...
    try {
      const openCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_open');
      if (openCard) {
      ...
      const closeCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_close');
      if (closeCard) {
      ...
      const setPosCard = this.homey.flow.getDeviceActionCard('scene_controller_4button_set_position');
      if (setPosCard) {
...
module.exports = SceneController4buttonDevice;
