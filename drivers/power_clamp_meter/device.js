  ...
  async _setupElectricalMeasurement(zclNode) {
    ...
    if (emCluster) {
      ...
      emCluster.on('attr.activePower', (value) => {
        await this.setCapabilityValue('measure_power', safeMultiply(value, 10)).catch(this.error);
      });

      emCluster.on('attr.rmsVoltage', (value) => {
        await this.setCapabilityValue('measure_voltage', safeMultiply(value, 10)).catch(this.error);
      });

      emCluster.on('attr.rmsCurrent', (value) => {
        await this.setCapabilityValue('measure_current', safeMultiply(safeDivide(value, 1000), this._ctRatio)).catch(this.error);
      });
    }
  }
  ...
  _handleDP(dp, value) {
    ...
    if (profile === 'pj1203a') {
      ...
      case 106: // Energy forward A (kWh ÷ 100)
        this._energyForwardA = safeDivide(value, 100);
      ...
      case 110: // Power factor A (÷100)
        if (this.hasCapability('measure_power_factor')) {
          await this.setCapabilityValue('measure_power_factor', safeDivide(value, 100)).catch(this.error);
        }
      ...
      case 111: // AC frequency (Hz ÷100)
        if (this.hasCapability('measure_frequency')) {
          await this.setCapabilityValue('measure_frequency', safeDivide(value, 100)).catch(this.error);
        }
        this.log(`[PJ1203A]  AC Frequency: ${value/100} Hz`);
        break;

      case 112: // Voltage (V ÷ 10)
        await this.setCapabilityValue('measure_voltage', safeDivide(value, 10)).catch(this.error);
      ...
      case 114: // Current B (A ÷1000)
        if (this.hasCapability('measure_current.phase2')) {
          await this.setCapabilityValue('measure_current.phase2', safeMultiply(safeDivide(value, 1000), this._ctRatio)).catch(this.error);
        }
      ...
      case 129: // Update frequency (seconds)
        this.log(`[PJ1203A]  Update Frequency: ${value} s`);
        break;
      }
    ...
    case 1: //Total energy (kWh ÷ 100)
      await this.setCapabilityValue('meter_power', safeDivide(value, 100)).catch(this.error);
      break;
