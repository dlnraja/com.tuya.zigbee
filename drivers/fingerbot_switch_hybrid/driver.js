...
class TuyaZigbeeDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = const card = this.homey.flow.getConditionCard('fingerbot_switch_hybrid_switch_1gang_is_on');
      if (card) {
    ...
    // ACTIONS
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_turn_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_turn_off');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_toggle');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_set_backlight');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_set_backlight_color');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.setBacklightMode === 'function') await args.device.setBacklightMode(args.mode || args.value);
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_set_backlight_brightness');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.setBacklightMode === 'function') await args.device.setBacklightMode(args.mode || args.value);
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_set_countdown');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_set_child_lock');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('fingerbot_switch_hybrid_switch_1gang_set_scene_mode');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
