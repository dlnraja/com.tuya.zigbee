      ...
      if (electrical) {
        ...
        electrical.readAttributes(['activePower', 'rmsVoltage', 'rmsCurrent']).then(data => {
          if (data?.activePower != null) {
            const power = safeDivide(data.activePower, 10);if (this.hasCapability('measure_power')) {await this.setCapabilityValue('measure_power', parseFloat(power)).catch(this.error);}
          }
