...
class EnergyMeter3phaseDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() { try {
    ...
    this.homey.flow.getConditionCard('energy_meter_3phase_power_above').registerRunListener(async (args) => {
      return (args.device.getCapabilityValue('measure_power') || 0) > args.power;
    });
    this.homey.flow.getActionCard('energy_meter_3phase_reset_meter').registerRunListener(async (args) => {
