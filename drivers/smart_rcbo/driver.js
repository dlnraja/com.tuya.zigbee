...
class SmartRcboDriver extends ZigBeeDriver {
  ...
  async onInit() {
    ...
    try{(() => { try { return this.homey.flow.getDeviceConditionCard('smart_rcbo_is_on'); } catch(e) { return null; } })()?.registerRunListener(async({device})=>device.getCapabilityValue('onoff')===true);}catch(e){this.log('[Flow]',e.message);}
