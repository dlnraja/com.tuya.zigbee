...
class SmartRcboDriver extends ZigBeeDriver {
  ...
  async onInit() {
    ...
    try{this.homey.flow.getDeviceConditionCard('smart_rcbo_is_on').registerRunListener(async({device})=>device.getCapabilityValue('onoff')===true);}catch(e){this.log('[Flow]',e.message);}
