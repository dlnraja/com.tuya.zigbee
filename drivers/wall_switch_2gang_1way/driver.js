...
class WallSwitch2Gang1WayDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    for (const id of triggers) {
      try { (() => { try { return this.homey.flow.getTriggerCard(id); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })(); } catch (err) { this.error(`Trigger ${id}: ${err.message}`); }
    }
