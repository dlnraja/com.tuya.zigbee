...
class MotionSensorRadarDevice extends PhysicalButtonMixin(BatteryMixin(SensorBase)) {
  ...
  async onNodeInit({ zclNode }) {
    await this._safeInvoke(async () => {
      ...
      // v6.1.0: Initialize bidirectional button support
  ...
  /**
   * v6.1.1: Handle user settings changes (generic)
   * Synchronizes Homey settings with device Tuya DPs using dpMappings
   */
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    this.log('[SETTINGS] ⚙️ Changing settings:', changedKeys);

    const mappings = this.dpMappings;
    if (!mappings) {
      this.log('[SETTINGS] ⚠️ No DP mappings found, cannot sync settings');
      return;
    }
    ...
    for (const key of changedKeys) {
      ...
      // Find DP ID for this setting key in dpMappings
      const dpId = Object.keys(mappings).find(id => mappings[id].setting === key);
      
      if (dpId) {
        const mapping = mappings[dpId];
        let type = mapping.type || 'value';
        
        // Normalize type for Tuya cluster
        if (type === 'bool') type = 'bool';
        else if (type.includes('enum')) type = 'enum';
        else type = 'value';

        // Handle divisor/multiplier for raw value
        let rawValue = value;
        if (mapping.divisor) rawValue = Math.round(value * mapping.divisor);
        else if (mapping.multiplier) rawValue = Math.round(value / mapping.multiplier);

        this.log(`[SETTINGS] 📤 Syncing "${key}" to DP${dpId}: human=${value} → raw=${rawValue} (type=${type})`);

        try {
          const tuya = this.zclNode?.endpoints?.[1]?.clusters?.tuya;
          if (tuya?.datapoint) {
            await tuya.datapoint({ dp: parseInt(dpId, 10), value: rawValue, type });
            this.log(`[SETTINGS] ✅ DP${dpId} updated successfully`);
          } else {
            this.log(`[SETTINGS] ⚠️ Tuya cluster not found for DP${dpId}`);
          }
        } catch (err) {
          this.log(`[SETTINGS] ❌ Failed to update DP${dpId}:`, err.message);
        }
      } else {
        this.log(`[SETTINGS] ℹ️ Key "${key}" not mapped to a Tuya DP`);
      }
    }
  }
