...
class LedControllerCctDriver extends ZigBeeDriver {
  ...
  async onInit() {
    ...
    try{(() => { try { return this.homey.flow.getDeviceConditionCard('led_controller_cct_is_on'); } catch(e) { return null; } })()?.registerRunListener(async({device})=>device.getCapabilityValue('onoff')===true);}catch(e){this.log('[Flow]',e.message);}
    try{(() => { try { return this.homey.flow.getDeviceActionCard('led_controller_cct_set_brightness'); } catch(e) { return null; } })()?.registerRunListener(async({device,brightness})=>{if(brightness!==undefined)await device.triggerCapabilityListener('dim',brightness);return true;});}catch(e){this.log('[Flow]',e.message);}
