...
async onInit() {
    ...
    try {
      const __card = this.homey.flow.getConditionCard('led_controller_cct_is_on');
      if (__card) {
    ...
    } catch (e) { this.error('[FLOW] led_controller_cct_is_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('led_controller_cct_turn_on');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] led_controller_cct_turn_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('led_controller_cct_turn_off');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.safeSetCapabilityValue === 'function') {
    ...
    } catch (e) { this.error('[FLOW] led_controller_cct_turn_off:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('led_controller_cct_toggle');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] led_controller_cct_toggle:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('led_controller_cct_set_brightness');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] led_controller_cct_set_brightness:', e.message); }
