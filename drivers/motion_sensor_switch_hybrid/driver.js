...
class TuyaZigbeeDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_is_on', 'condition');
      if (card) {
    ...
    // ACTIONS
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_turn_on', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_turn_off', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_toggle', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_set_backlight', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_set_backlight_color', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.setBacklightMode === 'function') await args.device.setBacklightMode(args.mode || args.value);
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_set_backlight_brightness', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.setBacklightMode === 'function') await args.device.setBacklightMode(args.mode || args.value);
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_set_countdown', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_set_child_lock', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = this._getFlowCard('motion_sensor_switch_hybrid_switch_1gang_set_scene_mode', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
