...
class LEDControllerDimmableDriver extends ZigBeeDriver {
  ...
  async onInit() {
    ...
    try{this.homey.flow.getDeviceConditionCard('led_controller_dimmable_is_on').registerRunListener(async({device})=>device.getCapabilityValue('onoff')===true);}catch(e){this.log('[Flow]',e.message);}
    try{this.homey.flow.getActionCard('led_controller_dimmable_set_brightness').registerRunListener(async({device,brightness})=>{if(brightness!==undefined)await device.triggerCapabilityListener('dim',brightness);return true;});}catch(e){this.log('[Flow]',e.message);}
