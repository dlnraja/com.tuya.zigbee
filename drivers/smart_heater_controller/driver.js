...
class SmartHeaterControllerDriver extends Driver {
  async onInit() {
    ...
    try{this.homey.flow.getActionCard('smart_heater_controller_set_temperature').registerRunListener(async({device,temperature})=>{if(temperature!==undefined)await device.setCapabilityValue('target_temperature',temperature);return true})}catch(e){this.log('[Flow]',e.message)}
