...
class FloorHeatingThermostatDevice extends TuyaZigbeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    this.log('[FloorHeat] 🚀 Initializing hardened driver...');
    ...
    this.registerCapabilityListener('target_temperature', async (value) => {
      return this.sendTuyaCommand(16, Math.round(value * 10), 'value');
    });

    this.registerCapabilityListener('thermostat_mode', async (value) => {
      return this.sendTuyaCommand(2, MODE_MAP_REV[value] ?? 0, 'enum');
    });

    this.log('[FloorHeat] ✅ Ready');
  }
