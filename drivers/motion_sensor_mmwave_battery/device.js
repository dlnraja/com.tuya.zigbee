...
class MotionSensorMmwaveBatteryDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    this.log('motion_sensor_mmwave_battery initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('motion_sensor_mmwave_battery');
  ...
  async onDeleted() {
    this.log('motion_sensor_mmwave_battery deleted');
  }
