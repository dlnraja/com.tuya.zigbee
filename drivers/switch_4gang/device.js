    ...
    const setupEndpointListener = (epNum, retryCount = 0) => {
      ...
      onOff.on('attr.onOff', (value) => {
        ...
        if (isPhysical && (mode === 'auto' || mode === 'both')) {
          ...
          (() => { try { return this.homey.flow.getDeviceTriggerCard(flowId); } catch(e) { return null; } })()?.trigger(this, { gang: gangNum, state: value }, {})
            .catch(() => {});
          this.log(`[BSEED-4G] 🔘 Physical G${gangNum} ${value ? 'ON' : 'OFF'}`);
        }

        if (isPhysical && (mode === 'auto' || mode === 'magic' || mode === 'both')) {
          (() => { try { return this.homey.flow.getDeviceTriggerCard(`switch_4gang_gang${gangNum}_scene`); } catch(e) { return null; } })()?.trigger(this, { action: value ? 'on' : 'off' }, {})
            .catch(() => {});
  ...
  _setupTuyaDPListener(zclNode) {
    ...
    const handleDPReport = (dpId, value) => {
      ...
      if (isPhysical && (mode === 'auto' || mode === 'both')) {
        ...
        (() => { try { return this.homey.flow.getDeviceTriggerCard(flowId); } catch(e) { return null; } })()?.trigger(this, { gang: dpId, state: boolVal }, {})
          .catch(() => {});
        this.log(`[BSEED-4G] 🔘 Physical G${dpId} ${boolVal ? 'ON' : 'OFF'} (via Tuya DP)`);
      }

      if (isPhysical && (mode === 'auto' || mode === 'magic' || mode === 'both')) {
        (() => { try { return this.homey.flow.getDeviceTriggerCard(`switch_4gang_gang${dpId}_scene`); } catch(e) { return null; } })()?.trigger(this, { action: boolVal ? 'on' : 'off' }, {})
          .catch(() => {});
