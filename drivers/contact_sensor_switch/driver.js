...
class TuyaZigbeeDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    } catch (err) { if (this.developerDebugMode) { this.error(`Condition contact_sensor_switch_switch_1gang_is_on: ${err.message}`); } }
    ...
    try {
      const card = this.homey.flow.getActionCard('contact_sensor_switch_1gang_set_backlight_br_80fbc');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) {return false;}
          if (typeof args.device.setBacklightMode === 'function') {await args.device.setBacklightMode(args.mode || args.value);}
    ...
    } catch (err) { if (this.developerDebugMode) { this.error(`Action contact_sensor_switch_switch_1gang_set_scene_mode: ${err.message}`); } }
