...
class Button3GangDriver extends ZigBeeDriver {
  ...
  registerUniversalFlowCards() {
    ...
    this.homey.flow.getDeviceTriggerCard('button_wireless_3_button_double_press')
      .registerRunListener(async (args, state) => {
        return true;
      });

    // Generic long press
    this.homey.flow.getDeviceTriggerCard('button_wireless_3_button_long_press')
      .registerRunListener(async (args, state) => {
        return true;
      });

    // Generic multi press
    this.homey.flow.getDeviceTriggerCard('button_wireless_3_button_multi_press')
      .registerRunListener(async (args, state) => {
        return true;
      });
  }
