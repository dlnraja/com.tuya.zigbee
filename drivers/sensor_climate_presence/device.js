...
class ClimatePresenceHybridDevice extends UnifiedSensorBase {
    ...
    for (const [dpId, dpConfig] of Object.entries(dpMap)) {
      ...
      if (dpConfig.cap === 'alarm_motion' || dpConfig.cap === 'alarm_human') {
        mappings[dp] = {
          ...
          alsoSets: { 'alarm_human': (v) => transformPresence(v, dpConfig.type, invertPresence) }
        };
  ...
  async onNodeInit({ zclNode }) {
    ...
    this.log('[CLIMATE-HYBRID] ✅ Climate presence sensor ready');
  }
  _handleTuyaResponse(data) {
    ...
    // Handle climate specific DPs if they are not in dpMappings
    if (dpId === 2) { /* temperature usually */ }
  }
  ...
  async _triggerPresenceFlows(detected) {
    const cardId = detected ? 'sensor_climate_presence_sensor_presence_rada_34c19' : 'sensor_climate_presence_sensor_presence_rada_05404';
    try { await this.homey.flow.getDeviceTriggerCard(cardId).trigger(this, {}).catch(() => {}); } catch (e) {}
    if (detected) {
      try { await this.homey.flow.getDeviceTriggerCard('sensor_climate_presence_sensor_radar_motion__34acb').trigger(this, {}).catch(() => {}); } catch (e) {}
    }
  }
  async _setupZclClusters(zclNode) {
    ...
    if (temp?.on) {temp.on('attr.measuredValue', (v) => this.safeSetCapabilityValue('measure_temperature', safeDivide(v, 100)).catch(() => {}));}
    const hum = ep1.clusters?.msRelativeHumidity;
    if (hum?.on) {hum.on('attr.measuredValue', (v) => this.safeSetCapabilityValue('measure_humidity', safeDivide(v, 100)).catch(() => {}));}
  }
...
module.exports = ClimatePresenceHybridDevice;
