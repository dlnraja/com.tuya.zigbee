...
class TuyaZigbeeDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // ACTIONS
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_set_windowcoverings_set', 'action');
      if (card) {
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_windowcoverings_open', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_windowcoverings_close', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_set_dim', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_stop', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_set_favorite', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_set_brightness', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('dim', args.brightness || args.value || 1).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_set_position', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('windowcoverings_set', args.position || args.value || 0).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_open', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = this._getFlowCard('curtain_motor_tilt_hybrid_curtain_motor_close', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
