...
/**
 * 6-GANG SWITCH - v5.5.922 + ZCL-Only Mode (packetninja technique)
 * Physical button detection via attribute reports
 */
const ZCL_ONLY_MANUFACTURERS_6G = [
...
class Switch6GangDevice extends PhysicalButtonMixin(VirtualButtonMixin(UnifiedSwitchBase)) {
  ...
  async _initZclOnlyMode(zclNode) {
    ...
    for (const epNum of [1, 2, 3, 4, 5, 6]) {
      ...
      onOff.on('attr.onOff', (value) => {
        ...
        if (this._zclState.lastState[epNum] !== value) {
          ...
          if (isPhysical && (mode === 'auto' || mode === 'magic' || mode === 'both')) {
            (() => { try { return this.homey.flow.getDeviceTriggerCard(`switch_wall_6gang_gang${epNum}_scene`); } catch(e) { return null; } })()?.trigger(this, { action: value ? 'on' : 'off' }, {}).catch(() => {});
