  ...
  _handleTuyaResponse(data) {
    ...
    if (dpMap[dpId]?.cap === 'measure_humidity') {
      ...
      if (humidity > 100 && rawHumid > 100) {
        humidity = Math.round(safeDivide(rawHumid, 10));
      }
    ...
    if (PRESENCE_DPS.includes(dpId)) {
      ...
      if (presenceValue !== null) {
        ...
        if (this._intelGate) {
          ...
          if (processed.skip) {
            this.log(`[RADAR] ðŸ§ Intelligence Gate: SKIPPED presence=${presenceValue} (reason: ${processed.reason})`);
            return;
          }
        }
    ...
    try {
      ...
      if (illumCluster?.on) {
        ...
        illumCluster.on('attr.measuredValue', (v) => {
          ...
          const lux = Math.pow(10, safeDivide(v - 1, 10000));
          ...
          if (lastLuxValue !== null && lastLuxValue > 0) {
            const changePercent = safeMultiply(safeDivide(Math.abs(roundedLux - lastLuxValue), lastLuxValue), 100);
