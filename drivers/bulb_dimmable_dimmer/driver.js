...
class SmartBulbDimmerDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this.homey.flow.getConditionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_smart_bulb_dimmer_is_on');
      if (card) {
    ...
    try {
      const card = this.homey.flow.getConditionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_is_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    } catch (err) { this.error(`Condition bulb_dimmable_dimmer_hybrid_bulb_dimmable_is_on: ${err.message}`); }

    // ACTIONS
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_smart_bulb_dimmer_turn_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_smart_bulb_dimmer_turn_off');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_smart_bulb_dimmer_toggle');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_smart_bulb_dimmer_set_dim');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_turn_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_turn_off');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_toggle');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this.homey.flow.getActionCard('bulb_dimmable_dimmer_hybrid_bulb_dimmable_set_brightness');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('dim', args.brightness || args.value || 1).catch(() => {});
    ...
    } catch (err) { this.error(`Action bulb_dimmable_dimmer_hybrid_bulb_dimmable_set_brightness: ${err.message}`); }
