...
class HumidifierDriver extends Driver {
  async onInit() {
    ...
    try{(() => { try { return this.homey.flow.getDeviceConditionCard('humidifier_is_on'); } catch(e) { return null; } })()?.registerRunListener(async({device})=>device.getCapabilityValue('onoff')===true);}catch(e){this.log('[Flow]',e.message);}
    try{(() => { try { return this.homey.flow.getDeviceActionCard('humidifier_set_brightness'); } catch(e) { return null; } })()?.registerRunListener(async({device,brightness})=>{if(brightness!==undefined)await device.triggerCapabilityListener('dim',brightness);return true;});}catch(e){this.log('[Flow]',e.message);}
