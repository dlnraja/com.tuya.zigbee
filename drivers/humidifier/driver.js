...
async onInit() {
    ...
    try {
      const __card = this.homey.flow.getConditionCard('humidifier_is_on');
      if (__card) {
    ...
    } catch (e) { this.error('[FLOW] humidifier_is_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('humidifier_turn_on');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] humidifier_turn_on:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('humidifier_turn_off');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.safeSetCapabilityValue === 'function') {
    ...
    } catch (e) { this.error('[FLOW] humidifier_turn_off:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('humidifier_toggle');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] humidifier_toggle:', e.message); }

    try {
      const __card = this.homey.flow.getActionCard('humidifier_set_brightness');
      if (__card) {
        __card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    } catch (e) { this.error('[FLOW] humidifier_set_brightness:', e.message); }
