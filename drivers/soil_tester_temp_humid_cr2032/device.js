...
class SoilTesterTempHumidCr2032Device extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('soil_tester_temp_humid_cr2032 initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('soil_tester_temp_humid_cr2032');
  ...
  async onDeleted() {
    this.log('soil_tester_temp_humid_cr2032 deleted');
  }

}

module.exports = SoilTesterTempHumidCr2032Device;
