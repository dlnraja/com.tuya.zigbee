...
class SmartBulbRgbDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    // TRIGGERS
    const _triggerIds = ["light_bulb_rgb_bulb_rgb_turned_on","light_bulb_rgb_bulb_rgb_turned_off","light_bulb_rgb_bulb_rgb_dim_changed","light_bulb_rgb_bulb_rgb_battery_low"];
    ...
    try {
      const card = this.homey.flow.getActionCard('light_bulb_rgb_bulb_rgb_set_dim');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) {return false;}
    ...
    } catch (err) { if (this.developerDebugMode) { this.error(`Action light_bulb_rgb_bulb_rgb_set_dim: ${err.message}`); } }
