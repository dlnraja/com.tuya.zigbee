...
class RGBBulbDevice extends UnifiedLightBase {
  ...
  _parseHSV(raw) {
    ...
    try {
      ...
      await this.setCapabilityValue('light_hue', safeMultiply(h, 360)).catch(() => { });
  ...
  async _setupColorCluster(zclNode) {
    ...
    try {
      ...
      if (colorCluster?.on) {
        colorCluster.on('attr.currentHue', (v) => await this.setCapabilityValue('light_hue', safeMultiply(v, 254)).catch(() => { }));
        colorCluster.on('attr.currentSaturation', (v) => await this.setCapabilityValue('light_saturation', safeMultiply(v, 254)).catch(() => { }));
