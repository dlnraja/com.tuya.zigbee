...
class SwitchWall8gangDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang1_is_on', 'condition');
      if (card) {
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang2_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang3_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang4_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang5_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang6_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang7_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_gang8_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    // ACTIONS
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang1', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang1', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang2', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang2', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang3', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang3', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang4', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang4', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang5', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang5', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang6', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang6', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang7', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang7', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_gang8', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_gang8', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang1', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang2', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang3', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang4', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang5', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang6', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang7', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle_gang8', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on_all', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off_all', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const caps = Object.keys(args.device.getCapabilities()).filter(c => c.startsWith('onoff'));
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_set_backlight', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_set_backlight_color', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.setBacklightMode === 'function') await args.device.setBacklightMode(args.mode || args.value);
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_set_backlight_brightness', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          if (typeof args.device.setBacklightMode === 'function') await args.device.setBacklightMode(args.mode || args.value);
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_on', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_turn_off', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_toggle', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('switch_wall_8gang_set_scene_mode', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
