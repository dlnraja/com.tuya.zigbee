...
class Button1GangDriver extends ZigBeeDriver {
  ...
  registerUniversalFlowCards() {
    ...
    this.homey.flow.getDeviceTriggerCard('button_wireless_1_button_pressed')
      .registerRunListener(async (args, state) => {
        return true; // Always trigger
      });

    // Generic double press
    this.homey.flow.getDeviceTriggerCard('button_wireless_1_button_double_press')
      .registerRunListener(async (args, state) => {
        return true;
      });

    // Generic long press
    this.homey.flow.getDeviceTriggerCard('button_wireless_1_button_long_press')
      .registerRunListener(async (args, state) => {
        return true;
      });

    // Generic multi press
    this.homey.flow.getDeviceTriggerCard('button_wireless_1_button_multi_press')
      .registerRunListener(async (args, state) => {
        return true;
      });
  }
