...
class LonsonhoContactSensorDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this._getFlowCard('contact_sensor_zigbee_hybrid_contact_sensor_is_open', 'condition');
      if (card) {
    ...
    try {
      const card = this._getFlowCard('contact_sensor_zigbee_hybrid_contact_sensor_battery_above', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('contact_sensor_zigbee_hybrid_contact_sensor_contact_open', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('contact_sensor_zigbee_hybrid_contact_sensor_tamper_active', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
