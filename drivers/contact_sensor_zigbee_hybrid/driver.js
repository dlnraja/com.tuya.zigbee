...
class LonsonhoContactSensorDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this.homey.flow.getConditionCard('contact_sensor_zigbee_hybrid_contact_sensor_is_open');
      if (card) {
    ...
    try {
      const card = this.homey.flow.getConditionCard('contact_sensor_zigbee_hybrid_contact_sensor_battery_above');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this.homey.flow.getConditionCard('contact_sensor_zigbee_hybrid_contact_sensor_contact_open');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this.homey.flow.getConditionCard('contact_sensor_zigbee_hybrid_contact_sensor_tamper_active');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
