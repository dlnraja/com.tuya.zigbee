...
class DoorLockBatteryDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    this.log('door_lock_battery initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('door_lock_battery');
  ...
  async onDeleted() {
    this.log('door_lock_battery deleted');
  }

}

module.exports = DoorLockBatteryDevice;
