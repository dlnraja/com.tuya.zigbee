...
function transformLux(rawValue, type, manufacturerName = '', deviceId = null) {
  ...
  if (oscState.history.length >= 3) {
    ...
    if (hasLowValue && hasHighValue && timeSpan < 30000) {
      if (!oscState.locked) {
        ...
        _moduleLog(`[LUX] ðŸ”’ OSCILLATION DETECTED: Locking to ${stableValue} lux (pattern: ${recent.map(r => r.value).join('->')})`);
      }
    }
  }
  ...
  if (lux > maxLux && isZYM100Series) {
    _moduleLog(`[LUX-FIX] ðŸ”’ Hard clamp for ZY-M100: ${lux} -> ${maxLux} lux`);
    lux = maxLux;
  } else if (lux > maxLux) {
...
function transformDistance(value, divisor = 100, manufacturerName = '', deviceId = '') {
  ...
  if (distanceDivisorCache.has(cacheKey)) {
    ...
    if (effectiveDivisor !== divisor) {
      _moduleLog(`[DISTANCE-FIX] 🔧 Auto-detected divisor for ${manufacturerName}: ${divisor} -> ${effectiveDivisor}`);
...
class PresenceSensorRadarDevice extends UnifiedSensorBase {
  ...
  _getSensorConfig() {
    ...
    this.log(`[RADAR] ðŸ” ManufacturerName: "${mfr}", ModelId: "${modelId}" -> config: ${this._sensorConfig.configName || 'DEFAULT'}`);
  ...
  async onNodeInit({ zclNode }) {
    ...
    if (!this._hasTuyaDPCluster && config.noIasMotion) {
      ...
      this.log('[RADAR] âšï¸ Overriding noIasMotion->false: IAS Zone is the ONLY motion source');
      ...
      if (!config.noTemperature || !config.noHumidity) {
        this.log('[RADAR] âšï¸ v5.8.88: ZCL-only device -> forcing noTemperature=true, noHumidity=true');
    ...
    try {
      const occCluster = ep1.clusters?.msOccupancySensing;if (occCluster?.on) {
        occCluster.on('attr.occupancy', (v) => {
          ...
          this.safeSetCapabilityValue('alarm_motion', occupied).catch(() => { });
    ...
    try {
      ...
      const permissiveConfig = this._getSensorConfig?.() || {};if (iasZone?.on) {
        iasZone.on('attr.zoneStatus', (status) => {
          ...
          this.safeSetCapabilityValue('alarm_motion', motion).catch(() => {});
  ...
  _setupPermissiveZclListeners(ep1) {
    ...
    listen(ep1.clusters?.msTemperatureMeasurement, 'attr.measuredValue', async (v) => {
      ...
      self.safeSetCapabilityValue('measure_temperature', t).catch(() => {});
      });
    ...
    listen(ep1.clusters?.msRelativeHumidity, 'attr.measuredValue', async (v) => {
      ...
      self.safeSetCapabilityValue('measure_humidity', h).catch(() => {});
      });
    ...
    listen(ep1.clusters?.msIlluminanceMeasurement, 'attr.measuredValue', async (v) => {
      ...
      self.safeSetCapabilityValue('measure_luminance', lux).catch(() => {});
      });
  ...
  _applyPresenceInversion(occupied) {
    ...
    if (invertPresence ) {
      ...
      this.log(`[RADAR] ðŸ”„ ZCL presence inversion: ${occupied} -> ${inverted}`);
      return inverted;
    }
    return occupied;
  }
  ...
  _handleTuyaResponse(data) {
          ...
          if (this.hasCapability(discovered.capability)) {
            ...
            if (discovered.capability === 'alarm_motion') {
              ...
              if (this.hasCapability('alarm_human')) {
                this.safeSetCapabilityValue('alarm_human', discovered.value).catch(() => { });
              }
            }
    ...
    if (dpMap[dpId]?.cap === 'measure_luminance') {
      ...
      const mfr = this._getManufacturerName();
      const deviceId = this.getData()?.id;const finalLux = transformLux(luxValue, dpMap[dpId].type || 'lux_direct', mfr, deviceId);
      ...
      if (config.luxSmoothingEnabled) {
        ...
        this.log(`[RADAR-LUX] â˜€ï¸ DP${dpId} -> ${finalLux} lux (change: ${changePercent.toFixed(1)}%)`);
      } else {
        this.log(`[RADAR-LUX] â˜€ï¸ DP${dpId} -> ${finalLux} lux`);
      }
      
      this.safeSetCapabilityValue('measure_luminance', parseFloat(finalLux)).catch(() => { });
    ...
    if (dpMap[dpId]?.cap === 'measure_temperature') {
      ...
      if (temp >= -40 && temp <= 80) {
        this.log(`[RADAR] ðŸŒ¡ï¸ DP${dpId} -> temperature = ${temp}Â°C (raw: ${rawTemp}, Ã·${divisor})`);
    ...
    if (dpMap[dpId]?.cap === 'measure_humidity') {
      ...
      if (humidity >= 0 && humidity <= 100) {
        this.log(`[RADAR] ðŸ’§ DP${dpId} -> humidity = ${humidity}% (raw: ${rawHumid}, Ã·${divisor}, Ã—${multiplier})`);
        this.safeSetCapabilityValue('measure_humidity', humidity).catch(() => { });
      } else {
    ...
    if (dpMap[dpId]?.cap === 'measure_battery') {
      ...
      if (battery >= 0 && battery <= 100) {
        ...
        if (batteryChange >= 5 || (now - lastBatteryUpdate) > throttleMs || lastBatteryUpdate === 0) {
          this.log(`[RADAR] ðŸ”‹ DP${dpId} -> battery = ${battery}% (change: ${batteryChange}%)`);
          this.safeSetCapabilityValue('measure_battery', battery).catch(() => { });
    ...
    if (dpMap[dpId] && dpMap[dpId].cap === 'measure_luminance.distance') {
      ...
      if (dist >= 0 && dist <= 20) {
        ...
        this.safeSetCapabilityValue('measure_luminance.distance', dist).catch(() => {});
      }
      return;
    }
  ...
  _handleDistanceWithPresenceInference(rawDistance) {
    ...
    this.safeSetCapabilityValue('measure_luminance.distance', parseFloat(distanceMeters)).catch(() => { });
    ...
    try {
      const illumCluster = ep1.clusters?.msIlluminanceMeasurement;if (illumCluster?.on) {
        ...
        illumCluster.on('attr.measuredValue', (v) => {
          ...
          this.safeSetCapabilityValue('measure_luminance', roundedLux).catch(() => { });
      });
