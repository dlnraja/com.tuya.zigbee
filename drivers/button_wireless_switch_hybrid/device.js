      ...
      onOff.on('attr.onOff', (value) => {
        ...
        if (this._zclState.lastState[epNum] !== value) {
          ...
          if (isPhysical && (mode === 'auto' || mode === 'both')) {
            ...
            this.homey.flow.getTriggerCard(flowId).trigger(this, { gang: epNum, state: value }, {})
              .catch(() => {});
            this.log(`[BSEED-2G]  Physical G${epNum} ${value ? 'ON' : 'OFF'}`);
          }
          if (isPhysical && (mode === 'auto' || mode === 'magic' || mode === 'both')) {
            this.homey.flow.getTriggerCard('switch_2gang_scene_gang' + epNum).trigger(this, { action: value ? 'on' : 'off' }, {}).catch(() => {});
