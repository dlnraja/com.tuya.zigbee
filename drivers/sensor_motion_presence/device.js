...
// v5.11.x: Module-level state for aggressive lux smoothing (Ronny #775 pattern)
...
const SENSOR_CONFIGS = {
  ...
  'ZG_204ZV_MULTISENSOR': {
    ...
    noIasMotion: true,             // v5.8.43: PR#125 michelhelsdingen - DP1 is authoritative, IAS Zone sends conflicting false
    dpMap: {
  ...
  'HOBEIAN_10G_MULTI': {
    ...
    noIasMotion: true,      // v5.8.43: PR#125 michelhelsdingen - DP1 is authoritative for motion, IAS sends conflicting false
    dpMap: {
      // ═══════════════════════════════════════════════════════════════════
      // PRESENCE DETECTION
      // ═══════════════════════════════════════════════════════════════════
...
class PresenceSensorRadarDevice extends BatteryMixin(SensorBase) {
    ...
    try {
      ...
      if (occCluster?.on) {
        occCluster.on('attr.occupancy', (v) => {
          ...
          await this.setCapabilityValue('alarm_motion', occupied).catch(() => { });
    ...
    try {
      ...
      if (iasZone?.on) {
        iasZone.on('attr.zoneStatus', (status) => {
          ...
          await this.setCapabilityValue('alarm_motion', motion).catch(() => {});
  ...
  _handleTuyaResponse(data) {
      ...
      if (this._dpAutoDiscovery) {
        ...
        if (discovered && discovered.confidence >= 70) {
          ...
          if (this.hasCapability(discovered.capability)) {
            await this.setCapabilityValue(discovered.capability, discovered.value).catch(() => { });
            ...
            if (discovered.capability === 'alarm_motion') {
              ...
              if (this.hasCapability('alarm_human')) {
                await this.setCapabilityValue('alarm_human', discovered.value).catch(() => { });
              }
            }
    ...
    if (dpMap[dpId]?.cap === 'measure_luminance') {
      ...
      await this.setCapabilityValue('measure_luminance', parseFloat(finalLux)).catch(() => { });
    ...
    if (dpMap[dpId] && dpMap[dpId].cap === 'measure_luminance.distance') {
      ...
      if (dist >= 0 && dist <= 20) {
        ...
        await this.setCapabilityValue('measure_luminance.distance', dist).catch(() => {});
      }
      return;
    }
  ...
  _handleDistanceWithPresenceInference(rawDistance) {
    ...
    await this.setCapabilityValue('measure_luminance.distance', parseFloat(distanceMeters)).catch(() => { });
  ...
  _handlePresenceWithDebounce(rawPresence, dpId) {
    ...
    if (presence) {
      ...
      if (this.hasCapability('alarm_human')) {
        await this.setCapabilityValue('alarm_human', true).catch(() => { });
      }
      this._updatePresenceTimestamp();
    ...
    if (this.hasCapability('alarm_human')) {
      await this.setCapabilityValue('alarm_human', false).catch(() => { });
    }
  ...
  async _triggerPresenceFlows(detected) {
    try {
      if (detected) {
        ...
        await (() => { try { return this.homey.flow.getDeviceTriggerCard('presence_sensor_radar_presence_detected'); } catch(e) { return null; } })()?.trigger(this).catch(() => { });
        this.log('[RADAR-FLOW] ✅ Triggered: presence_sensor_radar_presence_detected');
      } else {
        // v5.5.926: Fixed flow card IDs - must match driver.flow.compose.json
        // Trigger: presence_sensor_radar_presence_cleared
        await (() => { try { return this.homey.flow.getDeviceTriggerCard('presence_sensor_radar_presence_cleared'); } catch(e) { return null; } })()?.trigger(this).catch(() => { });
    ...
    try {
      ...
      if (illumCluster?.on) {
        ...
        illumCluster.on('attr.measuredValue', (v) => {
          ...
          await this.setCapabilityValue('measure_luminance', roundedLux).catch(() => { });
        });
