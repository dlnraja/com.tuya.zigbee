...
class BulbRgbwDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // ACTION: Turn on
    try {
      this.homey.flow.getConditionCard('bulb_rgbw_bulb_rgbw_turn_on')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Turn off
    try {
      this.homey.flow.getConditionCard('bulb_rgbw_bulb_rgbw_turn_off')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Toggle
    try {
      this.homey.flow.getConditionCard('bulb_rgbw_bulb_rgbw_toggle')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
