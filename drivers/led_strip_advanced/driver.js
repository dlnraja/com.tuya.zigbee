...
class LedStripAdvancedDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // ACTION: Turn on
    try {
      this.homey.flow.getConditionCard('led_strip_advanced_turn_on')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Turn off
    try {
      this.homey.flow.getConditionCard('led_strip_advanced_turn_off')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTION: Toggle
    try {
      this.homey.flow.getConditionCard('led_strip_advanced_toggle')
        .registerRunListener(async (args) => {
          if (!args.device) return false;
