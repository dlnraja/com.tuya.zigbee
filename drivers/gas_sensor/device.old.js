...
/**
 * TuyaGasSensorTs0601Device - v5.3.58 AUTO-ADAPTIVE VERSION
 *
 ...
 */
class TuyaGasSensorTs0601Device extends AutoAdaptiveDevice {
    ...
    try {
      ...
      endpoint.clusters.iasZone.onZoneStatusChangeNotification = async (payload) => {
        ...
        if (payload && payload.zoneStatus !== undefined) {
          ...
          await (async () => {
            this.log(`📝 [DIAG] setCapabilityValue: ${'alarm_co'} = ${alarm}`);
            try {
              await this.setCapabilityValue('alarm_co', alarm);
              this.log(`✅ [DIAG] setCapabilityValue SUCCESS: ${'alarm_co'}`);
            } catch (err) {
              this.error(`❌ [DIAG] setCapabilityValue FAILED: ${'alarm_co'}`, err.message);
              throw err;
            }
          })().catch(this.error);
