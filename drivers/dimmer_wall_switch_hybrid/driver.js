...
class TuyaZigbeeDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_gang1_is_on', 'condition');
      if (card) {
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_gang2_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_is_on', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          return args.device.getCapabilityValue('onoff') === true;
    ...
    // ACTIONS
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_on_gang1', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_off_gang1', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_on_gang2', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_off_gang2', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_toggle_gang1', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_toggle_gang2', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_on_all', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_off_all', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const caps = Object.keys(args.device.getCapabilities()).filter(c => c.startsWith('onoff'));
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_set_backlight', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_on', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', true).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_turn_off', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('onoff', false).catch(() => {});
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_toggle', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const current = args.device.getCapabilityValue('onoff');
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_switch_hybrid_switch_2gang_set_scene_mode', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
