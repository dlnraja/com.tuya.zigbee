...
class DimmerDualChannelDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = const card = this.homey.flow.getConditionCard('dimmer_dual_channel_is_on');
      if (card) {
    ...
    try {
      const card = const card = this.homey.flow.getConditionCard('dimmer_dual_channel_dim_above');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    // ACTIONS
    try {
      const card = const card = this.homey.flow.getActionCard('dimmer_dual_channel_turn_on');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('dimmer_dual_channel_turn_off');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('dimmer_dual_channel_toggle');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('dimmer_dual_channel_set_dim');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('dimmer_dual_channel_dim_up');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('dimmer_dual_channel_dim_down');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          // Generic action handler
    ...
    try {
      const card = const card = this.homey.flow.getActionCard('dimmer_dual_channel_set_brightness');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          await args.device.triggerCapabilityListener('dim', args.brightness || args.value || 1).catch(() => {});
