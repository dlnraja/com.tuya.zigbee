...
class Dimmer3GangDriver extends ZigBeeDriver {
  async onInit() {
    ...
    try{this.homey.flow.getActionCard('dimmer_3gang_set_brightness').registerRunListener(async({device,brightness})=>{if(brightness!==undefined)await device.setCapabilityValue('dim',brightness);return true;});}catch(e){this.log('[Flow]',e.message);}
