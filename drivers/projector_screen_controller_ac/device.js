...
class ProjectorScreenControllerAcDevice extends ZigBeeDevice {

  async onNodeInit({ zclNode }) {
    this.log('projector_screen_controller_ac initialized');

    // Call parent
    await super.onNodeInit({ zclNode });

    // Auto-detect device type and initialize Tuya cluster handler
    const deviceType = TuyaClusterHandler.detectDeviceType('projector_screen_controller_ac');
  ...
  async onDeleted() {
    this.log('projector_screen_controller_ac deleted');
  }

}

module.exports = ProjectorScreenControllerAcDevice;
