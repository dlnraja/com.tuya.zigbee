  ...
  async configureButtonClusters(zclNode, buttonCount) {
    for (let i = 1; i <= buttonCount; i++) {
      ...
      endpoint.clusters.onOff.on('commandOn', async () => {
        ...
        // v5.2.72: Trigger flow card
        try {
          const triggerCard = this.homey.flow.getDeviceTriggerCard('button_wireless_button_pressed');
          if (triggerCard) {
            await triggerCard.trigger(this, { button: i });
            this.log(`[FLOW] ✅ Triggered: button_wireless_button_pressed (button ${i})`);
          }
        } catch (flowErr) {
