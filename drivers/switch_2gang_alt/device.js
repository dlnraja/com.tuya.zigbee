...
/**
 * switch_2gang_alt - Hybrid-Enhanced Driver
 *
...
class TuyaZigbeeDevice extends HybridDevice {
  ...
  async registerFlowCardHandlers() {
    ...
    alarmCaps.forEach(alarmCap => {
      try {
        const conditionCard = this.homey.flow.getDeviceConditionCard(`switch_2gang_hybrid_${alarmCap}_is_active`);
        if (conditionCard) {
    ...
    measureCaps.forEach(measureCap => {
      try {
        // Greater than
        const gtCard = this.homey.flow.getDeviceConditionCard(`switch_2gang_hybrid_${measureCap}_greater_than`);
        if (gtCard) {
        ...
        // Less than
        const ltCard = this.homey.flow.getDeviceConditionCard(`switch_2gang_hybrid_${measureCap}_less_than`);
        if (ltCard) {
    ...
    // Action: Toggle
    try {
      const toggleCard = this.homey.flow.getDeviceActionCard('switch_2gang_hybrid_toggle');
      if (toggleCard) {
    ...
    try {
      ...
      const closeCard = this.homey.flow.getDeviceActionCard('switch_2gang_hybrid_close');
      if (closeCard) {
