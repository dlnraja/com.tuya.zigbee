...
class ClimateSensorPresenceDevice extends UnifiedSensorBase {
  ...
  async _triggerPresenceFlows(detected) {
    ...
    if (detected) {
      try {
        await (() => { try { return this.homey.flow.getTriggerCard(prefix + 'presence_sensor_radar_motion_detected_sensor_presence_radar_hybrid'); } catch(e) { return null; } })()?.trigger(this, {}).catch(() => {});
      } catch (e) {}
    }
  }
  ...
  async _setupZclClusters(zclNode) {
    ...
    if (power?.on) {
      power.on('attr.batteryPercentageRemaining', (v) => {
        await this.setCapabilityValue('measure_battery', Math.round(v / 2)).catch(() => {});
      });
    }
