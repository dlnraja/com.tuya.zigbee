...
class LedControllerRgbDriver extends ZigBeeDriver {
  ...
  async onInit() {
    ...
    try{this.homey.flow.getActionCard('led_controller_rgb_set_brightness').registerRunListener(async({device,brightness})=>{if(brightness!==undefined)await device.setCapabilityValue('dim',brightness);return true})}catch(e){this.log('[Flow]',e.message)}
