...
class PlugEnergyMonitorDriver extends ZigBeeDriver {
  ...
  _registerFlowCards() {
    ...
    // CONDITIONS
    try {
      const card = this._getFlowCard('dimmer_wall_plug_hybrid_plug_energy_monitor_is_on', 'condition');
      if (card) {
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_plug_hybrid_plug_energy_monitor_power_above', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_plug_hybrid_plug_energy_monitor_energy_above', 'condition');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
          const val = args.device.getCapabilityValue('measure_co2') || 0;
    ...
    // ACTIONS
    try {
      const card = this._getFlowCard('dimmer_wall_plug_hybrid_plug_energy_monitor_turn_on', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_plug_hybrid_plug_energy_monitor_turn_off', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_plug_hybrid_plug_energy_monitor_toggle', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
    ...
    try {
      const card = this._getFlowCard('dimmer_wall_plug_hybrid_plug_energy_monitor_reset_meter', 'action');
      if (card) {
        card.registerRunListener(async (args) => {
          if (!args.device) return false;
