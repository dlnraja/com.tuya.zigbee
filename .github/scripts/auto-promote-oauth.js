...
async function promoteRaw(apiTk) {
  ...
  const urls=['https://apps-api.athom.com/api/v1/app/'+APP+'/build','https://api.athom.com/api/v1/app/'+APP+'/builds'];
  let r,ok=false;
  for(const u of urls){
    r=await fetch(u,{headers:hd});
    const st=r.status;
    log('  GET '+u.split(APP)[1]+'  '+st);
    if(st===401){const t=await r.text().catch(()=>'');log('    401: '+t.slice(0,200))}
