...
const OWN=REPOS[0];
const DRY=process.env.DRY_RUN==='true' || process.env.SHADOW_MODE==='true';
...
const OWNER_USERS=new Set([REPO_OWNER,'dlnraja','github-actions','github-actions[bot]','app/github-actions','dependabot','dependabot[bot]','tuya-triage-bot','google-labs-jules','snyk-bot','renovate-bot'].map(u=>u.toLowerCase()));
const isOwner = (user) => user && (OWNER_USERS.has(user.toLowerCase()) || user.toLowerCase().includes('github-actions'));
...
async function processPR(repo,pr,state,report,extData){
  ...
  if(!hasBot){
    ...
    if(isUp){
      ...
      msg+='[Test version](https://homey.app/a/com.dlnraja.tuya.zigbee/test/) Â· [Forum](https://community.homey.app/t/app-pro-universal-tuya-zigbee-device-app-test/140352)';
    }
