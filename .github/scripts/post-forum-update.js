...
const SHADOW=process.env.SHADOW_MODE==='true' || process.env.DRY_RUN==='true';
...
async function editPost(postId,content,auth){
  if(SHADOW){console.log('[SHADOW] Would edit post',postId);return{id:postId}}
...
async function postReply(tid,raw,auth){
  ...
  if(SHADOW){console.log('[SHADOW] Would post reply on T'+tid);return{id:'shadow'}}
