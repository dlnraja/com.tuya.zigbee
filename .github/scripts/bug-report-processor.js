...
const { callAI, shouldSkipAI } = require('./ai-helper');
const { loadFingerprints, findAllDrivers } = require('./load-fingerprints');
const { fetchWithRetry } = require('./retry-helper');
const { extractFP: _vFP } = require('./fp-validator');
const { solveIssueLocal } = require('../../tools/ci/local-intelligent-solver');
...
async function ghGet(ep) {
  const r = await fetchWithRetry(
    ...
    { retries: 3, label: 'ghAPI' },
  );
  return r.ok ? r.json() : null;
}

function extractFP(t) {
  return _vFP(t);
}
function extractPID(t) {
  return [...new Set((t || '').match(/\bTS[0-9]{4}[A-Z]?\b/g) || [])];
}

async function main() {
  if (!ISSUE_NUM) {
    console.log('No ISSUE_NUMBER');
    process.exit(0);
  }
  ...
  // WHY(P2438): local sacred-couple solver first — never burn Grok/LLM when catalog answers
  const local = solveIssueLocal({
    number: Number(ISSUE_NUM),
    title: issue.title,
    body: issue.body,
  });
  console.log('[P2438] local solver:', local.summary);
  ...
  let details = local.findings
    .map(
      (f) =>
        `- ${f.mfr || ''}+${f.pid || ''} → ${f.expectedDriver || f.driver || '?'} (${f.action})`,
    )
    .join('\n');
  ...
  if (local.catalogOk && existingFPs.length > 0 && !hasFix) {
    summary = local.summary;
    details = (details || '') + '\nP2438: catalog locked; skip remote AI.';
  } else if (
    existingFPs.length > 0 &&
    !hasFix &&
    local.canAutoCodeFix === false &&
    !shouldSkipAI()
  ) {
    ...
    console.log('[P2438] skip AI (forfait / AI_FORCE_LOCAL)');
  }
  ...
  if (!details) {
    details =
      ...
      pids.join(', ') +
      '\nMatched drivers: ' +
      existingFPs.map((m) => m.drivers.join(',')).join('; ');
  }
