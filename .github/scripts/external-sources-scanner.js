  ...
  for(const url of urls){
    ...
    for(const block of fpMatches){
      ...
      for(const fp of mfrs){
        const dps=extractDPs(getSnippet(src,fp,1500,3000));
        const caps=extractCaps(getSnippet(src,fp,1500,3000));
        const snippet=getSnippet(src,fp,300,600);
        devices.push({fp,models,source:'z2m-converter',dps,caps,snippet});
      }
    }
    ...
    for(const fp of inlineFPs)if(!devices.find(d=>d.fp===fp)){
      const dps=extractDPs(getSnippet(src,fp,1500,3000));
      const caps=extractCaps(getSnippet(src,fp,1500,3000));
      devices.push({fp,models:[],source:'z2m-converter',dps,caps});
    }
...
async function scanZ2MIssuesPRs(){
  ...
  for(const q of queries){
    ...
    for(const iss of d.items){
      ...
      if(found.length){
        const dps=extractDPs(text);
        const isBug=extractBugSignals(text);
        const isQuirk=extractQuirkSignals(text);
        results.push(...found.map(fp=>({fp,source:'z2m-issue',num:iss.number,title:iss.title?.slice(0,80),url:iss.html_url,isPR:!!iss.pull_request,dps,isBug,isQuirk})));
      }
    }
    await sleep(2000);
  }
...
async function scanZHAQuirks(){
  ...
  if(src){const found=extractFP(src);for(const fp of found){
    const dps=extractDPs(getSnippet(src,fp,800,1500));
    devices.push({fp,source:'zha-quirk',dps});
  ...
  if(tree?.tree){
    ...
    for(const f of tuyaFiles){
      ...
      for(const fp of found)if(!devices.find(d=>d.fp===fp)){
        const dps=extractDPs(getSnippet(content,fp,800,1500));
        const clusters=[...new Set((getSnippet(content,fp,800,1500).match(/0x[0-9A-Fa-f]{4}/g)||[]))].slice(0,10);
        const snippet=getSnippet(content,fp,200,400);
        devices.push({fp,source:'zha-quirk',file:f.path,dps,clusters,snippet});
      }
      await sleep(300);
    }
...
async function scanZHAIssues(){
  ...
  for(const q of['repo:zigpy/zha-device-handlers+tuya+state:open','repo:home-assistant/core+tuya+zigbee+state:open']){
    ...
    for(const iss of d.items){const text=(iss.title||'')+' '+(iss.body||'');const found=extractFP(text);
      if(found.length){const dps=extractDPs(text);const isBug=extractBugSignals(text);
        results.push(...found.map(fp=>({fp,source:'zha-issue',num:iss.number,title:iss.title?.slice(0,80),url:iss.html_url,dps,isBug})))}}
    await sleep(2000);
  }
...
async function scanDeCONZ(){
  ...
  for(const f of jsonFiles){
    ...
    for(const fp of found){
      let clusters=[],config=[];
      try{const j=JSON.parse(content);clusters=(j.clusters||[]).map(c=>c.id||c);config=(j.config||[]).slice(0,8)}catch{}
      devices.push({fp,source:'deconz',file:f.path,clusters,config});
    }
    await sleep(200);
  }
...
async function scanHubitat(){
  ...
  if(d?.items){
    for(const item of d.items){
      ...
      const dps=extractDPs(fragments);
      for(const fp of found)results.push({fp,source:'hubitat',repo:item.repository?.full_name,file:item.name,dps});
    }
  }
  await sleep(2000);
...
async function searchVariants(unsupported,allDevices){
  ...
  for(const fp of unsupported.slice(0,25)){
    const prefix=fp.substring(0,fp.lastIndexOf('_'));
    const related=allDevices.filter(d=>d.fp.startsWith(prefix)&&d.fp!==fp);
    if(related.length){variants.set(fp,{fp,prefix,variants:related.map(d=>({fp:d.fp,source:d.source,model:d.model||d.zigbeeModel,dps:d.dps||[],caps:d.caps||[],category:d.category}))});
      console.log('  '+fp+': '+related.length+' variants (DPs: '+related.filter(r=>r.dps?.length).length+')')}
  }
  return variants;
}
...
async function scanSmartThings(){
  ...
  if(d?.items){for(const item of d.items){
    ...
    const found=extractFP((item.name||'')+' '+fragments);
    const dps=extractDPs(fragments);
    for(const fp of found)results.push({fp,source:'smartthings',repo:item.repository?.full_name,dps});
  }}
  await sleep(2000);
  ...
  for(const q of['tuya zigbee DP','TZE200 tuya datapoint','TZE204 tuya']){
    try{
      ...
      for(const t of(r.topics||[]).slice(0,10)){
        ...
        const found=extractFP(txt);const dps=extractDPs(txt);
        for(const fp of found)results.push({fp,source:'ha-forum',title:t.title?.slice(0,80),topicId:t.id,dps});
      }
    }catch{}
    await sleep(1000);
  }
...
function scanLocalDrivers(){
  ...
  for(const d of fs.readdirSync(DDIR)){
    ...
    try{
      ...
      for(const fp of mfrs)results.push({fp,source:'local',driver:d,pids,caps,dps,hasEF00,hasDPMappings});
    }catch{}
  }
...
async function main(){
  ...
  // Deduplicate and classify — merge DPs, caps, quirks, bugs, clusters, snippets
  const byFP=new Map();
  for(const d of allDevices){
    if(!byFP.has(d.fp))byFP.set(d.fp,{fp:d.fp,sources:[],models:new Set(),dps:new Set(),caps:new Set(),clusters:new Set(),bugs:[],quirks:[],snippets:[],localDriver:null,localCaps:[],localDPs:[]});
    const e=byFP.get(d.fp);
  ...
  const uniqueFPs=[...byFP.values()].map(d=>({...d,models:[...d.models],dps:[...d.dps],caps:[...d.caps],clusters:[...d.clusters],sources:[...new Set(d.sources)],inApp:fps.has(d.fp),drivers:findAllDrivers(d.fp)}));
  ...
  const aiInput={v:appVer,known:fps.size,ext:uniqueFPs.length,
    newDevs:topNew.map(d=>({fp:d.fp,dps:d.dps,caps:d.caps.slice(0,6),src:d.sources,bugs:d.bugs.length})),
    gaps:gapDevs.map(d=>({fp:d.fp,drv:d.localDriver,missDPs:d.dps.filter(n=>!d.localDPs.includes(n)),missCaps:d.caps.filter(c=>!c.startsWith('cat')).slice(0,5)})),
  ...
  fs.writeFileSync(FUNC_F,JSON.stringify({ts:data.timestamp,total:uniqueFPs.length,withDPs:withDPs.length,gaps:gapDevs.length,
    profiles:uniqueFPs.filter(d=>d.dps.length||d.bugs.length).slice(0,200).map(d=>({fp:d.fp,dps:d.dps,caps:d.caps.slice(0,10),src:d.sources,bugs:d.bugs.length,drv:d.localDriver,lDPs:d.localDPs})),
  ...
  const report={timestamp:data.timestamp,totalExternal:uniqueFPs.length,supported:supported.length,unsupported:unsupported.length,
    ...
    topUnsupported:unsupported.slice(0,30).map(d=>({fp:d.fp,dps:d.dps,sources:d.sources})),aiPlan:ai?.text};
