...
function listCompose(root){const out=[];if(!fs.existsSync(root))return out;const st=[root];while(st.length){const cur=st.pop();let s;try {s=fs.statSync(cur);} catch (error) {continue;}if(s.isDirectory()){for(const e of fs.readdirSync(cur)){const p=path.join(cur,e);try {const ss=fs.statSync(p);if(ss.isDirectory())st.push(p);else if(ss.isFile()&&/driver(\.compose)?\.json$/i.test(p))out.push(p);} catch (error) {continue;}}}}return out;}
...
(async function(){
  ...
  const repo=[]; if(fs.existsSync(DRV)){const st=[DRV];while(st.length){const cur=st.pop();let s;try {s=fs.statSync(cur);} catch (error) {continue;}if(s.isDirectory()){const comp=['driver.compose.json','driver.json'].map(n=>path.join(cur,n)).find(p=>fs.existsSync(p));if(comp)repo.push({dir:cur,compose:comp,obj:j(comp)});for(const e of fs.readdirSync(cur))st.push(path.join(cur,e));}}}}
