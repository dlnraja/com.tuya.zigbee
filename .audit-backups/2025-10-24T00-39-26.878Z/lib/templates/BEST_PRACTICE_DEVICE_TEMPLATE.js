...
class BestPracticeDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    if (this.hasCapability('measure_battery')) {
      this.registerCapability('measure_battery', CLUSTER.POWER_CONFIGURATION, {
        ...
        // ✅ Best Practice: Get value on device start
        getOpts: {
          getOnStart: true,
          getOnOnline: true,
        },
        
        // ✅ Best Practice: Integrated reportOpts
        reportOpts: {
          configureAttributeReporting: {
            minInterval: 3600,      // Min 1 hour
            maxInterval: 86400,     // Max 24 hours
   ...
   * ✅ Best Practice: Use triggerFlow() method from ZigBeeDevice
   * 
  ...
  async triggerFlowCard(flowId, tokens = {}) {
    try {
      await this.triggerFlow({ id: flowId, tokens });
      this.log(`✅ Flow triggered: ${flowId}`, tokens);
    } catch (err) {
      this.error(`❌ Error triggering flow '${flowId}':`, err);
      throw err;
    }
  }
  ...
  async setCapabilitySafe(capabilityId, value) {
    try {
      if (this.hasCapability(capabilityId)) {
        ...
        this.log(`✅ Capability '${capabilityId}' set to:`, value);
      } else {
        this.log(`⚠️ Capability '${capabilityId}' not available`);
      }
    } catch (err) {
