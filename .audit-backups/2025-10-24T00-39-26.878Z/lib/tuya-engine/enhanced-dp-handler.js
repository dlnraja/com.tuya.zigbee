...
class EnhancedDPHandler {
  ...
  async initialize(dpMapping) {
    ...
    try {
      await this.registerDatapointListener();
      this.device.log(`✅ Enhanced DP Handler initialized with ${this.dpMap.size} DPs`);
    } catch (err) {
  ...
  async registerDatapointListener() {
    ...
    this.device.log('✅ Datapoint listener registered');
  }
  ...
  async onDatapointReport(data) {
    try {
      ...
      this.device.log(`📥 Received DP ${dp} (type: ${datatype}):`, value);

      // Cache the value
      this.dpCache.set(dp, { value, datatype, timestamp: Date.now() });

      // Process the datapoint
      await this.processDatapoint(dp, datatype, value);
  ...
  async processDatapoint(dp, datatype, rawValue) {
    ...
    if (!config) {
      this.device.log(`⚠️  Unknown DP ${dp}, ignoring`);
      return;
    }
    ...
    try {
      ...
      this.device.log(`✅ DP ${dp} → ${config.capability}: ${convertedValue}`);

      // Update capability
      if (config.capability) {
        await this.device.setCapabilityValue(config.capability, convertedValue);
      }

      // Call custom handler if defined
      if (config.handler && typeof config.handler === 'function') {
        await config.handler.call(this.device, convertedValue, dp);
      }
  ...
  async sendDatapoint(dp, datatype, value) {
    try {
      this.device.log(`📤 Sending DP ${dp} (type: ${datatype}):`, value);
      ...
      this.device.log(`✅ DP ${dp} sent successfully`);
