...
async function withRetry(fn, { 
  ...
  for (let attempt = 1; attempt <= tries; attempt++) {
    try {
      ...
      if (device && attempt > 1) {
        device.log(`✅ Retry succeeded on attempt ${attempt}/${tries}`);
      }
      ...
      if (!isRetryable || attempt === tries) {
        ...
        if (device) {
          device.error(`❌ Operation failed (attempt ${attempt}/${tries}):`, err.message);
        }
        throw err;
      }
      ...
      // Log retry attempt
      if (device) {
        device.log(`⚠️ Retrying after error (attempt ${attempt}/${tries}): ${err.message}`);
