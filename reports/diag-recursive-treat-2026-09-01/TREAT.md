# Recursive diag + interview treat — 2026-09-01
...
Sources scanned: **820** · Unique cases: **376** · Actionable: **123** · Interview couples: **90** · Gmail bodies ingested: **635**
...
| `flow_guard_spam` | 57 |
| `scene_mode_unsupported` | 18 |
| `wrong_driver_hint` | 16 |
| `ef00_on_ias_only` | 13 |
| `ef00_leftover_ias` | 13 |
| `physical_vs_virtual_button` | 13 |
...
### c40705a1 (homey_app_diag)

- **Source:** `.github/state/homey-app-diag/c40705a1-9b08-444b-868f-c5a14ca4d2b2.json`
- **Drivers:** water_valve_garden, water_valve_smart, weather_station_outdoor, wifi_air_purifier, wifi_air_quality, wifi_camera, wifi_cover, wifi_dehumidifier, wifi_doorbell, wifi_door_lock, wifi_dimmer, wifi_ewelink_bulb, wifi_ewelink_led, wifi_ewelink_plug, wifi_ewelink_fan, wifi_ewelink_dimmer, wifi_ewelink_switch, wifi_ewelink_switch_2ch, wifi_ewelink_pow, wifi_ewelink_switch_4ch, wifi_ewelink_th, wifi_fan, wifi_garage_door, wifi_generic, wifi_heater, wifi_humidifier, wifi_light, wifi_ir_remote, wifi_led_strip, wifi_pet_feeder, wifi_power_strip, wifi_robot_vacuum, wifi_plug, wifi_sensor, wifi_siren, wifi_sonoff_dualr3, wifi_sonoff_thr316d, wifi_sonoff_tx_2ch, wifi_sonoff_tx_3ch, wifi_switch, wifi_switch_3gang, wifi_switch_2gang, wifi_switch_4gang, wifi_thermostat, wifi_water_valve, zigbee_repeater, wifi_water_tank_monitor
- **Couple:** none in text
- **Root causes:**
  - `wrong_driver_hint` [medium/BOTH] — Lock sacred couple in compose + registry
  - `physical_vs_virtual_button` [high/BOTH] — Physical button path silent; virtual/UI works
...
### 4217d5e3 (homey_app_diag)

- **Source:** `.github/state/homey-app-diag/4217d5e3-c845-4f0b-a351-5e5a59295cbb.json`
- **User:** Estoy tratando de configurar el sensor de presencia zigbee 24G con referencia_TZE204_clrdrnya pero siempre lo reconoce como desconocido y no me deja usar sus funciones de presencia y luminosidad
- **Drivers:** wifi_sonoff_tx_3ch, wifi_switch_2gang, wifi_switch_3gang, wifi_switch_4gang, wifi_thermostat, wifi_water_tank_monitor, wifi_water_valve, zigbee_repeater
- **Couple:** none in text
- **Root causes:**
  - `physical_vs_virtual_button` [high/BOTH] — Physical button path silent; virtual/UI works
    - Fix: 0xFD/0xFC PhysicalButtonMixin; re-pair while pressing
...
- **Couples:** `_TZE284_fhvpaltk+TS0601` → ? (in_log)
- **Root causes:**
  - `invalid_flow_card` [medium/BOTH] — Check driver.flow.compose.json IDs
  - `flow_guard_spam` [medium/BOTH] — Stale or mismatched flow card IDs
    - Fix: Hashed flow resolve / compose ID audit
...
- **Source:** `.github/state/diag-recursive-inbox/bodies/19ce786515b945fa.txt`
- **App:** 5.11.113
- **Drivers:** ir_blaster, air_quality_comprehensive, din_rail_meter, energy_meter_3phase, lcdtemphumidsensor, pet_feeder, power_clamp_meter, power_meter, temphumidsensor, usb_dongle_triple
- **Couples:** `_TZE204_yvx5lh6k+TS0601` → air_quality_comprehensive (derived_interview); `_TZE204_81yrt3lo+TS0601` → power_meter (derived_interview_soft); `_TZE284_81yrt3lo+TS0601` → power_meter (derived_interview_soft)
- **Root causes:**
  - `invalid_flow_card` [medium/BOTH] — Check driver.flow.compose.json IDs
  - `flow_guard_spam` [medium/BOTH] — Stale or mismatched flow card IDs
    - Fix: Hashed flow resolve / compose ID audit
...
- **Source:** `.github/state/diag-recursive-inbox/bodies/19d361617a68295e.txt`
- **App:** 5.11.138
- **User:** GitHub issue: TS0003 (_TZ3000_v4l4b0lp). Testing broken Flow Action cards for Gang 2 and Gang 3 to generate DP logs.
- **Drivers:** switch_3gang
- **Couples:** `_TZ3000_v4l4b0lp+TS0003` → ? (in_log)
- **Root causes:**
  - `invalid_flow_card` [medium/BOTH] — Check driver.flow.compose.json IDs
