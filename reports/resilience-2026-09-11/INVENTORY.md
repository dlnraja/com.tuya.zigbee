# Features + historical bugs inventory — 2026-09-11
...
| `rotary_knob_command_mode` | ERS-10 rotation dies when Homey forces scene mode  | No native TS004F operation_mode; OnOff 0 | rotary_scene_force, knob_mfr_missing, smart_knob_regex_catch_all |
| `declared_flow_card_wiring` | Compose declares scene_recall / set_brightness / b | SDK3 requires explicit registerRunListen | declared_only_flow_card, getDeviceTriggerCard_two_arg, buttondevice_forbids_dim_on_knob, scene_recall_app_level_only |
...
- P2448 rotary: command/dimmer 0x8004 + levelControl; never scene-force on smart_knob_rotary

#### rotary_knob_command_mode

- DeviceOperatingMode KNOB_MFR
- smart_knob_rotary command write
- 0xFC parallel RX
- rotary-knob-ssot.json
- p2448 gate

_Discoveries:_
- P2448 uri7ongn/ixla93vd/g9g2xnch → knob/dimmer
- compose button_mode default dimmer
- abrsvsou/4fjiwweb stay button_wireless_4 off KNOB_MFR
- P2449 DeclaredFlowCardAutoWire + scene_recall driver-scoped + brightness_changed on dim

#### declared_flow_card_wiring

- lib/flow/DeclaredFlowCardAutoWire.js
- UniversalFlowCardLoader autoWire
- ButtonDevice _tryCard scene_recall
- FlowCardHelper scene_recall register
- TuyaZigbeeDevice/SafeCapabilityMixin dim→brightness_changed
- SmartKnobRotationMixin

_Discoveries:_
- P2449 fleet auto-wire from driver.manifest.flow
- normalizeDim 0–100 → 0–1 for set_brightness
- kaflzta4 stays scene (P2439) while ERS-10 rotary stays dimmer
...
| buttons_bidirectional | 1 | hardened | 2/2 | No physical vs app distinction; mfr OnOff 0xFD/0xFC dro |
...
| flows | 2 | partial | 2/2 | titleFormatted [[device]]; Missing Capability Listener; |
