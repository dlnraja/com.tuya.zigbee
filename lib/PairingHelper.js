...
class PairingHelper {
    ...
    this.driver.onPair(async (session) => {
      ...
      session.setHandler('list_devices', async () => {
        ...
        try {
          ...
          this.driver.log(`✅ Found ${devices.length} devices`);
          return devices;
      ...
      session.setHandler('add_device', async (device) => {
        ...
        try {
          ...
          this.driver.log(`✅ Device added: ${device.data.ieeeAddress}`);
          return device;
  ...
  async validateDevice(device) {
    ...
    // Vérifier format IEEE address
    if (!/^[0-9a-fA-F]{16}$/.test(device.data.ieeeAddress.replace(/:/g, ''))) {
