...
class IASAlarmFallback {
  ...
  async init() {
    ...
    try {
      ...
      const sleepy = this.device.hasCapability?.('measure_battery') && this.device.mainsPowered !== true;
      if (sleepy && !BootBudget.shouldTxSleepy(this.device)) {
        this._startPolling();
        this.device.log('[IAS-FALLBACK] sleepy boot — bind/read on wake, slow backup poll');
        return;
      }
    } catch (_e) { /* fall through to bind */ }
    ...
    // WHY: IAS-only sleepy water (Peter #2184) — never wrap DP handlers / EF00.
    const iasOnly = this.device._iasOnlyProfile === true || this.device._skipTuyaDataQuery === true;
    if (this.useTuyaMirror && !iasOnly) {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            const attrs = await Promise.race([
              ...
              new Promise((_, rej) => this.homey.setTimeout(() => { if (this._destroyed) {return;} rej(new Error('timeout')); }, 3000))
            ]);
            
            this.device.log(`[IAS-FALLBACK] 📖 Initial read attempt ${attempt}:`, attrs);
            
            if (attrs?.zoneStatus !== undefined) {
              // v5.11.16 FIX Lasse_K: Route throughSensorBase for proper inversion
              if (typeof this.device._handleIASZoneStatus === 'function') {
                this.device._handleIASZoneStatus(attrs.zoneStatus);
              } else {
    ...
    try {
      ...
      if (ias && typeof ias.on === 'function') {
        ...
        ias.on('zoneStatusChangeNotification', (data) => {
          ...
          // v5.11.16 FIX Lasse_K: Route throughSensorBase handler when available
          // to use proper XOR inversion logic instead of duplicating with wrong flags
          const status = data?.zoneStatus ?? data?.payload?.zoneStatus ?? data;
          if (status !== undefined && status !== null) {
            if (typeof this.device._handleIASZoneStatus === 'function') {
              this.device._handleIASZoneStatus(status);
            } else {
   ...
   */
  _handleZoneStatus(rawStatus) {
    // v5.9.13: Parse Buffer/object to number first (HOBEIAN ZG-222Z fix)
    const status = this._parseZoneStatus(rawStatus);
    ...
    if (this.device.hasCapability('alarm_tamper')) {
      const setTamper = this.device.safeSetCapabilityValue || this.device.setCapabilityValue;
      setTamper.call(this.device, 'alarm_tamper', tamper).catch(() => {});
    }
  ...
  _startPolling() {
    const interval = this.pollInterval > 0 ? this.pollInterval : (6 * 60 * 60 * 1000);
    this.timer = this.homey.setInterval(() => {
      ...
      try {
        ...
        const sleepy = this.device.hasCapability?.('measure_battery') && this.device.mainsPowered !== true;
        if (sleepy && !BootBudget.isRecentlyAwake(this.device)) {return;}
      } catch (_e) { /* poll anyway if helper missing */ }
      this._poll();
    }, interval);
  }
  ...
  async _poll() {
    try {
      ...
      if (attrs?.zoneStatus !== undefined) {
        const status = this._parseZoneStatus(attrs.zoneStatus);
        if (status !== this.lastStatus) {
          ...
          if (typeof this.device._handleIASZoneStatus === 'function') {
            ...
            const alarm = (status & 0x03) > 0;
            this._updateAlarm(alarm);
          }
        }
      }
    } catch (e) { /* ponytail: poll failure is non-fatal; device may be offline */ }
  }
  ...
  _updateAlarm(value) {
    ...
    for (const cap of caps) {
      if (this.device.hasCapability(cap)) {
        // v5.11.16 FIX Lasse_K: Set IAS origin flags so device overrides skip re-inversion
        if (cap === 'alarm_water') {this.device._iasOriginatedWaterAlarm = true;}
        if (cap === 'alarm_contact') {this.device._iasOriginatedAlarm = true;}
        // WHY: L14 SanityFilter / anti-flood — never bypass safeSetCapabilityValue
        const setCap = this.device.safeSetCapabilityValue || this.device.setCapabilityValue;
        setCap.call(this.device, cap, value).catch(() => {});
  ...
  _parseZoneStatus(raw) {
    if (typeof raw === 'number') {return raw;}
    if (Buffer.isBuffer(raw)) {return raw.length >= 2 ? raw.readUInt16LE(0) : raw[0] || 0;}
    if (raw?.type === 'Buffer' && Array.isArray(raw.data)) {return this._parseZoneStatus(Buffer.from(raw.data));}
    if (typeof raw === 'object' && raw !== null) {
      return (raw.alarm1 ? 1 : 0) | (raw.alarm2 ? 2 : 0) | (raw.tamper ? 4 : 0) | (raw.batteryLow ? 8 : 0);
    }
    return 0;
  }
