...
class AdvancedFlowCardManager {
  ...
  _safeDeviceHandler(handler, cardName, defaultReturn = false) {
    return async (args, state) => {
      try {
        if (!args || !args.device) {
          this.safeApp?.error?.(`[ADV-FLOW] ${cardName}: No device in args`);
          return defaultReturn;
        }
        if (typeof args.device.getCapabilityValue !== 'function') {
          this.safeApp?.error?.(`[ADV-FLOW] ${cardName}: Invalid device reference`);
          return defaultReturn;
        }
        return await handler(args, state);
      } catch (err) {
        if (err.message?.toLowerCase().includes('device')) {
          this.safeApp?.error?.(`[ADV-FLOW] ${cardName}: ${err.message}`);
          return defaultReturn;
        }
        throw err;
      }
   ...
   *
   * Prefers shared HomeyCompensationLayer.safeGetFlowCard (does not touch
   * ZigBeeDriverFlowCardPatch / ZigBeeDriver.prototype).
   */
  _safeGetCard(type, id) {
    try {
      const { safeGetFlowCard } = require('./FlowCardGetters');
      const card = safeGetFlowCard(this.homey, id, type);
      if (card) {return card;}
    } catch (_e) { /* fall through to local cascade */ }

    if (!this.homey || !this.homey.flow) {return null;}
    try {
      switch (type) {
        case 'trigger': {
          if (typeof this.homey.flow.getDeviceTriggerCard === 'function') {
            return this.homey.flow.getDeviceTriggerCard(id);
          }
          if (typeof this.homey.flow.getTriggerCard === 'function') {
            return this.homey.flow.getTriggerCard(id);
          }
          if (typeof this.homey.flow.getTriggerCardById === 'function') {
            return this.homey.flow.getTriggerCardById(id);
          }
          return null;
        }
        case 'action': {
          if (typeof this.homey.flow.getDeviceActionCard === 'function') {
            return this.homey.flow.getDeviceActionCard(id);
          }
          if (typeof this.homey.flow.getActionCard === 'function') {
            return this.homey.flow.getActionCard(id);
          }
          if (typeof this.homey.flow.getActionCardById === 'function') {
            return this.homey.flow.getActionCardById(id);
          }
          return null;
        }
        case 'condition': {
          if (typeof this.homey.flow.getDeviceConditionCard === 'function') {
            return this.homey.flow.getDeviceConditionCard(id);
          }
          if (typeof this.homey.flow.getConditionCard === 'function') {
            return this.homey.flow.getConditionCard(id);
          }
          if (typeof this.homey.flow.getConditionCardById === 'function') {
            return this.homey.flow.getConditionCardById(id);
          }
          return null;
        }
        default: return null;
      ...
      // Card not registered in app.json or other error — skip silently
      this.safeApp?.error?.(`[ADV-FLOW] Failed to get ${type} card ${id}: ${err.message}`);
      return null;
    }
  }
  ...
  registerAllFlowCards() {
    ...
    for (const [name, fn] of categories) {
      try { fn(); ok++; } catch (e) {
        this.safeApp?.log?.(`[ADV-FLOW] ${name} cards failed (non-critical): ${e.message}`);
        fail++;
      }
    }
    this.safeApp?.log?.(`[ADV-FLOW] Registered ${ok} categories, ${fail} skipped`);
  }
  ...
  registerMotionSensorCards() {
    ...
    this._safeGetCard('action','enable_motion_sensor')?.registerRunListener(this._safeDeviceHandler(async (args) => {
        const device = args.device;
        await device.setCapabilityValue('alarm_motion_enabled', args.enabled).catch(() => {});
  ...
  registerButtonCards() {
    ...
    this._safeGetCard('action','enable_button')?.registerRunListener(this._safeDeviceHandler(async (args) => {
        const device = args.device;
        await device.setCapabilityValue('button_enabled', args.enabled).catch(() => {});
  ...
  registerLightCards() {
    ...
    this._safeGetCard('action','flash_light')?.registerRunListener(this._safeDeviceHandler(async (args) => {
        ...
        await device.setCapabilityValue('onoff', originalState).catch(() => {});
    ...
    this._safeGetCard('action','breathing_effect')?.registerRunListener(this._safeDeviceHandler(async (args) => {
        ...
        for (let i = 100; i >= 0; i -= 100 / steps) {
          await device.setCapabilityValue('dim', i / 100).catch(() => {});
