...
class BatteryHealthFlowHandler {
  ...
  _registerConditions() {
    ...
    // WHY(P2506): app-owned low flag — never Homey alarm_battery beside measure_battery
    try {
      const { CAP_ID } = require('../battery/SmartBatteryLowAlarm');
      const lowCard = this.homey.flow.getConditionCard('tuya_battery_is_low');
      if (lowCard) {
        lowCard.registerRunListener(async (args) => {
          const device = args.device;
          if (!device?.hasCapability?.(CAP_ID)) {return false;}
          return device.getCapabilityValue(CAP_ID) === true;
        });
      }
    } catch (_e) { /* soft */ }
  }
