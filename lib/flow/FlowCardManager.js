...
class FlowCardManager {
  ...
  _registerActionCard(cardId, handler) {
    try {
      const card = (() => { try { return (() => { try { return (() => { try { return (() => { try { return this.homey.flow.getDeviceActionCard(cardId); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })(); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })(); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })(); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })();
      if (card) {
