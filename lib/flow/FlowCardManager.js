...
class FlowCardManager {
  ...
  /**
   * v5.8.45: CLEANED UP - Only register flow cards that are actually defined in app.json
   * Previously 87 phantom registrations silently failed at startup.
   * DP cards (tuya_dp_send/received/equals) handled by UniversalFlowCardLoader.
   * Sub-capability cards (sub_capability_changed/is/set) handled by UniversalFlowCardLoader.
   */
  registerAll() {
    // v5.5.929: Switch backlight/countdown/child_lock action cards (defined in driver.flow.compose.json)
    this.registerSwitchBacklightCards();

    // v5.5.929: Plug LED indicator + power-on behavior action cards
