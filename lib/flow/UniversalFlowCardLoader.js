...
class UniversalFlowCardLoader {
  ...
  async initialize() {
    ...
    // WHY(P2449): fleet-wire declared brightness/scene/rotate cards left dead in compose
    try {
      const { autoWireDeclaredFlowCards } = require('./DeclaredFlowCardAutoWire');
      autoWireDeclaredFlowCards(this.homey, { log: (...a) => this.log(...a) });
    } catch (e) {
      this.log('P2449 auto-wire error:', e.message);
    }
    // WHY(P2635): fleet-wire switch/actuator Then/And cards (gang on/off/toggle)
    try {
      const { autoWireSwitchActuatorFlows } = require('./SwitchActuatorFlowAutoWire');
      autoWireSwitchActuatorFlows(this.homey, { log: (...a) => this.log(...a) });
    } catch (e) {
      this.log('P2635 switch auto-wire error:', e.message);
    }
  ...
  async _registerTypedDpCards() {
    ...
    // Condition: last known DP type matches (from tuya_dp_received store)
    try {
      const card = this.homey.flow.getConditionCard('tuya_dp_type_is');
      if (card) {
        card.registerRunListener(this._safeHandler(async (args) => {
          const dp = Number(args.dp);
          const want = args.dp_type?.id || args.dp_type;
          if (!Number.isInteger(dp) || !want) {return false;}
          const types = args.device.getStoreValue?.('lastDPTypes') || {};
          const got = types[String(dp)] || types[dp];
          return got === want;
        }, 'tuya_dp_type_is'));
        this._registered.conditions.add('tuya_dp_type_is');
      }
    } catch (e) { this.log('tuya_dp_type_is error:', e.message); }
  }
  ...
  async _registerSubCapabilityCards() {
    ...
    try {
      ...
      if (card) {
        card.registerRunListener(this._safeHandler(async (args) => {
          const rawCap = args.capability?.id || args.capability;
          if (!rawCap || typeof rawCap !== 'string') {return false;}
          const deviceCaps = typeof args.device.getCapabilities === 'function' ? args.device.getCapabilities() : [];
          const cap = deviceCaps.find(c => c.toLowerCase() === rawCap.toLowerCase()) || rawCap;
          const val = args.device.getCapabilityValue(cap);
    ...
    try {
      ...
      if (card) {
        card.registerRunListener(this._safeHandler(async (args) => {
          // v5.11.16 SEC: Validate capability exists on device before setting (caseless + object safe)
          const rawCap = args.capability?.id || args.capability;
          if (!rawCap || typeof rawCap !== 'string') {return false;}
          const deviceCaps = typeof args.device.getCapabilities === 'function' ? args.device.getCapabilities() : [];
          const cap = deviceCaps.find(c => c.toLowerCase() === rawCap.toLowerCase())
            || (args.device.hasCapability(rawCap) ? rawCap : null);
          if (!cap) {
            return false;
          }
          ...
          } else if (typeof args.device.setCapabilityValue === 'function') {
            await args.device.setCapabilityValue(cap, args.value).catch(() => {});
          } else if (typeof args.device.tx === 'function') {
            await args.device.tx({ capability: cap, value: args.value });
          }
          return true;
        }, 'sub_capability_set'));
    ...
    // Universal sub-capability toggle (LOADER_MANAGED — must register here)
    try {
      const card = this.homey.flow.getActionCard('sub_capability_toggle');
      if (card) {
        card.registerRunListener(this._safeHandler(async (args) => {
          const rawCap = args.capability?.id || args.capability;
          if (!rawCap || typeof rawCap !== 'string') {return false;}
          ...
          const cur = args.device.getCapabilityValue(cap);
          const next = !cur;
          this.log(`[FLOW] sub_capability_toggle ${cap}: ${cur} → ${next}`);
          if (typeof args.device.triggerCapabilityListener === 'function') {
            await args.device.triggerCapabilityListener(cap, next);
          } else if (typeof args.device.safeSetCapabilityValue === 'function') {
            await args.device.safeSetCapabilityValue(cap, next).catch(() => {});
          } else if (typeof args.device.setCapabilityValue === 'function') {
            await args.device.setCapabilityValue(cap, next).catch(() => {});
          } else if (typeof args.device.tx === 'function') {
            await args.device.tx({ capability: cap, value: next });
          }
          return true;
        }, 'sub_capability_toggle'));
        this._registered.actions.add('sub_capability_toggle');
      }
    } catch (e) { this.log('sub_capability_toggle error:', e.message); }

    // v5.9.15: Fallback Multi-Gang specific cards (Issue #170) with 5-tier cascade
    try {
      ...
      if (turnOnCard) {
        turnOnCard.registerRunListener(this._safeHandler(async (args) => {
          ...
          if (typeof args.device.setGangOnOff === 'function') {
            await args.device.setGangOnOff(gangNum, true);
            return true;
          }
          if (args.device.hasCapability && args.device.hasCapability(capName)) {
            if (typeof args.device.triggerCapabilityListener === 'function') {
              await args.device.triggerCapabilityListener(capName, true);
              return true;
            }
            await args.device.setCapabilityValue(capName, true).catch(() => {});
            return true;
          }
          if (gangNum === 1 && args.device.hasCapability && args.device.hasCapability('onoff')) {
            if (typeof args.device.triggerCapabilityListener === 'function') {
          ...
          if (typeof args.device.tx === 'function') {
            const r = await args.device.tx({ capability: capName, value: true });
            if (r?.ok) {return true;}
          }
          return false;
      ...
      if (turnOffCard) {
        turnOffCard.registerRunListener(this._safeHandler(async (args) => {
          ...
          if (typeof args.device.setGangOnOff === 'function') {
            await args.device.setGangOnOff(gangNum, false);
            return true;
          }
          if (args.device.hasCapability && args.device.hasCapability(capName)) {
            if (typeof args.device.triggerCapabilityListener === 'function') {
              await args.device.triggerCapabilityListener(capName, false);
              return true;
            }
            await args.device.setCapabilityValue(capName, false).catch(() => {});
            return true;
          }
          if (gangNum === 1 && args.device.hasCapability && args.device.hasCapability('onoff')) {
            if (typeof args.device.triggerCapabilityListener === 'function') {
          ...
          if (typeof args.device.tx === 'function') {
            const r = await args.device.tx({ capability: capName, value: false });
            if (r?.ok) {return true;}
          }
          return false;
  ...
  // WHY(P2659/Bastien 4c0d232b): compose tokens declare value as string — Homey rejects boolean.
  ...
  async triggerDPReceived(device, dp, value, dpType = null) {
    try {
      const card = this.homey.flow.getDeviceTriggerCard('tuya_dp_received');
      ...
      if (!type) {
        ...
        else if (typeof value === 'number') {type = 'value';}
        else if (Buffer.isBuffer(value)) {type = 'raw';}
        else if (typeof value === 'string') {type = 'string';}
      }
      if (type) {
        const types = device.getStoreValue?.('lastDPTypes') || {};
        types[String(dp)] = type;
        await device.setStoreValue?.('lastDPTypes', types);
      }
    } catch (e) {
