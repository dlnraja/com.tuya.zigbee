...
class UniversalFlowCardLoader {
  ...
  async _registerGenericDPCards() {
    ...
    // DP Condition
    try {
      const card = this.homey.flow.getDeviceConditionCard('tuya_dp_equals');
      if (card) {
    ...
    // DP Action
    try {
      const card = this.homey.flow.getDeviceActionCard('tuya_dp_send');
      if (card) {
        card.registerRunListener(this._safeHandler(async (args) => {
          const d = args.device;
  ...
  async _registerSubCapabilityCards() {
    ...
    try {
      const card = this.homey.flow.getDeviceConditionCard('sub_capability_is');
      if (card) {
        card.registerRunListener(this._safeHandler(async (args) => {
    ...
    try {
      const card = this.homey.flow.getDeviceActionCard('sub_capability_set');
      if (card) {
        card.registerRunListener(this._safeHandler(async (args) => {
    ...
    try {
      const turnOnCard = this.homey.flow.getDeviceActionCard('switch_multi_gang_turn_on');
      if (turnOnCard) {
      ...
      const turnOffCard = this.homey.flow.getDeviceActionCard('switch_multi_gang_turn_off');
      if (turnOffCard) {
