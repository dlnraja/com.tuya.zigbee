 ...
 * P2635: resolveFlowDevice — never hard-fail only on missing shape quirks.
 */
...
function registerOnoffFlowCards(driver, prefix, opts = {}) {
  ...
  try {
    ...
    if (card) {
      card.registerRunListener(async (args) => {
        const device = resolveFlowDevice(args); if (!device) { return false; }
        const cap = capability || resolveOnoffCapability(device, gang);
  ...
  try {
    ...
    if (card) {
      card.registerRunListener(async (args) => {
        const device = resolveFlowDevice(args); if (!device) { return false; }
        if (capability) { return setActuatorCapability(device, capability, true); }
        return setOnoff(device, true, gang);
      });
    }
  ...
  try {
    ...
    if (card) {
      card.registerRunListener(async (args) => {
        const device = resolveFlowDevice(args); if (!device) { return false; }
        if (capability) { return setActuatorCapability(device, capability, false); }
        return setOnoff(device, false, gang);
      });
    }
    ...
    if (card) {
      card.registerRunListener(async (args) => {
        ...
        if (capability) {
          ...
          return setActuatorCapability(device, capability, !cur);
        }
        return setOnoff(device, 'toggle', gang);
      });
    }
    ...
    try {
      ...
      if (card) {
        card.registerRunListener(async (args) => {
          ...
          // WHY(P2449): flow range 0–100 → Homey dim 0–1
          if (dim > 1) dim = dim / 100;
          dim = Math.max(0, Math.min(1, dim));
          return setActuatorCapability(device, dimCap, dim);
        });
      }
...
function registerCoverOpenClose(driver, openId, closeId, opts = {}) {
  ...
  try {
    ...
    if (card) {
      card.registerRunListener(async (args) => setPos(resolveFlowDevice(args), 1));
    }
  } catch (_) { /* */ }
  ...
  try {
    ...
    if (card) {
      card.registerRunListener(async (args) => setPos(resolveFlowDevice(args), 0));
    }
  } catch (_) { /* */ }
}
...
function registerAlarmCondition(driver, cardId, capability, expected = true) {
  try {
    ...
    card.registerRunListener(async (args) => {
      const device = resolveFlowDevice(args); if (!device) { return false; }
      return device.getCapabilityValue(capability) === expected;
    });
  } catch (_) { /* */ }
}

/**
 * P2449 — Register set_brightness / brightness_above / brightness_changed for a prefix.
 */
function registerBrightnessFlowCards(driver, prefix, opts = {}) {
  const dimCap = opts.dimCapability || 'dim';
  const log = (msg) => { try { driver.log?.(msg); } catch (_) { /* */ } };

  try {
    const card = driver.homey.flow.getActionCard(`${prefix}_set_brightness`)
      || driver.homey.flow.getActionCard(`${prefix}_set_dim`);
    if (card) {
      card.registerRunListener(async (args) => {
        ...
        if (raw == null) return false;
        if (typeof device.setKnobBrightnessPercent === 'function') {
          return device.setKnobBrightnessPercent(raw);
        }
        let dim = Number(raw);
        if (!Number.isFinite(dim)) return false;
        if (dim > 1) dim /= 100;
  ...
  } catch (err) { log(`[FLOW] ${prefix} set_brightness: ${err.message}`); }

  try {
    const card = driver.homey.flow.getConditionCard(`${prefix}_brightness_above`);
    if (card) {
      card.registerRunListener(async (args) => {
        const device = resolveFlowDevice(args);
        if (!device) return false;
        const v = Number(device.getCapabilityValue(dimCap) ?? 0);
        const pct = v > 1 ? v : v * 100;
        return pct > Number(args.level ?? 0);
      });
    }
  } catch (err) { log(`[FLOW] ${prefix} brightness_above: ${err.message}`); }

  try {
    const card = driver.homey.flow.getDeviceTriggerCard(`${prefix}_brightness_changed`);
    if (card?.registerRunListener) {
      // WHY(P2635): device triggers omit args.device in runListener
      card.registerRunListener(async () => true);
    }
  } catch (err) { log(`[FLOW] ${prefix} brightness_changed: ${err.message}`); }

  log(`[FLOW] Brightness cards registered for ${prefix}`);
}
