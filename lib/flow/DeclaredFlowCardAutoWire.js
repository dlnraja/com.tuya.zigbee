...
const { resolveFlowDevice, shouldRunForDeviceAndButton } = require('./FlowCardDeviceResolve');
...
function autoWireDeclaredFlowCards(homey, opts = {}) {
  ...
  // WHY(P2635): device triggers omit args.device — never gate on !!args.device
  const triggerHandler = async (args = {}, state = {}) => shouldRunForDeviceAndButton(args, state);
  // WHY(P2747): cards with dropdown args MUST have runListener or Homey never starts the Flow
  // (tag icon = tokens; args = filter). Wire ALL triggers that declare args.
  const triggerHandlerAlways = async () => true;
  ...
  for (const driver of drivers) {
    ...
    if (triggers) {
      for (const t of triggers) {
        ...
        const hasArgs = Array.isArray(t.args) && t.args.length > 0;
        const special = RE_BRIGHTNESS_CHANGED.test(id)
          || RE_SCENE_RECALL.test(id)
          || RE_ROTATE.test(id);
        // P2449 specials + P2747 every trigger that has filter args (button/gang/…)
        if (!special && !hasArgs) continue;
        const key = `trigger:${id}`;
        const handler = hasArgs ? triggerHandler : triggerHandlerAlways;
        if (safeRegister(getTrigger(id), handler, registered, key)) {
          stats.triggers += 1;
        }
      }
    }
    ...
    if (conditions) {
      for (const c of conditions) {
        ...
        if (safeRegister(getCondition(id), async (args) => {
          ...
          return getDimPercent(device) > Number(args.level ?? args.threshold ?? 0);
        }, registered, key)) {
          stats.conditions += 1;
        }
      }
    ...
    if (actions) {
      for (const a of actions) {
        ...
        if (safeRegister(getAction(id), async (args) => {
          ...
          return setActuatorCapability(device, 'dim', dim);
        }, registered, key)) {
          stats.actions += 1;
        }
      }
