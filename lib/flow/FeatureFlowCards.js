        ...
        if (data.type === 'battery_critical' || data.type === 'battery_low') {
          try {
            ...
            if (card && device) {
              ...
              const current = Number(batt.currentLevel);
              const drain = Math.abs(Number(batt.drainRate) || 0);
              const days = Number(batt.daysRemaining);
              // Homey rejects undefined number tokens ("Invalid value for token current_battery")
              card.trigger(device, {
                current_battery: Number.isFinite(current) ? current : 0,
                drain_rate_per_day: Number.isFinite(drain) ? drain : 0,
                days_remaining: Number.isFinite(days) ? days : 0,
                battery_type: batt.alert?.type || 'unknown',
