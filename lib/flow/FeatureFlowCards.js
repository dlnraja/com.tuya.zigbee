...
class FeatureFlowCards {
    ...
    if (this._predictiveHealthEngine) {
      this._predictiveHealthEngine.on('deviceCritical', (data) => {
        try {
          ...
          const card = this.homey.flow.getDeviceTriggerCard('health_failure_predicted');
          if (card && device) {
            // Number tokens must never be null (Homey rejects with "Invalid value")
            card.trigger(device, {
              ...
              days_until_failure: Number.isFinite(data.analysis?.daysUntilFailure) ? data.analysis.daysUntilFailure : 0,
      ...
      this._predictiveHealthEngine.on('alert', (data) => {
        if (data.type === 'battery_critical' || data.type === 'battery_low') {
          try {
            const device = this._resolveDevice(data.deviceId);
            const card = this.homey.flow.getDeviceTriggerCard('health_battery_replacement_predicted');
            if (card && device) {
              ...
              card.trigger(device, {
                ...
                days_remaining: Number.isFinite(analysis.battery?.daysRemaining) ? analysis.battery.daysRemaining : 0,
  ...
  _registerLightAlertCards() {
    // -- Action: Blink a light via ZCL Identify (fallback: toggle pulses) --
    this._safeRegister('action', 'light_alert_blink', async (args) => {
      ...
      // Preferred path: ZCL Identify cluster (cluster 3) — supported by most
      // Tuya bulbs/switches, no state change, hardware-native blink.
      try {
        const zclNode = device.zclNode;
        const identifyCluster = zclNode?.endpoints?.[1]?.clusters?.identify;
        if (identifyCluster && typeof identifyCluster.identify === 'function') {
          await identifyCluster.identify({ identifyTime: seconds });
          this.log(`[ALERT] Identify ${seconds}s on ${device.getName?.()}`);
          return true;
        }
      } catch (e) {
        this.log(`[ALERT] identify cluster failed, fallback toggle: ${e.message}`);
      }

      // Fallback: N on/off pulses, restoring the original state afterwards.
      if (typeof device.setCapabilityValue === 'function' && device.hasCapability?.('onoff')) {
        ...
        for (let i = 0; i < pulses; i++) {
          await device.setCapabilityValue('onoff', i % 2 === 0 ? !original : original).catch(() => {});
          await new Promise((r) => scheduler.setTimeout(r, 500));
        }
        await device.setCapabilityValue('onoff', original).catch(() => {});
        this.log(`[ALERT] ${pulses} toggle pulses on ${device.getName?.()}`);
        return true;
      }
