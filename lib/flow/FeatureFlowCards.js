  ...
  _registerSolarCards() {
    ...
    if (this._solarElevation) {
      this._solarElevation.on('sunrise', (data) => {
        try {
          const card = this.homey.flow.getTriggerCard('solar_sunrise_detected');
          const tokens = this._getSolarFlowTokens(data);
          card?.trigger(tokens, { _lastWasBelow: true, elevation_threshold: 0 });
        } catch (_e) { /* non-critical */ }
      });
      this._solarElevation.on('sunset', (data) => {
        try {
          const card = this.homey.flow.getTriggerCard('solar_sunset_detected');
          const tokens = this._getSolarFlowTokens(data);
          card?.trigger(tokens, { _lastWasAbove: true, elevation_threshold: 0 });
        } catch (_e) { /* non-critical */ }
      });
    }
