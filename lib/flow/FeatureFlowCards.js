 ...
 *   - P2687 Interaction history (list caps/actions + fleet ring)
 *
...
class FeatureFlowCards {
  ...
  _registerDeviceCapabilitiesCards() {
    ...
    // P2687 — fleet interaction ring (action-like + sensor changes)
    if (!this._interactionRing) {
      this._interactionRing = [];
      this._interactionMaxAge = 24 * 60 * 60 * 1000;
      this._interactionMaxEntries = 200;
      this._deviceInteractionRing = new Map(); // deviceId -> [{ts, capability, value, kind}]
    }
    ...
    // -- P2687 Trigger: action-like device interaction --
    this._safeRegister('trigger', 'device_interaction', (args, state) => {
      if (args.device && state.deviceId && args.device.id !== state.deviceId) {return false;}
      return true;
    });
    ...
    // -- P2687 Action: list all capabilities --
    this._safeRegister('action', 'list_device_capabilities', async (args) => {
      const device = args.device;
      if (!device || typeof device.getCapabilities !== 'function') {
        return { list: '', count: 0 };
      }
      const caps = device.getCapabilities() || [];
      return { list: caps.join(', '), count: caps.length };
    });

    // -- P2687 Action: list setable / action-like capabilities --
    this._safeRegister('action', 'list_device_actions', async (args) => {
      const device = args.device;
      if (!device) {return { list: '', count: 0 };}
      const actions = listActionableCapabilities(device);
      return { list: actions.join(', '), count: actions.length };
    });

    // -- P2687 Action: list recent interactions for one device --
    this._safeRegister('action', 'list_recent_interactions', async (args) => {
      const device = args.device;
      if (!device) {return { list: '', count: 0 };}
      const id = device.id || device.getData?.()?.id;
      const entries = (id && this._deviceInteractionRing?.get(id)) || [];
      return formatInteractionList(entries, args.limit);
    });

    // -- P2687 Action: fleet-wide recent interactions --
    this._safeRegister('action', 'list_fleet_recent_interactions', async (args) => {
      const ring = this._interactionRing || [];
      return formatInteractionList(ring, Math.min(50, Number(args.limit) || 20));
    });

    // -- P2687 Condition: device interacted recently --
    this._safeRegister('condition', 'device_interacted_recently', (args) => {
      const device = args.device;
      if (!device) {return false;}
      const id = device.id || device.getData?.()?.id;
      return interactedRecently(this._interactionRing || [], id, args.minutes);
    });
    ...
    this._registered.triggers.add('device_interaction');
    this._registered.actions.add('capability_historical_value');
    this._registered.actions.add('list_device_capabilities');
    this._registered.actions.add('list_device_actions');
    this._registered.actions.add('list_recent_interactions');
    this._registered.actions.add('list_fleet_recent_interactions');
    ...
    this._registered.conditions.add('device_interacted_recently');
    ...
    this.homey.log('[FEATURE-FLOW] Device Capabilities cards registered (P2687 interaction + battery % + virtual press)');
  }
   ...
   * @param {*} value
   * @param {{ oldValue?: *, deviceName?: string, kind?: string }} [meta]
   */
  storeCapabilityHistory(deviceId, capability, value, meta = {}) {
    ...
    // P2687 — also push fleet + per-device interaction rings
    try {
      let setable;
      const device = this._resolveDevice(deviceId);
      if (device && typeof device.getCapabilityOptions === 'function') {
        try {
          setable = device.getCapabilityOptions(capability)?.setable === true;
        } catch (_e) { /* soft */ }
      }
      const kind = meta.kind || classifyInteractionKind(capability, { setable });
      const deviceName = meta.deviceName
        || (device && (typeof device.getName === 'function' ? device.getName() : device.name))
        || '';
      const entry = {
        ts: now,
        deviceId,
      ...
      if (!this._interactionRing) {this._interactionRing = [];}
      pushInteraction(this._interactionRing, entry, {
        maxAgeMs: this._interactionMaxAge,
        maxEntries: this._interactionMaxEntries,
      });
      if (!this._deviceInteractionRing) {this._deviceInteractionRing = new Map();}
      let perDev = this._deviceInteractionRing.get(deviceId);
      if (!perDev) {
        perDev = [];
        this._deviceInteractionRing.set(deviceId, perDev);
      }
      pushInteraction(perDev, entry, {
        maxAgeMs: this._interactionMaxAge,
        maxEntries: 100,
      });
    } catch (_e) { /* non-critical */ }
  ...
  triggerCapabilityChanged(deviceId, capability, newValue, oldValue) {
    try {
      ...
      const kind = classifyInteractionKind(capability, { setable });
      const deviceName = (device && (typeof device.getName === 'function' ? device.getName() : device.name)) || '';

      // Store in history + P2687 interaction rings
      ...
      // P2687 — fire device_interaction on action-like changes only
      if (kind === 'action') {
        try {
          const ix = this.homey.flow.getTriggerCard('device_interaction');
          if (ix) {
            ix.trigger({
  ...
  _triggerFeatureCard(cardId, tokens = {}, state = {}) {
    try {
      ...
      if (result && typeof result.catch === 'function') {
        result.catch(err => {
          if (typeof this.homey.error === 'function') {
  ...
  _safeRegister(type, cardId, runListener, stateFactory) {
    try {
      // WHY(P2664 / Bastien 52ef684a): app restart / double init spammed
      // "Run listener was already registered" and left flow cards flaky.
      const bucket = type === 'trigger' ? this._registered.triggers
        : type === 'condition' ? this._registered.conditions
          : this._registered.actions;
      if (bucket.has(cardId)) return;
  ...
  destroy() {
    ...
    if (this._interactionRing) {
      this._interactionRing.length = 0;
      this._interactionRing = null;
    }
    if (this._deviceInteractionRing) {
      this._deviceInteractionRing.clear();
      this._deviceInteractionRing = null;
    }
