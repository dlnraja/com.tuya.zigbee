...
class FlowTriggerHelpers {
  ...
  async triggerButtonReleased(duration) {
    try {
      await this.device.homey.flow.getTriggerCard('button_released')
        .trigger(this.device, {
  ...
  async triggerTemperatureChanged(current, previous) {
    try {
      await this.device.homey.flow.getTriggerCard('temperature_changed')
        .trigger(this.device, {
  ...
  async triggerHumidityChanged(current, previous) {
    try {
      await this.device.homey.flow.getTriggerCard('humidity_changed')
        .trigger(this.device, {
  ...
  async triggerBatteryLow(battery, voltage = null) {
    try {
      ...
      await this.device.homey.flow.getTriggerCard('battery_low')
  ...
  async triggerMotionStarted() {
    try {
      await this.device.homey.flow.getTriggerCard('motion_started')
        .trigger(this.device, {
  ...
  async triggerMotionStopped(duration) {
    try {
      await this.device.homey.flow.getTriggerCard('motion_stopped')
        .trigger(this.device, {
  ...
  async triggerPresenceChanged(present) {
    try {
      await this.device.homey.flow.getTriggerCard('presence_changed')
        .trigger(this.device, {
          present: present,
  ...
  async triggerContactOpened(duration_closed = 0) {
    try {
      await this.device.homey.flow.getTriggerCard('contact_opened')
        .trigger(this.device, {
  ...
  async triggerContactClosed(duration_open = 0) {
    try {
      await this.device.homey.flow.getTriggerCard('contact_closed')
        .trigger(this.device, {
  ...
  async triggerAlarmTriggered(type) {
    try {
      await this.device.homey.flow.getTriggerCard('alarm_triggered')
        .trigger(this.device, {
          type: type,
          timestamp: new Date().toISOString()
        });
  ...
  async triggerDeviceOnline(offline_duration = 0) {
    try {
      await this.device.homey.flow.getTriggerCard('device_online')
        .trigger(this.device, {
  ...
  async triggerDeviceOffline() {
    try {
      await this.device.homey.flow.getTriggerCard('device_offline')
        .trigger(this.device, {
          timestamp: new Date().toISOString()
        });
  ...
  async triggerTargetTemperatureReached(target, current) {
    try {
      await this.device.homey.flow.getTriggerCard('target_temperature_reached')
        .trigger(this.device, {
