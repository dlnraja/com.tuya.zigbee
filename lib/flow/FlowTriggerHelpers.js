...
class FlowTriggerHelpers {
  ...
  async triggerButtonReleased(duration) {
    try {
      const card = this.device._getFlowCard('button_released');
      if (card) {
        await card.trigger(this.device, {
          duration: Math.round(duration *safeParse(10), 10)
        });
  ...
  async triggerTemperatureChanged(current, previous) {
    try {
      ...
      if (card) {
        await card.trigger(this.device, {
          current: Math.round(current *safeParse(10), 10),
          previous: Math.round(previous *safeParse(10), 10),
          change: Math.round((current - previous) *safeParse(10), 10)
        });
  ...
  async triggerBatteryLow(battery, voltage = null) {
    try {
      ...
      if (voltage !== null) {
        tokens.voltage = Math.round(voltage *safeParse(100), 100);
      }
  ...
  async triggerTargetTemperatureReached(target, current) {
    try {
      ...
      if (card) {
        await card.trigger(this.device, {
          target: Math.round(target *safeParse(10), 10),
          current: Math.round(current *safeParse(10), 10)
        });
