...
class IASZoneEnrollerEnhanced {
      ...
      async () => {
        ...
        if (zclNode?._node?.bridgeId) {
          ...
          if (Buffer.isBuffer(bridgeId) && bridgeId.length >= 8) {
            ...
            const hexOnly = String(bridgeId).replace(/[^0-9a-fA-F]/g, '').toLowerCase();
  ...
  updateCapability(isAlarm) {
    ...
    if (isAlarm) {
      ...
      this.device.Promise.resolve(setCapabilityValue(capability, true)).catch(err => {
      ...
      if (this.options.autoResetTimeout > 0) {
        ...
        this.resetTimer = setTimeout(() => {
          ...
          this.device.Promise.resolve(setCapabilityValue(capability, false)).catch(err => {
