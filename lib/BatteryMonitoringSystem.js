...
class BatteryMonitoringSystem {
  ...
  async init() {
    try {
      ...
      this.device.log('✅ Battery monitoring system initialized');
      return true;
    } catch (err) {
  ...
  async checkBatteryLevel(level) {
    try {
      ...
      switch (status) {
        case 'critical':
          await this.device.setWarning(`⚠️ CRITICAL: Battery ${level}% - Replace immediately!`);
      ...
      if (previousLevel !== null && previousLevel < 30 && level > 80) {
        this.device.log('🔋 Battery replacement detected!');
