 ...
 * ║  - Éviter le mode inutile quand on sait déjà quel protocole utiliser  ║
...
const PROTOCOL = {
  ...
  UNKNOWN: 'unknown',           // Unknown - use and detect
};
...
const PRODUCT_ID_PATTERNS = {
  // TS0601 = USUALLY Tuya DP, but NOT always!
  // Some TS0601 devices (e.g. _TZE200_3towulqd ZG-204ZL) are ZCL-only
  // Protocol must be determined at runtime by checking for 0xEF00 cluster
  'TS0601': { protocol: PROTOCOL.HYBRID, notes: 'Tuya DP device (0xEF00) OR ZCL-only - requires runtime cluster detection' },
 ...
 * v5.8.88: TS0601 is NOT always Tuya DP - some variants are ZCL-only
 * This function now returns true only for TS0601 (which MAY use Tuya DP),
 * but callers must also verify 0xEF00 cluster presence at runtime.
 */
function isTuyaDPProductId(productId) {
  // TS0601 is the primary Tuya DP productId, but runtime cluster check is required
  // because some TS0601 devices (e.g. _TZE200_3towulqd) are ZCL-only
  return productId && /^TS060[1-7]$/.test(productId.toUpperCase());
}
