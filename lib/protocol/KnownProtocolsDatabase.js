 ...
 * 
 *                                                                               
 *   Base de donnÃ©es des protocoles CONNUS par manufacturer/productId            
 *                                                                               
 *   Cette base permet de:                                                        
 *   - Ã‰viter le mode hybrid inutile quand on sait dÃ©jÃ quel protocole utiliser  
 *   - AccÃ©lÃ©rer l'initialisation des devices connus                             
 *   - Toujours vÃ©rifier aprÃ¨s 15 min (constructeurs chinois = surprises)        
 *                                                                               
 *   Sources:                                                                     
 *   - Zigbee2MQTT tuya.ts                                                        
 *   - ZHA quirks database                                                        
 *   - Johan Bendz com.tuya.zigbee                                               
 *   - ExpÃ©rience utilisateur Homey Community                                     
 *                                                                               
 * 
 */
...
// Protocol types
const PROTOCOL = {
  TUYA_DP: 'tuya_dp',           // Uses Tuya CLUSTERS.TUYA_EF00 cluster exclusively
...
const KNOWN_DEVICES = {
  // 
  // TS0601 DEVICES - Always Tuya DP (cluster CLUSTERS.TUYA_EF00)
  // 
...
const PRODUCT_ID_PATTERNS = {
  ...
  'TS0601': { protocol: PROTOCOL.TUYA_DP, notes: 'Tuya DP device (CLUSTERS.TUYA_EF00)' },
...
function lookupProtocol(manufacturerName, productId) {
  ...
  if (productId) {
    for (const [pattern, info] of Object.entries(PRODUCT_ID_PATTERNS)) {
      if (CI.startsWithCI(productId, pattern)) {
        return {
          source: 'productId',
          ...info
        };
...
function isTuyaDPManufacturer(manufacturerName) {
  ...
  return CI.startsWithCI(manufacturerName, ['_tze200_', '_tze204_', '_tze284_']);
}
...
function isTuyaDPProductId(productId) {
  return productId && CI.equalsCI(productId, 'TS0601');
}
