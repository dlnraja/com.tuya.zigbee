...
class IntelligentProtocolRouter {
  ...
  checkManufacturerRequirement(manufacturerName) {
    ...
    // Sacred BSEED / ZCL-only switches must NEVER be forced onto Tuya DP
    // (multi-gang DP probing cross-links gangs — PhysicalButtonMixin zcl_only).
    try {
      const { isSacredZclOnlyManufacturer } = require('./IntelligentProtocolDetect');
      if (isSacredZclOnlyManufacturer(manufacturerName)) {
        this.log('[PROTOCOL-ROUTER] Sacred ZCL-only manufacturer — do NOT force DP');
        return false;
      }
    } catch (_) { /* optional */ }

    // Other manufacturers that typically use Tuya DP (heuristic only — cluster truth wins)
