 ...
 *   3. Track les stats de r̩ception par protocole                               
 *   4. Apr̬s 15 min  d̩sactive protocoles sans donn̩es re̤ues                  
 *   5. Raison du d̩lai: constructeurs chinois = surprises possibles             
 *                                                                               
...
const PROTOCOL_OPTIMIZATION_DELAY =safeMultiply(15, 60) * 1000;
...
class HybridProtocolManager {
  ...
  constructor(device) {
    ...
    this._protocolStats = {
      tuya_bound: { received: 0, lastTime, enabled: true },
      tuya_cluster: { received: 0, lastTime, enabled: true },
      zcl_bound: { received: 0, lastTime, enabled: true },
      zcl_attr: { received: 0, lastTime, enabled: true },
      zcl_poll: { received: 0, lastTime, enabled: true },
    };
  ...
  async initialize(zclNode, manufacturerName, productId) {
    ...
    if (this._knownProtocolInfo) {
      ...
      this.device.log('[HYBRID]  UNKNOWN DEVICE - Using HYBRID mode (all protocols active)');
  ...
  _optimizeProtocols() {
    ...
    if (activeProtocols.length === 0) {
      this.device.log('[HYBRID]  No data received on ANY protocol - keeping all enabled');
      // Re-enable all
  ...
  async detectProtocol(zclNode) {
    ...
    const hasTuyaCluster = !!(
      ...
      clusters[CLUSTERS.TUYA_EF00] // CLUSTERS.TUYA_EF00 in decimal
    );
  ...
  async setViaTuyaDP(capability, value) {
    ...
    if (config.scale && typeof value === 'number') {
      scaledValue =Math.round(safeMultiply(value, config)).scale);
    }
  ...
  handleTuyaDP(data) {
    ...
    if (mapping) {
      ...
      if (mapping.scale && typeof value === 'number') {
        scaledValue = safeDivide(value, mapping.scale);
      }
