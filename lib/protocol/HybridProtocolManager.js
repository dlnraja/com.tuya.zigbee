...
try {
  TuyaProtocolManager = require('../tuya/TuyaProtocolManager');
} catch (e) {
...
class ProtocolManager {
  ...
  _configureKnownProtocols() {
    ...
    // v5.8.88: TS0601 devices may be ZCL-only (e.g. _TZE200_3towulqd)
    // Do NOT pre-configure protocols for TS0601 - wait for runtime cluster detection
    const productId = this.device.getData?.()?.modelId || '';
    if (/^TS060[1-7]$/.test(productId)) {
      this.device.log('[HYBRID] 📊 TS0601 detected - deferring protocol config to runtime cluster detection');
      return;
    }
   ...
   */
  _optimizeProtocols() {
    // BOTH (Peter #2184 / diag 1cf775a2): sleepy IAS devices (water/SOS/button)
    // often receive ZERO messages for 15+ min. Disabling protocols then
    // re-enabling still drops listeners. Never optimize-disable them.
    // WHY(P2294): also lock button remotes even if IntelligentBattery wrongly
    // stripped measure_battery (Peter 4b1a0dc9 on 9.0.669 — HYBRID then disabled zcl_bound).
    const driverId = String(this.device?.driver?.id || '');
    const isButtonish = /^button_|remote_|scene_switch_/i.test(driverId)
      || this.device?._deviceProfile?.skipBatteryReporting === true
      || (typeof this.device.getDeviceProfile === 'function'
        && this.device.getDeviceProfile()?.skipBatteryReporting === true);
    // WHY(P2363 / diag 05867379): covers often TX-OK with sparse RX for 15+ min.
    // Disabling tuya_* / zcl_* then leaves Homey UI waiting on stale position.
    const isCoverish = /curtain|cover|shutter|blind|windowcovering|garage/i.test(driverId)
      || (typeof this.device.hasCapability === 'function'
        && (this.device.hasCapability('windowcoverings_set')
          || this.device.hasCapability('windowcoverings_state')));
    // WHY(P2382 / PresentSky #2206/#2221 diag 60959c24): EF00 wall dimmers are
    // MCU-only (no genOnOff/levelControl). trackDataReceived is rarely wired, so
    // 15-min optimize can mark tuya_cluster dead while DP RX still works → UI TX gap.
    const isEf00Dimmer = /wall_dimmer|light_dimmer|_dimmer_tuya|dimmer_tuya/i.test(driverId)
      || (typeof this.device.hasCapability === 'function'
        && this.device.hasCapability('dim')
        && this.device.hasCapability('onoff')
        && !!(this.device.dpMappings?.[2] || this.device.dpMappings?.['2']));
    const isSleepy = !!(this.device._iasOnlyProfile
      || this.device._skipTuyaDataQuery
      || isButtonish
      || isCoverish
      || isEf00Dimmer
      || (typeof this.device.hasCapability === 'function'
    ...
    if (isSleepy) {
      this.device.log('[HYBRID] Skip protocol disable on sleepy/IAS-only/button/cover/EF00-dimmer device');
      return;
    }
    this.device.log('[HYBRID] ════════════════════════════════════════════════════');
