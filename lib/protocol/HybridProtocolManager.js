...
class HybridProtocolManager {
  ...
  configureTuyaDPMappings() {
    ...
    // Standard Tuya DP mappings (documentation officielle)
    const standardMappings = {
      // Multi-Gang Switches (DP1-4)
      1: { capability: 'onoff', description: 'Switch gang 1', type: 'bool' },
      2: { capability: 'onoff.gang2', description: 'Switch gang 2', type: 'bool' },
      3: { capability: 'onoff.gang3', description: 'Switch gang 3', type: 'bool' },
      4: { capability: 'onoff.gang4', description: 'Switch gang 4', type: 'bool' },
      
      // Countdown Timers (DP7-10)
      7: { capability: 'countdown.gang1', description: 'Countdown timer gang 1', type: 'value' },
      8: { capability: 'countdown.gang2', description: 'Countdown timer gang 2', type: 'value' },
      9: { capability: 'countdown.gang3', description: 'Countdown timer gang 3', type: 'value' },
      10: { capability: 'countdown.gang4', description: 'Countdown timer gang 4', type: 'value' },
      
      // Power-on behavior (DP14, DP29-32)
      14: { capability: 'power_on_behavior', description: 'Main power-on behavior', type: 'enum' },
      29: { capability: 'power_on_behavior.gang1', description: 'Power-on gang 1', type: 'enum' },
      30: { capability: 'power_on_behavior.gang2', description: 'Power-on gang 2', type: 'enum' },
      31: { capability: 'power_on_behavior.gang3', description: 'Power-on gang 3', type: 'enum' },
      32: { capability: 'power_on_behavior.gang4', description: 'Power-on gang 4', type: 'enum' },
      
      // LED & Backlight (DP15-16)
      15: { capability: 'led_behavior', description: 'LED indicator', type: 'enum' },
      16: { capability: 'backlight', description: 'Backlight', type: 'bool' },
      
      // Inching/Pulse (DP19)
      19: { capability: 'inching_mode', description: 'Inching/Pulse mode', type: 'raw' },
      
      // Sensors (DPs varies by device)
      101: { capability: 'onoff', description: 'Main switch', type: 'bool' },
      102: { capability: 'dim', description: 'Dimmer level', type: 'value' },
      103: { capability: 'onoff.usb2', description: 'USB port 2', type: 'bool' },
      
      // Environmental sensors
      1: { capability: 'measure_temperature', description: 'Temperature', type: 'value', scale: 10 },
      2: { capability: 'measure_humidity', description: 'Humidity', type: 'value', scale: 10 },
      3: { capability: 'measure_luminance', description: 'Illuminance', type: 'value' },
      4: { capability: 'measure_battery', description: 'Battery', type: 'value' },
      5: { capability: 'alarm_motion', description: 'Motion', type: 'bool' },
      
      // Contact sensors
      9: { capability: 'alarm_contact', description: 'Contact', type: 'bool' },
      
      // Alternative temperature/humidity
      18: { capability: 'measure_temperature', description: 'Temperature (alt)', type: 'value', scale: 10 },
      19: { capability: 'measure_humidity', description: 'Humidity (alt)', type: 'value', scale: 10 }
    };

    // Apply mappings
    for (const [dp, config] of Object.entries(standardMappings)) {
      if (this.device.hasCapability(config.capability)) {
        this.dpMapping.set(parseInt(dp), config);
        this.capabilities.set(config.capability, 'tuya');
        this.device.log(`[HYBRID] ✅ DP${dp} → ${config.capability} (${config.type})`);
      }
    }
