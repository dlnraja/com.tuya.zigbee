 ...
 *
 * WHY (P215 / P2269):
 * - Pourquoi: Homey interview often misses EF00 vs ZCL truth; one detector prevents forks.
 * - Comment: sacred zcl_only → cluster truth → HYBRID listen + cascade TX.
 * - Pour qui: Unified* bases + UniversalLayerBootstrap (Homey runtime).
 * - Quand: onNodeInit protocol pick + CapabilityCommandRouter.
 * - Contre quoi: forcing EF00 on BSEED / leftover EF00 TX on IAS-only.
 * TIP HUM: docs/architecture/PROTOCOL_TX_RX_SSOT.md · COMM_PATHFINDING.md
 *
...
function detectIntelligentProtocol(device, zclNode) {
  ...
  const interviewIncomplete = !hasTuyaCluster && !hasZclClusters;
  ...
  let knownEf00Only = !!(device?._ef00OnlyInterview || device?._forcePureTuyaDp);
  try {
    const { isKnownEf00OnlyManufacturer } = require('../zigbee/Ef00OnlyInterview');
    if (isKnownEf00OnlyManufacturer(mfr)) knownEf00Only = true;
  } catch (_) { /* optional */ }
  ...
  // WHY(P2574 / #2244): known EF00-only MCU (ogx8u5z6 / clrdrnya / valves) — never
  // wait for perfect Homey interview; pure DP + parallelDiscover DP/cluster/raw.
  if (knownEf00Only) {
    return {
      ...base,
      protocol: 'TUYA_DP',
      isTuyaDP: true,
      isPureTuyaDP: true,
      listenHybrid: interviewIncomplete || hasZclClusters,
      preferDpTx: true,
      parallelDiscover: true,
      reason: interviewIncomplete
        ? 'ef00_only_mfr_incomplete_interview'
        : 'ef00_only_mfr_forced',
    };
  }
  ...
  // ── 5. Heuristics when interview incomplete / unsupported ──────────────
  // WHY(P2574): Homey may omit EF00 from zclNode even when compose/mfr says MCU —
  // arm HYBRID listen + preferDpTx + parallelDiscover (DP ↔ ZCL ↔ raw).
  if (isTS060x || isTzeFamily) {
    return {
      ...
      parallelDiscover: true,
      reason: interviewIncomplete
        ? (isTS060x ? 'ts060x_incomplete_interview' : 'tze_incomplete_interview')
        : (isTS060x ? 'ts060x_ambiguous_hybrid' : 'tze_ambiguous_hybrid'),
    };
  }
...
function applyIntelligentProtocol(device, zclNode) {
  ...
  // WHY(P2574): do not wipe forcePureTuyaDp / driver pre-arm with a weaker detect.
  const forcedPure = !!(device._ef00OnlyInterview || device._forcePureTuyaDp || device._isPureTuyaDP);
  if (forcedPure && !info.isPureTuyaDP) {
    info.isPureTuyaDP = true;
    info.isTuyaDP = true;
    info.preferDpTx = true;
    info.parallelDiscover = true;
    if (info.protocol !== 'zcl_only') info.protocol = 'TUYA_DP';
    info.reason = `${info.reason || 'detect'}+preserve_forced_pure`;
  }
  device._protocolInfo = info;
  device._isPureTuyaDP = !!(info.isPureTuyaDP || forcedPure);
  device._listenHybrid = !!info.listenHybrid;
  device._preferDpTx = !!info.preferDpTx;
  device._parallelDiscover = !!info.parallelDiscover;
  device._interviewIncomplete = !!info.interviewIncomplete;
  ...
  // P2539 — soft Zigbee 4.0 / Suzi / Green Power radio capability (never invent couples)
  try {
    const { classifyHomeyRadioCapability } = require('../utils/zigbee-tuya-evolution');
    const radio = classifyHomeyRadioCapability({
      mfr: info.mfr || resolveIdentity(device).mfr,
      productId: info.modelId || resolveIdentity(device).modelId,
      zclNode,
    });
    device._radioCapability = radio;
    info.radioCapability = radio;
    if (radio.warnings?.length && typeof device.log === 'function') {
      device.log(`[P2539] ${radio.summary}`);
    }
  } catch (_) { /* evolution optional — never block protocol pick */ }
