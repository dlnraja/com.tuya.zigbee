 ...
 * 
 * Cas d'usage:
 * - USB outlet assignÃ© Ã switch_basic_2gang  dÃ©tecte USB + ajoute voltage/current
 * - Switch assignÃ© Ã USB outlet  dÃ©tecte switch + retire voltage/current
 * - Device avec Tuya EF00 mal dÃ©tectÃ©  ajoute DP capabilities
 * 
...
class HardwareDetectionShim {
  ...
  async calculateCorrections(detection) {
    ...
    const driverId = this.device.driver?.id || 'unknown';this.log(`[SHIM] Current driver: ${driverId}`);
