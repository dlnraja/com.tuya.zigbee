...
class ZigbeeProtocolComplete {
  ...
  static detectProtocol(manufacturerName) {
    ...
    for (const [prefix, info] of Object.entries(this.MANUFACTURERS)) {
      if (prefix.endsWith('_') && manufacturerName.startsWith(prefix)) {
  ...
  static detectZigbeeVersion(manufacturerName) {
    ...
    // Legacy prefixes
    if (manufacturerName.startsWith('_TYZB01_') || manufacturerName.startsWith('_TYZB02_')) {
      return '1.2';
    }
  ...
  static usesTuyaDP(manufacturerName) {
    ...
    return dpPrefixes.some(p => manufacturerName.startsWith(p));
  }
