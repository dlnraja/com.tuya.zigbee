...
class ZigbeeProtocolComplete {
  ...
  static PROPRIETARY_PROTOCOLS = {
    ...
    SINOPE: {
      ...
      cluster: 0xFF01,
      base: 'Zigbee 3.0',
      description: 'SinopÃ© heating/lighting',
    ...
    LIDL: {
      ...
      cluster: CLUSTERS.TUYA_EF00, // Uses Tuya
  ...
  static MANUFACTURERS = {
    ...
    'Sinope Technologies': { brand: 'SinopÃ©', protocol: 'SINOPE', zigbee: '3.0' },
    ...
    'MÃœLLER-LICHT': { brand: 'MÃ¼ller Licht', protocol: 'ZHA', zigbee: '3.0' },
  ...
  static detectProtocol(manufacturerName) {
    ...
    for (const [key, info] of Object.entries(this.MANUFACTURERS)) {
      if (CI.equalsCI(key, manufacturerName)) {
    ...
    for (const [prefix, info] of Object.entries(this.MANUFACTURERS)) {
      if (prefix.endsWith('_') && CI.startsWithCI(manufacturerName, prefix)) {
        return info.protocol;
      }
    }
  ...
  static detectZigbeeVersion(manufacturerName) {
    ...
    for (const [key, info] of Object.entries(this.MANUFACTURERS)) {
      if (CI.equalsCI(key, manufacturerName)) {
        return info.zigbee || '3.0';
      }
    }

    // Legacy prefixes
    if (CI.startsWithCI(manufacturerName, ['_tyzb01_', '_tyzb02_'])) {
      return '1.2';
    }
  ...
  static usesTuyaDP(manufacturerName) {
    ...
    return CI.startsWithCI(manufacturerName, dpPrefixes);
  }
