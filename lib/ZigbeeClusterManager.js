...
class ZigbeeClusterManager {
  ...
  async _enrollIasZone(cluster) {
    try {
      // Write CIE address (Homey's IEEE address) — never zero / never short attr name
      const ieeeAddress = this.device.driver?.homey?.zigbee?.ieeeAddress
        || this.device.homey?.zigbee?.ieeeAddress;
      const hex = ieeeAddress ? String(ieeeAddress).replace(/[:\-0x]/gi, '') : '';
      if (ieeeAddress && hex && !/^0+$/.test(hex)) {
        await cluster.writeAttributes({ iasCIEAddress: ieeeAddress });
        this.log('IAS CIE address written:', ieeeAddress);
      }

      // Send enroll response — zoneId 10 (Homey / Peter #2134 pattern)
  ...
  async registerTuyaCluster(options = {}) {
    ...
    try {
      ...
      const tuyaCluster = zclNode.endpoints[endpoint].clusters['tuya'] ||
  ...
  async registerBattery(options = {}) {
    ...
    try {
      ...
      powerCfg.on('attr.batteryPercentageRemaining', (value) => {
        ...
        this.device.setCapabilityValue('measure_battery', parseFloat(percent)).catch(err => this.error(err));
        this.device.setCapabilityValue('alarm_battery', percent < 20).catch(err => this.error(err));
      });
