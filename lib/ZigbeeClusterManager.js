  ...
  async registerBattery(options = {}) {
    ...
    try {
      ...
      powerCfg.on('attr.batteryPercentageRemaining', (value) => {
        ...
        const hasMeasureBattery = this.device.hasCapability?.('measure_battery') === true;
        const hasAlarmBattery = this.device.hasCapability?.('alarm_battery') === true;
        if (hasMeasureBattery) {
          this.device.setCapabilityValue('measure_battery', parseFloat(percent)).catch(this.error);
        } else if (hasAlarmBattery) {
