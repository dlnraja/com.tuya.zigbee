...
class WallTouchDevice extends BaseHybridDevice {
  ...
  async onNodeInit() {
    this.log('🎨 WallTouchDevice initializing...');
    ...
    this.log(`✅ WallTouch${this.buttonCount}Gang ready`);
    ...
    for (let i = 1; i <= this.buttonCount; i++) {
      ...
      if (endpoint?.clusters?.onOff) {
        endpoint.clusters.onOff.on('attr.onOff', async (value) => {
          this.log(`📥 Button ${i} cluster update: ${value}`);
        ...
        this.log(`✅ Button ${i} registered (SDK3)`);
      } else {
        this.log(`⚠️  Button ${i}: onOff cluster not available`);
      }
    }
  }
  ...
  async initializeTemperature() {
    ...
    this.log('🌡️ Initializing temperature monitoring (SDK3)...');
    
    const endpoint = this.zclNode.endpoints[1];
    if (!endpoint?.clusters?.msTemperatureMeasurement) {
      this.log('⚠️  msTemperatureMeasurement cluster not available');
      return;
    }
    ...
    endpoint.clusters.msTemperatureMeasurement.on('attr.measuredValue', async (value) => {
      ...
      this.log(`🌡️ Temperature: ${temp}°C (raw: ${value})`);
    ...
    this.log('✅ Temperature monitoring ready (SDK3)');
  }
  ...
  async initializeTamper() {
    ...
    this.log('🚨 Initializing tamper detection (SDK3)...');
    
    const endpoint = this.zclNode.endpoints[1];
    if (!endpoint?.clusters?.ssIasZone) {
      this.log('⚠️  ssIasZone cluster not available');
      return;
    }
    ...
    this.log('✅ Tamper detection ready (SDK3)');
  }
