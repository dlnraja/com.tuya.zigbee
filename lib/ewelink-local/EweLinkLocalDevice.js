...
class EweLinkLocalDevice extends Homey.Device {
  async onInit() {
    ...
    if (!this._deviceId) {
      this.setUnavailable('Missing device_id').catch(() => { });
      return;
    }

    this._client = new EweLinkLANClient({
    ...
    if (this._ip) {
      try {
        ...
        this.log('[EWE] Init connect fail:' + e.message);
        this.setUnavailable('Connection failed').catch(() => { });
      }
    } else {
      this.setUnavailable('No IP - configure in settings').catch(() => { });
    }
  ...
  _onDiscoveryIP(dr) {
    if (dr.address && dr.address !== this._ip) {
      ...
      this._ip = dr.address;
      if (this._client) {
        this._client.updateIP(dr.address);
