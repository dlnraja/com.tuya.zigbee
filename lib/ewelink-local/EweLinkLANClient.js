...
const MAX_LAN_RESPONSE_BYTES = 65536;
let _ewelinkLanBreaker;
function getEwelinkLanBreaker() {
  if (!_ewelinkLanBreaker) {
    _ewelinkLanBreaker = createNetworkBreaker('EweLinkLAN', { failureThreshold: 5, resetTimeout: 30000 });
  }
  return _ewelinkLanBreaker;
}

class EweLinkLANClient extends EventEmitter {
  constructor(o){super();this.homey=o.homey||globalThis;this.deviceId=o.deviceId;this.apiKey=o.apiKey;this.ip=o.ip;this.encrypt=o.encrypt!==false;this._connected=false;this._log=o.log||(()=>{});this._lastState={};this._pollTimer=null;this._pollInterval=o.pollInterval||10000;this._destroyed=false;this._seq=Date.now();this._reconnectTimer=null;}
  get connected(){return this._connected;}
  get lastState(){return{...this._lastState};}
  async connect(){if(this._destroyed||!this.ip){return;}if(isMemoryPressure()){this._log('[EWE-LAN] Skip connect — memory pressure');this._scheduleReconnect();return;}if(!getEwelinkLanBreaker().isAvailable){this._log('[EWE-LAN] Skip connect — circuit open');this._scheduleReconnect();return;}this._clearReconnect();try{const r=await getEwelinkLanBreaker().exec(()=>this._request('/zeroconf/info',{}));if(r&&r.error===0){this._connected=true;if(r.data){this._processState(r.data);}this._log(`[EWE-LAN] Connected ${this.ip}`);this.emit('connected');this._startPolling();}else {throw new Error(`err:${r?r.error:'null'}`);}}catch(e){this._connected=false;this._log(`[EWE-LAN] Fail:${e.message}`);this.emit('error',e);this._scheduleReconnect();}}
  ...
  _startPolling(){this._stopPolling();this._pollTimer=this.homey.setInterval(()=>{if(this._connected&&!isMemoryPressure()&&getEwelinkLanBreaker().isAvailable){this._request('/zeroconf/info',{}).then(r=>{if(r&&r.data){this._processState(r.data);}}).catch(()=>{});}},this._pollInterval);}
  _stopPolling(){if(this._pollTimer){clearInterval(this._pollTimer);this._pollTimer=null;}}
  _scheduleReconnect(){if(this._destroyed||this._reconnectTimer){return;}this._reconnectTimer=this.homey.setTimeout(()=>{this._reconnectTimer=null;this.connect();},5000);}
  ...
  _request(path,data){return new Promise((ok,fail)=>{const body=this._buildBody(data);const opts={hostname:this.ip,port:8081,path,method:'POST',headers:{'Content-Type':'application/json','Content-Length':Buffer.byteLength(body)},timeout:5000};const req=http.request(opts,res=>{let d='';let n=0;res.on('data',c=>{n+=c.length;if(n<=MAX_LAN_RESPONSE_BYTES){d+=c;}});res.on('end',()=>{try{const j=JSON.parse(d);if(j.data&&j.encrypt&&this.apiKey&&j.iv){j.data=EweLinkCrypto.decrypt(j.data,this.apiKey,j.iv);}ok(j);}catch(e){fail(e);}});});req.on('error',fail);req.on('timeout',()=>{req.destroy();fail(new Error('timeout'));});req.write(body);req.end();});}
