...
class SmartCapability {
  ...
  update(device, source, value, confidence = 1.0) {
    const decision = this.validator.recordAndCommit(source, value, Date.now(), confidence);
    if (decision.commit && device) {
      try {
        if (typeof device.safeSetCapabilityValue === 'function') {
          device.safeSetCapabilityValue(this.capability, decision.value, { source, confidence });
