...
/**
 * @deprecated v5.9.24 - OBSOLETE: Not imported anywhere. Driver matching handled by Homey SDK.
...
class DynamicDriverMatcher {
  ...
  async analyze() {
    ...
    const mfr = this.device.getSetting?.('zb_manufacturer_name') || '';// Score drivers by cluster match
    const scores = {
      presence_sensor_radar: clusters.has(CLUSTERS.TUYA_EF00) && mfr.match(/^_TZE/) ? 20 : 0,
  ...
  async _getClusters() {
    const set = new Set();
    const eps = this.device.zclNode?.endpoints || {};for (const ep of Object.values(eps)) {
