...
const V1_SMOKE_DATA_POINTS = {
    ...
    temperature: 23, // value | temperature x10 (upstream PR #1237, _TZE284_gyzlwu5q)
    humidity: 24 // value | humidity % (upstream PR #1237)
};

/**
 * Split a Tuya DP frame that packs SEVERAL datapoints into one report.
 * Some modules (e.g. Dooya DC1545R curtain modules, upstream PR #1431)
 * append extra DP records after the first declared payload; the standard
 * single-DP handlers silently drop them (position feedback never updates).
 * @param {Object} frame - { dp, datatype, length, data } as emitted by the cluster
 * @returns {Array<{dp:number, datatype:number, data:Buffer}>} one entry per DP
 */
function parseTuyaMultiDpFrame(frame) {
    const out = [];
    if (!frame || !Buffer.isBuffer(frame.data)) { return out; }
    const firstLen = typeof frame.length === 'number' ? frame.length : frame.data.length;
...
const V1_SINGLE_GANG_DIMMER_SWITCH_DATA_POINTS = {
    ...
    mode: 5, // Enum | white, color, scene, music (some 1-gang MCU use DP5 as max brightness)
    countdown: 6, // Integer | 0-4320 seconds
    powerOnBehavior: 14, // Enum | Off, On, Memory
    backlightMode: 21 // Enum | off / normal / inverted
};
