...
class IASZoneManager {
  ...
  async enrollIASZone() {
    ...
    try {
      ...
      try {
        const zoneState = await iasZone.Promise.resolve(readAttributes(['zoneState'])).catch(() => null);
  ...
  async _getIEEEAddress() {
    ...
    // Method 2: Homey Zigbee API (SDK3)
    try {
      const ieee = await device.homey.zigbee.getIeeeAddress(device.getData().id);
      if (ieee) {
        device.log('[IAS] IEEE from homey.zigbee.getIeeeAddress');
        return ieee;
      }
    } catch (err) {
      device.log('[IAS] homey.zigbee.getIeeeAddress failed:', err.message);
    }
    ...
    try {
      ...
      if (endpoint?.clusters?.basic) {
        const attrs = await endpoint.clusters.basic.Promise.resolve(readAttributes(['ieeeAddress'])).catch(() => null);
    ...
    try {
      ...
      if (status.alarm1) {
          ...
          for (const triggerId of triggerIds) {
            try {
              ...
              if (trigger) {
                ...
                trigger.Promise.resolve(trigger(device, {}, {})).catch(err => 
        ...
        if (device.hasCapability('alarm_generic')) {
          device.Promise.resolve(setCapabilityValue('alarm_generic', true)).catch(() => {});
          // Reset after 1s
          setTimeout(() => {
            device.Promise.resolve(setCapabilityValue('alarm_generic', false)).catch(() => {});
          }, 1000);
        }
      }
      ...
      if (status.alarm1 && device.hasCapability('alarm_motion')) {
        ...
        device.Promise.resolve(setCapabilityValue('alarm_motion', true)).catch(() => {});
        
        // Auto-reset after timeout
        const timeout = device.getSetting?.('motion_timeout') || 30;
        setTimeout(() => {
          device.Promise.resolve(setCapabilityValue('alarm_motion', false)).catch(() => {});
        }, timeout * 1000);
      }
      ...
      if (status.alarm1 && device.hasCapability('alarm_contact')) {
        ...
        device.Promise.resolve(setCapabilityValue('alarm_contact', true)).catch(() => {});
      } else if (!status.alarm1 && device.hasCapability('alarm_contact')) {
        device.Promise.resolve(setCapabilityValue('alarm_contact', false)).catch(() => {});
      }

      // Tamper alarm
      if (status.tamper && device.hasCapability('alarm_tamper')) {
        device.log('[IAS] ⚠️ TAMPER!');
        device.Promise.resolve(setCapabilityValue('alarm_tamper', true)).catch(() => {});
      }
      ...
      if (status.batteryLow) {
        ...
        if (device.hasCapability('alarm_battery')) {
          device.Promise.resolve(setCapabilityValue('alarm_battery', true)).catch(() => {});
        }
      }
