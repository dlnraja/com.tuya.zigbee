  ...
  static createHybridDevice(baseClass = ZigBeeDevice) {
    return class HybridDevice extends baseClass {
      ...
      async syncCapabilities(targetCapabilities) {
        ...
        // Remove extra (only if not in target)
      ...
      async setupMotionSensor() {
        ...
        this.registerAttrReportListener('occupancySensing', 'occupancy', async (value) => {
          await this.setCapabilityValue('alarm_motion', value.occupied);
        }, 1);
      }

      async setupContactSensor() {
        this.log('🚪 Setting up contact sensor...');
        this.registerAttrReportListener('iasZone', 'zoneStatus', async (value) => {
          await this.setCapabilityValue('alarm_contact', value.alarm1);
        }, 1);
      }

      async setupClimateSensor() {
        this.log('🌡️ Setting up climate sensor...');
        this.registerAttrReportListener('temperatureMeasurement', 'measuredValue', async (value) => {
          await this.setCapabilityValue('measure_temperature', value / 100);
        }, 1);

        this.registerAttrReportListener('relativeHumidity', 'measuredValue', async (value) => {
          await this.setCapabilityValue('measure_humidity', value / 100);
        }, 1);
      }
