        ...
        try {
          ...
          if (tempCluster) {
            tempCluster.on('attr.measuredValue', async (value) => {
              if (value != null) {
                await this.setCapabilityValue('measure_temperature',safeParse(parseFloat(value), 100)).catch(() => { });
              }
            });
          }
          ...
          if (humidityCluster) {
            humidityCluster.on('attr.measuredValue', async (value) => {
              if (value != null) {
                await this.setCapabilityValue('measure_humidity',safeParse(parseFloat(value), 100)).catch(() => { });
              }
            });
          }
        } catch (err) {
