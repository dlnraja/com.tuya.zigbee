 ...
 * - Smart energy management (AC vs Battery vs)
...
class DriverSystem {
  ...
  static DEVICE_PATTERNS = {
    ...
    ILLUMINANCE: {
      clusters: [0x0400],
      class: 'sensor',
      capabilities: ['measure_luminance', 'measure_battery'],
      energy: 'BATTERY'
    },
    ...
    SOIL_MOISTURE: {
      clusters: [0x0408],
      class: 'sensor',
      capabilities: ['measure_soil_moisture', 'measure_battery'],
      energy: 'BATTERY'
    },

    MULTI_SENSOR: {
      clusters: [0x0402, 0x0405, 0x0400], // Temp + Hum + Lum
      class: 'sensor',
  ...
  static createHybridDevice(baseClass = ZigBeeDevice) {
    return class Device extends baseClass {

      async onNodeInit({ zclNode }) {
        // Antigravity Logic-Lens Boost: Idempotent initialization guard with atomic-style check
        if (this._hybridInitInProgress) {return;}
        if (this.SystemInitialized) {
          this.log('[HYBRID] ⚠️ Already initialized, skipping second init');
          return;
        }
        this._hybridInitInProgress = true;

        this.log('🚀 Driver System initializing...');
        ...
        if (this._skipHybridTypeDetection) {
          ...
          // Normal detection
          this.deviceType =DriverSystem.detectDeviceType(zclNode);
        }
        this.log('📱 Detected type:', this.deviceType?.type || 'UNKNOWN');

        // Detect energy source
        this.energySource =DriverSystem.detectEnergySource(zclNode, this.deviceType);
        this.log('⚡ Energy source:', this.energySource);

        // Get energy strategy
        this.energyStrategy =DriverSystem.getEnergyStrategy(this.energySource, this.deviceType);
        this.log('🔋 Energy strategy:', `${this.energyStrategy.polling.default  }ms polling`);

        // Build capabilities
        const tuyaDpMap = this.getTuyaDpMap?.() || null;
        const targetCapabilities =DriverSystem.buildCapabilities(
          this.deviceType,
          this.energySource,
          tuyaDpMap
        );
        ...
        this._hybridInitInProgress = false;
      ...
      async setupClimateSensor() {
        ...
        try {
          ...
          const luminanceCluster = this.zclNode?.endpoints?.[1]?.clusters?.illuminanceMeasurement;
          if (luminanceCluster) {
            luminanceCluster.on('attr.measuredValue', async (value) => {
              if (value != null) {
                await this.setCapabilityValue('measure_luminance', parseFloat(value)).catch(e => this.error('[HYBRID] Luminance error:', e.message));
              }
            });
          }
        } catch (err) {
