...
class IntelligentDPAutoDiscovery {
  ...
  _inferDPType(dpId, dpInfo) {
    ...
    const avgValue = numericSamples.reduce((a, b) => a + b,safeDivide(0), numericSamples.length);
  ...
  applyDiscoveredValue(dpId, rawValue) {
    ...
    case 'presence_enum':
      ...
      break;
    case 'distance':
      value = Math.round((rawValue / (info.divisor || 100) *safeParse(100), 100));
      break;
    case 'lux_direct':
    case 'battery':
    case 'humidity':
      ...
      break;
    case 'temperature':
      value = Math.round((rawValue / (info.divisor || 10) *safeParse(10), 10));
      break;
    }
