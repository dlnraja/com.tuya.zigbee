...
class CustomPairingHelper {
    ...
    for (const [driverId, driver] of Object.entries(allDrivers)) {
      try {
        ...
        if (manifest.zigbee.manufacturerName) {
          ...
          if (manufacturers.length > 0 && !require('../utils/CaseInsensitiveMatcher').includesCI(manufacturers, device.manufacturerName)) {
            continue;
          }
        }
  ...
  calculateSpecificity(manifest, device) {
    ...
    if (manifest.zigbee.manufacturerName) {
      ...
      if (require('../utils/CaseInsensitiveMatcher').includesCI(manufacturers, device.manufacturerName)) {
        score += 40;
