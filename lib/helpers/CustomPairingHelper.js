...
class CustomPairingHelper {
  ...
  async findMatchingDrivers(device) {
    ...
    for (const [driverId, driver] of Object.entries(allDrivers)) {
      try {
        ...
        if (!CI.includesCI(productIds, device.productId)) continue;
        ...
        if (manifest.zigbee.manufacturerName) {
          ...
          if (manufacturers.length > 0 && !CI.includesCI(manufacturers, device.manufacturerName)) {
            continue;
          }
        }
  ...
  calculateSpecificity(manifest, device) {
    ...
    if (manifest.zigbee.manufacturerName) {
      ...
      if (CI.includesCI(manufacturers, device.manufacturerName)) {
        score += 40;
    ...
    if (manifest.zigbee.modelId && CI.equalsCI(manifest.zigbee.modelId, device.modelId)) {
      score += 30;
    }
  ...
  getDriverDescription(manifest) {
    ...
    const endpoints = Object.keys(manifest.zigbee?.endpoints || {}).length;// Battery powered
