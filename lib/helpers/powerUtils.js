...
module.exports = {
  ...
  async removeBatteryFromACDevices(device) {
    ...
    if (powerSource === 'mains' || powerSource === 'usb' || powerSource === 'dc') {
      ...
      // v5.8.72: User request - "Manage all energy types dynamically even if it switches back to mains"
      // Hybrid devices (Siren with battery backup, Solar valves) need to retain measure_battery!
      // We ONLY remove alarm_battery to prevent the "Double Battery Capability" SDK3 violation, 
      // but we keep measure_battery alive.
      
      // Remove alarm_battery
      if (device.hasCapability('alarm_battery')) {
        if (device.log) {device.log('[PowerUtils] Removing alarm_battery from AC device (keeping measure_battery for hybrid support)');}
