...
module.exports = {
  ...
  async removeBatteryFromACDevices(device) {
    ...
    // HYBRID APP: We do NOT remove battery capabilities for AC devices anymore,
    // because all devices are treated as Hybrid (AC + Battery Backup) per user request.
    if (device.log) {
      device.log('[PowerUtils] 🔌 HYBRID MODE: Keeping battery capabilities even if AC powered');
    }
    return false;
  },
