...
const { countOnOffGangs, classifyInterview } = require('../utils/interviewEndpoints');
...
class UnknownDeviceHandler {
  ...
  async analyzeDevice(data) {
    ...
    // P2218: soft hypothesis when mfr present but pid/model missing — never hard-lock
    if (manufacturerName && !modelId) {
      try {
        const { resolveUnknownCouple } = require('../enrichment/HeuristicUnknownResolver');
        ...
        if (ranked.softHypothesis || ranked.preferred) {
          const soft = ranked.preferred || ranked.softHypothesis;
          analysis.softHypothesis = soft;
          analysis.suggestedDriver = soft.driver || analysis.suggestedDriver;
          analysis.confidence = Math.max(analysis.confidence, Math.min(soft.confidence || 0, 70));
          analysis.matchedPatterns.push(`p2218:${ranked.tier}`);
          analysis.protocolHint = ranked.protocolHint;
        }
      } catch (_e) { /* heuristic optional */ }
    }
    ...
    // 4. Analyze by application OnOff endpoints (never Green Power 242)
    if (endpoints) {
      const classified = classifyInterview({ modelId, endpoints });
      analysis.gangCount = classified.gangs || countOnOffGangs({ endpoints });
      if (classified.class === 'button' || classified.class === 'sos') {
        analysis.deviceType = classified.class === 'sos' ? 'button' : 'button';
        analysis.confidence += 20;
        analysis.matchedPatterns.push(`interview:${classified.reason}`);
      } else if (analysis.deviceType === 'switch' && analysis.gangCount > 1) {
        analysis.matchedPatterns.push(`endpoints:${analysis.gangCount}_gang`);
      }
    }
  ...
  suggestDriver(analysis) {
    ...
    switch (deviceType) {
    case 'switch':
      driver = analysis.gangCount > 1
        ? `switch_${analysis.gangCount}gang`
        : 'switch_1gang';
      break;
    case 'button':
      driver = analysis.gangCount >= 4 ? 'button_wireless_4'
        : analysis.gangCount === 3 ? 'button_wireless_3'
          : analysis.gangCount === 2 ? 'button_wireless_2'
            : 'button_wireless_1';
      break;
