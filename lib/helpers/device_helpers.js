...
function getDeviceOverride(modelId, manufacturer) {
  ...
  if (manufacturer) {
    const { startsWithCI, equalsCI } = require('../utils/CaseInsensitiveMatcher');
    for (const [key, config] of Object.entries(overrides)) {
      if (config.manufacturerPattern && startsWithCI(manufacturer, config.manufacturerPattern)) {
        if (!config.modelId || equalsCI(config.modelId, modelId)) {
...
function isTuyaDP(deviceInfo, device = null) {
  // v2026-07-12: Re-export from lib/utils/tuya-dp-detector.js to break circular dependency.
  // Original function lived here, but was extracted to lib/utils/tuya-dp-detector.js
  // because both this file and lib/utils/safe-auto-migrate.js needed it (forming a cycle).
  return require('../utils/tuya-dp-detector').isTuyaDP(deviceInfo, device);
}
