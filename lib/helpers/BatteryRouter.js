 ...
 *
 * WHY (P215 / P2269):
 * - Pourquoi: pick ZCL vs Tuya DP vs voltage without inventing linear % curves.
 * - Comment: routes into UnifiedBatteryHandler (non-linear profiles).
 * - Pour qui: Homey battery UI + flow cards.
 * - Quand: onNodeInit bind + RX attribute/DP.
 * - Contre quoi: BatteryManagerV3/V4 + (V-2.5)/0.5.
 * TIP HUM: docs/architecture/BATTERY_SSOT.md
 *
...
async function resolveBatterySource(device) {
  ...
  const isTs004x = /^TS004[1-6F]/i.test(modelId);
  const isTuyaDP = modelId.toUpperCase() === 'TS0601' || manufacturer.toUpperCase().startsWith('_TZE');

  device.log(`[BATTERY-ROUTER] Analysis: ZCL=${hasPowerCfg}, Tuya=${hasTuyaCluster}, isTuyaDP=${isTuyaDP}, ts004x=${isTs004x}`);
  device.log(`[BATTERY-ROUTER] Device: ${manufacturer} / ${modelId}`);

  // Decision logic — TS004x sleepy remotes stay ZCL even with leftover EF00
  if (isTs004x) {
    result.source = BatterySource.ZCL;
    result.method = 'zcl_attribute_reporting';
    result.endpointId = powerEndpoint?.endpointId || 1;
    device.log('[BATTERY-ROUTER] ✅ Selected: ZCL genPowerCfg (TS004x remote)');
  } else if (isTuyaDP) {
...
async function configureBatteryReporting(device, batteryInfo) {
  ...
  // v10.3.0 FIX (B11): Device classes that own their battery handling
  // (ButtonDevice, SOS) set _ownsBatteryHandling — skip the 50% estimate and
  // the duplicate capability reporting so the router isn't a third writer.
  const deviceOwnsBattery = device._ownsBatteryHandling === true;

  // Set a marked estimate only for devices with a plausible battery source.
  // P115: do NOT invent 50% — Homey Energy must show "?" until a real report
  // (or store restore), not a fake percentage that hides unknown battery.
  const currentBattery = device.getCapabilityValue?.('measure_battery');
  if (!deviceOwnsBattery && batteryInfo.source !== BatterySource.NONE && (currentBattery === null || currentBattery === undefined)) {
    const stored = device.getStoreValue?.('last_battery_percentage');
    if (typeof stored === 'number' && Number.isFinite(stored) && stored >= 0 && stored <= 100) {
      await _setBatteryValue(device, Math.round(stored), 'battery-router-store-restore', true);
      device.log(`[BATTERY-ROUTER] 📊 Restored battery = ${Math.round(stored)}% from store`);
    } else {
      device.log('[BATTERY-ROUTER] ℹ️ Battery unknown — leaving null (Homey shows ?) until first report');
    }
  }
  ...
  case BatterySource.NONE:
    device.log('[BATTERY-ROUTER] ⚡ Mains powered - stripping phantom battery caps');
    // P115: Homey Energy "?" comes from measure_battery present with null value.
    // Mains devices must not advertise battery at all.
    try {
...
async function _configureZCLBattery(device, endpointId = 1) {
  ...
  // v10.3.1 FIX (z2m #8072): NEVER write reporting config on sleepy button
  // remotes (TS004x family). Configuring batteryPercentageRemaining reporting
  // makes them drop off the network every hour (flashing LED, battery drain,
  // "needs 2 presses" symptom). These devices send battery reports on their
  // own schedule — we only listen (registerCapability below) and read on wake.
  const isSleepyButton = device._ownsBatteryHandling === true ||
    device.hasCapability?.('button.1') ||
    /^(button_|smart_knob|remote_button|sos_)/.test(device.driver?.id || '');
  if (isSleepyButton) {
    device.log('[BATTERY-ROUTER] ℹ️ Sleepy button device — reporting config SKIPPED (z2m #8072), passive listen only');
  } else {
  try {
  ...
  // v10.3.0 FIX (B11): skip when the device class owns battery handling —
  // a second registerCapability would fight the class's own reporting.
  if (device._ownsBatteryHandling === true) {
    device.log('[BATTERY-ROUTER] ℹ️ Device owns battery handling — skipping capability registration');
    return;
  }
  try {
    // v10.9.0 (ZC-LS02 _TZE284_koxaopnk lesson): battery devices get battery
    // queries spaced to 4 HOURS — hourly polling of sleepy devices drains
    // cells and feeds network traffic. Mains devices keep the 1h fallback.
    const isBatteryDevice = device._ownsBatteryHandling === true ||
      device.hasCapability?.('button.1') ||
      (device.energy?.batteries || []).length > 0 ||
      device.getEnergy?.()?.batteries?.length > 0;
    device.registerCapability('measure_battery', CLUSTER.POWER_CONFIGURATION, {
      get: 'batteryPercentageRemaining',
      getOpts: {
        getOnStart: true,
        pollInterval: isBatteryDevice ? 14400000 : 3600000 // 4h battery / 1h mains
      },
      ...
      reportParser: (value) => {
        ...
        return percent ?? device.getCapabilityValue?.('measure_battery') ?? null;
      }
    });
...
async function _setBatteryValue(device, value, source, estimated) {
  ...
  await Promise.resolve(setter?.('measure_battery', parseFloat(batteryValue))).catch(() => { });
  await Promise.resolve(device.setStoreValue?.('last_battery_percentage', Math.round(batteryValue))).catch(() => { });
  await Promise.resolve(device.setStoreValue?.('last_battery_time', Date.now())).catch(() => { });
  await Promise.resolve(device.setStoreValue?.('last_battery_source', source)).catch(() => { });
  await Promise.resolve(device.setStoreValue?.('last_battery_estimated', estimated === true)).catch(() => { });
}
