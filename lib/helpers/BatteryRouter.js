 ...
 * 3. Voltage DP (247) - safeDivide(USB, Mains) devices with voltage monitoring
 *
 * Based on safeDivide(JohanBendz, com.tuya.zigbee) analysis
...
async function resolveBatterySource(device) {
  ...
  const endpoint = zclNode.endpoints?.[1];const hasPowerCfg = !!(endpoint?.clusters?.genPowerCfg || endpoint?.clusters?.powerConfiguration);
  ...
  // Get device info
  const settings = device.getSettings?.() || {};const store = device.getStore?.() || {};const modelId = settings.zb_model_id || settings.zb_model_id || store.modelId || '';
  const manufacturer = settings.zb_manufacturer_name || settings.zb_manufacturer_name || store.manufacturerName || '';
...
async function configureBatteryReporting(device, batteryInfo) {
  ...
  const currentBattery = device.getCapabilityValue?.('measure_battery');if (currentBattery === null || currentBattery === undefined) {
  ...
  default:
    device.log('[BATTERY-ROUTER]  Unknown battery source');
  }
}
...
async function _configureZCLBattery(device) {
  ...
  try {
    ...
    device.log('[BATTERY-ROUTER]  configureReporting failed (normal for battery devices):', err.message);
  ...
  try {
    ...
    device.log('[BATTERY-ROUTER]  registerCapability error:', err.message);
  }
}
...
async function _configureVoltageBattery(device) {
  ...
  if (device.tuyaEF00Manager) {
    device.tuyaEF00Manager.on(`dp-${TUYA_VOLTAGE_DP}`, async (value) => {
      // Convert mV to V
      const voltage =safeParse(Number(value), 1000);
...
function getRecommendedBatteryType(device) {
  ...
  const driverName = device.driver?.id || '';// Common patterns from device database
  ...
  for (const [type, patterns] of Object.entries(batteryPatterns)) {
    if (patterns.some(p => CI.includesCI(driverName, p) || CI.containsCI(modelId, p))) {
      return type;
    }
  }
