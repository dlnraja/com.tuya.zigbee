...
const DRIVER_MAPPING_PATH = path.join(ROOT, 'data', 'driver-mapping-database.json');
...
let _fingerprintMatcher = undefined;
...
function getFingerprintMatcher() {
  if (_fingerprintMatcher !== undefined) {return _fingerprintMatcher;}
  // P92: TUYA_FP_HEURISTIC=0 disables heuristic matching (exact/CI only).
  if (process.env.TUYA_FP_HEURISTIC === '0') {
    _fingerprintMatcher = null;
    return null;
  }
  _fingerprintMatcher = safeRequire('../utils/fingerprint-matcher') || null;
  return _fingerprintMatcher;
}

// P92: one-time degraded-mode warning. scripts/prepare-publish.js strips
// data/mfs_db.json from the published payload unless HOMEY_INCLUDE_MFS_DB=1,
// so on Homey the mfs evidence source is expected to be absent: the matcher
// then relies on fingerprints.json (bundled) and other sources only.
let _mfsAbsenceLogged = false;
function logMfsAbsence(homeyLog) {
  if (_mfsAbsenceLogged) {return;}
  _mfsAbsenceLogged = true;
  const sink = typeof homeyLog === 'function' ? homeyLog
    : (typeof console !== 'undefined' && console.log ? console.log.bind(console) : null);
  if (sink) {
    sink('[ProbabilisticDeviceDetector] data/mfs_db.json absent - mfs_db evidence source disabled ' +
      '(expected in published payload unless HOMEY_INCLUDE_MFS_DB=1); heuristic matching continues on bundled fingerprints');
  }
}
...
class ProbabilisticDeviceDetector {
  ...
  _addMfsEvidence() {
    ...
    if (!db || (!db.devices && Object.keys(db).length === 0)) {
      // Graceful degradation: mfs_db.json is stripped from the published
      // payload — skip this source, other evidence sources still apply.
      logMfsAbsence(this.homey?.log?.bind(this.homey));
      this._addSourceSummary('mfs_db', 'unavailable');
      return;
    }
    const key = normalizeText(this.data.manufacturerName);
    let entry = this._lookupCaseInsensitive(db.devices, key) || this._lookupCaseInsensitive(db, this.data.manufacturerName);
    let matchType = 'exact';
    let matchScore = 1;

    // P92: heuristic fallback (interchangeable TZE prefixes, fuzzy suffix)
    // when the exact / case-insensitive lookup missed.
    if (!entry?.driverId && !entry?.driver && !entry?.driverHint) {
      const matcher = getFingerprintMatcher();
      const match = matcher && db.devices
        ? matcher.matchFingerprint(this.data.manufacturerName, this.data.modelId, db.devices)
        : null;
      if (match?.entry) {
        entry = match.entry;
    ...
    // mfs_db entries expose the driver as `driverHint` (older rows may use
    // driverId/driver) — accept all three.
    // WHY(P2251): multiCouple brands (HOBEIAN) resolve via byPid — never brand→soil only
    let driver = null;
    const pidKey = String(this.data.modelId || '').trim();
    if (entry?.byPid && typeof entry.byPid === 'object' && pidKey) {
      const direct = entry.byPid[pidKey];
      if (direct) {
        driver = direct;
      } else {
        const hit = Object.keys(entry.byPid).find((k) => normalizeText(k) === normalizeText(pidKey));
        if (hit) driver = entry.byPid[hit];
      }
    }
    if (!driver) {
      driver = entry?.driverId || entry?.driver || entry?.driverHint;
    }
    // multiCouple without matching pid → miss (do not steal climate onto soil)
    if (entry?.multiCouple && pidKey && entry.byPid && !driver) {
      this._addSourceSummary('mfs_db', 'miss', { reason: 'multiCouple_pid_unknown' });
      return;
    }
    if (entry?.multiCouple && pidKey && entry.byPid) {
      const listed = Array.isArray(entry.modelIds)
        && entry.modelIds.some((m) => normalizeText(m) === normalizeText(pidKey));
      if (!listed && !(entry.byPid[pidKey] || Object.keys(entry.byPid).some((k) => normalizeText(k) === normalizeText(pidKey)))) {
        this._addSourceSummary('mfs_db', 'miss', { reason: 'multiCouple_pid_not_listed' });
        return;
      }
    }
    if (!driver) {
      // P92: mfr totally unknown -> pid-only suggestion (most frequent
      // driverHint among mfs_db devices carrying this modelId).
      const matcher = getFingerprintMatcher();
      const suggestion = matcher && db.devices
        ? matcher.suggestDriverFromPid(this.data.modelId, db.devices)
        : null;
      if (suggestion?.driverHint) {
        this._vote(suggestion.driverHint, 14 + Math.min(6, suggestion.count), 'mfs_db', `pid_only:${suggestion.modelId}`, {
          deviceTypes: [],
        });
        this._addSourceSummary('mfs_db', 'hit', {
          driver: suggestion.driverHint,
          matchType: 'pid_only',
          supportingEntries: suggestion.count,
        });
        return;
      }
    ...
    const base = matchType === 'exact'
      ? 28 + Math.min(12, sourceCount * 3)
      : Math.round((28 + Math.min(12, sourceCount * 3)) * matchScore * 0.8);
    this._vote(driver, base, 'mfs_db', `sources:${sourceCount}`, {
      capabilities: entry.capabilities,
      deviceTypes: [entry.deviceType, entry.type],
    });
    this._addSourceSummary('mfs_db', 'hit', { driver, sourceCount, matchType, matchScore });
  }
