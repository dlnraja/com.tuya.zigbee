...
const DEVICE_HINTS = {
  ...
  '_TZE200_3towulqd': {
    ...
    protocol: 'zcl_or_dp',
    productId: 'TS0601',
    description: 'ZG-204ZL/ZV — some interviews have no 0xEF00 (IAS+lux only)',
    capabilities: ['alarm_motion', 'measure_luminance', 'measure_battery']
  },
  '_TZE200_ppuj1vem': {
  ...
  '_TZE204_sxm7l9xa': {
    ...
    capabilities: ['alarm_motion', 'measure_luminance', 'measure_luminance.distance']
  },
  '_TZE200_ikvncluo': {
  ...
  '_TZ3000_a7ouggvs': {
    ...
    protocol: 'zigbee_standard',
    productId: 'TS0043',
    description: 'Zemismart ZB-L03C-H 3-button wireless scene switch (CR2032)',
    capabilities: ['measure_battery', 'button.1', 'button.2', 'button.3'],
    endpoints: 3
  },
  '_TZ3000_qzjcsmar': {
    driverId: 'button_wireless_3',
  ...
  '_TZ3000_blhvsaqf': {
    ...
    protocol: 'zcl_only',
    productId: 'TS0001',
    description: 'BSEED 1-gang ZCL (GitHub #104/#1339)',
    capabilities: ['onoff'],
    endpoints: [1]
  },
  '_TZ3000_ysdv91bk': {
    ...
    protocol: 'zcl_only',
    productId: 'TS0001',
    description: 'BSEED 1-gang ZCL (GitHub #105)',
    capabilities: ['onoff']
  },
  '_TZ3000_l9brjwau': {
    ...
    protocol: 'zcl_only',
    productId: 'TS0002',
    description: 'BSEED 2-gang ZCL (GitHub #103)',
    capabilities: ['onoff', 'onoff.gang2'],
    endpoints: [1, 2]
  },
  '_TZ3000_qkixdnon': {
    driverId: 'switch_3gang',
    deviceType: 'switch',
    protocol: 'zcl_only',
    productId: 'TS0003',
    description: 'BSEED 3-gang ZCL (GitHub #102)',
...
function findBestDriver(deviceInfo) {
  ...
  for (const [mfr, hint] of Object.entries(DEVICE_HINTS)) {
    if (manufacturer && manufacturer.toUpperCase().startsWith(mfr.slice(0, -3).toUpperCase())) {
      return {
        driverId: hint.driverId,
        confidence: 0.80,
  ...
  if (modelId) {
    ...
    for (const [mfr, hint] of Object.entries(DEVICE_HINTS)) {
      if (hint.productId && TU.normalize(hint.productId) === modelLower) {
        return {
          driverId: hint.driverId,
          confidence: 0.70,
          source: 'productId',
