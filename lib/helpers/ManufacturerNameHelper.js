...
/** Check if value is valid and cleanup garbage characters */
function _v(x) { 
  if (!x || typeof x !== 'string') return false;
  // v7.4.5+: Aggressive cleanup: remove NULL bytes (\0), control characters (\x00-\x1F, \x7F-\x9F), and whitespace
  const clean = x.replace(/[\x00-\x1F\x7F-\x9F]/g, '').trim(); 
  // Reject common garbage/empty strings
  return clean.length > 0 && !/^(unknown|none|null|undefined|false|true|nan|0|\[object.*\])$/i.test(clean); 
}
...
function _g(f) { 
  try { 
    ...
    const clean = r.replace(/[\x00-\x1F\x7F-\x9F]/g, '').trim();
    // Use _v for final validation to catch "unknown" etc.
    return _v(clean) ? clean  :  null;
  } catch(e) { 
    return null; 
  } 
}
...
function getManufacturerName(device) {
  ...
  return _g(() => device.getSetting?.('zb_manufacturer_name'))
      || _g(() => device.getSetting?.('zb_manufacturer_name'))
...
function getModelId(device) {
  ...
  return _g(() => device.getSetting?.('zb_model_id'))
      || _g(() => device.getSetting?.('zb_model_id'))
...
async function ensureManufacturerSettings(device) {
  ...
  try {
    ...
    const settings = device.getSettings?.() || {};const updates = {};
    ...
    device.log?.(`[MFR-ENSURE]  ${e.message}`);
  }
}
...
function debugManufacturerSources(device) {
  ...
  device.log(`[MFR-DEBUG] this.getStoreValue('manufacturerName'): ${device.getStoreValue?.('manufacturerName') || '(empty)'}`);
