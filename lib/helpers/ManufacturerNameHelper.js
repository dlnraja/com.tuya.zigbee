 ...
 */
async function ensureManufacturerSettings(device, zclNode = null) {
  if (!device) {return;}
  try {
    // WHY (f647d35b): callers may pass zclNode before device.zclNode is assigned
    if (zclNode && !device.zclNode) {device.zclNode = zclNode;}

    let mfr = getManufacturerName(device);
    let model = getModelId(device);

    // Homey pairing data often has the sacred couple even when settings are blank
    if (!mfr || !model) {
      const data = device.getData?.() || {};
      mfr = mfr || (_v(data.manufacturerName) ? data.manufacturerName.trim() : '');
      model = model || (_v(data.productId) ? data.productId.trim()
        : (_v(data.modelId) ? data.modelId.trim() : ''));
    }
    ...
    // Dynamic brand/model canonical normalization at onLoad/onInit time (case-insensitive)
    if (_v(mfr)) {
      const mfrLower = mfr.toLowerCase();
      if (mfrLower === 'hobeian') {
        mfr = 'HOBEIAN';
      } else if (mfrLower === 'sonoff') {
        mfr = 'SONOFF';
      } else if (mfrLower === 'ewelink') {
        mfr = 'eWeLink';
      } else if (mfrLower === 'moes') {
        mfr = 'Moes';
      } else if (mfrLower === 'bseed') {
        mfr = 'BSEED';
      } else if (mfrLower === 'lumi' || mfrLower === 'lumi.') {
        mfr = 'LUMI';
      } else if (mfrLower === 'xiaomi') {
        mfr = 'Xiaomi';
      } else if (mfrLower === 'aqara') {
        mfr = 'Aqara';
      } else if (mfrLower === 'philips' || mfrLower === 'signify netherlands b.v.') {
        mfr = 'Philips';
      } else if (mfrLower === 'ikea' || mfrLower === 'ikea of sweden') {
        mfr = 'IKEA of Sweden';
      } else if (mfrLower === 'heiman') {
        mfr = 'HEIMAN';
      } else if (mfrLower === 'lidl' || mfrLower === 'silvercrest') {
        mfr = 'Lidl';
      } else if (mfrLower === 'innr') {
        mfr = 'Innr';
      } else if (mfrLower === 'gledopto') {
        mfr = 'GLEDOPTO';
      } else if (mfrLower === 'legrand' || mfrLower === 'netatmo') {
        mfr = 'Legrand';
      } else if (mfrLower === 'schneider' || mfrLower === 'schneider electric' || mfrLower === 'wiser') {
        mfr = 'Schneider Electric';
      } else if (mfrLower === 'tuya') {
        mfr = 'Tuya';
      }
    }
    ...
    if (_v(model)) {
      ...
      // Normalize Hobeian and standard model variants to standard upper case
      if (modelUpper.startsWith('ZG-') || modelUpper.startsWith('TS')) {
        model = modelUpper;
      } else if (modelUpper === 'HOBEIAN') {
        // WHY(P2434): some interviews put brand string in modelId/productId
        model = 'HOBEIAN';
        if (!_v(mfr) || String(mfr).toLowerCase() === 'hobeian') {
          mfr = 'HOBEIAN';
        }
      }
    }

    // WHY(P2461 / Peter #2230 diag 048cff91): mfr known (mrpevh8p) but pid=ABSENT on wake.
    // Profile locks productId=TS0041 — soft-fill only for verified SH-SC07 siblings.
    if (!_v(model) && _v(mfr) && /mrpevh8p|5bpeda8u|b4awzgct/i.test(mfr)) {
      model = 'TS0041';
      device.log?.(`[MFR-ENSURE] P2461 soft-filled zb_model_id=TS0041 for ${mfr}`);
    } else if (!_v(model) && typeof device.getDeviceProfile === 'function') {
      try {
        const pid = device.getDeviceProfile()?.productId;
        if (_v(pid) && /^TS00/i.test(pid)) {
          model = String(pid).toUpperCase();
          device.log?.(`[MFR-ENSURE] P2461 soft-filled zb_model_id=${model} from profile`);
        }
      } catch (_e) { /* soft */ }
    }

    // Write back to settings if they were blank or mismatched/normalized
    const settings = device.getSettings?.() || {};
    const updates = {};

    if (settings.zb_manufacturer_name !== mfr && _v(mfr)) {
      updates.zb_manufacturer_name = mfr;
    }
    if (settings.zb_model_id !== model && _v(model)) {
      updates.zb_model_id = model;
    }

    if (Object.keys(updates).length > 0) {
      await device.setSettings?.(updates);
      device.log?.(`[MFR-ENSURE] ✅ Wrote missing or normalized settings: ${JSON.stringify(updates)}`);
      // WHY(P2298): Peter diag 4b1a0dc9 — MFR-ENSURE often lands ~6min after onNodeInit
      // (deferred wake). PhysicalButtonMixin already bound with default profile → no
      // mapAllEndpointsToButton1 / SH-SC07 0xFD path. Re-arm listeners now that identity is known.
      try {
        if (typeof device._onZigbeeIdentityResolved === 'function') {
          await device._onZigbeeIdentityResolved(updates);
        }
      } catch (hookErr) {
        device.log?.(`[MFR-ENSURE] identity hook: ${hookErr.message}`);
      }
    }
...
function isTuyaDPDevice(device) {
  ...
  if (CI.containsCI(mfr, 'HOBEIAN')) {
    const knownDPModels = ['ZG-204ZM', 'ZG-204ZV', 'ZG-204ZH', 'ZG-204ZE', 'ZG-204ZK', 'ZG-204ZQ', 'ZG-204ZX', 'ZG-302ZM', 'ZG-302ZL', 'ZG-227Z', 'ZG-303Z', 'ZG-102ZM', 'ZG-103Z', 'ZG-223Z', 'ZG-226Z', 'ZG-228Z'];
    if (knownDPModels.some(m => CI.containsCI(model, m))) {
      return true;
    }
  }
