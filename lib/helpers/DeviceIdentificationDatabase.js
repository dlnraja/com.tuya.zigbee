...
// console.error('[ID DATABASE] Cannot build: homey not initialized');
    ...
    try {
      ...
      if (!this.homey.drivers || typeof this.homey.drivers.getDrivers !== 'function') {
        ...
        safeSetTimeout(this, () => { if (this._destroyed) return; this._rebuildAfterDriversLoaded(); }, 5000);   // 5s
        safeSetTimeout(this, () => { if (this._destroyed) return; this._rebuildAfterDriversLoaded(); }, 15000);  // 15s
        safeSetTimeout(this, () => { if (this._destroyed) return; this._rebuildAfterDriversLoaded(); }, 30000);  // 30s
        return;
      }
