...
class DeviceIdentificationDatabase {
  ...
  constructor(homey) {
    try {
      ...
      this.log = (homey && typeof homey.log === 'function') ? homey.log.bind(homey) : console.log;
      this.error = (homey && typeof homey.error === 'function') ? homey.error.bind(homey) : console.error;
    } catch (err) {
      // Crash-proof: never let constructor fail
      this.homey = null;
      this.database = { manufacturerIds: {}, productIds: {}, driverMappings: {}, deviceClasses: {}, capabilities: {}, lastUpdate: null };
      this.log = console.log;
      this.error = console.error;
      console.error('[DeviceIdentificationDatabase] Constructor fallback:', err.message);
    }
  }
  ...
  async buildDatabase() {
    if (!this.homey) {
      console.error('[ID DATABASE] Cannot build: homey not initialized');
      return;
    }
    ...
    try {
      // Use Homey API to get all drivers safely
      if (!this.homey.drivers || typeof this.homey.drivers.getDrivers !== 'function') {
        this.log('⏳ [ID DATABASE] Drivers API not available yet, scheduling rebuilds...');
  ...
  findDriverByManufacturer(manufacturerName) {
    const mfrNormalized = ManufacturerResolver.normalize(manufacturerName);
    if (!mfrNormalized) return null;
    for (const [driverId, mapping] of Object.entries(this.database.driverMappings)) {
      if (mapping.manufacturerNames && mapping.manufacturerNames.some(m => ManufacturerResolver.normalize(m) === mfrNormalized)) {
        return {
          driverId,
  ...
  findBestMatch(deviceInfo) {
    ...
    const mfrNormalized = deviceInfo.manufacturer ? ManufacturerResolver.normalize(deviceInfo.manufacturer) : null;
    const pidLower = deviceInfo.modelId ? deviceInfo.modelId.toLowerCase() : null;

    for (const [driverId, mapping] of Object.entries(this.database.driverMappings)) {
      const hasMfr = mfrNormalized && mapping.manufacturerNames && mapping.manufacturerNames.some(m => ManufacturerResolver.normalize(m) === mfrNormalized);
      const hasPid = pidLower && mapping.productIds && mapping.productIds.some(p => p.toLowerCase() === pidLower);
  ...
  isManufacturerMatch(manufacturerName, deviceType) {
    ...
    const mfrNormalized = ManufacturerResolver.normalize(manufacturerName);
    if (!mfrNormalized) return false;
    return ids.some(id => {
      const idNormalized = ManufacturerResolver.normalize(id);
      return idNormalized && mfrNormalized.includes(idNormalized);
    });
  }
  ...
  isProductIdMatch(productId, deviceType) {
    ...
    return ids.some(id => (productId || '').toLowerCase().includes(id.toLowerCase()));
  }
  ...
  async _rebuildAfterDriversLoaded() {
    ...
    if (!this.homey || !this.homey.drivers || typeof this.homey.drivers.getDrivers !== 'function') {
      this.log('⚠️ [ID DATABASE] Drivers API not available during rebuild');
      return;
    }
