...
// console.error('[ID DATABASE] Cannot build: homey not initialized');
    ...
    try {
      ...
      if (!this.homey.drivers || typeof this.homey.drivers.getDrivers !== 'function') {
        ...
        this.homey.setTimeout(() => { if (this._destroyed) return; this._rebuildAfterDriversLoaded(); }, 5000);   // 5s
        this.homey.setTimeout(() => { if (this._destroyed) return; this._rebuildAfterDriversLoaded(); }, 15000);  // 15s
        this.homey.setTimeout(() => { if (this._destroyed) return; this._rebuildAfterDriversLoaded(); }, 30000);  // 30s
        return;
      }
  ...
  findDriverByProductId(productId) {
    const { matchesProductId } = require('../utils/CaseInsensitiveMatcher');
    for (const [driverId, mapping] of Object.entries(this.database.driverMappings)) {
      if (mapping.productIds && matchesProductId(productId, mapping.productIds)) {
        return {
          driverId,
          deviceType: mapping.deviceType,
          confidence: 0.90
  ...
  isProductIdMatch(productId, deviceType) {
    ...
    return ids.some(id => containsCI(productId, id));
  }
