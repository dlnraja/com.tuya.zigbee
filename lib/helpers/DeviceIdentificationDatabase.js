...
class DeviceIdentificationDatabase {
  ...
  async buildDatabase() {
    ...
    try {
      ...
      // If no drivers found, schedule a rebuild after drivers initialize
      if (driverIds.length === 0) {
        this.log('⏳ [ID DATABASE] No drivers loaded yet, will rebuild in 10 seconds...');
        setTimeout(() => this._rebuildAfterDriversLoaded(), 10000);
      }

      this.log(`📂 [ID DATABASE] Found ${driverIds.length} drivers`);
  ...
  async _rebuildAfterDriversLoaded() {
    ...
    if (driverCount > 0) {
      this.log(`🔄 [ID DATABASE] Rebuilding with ${driverCount} drivers now available...`);
