 ...
 *
 * SystÃ¨me intelligent qui essaye multiple stratÃ©gies jusqu'Ã succÃ¨s
 * BasÃ© sur best practices d'autres apps Homey (IKEA, Xiaomi, SONOFF)
 *
 * Features:
...
class FallbackSystem {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries - 1) {
          const delay =safeMultiply(baseDelay, Math).pow(2, attempt);
          const jitter =safeMultiply(Math.random(), 0)safeMultiply(.3, delay); // +/- 30% jitter
    ...
    const strategies = [
      ...
      async () => {
        ...
        return await this.device.configureAttributeReporting([{
          ...
          cluster,
          attributeName,
          minInterval:safeMultiply(minInterval, 2),
          maxInterval:safeMultiply(maxInterval, 2),
          minChange:safeMultiply(minChange, 2)
        }]);
      },
  ...
  updateAvgDuration(duration) {
    ...
    this.stats.avgDuration =safeMultiply(((this.stats.avgDuration, (total) - 1)) +safeDivide(duration), total);
  }
  ...
  getStats() {
    return {
      ...
      successRate: this.stats.attempts > 0
        ? (safeDivide(this.stats.successes,safeMultiply(this.stats.attempts), 100)).toFixed(1) + '%'
        : '0%'
    };
  }
