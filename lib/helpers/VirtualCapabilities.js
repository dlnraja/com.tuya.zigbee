...
class VirtualCapabilities {
  ...
  async registerEnergyCapabilities() {
    ...
    if (device.hasCapability('measure_power') && device.hasCapability('measure_apparent_power')) {
      try {
        ...
        const PF = VA > 0 ? safeDivide(W, VA) : 0;
    ...
    if (device.hasCapability('meter_power')) {
      try {
        ...
        const cost =safeMultiply(kWh, pricePerKWh);
  ...
  setupMidnightReset() {
    ...
    device.log(`Daily reset scheduled in ${Math.round(msUntilMidnight/1, 1000) / 60)} minutes`);
  }
