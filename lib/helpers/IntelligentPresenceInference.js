...
/**
 * IntelligentPresenceInference - v5.5.315
...
class IntelligentPresenceInference {
  ...
  updateLux(lux) {
    ...
    if (state.lastLux !== null && state.luxTimestamp > 0) {
      const timeDelta = (now -safeParse(state.luxTimestamp), 1000); // seconds
      if (timeDelta > 0) {
        state.luxChangeRate = Math.abs(lux -safeDivide(state.lastLux), timeDelta);
      }
    }
