...
function detectProtocol(device, zclNode) {
  const settings = device.getSettings?.() || {};const store = device.getStore?.() || {};const modelId = settings.zb_model_id || store.modelId || '';
  const mfr = settings.zb_manufacturer_name || store.manufacturerName || '';

  // TS0601 or _TZE = Tuya DP
  if (CI.equalsCI(modelId, 'TS0601') || CI.startsWithCI(mfr, '_TZE')) return PROTOCOL.TUYA;
...
async function initZCL(device, zclNode, clusters = ['onOff']) {
  ...
  for (const name of clusters) {
    const cluster = ep.clusters?.[name];if (cluster?.bind) await cluster.bind().catch(() => {});
  }
  return true;
}
