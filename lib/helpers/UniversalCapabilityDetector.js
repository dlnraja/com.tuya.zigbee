...
let TuyaUniversalMapping, EnergyCapabilityDetector, EnrichedDPMappings;
try {
  TuyaUniversalMapping = require('../tuya/tuya-universal-mapping');
  EnergyCapabilityDetector = require('../detectors/EnergyCapabilityDetector');
  EnrichedDPMappings = require('../tuya/EnrichedDPMappings');
} catch (err) {
...
class UniversalCapabilityDetector {
  ...
  static _detectTuyaCustom(zclNode, manufacturerId) {
    ...
    const mappings = (EnrichedDPMappings?.[manufacturerId]?.profile) || {};
  ...
  static _generateRecommendations(result) {
    ...
    if (result.zigbeeStandard.length > 0 && result.tuyaCustom.length > 0) {
      recommendations.push({
        type: 'hybrid_device',
        message: 'Devicee détecté: utilise clusters standard + Tuya custom',
