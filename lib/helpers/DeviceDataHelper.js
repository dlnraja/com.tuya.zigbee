...
function isTuyaDPDevice(device) {
  ...
  const driverId = device.driver?.id || '';if (tuyaDPOnlyDrivers.includes(driverId)) {
    return true;
  }
...
function isMainsPowered(device) {
  const modelId = getModelId(device);
  const driverId = device.driver?.id || '';// Mains-powered models (case-insensitive)
  const mainsPoweredModels = [
    ...
    'TS000B', 'TS000F',                          // Specialized switch modules
  ];
  ...
  const mainsPoweredDrivers = [
    ...
    'dimmer', 'dimmer_2gang', 'dimmer_3gang', 'light_rgb', 'light_rgbw',
    'curtain_motor', 'curtain_motor_tilt', 'garage_door', 'garage_door_opener',
    'shutter_roller_controller', 'wall_curtain_switch', 'fan_controller',
    'smart_breaker', 'din_rail_switch', 'din_rail_meter',
    'siren', 'valve_irrigation', 'water_valve_smart',
  ];
