...
function isTuyaDPDevice(device) {
  ...
  // Note: HOBEIAN is removed from here because Hobeian makes both ZCL and Tuya DP devices!
  const nonTuyaManufacturers = [
    'PHILIPS', 'IKEA', 'OSRAM', 'LEDVANCE', 'HEIMAN',
  ...
  // HOBEIAN ZG-204ZM, ZG-204ZV, ZG-227Z, ZG-303Z are Tuya DP / hybrid devices (case-insensitive)
  if (MfrHelper.containsCI(manufacturer, 'HOBEIAN')) {
    ...
    if (knownDPModels.some(m => MfrHelper.containsCI(modelId, m))) {
      return true;
    }
  }
