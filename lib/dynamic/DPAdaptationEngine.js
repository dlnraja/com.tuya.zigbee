...
class DPAdaptationEngine {
  ...
  async save() {
    ...
    try {
      ...
      // WHY: Gmail diags f647d35b / Nobø — setStoreValue races after device delete → "Not Found: Device with ID …"
      // Soft-log only; never spam stderr as crash signal.
      const msg = String(err && err.message || err);
      if (/not found:\s*device/i.test(msg)) {
        this.log('Save patterns skipped (device gone):', msg.slice(0, 120));
      } else {
        this.error('Save patterns failed:', msg);
      }
    }
  }
