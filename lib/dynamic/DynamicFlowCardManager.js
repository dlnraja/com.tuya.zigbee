...
class DynamicFlowCardManager {
  ...
  async _tryGenericTriggers(capabilityId, value) {
    ...
    if (typeof value === 'boolean') {
      ...
      try {
        const triggerCard = this.homey.flow.getTriggerCard(boolTriggerId);
        if (triggerCard) {
    ...
    if (capabilityId.startsWith('alarm_')) {
      try {
        const alarmTrigger = this.homey.flow.getTriggerCard(`${this.driverId}_alarm_changed`);
        if (alarmTrigger) {
    ...
    if (capabilityId.startsWith('measure_')) {
      try {
        const measureTrigger = this.homey.flow.getTriggerCard(`${this.driverId}_measure_changed`);
        if (measureTrigger) {
