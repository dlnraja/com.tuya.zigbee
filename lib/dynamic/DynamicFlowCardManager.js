...
class DynamicFlowCardManager {
    ...
    try {
      ...
      if (actionCard) {
        actionCard.registerRunListener(async (args, state) => {
          ...
          if (typeof this.device.triggerCapabilityListener === 'function') {
            await this.device.triggerCapabilityListener(capabilityId, value);
          } else if (typeof this.device.safeSetCapabilityValue === 'function') {
            await this.device.safeSetCapabilityValue(capabilityId, value);
          } else {
  ...
  async triggerCapabilityChanged(capabilityId, value) {
    ...
    } = require('../flow/FlowCardHeuristics');

    const tokenName = capabilityId.replace(/^measure_/, '').replace(/^meter_/, 'energy');
    const tokens = { [tokenName]: value, value };

    const declared = collectDeclaredFlowIds(this.homey);
    const candidates = buildCapabilityFlowCandidates(this.driverId, capabilityId);
    const resolved = resolveFlowCardId(candidates, declared);

    if (resolved) {
      const ok = await triggerFlowCardHeuristic(this.device, candidates, tokens, 'trigger');
      if (ok) {
        this.log(`🔔 Triggered capability flow: ${resolved}`);
        return;
      }
    }
