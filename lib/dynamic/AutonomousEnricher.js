...
class AutonomousEnricher {
  ...
  _rssTooHigh() {
    return isMemoryPressure({ heapLimit: 38 * 1024 * 1024 });
  }
  ...
  async loadLocalMappings() {
    ...
    try {
      if (fs.existsSync(this.LOCAL_DB_PATH)) {
        ...
        // WHY(P2678): JSON.parse(Buffer) — never toString('utf8') (double heap → OOM)
        // mfs_db ~5MB: refuse full parse on Homey / under pressure; CI may set HOMEY_INCLUDE_MFS_DB=1
        const st = fs.statSync(this.LOCAL_DB_PATH);
        const allowHuge = process.env.HOMEY_INCLUDE_MFS_DB === '1';
        if (!allowHuge && st.size > 512 * 1024) {
          this.logger.log('[AUTONOMOUS-ENRICHER] Skip full mfs_db parse (size>512KB) — use compose/FP shards');
          this.liveMappings = {};
  ...
  async fetchLatestMappings() {
    try {
      ...
      if (this.currentHash !== hash) {
        ...
        try {
          // WHY(P2678): parse Buffer directly (HTTP body already Buffer)
          const parsed = Buffer.isBuffer(rawBuf)
            ? JSON.parse(rawBuf)
            : JSON.parse(String(rawBuf));
