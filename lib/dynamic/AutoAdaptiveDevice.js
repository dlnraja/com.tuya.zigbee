...
const VirtualTelemetryCompensationEngine = require('./VirtualTelemetryCompensationEngine');
const DPAdaptationEngine = require('./DPAdaptationEngine');
const DPAutoMapper = require('./DPAutoMapper');
...
class AutoAdaptiveDevice extends BaseUnifiedDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v5.5.1: FIX - Longer timeout for battery devices, no rollback on timeout
    const pairingStartTime = Date.now();
    // v5.5.1: Increased timeout to 120s for battery devices (they may be sleeping)
    const PAIRING_TIMEOUT = 120000; // 120 seconds max for pairing
    ...
    try {
      ...
      // v5.5.1: Initialize base - timeout is now a warning, not an error
      // Battery devices may not respond immediately but will work when they wake up
      const initPromise = super.onNodeInit({ zclNode });
      const timeoutPromise = new Promise((resolve) =>
        this.homey.setTimeout(() => {
          if (this._destroyed) {return;}
          this.log('[AUTO-ADAPTIVE] ⚠️ Init taking longer than expected (battery device may be sleeping)');
          resolve('timeout_warning');
        }, PAIRING_TIMEOUT)
      );

      const result = await Promise.race([initPromise, timeoutPromise]);
      if (result === 'timeout_warning') {
        this.log('[AUTO-ADAPTIVE] ℹ️ Continuing with partial init - device will sync when it wakes up');
        // Wait for the actual init to complete in background (don't block)
        initPromise.catch(err => this.log('[AUTO-ADAPTIVE] Background init completed with:', err?.message || 'success'));
      }
      ...
      this.virtualTelemetryCompensationEngine = new VirtualTelemetryCompensationEngine(this);
      this.dpAdaptationEngine = new DPAdaptationEngine(this);
      this.dpAutoMapper = new DPAutoMapper(this);
      ...
      await this.virtualTelemetryCompensationEngine.initialize();
      await this.dpAdaptationEngine.initialize();
      await this.dpAutoMapper.initialize();
      ...
      // Bind initial state listeners for virtual load emulation
      if (this.hasCapability('onoff')) {
        try {
          this.registerCapabilityListener('onoff', async (value) => {
            if (this.virtualTelemetryCompensationEngine) {
              this.homey.setTimeout(() => { if (this._destroyed) {return;} this.virtualTelemetryCompensationEngine.updateEmulatedPower(); }, 50);
            }
          });
        } catch (e) { this.log('[AUTO-ADAPT] onoff listener registration failed:', e.message); }
      }
      if (this.hasCapability('dim')) {
        ...
        } catch (e) { this.log('[AUTO-ADAPT] dim listener registration failed:', e.message); }
      }
      ...
      const adaptStats = this.dpAdaptationEngine ? this.dpAdaptationEngine.getActivePatterns().length : 0;
      ...
      this.log(`[AUTO-ADAPTIVE]    Discovered DPs: ${this._safeGetDiscoveries().length}`);
      ...
      this.log(`[AUTO-ADAPTIVE]    DP Patterns Tracked: ${adaptStats}`);
      this.log('[AUTO-ADAPTIVE] ═══════════════════════════════════════════════');
      ...
      // v5.5.1: Only rollback for CRITICAL errors, not timeouts
      const isCriticalError = err.message?.includes('Invalid Capability') ||
                              err.message?.includes('Invalid cluster') ||
                              err.message?.includes('Not Found');

      if (isCriticalError) {
        this.error('[AUTO-ADAPTIVE] ❌ CRITICAL ERROR - Rolling back device creation');
        ...
        // v5.5.1: Non-critical errors (timeouts, etc) - continue with partial init
        this.log('[AUTO-ADAPTIVE] ⚠️ Non-critical error during init:', err.message);
        this.log('[AUTO-ADAPTIVE] ℹ️ Device added with partial initialization');
        this.log('[AUTO-ADAPTIVE] ℹ️ Device will fully sync when it communicates');

        // Mark as partially initialized
        await this.setStoreValue('partial_init', true).catch(() => {});
        await this.setStoreValue('init_error', err.message).catch(() => {});

        // Don't throw - let device be added
      }
    }
  }
   ...
   * v6.0.0: Uses DPAdaptationEngine for pattern analysis and DPAutoMapper for inference
   */
  async _onDPReceived(dpId, value, dpType = null) {
    ...
    // v6.0.0: Run adaptation engine analysis (pattern tracking, anomaly detection)
    let adaptationResult = { isNew: false, isAnomaly: false };
    if (this.dpAdaptationEngine) {
      adaptationResult = this.dpAdaptationEngine.analyzeDP(dpId, parsed.value, dpType);
    }
    ...
    let processedValue = convertedValue;
    if (this.virtualTelemetryCompensationEngine) {
      const result = this.virtualTelemetryCompensationEngine.processTelemetry(context, convertedValue);
      processedValue = result.validatedValue;
    }

    try {
      // v6.0.0: For new/unknown DPs, use DPAutoMapper for value-range inference
      if (adaptationResult.isNew && this.dpAutoMapper && !this.dynamicCapabilityManager._discoveredDPs.has(dpId)) {
        const dpTypeStr = dpType !== null ? this._dpTypeToString(dpType) : typeof parsed.value === 'boolean' ? 'bool' : typeof parsed.value === 'number' ? 'value' : 'unknown';
        const existingDPs = Array.from(this.dynamicCapabilityManager._discoveredDPs.keys());
        const autoMapping = this.dpAutoMapper.mapDPToCapability(dpId, parsed.value, dpTypeStr, {
          manufacturer: this.getSetting?.('zb_manufacturer_name') || null,
          deviceType: this.driver?.id || null,
          existingDPs,
        });

        if (autoMapping && autoMapping.confidence >= 0.6) {
          this.log(`[AUTO-ADAPTIVE] Auto-mapped DP${dpId} -> ${autoMapping.capability} (confidence: ${(autoMapping.confidence * 100).toFixed(0)}%)`);
        }
      }

      // Process for capabilities with converted value
      const mapping = await this.dynamicCapabilityManager.processDP(dpId, processedValue, dpType);

      // Process for energy (power, voltage, current, energy)
      if (this.dynamicEnergyManager) {
        await this.dynamicEnergyManager.processDP(dpId, processedValue, dpType);
      }

      if (mapping) {
        // Also call device-specific handler if exists
        await this._handleDeviceSpecificDP(dpId, processedValue, mapping);
      }
    } catch (err) {
  ...
  /**
   * v6.0.0: Convert numeric DP type code to string
   */
   ...
   */
  /** Safe discoveries — optional-call on missing method still throws in JS. */
  _safeGetDiscoveries() {
    const mgr = this.dynamicCapabilityManager;
    if (!mgr || typeof mgr.getDiscoveries !== 'function') {return [];}
    try { return mgr.getDiscoveries() || []; } catch (_e) { return []; }
  }

  getAutoAdaptiveStatus() {
    const discoveries = this._safeGetDiscoveries();
    const flowStats = this.dynamicFlowCardManager?.getStats() || {};
    const adaptPatterns = this.dpAdaptationEngine?.getActivePatterns() || [];
    const adaptDecisions = this.dpAdaptationEngine?.getDecisions() || [];
    const mappingLog = this.dpAutoMapper?.exportMappingLog() || [];
    ...
    return {
      ...
      dpPatterns: adaptPatterns,
      recentAdaptations: adaptDecisions.slice(-20),
      autoMappingLog: mappingLog.slice(-20),
    };
  }
  ...
  async _forceDeviceWakeUp() {
    ...
    try {
      // WHY(P2739 / Bastien diag ed627371 @ 1.0.93): TS004x remotes are ButtonDevice
      // → AutoAdaptive. Forced DP query + ZCL batt read after init burns the awake
      // window (mcuVersionRequest / WAKE-UP Tuya DP / configureReporting) while
      // user sees "boutons reconnus mais rien ne fonctionne". Listen-only.
      let skipWakeTx = false;
      try {
        const p = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
        skipWakeTx = !!(p?.snappyRelayFlow || p?.skipBatteryReporting || p?.noEf00Tx || p?.noEf00);
        if (!skipWakeTx && typeof this.buttonCount === 'number' && this.buttonCount >= 1
          && (String(this.driver?.id || '').includes('button_wireless')
            || String(this.driver?.id || '').includes('scene_switch'))) {
          skipWakeTx = true;
        }
      } catch (_e) { /* soft */ }
      if (skipWakeTx) {
        this.log('[WAKE-UP] P2739 skip force wake TX (snappy / noEf00 remote — listen-only)');
        return;
      }
  ...
  _scheduleDataCheckTimeout() {
    ...
    this._dataCheckTimer = this.homey.setTimeout(async () => {
      if (this._destroyed) {return;}
      try {
        const discoveries = this._safeGetDiscoveries();
  ...
  _isValidCapability(capabilityId) {
    if (typeof capabilityId !== 'string' || capabilityId.length === 0) {return false;}
    ...
    if (VALID_CAPABILITIES.includes(capabilityId)) {
      return true;
    }

    // v9.0.394: Sub-capabilities (e.g. 'button.1', 'button.toggle',
    // 'measure_humidity.soil') are valid when their base capability is
    // standard. Fixes "Invalid capabilities in manifest: button.1, button.2,
    // button.3" pairing crashes (Gmail diagnostics 2026-07-16).
    const baseCapability = capabilityId.split('.')[0];
    if (baseCapability !== capabilityId && VALID_CAPABILITIES.includes(baseCapability)) {
      return true;
    }

    // App-defined custom capabilities (app.json "capabilities" section)
    const appCapabilities = this.homey?.app?.manifest?.capabilities || {};
    return Object.prototype.hasOwnProperty.call(appCapabilities, capabilityId);
  }
   ...
   */
  async onDeleted() {
    this.log('[AUTO-ADAPTIVE] Device deleted - Cleaning up listeners...');

    try {
      if (this.virtualTelemetryCompensationEngine) {
        this.virtualTelemetryCompensationEngine.destroy();
      }

      // v6.0.0: Destroy adaptation and auto-mapper engines
      if (this.dpAdaptationEngine) {
        // Save patterns before destroying
        await this.dpAdaptationEngine.save().catch(() => {});
        this.dpAdaptationEngine.destroy();
      }

      if (this.dpAutoMapper) {
        this.dpAutoMapper = null;
      }
