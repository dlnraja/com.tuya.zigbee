...
class LiveDataUpdater {
  ...
  async start() {
    ...
    // v5.12.62 (P92.129): Homey-managed timers (TITAN lifecycle gate)
    this._timer = this.homey.setTimeout(() => this.checkNow().catch(() => {}), 120000 + Math.floor(Math.random() * 60000));
    // Then daily
  ...
  async checkNow() {
    ...
    let payload;
    try {
      // v10.16.0 (PRD v1.1): prefer SEGMENTED feeds (RAM-friendly progressive
      // download on Homey Pro). Falls back to the single full file when the
      // manifest is absent (older gh-pages).
      payload = await this._fetchSegmented();
      if (!payload) {
  ...
  /**
   * v10.16.0 (PRD v1.1): progressive segmented download. Fetch the manifest,
   * then each segment file in ascending size order (quick wins first,
   * bounded memory per request). Returns a merged payload identical in
   * shape to the full feed, or null when no manifest exists.
   */
  async _fetchSegmented() {
    let manifest;
    try {
      manifest = await this._fetchJson(MANIFEST_URL);
    } catch {
      return null; // no segmented feed published yet → caller falls back
    }
    ...
    for (const [seg, info] of entries) {
      try {
        ...
        if (!this._validatePayload(segPayload)) {
          this.log(`[LIVE-DATA] segment ${seg} invalide — ignoré`);
          continue;
        }
        Object.assign(devices, segPayload.devices);
        version = segPayload.version || version;
        this.log(`[LIVE-DATA] segment ${seg}: ${Object.keys(segPayload.devices).length} fabricants`);
      } catch (err) {
        this.log(`[LIVE-DATA] segment ${seg} inaccessible (${err.message}) — ignoré`);
      }
    }
