...
const MAX_ENTRIES = 800; // P2718: was 1500 — Bastien OOM @64MB; house/Universal don't need fat overlay
const MAX_STORE_CHARS = 120000; // ~120KB serialized — Homey settings + V8
const MAX_SEGMENTS = 6; // P2718: stop after a few small segments — Contre quoi heap OOM
const MFR_RX = /^[^\u0000-\u001F\u007F]{1,64}$/u;
...
class LiveDataUpdater {
  ...
  async start() {
    // WHY(P2718 Bastien 27b0bd04 OOM): house track has locked couples — never pull
    // GitHub Pages mfs overlay (segment merge blew 64MB heap → SIGABRT → all remotes dead).
    // WHY(P2720): also PURGE any leftover overlay from tip ≤1.0.82 — skip alone left
    // live_data_overlay in Homey settings eating RAM after upgrade.
    try {
      const id = String(
        this.homey?.manifest?.id
        || this.homey?.app?.manifest?.id
        || ''
      );
      if (/\.bastien\b/i.test(id)) {
        try {
          const leftover = this.homey.settings.get('live_data_overlay');
          if (leftover) {
            this.homey.settings.unset('live_data_overlay');
            this.homey.settings.unset('live_data_version');
            this.log('[LIVE-DATA] P2720 purged leftover Bastien overlay (heap Contre quoi)');
          }
        } catch { /* soft */ }
        this._overlay = null;
        this.log('[LIVE-DATA] P2718/P2720 skip — Bastien house (no OTA overlay)');
        return;
      }
    } catch { /* continue */ }
  ...
  _heapTooHigh() {
    // WHY(P2586): Homey counts RSS ≈64MB — BootBudget is the single gate
    try {
      const BootBudget = require('../performance/BootBudget');
      return BootBudget.isHeapCritical() || !BootBudget.shouldStartHeavyFeatures();
    } catch {
      return isMemoryPressure({ heapLimit: 40 * 1024 * 1024, rssLimit: 55 * 1024 * 1024 });
    }
  }
  ...
  _validatePayload(p) {
    ...
    for (const k of keys) {
      ...
      // WHY: refuse mfr-only overlay payloads. Couple = mfr + modelIds.
      if (!Array.isArray(e.modelIds) || e.modelIds.length < 1 || e.modelIds.length > 60) return false;
  ...
  async _fetchSegmented(currentVersion) {
    ...
    for (const [seg, info] of entries) {
      ...
      if (segmentsDone >= MAX_SEGMENTS) {
        this.log(`[LIVE-DATA] Stop segments early at ${seg} (max ${MAX_SEGMENTS} segments)`);
        break;
      }
      try {
        ...
        // Merge with hard cap — only accept couple-shaped entries (mfr + modelIds).
        // WHY(P2718 Bastien 27b0bd04): previously accepted driverId-only rows → filled
        // 1500 junk entries → _validatePayload rejected AFTER heap was already spent.
        for (const [mfr, entry] of Object.entries(segPayload.devices)) {
          if (Object.keys(devices).length >= MAX_ENTRIES) break;
          if (FORBIDDEN_KEYS.has(mfr) || !MFR_RX.test(mfr)) continue;
          if (!entry || typeof entry !== 'object' || Array.isArray(entry)) continue;
          if (typeof entry.driverId !== 'string' || !DRIVER_RX.test(entry.driverId)) continue;
          if (!Array.isArray(entry.modelIds) || entry.modelIds.length < 1 || entry.modelIds.length > 60) continue;
          let modelOk = true;
          for (const m of entry.modelIds) {
            if (typeof m !== 'string' || m.length > 60) { modelOk = false; break; }
          }
          if (!modelOk) continue;
          devices[mfr] = entry;
        }
        version = segPayload.version || version;
        const added = Object.keys(devices).length - before;
        this.log(`[LIVE-DATA] segment ${seg}: +${added} kept (overlay ${Object.keys(devices).length})`);
        segmentsDone += 1;
  ...
  _rawFetchJson(url) {
    return new Promise((resolve, reject) => {
      ...
      const req = https.get(url, { headers: { 'User-Agent': 'homey-tuya-app/9.0' } }, (res) => {
        ...
        res.on('end', () => {
          try {
            // WHY(P2586): JSON.parse(Buffer) — no UTF-16 intermediate string
