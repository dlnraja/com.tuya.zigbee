...
const DP_HEURISTICS = [
  // === TEMPERATURE ===
  // v10.7.0: smart converters (magnitude-aware) instead of hardcoded ÷10 —
  // hardcoded divisors mis-scaled ×1 devices (audit Part 2, gap 4).
  { dpIds: [1, 3, 5, 18, 24, 103], type: 'value', valueRange: [-400, 800], capability: 'measure_temperature', converter: 'smartTemperature', priority: 20, category: 'climate' },
  { dpIds: [1], type: 'value', valueRange: [0, 1000], capability: 'target_temperature', converter: 'smartTemperature', priority: 5, category: 'thermostat' },

  // === HUMIDITY ===
  { dpIds: [2, 4, 6, 19, 25], type: 'value', valueRange: [0, 10000], capability: 'measure_humidity', converter: 'smartHumidity', priority: 20, category: 'climate' },
...
const VALUE_CONVERTERS = {
  ...
  // v10.7.0: magnitude-aware converters (SmartDivisorManager-aligned)
  smartHumidity: (v) => (v > 100 ? v / 10 : v),
  smartTemperature: (v) => (Math.abs(v) > 400 ? v / 10 : v),
...
class DynamicCapabilityManager {
  ...
  async initialize() {
    try {
      // WHY(P2386 / VicHY #2222): presence radars must never restore curtain/dim discoveries
      // from store after app updates — that re-applies "blind mode" UI + stuck motion.
      if (this._isDynCapDisabledForDevice()) {
        try { await this.device.unsetStoreValue('dynamic_capabilities').catch(() => {}); } catch (_e) { /* soft */ }
        this._discoveredDPs.clear();
        this.log('DynCap disabled for driver — cleared stored discoveries');
        return;
      }
      ...
      for (const [dpId, mapping] of Object.entries(stored)) {
        const id = parseInt(dpId, 10);
        // WHY(P2326): drop poisoned discoveries that collide with driver mappings
        if (this._isDriverOwnedDp(id)) {
          this.log(`Dropping stored dyn-cap DP${id} (driver-owned)`);
          continue;
        }
        const cap = mapping && mapping.capability;
        if (cap && this._isIrrelevantCap(cap)) {
          this.log(`Dropping stored dyn-cap DP${id} -> ${cap} (irrelevant for driver)`);
          continue;
        }
        this._discoveredDPs.set(id, mapping);
        this.log(`Restored DP${id} -> ${mapping.capability} (confidence: ${mapping.confidence})`);
      }

      // Persist cleaned map so poisoned DP36→target_temperature does not resurrect
      try { await this._saveDiscoveries(); } catch (_e) { /* noop */ }
  ...
  async processDP(dpId, value, dpType = null) {
    // P2230: never invent climate caps from Tuya RAW/byte_array (Tongou DP6→humidity on smart_rcbo 3a1f196d)
    const detectedEarly = dpType !== null ? this._dpTypeToString(dpType) : this._detectType(value);
    if (detectedEarly === 'raw' || Buffer.isBuffer(value) || Array.isArray(value)) {
      if (this._isForbiddenRawClimateDp(dpId, value)) {
        this.log(`DP${dpId}: skip dyn-cap (raw/byte_array — not humidity/temp)`);
        return null;
      }
    }

    // WHY(P2326 / Adam a095345e): never invent caps for DPs the driver already owns
    // (FCU DP36 valve was dyn-mapped to target_temperature → UI setpoint jumps to 5).
    if (this._isDriverOwnedDp(dpId)) {
      this.log(`DP${dpId}: skip dyn-cap (driver dpMappings / reserved)`);
      return null;
    }

    // WHY(P2364): remotes, SOS, IAS sleepy sensors own their DPs — no DynCap at all
    if (this._isDynCapDisabledForDevice()) {
      this.log(`DP${dpId}: skip dyn-cap (driver class disabled)`);
      return null;
    }
  ...
  async _discoverCapability(dpId, value, dpType) {
    ...
    if (detectedType === 'raw') {
      this.log(`DP${dpId}: raw type — no auto climate/number mapping`);
      return null;
    }
    const samples = this._dpSamples.get(dpId);
    ...
    for (const heuristic of DP_HEURISTICS) {
      // WHY(P2548 / VicHY #2241): DP2/3/102 on mmWave are sensitivity/range/delay — never
      // score cover/dim heuristics or Homey flips class to curtain without an app update.
      if ((heuristic.category === 'cover' || heuristic.capability === 'dim')
        && this._isPresenceRadarDriver()) {
        continue;
      }
    ...
    if (bestMatch && bestScore >= this.MIN_CONFIDENCE) {
      if (this._isIrrelevantCap(bestMatch.capability)) {
        this.log(`DP${dpId}: skip ${bestMatch.capability} (irrelevant for driver)`);
        return null;
      }
      const mapping = {
  ...
  async _createGenericCapability(dpId, value, type) {
    if (this._isDynCapDisabledForDevice()) {
      this.log(`DP${dpId}: skip generic capability (driver disabled)`);
      return null;
    }
  ...
  async _ensureCapability(capabilityId) {
    if (this._isIrrelevantCap(capabilityId)) {
      this.log(`Skip irrelevant capability ${capabilityId}`);
      return false;
    }
  ...
  async _applyValue(mapping, value) {
    try {
      // Never push Buffer/array into measure_humidity / measure_temperature (diag 3a1f196d)
      if ((mapping.capability === 'measure_humidity' || mapping.capability === 'measure_temperature')
        && (Buffer.isBuffer(value) || Array.isArray(value) || (value && typeof value === 'object'))) {
        this.log(`Skip ${mapping.capability}: non-numeric DP${mapping.dpId} value`);
        return;
      }
  ...
  /**
   * P2230 — Tongou TO-Q-SYS-JZT DP6 is electricity composite (type 0), not humidity.
   * Also block any raw climate guess when mfr is a known energy meter couple.
   */
  /**
   * WHY(P2326): Driver-owned DPs (explicit dpMappings or _dynCapBlockDps) must never
   * be reinvented — e.g. FCU DP36 valve ≠ target_temperature (Adam diag a095345e).
   */
  _isDriverOwnedDp(dpId) {
    const id = Number(dpId);
    if (!Number.isFinite(id)) {return false;}
    try {
      const driverId = String(this.device?.driver?.id || '');
      // WHY(P2333 / Adam a095345e on 9.0.699): DynCap init runs inside super.onNodeInit
      // BEFORE wall_thermostat._installDpMappings — hard-reserve FCU valve DP36.
      if (/wall_thermostat/i.test(driverId) && id === 36) {return true;}
      // WHY(P2333 / PresentSky 60959c24): dimmer DP1/DP2 are onoff/brightness — never climate.
      if (/wall_dimmer|light_dimmer|_dimmer/i.test(driverId) && (id === 1 || id === 2)) {
        return true;
      }
      // WHY(P2364): SOS / IAS sleepy remotes — driver owns alarm + battery DPs
      if (/button_emergency_sos|remote_button_emergency|sos/i.test(driverId)
        && [1, 4, 13, 14, 15, 101].includes(id)) {
        return true;
      }
      if (/water_leak|contact_sensor/i.test(driverId) && [1, 2, 4, 15, 101].includes(id)) {
        return true;
      }
      // WHY(P2379 / VicHY): MTG/ZY-M100 radar DPs are settings+presence — never DynCap invent
      if (/presence_sensor_radar|motion_sensor_radar|pir_mmwave/i.test(driverId)
        && [1, 2, 3, 4, 6, 9, 12, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115].includes(id)) {
        return true;
      }
      if (this.device._dynCapBlockDps instanceof Set && this.device._dynCapBlockDps.has(id)) {
        return true;
      }
      const live = this.device.dpMappings;
      if (live && typeof live === 'object' && (live[id] != null || live[String(id)] != null)) {
        return true;
      }
    } catch (_e) { /* noop */ }
    return false;
  ...
  /**
   * WHY(P2333): call after late driver dpMappings install (FCU arm) to eject
   * poisoned store entries that raced ahead of _installDpMappings.
   */
  async purgeDriverOwnedDiscoveries() {
    let changed = false;
    for (const dpId of [...this._discoveredDPs.keys()]) {
      if (this._isDriverOwnedDp(dpId)) {
        this._discoveredDPs.delete(dpId);
        changed = true;
        this.log(`Purged dyn-cap DP${dpId} (driver-owned after late map)`);
      }
    }
    if (changed) {
      try { await this._saveDiscoveries(); } catch (_e) { /* noop */ }
  ...
  /**
   * WHY(P2364): parity with lib/managers/DynamicCapabilityManager — block phantom
   * onoff/climate on remotes, SOS, dimmers, meters, covers.
   */
  _isIrrelevantCap(cap) {
    const driverId = this.device.driver?.manifest?.id || this.device.driver?.id || '';
    const cls = this.device.driver?.manifest?.class || this.device.getClass?.() || '';
    const forbidden = this.device._forbiddenCapabilities || [];
    if (Array.isArray(forbidden) && forbidden.includes(cap)) {return true;}

    const { isButtonActuatorDriverId, isSceneRemoteDriverId } = require('../utils/scene-remote-classify');
    const isActuator = isButtonActuatorDriverId(driverId);
    const isRemoteButton = !isActuator && (
      cls === 'button'
      || isSceneRemoteDriverId(driverId)
      || /knob/.test(driverId)
      || /button_emergency_sos|remote_button_emergency|sos/.test(driverId)
    );
    if (isRemoteButton) {
      if (['onoff', 'dim', 'windowcoverings_state', 'measure_temperature', 'measure_humidity', 'alarm_contact', 'alarm_battery'].includes(cap)) {
        return true;
      }
    }
    ...
    if (/light|bulb|led_strip|dimmer/.test(driverId)) {
      if (['measure_luminance', 'measure_humidity', 'measure_temperature'].includes(cap)) {return true;}
    }
    if (/smart_rcbo|din_rail|power_meter|energy_meter|plug_energy|socket.*meter/.test(driverId)) {
      if (['measure_humidity', 'measure_temperature', 'alarm_contact', 'alarm_motion'].includes(cap)) {
        return true;
      }
    }
    // WHY(P2379 / VicHY): mmWave presence radars — DP2/3/102 are sensitivity/range/delay,
    // never curtain position. DynCap inventing windowcoverings_set flips Homey UI to "curtain"
    // and DP102→alarm_motion sticks presence true (departure_delay > 0).
    if (/presence|motion_sensor|radar|mmwave|pir_mmwave|occupancy/i.test(driverId) || cls === 'sensor') {
      if ([
        'windowcoverings_set',
      ...
      // WHY(P2477 / VicHY c5165a37): block ZCL/DynCap battery re-inject on mains MTG
      if (['measure_battery', 'alarm_battery'].includes(cap) && this._isMainsPresenceRadar()) {
        return true;
      }
    }
    return false;
  ...
  /**
   * P2477 — mains mmWave must never gain phantom battery from DynCap.
   * @returns {boolean}
   */
  _isMainsPresenceRadar() {
    try {
      if (this.device?.mainsPowered) return true;
      const mfr = String(
        this.device?.getSetting?.('zb_manufacturer_name')
        || this.device?._protocolInfo?.mfr
        || ''
      ).toLowerCase();
      if (/clrdrnya|sbyx0lm6|szr07/.test(mfr)) return true;
      const cfg = this.device?._getRadarConfig?.();
      if (cfg?.mainsPowered || cfg?.noBatteryCapability || cfg?.suppressBatteryCapability) return true;
    } catch (_e) { /* soft */ }
    return false;
  }

  /**
   * WHY(P2548): presence/radar drivers — skip cover/dim invent heuristics.
   */
  _isPresenceRadarDriver() {
    try {
      const driverId = String(this.device?.driver?.id || this.device?.driver?.manifest?.id || '');
      if (/presence_sensor_radar|motion_sensor_radar|pir_mmwave|sensor_presence|mmwave/i.test(driverId)) {
        return true;
      }
      if (this._isMainsPresenceRadar()) return true;
      const cls = this.device?.getClass?.() || this.device?.driver?.manifest?.class || '';
      if (cls === 'sensor' && /presence|radar|motion/i.test(driverId)) return true;
    } catch (_e) { /* soft */ }
    return false;
  }

  /**
   * WHY(P2364): whole-driver opt-out — remotes/SOS/IAS sleepy must never get DynCap.
   */
  _isDynCapDisabledForDevice() {
    try {
      const driverId = String(this.device?.driver?.id || '');
      const cls = this.device.driver?.manifest?.class || this.device.getClass?.() || '';
      if (cls === 'button') {return true;}
      if (/button_emergency_sos|remote_button_emergency|sos|water_leak|contact_sensor|button_wireless|scene_switch|knob/.test(driverId)) {
        return true;
      }
      // WHY(P2379): presence radars own full dpMap in configs.js — DynCap must not invent
      if (/presence_sensor_radar|motion_sensor_radar|pir_mmwave|sensor_presence|sensor_motion_presence|climate_sensor_presence/i.test(driverId)) {
        return true;
      }
    } catch (_e) { /* noop */ }
    return false;
  ...
  _isForbiddenRawClimateDp(dpId, value) {
    const id = Number(dpId);
    if (![2, 4, 6, 19, 25, 1, 3, 5, 18, 24].includes(id)) {return true;} // treat raw as non-climate by default
    try {
      const mfr = String(this.device.getSetting?.('zb_manufacturer_name')
        || this.device._protocolInfo?.mfr || '').toLowerCase();
      if (mfr.includes('6ocnqlhn') && id === 6) {return true;}
      const driverId = this.device.driver?.id || '';
      if (/smart_rcbo|din_rail|energy|meter|plug/i.test(driverId) && id === 6) {return true;}
      // WHY Ship M: under heap pressure skip speculative climate invent from raw DPs
      try {
        const BootBudget = require('../performance/BootBudget');
        if (BootBudget.isHeapCritical()) {return true;}
      } catch { /* ignore */ }
    } catch (_e) { /* noop */ }
  ...
  /**
   * Athom-style capability audit (parity with lib/managers/DynamicCapabilityManager).
   * Auto-adaptive / DP-first devices skip ZCL phantom removal — clusters are unreliable.
   * Must exist so BaseUnifiedDevice's delayed audit timer never throws
   * "auditCapabilities is not a function" after this manager replaces the managers/ DCM.
   */
  async auditCapabilities() {
    if (!this.device || this.device._destroyed || this.device.destroyed) {return;}
    this.log('Skipping ZCL capability audit (dynamic/DP mode)');
  }
