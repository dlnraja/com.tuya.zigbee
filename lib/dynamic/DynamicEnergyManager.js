...
class DynamicEnergyManager {
  ...
  async initialize() {
    ...
    // WHY(P2672): defer to IntelligentEnergyAdapter — skip phantom energy on non-metered
    try {
      const IntelligentEnergyAdapter = require('../energy/IntelligentEnergyAdapter');
      this._energyProfile = await IntelligentEnergyAdapter.apply(this.device);
      if (this._energyProfile?.skipZclEnergyBind || this._energyProfile?.mode !== 'metered') {
        this.log(`P2672 skip DynamicEnergy discovery (mode=${this._energyProfile?.mode || 'unknown'})`);
        return;
      }
    } catch (e) {
      this.log(`P2672 adapter soft-fail: ${e.message}`);
    }
    ...
    // v7.2 ZCL Polling Fallback: If Tuya DPs are silent, try polling standard ZCL
    this._startZclPollingFallback();

    this.log(`✅ Energy manager initialized with ${this._discoveredEnergyDPs.size} discovered DPs`);
  }

  /**
   * ZCL Polling Fallback (Inspiration Z2M)
   * Tuya chips sometimes stop sending DP reports but still reply to ZCL readAttributes
   */
  _startZclPollingFallback() {
    if (this._zclPollInterval) {this.homey.clearInterval(this._zclPollInterval);}

    // Poll every 5 minutes
    this._zclPollInterval = this.homey.setInterval(async () => {
      if (this._destroyed || !this.device || !this.device.zclNode) {return;}
      
      try {
        const ep1 = this.device.zclNode.endpoints?.[1];
        if (!ep1?.clusters) {return;}
        
        // Poll 0x0B04 (ElectricalMeasurement)
        const em = ep1.clusters.haElectricalMeasurement;
        if (em && typeof em.readAttributes === 'function') {
          await em.readAttributes(['rmsVoltage', 'rmsCurrent', 'activePower']).catch(() => {});
        }
        
        // Poll 0x0702 (SimpleMetering)
        const sm = ep1.clusters.seMetering;
        if (sm && typeof sm.readAttributes === 'function') {
          await sm.readAttributes(['currentSummDelivered']).catch(() => {});
        }
      } catch (err) {
        // Silent catch for ZCL polling errors
      }
    }, 5 * 60 * 1000);
  }
  ...
  async _applyEnergyValue(mapping, rawValue) {
    ...
    // Convert value with Max Fallbacks
    let value = 0;
    try {
      const converter = capDef?.converters?.[mapping?.converter] || capDef?.converters?.default;
      value = converter ? converter(rawValue) : rawValue;
      
      if (typeof value !== 'number' || isNaN(value) || !isFinite(value)) {
        this.error(`[ENERGY] ❌ Invalid calculated value for ${mapping?.capability}: ${value} (raw: ${rawValue})`);
        return;
      }
      
      // Z2M strict heuristic bounds
      if (value < 0) {value = 0;}
      if (mapping.capability === 'measure_voltage' && value > 300) {value = 300;} // Unrealistic for smart home
      
    } catch (e) {
      this.error(`[ENERGY] ❌ Converter crash:`, e?.message || e);
      return;
    }
    ...
    // Set capability — measured DP path via SmartEnergyManager when present
    if (this.device && typeof this.device.hasCapability === 'function' && this.device.hasCapability(mapping.capability)) {
      try {
        const smart = this.device.smartEnergy;
        if (smart && typeof smart.reportMeasured === 'function') {
          await smart.reportMeasured(mapping.capability, value, `dynamic-dp-${mapping.dpId}`).catch(() => {});
        } else if (typeof this.device.safeSetCapabilityValue === 'function') {
          await this.device.safeSetCapabilityValue(mapping.capability, value).catch(() => {});
        } else {
        ...
        this.error(`Failed to set ${mapping.capability}:`, err?.message || err);
      }
    }
  }
  ...
  async _ensureEnergyCapabilities() {
    for (const [capability, def] of Object.entries(ENERGY_CAPABILITIES)) {
      ...
      if (hasDiscoveredDP && !this.device.hasCapability(capability)) {
        // Safe Listening / Adaptation Gating Check
        if (this.device.virtualTelemetryCompensationEngine) {
          if (!this.device.virtualTelemetryCompensationEngine.gateAdaptation(capability, 'ADD')) {
            this.log(`🛑 GATED: Postponing addition of energy capability ${capability} until safe epoch completed.`);
            continue;
          }
        }
        try {
  ...
  _startPeriodicSave() {
    ...
    this._saveInterval = this.homey.setInterval(() => {
