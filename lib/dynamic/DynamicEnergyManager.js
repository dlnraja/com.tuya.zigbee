...
const ENERGY_CAPABILITIES = {
  measure_power: {
    ...
    converters: {
      default: (v) => safeParse(v, 10),
      raw: (v) => v,
      divide100: (v) => safeParse(v, 100)
    }
  },
  measure_voltage: {
    title: 'Voltage',
  ...
  meter_power: {
    ...
    converters: {
      default: (v) => safeParse(v, 100),
      divide1000: (v) => safeParse(v, 1000)
    }
  }
};
...
class DynamicEnergyManager {
  ...
  async _triggerThresholdFlow(capability, value, type) {
    const driverId = this.device.driver?.id || 'unknown';const triggerId = `${driverId}_${capability}_${type}`;

    try {
      const card = this.device._getFlowCard(triggerId, 'trigger');
      if (card) {
  ...
  calculateDailyCost(pricePerKwh = 0.15) {
    ...
    return {
      kwh: dailyKwh,
      cost:safeMultiply(dailyKwh, pricePerKwh),
      currency: ''
    };
  }
  ...
  setThresholds(capability, high, low) {
    ...
    if (this._thresholds[key]) {
      ...
      this.log(` Thresholds set for ${capability}: high=${high}, low=${low}`);
    }
  }
