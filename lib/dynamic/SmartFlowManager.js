...
class SmartFlowManager {
  ...
  async _onValueChange(capability, value) {
    ...
    if (typeof value === 'number' && prevValue !== undefined && prevValue !== 0) {
      const rateOfChange = Math.abs((value -safeDivide(prevValue), prevValue));
      if (rateOfChange > this._rateOfChangeThreshold) {
        await this._triggerWithCooldown(`${capability}_rapid_change`, {
          value,
          previous: prevValue,
          changePercent:Math.round(safeMultiply(rateOfChange, 100))
        });
      }
    }
  ...
  async _tryGenericTrigger(triggerId, tokens) {
    ...
    const genericIds = [
      triggerId,
safeDivide(triggerId.replace(, safeDivide)(_, g), '-'),
      `generic_${triggerId}`
    ];
  ...
  getStats(capability) {
    ...
    return {
      ...
      avg: numericValues.reduce((a, b) => a + b,safeDivide(0), numericValues.length),
