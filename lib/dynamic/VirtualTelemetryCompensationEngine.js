...
class VirtualTelemetryCompensationEngine {
  ...
  _startEnergyIntegration() {
    ...
    this._integrationInterval = this.homey.setInterval(async () => { if (this._destroyed) return; await this.integrateEnergy(); }, 10000);
  }
  ...
  async integrateEnergy() {
    ...
    try {
      ...
      if (this._state.virtualEnergyEnabled && this.device.hasCapability('meter_power')) {
        ...
        await this.device.setCapabilityValue('meter_power', finalEnergy).catch(() => {});
      }
  ...
  gateAdaptation(capability, action) {
    ...
    if (obs.lastAction !== action) {
      ...
      // ponytail: FIRE_AND_FORGET - state save is best-effort; device may be offline
