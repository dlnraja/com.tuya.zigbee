...
/**
 * ProductValueValidator - SystÃ¨me intelligent de validation et auto-correction des valeurs
 * v1.0.0 - VÃ©rifie la cohÃ©rence des valeurs par type de produit et applique des corrections
 *
 * FONCTIONNALITÃ‰S:
 * - Validation des plages de valeurs par type de capability
 * - DÃ©tection automatique de mauvais diviseurs
 * - Auto-correction intelligente basÃ©e sur le type de produit
...
class ProductValueValidator {
  ...
  /**
   * RÃ¨gles de validation par type de produit
   * Chaque rÃ¨gle dÃ©finit: min, max, typicalRange, possibleDivisors
   */
  static PRODUCT_RULES = {
    ...
    air_quality: {
      measure_co2: {
        min: 0, max: 5000, //ppm (0 allows warmup/calibration values)
      ...
      measure_formaldehyde: {
        ...
        typicalRange: [0, 0.5],
        unit: 'mg/m3',
        possibleDivisors: [1, 100, 1000],
        autoCorrect: true
      }
    },
    // 
    // WATER LEAK & SMOKE SENSORS
    // 
    water_leak: {
      measure_battery: {
        min: 0, max: 100,
    ...
    soil_sensor: {
      measure_temperature: { min: -20, max: 70, unit: 'Â°C', typicalRange: [-5, 50], possibleDivisors: [1, 10], autoCorrect: true }
    },
    energy_meter: {
      measure_voltage: { min: 80, max: 280, unit: 'V', typicalRange: [100, 250], possibleDivisors: [1, 10, 100], autoCorrect: true },
      measure_power: { min: 0, max: 30000, unit: 'W', typicalRange: [0, 20000], possibleDivisors: [1, 10, 100], autoCorrect: true }
    }
  };

  /**
   * DÃ©tecte le type de produit basÃ© sur le driverType ou manufacturerName
   */
  static detectProductType(driverType, manufacturerName = '') {
    ...
    // High power plugs
    if (CI.containsCI(dt, '30a') || CI.containsCI(mfr, 'f1bapcit')) {
      return 'plug_high_power';
    }

    // Standard plugs
    if (CI.containsCI(dt, ['plug', 'outlet', 'socket', 'usb'])) {
      return 'plug';
    }

    // Thermostats & Valves
    if (CI.containsCI(dt, ['thermostat', 'radiator', 'valve', 'trv'])) {
      return 'thermostat';
    }

    // Climate sensors
    if (CI.containsCI(dt, ['climate', 'temperature', 'humidity'])) {
      return 'climate_sensor';
    }

    // Motion sensors
    if (CI.containsCI(dt, ['motion', 'presence', 'radar', 'pir'])) {
      return 'motion_sensor';
    }

    // Curtains
    if (CI.containsCI(dt, ['curtain', 'blind', 'cover', 'shutter'])) {
      return 'curtain';
    }

    // Lights & Dimmers
    if (CI.containsCI(dt, ['light', 'bulb', 'dimmer', 'led'])) {
      return 'light';
    }

    // Air quality
    if (CI.containsCI(dt, ['air_quality', 'co2', 'voc'])) {
      return 'air_quality';
    }

    // Soil sensors
    if (CI.containsCI(dt, 'soil')) {
      return 'soil_sensor';
    }

    // Water leak sensors
    if (CI.containsCI(dt, ['water', 'leak', 'flood'])) {
      return 'water_leak';
    }

    // Smoke detectors
    if (CI.containsCI(dt, ['smoke', 'gas', 'co_alarm'])) {
      return 'smoke_detector';
    }

    // Energy meters (3-phase, DIN rail)
    if (CI.containsCI(dt, ['energy_meter', '3phase', 'din_rail'])) {
      return 'energy_meter';
    }
  ...
  /**
   * Valide et corrige une valeur pour une capability donnÃ©e
  ...
  static validateAndCorrect(value, capability, productType, options = {}) {
    ...
    if (!rules) {
      return { isValid: true, correctedValue: value, correction };
    }
    ...
    if (value > rule.max && rule.possibleDivisors) {
      for (const divisor of rule.possibleDivisors.sort((a, b) => b - a)) {
        ...
        const divided =safeDivide(value, divisor);
    ...
    if (value > rule.max && rule.possibleMultipliers) {
      for (const multiplier of rule.possibleMultipliers) {
        const multiplied =safeMultiply(value, multiplier);
        if (multiplied >= rule.min && multiplied <= rule.max) {
          ...
          result.message = `Auto-corrected: ${value} Ã— ${multiplier} = ${multiplied}${rule.unit}`;
          return result;
        }
      }
    }
  ...
  /**
   * DÃ©tecte automatiquement le diviseur correct basÃ© sur les valeurs reÃ§ues
   * Utilise un historique de valeurs pour dÃ©terminer le pattern
   */
  static detectCorrectDivisor(values, capability, productType) {
    if (!values || values.length < 3) return null;

    const rules = this.PRODUCT_RULES[productType]?.[capability];if (!rules) return null;
    ...
    if (bestScore >= safeMultiply(values.length, 0.5)) {
      return bestDivisor;
    }
  ...
  /**
   * CrÃ©e un validateur avec Ã©tat pour un device spÃ©cifique
...
/**
 * Validateur avec Ã©tat pour un device spÃ©cifique
...
class DeviceValueValidator {
  ...
  validate(value, capability) {
    ...
    if (this.learnedDivisors[capability]) {
      const divided = safeDivide(value, this.learnedDivisors[capability]);
      const rules = ProductValueValidator.PRODUCT_RULES[this.productType]?.[capability];if (rules && divided >= rules.min && divided <= rules.max) {
        return {
          isValid: true,
  ...
  /**
   * RÃ©initialise l'apprentissage pour une capability
   */
