...
class ProductValueValidator {
  ...
  static validateAndCorrect(value, capability, productType, options = {}) {
    ...
    // v9.0.364: prefer the SMALLEST divisor landing in typicalRange (least
    ...
    if (value > rule.max && rule.possibleDivisors) {
      ...
      for (const divisor of candidates) {
        ...
        if (!firstInRange) {firstInRange = { divided, divisor };}
      }
      if (firstInRange) {
        result.correctedValue = firstInRange.divided;
        result.correction = 'divisor';
        result.divisorApplied = firstInRange.divisor;
        result.message = `Auto-corrected: ${value} / ${firstInRange.divisor} = ${firstInRange.divided}${rule.unit}`;
        return result;
      }
    }
