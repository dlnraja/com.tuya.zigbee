...
class ProductValueValidator {
  ...
  static PRODUCT_RULES = {
    ...
    climate_sensor: {
      measure_temperature: {
        min: -40, max: 100, // Indoor, outdoor, hot-water probe (GH #513)
        typicalRange: [-30, 85],
        unit: '°C',
        possibleDivisors: [1, 10, 100],
        autoCorrect: true
    ...
    soil_sensor: {
      measure_temperature: { min: -20, max: 70, unit: '°C', typicalRange: [-5, 50], possibleDivisors: [1, 10], autoCorrect: true },
      // v10.7.0: soil moisture was validated NOWHERE — a ×10 report (650)
      // passed through uncorrected (capabilities audit Part 2, gap 3).
      'measure_humidity.soil': { min: 0, max: 100, unit: '%', typicalRange: [0, 100], possibleDivisors: [1, 10], autoCorrect: true },
      measure_humidity: { min: 0, max: 100, unit: '%', typicalRange: [0, 100], possibleDivisors: [1, 10], autoCorrect: true }
    },
    energy_meter: {
  ...
  static detectProductType(driverType, manufacturerName = '') {
    ...
    const mfr = TU.normalize(manufacturerName);
    ...
    // Water leak sensors
    if (dt.includes('water') || dt.includes('leak') || dt.includes('flood')) {
      return 'water_leak';
    }

    // Smoke detectors
    if (dt.includes('smoke') || dt.includes('gas') || dt.includes('co_alarm')) {
      return 'smoke_detector';
    }

    // Energy meters (3-phase, DIN rail)
    if (dt.includes('energy_meter') || dt.includes('3phase') || dt.includes('din_rail')) {
      return 'energy_meter';
    }
  ...
  static validateAndCorrect(value, capability, productType, options = {}) {
    ...
    // v9.0.364: prefer the SMALLEST divisor landing in [min,max] (least
    // aggressive plausible correction). The previous largest-first order
    // turned x10 values (e.g. 215 = 21.5°C) into 2.15 (issue #513).
    // v9.0.493: typicalRange must NOT veto a smaller in-range divisor.
    // Indoor-typical max 50 made raw 530 prefer ÷100 (5.3°C) over ÷10 (53°C)
    // for hot-water probes (GH #513 Finnamu).
    // Also: never sort the rule array in place (mutates shared PRODUCT_RULES).
    // ═══════════════════════════════════════════════════════════════════════
    if (value > rule.max && rule.possibleDivisors) {
      const candidates = rule.possibleDivisors.filter(d => d !== 1).sort((a, b) => a - b);
      for (const divisor of candidates) {
        const divided = value / divisor;
        if (divided < rule.min || divided > rule.max) {continue;}
  ...
  static detectCorrectDivisor(values, capability, productType) {
    ...
    for (const divisor of possibleDivisors) {
      ...
      for (const value of values) {
        const divided = value / divisor;
        if (divided >= rules.min && divided <= rules.max) {
          divisorScores[divisor] += 1;
          if (typicalRange && divided >= typicalRange[0] && divided <= typicalRange[1]) {
            divisorScores[divisor] += 0.25;
          }
        }
      }
    }
...
class DeviceValueValidator {
  ...
  validate(value, capability) {
    ...
    // Check if we have a learned divisor. Never re-apply it when the sample
    // is already inside the valid range (would turn 53°C into 5.3°C).
