...
class ColorEffectManager {
  ...
  async breatheEffect(device, options, state) {
    ...
    do {
      for (let i = 0; i <= steps && state.running; i++) {
        const dim = minBrightness + (maxBrightness - minBrightness) * (i / steps);
  ...
  async fadeEffect(device, options, state) {
    ...
    do {
      for (let i = 0; i <= steps && state.running; i++) {
        const p = i / steps;
        const h = fromColor.hue + (toColor.hue - fromColor.hue) * p;
        const s = fromColor.saturation + (toColor.saturation - fromColor.saturation) * p;
