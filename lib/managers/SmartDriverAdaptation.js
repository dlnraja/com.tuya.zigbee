...
class SmartDriverAdaptation {
  ...
  async analyzeAndAdapt() {
    ...
    try {
      ...
      if (clusterAnalysis.isTuyaDP) {
        ...
        this.log('      ⚠️  Cluster CLUSTERS.TUYA_EF00 not visible - cluster analysis unreliable');
  ...
  async analyzeClusters(deviceInfo) {
    ...
    if (isTuyaDPDevice) {
      ...
      this.log('   ⚠️  Device uses cluster CLUSTERS.TUYA_EF00 (not visible in standard list)');
    ...
    const mappings = {
      ...
      'dim': {
        ...
        setCommand: async (val) => {
          ...
          return await cluster.moveToLevelWithOnOff({ level:Math.round(safeMultiply(val, 254)), transitionTime: 0 });
        }
      },
      'measure_temperature': {
