...
class SmartDriverAdaptation {
  ...
  async analyzeAndAdapt() {
    ...
    try {
      ...
      if (clusterAnalysis.isTuyaDP) {
        this.log('🛡️  [SMART ADAPT] TUYA DP DEVICE - Adaptation disabled');
        this.log(`      Model: ${deviceInfo.modelId || 'Unknown'}`);
        this.log(`      Manufacturer: ${deviceInfo.manufacturer || 'Unknown'}`);
        this.log('      ⚠️  Cluster 0xEF00 not visible - cluster analysis unreliable');
      ...
      if (!migrationCheck.currentDriver || migrationCheck.currentDriver === 'undefined') {
        this.log('⚠️ [SAFE-SWITCH] Cannot determine current driver - skipping migration check');
      ...
      if (!migrationCheck.recommendedDriver || migrationCheck.recommendedDriver === 'undefined') {
        this.log('⚠️ [SAFE-SWITCH] No valid recommended driver - keeping current');
        // v5.5.18: Return valid result instead of undefined
      ...
      if (migrationCheck.shouldMigrate) {
        this.log('⚠️  [SAFE-SWITCH] Driver mismatch detected!');
        ...
        if (switchDecision.action === 'recommended') {
          ...
          if (migrationResult === true || (migrationResult?.queued && !migrationResult?.requiresManualAction)) {
            ...
            this.log('ℹ️  SDK3 limitation: Device must be removed and re-paired');
          } else {
            this.log('⚠️  [SAFE-SWITCH] FAILED - Manual migration required');
            this.log('ℹ️  User action needed: Remove device and re-pair with correct driver');
          }
          ...
          this.log('ℹ️  User explicitly chose this driver - not overriding');
        } else {
          this.log('⚠️  [SAFE-SWITCH] Migration NOT SAFE - keeping current driver');
          this.log(`ℹ️  ${switchDecision.reason}`);
        }
      } else {
      ...
      try {
        ...
        if (this.device.zclNode) {
          ...
          this.log('⚠️  [DATA-COLLECTOR] No ZCL node - skipping data collection setup');
        }
      } catch (err) {
   ...
   * Basé sur: athombv/node-homey-zigbeedriver & node-zigbee-clusters
      ...
      if (info.modelId && info.manufacturer) {
        ...
        if (dbInfo) {
          ...
          if (deprecation.deprecated) {
            this.log('   ⚠️  [DATABASE] Current driver is DEPRECATED!');
          ...
          this.log('   ℹ️  [DATABASE] Device not in database (using detection fallback)');
        }
      }
    ...
    if (this.device.zclNode) {
      ...
      try {
        ...
        if (batteryData.percent !== null || batteryData.voltage !== null) {
          ...
          this.log(`   ℹ️  Battery read: No data (source: ${batteryData.source})`);
          info.battery = null;
        }
      } catch (err) {
        this.error('   ⚠️  Battery read failed:', err.message);
        info.battery = null;
      }
      ...
      try {
        ...
        if (info.powerSource === 'mains') {
          ...
          if (energyData.power !== null || energyData.voltage !== null || energyData.current !== null) {
            ...
            this.log(`   ℹ️  Energy read: No data (source: ${energyData.source})`);
            info.energy = null;
          }
        }
      } catch (err) {
        this.error('   ⚠️  Energy read failed:', err.message);
        info.energy = null;
      }
    }
  ...
  async analyzeClusters(deviceInfo) {
    ...
    if (override) {
      ...
      if (override.preventAdaptation) {
        this.log('   🛡️  preventAdaptation = true → Skipping analysis');
    ...
    if (isTuyaDPDevice) {
      this.log('   ⚠️  TUYA DP DEVICE DETECTED (TS0601 / _TZE*)');
      this.log('   ⚠️  Cluster analysis will be UNRELIABLE!');
      this.log('   ⚠️  Device uses cluster 0xEF00 (not visible in standard list)');
      this.log('   ℹ️  Will trust current driver instead of cluster analysis');
      ...
      if (currentDriver.includes('climate') || currentDriver.includes('sensor') || currentDriver.includes('soil')) {
        ...
        this.log('   ⚠️  Unknown Tuya DP device - trusting current driver');
      }
    ...
    const isUsbOutlet = (
      (deviceInfo.modelId && usbOutletProductIds.some(id => deviceInfo.modelId.toLowerCase().includes(id.toLowerCase()))) ||
      (deviceInfo.manufacturer && usbOutletManufacturers.some(id => deviceInfo.manufacturer.toLowerCase().includes(id.toLowerCase()))) ||
    ...
    if (isUsbOutlet) {
      ...
      // 🔌 DÉTECTION MULTI-ENDPOINT (using helper)
    ...
    // Switch/Outlet detection (ONLY if NOT a button!)
      ...
      if (hasMultipleOnOffEndpoints || endpointCount >= 2) {
        ...
        this.log('   ℹ️  Should use switch_basic_2gang or outlet_2gang driver');
      } else {
        analysis.subType = '1gang';
      }
  ...
  async adaptDriver(comparison, realCapabilities) {
    ...
    for (const cap of comparison.incorrect) {
      try {
        if (this.device.hasCapability(cap)) {
          // Safe Listening Gating Check
          if (this.device.virtualTelemetryCompensationEngine) {
            if (!this.device.virtualTelemetryCompensationEngine.gateAdaptation(cap, 'REMOVE')) {
              this.log(`      🛑 GATED: Postponing removal of capability ${cap} until safe epoch completed.`);
              continue;
            }
          }
    ...
    for (const cap of comparison.missing) {
      // Safe Listening Gating Check
      if (this.device.virtualTelemetryCompensationEngine) {
        if (!this.device.virtualTelemetryCompensationEngine.gateAdaptation(cap, 'ADD')) {
          this.log(`      🛑 GATED: Postponing addition of capability ${cap} until safe epoch completed.`);
          continue;
        }
      }
