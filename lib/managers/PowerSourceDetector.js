...
class PowerSourceDetector {
  ...
  static isPowered(device) {
    ...
    const isPowered = poweredPatterns.some(pattern =>
      normalizeCI(modelId).includes(normalizeCI(pattern)) ||
      normalizeCI(productId).includes(normalizeCI(pattern))
    );
  ...
  static getDeviceType(device) {
    const data = device.getData() || {};
    const modelId = normalizeCI(data.modelId);
    const productId = normalizeCI(data.productId);
