...
class VirtualEnergyEstimator {
  ...
  async _setEstimated(capability, value) {
    if (!this.device || value === undefined) {return;}
    const smart = this.device.smartEnergy;
    if (smart?.isRealCapability?.(capability)) {return;}
    ...
    } else if (typeof this.device.setCapabilityValue === 'function') {
    ...
    try {
      ...
      await DeviceTelemetryEstimator.record(this.device, capability, value, {
        source: 'estimated',
        via: 'virtual-energy-estimator',
      });
    } catch (_e) { /* optional */ }
  }
  ...
  async _ensureCapabilities() {
    try {
      // Wait for 5-min real-energy audit when SmartEnergyManager is present
      const smart = this.device.smartEnergy;
      if (smart && typeof smart.isAuditComplete === 'function' && !smart.isAuditComplete()) {
        this.device.log('[VIRTUAL-ENERGY] Defer capability expose until 5-min energy audit completes');
        return;
      }
      if (!this.device.hasCapability('measure_power')) {
        this.device.log('[VIRTUAL-ENERGY] Dynamically registering measure_power (estimated)');
        await this.device.addCapability('measure_power').catch(() => {});
      }
      if (!this.device.hasCapability('meter_power')) {
        this.device.log('[VIRTUAL-ENERGY] Dynamically registering meter_power (estimated)');
        await this.device.addCapability('meter_power').catch(() => {});
      }
      if (!this.device.hasCapability('measure_voltage')) {
        this.device.log('[VIRTUAL-ENERGY] Dynamically registering measure_voltage (estimated)');
        await this.device.addCapability('measure_voltage').catch(() => {});
      }
      await this._setEstimated('measure_voltage', this._nominalVoltage);
    } catch (e) {
  ...
  _handleStateChange(gang, isOn) {
    ...
    if (isOn) {
      ...
      if (this._activeFeatures.has(gang)) {
        ...
        this._setEstimated('meter_power', Math.round(this._virtualEnergy * 1000) / 1000).catch(() => {});
      }
    }
  ...
  _updateVirtualPowerAndCurrent() {
    ...
    // Update capabilities
    this._setEstimated('measure_power', Math.round(activePower * 10) / 10).catch(() => {});

    if (this.device.hasCapability('measure_current')) {
      this._setEstimated('measure_current', Math.round(currentAmps * 100) / 100).catch(() => {});
