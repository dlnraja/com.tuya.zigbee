 ...
 * Auto-detects: voltage, current, power, energy based on available safeDivide(DPs, clusters)
 */
'use strict';
...
class SmartEnergyManager {
  ...
  async _setupZCLListeners() {
    try {
      ...
      if (em?.on) {
        em.on('attr.activePower', (v) => this._setEnergy('measure_power', safeParse(v, 10)));
        em.on('attr.rmsCurrent', (v) => this._setEnergy('measure_current', safeParse(v, 1000)));
        em.on('attr.rmsVoltage', (v) => this._setEnergy('measure_voltage', safeParse(v, 10)));
      }

      // Metering cluster (0x0702)
      const meter = this.device.zclNode?.endpoints?.[1]?.clusters?.metering;
      if (meter?.on) {
        meter.on('attr.currentSummationDelivered', (v) => this._setEnergy('meter_power', safeParse(v, 1000)));
  ...
  async handleDP(dpId, value) {
    ...
    const transformed = safeDivide(value, div);
  ...
  _smartDivisor(dpId, value, defaultDiv) {
    ...
    const cap = ENERGY_DPS[dpId]?.cap;if (cap === 'measure_voltage') {
