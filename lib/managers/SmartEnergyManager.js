...
class SmartEnergyManager {
  ...
  _startAuditWindow() {
    ...
    this._auditComplete = false;
    const timerApi = this.device.homey?.setTimeout?.bind(this.device.homey) || setTimeout;
    if (this._auditTimer) {
      try { (this.device.homey?.clearTimeout || clearTimeout)(this._auditTimer); } catch (_e) { /* noop */ }
    }
    this._auditTimer = timerApi(() => {
      this._auditTimer = null;
