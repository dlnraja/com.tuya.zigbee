...
/**
 * SmartEnergyManager — measured vs estimated energy discovery (P108 + P2556 provenance)
 *
 * Lifecycle:
 *  1. Init: predict candidate energy caps (driver/class/ZCL/DP heuristics).
 *  2. First 5 minutes: observe real DP/ZCL reports; mark telemetry_*_origin=measured.
 *  3. After audit: keep caps that reported; allow estimators only for silent caps
 *     on mains devices; never overwrite measured with estimated/calculated.
 *  4. Late advertising: if a real value arrives after audit, promote to measured
...
class SmartEnergyManager {
  ...
  async init() {
    ...
    // WHY(P2672): resolve metered vs approximate vs battery BEFORE ZCL energy bind —
    // Contre quoi: UniversalEnergyHandler storm + phantom Energy UI on non-metered.
    try {
      const IntelligentEnergyAdapter = require('../energy/IntelligentEnergyAdapter');
      const profile = await IntelligentEnergyAdapter.apply(this.device);
      this._energyProfile = profile;
      if (profile?.skipZclEnergyBind) {
        this.device.log(`[ENERGY] P2672 skip UniversalEnergyHandler bind (mode=${profile.mode})`);
      } else {
      ...
      this.device.log(`[ENERGY] P2672 adapter soft-fail: ${e.message}`);
      this.universalHandler = new UniversalEnergyHandler(this.device);
      await this.universalHandler.init().catch((err) => {
        this.device.log('[ENERGY] Universal Handler init failed:', err.message);
      });
    }
  ...
  async _predictCandidateCapabilities() {
    ...
    // WHY(P2672): honor IntelligentEnergyAdapter mode for all device classes
    const mode = this._energyProfile?.mode || d._energyMode || d.getStoreValue?.('energy_mode');
    if (mode && mode !== 'metered' && mode !== 'unknown') {
      this._candidateCaps.clear();
      this.device.log(`[ENERGY] P2672 mode=${mode} — clear energy candidates (no phantom meter)`);
      await this._setStore('energy_candidates', []);
  ...
  async _finalizeAudit() {
    ...
    for (const cap of silent) {
      const origin = DataProvenance.normalizeOrigin(this._getStore(`telemetry_${cap}_origin`, null))
        || DataProvenance.normalizeOrigin(this._getStore(`telemetry_${cap}_source`, null));
      if (!DataProvenance.isMeasured(origin)) {
        try {
          await DataProvenance.stampProvenance(this.device, cap, 'estimated', {
            reason: 'audit-silent-5min',
            applyUx: true,
          });
        } catch (_e) {
          await this._setStore(`telemetry_${cap}_source`, 'estimated');
          await this._setStore(`telemetry_${cap}_origin`, 'estimated');
  ...
  async _setEnergy(capability, value, meta = {}) {
    ...
    // Distinguish measured from estimated/calculated for Flow + estimators (P2556)
    try {
      ...
      try {
        await DataProvenance.stampProvenance(this.device, cap, 'measured', {
          reason: meta.via || 'smart-energy',
          applyUx: true,
        });
      } catch (_e2) { /* soft */ }
    }
  ...
  isRealCapability(capability) {
    ...
    const origin = DataProvenance.normalizeOrigin(this._getStore(`telemetry_${cap}_origin`, null))
      || DataProvenance.normalizeOrigin(this._getStore(`telemetry_${cap}_source`, null));
    return DataProvenance.isMeasured(origin);
  }
