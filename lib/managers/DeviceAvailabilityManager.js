...
class DeviceAvailabilityManager extends EventEmitter {
  ...
  markSeen(deviceId) {
    ...
    if (entry.unavailableSince !== null) {
      ...
      this.emit('back_online', { deviceId, name: entry.name, minutes_silent: silentMinutes, is_battery: entry.isBattery });
    }
  }
  ...
  _evaluate() {
    ...
    for (const [deviceId, entry] of this._devices) {
      ...
      if (silentMs > this._timeoutFor(entry)) {
        ...
        this.emit('unavailable', { deviceId, name: entry.name, minutes_silent: silentMinutes, is_battery: entry.isBattery });
      }
    }
  }
