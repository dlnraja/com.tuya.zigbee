...
class AutonomousMigrationManager {
  ...
  async _analyzeDevice() {
    ...
    const analysis = {
      modelId: data.modelId || data.zb_model_id || '',
    ...
    // Detect Tuya DP
    if (analysis.modelId?.toUpperCase().startsWith('TS0601') ||
      analysis.manufacturer?.toUpperCase().startsWith('_TZE')) {
    ...
    if (analysis.modelId === 'TS0002' && normalizeCI(analysis.manufacturer) === '_tz3000_h1ipgkwn') {
  ...
  async _getRecommendation(analysis) {
    ...
    // P2431: Never convert radar presence sensors to other drivers (especially curtain/cover/switch)
    if (/presence|radar|mmwave/i.test(currentDriver)) {
      this.device.log(`⚠️ [AUTONOMOUS] BLOCKED: Radar presence driver (${currentDriver}) is protected from adaptation`);
      recommended = null;
    }

    // P2431: Never convert sensors to covers/curtains
    if (/sensor|radar|motion|contact|climate/i.test(currentDriver) && /curtain|cover|blind|shutter/i.test(recommended)) {
      this.device.log(`⚠️ [AUTONOMOUS] BLOCKED: Will not convert sensor driver (${currentDriver}) to cover (${recommended})`);
      recommended = null;
    }
  ...
  _registerOnOffListener() {
    ...
    } catch (err) { this.device.log('[AUTONOMOUS] Switch listener registration failed:', err.message); }
  }
  ...
  _registerSensorListener() {
    ...
    } catch (err) { this.device.log('[AUTONOMOUS] Sensor listener registration failed:', err.message); }
  }
  ...
  _registerClimateListener() {
    ...
    } catch (err) { this.device.log('[AUTONOMOUS] Climate listener registration failed:', err.message); }
  }
  ...
  _registerEnergyListener() {
    ...
    } catch (err) { this.device.log('[AUTONOMOUS] Energy listener registration failed:', err.message); }
  }
  ...
  _registerBatteryListener() {
    ...
    } catch (err) { this.device.log('[AUTONOMOUS] Battery listener registration failed:', err.message); }
  }
