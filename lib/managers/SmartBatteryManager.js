...
class SmartBatteryManager {
  ...
  /**
   * L14 commit gate (P205 / P2545g) — never bypass SanityFilter.
   * WHY: syntax-check l14-capability-writers-gate requires safeSetCapabilityValue + async _safeSet(.
   * Contre quoi: raw setCapabilityValue floods flows / invents UI battery.
   */
  async _safeSet(capabilityId, value, meta = {}) {
    if (!this.device || value === undefined) {return;}
    const cap = this._resolveCapabilityId(capabilityId);
    if (!cap) {return;}

    if (cap === 'measure_battery' || capabilityId === 'measure_battery') {
      try {
        const { commitBatteryPercent } = require('../battery/MultiProtocolBatteryPercent');
        const r = await commitBatteryPercent(this.device, value, {
          protocol: meta.source || meta.protocol || 'zcl',
    ...
    if (typeof this.device.safeSetCapabilityValue === 'function') {
      await this.device.safeSetCapabilityValue(cap, value, {
        source: meta.source || meta.protocol || 'battery',
        confidence: meta.confidence,
      }).catch(() => {});
      return;
    }
  ...
  _hasCap(capabilityId) {
    const cap = this._resolveCapabilityId(capabilityId);
    if (!cap || !this.device) {return false;}
    return typeof this.device.hasCapability === 'function' ? this.device.hasCapability(cap) : false;
  }
  ...
  async _adaptCapabilities(hasZcl, hasIas, isMains, isKinetic) {
    try {
      ...
      if (isMains || isKinetic) {
        ...
        if (typeof this.device.setEnergy === 'function') {
          await this.device.setEnergy({ batteries: null, mains: true }).catch(() => {});
        }
        return;
      }
  ...
  async setBattery(percent, meta = {}) {
    ...
    if (this._hasCap('measure_battery')) {
      await this._safeSet('measure_battery', percent, meta);
    }
  ...
  async setAlarmBattery(alarmVal) {
    ...
    // P115/P2545g: do NOT invent fake 10%/100% from alarm alone — leave ? until real sample
