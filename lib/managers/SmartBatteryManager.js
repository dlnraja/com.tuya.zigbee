...
class SmartBatteryManager {
  ...
  async _adaptCapabilities(hasZcl, hasIas, isMains, isKinetic) {
    try {
      ...
      // v5.8.72: Only remove for purely kinetic. For mains devices, we keep measure_battery 
      // to support hybrid usage (e.g. battery backup on sirens or solar panels on AC valves).
      if (isKinetic) {
  ...
  async handleDP(dpId, value) {
    ...
    // v5.12.1 / v5.8.72: We DO NOT ignore battery DPs for mains-powered devices anymore. 
    // Many hybrid devices (like sirens with battery backup or solar valves) need dynamic 
    // battery updates even when operating on mains power.
    if (this._isKineticDevice()) {
      return false;
    }
