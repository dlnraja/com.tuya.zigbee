...
class CircadianEngine extends EventEmitter {
  ...
  async applyNow() {
    ...
    let applied = 0;
    // P2564 — large jumps use Soft Daylight Fade when app is available
    let SoftDaylightFade = null;
    try { SoftDaylightFade = require('../features/SoftDaylightFade'); } catch (_e) { /* */ }
    ...
    for (const [, entry] of this._devices) {
      ...
      try {
        ...
        const jumpK = entry.currentKelvin != null ? Math.abs(targets.kelvin - entry.currentKelvin) : 0;
        if (SoftDaylightFade && app && jumpK > 600) {
          SoftDaylightFade.startOnDevice(app, entry.device, {
            minutes: 3,
            steps: 8,
            forceOn: false,
            toDim: targets.bright,
            toTemperature: targets.temperature,
          });
          entry.currentKelvin = targets.kelvin;
          entry.currentBright = targets.bright;
        } else {
