...
/**
 * VirtualEnergyManager — simulates energy monitoring when no metering chip.
 * Prefer VirtualEnergyMeterMixin on device classes; this manager is the older path.
 * All capability writes go through safeSetCapabilityValue (L14) when available.
 * Never overwrites SmartEnergy "real" capabilities.
 */
class VirtualEnergyManager {
  ...
  startAccumulation() {
    ...
    this.accumulationInterval = safeSetInterval(this.device, () => this.accumulate(), 60000);
  }

  stopAccumulation() {
    safeClearInterval(this.device, this.accumulationInterval);
    this.accumulationInterval = null;
  }
  ...
  async _safeSet(capabilityId, value) {
    ...
    // WHY(P2557): nominal W = estimated; kWh integral / I from P÷V = calculated
    const origin = cap === 'meter_power' || cap === 'measure_current'
      ? 'calculated'
      : 'estimated';

    if (typeof this.device.safeSetCapabilityValue === 'function') {
      await this.device.safeSetCapabilityValue(cap, value, { source: origin, origin }).catch(() => {});
    ...
    try {
      ...
      await DeviceTelemetryEstimator.record(this.device, cap, value, {
        source: origin,
        origin,
        via: `virtual-energy-manager:${origin}`,
      });
    } catch (_e) { /* optional */ }
  }
