 ...
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  WHY (P215 / P2269): anti double-division — AdaptiveDataParser then mapping ║
 * ║  Pourquoi: OEM temp/humidity scales differ (x10 vs x100).                   ║
 * ║  Comment: range detect + cache by mfr+dpId + VALID_RANGES.                  ║
 * ║  Pour qui: Homey measure_* capabilities.                                    ║
 * ║  Quand: every DP/ZCL scale before safeSetCapabilityValue.                   ║
 * ║  Contre quoi: hardcoded /10|/100 twice → check:double-division.             ║
...
const KNOWN_DIVISORS = {
  ...
  temperature: {
    ...
    '_TZE284_hodyryli': { dp: { 1: 10, 38: 10 } }, // ZT08 / GH #513 probe
  },
  ...
  humidity: {
    ...
    '_TZE284_hodyryli': { dp: { 2: 1 } },    // ZT08 humidity is 0-100 raw (Z2M)
  },
...
/**
 * Persist a learned energy/sensor divisor (e.g. EnergyJumpGuard sticky correction).
 * WHY: Jump guard sees parsed kWh jumps; SmartDivisor owns the SSOT cache for next parses.
 * @param {string} manufacturerName
 * @param {number|string} dpId
 * @param {string} capability
 * @param {number} divisor
 * @returns {boolean}
 */
function rememberLearnedDivisor(manufacturerName, dpId, capability, divisor) {
  if (typeof divisor !== 'number' || !isFinite(divisor) || divisor < 1) {return false;}
...
/**
 * Read learned divisor if present (tests / EnergyJumpGuard introspection).
 */
function getLearnedDivisor(manufacturerName, dpId, capability) {
  const cacheKey = `${manufacturerName || ''}|${dpId}|${capability || ''}`;
  return LEARNED_DIVISORS.has(cacheKey) ? LEARNED_DIVISORS.get(cacheKey) : null;
}
