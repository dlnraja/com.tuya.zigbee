...
class EnergyManager {
  ...
  async _safeSet(capabilityId, value) {
    ...
    if (typeof this.device.hasCapability === 'function' && !this.device.hasCapability(cap)) {return;}

    // Real metering path — prefer SmartEnergyManager so 5-min audit + telemetry source=direct
    const smart = this.device.smartEnergy;
    if (smart && typeof smart.reportMeasured === 'function'
      && /^(measure_power|measure_voltage|measure_current|meter_power)$/i.test(cap)) {
      await smart.reportMeasured(cap, value, 'energy-manager').catch(() => {});
      return;
    }
  ...
  async updatePower(power) {
    ...
    if (this.device.hasCapability('measure_power')) {
      await this._safeSet('measure_power', parseFloat(power));
    }
  ...
  async updateVoltage(voltage) {
    ...
    if (this.device.hasCapability('measure_voltage')) {
      await this._safeSet('measure_voltage', parseFloat(voltage));
    }
  ...
  async updateCurrent(current) {
    ...
    if (this.device.hasCapability('measure_current')) {
      await this._safeSet('measure_current', parseFloat(current));
    }
  ...
  async updateEnergy(energy) {
    ...
    if (this.device.hasCapability('meter_power')) {
      await this._safeSet('meter_power', parseFloat(energy));
    }
  ...
  _scheduleDailyReset() {
    if (this._dailyResetTimer) {
      this.homey.clearTimeout(this._dailyResetTimer);
    }
    ...
    this._dailyResetTimer = this.homey.setTimeout(() => {
      if (this._destroyed) {return;}
