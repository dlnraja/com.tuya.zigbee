...
class EnergyManager {
  ...
  async _checkVoltageAnomaly(voltage) {
    const tolerance = safeMultiply(this.options.nominalVoltage, safeDivide(this.options.voltageTolerancePercent, 100));
  ...
  getStatistics() {
    ...
    return {
      ...
      averages: {
        dailyAvgPower: dailyDuration > 0 ? safeMultiply(safeDivide(this.state.dailyEnergy, dailyDuration), 1000) : 0,
        sessionAvgPower: sessionDuration > 0 ? safeMultiply(safeDivide(this.state.sessionEnergy, sessionDuration), 1000) : 0
      },
