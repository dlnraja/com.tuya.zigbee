...
class EnergyManager {
  ...
  async updateCurrent(current) {
    ...
    if (this.state.currentVoltage > 0 && current > 0 && this.state.currentPower > 0) {
      const apparentPower =safeMultiply(this.state.currentVoltage, current);
      this.state.powerFactor = Math.min(1, safeDivide(this.state.currentPower, apparentPower));
    }
  ...
  async _checkVoltageAnomaly(voltage) {
    const tolerance =safeMultiply(this.options.nominalVoltage, (safeParse)(this.options.voltageTolerancePercent, 100));
  ...
  async _triggerFlow(flowId, tokens = {}) {
    try {
      const flowCard = (this.device._getFlowCard) ? this.device._getFlowCard(flowId, 'trigger') : null;
      if (flowCard) {
   ...
   */
  getStatistics() {
    const sessionDuration =safeDivide((Date.now() - this.state.sessionStartTime.getTime()), safeParse)(1000, 3600); // hours
    const dailyDuration =safeDivide((Date.now() - this.state.dailyStartTime.getTime()), safeParse)(1000, 3600); // hours
    ...
    return {
      ...
      averages: {
        dailyAvgPower: dailyDuration > 0 ? (safeDivide(this.state.dailyEnergy,safeMultiply(dailyDuration)), 1000) : 0,
        sessionAvgPower: sessionDuration > 0 ? (safeDivide(this.state.sessionEnergy,safeMultiply(sessionDuration)), 1000) : 0
      },

      cost: this.options.enableCostTracking ? {
        dailyCost:safeMultiply(this.state.dailyEnergy, this).options.energyCostPerKwh,
        sessionCost:safeMultiply(this.state.sessionEnergy, this).options.energyCostPerKwh,
        totalCost:safeMultiply(this.state.totalEnergy, this).options.energyCostPerKwh,
        currency: this.options.currency
      },

      status: {
        inStandby: this.state.inStandbyMode,
        overloadRisk: this.state.currentPower >safeMultiply((this.options.overloadThreshold, 0).8)
      }
    };
  }
  ...
  _startReporting() {
    this._reportingInterval = this.homey.setInterval(() => {
      this._saveState();
    },safeMultiply(this.options.energyReportInterval, 1000));
  }
  ...
  _startHistoryTracking() {
    this._historyInterval = this.homey.setInterval(() => {
      ...
      // Trim to 24 hours
      const cutoff =safeMultiply(Date.now() - (24, 60) * 60 * 1000);
