...
const SAFE_CLUSTERS = {
  0x0001: { cap: 'measure_battery', attr: 'batteryPercentageRemaining', xform: v => normalizeZclBatteryPercent(v) ?? 0 },
  ...
  // v9.0.87: Universal Zigbee Engine — additional clusters for non-Tuya devices
  0x040B: { cap: 'measure_pm1', attr: 'measuredValue', xform: v => v }, // PM1.0 (Aqara, IKEA VINDSTYRKA)
  0x040E: { cap: 'measure_voc', attr: 'measuredValue', xform: v => v }, // VOC (Aqara, IKEA air quality)
  0x040F: { cap: 'measure_so2', attr: 'measuredValue', xform: v => v }, // SO2 (industrial sensors)
  0x0203: { cap: 'thermostat_mode', attr: 'systemMode', xform: v => v }, // Dehumidifier
...
const CLUSTER_NAMES = {
  ...
  // v9.0.87: Universal Zigbee Engine — additional cluster names
  0x040B: ['pm1Concentration'],
  0x040E: ['vocConcentration'],
  0x040F: ['so2Concentration'],
  0x0203: ['dehumidifierControl'],
  0x0500: ['iasZone'],
  0x0502: ['iasWd'],
...
const CLUSTER_CAPABILITY_FALLBACK = {
  ...
  0x0001: { caps: ['measure_battery', 'alarm_battery'], attrs: ['batteryPercentageRemaining', 'batteryVoltage'], xforms: [v => normalizeZclBatteryPercent(v) ?? 0, v => v / 100] },
...
class DynamicCapabilityManager {
  ...
  async initialize() {
    ...
    try {
      ...
      // WHY(P2386 / VicHY #2222): never restore curtain/dim caps onto presence radars after update
      const driverId = this.device.driver?.manifest?.id || this.device.driver?.id || '';
      const stripCurtain = /presence|radar|mmwave|occupancy/i.test(driverId);
      for (const c of stored) {
        if (stripCurtain && /windowcoverings|^(dim|target_temperature|thermostat_mode)$/.test(c)) {
          continue;
        }
        this._added.add(c);
      }
  ...
  _isIrrelevantCap(cap) {
    ...
    // WHY (P2235): honor device-declared forbidden list (ButtonDevice strips onoff)
    if (Array.isArray(forbidden) && forbidden.includes(cap)) {return true;}

    // WHY (f647d35b / P2235): remotes + SOS must never gain switch/climate phantoms from DCM-FB
    // Carve-out: real actuators that share button_wireless_* prefix (plug/switch/usb/valve/fingerbot)
    ...
    if (isRemoteButton) {
      // WHY(P2299/f647d35b): DCM-FB also invented alarm_battery beside measure_battery
      // → SDK3 UI conflict on sleepy remotes.
    ...
    // WHY (P2243 Toni 3a1f196d): DIN/RCBO/power meters never measure climate —
    // leftover EF00 DP6 was mapped to humidity when Tongou sat on smart_rcbo.
    ...
    // WHY(P2379 / VicHY): mmWave presence — never invent curtain/dim from radar setting DPs
    if (/presence|motion_sensor|radar|mmwave|pir_mmwave|occupancy/i.test(driverId) || cls === 'sensor') {
      ...
      // WHY(P2477 / VicHY c5165a37): ZCL genPowerCfg (0x0001) re-injects measure_battery
      // on mains MTG even after compose dropped energy.batteries — block when mains.
  ...
  /**
   * P2477 — mains mmWave (clrdrnya / MTG) must never gain ZCL battery from DCM scan.
   * @returns {boolean}
   */
  ...
  async auditCapabilities() {
    ...
    const allDeviceClusters = new Set();
