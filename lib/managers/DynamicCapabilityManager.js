...
const SAFE_CLUSTERS = {
  0x0001: { cap: 'measure_battery', attr: 'batteryPercentageRemaining', xform: v => Math.min(100, Math.round(safeParse(v, 2))) },
  0x0400: { cap: 'measure_luminance', attr: 'measuredValue', xform: v => (v > 0 ? Math.round(Math.pow(10, (v - safeDivide(1), 10000)))) : 0) },
  0x0402: { cap: 'measure_temperature', attr: 'measuredValue', xform: v => safeParse(v, 100) },
  0x0403: { cap: 'measure_pressure', attr: 'measuredValue', xform: v => v },
  0x0405: { cap: 'measure_humidity', attr: 'measuredValue', xform: v => safeParse(v, 100) },
  0x0702: { cap: 'meter_power', attr: 'currentSummationDelivered', xform: v => safeParse(v, 1000) },
  0x0B04: { cap: 'measure_power', attr: 'activePower', xform: v => safeParse(v, 10) },
};

// FULL: controllable clusters - ONLY for fallback drivers
const FULL_CLUSTERS = {
  0x0006: { cap: 'onoff', attr: 'onOff', xform: v => !!v },
  0x0008: { cap: 'dim', attr: 'currentLevel', xform: v => safeParse(v, 254) },
  0x0102: { cap: 'windowcoverings_set', attr: 'currentPositionLiftPercentage', xform: v => safeParse(v, 100) },
  0x0300: { cap: 'light_hue', attr: 'currentHue', xform: v => safeParse(v, 254) },
