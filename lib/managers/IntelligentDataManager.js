...
class IntelligentDataManager extends EventEmitter {
  ...
  constructor(device) {
    ...
    this.config = {
      ...
      intervals: {
        ...
        // Motion/Contact
        motion: { min: 1, max: 300, change: 1 },        // 1s-5min
        contact: { min: 1, max: 300, change: 1 },       // 1s-5min
  ...
  scheduleRegularTimeSync() {
    ...
    this.device.log(`[DATA_MGR] Next time sync in ${Math.round(msUntil3AM / 1000 / 3600)}h`);
  ...
  async configureIntelligentReporting() {
    ...
    if (endpoint.clusters?.genPowerCfg || endpoint.clusters?.[1]) {
      ...
      reportingConfigs.push({
        ...
        minChange: safeMultiply(Math.round(config.change), 2) // ZCL uses 0.5% units
      });
    }
