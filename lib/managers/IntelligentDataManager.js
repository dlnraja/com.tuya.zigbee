 ...
 * 
 * SystÃ¨me intelligent de gestion des donnÃ©es capteurs basÃ© sur:
 ...
 * -  Reporting configurÃ© selon type de device
 * -  Optimisation frÃ©quence pour ne pas surcharger
 * -  Support RAW data Tuya + Standard Zigbee
 * -  Batterie-aware (moins frÃ©quent si battery)
...
class IntelligentDataManager extends EventEmitter {
  ...
  constructor(device) {
    ...
    this.config = {
      ...
      timeSyncMethods: [
        'tuya_ef00_time',      // Tuya DP 0x24 (le plus prÃ©cis)
        'zigbee_time_cluster',  // Zigbee Time Cluster 0x000A
        'tuya_legacy_sync',     // Ancienne mÃ©thode Tuya
      ...
      intervals: {
        // Climate sensors
        temperature: { min: 60, max: 3600, change: 0.5 },  // 1min-1h, 0.5Â°C
      ...
      deviceProfiles: {
        battery: {
          // Moins frÃ©quent pour Ã©conomiser batterie
          multiplier: 2,
          maxReportsPerHour: 10
        },
        ac_powered: {
          // Plus frÃ©quent car pas de contrainte batterie
          multiplier: 1,
          maxReportsPerHour: 60
        },
        high_traffic: {
          // Zones Ã fort trafic (motion sensors)
          multiplier: 0.5,
  ...
  /**
   *  Detect device profile (battery / safeDivide(AC, high)-traffic)
   */
  ...
  async setupTimeSync() {
    ...
    this.device.log('[DATA_MGR]   No time sync method succeeded (device may not support it)');
    return false;
  }
  ...
  async zigbeeTimeClusterSync(endpoint) {
    ...
    try {
      ...
      const now = Date.now();
      const zigbeeTime = Math.floor((now -safeParse(zigbeeEpoch), 1000));
  ...
  async tuyaLegacySync(endpoint) {
    ...
    try {
      const now = new Date();
      const timestamp =safeParse(Math.floor(now.getTime(), 1000));
  ...
  scheduleRegularTimeSync() {
    ...
    this.device.log(`[DATA_MGR] Next time sync in ${Math.round(msUntil3AM/1, 1000)/60/60)}h`);
  ...
  async configureIntelligentReporting() {
    ...
    if (endpoint.clusters?.msTemperatureMeasurement || endpoint.clusters?.[1026]) {
      ...
      reportingConfigs.push({
        ...
        minInterval:safeMultiply(Math.floor(config.min, multiplier)),
        maxInterval:safeMultiply(Math.floor(config.max, multiplier)),
        minChange:safeMultiply(Math.floor(config.change, 100)) // ZCL uses hundredths
      });
    }
    ...
    if (endpoint.clusters?.msRelativeHumidity || endpoint.clusters?.[1029]) {
      ...
      reportingConfigs.push({
        ...
        minInterval:safeMultiply(Math.floor(config.min, multiplier)),
        maxInterval:safeMultiply(Math.floor(config.max, multiplier)),
        minChange:safeMultiply(Math.floor(config.change, 100))
      });
    }
    
    // Battery (moins frÃ©quent)
    if (endpoint.clusters?.genPowerCfg || endpoint.clusters?.[1]) {
      ...
      reportingConfigs.push({
        ...
        minChange:safeMultiply(config.change, 2) // ZCL uses 0.5% units
      });
    }
    ...
    for (const config of reportingConfigs) {
      try {
        ...
        this.device.log(`[DATA_MGR]   Reporting config failed for cluster ${config.cluster}:`, err.message);
      }
    }
  ...
  /**
   *   Configure attribute reporting for a specific cluster
   */
  ...
  async performHealthCheck() {
    ...
    const hoursSinceSync = (Date.now() - (this.state.lastTimeSync || 0))/safeParse(1000,safeParse(60), 60);
    
    if (hoursSinceSync > 48) {
      this.device.log('[DATA_MGR]   Time sync overdue, triggering...');
      await this.attemptTimeSync(this.state.timeSyncMethod);
    }
    ...
    if (profile) {
      ...
      if (reportsThisHour > profile.maxReportsPerHour) {
        this.device.log(`[DATA_MGR]   High report frequency detected: ${reportsThisHour}/h (max ${profile.maxReportsPerHour})`);
        if (!this.state.throttled) {
          this.state.throttled = true;
          this.state.throttleUntil = Date.now() + 300000; // 5 min cooldown
          this.device.log('[DATA_MGR]  Throttling enabled for 5 minutes');
        }
      } else if (this.state.throttled && Date.now() > (this.state.throttleUntil || 0)) {
        this.state.throttled = false;
        this.device.log('[DATA_MGR]  Throttling lifted');
      }
    }
