...
class IntelligentDataManager extends EventEmitter {
  ...
  async zigbeeTimeClusterSync(endpoint) {
    ...
    try {
      ...
      if (!shouldProactiveTimeSync(this.device)) {
        this.device.log('[DATA_MGR] Skip proactive 0x000A (sleepy / no clock)');
        return false;
      }
  ...
  scheduleRegularTimeSync() {
    ...
    this.timers.timeSync = this.homey.setTimeout(async () => {
  ...
  setupHealthMonitoring() {
    ...
    // Check every hour
    this.timers.healthCheck = this.homey.setInterval(() => { if (this._destroyed) {return;} this.performHealthCheck(); }, 3600000); // 1h
