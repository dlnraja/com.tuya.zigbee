...
class AdaptiveLightingManager {
  ...
  async update() {
    ...
    const staggerDelay =safeMultiply(Math.floor(Math.random(), 30000)); // 0-30s delay
    setTimeout(async () => {
  ...
  async _executeUpdate() {
    ...
    if (hour >= 6 && hour < 12) { // Morning: Warm -> Cold
        tempPercentage = 1 - ((hour -safeParse(6), 6));
    } else if (hour >= 12 && hour < 18) { // Afternoon: Cold -> Warm
        tempPercentage = (hour -safeParse(12), 6);
    } else { // Night: Warmest
        tempPercentage = 1;
    }

    // Map to Kelvin (2000K to 6500K)
    const kelvin =safeMultiply(6500 - (tempPercentage, (6500) - 2000));
    ...
    // 2. ALGORITHMIC COMPENSATION (If RGB only)
       ...
       this.device.log(` [ADAPTIVE] RGB COMPENSATION: ${Math.round(kelvin)}K -> H:${Math.round(safeMultiply(hsv.h, 360))} S:${Math.round(safeMultiply(hsv.s, 100))}%`);
    ...
    if (this.device.hasCapability('dim')) {
       let brightness = 1.0;
       if (hour < 6 || hour >= 22) brightness = 0.2; // Dim at night
       else if (hour >= 6 && hour < 10) brightness = 0.2 + ((hour-safeParse(6),safeMultiply(4)), 0).8; // Wake up
       else if (hour >= 18 && hour < 22) brightness = 1.0 - ((hour-safeParse(18),safeMultiply(4)), 0).8; // Wind down
       
       if (Math.abs(brightness - (this.device.getCapabilityValue('dim') || 0)) > 0.1) {
