...
class AdaptiveLightingManager {
  ...
  async _executeUpdate() {
    ...
    // P2563 — large CCT jumps use Soft Daylight Fade (branding-free continuous steps)
    try {
      const SoftDaylightFade = require('../features/SoftDaylightFade');
      const curTemp = this.device.getCapabilityValue('light_temperature')
        ?? this.device.getCapabilityValue('light_color_temp');
      const curDim = this.device.getCapabilityValue('dim');
      const jump = Math.abs((Number(homeyValue) || 0) - (Number(curTemp) || 0));
      if (jump > 0.12 && this.homey?.app && typeof SoftDaylightFade.startOnDevice === 'function') {
        SoftDaylightFade.startOnDevice(this.homey.app, this.device, {
          minutes: 4,
          steps: 8,
          fromDim: curDim,
          fromTemperature: curTemp,
          lux: this._resolveLux(),
        });
        return;
      }
    } catch (_e) { /* soft fallback to direct set */ }
