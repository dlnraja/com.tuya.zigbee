...
// Dynamic Auto-Discovery DP Map (Athom / Johan Bendz inspired)
const DP_CAP_MAP = {
  // --- Sensors ---
  ...
  101: { caps: ['measure_humidity', 'measure_temperature', 'onoff.2'], type: 'auto' }, // auto handled in logic
  111: { caps: ['measure_temperature'], type: 'value', div: 10 },
  
  // --- Intelligent Auto-discovery mappings (Alarms) ---
  // Note: Added dynamically based on driver category
  1: { caps: ['alarm_motion', 'alarm_contact', 'onoff'], type: 'bool' },
  2: { caps: ['measure_battery', 'alarm_water', 'onoff.2'], type: 'auto' },
  
  // --- Multi-gang Switches Auto-discovery ---
  102: { caps: ['onoff.3', 'onoff.usb'], type: 'bool' },
  104: { caps: ['onoff.4'], type: 'bool' },
  105: { caps: ['onoff.5'], type: 'bool' },
  106: { caps: ['onoff.6'], type: 'bool' },
};
...
class UniversalVariantManager {
  ...
  async processDP(dpId, value, dpType) {
    ...
    // If no existing cap, try to add a safe one using intelligence
    if (!cap && mapping.caps.length > 0) {
      // Intelligent Driver Correlation
      const driverId = this.device.driver?.manifest?.id || this.device.driver?.id || '';
      const isSwitch = /switch|plug|socket|relay|dimmer/.test(driverId);
      const isMotion = /motion|pir|presence|radar/.test(driverId);
      const isContact = /contact|door|window/.test(driverId);
      const isWater = /water|leak|flood/.test(driverId);

      for (const c of mapping.caps) {
        // Smart filtering based on driver identity
        if (c.startsWith('onoff') && !isSwitch) continue;
        if (c === 'alarm_motion' && !isMotion) continue;
        if (c === 'alarm_contact' && !isContact) continue;
        if (c === 'alarm_water' && !isWater) continue;
        
        if (SAFE_CAPS.has(c) || c.startsWith('onoff') || c.startsWith('alarm_')) {
          cap = await this._tryAddCap(c, dpId, value);
          if (cap) break;
        }
      }
    }
    ...
    if (mapping.type === 'bool') {
      ...
      try {
        const { smartParse } = require('./SmartDivisorManager');
        converted = smartParse(value, dpId, {
          manufacturerName: this.device.getSetting ? (this.device.getSetting('zb_manufacturer_name') || '') : '',
          capability: cap,
          deviceId: this.device.getData ? (this.device.getData()?.id || '') : '',
        });
      } catch (err) {
        // Fallback to static mapping
        if (mapping.type === 'auto') {
          if (cap.startsWith('onoff') || cap.startsWith('alarm_')) converted = Boolean(value);
          else if (typeof value === 'boolean') converted = value ? 100 : 0;
          else if (mapping.div) converted = value / mapping.div;
        } else if (mapping.div) {
          converted = value / mapping.div;
        }
      }
      
      // Fallback to static divisor if smartParse didn't apply a scale division
      if (typeof converted === 'number' && mapping.div && converted === value) {
        converted = value / mapping.div;
      }
    }
