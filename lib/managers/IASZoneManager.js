...
class IASZoneManager {
   ...
   * NEW: Exponential backoff pour gérer erreurs temporaires
   */
  async enrollIASZone() {
    ...
    try {
      ...
      if (!ieeeAddress) {
        device.error('[IAS] ❌ Cannot get IEEE address after all attempts!');
      ...
      device.error(`[IAS] ❌ Enrollment attempt ${this.enrollmentAttempts}/${this.maxRetries} failed:`, err);
  ...
  _setupStatusListener(iasZone) {
    ...
    try {
      iasZone.onZoneStatusChangeNotification = (payload) => {
        ...
        // CRITICAL FIX: payload.zoneStatus is a Bitmap object, not a number!
        // Must call .valueOf() to get the numeric value for bitwise operations
        let zoneStatus = payload.zoneStatus;
        if (zoneStatus && typeof zoneStatus.valueOf === 'function') {
          zoneStatus = zoneStatus.valueOf();
        }
        // Fallback: if still not a number, try to extract from Bitmap
        if (typeof zoneStatus !== 'number') {
          device.log('[IAS] ⚠️ zoneStatus is not a number, trying to parse:', typeof zoneStatus, zoneStatus);
          zoneStatus = Number(zoneStatus) || 0;
        }

        device.log('[IAS] zoneStatus numeric value:', zoneStatus);

        // Parse zone status bits (now working with actual number)
