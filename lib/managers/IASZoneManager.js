...
class IASZoneManager {
  ...
  _handleZoneStatusChange(status) {
    ...
    try {
      // Determine if this is likely a button/SOS device rather than a standard sensor
      // Hybrid devices (SOS + contact) have alarm_contact, so we must also check _forcedDeviceType.
      const isButton = device._forcedDeviceType === 'BUTTON' || 
                       ((device.hasCapability('button') || device.hasCapability('alarm_generic')) && 
                       !device.hasCapability('alarm_motion') && 
                       !device.hasCapability('alarm_contact') && 
                       !device.hasCapability('alarm_water') && 
                       !device.hasCapability('alarm_smoke'));

      // Edge detection for alarm1 (button press) to prevent broadcast spam
      const prevAlarm1 = device._lastIasAlarm1 || false;
      const isAlarm1Edge = status.alarm1 && !prevAlarm1;
      device._lastIasAlarm1 = status.alarm1;

      // Button press (alarm1)
      if (status.alarm1 && isButton) {
        if (!isAlarm1Edge) {
           device.log('[IAS] ⏳ Button press ignored (broadcast spam prevention)');
        } else {
