...
const IEEEAdvancedEnrollment = require('./IEEEAdvancedEnrollment');
...
class IASZoneManager {
   ...
   *
   * WHY(P2729 / Bastien diag 1.0.93 ed627371): TS004x remotes advertise iasZone in interview
   * but are NOT SOS/contact — proactive enroll + CIE write + bind while asleep burns mesh
   * airtime and delays 0xFD capture ("boutons reconnus mais rien ne fonctionne").
   * Contre quoi: BaseUnifiedDevice background always called enrollIASZone on any iasZone cluster.
   */
  static isWirelessButtonRemoteNotIas(device) {
    const driverId = String(device?.driver?.id || device?.driver?.manifest?.id || '');
    return /button_wireless|scene_switch|smart_remote|remote_button|wall_remote|knob/.test(driverId)
      && !/sos|emergency|panic|contact|water_leak|smoke|gas/.test(driverId);
  }
  ...
  async enrollIASZone() {
    ...
    // WHY(P2729): early exit before Zigbee-ready wait / CIE TX storm
    if (IASZoneManager.isWirelessButtonRemoteNotIas(device)) {
      device.log('[IAS] Skip proactive enroll for wireless button/remote (P2729 listen-only)');
      this.enrolled = true;
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (zoneState?.zoneState === 'enrolled') {
          ...
          this.enrolled = true;
          // v9.0.390 (forum #2118): register the status-change listener here too.
          // Listeners do NOT survive app restarts — after any app update/restart
          // the device is already enrolled, so this early return used to skip
          // _setupStatusListener and IAS alarms (water/contact) went dead until
          // the next re-pair.
          this._setupStatusListener(iasZone);
          // WHY P2203 / Peter #2184 diag 1cf775a2: enroll alone ≠ reports.
          // App restart + wake both hit this early return; without bind,
          // zoneStatusChangeNotification never reaches Homey (0 iasZone msgs).
          await this._ensureIasBound(iasZone);
          return true;
        }
      } catch (err) {
      ...
      // P2184/P2203: bind so zoneStatusChangeNotification reaches Homey
      await this._ensureIasBound(iasZone);

      const sleepy = device.hasCapability?.('measure_battery') && device.mainsPowered !== true;
      if (sleepy) {
        this.enrolled = true;
        this._scheduleDeferredRetries(device);
        // Restore a short verify while we still have the enroll TX in flight.
        try {
          const raced = Promise.race([
            iasZone.readAttributes(['zoneState']),
            this._wait(1500).then(() => null)
          ]);
          raced.then((finalState) => {
            if (finalState?.zoneState === 'enrolled' || finalState?.zoneState === 1) {
              device.log('[IAS] 🎉 Sleepy enroll verified on short wait');
            }
          }).catch(() => {});
        } catch (_e) { /* listen-only is enough */ }
        return true;
      }

      // Verify enrollment (mains only — sleepy times out here)
      ...
      if (finalState?.zoneState === 'enrolled') {
        ...
        this.enrolled = true;
        // v9.0.366: sleepy devices are often unreachable during init — schedule
        // deferred retries so enrollment completes when the device wakes up
        // (contact/water sensors stuck at "no alarm" — blutch32, Lasse_K).
        this._scheduleDeferredRetries(device);
        return true;
      }
      ...
      const sleepyTimeout = /reageert niet|not powered|niet van stroom|timeout|no response|ECONNRESET/i.test(errorMsg);
      // WHY: Peter #2183 SOS/water — this.error + recursive enroll while asleep
      // floods stderr and greys Homey Flows. Defer to wake instead.
      device.log(`[IAS] Enrollment attempt ${this.enrollmentAttempts}/${this.maxRetries} failed:`, err.message);
      ...
      device.log('[IAS] All enrollment attempts exhausted — listeners stay attached');
      this.enrolled = true;
      return false;
    }
  }
  ...
  /**
   * v9.0.366: Deferred IAS enrollment retries for sleepy devices.
   * When initial enrollment is "uncertain" (device likely asleep during
   * pairing), retry in the background at 30s and 120s — by then a woken
   * device has usually checked in. All timers are cleared via device._destroyed
   * guards and retryIASEnrollment early-exits once enrolled.
   */
  _scheduleDeferredRetries(device) {
    if (!device) {return;}
    if (this._deferredRetriesScheduled) {return;}
    // WHY (f647d35b): wireless remotes advertise iasZone but are not SOS — deferred enroll
    // while asleep only burns airtime and delays button 0xFD capture.
    if (IASZoneManager.isWirelessButtonRemoteNotIas(device)) {
      device.log('[IAS] Skip deferred enroll for wireless button/remote');
      return;
    }
    this._deferredRetriesScheduled = true;
    if (typeof device.retryIASEnrollment !== 'function') {return;}
    for (const delay of [30000, 120000]) {
      safeSetTimeout(device, async () => {
        if (!device || device._destroyed) {return;}
        device.log(`[IAS] 🔄 Deferred enrollment retry (+${Math.round(delay / 1000)}s)`);
        try {
          const ok = await device.retryIASEnrollment(3);
          if (ok) {device.log('[IAS] ✅ Deferred retry enrolled the device');}
        } catch (e) {
          device.log(`[IAS] Deferred retry error: ${e.message}`);
        }
      }, delay);
    }
  }
  ...
  /**
   * Bind IAS Zone to the coordinator so notifications are delivered.
   * WHY: Homey zigbee-clusters may expose Cluster.bind() OR endpoint.bind('iasZone').
   * Contre quoi: listener-only without bind → silent wet/dry (Peter water 1cf775a2).
   * @param {object} iasZone
   * @returns {Promise<boolean>}
   */
  async _ensureIasBound(iasZone) {
    const device = this.device;
    if (!iasZone) {return false;}

    if (typeof iasZone.bind === 'function') {
      try {
        await iasZone.bind();
        device.log('[IAS] ✅ IAS Zone cluster bound to coordinator');
        return true;
      } catch (err) {
        device.log('[IAS] ⚠️ IAS cluster.bind failed (try endpoint):', err.message);
      }
    }
    ...
    try {
      ...
      if (typeof endpoint?.bind === 'function') {
        ...
        device.log('[IAS] ✅ IAS Zone bound via endpoint.bind');
        return true;
      }
    } catch (err) {
      device.log('[IAS] ⚠️ IAS endpoint.bind failed (retry on wake):', err.message);
    }
    return false;
  }
  ...
  _handleZoneStatusChange(status) {
    ...
    try {
      // Button / SOS press — polarity-aware (do not use raw alarm1 alone)
      {
        const isButtonLike = device.hasCapability('alarm_generic')
          && !device.hasCapability('alarm_contact')
          && !device.hasCapability('alarm_water')
          && !device.hasCapability('alarm_motion');
        if (isButtonLike) {
          const { applyPolarity, observeRaw } = require('./AlarmPolarityManager');
          const raw = !!(status.alarm1 || status.alarm2);
          observeRaw(device, raw, 'sos');
          const { value: sosAlarm, meta } = applyPolarity(device, raw, 'sos');
          if (sosAlarm) {
            device.log(`[IAS] 🔘 BUTTON/SOS PRESSED (raw=${raw}, ${meta.reason})`);
            if (device.homey?.flow && device.driver?.id) {
              const triggerIds = [
                `${device.driver.id}_pressed`,
                `${device.driver.id}_button_pressed`,
                'button_pressed',
              ];
            ...
            commitCapabilityCatch(device, 'alarm_generic', true, 'ias');
            const alarmClearMs = (device.getSetting?.('alarm_clear_timeout') || 5) * 1000;
            if (this._alarmClearTimer) {safeClearTimeout(this.device || { homey: this.homey }, this._alarmClearTimer);}
            this._alarmClearTimer = safeSetTimeout(this.device || { homey: this.homey }, () => {
              if (!device || device._destroyed) {return;}
              commitCapabilityCatch(device, 'alarm_generic', false, 'ias');
              this._alarmClearTimer = null;
            }, alarmClearMs);
          }
        }
      }

      // Motion detected — polarity-aware
      if (device.hasCapability('alarm_motion')) {
        const { applyPolarity, observeRaw } = require('./AlarmPolarityManager');
        const raw = !!status.alarm1;
        observeRaw(device, raw, 'motion');
        const { value: motionAlarm, meta } = applyPolarity(device, raw, 'motion');
        if (motionAlarm) {
          device.log(`[IAS] 🚶 MOTION DETECTED! (raw=${raw}, ${meta.reason})`);
          commitCapabilityCatch(device, 'alarm_motion', true, 'ias');
          ...
          safeSetTimeout(this.device || { homey: this.homey }, () => {
            if (device?._destroyed || this._destroyed) {return;}
            commitCapabilityCatch(device, 'alarm_motion', false, 'ias');
            if (device.dpLogger && typeof device.dpLogger.logMotion === 'function') {
              device.dpLogger.logMotion(false);
            }
          }, timeout * 1000);
        } else if (!motionAlarm && device.getCapabilityValue?.('alarm_motion')) {
          // Logical clear (may be inverted OEM)
          commitCapabilityCatch(device, 'alarm_motion', false, 'ias');
        }
      }

      // Contact sensor (alarm1) — polarity via AlarmPolarityManager
      if (device.hasCapability('alarm_contact')) {
        const { applyPolarity, observeRaw } = require('./AlarmPolarityManager');
        const raw = !!status.alarm1;
        observeRaw(device, raw, 'contact');
        const { value: contactAlarm, meta } = applyPolarity(device, raw, 'contact');
        device.log(`[IAS] 🚪 Contact: ${contactAlarm ? 'OPENED' : 'closed'} (raw=${raw}, ${meta.reason})`);
        device._iasOriginatedAlarm = true;
        commitCapabilityCatch(device, 'alarm_contact', contactAlarm, 'ias');
      }

      // Water leak — polarity via AlarmPolarityManager
      if (device.hasCapability('alarm_water')) {
        const { applyPolarity, observeRaw } = require('./AlarmPolarityManager');
        const raw = !!(status.alarm1 || status.alarm2);
        observeRaw(device, raw, 'water');
        const { value: waterAlarm, meta } = applyPolarity(device, raw, 'water');
        device.log(`[IAS] 💧 Water: ${waterAlarm ? 'LEAK DETECTED!' : 'dry'} (raw=${raw}, ${meta.reason})`);
        device._iasOriginatedWaterAlarm = true;
        commitCapabilityCatch(device, 'alarm_water', waterAlarm, 'ias');
      }

      // Tamper: always publish boolean so Homey UI is not stuck on "-" (#2134)
      if (device.hasCapability('alarm_tamper') && typeof status.tamper === 'boolean') {
        if (status.tamper) {device.log('[IAS] ⚠️ TAMPER!');}
        commitCapabilityCatch(device, 'alarm_tamper', !!status.tamper, 'ias');
      } else if (status.tamper && device.hasCapability('alarm_tamper')) {
        device.log('[IAS] ⚠️ TAMPER!');
        commitCapabilityCatch(device, 'alarm_tamper', true, 'ias');
      }

      // Battery low — NEVER invent measure_battery % (P115/P212). alarm_battery only.
      if (status.batteryLow) {
        device.log('[IAS] 🔋 BATTERY LOW WARNING!');
        if (device.hasCapability('alarm_battery')) {
          commitCapabilityCatch(device, 'alarm_battery', true, 'ias');
        }
      }
  ...
  _wait(ms) {
    return new Promise((resolve) => {
      safeSetTimeout(this.device || { homey: this.homey }, resolve, ms);
    });
  }

  /**
   * v9.0.40: Cleanup timers on device destruction
   */
  destroy() {
    if (this._alarmClearTimer) {
      clearTimeout(this._alarmClearTimer);
      this._alarmClearTimer = null;
    }
  }
