...
class IASZoneManager {
    ...
    try {
      ...
      try {
        ...
        if (zoneState?.zoneState === 'enrolled') {
          ...
          this.enrolled = true;
          // v5.12.48 (backport P92.91, forum #2118): register the status-change
          // listener here too — listeners do NOT survive app restarts, so this
          // early return used to kill IAS alarms (water/contact) until re-pair.
          this._setupStatusListener(iasZone);
          // P2203 BOTH: cluster.bind + endpoint.bind fallback (Peter 1cf775a2)
      ...
      // P2184/P2203 BOTH: bind so notifications reach Homey
      await this._ensureIasBound(iasZone);
      ...
      // WHY: Peter #2183 — this.error while asleep floods stderr and greys Flows.
  ...
  /**
   * v5.12.33: Deferred IAS enrollment retries for sleepy devices.
   ...
   * device has usually checked in. Timers are guarded by device._destroyed
   * and retryIASEnrollment early-exits once enrolled.
   */
  _scheduleDeferredRetries(device) {
    ...
    for (const delay of [30000, 120000]) {
      // BOTH: never touch device.homey.setTimeout when homey is undefined (Peter #2137).
  ...
  /**
   * P2203 BOTH: bind IAS Zone (cluster.bind or endpoint.bind fallback).
   */
    ...
    try {
      ...
      if (status.alarm1) {
        ...
        if (device.hasCapability('alarm_generic')) {
          ...
          // v9.0.40: Configurable auto-clear timeout for IAS zone alarms
          const alarmClearMs = (device.getSetting?.('alarm_clear_timeout') || 120) * 1000;
          if (this._alarmClearTimer) {
            safeClearTimeout(this.device || { homey: this.homey }, this._alarmClearTimer);
          }
      ...
      if (status.alarm1 && device.hasCapability('alarm_motion')) {
        ...
        // Auto-reset after timeout (BOTH: safeSetTimeout — never bare this.homey.setTimeout)
      ...
      // Battery low - NEVER invent measure_battery % (P115/P212/P2611). alarm_battery only.
