  ...
  determineBestDriver(deviceInfo, clusterAnalysis) {
    ...
    if (deviceType === 'usb_outlet' || isUsbOutletByManufacturer || isCurrentlyUsb) {
      ...
      if (!isCurrentlySwitch) {
        ...
        this.log('[MIGRATION] ⚠️ Skipping USB outlet detection - device is a switch');
      }
    }
    // OUTLETS / PLUGS
...
Confidence: ${Math.round(safeMultiply(bestDriver.confidence, 100))}%
...
You can migrate manually in the device settings.
  ...
  generateMigrationReport(currentDriverId, bestDriver, needsMigration) {
    ...
    report.push(`📊 Confidence: ${Math.round(safeMultiply(bestDriver.confidence, 100))}%`);
    report.push('');
    report.push('💡 Reasons:');
