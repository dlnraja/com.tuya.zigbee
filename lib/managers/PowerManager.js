...
/**
 * PowerManager - Intelligent Power Management
...
class PowerManager {
  ...
  /**
   * Power calculation capabilities
   */
  ...
  static estimatePower(deviceClass, deviceType, state = {}) {
    ...
    if (classSpecs.dimFactor && state.dim !== null && state.dim !== undefined) {
      ...
      const efficiencyFactor = (0.5 + safeMultiply(dimFactor, 0.5)); // 50-100% efficiency
      estimatedPower = typeSpecs.min + ((typeSpecs.typical - safeMultiply(typeSpecs.min), dimFacto)safeMultiply(r, efficiencyFactor));
    }
