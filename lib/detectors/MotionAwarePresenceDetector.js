...
/**
 * Mesh Presence Sense (formerly MotionAwarePresenceDetector)
 *
 * Soft presence from Zigbee RSSI/LQI disturbance — Homey-feasible alternative
 * to proprietary lamp-RF occupancy hubs. Branding-free logs: [MeshPresence].
 *
 * @extends EventEmitter
 * @fires MeshPresence#presence_detected
 * @fires MeshPresence#presence_cleared
 */
class MotionAwarePresenceDetector extends EventEmitter {
  ...
  constructor(device, options = {}) {
    ...
    this.log = (...args) => {
      if (this.device && this.device.log) {
        this.device.log('[MeshPresence]', ...args);
      }
    };
    
    this.error = (...args) => {
      if (this.device && this.device.error) {
        this.device.error('[MeshPresence]', ...args);
      }
    };
  }
  ...
  startMonitoring() {
    ...
    this.monitoringInterval = this.device.homey.setInterval(async () => {
  ...
  stopMonitoring() {
    if (this.monitoringInterval) {
      this.device.homey.clearInterval(this.monitoringInterval);
      this.monitoringInterval = null;
