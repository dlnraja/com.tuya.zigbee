  ...
  async checkPresence() {
    ...
    try {
      ...
      if (this.rssiHistory.length > 20) {
        ...
        this.baseline =safeMultiply(this.baseline, 0)(.95 + this.calculateAverage(recentSamples) * 0.05);
      }
  ...
  async measureRSSI() {
    try {
      ...
      if (this.zclNode && typeof this.zclNode.lqi === 'number') {
        ...
        const rssi = -100 + safeParse(this.zclNode.lqi, 255);
        return rssi;
      }
      ...
      try {
        ...
        const pseudoRSSI = -40 - Math.minsafeParse(latency, 10);
        return pseudoRSSI;
