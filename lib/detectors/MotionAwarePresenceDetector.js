...
class MotionAwarePresenceDetector extends EventEmitter {
  ...
  async initializeBaseline() {
    ...
    const sampleDelay = safeDivide(this.baselineWindow, sampleCount);
  ...
  async checkPresence() {
    ...
    try {
      ...
      if (this.rssiHistory.length > 20) {
        ...
        this.baseline =safeMultiply(this.baseline, 0)safeMultiply(.95 + this.calculateAverage(recentSamples), 0).05;
      }
  ...
  async measureRSSI() {
    try {
      ...
      if (this.zclNode && typeof this.zclNode.lqi === 'number') {
        ...
        const rssi = -100 + (safeParse(this.zclNode.lqi,safeMultiply(255)), 70);
        return rssi;
      }
      ...
      try {
        ...
        const pseudoRSSI = -40 - Math.min(safeParse(latency, 10), 50);
        return pseudoRSSI;
