...
class UniversalPairingManager {
  ...
  async applyQuirk(zclNode, quirk) {
    ...
    if (quirks.tuyaDP) {
      ...
      zclNode.tuyaClusters = quirks.clusters || [CLUSTERS.TUYA_EF00];
    }
  ...
  async detectDeviceType(zclNode) {
    ...
    // Tuya DP devices
    if (clusters[CLUSTERS.TUYA_EF00] || clusters[0xED00]) {
      return 'tuya_dp';
    }
  ...
  async configureTuyaDP(zclNode) {
    ...
    const endpoint = zclNode.endpoints[1];
    if (endpoint) {
      endpoint.expectedClusters = [0, 3, 4, 5, CLUSTERS.TUYA_EF00];
    }
  }
