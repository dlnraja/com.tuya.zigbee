 ...
 *
 * v10.0.0 (P82): Now uses TuyaNormalizer ULTIMATE (NFKD + accents + emojis
 *   + multi-PID + variants + scoring). Backward-compat with v5.5.700.
 * v5.5.700: Used Util.js for case-insensitive matching
 * v5.5.688: CRITICAL FIX - Case-insensitive matching
...
class PermissiveMatchingEngine {
  ...
  shouldAcceptDevice(zclNode) {
    ...
    // v10.0.0: Using TuyaNormalizer ULTIMATE for case-insensitive matching
    ...
    if (TU.includesCI(driverMfrs, mfr) && TU.includesCI(driverPids, pid)) {
    ...
    if (TU.includesCI(driverMfrs, mfr)) {
    ...
    if (TU.includesCI(driverPids, pid)) {
  ...
  scheduleEnrichment(zclNode, delayMs = 3000) {
    ...
    this._pendingTimers.push(this.homey.setTimeout(async () => {
  ...
  _detectProtocol(clusters) {
    ...
    for (const raw of clusters) {
      ...
      if (str === '61184' || str === '0xef00' || str === 'tuya' || str === 'manuspecifictuya' || str === 'tuyaef00') {
        hasTuya = true;
      }
      if (str === '6' || str === '0x0006' || str === 'onoff' || str === 'genonoff') {
        hasOnOff = true;
      }
      try {
        const { normalizeClusterId } = require('../zigbee/ZclClusterLexicon');
        const id = normalizeClusterId(raw);
        if (id === 0xEF00 || id === 61184) {hasTuya = true;}
        if (id === 0x0006 || id === 6) {hasOnOff = true;}
      } catch (_e) { /* continue */ }
    }
  ...
  /**
   * Cancel all pending timers (call on device deletion).
   */
  destroy() {
