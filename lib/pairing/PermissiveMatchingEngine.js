...
class PermissiveMatchingEngine {
  ...
  shouldAcceptDevice(zclNode) {
    ...
    // Level 1: Exact match (manufacturerName + productId)
    if (driverMfrs.includes(mfr) && driverPids.includes(pid)) {
      this._matchLevel = 'exact';
      this.log('[MATCH] ✅ EXACT match (mfr + pid)');
      return { accept: true, level: 'exact', confidence: 100 };
    }

    // Level 2: ManufacturerName match only
    if (driverMfrs.includes(mfr)) {
      this._matchLevel = 'manufacturer';
      this.log('[MATCH] ✅ MANUFACTURER match (pid may differ)');
      return { accept: true, level: 'manufacturer', confidence: 80 };
    }

    // Level 3: ProductId match only (rare but valid)
    if (driverPids.includes(pid)) {
      this._matchLevel = 'productId';
      this.log('[MATCH] ⚠️ PRODUCTID only match');
