...
function _norm(s) {
  ...
  return n === 'heobian' ? 'hobeian' : n;
}

function loadRegistry(force = false) {
  // WHY(P2749e / P2522): never permanently cache a pressure-deferred empty registry —
  // buildDriverIndex() can trip BootBudget.isHeapCritical, then invent pads look
  // NOT_IN_CATALOG forever and publish gates fail (DO_NOT_LOCK Contre quoi).
  if (_cache && !force && !_cache._deferredPressure) {return _cache;}
  // WHY(P2674): skip 200KB misattrib JSON.parse during heap-critical device storm
  try {
    const BootBudget = require('../performance/BootBudget');
    if (!force && typeof BootBudget.isHeapCritical === 'function' && BootBudget.isHeapCritical()) {
      return { version: 0, cases: [], _deferredPressure: true };
    }
  } catch { /* soft */ }
  try {
...
function lookup(mfr, pid) {
  ...
  for (const c of cases || []) {
    ...
    // WHY(P2590d): Z2M enrich may leave productId as a string — never .map on non-array
    const rawPid = c.productId;
    const pidList = Array.isArray(rawPid) ? rawPid : (rawPid != null && rawPid !== '' ? [rawPid] : []);
    let pids = pidList.map(_norm);
 ...
 * WHY(P2523 / Fleet P2372): doNotLock couple cases MUST still block mfr-only
 * placement on forbiddenDrivers (krwtzhfd→climate_sensor re-inject).
 */
function isForbiddenPlacement(mfr, driverId) {
  ...
  const { cases } = loadRegistry();
  for (const c of cases || []) {
    const coupleOnly = _norm(c.forbidMode) === 'couple' && !c.doNotLock;
    if (coupleOnly) {continue;}
