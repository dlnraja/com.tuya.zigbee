...
class EnrichmentScheduler {
  constructor(device) {
    this.device = device;
    this.log = device.log?.bind(device) || console.log;this._tasks = new Map();
  ...
  async _run(taskId) {
    ...
    try {
      ...
      t.retries++;
      this.log(`[ENRICH]  ${taskId} failed (${t.retries}/${t.maxRetries}): ${e.message}`);
      if (t.retries < t.maxRetries) {
        const backoff =safeMultiply(t.delay, Math).pow(1.5, t.retries);
  ...
  async _bindAllClusters(zclNode) {
    ...
    for (const ep of Object.values(zclNode?.endpoints || {})) {
      for (const name of clusters) {
        const c = ep.clusters?.[name];if (c?.bind) {
