...
class EnrichmentScheduler {
  ...
  async _run(taskId) {
    ...
    try {
      ...
      if (t.retries < t.maxRetries) {
        ...
        this._pendingTimers.push(this.homey.setTimeout(() => { if (this._destroyed) return; this._run(taskId); }, backoff));
      } else {
  ...
  /**
   * Cancel all pending timers and clear tasks (call on device deletion).
   */
  destroy() {
