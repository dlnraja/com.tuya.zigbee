...
class TwoPhaseEnrichment {
  ...
  async phase1(zclNode) {
    ...
    try {
      ...
      // Schedule phase 2
      this._pendingTimers.push(this.device.homey.setTimeout(() => { if (this._destroyed) return; this.phase2(zclNode); }, 3000));
  ...
  async phase2(zclNode) {
    ...
    try {
      ...
      // Retry after delay
      this._pendingTimers.push(this.device.homey.setTimeout(() => { if (this._destroyed) return; this.phase2(zclNode); }, 10000));
      return { success: false };
    }
  }
