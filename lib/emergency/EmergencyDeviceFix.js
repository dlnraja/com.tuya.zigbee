...
let _DeviceFingerprintDB = null;
const getDeviceFingerprintDB = () => {
  if (!_DeviceFingerprintDB) {_DeviceFingerprintDB = require('../tuya/DeviceFingerprintDB');}
  return _DeviceFingerprintDB;
};
...
class EmergencyDeviceFix {
  ...
  /**
   * Get a logger instance bound to the homey app context
   */
  static _getLogger(homey) {
    const log = homey && typeof homey.log === 'function' ? homey.log.bind(homey) : (msg) => {};
    const error = homey && typeof homey.error === 'function' ? homey.error.bind(homey) : (msg) => {};
    return { log, error };
  }
  ...
  static async runAll(homey) {
    const { log, error } = this._getLogger(homey);
    log('[EMERGENCY] Starting comprehensive repair...');
    ...
    try {
      ...
      log('[EMERGENCY] [1/4] Fixing stuck migrations...');
      results.migrationFixed = await this.fixStuckMigrations(homey);

      // 2. Fix problematic devices
      log('[EMERGENCY] [2/4] Fixing problematic devices...');
      results.devicesFixed = await this.fixProblematicDevices(homey);

      // 3. Force DP collection
      log('[EMERGENCY] [3/4] Forcing DP collection...');
      results.dpRequestsSent = await this.forceDataCollection(homey);

      // 4. Clear error states
      log('[EMERGENCY] [4/4] Clearing error states...');
      await this.clearErrorStates(homey);

    } catch (err) {
      error('[EMERGENCY] Emergency fix error:', err.message);
      results.errors.push(err.message);
    }

    log(`[EMERGENCY] Fix complete: ${results.migrationFixed} migrations, ${results.devicesFixed} devices, ${results.dpRequestsSent} DP requests, ${results.errors.length} errors`);
  ...
  static async fixStuckMigrations(homey) {
    ...
    try {
      ...
      if (queue.length === 0) {
        log('[EMERGENCY] [MIGRATION] No pending migrations');
        return 0;
      }

      log(`[EMERGENCY] [MIGRATION] Found ${queue.length} pending migration(s)`);
      ...
      for (const job of queue) {
        ...
        if (resolvedDriverId !== targetDriverId) {
          log(`[EMERGENCY] [MIGRATION] Resolved alias: ${targetDriverId} -> ${resolvedDriverId}`);
        ...
        try {
          ...
          log(`[EMERGENCY] [MIGRATION] Skipping invalid driver: ${resolvedDriverId}`);
          fixed++;
        }
      }
      ...
      log(`[EMERGENCY] [MIGRATION] Queue cleaned: ${queue.length} -> ${validQueue.length}`);

    } catch (err) {
      error('[EMERGENCY] [MIGRATION] Fix error:', err.message);
    }
      ...
      for (const driver of Object.values(drivers)) {
        ...
        for (const device of devices) {
          try {
            ...
            const model = device.getSetting('zb_model_id');
            ...
            if (fix) {
              log(`[EMERGENCY] [DEVICE-FIX] Checking ${device.getName()} (${manufacturer})`);

              // Verify capabilities
              for (const cap of fix.forceCapabilities || []) {
                if (!device.hasCapability(cap)) {
                  log(`[EMERGENCY] [DEVICE-FIX] Missing capability: ${cap}`);
                  try {
                    await device.addCapability(cap);
                    log(`[EMERGENCY] [DEVICE-FIX] Added capability: ${cap}`);
                    fixed++;
                  } catch (e) {
                    log(`[EMERGENCY] [DEVICE-FIX] Cannot add ${cap}: ${e.message}`);
                  }
                }
              }
            ...
            const fingerprint = getDeviceFingerprintDB().getFingerprint(manufacturer);
            if (fingerprint && fingerprint.capabilities) {
              for (const cap of fingerprint.capabilities) {
                if (!device.hasCapability(cap)) {
                  try {
                    await device.addCapability(cap);
                    log(`[EMERGENCY] [DEVICE-FIX] Added from fingerprint: ${cap}`);
                    fixed++;
                  } catch (e) {
            ...
            error(`[EMERGENCY] [DEVICE-FIX] Error on ${device.getName()}: ${deviceError.message}`);
          }
        }
      }

    } catch (err) {
      error('[EMERGENCY] [DEVICE-FIX] Error:', err.message);
    }
  ...
  static async forceDataCollection(homey) {
    ...
    try {
      ...
      for (const driver of Object.values(drivers)) {
        ...
        for (const device of devices) {
          try {
            ...
            log(`[EMERGENCY] [DP-FORCE] Requesting DPs for ${device.getName()}...`);
              ...
              for (const dp of criticalDPs) {
                try {
                  ...
                  await Promise.race([
                    ...
                    new Promise((_, reject) => (homey && typeof homey.setTimeout === 'function' ? homey : globalThis).setTimeout(() => { if (this._destroyed) {return;} reject(new Error('timeout')); }, 2000))
                  ]);
      ...
      error('[EMERGENCY] [DP-FORCE] Error:', err.message);
    }
  ...
  static async clearErrorStates(homey) {
    ...
    try {
      ...
      log('[EMERGENCY] [CLEANUP] Error states cleared');

    } catch (err) {
      error('[EMERGENCY] [CLEANUP] Error:', err.message);
    }
  }
      ...
      for (const driver of Object.values(drivers)) {
        ...
        for (const device of devices) {
          const deviceInfo = {
            ...
            model: device.getSetting('zb_model_id'),
