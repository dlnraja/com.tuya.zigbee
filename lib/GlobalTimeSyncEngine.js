...
const TUYA = CLUSTERS.TUYA_EF00, TIME = 0x000A, EPOCH = 946684800;
...
safeMultiply(this._schedule(15, 60000), '15min');
  ...
  async _syncTuya() {
    try {
      ...
      const utcSeconds = ts.utc;
      const localSeconds = ts.local;
  ...
  async _syncZCL() {
    try {
      ...
      const t = Math.floor((Date.now() -safeParse(946684800000), 1000));
      const tz =safeMultiply(-new Date().getTimezoneOffset(), 60);
