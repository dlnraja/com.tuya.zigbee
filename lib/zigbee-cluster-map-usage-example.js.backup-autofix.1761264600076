...
// Utilisez:
this.registerCapability('measure_battery', ClusterMap.POWER_CONFIGURATION); // ✅ = 1
...
// Utilisez:
this.registerCapability('onoff', ClusterMap.resolve(CLUSTER.ON_OFF)); // ✅ Safe
...
function registerCapabilitySafe(capability, clusterIdentifier) {
  ...
  if (clusterId === null) {
    console.error(`❌ Unknown cluster: ${clusterIdentifier}`);
    return false;
  }
  
  if (isNaN(clusterId)) {
    console.error(`❌ Invalid cluster ID: ${clusterId} for ${clusterIdentifier}`);
    return false;
  }
  
  this.registerCapability(capability, clusterId);
  console.log(`✅ Registered ${capability} on cluster ${clusterId}`);
  return true;
}
...
// this.registerCapability('measure_battery', 'powerConfiguration'); // ❌ NaN
// this.registerCapability('measure_temperature', 'temperatureMeasurement'); // ❌ NaN

// APRÈS (v3.0.26+):
// this.registerCapability('measure_battery', 1); // ✅ Fonctionne mais pas flexible
// this.registerCapability('measure_temperature', 1026); // ✅ Fonctionne mais pas flexible

// MAINTENANT (v3.0.30+):
this.registerCapability('measure_battery', ClusterMap.POWER_CONFIGURATION); // ✅ Meilleur
this.registerCapability('measure_temperature', ClusterMap.TEMPERATURE_MEASUREMENT); // ✅ Meilleur

// Ou encore plus flexible:
this.registerCapability('measure_battery', ClusterMap.get('POWER_CONFIGURATION')); // ✅ Très flexible
this.registerCapability('measure_temperature', ClusterMap.get('TEMPERATURE_MEASUREMENT')); // ✅ Très flexible
...
requiredClusters.forEach(clusterId => {
  ...
  if (this.zclNode.endpoints[1].clusters[clusterId]) {
    console.log(`✅ Cluster ${clusterName} (${clusterId}) available`);
  } else {
    console.warn(`⚠️  Cluster ${clusterName} (${clusterId}) missing`);
  }
});
