...
function deviceMatchesDriver(device, manifest) {
  ...
  const mfrMatch = CI.includesCI(supportedMfrs, deviceMfr);
  ...
  const productMatch = supportedProducts.length === 0 || 
                       CI.includesCI(supportedProducts, deviceProduct);
...
function findBestDriver(device, drivers) {
  ...
  for (const driver of drivers) {
    const supportedMfrs = driver.zigbee?.manufacturerName || [];const supportedProducts = driver.zigbee?.productId || [];const mfrMatch = CI.includesCI(supportedMfrs, deviceMfr);
    const productMatch = CI.includesCI(supportedProducts, deviceProduct);
