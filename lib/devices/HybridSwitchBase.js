  ...
  _registerFlowActionListeners(gang, capability) {
    ...
    for (const action of actions) {
      ...
      for (const id of cardIds) {
        try {
          card = (() => { try { return this.homey.flow.getDeviceActionCard(id); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })();
          if (card) break;
        } catch (e) { }
      }
      ...
      if (card) {
        ...
        card.registerRunListener(async (args) => {
          ...
          if (typeof targetDevice._setGangOnOff === 'function') {
            ...
            // Fallback for non-gang devices using this mixin
    ...
    if (this._isPureTuyaDP && this.tuyaEF00Manager) {
      ...
      if (cluster) {
        try {
          ...
          await this.tuyaEF00Manager?.sendDP(gang, value ? 1 : 0, 'bool').catch(e =>
