...
class HybridSwitchBase extends ZigBeeDevice {
  ...
  async registerCapability(capabilityId, clusterId, opts = {}) {
    ...
    if (capabilityId.startsWith('onoff.gang')) {
      const gang = parseInt(capabilityId.replace('onoff.gang', ''), 10);
      if (!isNaN(gang)) {
        opts.endpoint = opts.endpoint || gang;
      }
    } else if (capabilityId.startsWith('onoff.')) {
      const gang = parseInt(capabilityId.replace('onoff.', ''), 10);
      if (!isNaN(gang)) {
        opts.endpoint = opts.endpoint || gang;
      }
