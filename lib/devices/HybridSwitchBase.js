  ...
  async _setupZCLMode(zclNode) {
    ...
    for (let gang = 1; gang <= this.gangCount; gang++) {
      ...
      if (this.hasCapability(capability)) {
        // v5.5.812: BSEED TS0726 PHYSICAL BUTTON FIX
        // These devices send ZCL COMMANDS (toggle/on/off), not attribute reports
        // Listen for both attribute changes AND commands
        if (needsCommandListeners) {
          // Bind cluster first for command reception
          ...
          // Command listeners for physical button presses
          ...
          // Listen for specific command events (SDK3 pattern)
          if (typeof onOffCluster.on === 'function') {
            ...
            this.log(`[ZCL] ✅ Command listeners registered for EP${gang} (9 patterns)`);
          }
        }
