...
try {
  ...
  console.log('[HybridSwitchBase] TuyaOnOffCluster not available:', e.message);
}
 ...
 * - Scene switches
 */
class HybridSwitchBase extends BaseHybridDevice {
  ...
  async onNodeInit({ zclNode }) {
    // Call BaseHybridDevice init first
    await super.onNodeInit({ zclNode });

    if (this._hybridSwitchInited) {
      this.log('[HYBRID-SWITCH] ⚠️ Already initialized (SwitchBase), skipping');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        this.log('[HYBRID-SWITCH] ⚠️ Tuya Magic Spell failed:', err.message);
      }
      try {
  ...
  _registerCapabilityListeners() {
    ...
    for (let gang = 1; gang <= this.gangCount; gang++) {
      ...
      if (this.hasCapability(capability)) {
        ...
        // v5.13.1: CRITICAL FIX (Issue #170) — Register custom Flow Action listeners
        // for "Turn on/off gang X" cards to route them through the capability logic.
        // This links the custom cards in driver.flow.compose.json to device actions.
        const actions = ['turn_on', 'turn_off', 'toggle'];
        for (const action of actions) {
          const cardId = `${driverId}_${action}_gang${gang}`;
          try {
            const card = this.homey.flow.getActionCard(cardId);
            if (card) {
              card.registerRunListener(async (args) => {
                this.log(`[FLOW-ACTION] ⚡ ${cardId} triggered`);
                let newValue = action === 'turn_on';
                if (action === 'toggle') {
                  newValue = !this.getCapabilityValue(capability);
                }
                return this._setGangOnOff(gang, newValue);
              });
              this.log(`[FLOW] ✅ Registered action card: ${cardId}`);
            }
          } catch (e) {
            // Card might not exist for this driver, skip silently
          }
        }
      }
  ...
  _setupRawFrameFallback() {
    ...
    this.log('🛡️ [RX/TX] Setup Universal Raw Frame Fallback (HybridSwitchBase)');
