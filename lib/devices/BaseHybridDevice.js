...
class BaseHybridDevice extends ZigBeeDevice {
  ...
  async registerAllCapabilitiesWithReporting() {
    ...
    if (!endpoint) {
      this.log('[REGISTER] ⚠️  No endpoint 1 available');
      return;
    }
    ...
    if (this.hasCapability('measure_luminance') && endpoint.clusters?.illuminanceMeasurement) {
      try {
        ...
        await this.registerCapability('measure_luminance', 'illuminanceMeasurement', {
          get: 'measuredValue',
          report: 'measuredValue',
    ...
    if (this.hasCapability('alarm_motion') && endpoint.clusters?.occupancySensing) {
      try {
        ...
        await this.registerCapability('alarm_motion', 'occupancySensing', {
          get: 'occupancy',
          report: 'occupancy',
