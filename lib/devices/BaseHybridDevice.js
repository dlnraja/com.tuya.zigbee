    ...
    if (isButtonDevice) {
      ...
      if (this.hasCapability('measure_battery') && this.zclNode?.endpoints?.[1]?.clusters?.powerConfiguration) {
        try {
          ...
          // v5.5.21: Skip initial read for buttons - they sleep!
          // Battery will be reported when device wakes up
          this.log(`[REGISTER] ✅ Button battery configured (polling disabled - awaiting device wake)`)
        } catch (err) {
