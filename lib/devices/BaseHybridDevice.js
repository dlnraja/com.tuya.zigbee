...
const DriverMigrationManager = require('../DriverMigrationManager');
const IntelligentDataManager = require('../IntelligentDataManager');
...
class BaseHybridDevice extends ZigBeeDevice {
  ...
  async _runBackgroundInitialization() {
    try {
      ...
      */
      ...
      // Step FINAL: Smart Driver Adaptation & Migration Check (NON-DESTRUCTIVE)
      ...
      try {
        ...
        if (adaptResult.success && adaptResult.deviceInfo) {
          ...
          // 🤖 NON-DESTRUCTIVE: Use SuggestionEngine instead of direct apply
          if (this.homey.app?.suggestionEngine && adaptResult.clusterAnalysis) {
            this.log('[SUGGESTION] 🧠 Generating non-destructive suggestions...');
            ...
            if (suggestions.recommendations.length > 0) {
              this.log(`[SUGGESTION] 💡 ${suggestions.recommendations.length} suggestions generated`);
              this.log(`[SUGGESTION] Confidence: ${(suggestions.confidence * 100).toFixed(0)}%`);
              this.log(`[SUGGESTION] Safe to apply: ${suggestions.safeToApply ? 'YES' : 'NO'}`);
              this.log(`[SUGGESTION] Needs confirmation: ${suggestions.needsConfirmation ? 'YES' : 'NO'}`);
              
              // Only auto-apply if SAFE
              if (suggestions.safeToApply) {
                this.log('[SUGGESTION] ✅ Safe suggestions detected, applying automatically...');
                const applyResult = await this.homey.app.suggestionEngine.applySuggestions(
                  this.getData().id,
                  this,
                  false // No confirmation needed for safe changes
                );
                this.log(`[SUGGESTION] Applied: ${applyResult.applied.length}, Failed: ${applyResult.failed.length}`);
              } else {
                this.log('[SUGGESTION] ⚠️  Suggestions need manual review (logged for MCP/AI)');
                
                // Log each recommendation
                suggestions.recommendations.forEach((r, i) => {
                  this.log(`[SUGGESTION] ${i + 1}. [${r.priority}] ${r.type}: ${r.capability}`);
                  this.log(`           Confidence: ${(r.confidence * 100).toFixed(0)}%, Safe: ${r.safe ? 'YES' : 'NO'}`);
                  if (r.warning) {
                    this.log(`           ⚠️ ${r.warning}`);
                  }
                });
              }
            } else {
              this.log('[SUGGESTION] ✅ No changes needed - device capabilities are correct');
            }
          }
      ...
      if (endpoint.clusters?.temperatureMeasurement || endpoint.clusters?.msTemperatureMeasurement) {
        try {
          ...
          if (tempData && tempData.measuredValue !== null) {
            ...
            if (this.hasCapability('measure_temperature')) {
              await Promise.resolve(this.setCapabilityValue('measure_temperature', tempData.measuredValue / 100)).catch(this.error);
            }
          }
        } catch (err) {
      ...
      if (endpoint.clusters?.relativeHumidity || endpoint.clusters?.msRelativeHumidity) {
        try {
          ...
          if (humidityData && humidityData.measuredValue !== null) {
            ...
            if (this.hasCapability('measure_humidity')) {
              await Promise.resolve(this.setCapabilityValue('measure_humidity', humidityData.measuredValue / 100)).catch(this.error);
            }
          }
        } catch (err) {
  ...
  /**
   * Register capability with retry and polling fallback
   * Uses configureReportingWithRetry, falls back to polling if reporting fails
   * 
   * @param {string} capability - Capability name
   * @param {number} clusterId - Cluster ID (use CLUSTER.*)
   * @param {Object} options - registerCapability options
   * @returns {Promise<boolean>} True if successful
   */
  async registerCapabilityWithRetry(capability, clusterId, options) {
    try {
      // First, register the capability normally (sets up listeners)
      await this.registerCapability(capability, clusterId, options);
      
      // If reportOpts are provided, try configureReporting with retry
      if (options.reportOpts?.configureAttributeReporting) {
        const endpoint = this.zclNode.endpoints[this.getClusterEndpoint(clusterId)];
        const clusterName = Object.keys(CLUSTER).find(key => CLUSTER[key] === clusterId);
        const cluster = endpoint?.clusters?.[clusterName] || endpoint?.clusters?.[clusterId];
        
        if (cluster && options.report) {
          const reportConfig = options.reportOpts.configureAttributeReporting;
          ...
          if (!success) {
            // FALLBACK: Setup polling instead
            this.log(`[FALLBACK] 📊 configureReporting failed for ${capability}, using polling`);
            
            // Setup polling interval (use maxInterval from reportOpts)
            const pollInterval = reportConfig.maxInterval * 1000; // Convert to ms
            ...
            const intervalId = setInterval(async () => {
              try {
                if (options.get) {
                  const value = await cluster.readAttributes([options.get]);
                  if (value && value[options.get] != null) {
                    const parsed = options.reportParser ? 
                      options.reportParser(value[options.get]) : 
                      value[options.get];
                    await this.setCapabilityValue(capability, parsed);
                  }
                }
              } catch (pollErr) {
            ...
            this._pollingIntervals.set(capability, intervalId);
            this.log(`[FALLBACK] ✅ Polling active for ${capability} (every ${pollInterval/1000}s)`);
          } else {
            this.log(`[RETRY] ✅ configureReporting succeeded for ${capability}`);
          }
        }
      }
      ...
      this.error(`[REGISTER-RETRY] ❌ Failed to register ${capability}:`, err.message);
      return false;
    }
  }
  
  /**
   * Retry battery read with exponential backoff
   * Handles timing issues where battery cluster isn't ready yet
   */
  async retryBatteryRead(maxRetries = 3) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        this.log(`[BATTERY] 🔄 Retry ${attempt + 1}/${maxRetries} - Reading battery...`);
        ...
        const endpoint = this.zclNode?.endpoints?.[this.getClusterEndpoint(CLUSTER.POWER_CONFIGURATION)];
        if (!endpoint?.clusters?.powerConfiguration) {
          this.log(`[BATTERY] ⚠️ powerConfiguration cluster not available (attempt ${attempt + 1})`);
          continue;
        }
        ...
        if (batteryPercentage?.batteryPercentageRemaining !== undefined) {
          const battery = Math.round(batteryPercentage.batteryPercentageRemaining / 2);
          this.log(`[BATTERY] ✅ Battery read successful: ${battery}%`);
        ...
        this.log(`[BATTERY] ⚠️ Attempt ${attempt + 1} failed: ${err.message}`);
        
        if (attempt === maxRetries - 1) {
          this.log(`[BATTERY] ❌ All ${maxRetries} attempts failed`);
          return null;
        }
      }
    }
