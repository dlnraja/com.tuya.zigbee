...
class BaseHybridDevice extends ZigBeeDevice {
  ...
  _detectPureTuyaDP() {
    try {
      // Check stored settings for TS0601 model
  ...
  async _setupUniversalHybridMode() {
    ...
    try {
      // v5.2.84: Check if this is a pure Tuya DP device that shouldn't use ZCL
      const modelId = this.getSetting?.('zb_modelId') || '';
      const mfr = this.getSetting?.('zb_manufacturerName') || '';
      const isTuyaDP = modelId === 'TS0601' || mfr.startsWith('_TZE');

      if (isTuyaDP) {
        // Check if EF00 cluster is available (indicates Tuya DP device)
        const hasTuyaCluster = this.tuyaEF00Manager?._isInitialized ||
          this.zclNode?.endpoints?.[1]?.clusters?.['tuya'] ||
          this.zclNode?.endpoints?.[1]?.clusters?.['61184'];

        if (hasTuyaCluster) {
          this.log('[HYBRID] ℹ️ Pure Tuya DP device detected (TS0601)');
  ...
  async registerAllCapabilitiesWithReporting() {
    ...
    if (this.hasCapability('measure_humidity') && humidityCluster) {
      try {
        ...
        // v5.2.84: Use RELATIVE_HUMIDITY_MEASUREMENT if available, fallback to direct cluster binding
        const clusterConstant = CLUSTER.RELATIVE_HUMIDITY_MEASUREMENT || CLUSTER.RELATIVE_HUMIDITY || 'relativeHumidity';
