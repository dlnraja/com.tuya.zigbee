...
class BaseHybridDevice extends ZigBeeDevice {
   ...
   */
  /**
   * v5.13.1: ROBUST UNIVERSAL CAPABILITY SETTER (Master implementation)
   *
   * FEATURES:
   * - Throttling & Anti-flooding (via UniversalThrottleManager)
   * - Dynamic capability addition (if in manifest or DYNAMIC_CAPABILITIES)
   * - Bizarre value blocking (sanity checks)
   * - Calibration application (offsets)
   * - Sub-capability flow triggering
   * - Intelligent data point recording
   * - Device availability guarding
   * - Updated lastSeenAt management
   *
   * @param {string} capability - Capability ID
   * @param {any} value - Value to set
   * @param {Object} options - Throttling and other options
   */
  /**
   * v5.13.3: ROBUST UNIVERSAL CAPABILITY SETTER (Hotfix version)
   * Ensures this method is ALWAYS available on the prototype and handles crashes.
   */
  async _safeSetCapability(capability, value, options = {}) {
    try {
      if (!this || this._deleted || !this.homey) return false;

      // 1. Sanity Check - Block bizarre/garbage values
      if (this._blockBizarreValue(capability, value)) return false;

      // 2. Calibration - Apply offsets
      let calibratedValue = this._applyCalibration(capability, value);

      // 3. Contact Inversion (v5.12.2 Forum #1637 fix)
      const invertActive = capability === 'alarm_contact' && (this._invertContact || this._userExplicitInvert);
      if (invertActive && typeof calibratedValue === 'boolean') {
        calibratedValue = !calibratedValue;
      }

      // 4. Dynamic Addition check
      if (!this.hasCapability(capability)) {
        const manifestCaps = this.driver?.manifest?.capabilities || [];
        const isDynamic = BaseHybridDevice.DYNAMIC_CAPABILITIES.includes(capability);
        
        if (manifestCaps.includes(capability) || isDynamic) {
          try {
            await this.addCapability(capability);
            this.log(`[CAP] ✨ Dynamically added: ${capability}`);
          } catch (err) {
            this.log(`[CAP] ⚠️ Could not add ${capability}: ${err.message}`);
            return false;
          }
        } else {
          return false;
      ...
      // 5. Throttling & De-flooding (Sensor-grade)
      ...
      const THROTTLE = {
        ...
        'alarm_water': 1000, 'alarm_smoke': 1000, 'alarm_gas': 1000, 'alarm_co': 1000,
        'alarm_tamper': 2000, 'alarm_vibration': 2000, 'alarm_sos': 1000,
      };
      const throttleMs = THROTTLE[capability] || 10000;

      const SIGNIFICANT = {
        'measure_battery': 2, 
        'measure_temperature': 0.3, 'measure_temperature.probe': 0.3,
      ...
      if (tracker && !options.skipThrottle && !isBooleanChange) {
        ...
        if (elapsed < throttleMs && !sigChange && !valueChanged && !invertActive) {
          return false;
        }
      }
      ...
      // 7. ACTUAL SET
      await this.setCapabilityValue(capability, calibratedValue).catch(err => {
        this.error(`[SDK] Failed to set ${capability}:`, err.message);
      });

      // 8. FLOW TRIGGERS
      if (valueChanged || invertActive) {
  ...
  /**
   * v5.5.497: Block bizarre/garbage values (Sanity Engine)
   */
  _blockBizarreValue(capability, value) {
    if (typeof value !== 'number') return false;

    // Presence/mmWave sensors garbage Temp/Hum
    ...
    // Battery extreme jumps
    if (capability === 'measure_battery') {
      if (value < 0 || value > 100) return true;
      const current = this.getCapabilityValue('measure_battery');
      const now = Date.now();
      const tracker = this._batteryJumpTracker;
      if (current !== null && tracker.lastTime > 0) {
        const jump = Math.abs(value - current);
        const elapsed = now - tracker.lastTime;
        // Block 0-100% jump in < 5s (sleepy artifact)
        if (jump >= 80 && elapsed < 5000) return true;
        // Block 0% if currently > 20%
        if (value === 0 && current > 20) return true;
      }
      tracker.lastValue = value;
      tracker.lastTime = now;
    }

    // Temperature/Humidity valid range
    if (capability === 'measure_temperature' && (value < -40 || value > 100)) return true;
  ...
  /**
   * v5.12.16: Trigger gang-specific flows for multi-gang devices
   */
  async _triggerGangFlows(capability, value) {
    try {
      ...
      if (triggerCard) {
        ...
        this.log(`[FLOW] ⚡ Triggered gang ${gangNum} ${stateStr}`);
      }
    } catch (err) { /* ignore */ }
  }
