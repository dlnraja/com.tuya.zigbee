...
class BaseHybridDevice extends ZigBeeDevice {
  ...
  async _runBackgroundInitialization() {
    try {
      ...
      */
      // v5.8.38: DynamicCapabilityManager ONLY for universal_fallback driver
      // v5.8.36 enabled it globally which broke climate sensors, presence sensors, switches
      // because DP heuristics conflict with existing handlers (DP1=onoff vs alarm_motion, etc)
      const driverId = this.driver?.manifest?.id || this.driver?.id || '';
      if (driverId === 'universal_fallback' || driverId === 'zigbee_universal') {
        try {
          this._dynCapMgr = new DynamicCapabilityManager(this);
          await this._dynCapMgr.initialize();
          this.log('[BACKGROUND] Step 3b/8: DynamicCapabilityManager active (fallback driver)');
        } catch (dcmErr) {
          this.log('[BACKGROUND] Step 3b/8: DynCap init error:', dcmErr.message);
        }
      } else {
        this.log('[BACKGROUND] Step 3b/8: DynamicCapabilityManager skipped (has dedicated driver)');
      }
  ...
  _setupTuyaDPHandler() {
    ...
    if (!this.handleTuyaDP) {
      this.handleTuyaDP = async (data) => {
        ...
        // v5.8.38: Only feed DPs to DynCapMgr for fallback/universal drivers
        // v5.8.36 fed ALL DPs which broke climate sensors, switches, presence sensors
        if (this._dynCapMgr && data?.dp !== undefined) {
          try {
            await this._dynCapMgr.processDP(data.dp, data.value, data.datatype);
          } catch (e) { /* non-critical */ }
        }
