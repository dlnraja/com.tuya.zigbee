...
class BaseHybridDevice extends ZigBeeDevice {
  ...
  async _runBackgroundInitialization() {
    try {
      ...
      */
      ...
      // Step FINAL: Smart Driver Adaptation (SIMPLE CONFIDENCE CHECK)
      this.log('[BACKGROUND] Step FINAL: Smart driver adaptation check...');
      try {
        ...
        if (adaptResult.success && adaptResult.deviceInfo) {
          ...
          // SIMPLE: Check manufacturer/model before applying changes
          const manufacturer = adaptResult.deviceInfo.manufacturerName || '';
          const model = adaptResult.deviceInfo.modelId || '';
          const isUnknown = manufacturer === 'Unknown' || manufacturer === '' || model === 'Unknown' || model === '';
          
          if (isUnknown) {
            // Unknown device - DON'T auto-adapt (too risky)
            this.log('[SMART ADAPT] ⚠️ Unknown device - skipping auto-adaptation for safety');
            this.log('[SMART ADAPT] Manufacturer:', manufacturer);
            this.log('[SMART ADAPT] Model:', model);
            this.log('[SMART ADAPT] 💡 Manual review recommended');
          } else {
            // Known device - OK to adapt
            this.log('[SMART ADAPT] ✅ Known device - adaptations applied');
          }
          
          // Check if migration is needed (simple)
          ...
          if (bestDriver && bestDriver.driverId !== this.driver.id && bestDriver.confidence > 0.7) {
            this.log(`[MIGRATION] ⚠️ Wrong driver detected!`);
            this.log(`[MIGRATION] Current: ${this.driver.id}`);
            this.log(`[MIGRATION] Recommended: ${bestDriver.driverId} (${(bestDriver.confidence * 100).toFixed(0)}%)`);
            
            // Simple notification (no try-catch complexity)
        ...
        this.log('[SMART ADAPT] ⚠️ Skipped:', adaptErr.message);
      }
  ...
  /**
   * Simple capability register with retry (like old versions)
   * Just try 2 times, no complex polling
   * 
  ...
  async registerCapabilityWithRetry(capability, clusterId, options) {
    ...
    try {
      ...
      try {
        ...
        // Failed - but don't crash, just log
        this.log(`[REGISTER] ⚠️ ${capability} registration failed (continuing anyway)`);
        return false;
      }
    }
  }
