...
class HybridPlugBase extends ZigBeeDevice {
  ...
  _startEnergyPoll(zclNode) {
    ...
    this._energyPollTimer = this.homey.setInterval(async () => {
      try {
        ...
        if (attrs.activePower !== undefined && this.hasCapability('measure_power')) {
          await this.setCapabilityValue('measure_power', Math.max(0, attrs.activePower / 10)).catch(() => {});
        ...
        if (attrs.rmsVoltage !== undefined && this.hasCapability('measure_voltage')) {
          await this.setCapabilityValue('measure_voltage', attrs.rmsVoltage / 10).catch(() => {});
          this._energyCapReceived = this._energyCapReceived || {};
          this._energyCapReceived['measure_voltage'] = true;
        }
        if (attrs.rmsCurrent !== undefined && this.hasCapability('measure_current')) {
          await this.setCapabilityValue('measure_current', attrs.rmsCurrent / 1000).catch(() => {});
          this._energyCapReceived = this._energyCapReceived || {};
    ...
    this.log('[PLUG] ⏱️ v5.9.7: Energy poll started (60s interval)');
  }
  ...
  async _setupZCLReporting(zclNode) {
    ...
    if (elec?.configureReporting) {
      await elec.configureReporting({
        activePower: { minInterval: 5, maxInterval: 300, minChange: 5 },
        rmsVoltage: { minInterval: 10, maxInterval: 600, minChange: 5 },
        rmsCurrent: { minInterval: 10, maxInterval: 600, minChange: 50 },
      }).catch(() => {});
    }
    const meter = ep.clusters.seMetering || ep.clusters.metering || ep.clusters[0x0702];
    if (meter?.configureReporting) {
      await meter.configureReporting({ currentSummationDelivered: { minInterval: 30, maxInterval: 900, minChange: 1 } }).catch(() => {});
    }
