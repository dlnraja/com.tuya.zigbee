...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async _setupIASZoneListeners(zclNode) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // v5.5.600: IAS ZONE ENROLLMENT - Required for TS0203 contact sensors
    // Some sensors won't send zone status until properly enrolled
    // ═══════════════════════════════════════════════════════════════════════
    try {
      ...
      // Try to write CIE address to enable zone status reports
      // CIE = Control and Indicating Equipment (the coordinator/hub)
      try {
        const ieeeAddress = this.homey?.zigbee?.ieeeAddress || this.homey?.zigbee?.address;
        if (ieeeAddress && typeof iasCluster.writeAttributes === 'function') {
          await iasCluster.writeAttributes({
            iasCieAddress: ieeeAddress
          }).catch(() => {});
          this.log('[IAS] ✅ CIE address written for zone enrollment');
        }
      } catch (e) {
        this.log('[IAS] ⚠️ CIE address write skipped:', e.message);
      }
      ...
      if (typeof iasCluster.readAttributes === 'function') {
        try {
          ...
          this.log('[IAS] 📖 Initial zone attributes:', attrs);
