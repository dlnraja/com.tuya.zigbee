...
class HybridSensorBase extends BaseHybridDevice {
  ...
  _handleCommonDP(dp, rawValue) {
    ...
    if (hasButtonCapability && !dpIsMapped && buttonDPs.includes(dp) && (value === 0 || value === 1 || value === 2)) {
      ...
      (() => { try { return (() => { try { return this.homey.flow.getTriggerCard('button_pressed'); } catch(e) { return null; } })(); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })().trigger(this, {
        button: dp.toString(),
    ...
    try {
      ...
      if (capability === 'alarm_smoke') {
        ...
        for (const cardId of cardIds) {
          try {
            const card = (() => { try { return this.homey.flow.getTriggerCard(cardId);
            if (card) {
      ...
      if (capability === 'alarm_gas') {
        ...
        for (const cardId of cardIds) {
          try {
            const card = (() => { try { return this.homey.flow.getTriggerCard(cardId); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })();
            if (card) {
      ...
      if (capability === 'measure_luminance' && typeof value === 'number') {
        ...
        try {
          await (() => { try { return (() => { try { return this.homey.flow.getTriggerCard('measure_luminance_changed'); } catch(e) { return null; } })(); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })().trigger(this, { lux: value });
        } catch {
          try {
            await (() => { try { return (() => { try { return this.homey.flow.getTriggerCard('measure_luminance_changed'); } catch(e) { return null; } })(); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })().trigger(this, { lux: value });
          } catch { /* No flow card */ }
