      ...
      try {
        if (typeof iasCluster.readAttributes === 'function') {
          ...
          if (isNullCie) {
            ...
            // v5.11.99: Use IEEEAddressManager (7 methods + retry) instead of inline 3-method lookup
            // FIX Lasse_K forum #802: HOBEIAN ZG-102Z CIE=zeros, bindings=empty
            const ieeeManager = new IEEEAddressManager(this);
            let homeyIeee = await ieeeManager.getCoordinatorIeeeAddress().catch(() => null);
            
            // IEEEAddressManager already tried 7 methods (SDK3 zigbee, address, driver, zclNode, async, network)
            
            if (homeyIeee) {
              // v5.11.99: Use writeCieAddress (multi attr names + 3 retries)
              const cieOk = await ieeeManager.writeCieAddress(iasCluster).catch(() => false);
              this.log(`[IAS] CIE write via IEEEAddressManager: ${cieOk}`);
              ...
              if (!isTuyaDP) {
                ...
                this.setWarning?.('IAS enrollment failed. If sensor not responding, remove and re-add device.');
              }
            }
            ...
            try {
              ...
              if (newState === 1 || newState === 'enrolled') {
                ...
                this._iasEnrollmentFailed = true;
                // v5.11.99: Warn ALL devices (hybrid too — HOBEIAN ZG-102Z needs IAS)
