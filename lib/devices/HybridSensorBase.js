...
class HybridSensorBase extends ZigBeeDevice {
  ...
  _handleIASZoneStatus(status) {
    ...
    this.log(`[IAS-CTX] 🎯 Zone status=${status} (0x${status.toString(16)}) → ${primaryCapability}=${alarmValue} (alarm1=${alarm1}, alarm2=${alarm2}, mfr: ${mfr})`);
  ...
  async _triggerCustomFlowsIfNeeded(capability, value, previousValue) {
    ...
    try {
      ...
      // Smoke detector flows - v5.5.588: Try driver-specific cards first, then generic
      if (capability === 'alarm_smoke') {
        const driverCardId = value ? `${driverId}_alarm_smoke_true` : `${driverId}_alarm_smoke_false`;
        const genericCardId = value ? 'smoke_alarm_triggered' : 'smoke_alarm_cleared';
        
        // Try driver-specific card first
        try {
          await this.homey.flow.getDeviceTriggerCard(driverCardId).trigger(this, { smoke: value });
          this.log(`[FLOW] 🔥 Triggered: ${driverCardId}`);
        } catch {
          // Fallback to generic card
          try {
            await this.homey.flow.getDeviceTriggerCard(genericCardId).trigger(this, { smoke: value });
            this.log(`[FLOW] 🔥 Triggered: ${genericCardId}`);
          } catch { /* No flow card available */ }
        }
      }

      // Gas sensor flows - v5.5.588: Try driver-specific cards first
      if (capability === 'alarm_gas') {
        const driverCardId = value ? `${driverId}_alarm_gas_true` : `${driverId}_alarm_gas_false`;
        const genericCardId = value ? 'gas_alarm_triggered' : 'gas_alarm_cleared';
        try {
          await this.homey.flow.getDeviceTriggerCard(driverCardId).trigger(this, { gas: value });
          this.log(`[FLOW] ⛽ Triggered: ${driverCardId}`);
        } catch {
          try {
            await this.homey.flow.getDeviceTriggerCard(genericCardId).trigger(this, { gas: value });
            this.log(`[FLOW] ⛽ Triggered: ${genericCardId}`);
          } catch { /* No flow card */ }
        }
      }

      // Water leak sensor flows - v5.5.588: Try driver-specific cards first
      if (capability === 'alarm_water') {
        const driverCardId = value ? `${driverId}_alarm_water_true` : `${driverId}_alarm_water_false`;
        const genericCardId = value ? 'water_leak_detected' : 'water_leak_cleared';
        try {
          await this.homey.flow.getDeviceTriggerCard(driverCardId).trigger(this, { water: value });
          this.log(`[FLOW] 💧 Triggered: ${driverCardId}`);
        } catch {
          try {
            await this.homey.flow.getDeviceTriggerCard(genericCardId).trigger(this, { water: value });
            this.log(`[FLOW] 💧 Triggered: ${genericCardId}`);
          } catch { /* No flow card */ }
        }
      }

      // Contact sensor flows - v5.5.588: Try driver-specific cards first
      if (capability === 'alarm_contact') {
        const driverCardId = value ? `${driverId}_contact_opened` : `${driverId}_contact_closed`;
        const genericCardId = value ? 'contact_opened' : 'contact_closed';
        try {
          await this.homey.flow.getDeviceTriggerCard(driverCardId).trigger(this, {});
          this.log(`[FLOW] 🚪 Triggered: ${driverCardId}`);
        } catch {
          try {
            await this.homey.flow.getDeviceTriggerCard(genericCardId).trigger(this, {});
            this.log(`[FLOW] 🚪 Triggered: ${genericCardId}`);
          } catch { /* No flow card */ }
        }
      }
