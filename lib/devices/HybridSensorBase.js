...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // GUARD 2: Detect and reject phantom sub-devices
    // ─────────────────────────────────────────────────────────────────────────
    const deviceData = this.getData();
    if (deviceData.subDeviceId !== undefined) {
      this.error('╔══════════════════════════════════════════════════════════════╗');
      this.error('║ ⚠️  PHANTOM SUB-DEVICE DETECTED - SHOULD NOT EXIST!          ║');
      this.error(`║ subDeviceId: ${deviceData.subDeviceId}`);
      this.error('║ Sensors do NOT have sub-devices!');
      this.error('║ Please DELETE this device from Homey.                        ║');
      this.error('╚══════════════════════════════════════════════════════════════╝');

      await this.setUnavailable(
        `⚠️ Appareil fantôme (subDevice ${deviceData.subDeviceId}). Supprimez cet appareil.`
      ).catch(() => { });
      return;
    }
