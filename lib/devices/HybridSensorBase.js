...
class HybridSensorBase extends ZigBeeDevice {
  ...
  _handleIASZoneStatus(status) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // v5.5.918: CRITICAL FIX - IAS Zone alarm_contact inversion (Lasse_K forum)
    // Z2M semantics: contact=true=CLOSED, contact=false=OPEN
    // Homey semantics: alarm_contact=true=OPEN (alarm!), alarm_contact=false=CLOSED
    // IAS Zone: alarm1=1 means OPEN (alarm state)
    // 
    // ISSUE: dpMappings inverts by default, but IAS Zone didn't - causing inconsistency
    // FIX: For alarm_contact, IAS Zone alarm1=1 already means OPEN, which IS alarm_contact=true
    //      So NO default inversion needed for IAS Zone (it's already correct!)
    //      But user invert_contact setting should still toggle it if needed.
    // ═══════════════════════════════════════════════════════════════════════
    if (primaryCapability === 'alarm_motion' || primaryCapability === 'alarm_contact') {
      const invertSetting = this.getSetting?.('invert_presence') ?? this.getSetting?.('invert_contact');
      if (invertSetting) {
        const inverted = !alarmValue;
        this.log(`[IAS] 🔄 User inversion applied: ${alarmValue} → ${inverted}`);
        alarmValue = inverted;
      }
    }
  ...
  _handleDP(dpId, rawValue) {
    ...
    const THROTTLE_WINDOWS = {
      ...
      'measure_distance': 1000,  // 1s min between distance updates (same value)
    };
  ...
  async _safeSetCapability(capability, value) {
    ...
    const THROTTLE = {
      ...
      'measure_luminance': 10000, 'measure_distance': 5000,
    ...
    const SIGNIFICANT = {
      ...
      'measure_humidity': 2, 'measure_luminance': 50, 'measure_distance': 0.1,
    };
