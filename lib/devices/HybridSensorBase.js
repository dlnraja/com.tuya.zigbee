...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async _handleTimeSyncRequest(frame) {
    try {
      ...
      if (!tuyaCluster) {
        this.log('[TIME-SYNC] ⚠️ Tuya cluster not found, cannot respond');
        return;
      }
  ...
  _handleDP(dpId, rawValue) {
    ...
    if (capability === 'measure_humidity' && typeof value === 'number') {
      // v5.5.431: Z2M-researched firmware bug compensation for TS0201 humidity
      // Some manufacturers report humidity /10, need to multiply back
      // Reference: zigbee-herdsman-converters TS0201_humidity converter
      const mfr = this.getSetting?.('zb_manufacturer_name') || this.getData()?.manufacturerName || '';
      const humidityBuggyManufacturers = ['_TZ3210_ncw88jfq', '_TZ3000_ywagc4rj', '_TZ3000_isw9u95y'];
      if (humidityBuggyManufacturers.includes(mfr) && value < 10 && value > 0) {
        const corrected = value * 10;
        this.log(`[DP] 🔧 Humidity firmware bug compensation: ${value}% → ${corrected}% (${mfr})`);
        value = corrected;
      }
    ...
    if (capability === 'measure_battery' && typeof value === 'number') {
      ...
      // v5.5.368: UNIFIED battery throttling for ALL paths (4x4_Pete forum fix)
      // Same logic as _handleBatteryDP - prevents spam from both Tuya DP and ZCL
      const now = Date.now();
      ...
      // v5.5.368: Global throttle - report if:
      // 1. Battery changed by >= 5% (significant change)
      // 2. OR more than 5 minutes since last report
      // 3. OR this is the first report
      const THROTTLE_MS = 300000;  // 5 minutes
      const SIGNIFICANT_CHANGE = 5;  // 5% change threshold

      if (batteryChange < SIGNIFICANT_CHANGE && timeSinceLastUpdate < THROTTLE_MS && lastBatteryUpdate !== 0) {
        // Suppress spam - silent ignore
        return;
      }
      ...
      if (lastBatteryValue !== null && timeSinceLastUpdate < 60000 && batteryChange > 30) {
        this.log(`[DP] ⚠️ Battery glitch: ${lastBatteryValue}% → ${value}% (${batteryChange}% in ${Math.round(timeSinceLastUpdate / 1000)}s) - IGNORED`);
        return;
      }
