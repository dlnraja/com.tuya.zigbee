...
const SDK3_COMPLIANCE = {
  ...
  INIT_COMMUNICATION_DELAY_MS: 5000,
...
const TIMING = {
  ...
  DATA_RETRY_DELAY_MS: 240000,     // 4 minutes initial retry
  SECOND_RETRY_DELAY_MS: 120000,   // 2 minutes second retry
...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    if (needsReportingConfig) {
      ...
      await this.setStoreValue('lastConfiguredVersion', '5.5.825').catch(() => {});
    }
      ...
      try {
        if (typeof iasCluster.readAttributes === 'function') {
          ...
          if (isNullCie) {
            ...
            // v5.5.807: SDK3 COMPLIANCE - Try to get REAL Homey IEEE address (no hardcoded fallback!)
            // Per SDK3 docs: Homey should have written CIE during pairing, but try recovery
            let homeyIeee = null;
            
            // Method 1: From Homey's Zigbee manager (SDK3 preferred)
            if (this.homey?.zigbee?.ieeeAddress) {
              homeyIeee = this.homey.zigbee.ieeeAddress;
              this.log(`[IAS] 📡 Got Homey IEEE from zigbee manager: ${homeyIeee}`);
            }
            // Method 2: From zclNode network info
            if (!homeyIeee && zclNode?.networkAddress?.ieeeAddr) {
              homeyIeee = zclNode.networkAddress.ieeeAddr;
              this.log(`[IAS] 📡 Got Homey IEEE from network info: ${homeyIeee}`);
            }
            // Method 3: From driver's homey.zigbee
            if (!homeyIeee && this.driver?.homey?.zigbee?.ieeeAddress) {
              homeyIeee = this.driver.homey.zigbee.ieeeAddress;
              this.log(`[IAS] 📡 Got Homey IEEE from driver: ${homeyIeee}`);
            }
            
            if (homeyIeee) {
              // Only write CIE if we have a REAL address (not hardcoded)
              this.log(`[IAS] 📝 Writing verified CIE address: ${homeyIeee}`);
              try {
              ...
              // Send enrollment response after CIE write
              try {
                if (typeof iasCluster.zoneEnrollResponse === 'function') {
                  await iasCluster.zoneEnrollResponse({
                  ...
                  this.log('[IAS] ✅ Sent enrollment response after CIE write');
                }
              } catch (enrollErr) {
                this.log(`[IAS] ⚠️ Enrollment response failed: ${enrollErr.message}`);
              }
            } else {
              ...
              if (!isTuyaDP) {
                ...
                this.setWarning?.('IAS Zone enrollment failed. Please remove and re-add this device.');
              } else {
                // Hybrid device - IAS optional, Tuya DP should work
                this.log('[IAS] ℹ️ Hybrid device - Tuya DP fallback available, IAS optional');
                // Don't show warning - device will work via Tuya DP
              }
            }
            ...
            try {
              ...
              if (newState === 1 || newState === 'enrolled') {
                ...
                this._iasEnrollmentFailed = true;
                // v5.5.850: Only warn for pure IAS devices
                if (!isTuyaDP) {
                  this.setWarning?.('IAS Zone enrollment incomplete. Remove and re-add device if sensor not responding.');
                }
              }
            } catch (verifyErr) {
  ...
  _handleIASZoneStatus(status) {
    ...
    if (primaryCapability === 'alarm_contact') {
      const contactInvertedByDefaultMfrs = [
        // v5.9.13: REMOVED HOBEIAN — follows ZCL standard (alarm1=true=open). Lasse_K confirms still reversed with inversion.
      ...
      const invertSetting = this.getSetting?.('invert_contact');
    ...
    if (primaryCapability === 'alarm_motion') {
      ...
      const fallbackCaps = ['alarm_contact', 'alarm_motion', 'alarm_water', 'alarm_smoke', 'alarm_gas', 'alarm_co', 'alarm_generic.vibration', 'alarm_sos'];
  ...
  async _respondToTimeSyncRequest() {
    ...
    try {
      // v5.10.3: Unix 1970 epoch per official Tuya UART docs (cmd 0x24)
      const now = Math.floor(Date.now() / 1000);
      const tzOffsetSeconds = new Date().getTimezoneOffset() * -60;
      const localTime = now + tzOffsetSeconds;

      this.log(`[TIME-SYNC] 🕐 UTC: ${now} | Local: ${localTime} | TZ: ${tzOffsetSeconds}s`);
  ...
  async _handleTimeSyncRequest(frame) {
    try {
      ...
      const now = Math.floor(Date.now() / 1000);
      const tzOffsetSeconds = new Date().getTimezoneOffset() * -60;
      const utcTimeTuya = now;
      const localTimeTuya = now + tzOffsetSeconds;

      this.log(`[TIME-SYNC] UTC: ${utcTimeTuya} | Local: ${localTimeTuya} | TZ: ${tzOffsetSeconds}s`);
      ...
      if (!sent && timeCluster) {
        try {
          // ZCL Time cluster uses seconds since 2000-01-01 (different from Tuya EF00!)
          const ZCL_EPOCH = 946684800;
          if (typeof timeCluster.writeAttributes === 'function') {
            await timeCluster.writeAttributes({
              time: utcTimeTuya - ZCL_EPOCH,
              timeStatus: 0x02, // Master bit set
              localTime: localTimeTuya - ZCL_EPOCH
            });
  ...
  _handleCommonDP(dp, rawValue) {
    ...
    if (dp === 9 && this.hasCapability('measure_luminance.distance')) {
      ...
      this.setCapabilityValue('measure_luminance.distance', parseFloat(distance)).catch(() => { });
    }
  }
  ...
  static get DYNAMIC_CAPABILITIES() {
    return [
      ...
      'measure_pm25', 'measure_voc', 'measure_luminance.distance',
      'alarm_motion', 'alarm_contact', 'alarm_water', 'alarm_smoke',
      'alarm_gas', 'alarm_co', 'alarm_tamper', 'alarm_battery', 'alarm_generic.vibration',
  ...
  async _safeSetCapability(capability, value) {
    ...
    // v5.8.70: ANTI-FLOOD — Same-value dedup + min-interval throttle
    ...
    // Throttle windows per capability type (ms)
    const THROTTLE = {
      ...
      'alarm_tamper': 2000, 'alarm_generic.vibration': 2000, 'alarm_sos': 1000,
    };
    const throttleMs = THROTTLE[capability] || 10000;

    // Significant change thresholds (bypass throttle if exceeded)
    const SIGNIFICANT = {
      'measure_battery': 2, 'measure_temperature': 0.3,
      'measure_humidity': 2, 'measure_luminance': 50, 'measure_luminance.distance': 0.1,
    };
    ...
    if (tracker) {
      ...
      if (elapsed < throttleMs && !sigChange) return;
    }
