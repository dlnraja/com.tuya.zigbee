...
class HybridSensorBase extends ZigBeeDevice {
  ...
  /**
   * v5.5.111: Read battery while device is awake (universal for all sensors)
   * Called automatically from updateRadioActivity()
   */
  async _readBatteryWhileAwake() {
    try {
      ...
      this.log('[BATTERY-WAKE] 🔋 Device awake - reading battery...');
      ...
      if (data?.batteryPercentageRemaining !== undefined && data.batteryPercentageRemaining !== 255) {
        ...
        this.log(`[BATTERY-WAKE] 🔋 Battery: ${battery}% (raw: ${data.batteryPercentageRemaining})`);
        ...
        this.log(`[BATTERY-WAKE] 🔋 Battery from voltage: ${voltage}V → ${battery}%`);
        await this.setCapabilityValue('measure_battery', battery).catch(() => { });
      }
    } catch (e) {
      // Silent - device may have gone back to sleep
