...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async _setupIASZoneListeners(zclNode) {
    ...
    try {
      ...
      // v5.5.601: CRITICAL FIX for HOBEIAN and similar sensors
      // Force-write CIE address even if device shows "enrolled"
      // The device may be enrolled to an OLD coordinator address
      const homeyZigbee = this.homey?.zigbee || this.getStore()?.zigbee;
      const coordinatorAddress = homeyZigbee?.ieeeAddress || homeyZigbee?.address;
      
      if (coordinatorAddress) {
        this.log(`[IAS] 🔧 Coordinator IEEE: ${coordinatorAddress}`);
        
        // Read current CIE address to check if update needed
        try {
          if (typeof iasCluster.readAttributes === 'function') {
            ...
            // Check if CIE address matches our coordinator
            const cieMatches = currentCie && (
              currentCie === coordinatorAddress ||
            ...
            if (!cieMatches) {
              ...
              this.log('[IAS] 🔧 Force-writing correct CIE address...');
              
              // Force-write the correct CIE address
              if (typeof iasCluster.writeAttributes === 'function') {
                await iasCluster.writeAttributes({
                  iasCieAddress: coordinatorAddress
                }).catch(e => this.log('[IAS] CIE write error:', e.message));
                this.log('[IAS] ✅ CIE address updated to Homey coordinator');
              }
            } else {
              this.log('[IAS] ✅ CIE address already correct');
            }
          }
        } catch (e) {
          this.log('[IAS] ⚠️ CIE check failed, attempting blind write:', e.message);
          // Fallback: blind write CIE address
          if (typeof iasCluster.writeAttributes === 'function') {
            await iasCluster.writeAttributes({
              iasCieAddress: coordinatorAddress
            }).catch(() => {});
        ...
        this.log('[IAS] ⚠️ Could not get coordinator address');
      }
