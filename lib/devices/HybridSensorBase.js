  ...
  _inferCapabilityFromValue(dp, value) {
    ...
    if (isNaN(numVal)) {
      if (value === true || value === false || value === 0 || value === 1) {
        if (dp >= 101) return { capability: 'alarm_motion', value: !!value, confidence: 60 };
        if (dp <= 5) return { capability: 'alarm_contact', value: !!value, confidence: 50 };
      }
      return null;
    }
    ...
    if (numVal >= 0 && numVal <= 100 && Number.isInteger(numVal)) {
      if (dp === 4 || dp === 5 || dp === 14 || dp === 15) return { capability: 'measure_battery', value: numVal };
      if (this.hasCapability('measure_temperature')) return { capability: 'measure_humidity', value: numVal };
    }
    // Illuminance
    if (numVal >= 0 && numVal <= 100000 && [7, 9, 12, 102, 106].includes(dp)) {
