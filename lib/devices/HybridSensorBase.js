...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async _setupTimeSyncBoundCluster() {
    ...
    try {
      ...
      class TimeSyncBoundCluster {
         ...
         * This is called when the device sends commandMcuSyncTime
         */
        mcuSyncTime(payload) {
        ...
        timeResponse(payload) {
          device.log('[TIME-SYNC] timeResponse received');
        }
        ...
        dataReport(payload) {
          ...
          if (payload?.commandIdentifier === 0x24) {
            device.log('[TIME-SYNC] ⏰ Time sync request in dataReport!');
            device._respondToTimeSyncRequest?.();
          }
        }
      }
  ...
  async _respondToTimeSyncRequest() {
    ...
    try {
      ...
      this.log(`[TIME-SYNC] UTC (Tuya): ${utcTimeTuya} | Local: ${localTimeTuya} | TZ: ${tzOffsetSeconds}s`);
