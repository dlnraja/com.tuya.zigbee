        ...
        if (typeof iasCluster.readAttributes === 'function') {
          ...
          if (isNullCie) {
            ...
            try {
              ...
              // v5.11.27: isTuyaDP scoped here to avoid ReferenceError
              const isTuyaDPVerify = this._protocolInfo?.isTuyaDP || this._protocolInfo?.hasTuyaCluster;
              ...
              this._iasEnrollmentFailed = true;
              // v5.5.850: Only warn for pure IAS devices
              const isTuyaDPCatch = this._protocolInfo?.isTuyaDP || this._protocolInfo?.hasTuyaCluster;
              if (!isTuyaDPCatch) {
                this.setWarning?.('IAS Zone enrollment incomplete. Remove and re-add device if sensor not responding.');
              }
            }
  ...
  _handleIASZoneStatus(status) {
    ...
    if (primaryCapability === 'alarm_contact') {
      const contactInvertedByDefaultMfrs = [
        // v5.11.26: RE-ADDED HOBEIAN — Lasse_K forum #1513/#1515 confirms inverted again after v5.9.13 removal
        'HOBEIAN',
