...
class HybridSensorBase extends ZigBeeDevice {
  ...
  _handleBatteryDP(dp, value) {
    if (typeof value === 'number' && value >= 0 && value <= 100) {
      // v5.5.353: Battery report throttling for spam prevention (4x4_Pete forum #830)
      ...
      const isZG204ZM = ['_TZE200_3towulqd', '_tze200_3towulqd'].includes(manufacturerName);

      // For ZG-204ZM: only report battery changes > 5% or every 6 hours
      if (isZG204ZM) {
        const lastBatteryReport = this._lastBatteryReport || 0;
        const timeSinceLastReport = Date.now() - lastBatteryReport;
        const batteryChange = Math.abs((current || 0) - value);

        // Only report if significant change OR enough time passed
        if (batteryChange >= 5 || timeSinceLastReport > 6 * 60 * 60 * 1000) {
          this.log(`[BATTERY] 🔋 ZG-204ZM Battery: ${value}% (change: ${batteryChange}%, time: ${Math.round(timeSinceLastReport / 60000)}min)`);
          this.setCapabilityValue('measure_battery', value).catch(() => { });
          this._lastBatteryReport = Date.now();
        } else {
          this.log(`[BATTERY] ⏸️ ZG-204ZM Battery: ${value}% (throttled - change: ${batteryChange}%)`);
        }
      } else {
        // Normal battery reporting for other devices
  ...
  _handleIASZoneStatus(status) {
    ...
    this.log(`[IAS-CTX] 🎯 Zone status=${status} → ${primaryCapability}=${alarm1} (mfr: ${mfr})`);

    // Set the primary alarm capability
    if (this.hasCapability(primaryCapability)) {
      this._safeSetCapability(primaryCapability, alarm1);
    } else {
      ...
      for (const cap of fallbackCaps) {
        if (this.hasCapability(cap)) {
          this._safeSetCapability(cap, alarm1);
          this.log(`[IAS-CTX] 📝 Fallback: ${cap}=${alarm1}`);
          break;
        }
      }
    }
        ...
        if (possibleType >= 0x00 && possibleType <= 0x05) {
          ...
          if (len > 0 && len < 256 && offset + 4 + len <= frame.length) {
            ...
            if (possibleType === 0x01 && len === 1) { // Bool
              value = valueData[0] !== 0;
            } else if (possibleType === 0x02 && len === 4) { // Value (4-byte signed)
  ...
  _handleDP(dpId, rawValue) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // v5.5.305: UNIVERSAL SANITY CHECKS - Reject out-of-range/invalid values
    // This prevents erratic readings from corrupted data or wrong divisors
    // v5.5.305: Added NaN check to fix DP12 illuminance NaN issue
    // ═══════════════════════════════════════════════════════════════════════
    if (value === null || value === undefined) {
    ...
    // v5.5.305: CRITICAL FIX - Reject NaN values (fixes DP12 illuminance NaN issue)
    if (typeof value === 'number' && isNaN(value)) {
      this.log(`[DP] ⚠️ DP${dp} → ${capability} = NaN (invalid data, raw: ${JSON.stringify(rawValue)}) - IGNORED`);
      return;
    }

    if (capability === 'measure_temperature' && typeof value === 'number') {
      if (value < -40 || value > 80) {
        this.log(`[DP] ⚠️ Temperature out of range: ${value}°C (raw: ${rawValue}) - IGNORED`);
        return;
      }
    }
    ...
    if (capability === 'measure_humidity' && typeof value === 'number') {
      if (value < 0 || value > 100) {
        this.log(`[DP] ⚠️ Humidity out of range: ${value}% (raw: ${rawValue}) - IGNORED`);
        return;
      }
    }
    ...
    if (capability === 'measure_battery' && typeof value === 'number') {
      if (value < 0 || value > 100) {
        this.log(`[DP] ⚠️ Battery out of range: ${value}% (raw: ${rawValue}) - clamping`);
        value = Math.max(0, Math.min(100, value));
      }

      // v5.5.139: Anti-spam filter for erratic battery reports (ZG-204ZM issue)
      ...
      if (lastBatteryValue !== null && timeSinceLastUpdate < 60000) {
        const batteryDiff = Math.abs(value - lastBatteryValue);
        if (batteryDiff > 30) {
          this.log(`[DP] ⚠️ Battery spam detected: ${lastBatteryValue}% → ${value}% (${batteryDiff}% change in ${Math.round(timeSinceLastUpdate / 1000)}s) - IGNORED`);
          return;
        }
      }

      // Debounce: ignore same value within 30 seconds
      if (lastBatteryValue === value && timeSinceLastUpdate < 30000) {
    ...
    if (capability === 'measure_luminance' && typeof value === 'number') {
      if (value < 0 || value > 100000) {
        this.log(`[DP] ⚠️ Luminance out of range: ${value} lux (raw: ${rawValue}) - IGNORED`);
        return;
      }
    }

    if (capability === 'measure_pressure' && typeof value === 'number') {
      if (value < 300 || value > 1100) {
        this.log(`[DP] ⚠️ Pressure out of range: ${value} hPa (raw: ${rawValue}) - IGNORED`);
        return;
      }
    }

    if (capability === 'measure_co2' && typeof value === 'number') {
      if (value < 0 || value > 10000) {
        this.log(`[DP] ⚠️ CO2 out of range: ${value} ppm (raw: ${rawValue}) - IGNORED`);
        return;
      }
    }
  ...
  async _triggerCustomFlowsIfNeeded(capability, value, previousValue) {
    ...
    try {
      ...
      if (capability === 'alarm_smoke') {
        const cardId = value ? 'smoke_alarm_triggered' : 'smoke_alarm_cleared';
        await this.homey.flow.getDeviceTriggerCard(cardId)
          .trigger(this, { smoke: value }).catch(() => { });
        this.log(`[FLOW] 🔥 Triggered: ${cardId}`);
      }

      // Gas sensor flows
      if (capability === 'alarm_gas') {
        const cardId = value ? 'gas_alarm_triggered' : 'gas_alarm_cleared';
        await this.homey.flow.getDeviceTriggerCard(cardId)
          .trigger(this, { gas: value }).catch(() => { });
        this.log(`[FLOW] ⛽ Triggered: ${cardId}`);
      }

      // Water leak sensor flows
      if (capability === 'alarm_water') {
        const cardId = value ? 'water_leak_detected' : 'water_leak_cleared';
        await this.homey.flow.getDeviceTriggerCard(cardId)
          .trigger(this, { water: value }).catch(() => { });
        this.log(`[FLOW] 💧 Triggered: ${cardId}`);
      }
