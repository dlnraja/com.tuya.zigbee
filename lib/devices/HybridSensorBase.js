...
/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║           HybridSensorBase - v5.3.73 UNIVERSAL HANDLER                       ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║                                                                              ║
...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    this.log('');
    this.log('╔══════════════════════════════════════════════════════════════╗');
    this.log('║          HYBRID SENSOR BASE v5.3.73 UNIVERSAL                ║');
    this.log('╠══════════════════════════════════════════════════════════════╣');
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // STEP 5: Setup ADDITIONAL listeners based on protocol (fallback/redundancy)
    // ─────────────────────────────────────────────────────────────────────────
  ...
  _handleZCLData(cluster, attr, value) {
    ...
    // Map ZCL attributes to capabilities
    const zclMappings = {
      ...
      'illuminanceMeasurement.measuredValue': { cap: 'measure_luminance', transform: (v) => Math.round(Math.pow(10, (v - 1) / 10000)) },
      'powerConfiguration.batteryPercentageRemaining': { cap: 'measure_battery', div: 2 },
      'genPowerCfg.batteryPercentageRemaining': { cap: 'measure_battery', div: 2 },
      'occupancySensing.occupancy': { cap: 'alarm_motion', transform: (v) => v > 0 },
      'msOccupancySensing.occupancy': { cap: 'alarm_motion', transform: (v) => v > 0 },
      'iasZone.zoneStatus': { cap: 'alarm_contact', transform: (v) => (v & 1) !== 0 }
    };

    const key = `${cluster}.${attr}`;
    const mapping = zclMappings[key];

    if (mapping) {
      let finalValue = value;
      if (mapping.div) finalValue = value / mapping.div;
      if (mapping.transform) finalValue = mapping.transform(value);

      this.log(`[ZCL-DATA] → ${mapping.cap} = ${finalValue}`);
      this._safeSetCapability(mapping.cap, finalValue);
    }
  }
