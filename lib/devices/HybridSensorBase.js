...
class HybridSensorBase extends ZigBeeDevice {
  ...
  _handleIASZoneStatus(status) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // v5.5.791: PROJECT-WIDE PRESENCE INVERSION SUPPORT
    // Some sensors report inverted values - support invert_presence setting
    // This applies to motion/presence sensors with IAS Zone
    // ═══════════════════════════════════════════════════════════════════════
    if (primaryCapability === 'alarm_motion' || primaryCapability === 'alarm_contact') {
      ...
      if (invertSetting) {
        ...
        this.log(`[IAS] 🔄 Inversion applied: ${alarmValue} → ${inverted}`);
        alarmValue = inverted;
      }
    }
