...
class HybridSensorBase extends ZigBeeDevice {
  ...
  _handleIASZoneStatus(status) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // v5.5.971: FIX - IAS Zone alarm_water inversion (Lasse_K forum #1274)
    // Some water sensors report inverted IAS Zone status
    // User setting invert_alarm should toggle it
    // ═══════════════════════════════════════════════════════════════════════
    if (primaryCapability === 'alarm_water') {
      const invertSetting = this.getSetting?.('invert_alarm');
      if (invertSetting) {
        const inverted = !alarmValue;
        this.log(`[IAS] 🔄 Water alarm inversion applied: ${alarmValue} → ${inverted}`);
        alarmValue = inverted;
      }
    }
    
    // ═══════════════════════════════════════════════════════════════════════
    // v5.5.918: FIX - Apply default inversion for known inverted manufacturers
    // Some manufacturers report IAS Zone inverted (closed=alarm1, open=no alarm)
    // This matches the invertedByDefault list in contact_sensor/device.js
    // ═══════════════════════════════════════════════════════════════════════
    if (primaryCapability === 'alarm_contact') {
      const invertedByDefaultMfrs = [
      ...
      const needsDefaultInvert = invertedByDefaultMfrs.some(id => 
        mfr.toLowerCase().includes(id.toLowerCase())
      );
      if (needsDefaultInvert && !this.getSetting?.('invert_contact')) {
        const inverted = !alarmValue;
        this.log(`[IAS] 🔄 Default inversion for ${mfr}: ${alarmValue} → ${inverted}`);
        alarmValue = inverted;
      }
    }
