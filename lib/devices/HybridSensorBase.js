...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async _setupIASZoneListeners(zclNode) {
    ...
    try {
      ...
      // v5.5.646: CRITICAL FIX for HOBEIAN and similar sensors
      // Force-write CIE address even if device shows "enrolled"
      // The device may be enrolled to an OLD coordinator address
      // SDK3: Get coordinator address from multiple sources
      let coordinatorAddress = null;
      try {
        // Method 1: From homey.zigbee (if available)
        coordinatorAddress = this.homey?.zigbee?.ieeeAddress;
        // Method 2: From driver manifest
        if (!coordinatorAddress) {
          coordinatorAddress = this.driver?.manifest?.zigbee?.endpoints?.[0]?.ieeeAddress;
        }
        // Method 3: From zclNode (device's own address as fallback for CIE)
        if (!coordinatorAddress && zclNode) {
          // Use Homey's coordinator - typically 0x00124b0000000000 format
          // We'll read from the device and use that
        }
      } catch (e) {
        this.log(`[IAS] ⚠️ Coordinator lookup error: ${e.message}`);
      }
      
      this.log(`[IAS] 🔧 Coordinator IEEE: ${coordinatorAddress || 'not found'}`);
      
      // v5.5.646: Read CIE address and zone status regardless of coordinator lookup
      // The device may already be enrolled correctly to Homey
      try {
        if (typeof iasCluster.readAttributes === 'function') {
          const cieAttrs = await iasCluster.readAttributes(['iasCieAddress', 'zoneState']).catch(() => ({}));
          const currentCie = cieAttrs?.iasCieAddress;
          const zoneState = cieAttrs?.zoneState;
          
          this.log(`[IAS] 📖 Current CIE: ${currentCie}, Zone state: ${zoneState}`);
          
          // Only try to update CIE if we have a coordinator address and it doesn't match
          if (coordinatorAddress && currentCie) {
            const cieMatches = currentCie === coordinatorAddress ||
              (typeof currentCie === 'string' && typeof coordinatorAddress === 'string' &&
               currentCie.toLowerCase() === coordinatorAddress.toLowerCase());
            
            if (!cieMatches) {
              this.log(`[IAS] ⚠️ CIE mismatch! Device: ${currentCie} vs Homey: ${coordinatorAddress}`);
              if (typeof iasCluster.writeAttributes === 'function') {
                await iasCluster.writeAttributes({ iasCieAddress: coordinatorAddress })
                  .catch(e => this.log('[IAS] CIE write error:', e.message));
                this.log('[IAS] ✅ CIE address updated');
              }
            } else {
              this.log('[IAS] ✅ CIE address correct');
            }
          }
        }
      } catch (e) {
