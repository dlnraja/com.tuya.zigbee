...
class HybridSensorBase extends ZigBeeDevice {
  ...
  async _setupTuyaDPMode() {
    ...
    this.log('[TUYA-DP] ✅ Data arrives via onTuyaStatus() callback');
    this.log('[TUYA-DP] ════════════════════════════════════════════════════');
    ...
    // Setup additional cluster listeners as fallback
    this._setupTuyaClusterListeners();
  ...
  /**
   * CRITICAL: Override onTuyaStatus from ZigBeeDevice
   * This is the STANDARD way homey-zigbeedriver delivers Tuya DP data
   * Called by the framework when cluster 0xEF00 receives data
   */
  onTuyaStatus(status) {
    this.log('[TUYA-DP] 📥 onTuyaStatus received!');

    if (!status) {
      this.log('[TUYA-DP] ⚠️ Empty status received');
      return;
    }

    // Log raw status for debugging
    this.log(`[TUYA-DP] Raw status: ${JSON.stringify(status).substring(0, 200)}`);

    // Handle different status formats
    try {
      // Format 1: { dp: X, datatype: Y, data: Z } (single DP)
      if (status.dp !== undefined && status.data !== undefined) {
        this._handleDP(status.dp, status.data);
        return;
      }

      // Format 2: Array of datapoints
      if (Array.isArray(status)) {
        for (const dp of status) {
          if (dp.dp !== undefined) {
            this._handleDP(dp.dp, dp.data || dp.value);
          }
        }
        return;
      }
      ...
      // Format 3: { datapoints: [...] }
      if (status.datapoints && Array.isArray(status.datapoints)) {
        for (const dp of status.datapoints) {
          this._handleDP(dp.dp || dp.dpId, dp.data || dp.value);
        }
        return;
      }

      // Format 4: Direct object with dpId
      if (status.dpId !== undefined) {
        this._handleDP(status.dpId, status.dpValue || status.data);
        return;
      }

      this.log('[TUYA-DP] ⚠️ Unknown status format');
    } catch (err) {
      this.error('[TUYA-DP] Error processing status:', err.message);
    }
  }

  _setupTuyaClusterListeners() {
    try {
      ...
      const tuyaCluster = endpoint.clusters.tuya ||
        ...
        endpoint.clusters['manuSpecificTuyaSend'];
      ...
      // Setup various event listeners as fallback
      const events = ['response', 'dataReport', 'reporting', 'report', 'datapoint'];
      ...
      this.log('[TUYA-DP] ✅ Cluster listeners registered');
    } catch (err) {
      this.log('[TUYA-DP] ⚠️ Could not setup cluster listeners:', err.message);
    }
  }
