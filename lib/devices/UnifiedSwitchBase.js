  ...
  _registerButtonCapabilityListeners() {
    ...
    for (let gang = 1; gang <= this.gangCount; gang++) {
      ...
      this.registerCapabilityListener(capability, async () => {
        ...
        const isActuator = /button_wireless_(plug|switch|usb|valve|fingerbot)/.test(driverId);
        const isSceneRemote = !isActuator && (
          this._forcedDeviceType === 'BUTTON'
          || this.driver?.manifest?.class === 'button'
          || /^(button_wireless(_\d+)?|scene_switch|smart_remote|remote_button|wall_remote|handheld_remote|smart_knob|button_emergency)/.test(driverId)
        );
