...
class UnifiedSwitchBase extends TuyaZigbeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    let initError = null;
    try {
      // v5.12.53 (P92.121): Tuya magic packet FIRST — Tuya's own docs
    ...
    // v5.12.53 (P92.121): re-apply persisted LED/backlight/child-lock
    // settings at boot (onSettings only fires on change).
    try {
      await this._applyStoredOnOffSettings();
    } catch (e) {
      this.error(`[HYBRID-SWITCH] ⚠️ Stored onoff settings: ${e.message}`);
    }

    // v5.12.59 (P92.126): virtual energy estimation when the hardware
    // exposes NO metering path (no electricalMeasurement/metering cluster,
    // no energy DP). On-time × configurable load (estimated_load_w), per
    // gang, persisted. Disabled via the enable_power_estimation setting.
    try {
    ...
    // v5.12.50 (backport P92.114): physical button detection for ALL
    // UnifiedSwitchBase subclasses (base does not chain to
    // TuyaZigbeeDevice.onNodeInit where initPhysicalButtonDetection lives).
    try {
  ...
  _detectProtocol() {
    ...
    const isTuyaDP = modelId.toUpperCase() === 'TS0601' || mfr.toUpperCase().startsWith('_TZE') || hasTuyaCluster;
    ...
    for (let gang = 1; gang <= this.gangCount; gang++) {
      ...
      if (this.hasCapability(capability)) {
        ...
        if (needsCommandListeners) {
          ...
          const handlePhysicalButton = async (cmdName, newValue) => {
            ...
            if (!this.protocolOptimizer || this.protocolOptimizer.isActive('zcl')) {
              ...
              // v5.13.1: CRITICAL FIX (Issue #170) — Route through safeSetCapabilityValue
              // to trigger custom per-gang Flow cards on physical button presses
              this.safeSetCapabilityValue(capability, newValue);
            }
          };
        ...
        onOffCluster.readAttributes(['onOff']).then(async data => {
          if (data?.onOff != null) {
            await this.safeSetCapabilityValue(capability, data.onOff).catch(() => { });
          }
        }).catch(() => { });
      }
    }
  ...
  _registerButtonCapabilityListeners() {
    // v5.12.50: skip if the universal TuyaZigbeeDevice version already ran.
    ...
    // v5.12.51 (P92.119): named maintenance buttons (button.toggle,
    ...
    try {
      if (this._isPureTuyaDP && this.tuyaEF00Manager) {
        try {
          ...
          if (!cluster) {throw dpErr;}
          try {
            await (value ? cluster.setOn() : cluster.setOff());
          } catch (e) {
            this.log(`[TX-FALLBACK] ZCL also failed: ${e.message}`);
            throw e;
          }
        }
      } else {
        ...
        if (cluster) {
          try {
            ...
            if (this.gangCount > 1 && this._isPureTuyaDP !== true) {
              this.log(`[TX-FALLBACK] ZCL failed for gang ${gang}; skipping leftover EF00`);
              throw zclErr;
            }
            ...
            try {
              ...
              this.log(`[TX-FALLBACK] Tuya DP also failed: ${e.message}`);
              throw e;
            }
          }
        } else {
          if (this.gangCount > 1 && this._isPureTuyaDP !== true) {
            throw new Error(`No onOff cluster on EP${gang}; skipping leftover EF00`);
          }
          this.log(`[TX-FALLBACK] No onOff cluster on EP${gang}, trying Tuya DP...`);
          if (typeof this._sendTuyaDP !== 'function') {
            throw new Error(`No onOff cluster or Tuya DP sender available for gang ${gang}`);
          }
          try {
            await this._sendTuyaDP(gang, value ? 1 : 0, 'bool');
          } catch (e) {
            this.log(`[TX-FALLBACK] Tuya DP failed for gang ${gang}: ${e.message}`);
            throw e;
          }
        }
      }
    ...
    try {
      ...
      if (data) {
        ...
        if (data.powerOnBehavior !== undefined && data.powerOnBehavior !== null) {
          ...
          const stored = typeof this.getSetting === 'function' ? this.getSetting('power_on_behavior') : null;
          if (stored && stored !== val && !(stored === 'previous' && val === 'memory')) {
            const pobValue = { off: 0, on: 1, memory: 2, previous: 2 }[stored] ?? 2;
            this.log(`[E001] powerOnBehavior dump=${data.powerOnBehavior} ≠ Homey ${stored} — re-force`);
            await this._writeE001Attribute('powerOnBehavior', pobValue);
          } else if (!stored) {
            this.log(`[E001] Read powerOnBehavior=${data.powerOnBehavior} → ${val}`);
            await this.setSettings({ power_on_behavior: val }).catch(() => {});
            if (this.hasCapability('power_on_behavior')) {
              const capVal = { off: 'off', on: 'on', memory: 'previous' }[val] ?? 'previous';
              await this.safeSetCapabilityValue('power_on_behavior', capVal).catch(() => {});
            }
          }
        }
        if (data.switchMode !== undefined && data.switchMode !== null) {
          ...
          const stored = typeof this.getSetting === 'function' ? this.getSetting('switch_mode') : null;
          if (stored && stored !== val) {
            const smValue = { toggle: 0, state: 1, momentary: 2 }[stored] ?? 0;
            this.log(`[E001] switchMode dump=${data.switchMode} ≠ Homey ${stored} — re-force`);
            await this._writeE001Attribute('switchMode', smValue);
          } else if (!stored) {
  ...
  async _writeOnOffAttribute(attrName, value) {
    ...
    // v5.12.53 (P92.121): skip silently once known unsupported on this
    ...
    try {
      ...
      // v5.12.53: mark unsupported once — no per-boot/per-change log flood
  ...
  /**
   * v5.12.53 (P92.121): apply persisted on/off-cluster settings at boot.
   * onSettings only fires on CHANGE, so LED/backlight/child-lock choices
   * were lost on every app restart until the user re-touched the setting.
   * ZCL write first, Tuya DP fallback (classic Tuya split-brain).
   */
  async _applyStoredOnOffSettings() {
    try {
      const backlight = typeof this.getSetting === 'function' ? this.getSetting('backlight_mode') : null;
      if (backlight) {
        const value = { off: 0, normal: 1, inverted: 2 }[backlight] ?? 1;
        if (!(await this._writeOnOffAttribute('backlightMode', value))) {
          if (typeof this._sendTuyaDP === 'function') {
            await this._sendTuyaDP(15, value, 'enum').catch(() => {});
          }
        }
        this.log(`[SETTINGS] backlight_mode applied at boot: ${backlight}`);
      }
      const pob = typeof this.getSetting === 'function' ? this.getSetting('power_on_behavior') : null;
      if (pob) {
        const pobValue = { off: 0, on: 1, memory: 2, previous: 2 }[pob] ?? 2;
        if (!(await this._writeE001Attribute('powerOnBehavior', pobValue))) {
          if (typeof this._sendTuyaDP === 'function') {
            await this._sendTuyaDP(14, pobValue, 'enum').catch(() => {});
          }
        }
        this.log(`[SETTINGS] power_on_behavior applied at boot: ${pob}`);
      }
      ...
      if (childLock !== undefined && childLock !== null && childLock !== '') {
        ...
        this.log(`[SETTINGS] child_lock applied at boot: ${locked}`);
      }
    } catch (e) {
      this.log(`[SETTINGS] ⚠️ stored onoff settings: ${e.message}`);
    }
  }
      ...
      try {
        ...
        if (onOffCluster && typeof onOffCluster.readAttributes === 'function') {
          ...
          if (attrs?.onOff !== undefined) {
            await this.safeSetCapabilityValue(capability, attrs.onOff).catch(() => { });
