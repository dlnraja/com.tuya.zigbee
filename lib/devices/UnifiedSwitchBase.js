...
class UnifiedSwitchBase extends TuyaZigbeeDevice {
  ...
  _registerButtonCapabilityListeners() {
    if (!this._buttonCapabilityListenersRegistered) {
      this._buttonCapabilityListenersRegistered = new Set();
    }

    for (let gang = 1; gang <= this.gangCount; gang++) {
      const capability = `button.${gang}`;
      if (!this.hasCapability(capability) || this._buttonCapabilityListenersRegistered.has(capability)) {
        continue;
      }

      this.registerCapabilityListener(capability, async () => {
        this.log(`[HYBRID-SWITCH] Virtual ${capability} pressed`);
        if (typeof this.triggerButtonPress === 'function') {
          await this.triggerButtonPress(gang, 'single', 1, { source: 'virtual' });
        } else if (typeof this._triggerPhysicalFlow === 'function') {
          await this._triggerPhysicalFlow(gang, 'single', { source: 'virtual', _internalTrigger: true });
        }
        const stateCapability = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
        const current = this.hasCapability(stateCapability) ? this.getCapabilityValue(stateCapability) : false;
        await this._setGangOnOff(gang, !current);
        return true;
      });

      this._buttonCapabilityListenersRegistered.add(capability);
    }
  }
  ...
  async _setGangOnOff(gang, value) {
    ...
    if (this._isPureTuyaDP && this.tuyaEF00Manager) {
      try {
        ...
        if (cluster) {
          await (value ? cluster.setOn() : cluster.setOff()).catch(e =>
      ...
      if (cluster) {
        try {
          ...
          await this._sendTuyaDP(gang, value ? 1 : 0, 'bool').catch(e =>
        ...
        await this._sendTuyaDP(gang, value ? 1 : 0, 'bool').catch(() => {});
      }
    }
  }
