...
class UnifiedSwitchBase extends TuyaZigbeeDevice {
  ...
  get dpMappings() {
    ...
    return {
      ...
      // P2199: DP15 = backlight enum (off/normal/inverted) — not bool led_indicator
      15: {
        capability: null,
        setting: 'backlight_mode',
        transform: (v) => ({ 0: 'off', 1: 'normal', 2: 'inverted' }[v] ?? 'normal'),
      },
    };
  }
  ...
  async onNodeInit({ zclNode }) {
    ...
    // P2221: BIDIRECTIONAL vision — virtual UI + Homey button.N listeners MUST
    // run here too (TuyaZigbeeDevice.onNodeInit is not chained). Without this,
    // wall switches get physical RX but dead Homey UI / Missing Capability Listener.
    try {
      if (typeof this.initVirtualButtons === 'function') {
        await this.initVirtualButtons();
      }
    } catch (e) {
      this.log(`[HYBRID-SWITCH] ⚠️ Virtual buttons: ${e.message}`);
    }
    try {
      if (typeof this._registerButtonCapabilityListeners === 'function') {
        await Promise.resolve(this._registerButtonCapabilityListeners());
      }
    } catch (e) {
      this.log(`[HYBRID-SWITCH] ⚠️ UI button listeners: ${e.message}`);
    }
  ...
  async _migrateCapabilities() {
    // WHY(P2397): heal missing onoff.gangN / button.N before listeners bind
    try {
  ...
  _registerButtonCapabilityListeners() {
    ...
    // WHY(P2397): soft-ensure UI caps before wiring Homey listeners
    try {
      const { ensureGangUiCapabilities } = require('../utils/ensureGangUiCapabilities');
      Promise.resolve(ensureGangUiCapabilities(this)).catch(() => {});
    } catch (_e) { /* soft */ }

    let gangMax = this.gangCount || 1;
    try {
      const { resolveGangCount } = require('../utils/BidirectionalButtonState');
      gangMax = resolveGangCount(this);
    } catch (_e) { /* soft */ }

    for (let gang = 1; gang <= gangMax; gang++) {
      ...
      this.registerCapabilityListener(capability, async () => {
        ...
        // P2221 / P2235: prefer VirtualButtonMixin for real switches; scene remotes → flow only
        ...
        } else if (!isSceneRemote) {
          if (typeof this.markAppCommand === 'function') {
            const cap = capabilityForOnOffEndpoint(gang, this.gangCount)
              || (gang === 1 ? 'onoff' : `onoff.gang${gang}`);
            const cur = this.getCapabilityValue(cap) === true;
            this.markAppCommand(gang, !cur);
          }
  ...
  async _setGangOnOff(gang, value) {
    ...
    // WHY(P2704): house snappyTx (HOBEIAN / ltt60asa) — skip inter-cmd jitter +
    // shorter retry so Homey UI / Flow → relay feels instant.
    try {
      const { writeCapabilityWithFallbacks } = require('../zigbee/CapabilityCommandRouter');
      this._capabilityEndpointHint = gang;
      const profile = (typeof this.getDeviceProfile === 'function' && this.getDeviceProfile()) || {};
      const snappy = profile.snappyTx === true;
      const r = await paceZigbeeCommand(this, () => writeCapabilityWithFallbacks(this, capability, value, {
        ...
        enabled: this.gangCount > 1 && !snappy,
        minJitter: snappy ? 0 : 15,
        maxJitter: snappy ? 0 : 50,
        maxRetries: snappy ? 2 : 3,
        retryDelayMs: snappy ? 120 : 350,
      });
      if (r.ok) {return;}
   ...
   * Contre quoi: blind overwrite that orphaned PhysicalButtonMixin 0xFD.
   */
  _setupRawFrameFallback() {
    // Parent registers tag `raw-l0-fallback` via BidirectionalButtonState.wrapHandleFrame
    ...
    if (oldValue !== value) {
      ...
      if (capability.startsWith('onoff')) {
        try {
          ...
          // WHY(P2331/P2334/P2644): declared-only via CoreCapabilityMixin / _safeTriggerFlow.
          // Prefer bare `_turned_*` when driverId already ends with `_Ngang`.
          if (typeof this._triggerGangFlows === 'function') {
            await this._triggerGangFlows(capability, value);
          } else if (typeof this._safeTriggerFlow === 'function') {
            const driverId = this.driver.id;
            const { buildOnOffTurnedFlowCandidates } = require('../flow/FlowCardHeuristics');
            const candidates = buildOnOffTurnedFlowCandidates(driverId, gangNum, value);
            for (const tid of candidates) {
              // eslint-disable-next-line no-await-in-loop
              if (await this._safeTriggerFlow(tid, { gang: gangNum, button: String(gangNum) }, { type: 'usb-gang' })) {
                this.log(`[FLOW] ⚡ Triggered ${tid}`);
                break;
              }
            }
          }
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    for (const key of changedKeys) {
      try {
        ...
        case 'led_indicator':
          // P2199: legacy bool setting → DP15 backlight enum (same wire as backlight_mode)
          {
            const legacyMode = newSettings[key] ? 'normal' : 'off';
            const legacyValue = BACKLIGHT_TO_ZCL[legacyMode] ?? 1;
            if (await this._writeOnOffAttribute('backlightMode', legacyValue)) {
              this.log(`[SETTINGS] LED indicator (legacy→backlight): ${legacyMode}`);
            } else {
              await this._sendTuyaDP(15, legacyValue, 'enum');
              this.log(`[SETTINGS] LED indicator (legacy→backlight): ${legacyMode} (DP15=${legacyValue})`);
            }
            if (typeof this.getSetting === 'function' && this.getSetting('backlight_mode') !== legacyMode) {
              await this.setSettings({ backlight_mode: legacyMode }).catch(() => {});
            }
          }
          break;
  ...
  async _setupReporting(zclNode) {
    ...
    // WHY(P2682): default 1s floor — Bastien diag f37e8a91 saw interval=90ms count=241
    // when min was 0; still immediate enough for wall switches, calms mesh retransmits.
    const minReportInterval = parseInt(settings.report_min_interval, 10);
    const minIv = Number.isFinite(minReportInterval) && minReportInterval >= 0
      ? minReportInterval
      : 1;
    const maxReportInterval = parseInt(settings.report_max_interval, 10) || 300;    // Default: 5 min
    ...
    for (let gang = 1; gang <= this.gangCount; gang++) {
      try {
        ...
        if (onOffCluster && typeof onOffCluster.configureReporting === 'function') {
          ...
          this.log(`[REPORTING] ✅ EP${gang} onOff reporting configured (min=${minIv}s, max=${maxIv}s)`);
        }
      } catch (err) {
  ...
  async _sendTuyaDP(dpId, value, dataType = 'bool') {
    ...
    try {
      ...
      if (typeof tuyaCluster.datapoint === 'function') {
        // WHY(P2711): full EF00 frame — never pass `value` (unexpected property)
