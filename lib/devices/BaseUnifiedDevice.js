...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
   ...
   */
  static _safeBatteryPercent(rawValue) {
    if (rawValue === null || rawValue === undefined || isNaN(rawValue)) {return null;}
    if (rawValue === 255 || rawValue === 0xFFFF) {return null;}  // unknown/unsupported
    if (rawValue === 200) {return null;}  // "not available" sentinel (Z2M #11470)
    if (rawValue < 0 || rawValue > 200) {return null;}  // out of ZCL range
    return Math.round(rawValue / 2);  // ZCL 0-200 → 0-100%
  }
