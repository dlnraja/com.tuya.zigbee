...
const { assertZCLNode } = require('../util');
const { configureReportingWithRetry } = require('../utils/ZigbeeRetry');
const { CONFIGS: REPORTING } = require('../constants/REPORTING_CONFIGS');
const { safeReadCluster, safeConfigureReporting, safeBindCluster, getCluster } = require('../utils/clusterUtils');
...
const { ensureManufacturerSettings, getManufacturerName, getModelId } = require('../helpers/ManufacturerNameHelper');
...
const CapabilityFallbackManager = require('../managers/CapabilityFallbackManager');
const { BATTERY, THROTTLE, DETECTION, BUTTON_KEYWORDS, TIMER_NAMES, POWER, TUYA_SKIP_CLUSTERS } = require('../constants/DeviceConstants');
const { removeClusterListeners, safeCleanupManager, clearTimers, clearIntervals } = require('../utils/LifecycleCleanup');
...
// v5.8.36: Custom Cluster Manager - discovers skipped clusters (0xFC00-0xFFFF, Tuya on alt EPs)
let CustomClusterManager;
try {
  CustomClusterManager = require('../clusters/CustomClusterManager');
} catch (e) {
  CustomClusterManager = null;
}

// v5.11.23: Raw Cluster Fallback - captures ALL unhandled standard ZCL clusters
...
// v5.7.36: Universal Throttle Manager for deduplication and anti-flooding
...
// v5.5.672: Permissive Pairing + Global TimeSync + Arbitration
let TwoPhaseEnrichment, EventDeduplicator, PermissiveMatchingEngine, EnrichmentScheduler;
let GlobalTimeSyncEngine,ProtocolArbitrator;
try {
...
try {
 ...
 ProtocolArbitrator = require('../protocol/HybridProtocolArbitrator');
} catch (e) {
  GlobalTimeSyncEngine = null;
 ProtocolArbitrator = null;
}
...
// v5.6.0: ZigbeeProtocolPatchManager - known bug patches for all Zigbee ecosystems
let ZigbeeProtocolPatchManager;
try {
  ZigbeeProtocolPatchManager = require('../zigbee/ZigbeeProtocolPatchManager');
} catch (e) {
  ZigbeeProtocolPatchManager = null;
}
...
// v5.5.33:DataQuery for TRUE HYBRID (ZCL + Tuya DP) queries
let DataQuery, ZigbeeDataQuery;
try {
  const zigbeeQueryModule = require('../zigbee/ZigbeeDataQuery');
  DataQuery = zigbeeQueryModule.DataQuery;
...
// v5.8.80: DeviceProfileRegistry - centralized per-fingerprint config
let deviceProfileRegistry;
try {
  deviceProfileRegistry = require('../registry').registry;
} catch (e) {
  deviceProfileRegistry = null;
}

/**
 * BaseUnifiedDevice - Universal base class for power devices
 ...
 */
class BaseUnifiedDevice extends TuyaZigbeeDevice {

  constructor(...args) {
    super(...args);

    // v5.8.1: SDK3 battery threshold (Athom best practice)
    this.batteryThreshold = BATTERY.LOW_THRESHOLD;

    // v5.2.86: Early Tuya DP detection to prevent ZCL timeouts
    this._isPureTuyaDP = this._detectPureTuyaDP();
    if (this._isPureTuyaDP) {
      this.log('[TUYA-EARLY] Pure Tuya DP device detected in constructor');
    }
    ...
    /**
     * v5.12.0: 🌌 ANTIGRAVITY FAULT ISOLATOR
     * Proactively mutes non-fatal device retrieval failures.
     * Logs to DiagnosticAPI instead of crashing or spamming main logs.
     */
    ...
    // v5.12.0: Capability Fallback Manager Injection
    // Shields the Homey SDK from out-of-bounds Zigbee values
    CapabilityFallbackManager.inject(this);
  }
  ...
  /** v5.8.80: Resolve device profile from registry */
  _resolveDeviceProfile(zclNode) {
    try {
      if (!deviceProfileRegistry) {return null;}
      const s = this.getSettings?.() || {};
      const st = this.getStore?.() || {};
      const mfr = s.zb_manufacturer_name || s.zb_manufacturerName || st.manufacturerName || '';
      const pid = s.zb_model_id || s.zb_modelId || st.modelId || '';
      let p = deviceProfileRegistry.getProfile(mfr, pid);
      if (!p && zclNode) {p = deviceProfileRegistry.buildDynamicProfile(this, zclNode);}
      if (p) {this.log(`[PROFILE] id=${p.id} proto=${p.protocol} type=${p.deviceType}`);}
      return p || null;
    } catch (e) { this.log(`[PROFILE] Error: ${e.message}`); return null; }
  }

  /**
   * v5.8.80 / P2681: Get resolved profile.
   * WHY(P2681 / Bastien f37e8a91+8f0915fa): subclass used to return only
   * `this._deviceProfile` and swallow PhysicalButtonMixin DEVICE_PROFILES →
   * "Using undefined device profile" + 200ms debounce → random Homey Flows.
   * Registry (_deviceProfile) = DP/capability map; mixin = button timing/protocol.
   */
  getDeviceProfile(overrides = null) {
    const registry = this._deviceProfile || null;
    let mixinProfile = null;
    try {
      if (typeof super.getDeviceProfile === 'function') {
        mixinProfile = super.getDeviceProfile(overrides);
      }
    } catch (_e) { /* soft */ }
    if (mixinProfile && typeof mixinProfile === 'object'
      && (mixinProfile.brand || mixinProfile.source || mixinProfile.debounceMs
        || mixinProfile.productId || mixinProfile.skip8004 || mixinProfile.protocol)) {
      if (!registry) return mixinProfile;
      return {
        ...mixinProfile,
        ...registry,
        brand: mixinProfile.brand || registry.brand,
        manufacturerName: mixinProfile.manufacturerName || registry.manufacturerName,
        detectedName: mixinProfile.detectedName || registry.detectedName,
        protocol: mixinProfile.protocol || registry.protocol,
        debounceMs: mixinProfile.debounceMs ?? registry.debounceMs,
        crossPathDedupMs: mixinProfile.crossPathDedupMs ?? registry.crossPathDedupMs,
        skip8004: mixinProfile.skip8004 ?? registry.skip8004,
        writeSceneAttr: mixinProfile.writeSceneAttr ?? registry.writeSceneAttr,
        buttonCount: mixinProfile.buttonCount ?? registry.buttonCount,
        source: mixinProfile.source || registry.source,
      };
    }
    return registry || mixinProfile || null;
  }

  /** v5.8.80: Check profile quirk */
  hasProfileQuirk(q) { return !!this._deviceProfile?.quirks?.[q]; }

  /** v5.8.80: Get DP mapping from profile */
  getProfileDPMapping(dpId) { return this._deviceProfile?.dpMappings?.[dpId] || null; }

  /**
   * v9.0.40: Cached capability map - maps capabilities to DPs and transforms.
   * Uses CapabilityMapCache (WeakMap) to avoid recomputing closures on every
   * _onData() / command processing call. Invalidated on capability changes.
   *
   * @returns {Array<{capability, dp, toDevice, fromDevice}>}
   */
  get capabilityMap() {
    ...
    const map = [];
    const dpMappings = this._deviceProfile?.dpMappings;

    if (dpMappings) {
      for (const [dp, cfg] of Object.entries(dpMappings)) {
    ...
    // Only cache non-empty maps (empty means no profile dpMappings yet)
    if (map.length > 0) {
  ...
  /**
   * v9.0.40: Invalidate cached capability map.
   * Call after addCapability/removeCapability or dpMappings changes.
   */
  _invalidateCapabilityMap() {
    CapabilityMapCache.invalidate(this);
  }

  /**
   * v5.8.1: Get swBuildId from Basic cluster (Athom SDK3)
   */
  async getSwBuildId({ endpointId = 1, skipCache = false } = {}) {
    const cached = this.getStoreValue('swBuildId');
    if (!skipCache && cached) {return cached;}
    try {
      const basic = this.zclNode?.endpoints?.[endpointId]?.clusters?.basic;
      if (basic?.readAttributes) {
        const attrs = await basic.readAttributes(['swBuildId']);
        if (attrs?.swBuildId) {
          await this.setStoreValue('swBuildId', attrs.swBuildId);
          return attrs.swBuildId;
        }
      }
    } catch (e) { this.log(`[SDK3] getSwBuildId error: ${e.message}`); }
    return null;
  }

  /**
   * v5.8.1: Read Basic cluster info (Athom SDK3 pattern)
   */
  async readBasicClusterInfo(endpointId = 1) {
    try {
      const basic = this.zclNode?.endpoints?.[endpointId]?.clusters?.basic;
      if (basic?.readAttributes) {
        const attrs = await basic.readAttributes([
          'manufacturerName', 'modelId', 'dateCode', 'swBuildId', 'hwVersion'
        ]);
        this.log('[SDK3] Basic info:', JSON.stringify(attrs));
        return attrs;
      }
    } catch (e) { this.log(`[SDK3] readBasicClusterInfo error: ${e.message}`); }
    return {};
  }
  ...
  _isExpectedCapabilitySetFailure(err) {
    ...
    // WHY(P2714 / Bastien e1654535): Athom "Invalid Capability: measure_battery" after
    // strip/rehydrate race — expected soft, never CPU-storm log spam.
    return this._destroyed
      || this.destroyed
      || !this.homey
      || /destroy|delete|deleted|removed|unavailable|not available|not found|no longer|invalid capability/.test(message);
  }

  async safeSetCapabilityValue(capabilityId, value, options = {}) {
    try {
      ...
      // P116: EnergyJumpGuard on all BaseUnifiedDevice paths (Z2M-style
      // defensive divisor correction — forum #2092/#2093 ×660 kWh).
      if (typeof capabilityId === 'string' && capabilityId.startsWith('meter_power')) {
        try {
          const EnergyJumpGuard = require('../tuya/EnergyJumpGuard');
          value = EnergyJumpGuard.check(this, value);
        } catch (_e) { /* optional */ }
      }

      // WHY(P2470 / Peter diag 8afffc76 @9.0.882): Homey UI stayed `?` while logs
      // showed battery 100% — throttle cached the value before the UI paint landed
      // (`duplicate_value` 12ms later). Never throttle measure_battery while UI is null.
      let skipThrottle = options.skipThrottle === true;
      if (!skipThrottle && capabilityId === 'measure_battery' && this.hasCapability?.('measure_battery')) {
        try {
          const cur = this.getCapabilityValue('measure_battery');
          if (cur == null || cur === undefined) skipThrottle = true;
        } catch (_e) { /* soft */ }
      }

      // v5.7.36: Deduplication and throttling to prevent flooding
      if (UniversalThrottleManager && !skipThrottle) {
        const deviceId = this.getData?.()?.id || this.__id || 'unknown';
        const { shouldUpdate, reason } = UniversalThrottleManager.shouldUpdateCapability(
          deviceId, capabilityId, value, { forceDedupe: options.forceDedupe }
        );
        if (!shouldUpdate) {
          // Only log throttle reason occasionally to avoid log spam
          if (UniversalThrottleManager.shouldLog(`throttle_${deviceId}_${capabilityId}`, 30000)) {
            this.log(`[THROTTLE] ${capabilityId}=${value} skipped: ${reason}`);
          }
          return false;
        }
      }
      ...
      if (!this.hasCapability(capabilityId)) {
        ...
        if (manifestCaps.includes(capabilityId)) {
          try {
            ...
            this._invalidateCapabilityMap(); // v9.0.40: capability list changed
      ...
      // WHY(P2747): capture previous BEFORE paint — tagged *_changed Flows need delta
      let previousValue;
      try { previousValue = this.getCapabilityValue(capabilityId); } catch (_e) { previousValue = null; }

      // v9.0.77/v9.0.148: Call SDK setCapabilityValue directly, bypassing TuyaZigbeeDevice.setCapabilityValue.
      const sdkSetCapabilityValue = Object.getPrototypeOf(TuyaZigbeeDevice.prototype)?.setCapabilityValue;
      const setOk = await sdkSetCapabilityValue.call(this, capabilityId, value).then(
        () => true,
        err => {
          if (!this._isExpectedCapabilitySetFailure(err)) {
            this.error(`[SET-CAP] Failed to set ${capabilityId}:`, err.message);
          }
          return false;
        }
      );
      ...
      // WHY(P2747): BaseUnifiedDevice bypasses TuyaZigbeeDevice.setCapabilityValue —
      // still emit tagged *_changed Flow cards when value paints.
      try {
        const { emitCapabilityChangedFlows } = require('../flow/CapabilityChangedFlowEmitter');
        void emitCapabilityChangedFlows(this, capabilityId, value, previousValue).catch(() => {});
      } catch (_e) { /* soft */ }

      await DeviceTelemetryEstimator.record(this, capabilityId, value, { options }).catch(() => {});
  ...
  parseBattery(value, options = {}) {
    ...
    // Fallback — v9.0.79: Fix halving bug (same as normalizeZigbeeValue fix)
    // ZCL uses 0-200 (200=100%), Tuya devices report 0-100 directly
    let battery = typeof value === 'number' ? value : parseFloat(value);
    if (battery > 100 && battery <= 200) { battery = battery / 2; }  // ZCL 0-200 → 0-100
    // Values 0-100 are already percentages — do NOT halve
    return Math.max(0, Math.min(100, Math.round(battery)));
  }

  /**
   * v9.0.88: Sentinel-safe ZCL battery percentage conversion.
   * SINGLE source of truth for all batteryPercentageRemaining → % conversions.
   * Filters ZCL sentinels (255=unknown, 200=not available) before dividing by 2.
   *
   * @param {number} rawValue - Raw ZCL batteryPercentageRemaining (0-200 scale)
   * @returns {number|null} Battery percentage 0-100, or null if sentinel/invalid
   */
  /**
   * WHY: ZCL Power Config 0x0021 is spec'd as 0.5% units (0–200 → 0–100%).
   * Tuya/IKEA often already send 0–100. Blind `/2` turns a full battery into 50%
   * (Homey T140352 p15 SOS, SmartThings #2679, Z2M dontDividePercentage).
   * Never divide when raw ≤ 100; 200 → 100%; 255 → unknown.
   */
  static _safeBatteryPercent(rawValue, deviceOrMfr) {
    const { normalizeZclBatteryPercent, manufacturerOf } = require('../battery/zcl-percent');
    let manufacturer = '';
    if (typeof deviceOrMfr === 'string') {
      manufacturer = deviceOrMfr;
    } else if (deviceOrMfr) {
      manufacturer = manufacturerOf(deviceOrMfr);
    }
    return normalizeZclBatteryPercent(rawValue, { manufacturer });
  }
  ...
  _detectPureTuyaDP() {
    try {
      ...
      this.log(`[TUYA-DETECT] Constructor detection: model="${modelId}" mfr="${mfr}"`);

      // v5.8.76: Cache constructor-detected values for ensureManufacturerSettings
      // Fixes blank manufacturer bug (_TZ3000_5bpeda8u etc) where getData() has mfr
      // at construction time but zclNode isn't ready during onNodeInit
      if (mfr) {this._cachedManufacturerName = mfr;}
      if (modelId) {this._cachedModelId = modelId;}

      // TS0601 or _TZE manufacturer = Pure Tuya DP
      if (equalsCI(modelId, 'TS0601') || startsWithCI(mfr, '_TZE')) {
        this.log('[TUYA-DETECT] Pure Tuya DP device detected');
        return true;
      }
      ...
      if (driverId === 'climate_sensor' || driverId === 'soil_sensor') {
        ...
        this.log(`[TUYA-DETECT] Driver ${driverId} - assuming Tuya DP (will verify later)`);
        return true;
      }

      return false;
    } catch (err) {
      this.error('[TUYA-DETECT] Detection error:', err.message);
      return false;
    }
  }
  ...
  async registerCapability(capabilityId, cluster, opts) {
    ...
    if (this._isPureTuyaDP) {
      ...
      if (cluster && typeof cluster === 'object' && cluster.ID !== undefined) {
        this.log(`[TUYA-SKIP] Skipping ZCL registerCapability for ${capabilityId} (Tuya DP device)`);
        return;
      }
      if (typeof cluster === 'string' && cluster !== 'onoff') {
        // Allow onoff for switches, skip others for Tuya DP
        if (TUYA_SKIP_CLUSTERS.includes(cluster)) {
          this.log(`[TUYA-SKIP] Skipping ZCL registerCapability for ${capabilityId} cluster ${cluster} (Tuya DP device)`);
          return;
        }
      }
    }
  ...
  /**
   * Initialize device with power detection
  ...
  async onNodeInit({ zclNode }) {
    ...
    // WHY (f647d35b): assign zclNode BEFORE ensureManufacturerSettings — otherwise
    // getManufacturerName/readManufacturerFromCluster see empty node → false D101/D102.
    ...
    // v5.9.24: Per-device diagnostic engine
    this._diag = new DiagnosticEngine(this);

    // v5.8.57: Ensure zb_manufacturer_name / zb_model_id settings populated
    await ensureManufacturerSettings(this, zclNode).catch(() => {});

    // v5.6.0: Apply known Zigbee protocol bug patches for all ecosystems
    if (ZigbeeProtocolPatchManager) {
      try {
        this._patchManager = new ZigbeeProtocolPatchManager(this);
        await this._patchManager.applyPatches();
      } catch (e) {
        this.log('[PATCH] Non-critical patch error:', e.message);
      }
    }

    this.log('[ZCLNODE] zclNode received from Homey');

    // 🔌 P102 Phase 1: light I/O façade + protocol pick (bases consume this.io / ioChannels)
    if (typeof this._initDeviceIO === 'function') {
      await this._initDeviceIO(zclNode).catch((err) => {
        this.log(`[IO] ⚠️ _initDeviceIO failed: ${err?.message || err}`);
      });
    }

    // v9.0.40: Defer heavy cluster scanning to avoid blocking device startup
    safeSetTimeout(this, async () => {
      if (this._destroyed) {return;}
      try {
        // 🔗 SCAN AND BIND UNKNOWN CLUSTERS
        // v9.2.0: Ensure non-standard clusters are handled for BaseUnifiedDevice
        await this.scanUnknownClusters().catch(err => this.error(`[INIT] ⚠️ scanUnknownClusters failed:`, err?.message || err));
      } catch (err) {
        this.error('[DEFERRED-INIT] scanUnknownClusters error:', err?.message || err);
      }
    }, 1000);
    ...
    try {
      ...
      this.log('[DEVICE START]', name, '|', id);
    } catch (err) {
      this.error('[DEVICE START] Error getting info:', err.message);
    }
    ...
    try {
      ...
      if (!this._isPureTuyaDP) {
        const settings = this.getSettings?.() || {};
        const store = this.getStore?.() || {};
        // WHY (f647d35b): use ManufacturerNameHelper — settings alone are often blank on sleepy buttons
        ...
        // v5.9.24: DiagnosticEngine driver match check
        this._diag.checkDriverMatch(mfr, modelId);
      ...
      // v5.8.80: DeviceProfileRegistry lookup
      this._deviceProfile = this._resolveDeviceProfile(zclNode);

      // v9.0.40: Warm up capability map cache after profile is resolved
      ...
      await this.telemetryEstimator?.refresh?.('base-unified-init').catch(err => this.log(`[TELEMETRY] init refresh failed: ${err.message}`));

      // v9.8.0: Ensure all physical button UI capabilities have a listener to prevent "Missing capacity listener" crashes
      this._registerDynamicButtonListeners();
    ...
    try {
      ...
      // WHY(P2486): do not stomp a manager already attached by UniversalLayerBootstrap
      ...
      // v5.12.0: Advanced Intelligent Capability Management
      this.dynamicCapabilityManager.initialize().catch((e) => {
        try { this.log(`[DCM] Init error: ${e && e.message}`); } catch (_e) { /* readonly */ }
      });
      // Guard: AutoAdaptiveDevice (and others) may replace this.dynamicCapabilityManager
      // with lib/dynamic/DynamicCapabilityManager which historically lacked
      // auditCapabilities — bare call crashed Homey (Gmail 2026-08-14).
      safeSetTimeout(this, () => {
        if (this._destroyed) {return;}
        const dcm = this.dynamicCapabilityManager;
        if (dcm && typeof dcm.auditCapabilities === 'function') {
          Promise.resolve(dcm.auditCapabilities()).catch((e) => {
            try { this.log(`[DCM-AUDIT] ${e && e.message}`); } catch (_e) { /* readonly error */ }
          });
        }
      }, 5 * 60 * 1000); // Audit 5 minutes after pairing
      ...
      this.variantManager.init().catch((e) => {
        try { this.log(`[VARIANT] Init error: ${e && e.message}`); } catch (_e) { /* readonly */ }
      });
      ...
      // v5.5.672: Initialize Protocol Arbitrator (ZCL vs Tuya DP)
      if (ProtocolArbitrator) {
        this.protocolArbitrator = new ProtocolArbitrator(this);
        this.log('[INIT] ✅ ProtocolArbitrator initialized');
      }

      this.log('[INIT] ✅ Intelligent managers initialized: Battery, Power, Data, Capability');

      this.log('[INIT] Defaults set:', { powerType: this.powerType, batteryType: this.batteryType });
    } catch (err) {
      this.error('[INIT] Error setting defaults:', err.message);
    }
    ...
    try {
      this.log('[AVAILABILITY] Setting device available...');
      await Promise.resolve(this.setAvailable()).catch(err => {
        this.error('[AVAILABILITY] setAvailable failed:', err.message);
      });
      ...
      this.log('[AVAILABILITY] Device marked available');
    } catch (err) {
      this.error('[AVAILABILITY] Failed to set available:', err.message);
      this._safeRejectAvailable(err);
    }
    ...
    try {
      ...
      this.log('[BACKGROUND] Starting background initialization...');
      Promise.resolve(this._runBackgroundInitialization()).catch(err => {
        this.error('[BACKGROUND] Background init failed:', err.message);
        this.log('[WARN] Device will use safe defaults (Battery/CR2032)');
      });
    } catch (err) {
      this.error('[BACKGROUND] Failed to start background init:', err.message);
    }
    ...
    try {
      ...
      this.log('[COMPLETE] onNodeInit complete - device ready');
      ...
      this.error('⚠️ [COMPLETE] Error logging completion:', err.message);
    }
  }

  /**
   * v9.8.0: Register dummy or functional listeners for all `button.X` capabilities
   * This prevents "Missing capability listener" crashes when a user clicks the hidden UI buttons
   * or uses them in flows.
   */
  _registerDynamicButtonListeners() {
    try {
      const caps = this.getCapabilities();
      for (const cap of caps) {
        if (cap.startsWith('button.')) {
          this.registerCapabilityListener(cap, async (value) => {
            this.log(`[BUTTON-UI] Capability ${cap} pressed via UI or Flow`);
            
            // Extract gang number if any
            const match = cap.match(/button\.(\d+)/);
            const gang = match ? parseInt(match[1], 10) : 1;

            // Trigger physical flow if PhysicalButtonMixin is available
            if (typeof this._triggerPhysicalFlow === 'function') {
              this._triggerPhysicalFlow(gang, 'single');
            }

            // Optional: If it's a switch, toggle the corresponding relay
            if (typeof this._setGangOnOff === 'function') {
              const onoffCap = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
              if (this.hasCapability(onoffCap)) {
                const currentVal = this.getCapabilityValue(onoffCap);
                await this._setGangOnOff(gang, !currentVal);
              }
            }

            return true;
          });
          this.log(`[INIT] ✅ Registered dynamic listener for ${cap}`);
        }
      }
    } catch (err) {
      this.error(`[INIT] ⚠️ Failed to register dynamic button listeners: ${err.message}`);
    }
  }
  ...
  async _runBackgroundInitialization() {
    try {
      ...
      if (needsReportingConfig) {
        ...
        await new Promise(resolve => safeSetTimeout(this, resolve, 2000));
      ...
      // CRITICAL: IAS Zone enrollment (sensors / SOS) — NOT wireless scene remotes
      // WHY(P2729): Bastien diag ed627371 @ 1.0.93 — button_wireless_* hit enroll storm
      if (this.zclNode?.endpoints?.[1]?.clusters?.iasZone) {
        const IASZoneManager = require('../managers/IASZoneManager');
        if (IASZoneManager.isWirelessButtonRemoteNotIas(this)) {
          this.log('[CRITICAL] Skip IAS enroll for wireless button/remote (P2729)');
        } else {
      ...
      // WHY(P2729): remotes with skipBatteryReporting must not get BatteryRouter TX storm
      const _battProfile = this._tuyaProfile || this.getDeviceProfile?.() || {};
      const _skipBattTx = !!(
        _battProfile.skipBatteryReporting
        || _battProfile.noEf00Tx
        || this.homey?.settings?.get?.('bastien_skip_battery_tx') === true
      );
      if (BatteryRouter && this.hasCapability('measure_battery') && !_skipBattTx) {
        try {
        ...
        this.log('[BATTERY-ROUTER] Skip configure (P2729 skipBatteryReporting / Bastien gate)');
      }
      ...
      // Step 3b: DYNAMIC CAPABILITY MANAGER
      // v5.8.44: Rewritten - now safe for ALL drivers
      // SAFE mode: only adds read-only measurement caps (temp, humidity, lux, battery, power)
      // FULL mode: also adds controllable caps (only for universal_fallback)
      try {
        this.dynamicCapabilityManager = new DynamicCapabilityManager(this);
        await this.dynamicCapabilityManager.initialize();
        this.log('[BACKGROUND] Step 3b/8: DynamicCapabilityManager active');
      } catch (dcmErr) {
        this.log('[BACKGROUND] Step 3b/8: DCM init error:', dcmErr.message);
      }
      ...
      if (hasTuyaEF00) {
        ...
        // WHY(P2472b): attach MCU pipeline (format guess + soft 0/5/60s re-sync) after EF00 ready
        try {
          const TuyaMCUManager = require('../tuya/TuyaMCUManager');
          this.tuyaMcuManager = this.tuyaMcuManager || this._tuyaMcuManager || new TuyaMCUManager(this);
          this._tuyaMcuManager = this.tuyaMcuManager;
          if (typeof this.tuyaMcuManager.attach === 'function') {
            await this.tuyaMcuManager.attach({ force: false }).catch(() => {});
            this.log('[BACKGROUND] ✅ P2472b TuyaMCUManager attached');
          }
        } catch (mcuErr) {
          this.log('[BACKGROUND] ⚠️ MCU attach soft-fail:', mcuErr.message);
        }
        ...
        try {
          ...
          this._dpLoggerStatusTimer = safeSetTimeout(this, () => { if (this._destroyed) {return;} if (this.dpLogger) {
      ...
      // v5.8.36: Custom Cluster Discovery - scan ALL EPs for skipped clusters
      this.log('[BACKGROUND] Step 3c.4/7: Custom cluster discovery...');
      try {
        if (CustomClusterManager) {
          this._customClusterManager = new CustomClusterManager(this);
          const ccmCount = await this._customClusterManager.initialize(this.zclNode);
          if (ccmCount > 0) {
            this.log(`[CCM] ✅ Discovered ${ccmCount} custom clusters`);
          } else {
            this.log('[CCM] No custom clusters found');
          }
        }
      } catch (ccmErr) {
        this.log('[CCM] Init error (non-critical):', ccmErr.message);
      }

      // v5.11.23: Raw Cluster Fallback - capture ALL unhandled standard ZCL clusters
      this.log('[BACKGROUND] Step 3c.5/7: Raw cluster fallback...');
      try {
        if (RawClusterFallback) {
          this._rawClusterFallback = new RawClusterFallback(this);
          const rcfCount = await this._rawClusterFallback.initialize(this.zclNode);
          if (rcfCount > 0) {
            this.log(`[RAW-FALLBACK] ✅ Watching ${rcfCount} unhandled standard clusters`);
          } else {
            this.log('[RAW-FALLBACK] All clusters handled by driver');
          }
        }
      } catch (rcfErr) {
        this.log('[RAW-FALLBACK] Init error (non-critical):', rcfErr.message);
      }

      // v9.2.0: Universal Cluster Binder - bind specific manufacturer clusters (0xE001-E003, Zosung)
      this.log('[BACKGROUND] Step 3c.6/7: Universal cluster binder...');
      try {
        const UniversalClusterBinder = require('../clusters/UniversalClusterBinder');
        this._universalClusterBinder = new UniversalClusterBinder(this, this.zclNode);
        await this._universalClusterBinder._bindManufacturerClusters();
        this.log('[UCB] ✅ Manufacturer clusters bound');
      } catch (ucbErr) {
        this.log('[UCB] Init error (non-critical):', ucbErr.message);
      }
      ...
      if (this.hasCapability('measure_battery')) {
        ...
        try {
          ...
          this._batteryConfigTimer = safeSetTimeout(this, async () => { if (this._destroyed) {return;} await this.configureStandardBatteryReporting(); }, 3000);

          // BatteryReportingManager (v4.9.341)
          this._batteryInitTimer = safeSetTimeout(this, async () => { if (this._destroyed) {return;} await this.batteryReportingManager.initialize(this.zclNode); }, 5000);
        ...
        if (this.hasCapability('measure_humidity')) {
          ...
          if (humidCluster) {
            try {
              ...
              await this.safeSetCapabilityValue('measure_humidity', parseFloat(humidity)).catch(() => {});
      ...
      try {
        ...
        if (adaptResult.success && adaptResult.deviceInfo) {
          ...
          if (bestDriver && bestDriver.driverId !== this.driver.id && bestDriver.confidence > 0.5) {
            const listed = (this.driver?.manifest?.zigbee?.manufacturerName || [])
              .map((x) => String(x).toLowerCase());
            const mfr = String(this.getData?.()?.manufacturerName || this.getSetting?.('zb_manufacturer_name') || '').toLowerCase();
            const pid = String(this.getData?.()?.productId || this.getSetting?.('zb_model_id') || '').toUpperCase();
            let skipNag = false;
            if (mfr && listed.includes(mfr)) {
              skipNag = true;
              this.log('[MIGRATION] Current driver already lists this manufacturer — skip nag');
            }
            // WHY(P2338): sacred couple + registry beat endpoint heuristics (meter91 TS0044 toast)
            if (!skipNag && mfr && pid) {
              try {
                const FPDB = require('../DeviceFingerprintDB');
                const locked = FPDB.lookup?.(mfr, pid);
                if (locked?.driver && locked.driver === this.driver.id) {
                  skipNag = true;
                  this.log('[MIGRATION] Sacred couple lock matches current driver — skip nag');
                }
              } catch (_fpdb) { /* optional */ }
              if (!skipNag) {
                try {
                  const Misattrib = require('../pairing/UserMisattributionRegistry');
                  const reg = Misattrib.lookup?.(mfr, pid);
                  if (reg?.canonicalDriver === this.driver.id) {
                    skipNag = true;
                    this.log('[MIGRATION] Misattribution registry canonical matches current — skip nag');
                  }
                } catch (_reg) { /* optional */ }
              }
              if (!skipNag) {
                try {
                  const MVM = require('../ManufacturerVariationManager');
                  const resolved = MVM.resolveDriverType?.(mfr, pid);
                  if (resolved && resolved === this.driver.id) {
                    skipNag = true;
                    this.log('[MIGRATION] resolveDriverType matches current driver — skip nag');
                  }
                } catch (_mvm) { /* optional */ }
              }
            }
      ...
      // v5.5.33: InitializeDataQuery for TRUE HYBRID (ZCL + Tuya)
      ...
      // v5.9.24: Diagnostic summary
      if (this._diag) {
        const rpt = this._diag.getReport();
        if (rpt.count > 0) {
          this.log(`[DIAG] ${rpt.count} issue(s):`);
          rpt.issues.forEach(i => this.log(`[DIAG]  ${i.code}: ${i.msg}`));
        } else {
          this.log('[DIAG] No issues detected');
        }
      }
  ...
  /**
   * v5.8.32: Override setCapabilityValue to feed ALL data into IntelligentDeviceAdapter
   * This is the universal choke point - ALL data (ZCL, Tuya DP, manual) flows through here
   */
  async setCapabilityValue(capabilityId, value) {
    // Feed data to intelligent adapter for learning (non-blocking)
    if (this.intelligentAdapter && typeof this.intelligentAdapter.recordDataPoint === 'function') {
      try {
        this.intelligentAdapter.recordDataPoint(capabilityId, value, 'capability_update');
  ...
  async _initDataRecoveryManager() {
    ...
    // P2184: Peter overnight OOM — defer recovery engine when heap is critical
    try {
      const BootBudget = require('../performance/BootBudget');
      if (BootBudget.isHeapCritical()) {
        this.log('[DATA-RECOVERY] Skipped init (heap critical — retry on wake)');
        this._deferDataRecoveryInit = true;
        return;
      }
    } catch (_e) { /* BootBudget optional in tests */ }
    ...
    try {
      ...
      // v7.5.39: Guard triggerRecovery to prevent crashes if class definition is cached/mismatched
      if (this._dataRecoveryManager && typeof this._dataRecoveryManager.triggerRecovery !== 'function') {
        this._dataRecoveryManager.triggerRecovery = async () => {
          if (typeof this._dataRecoveryManager.forceRecovery === 'function') {
            return this._dataRecoveryManager.forceRecovery();
          }
        };
      }
  ...
  /**
   * v5.5.33: InitializeDataQuery for BOTH ZCL and Tuya queries
  ...
  async _initHybridDataQuery() {
    ...
    // Skip if DataQuery not available
    if (!DataQuery) {
    ...
    try {
      this.DataQuery = new DataQuery(this);
      await this.DataQuery.initialize();
      this.log('[HYBRID-QUERY] ✅DataQuery initialized for ALL devices');
    } catch (err) {
   ...
   * Detects and initializes ZCL, Tuya DP, or protocols
  ...
  async _initUniversalProtocol() {
    try {
      ...
      default:
        ...
        await initZCL(this, this.zclNode, ['onOff']).catch(e => this.warn('[UNIVERSAL-INIT] ZCL fallback failed:', e.message));
        await initTuyaDP(this, this.zclNode).catch(e => this.warn('[UNIVERSAL-INIT] TuyaDP fallback failed:', e.message));
      }
  ...
  _setupTuyaDPHandler() {
    ...
    if (!this.handleTuyaDP) {
      this.handleTuyaDP = async (data) => {
        ...
        // v5.8.44: DCM processDP is now driver-aware (no-op for dedicated drivers)
        if (this.dynamicCapabilityManager && data?.dp !== undefined) {
          try {
            await this.dynamicCapabilityManager.processDP(data.dp, data.value, data.datatype);
          } catch (e) { /* non-critical */ }
        }
    ...
    try {
      ...
      if (basicCluster && typeof basicCluster.readAttributes === 'function') {
        ...
        if (attributes?.powerSource !== undefined) {
          ...
          const powerSourceStr = typeof powerSource.powerSource === 'string' ? powerSource.powerSource.toLowerCase() : null;
    ...
    try {
      ...
      if (powerCluster) {
        ...
        if (batteryVoltage?.batteryVoltage) {
          ...
          // v5.8.67: Use driver.id for detection (same mfr can be TRV=2xAA or button=CR2032)
          this.batteryType = BatteryManager.detectBatteryTypeFromVoltage(voltage, this.driver?.id || '');
          this.log(`[OK] Intelligent detection: ${this.batteryType} (voltage: ${voltage}V, driver: ${this.driver?.id})`);
  ...
  async configurePowerCapabilities() {
    ...
    try {
      if (this.powerType === 'AC' || this.powerType === 'DC') {
        ...
        for (const capability of recommended) {
          if (!this.hasCapability(capability)) {
            await Promise.resolve(this.addCapability(capability)).catch(err => {
        ...
        for (const capability of ['measure_voltage', 'measure_current', 'measure_power', 'meter_power']) {
          if (this.hasCapability(capability)) {
            ...
            if (shouldHide) {
              await Promise.resolve(this.removeCapability(capability)).catch(() => { });
        ...
        if (this.hasCapability('measure_battery')) {
          await Promise.resolve(this.removeCapability('measure_battery')).catch(() => { });
        ...
        if (!this.hasCapability('measure_battery')) {
          await Promise.resolve(this.addCapability('measure_battery')).catch(() => { });
      ...
      this._invalidateCapabilityMap(); // v9.0.40: capabilities may have changed
      this.log('[OK] Power capabilities configured intelligently');
  ...
  async _setupTemperatureReporting(endpoint, minInterval, maxInterval) {
    ...
    try {
      await cluster.configureReporting({
        measuredValue: { ...REPORTING.temperature, minInterval, maxInterval }
      });
  ...
  async _setupHumidityReporting(endpoint, minInterval, maxInterval) {
    ...
    try {
      await cluster.configureReporting({
        measuredValue: { ...REPORTING.humidity, minInterval, maxInterval }
      });
  ...
  async _setupIlluminanceReporting(endpoint, minInterval, maxInterval) {
    ...
    try {
      await cluster.configureReporting({
        measuredValue: { ...REPORTING.luminance, minInterval, maxInterval }
      });
  ...
  async setupStandardBatteryMonitoring() {
    ...
    // WHY(P2290): Z2M #8072 / SH-SC07 — never configureReporting battery on sleepy remotes
    try {
      const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
      if (profile?.skipBatteryReporting) {
        this.log('[BATTERY] Skipping configureReporting (profile.skipBatteryReporting)');
        return false;
      }
    } catch (_e) { /* soft */ }
    ...
    try {
      ...
      try {
        ...
        if (batteryData) {
          // v5.8.67: Zigbee batteryPercentageRemaining is 0-200 (half-percent), divide by 2
          const batteryPercent = BaseUnifiedDevice._safeBatteryPercent(batteryData.batteryPercentageRemaining) ?? 0;
          if (this.hasCapability('measure_battery')) {
            await Promise.resolve(await this.safeSetCapabilityValue('measure_battery', batteryPercent)).catch(() => {});
      ...
      await endpoint.clusters.powerConfiguration.configureReporting({
        batteryPercentageRemaining: REPORTING.batteryFast
      });
      ...
      try {
        ...
        if (tempCluster) {
          ...
          if (temp?.measuredValue !== undefined) {
            ...
            await Promise.resolve(await this.safeSetCapabilityValue('measure_temperature', parseFloat(tempC))).catch(() => {});
      ...
      try {
        ...
        if (humCluster) {
          ...
          if (hum?.measuredValue !== undefined) {
            ...
            await Promise.resolve(await this.safeSetCapabilityValue('measure_humidity', parseFloat(humPercent))).catch(() => {});
      ...
      try {
        ...
        if (illumCluster) {
          ...
          if (illum?.measuredValue !== undefined) {
            ...
            await Promise.resolve(await this.safeSetCapabilityValue('measure_luminance', parseFloat(lux))).catch(() => {});
        ...
        if (powerCluster) {
          ...
          if (power?.activePower !== undefined) {
            ...
            try {
              ...
              await commitCapability(this, 'measure_power', parseFloat(watts), 'zcl');
            } catch (_e) {
              await Promise.resolve(this.safeSetCapabilityValue
                ? this.safeSetCapabilityValue('measure_power', parseFloat(watts), { source: 'zcl' })
                : this.setCapabilityValue('measure_power', parseFloat(watts))).catch(err => this.error(err));
            }
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (voltage?.rmsVoltage !== undefined) {
            ...
            try {
              const { commitCapability } = require('../layers/commitCapability');
              await commitCapability(this, 'measure_voltage', parseFloat(volts), 'zcl');
            } catch (_e) {
              await Promise.resolve(this.safeSetCapabilityValue
                ? this.safeSetCapabilityValue('measure_voltage', parseFloat(volts), { source: 'zcl' })
                : this.setCapabilityValue('measure_voltage', parseFloat(volts))).catch(err => this.error(err));
            }
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (current?.rmsCurrent !== undefined) {
            ...
            try {
              const { commitCapability } = require('../layers/commitCapability');
              await commitCapability(this, 'measure_current', parseFloat(amps), 'zcl');
            } catch (_e) {
              await Promise.resolve(this.safeSetCapabilityValue
                ? this.safeSetCapabilityValue('measure_current', parseFloat(amps), { source: 'zcl' })
                : this.setCapabilityValue('measure_current', parseFloat(amps))).catch(err => this.error(err));
            }
      ...
      try {
        ...
        if (onOffCluster) {
          ...
          if (state?.onOff !== undefined) {
            await this.safeSetCapabilityValue('onoff', state.onOff, { source: 'zcl' }).catch(err => this.error(err));
      ...
      try {
        ...
        if (occCluster) {
          ...
          if (occ?.occupancy !== undefined) {
            ...
            await this.safeSetCapabilityValue('alarm_motion', occupied, { source: 'zcl' }).catch(err => this.error(err));
      ...
      try {
        ...
        if (iasCluster) {
          ...
          if (zone?.zoneStatus !== undefined) {
            ...
            await this.safeSetCapabilityValue('alarm_contact', open, { source: 'zcl' }).catch(err => this.error(err));
  ...
  async registerBatteryCapability(options = {}) {
    ...
    try {
      await this.registerCapability('measure_battery', cluster, {
        ...
        reportParser: value => {
          return BaseUnifiedDevice._safeBatteryPercent(value) ?? 0; // Zigbee reports 0-200, we want 0-100
        }
      });
  ...
  async registerTemperatureCapability() {
    ...
    try {
      ...
      tempCluster.on('attr.measuredValue', async (value) => {
        ...
        if (this._shouldThrottleCap('measure_temperature', temp)) {return;}
      ...
      try {
        await tempCluster.configureReporting({
          measuredValue: REPORTING.temperature
        });
  ...
  async registerHumidityCapability() {
    ...
    try {
      ...
      humidCluster.on('attr.measuredValue', async (value) => {
        ...
        if (this._shouldThrottleCap('measure_humidity', humidity)) {return;}
      ...
      try {
        await humidCluster.configureReporting({
          measuredValue: REPORTING.humidity
        });
    ...
    try {
      ...
      try {
        await luxCluster.configureReporting({
          measuredValue: REPORTING.luminance
        });
  ...
  /**
   * v5.8.67: Non-linear voltage-to-percent fallback (CR2032 curve, mV input)
   * Used when SmartBatteryManager is not available
   */
  _voltageToPercentFallback(mV) {
    const curve = [[3000,100],[2950,95],[2900,90],[2850,85],[2800,80],[2750,70],[2700,60],[2650,50],[2600,40],[2550,30],[2500,20],[2400,10],[2300,5],[2100,0]];
  ...
  /**
   * v5.8.70: Anti-flood capability throttle
   * @returns {boolean} true if update should be skipped
   */
  _shouldThrottleCap(cap, newVal) {
    const now = Date.now();
    if (!this._capFloodGuard) {this._capFloodGuard = {};}
    const t = this._capFloodGuard[cap] || { time: 0 };
    const prev = this.getCapabilityValue(cap);
    if (prev === newVal) {return true;}
    const elapsed = now - t.time;
    const cfg = { measure_battery: [300000,2], measure_temperature: [30000,0.3], measure_humidity: [30000,2] };
    const [minMs, sig] = cfg[cap] || [10000, null];
    if (elapsed < minMs) {
      if (sig && typeof newVal === 'number' && typeof prev === 'number') {
        if (Math.abs(newVal - prev) < sig) {return true;}
      } else { return true; }
    }
    this._capFloodGuard[cap] = { time: now };
    return false;
  }

  /** v5.8.70: Shorthand for battery throttle */
  _shouldThrottleBattery(v) { return this._shouldThrottleCap('measure_battery', v); }
  ...
  async applyEnergyOptimization(mode) {
    ...
    // WHY(P2723): settings UI still exposes optimization_mode + battery_report_interval on
    // sleepy remotes (Bastien TS004x). Changing them must NOT configureReporting — Homey TX
    // storm → 65–84% TX Error on enddevices (live Zigbee tools 2026-09-24).
    try {
      const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
      if (profile?.skipBatteryReporting || profile?.noEf00Tx) {
        this.log('[BATTERY] P2723 skip applyEnergyOptimization configureReporting (skipBatteryReporting)');
        return;
      }
    } catch (_e) { /* soft */ }
  ...
  async requestBatteryUpdate() {
    ...
    // WHY(P2723): flow/action must not ZCL-read sleepy remotes (same TX storm class)
    try {
      const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
      if (profile?.skipBatteryReporting || profile?.noEf00Tx) {
        this.log('[BATTERY] P2723 skip requestBatteryUpdate ZCL read (skipBatteryReporting)');
        return this.hasCapability('measure_battery') ? this.getCapabilityValue('measure_battery') : null;
      }
    } catch (_e) { /* soft */ }
    ...
    try {
      ...
      if (powerCluster) {
        ...
        if (battery?.batteryPercentageRemaining !== undefined) {
          const level = BaseUnifiedDevice._safeBatteryPercent(battery.batteryPercentageRemaining) ?? 0;
          if (this.hasCapability('measure_battery')) {
            await this.safeSetCapabilityValue('measure_battery', parseFloat(level)).catch(() => {});
          }
  ...
  async handleEndpointCommand(endpointId, clusterName, command, payload = {}) {
    ...
    try {
      // v5.5.396/v9.0.149: Button devices should never become switches here.
      // Convert standard ZCL commands into button flow actions across clusters.
      ...
      if (isButtonDevice) {
        ...
        const rawCommand = payload?._rawCommand || command;
        const rawLower = String(rawCommand || command || '').toLowerCase();
        const triggerButton = async (pressType, count = 1) => {
          this.log(`[CMD] Button press detected on endpoint ${buttonIndex}: ${pressType}`);
          if (typeof this.triggerButtonPress === 'function') {
            await this.triggerButtonPress(buttonIndex, pressType, count, { source: 'physical' });
          } else {
            // v10.3.0 FIX (B12): 'button_${i}_pressed' cards don't exist in
            // app.json — use the real generic 'button_pressed' card with
            // its declared 'button'/'type' tokens.
            try {
              const flowTrigger = this.homey?.flow?.getDeviceTriggerCard('button_pressed');
              if (flowTrigger) {
                await flowTrigger.trigger(this, { button: String(buttonIndex), type: pressType }, {}).catch(() => { });
              }
            } catch (err) {
              this.log('[CMD] Flow card button_pressed unavailable:', err.message);
            }
          }
        ...
        if (clusterName === 'onOff') {
          const pressType = rawLower.includes('toggle')
            ? rawLower.includes('command') ? 'long' : 'single'
            : rawLower.includes('on')
              ? 'single'
              : rawLower.includes('off')
                ? 'double'
                : 'single';
          await triggerButton(pressType, pressType === 'double' ? 2 : 1);
          return;
        }

        if (clusterName === 'scenes') {
          const sceneId = payload?.sceneId ?? payload?.sceneid ?? payload?.scene ?? payload?.value ?? 0;
          await triggerButton(resolvePressType(sceneId, 'CMD-scene'));
          return;
        }

        if (clusterName === 'levelControl') {
          const pressType = rawLower.includes('move')
            ? 'long'
            : rawLower.includes('stop')
              ? 'release'
              : 'single';
          await triggerButton(pressType);
          return;
        }

        if (clusterName === 'multistateInput') {
          const value = payload?.presentValue ?? payload?.value ?? payload;
          await triggerButton(resolvePressType(value, 'CMD-multistate'));
          return;
        }
      }
      ...
      if (clusterName === 'onOff') {
        ...
        if (command === 'on' || command === 'off') {
          ...
          if (!this.hasCapability(cap)) {
            await Promise.resolve(this.addCapability(cap)).catch(err => this.error(err));
          }

          await this.safeSetCapabilityValue(cap, state, { source: 'zcl' }).catch(err => this.error(err));
        ...
        if (command === 'toggle') {
          const cap = endpointId > 1 ? `onoff.ep${endpointId}` : 'onoff';
          const current = await Promise.resolve(this.getCapabilityValue(cap)).catch(() => false);
          await this.safeSetCapabilityValue(cap, !current, { source: 'zcl' }).catch(err => this.error(err));
          return;
        }
      }
  ...
  async safeBind(cluster, targetEndpoint) {
    ...
    try {
      await cluster.bind(targetEndpoint).catch(() => { });
  ...
  async setTuyaDpValue(dp, value) {
    ...
    if (!cap) {
      ...
      if (!this.hasCapability(cap)) {
        try {
          ...
          this.log(`[DP] Skipping DP ${dp}: capability ${cap} not registered (${err.message})`);
          this.dpMap[dp] = 'failed';
          return;
        }
      }
    ...
    await Promise.resolve(this.safeSetCapabilityValue(cap, value)).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
  }
  ...
  async forceInitialRead() {
    ...
    // v5.6.0: FIX - Use correct settings keys (zb_model_id NOT zb_modelId)
    const modelId = this.getSetting?.('zb_model_id') || this.getSetting?.('zb_modelId') || '';
    const mfr = this.getSetting?.('zb_manufacturer_name') || this.getSetting?.('zb_manufacturerName') || '';
    const isPureTuyaDP = (equalsCI(modelId, 'TS0601') || startsWithCI(mfr, '_TZE')) && this._isPureTuyaDP;

    if (isPureTuyaDP) {
      this.log('[INITIAL-READ] ℹ️ Pure Tuya DP device - sending dataQuery instead of ZCL reads');
      await this._sendInitialDataQuery();
      return;
    }
    ...
    try {
      ...
      if (this.hasCapability('measure_battery') && endpoint.clusters?.powerConfiguration && !this.skipZclBatteryPolling && !this.usesTuyaDPBattery) {
        try {
          ...
          const batteryData = await Promise.race([
            batteryPromise,
            new Promise((_, reject) => safeSetTimeout(this, () => { if (this._destroyed) {return;} reject(new Error('Timeout')); }, 5000))
          ]).catch(() => null);

          if (batteryData && batteryData.batteryPercentageRemaining != null) {
            const battery = BaseUnifiedDevice._safeBatteryPercent(batteryData.batteryPercentageRemaining) ?? 0;
            ...
            if (voltageData && voltageData.batteryVoltage) {
              ...
              // v5.8.67: Use non-linear curve instead of broken linear formula
              const mV = Math.round(voltage * 1000);
              const percent = this._voltageToPercentFallback(mV);
              this.log(`[INITIAL-READ] ✅ Battery: ${percent}% (from ${voltage}V)`);
              if (this.hasCapability('measure_battery')) {
                await this.safeSetCapabilityValue('measure_battery', parseFloat(percent)).catch(() => {});
              }
            }
          }
        } catch (err) {
      ...
      if (endpoint.clusters?.illuminanceMeasurement) {
        try {
          // v9.1.2: FIX - was calling .Promise.resolve() on cluster object (always returns undefined)
          const luxData = await endpoint.clusters.illuminanceMeasurement.readAttributes(['measuredValue']).catch(() => null);
  ...
  /** v5.11.99: Send Tuya dataQuery on init - fixes "works after X minutes" */
  async _sendInitialDataQuery() {
    const ep = this.zclNode?.endpoints?.[1];
    if (!ep) {return;}
    const tc = ep.clusters?.tuya || ep.clusters?.manuSpecificTuya || ep.clusters?.[61184];
    const tmout = p => Promise.race([p, new Promise((_, r) => safeSetTimeout(this, () => { if (this._destroyed) {return;} r(new Error('T')); }, 5000))]);
    if (tc?.dataQuery) {
      try { await tmout(tc.dataQuery({ seq: Date.now() % 65535 })); this.log('[INITIAL-READ] ✅ dataQuery sent'); return; } catch (e) { /* next */ }
    }
    if (tc?.command) {
      try { await tmout(tc.command('dataQuery', { seq: Date.now() % 65535 })); this.log('[INITIAL-READ] ✅ dataQuery cmd sent'); return; } catch (e) { /* next */ }
    }
    if (this.tuyaEF00Manager?.requestAllDPs) {
      try { await tmout(this.tuyaEF00Manager.requestAllDPs()); this.log('[INITIAL-READ] ✅ requestAllDPs sent'); return; } catch (e) { /* next */ }
    }
    this.log('[INITIAL-READ] ⚠️ No dataQuery method available');
  }
  ...
  async _setupUniversalHybridMode() {
    ...
    try {
      ...
      const isTuyaDP = modelId.toUpperCase() === 'TS0601' || mfr.toUpperCase().startsWith('_TZE') || this._isPureTuyaDP || this._blockZCL;

      this.log(`[HYBRID] Device check: modelId="${modelId}", mfr="${mfr}", isTuyaDP=${isTuyaDP}`);

      if (isTuyaDP) {
        // v5.3.62: For Tuya DP devices, ALWAYS skip mode
      ...
      if (tempCluster && this.hasCapability('measure_temperature')) {
        ...
        try {
          ...
          safeAddListener(tempCluster, 'attr.measuredValue', async function (value) {
            const temp = Math.round((value / 100) * 10) / 10;
            if (this._shouldThrottleCap('measure_temperature', temp)) {return;}
            this.log(`[HYBRID] Temperature: ${temp}C`);
            await this.safeSetCapabilityValue('measure_temperature', parseFloat(temp)).catch(() => {});
          ...
          const readPromise = tempCluster.readAttributes(['measuredValue'])
            .then(async data => {
              if (data?.measuredValue != null) {
                ...
                this.log(`[HYBRID] Initial temperature: ${temp}C`);
                await this.safeSetCapabilityValue('measure_temperature', parseFloat(temp)).catch(() => {});
              }
            })
            .catch(() => this.log('[HYBRID] Temperature read failed (device may be sleeping)'));

          // Don't wait more than 3s for read
          await Promise.race([readPromise, new Promise(r => safeSetTimeout(this, r, 3000))]);

          // Configure reporting in background (don't block - battery devices often fail)
          safeConfigureReporting(tempCluster, { measuredValue: REPORTING.temperature }, 3000)
            .then(ok => { if (!ok) {this.log('[HYBRID] ⚠️ Temperature reporting config skipped');} });
      ...
      if (humCluster && this.hasCapability('measure_humidity')) {
        ...
        try {
          // CRITICAL: Use safe listener registration
          safeAddListener(humCluster, 'attr.measuredValue', async function (value) {
            const hum = Math.round(value / 100);
            if (this._shouldThrottleCap('measure_humidity', hum)) {return;}
          ...
          const readPromise = humCluster.readAttributes(['measuredValue'])
            ...
            .catch(() => this.log('[HYBRID] Humidity read failed (device may be sleeping)'));

          await Promise.race([readPromise, new Promise(r => safeSetTimeout(this, r, 3000))]);

          // Configure reporting in background (don't block)
          safeConfigureReporting(humCluster, { measuredValue: REPORTING.humidity }, 3000)
            .then(ok => { if (!ok) {this.log('[HYBRID] ⚠️ Humidity reporting config skipped');} });
      ...
      if (powerCluster && this.hasCapability('measure_battery')) {
        ...
        try {
          // CRITICAL: Use safe listener registration
          safeAddListener(powerCluster, 'attr.batteryPercentageRemaining', async function (value) {
            const battery = BaseUnifiedDevice._safeBatteryPercent(value);
            if (battery === null) {return;}
            if (this._shouldThrottleBattery(battery)) {return;}
            this._lastZclBatteryPercentTs = Date.now();
          ...
          safeAddListener(powerCluster, 'attr.batteryVoltage', async function (value) {
            const voltage = value / 10;
            // WHY(P2442 / diag a342c411): ZCL % (raw 200 → 100%) is authoritative;
            // voltage curve wrongly overwrote 100% → 60% on the same wake window.
            const lastPct = Number(this.getCapabilityValue?.('measure_battery'));
            const lastZcl = this._lastZclBatteryPercentTs || 0;
            if (Number.isFinite(lastPct) && (Date.now() - lastZcl) < 180000) {
              try {
                await this.setStoreValue?.('battery_voltage', voltage).catch?.(() => {});
              } catch (_e) { /* soft */ }
              return;
            }
            const battery = this._voltageToPercentFallback(Math.round(voltage * 1000));
            if (this._shouldThrottleBattery(battery)) {return;}
          ...
          const readPromise = powerCluster.readAttributes(['batteryPercentageRemaining', 'batteryVoltage'])
            .then(async data => {
              if (data?.batteryPercentageRemaining != null) {
                const battery = this.constructor._safeBatteryPercent(
          ...
          // Configure reporting in background (don't block)
          safeConfigureReporting(powerCluster, { batteryPercentageRemaining: REPORTING.battery }, 3000)
            .then(ok => { if (!ok) {this.log('[HYBRID] ⚠️ Battery reporting config skipped');} });
        ...
        try {
          ...
          if (this.hasCapability('measure_voltage')) {
            elecCluster.on('attr.rmsVoltage', async (value) => {
              ...
              await this.safeSetCapabilityValue('measure_voltage', parseFloat(voltage)).catch(() => {});
            });
          }
          ...
          if (this.hasCapability('measure_current')) {
            elecCluster.on('attr.rmsCurrent', async (value) => {
              ...
              await this.safeSetCapabilityValue('measure_current', parseFloat(current)).catch(() => {});
            });
          }
      ...
      if (meterCluster && this.hasCapability('meter_power')) {
        ...
        try {
          meterCluster.on('attr.currentSummationDelivered', async (value) => {
            ...
            await this.safeSetCapabilityValue('meter_power', parseFloat(energy)).catch(() => {});
          });
      ...
      if (activeCapabilities.length > 0) {
        ...
        this.Capabilities = hybridCapabilities;
      } else {
  ...
  async removeBatteryFromACDevices() {
    ...
    if (this.powerType !== POWER.AC && this.powerType !== POWER.DC) {
      return;
    }
  ...
  async logDeviceIdentity() {
    try {
      ...
      if (this.zclNode?.endpoints?.[1]?.clusters?.basic) {
        ...
        for (let attempt = 1; attempt <= DETECTION.MAX_READ_RETRIES; attempt++) {
          try {
            this.log(`  Attempt ${attempt}/${DETECTION.MAX_READ_RETRIES}...`);
            ...
            if (attempt < 3) {
              await new Promise(r => safeSetTimeout(this, r, attempt * 1000)); // 1s, 2s delays
            }
          }
        }
      }
  ...
  async retryBatteryRead(maxRetries = 3) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          await new Promise(resolve => safeSetTimeout(this, resolve, delays[attempt - 1] || 5000));
        }
        ...
        if (batteryData?.batteryPercentageRemaining != null) {
          ...
          if (this.hasCapability('measure_battery')) {
            await this.safeSetCapabilityValue('measure_battery', parseFloat(battery)).catch(() => {}); // Silent fail
          }
    ...
    // v9.1.2: Do NOT set a fake 50% battery - this misleads users and
    // flow cards. Better to leave the last-known value and wait for the
    // device to report naturally. Setting 50% can cause automations to
    // skip low-battery alerts.
    this.log('[BATTERY] All read attempts failed - keeping previous battery value');
  ...
  _shouldConfigureReporting() {
    ...
    // v9.0.53 FIX - Read actual app version for reporting reconfiguration.
    // v9.0.47: Le chemin était '../../../package.json' (un ../ de trop) → require échouait
    // silencieusement → currentVersion restait bloqué à '9.0.37' → reporting JAMAIS reconfiguré.
    let currentVersion = '9.0.40';
    try {
    ...
    for (const clusterName of essentialClusters) {
      ...
      if (cluster && typeof cluster.bind === 'function') {
        try {
          ...
          bindResults[clusterName] = true;
          boundCount++;
        } catch (err) {
          bindResults[clusterName] = false;
        }
      }
    }

    this.log(`[FALLBACK-BIND] Bound ${boundCount} clusters`);
    if (this._diag) {this._diag.checkBindings(bindResults);}
  }
  ...
  async registerAllCapabilitiesWithReporting() {
    ...
    // Check if this is a button/remote device
    const driverName = this.driver?.id || '';
    const isButtonDevice = BUTTON_KEYWORDS.some(kw => driverName.includes(kw));
    ...
    if (isButtonDevice) {
      ...
      if (this.hasCapability('measure_battery') && this.zclNode?.endpoints?.[1]?.clusters?.powerConfiguration) {
        try {
          ...
          // WHY(P2470 / Z2M#8072 / Peter 8afffc76 / Bastien 4d4e1684):
          // Sleepy remotes mute if configureAttributeReporting / getOnStart while asleep.
          const btnProfile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
          let sleepyRemote = false;
          try {
            const { isSleepyRemote } = require('../zigbee/TimeClusterPolicy');
            sleepyRemote = !!isSleepyRemote(this);
          } catch (_ePol) { /* soft */ }
          const isButtonDriver = /button|remote|scene_switch|smart_knob|wall_remote/i.test(String(this.driver?.id || ''))
            || this.getClass?.() === 'button'
            || this.fastInitMode === true;
          const skipBattCfg = !!(btnProfile?.skipBatteryReporting || btnProfile?.noEf00Tx
            || sleepyRemote || isButtonDriver);
          const reportOpts = skipBattCfg
            ? undefined
            : {
              reportOpts: {
                configureAttributeReporting: {
          ...
          if (skipBattCfg) {
            this.log('[REGISTER] P2470 skip battery configureAttributeReporting + getOnStart (sleepy P2645)');
          }
          ...
          await this.registerCapability('measure_battery', CLUSTER.POWER_CONFIGURATION, {
            ...
            reportParser: async value => {
              // WHY(P2499 / Peter #2238 diag 77394256): pass device for mfr quirks;
              // never coerce null→0 (UI showed 0% / “no battery” after empty get).
              const percent = BaseUnifiedDevice._safeBatteryPercent(value, this);
              if (percent === null) {return null;}
              this.log(`🔋 [BUTTON BATTERY] ${percent}% (raw: ${value})`);
              // WHY(P2470): Homey reportParser return alone can race throttle — force UI + store
              try {
                await this.setStoreValue('last_battery_percentage', percent).catch(() => {});
                const cur = this.getCapabilityValue?.('measure_battery');
                if (cur == null || cur === undefined || Number(cur) !== Number(percent)) {
                  await this.safeSetCapabilityValue('measure_battery', percent, { skipThrottle: true }).catch(() => {});
                }
              } catch (_e) { /* soft */ }
              ...
              if (this.hasCapability('alarm_battery')) {
                ...
                await this.safeSetCapabilityValue('alarm_battery', isLow).catch(() => {});
                this.log(`[BUTTON BATTERY] Alarm: ${isLow ? 'LOW' : 'OK'}`);
              }

              return percent;
            },
            getParser: value => BaseUnifiedDevice._safeBatteryPercent(value, this),
            // WHY(P2645): never getOnStart/getOnOnline on sleepy — Bastien 4d4e1684 join storm
            getOpts: {
              pollInterval: 0,
              getOnStart: false,
              getOnOnline: false,
          ...
          this._bgBatteryReadTimer = safeSetTimeout(this, async () => {
            if (this._destroyed) {return;}
            try {
              ...
              const battery = await endpoint.clusters.powerConfiguration.readAttributes(['batteryPercentageRemaining'])
                .catch(() => null);
              // v9.0.85: Filter ZCL sentinels — 255 = unknown, 200 = not available (Z2M #11470)
              if (battery?.batteryPercentageRemaining != null && battery.batteryPercentageRemaining !== 255 && battery.batteryPercentageRemaining !== 200) {
                const percent = BaseUnifiedDevice._safeBatteryPercent(battery.batteryPercentageRemaining, this);
                if (percent == null) {return;}
                if (this.hasCapability('measure_battery')) {
                  await this.safeSetCapabilityValue('measure_battery', parseFloat(percent), { skipThrottle: true }).catch(() => {});
                }
                await this.setStoreValue('last_battery_percentage', percent).catch(() => { });
      ...
      try {
        ...
        await this.registerCapability('measure_battery', CLUSTER.POWER_CONFIGURATION, {
          ...
          reportParser: async value => {
            ...
            const simplePercent = BaseUnifiedDevice._safeBatteryPercent(value);

            // v9.1.2: FIX - was returning simplePercent (triggering update) instead of skipping
            // When throttled, return null to prevent the reportParser from setting the value
            if (this._shouldThrottleBattery(simplePercent)) {return null;}
                ...
                if (storedVoltage) {
                  ...
                  if (accuratePercent !== null) {
                    ...
                    if (this.hasCapability('alarm_battery')) {
                      ...
                      await this.safeSetCapabilityValue('alarm_battery', isLow).catch(() => {});
                      this.log(`[BATTERY] Alarm: ${isLow ? 'LOW' : 'OK'} (${accuratePercent}% vs ${threshold}%)`);
                    }

                    // v5.8.69: Persist for sleeping device restore
                    this.setStoreValue('last_battery_percentage', Math.round(accuratePercent)).catch(() => {});
            ...
            // v5.8.69: Persist for sleeping device restore
            this.setStoreValue('last_battery_percentage', simplePercent).catch(() => {});
            ...
            if (this.hasCapability('alarm_battery')) {
              ...
              await this.safeSetCapabilityValue('alarm_battery', isLow).catch(() => {});
              this.log(`[BATTERY] Alarm: ${isLow ? 'LOW' : 'OK'} (${simplePercent}% vs ${threshold}%)`);
            }
        ...
        if (batteryAttrs) {
          let finalPercent = null; // v5.8.69: Don't default to 50 — use null until real data
          ...
          if (batteryAttrs.batteryPercentageRemaining != null && finalPercent === null) {
            finalPercent = this.constructor._safeBatteryPercent(
              batteryAttrs.batteryPercentageRemaining,
              this,
            );
            this.log(`[REGISTER] 🔋 Battery: ${finalPercent}% (simple calculation)`);
          }

          // v5.8.69: Only set if we got real data
          if (finalPercent !== null && this.hasCapability('measure_battery')) {
            await this.safeSetCapabilityValue('measure_battery', parseFloat(finalPercent)).catch(() => {});
            await this.setStoreValue('last_battery_percentage', finalPercent).catch(() => {});
        ...
        if (this.hasCapability('measure_battery') && this.getCapabilityValue('measure_battery') === null) {
          ...
          if (stored !== null && stored !== undefined && typeof stored === 'number') {
            await this.safeSetCapabilityValue('measure_battery', parseFloat(stored)).catch(() => {});
            this.log(`[REGISTER] Restored stored battery: ${stored}% (sleeping device)`);
          }
        }
      } catch (err) {
        this.error('[REGISTER] measure_battery failed:', err.message);
        // v5.8.69: Even on error, try to restore stored battery to avoid "?"
        if (this.hasCapability('measure_battery') && this.getCapabilityValue('measure_battery') === null) {
          const stored = this.getStoreValue('last_battery_percentage');
          if (stored !== null && stored !== undefined && typeof stored === 'number') {
            await this.safeSetCapabilityValue('measure_battery', parseFloat(stored)).catch(() => {});
            this.log(`[REGISTER] Restored stored battery on error: ${stored}%`);
          }
        }
      }
    }
    ...
    if (this.hasCapability('measure_temperature') && endpoint.clusters?.temperatureMeasurement) {
      try {
        ...
        if (temp?.measuredValue != null) {
          ...
          await this.safeSetCapabilityValue('measure_temperature', parseFloat(temperature)).catch(() => {});
    ...
    if (this.hasCapability('measure_luminance') && endpoint.clusters?.illuminanceMeasurement) {
      try {
        ...
        if (lux?.measuredValue != null) {
          ...
          await this.safeSetCapabilityValue('measure_luminance', parseFloat(luminance)).catch(() => {});
    ...
    if (this.hasCapability('alarm_motion') && endpoint.clusters?.occupancySensing) {
      try {
        ...
        if (occ?.occupancy != null) {
          ...
          await this.safeSetCapabilityValue('alarm_motion', motion).catch(() => {});
  ...
  async configureStandardBatteryReporting() {
    try {
      ...
      endpoint.clusters.powerConfiguration.on('attr.batteryPercentageRemaining', async value => {
        const percent = BaseUnifiedDevice._safeBatteryPercent(value);
          if (percent === null) {return;}
        if (this._shouldThrottleBattery(percent)) {return;}
      ...
      try {
        await endpoint.clusters.powerConfiguration.configureReporting({
          batteryPercentageRemaining: REPORTING.battery,
        });
  ...
  async onDeleted() {
    ...
    this._invalidateCapabilityMap();

    this.log('[CLEANUP] Device being removed...');

    try {
      // Use shared cleanup utilities for consistency and reduced duplication
      safeCleanupManager(this, this._patchManager, 'PatchManager');
      removeClusterListeners(this);

      // Cleanup managers (each with cleanup() or destroy() method)
      const managers = [
        { instance: this.dynamicCapabilityManager, name: 'DynamicCapabilityManager' },
        { instance: this.iasZoneManager, name: 'IASZoneManager' },
        { instance: this.tuyaEF00Manager, name: 'TuyaEF00Manager' },
        { instance: this.multiEndpointManager, name: 'MultiEndpointManager' },
        { instance: this.commandListener, name: 'CommandListener' },
        { instance: this.intelligentAdapter, name: 'IntelligentDeviceAdapter' },
        { instance: this.intelligentBattery, name: 'IntelligentBatteryManager' },
        { instance: this.intelligentPower, name: 'IntelligentPowerManager' },
        { instance: this._dataRecoveryManager, name: 'DataRecoveryManager' },
      ];
      for (const { instance, name } of managers) {
        safeCleanupManager(this, instance, name);
      }

      // Null out references for GC
      this.intelligentAdapter = null;
      this.intelligentBattery = null;
      this.intelligentPower = null;
      this._dataRecoveryManager = null;
      this.DataQuery = null;

      // Clear pending timers and intervals
      clearTimers(this, TIMER_NAMES);
      clearIntervals(this, ['periodicReadTimer']);

      this.log('[CLEANUP] Device cleanup complete');
    } catch (err) {
      this.error('[CLEANUP] Device cleanup failed:', err);
    }
  ...
  // ═════════════════════════════════════════════════════════════════════════════
  // v9.0.87: UNIVERSAL ZIGBEE ENGINE — Multi-Endpoint Battery Scanning
  // ═════════════════════════════════════════════════════════════════════════════

  /**
   * Scan ALL endpoints for battery cluster (genPowerCfg 0x0001).
   * Some non-Tuya devices (Xiaomi, Sonoff SNZB-02P) report battery on EP2/EP3.
   * Also handles Xiaomi/Aqara proprietary battery via basic cluster 0xFF01 TLV.
   *
   * @returns {Object|null} { endpoint, percent, source } or null if not found
   */
  async _scanAllEndpointsForBattery() {
    if (!this.zclNode?.endpoints) {return null;}

    const endpoints = Object.entries(this.zclNode.endpoints);
    this.log(`[UNIVERSAL-BATTERY] Scanning ${endpoints.length} endpoints for battery...`);

    for (const [epId, endpoint] of endpoints) {
      if (!endpoint?.clusters) {continue;}

      // Strategy 1: Standard ZCL genPowerCfg (0x0001)
      const powerCfg = endpoint.clusters.powerConfiguration ||
                       endpoint.clusters.genPowerCfg ||
                       endpoint.clusters?.[0x0001];
      if (powerCfg) {
        try {
          const attrs = await powerCfg.readAttributes(['batteryPercentageRemaining', 'batteryVoltage']);
          if (attrs?.batteryPercentageRemaining != null && attrs.batteryPercentageRemaining !== 255 && attrs.batteryPercentageRemaining !== 200) {
            const percent = BaseUnifiedDevice._safeBatteryPercent(attrs.batteryPercentageRemaining) ?? 0;
            this.log(`[UNIVERSAL-BATTERY] ✅ Found battery on EP${epId} (ZCL): ${percent}%`);
            return { endpoint: epId, percent, source: 'zcl_powerCfg' };
          }
          if (attrs?.batteryVoltage != null && attrs.batteryVoltage > 0 && attrs.batteryVoltage < 255) {
            const voltage = attrs.batteryVoltage / 10;
            const UBH = require('../battery/UnifiedBatteryHandler');
            // v9.0.388: pas de fallback linéaire — UBH a un fallback CR2032 interne
            const percent = UBH.calculateFromVoltage(voltage, 'CR2032');
            this.log(`[UNIVERSAL-BATTERY] ✅ Found battery on EP${epId} (voltage): ${voltage}V = ${percent}%`);
            return { endpoint: epId, percent: Math.max(0, Math.min(100, percent)), source: 'zcl_voltage' };
          }
        } catch {}
      }

      // Strategy 2: Xiaomi/Aqara proprietary battery via basic cluster 0xFF01 TLV
      const basic = endpoint.clusters.basic || endpoint.clusters.genBasic || endpoint.clusters?.[0x0000];
      if (basic) {
        try {
          const attrs = await basic.readAttributes([0xFF01]);
          if (attrs?.[0xFF01] !== undefined) {
            const tlvData = attrs[0xFF01];
            // Xiaomi TLV format: type(1) + length(1) + value(N) pairs
            // Battery is typically type 0x01 with 1-byte value (0-100)
            if (Buffer.isBuffer(tlvData) && tlvData.length >= 2) {
              let offset = 0;
              while (offset < tlvData.length - 1) {
                const type = tlvData[offset];
                const len = tlvData[offset + 1];
                if (type === 0x01 && len === 1 && offset + 2 < tlvData.length) {
                  const battery = tlvData[offset + 2];
                  if (battery >= 0 && battery <= 100) {
                    this.log(`[UNIVERSAL-BATTERY] ✅ Found battery on EP${epId} (Xiaomi TLV): ${battery}%`);
                    return { endpoint: epId, percent: battery, source: 'xiaomi_tlv' };
                  }
                }
                offset += 2 + len;
              }
    ...
    this.log('[UNIVERSAL-BATTERY] No battery found on any endpoint');
    return null;
  }

  /**
   * v9.0.87: Universal onEndDeviceAnnounce handler.
   * Re-applies critical configurations when a sleepy device wakes up.
   * Called automatically by the Zigbee SDK when a device announces itself.
   */
  async onEndDeviceAnnounce() {
    ...
    // Re-read battery if needed (5min/2% dedup)
    // WHY(P2733 / Bastien TS004x): skipBatteryReporting remotes — never
    // _scanAllEndpointsForBattery on wake (TX storm → mesh TX err + dead presses).
    if (this.hasCapability('measure_battery')) {
      let skipBattScan = false;
      try {
        const p = this._tuyaProfile || this.getDeviceProfile?.() || {};
        skipBattScan = !!(p.skipBatteryReporting || p.snappyRelayFlow || p.noEf00Tx);
        if (!skipBattScan) {
          const { shouldSkipSleepyRemoteBatteryTx } = require('../zigbee/PowerClusterPolicy');
          skipBattScan = shouldSkipSleepyRemoteBatteryTx(p, { homey: this.homey });
        }
      } catch (_e) { /* soft */ }
      if (skipBattScan) {
        this.log('[ANNOUNCE] P2733 skip battery endpoint scan (skipBatteryReporting / snappy)');
      } else {
      const lastBattery = await this.getStoreValue('last_battery_percentage').catch(() => null);
      const lastBatteryTime = await this.getStoreValue('last_battery_time').catch(() => 0);
      const now = Date.now();
      const elapsed = now - (lastBatteryTime || 0);

      if (elapsed > 300000 || lastBattery === null) { // 5 min dedup
        this.log('[ANNOUNCE] 🔋 Re-reading battery after wake...');
        const batteryResult = await this._scanAllEndpointsForBattery();
        if (batteryResult && batteryResult.percent !== lastBattery) {
          await this.safeSetCapabilityValue('measure_battery', batteryResult.percent).catch(() => {});
          await this.setStoreValue('last_battery_percentage', batteryResult.percent).catch(() => {});
          await this.setStoreValue('last_battery_time', now).catch(() => {});
          this.log(`[ANNOUNCE] 🔋 Battery updated: ${batteryResult.percent}% (from EP${batteryResult.endpoint})`);
        }
      }
      }
    }

    // Re-apply scene mode for TS004F/TS0044 button devices
    // WHY(P2733): snappy TS0041–44 skip8004 — never wake-write 0x8004 from base
    if (this._universalSceneModeSwitch) {
      try {
        const p = this._tuyaProfile || this.getDeviceProfile?.() || {};
        if (p.skip8004 || p.writeSceneAttr === false || p.snappyRelayFlow) {
          this.log('[ANNOUNCE] P2733 skip scene-mode wake write (skip8004 / snappy)');
        } else {
    ...
    // Re-configure energy reporting for plug devices
    if (this.hasCapability('measure_power') && this._configureEnergyReporting) {
      try {
        await this._configureEnergyReporting();
      } catch {}
    }

    // WHY(P2475 / Z2M forceTimeUpdates): MCU clocks drift after rejoin — re-arm EF00 + mcuSyncTime soft.
    // WHY(P2741 / Bastien 6bdc3c5e @ 1.0.107): snappy remotes have no MCU clock — mcuSyncTime
    // on endDeviceAnnounce still TX and can mute pile (log showed P2475 after P2733 listen-only).
    try {
      const profile = (typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null)
        || this._deviceProfile || {};
      const skipMcuSync = !!(profile.skipBatteryReporting || profile.snappyRelayFlow || profile.noEf00Tx);
      if (skipMcuSync) {
        this.log?.('[P2741] skip MCU time re-sync on announce (snappy / skipBatteryReporting)');
      } else {
        const zcl = this.zclNode;
        const ef00 = this.tuyaEF00Manager || this._tuyaEF00Manager || this.io?.ef00;
        if (zcl && ef00 && typeof ef00.sendTimeSync === 'function') {
          const last = Number(this._p2475LastMcuSyncMs) || 0;
          if (Date.now() - last > 120000) {
            this._p2475LastMcuSyncMs = Date.now();
            ef00.sendTimeSync(zcl, { forceSync: true, useFallbackChain: true }).catch(() => {});
            this.log?.('[P2475] MCU time re-sync on endDeviceAnnounce');
          }
        }
      }
    } catch (_e) { /* soft */ }
...
BaseUnifiedDevice.BaseUnifiedDevice = BaseUnifiedDevice;
