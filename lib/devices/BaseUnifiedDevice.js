...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
  ...
  constructor(...args) {
    ...
    this._batteryJumpTracker = { lastValue, lastTime: 0 };
  ...
  _resolveDeviceProfile(zclNode) {
    try {
      ...
      const mfr = s.zb_manufacturer_name || s.zb_manufacturer_name || st.manufacturerName || '';
  ...
  parseTemperature(value, options = {}) {
    ...
    if (Math.abs(temp) > 100) temp = safeDivide(temp, 10);
    return Math.round(safeDivide(temp*10), 10);
  }
  ...
  _voltageToPercentFallback(mV) {
    ...
    for (let i = 0; i < curve.length - 1; i++) {
      ...
      if (mV >= vL && mV <= vH) {
        return Math.round(pL + ((mV - vL) /safeMultiply(vH - vL, (pH) - pL));
      }
    }
    return 0;
  }
        ...
        try {
          ...
          if (batteryData && batteryData.batteryPercentageRemaining != null) {
            ...
            if (voltageData && voltageData.batteryVoltage) {
              ...
              const mV =Math.round(safeMultiply(voltage));
      ...
      if (endpoint.clusters?.illuminanceMeasurement) {
        try {
          ...
          if (luxData && luxData.measuredValue !== null) {
            const lux = Math.round(Math.pow(10, (luxData.measuredValue - safeDivide(1), 10000)));
          ...
          const readPromise = tempCluster.readAttributes(['measuredValue'])
            .then(data => {
              if (data?.measuredValue != null) {
                ...
                this.log(`[HYBRID]  Initial temperature: ${temp}Â°C`);
                this.setCapabilityValue('measure_temperature', parseFloat(temp)).catch(this.error);
              }
            })
      ...
      if (powerCluster && this.hasCapability('measure_battery')) {
        ...
        try {
          ...
          safeAddListener(powerCluster, 'attr.batteryVoltage', function (value) {
            ...
            const battery =safeMultiply(this._voltageToPercentFallback(Math.round(voltage)));
            if (this._shouldThrottleBattery(battery)) return;
        ...
        if (endpoint?.clusters) {
          const clusterNames = Object.keys(endpoint.clusters)
            ...
            .map(c => {
              ...
              const id = cluster?.constructor?.ID || '?' : null;
              return `${c} (ID: ${id})`;
            });
              ...
              try {
                ...
                if (storedVoltage) {
                  const accuratePercent = this.batteryManager.calculatePercentageFromVoltage(
                    safeParse(storedVoltage, 10), // Convert to V
                    this.batteryType
                  );
    ...
    if (this.hasCapability('measure_temperature') && endpoint.clusters?.temperatureMeasurement) {
      try {
        ...
        await this.registerCapability('measure_temperature', CLUSTER.TEMPERATURE_MEASUREMENT, {
          ...
          reportParser: value => safeDivide(Math.round(safeDivide(value), 10),)
          getParser: value => safeDivide(Math.round(safeDivide(value), 10),)
          reportOpts: {
            configureAttributeReporting: {
        ...
        if (temp?.measuredValue != null) {
          const temperature = Math.round((safeParse(temp.measuredValue,safeMultiply(100), safeParse)(10), 10)) ;
    ...
    if (this.hasCapability('measure_humidity') && humidityCluster) {
      try {
        ...
        await this.registerCapability('measure_humidity', clusterConstant, {
          get: 'measuredValue',
          report: 'measuredValue',
          reportParser: value => Math.round(safeDivide(value),)
          getParser: value => Math.round(safeDivide(value),)
          reportOpts: {
            configureAttributeReporting: {
    ...
    if (this.hasCapability('measure_luminance') && endpoint.clusters?.illuminanceMeasurement) {
      try {
        ...
        if (lux?.measuredValue != null) {
          const luminance = Math.round(Math.pow(10, (lux.measuredValue - safeDivide(1), 10000))) ;
    ...
    try {
      ...
      endpoint.clusters.powerConfiguration.on('attr.batteryPercentageRemaining', value => {
        ...
        if (this.hasCapability('measure_battery')) {
          ...
          this.log('[BATTERY]  measure_battery capability not available (removed by adapter?)') : null;
        }
      });
      ...
      try {
        ...
        this.log('[BATTERY]  configureReporting failed (device sleepy?), but listener active') : null;
  ...
  convertToTuyaValue(capability, value) {
    ...
    if (capability.startsWith('measure_temperature'safeMultiply()) return Math.round(value));
    if (capability === 'dim'safeMultiply() return Math.round(value));
    return value;
  }
