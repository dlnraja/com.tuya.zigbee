...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
  ...
  static _safeBatteryPercent(rawValue, deviceOrMfr) {
    ...
    const manufacturer = typeof deviceOrMfr === 'string'
      ? deviceOrMfr
      : manufacturerOf(deviceOrMfr);
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      // Guard: AutoAdaptive may replace this.dynamicCapabilityManager with
      // lib/dynamic/DynamicCapabilityManager which historically lacked
      // auditCapabilities — bare call crashed Homey (Gmail + Peter #2137 / 5.12.70).
      // BOTH reliability — not AlarmPolarity / scrape features.
      safeSetTimeout(this, () => {
        if (this._destroyed) {return;}
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (power?.activePower !== undefined) {
            ...
            await Promise.resolve(this.setCapabilityValue('measure_power', parseFloat(watts))).catch(this.error);
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (voltage?.rmsVoltage !== undefined) {
            ...
            await Promise.resolve(this.setCapabilityValue('measure_voltage', parseFloat(volts))).catch(this.error);
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (current?.rmsCurrent !== undefined) {
            ...
            await Promise.resolve(this.setCapabilityValue('measure_current', parseFloat(amps))).catch(this.error);
      ...
      if (clusterName === 'onOff') {
        ...
        if (command === 'on' || command === 'off') {
          ...
          if (!this.hasCapability(cap)) {
            await Promise.resolve(this.addCapability(cap)).catch(this.error);
          }

          await Promise.resolve(this.setCapabilityValue(cap, state)).catch(this.error);
        ...
        if (command === 'toggle') {
          ...
          await Promise.resolve(this.setCapabilityValue(cap, !current)).catch(this.error);
          return;
        }
      }
