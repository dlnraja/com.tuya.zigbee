...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
    ...
    try {
      ...
      if (hasTuyaEF00) {
        ...
        try {
          ...
          this._dpLoggerStatusTimer = this.homey.setTimeout(() => { if (this._destroyed) return; if (this.dpLogger) {
      ...
      if (this.hasCapability('measure_battery')) {
        ...
        try {
          ...
          this._batteryConfigTimer = this.homey.setTimeout(async () => { if (this._destroyed) return; await this.configureStandardBatteryReporting(); }, 3000);

          // BatteryReportingManager (v4.9.341)
          this._batteryInitTimer = this.homey.setTimeout(async () => { if (this._destroyed) return; await this.batteryReportingManager.initialize(this.zclNode); }, 5000);
      ...
      if (this.hasCapability('measure_battery') && endpoint.clusters?.powerConfiguration && !this.skipZclBatteryPolling && !this.usesTuyaDPBattery) {
        try {
          ...
          const batteryData = await Promise.race([
            batteryPromise,
            new Promise((_, reject) => this.homey.setTimeout(() => { if (this._destroyed) return; reject(new Error('Timeout')); }, 5000))
          ]).catch(() => null);
  ...
  async _sendInitialDataQuery() {
    ...
    const tmout = p => Promise.race([p, new Promise((_, r) => this.homey.setTimeout(() => { if (this._destroyed) return; r(new Error('T')); }, 5000))]);
    if (tc?.dataQuery) {
    ...
    try {
      ...
      if (tempCluster && this.hasCapability('measure_temperature')) {
        ...
        try {
          ...
          await Promise.race([readPromise, new Promise(r => this.homey.setTimeout(r, 3000))]);
      ...
      if (this.zclNode?.endpoints?.[1]?.clusters?.basic) {
        ...
        for (let attempt = 1; attempt <= DETECTION.MAX_READ_RETRIES; attempt++) {
          try {
            ...
            if (attempt < 3) {
              await new Promise(r => this.homey.setTimeout(r, attempt * 1000)); // 1s, 2s delays
            }
          }
        }
      }
  ...
  async retryBatteryRead(maxRetries = 3) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          await new Promise(resolve => this.homey.setTimeout(resolve, delays[attempt - 1] || 5000));
        }
    ...
    if (isButtonDevice) {
      ...
      if (this.hasCapability('measure_battery') && this.zclNode?.endpoints?.[1]?.clusters?.powerConfiguration) {
        try {
          ...
          this._bgBatteryReadTimer = this.homey.setTimeout(async () => {
            if (this._destroyed) return;
            try {
