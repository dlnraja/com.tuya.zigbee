...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
  ...
  /**
   * 🌌 Antigravity Safe Invoke with Exponential Retry
   * Wraps an async call with fault isolation and exponential backoff retry.
   */
  async _safeInvoke(fn, context = 'unknown', fallback = null, maxRetries = 3) {
    if (this._destroyed) {
      this.log(`[ANTIGRAVITY] Device destroyed, aborting ${context}`);
      return fallback;
    }
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (this._destroyed) return fallback;

        const errorMsg = err.message || String(err);
        const isMuted = [...this._antigravityIsolator.mutedErrors].some(m => errorMsg.includes(m));

        if (attempt < maxRetries && isMuted) {
          attempt++;
          this.log(`[ANTIGRAVITY-RETRY] ${context} failed (${errorMsg}). Retrying ${attempt}/${maxRetries} in ${delay}ms...`);
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      setTimeout(() => {
        if (!this._deleted) this.dynamicCapabilityManager.auditCapabilities();
  ...
  async setTuyaDpValue(dp, value) {
    ...
    await Promise.resolve(await this.setCapabilityValue(cap, value)).catch(this.error.bind(this));
  }
