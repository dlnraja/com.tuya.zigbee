    ...
    if (isButtonDevice) {
      ...
      if (this.hasCapability('measure_battery') && this.zclNode?.endpoints?.[1]?.clusters?.powerConfiguration) {
        try {
          ...
          // WHY(P2470 / Z2M#8072 / Peter 8afffc76): skipBatteryReporting buttons
          // (SH-SC07 mrpevh8p) die / go mute if we configureAttributeReporting on wake.
          ...
          if (skipBattCfg) {
            this.log('[REGISTER] P2470 skip battery configureAttributeReporting (Z2M#8072)');
          }
