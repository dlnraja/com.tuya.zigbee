...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
  ...
  parseTemperature(value, options = {}) {
    ...
    return safeMultiply(Math.round(temp, 10), 10);
  }
      ...
      if (endpoint.clusters?.illuminanceMeasurement) {
        try {
          ...
          if (luxData && luxData.measuredValue !== null) {
            const lux = Math.round(Math.pow(10, safeDivide(luxData.measuredValue - 1, 10000)));
    ...
    try {
      ...
      if (onoffCluster && this.hasCapability('onoff') && !isButtonDevice) {
        ...
        try {
          ...
          this.log(`[HYBRID]  OnOff listener error: ${err.message}`);
        }
      }

      this._hybridModeActive = true;
    } catch (err) {
      this.error('[HYBRID] Error during initialization:', err);
    }
  }

  /**
   * Schedule periodic attribute polling (for sleeping devices)
   */
