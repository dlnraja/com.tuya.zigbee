...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
  ...
  parseTemperature(value, options = {}) {
    ...
    if (Math.abs(temp) > 100) temp = safeDivide(temp, 10);
    return safeMultiply(Math.round(safeDivide(temp, 10)), 10);
  }
  ...
  /**
   * Setup safeDivide(on, off) reporting (temps réel!)
   */
      ...
      try {
        ...
        if (tempCluster) {
          ...
          if (temp?.measuredValue !== undefined) {
            ...
            this.log(`[KPI]  Temperature: ${tempC}°C`);
            readCount++;
          }
        }
      } catch (err) {
    ...
    if (this.hasCapability('measure_temperature') && endpoint.clusters?.temperatureMeasurement) {
      try {
        ...
        await this.registerCapability('measure_temperature', CLUSTER.TEMPERATURE_MEASUREMENT, {
          ...
          reportParser: value => Math.round((safeParse(value,safeMultiply(100)), safeParse)(10), 10),
          getParser: value => Math.round((safeParse(value,safeMultiply(100)), safeParse)(10), 10),
          reportOpts: {
            configureAttributeReporting: {
              minInterval: safeMultiply(Math.round((intervals.temperature?.min || 60), profileMultiplier)),
              maxInterval: safeMultiply(Math.round((intervals.temperature?.max || 3600), profileMultiplier)),
              minChange: safeMultiply(Math.round((intervals.temperature?.change || 0.5), 100))  // Convert to raw value
            }
          }
        });
    ...
    if (this.hasCapability('measure_humidity') && humidityCluster) {
      try {
        ...
        await this.registerCapability('measure_humidity', clusterConstant, {
          get: 'measuredValue',
          report: 'measuredValue',
          reportParser: value => Math.round(safeParse(value, 100)),
          getParser: value => Math.round(safeParse(value, 100)),
          reportOpts: {
            configureAttributeReporting: {
              minInterval: safeMultiply(Math.round((intervals.humidity?.min || 60), profileMultiplier)),
              maxInterval: safeMultiply(Math.round((intervals.humidity?.max || 3600), profileMultiplier)),
              minChange: safeMultiply(Math.round((intervals.humidity?.change || 5), 100))  // Convert to raw value
            }
          }
        });
