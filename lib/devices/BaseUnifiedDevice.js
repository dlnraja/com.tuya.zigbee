  ...
  async _safeInvoke(fn, context = 'unknown', fallback = null) {
    try {
      ...
      if (isMuted) {
        ...
        if (this.homey.app.diagnosticAPI) {
          this.homey.app.diagnosticAPI.addLog('DEBUG', 'ANTIGRAVITY', `Isolated ${context}: ${errorMsg}`, this.getName());
        }
        return fallback;
      }
  ...
  _antigravityGuard(handler, context = 'listener') {
    return async (...args) => {
      try {
        ...
        const errorMsg = err.message || String(err);
        if (this.homey.app.diagnosticAPI) {
          this.homey.app.diagnosticAPI.addLog('WARN', 'ANTIGRAVITY', `Guarded ${context} caught error: ${errorMsg}`, this.getName());
        }
      }
    };
  }
