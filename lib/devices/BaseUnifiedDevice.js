...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
   ...
   * Filters ZCL sentinel 255=unknown before dividing by 2.
   *
  ...
  static _safeBatteryPercent(rawValue) {
    ...
    if (rawValue === 200) return 100;  // ZCL 0-200 scale: 200 = 100%
          ...
          this._bgBatteryReadTimer = this.homey.setTimeout(async () => {
            ...
            try {
              ...
              const battery = await endpoint.clusters.powerConfiguration.readAttributes(['batteryPercentageRemaining'])
                .catch(() => null);
              // v9.0.146: Filter ZCL sentinel 255=unknown. ZCL 200 means 100%.
    ...
    for (const [epId, endpoint] of endpoints) {
      ...
      if (powerCfg) {
        try {
          ...
          if (attrs?.batteryPercentageRemaining != null && attrs.batteryPercentageRemaining !== 255) {
