...
class BaseUnifiedDevice extends TuyaZigbeeDevice {
  ...
  async safeSetCapabilityValue(capabilityId, value, options = {}) {
    try {
      ...
      // v9.0.77: Call SDK setCapabilityValue directly (NOT safeSetCapabilityValue — that's THIS method!)
      const setOk = await super.setCapabilityValue(capabilityId, value).then(
        () => true,
        err => {
