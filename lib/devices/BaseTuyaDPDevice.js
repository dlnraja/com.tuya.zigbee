 ...
 *   }
 */
class BaseTuyaDPDevice extends TuyaZigbeeDevice {
  ...
  async onNodeInit() {
    ...
    this._dpAutoDiscovery = this._dpAutoDiscovery || new IntelligentDPAutoDiscovery(this);
    ...
    // v5.8.69: Restore stored battery instead of misleading 100% default
    if (this.hasCapability('measure_battery')) {
      ...
      if (currentBattery === null || currentBattery === undefined) {
        ...
        if (stored !== null && stored !== undefined && typeof stored === 'number') {
          this.log(`[TUYA-DP] 🔋 Restored stored battery: ${stored}%`);
          await this.safeSetCapabilityValue('measure_battery', parseFloat(stored)).catch(() => { });
        } else {
          this.log('[TUYA-DP] 🔋 No stored battery, waiting for device report');
        }
      }
    }

    this.telemetryEstimator = DeviceTelemetryEstimator.attach(this);
    await this.telemetryEstimator?.refresh?.('tuya-dp-init').catch(err => this.log(`[TELEMETRY] init refresh failed: ${err.message}`));
  ...
  _onTuyaDataReport(payload) {
    ...
    this._dpAutoDiscovery?.recordFrame?.({
      clusterId: 0xEF00,
      endpointId: this._tuyaEndpointId,
      payload,
      source: 'base_tuya_dp_report'
    }, 'rx', { source: 'base_tuya_dp_report' });
    ...
    if (payload.dp !== undefined && payload.data !== undefined) {
      ...
      // into one report frame — split and process each, otherwise trailing DPs
      // are silently dropped.
      if (Buffer.isBuffer(payload.data) && typeof payload.length === 'number'
        && payload.data.length > payload.length) {
        dpValues = parseTuyaMultiDpFrame({ datatype: payload.dataType, ...payload })
          .map((sub) => ({ dp: sub.dp, dataType: sub.datatype, data: sub.data }));
      } else {
    ...
    for (const dpValue of dpValues) {
      ...
      const parsedValue = this._parseDPValue(dataType, data);

      this.log(`[TUYA-DP] DP${dp} (type=${dataType}) = ${JSON.stringify(parsedValue)}`);
      this._dpAutoDiscovery?.recordDP?.(dp, parsedValue, {
        dpType: dataType,
        direction: 'rx',
        source: 'base_tuya_dp_report'
      });
  ...
  async _initBatteryHandling() {
    ...
    this.on('tuya_dp_battery', async (value) => {
      ...
      if (prev !== null && (now - this._battLastDP) < 300000 && Math.abs(rounded - prev) < 2) {return;}
      this._battLastDP = now;

      this.log(`[TUYA-DP] 🔋 Battery update from Tuya DP: ${rounded}%`);
      await this.safeSetCapabilityValue('measure_battery', parseFloat(rounded)).catch(e => this.error(e));
      await this.setStoreValue('last_battery_percentage', rounded).catch(() => {});
      this._triggerBatteryFlowCard(rounded);
    });
  }
  ...
  async sendDP(dp, dataType, value) {
    ...
    this._dpAutoDiscovery?.recordDP?.(dp, value, {
      dpType: dataType,
      direction: 'tx',
      source: 'base_tuya_dp_send'
    });
