...
async function installWallSceneRemoteHybrid(device, zclNode, opts = {}) {
  ...
  const onPress = typeof opts.onPress === 'function'
    ? opts.onPress
    : async (btn, press) => {
      // WHY(P2706): never await triggerButtonPress — card cascade must not block
      // Zigbee RX handlers (TS0042/43 multi-path storms crashed / lagged TS0041).
      const firePress = () => {
        if (typeof device.triggerButtonPress === 'function') {
          return device.triggerButtonPress(btn, press, 1, { source: `wall-hybrid-${maxButtons}` });
        }
        if (typeof device.triggerButtonFlow === 'function') {
          return device.triggerButtonFlow(btn, press);
        }
        if (typeof device._triggerButtonPress === 'function') {
          return device._triggerButtonPress(btn, press);
        }
        return Promise.resolve();
      };
      Promise.resolve(firePress()).catch(() => {});
    };
  ...
  const snappyDedupMs = (() => {
    try {
      ...
      if (p.snappyRelayFlow) return Math.max(40, Number(p.crossPathDedupMs) || 60);
    } catch (_e) { /* soft */ }
    return 400;
  })();
  const isDeduped = (key, ms = snappyDedupMs) => {
  ...
  const fire = async (btn, press, src) => {
    ...
    if (isDeduped(`${b}-${p}`)) return; // WHY(P2706): path-agnostic key — collapse E000+0xFD storms
    log(`${src} btn${b} ${p}`);
    try {
      const r = onPress(b, p);
      if (r && typeof r.then === 'function') Promise.resolve(r).catch(() => {});
    } catch (_e) { /* soft — never crash on press */ }
    // WHY(P2614+P2707): Homey device-view tiles for scene remotes — fire-and-forget;
    // WHY(P2734): skipUiPulse uses soft complementary pulse (bi-dir) — never dead tiles.
    try {
      const profile = device.getDeviceProfile?.() || {};
      if (profile.skipUiPulse) {
        const { softPulsePhysicalUi } = require('../utils/HomeyButtonUiCharter');
        softPulsePhysicalUi(device, b, { press: p, source: `wall-hybrid-${src}`, snappy: true, pulseMs: 50 });
      } else {
        const { syncPhysicalToHomeyUi } = require('../utils/HomeyButtonUiCharter');
        Promise.resolve(syncPhysicalToHomeyUi(device, b, { press: p, source: `wall-hybrid-${src}` })).catch(() => {});
      }
    } catch (_e) { /* soft — charter optional */ }
  };
    ...
    wrapHandleFrame(zclNode, `${tag}-raw`, async (args, next) => {
      ...
      try {
        if (!device._onOffFdBoundClusterInitialized && (cId === 6 || cId === 0x0006)) {
          ...
          // WHY(P2616/P2328): complementary precise 0xFD decode via parseZclHeader
          let hdr = null;
          try {
            const { parseZclHeader } = require('../zigbee/ZigbeeHelpers');
            hdr = d ? parseZclHeader(d) : null;
          } catch (_e) { /* soft */ }
          const hdrCmd = hdr?.commandId ?? hdr?.cmdId;
          const looksFd = cmd === 0xFD || cmd === 253 || hdrCmd === 0xFD || hdrCmd === 253
            || (d && d.length && d.includes(0xFD));
          if (looksFd) {
            const scene = (hdr && hdr.payload && hdr.payload.length)
              ? hdr.payload[0]
              : ((typeof cmd === 'number' && cmd === 0xFD && d && d.length) ? d[0]
                : (d && d.length > 1 ? d[d.length - 1] : 0));
            const pt = PRESS_MAP[scene] || 'single';
            // WHY(P2636): TS0042 + 4-EP phantom firmware — never map EP3/4 → button 1
            if (epId < 1 || epId > maxButtons) return next();
            const gang = epId;
            await fire(gang, pt, 'raw-0xFD-hdr');
          }
        }
