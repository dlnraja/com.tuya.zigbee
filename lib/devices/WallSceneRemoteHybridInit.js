...
async function installWallSceneRemoteHybrid(device, zclNode, opts = {}) {
  ...
  const fire = async (btn, press, src) => {
    ...
    log(`${src} btn${b} ${p}`);
    await onPress(b, p);
    // WHY(P2614): keep Homey device-view button tiles in sync with physical press
    try {
      const { syncPhysicalToHomeyUi } = require('../utils/HomeyButtonUiCharter');
      await Promise.resolve(syncPhysicalToHomeyUi(device, b, { press: p, source: `wall-hybrid-${src}` }));
