...
async function installWallSceneRemoteHybrid(device, zclNode, opts = {}) {
  ...
  // WHY(P2614): use internal wallRemoteDedup only — driver `_isDeduped(ep, cmd)`
  // has a different signature and would mis-key hybrid events.
  ...
  const fire = async (btn, press, src) => {
    ...
    log(`${src} btn${b} ${p}`);
    await onPress(b, p);
    // WHY(P2614): ensure Homey device-view Button N pulse even if onPress skips mixin
    try {
      const { syncPhysicalToHomeyUi } = require('../utils/HomeyButtonUiCharter');
      Promise.resolve(syncPhysicalToHomeyUi(device, b, { source: 'physical' })).catch(() => {});
    } catch (_e) { /* soft */ }
  };
  ...
  // WHY(P2616): also try parseZclHeader (P2328) — complementary precise path before heuristic
  try {
    ...
    wrapHandleFrame(zclNode, `${tag}-raw`, async (args, next) => {
      ...
      try {
        if (!device._onOffFdBoundClusterInitialized && (cId === 6 || cId === 0x0006)) {
          ...
          let fired = false;
          // P2328 complementary: precise ZCL header decode (mfr + cmdId + payload)
          try {
            const { parseZclHeader } = require('../zigbee/ZigbeeHelpers');
            const hdr = d ? parseZclHeader(d) : null;
            if (hdr && hdr.cmdId === 0xFD
                && d && d.length === hdr.payloadOffset + 1
                && [0, 1, 2].includes(d[hdr.payloadOffset])) {
              const pt = PRESS_MAP[d[hdr.payloadOffset]] || resolvePressType(d[hdr.payloadOffset], tag);
              const gang = (epId >= 1 && epId <= maxButtons) ? epId : 1;
              await fire(gang, pt, 'raw-0xFD-hdr');
              fired = true;
            }
          } catch (_e) { /* fall through heuristic */ }
          if (!fired) {
    ...
    log('raw wrapHandleFrame armed (heuristic + P2328 parseZclHeader)');
  } catch (e) {
    log('raw wrap soft-skip:', e.message);
  }

  // 6) Broader E000 cluster name variants (complementary to primary loop)
  try {
    for (let ep = 1; ep <= maxButtons; ep++) {
      const endpoint = zclNode?.endpoints?.[ep];
      ...
      for (const e000 of alts) {
        if (typeof e000.on !== 'function') continue;
        for (const cmdName of ['cmd0', 'cmd1', 'cmdFD', 'cmdFE', 'cmdFF']) {
          try {
            e000.on(cmdName, async ({ data } = {}) => {
              let btn = ep;
              let press = 'single';
              ...
              await fire(btn, press, `E000-alt-${cmdName}`);
            });
          } catch (_e) { /* */ }
        }
      }
