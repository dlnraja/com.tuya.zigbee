...
class UnifiedThermostatBase extends TuyaZigbeeDevice {
  ...
  get dpMappings() {
    return {
      ...
      // WHY: Homey setting id is temperature_calibration (matches _applyTrvTemperatureCalibration)
      27: { capability: null, setting: 'temperature_calibration', divisor: 10 },
      47: { capability: null, setting: 'temperature_calibration', divisor: 10 }, // Alternative
      // Min/Max temp limits
  ...
  async onNodeInit({ zclNode }) {
    ...
    // WHY(P2596 / Michaelp 6eabd9c4): battery TRV must still query DPs after pair
    this.forceActiveTuyaMode = true;
    this._deviceInitTime = Date.now();
    this._trvInitAt = this._deviceInitTime;
    ...
    // registration — partial init must stay controllable.
    try {
      if (this._protocolInfo.isTuyaDP || this._isPureTuyaDP || this._forcePureTuyaDp) {
      ...
      this.error(`[THERMOSTAT] ⚠️ mode setup error (continuing): ${e.message}`);
    }

    try {
      this._registerCapabilityListeners();
    } catch (e) {
      this.error(`[THERMOSTAT] ❌ Capability listeners: ${e.message}`);
    }
  ...
  async _applyManufacturerConfig() {
    ...
    const mfrLower = normalizeCI(manufacturerName);
    ...
    // WHY(P2594 / Michaelp #2253): never wipe driver me167/ogx8u5z6 sacred map —
    // ManufacturerVariationManager generic maps made all caps stay null.
    if (config.dpMappings && Object.keys(config.dpMappings).length > 0) {
      const driverMaps = (typeof this.dpMappings === 'object' && this.dpMappings) ? this.dpMappings : {};
      const me167Family = /ogx8u5z6|o3x45p96|p3dbf6qs|rv6iuyxb|c6wv4xyo|hvaxb2tc/i.test(manufacturerName);
      if (me167Family || this.dpProfile === 'me167') {
        this._dynamicDpMappings = { ...config.dpMappings, ...driverMaps };
        this.log('[THERMOSTAT] 🔄 me167/driver DP maps win over variation (P2594)');
      } else {
  ...
  /**
   * WHY(P2594 / Michaelp #2253): same Contre quoi as Cover P2363 —
   * UnifiedThermostatBase skips TuyaZigbeeDevice super.onNodeInit, so
   * tuyaEF00Manager was never created → dpReport never hooked → all caps null.
   * Soft-create + launchOnce + cluster fallback (Cover pattern).
   */
  async _setupTuyaDPMode() {
    if (!this.tuyaEF00Manager) {
      try {
        ...
        this.log('[THERMOSTAT] ✅ TuyaEF00Manager created (P2594)');
      } catch (e) {
        this.log(`[THERMOSTAT] ⚠️ TuyaEF00Manager create failed: ${e.message}`);
      }
    }
    if (this.tuyaEF00Manager) {
      if (typeof this.tuyaEF00Manager.on === 'function' && !this._thermoEf00DpHooked) {
        this._thermoEf00DpHooked = true;
      ...
      if (zcl) {
        try {
          ...
          this.log('[THERMOSTAT] ✅ TuyaEF00Manager.launchOnce (P2594)');
        } catch (e) {
          this.log(`[THERMOSTAT] ⚠️ TuyaEF00Manager.launchOnce failed: ${e.message}`);
        }
      }
      return;
    }
    // Direct cluster fallback when manager unavailable
    const ep = this.zclNode?.endpoints?.[1]?.clusters;
    const tc = ep?.tuya || ep?.[61184] || ep?.['61184'];
    if (!tc?.on || this._thermoClusterDpHooked) {return;}
    this._thermoClusterDpHooked = true;
    const h = (d) => {
      if (d?.dp === undefined) {return;}
      const r = Buffer.isBuffer(d.data)
        ? (d.data.length === 1 ? d.data[0] : d.data.readUIntBE(0, Math.min(d.data.length, 4)))
        : (d.value ?? d.data);
      this._handleDP(d.dp, r);
    };
    try {
      tc.on('response', (_s, _t, d) => h(d));
      tc.on('dataReport', h);
      tc.on('reporting', h);
      this.log('[THERMOSTAT] Direct Tuya cluster listener (no manager)');
    } catch (_e) { /* soft */ }
  }
  ...
  _scheduleThermostatInitCommands() {
    (this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout)(async () => {
      ...
      try {
        // Prefer live driver maps (me167) over stale variation wipe
        const maps = (this.dpProfile === 'me167' && this.dpMappings)
          ? this.dpMappings
          : (this._dynamicDpMappings || this.dpMappings || {});
        let dps = Object.keys(maps)
          .map(Number)
          .filter((d) => d > 0 && d < 200)
          .slice(0, 12);
        // WHY(P2596): me167 sacred DPs if maps empty/wrong at first fire
        if (this.dpProfile === 'me167' && !dps.includes(4)) {
          dps = [2, 3, 4, 5, 7, 13, 15, 35];
        }
        if (dps.length === 0) {return;}
        await tuyaDataQuery(this, dps, { logPrefix: '[THERMOSTAT-INIT]', silent: false, force: true });
  ...
  _handleDP(dpId, raw) {
    ...
    // WHY P2278: Tuya VALUE often packs signed cal as unsigned 32-bit
    if (typeof value === 'number' && value > 0x7fffffff) {
      value -= 0x100000000;
    }
    if (mapping.smartDivisor === true && mapping.capability) {
    ...
    // WHY: settings-only DPs (cal, min/max) must update Homey settings — not only capabilities
    if (mapping.setting && !mapping.capability) {
      if (value === null || value === undefined) {return;}
      this.log(`[THERMOSTAT] DP${dpId} → setting ${mapping.setting} = ${value}`);
  ...
  /**
   * WHY P2278: TX for writable setting DPs (temp_calibration / temperature_calibration)
   * Uses mapping.divisor so ogx8u5z6 tenths (÷10) and ME167 whole-degrees (÷1) both work.
   */
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    const mappings = this._dynamicDpMappings || this.dpMappings || {};
    const calKeys = ['temperature_calibration', 'temp_calibration'];
    for (const key of changedKeys || []) {
      if (!calKeys.includes(key)) {continue;}
      const candidates = Object.entries(mappings)
        .filter(([, m]) => m.setting === key || (calKeys.includes(m.setting) && calKeys.includes(key)))
        .sort((a, b) => Number(a[0]) - Number(b[0]));
      // Prefer highest DP id (47 over 27) when both map to calibration
      const pick = candidates[candidates.length - 1];
      if (!pick) {continue;}
      const targetDp = Number(pick[0]);
      const divisor = pick[1].divisor || 1;
      const human = Number(newSettings[key]);
      if (!Number.isFinite(human)) {continue;}
      const dpValue = Math.round(human * divisor);
      if (this.tuyaEF00Manager?.sendDP) {
        await this.tuyaEF00Manager.sendDP(targetDp, dpValue, 'value').catch((e) => {
          this.log(`[THERMOSTAT] cal TX DP${targetDp} failed:`, e.message);
        });
      } else if (typeof this._sendTuyaDP === 'function') {
        await this._sendTuyaDP(targetDp, dpValue, 'value').catch(() => {});
      }
      this.log(`[THERMOSTAT] cal TX DP${targetDp}=${dpValue} (human=${human}, ×${divisor})`);
    }
    return true;
  }
  ...
  async _executeValveCommand(position) {
    ...
    try {
      ...
      if (this._isPureTuyaDP && this.tuyaEF00Manager) {
        ...
        (this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout)(() => {
          if (this._destroyed) {return;}
          this._verifyValvePosition();
        }, 3000);
      ...
      if (this.hasCapability('dim')) {
        await this.safeSetCapabilityValue('dim', position / 100);
        state.currentPosition = position;
        return true;
      }
      ...
      if (state.retryCount <= state.maxRetries) {
        ...
        state.pendingRetry = (this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout)(async () => {
          if (this._destroyed) {return;}
