...
class UnifiedThermostatBase extends BaseUnifiedDevice {
  ...
  get dpMappings() {
    return {
      ...
      6: { capability: null, internal: 'preset' }, // schedule / manual / boost/comfort / safeDivide(eco, away)
    ...
    if (this.hasCapability('target_temperature')) {
      this.registerCapabilityListener('target_temperature', async (value) => {
        if (this._isPureTuyaDP && this.tuyaEF00Manager) {
          ...
          await this.tuyaEF00Manager.sendDP(targetDP, Math.round(safeMultiply(value, divisor), 'value'));
        }
      });
    }
  }
