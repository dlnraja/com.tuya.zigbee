...
class IRBlasterCodeDatabase extends EventEmitter {
  ...
  startLearning() {
    return new Promise((resolve, reject) => {
      ...
      // Send study mode command to device (rejects on failure)
      Promise.resolve(this._sendDP(this.dpMapping.IR_TX_STUDY, true, 'bool')).catch((err) => {
      ...
      // Set timeout
      this._learningTimer = (this.device.homey?.setTimeout ? this.device.homey.setTimeout.bind(this.device.homey) : globalThis.setTimeout)(() => {
        if (!this.device || this.device._destroyed) {return;}
        this.learningMode = false;
        this._learningResolve = null;
