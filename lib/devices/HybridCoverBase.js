...
class HybridCoverBase extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v5.5.929: Save manufacturer info from zclNode to settings for protocol detection
    // This fixes "unknown/unknown" issue when settings don't have manufacturer info
    try {
      const node = zclNode?.endpoints?.[1]?.node || zclNode;
      const mfrName = node?.manufacturerName || zclNode?.manufacturerName;
      const modelId = node?.modelId || zclNode?.modelId;
      if (mfrName || modelId) {
