...
class HybridCoverBase extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    this.log('[COVER] 📦 App version:', getAppVersionPrefixed());
  ...
  /**
   * v5.5.202: Enhanced diagnostic check to identify communication issues
   * Helps troubleshoot curtain roller non-responsive issues + "could not get device by id" errors
   */
  async _diagnosticCheck() {
    try {
      // Check if device exists in Homey's device registry
      const deviceId = this.getData()?.id;
      if (!deviceId) {
        throw new Error('Device ID not available - device may not be properly registered');
      }

      // Verify device is still in homey registry
      try {
        const device = this.homey.app.getDevice?.({ id: deviceId });
        if (!device) {
          this.log('[COVER] ⚠️ Device not found in registry, attempting re-registration');
        }
      } catch (registryError) {
        this.log('[COVER] ⚠️ Registry check failed:', registryError.message);
      }
      ...
      if (this._isPureTuyaDP && !tuyaCluster) {
        throw new Error('Tuya cluster not available - device may have disconnected, check Zigbee mesh');
      }

      // Check TuyaEF00Manager availability for DP devices
      ...
      this.log('[COVER] 🔍 Enhanced diagnostic check passed - proceeding with command');
      ...
      try {
        ...
        if (tuyaCluster) {
          ...
          if (typeof tuyaCluster.dataRequest === 'function') {
            ...
            this.log(`[COVER] ✅ DP${dpId} sent via dataRequest`);
            return;
          } else if (typeof tuyaCluster.setData === 'function') {
            await tuyaCluster.setData({ data: dpData });
            this.log(`[COVER] ✅ DP${dpId} sent via setData`);
            return;
          } else if (typeof tuyaCluster.command === 'function') {
            await tuyaCluster.command('dataRequest', dpData);
            this.log(`[COVER] ✅ DP${dpId} sent via command`);
            return;
          }
        }

        // Method 3: Raw cluster write (fallback)
        if (tuyaCluster && typeof tuyaCluster.writeAttributes === 'function') {
          await tuyaCluster.writeAttributes({ [dpId]: value });
          this.log(`[COVER] ✅ DP${dpId} sent via writeAttributes`);
          return;
        }
