...
class HybridCoverBase extends ZigBeeDevice {
  ...
  /**
   * v5.5.202: Diagnostic check to identify communication issues
   * Helps troubleshoot curtain roller non-responsive issues
   */
  async _diagnosticCheck() {
    if (!this.zclNode) {
      throw new Error('ZCL node not available - device may be offline');
    }

    const ep = this.zclNode?.endpoints?.[1];
    if (!ep) {
      throw new Error('Endpoint 1 not available - device communication issue');
    }
    ...
    if (this._isPureTuyaDP && !tuyaCluster) {
      throw new Error('Tuya cluster not available - device may have disconnected');
    }
