  ...
  async _sendTuyaDP(dpId, value, dataType = 'value') {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Method 2: Direct cluster datapoint command (v5.5.937 FIX)
        const ep = this.zclNode?.endpoints?.[1];
        const tuyaCluster = ep?.clusters?.tuya || ep?.clusters?.manuSpecificTuya || ep?.clusters?.[61184];

        if (tuyaCluster) {
          // Build correct payload format: { dp, datatype, data }
          ...
          if (dataType === 'bool') {
            ...
            dataBuf = Buffer.from([value]);
          } else {
            dataBuf = Buffer.alloc(4);
            dataBuf.writeInt32BE(value, 0);
          }

          // v5.5.937: Use correct method - tuyaCluster.datapoint()
          if (typeof tuyaCluster.datapoint === 'function') {
            await tuyaCluster.datapoint({ dp: dpId, datatype: dt, data: dataBuf });
            this.log(`[COVER] ✅ DP${dpId} sent via datapoint()`);
            return;
          } else if (typeof tuyaCluster.setDataValue === 'function') {
