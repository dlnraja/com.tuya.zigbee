...
class HybridCoverBase extends ZigBeeDevice {
  ...
  /**
   * v5.7.9: Enhanced Tuya DP communication with:
   * - Exponential backoff (500ms, 1500ms, 3000ms)
   * - Wake-up ping for sleepy/battery devices
  ...
  async _sendTuyaDP(dpId, value, dataType = 'value') {
    ...
    // v5.7.9: Track communication health
    this._commFailures = this._commFailures || 0;
    this._lastCommSuccess = this._lastCommSuccess || 0;

    // v5.7.9: Wake-up ping for battery/sleepy devices before first attempt
    if (!this.mainsPowered || Date.now() - this._lastCommSuccess > 60000) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (tuyaCluster && typeof tuyaCluster.datapoint === 'function') {
          // v5.7.20: Build data buffer for both old and new format
          ...
          // v5.7.20: Try BOTH formats - some devices need old format, some need new
          // OLD format (v5.5.937): { dp, datatype, data: Buffer }
          // NEW format (v5.5.939): { data: completeFrame }
          
          // Try OLD format first (v5.5.937 - worked for elgato7)
          try {
            await tuyaCluster.datapoint({ dp: dpId, datatype: dt, data: dataBuf });
            this.log(`[COVER] ✅ DP${dpId} sent via datapoint({dp,datatype,data}) - OLD format`);
            this._commFailures = 0;
            this._lastCommSuccess = Date.now();
            return;
          } catch (oldErr) {
            this.log(`[COVER] ⚠️ OLD format failed: ${oldErr.message}, trying NEW format...`);
          }

          // Try NEW format (v5.5.939 - complete frame)
          try {
            ...
            this.log(`[COVER] ✅ DP${dpId} sent via datapoint({data}) - NEW format`);
            this._commFailures = 0;
            this._lastCommSuccess = Date.now();
            return;
          } catch (newErr) {
            this.log(`[COVER] ⚠️ NEW format also failed: ${newErr.message}`);
          }

          // v5.7.20: Try setDataValue as last resort
          if (typeof tuyaCluster.setDataValue === 'function') {
            await tuyaCluster.setDataValue({ dp: dpId, datatype: dt, data: dataBuf });
            this.log(`[COVER] ✅ DP${dpId} sent via setDataValue()`);
            this._commFailures = 0;
            this._lastCommSuccess = Date.now();
            return;
          }
        ...
        this.error(`[COVER] DP${dpId} attempt ${attempt} failed:`, errorMsg);

        if (attempt === maxRetries) {
          this._commFailures++;
          this.error(`[COVER] ❌ All ${maxRetries} attempts failed for DP${dpId}=${value}`);
          
          // v5.7.9: Enhanced error with recovery suggestions
          ...
          if (isTimeout || isNotResponding) {
            ...
            // Don't throw for transient issues - device may still respond
            if (this._commFailures < 5) {
              this.log('[COVER] ⚠️ Transient failure - command may still execute');
              return; // Soft fail - don't throw
            }
          }
          
          // v5.7.9: Set unavailable after repeated failures
          if (this._commFailures >= 5) {
            this.log('[COVER] 🔴 Multiple failures detected - marking device as potentially offline');
            this.setWarning('Communication issues - check device power and Zigbee mesh').catch(() => {});
          }
          
          throw new Error(`Device not responding after ${maxRetries} attempts. Try: power cycle device, check Zigbee range, or re-pair.`);
        }

        // v5.7.9: Exponential backoff (500ms, 1500ms, 3000ms)
        const delay = baseDelay * Math.pow(2, attempt - 1);
        this.log(`[COVER] ⏳ Waiting ${delay}ms before retry...`);
