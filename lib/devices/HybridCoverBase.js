...
class HybridCoverBase extends ZigBeeDevice {
  ...
  async _applyManufacturerConfig() {
    ...
    // v5.5.997: Set inverted control flag for _TZE204_ Longsam etc.
    if (config.invertedControl) {
      this._invertedControl = true;
      this.log(`[COVER] 🔄 INVERTED control mode enabled (open=2, close=0)`);
    }
    ...
    if (this.hasCapability('windowcoverings_state')) {
      this.registerCapabilityListener('windowcoverings_state', async (state) => {
        try {
          ...
          if (this._isPureTuyaDP) {
            // v5.5.997: Check for inverted control (Longsam _TZE204_xu4a5rhj etc.)
            // Standard: open=0, stop=1, close=2
            // Inverted: open=2, stop=1, close=0
            let cmd;
            if (this._invertedControl) {
              cmd = state === 'up' ? 2 : state === 'down' ? 0 : 1;
              this.log(`[COVER] Using INVERTED control: ${state} → DP1=${cmd}`);
            } else {
              cmd = state === 'up' ? 0 : state === 'down' ? 2 : 1;
            }
