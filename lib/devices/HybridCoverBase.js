...
class HybridCoverBase extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v5.5.930: ENHANCED - Save manufacturer info from ALL sources to settings
    // This fixes "unknown/unknown" issue when settings don't have manufacturer info
    try {
      const data = this.getData() || {};
      const store = this.getStore?.() || {};
      const settings = this.getSettings?.() || {};
      
      // v5.5.930: Try multiple sources for manufacturer info (prioritize zclNode)
      ...
      // v5.5.930: Also try reading from basic cluster
      ...
      // v5.5.930: Log what we found for debugging
  ...
  _detectProtocol() {
    ...
    this.log(`[COVER] 🔍 Protocol detection: model=${modelId}, mfr=${mfr}, isTuyaDP=${isTuyaDP}`);
