...
class TuyaHybridDevice extends ZigBeeDevice {
  ...
  async _setupTuyaBoundCluster(endpoint) {
    ...
    class TuyaInlineBoundCluster extends BoundCluster {
      /**
       * v5.5.76: BoundCluster receives raw payload for each command
       * Command names MUST match TuyaSpecificCluster.COMMANDS
       *
       * Payload format (raw Buffer):
      ...
      _handleTuyaPayload(cmdName, payload) {
        try {
          ...
          if (Buffer.isBuffer(payload)) {
            device.log(`[TUYA-BOUND] Raw buffer: ${payload.toString('hex')}`);
          } else if (payload && typeof payload === 'object') {
            device.log(`[TUYA-BOUND] Payload keys: ${Object.keys(payload).join(', ')}`);
            // Check for frame/data property
            if (payload.frame) {
              device.log(`[TUYA-BOUND] Frame: ${payload.frame.toString('hex')}`);
            }
            if (payload.data) {
              device.log(`[TUYA-BOUND] Data: ${Buffer.isBuffer(payload.data) ? payload.data.toString('hex') : JSON.stringify(payload.data)}`);
            }
          }
      ...
      // Handler for 'dataReport' command (0x01) - spontaneous report
      dataReport(payload) {
        this._handleTuyaPayload('dataReport', payload);
      }

      // Handler for 'dataResponse' command (0x02) - response to getData
      dataResponse(payload) {
        this._handleTuyaPayload('dataResponse', payload);
      }

      // Handler for 'setDataPoint' (0x04) - if device echoes back
      setDataPoint(payload) {
        this._handleTuyaPayload('setDataPoint', payload);
      }

      // Handler for 'mcuSyncTime' (0x24)
      mcuSyncTime(payload) {
      ...
      // Handler for 'mcuVersionResponse' (0x11)
      mcuVersionResponse(payload) {
        this._handleTuyaPayload('mcuVersionResponse', payload);
      }

      // Handler for 'mcuGatewaySenceData' (0x25)
      mcuGatewaySenceData(payload) {
        this._handleTuyaPayload('mcuGatewaySenceData', payload);
      }

      // Legacy/fallback handlers for different naming conventions
      response(payload) {
        this._handleTuyaPayload('response (legacy)', payload);
      }

      reporting(payload) {
        this._handleTuyaPayload('reporting (legacy)', payload);
      }
    }
  ...
  _setupRawFrameListener(endpoint) {
    ...
    try {
      if (typeof endpoint.on !== 'function') {
        this.log('[TUYA-P3] Endpoint has no .on() method');
        return;
      }
