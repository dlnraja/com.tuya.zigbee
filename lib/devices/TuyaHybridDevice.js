...
class TuyaHybridDevice extends ZigBeeDevice {
  ...
  async _setupTuyaBoundCluster(endpoint) {
    ...
    class TuyaInlineBoundCluster extends BoundCluster {
      // Handler for 'response' command (0x01)
      response(payload) {
        device.log('[TUYA-BOUND] 📥 response command received');
        device._registerTuyaHit();
        device._processTuyaData(payload);
      }

      // Handler for 'reporting' command (0x02)
      reporting(payload) {
        device.log('[TUYA-BOUND] 📥 reporting command received');
        device._registerTuyaHit();
        device._processTuyaData(payload);
      }

      // Handler for 'dataReport' (alternative name)
      dataReport(payload) {
        device.log('[TUYA-BOUND] 📥 dataReport command received');
        device._registerTuyaHit();
        device._processTuyaData(payload);
      }
      ...
      mcuSyncTime(payload) {
        ...
        // Could respond with time sync here
      }
    }
