...
class TuyaHybridDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    this.log('');
    this.log('╔══════════════════════════════════════════════════════════════╗');
    this.log('║          TUYA HYBRID DEVICE v5.5.48                          ║');
    this.log('╚══════════════════════════════════════════════════════════════╝');
  ...
  async _setupTuyaBoundCluster(endpoint) {
    ...
    class TuyaInlineBoundCluster extends BoundCluster {
      /**
       * v5.5.74: Command names MUST match TuyaSpecificCluster.COMMANDS
       * - dataResponse (0x02): Device responds to getData
       * - dataReport (0x01): Device spontaneously reports data
       * - mcuSyncTime (0x24): Device requests time
       */

      // Handler for 'dataResponse' command (0x02) - response to getData
      dataResponse(payload) {
        device.log('[TUYA-BOUND] 📥 dataResponse received');
      ...
      dataReport(payload) {
        device.log('[TUYA-BOUND] 📥 dataReport received');
        device._registerTuyaHit();
        device._processTuyaData(payload);
      }
      ...
      response(payload) {
        device.log('[TUYA-BOUND] 📥 response received (legacy)');
        device._registerTuyaHit();
        device._processTuyaData(payload);
      }

      reporting(payload) {
        device.log('[TUYA-BOUND] 📥 reporting received (legacy)');
        device._registerTuyaHit();
        device._processTuyaData(payload);
      }
      ...
      mcuSyncTime(payload) {
        ...
        // Respond with time sync
        device._respondToTimeSync?.();
      }

      // Handler for 'setDataPoint' (0x04) - if device sends this
      setDataPoint(payload) {
        device.log('[TUYA-BOUND] 📥 setDataPoint received');
        device._registerTuyaHit();
        device._processTuyaData(payload);
      }
    }
