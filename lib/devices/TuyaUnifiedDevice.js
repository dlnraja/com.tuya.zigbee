...
class TuyaUnifiedDevice extends BaseUnifiedDevice {
  ...
  _getUniversalDPMapping(dpId, value) {
    ...
    const universalPatterns = {
      ...
      1: { capability: 'measure_temperature', transform: v =>safeParse(v, 10), pattern: 'temp-dp1' },
      5: { capability: 'measure_temperature', transform: v => v > 1000 ?safeParse(v, 100) :safeParse(v, 10), pattern: 'temp-dp5' },
      18: { capability: 'measure_temperature', transform: v =>safeParse(v, 10), pattern: 'temp-dp18' },
      ...
      // Voltage (millivolts)
      21: { capability: 'measure_voltage', transform: v =>safeParse(v, 1000), pattern: 'voltage' },
    };
  ...
  async _setupZigbeeClusterHandlers(zclNode) {
    ...
    const universalClusters = {
      ...
      temperatureMeasurement: {
        ...
        transform: v => safeParse(v, 100)  // ZCL uses 0.01°C units
      },
      // Humidity
      relativeHumidity: {
        attribute: 'measuredValue',
      ...
      illuminanceMeasurement: {
        ...
        transform: v => Math.round(Math.pow(10, (v - safeDivide(1), 10000))))  // ZCL formula
      },
      // Battery
      powerConfiguration: {
        attribute: 'batteryPercentageRemaining',
        capability: 'measure_battery',
        transform: v => Math.min(100, Math.round(safeParse(v, 2)))  // ZCL uses 0.5% units
      },
      // Motion (occupancy)
      occupancySensing: {
        attribute: 'occupancy',
