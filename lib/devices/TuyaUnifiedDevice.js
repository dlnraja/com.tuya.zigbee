...
class TuyaUnifiedDevice extends TuyaZigbeeDevice {
  ...
  async _setupLowLevelNodeHandler() {
    ...
    try {
      ...
      // WHY(P2284): append-only wrap — never orphan Physical 0xFD / L0 / IO
      const device = this;
      const { wrapHandleFrame } = require('../utils/BidirectionalButtonState');
      wrapHandleFrame(node, 'tuya-unified-p0-ef00', (args, next) => {
      ...
      this.log('[TUYA-P0] ✅ handleFrame wrap installed (P2284 chain)');
  ...
  async _handleDP(dpId, value) {
    ...
    if (mapping) {
      ...
      if (this.hasCapability(mapping.capability)) {
        await this.safeSetCapabilityValue(mapping.capability, finalValue).catch(err => {
    ...
    if (autoMapping) {
      const finalValue = autoMapping.transform ? autoMapping.transform(value) : value;
      ...
      if (this.hasCapability(autoMapping.capability)) {
        await this.safeSetCapabilityValue(autoMapping.capability, finalValue).catch(err => {
  ...
  _enumToBattery(value) {
    ...
    return Object.prototype.hasOwnProperty.call(states, value) ? states[value] : null;
  }
  ...
  async _handleBatteryDP(dpId, value) {
    ...
    if (percentage !== null && this.hasCapability('measure_battery')) {
      ...
      await this.safeSetCapabilityValue('measure_battery', parseFloat(rounded)).catch(err => {
  ...
  _setupClusterListener(cluster, clusterName, epId, driverHandler, universalHandler) {
    try {
      ...
      cluster.on('attr', async (attrName, value) => {
        ...
        if (driverHandler?.attributeReport) {
          if (this.Mode.zigbeeActive || !this.DecisionMade) {
        ...
        if (universalHandler && universalHandler.capability) {
          if (attrName === universalHandler.attribute || !universalHandler.attribute) {
            ...
            if (this.hasCapability(universalHandler.capability)) {
              await this.safeSetCapabilityValue(universalHandler.capability, finalValue).catch(err => {
  ...
  _registerTuyaHit() {
    ...
    if (!this.mainsPowered && !this._wakeUpCalled) {
      ...
      if (typeof this.onWakeUp === 'function') {
        this.homey.setTimeout(() => { if (this._destroyed) {return;} this.onWakeUp().catch(e => this.log('[HYBRID] onWakeUp error:', e.message)); }, 500); // Small delay to let initial data process
      }
    }
  }
  ...
  _finalizeHybridMode() {
    ...
    if (mode.tuyaHits > 0 && mode.zigbeeHits === 0) {
      ...
      if (forceActive) {
        ...
        this.log('[HYBRID] ⚠️ No data received - keeping mode');
      }
    }
...
TuyaUnifiedDevice.TuyaUnifiedDevice = TuyaUnifiedDevice;
