...
class TuyaUnifiedDevice extends TuyaZigbeeDevice {
  ...
  _registerTuyaHit() {
    ...
    if (!this.mainsPowered && !this._wakeUpCalled) {
      ...
      if (typeof this.onWakeUp === 'function') {
        this._wakeUpTimer = this.homey.setTimeout(() => {
  ...
  onUninit() {
    ...
    if (this._wakeUpTimer) {
      this.homey.clearTimeout(this._wakeUpTimer);
      this._wakeUpTimer = null;
    }
    if (this._dpBatchTimer) {
      clearTimeout(this._dpBatchTimer);
      this._dpBatchTimer = null;
    }
    this.log('[HYBRID] Device uninitialized (timers cleared)');
    if (super.onUninit) {
      super.onUninit();
    }
  }
