...
class SwitchDevice extends AutoAdaptiveDevice {
  ...
  async toggleGang(gang = 1) {
    ...
    if (this.hasCapability(capabilityId)) {
      ...
      await this.setCapabilityValue(capabilityId, !currentValue).catch(err => {
  ...
  async allOn() {
    ...
    for (let gang = 1; gang <= gangCount; gang++) {
      const capabilityId = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
      if (this.hasCapability(capabilityId)) {
        promises.push(this.setCapabilityValue(capabilityId, true).catch(err => {
  ...
  async allOff() {
    ...
    for (let gang = 1; gang <= gangCount; gang++) {
      const capabilityId = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
      if (this.hasCapability(capabilityId)) {
        promises.push(this.setCapabilityValue(capabilityId, false).catch(err => {
