...
class SwitchDevice extends AutoAdaptiveDevice {
  ...
  async toggleGang(gang = 1) {
    ...
    if (this.hasCapability(capabilityId)) {
      ...
      await this.safeSetCapabilityValue(capabilityId, !currentValue);
      return !currentValue;
    }
  ...
  async allOn() {
    ...
    for (let gang = 1; gang <= gangCount; gang++) {
      const capabilityId = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
      if (this.hasCapability(capabilityId)) {
        promises.push(await this.safeSetCapabilityValue(capabilityId, true));
      }
    }
  ...
  async allOff() {
    ...
    for (let gang = 1; gang <= gangCount; gang++) {
      const capabilityId = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
      if (this.hasCapability(capabilityId)) {
        promises.push(await this.safeSetCapabilityValue(capabilityId, false));
      }
    }
