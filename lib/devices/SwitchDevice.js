...
class SwitchDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    // Initialize base (power detection)
    ...
    for (let gang = 2; gang <= gangCount; gang++) {
      ...
      if (this.hasCapability(capabilityId)) {
        this.registerCapabilityListener(capabilityId, async (value) => {
          if (typeof this.markAppCommand === 'function') {this.markAppCommand(gang, value);}
  ...
  async toggleGang(gang = 1) {
    ...
    if (this.hasCapability(capabilityId)) {
      ...
      await this.safeSetCapabilityValue(capabilityId, !currentValue).catch(err => {
        this.error(`[TOGGLE] Failed to set ${capabilityId}:`, err.message);
      });
      return !currentValue;
    }
  ...
  async allOn() {
    ...
    for (let gang = 1; gang <= gangCount; gang++) {
      const capabilityId = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
      if (this.hasCapability(capabilityId)) {
        promises.push(this.safeSetCapabilityValue(capabilityId, true).catch(err => {
          this.error(`[ALL-ON] Failed to set ${capabilityId}:`, err.message);
        }));
      }
    }
  ...
  async allOff() {
    ...
    for (let gang = 1; gang <= gangCount; gang++) {
      const capabilityId = gang === 1 ? 'onoff' : `onoff.gang${gang}`;
      if (this.hasCapability(capabilityId)) {
        promises.push(this.safeSetCapabilityValue(capabilityId, false).catch(err => {
          this.error(`[ALL-OFF] Failed to set ${capabilityId}:`, err.message);
        }));
      }
    }
