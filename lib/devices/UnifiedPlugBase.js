 ...
 */
class UnifiedPlugBase extends TuyaZigbeeDevice {
  ...
  // v5.12.25: Configurable ZCL energy divisors with user-defined scaling support
  get zclEnergyDivisors() {
    ...
    // Z2M/forum: PJ-1203A (81yrt3*) and TS011F USB plugs (w0qqde0g) report activePower in Watts (÷1), not deciWatts.
    const isDirectWatt = containsCI(mfr, '81yrt3')
      || containsCI(mfr, 'w0qqde0g');

    return { 
      power: (isDirectWatt ? 1 : 10) / powerScale, 
      voltage: 10 / voltageScale, 
      current: 1000 / currentScale,
      energy: 100 / energyScale
    };
  }
  ...
  get dpMappings() {
    return {
      ...
      121: { capability: 'power_on_behavior', transform: (v) => ({ 0: 'off', 1: 'on', 2: 'previous' }[v] ?? 'previous') }, // Alternative DP

      // Energy monitoring (v9.1: smartDivisor for auto-detection)
  ...
  async onNodeInit({ zclNode }) {
    // v5.12.60 (P92.128): partial-init resilience — base crash must not
    // kill the plug's own (already guarded) init chain.
    if (super.onNodeInit) {
      try {
        await super.onNodeInit({ zclNode });
      } catch (e) {
        this.error(`[HYBRID-PLUG] ⚠️ base onNodeInit failed (continuing): ${e.message}`);
    ...
    try {
      ...
      this.log(`[HYBRID-PLUG] ⚠️ Validator init: ${e.message}`);
    }
    ...
    try {
      ...
      await this._safeInvoke(async () => await this._applyManufacturerConfig(), 'applyManufacturerConfig');
      ...
      await this._migrateCapabilities().catch(e => this.log(`[HYBRID-PLUG] ⚠️ Migrate: ${e.message}`));
      ...
      try {
        ...
        this.log(`[HYBRID-PLUG] ⚠️ ProtocolOptimizer: ${e.message}`);
      }
    ...
    try {
      if (this.hasCapability('power_on_behavior')) {
        this.registerCapabilityListener('power_on_behavior', async (value) => {
          await this.setPowerOnBehavior(value);
          await this.setSettings({ power_on_behavior: value === 'previous' ? 'memory' : value }).catch(() => {});
          return true;
        });
      }
    } catch (e) {
      this.error(`[HYBRID-PLUG] ❌ power_on_behavior listener: ${e.message}`);
    }

    // v5.8.49: ZCL attribute reporting + initial state read (SDK3 best practice)
    await this._setupZCLReporting(zclNode).catch(e =>
      this.log(`[PLUG] ⚠️ ZCL reporting: ${e.message}`)
    );
    await this._readInitialPlugState(zclNode).catch(e =>
      this.log(`[PLUG] ⚠️ Initial state read: ${e.message}`)
    );
    ...
    // v9.0.418 (P92.126): virtual energy estimation when the hardware
    // exposes NO metering path — on-time × configurable load per gang.
    // NOTE: runs BEFORE the watchdog's 15min verdict would matter; the
    // estimator only activates when no real metering exists, and feeds
    // measure_power/meter_power so the watchdog keeps the caps alive.
    try {
      ...
      if (!hasRealMetering && !hasEnergyDp && estimationEnabled && !this.virtualEnergyEstimator) {
        ...
        this.log('[VIRTUAL-ENERGY] ⚡ no hardware metering → on-time estimation enabled');
      }
    } catch (e) {
    ...
    if (initError) {
      this.log('[HYBRID-PLUG] ⚠️ Initialization completed with errors');
    } else {
  ...
  async _applyManufacturerConfig() {
    ...
    this.log(`[PLUG] 🔍 Config: ${manufacturerName} / ${productId} (${driverType})`);
  ...
  _startEnergyPoll(zclNode) {
    ...
    const POLL_INTERVAL = 120 * 1000; // 120 seconds (v5.12.12: increased from 60s to reduce network flooding)
    this._energyPollTimer = this.homey.setInterval(async () => {
      ...
      try {
        ...
        if (attrs.activePower !== undefined && this.hasCapability('measure_power')) {
          await this.safeSetCapabilityValue('measure_power', Math.max(0, attrs.activePower / div.power)).catch(() => {});
        ...
        if (attrs.rmsVoltage !== undefined && this.hasCapability('measure_voltage')) {
          await this.safeSetCapabilityValue('measure_voltage', attrs.rmsVoltage / div.voltage).catch(() => {});
          this._energyCapReceived = this._energyCapReceived || {};
          this._energyCapReceived['measure_voltage'] = true;
        }
        if (attrs.rmsCurrent !== undefined && this.hasCapability('measure_current')) {
          await this.safeSetCapabilityValue('measure_current', attrs.rmsCurrent / div.current).catch(() => {});
          this._energyCapReceived = this._energyCapReceived || {};
    ...
    this.log('[PLUG] ⏱️ v5.9.7: Energy poll started (120s interval)');
  }
   ...
   */
  _detectProtocol() {
    // WHY(P214/P2521f BOTH): delegate to IntelligentProtocolDetect
    try {
      ...
      this.log(`[PROTOCOL] ${info.protocol} (${info.reason})`);
      return info;
    } catch (e) {
      this.log?.(`[PROTOCOL] detect fallback: ${e.message}`);
    }
  ...
  async _setupZCLMode(zclNode) {
    ...
    if (onOffCluster && this.hasCapability('onoff')) {
      ...
      onOffCluster.readAttributes(['onOff']).then(async data => {
        if (data?.onOff != null) {
          await this.safeSetCapabilityValue('onoff', data.onOff).catch(() => { });
        }
      }).catch(() => { });
    }
    ...
    if (meteringCluster) {
      meteringCluster.on('attr.instantaneousDemand', async (value) => {
        this.log(`[ZCL] power = ${value}W`);
        await this.safeSetCapabilityValue('measure_power', parseFloat(value)).catch(() => { });
      });
    }

    if (electricalCluster) {
      electricalCluster.on('attr.activePower', async (value) => {
        // v5.11.17: Use configurable divisor (default 10 = deciWatts → Watts)
        const watts = value / this.zclEnergyDivisors.power;
        if (watts >= 0 && watts <= 50000) {
          await this.safeSetCapabilityValue('measure_power', parseFloat(watts)).catch(() => { });
        }
      });
      electricalCluster.on('attr.rmsVoltage', async (value) => {
        // v5.11.99: FIX #137 — use configurable divisor like activePower/rmsCurrent
        const volts = value / this.zclEnergyDivisors.voltage;
        if (volts >= 50 && volts <= 300) {
          await this.safeSetCapabilityValue('measure_voltage', parseFloat(volts)).catch(() => { });
        }
      });
      electricalCluster.on('attr.rmsCurrent', async (value) => {
        // v5.11.17: Use configurable divisor (default 1000 = milliAmps → Amps)
        const amps = value / this.zclEnergyDivisors.current;
        if (amps >= 0 && amps <= 100) {
          await this.safeSetCapabilityValue('measure_current', parseFloat(amps)).catch(() => { });
        }
      });
    }

    // v5.12.25: Added missing Metering cluster energy listener
    if (meteringCluster && this.hasCapability('meter_power')) {
      const energyHandler = async (value) => {
        const raw = typeof value === 'object' ? value[0] || 0 : value;
        const kWh = raw / this.zclEnergyDivisors.energy;
        this.log(`[ZCL] energy = ${kWh}kWh (raw=${raw})`);
        await this.safeSetCapabilityValue('meter_power', parseFloat(kWh)).catch(() => { });
      };
      
      meteringCluster.on('attr.currentSummDelivered', energyHandler);
      meteringCluster.on('attr.currentSummationDelivered', energyHandler);
    }
    ...
    // becausePlugBase already has dedicated listeners with configurable
  ...
  _handleDP(dpId, rawValue) {
    ...
    } else if (mapping.divisor) {value = value / mapping.divisor;}
  ...
  static get DYNAMIC_CAPABILITIES() {
    return [
      ...
      'meter_power', 'measure_temperature', 'measure_humidity'
    ];
  }

  /**
   * safeSetCapabilityValue override to include energy capability watchdog marking
   * v5.13.6: Integrated with TuyaZigbeeDevice hardening
   */
    ...
    if (this._isPureTuyaDP && this.tuyaEF00Manager) {
      ...
      if (cluster) {
        try {
          ...
          if (!this.tuyaEF00Manager) {throw err;}
          this.log(`[PLUG] ZCL onoff failed (${err.message}), falling back to Tuya DP1`);
          await this.tuyaEF00Manager.sendDP(1, value ? 1 : 0, 'bool');
        }
      } else if (this.tuyaEF00Manager) {
        await this.tuyaEF00Manager.sendDP(1, value ? 1 : 0, 'bool');
      } else {
        const message = '[PLUG] No on/off control path available (missing ZCL OnOff cluster and Tuya DP manager)';
        this.error(message);
  ...
  // v5.8.49: Configure ZCL attribute reporting for plug onOff + power metering
  async _setupZCLReporting(zclNode) {
    ...
    if (elec?.configureReporting) {
      ...
      const minChangeRaw = this.getSetting?.('report_change_power') ?? REPORTING.power.minChange;
      // Note: activePower is usually W * 10, so minChange is usually requested in Watts * 10.
  ...
  async _readInitialPlugState(zclNode) {
    ...
    if (onOff?.readAttributes && this.hasCapability('onoff')) {
      ...
      if (a?.onOff !== undefined) {await this.safeSetCapabilityValue('onoff', a.onOff).catch(() => {});}
    }
    // v5.8.91: Read initial energy values for ZCL plugs (GH#110)
    const elec = ep.clusters.haElectricalMeasurement || ep.clusters.electricalMeasurement || ep.clusters[0x0B04];
    if (elec?.readAttributes) {
      const attrs = await elec.readAttributes(['activePower', 'rmsVoltage', 'rmsCurrent']).catch(() => null);
      if (attrs) {
        const div = this.zclEnergyDivisors;
        if (attrs.activePower !== undefined && this.hasCapability('measure_power'))
    ...
    // v5.12.25: Read initial energy (meter_power)
    const meter = ep.clusters.seMetering || ep.clusters.metering || ep.clusters[0x0702];
    if (meter?.readAttributes && this.hasCapability('meter_power')) {
      const div = this.zclEnergyDivisors;
      const mAttrs = await meter.readAttributes(['currentSummDelivered', 'currentSummationDelivered']).catch(() => null);
      if (mAttrs) {
        const rawE = mAttrs.currentSummDelivered !== undefined ? mAttrs.currentSummDelivered : mAttrs.currentSummationDelivered;
        if (rawE !== undefined) {
          const raw = typeof rawE === 'object' ? rawE[0] || 0 : rawE;
          await this.safeSetCapabilityValue('meter_power', raw / div.energy).catch(() => {});
        }
      }
    }
  }
  ...
  async setPowerOnBehavior(behavior) {
    const behaviorMap = { off: 0, on: 1, memory: 2, previous: 2 };
    ...
    if (this.hasCapability('power_on_behavior')) {
      const capVal = { off: 'off', on: 'on', memory: 'previous' }[behavior] ?? 'previous';
  ...
  async onDeleted() {
    ...
    // v9.0.418 (P92.126): Cleanup virtual energy estimator
...
UnifiedPlugBase.UnifiedPlugBase = UnifiedPlugBase;
