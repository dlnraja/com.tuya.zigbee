...
class UnifiedPlugBase extends TuyaZigbeeDevice {
  ...
  get zclEnergyDivisors() {
    ...
    const isPJ1203A = mfr.toLowerCase().includes('81yrt3');
    ...
    if (meter?.readAttributes && this.hasCapability('meter_power')) {
      ...
      if (mAttrs) {
        ...
        if (rawE !== undefined) {
          ...
          await this.setCapabilityValue('meter_power', raw / div.energy).catch(() => {});
        }
      }
    }
  }
  ...
  async setPowerOnBehavior(behavior) {
    ...
    if (this.hasCapability('power_on_behavior')) {
      ...
      await this.setCapabilityValue('power_on_behavior', capVal).catch(() => {});
    }
    return true;
  }
