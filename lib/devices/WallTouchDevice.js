 ...
 * - Tamper detection
 * - Battery vs AC auto-detection (via BaseUnifiedDevice)
...
class WallTouchDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // Init base (auto-détection power source)
  ...
  async initializeButtons() {
    ...
    for (let i = 1; i <= this.buttonCount; i++) {
      ...
      if (!this.hasCapability(capabilityId)) {
        await Promise.resolve(this.addCapability(capabilityId)).catch(err => 
      ...
      if (endpoint?.clusters?.onOff) {
        endpoint.clusters.onOff.on('attr.onOff', async (value) => {
          ...
          await Promise.resolve(this.setCapabilityValue(capabilityId, value)).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
  ...
  async onButtonChanged(buttonNumber, value) {
    ...
    if (switchType === 'momentary') {
      ...
      this.homey.setTimeout(async () => {
        ...
        await Promise.resolve(this.setCapabilityValue(capabilityId, false)).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
      }, 200);
    }
  ...
  checkButtonCombinations() {
    ...
    this.debounceTimer = this.homey.setTimeout(async () => {
      if (this._destroyed) {return;}
  ...
  async initializeTemperature() {
    ...
    endpoint.clusters.msTemperatureMeasurement.on('attr.measuredValue', async (value) => {
      ...
      if (this.hasCapability('measure_temperature')) {
        await Promise.resolve(this.setCapabilityValue('measure_temperature', parseFloat(temp))).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
      }
    });
  ...
  async initializeTamper() {
    ...
    endpoint.clusters.ssIasZone.on('zoneStatusChangeNotification', async (payload) => {
      ...
      await Promise.resolve(this.setCapabilityValue('alarm_tamper', tamper)).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
    });
