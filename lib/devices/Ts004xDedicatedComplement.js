...
async function installTs004xDedicatedComplement(device, zclNode, opts = {}) {
  ...
  // WHY(P2693): sticky TS0041–44 scene remotes — LevelControl stop→release invent
  // toggles the "other" lamp; disable via opts or profile.disableLevelControlComplement.
  const profileDisablesLc = !!(device.getDeviceProfile?.()?.disableLevelControlComplement);
  if (opts.enableLevelControl !== false && !profileDisablesLc) {
    try {
    ...
    log('LevelControl skipped (scene sticky / P2693)');
  }
