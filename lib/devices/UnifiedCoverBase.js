...
class UnifiedCoverBase extends TuyaZigbeeDevice {
  ...
  async _sendMoesZtsMotion(state, homeyPosition = null) {
    ...
    let pos = null;
    if (state === 'idle' && homeyPosition == null) {
      pos = null;
    } else if (homeyPosition != null && Number.isFinite(homeyPosition)) {
    ...
    // Position first (MCU target), then motion enum — Z2M users often set position OR state
    if (pos != null) {
      await this._sendTuyaDP(2, pos, 'value');
      await new Promise((r) => {
        // WHY(P2412): prefer Homey timer — Syntax/TITAN gate flags bare setTimeout in lib/
