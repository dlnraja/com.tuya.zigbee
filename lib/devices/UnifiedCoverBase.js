...
class UnifiedCoverBase extends TuyaZigbeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v5.12.60 (P92.128): setup crashes must never skip listener
  ...
  _detectProtocol() {
    ...
    const isTuyaDP = !isTS130F && (
      modelId.toUpperCase() === 'TS0601' || modelId.toUpperCase().startsWith('TS0601') || 
  ...
  _setupTuyaDPMode() {
    // WHY(P2363 / diag 05867379): soft-create EF00 manager (UnifiedCoverBase skips BaseUnifiedDevice)
  ...
  /**
   * WHY(P2363): Homey UI timeout when motor TX succeeds but MCU never echoes DP reports.
   */
  async _optimisticCoverUpdate({ position, state } = {}) {
    try {
      ...
      if (this._hybridProtocolManager?._protocolStats?.tuya_cluster) {
        const st = this._hybridProtocolManager._protocolStats.tuya_cluster;
