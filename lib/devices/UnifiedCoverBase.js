...
class UnifiedCoverBase extends TuyaZigbeeDevice {
  ...
  _setupTuyaDPMode() {
    ...
    if (this.tuyaEF00Manager) {
      ...
      if (typeof this.tuyaEF00Manager.start === 'function') {
        Promise.resolve(this.tuyaEF00Manager.start()).catch(() => {});
      } else if (typeof this.tuyaEF00Manager.init === 'function') {
        Promise.resolve(this.tuyaEF00Manager.init()).catch(() => {});
      }
      return;
    }
  ...
  async _sendMoesZtsMotion(state, homeyPosition = null) {
    ...
    this.log(`[COVER] P2412/P2464 Moes ZTS TX state=${state} DP1=${cmd} DP2=${pos == null ? '—' : pos} invertPos=${invertPos} invertCtrl=${this._moesControlInverted()}`);
    ...
    // Direct motion command DP1 (enum: 0=open, 1=stop, 2=close)
    await this._sendTuyaDP(1, cmd, 'enum');

    // Pair DP2 whenever we have a target (explicit slider OR open/close extremes)
    if (pos != null) {
      await new Promise((r) => {
