...
class UnifiedCoverBase extends TuyaZigbeeDevice {
  ...
  /**
   * WHY(P2393 / Salvagr #533): arm after open/close so Homey UI "release → idle"
   * does not cancel motor travel. Explicit STOP after the window still works.
   */
  _armCoverMotionGuard(state) {
    ...
    // Moes ZTS wall switch travel is longer; other EF00 covers get a shorter guard
    const moesZts = /5slehgeo|nhyj64w2|127x7wnl|upt8lzi0|i8sdouy0/.test(mfr);
    const ms = moesZts ? 5000 : 2500;
    this._coverMotionGuardUntil = Date.now() + ms;
    this._coverMotionGuardState = state;
    this.log(`[COVER] P2393 motion guard ${ms}ms after ${state}`);
  }
        ...
        try {
          ...
          if (this._isPureTuyaDP) {
            ...
            await this._optimisticCoverUpdate({
              position: value,
              state: value >= 0.95 ? 'up' : value <= 0.05 ? 'down' : 'idle',
            });
          } else {
