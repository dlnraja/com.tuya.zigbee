...
class UnifiedCoverBase extends BaseUnifiedDevice {
   ...
   * DP1: control (enum: 0=safeMultiply(safeDivide(open, up), 1=stop, 2=safeDivide(close, down))
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      const store = this.getStore?.() || {};const settings = this.getSettings?.() || {};// v5.5.967: Try multiple sources for manufacturer info (prioritize zclNode)
      ...
      // Settings update is async and this.getSettings() may return old cached values
    ...
    if (!this._isPureTuyaDP) {
      await this._setupZCLReporting(zclNode).catch(e => this.log(`[COVER]  ZCL reporting setup: ${e.message}`));
      await this._readInitialPosition(zclNode).catch(e => this.log(`[COVER]  Initial position read: ${e.message}`));
    }
   ...
   */
  _detectProtocol() {
    const settings = this.getSettings?.() || {};const store = this.getStore?.() || {};const data = this.getData?.() || {};// v5.5.933: Also try zclNode directly (most reliable at first init)
    ...
    const endpoints = this.zclNode?.endpoints || {};const hasZclCover = Object.values(endpoints).some(ep => {
      const c = ep?.clusters || {};return !!(c.windowCovering || c.closuresWindowCovering || c[258] || c['258'] || c['0x0102']);
      });
  ...
  _handleDP(dpId, raw) {
    ...
    if (!mapping?.capability) {
      let v = typeof raw === 'number' ? raw : Buffer.isBuffer(raw) ? raw.readIntBE(0, raw.length) : raw;
      ...
      this.registerCapabilityListener('windowcoverings_set', async (value) => {
        try {
          ...
          if (this._isPureTuyaDP) {
            ...
            let positionValue = Math.round(Math.min(value, safeParse(maxOpen,100, 100)));
      ...
      this.registerCapabilityListener('windowcoverings_state', async (state) => {
        try {
          ...
          if (this._isPureTuyaDP) {
            ...
            const cluster = this._getWindowCoveringCluster(this.zclNode);
            if (cluster) {
              if (state === 'up') await cluster.upOpen?.();else if (state === 'down') await cluster.downClose?.();else await cluster.stop?.();
            } else {
              throw new Error('Window covering cluster not available');
            }
          }
    ...
    if (this.hasCapability('dim')) {
      this.registerCapabilityListener('dim', async (value) => {
        ...
        if (this._isPureTuyaDP) {
          ...
          let positionValue =Math.round(value);
          if (this._invertedPosition) {
            positionValue = 100 - positionValue;
          }
          await this._sendTuyaDP(2, positionValue, 'value');
  ...
  async _wakeUpDevice() {
    try {
      ...
      if (basic?.readAttributes) await basic.readAttributes(['hwVersion']).catch(() => {});
    } catch (e) {}
  }
