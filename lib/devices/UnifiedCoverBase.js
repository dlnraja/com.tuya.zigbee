...
class UnifiedCoverBase extends TuyaZigbeeDevice {
  ...
  async _sendMoesZtsMotion(state, homeyPosition = null) {
    ...
    this.log(`[COVER] P2412 Moes ZTS TX state=${state} DP1=${cmd} DP2=${pos == null ? '—' : pos} invertPos=${invertPos} invertCtrl=${this._moesControlInverted()}`);
    ...
    // Only write position DP2 when a numerical target was explicitly commanded
    if (pos != null) {
      await new Promise((r) => {
