...
} catch (e) { /* Cluster may already be registered */ }
 ...
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
class UnifiedLightBase extends TuyaZigbeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v5.12.60 (P92.128): a crash here must NEVER skip listener registration
    // below — a partially initialized device must still be controllable.
    await this._registerHomeyCapabilities(zclNode).catch(e =>
      this.log(`[LIGHT] ⚠️ Homey capabilities registration: ${e.message}`)
    );

    try {
      this._registerCapabilityListeners();
    } catch (e) {
      this.error(`[LIGHT] ❌ Capability listeners: ${e.message}`);
    }
  ...
  async _setupZCLMode(zclNode) {
    ...
    if (onOffCluster) {
      onOffCluster.on('attr.onOff', (value) => {
        ...
        this.safeSetCapabilityValue('onoff', value);
      });
    ...
    if (levelCluster) {
      levelCluster.on('attr.currentLevel', (value) => {
        ...
        this.safeSetCapabilityValue('dim', dim);
      });
      this.log('[ZCL] ✅ levelControl listener');
    }

    // Custom Tuya TS110E Level Control listener
    const tuyaLevelCluster = endpoint?.clusters?.tuyaLevelCtrl;
    if (tuyaLevelCluster) {
      tuyaLevelCluster.on('attr.tuyaBrightness', (parsedValue) => {
        this.log(`[ZCL] Received Tuya brightness update: ${parsedValue}`);
        const minB = 10;
        const maxB = 1000;
        const homeyDimValue = Math.max(0, Math.min(1, ((parsedValue - minB) * (1 - 0)) / (maxB - minB) + 0));
        this.safeSetCapabilityValue('dim', homeyDimValue);
      });
      this.log('[ZCL] ✅ tuyaLevelCtrl listener active');
    }
    ...
    if (colorCluster) {
      colorCluster.on('attr.colorTemperatureMireds', (value) => {
        ...
        this.safeSetCapabilityValue('light_temperature', temp);
      });
      ...
      colorCluster.on('attr.currentHue', (value) => {
        ...
        this.safeSetCapabilityValue('light_hue', hue);
      });
      ...
      colorCluster.on('attr.currentSaturation', (value) => {
        ...
        this.safeSetCapabilityValue('light_saturation', sat);
      });
  ...
  _registerCapabilityListeners() {
    ...
    // v9.0.40: Register multiple capabilities together for debounced updates
    // This prevents rapid sequential commands when multiple capabilities change together
    const multiCaps = [];
    ...
    if (this.hasCapability('light_hue')) {multiCaps.push('light_hue');}
    if (this.hasCapability('light_saturation')) {multiCaps.push('light_saturation');}

    if (multiCaps.length >= 2) {
      this.registerMultipleCapabilities(multiCaps, async (valueObj, optsObj) => {
        this.log(`[LIGHT] Multi-capability update: ${JSON.stringify(valueObj)}`);

        // Mark app command for all changed capabilities
        for (const [cap, value] of Object.entries(valueObj)) {
        ...
        if (valueObj.onoff !== undefined && valueObj.dim !== undefined) {
          // If turning on with a specific dim level, send dim first
          if (valueObj.onoff && valueObj.dim > 0) {
            await this._setDim(valueObj.dim);
        ...
        } else if (valueObj.onoff !== undefined) {
          await this._setOnOff(valueObj.onoff);
        } else if (valueObj.dim !== undefined) {
          await this._setDim(valueObj.dim);
        }

        // Handle color changes
        if (valueObj.light_hue !== undefined || valueObj.light_saturation !== undefined) {
          const hue = valueObj.light_hue ?? this.getCapabilityValue('light_hue') ?? 0;
          const sat = valueObj.light_saturation ?? this.getCapabilityValue('light_saturation') ?? 1;
          await this._setColor(hue, sat);
        }

        // Handle color temperature
        if (valueObj.light_temperature !== undefined) {
          await this._setColorTemperature(valueObj.light_temperature);
        }
      });
    }
  }
  ...
  async _setDim(value) {
    ...
    if (this._isPureTuyaDP) {
      ...
      if (value > 0 && this.getCapabilityValue('onoff') === false) {
        ...
        this.safeSetCapabilityValue('onoff', false);
        return;
      }
      ...
      const endpoint = this.zclNode?.endpoints?.[1];
      const model = (this.getSettings?.()?.zb_model_id || '').toUpperCase();

      // Custom TS110E proprietary Level Control
      if (endpoint && endpoint.clusters.tuyaLevelCtrl && model === 'TS110E') {
        const minB = 10;
        const maxB = 1000;
        ...
        if (value > 0 && this.getCapabilityValue('onoff') === false) {
          const onOffCluster = endpoint.clusters.onOff || endpoint.clusters.genOnOff;
          if (onOffCluster) {await onOffCluster.setOn().catch(() => {});}
          this.safeSetCapabilityValue('onoff', true);
        }
        ...
        this.log(`[ZCL] ts110e dimmer set to level: ${tuyaLevel}`);
        return;
      }
  ...
  async _readInitialLightState(zclNode) {
    ...
    if (level?.readAttributes && this.hasCapability('dim')) {
      ...
      if (a?.currentLevel !== undefined) {await this.safeSetCapabilityValue('dim', Math.max(0, Math.min(1, a.currentLevel / 254))).catch(() => {});}
    }

    const tuyaLevel = ep.clusters.tuyaLevelCtrl;
    const model = (this.getSettings?.()?.zb_model_id || '').toUpperCase();
    if (tuyaLevel?.readAttributes && this.hasCapability('dim') && model === 'TS110E') {
      const a = await tuyaLevel.readAttributes(['tuyaBrightness']).catch(() => null);
      if (a?.tuyaBrightness !== undefined) {
        const minB = 10;
        const maxB = 1000;
        const homeyDimValue = Math.max(0, Math.min(1, ((a.tuyaBrightness - minB) * (1 - 0)) / (maxB - minB) + 0));
        await this.safeSetCapabilityValue('dim', homeyDimValue).catch(() => {});
      }
    }
  }
...
UnifiedLightBase.UnifiedLightBase = UnifiedLightBase;
