...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async _ensureBatteryCapabilityUi() {
    ...
    // WHY(P2550): recycle once so History/Insights tab gets a real log after tip.
    try {
      const recycled = typeof this.getStoreValue === 'function'
        ? this.getStoreValue('p2550_batt_insights_recycle')
        : null;
      if (!recycled
        ...
        if (Number.isFinite(last) && last > 0) {
          if (typeof this.safeSetCapabilityValue === 'function') {
            await this.safeSetCapabilityValue('measure_battery', last, { skipThrottle: true }).catch(() => {});
          } else if (typeof this.setCapabilityValue === 'function') {
            await this.setCapabilityValue('measure_battery', last).catch(() => {});
          }
        }
        this.log('[BUTTON-BATTERY] P2550 recycled measure_battery for Insights/History');
      }
    } catch (_e2) { /* soft */ }
    return true;
  }
