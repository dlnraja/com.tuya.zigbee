...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async triggerButtonPress(buttonNumber, pressType = 'single', count = 1, options = {}) {
    ...
    if (snappy) {
      // WHY(P2707 / Bastien black TS0042/43 crash): full tryOnce cascade (matrix +
      // heuristics + legacy invent) still OOMs Homey when hybrid multi-path fires.
      // Contre quoi: press → dozens of getDeviceTriggerCard/trigger → app crash.
      // Snappy = ONE primary Flow + optional universal button_pressed only.
      // WHY(P2721 Bastien IG): never await heuristic/card chain — Homey Flow queue
      // made press2 wait ~7s while press1's card storm + powerCfg finished.
      // Fire-and-forget both; return immediately so next press can enqueue.
      try {
        const driverId = this.driver?.id || 'button_wireless';
        const gangCount = Math.max(1, Number(this.buttonCount || this.gangCount || 1) || 1);
        const btnNum = Number(button) || 1;
        ...
        const tokens = { button: btnStr, count, action: heurPress, gang: btnNum };
        const state = { button: btnStr, count };
        void triggerFlowCardHeuristic(this, candidates, tokens, 'trigger', state).catch(() => {});
