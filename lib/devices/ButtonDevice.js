  ...
  async triggerButtonPress(button, type = 'single', count = 1, options = {}) {
    ...
    try {
      ...
      if (type === 'single') {
        ...
        // Try 1: Generic button_pressed with button token (Homey built-in expects number)
        // v5.8.76: Force Number type — parseInt(x)||x could return string when NaN
        try {
          ...
          await this.homey.flow.getDeviceTriggerCard('button_pressed')
            .trigger(this, { button: btnNum }, { button: btnNum });
        ...
        for (const fid of [`${driverId}_button_pressed`, `${driverId}_button_${button}_pressed`]) {
          try { await this.homey.flow.getDeviceTriggerCard(fid).trigger(this, { button: button.toString() }, { button: button.toString() }); this.log(`[FLOW-TRIGGER] ✅ ${fid}`); } catch (e) { /* skip */ }
        }
        ...
        // v5.8.59: scene_switch_* fallback (no _button_Xgang infix)
        for (const fid of [`${driverId}_button_double_press`, `${driverId}_button_${button}_double`]) {
          try { await this.homey.flow.getDeviceTriggerCard(fid).trigger(this, { button: button.toString() }, { button: button.toString() }); this.log(`[FLOW-TRIGGER] ✅ ${fid}`); } catch (e) { /* skip */ }
        }
        ...
        try {
          ...
          const btnNum = parseInt(button) || button;
        ...
        // v5.8.59: scene_switch_* fallback (no _button_Xgang infix)
        for (const fid of [`${driverId}_button_long_press`, `${driverId}_button_${button}_long`]) {
          try { await this.homey.flow.getDeviceTriggerCard(fid).trigger(this, { button: button.toString() }, { button: button.toString() }); this.log(`[FLOW-TRIGGER] ✅ ${fid}`); } catch (e) { /* skip */ }
        }
        ...
        try {
          ...
          await this.homey.flow.getDeviceTriggerCard('button_multi_press')
            .trigger(this, { button: btnNum, count }, { button: btnNum, count });
        ...
        // v5.9.3: Fallback for drivers without gangCount infix (button_wireless, scene_switch_*)
        for (const fid of [`${driverId}_button_release`, `${driverId}_button_${button}_release`]) {
          try { await this.homey.flow.getDeviceTriggerCard(fid).trigger(this, { button: button.toString() }, { button: button.toString() }); this.log(`[FLOW-TRIGGER] ✅ ${fid}`); } catch (e) { /* skip */ }
        }
      }
