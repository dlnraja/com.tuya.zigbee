...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      if (profile?.noEf00 || profile?.protocol === 'zcl_0xfd_e000' || multi) {
        minInterval = 120;
      }
    } catch (_e) { /* soft */ }
  ...
  async triggerButtonPress(buttonNumber, pressType = 'single', count = 1, options = {}) {
    ...
    // v5.5.430 / v10.6.0: Reset button capability to false after trigger (fixes "stays true" issue and gives real-time visual pulse in Homey UI)
    const capsToPulse = [];
    ...
    for (const cap of capsToPulse) {
      await this.safeSetCapabilityValue(cap, true).catch(() => {});
      safeSetTimeout(this, async () => {
        if (this._destroyed) {return;}
        await this.safeSetCapabilityValue(cap, false).catch(() => {});
      }, 500);
    }
