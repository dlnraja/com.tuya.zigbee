    ...
    try {
      ...
      try {
        ...
        for (const id of candidates) {
          ...
          await this._tryCard(id, tokens, state);
        }
  ...
  async _tryCard(cardId, tokens = {}, state = {}) {
    try {
      ...
      if (declared.size && !resolved) {
        ...
        const driverScoped = driverId
          && String(cardId).toLowerCase().startsWith(`${driverId.toLowerCase()}_`);
        // WHY(P2381): driver-scoped compose cards may be missing from declared set
        // if getDrivers() is empty early — still probe getDeviceTriggerCard.
        if (!appLevel.test(cardId) && !driverScoped) {return false;}
        resolved = cardId;
      }
      // WHY (P2247): only call Homey getters for resolved/declared IDs — no speculative spray
      // WHY(P2381): driver-scoped miss → null gate so safeGetFlowCard can hit live driver card
