...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async _tryCard(id, tok = {}, st = {}) {
    try {
      const card = await (() => { try { return this.homey.flow.getDeviceTriggerCard(id); } catch (e) { this.error('[FLOW-SAFE] Failed to load card:', e.message); return null; } })();
      if (card) {
      ...
      // Just fail silently, card might not exist for this driver
    }
    return false;
  }
