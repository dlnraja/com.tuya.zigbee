...
/**
 * ButtonDevice - v5.5.492 MANUFACTURER-SPECIFIC PHYSICAL BUTTON FIX
 *
...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v5.5.522: Anti-auto-trigger protection (forum fix)
    this._buttonTriggerProtection = {
      lastTrigger: 0,
      minInterval: 2000, // Minimum 2 seconds between triggers
  ...
  async triggerButtonPress(button, type = 'single', count = 1) {
    ...
    // v5.5.522: Anti-auto-trigger protection (forum fix Cam)
    // Detect and prevent hourly x:30 pattern triggers
    if (this._buttonTriggerProtection) {
      ...
      // Check for suspicious hourly x:30 pattern
      if (currentMinute === 30 && timeSinceLastTrigger > (50 * 60 * 1000) && timeSinceLastTrigger < (70 * 60 * 1000)) {
        this._buttonTriggerProtection.hourlyPattern = true;
        this.log(`[ANTI-TRIGGER] 🚫 BLOCKED: Suspicious x:30 hourly pattern detected (${timeSinceLastTrigger}ms since last)`);
        return;
      }
      
      // Minimum interval protection
      if (timeSinceLastTrigger < this._buttonTriggerProtection.minInterval) {
        this.log(`[ANTI-TRIGGER] 🚫 BLOCKED: Too rapid trigger (${timeSinceLastTrigger}ms < ${this._buttonTriggerProtection.minInterval}ms)`);
        return;
      }
