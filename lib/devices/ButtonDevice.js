    ...
    for (let ep = 1; ep <= endpoints; ep++) {
      try {
        ...
        if (onOffCluster) {
          ...
          // v5.5.261: CRITICAL - Listen for onOff ATTRIBUTE changes
          // Moes scene buttons send onOff=false as toggle events!
          // Track last value to detect ANY change (toggle = button press)
          let lastOnOffValue = null;
          let lastOnOffTime = 0;

          onOffCluster.on('attr.onOff', async (value) => {
            const now = Date.now();
            this.log(`[ONOFF-ATTR] Button ${ep} onOff attribute changed: ${value}`);

            // v5.5.261: Debounce rapid duplicate events (within 100ms)
            if (now - lastOnOffTime < 100 && value === lastOnOffValue) {
              this.log(`[ONOFF-ATTR] Debounced duplicate event`);
              return;
            }

            // v5.5.261: MOES FIX - Detect ANY attribute change as button press
            // Some devices send true, some send false, some toggle
            // The key is that ANY change = button was pressed
            const isNewEvent = lastOnOffValue === null || value !== lastOnOffValue || (now - lastOnOffTime > 500);

            if (isNewEvent) {
              this.log(`🔘 Button ${ep} PRESSED (onOff attr=${value})`);
