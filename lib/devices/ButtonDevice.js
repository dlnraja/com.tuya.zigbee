...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v9.0.50 FIX batterie '?' : fallback robuste timer (this.homey?.setTimeout || setTimeout)
    const _batTimer = this.homey?.setTimeout || setTimeout;
    _batTimer(async () => {
      try {
    ...
    // v9.0.50 FIX CRITICAL (diag "Missing Capability Listener button.1/2/3") :
    // Le TITAN V5 a supprimé _registerButtonCapabilityListeners(). Sans cela,
    // Homey ne trouve pas de listener pour button.X quand l'utilisateur clique
    // dans l'app UI ou utilise un flow → "Missing Capability Listener Error".
    // Restauration depuis v5.11.205-stable (qui marchait).
    await this._registerButtonCapabilityListeners().catch((err) => {
      this.error('[BUTTON] registerCapabilityListeners failed:', err.message);
    });
   ...
   * v5.5.224: Register capability listeners for button capabilities
   * Buttons are "virtual" - they trigger flows but don't have settable values
   * v5.7.14: Added bidirectional deduplication (virtual vs physical)
   */
  async _registerButtonCapabilityListeners() {
        ...
        if (powerCluster) {
          ...
          if (typeof powerCluster.readAttributes === 'function') {
            const data = await Promise.race([
              ...
              new Promise((_, reject) => this.homey.setTimeout(() => { if (this._destroyed) return; reject(new Error('Timeout')); }, 3000))
            ]).catch(e => {
            ...
            if (data?.batteryPercentageRemaining !== undefined &&
              ...
              if (this.hasCapability('measure_battery')) {
                await this.safeSetCapabilityValue('measure_battery', parseFloat(battery)).catch((err) => {
  ...
  async triggerButtonPress(buttonNumber, pressType = 'single', count = 1, options = {}) {
    ...
    if (typeof this._triggerPhysicalFlow === 'function') {
      this._triggerPhysicalFlow(button, type);
    }
    ...
    try {
      ...
      if (type === 'single') {
        ...
        if (hasNativeTriple) {
          ...
          state.timer = (this.homey?.setTimeout ?? setTimeout)(() => {
            if (this._destroyed) return;
      ...
      if (type === 'long') {
        ...
        if (hasNativeRelease) {
          ...
          this._holdReleaseTimers[releaseKey] = (this.homey?.setTimeout ?? setTimeout)(() => {
            if (this._destroyed) return;
