...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  _registerSceneModeAttributeListener(zclNode) {
    if (this._sceneModeAttrListenerRegistered) {return;}
    this._sceneModeAttrListenerRegistered = true;
    const MODE_ATTRIBUTE = 0x8004;
    for (const [epKey, ep] of Object.entries(zclNode?.endpoints || {})) {
      const onOffCluster = ep?.clusters?.onOff || ep?.clusters?.genOnOff || ep?.clusters?.[6];
      if (!onOffCluster || typeof onOffCluster.on !== 'function') {continue;}
      const handler = (attrId, value) => {
        try {
          ...
          const v = typeof attrId === 'object' && attrId !== null && value === undefined
            ? (attrId.value) : value;
          if (Number(id) !== MODE_ATTRIBUTE) {return;}
          const mode = Number(v) === 1 ? 'scene' : 'dimmer';
          this.log(`[BUTTON-MODE] 🔄 Manual mode toggle detected on EP${epKey}: ${mode.toUpperCase()}`);
          this.setStoreValue('button_mode', mode).catch(() => {});
          this.sceneMode = mode;
          this.setSettings?.({ button_mode: mode }).catch(() => {});
        } catch (_e) {}
      };
      try {
        onOffCluster.on('attr', handler);
        // Some SDK versions emit per-attribute events (attr.32772)
        onOffCluster.on(`attr.${MODE_ATTRIBUTE}`, (value) => handler(MODE_ATTRIBUTE, value));
      } catch (_e) {}
    }
  }
  ...
  async setButtonMode(mode) {
    ...
    // P28: Try all endpoints with onOff cluster (some devices have it on EP2/3/4)
    const endpoints = zclNode.endpoints || {};
    let lastErr = null;

    for (const [epKey, ep] of Object.entries(endpoints)) {
      const epId = Number(epKey) || 1;
      if (!ep || !ep.clusters) {continue;}
      const onOffCluster = ep.clusters.onOff || ep.clusters.genOnOff || ep.clusters[6];
      ...
      try {
        if (typeof onOffCluster.writeAttributes === 'function') {
          await onOffCluster.writeAttributes({ [MODE_ATTRIBUTE]: numMode });
        } else if (typeof onOffCluster.write === 'function') {
          await onOffCluster.write({ attributeId: MODE_ATTRIBUTE, dataType: 0x30, value: numMode });
        }
        this.log(`[BUTTON-MODE] ✓ Set to mode=${numMode} (${numMode === 1 ? 'scene' : 'dimmer'}) on EP${epId}`);

        // P28: Update both setting and store
        await this.setStoreValue('button_mode', numMode === 1 ? 'scene' : 'dimmer').catch(() => {});
        await this.setStoreValue('scene_mode_switch_failed', false).catch(() => {});
        try {
          await this.setSettings?.({ button_mode: numMode === 1 ? 'scene' : 'dimmer' });
        } catch (_e) { /* setting may be locked */ }
        return true;
      } catch (err) {
        lastErr = err;
        this.log(`[BUTTON-MODE] EP${epId} write failed: ${err.message}`);
      }
    }

    this.log(`[BUTTON-MODE] ✗ Failed to set mode=${numMode} on any endpoint: ${lastErr?.message || 'no onOff cluster'}`);
    // v10.4.0 (SCENE_MODE_RESEARCH #3): persist the failure for diagnostics —
    // users can be told to toggle manually (hold buttons 2+3 ~5s).
  ...
  async _universalSceneModeSwitch(zclNode) {
    ...
    // P28: Read the user setting. Default = 'scene' (per P28 user feedback).
    const buttonModeSetting = this.getSetting?.('button_mode') || 'scene';

    // P28: If user explicitly chose 'dimmer', don't override
    if (buttonModeSetting === 'dimmer') {
      this.log('[BUTTON-MODE] User chose dimmer mode — skipping scene mode switch');
      return;
    }
    ...
    try {
      ...
      for (let attempt = 0; attempt < retryDelays.length; attempt++) {
        if (attempt > 0) {
          await new Promise(r => safeSetTimeout(this, r, retryDelays[attempt]));
        }
      ...
      for (const { endpointId, cluster: powerCluster } of powerClusters) {
        ...
        if (typeof powerCluster.readAttributes === 'function') {
          ...
          if (!batteryRead && data?.batteryVoltage !== undefined && data.batteryVoltage > 0) {
            // Dividing by 10 assumed the ZCL 100mV unit; remotes that report
            // plain millivolts came out ten times too high and clamped to 100%.
            const context = this._getButtonBatteryProfile();
            const battery = normalizeZclBatteryVoltagePercent(data.batteryVoltage, { batteryType: context.chemistry });
            if (battery !== null) {
              batteryRead = await this._setButtonBattery(battery, `ZCL voltage EP${endpointId} (raw ${data.batteryVoltage})`);
            }
          }
        }
  ...
  async triggerButtonPress(buttonNumber, pressType = 'single', count = 1, options = {}) {
    ...
    try {
      ...
      // v10.5.0 (Hue RWL022 / Aqara pattern): fire the dropdown MATRIX card —
      // one card, state {button, action}; users pick button + press type from
      // dropdowns instead of hunting through N separate cards.
      await this._tryCard('button_matrix',
        { button: String(button), action: type },
        { button: String(button), action: type });
  ...
  async _tryCard(cardId, tokens = {}, state = {}) {
    try {
      ...
      const { expandIdCaseVariants, findDeclaredCI, collectDeclaredFlowIds } = require('../flow/FlowCardHeuristics');
      ...
      const tryIds = resolved ? [resolved, ...variants] : variants;
      for (const id of tryIds) {
        let card = null;
        try { card = this.homey.flow.getDeviceTriggerCard(id); } catch (_) { card = null; }
        if (typeof card?.trigger !== 'function') {
          try { card = this.homey.flow.getTriggerCard(id); } catch (_) { card = null; }
        }
        if (typeof card?.trigger !== 'function') {continue;}
