...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onEndDeviceAnnounce() {
          ...
          if (typeof endpoint.bind === 'function') {
            ...
            endpoint.bind(cid).then(() => {
              ...
              if (!err.message?.includes('timeout')) {
                this.log(`[BIND] ℹ️ Cluster 0x${cid.toString(16)} bind EP${ep}: ${err.message}`);
              }
            });
          }
        } catch (e) {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        this.log(`[BUTTON] ⚠️ Scene mode recovery attempt ${i+1} failed: ${err.message}`);
      }
    }
  }
  ...
  async _universalSceneModeSwitch(zclNode) {
    ...
    if (hasE000) {
      this.log('[BUTTON-MODE] ℹ️ E000 cluster detected — will still attempt scene mode switch just in case');
    ...
    try {
      ...
      this.registerCapabilityListener('alarm_contact', async (value) => {
        ...
        try {
          ...
          await (() => {
            try {
              return this.homey.flow.getTriggerCard(cardId);
            } catch (e) {
          ...
          onOffCluster.on('attr.onOff', async (value) => {
            ...
            if (timeSinceLastEvent > 86400000) { // > 24 hours = impossible, must be uninitialized
              ...
              if (timeSinceInit > 30000) {
                this.log(`[ONOFF-ATTR] 🔘 Wake-up press EP${currentEp} (slept ${Math.round(timeSinceInit/1, 1000))}s)`);
                sharedState.globalInitialized = true;
                await this.triggerButtonPress(currentEp, 'single');
              } else {
                this.log(`[ONOFF-ATTR] ⏭️ Ignored: uninitialized state (sinceEvent=${Math.round(timeSinceLastEvent/1, 1000))}s)`);
              }
              return;
            }
            ...
            if (epState.lastValue === null) {
              if (!sharedState.globalInitialized) {
                ...
                if (timeSinceInit > 30000) {
                  this.log(`[ONOFF-ATTR] 🔘 First wake-up press EP${currentEp} (${Math.round(timeSinceInit/1, 1000))}s after init)`);
                  await this.triggerButtonPress(currentEp, 'single');
                } else {
                ...
                sharedState.endpoints[currentEp] = { lastValue: value, lastTime: now };
                if (timeSinceInit > 30000) {
                  this.log(`[ONOFF-ATTR] 🔘 Late wake-up press EP${currentEp} (${Math.round(timeSinceInit/1, 1000))}s after init)`);
                  await this.triggerButtonPress(currentEp, 'single');
                } else {
                  this.log(`[ONOFF-ATTR] ⏭️ Ignored: late endpoint init on EP${currentEp} (${Math.round(timeSinceInit/1, 1000))}s after global init)`);
                }
                return;
              }
            }
  ...
  async _tryCard(id, tok = {}, st = {}) {
    try {
      ...
      if (id.includes(driverId)) {
        ...
        for (const legacyId of legacyDriverIds) {
          ...
          const legacyCard = (app && typeof app._safeGetTriggerCard === 'function')
            ? app._safeGetTriggerCard(legacyFlowId)
            : null;
      ...
      this.log(`[FLOW] ⚠️ _tryCard failed for ${id}: ${e.message}`);
    }
    return false;
  }
  ...
  async triggerButtonPress(button, type = 'single', count = 1, options = {}) {
    ...
    if (this._buttonTriggerProtection) {
      ...
      if (currentMinute === 30 && timeSinceLastTrigger >safeMultiply((50, 60) * 1000) && timeSinceLastTrigger <safeMultiply((70, 60) * 1000)) {
      ...
      if (ep1) {
        ...
        if (powerCluster) {
          ...
          if (!batteryRead && powerCluster.batteryPercentageRemaining !== undefined) {
            ...
            if (raw !== 255) {
              const battery = Math.round(safeParse(raw, 2));
  ...
  async _handleTuyaBatteryDP(dp, value) {
    ...
    if (dp === 4) {
      battery = value > 100 ? Math.round(safeParse(value, 2)) : value;
    }
