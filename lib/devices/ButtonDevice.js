...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    const manufacturerName = settings.zb_manufacturer_name || store.manufacturerName || data.manufacturerName || '';
    const productId = settings.zb_model_id || store.modelId || data.productId || '';
  ...
  async triggerButtonPress(button, type = 'single', count = 1) {
    ...
    try {
      ...
      // Track rapid single presses to detect triple press for devices that don't report it natively
      if (type === 'single') {
        ...
        state.timer = setTimeout(() => {
          if (state.count >= 3) {
            this.log(`[CLICK-PATTERN] 🔥 Triple click detected on button ${button}!`);
            // Trigger triple press flow card
      ...
      if (type === 'long') {
        ...
        this._holdReleaseTimers[releaseKey] = setTimeout(() => {
          this.log(`[HOLD-RELEASE] 🔓 Button ${button} released (after hold timeout)`);
