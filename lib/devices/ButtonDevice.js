...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  /**
   * v5.11.219 BACKPORT from v5.11.205-stable : _updateBattery
   * Méthode unifiée de mise à jour batterie (capability + store).
   * Sans cette méthode, la batterie reste '?' (null) car aucune écriture unifiée.
   */
  async _updateBattery(percentage) {
    ...
    // v5.11.219 FIX : _safeSetCapability peut ne pas exister dans la chaîne
  ...
  /**
   * v5.11.219 : Fallback _safeSetCapability si CapabilityManagerMixin absent.
   */
  async _safeSetCapabilityFallback(capability, value) {
    if (!capability || value === undefined || value === null) return false;
    if (this.mainsPowered && (capability === 'measure_battery' || capability === 'alarm_battery')) return false;
    try {
  ...
  /**
   * v5.11.219 BACKPORT from v5.11.205-stable : onEndDeviceAnnounce
   * Handler crucial pour boutons sleepy (TS0041-44). Au wake/rejoin :
   *   1. Re-bind les clusters (scenes/onOff/levelCtrl) — nécessaire car les
   *      bindings peuvent se perdre après un reboot mesh ou un changement de pan.
   *   2. Lit la batterie immédiatement (le device est éveillé brièvement).
   * Sans cette méthode, la batterie n'est JAMAIS lue au wake → '?' permanent.
   */
  async onEndDeviceAnnounce() {
    this.log('[BUTTON] Device announce (wake/rejoin) — re-bind + battery read');
    ...
    for (let ep = 1; ep <= endpoints; ep++) {
      ...
      // Re-bind clusters critiques pour boutons (scenes, onOff, multistate, levelCtrl)
      for (const cid of [5, 6, 18, 8]) {
        try {
    ...
    // v5.11.201 : lecture batterie au wake (sleepy devices)
  ...
  _isHybridButtonContactDevice(manufacturerName, productId) {
    ...
    constPatterns = ['SOS', 'Emergency', 'Door', 'Window', 'Contact'];
  ...
  async _readBatteryWhileAwake() {
    ...
    if (!batteryRead) {
      ...
      if (storedBattery !== null && storedBattery !== undefined) {
        ...
        if (this.hasCapability('measure_battery')) {
          await this.safeSetCapabilityValue('measure_battery', parseFloat(storedBattery)).catch(() => { });
        }
        batteryRead = true;
      }
    }
  ...
  async onUninit() {
    ...
    if (this._sceneModeRecoveryTimer) {
      this.log('[BUTTON] Clearing scene mode recovery interval');
      clearInterval(this._sceneModeRecoveryTimer);
    ...
    // Call super onUninit if it exists
