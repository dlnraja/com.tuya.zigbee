...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      if (profile?.snappyRelayFlow || (Number(profile?.debounceMs) || 0) > 0 && Number(profile.debounceMs) <= 100) {
        minInterval = 50;
  ...
  async triggerButtonPress(buttonNumber, pressType = 'single', count = 1, options = {}) {
    ...
    const pulseMs = (typeof this.getDeviceProfile === 'function' && this.getDeviceProfile()?.snappyRelayFlow) ? 120 : 500;
