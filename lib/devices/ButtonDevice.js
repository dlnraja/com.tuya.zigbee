...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  /**
   * v5.13.3: Robust capability setter (Hotfix for possible inheritance gaps)
   * Ensures this method exists locally AND attempts to use parent version.
   */
  async _safeSetCapability(capability, value, options = {}) {
    try {
      if (!this || this._deleted || !this.homey) return false;
      
      // 1. ATTEMPT SUPER - parent implementation has rich logic (v5.13.1+)
      try {
        if (super._safeSetCapability) {
          return await super._safeSetCapability(capability, value, options);
        }
      } catch (e) {
        // Fallback to local if super fails or doesn't have it yet
      }

      // Fallback: implementation directly in ButtonDevice to avoid crash
      if (!this.hasCapability(capability)) return false;

      const previousValue = this.getCapabilityValue(capability);
      if (value === previousValue && !options.skipThrottle) return true;
      ...
      this.error(`[SAFE-SET] ❌ Failure on ${capability}:`, err.message);
      return false;
    }
  }
