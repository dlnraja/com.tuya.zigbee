...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // v5.5.289: Remove alarm_contact if incorrectly added (buttons are NOT contact sensors!)
    // Fixes: DATA-RECOVERY trying to recover alarm_contact on button devices
    if (this.hasCapability('alarm_contact')) {
      this.log('[BUTTON] ⚠️ Removing incorrect alarm_contact capability (buttons are NOT contact sensors)');
