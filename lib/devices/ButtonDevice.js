...
const BUTTON_BATTERY_CORE_PERCENT_DPS = [4, 15, 101];
const BUTTON_BATTERY_EXTENDED_PERCENT_DPS = [10, 21, 100, 102, 104, 105, 121];
const BUTTON_BATTERY_PERCENT_DPS = [
  ...BUTTON_BATTERY_CORE_PERCENT_DPS,
  ...BUTTON_BATTERY_EXTENDED_PERCENT_DPS,
];
const BUTTON_BATTERY_STATE_DPS = [3, 14];
const BUTTON_BATTERY_VOLTAGE_DPS = [33, 35, 247];
const BUTTON_BATTERY_DPS = [
...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  _getButtonBatteryDPs() {
    return BUTTON_BATTERY_DPS;
  }
  ...
  _normalizeButtonDpBattery(dp, value) {
    ...
    if (BUTTON_BATTERY_STATE_DPS.includes(Number(dp)) && raw >= 0 && raw <= 2) {
      return raw === 0 ? 10 : raw === 1 ? 50 : 100;
    }

    if (!BUTTON_BATTERY_PERCENT_DPS.includes(Number(dp))) {return null;}
    if (BUTTON_BATTERY_EXTENDED_PERCENT_DPS.includes(Number(dp)) && raw <= 2) {return null;}
    if (raw === 255 || raw === 0xFFFF || raw < 0 || raw > 200) {return null;}
    return raw > 100 ? Math.round(raw / 2) : Math.round(raw);
  }

  _isButtonBatteryDP(dp) {
    return BUTTON_BATTERY_DPS.includes(Number(dp));
  }
  ...
  _normalizeButtonBatteryVoltage(rawVoltage) {
    ...
    if (raw > 1000 && raw <= 6000) {return raw / 1000;}
    if (raw > 100 && raw <= 600) {return raw / 100;}
    if (raw > 20 && raw <= 60) {return raw / 10;}
    if (raw >= 1.5 && raw <= 6) {return raw;}
    return null;
  }

  _buttonBatteryPercentFromVoltage(voltage) {
    const normalizedVoltage = this._normalizeButtonBatteryVoltage(voltage);
    if (normalizedVoltage === null) {return null;}

    const context = this._getButtonBatteryProfile();
    const battery = UnifiedBatteryHandler?.calculateFromVoltage
      ? UnifiedBatteryHandler.calculateFromVoltage(normalizedVoltage, context.chemistry)
      : this._voltageToPercentCurve(Math.round(normalizedVoltage * 1000));

    return Number.isFinite(Number(battery)) ? Math.max(0, Math.min(100, Math.round(Number(battery)))) : null;
  }

  _getButtonTuyaClusters() {
    ...
    for (const endpointId of orderedIds) {
      ...
      const cluster =
        ...
        endpoint?.clusters?.manuSpecificTuyaDimmer ||
  ...
  async _storeButtonBatteryVoltage(rawVoltage) {
    const voltage = this._normalizeButtonBatteryVoltage(rawVoltage);
    if (voltage === null) {return null;}

    await this.setStoreValue('battery_voltage', voltage).catch(() => {});
    await this.setStoreValue('batteryVoltage', rawVoltage).catch(() => {});
    return voltage;
  }
  ...
  _normalizeButtonStoredBattery(value) {
    const raw = this._coerceButtonBatteryDPValue(value);
  ...
  async _restoreButtonBatteryFromStore() {
    const percentKeys = [
    ...
    for (const key of percentKeys) {
      ...
      const percent = this._normalizeButtonStoredBattery(stored);
      if (percent !== null) {
        return this._setButtonBattery(percent, `stored ${key}`);
      }
    }
    ...
    for (const key of voltageKeys) {
      const stored = this.getStoreValue?.(key);
      const voltage = this._normalizeButtonBatteryVoltage(stored);
      const percent = this._buttonBatteryPercentFromVoltage(voltage);
      if (percent !== null) {
        return this._setButtonBattery(percent, `stored ${key} (${voltage}V)`);
      }
    }
  ...
  async _queryButtonBatteryDPs() {
    const dps = this._getButtonBatteryDPs();
    let queried = 0;

    const withTimeout = async (promise, label) => {
      const setTimer = this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : setTimeout;
      const clearTimer = this.homey?.clearTimeout ? this.homey.clearTimeout.bind(this.homey) : clearTimeout;
      let timer;
      try {
        return await Promise.race([
          Promise.resolve(promise),
          new Promise((_, reject) => {
            timer = setTimer(() => reject(new Error(`${label} timeout`)), 1200);
          }),
        ]);
      } finally {
    ...
    for (const { endpointId, cluster } of this._getButtonTuyaClusters()) {
      for (const dp of dps) {
        try {
          let query;
          let label;
          ...
          if (typeof cluster.dataQuery === 'function') {
            query = cluster.dataQuery({ dp });
            label = `EP${endpointId} DP${dp} dataQuery`;
            hasQueryMethod = true;
          } else if (typeof cluster.getData === 'function') {
            query = cluster.getData(dp);
            label = `EP${endpointId} DP${dp} getData`;
            hasQueryMethod = true;
          } else if (typeof cluster.readDP === 'function') {
            query = cluster.readDP(dp);
            label = `EP${endpointId} DP${dp} readDP`;
            hasQueryMethod = true;
          } else if (typeof cluster.requestDP === 'function') {
            query = cluster.requestDP(dp);
            label = `EP${endpointId} DP${dp} requestDP`;
            hasQueryMethod = true;
          }
          if (!hasQueryMethod) {continue;}

          const result = await withTimeout(query, label);
          queried += 1;
          if (result !== undefined) {
            await this._handleButtonBatteryDPEvent(result).catch(() => {});
          }
        } catch (e) {
          this.log(`[BUTTON-BATTERY] ⚠️ Tuya EP${endpointId} DP${dp} query failed: ${e.message}`);
        }
      }
    }
    ...
    if (manager) {
      if (typeof manager.requestAllDPs === 'function') {
        try {
          const result = await withTimeout(manager.requestAllDPs(), 'manager requestAllDPs');
          queried += 1;
          await this._handleButtonBatteryDPEvent(result).catch(() => {});
        } catch (e) {
          this.log(`[BUTTON-BATTERY] ⚠️ Tuya manager requestAllDPs failed: ${e.message}`);
        }
      }
      ...
      for (const dp of dps) {
        try {
          ...
          let label;
          let hasQueryMethod = false;
          if (typeof manager.requestDP === 'function') {
            query = manager.requestDP(dp);
            label = `manager DP${dp} requestDP`;
            hasQueryMethod = true;
          } else if (typeof manager.queryDP === 'function') {
            query = manager.queryDP(dp);
            label = `manager DP${dp} queryDP`;
            hasQueryMethod = true;
          } else if (typeof manager.getData === 'function') {
            query = manager.getData(dp);
            label = `manager DP${dp} getData`;
            hasQueryMethod = true;
          }
          if (!hasQueryMethod) {continue;}
          ...
          this.log(`[BUTTON-BATTERY] ⚠️ Tuya manager DP${dp} query failed: ${e.message}`);
        }
      }
    }
    ...
    _batTimer(async () => {
      try {
        ...
        if (this._hasMissingButtonBatteryValue()) {
          // Restore old v5/v8 store keys first so Homey does not remain on '?'.
          const restored = await this._restoreButtonBatteryFromStore();
          if (!restored) {
  ...
  async _readBatteryWhileAwake() {
      ...
      for (const { endpointId, cluster: powerCluster } of powerClusters) {
        ...
        if (typeof powerCluster.readAttributes === 'function') {
          ...
          if (!batteryRead && data?.batteryVoltage !== undefined && data.batteryVoltage > 0) {
            const voltage = await this._storeButtonBatteryVoltage(data.batteryVoltage);
            const battery = this._buttonBatteryPercentFromVoltage(voltage);
        ...
        if (!batteryRead && powerCluster.batteryVoltage !== undefined && powerCluster.batteryVoltage > 0) {
          const voltage = await this._storeButtonBatteryVoltage(powerCluster.batteryVoltage);
          const battery = this._buttonBatteryPercentFromVoltage(voltage);
    ...
    // METHOD 2: Tuya DP (percent, enum and voltage DPs seen across old app versions/Z2M)
    if (!batteryRead) {
      try {
        this.log('[BUTTON-BATTERY] 📡 Trying Tuya DP cascade for battery...');
        await this._queryButtonBatteryDPs();
        const current = this.getCapabilityValue('measure_battery');
        batteryRead = current !== null && current !== undefined && Number.isFinite(Number(current));
      } catch (e) {
        this.log(`[BUTTON-BATTERY] ⚠️ Tuya DP method failed: ${e.message}`);
      }
    }

    // METHOD 3: Check stored value from old/new report keys
    if (!batteryRead) {
      batteryRead = await this._restoreButtonBatteryFromStore();
    }
  ...
  async _handleTuyaBatteryDP(dp, value) {
    ...
    const numericValue = this._coerceButtonBatteryDPValue(value);
    let battery = null;
    let source = `Tuya DP${dp}`;

    if (BUTTON_BATTERY_VOLTAGE_DPS.includes(Number(dp))) {
      const voltage = await this._storeButtonBatteryVoltage(numericValue);
      battery = this._buttonBatteryPercentFromVoltage(voltage);
      source = `Tuya voltage DP${dp} (${voltage}V)`;
    } else {
      battery = this._normalizeButtonDpBattery(Number(dp), numericValue);
    }
    ...
    await this._setButtonBattery(battery, source);
    return true;
  }
