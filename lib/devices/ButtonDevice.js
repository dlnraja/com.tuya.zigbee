    ...
    if (snappy) {
      ...
      void (async () => {
        try {
          ...
          const state = { button: btnStr, count };
          await triggerFlowCardHeuristic(this, candidates, tokens, 'trigger', state);
          // Universal app-level card (many Bastien Flows use this)
          if (type === 'single') {
            await this._tryCard('button_pressed', { button: btnNum }, { button: btnNum }).catch(() => {});
          } else if (type === 'double') {
            await this._tryCard('button_double_press', { button: btnNum }, { button: btnNum }).catch(() => {});
          } else if (type === 'long') {
            await this._tryCard('button_long_press', { button: btnNum }, { button: btnNum }).catch(() => {});
          }
        } catch (err) {
          try { this.error(`[BUTTON-FLOW] P2707 snappy primary soft: ${err?.message || err}`); } catch (_e) { /* */ }
        }
      })();
      return;
    }
