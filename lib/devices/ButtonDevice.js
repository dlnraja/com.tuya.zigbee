...
class ButtonDevice extends AutoAdaptiveDevice {
    ...
    for (let ep = 1; ep <= endpoints; ep++) {
      try {
        ...
        if (onOffCluster) {
          ...
          // v5.5.186: CRITICAL - Listen for onOff ATTRIBUTE changes (more reliable for TS0044!)
          // Many Tuya buttons report via attribute changes, not commands
          onOffCluster.on('attr.onOff', async (value) => {
            this.log(`[ONOFF-ATTR] Button ${ep} onOff attribute changed: ${value}`);
            // Attribute change = button press (value true = pressed)
            if (value === true || value === 1) {
              this.log(`🔘 Button ${ep} PRESSED (onOff attr)`);
  ...
  /**
   * v5.5.111: Read battery while device is awake (after button press)
   * Buttons are sleepy devices - only respond when just pressed!
   */
  async _readBatteryWhileAwake() {
    // Debounce - don't read too often
    const now = Date.now();
    const lastRead = this._lastBatteryRead || 0;
    if (now - lastRead < 60000) return; // Max once per minute
    ...
    try {
      ...
      if (!powerCluster?.readAttributes) return;

      this.log('[BUTTON-BATTERY] 🔋 Button pressed - reading battery...');
      ...
      if (data?.batteryPercentageRemaining !== undefined && data.batteryPercentageRemaining !== 255) {
        ...
        this.log(`[BUTTON-BATTERY] 🔋 Battery: ${battery}%`);
