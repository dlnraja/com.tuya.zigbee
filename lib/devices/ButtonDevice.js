...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  _getButtonBatteryProfile() {
    ...
    // WHY(P2470 / Peter 8afffc76): compose lists CR2032 first → Homey shows wrong chemistry
    // for SH-SC07 (Z2M CR2450). Prefer profile chemistry over manifest order.
    const shSc07 = /mrpevh8p|5bpeda8u|b4awzgct/i.test(String(manufacturer || ''));

    return {
      manufacturer,
      modelId,
      profile,
      chemistry: profile?.chemistry || (shSc07 ? 'CR2450' : null) || energyType || 'CR2032',
    };
  }
  ...
  _normalizeButtonZclBattery(rawValue) {
    ...
    // WHY(P2507 / Peter 77394256): 0 = unread placeholder; 253 = phantom EP garbage (Johan#1120)
    if (raw === 0 || raw === 253 || raw === 255 || raw === 0xFFFF) {return null;}
  ...
  async _setButtonBattery(percent, source = 'unknown') {
    ...
    // WHY(P2507 / Peter 77394256 @ 9.0.926): painting 0% from failed/placeholder
    // reads leaves Homey UI blank/? and blocks Insights/History until a real % arrives.
    if (value === 0) {
      this.log(`[BUTTON-BATTERY] ignore 0% placeholder (${source})`);
      return false;
    }
    ...
    if (typeof this.ingestBatteryPercent === 'function') {
      const r = await this.ingestBatteryPercent(value, { protocol: 'cached', source, skipThrottle: previous == null || previous === 0 });
    ...
    if (typeof this._updateBattery === 'function') {
      ...
      await this.safeSetCapabilityValue('measure_battery', value, { skipThrottle: true }).catch((err) => {
  ...
  /**
   * WHY(P2507): Homey hides Battery + History when getable:false / preventInsights.
   * Contre quoi: Peter #2238 after re-add still blank despite ZCL paint.
   * WHY(P2550 / Peter #2239 diag b8b78521): % can paint while Insights History stays
   * empty — Homey never created the insight log when the device was paired under
   * getable:false. One-shot remove+add recreates the log; then re-paint %.
   * WHY(P2555 / Peter #2239 soft-dismiss "History not so important"): still fix it —
   * bump recycle key so tips after P2550 re-run recycle when first pass left empty Insights.
   */
  async _ensureBatteryCapabilityUi() {
    ...
    if (typeof this.setCapabilityOptions !== 'function') {return true;}
    try {
      const cur = (typeof this.getCapabilityOptions === 'function'
        && this.getCapabilityOptions('measure_battery')) || {};
      if (cur.getable === false || cur.preventInsights === true
          || cur.getable !== true || cur.preventInsights == null) {
        ...
        this.log('[BUTTON-BATTERY] P2507/P2512 restored measure_battery getable/insights');
      }
    } catch (_e) { /* soft */ }

    // Fleet heal (P2553) — complementary, never shrink
    try {
      const { healSensorCapabilityGetable } = require('../utils/HomeyCapabilityUx');
      await healSensorCapabilityGetable(this);
    } catch (_eHeal) { /* soft */ }

    // WHY(P2550/P2555): recycle so History/Insights tab gets a real log after tip.
    try {
      const recycled = typeof this.getStoreValue === 'function'
        ? this.getStoreValue('p2555_batt_insights_recycle')
        : null;
      if (!recycled
          && typeof this.removeCapability === 'function'
          && typeof this.addCapability === 'function') {
        const lastRaw = (typeof this.getStoreValue === 'function'
          && this.getStoreValue('last_battery_percentage'))
        ...
        await this.setCapabilityOptions('measure_battery', {
          ...
          units: '%',
          title: {
            en: 'Battery', nl: 'Batterij', fr: 'Batterie', de: 'Batterie',
            it: 'Batteria', sv: 'Batteri', no: 'Batteri', es: 'Batería', da: 'Batteri',
          },
        }).catch(() => {});
        if (typeof this.setStoreValue === 'function') {
          await this.setStoreValue('p2555_batt_insights_recycle', 1).catch(() => {});
          // Keep legacy flag so older tips don't double-recycle forever
          await this.setStoreValue('p2550_batt_insights_recycle', 1).catch(() => {});
        }
        const paint = async (pct) => {
          if (!(Number.isFinite(pct) && pct > 0)) return;
          if (typeof this.safeSetCapabilityValue === 'function') {
            await this.safeSetCapabilityValue('measure_battery', pct, { skipThrottle: true }).catch(() => {});
          } else if (typeof this.setCapabilityValue === 'function') {
            await this.setCapabilityValue('measure_battery', pct).catch(() => {});
          }
        };
        await paint(last);
        // WHY(P2555): Homey Insights sometimes needs a delayed second sample
        try {
          const { safeSetTimeout } = require('../utils/safe-timers');
          safeSetTimeout(this, () => { paint(last); }, 1500);
          safeSetTimeout(this, () => { paint(last); }, 6000);
        } catch (_eDelay) { /* soft */ }
        this.log('[BUTTON-BATTERY] P2555 recycled measure_battery for Insights/History');
      }
    } catch (_e2) { /* soft */ }
    return true;
  }
  ...
  async _ingestButtonBatterySample(raw, opts = {}) {
    ...
    const context = this._getButtonBatteryProfile();
    const previous = this.hasCapability?.('measure_battery') ? this.getCapabilityValue('measure_battery') : null;
    // WHY(P2461/P2507): empty UI (`?`) OR placeholder 0% must bypass throttle
    const forceUi = opts.skipThrottle === true
      || previous == null
      || previous === undefined
      || previous === 0;
    if (typeof this.ingestBatteryPercent === 'function') {
      const r = await this.ingestBatteryPercent(raw, {
        ...
        ...opts,
        skipThrottle: forceUi || opts.skipThrottle,
      });
      if (r?.ok && Number.isFinite(r.percent) && r.percent > 0) {
        if (forceUi && this.hasCapability?.('measure_battery')) {
          const painted = this.getCapabilityValue('measure_battery');
          if (painted == null || painted === undefined || painted === 0 || painted !== r.percent) {
            await this.safeSetCapabilityValue('measure_battery', r.percent, { skipThrottle: true }).catch(() => {});
          }
        }
    ...
    let percent = null;
    if (protocol === 'voltage') {
      // WHY(P2706): coin remotes with zcl200IsPercent — never paint from voltage
      if (context.profile?.zcl200IsPercent || context.profile?.skipBatteryReporting) {
        return false;
      }
    ...
    if (percent === null || !Number.isFinite(percent) || percent <= 0) {return false;}
    if (forceUi && this.hasCapability?.('measure_battery')) {
  ...
  constructor(...args) {
    ...
    // WHY(P2299/f647d35b): never let DCM-FB add onoff/dim/alarm_battery phantoms
    this._forbiddenCapabilities = ['onoff', 'dim', 'alarm_battery'];
  ...
  async _updateBattery(percentage) {
    ...
    await this.setStoreValue('last_battery_percentage', value).catch(() => {});
    // WHY(P2745): Homey "battery changed" tag Flows need a real capability delta +
    // declared driver card — snappy remotes skip press TX so passive paint must fire.
    if (previous === null || previous === undefined || Number(previous) !== value) {
      await this._maybeTriggerBatteryChangedFlow(value).catch(() => {});
    }
    await this._maybeTriggerBatteryLowFlow(value, previous).catch(() => {});
  }

  async _maybeTriggerBatteryChangedFlow(value) {
  ...
  async onEndDeviceAnnounce() {
    ...
    const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
    // WHY(P2733 / Bastien IG+mesh): snappy TS004x wake must NOT await multi-EP
    // cluster.bind() — Homey tools showed 40–59% TX err + ~2h last-seen + ~7s
    // dead between presses (bind timeouts burn the awake window). Contre quoi:
    // await bind storm after every press announce.
    const snappySleepy = !!(profile?.snappyRelayFlow || profile?.skipBatteryReporting || profile?.noEf00Tx);
    if (snappySleepy) {
      this.log('[BUTTON] P2733 snappy wake — listen-only (no mass bind / no batt TX)');
      try { await super.onEndDeviceAnnounce?.(); } catch (_e) { /* soft */ }
      if (!zclNode) {return;}
      await this._registerSceneRecallListener(zclNode).catch(() => {});
      this._registerSceneModeAttributeListener(zclNode);
      // WHY(P2739 / Bastien ed627371): magic packet + ZclBatteryMonitor.attach still
      // TX on wake (basic 0xFFDE write / configureReporting) — skip on snappy noEf00.
      // Contre quoi: "listen-only" that still floods Homey Zigbee queue after every press.
      return;
    }
    ...
    if (!zclNode) {return;}
    // WHY(P2312): Peter SH-SC07 / mrpevh8p — interview lists phantom EP2–4 that still
    // emit 0xFD on wake. buttonCount===1 must NOT limit rebind to EP1 only.
    const map1 = !!(profile?.mapAllEndpointsToButton1 || profile?.collapsePhantomEndpoints);
    const gangN = Math.max(1, Number(this.buttonCount || this.gangCount || 1) || 1);
    const epKeys = Object.keys(zclNode.endpoints || {}).map(Number).filter((n) => n >= 1 && n <= 8);
    const endpoints = map1
      ? Math.max(gangN, ...epKeys, 4)
      : gangN;
    const CLUSTER_NAMES = {
      5: ['genScenes', 'scenes'],
      6: ['genOnOff', 'onOff'],
    ...
    for (let ep = 1; ep <= endpoints; ep++) {
      ...
      const clusterIdsToBind = [5, 6, 18, 8, 0xE000];
      for (const cid of clusterIdsToBind) {
        try {
          ...
          // Fire-and-forget — never await sleepy bind timeouts (P2733)
          if (cluster?.bind) {void cluster.bind().catch(() => {});}
         } catch (_e) {}
      }
    }
    if (map1) {
      this.log(`[BUTTON] P2312 wake rebind EP1–${endpoints} (mapAllEndpointsToButton1)`);
    }
    ...
    this._registerSceneModeAttributeListener(zclNode);
    // Forum #2134 + WHY(P2316): scene 0x8004 only for true TS004F.
    // Peter/meter91/Nobø (writeSceneAttr=false) — never write on wake (kills 0xFD).
    try {
      const fam = require('../zigbee/DeviceOperatingMode').classifyOperatingFamily(this);
      const profileSkip = !!(profile?.skip8004 || profile?.writeSceneAttr === false);
      if (fam.writeSceneAttr && !profileSkip) {
        this.log(`[BUTTON] P2450 wake write 0x8004 (family=${fam.family} default=${fam.defaultMode})`);
        await this._universalSceneModeSwitch(zclNode);
        await this._reapplySceneModeOnWake();
      } else {
        this.log(`[BUTTON] P2316 skip wake 0x8004 (family=${fam.family} write=${!!fam.writeSceneAttr})`);
      }
    } catch (_e) { /* non-fatal */ }
    // WHY(P2317): Z2M configureMagicPacket / first-press-after-sleep — force
    // genBasic 0xFFDE=0x13 on every wake for TS004x / map1 / skip8004 fleets.
    // Soft-fail; never blocks battery rebind.
    try {
      const needsMagic = !!(profile?.mapAllEndpointsToButton1 || profile?.collapsePhantomEndpoints
        || profile?.skip8004 || profile?.writeSceneAttr === false
        // WHY(P2554 / Z2M#8149 / HA first-action-ignored): ALL TS004x + TS0215 remotes
        || /TS004[1-6F]|TS0215/i.test(String(this.getSetting?.('zb_model_id') || this.getData?.()?.productId || '')));
      if (needsMagic) {
        ...
        if (typeof scenesCluster.on === 'function') {
          scenesCluster.on('commandRecall', async (msg) => {
            try {
              ...
              // Always emit scene_recall — app-level + driver-scoped compose cards
              try {
                const sceneIdStr = String(sceneId);
                const tokens = {
                  scene_id: Number.isFinite(Number(sceneId)) ? Number(sceneId) : sceneIdStr,
                  button: String(buttonIdx || 0),
                  press_type: pressType,
                };
                // Prefer declared driver cards via _tryCard (P2449 — was app-level only)
                const driverId = this.driver?.id || 'button';
                const gangCount = this.buttonCount || this.gangCount || 1;
                const sceneCandidates = [
                  `${driverId}_button_${gangCount}gang_button_scene_recall`,
                  `${driverId}_button_${gangCount}gang_button_${buttonIdx || 1}_scene_recall`,
                  `${driverId}_button_${buttonIdx || 1}_scene_recall`,
                  `${driverId}_scene_recall`,
                ];
                let fired = false;
                for (const cardId of sceneCandidates) {
                  if (await this._tryCard(cardId, tokens, tokens)) {
                    fired = true;
                    break;
                  }
                }
                // WHY(P2624/e8d98608): never raw-get undeclared app-level scene_recall —
                // FLOW-GUARD Invalid Flow Card ID spam. Declared driver cards only via _tryCard (P2449).
              } catch {}
            } catch (err) {
  ...
  async onNodeInit({ zclNode }) {
    ...
    // WHY(P2664 / Bastien 4c0d232b+e8d98608): TS0043 wall remotes fire 0xFD single+double
    // within <50ms; 500ms per-button debounce ate the real press → "aucun bouton / flows morts".
    // WHY(P2702 / diag 1f4dcf2e): snappyRelayFlow remotes need ~50ms so rapid press→relay feels instant.
    let minInterval = 500;
    try {
      const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
      const multi = Math.max(Number(this.buttonCount) || 0, Number(profile?.maxButtons) || 0) > 1;
      // WHY(P2706 / Bastien): TS0042/43 felt slow at 50ms floor — 25ms for snappy
      if (profile?.snappyRelayFlow || (Number(profile?.debounceMs) || 0) > 0 && Number(profile.debounceMs) <= 80) {
        minInterval = 25;
      } else if (profile?.noEf00 || profile?.protocol === 'zcl_0xfd_e000' || multi) {
        minInterval = 120;
      }
    } catch (_e) { /* soft */ }
      ...
      try {
        ...
        if (this.hasCapability('measure_battery') && this.getCapabilityValue('measure_battery') === null) {
          ...
          if (stored !== null && typeof stored === 'number') {
            ...
            // WHY(P2733): snappy/skipBatt — never ZCL readAttributes on boot timer
            let skip = false;
            try {
              const p = this.getDeviceProfile?.() || {};
              skip = !!(p.skipBatteryReporting || p.snappyRelayFlow || p.noEf00Tx);
            } catch (_e) { /* soft */ }
            if (skip) {
              this.log('[BUTTON-BATTERY] P2733 skip init ZCL read (skipBatteryReporting)');
            } else {
  ...
  async _registerButtonCapabilityListeners() {
    ...
    const candidateCaps = [];
    if (this.hasCapability('button')) candidateCaps.push({ capId: 'button', buttonNum: 1 });
    for (let i = 1; i <= buttonCount; i++) {
      const capId = `button.${i}`;
      if (this.hasCapability(capId)) candidateCaps.push({ capId, buttonNum: i });
    }

    for (const { capId, buttonNum } of candidateCaps) {
      try {
        this.registerCapabilityListener(capId, async () => {
          this.log(`[BUTTON] Button ${buttonNum} capability triggered (${capId}, virtual press)`);
          const now = Date.now();
  ...
  _isHybridButtonContactDevice(manufacturerName, productId) {
    ...
    const hybridManufacturers = [
      ...
      '_TZ3000_ja5osu5g',  // P2453: 1-btn TS004F scene → smart_knob
  ...
  async _readBatteryWhileAwake() {
    ...
    const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
    // WHY(P2461 / Peter mrpevh8p): no EF00 — skip broken dataQuery that burns awake window
    // WHY(P2691 / Bastien 885a9901): also sceneSwitch / TS004x class — never powerCfg TX
    let noEf00Battery = !!(profile?.noEf00Tx || profile?.skipBatteryReporting);
    try {
      const { shouldSkipSleepyRemoteBatteryTx } = require('../zigbee/PowerClusterPolicy');
      // WHY(P2727): pass homey so Bastien bastien_skip_battery_tx boot gate applies
      noEf00Battery = shouldSkipSleepyRemoteBatteryTx(profile, { homey: this.homey });
    } catch (_e) { /* soft — keep local flags */ }

    // WHY(P2720 Bastien 27b0bd04): sleepy remotes must NOT log+allocate on every press
    // before the skip path — heap was already critical after LIVE-DATA OOM.
    // Resolve skipBatteryReporting FIRST; only log when we actually attempt a ZCL read.
    if (noEf00Battery) {
      const storedEarly = await Promise.resolve(this.getStoreValue('last_battery_percentage')).catch(() => null);
      if (storedEarly !== null && storedEarly !== undefined) {
        const cur = this.getCapabilityValue('measure_battery');
        if (cur != null && cur !== undefined && Number(cur) === Number(storedEarly)) {
          return; // UI already painted — zero work on press
        }
        batteryRead = await this._ingestButtonBatterySample(storedEarly, {
          protocol: 'cached',
          source: 'store-restore-first',
          skipThrottle: cur == null || cur === undefined,
        });
        if (!batteryRead && (cur == null || cur === undefined)) {
          try {
            await this.safeSetCapabilityValue('measure_battery', Number(storedEarly), { skipThrottle: true });
            batteryRead = true;
          } catch (_e) { /* soft */ }
        }
        if (batteryRead) {
          this.log(`[BUTTON-BATTERY] ✅ UI restored from store ${storedEarly}% (skip ZCL storm)`);
          return;
        }
      }
      // WHY(P2685 / Z2M#8072 / Bastien pile): no store yet — still do NOT TX powerCfg.
      // Passive ZclBatteryMonitor listeners only; UI stays `?` until report or next wake with store.
      this.log('[BUTTON-BATTERY] P2685 skip ZCL readAttributes (skipBatteryReporting / pile)');
      return;
    }

    this.log('[BUTTON-BATTERY] 🔋 Button pressed - reading battery (device awake)...');

    // P2689 — piggyback ZCL only when sample is stale (Z2M: don't hammer awake window)
    try {
      const { shouldPiggybackBatteryRead } = require('../battery/SmartBatteryAdaptivePrecision');
      const lastTs = Number(await Promise.resolve(this.getStoreValue('last_battery_time')).catch(() => 0))
        || this._lastSuccessfulBatteryTs
        || 0;
      const pct = this.getCapabilityValue('measure_battery');
      if (pct != null && pct !== undefined && !shouldPiggybackBatteryRead({
        lastTs,
        percent: pct,
        skipBatteryReporting: noEf00Battery,
      })) {
        this.log('[BUTTON-BATTERY] P2689 skip piggyback read (fresh sample)');
        return;
      }
    } catch (_e) { /* soft */ }
    ...
    try {
      ...
      for (const { endpointId, cluster: powerCluster } of powerClusters) {
        ...
        if (typeof powerCluster.readAttributes === 'function') {
          ...
          // WHY(P2706 / Bastien TS0043 0–84%): never let CR2032 voltage curve
          // overwrite a valid ZCL % (2.9V≈85% looked like a hard 84% ceiling).
          if (!batteryRead && data?.batteryVoltage !== undefined && data.batteryVoltage > 0
            && !profile?.zcl200IsPercent && !profile?.skipBatteryReporting) {
    ...
    // WHY(P2461): throttle cache can mark 100% as duplicate while Homey UI still shows `?`
    if (!batteryRead) {
      ...
      if (storedBattery !== null && storedBattery !== undefined) {
        const cur = this.hasCapability?.('measure_battery')
          ? this.getCapabilityValue('measure_battery')
          : null;
        this.log(`[BUTTON-BATTERY] ℹ️ Using stored battery: ${storedBattery}% (ui=${cur})`);
        batteryRead = await this._ingestButtonBatterySample(storedBattery, {
          protocol: 'cached',
          source: 'store-restore',
          skipThrottle: cur == null || cur === undefined,
        });
        if (!batteryRead && (cur == null || cur === undefined) && this.hasCapability?.('measure_battery')) {
          try {
            await this.safeSetCapabilityValue('measure_battery', Number(storedBattery), { skipThrottle: true });
            batteryRead = true;
          } catch (_e) { /* soft */ }
        }
      }
  ...
  async triggerButtonPress(buttonNumber, pressType = 'single', count = 1, options = {}) {
    ...
    if (this._buttonTriggerProtection) {
      ...
      // v5.5.805/v10.1.3 + P2664: debounce identical button+pressType only.
      // Multi-gang remotes may fire different buttons / single-vs-double close together.
      if (!this._buttonTriggerProtection.lastByButton) {
        this._buttonTriggerProtection.lastByButton = {};
      }
      if (!this._buttonTriggerProtection.lastTypeByButton) {
        this._buttonTriggerProtection.lastTypeByButton = {};
      }
      const lastForButton = this._buttonTriggerProtection.lastByButton[button] || 0;
      const lastType = this._buttonTriggerProtection.lastTypeByButton[button];
      const timeSinceLastButton = now - lastForButton;
      const sameType = lastType != null && String(lastType) === String(type);
      if (sameType && timeSinceLastButton < this._buttonTriggerProtection.minInterval && timeSinceLastButton > 0) {
        this.log(`[ANTI-TRIGGER] Debounced button ${button}/${type} (${timeSinceLastButton}ms < ${this._buttonTriggerProtection.minInterval}ms)`);
        return;
      }

      this._buttonTriggerProtection.lastByButton[button] = now;
      this._buttonTriggerProtection.lastTypeByButton[button] = type;
      this._buttonTriggerProtection.lastTrigger = now;
    }

    this._lastTriggerTime[`${button}_${type}`] = now;

    // WHY(P2702 / Bastien 1f4dcf2e): fire Flow BEFORE UI pulse — await setCapability
    // used to delay Homey Flow → relay by hundreds of ms ("latence enorme").
    // WHY(P2706): after that, NEVER await the card-cascade on snappy remotes —
    // Homey Flow is app-wide; TS0042/43 await-storm made TS0041 slow too + crash.
    let snappyProfile = null;
    try {
      snappyProfile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
    } catch (_sp) { /* soft */ }
    const snappy = !!(snappyProfile?.snappyRelayFlow);
    const skipUiPulse = !!(snappyProfile?.skipUiPulse || (snappy && (Number(this.buttonCount) || 0) > 1));

    this.log(`[BUTTON-FLOW] triggerButtonPress(btn=${button}, type=${type}${snappy ? ' snappy' : ''})`);
    if (typeof this._triggerPhysicalFlow === 'function') {
      try {
        this._triggerPhysicalFlow(button, type, { _internalTrigger: true });
      } catch (_tf) { /* soft — never crash app on flow dispatch */ }
    }

    // v5.5.430 / v10.6.0: Reset button capability to false after trigger (visual pulse)
    // Pulse is fire-and-forget so it never blocks Flow→relay.
    // WHY(P2706): multi-gang snappy skip FULL pulse storm.
    // WHY(P2734): still soft complementary pulse for bi-dir Homey tiles.
    if (skipUiPulse) {
      try {
        const { softPulsePhysicalUi } = require('../utils/HomeyButtonUiCharter');
        softPulsePhysicalUi(this, button, { source: 'physical', snappy: true, pulseMs: 50 });
      } catch (_e) { /* soft */ }
    } else {
      const capsToPulse = [];
      if (this.hasCapability(`button.${button}`)) capsToPulse.push(`button.${button}`);
      if (button === 1 && this.hasCapability('button')) capsToPulse.push('button');
      const pulseMs = snappy ? 50 : 500;
      for (const cap of capsToPulse) {
        Promise.resolve()
          .then(() => this.safeSetCapabilityValue(cap, true, { skipThrottle: true }))
          .catch(() => {})
          .then(() => {
            safeSetTimeout(this, async () => {
              if (this._destroyed) {return;}
              await this.safeSetCapabilityValue(cap, false, { skipThrottle: true }).catch(() => {});
            }, pulseMs);
          });
      }
    }

    const runCardCascade = async () => {
    try {
      const driverId = this.driver?.id || 'button_wireless';
      if (source === 'virtual') {
      ...
      // WHY(P2686 / Bastien diag 885a9901): cancel stale invent timers on ANY new press
      // Contre quoi: btn1 single mid-flow firing button_2_release from leftover long timer
      try {
        if (this._holdReleaseTimers && typeof this._holdReleaseTimers === 'object') {
          for (const k of Object.keys(this._holdReleaseTimers)) {
            try { clearTimeout(this._holdReleaseTimers[k]); } catch (_e) { /* soft */ }
            delete this._holdReleaseTimers[k];
          }
        }
      } catch (_clr) { /* soft */ }
      ...
      // WHY(P2686): TS004x scene remotes send discrete 0xFD long — never invent soft release
      if (type === 'long') {
        const releaseKey = `release_${button}`;
        const HOLD_RELEASE_DELAY = 2000;

        const hasNativeRelease = this._nativeEventTracker.nativeReleaseSupport === true;
        let skipSoftRelease = false;
        try {
          const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
          skipSoftRelease = !!(profile?.skipSoftwareHoldRelease || profile?.sceneSwitch
            || profile?.skipBatteryReporting || profile?.noEf00Tx);
        } catch (_p) { /* soft */ }

        if (hasNativeRelease || skipSoftRelease) {
          this.log(skipSoftRelease
            ? '[HOLD-RELEASE] P2686 skip software release (scene/0xFD discrete remote)'
            : '[SMART-DEDUP] Skipping timer-based release (device has native support)');
        } else {
          if (!this._holdReleaseTimers) this._holdReleaseTimers = {};
          if (this._holdReleaseTimers[releaseKey]) {
            clearTimeout(this._holdReleaseTimers[releaseKey]);
          }
      ...
      // WHY(P2378 / Peter cfbf687f): 1-gang remotes declare *_button_1gang_* — must fire
      // those cards too (previously gated behind gangCount > 1 → Flows never ran).
      ...
      // WHY(P2461): one finger press must not re-trigger the same card via heuristics + legacy
      const firedCards = new Set();
      const tryOnce = async (cardId, tokens = {}, state = {}) => {
        const key = String(cardId || '').toLowerCase();
        if (!key || firedCards.has(key)) {return false;}
        const ok = await this._tryCard(cardId, tokens, state);
        if (ok) {firedCards.add(key);}
        return ok;
      };
      const bw1Strict = /button_wireless_1/i.test(driverId) && gangCount === 1;
      ...
      await tryOnce('button_matrix',
        { button: btnStr, action: matrixAction },
        { button: btnStr, action: matrixAction });

      // Drive driver-scoped cards from FlowCardHeuristics (includes 1gang compose IDs)
      let heurHit = false;
      try {
        const { buildPhysicalFlowCandidates } = require('../flow/FlowCardHeuristics');
        const heurPress = type === 'multi' && count === 3 ? 'triple' : type;
        const candidates = buildPhysicalFlowCandidates(driverId, btnNum, heurPress, {
          gangCount,
          isButtonDevice: true,
        });
        const tokens = { button: btnStr, count };
        const state = { button: btnStr, count };
        for (const id of candidates) {
          if (/^(button_pressed|button_double_press|button_long_press|button_matrix|virtual_button_pressed|button_triple_clicked|button_multi_press|button_release)$/i.test(id)) {
            continue; // app-level cards handled below
          }
          if (await tryOnce(id, tokens, state)) {heurHit = true;}
        }
      } catch (_heurErr) { /* soft — fall through to legacy IDs */ }

      // WHY(P2461): when heuristics already fired a declared driver card, only add app-level
      // universals — skip legacy invent (`*_button_pressed`, `*_button_1_pressed`).
      if (type === 'single') {
        await tryOnce('button_pressed', { button: btnNum }, { button: btnNum });
        if (!bw1Strict || !heurHit) {
          await tryOnce(`${driverId}_button_${gangCount}gang_button_pressed`, { button: btnStr }, { button: btnStr });
          if (!bw1Strict) {
            await tryOnce(`${driverId}_button_${gangCount}gang_button_${button}_pressed`);
            await tryOnce(`${driverId}_button_pressed`, { button: btnStr }, { button: btnStr });
            if (sceneStyle || gangCount === 1) {
              await tryOnce(`${driverId}_button_${button}_pressed`, { button: btnStr }, { button: btnStr });
            }
          }
        }
      } else if (type === 'double') {
        await tryOnce('button_double_press', { button: btnNum }, { button: btnNum });
        if (!bw1Strict || !heurHit) {
          await tryOnce(`${driverId}_button_${gangCount}gang_button_double_press`, { button: btnStr }, { button: btnStr });
          if (!bw1Strict) {
            await tryOnce(`${driverId}_button_${gangCount}gang_button_${button}_double`);
            await tryOnce(`${driverId}_button_double_press`, { button: btnStr }, { button: btnStr });
            if (sceneStyle || gangCount === 1) {
              await tryOnce(`${driverId}_button_${button}_double`, { button: btnStr }, { button: btnStr });
            }
          }
        }
      } else if (type === 'long') {
        await tryOnce('button_long_press', { button: btnNum }, { button: btnNum });
        if (!bw1Strict || !heurHit) {
          await tryOnce(`${driverId}_button_${gangCount}gang_button_long_press`, { button: btnStr }, { button: btnStr });
          if (!bw1Strict) {
            await tryOnce(`${driverId}_button_${gangCount}gang_button_${button}_long`);
            await tryOnce(`${driverId}_button_long_press`, { button: btnStr }, { button: btnStr });
            if (sceneStyle || gangCount === 1) {
              await tryOnce(`${driverId}_button_${button}_long`, { button: btnStr }, { button: btnStr });
            }
          }
        }
      } else if (type === 'multi') {
        await tryOnce('button_multi_press', { button: btnNum, count }, { button: btnNum, count });
        if (!bw1Strict || !heurHit) {
          await tryOnce(`${driverId}_button_${gangCount}gang_button_multi_press`, { button: btnStr, count }, { button: btnStr, count });
          if (count === 3) {
            await tryOnce('button_triple_clicked', { button: btnNum }, { button: btnNum });
            if (!bw1Strict) {
              await tryOnce(`${driverId}_button_${gangCount}gang_button_${button}_triple`);
              if (sceneStyle || gangCount === 1) {
                await tryOnce(`${driverId}_button_${button}_triple`, { button: btnStr }, { button: btnStr });
              }
            }
          }
        }
      } else if (type === 'release') {
        await tryOnce('button_release', { button: btnNum }, { button: btnNum });
        if (!bw1Strict || !heurHit) {
          await tryOnce(`${driverId}_button_${gangCount}gang_button_${button}_release`);
          if (!bw1Strict) {
            await tryOnce(`${driverId}_button_release`, { button: btnStr }, { button: btnStr });
            if (sceneStyle || gangCount === 1) {
              await tryOnce(`${driverId}_button_${button}_release`, { button: btnStr }, { button: btnStr });
            }
          }
        }
      }
      ...
      // WHY(P2706): snappy remotes — never TX powerCfg / awake read on press path
      // (passive ZCL attr listeners only). Contre quoi: press→read→crash/lag.
      if (!snappy) {
    ...
    if (snappy) {
      // WHY(P2707 / Bastien black TS0042/43 crash): full tryOnce cascade OOMs Homey.
      // WHY(P2745 / Bastien IG flow tags): preferred-only heuristic left dropdown
      // cards (Bouton appuyé / double / long / multi — Homey tag icon) dead while
      // capability Bouton N still pulsed. Fire declared generic + numbered only
      // (≤4 cards), fire-and-forget — never await the Homey Flow queue.
      // WHY(P2746): also scene_switch short IDs + 1gang / *_button_N_* siblings.
      try {
        const driverId = this.driver?.id || 'button_wireless';
        const gangCount = Math.max(1, Number(this.buttonCount || this.gangCount || 1) || 1);
        const btnNum = Number(button) || 1;
        const btnStr = String(button);
        const tokens = { button: btnStr, button_id: btnStr, count, action: type, gang: btnNum };
        const state = { button: btnStr, count };
        const base = `${driverId}_button_${gangCount}gang`;
        const sceneStyle = /scene_switch/i.test(driverId);
        const declared = [];
        if (type === 'single') {
          declared.push(`${base}_button_pressed`);
          declared.push(`${base}_button_${btnNum}_pressed`);
          if (sceneStyle) {
            declared.push(`${driverId}_button_pressed`);
            declared.push(`${driverId}_button_${btnNum}_pressed`);
          }
          if (gangCount === 1) {
            declared.push(`${driverId}_button_1gang_button_pressed`);
            declared.push(`${driverId}_button_1gang_button_1_pressed`);
          }
        } else if (type === 'double') {
          declared.push(`${base}_button_double_press`);
          declared.push(`${base}_button_${btnNum}_double`);
          if (sceneStyle) {
            declared.push(`${driverId}_button_double_press`);
            declared.push(`${driverId}_button_${btnNum}_double`);
          }
          if (gangCount === 1) {
            declared.push(`${driverId}_button_1gang_button_double_press`);
            declared.push(`${driverId}_button_1gang_button_1_double`);
          }
        } else if (type === 'long') {
          declared.push(`${base}_button_long_press`);
          declared.push(`${base}_button_${btnNum}_long`);
          if (sceneStyle) {
            declared.push(`${driverId}_button_long_press`);
            declared.push(`${driverId}_button_${btnNum}_long`);
          }
          if (gangCount === 1) {
            declared.push(`${driverId}_button_1gang_button_long_press`);
            declared.push(`${driverId}_button_1gang_button_1_long`);
          }
        } else if (type === 'multi') {
          declared.push(`${base}_button_multi_press`);
          if (count === 3) {
            declared.push(`${base}_button_${btnNum}_triple`);
            if (sceneStyle) declared.push(`${driverId}_button_${btnNum}_triple`);
            if (gangCount === 1) declared.push(`${driverId}_button_1gang_button_1_triple`);
          }
          if (sceneStyle) declared.push(`${driverId}_button_multi_press`);
          if (gangCount === 1) declared.push(`${driverId}_button_1gang_button_multi_press`);
        } else if (type === 'release') {
          declared.push(`${base}_button_${btnNum}_release`);
          if (sceneStyle) declared.push(`${driverId}_button_${btnNum}_release`);
          if (gangCount === 1) declared.push(`${driverId}_button_1gang_button_1_release`);
        }
        // Dedupe while preserving order; soft-cap 4 driver cards to stay snappy
        const seen = new Set();
        const capped = [];
        for (const id of declared) {
          ...
          seen.add(k);
          capped.push(id);
          if (capped.length >= 4) break;
        }
        for (const id of capped) {
          void this._tryCard(id, tokens, state).catch(() => {});
        }
        // Universal app-level card (many Bastien Flows use this)
        if (type === 'single') {
          void this._tryCard('button_pressed', { button: btnNum }, { button: btnNum }).catch(() => {});
        } else if (type === 'double') {
          void this._tryCard('button_double_press', { button: btnNum }, { button: btnNum }).catch(() => {});
        } else if (type === 'long') {
          void this._tryCard('button_long_press', { button: btnNum }, { button: btnNum }).catch(() => {});
        } else if (type === 'multi') {
          void this._tryCard('button_multi_press', { button: btnNum, count }, { button: btnNum, count }).catch(() => {});
        }
      } catch (err) {
        try { this.error(`[BUTTON-FLOW] P2707/P2746 snappy primary soft: ${err?.message || err}`); } catch (_e) { /* */ }
      }
      return;
    }
    await runCardCascade();
  }
  ...
  async _triggerHoldRelease(button) {
    try {
      ...
      // WHY(P2332): declared-only via _tryCard — never invent with raw getDeviceTriggerCard
      // (internet/Z2M hold→release; Homey FLOW-GUARD on undeclared IDs).
      const candidates = gangCount === 1
        ? [
          `${driverId}_btn1_release`,
          `${driverId}_btn_release`,
          `${driverId}_button_1gang_button_${btnNum}_release`,
          `${driverId}_button_${btnNum}_release`,
          `${driverId}_button_release`,
        ]
        : [
      ...
      this.log(`[HOLD-RELEASE] Trying release cards for button ${btnNum}`);
      for (const releaseCardId of candidates) {
        // eslint-disable-next-line no-await-in-loop
        if (await this._tryCard(releaseCardId, { button: btnNum }, { button: btnNum })) {
  ...
  async _reapplySceneModeOnWake() {
    ...
    const family = DeviceOperatingMode.classifyOperatingFamily(this);
    const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
    // WHY(P2316): profile.skip8004 must win even if pid mis-labels as TS004F
    if (!family.writeSceneAttr || profile?.skip8004 || profile?.writeSceneAttr === false) {
      return;
    }
  ...
  async _tryCard(cardId, tokens = {}, state = {}) {
    try {
      ...
      // WHY(P2381): merge driver.manifest.flow — app.json no longer lists driver triggers
      ...
      if (declared.size && !resolved) {
        ...
        const appLevel = /^(button_pressed|button_double_press|button_long_press|button_matrix|virtual_button_pressed|button_triple_clicked|button_multi_press|button_release|remote_button_pressed)$/i;
        // WHY(P2461 / Peter #2230): never probe undeclared driver-scoped IDs when the
        // declared set is known — invented aliases (…_button_pressed, UPPERCASE) hit
        // FLOW-GUARD and still burned time next to real 1gang + button_matrix fires.
        // WHY(P2381): if declared is empty (early boot), fall through to variants below.
        if (!appLevel.test(cardId)) {return false;}
        resolved = cardId;
      }
      // WHY (P2247): only call Homey getters for resolved/declared IDs — no speculative spray
      // WHY(P2381): empty declared → probe variants; non-empty → resolved only
      const tryIds = resolved ? [resolved] : (declared.size ? [] : variants);
      const useGate = resolved && findDeclaredCI(declared, resolved) ? declared : null;
      for (const id of tryIds) {
        const card = safeGetFlowCard(this.homey, id, 'trigger', useGate);
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    if (changedKeys.includes('button_mode')) {
      ...
      const family = DeviceOperatingMode.classifyOperatingFamily(this);
      // WHY(P2450): remember explicit user dimmer so applyDesiredMode won't reset kaflzta4
      try {
        const wantDimmer = newSettings.button_mode === 'dimmer' || newSettings.button_mode === 'command';
        await this.setStoreValue('p2450_user_force_dimmer', wantDimmer).catch(() => {});
      } catch (_e) { /* optional */ }
      if (!family.writeSceneAttr) {
