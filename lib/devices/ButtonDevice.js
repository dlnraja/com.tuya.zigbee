...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  /**
   * v5.5.293: Identify hybrid button+contact devices
  ...
  _isHybridButtonContactDevice(manufacturerName, productId) {
    // SOS/Emergency buttons (TS0215A series)
    const sosButtons = ['TS0215A', 'TS0215'];
    if (sosButtons.some(model => productId.includes(model))) {
      return true;
    }

    // Specific manufacturer IDs known to support both button and contact functionality
    const hybridManufacturers = [
      '_TZ3000_p6ju8myv',  // SOS button with magnetic contact
      '_TZ3000_fsiepnrh',  // Emergency button with door/window sensor
      '_TZ3000_ixla93vd',  // Multi-function button
      '_TZ3000_ja5osu5g',  // Wireless switch with contact detection
      '_TZ3400_keyjhapk',  // Smart button with contact sensor
    ];

    if (hybridManufacturers.includes(manufacturerName)) {
      this.log(`[BUTTON] 🔍 Identified hybrid device: ${manufacturerName} / ${productId}`);
      return true;
    }
