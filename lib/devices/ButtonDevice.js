...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async _registerSceneRecallListener(zclNode) {
    ...
    // Get the button_mode setting (auto/scene/dimmer)
    const setting = this.getSetting?.('button_mode') || 'auto';
  ...
  async setButtonMode(mode) {
    ...
    if (!onOffCluster) return false;

    const MODE_ATTRIBUTE = 0x8004;
    try {
      if (typeof onOffCluster.writeAttributes === 'function') {
        await onOffCluster.writeAttributes({ [MODE_ATTRIBUTE]: mode });
      } else if (typeof onOffCluster.write === 'function') {
        await onOffCluster.write({ attributeId: MODE_ATTRIBUTE, dataType: 0x30, value: mode });
      }
      this.log(`[BUTTON-MODE] ✓ Set to mode=${mode} (${mode === 1 ? 'scene' : 'dimmer'})`);
      await this.setStoreValue('button_mode', mode === 1 ? 'scene' : 'dimmer').catch(() => {});
      return true;
    } catch (err) {
      this.log(`[BUTTON-MODE] ✗ Failed to set mode=${mode}: ${err.message}`);
      return false;
    }
  }
