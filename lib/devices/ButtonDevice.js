...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async onEndDeviceAnnounce() {
    ...
    // v5.12.64 (P92.131): persistent ZCL battery listeners + boot read —
    // battery was only read on physical press (forum Peter #2072).
    try {
  ...
  async _registerButtonCapabilityListeners() {
    // v5.12.50: shared guard with TuyaZigbeeDevice universal version.
  ...
  async _tryCard(cardId, tokens = {}, state = {}) {
    try {
      ...
      // v10.5.0: dual semantics — device trigger cards (no device arg) via
      // getDeviceTriggerCard, app-level cards WITH a device arg (like the
      // generic button_pressed / button_matrix) via getTriggerCard.
      let card = null;
      try { card = this.homey.flow.getDeviceTriggerCard(cardId); } catch (_) { card = null; }
      if (typeof card?.trigger !== 'function') {
        try { card = this.homey.flow.getTriggerCard(cardId); } catch (_) { card = null; }
      }
