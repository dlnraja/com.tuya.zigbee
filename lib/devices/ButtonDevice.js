...
class ButtonDevice extends AutoAdaptiveDevice {
    ...
    for (let ep = 1; ep <= endpoints; ep++) {
      try {
        ...
        if (scenesCluster) {
          ...
          // v5.13.8: CRITICAL FIX - Verify on() method exists before registering listeners
          // Prevents "addListener argument must be function. Received undefined" error
          if (typeof scenesCluster.on === 'function') {
            ...
            this.log(`[WARN] ⚠️ scenesCluster.on() not available on EP${ep} (SDK3 limitation)`);
          }
        }
        ...
        const e000 = this.zclNode.endpoints[ep]?.clusters?.tuyaE000
  ...
  async _triggerHoldRelease(button) {
    try {
      ...
      await this.homey.flow.getDeviceTriggerCard(releaseCardId).trigger(this, {}, {}).catch(() => {});
