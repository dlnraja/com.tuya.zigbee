...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async _universalSceneModeSwitch(zclNode) {
    // Get device info from multiple sources (getData may be empty on init)
      ...
      try {
        ...
        if (onOffCluster) {
          ...
          onOffCluster.on('attr.onOff', async (value) => {
            ...
            this.log(`[ONOFF-ATTR] 📥 RECEIVED: Button ${ep} onOff: ${value}`);
            this.log(`[ONOFF-ATTR] 📊 Stats: last=${lastOnOffValue}, sinceEvent=${timeSinceLastEvent}ms, sinceInit=${timeSinceInit}ms`);
