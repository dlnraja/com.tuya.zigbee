...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async triggerButtonPress(buttonNumber, pressType = 'single', count = 1, options = {}) {
    ...
    if (snappy) {
      // Start immediately (runs until first await) — do not .then() defer.
      void runCardCascade().catch((err) => {
        try { this.error(`[BUTTON-FLOW] snappy cascade soft: ${err?.message || err}`); } catch (_e) { /* */ }
      });
      return;
    }
