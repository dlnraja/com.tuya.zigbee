...
class ButtonDevice extends AutoAdaptiveDevice {
  ...
  async setupButtonDetection() {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // v5.9.9: F5 FIX — Auto-detect 1-indexed manufacturers (Tividor _TZ3000_bgtzm4ny TS0044)
    const mfr = this._manufacturerName || this.getSetting?.('zb_manufacturer_name') || '';
    const ONE_INDEXED_MFRS = ['_TZ3000_bgtzm4ny', '_TZ3000_abrsvsou', '_TZ3000_rrjr1q0u'];
    const is1Indexed = ONE_INDEXED_MFRS.some(id => mfr.toLowerCase().includes(id.toLowerCase()));
    const autoMapping = is1Indexed ? { 1: 'single', 2: 'double', 3: 'long' } : null;
    const manufacturerConfig = this._manufacturerConfig || {};
    const customPressMapping = autoMapping || manufacturerConfig.pressTypeMapping;
