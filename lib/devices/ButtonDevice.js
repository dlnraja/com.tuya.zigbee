  ...
  async onEndDeviceAnnounce() {
    ...
    for (let ep = 1; ep <= endpoints; ep++) {
      ...
      const clustersToBind = [
        endpoint.clusters?.scenes,
        endpoint.clusters?.onOff,
        endpoint.clusters?.multistateInput || endpoint.clusters?.genMultistateInput,
        endpoint.clusters?.levelControl || endpoint.clusters?.genLevelCtrl
      ];

      for (const cluster of clustersToBind) {
        if (cluster && typeof cluster.bind === 'function') {
          cluster.bind()
            .then(() => this.log(`[BIND] ✅ Re-bound ${cluster.constructor.name || 'cluster'} on EP${ep}`))
            .catch(err => this.log(`[BIND] ⚠️ Re-bind failed on EP${ep}:`, err.message));
        }
      }
    }
      ...
      const card = (app && typeof app._safeGetTriggerCard === 'function') 
        ...
        : await (async () => { 
            try { 
              ...
              // Fallback for drivers still using getDeviceTriggerCard pattern
              try {
