...
class UnifiedSensorBase extends TuyaZigbeeDevice {
  ...
  get dpMappings() {
    if (this._dpMappings) return this._dpMappings;
    if (this._dynamicDpMappings) return this._dynamicDpMappings;
    return {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      this.universalDataHandler.on('capability', async (capability, value) => {
  ...
  _setupRawFrameFallback() {
    ...
    this.node.handleFrame = (endpointId, clusterId, frame, meta) => {
      // Destruction guard — skip processing if device is destroyed
  ...
  async onUninit() {
    ...
    // Clear capability status timer (30-min diagnostic report)
    if (this._capabilityStatusTimer) {
      this.homey.clearTimeout(this._capabilityStatusTimer);
      this._capabilityStatusTimer = null;
      this.log('[CLEANUP] ✓ Cleared capability status timer');
    }

    // Clean up device learner
    if (this._deviceLearner) {
      try { this._deviceLearner.destroy?.() || this._deviceLearner.cleanup?.(); } catch(e) {}
      this._deviceLearner = null;
      this.log('[CLEANUP] ✓ Cleaned up device learner');
    }
  ...
  async _sendDataQueryOnWake() {
    // Destruction guard
    if (this._destroyed || this.destroyed) return;
   ...
   */
  async _handleBatteryDP(dp, value) {
   ...
   */
  async _scheduleCapabilityStatusLog() {
  ...
  _checkMissingCapabilities() {
    const missing = [];
    const capsToCheck = ['measure_temperature', 'measure_humidity', 'measure_battery', 'alarm_contact', 'alarm_motion', 'measure_pressure', 'measure_luminance'];
  ...
  async _handleIASZoneStatus(status) {
    ...
    if (primaryCapability === 'alarm_motion') {
      ...
      await this.setCapabilityValue(primaryCapability, alarmValue).finally(() => {
      ...
      for (const cap of fallbackCaps) {
        if (this.hasCapability(cap)) {
          ...
          await this.setCapabilityValue(cap, value).finally(() => {
    ...
    if (this.hasCapability('alarm_battery') && battery) {
      await this.setCapabilityValue('alarm_battery', battery);
    }
  }

  // v5.8.3: Motion holdoff (Forum #1335 FinnKje)
  async _handleMotionWithHoldoff(motionDetected) {
  ...
  async _sendTuyaDataQuery() {
    ...
    try {
      ...
      // Method 1: Raw frame (most reliable)
      const seq = Date.now() % 65535;
      try {
        if (typeof endpoint.sendFrame === 'function') {
          const frame = Buffer.from([
            0x00, (seq >> 8) & 0xFF, seq & 0xFF,
            0x03, // dataQuery command
            0x00, 0x02, (seq >> 8) & 0xFF, seq & 0xFF
          ]);
          await endpoint.sendFrame(0xEF00, frame, 0x01).catch(() => { });
        }
      } catch (e) { /* fallback below */ }

      // Method 2: Cluster dataQuery (no args)
      ...
      // Method 3: Direct DP requests (fallback)
  ...
  async _handleDP(dpId, rawValue) {
    ...
    // v8.1.151: CRITICAL — Check forceActiveTuyaMode
    // Battery devices MUST keep Tuya DP mode active even after learning phase
    // Without this check, the device stops sending DPs after pairing
    // NOTE: this.Mode may not exist in UnifiedSensorBase (only in TuyaUnifiedDevice)
    // So we check if Mode exists AND if tuyaActive is true
    const forceActive = this.forceActiveTuyaMode === true;
    const isLearningPhase = !this.DecisionMade;
    const modeActive = this.Mode ? this.Mode.tuyaActive : true; // Default true if Mode not initialized

    if (!modeActive && !isLearningPhase && !forceActive) {
    ...
    if (!mapping) {
      ...
      if (autoMapping) {
        ...
        await this.setCapabilityValue(autoMapping.capability, finalValue);
        return;
      }
      ...
      if (inferredCap) {
        ...
        if (!this.hasCapability(inferredCap.capability)) {
          await this._addCapabilityPermissive(inferredCap.capability).catch(() => {});
        }
        await this.setCapabilityValue(inferredCap.capability, inferredCap.value);
      ...
      if (this._discovery) {
        ...
        if (discovered && discovered.confidence >= 80) {
          ...
          await this.setCapabilityValue(discovered.capability, discovered.value);
          return;
        }
      }
      return;
    ...
    if (mapping.alsoSets && typeof mapping.alsoSets === 'object') {
      ...
      for (const [alsoCap, alsoTransform] of Object.entries(mapping.alsoSets)) {
        if (this.hasCapability(alsoCap)) {
          ...
          await this.setCapabilityValue(alsoCap, alsoValue);
        }
      }
    }
  }
  ...
  async _handleCommonDP(dp, rawValue) {
    ...
    if ((dp === 13 || dp === 101) && this.hasCapability('onoff.led')) {
      ...
      await this.setCapabilityValue('onoff.led', ledOn).catch(() => { });
    }
    ...
    // v8.1.96: Skip battery handling if DP is already mapped to a capability (fixes #328 bed_sensor DP104 conflict)
    if ([4, 10, 14, 15, 21, 100, 101, 102, 104, 105].includes(dp) && this.hasCapability('measure_battery') && !dpIsMapped) {
    ...
    if (dp === 9 && this.hasCapability('measure_luminance.distance')) {
      ...
      await this.setCapabilityValue('measure_luminance.distance', parseFloat(distance)).catch(() => { });
    }
  }
   ...
   */
  async _handleZCLData(cluster, attr, value) {
    // Track protocol stats
    ...
    if (capability) {
      ...
      // v8.1.131: ZCL illuminance log-scale conversion
      // ZCL formula: measuredValue = 10000 * log10(lux) + 1
      // Inverse: lux = 10^((measuredValue - 1) / 10000)
      if (capability === 'measure_luminance' && typeof finalValue === 'number' && finalValue > 1) {
        finalValue = Math.round(Math.pow(10, (finalValue - 1) / 10000));
        this.log(`[ZCL-DATA] Illuminance converted: ZCL→${finalValue} lux`);
      }
  ...
  async _requestInitialDPs() {
    ...
    try {
      ...
      if (typeof this.tuyaEF00Manager.requestDP === 'function') {
        for (const dp of dps) {
          await this.tuyaEF00Manager.requestDP(dp).catch(() => {});
        }
      }
  ...
  async _setupZCLCluster(clusters, clusterNames, capability, transform, attribute = 'measuredValue') {
    ...
    try {
      ...
      cluster.on(attrEvent, async (value) => {
        // Track protocol stats
        if (this._protocolStats?.zcl) {
          this._protocolStats.zcl.received++;
          this._protocolStats.zcl.lastTime = Date.now();
        ...
        await this.setCapabilityValue(capability, transformed);
      });
