...
class UnifiedSensorBase extends TuyaZigbeeDevice {
  ...
  /**
   * v10.7.0 (z2m #28987): manufacturers whose humidity cluster reports
  ...
  async onNodeInit({ zclNode }) {
    // v5.12.60 (P92.128): a crash in the base init must NOT kill sensor
  ...
  async onEndDeviceAnnounce() {
    ...
    // no-op at best and can mark the device unreachable (Peter water leak).
    ...
    if (hasEf00) {
      try {
        ...
        this.log('[SENSOR-WAKE] dataQuery failed:', e.message);
      }
    } else {
    ...
    if (this._sendTimeSync) {
      try {
        ...
        this.log('[SENSOR-WAKE] Time sync failed:', e.message);
      }
    }
   ...
   * WHY: Peter #2183 — after app update, Homey drops IAS listeners.
   */
  ...
  _handleIASZoneStatus(status) {
    ...
    if (primaryCapability === 'alarm_water') {
      ...
      const needsDefaultInvert = waterInvertedByDefaultMfrs.some(id =>
        normalizeCI(mfr).includes(normalizeCI(id))
      );
  ...
  async _setupZCLMode(zclNode) {
    ...
    if (!customHandlers.relativeHumidity && !customHandlers.msRelativeHumidity) {
      await this._setupZCLCluster(clusters,
        ...
        'measure_humidity',
        // v10.7.0 (z2m #28987): TS0201-clone firmwares report humidity as
        // raw% ×10 in a field that is 0.01%-units per spec — a 58% reading
        // arrives as 580 and /100 shows 5.8%. Curated mfr list, same pattern
        // as BATTERY_0_50_SCALE_MFRS.
        (value) => {
          const X10 = UnifiedSensorBase.ZCL_HUMIDITY_X10_MFRS;
          if (X10 && X10.has(String(this.getSetting?.('zb_manufacturer_name') || '').toLowerCase())) {
    ...
    if (!customHandlers.illuminanceMeasurement && !customHandlers.msIlluminanceMeasurement) {
      await this._setupZCLCluster(clusters,
        ...
        // (forum Peter_van_Werkhoven #2114: 551 shown vs 1153 real).
        (value) => this._applyLuxCalibration(Math.round(Math.pow(10, (value - 1) / 10000)))
      );
    }
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    const SETTING_DP_MAP = {
      ...
      // v5.12.63 (P92.130): canonical Tuya mmWave radar settings
