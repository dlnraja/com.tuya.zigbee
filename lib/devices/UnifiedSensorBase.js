...
class UnifiedSensorBase extends TuyaZigbeeDevice {
  ...
  _handleIASZoneStatus(status) {
    ...
    if (primaryCapability === 'alarm_motion') {
      ...
      this.setCapabilityValue(primaryCapability, alarmValue).finally(() => {
      ...
      for (const cap of fallbackCaps) {
        if (this.hasCapability(cap)) {
          ...
          this.setCapabilityValue(cap, value).finally(() => {
    ...
    if (this.hasCapability('alarm_tamper')) {
      this.setCapabilityValue('alarm_tamper', tamper).catch(() => {});
    }

    // Battery low is universal
    if (this.hasCapability('alarm_battery') && battery) {
      this.setCapabilityValue('alarm_battery', battery).catch(() => {});
    }
  }
  ...
  _handleMotionWithHoldoff(motionDetected) {
    ...
    if (holdoffMs <= 0) { this.setCapabilityValue('alarm_motion', false).catch(() => {}); return; }
  ...
  _handleDP(dpId, rawValue) {
    ...
    if (!mapping) {
      ...
      if (autoMapping) {
        ...
        this.setCapabilityValue(autoMapping.capability, finalValue).catch(() => {});
        return;
      }
      ...
      if (inferredCap) {
        ...
        if (!this.hasCapability(inferredCap.capability)) {
          this._addCapabilityPermissive(inferredCap.capability).then(() => {
            this.setCapabilityValue(inferredCap.capability, inferredCap.value).catch(() => {});
          }).catch(() => {});
        } else {
          this.setCapabilityValue(inferredCap.capability, inferredCap.value).catch(() => {});
        }
    ...
    if (mapping.alsoSets && typeof mapping.alsoSets === 'object') {
      ...
      for (const [alsoCap, alsoTransform] of Object.entries(mapping.alsoSets)) {
        if (this.hasCapability(alsoCap)) {
          ...
          this.setCapabilityValue(alsoCap, alsoValue).catch(() => {});
        }
      }
    }
  }
  ...
  _handleZCLData(cluster, attr, value) {
    ...
    if (capability) {
      ...
      this.setCapabilityValue(capability, finalValue).catch(() => {});
    }
  }
  ...
  async _setupZCLCluster(clusters, clusterNames, capability, transform, attribute = 'measuredValue') {
    ...
    try {
      ...
      cluster.on(attrEvent, (value) => {
        ...
        this.setCapabilityValue(capability, transformed).catch(() => {});
      });
