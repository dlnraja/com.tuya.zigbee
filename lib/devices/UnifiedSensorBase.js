...
const { readAttributesSmart, configureReportingSmart } = require('../zigbee/UnsupportedRegistry');
...
const { DataQuery, ZigbeeDataQuery } = require('../zigbee/ZigbeeDataQuery');
...
const SDK3_COMPLIANCE = {
  ...
  // P2547: keep delay aligned with Sdk3ZclSafe (Athom SDK3 docs)
  INIT_COMMUNICATION_DELAY_MS: (() => {
    try { return require('../utils/Sdk3ZclSafe').INIT_COMMUNICATION_DELAY_MS || 2000; } catch (_e) { return 2000; }
  })(),
 ...
 * ║  - Auto-detection of protocol (Tuya DP / ZCL /)                       ║
 * ║                                                                              ║
 ...
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
class UnifiedSensorBase extends TuyaZigbeeDevice {

  /**
   * v10.7.0 (z2m #28987 / GH #516): manufacturers whose humidity cluster reports
   * raw% ×10 in the 0.01%-units field (TS0201-clone firmwares).
   * Keys are LOWERCASE manufacturerName.
   * Example: measuredValue=580 means 58% → divide by 10 (not 100).
   */
  static ZCL_HUMIDITY_X10_MFRS = new Set([
    '_tz3000_isw9u95y',
    '_tz3000_xr3htd96',
    '_tz3000_zl1kmjqx',
    '_tz3000_yupc0pb7',
  ...
  /** Resolve mfr for ZCL humidity scale (settings → store → data → node). */
  _resolveZclHumidityMfr() {
    try {
      const MfrHelper = require('../helpers/ManufacturerNameHelper');
  ...
  /**
   * Scale ZCL relativeHumidity measuredValue → Homey %RH.
   * P127: shared by listener + DATA-RETRY so GH #516 cannot regress via /100 path.
   */
  _scaleZclHumidity(raw) {
    const n = Number(raw);
    if (!Number.isFinite(n)) return null;
    const X10 = UnifiedSensorBase.ZCL_HUMIDITY_X10_MFRS;
    if (X10 && X10.has(this._resolveZclHumidityMfr())) {
      return Math.round(n / 10);
    }
    // Adaptive fallback: values in 0–1000 with typical indoor RH look like ×10 quirk
    if (n > 0 && n <= 1000 && n % 1 === 0) {
      const asStd = n / 100;
      if (asStd > 0 && asStd < 15 && n >= 100) {
        // 580 → 5.8% is implausible indoor RH; prefer /10 → 58%
        return Math.round(n / 10);
      }
    }
  ...
  async onNodeInit({ zclNode }) {
    // v9.0.420 (P92.128): a crash in the base init must NOT kill sensor
    // init — a partially initialized sensor must still report. The base
    // registers the universal button listeners; on failure we continue
    // with our own minimal setup so the device keeps working.
    if (super.onNodeInit) {
      try {
        await super.onNodeInit({ zclNode });
      } catch (e) {
        this.error(`[HYBRID-SENSOR] ⚠️ base onNodeInit failed (continuing with partial init): ${e.message}`);
        this.zclNode = this.zclNode || zclNode;
      }
    }
    ...
    // v8.0.0: INTELLIGENT DP AUTO-DISCOVERY
    ...
    if (needsReportingConfig) {
      ...
      await this._safeInvoke(async () => await this._configureAttributeReportingSDK3(zclNode), 'configureAttributeReportingSDK3');
    ...
    // v6.0: Intelligent Power Source Detection (Replaced with BatteryHybridManager)
    this.log('');
    this.log('🔄 STEP 2.1: Initializing Battery Hybrid Manager...');
    try {
    ...
    try {
      ...
      this.universalDataHandler.on('capability', (capability, value) => {
        ...
        this.safeSetCapabilityValue(capability, value).catch(() => {});
      });
    ...
    this._optimizationTimer = safeSetTimeout(this, () => {
  ...
  async _fastInit(zclNode) {
    ...
    safeSetTimeout(this, async () => { if (this._destroyed) {return;} this.log('[FAST-INIT] ⏳ Starting deferred initialization...');
      try {
        // Manufacturer config
        await this._applyManufacturerConfig?.();
   ...
   * WHY(P2284): delegate to TuyaZigbeeDevice L0 (wrapHandleFrame chain).
   * Contre quoi: blind overwrite that orphaned Physical/IO/IAS siblings.
   */
  _setupRawFrameFallback() {
    return super._setupRawFrameFallback();
  }
  ...
  async onUninit() {
    ...
    if (this._optimizationTimer) {
      safeClearTimeout(this, this._optimizationTimer);
    ...
    if (this._delayedRetryTimer) {
      safeClearTimeout(this, this._delayedRetryTimer);
    ...
    if (this._motionAlarmTimeout) {
      safeClearTimeout(this, this._motionAlarmTimeout);
      this._motionAlarmTimeout = null;
      this.log('[CLEANUP] ✓ Cleared motion alarm timeout');
    }

    // v9.0.40: Clear keep-alive timer for occupancy sensors
    if (this._keepAliveTimer) {
      (this.homey?.clearTimeout ? this.homey.clearTimeout.bind(this.homey) : globalThis.clearTimeout)(this._keepAliveTimer);
      this._keepAliveTimer = null;
      this.log('[CLEANUP] ✓ Cleared keep-alive timer');
    }

    // Clear suppress timeout (for occupancy sensors)
    if (this._suppressTimeout) {
      safeClearTimeout(this, this._suppressTimeout);
    ...
    if (this.DataQuery) {
      this.DataQuery = null;
      this.log('[CLEANUP] ✓ Cleaned up Data Query');
    }
  ...
  async onEndDeviceAnnounce() {
    ...
    // WHY: IAS / ZCL sleepy sensors have no EF00. dataQuery on wake is a
    // no-op at best and can mark the device unreachable (water leak went
    // silent after the first successful report).
    ...
    if (hasEf00) {
      ...
      this.log('[SENSOR-WAKE] no EF00 — skip dataQuery (IAS/ZCL sleepy)');
    }

    try {
      await this._reattachIasOnWake();
    } catch (e) {
      this.log('[SENSOR-WAKE] IAS re-attach failed:', e.message);
    }
  ...
  async _sendDataQueryOnWake() {
    ...
    if (dpIds.length > 0) {
      ...
      for (const dpId of dpIds) {
        ...
        await new Promise(r => safeSetTimeout(this, r, 100)); // Small delay between requests
      }
  ...
  _handleBatteryDP(dp, value) {
    // v6.1.1: Normalize high-DP battery reporting (ignore phantom 0%)
    // Many Tuya devices (esp. radar/presence) report 0% on DP101/110/121 before reading actual capacity
    if (value === 0 && (dp > 100 || !this._batteryReceivedOnce)) {
      this.log(`[BATTERY] ⚠️ Ignoring phantom 0% report from DP${dp} (initial or high-DP)`);
      return;
    }

    // v5.5.793: Use VALIDATION constants
    if (typeof value === 'number' && value >= VALIDATION.BATTERY_MIN && value <= VALIDATION.BATTERY_MAX) {
      this._batteryReceivedOnce = true;
      ...
      // P2689 — dynamic throttle (finer when SOC low); fallback to TIMING constants
      let accept = batteryChange >= TIMING.BATTERY_SIGNIFICANT_CHANGE
        || timeSinceLastReport > TIMING.BATTERY_THROTTLE_MS
        || lastBatteryReport === 0;
      try {
        const { shouldAcceptBatterySample } = require('../battery/SmartBatteryAdaptivePrecision');
        accept = shouldAcceptBatterySample({
          prev: current,
          next: value,
          lastTs: lastBatteryReport,
          now,
        }).accept;
      } catch (_e) { /* soft */ }
      ...
      if (accept) {
        ...
        this.safeSetCapabilityValue('measure_battery', parseFloat(value)).catch(() => { });
  ...
  async _initHybridDataQuery() {
    try {
      const { DataQuery } = require('../zigbee/ZigbeeDataQuery');
      this.DataQuery = new DataQuery(this);
      await this.DataQuery.initialize();
      this.log('[HYBRID-QUERY] ✅DataQuery initialized');
  ...
  _scheduleCapabilityStatusLog() {
    this._capabilityStatusTimer = safeSetTimeout(this, async () => {
      if (this._destroyed) {return;}
      this.log('');
      this.log('═══════════════════════════════════════════════════════════════');
      ...
      for (const cap of caps) {
        ...
        if (value === null || value === undefined) {
          if (cap.startsWith('tuya_dp_') || cap === 'tuya_cluster_event') {continue;}
          if (cap.startsWith('button.') && !String(this.driver?.id || '').includes('button')) {continue;}
          missing.push(cap);
        } else {
      ...
      if (this.DataQuery?.zigbeeQuery) {
        await this.DataQuery.zigbeeQuery.logClusterInfo();
      }

      // If still missing data, force query
      if (missing.length > 0) {
        const skipForce = this._skipTuyaDataQuery || this._iasOnlyProfile
          || this._deviceProfile?.type === 'ias_zone';
        if (skipForce) {
          this.log('🔄 Skip force query (IAS-only / no EF00)');
        } else {
          this.log('🔄 Forcing query for missing data...');
          await this.DataQuery?.forceQuery?.().catch(() => { });
        }
      }
  ...
  _setupDelayedDataRetry() {
    ...
    this._fastRetryTimer = safeSetTimeout(this, async () => {
      if (this._destroyed) {return;}
    ...
    this._dataRetryTimer = safeSetTimeout(this, async () => {
      if (this._destroyed) {return;}
  ...
  async _readZCLAttributes() {
    ...
    // Humidity (P127: same X10 scale as ZCL listener — GH #516)
    const humCluster = endpoint.clusters?.relativeHumidityMeasurement
      || endpoint.clusters?.relativeHumidity
      || endpoint.clusters?.msRelativeHumidity;
    if (humCluster?.readAttributes) {
      try {
        const data = await humCluster.readAttributes(['measuredValue']);
        if (data?.measuredValue != null) {
          const hum = this._scaleZclHumidity(data.measuredValue);
          if (hum != null) {
            this.log(`[DATA-RETRY] 💧 Got humidity: ${hum}% (raw=${data.measuredValue})`);
  ...
  async _readBatteryWithFallback(endpoint) {
    ...
    try {
      ...
      if (result?.batteryPercentageRemaining !== undefined && result.batteryPercentageRemaining !== 255) {
        const percent = normalizeZclBatteryPercent(
          result.batteryPercentageRemaining,
          { manufacturer: manufacturerOf(this) },
        );
    ...
    try {
      ...
      if (allAttrs?.batteryPercentageRemaining !== undefined && allAttrs.batteryPercentageRemaining !== 255) {
        const percent = normalizeZclBatteryPercent(
          allAttrs.batteryPercentageRemaining,
          { manufacturer: manufacturerOf(this) },
        );
        if (percent == null) {return null;}
        await this.safeSetCapabilityValue('measure_battery', parseFloat(percent)).catch(() => { });
  ...
  _scheduleSecondRetry() {
    ...
    this._secondRetryTimer = safeSetTimeout(this, async () => {
      if (this._destroyed) {return;}
  ...
  async _setupIASZoneListeners(zclNode) {
    ...
    try {
      ...
      iasCluster.onZoneEnrollRequest = async (data) => {
        ...
        try {
          // Respond with successful enrollment. zoneId 10 = Homey/Peter pattern
          // (zoneId 0/weak enrollment → stuck "Laatste waarde onbekend", no alarms).
      ...
      try {
        if (typeof iasCluster.readAttributes === 'function') {
          ...
          if (isNullCie) {
            ...
            // zoneId MUST be 10 (Homey / IASZoneManager / SOS Peter #2134).
            // zoneId 1 left water/contact sensors enrolled-but-silent.
          const sleepy = this.hasCapability?.('measure_battery') && this.mainsPowered !== true;
          if (sleepy) {
            // WHY: Peter #2183 contact/water — 10s CIE poll at boot times out
            // and blocks onNodeInit. Listeners are already on; enroll on announce.
            this._iasEnrollmentPending = true;
            this.log('[IAS] sleepy — skip 10s CIE poll; retry on announce');
          } else {
            const enrolled = await ieeeManager.fullEnrollmentFlow({ zoneId: 10, maxWait: 10000 }).catch(() => false);
            ...
            if (enrolled) {
              ...
              if (!isTuyaDP) {
                ...
                this.log('[IAS] ⚠️ device - IAS enrollment failed, DP may not carry contact state');
      ...
      const sleepyBind = this.hasCapability?.('measure_battery') && this.mainsPowered !== true;
      if (!sleepyBind) {
        try {
        ...
        this.log('[IAS] sleepy — skip boot IAS bind (retry on announce)');
      }

      // Try to read current zone status on init (mains only — sleepy times out)
      if (!sleepyBind && typeof iasCluster.readAttributes === 'function') {
        try {
    ...
    // Listen for zone status changes (once — re-attach on wake must not stack)
    if (!this._iasZoneListenersReady && typeof iasCluster.on === 'function') {
    ...
    this._iasZoneListenersReady = true;
  }

  /**
   * Re-attach IAS listeners and send enroll on sleepy wake.
   * WHY: Peter #2183 — after app update, Homey drops IAS listeners; contact
   * overrode announce without re-enroll so windows never fired Flows.
   * Do not stack .on() listeners; do not poll zoneState here.
   */
  async _reattachIasOnWake() {
    const endpoint = this.zclNode?.endpoints?.[1];
    ...
    if (!this._iasZoneListenersReady) {
      await this._setupIASZoneListeners(this.zclNode);
      return true;
    }

    try {
      iasCluster.onZoneEnrollRequest = async (data) => {
        this.log('[IAS] 📥 Zone enroll request on wake:', data);
        await iasCluster.zoneEnrollResponse({
      ...
      if (typeof iasCluster.zoneEnrollResponse === 'function') {
        ...
        this._iasEnrollmentPending = false;
        this.log('[IAS] ✅ Wake enroll response sent (zoneId 10)');
      }
      // Restore bind + short zoneStatus read while the node is actually awake.
      const BootBudget = require('../performance/BootBudget');
      if (endpoint.bind && BootBudget.shouldTxSleepy(this)) {
        await endpoint.bind('iasZone').catch(() => {});
      }
      if (typeof iasCluster.readAttributes === 'function' && BootBudget.shouldTxSleepy(this)) {
        const attrs = await Promise.race([
          iasCluster.readAttributes(['zoneStatus']),
          new Promise((_, rej) => this.homey.setTimeout(() => rej(new Error('timeout')), 1500))
        ]).catch(() => null);
        if (attrs?.zoneStatus !== undefined) {
          this._handleIASZoneStatus(attrs.zoneStatus);
        }
      ...
      this._iasEnrollmentPending = true;
      this.log('[IAS] wake enroll deferred:', err.message);
    }
    return true;
  }
  ...
  _handleIASZoneStatus(status) {
    try {
      require('../performance/BootBudget').markRadioActivity(this);
    } catch (_e) { /* keep IAS path */ }
    ...
    // Peter #2184: never parseInt(object)→0 (logged 0x[object Object], overlay DP1)
    if (typeof statusValue !== 'number' || !Number.isFinite(statusValue)) {
      const fromNamed = status && typeof status === 'object'
        && ('alarm1' in status || 'alarm2' in status);
      const fromGet = status && typeof status.get === 'function'
        ? { a1: status.get('alarm1'), a2: status.get('alarm2') }
        : null;
      if (fromNamed) {
        statusValue = (status.alarm1 ? 1 : 0) | (status.alarm2 ? 2 : 0)
          | (status.tamper ? 4 : 0)
          | ((status.batteryLow || status.battery) ? 8 : 0);
        this.log(`[IAS] 🔄 Bitmap bits → ${statusValue}`);
      } else if (fromGet && (fromGet.a1 !== undefined || fromGet.a2 !== undefined)) {
        statusValue = (fromGet.a1 ? 1 : 0) | (fromGet.a2 ? 2 : 0);
        this.log(`[IAS] 🔄 Bitmap get() bits → ${statusValue}`);
      } else {
        this.log(`[IAS] Ignoring invalid zoneStatus (${Object.prototype.toString.call(status)}) — do not overlay DP1`);
        return;
      }
    }
    ...
    if (primaryCapability === 'alarm_motion') {
      const { applyPolarity, observeRaw } = require('../managers/AlarmPolarityManager');
      observeRaw(this, alarmValue, 'motion');
      const { value, meta } = applyPolarity(this, alarmValue, 'motion');
      if (value !== alarmValue) {
        this.log(`[IAS] 🔄 Motion polarity (${meta.reason}): ${alarmValue} → ${value}`);
      }
      alarmValue = value;
    }
    
    // ═══════════════════════════════════════════════════════════════════════
    // Water / contact / SOS polarity — AlarmPolarityManager (lists + XOR + smart)
    // ═══════════════════════════════════════════════════════════════════════
    if (primaryCapability === 'alarm_water') {
      const { applyPolarity, observeRaw } = require('../managers/AlarmPolarityManager');
      observeRaw(this, alarmValue, 'water');
      const { value, meta } = applyPolarity(this, alarmValue, 'water');
      if (value !== alarmValue) {
        this.log(`[IAS] 🔄 Water polarity (${meta.reason}): ${alarmValue} → ${value}`);
      }
      alarmValue = value;
    }
    ...
    if (primaryCapability === 'alarm_contact') {
      const { applyPolarity, observeRaw } = require('../managers/AlarmPolarityManager');
      observeRaw(this, alarmValue, 'contact');
      const { value, meta } = applyPolarity(this, alarmValue, 'contact');
      if (value !== alarmValue) {
        this.log(`[IAS] 🔄 Contact polarity (${meta.reason}): ${alarmValue} → ${value}`);
      }
      alarmValue = value;
    }

    if (primaryCapability === 'alarm_sos' || primaryCapability === 'alarm_generic') {
      const { applyPolarity, observeRaw } = require('../managers/AlarmPolarityManager');
      observeRaw(this, alarmValue, 'sos');
      const { value, meta } = applyPolarity(this, alarmValue, 'sos');
      if (value !== alarmValue) {
        this.log(`[IAS] 🔄 SOS polarity (${meta.reason}): ${alarmValue} → ${value}`);
      }
      alarmValue = value;
    }

    // (legacy inline water/contact invert blocks replaced by AlarmPolarityManager above)
    ...
    if (primaryCapability === 'alarm_motion') {
      ...
      this.safeSetCapabilityValue(primaryCapability, alarmValue).finally(() => {
        // v5.12.1: Reset IAS origin flags after setCapabilityValue — prevents stale flag
      ...
      for (const cap of fallbackCaps) {
        if (this.hasCapability(cap)) {
          ...
          if (cap === 'alarm_contact') {this._iasOriginatedAlarm = true;}
          if (cap === 'alarm_water') {this._iasOriginatedWaterAlarm = true;}
          this.safeSetCapabilityValue(cap, value).finally(() => {
            if (cap === 'alarm_contact') {this._iasOriginatedAlarm = false;}
            if (cap === 'alarm_water') {this._iasOriginatedWaterAlarm = false;}
          });
    ...
    if (this.hasCapability('alarm_tamper')) {
      this.safeSetCapabilityValue('alarm_tamper', tamper).catch(() => {});
    }

    // Battery low is universal
    if (this.hasCapability('alarm_battery') && battery) {
      this.safeSetCapabilityValue('alarm_battery', battery).catch(() => {});
    }
  }

  // v5.8.3: Motion holdoff (Forum #1335 FinnKje)
  // v9.0.40: Keep-alive timeout prevents occupancy sensors from getting stuck in "detected" state
  _handleMotionWithHoldoff(motionDetected) {
    ...
    if (motionDetected) {
      ...
      // v9.0.40: Keep-alive timeout - auto-clear alarm_motion if no further reports arrive
      // Prevents sensors from getting permanently stuck in "detected" state
      const keepAliveMs = (this.getSetting?.('keep_alive_timeout') || 90) * 1000;
      if (this._keepAliveTimer) {clearTimeout(this._keepAliveTimer);}
      this._keepAliveTimer = (this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout)(() => {
        if (this._destroyed) {return;}
        this.log('[MOTION] ⏰ Keep-alive timeout - auto-clearing stale motion');
    ...
    if (holdoffMs <= 0) { this.safeSetCapabilityValue('alarm_motion', false).catch(() => {}); return; }
    if (this._motionHoldoffTimer) {return;} // Already pending
    this.log(`[MOTION] ⏰ No motion - holdoff ${holdoffMs/1000}s before clearing`);
    this._motionHoldoffTimer = (this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout)(() => {
      if (this._destroyed) {return;}
      this._motionHoldoffTimer = null;
      this.safeSetCapabilityValue('alarm_motion', false).catch(() => {});
  ...
  _detectProtocol() {
    try {
      ...
      const info = applyIntelligentProtocol(this, this.zclNode);
      if (info.hasTuyaCluster) {
        this.log('[PROTOCOL] ✅ Tuya cluster (0xEF00) detected');
      }
      this.log(`[PROTOCOL] ${info.protocol} (${info.reason}) hybrid=${!!info.listenHybrid}`);
      return info;
    } catch (err) {
      ...
      return {
        ...
        modelId: '',
        mfr: '',
        reason: 'detect_error_fallback_hybrid',
      };
    }
  }
  ...
  async _setupTuyaDPMode() {
    ...
    try {
      ...
      if (initialized) {
        ...
        const wakeOk = await this._sendTuyaDataQuery().catch(e => {
          this.log('[TUYA-DP] ⚠️ Initial dataQuery skipped:', e.message);
          return false;
        });

        // Periodic dataQuery only when the Tuya cluster is actually present.
        // Peter #2183 logged "Tuya cluster not found" every 30 min on IAS windows.
        // Restore periodic query for battery Tuya devices. Interval still
        // fires, but each tick no-ops unless the node is awake and heap is OK.
        // WHY(P2601 / GH#550): mains ceiling radars with forceActiveTuyaMode also need
        // periodic EF00 query — otherwise lux floods while DP1/9 stay cold forever.
        if (wakeOk && (!this.mainsPowered || this.forceActiveTuyaMode === true)) {
  ...
  async _writeZclTimeAttribute() {
    try {
      ...
      try {
        const { shouldProactiveTimeSync } = require('../zigbee/TimeClusterPolicy');
        if (!shouldProactiveTimeSync(this)) {
          this.log('[TIME-SYNC] Skip proactive 0x000A write (sleepy remote/sensor)');
          return;
        }
      } catch (_e) { /* policy optional */ }
  ...
  async _setupLowLevelNodeHandler() {
    ...
    try {
      ...
      // WHY(P2284): append-only wrap — never orphan Physical/IAS/L0 siblings
      const device = this;
      const { wrapHandleFrame } = require('../utils/BidirectionalButtonState');
      wrapHandleFrame(node, 'unified-sensor-p0-ef00', (args, next) => {
        ...
        try {
          ...
          device.log(`[TUYA-P0] wrap error: ${e.message}`);
        }
        return next(...args);
      });
  ...
  _parseTuyaRawFrameFromNode(frame) {
    try {
      ...
      if (fcBits === 0x01 && cmdId === 0x11 && frame.length >= 6) {
        ...
        try { this.setSettings({ mcu_version: `${ma}.${mi}.${pa}` }).catch(() => {}); } catch (e) { /* Settings update failed */ }
        return;
      }
  ...
  async _sendTuyaDataQuery() {
    try {
      ...
      if (!tuyaCluster) {
        if (!this._tuyaWakeClusterMissingLogged) {
          this.log('[TUYA-WAKE] Tuya cluster not found — skip periodic wake');
          this._tuyaWakeClusterMissingLogged = true;
        }
        return false;
      }
  ...
  _startPeriodicDataQuery() {
    ...
    if (this._dataQueryInterval) {
      this.homey.clearInterval(this._dataQueryInterval);
    }

    // Query every 30 minutes (Tuya sync interval)
    const intervalMs = 30 * 60 * 1000;
    this._dataQueryInterval = this.homey.setInterval(() => {
      if (this._destroyed) {return;}
      try {
        const BootBudget = require('../performance/BootBudget');
        if (BootBudget.isHeapCritical()) {return;}
        if (!this.mainsPowered && !BootBudget.isRecentlyAwake(this)) {return;}
      } catch (_e) { /* send anyway if helper missing */ }
      this._sendTuyaDataQuery().catch(() => { });
    }, intervalMs);
  ...
  _handleDP(dpId, rawValue) {
    ...
    try {
      require('../performance/BootBudget').noteSeenDp(this, dp);
    } catch (_e) { /* keep DP path even if budget helper fails */ }
    ...
    // v5.5.904: Enhanced diagnostic logging (same asSwitchBase)
    ...
    if (!mapping) {
      ...
      if (autoMapping) {
        ...
        this.safeSetCapabilityValue(autoMapping.capability, finalValue).catch(() => {});
        return;
      }
      ...
      // v9.0.259 (P63): Defensive wrap — 7x "this._inferCapabilityFromValue is
      // not a function" crashes seen in Gmail diagnostics. Some subclasses
      // or refactors lose the method from the prototype chain. Skip permissive
      // inference cleanly instead of throwing.
      // ═══════════════════════════════════════════════════════════════════════
      const parsedVal = this._parseValue(rawValue);
      let inferredCap = null;
      try {
        if (typeof this._inferCapabilityFromValue === 'function') {
          inferredCap = this._inferCapabilityFromValue(dp, parsedVal);
        }
      } catch (e) {
        // Best-effort — never let permissive mode crash the device
        if (this.log) {this.log(`[DP-PERMISSIVE] _inferCapabilityFromValue threw: ${e.message}`);}
      }
      ...
      if (inferredCap) {
        ...
        if (!this.hasCapability(inferredCap.capability)) {
          this._addCapabilityPermissive(inferredCap.capability).then(() => {
            this.safeSetCapabilityValue(inferredCap.capability, inferredCap.value).catch(() => {});
          }).catch(() => {});
        } else {
          this.safeSetCapabilityValue(inferredCap.capability, inferredCap.value).catch(() => {});
        }
      ...
      // v8.0.0: INTELLIGENT DP AUTO-DISCOVERY FALLBACK
      if (this._discovery) {
        this._discovery.analyzeDP(dp, rawValue);
        const discovered = this._discovery.applyDiscoveredValue(dp, rawValue);
        if (discovered && discovered.confidence >= 80) {
          this.log(`[DP-DISCOVERY] 🚀 Auto-discovered DP${dp} → ${discovered.capability} = ${discovered.value} (conf: ${discovered.confidence}%)`);
          this.safeSetCapabilityValue(discovered.capability, discovered.value).catch(() => {});
          return;
        }
      }
      return;
    ...
    const { capability, divisor = 1, transform, dynamicAdd, skipValidator } = mapping;
    ...
    if (dp === 1 && capability === 'alarm_motion') {
      // v9.0.390 (forum #2114/#2118): the early return above also skipped the
      // alsoSets block below, so hybrid door/window sensors (HOBEIAN ZG-227Z,
      // sensor_contact_presence) NEVER updated alarm_contact from DP1 — the
      // P85 fix was dead code. Apply non-motion alsoSets here before returning
      // (alarm_motion/alarm_human stay on the holdoff path).
      if (mapping.alsoSets && typeof mapping.alsoSets === 'object') {
        ...
        for (const [alsoCap, alsoTransform] of Object.entries(mapping.alsoSets)) {
          if (alsoCap === 'alarm_motion' || alsoCap === 'alarm_human') {continue;}
          if (this.hasCapability(alsoCap)) {
            ...
            this.safeSetCapabilityValue(alsoCap, alsoValue).catch(() => {});
          }
        }
      }
      return;
    ...
    if (mapping.smartDivisor === true) {
      ...
      value = smartParse(value, dp, {
        manufacturerName: this.getSetting('zb_manufacturer_name') || '',
        capability: capability,
        deviceId: this.getData()?.id || '',
      });
    } else if (divisor > 1 && typeof value === 'number' && !Number.isInteger(value) && Math.abs(value) < divisor) {
    ...
    // v9.0.340: skipValidator flag bypasses validation for trusted sources (e.g. lux_direct)
    if (this._valueValidator && capability && !mapping?.skipValidator) {
  ...
  _inferCapabilityFromValue(dp, value) {
    ...
    if (finite && [1, 5, 101, 102, 103, 104, 105].includes(dpId) && canUse('measure_temperature')) {
      ...
      if (numeric >= -40 && numeric <= 100 && (!learned || learned?.capability !== 'measure_temperature' || learned?.source === 'temperature_range')) {
        return { capability: 'measure_temperature', value: Math.round(numeric * 10) / 10, source: 'temperature_range' };
      }
      if (numeric >= -400 && numeric <= 1000) {
        return { capability: 'measure_temperature', value: Math.round(numeric) / 10, source: 'temperature_range_scaled' };
      }
    }
  ...
  /**
   * v5.5.904: Detect data type for diagnostic logging (same asSwitchBase)
   */
  ...
  async _handleCommonDP(dp, rawValue) {
    ...
    // v10.3.0 FIX (B8): DPs 101/102/104/105 double as battery-percent DPs.
    // When the device has measure_battery, a 0-2% battery report must NOT be
    // decoded as a button press (and a button DP must not set battery 0-2%).
    const batteryDPs = [4, 10, 14, 15, 21, 100, 101, 102, 104, 105];
    const dpIsBattery = batteryDPs.includes(dp) && this.hasCapability('measure_battery');

    // Only process as button if: has button capability, DP not mapped, value is button-like
    if (hasButtonCapability && !dpIsMapped && !dpIsBattery && buttonDPs.includes(dp) && (value === 0 || value === 1 || value === 2)) {
      ...
      if (this.hasCapability('button')) {
        ...
        (this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout)(async () => {
          if (this._destroyed) {return;}
          await this.safeSetCapabilityValue('button', false).catch(() => { });
        }, 500);
      }

      // Emit event for flows
      // v10.3.0 FIX (B8): the button_pressed card declares a 'type' token
      // (app.json), not 'action' — the old token name was silently dropped.
      this.homey.flow.getDeviceTriggerCard('button_pressed')?.trigger(this, {
        button: dp.toString(),
        type: value === 0 ? 'single' : value === 1 ? 'double' : 'hold'
      }).catch(() => { });
    }
    ...
    if ((dp === 13 || dp === 101) && this.hasCapability('onoff.led')) {
      ...
      await this.safeSetCapabilityValue('onoff.led', ledOn).catch(() => { });
    }
    ...
    // P143: skip battery steal when this DP is already mapped (e.g. contact DP101 → lux)
    if ([4, 10, 14, 15, 21, 100, 101, 102, 104, 105].includes(dp) && this.hasCapability('measure_battery')) {
      const mappedCap = this.dpMappings?.[dp]?.capability;
      if (mappedCap && mappedCap !== 'measure_battery' && mappedCap !== 'alarm_battery') {
        // Sacred mapped DP (lux/temp/etc.) — do not treat as battery %
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // ILLUMINANCE (DP 12, 101, 103, or 106 — 101 = ZG-102ZL / contact with lux)
    // v5.5.226: Added DP106 for ZG-204ZM radar sensors
    // v5.5.306: RONNY FIX #760 - Clamp lux to 2000 max for TZE devices
    // P143: include DP101 + apply lux calibration (was missing for mapped contacts)
    // ─────────────────────────────────────────────────────────────────────────────
    if ([12, 101, 103, 106].includes(dp) && this.hasCapability('measure_luminance')) {
      // Prefer explicit lux mapping; if DP101 is battery-only mapping, skip
      const mappedCap = this.dpMappings?.[dp]?.capability;
      if (dp === 101 && mappedCap && mappedCap !== 'measure_luminance') {
        // not lux
        ...
        // v9.0.415 (P92.123): user calibration factor, applied AFTER the
        // clamp so the user can intentionally exceed the device spec range.
        luxValue = this._applyLuxCalibration(luxValue);
    ...
    if (dp === 9 && this.hasCapability('measure_luminance.distance')) {
      ...
      await this.safeSetCapabilityValue('measure_luminance.distance', parseFloat(distance)).catch(() => { });
    }
  }
  ...
  _handleZCLData(cluster, attr, value) {
    ...
    if (capability) {
      ...
      this.safeSetCapabilityValue(capability, finalValue).catch(() => {});
    }
  }
  ...
  async _setupZCLMode(zclNode) {
    ...
    if (!customHandlers.relativeHumidity && !customHandlers.msRelativeHumidity) {
      await this._setupZCLCluster(clusters,
        ['relativeHumidity', 'msRelativeHumidity', 'relativeHumidityMeasurement'],
        'measure_humidity',
        // v10.7.0 / P127 (z2m #28987, GH #516): TS0201-clone firmwares report
        // humidity as raw% ×10 — 580 → 58% via _scaleZclHumidity (not /100).
        (value) => this._scaleZclHumidity(value)
      );
    }
    ...
    if (!customHandlers.powerConfiguration && !customHandlers.genPowerCfg) {
      await this._setupZCLCluster(clusters,
        ...
        'measure_battery',
        (value) => normalizeZclBatteryPercent(value, { manufacturer: manufacturerOf(this) }) ?? 0,
        'batteryPercentageRemaining'
      );
    }
    ...
    if (!customHandlers.illuminanceMeasurement && !customHandlers.msIlluminanceMeasurement) {
      await this._setupZCLCluster(clusters,
        ...
        'measure_luminance',
        // v9.0.415 (P92.123): user calibration factor (Z2M-style
        // illuminance_calibration) — Tuya lux encodings vary wildly
        // (forum Peter_van_Werkhoven #2114/#2134: shown lux ~0.15–0.5× real).
        // Adaptive: small values are often already linear lux; large values
        // are ZCL log-encoded measuredValue.
        (value) => this._applyLuxCalibration(this._decodeIlluminanceRaw(value))
      );
    }
  ...
  async _setupZCLCluster(clusters, clusterNames, capability, transform, attribute = 'measuredValue') {
    ...
    try {
      ...
      cluster.on(attrEvent, (value) => {
        ...
        this.safeSetCapabilityValue(capability, transformed).catch(() => {});
      });

      // v9.0.412 (P92.120): smart read — attributes this device already
      // answered UNSUPPORTED_ATTRIBUTE for are skipped silently (negative
      // cache persisted in the device store), so diag logs stop flooding
      // with "not supported" on every boot/poll. Transient errors
      // (sleepy device, timeout) are never blacklisted.
      const readPromise = readAttributesSmart(this, cluster, clusterLabel, [attribute])
        .then(data => {
      ...
      if (typeof cluster.configureReporting === 'function') {
        ...
        // v9.0.412 (P92.120): skipped/blacklisted once unsupported instead
        // of logging a failure line on every boot.
        // v9.0.414 (P92.122): when reporting CANNOT be configured, the
        // device must not stay blind — start a polling fallback that
        // reads the attribute through the full fallback chain (named
        // cluster → raw/numeric cluster → Tuya DP).
        configureReportingSmart(this, cluster, clusterLabel, {
          ...
          if (ok) {
            ...
            this._setupCapabilityPollingFallback(cluster, clusterLabel, capability, transform, attribute, reportingConfig);
          }
        }).catch(() => {
          this._setupCapabilityPollingFallback(cluster, clusterLabel, capability, transform, attribute, reportingConfig);
        });
  ...
  /**
   * v9.0.414 (P92.122): polling fallback for a capability whose ZCL
   * reporting could not be configured (unsupported / refused). Reads the
   * attribute periodically through the full fallback chain so the device
   * never stays blind. One timer per capability, cleaned up on uninit.
   */
  _setupCapabilityPollingFallback(cluster, clusterLabel, capability, transform, attribute, reportingConfig) {
    try {
      this._capPollTimers = this._capPollTimers || {};
      if (this._capPollTimers[capability]) { return; } // already polling
      const seconds = Math.min(Math.max(reportingConfig?.maxInterval || 900, 300), 3600);
      const scheduler = (this.homey && typeof this.homey.setInterval === 'function') ? this.homey : globalThis;
      this._capPollTimers[capability] = scheduler.setInterval(async () => {
        if (this._destroyed) { return; }
        try {
          const { ok, value, via } = await readSensorWithFallbacks(this, cluster, clusterLabel, attribute);
          if (ok && value !== undefined && value !== null) {
            const transformed = transform(value);
            if (this.getCapabilityValue(capability) !== transformed) {
              this.log(`[ZCL-POLL] 📥 ${capability} = ${transformed} (via ${via})`);
              this.safeSetCapabilityValue(capability, transformed).catch(() => {});
            }
          }
        } catch (_e) { /* sleepy device — next tick retries */ }
      }, seconds * 1000);
      this.log(`[ZCL-POLL] ⏰ ${capability}: reporting unsupported → polling every ${seconds}s (fallback chain active)`);
    } catch (e) {
      this.log(`[ZCL-POLL] ⚠️ polling fallback setup failed for ${capability}: ${e.message}`);
    }
  }

  /**
   * Decode raw illuminance attribute / DP into lux.
   * - ZCL measuredValue is log-encoded: lux = 10^((v-1)/10000) (typically v≫5000)
   * - Many Tuya firmwares put linear lux in the same attribute (v < ~5000)
   * Applying the log formula to linear lux under-reports (~155 vs ~1000, #2134).
   */
  _decodeIlluminanceRaw(value) {
    const n = Number(value);
    if (!Number.isFinite(n) || n <= 0) {return 0;}
    if (n < 5000) {
      return Math.round(n);
    }
    return Math.round(Math.pow(10, (n - 1) / 10000));
  }

  /**
   * v9.0.415 (P92.123): Z2M-style illuminance calibration.
   * Setting `illuminance_calibration` = percentage (10-1000, default 100).
   * Tuya lux encodings vary per firmware (raw lux, log-scale, ADC counts,
   * half-scale...) — let the user align the reported value with reality.
   * @param {number} lux
   * @returns {number}
   */
  _applyLuxCalibration(lux) {
    if (typeof lux !== 'number' || !Number.isFinite(lux)) { return lux; }
    const pct = Number(this.getSetting?.('illuminance_calibration'));
    if (!Number.isFinite(pct) || pct <= 0 || pct === 100) { return lux; }
    const factor = Math.min(Math.max(pct, 10), 1000) / 100;
    return Math.round(lux * factor);
  }
  ...
  /**
   * v5.13.6: Sensor-specific Flow Triggering
   * Overrides TuyaZigbeeDevice._triggerCustomFlowsIfNeeded to handle advanced sensor flows
   */
    ...
    try {
      ...
      if (capability === 'alarm_smoke') {
        ...
        for (const cardId of cardIds) {
          if (await this.triggerFlowCard(cardId, { smoke: value })) {break;}
        }
      }
      ...
      if (capability === 'alarm_gas') {
        ...
        for (const cardId of cardIds) {
          if (await this.triggerFlowCard(cardId, { gas: value })) {break;}
        }
      }

      // Water leak sensor flows
      if (capability === 'alarm_water') {
        ...
        for (const cardId of cardIds) {
          if (await this.triggerFlowCard(cardId, { water: value })) {break;}
        }
      }

      // Contact sensor flows — include canonical opened/closed IDs used by compose manifests
      if (capability === 'alarm_contact') {
        const cardIds = value 
          ? [
            `${driverId}_alarm_contact_true`,
            `${driverId}_opened`,
            `${driverId}_contact_opened`,
            `${driverId}_contact_sensor_opened`,
            'contact_sensor_alarm_contact_true',
            'contact_sensor_opened',
            'contact_opened',
          ]
          : [
            `${driverId}_alarm_contact_false`,
            `${driverId}_closed`,
            `${driverId}_contact_closed`,
            `${driverId}_contact_sensor_closed`,
            'contact_sensor_alarm_contact_false',
            'contact_sensor_closed',
            'contact_closed',
          ];
        
        for (const cardId of cardIds) {
          if (await this.triggerFlowCard(cardId)) {break;}
        }
      }

      // Tamper flows
      if (capability === 'alarm_tamper' && value === true) {
        const cardIds = [`${driverId}_tamper_true`, `${driverId}_alarm_tamper_true`, 'contact_sensor_tamper_true', 'vibration_sensor_tamper_true'];
        for (const cardId of cardIds) {
          if (await this.triggerFlowCard(cardId)) {break;}
        }
      }
      ...
      if ((capability === 'alarm_vibration' || capability === 'alarm_generic.vibration') && value === true) {
        const cardIds = ['vibration_sensor_vibration_detected', 'vibration_sensor_vibration_alarm', `${driverId}_vibration_detected`];
        for (const cardId of cardIds) {
          await this.triggerFlowCard(cardId);
        }
      }

      // Illuminance change
      if (capability === 'measure_luminance' && typeof value === 'number') {
        await this.triggerFlowCard(`${driverId}_illuminance_changed`, { lux: value }).catch(() => {
          return this.triggerFlowCard('motion_illuminance_changed', { lux: value });
        });
      }

      // v5.13.4: Hook probe observation
      if (this._probeObservationSamples && capability.startsWith('measure_temperature')) {
        this._recordProbeObservation?.();
      }

    } catch (err) {
      this.log(`⚠️ [FLOW] Trigger error: ${err.message}`);
    }
  }
   ...
   * v5.5.33: Also mark device awake forDataQuery
   * v5.5.111: Also read battery while device is awake!
   */
  updateRadioActivity() {
    updateRadioActivity(this);

    // v5.5.33: Mark awake for queries
    if (this.DataQuery) {
      this.DataQuery.markAwake();
    }
  ...
  async _readBatteryWhileAwake() {
    ...
    try {
      ...
      const data = await Promise.race([
        ...
        new Promise((_, reject) => safeSetTimeout(this, () => { if (this._destroyed) {return;} reject(new Error('Timeout')); }, 2000))
      ]);

      if (data?.batteryPercentageRemaining !== undefined && data.batteryPercentageRemaining !== 255) {
        const battery = normalizeZclBatteryPercent(
  ...
  async onDeleted() {
    ...
    // v9.0.414 (P92.122): Clear capability polling fallback timers
    if (this._capPollTimers) {
      for (const cap of Object.keys(this._capPollTimers)) {
        try {
          const clearer = (this.homey && typeof this.homey.clearInterval === 'function')
            ? this.homey.clearInterval.bind(this.homey) : clearInterval;
          clearer(this._capPollTimers[cap]);
        } catch (_e) { /* no-op */ }
      }
      this._capPollTimers = {};
    }
    ...
    // v9.0.349: Call parent onDeleted so zigbee listeners are removed
    // (prevents "Cannot read properties of null (reading '_onDeleted')" crashes)
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    const SETTING_DP_MAP = {
      ...
      // v9.0.423 (P92.130): canonical Tuya mmWave radar settings
      // (Z2M MTG075/ZY-M100 map — the same one presence_sensor_radar uses).
      // The presence-hybrid drivers (air_purifier_presence,
      // climate_sensor_presence, device_air_purifier_presence) declare
      // these settings; unknown DPs are safely rejected by other devices.
      radar_sensitivity: { dp: 2, type: 'value' },
      min_range: { dp: 3, type: 'value', scale: 100 },        // m → cm
      max_range: { dp: 4, type: 'value', scale: 100 },        // m → cm
      detection_range: { dp: 4, type: 'value', scale: 100 },  // alias
      shield_range: { dp: 3, type: 'value', scale: 100 },     // alias
      entry_filter_time: { dp: 101, type: 'value', scale: 10 },
      fading_time: { dp: 102, type: 'value', scale: 10 },
      indicator: { dp: 108, type: 'bool' },
    };
...
UnifiedSensorBase.UnifiedSensorBase = UnifiedSensorBase;
UnifiedSensorBase.SensorBase = UnifiedSensorBase;
module.exports = UnifiedSensorBase;
