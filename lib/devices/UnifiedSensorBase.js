...
class UnifiedSensorBase extends TuyaZigbeeDevice {
  ...
  _handleDP(dpId, rawValue) {
    ...
    if (dp === 1 && capability === 'alarm_motion') {
      // v5.12.48 (backport P92.91, forum #2114/#2118): the early return also skipped
      // the alsoSets block, so hybrid door/window sensors never updated
      // alarm_contact from DP1. Apply non-motion alsoSets here before returning
  ...
  async _handleCommonDP(dp, rawValue) {
    ...
    if ([12, 103, 106].includes(dp) && this.hasCapability('measure_luminance')) {
      if (typeof value === 'number' && value >= 0) {
        ...
        // v5.12.55 (P92.123): calibration AFTER the clamp
  ...
  async _setupZCLMode(zclNode) {
    ...
    if (!customHandlers.illuminanceMeasurement && !customHandlers.msIlluminanceMeasurement) {
      await this._setupZCLCluster(clusters,
        ...
        'measure_luminance',
        // v5.12.55 (P92.123): user calibration factor (Z2M-style)
  ...
  async _setupZCLCluster(clusters, clusterNames, capability, transform, attribute = 'measuredValue') {
    ...
    try {
      ...
      // v5.12.52 (P92.120): smart read — attributes this device already
      ...
      if (typeof cluster.configureReporting === 'function') {
        ...
        // v5.12.52 (P92.120): skipped/blacklisted once unsupported instead
        // of logging a failure line on every boot.
        // v5.12.54 (P92.122): when reporting CANNOT be configured, the
  ...
  /**
   * v5.12.54 (P92.122): polling fallback for a capability whose ZCL
  ...
  /**
   * v5.12.55 (P92.123): Z2M-style illuminance calibration.
   * Setting  = percentage (10-1000, default 100).
   */
  _applyLuxCalibration(lux) {
    if (typeof lux !== 'number' || !Number.isFinite(lux)) { return lux; }
    const pct = Number(this.getSetting && this.getSetting('illuminance_calibration'));
  ...
  async onDeleted() {
    ...
    // v5.12.54 (P92.122): Clear capability polling fallback timers
