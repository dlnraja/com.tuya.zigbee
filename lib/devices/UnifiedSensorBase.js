...
class UnifiedSensorBase extends TuyaZigbeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      this.universalDataHandler.on('capability', (capability, value) => {
        ...
        this.safeSetCapabilityValue(capability, value);
      });
  ...
  _handleIASZoneStatus(status) {
    ...
    if (this.hasCapability('alarm_tamper')) {
      this.safeSetCapabilityValue('alarm_tamper', tamper);
    }

    // Battery low is universal
    if (this.hasCapability('alarm_battery') && battery) {
      this.safeSetCapabilityValue('alarm_battery', battery);
    }
  }
  ...
  _handleMotionWithHoldoff(motionDetected) {
    ...
    const clearTimer = this.homey?.clearTimeout ? this.homey.clearTimeout.bind(this.homey) : globalThis.clearTimeout;

    if (motionDetected) {
      if (this._motionHoldoffTimer) clearTimer(this._motionHoldoffTimer);
    ...
    this._motionHoldoffTimer = setTimer(() => {
      this._motionHoldoffTimer = null;
  ...
  _handleDP(dpId, rawValue) {
    ...
    if (!mapping) {
      ...
      if (autoMapping) {
        ...
        this.safeSetCapabilityValue(autoMapping.capability, finalValue);
        return;
      }
      ...
      if (inferredCap) {
        ...
        if (!this.hasCapability(inferredCap.capability)) {
          this._addCapabilityPermissive(inferredCap.capability).then(() => {
            this.safeSetCapabilityValue(inferredCap.capability, inferredCap.value);
          }).catch(() => {});
        } else {
          this.safeSetCapabilityValue(inferredCap.capability, inferredCap.value);
        }
      ...
      if (this._discovery) {
        ...
        if (discovered && discovered.confidence >= 80) {
          ...
          this.safeSetCapabilityValue(discovered.capability, discovered.value);
          return;
        }
      }
      return;
    ...
    if (mapping.alsoSets && typeof mapping.alsoSets === 'object') {
      ...
      for (const [alsoCap, alsoTransform] of Object.entries(mapping.alsoSets)) {
        if (this.hasCapability(alsoCap)) {
          ...
          this.safeSetCapabilityValue(alsoCap, alsoValue);
        }
      }
    }
  }
  ...
  _handleZCLData(cluster, attr, value) {
    ...
    if (capability) {
      ...
      this.safeSetCapabilityValue(capability, finalValue);
    }
  }
  ...
  async _setupZCLCluster(clusters, clusterNames, capability, transform, attribute = 'measuredValue') {
    ...
    try {
      ...
      cluster.on(attrEvent, (value) => {
        ...
        this.safeSetCapabilityValue(capability, transformed);
      });
