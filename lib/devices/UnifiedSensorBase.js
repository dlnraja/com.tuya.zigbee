...
class UnifiedSensorBase extends TuyaZigbeeDevice {
  ...
  async _fastInit(zclNode) {
    ...
    this.homey.setTimeout(async () => { if (this._destroyed) return; this.log('[FAST-INIT] ⏳ Starting deferred initialization...');
      try {
        // Manufacturer config
        await this._applyManufacturerConfig?.();
  ...
  async _sendDataQueryOnWake() {
    ...
    if (dpIds.length > 0) {
      ...
      for (const dpId of dpIds) {
        ...
        await new Promise(r => this.homey.setTimeout(r, 100)); // Small delay between requests
      }
  ...
  async _readBatteryWhileAwake() {
    ...
    try {
      ...
      const data = await Promise.race([
        ...
        new Promise((_, reject) => this.homey.setTimeout(() => { if (this._destroyed) return; reject(new Error('Timeout')); }, 2000))
      ]);
