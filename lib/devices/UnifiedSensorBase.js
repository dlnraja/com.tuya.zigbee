...
class UnifiedSensorBase extends ZigBeeDevice {
  ...
  _handleIASZoneStatus(status) {
    ...
    if (primaryCapability === 'alarm_motion') {
      ...
      this._safeSetCapability(primaryCapability, alarmValue).finally(() => {
      ...
      for (const cap of fallbackCaps) {
        if (this.hasCapability(cap)) {
          ...
          this._safeSetCapability(cap, value).finally(() => {
  ...
  _handleCommonDP(dp, rawValue) {
    ...
    if (hasButtonCapability && !dpIsMapped && buttonDPs.includes(dp) && (value === 0 || value === 1 || value === 2)) {
      ...
      if (this.hasCapability('button')) {
        ...
        setTimeout(() => await this.setCapabilityValue('button', false).catch(() => { }), 500);
      }
  ...
  static get DYNAMIC_CAPABILITIES() {
    return [
      ...
      'measure_temperature.probe',
      'alarm_motion', 'alarm_contact', 'alarm_water', 'alarm_smoke',
      'alarm_gas', 'alarm_co', 'alarm_tamper', 'alarm_battery', 'alarm_vibration', 'alarm_generic.vibration',
  ...
  async _safeSetCapability(capability, value) {
    ...
    // v5.13.2: Added sub-capability variants (.probe, .soil, etc.)
    const THROTTLE = {
      ...
      'alarm_tamper': 2000, 'alarm_vibration': 2000, 'alarm_generic.vibration': 2000, 'alarm_sos': 1000,
    };
    ...
    const SIGNIFICANT = {
      ...
      'measure_luminance': 50, 'measure_luminance.distance': 0.1,
    };
    ...
    if (tracker) {
      ...
      // v5.12.2: FIX _TZE200_pay2byax invert stuck (forum #1637)
      // alarm_contact inversion in setCapabilityValue makes pre-inversion
      // value match displayed post-inversion value, blocking updates
      const invertActive = capability === 'alarm_contact' && (this._invertContact || this._userExplicitInvert);
      if (!valueChanged && !invertActive) return;
      // v5.11.5: Boolean alarm state CHANGES bypass throttle (Lasse_K "no alarm" root cause)
      const isBooleanChange = typeof calibratedValue === 'boolean' && valueChanged;
      if (elapsed < throttleMs && !sigChange && !isBooleanChange) return;
    }
    ...
    // v5.13.4: Hook probe observation for dedup analysis
