...
class UnifiedSensorBase extends TuyaZigbeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      const { BatteryHybridManager } = require('../battery');
      ...
      const productId = settings.zb_product_id || settings.zb_productId || store.productId || '';
      
      await this.batteryHandler.initialize(manufacturerName, productId);
    } catch (e) {
      this.log(`⚠️ BatteryHybridManager initialization failed: ${e.message}`);
    }
  ...
  async onUninit() {
    ...
    if (this._keepAliveTimer) {
      (this.homey?.clearTimeout ?? clearTimeout)(this._keepAliveTimer);
  ...
  _handleMotionWithHoldoff(motionDetected) {
    ...
    if (motionDetected) {
      ...
      this._keepAliveTimer = (this.homey?.setTimeout ?? setTimeout)(() => {
        if (this._destroyed) return;
    ...
    this._motionHoldoffTimer = (this.homey?.setTimeout ?? setTimeout)(() => {
      if (this._destroyed) return;
      this._motionHoldoffTimer = null;
      this.safeSetCapabilityValue('alarm_motion', false).catch(() => {});
  ...
  async _handleCommonDP(dp, rawValue) {
    ...
    if (hasButtonCapability && !dpIsMapped && buttonDPs.includes(dp) && (value === 0 || value === 1 || value === 2)) {
      ...
      if (this.hasCapability('button')) {
        ...
        (this.homey?.setTimeout ?? setTimeout)(async () => {
          if (this._destroyed) return;
