...
function resolveWiFiTransport(opts = {}) {
  ...
  if (deviceId && localKey) {
    // T15811 (TuyAPI Neo Coolcam): static settings IP is more reliable than flaky UDP rediscovery
    // Prefer settings IP when present; still allow UDP self-heal when settings IP empty.
    const preferStaticIp = !!ip;
    const ipSource = ip ? 'settings-static' : (discoveredIp ? 'udp-discovery' : null);
    reasons.push(`local credentials present (device_id=${deviceId})`);
    reasons.push(ipSource
      ? (preferStaticIp
        ? `IP known via ${ipSource} (T15811 prefer static IP over UDP churn)`
    ...
    if (policy.localDiscovery) {
      reasons.push(preferStaticIp
        ? 'localDiscovery=true but settings IP pinned (skip cloud; UDP only as fallback reconnect)'
        : 'localDiscovery=true: dynamic IP self-healing active');
    }
    if (cloudSick) {
