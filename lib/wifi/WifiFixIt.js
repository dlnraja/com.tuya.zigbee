  ...
  for (const d of devices) {
    ...
    if (probe.verdict === 'both_closed') {
      findings.push({
        ...
        port6667: false,
        message:
          'Both Tuya LAN ports closed (6668/6667). Often Matter/cloud-only, deep-sleep battery sensor, or Bluetooth-behind-gateway — local protocol cannot talk. Not a key typo. (UDP discovery 6666/6667/7000 may still see awake devices — TinyTuya.)',
      });
    } else {
      findings.push({
        deviceId: d.id,
...
/** Static notes from com.tuyalocal Fix It + TinyTuya / HA localtuya (P2656 complementary — never wipe). */
const LAN_LIMITATION_NOTES = [
  ...
  // WHY(P2656): TinyTuya also uses UDP 6666/6667/7000 for discovery — closed TCP ≠ “no device”; try scan/solicitation.
  'Discovery uses UDP 6666 / 6667 / 7000 (TinyTuya). Closed TCP 6668/6667 can still allow UDP discovery for online devices.',
  // WHY(P2656): Zigbee behind Tuya WiFi hub — prefer Homey Zigbee coordinator; hub LAN sessions often capped 1–3.
  'Zigbee sub-devices behind a Tuya WiFi hub need hub local_key + child node_id (cid). Prefer Homey Zigbee coordinator for large fleets (hub LAN connection limits).',
  // WHY(P2656): Z2M/ZHA doctrine — Zigbee end-devices on Homey need no Tuya local_key (WiFi ones do).
  'Tuya Zigbee paired to Homey (or Z2M/ZHA) needs no local_key. Local keys apply to WiFi LAN devices and to the WiFi hub when using hub cid fallback.',
  // WHY(P2656): tuyapi CLI / TinyTuya wizard — keys refresh after re-pair.
  'After re-pair or factory reset, local_key changes — re-run tuya-cli wizard / TinyTuya wizard / Smart Life QR / IoT Cloud Lookup.',
  // WHY(P2657): nl.rebtor.tuya / tuyapi — only one LAN TCP session per device.
  'Only one LAN TCP session per WiFi device (tuyapi / rebtor). Close the Smart Life / Tuya phone app while Homey is connected, or Homey will look “unavailable”.',
];
