 ...
 *      ZHA/Hubitat/Z2M attrs 57348/57349/57355; Z2M name manuSpecificTuya2 = 0xE002
 *      (P2267). Fallback 0xE001 if write rejected.
 * Contre quoi: Homey settings-save errors (A_Tas #2199 / T158757 / P2289).
 *
...
/** Named writeAttributes keys (zigbee-clusters / Homesuite ManuSpecificTuya3) */
const ATTR_NAME = Object.freeze({
  [ATTR.presenceKeepTime]: 'presenceKeepTime',
  [ATTR.motionSensitivity]: 'motionSensitivity',
  [ATTR.staticSensitivity]: 'staticSensitivity',
  [ATTR.ledIndicator]: 'ledIndicator',
  [ATTR.targetDistance]: 'targetDistance',
  [ATTR.motionDistance]: 'motionDetectionDistance',
});

/** Homey setting id → ZCL attribute write plan */
const SETTING_ATTR_MAP = Object.freeze({
  motion_detection_sensitivity: { attr: ATTR.motionSensitivity, attrName: 'motionSensitivity', coerce: (v) => clampInt(v, 0, 5) },
  static_detection_sensitivity: { attr: ATTR.staticSensitivity, attrName: 'staticSensitivity', coerce: (v) => clampInt(v, 0, 5) },
  motion_detection_distance: {
    attr: ATTR.motionDistance,
    attrName: 'motionDetectionDistance',
    // Z2M: min 75, max 600, step 75
    coerce: (v) => {
      const n = clampInt(v, 75, 600);
      return Math.round(n / 75) * 75;
    },
  },
  led_indicator: { attr: ATTR.ledIndicator, attrName: 'ledIndicator', coerce: (v) => (v ? 1 : 0), optional: true },
  // Legacy compose key — map 0–9 UI to Linptech 0–5 motion scale
  radar_sensitivity: { attr: ATTR.motionSensitivity, attrName: 'motionSensitivity', coerce: (v) => clampInt(Math.round(Number(v) * (5 / 9)), 0, 5) },
});

/** Settings that must not fail Homey save when firmware rejects the attr (Z2M #8889 LED) */
const OPTIONAL_SETTING_KEYS = new Set(
  Object.entries(SETTING_ATTR_MAP).filter(([, v]) => v.optional).map(([k]) => k),
);
...
function isLinptechES1(mfr, pid) {
  ...
  // WHY(P2289): A_Tas #2199 — Homey settings store sometimes omits modelId;
  // verified mfr markers alone are enough to take the ES1 ZCL settings path.
...
function planSettingWrite(key, rawValue) {
  if (SETTING_ATTR_MAP[key]) {
    const { attr, attrName, coerce } = SETTING_ATTR_MAP[key];
    return {
      kind: 'zcl',
      cluster: CLUSTER_LINPTECH_PRIMARY,
      fallbackClusters: [CLUSTER_LINPTECH_FALLBACK],
      attr,
      attrName: attrName || ATTR_NAME[attr],
...
function isOptionalLinptechSetting(key) {
  return OPTIONAL_SETTING_KEYS.has(key);
}
