...
// v9.0.240 (P58): safeSetCapabilityValue / smartCap are installed globally in
// app.js on the root ZigBeeDevice — every base class inherits the methods.
...
class UniversalZigbeeDevice extends ZigBeeDeviceWithDiagnostics {
  ...
  async onNodeInit() {
    ...
    // 12. DEEP SLEEP RECOVERY (Resilience)
    if (this.node) {
      this.node.on('endDeviceAnnounce', () => {
        this.log('[UNIVERSAL] 🔄 Device woke up (EndDeviceAnnounce)! Re-applying critical configs.');
        this._onDeviceWakeup();
      });
    }
  }

  async _onDeviceWakeup() {
    // Re-init configs that might be lost during deep sleep or battery swap
    try {
      if (this.smartEnergy && typeof this.smartEnergy.init === 'function') {
         await this.smartEnergy.init().catch(() => {});
      }
      
      // Quirks specific recovery: TS004F Scene Mode
      const modelId = this.getSetting('zb_model_id') || '';
      if (modelId === 'TS004F') {
         this.log('[WAKEUP] Re-sending Tuya Scene Mode config...');
         const ep = this.getSafeCluster(0x0000, 1);
         if (ep && ep.clusters && ep.clusters.basic) {
            await ep.clusters.basic.writeAttributes({ 0x8004: 1 }).catch(() => {});
         }
      }
    } catch(e) {
      this.log('[WAKEUP] Error during wakeup recovery:', e.message);
    }
  }
  ...
  async runIntelligentAdaptation() {
    try {
      // P2502: same BootBudget defer as TuyaZigbeeDevice (Peter OOM complementary)
      try {
        const BootBudget = require('./performance/BootBudget');
        if (BootBudget.isHeapCritical() || !BootBudget.shouldStartHeavyFeatures()) {
          this.log(`[ADAPT] Deferred (heap ${BootBudget.heapUsedMb()}MB) — P2502`);
          return;
        }
      } catch (_e) { /* optional */ }
  ...
  async initManagers() {
    try {
      // WHY(P2673 Bastien-first): profile energy before SmartEnergy audit
      try {
      ...
      this.smartBattery = new UnifiedBatteryHandler(this);
  ...
  /**
   * WHY(P2284): append-only wrapHandleFrame — never blind-overwrite or early-return
   * without forwarding (orphans Physical 0xFD / IO EF00 / BoundCluster).
   */
  _setupRawFrameFallback() {
    ...
    const self = this;
    wrapHandleFrame(this.node, 'universal-zigbee-l0', async (args, next) => {
      ...
      try {
        if (self.frameAnalyzer) {
          try {
            decoded = await self.frameAnalyzer.parse(endpointId, clusterId, frame, meta);
            if (decoded) {
              const cacheKey = `${decoded.clusterId}_${decoded.commandId}`;
              let skipDeep = false;
              if (self._decoderCache?.has(cacheKey)) {
                if (self._decoderCache.get(cacheKey) === 'SKIP') {skipDeep = true;}
              }
              if (!skipDeep) {
                const correlationKey = `${decoded.type}_${decoded.seqNum}_${decoded.endpoint}`;
                if (!self._isDuplicateCorrelation?.(correlationKey)) {
                  if (meta && meta.lqi) {self._processRadioSensing?.(meta.lqi);}
                  await self._dynamicallyEnrichCapabilities?.(decoded);
                  if (decoded.commandName === 'onOff' || decoded.isClick) {
                    self.gestureEngine?.process?.(
                      'zcl',
                      decoded.commandName === 'onOff' ? 'click' : decoded.type,
                      decoded.seqNum,
                    );
                  }
                  self.emit('zigbee_message', decoded);
                }
              }
            }
          } catch (e) {
            self.log(` [DEEP-PARSER] Error: ${e.message}`);
          }
        }

        if (typeof self.onZigBeeMessage === 'function') {
          try {
            if (decoded && decoded.attributes) {
              decoded.attributes.forEach((attr) => self._trackCapabilityActivity?.(attr));
            }
            self.onZigBeeMessage(endpointId, clusterId, frame, meta);
          } catch (e) {
            self.log(` [RX] Error calling onZigBeeMessage: ${e.message}`);
          }
        }

        self.trackIncomingReport?.();
      } catch (e) {
        self.log(` [L0] universal wrap error: ${e.message}`);
      }
  ...
  async sendRawFrame(endpointId, clusterId, frame) {
    try {
      ...
      this.frameAnalyzer?.recordTx?.(endpointId, clusterId, { payload: data }, { source: 'send_raw_frame' });
  ...
  _processRadioSensing(lqi) {
    ...
    if (this._lqiBuffer.length === SENSING_WINDOW_SIZE) {
       ...
       if (diff > SENSING_THRESHOLD) {
          ...
          this._sensingTimer = this.homey.setTimeout(() => {
  ...
  async _dynamicallyEnrichCapabilities(decoded) {
    try {
      ...
      this.log?.('[ENRICH] Dynamic capability enrichment failed:', e.message);
    }
  }
