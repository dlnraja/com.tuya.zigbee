...
class TuyaTimeDebugProbe {
  ...
  async _sendVariant({ epoch2000, extended }) {
    const now = new Date();
    let utc =safeParse(Math.floor(now.getTime(), 1000));
    if (epoch2000) utc -= 946684800;

    const offset =safeMultiply(-now.getTimezoneOffset(), 60);
    ...
    for (const epId in this.device.node.endpoints) {
      ...
      if (ep?.clusters?.[CLUSTERS.TUYA_EF00]) {
        await ep.sendFrame(CLUSTERS.TUYA_EF00, 0x24, payload);
      }
    }
  }
