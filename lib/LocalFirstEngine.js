...
const { createNetworkBreaker, createLocalCache, isMemoryPressure } = require('./utils/NetworkResilience');
...
const RULES = [
  ...
  // ─── BUTTON RULES (P27.1, P28, P29) ───────────────────────────────────
  {
    ...
    category: 'button',
    severity: 'high',
    name: 'TS004F scene mode not configured',
    description: 'TS004F needs attribute 0x8004=1 on cluster 6 (onOff) for scene mode. TS0041–TS0044 are multi-EP remotes — do NOT write 0x8004.',
    predicate: (ctx) => ctx.code && /TS004F.*?scene/i.test(ctx.code) && !/TS004[1-4]/i.test(ctx.code) && !/0x8004|attributeId.*?0x8004/.test(ctx.code),
    suggestedFix: 'Add DeviceOperatingMode write for TS004F only (P27.1 / P2235)',
    autoFixable: false,
    reference: '071ed0322',
  },
  {
...
function predictIssues(ctx) {
  ...
  if (ctx.driverCompose && ctx.fingerprints) {
    ...
    for (const [mfr, fp] of topFps) {
      ...
      if (driver && !TU.includesCI(driver.manufacturerName, mfr)) {
        predictions.push({
...
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 4: AI BONUS LAYER (optional, never blocking, P36 budget-aware)
// ═══════════════════════════════════════════════════════════════════════════════

// P36: AI Budget Guard — graceful degradation when quota exhausted
let _aiBudgetGuard = null;
const _aiBreakers = new Map();
const _aiResponseCache = createLocalCache(32);
const MAX_AI_RESPONSE_BYTES = 512 * 1024;

function aiBreakerFor(name) {
  if (!_aiBreakers.has(name)) {
    _aiBreakers.set(name, createNetworkBreaker(`LocalFirstAI:${name}`, {
      failureThreshold: 3,
      resetTimeout: 120000,
    }));
  }
  return _aiBreakers.get(name);
}

function getAIBudgetGuard() {
  if (!_aiBudgetGuard) {
    try {
      const { AIBudgetGuard } = require('./SDK3CompatBridge');
      _aiBudgetGuard = new AIBudgetGuard({
        caps: {
          MiniMax: parseInt(process.env.MINIMAX_DAILY_CAP || '10000'),
          'github-models': parseInt(process.env.GH_MODELS_DAILY_CAP || '150'),
          openai: parseInt(process.env.OPENAI_DAILY_CAP || '200'),
          anthropic: parseInt(process.env.ANTHROPIC_DAILY_CAP || '100'),
          ollama: 10000,
        },
        stateFile: require('path').join(
          require('path').resolve(__dirname, '..'),
          '.github', 'state', 'ai-budget.json'
        ),
      });
    } catch (e) { /* no budget guard, allow all */ }
  }
  return _aiBudgetGuard;
}

async function callAI(prompt, options = {}) {
  const cacheKey = crypto.createHash('sha256').update(String(prompt)).digest('hex').slice(0, 24);
  if (isMemoryPressure({ rssLimit: 512 * 1024 * 1024, heapLimit: 400 * 1024 * 1024 })) {
  ...
  const guard = getAIBudgetGuard();

  const providers = [];

  // Tier 0: MiniMax (M3 unlimited — user has unlimited subscription, P35)
  if (minimaxKey && (!guard || guard.canUse('MiniMax'))) {
    providers.push({
      name: 'MiniMax',
  ...
  if (githubToken && (!guard || guard.canUse('github-models'))) {
    providers.push({
      name: 'github-models',
  ...
  if (openaiKey && (!guard || guard.canUse('openai'))) {
    providers.push({
      name: 'openai',
  ...
  if (anthropicKey && (!guard || guard.canUse('anthropic'))) {
    providers.push({
      name: 'anthropic',
  ...
  if (ollamaHost && (!guard || guard.canUse('ollama'))) {
    providers.push({
      name: 'ollama',
      url: `${ollamaHost}/api/chat`,
      headers: {},
      body: { model: 'llama3.2', stream: false, max_tokens: maxTokens },
      transformRequest: (messages) => ({
        model: 'llama3.2',
        messages,
        stream: false,
  ...
  for (const provider of providers) {
    const breaker = aiBreakerFor(provider.name);
      ...
      const result = await breaker.exec(async () => {
        ...
        if (!res.ok) {
          if (res.status === 429 || res.status === 402) {
            if (guard) { guard.markQuotaExhausted(provider.name); }
          } else if (guard) {
            guard.recordError(provider.name, `HTTP ${res.status}`);
          }
          throw new Error(`HTTP ${res.status}`);
        }
        const buf = Buffer.from(await res.arrayBuffer());
        if (buf.length > MAX_AI_RESPONSE_BYTES) {
          throw new Error(`AI response too large (${buf.length} bytes)`);
        }
        ...
        if (guard) { guard.recordUsage(provider.name, maxTokens); }
        return { provider: provider.name, content };
      });
      _aiResponseCache.set(cacheKey, result);
      return result;
    } catch (e) {
      if (guard && !String(e.message || '').startsWith('HTTP')) {
        guard.recordError(provider.name, e.message);
      }
      continue;
    }
  }
  return _aiResponseCache.get(cacheKey) || null;
}
