...
class AdvancedAnalytics extends EventEmitter {
  ...
  calculateUptime(deviceId) {
    ...
    return safeDivide(Math.round(uptime * 10), 10);
  }
  ...
  async analyzeEnergy(device) {
    ...
    const cost = Math.round((dailyEnergy / 1000) * 0.20 * 100) / 100;
  ...
  async generateReport() {
    ...
    if (devices.length > 0) {
      report.averageUptime = Math.round(uptimeSum     / devices.length);
      report.averageSuccessRate = Math.round(successRateSum    / devices.length);
    }
