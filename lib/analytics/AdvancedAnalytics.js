...
class AdvancedAnalytics extends EventEmitter {
  ...
  calculateUptime(deviceId) {
    ...
    const uptime = Math.max(0, 100 - safeMultiply(safeDivide(downtime, 1440), 100));
  ...
  async analyzeEnergy(device) {
    ...
    const cost = safeDivide(Math.round(safeMultiply(safeMultiply(safeDivide(dailyEnergy, 1000), 0.20), 100)), 100);
  ...
  async trackCommand(deviceId, success, duration) {
    ...
    const successRate = total > 0 ? Math.round((metrics.commands.success / total) * 100) : 100;
  ...
  async generateReport() {
    ...
    if (devices.length > 0) {
      report.averageUptime = Math.round(safeDivide(uptimeSum, devices.length));
      report.averageSuccessRate = Math.round(safeDivide(successRateSum, devices.length));
    }
