...
class AdvancedAnalytics extends EventEmitter {
  ...
  async initialize() {
    ...
    if (!this.homey || !this.homey.log) { return; }
  ...
  getDeviceSummary(deviceId) {
    ...
    const successRate = (metrics.command_success_rate && metrics.command_success_rate.value) || 100;
    const uptime = (metrics.device_uptime && metrics.device_uptime.value) || 100;
  ...
  /**
   * v8.5.17: Destroy analytics — stop all intervals, release homey reference
   * Called from app.js onUninit() to prevent 'app instance destroyed' crashes
   */
  destroy() {
    ...
    for (const id of this._intervals) {
      try { clearInterval(id); } catch (e) { /* no-op */ }
    }
    this._intervals = [];

    // Release Homey reference so stale callbacks can't crash
    this.homey = null;
    ...
    // Remove all event listeners (avoid EventEmitter memory leaks)
