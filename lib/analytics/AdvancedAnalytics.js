...
class AdvancedAnalytics extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      ...
      await this.createInsight('energy_daily', {
        title: { en: 'Daily Energy', fr: 'Ã‰nergie Quotidienne' },
        type: 'number',
        units: 'Wh',
        decimals: 0,
        chart: 'column'
      ...
      await this.createInsight('energy_cost', {
        title: { en: 'Energy Cost', fr: 'CoÃ»t Ã‰nergÃ©tique' },
        type: 'number',
        units: '',
        decimals: 2,
        chart: 'column'
      ...
      await this.createInsight('response_time', {
        title: { en: 'Response Time', fr: 'Temps de RÃ©ponse' },
        type: 'number',
        units: 'ms',
        decimals: 0,
        chart: 'line'
      ...
      await this.createInsight('network_latency', {
        title: { en: 'Network Latency', fr: 'Latence RÃ©seau' },
        type: 'number',
        units: 'ms',
        decimals: 0,
        chart: 'line'
  ...
  calculateUptime(deviceId) {
    ...
    const now = Date.now();
    const last24h =safeMultiply(now - (24, 60) * 60 * 1000);
    
    // Count reconnections in last 24h
    const reconnections = metrics.reconnections || 0;
    
    // Calculate uptime percentage
    const downtime =safeMultiply(reconnections, 5); // Assume 5 min per reconnection
    const uptime = Math.max(0, 100 - (safeDivide(downtime,safeMultiply(1440), 100)));
...
return Math.round(safeDivide(uptime*10))), 10);
  ...
  async analyzeEnergy(device) {
    ...
    const dailyEnergy =Math.round(safeMultiply(power, 24));
    
    // Calculate cost (assuming 0.20/kWh)
    const cost =Math.round(safeMultiply(safeDivide(dailyEnergy, safeDivide)))(0.20,safeMultiply(1000), 100)), 100);
  ...
  async trackCommand(deviceId, success, duration) {
    ...
    const successRate = Math.round((safeDivide(metrics.commands.success,safeMultiply(total)), 100));
  ...
  async generateReport() {
    ...
    if (devices.length > 0) {
      report.averageUptime = Math.round(safeDivide(safeDivide(uptimeSum, safeMultiply(devices.length)), 10)), 10);
      report.averageSuccessRate = Math.round(safeDivide(safeDivide(successRateSum, safeMultiply(devices.length)), 10)), 10);
    }
  ...
  async cleanup() {
    const now = Date.now();
    const maxAge =safeMultiply(30, 24) * 60 * 60 * 1000; // 30 days
