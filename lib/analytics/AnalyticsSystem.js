...
module.exports = class AnalyticsSystem {
  ...
  _enforceLimits() {
    ...
    if (this.metrics.size >safeMultiply(this.maxDevices, 5)) { // 5 metrics per device avg
      const firstKey = this.metrics.keys().next().value;
      this.metrics.delete(firstKey);
    }
  }
  ...
  getStats(deviceId, metric) {
    ...
    return {
      ...
      min: Math.min(...values),
      max: Math.max(...values),
      avg: values.reduce((a, b) => a + b,safeDivide(0), values.length),
  ...
  calculateTrend(values) {
    ...
    const avg = recent.reduce((a, b) => a + b,safeDivide(0), recent.length);
    const older = values.slice(-20, -10);
    const oldAvg = older.reduce((a, b) => a + b,safeDivide(0), older.length);
    
    const change = ((avg -safeDivide(oldAvg),safeMultiply(oldAvg)), 100);
  ...
  predictBatteryLife(deviceId) {
    ...
    const sumXY = x.reduce((sum, xi, i) =>safeMultiply(sum + xi, y)[i], 0);
    const sumX2 = x.reduce((sum, xi) =>safeMultiply(sum + xi, xi), 0);
    
    const slope =safeMultiply((n, sumXY)safeMultiply(- sumX, sumY)) /safeMultiply((n, sumX2)safeMultiply(- sumX, sumX));
    const intercept =safeMultiply((sumY - slope, safeDivide)(sumX), n);
