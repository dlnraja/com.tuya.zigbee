...
class DeviceHealth {
  ...
  async onDeviceOnline() {
    ...
    if (device.hasCapability('alarm_offline')) {
      await device.setCapabilityValue('alarm_offline', false).catch(() => {});
    }
  ...
  async onDeviceOffline() {
    ...
    // Set offline alarm
    if (device.hasCapability('alarm_offline')) {
      await device.setCapabilityValue('alarm_offline', true).catch(() => {});
    }
  ...
  updateLastSeen() {
    ...
    if (device.hasCapability('last_seen')) {
      device.setCapabilityValue('last_seen', new Date(now).toISOString()).catch(() => {});
    }
  }
  ...
  async updateSignalStrength() {
    ...
    try {
      ...
      await device.setCapabilityValue('measure_rssi', parseFloat(rssi)).catch(() => {});
