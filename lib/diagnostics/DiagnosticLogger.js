...
class DiagnosticLogger {
  ...
  _isVerboseEnabled() {
    try {
      // WHY (P2248): env TUYA_DIAG_VERBOSE=1 forces verbose in Homey diags without
      // requiring every tile to flip a setting (Peter fleet interview harvest).
      if (process.env.TUYA_DIAG_VERBOSE === '1' || process.env.TUYA_DIAG_VERBOSE === 'true') {
        return true;
      }
  ...
  capabilityChange(capability, value, previousValue) {
    // WHY (P2248): contact/SOS/water capability flips must appear in Homey logs
    // even when verbose_diagnostic_logging is off — otherwise Peter-style
    // "pulse" and battery flips are invisible in Gmail diags.
    const critical = /^(alarm_|measure_battery|button)/i.test(String(capability || ''));
    const payload = { capability, value, previousValue };
    if (critical) {return this.info('Capability changed', payload);}
    return this.verbose('Capability changed', payload);
  }
