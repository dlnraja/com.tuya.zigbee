...
function DiagnosticLogsCollector(superclass) {
  return class extends superclass {
    ...
    constructor(...args) {
      ...
      // Intercepter log/error ONLY when the property is configurable.
      // On Homey SDK3, log/error are often non-writable accessors — any
      // assignment or redefine throws "Cannot assign to read only property
      // 'error'" and aborts device construction (Gmail crash reports).
      try {
        this._originalLog = typeof this.log === 'function' ? this.log.bind(this) : null;
        this._originalError = typeof this.error === 'function' ? this.error.bind(this) : null;
      } catch (_e) {
        this._originalLog = null;
        this._originalError = null;
      }

      const canOverride = (name) => {
        try {
          let proto = this;
          while (proto) {
            const desc = Object.getOwnPropertyDescriptor(proto, name);
            if (desc) {
              // Homey SDK3: log/error are often accessors — never redefine.
              // Redefining or assigning throws "Cannot assign to read only
              // property 'error'" and aborts Sos/Contact/WaterLeak/Button init.
      ...
      if (canOverride('log') && this._originalLog) {
        try {
        ...
        } catch (_e) { /* locked — skip interception */ }
      }

      if (canOverride('error') && this._originalError) {
        try {
    ...
    async collectTuyaState() {
      ...
      try {
        ...
        for (let dp = 1; dp <= 120; dp++) {
          ...
          } catch(e) { /* Store read failed during diagnostics */ }
        }
    ...
    async collectStoreValues() {
      ...
      for (const k of keys) {
        ...
        } catch(e) { /* Store key not available */ }
      }
      return store;
    }
      ...
      try {
        ...
        if (this.getDeviceProfile) {
          try {
            ...
            info.deviceProfile = p ? { id: p.id, deviceType: p.deviceType, protocol: p.protocol, gangCount: p.gangCount, quirks: p.quirks } : null;
          } catch(e) { /* Store read failed during diagnostics */ }
        }
    ...
    async formatDiagnosticReport() {
      ...
      report.push('─'.repeat(70));
      report.push('🏗️  ARCHITECTURE REFERENCE (for AI reasoning)');
      report.push('─'.repeat(70));
