...
const TuyaSmartLifeAuth = require('../tuya-local/TuyaSmartLifeAuth');
...
class DeviceDiagnostics {
  ...
  getDeviceInfo() {
    ...
    return {
      ...
      classChain: this.getClassChain(),
      hasEF00Manager: !!this.device._tuyaEF00Manager,
      mainsPowered: typeof this.device.mainsPowered !== 'undefined' ? this.device.mainsPowered : false,
      gangCount: this.device._gangCount || null
    };
  }

  /**
   * Get prototype chain for AI context
   */
  getClassChain() {
    try {
      const chain = [];
      let proto = Object.getPrototypeOf(this.device);
  ...
  isTuyaTS0601() {
    ...
    return CI.equalsCI(modelId, 'TS0601') ||
      CI.startsWithCI(manufacturer, '_TZE200_') ||
      CI.startsWithCI(manufacturer, '_TZE204_') ||
      CI.startsWithCI(manufacturer, '_TZE284_');
  }
  ...
  async checkClusters() {
    ...
    try {
      ...
      // Enumerate ALL endpoints and their clusters
      result.allEndpoints = {};
      const tuyaClusterNames = ['tuya', 'tuyaManufacturer', 'tuyaSpecific',
        'manuSpecificTuya', CLUSTERS.TUYA_EF00, 61184];
      ...
      for (const [epId, endpoint] of Object.entries(zclNode.endpoints || {})) {
        ...
        for (const [clName, cluster] of Object.entries(endpoint.clusters || {})) {
          epInfo.clusterDetails[clName] = {
            id: cluster.id,
            attributes: Object.keys(cluster.attributes || {})
          };
          if (tuyaClusterNames.includes(clName)) {
            result.tuyaClusterFound = true;
            result.tuyaClusterName = clName;
            result.tuyaClusterEndpoint = epId;
          }
        }
        result.allEndpoints[epId] = epInfo;
        if (epId === '1') {
          result.endpoint1Available = true;
          result.available = epInfo.clusters;
        }
      }

      if (!result.endpoint1Available) {
        result.available = [];
      }
  ...
  async checkBatteryStatus() {
    const result = {
      hasBatteryCapability: false, batteryValue: null, lastBatteryUpdate: null, batteryManager: null, source: 'unknown'
    };
  ...
  async testCommunication() {
    const result = {
      canCommunicate: false, responseTime: null, method: null, errors: []
    };
    ...
    try {
      ...
      const endpoint = zclNode.endpoints?.[1];if (!endpoint) {
  ...
  async checkDataFlow() {
    const result = {
      hasRecentData: false, lastDataReceived: null, dataPoints: {},
      listeners: {
        tuyaDP: false,
    ...
    try {
      ...
      // Check if device has registered listeners + EF00 manager state
      const mgr = this.device._tuyaEF00Manager || this.device.tuyaEF00Manager;
      if (mgr) {
        result.listeners.tuyaDP = true;
        if (mgr.passiveMode) result.listeners.mode = 'passive';
        result.ef00ManagerState = {
        ...
        if (mgr._lastDpValues) {
          for (const [dp, val] of Object.entries(mgr._lastDpValues)) {
            result.dataPoints['EF00_DP' + dp] = { value: val.value, type: val.type, ts: val.timestamp };
          }
        }
      }

      // Dump dpMappings if available
      if (typeof this.device.dpMappings === 'object') {
        try { result.dpMappings = JSON.parse(JSON.stringify(this.device.dpMappings)); } catch(e) {}
      }
      ...
      if (result.lastDataReceived) {
        ...
        result.dataAge = Math.floor(safeParse(age, 60000)) + ' minutes';
      }
  ...
  checkDriverAssignment() {
    const result = { currentDriver: null, recommendedDriver: null, isCorrect: null, confidence: 0
    };
    ...
    try {
      ...
      for (const [mfr, driver] of Object.entries(KNOWN_MAPPINGS)) {
        if (manufacturer.toUpperCase().startsWith(mfr.toUpperCase())) {
  ...
  calculateHealthScore() {
    ...
    if (caps) {
      const missingRatio = safeDivide(caps.withoutValues, caps.total);
      if (missingRatio > 0.5) {
        score -= 20;
  ...
  /**
   * v7.5.0: Attempt Local Key Extraction (Shadow Mode Extraction)
   * Only for Wi-Fi devices or those with missing local_key
   */
  async extractLocalKey() {
    this.device.log('[DIAG]  Starting Local Key Extraction...');
    const settings = this.device.getSettings();
    const deviceId = settings.device_id || this.device.getData().id;
    ...
    try {
      const auth = new TuyaSmartLifeAuth({
        region: this.device.homey.settings.get('tuya_cloud_region') || 'eu',
        log: this.device
      });

      // Attempt login with API Key from global settings
      const loginRes = await auth.loginWithApiKey(
        this.device.homey.settings.get('tuya_cloud_access_id'),
        this.device.homey.settings.get('tuya_cloud_access_secret')
      );

      if (!loginRes.success) {
        return { success: false, error: 'Cloud Authentication Failed: ' + loginRes.error };
      }

      const devRes = await auth.getDevicesWithLocalKeys();
      if (!devRes.success) {
        return { success: false, error: 'Failed to fetch cloud devices' };
      }

      const found = devRes.devices.find(d => d.id === deviceId);
      if (found && found.local_key) {
        this.device.log(`[DIAG]  Local Key found: ${found.local_key.slice(0, 4)}****`);
        
        // Update settings silently
        await this.device.setSettings({
          local_key: found.local_key,
          ip_address: found.ip || settings.ip_address
        });

        return { success: true, localKey: found.local_key };
      }

      return { success: false, error: 'Device not found in cloud account or missing local_key' };
    } catch (err) {
      this.device.error('[DIAG]  Extraction failed:', err.message);
