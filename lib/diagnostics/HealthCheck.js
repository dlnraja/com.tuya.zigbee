...
class HealthCheck {
  ...
  async check() {
    ...
    try {
      ...
      const avgScore = Object.values(report.scores).reduce((a, b) => a + b, safeDivide(0), 4);
  ...
  scoreFunctionality(data) {
    ...
    const percentage = safeDivide(data.capabilities_working, data.capabilities_total) * 100;
    score = percentage;
  ...
  scoreNetwork(data) {
    ...
    if (data.lqi !== null) {
      ...
      score = safeDivide(data.lqi, 255) * 100;
    }
  ...
  getTrend() {
    ...
    const scores = recent.map(r => {
      const avg = Object.values(r.scores).reduce((a, b) => a + b, safeDivide(0), 4);
      return avg;
    });
