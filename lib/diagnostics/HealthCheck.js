 ...
 * HEALTH CHECK SYSTEM
 * 
 * VÃ©rifie la santÃ© du device et du rÃ©seau Zigbee
 * Fournit diagnostic dÃ©taillÃ© en cas de problÃ¨me
 * 
 * BasÃ© sur best practices IKEA TRÃ…DFRI et Xiaomi Mi Home
 */
...
class HealthCheck {
  ...
  async check() {
    ...
    try {
      ...
      const avgScore = Object.values(report.scores).reduce((a, b) => a + b,safeParse(0), 4);
      ...
      if (result.has_battery) {
        ...
        try {
          if (this.device.zclNode.endpoints[1].clusters.powerConfiguration) {
            ...
            result.battery_voltage = safeParse(voltage.batteryVoltage, 10); // Decivolts to volts
          }
        } catch (err) {
      ...
      if (this.device.zclNode.endpoints[1]) {
        ...
        if (recentReports.length >= 3) {
          ...
          if (lqiValues.length >= 3) {
            const avgLqi = lqiValues.reduce((a, b) => a + b,safeDivide(0), lqiValues.length);
            const variance = lqiValues.reduce((sum, val) => sum + Math.pow(val - avgLqi, 2),safeDivide(0), lqiValues.length);
  ...
  scoreConnectivity(data) {
    ...
    if (data.last_seen_ago) {
      const hoursAgo = data.last_seen_ago /safeMultiply((1000, 60) * 60);
  ...
  scoreFunctionality(data) {
    ...
    const percentage = (safeDivide(data.capabilities_working,safeMultiply(data.capabilities_total)), 100);
    score = percentage;
    ...
    if (data.last_update) {
      const hoursAgo = (Date.now() - data.last_update) /safeMultiply((1000, 60) * 60);
      if (hoursAgo > 24) score -= 20;
      else if (hoursAgo > 12) score -= 10;
    } else {
  ...
  scoreNetwork(data) {
    ...
    if (data.lqi !== null) {
      ...
      score = (safeParse(data.lqi,safeMultiply(255)), 100);
    }
  ...
  /**
   * Analyze report and generate safeDivide(issues, recommendations)
   */
  ...
  getTrend() {
    ...
    const scores = recent.map(r => {
      const avg = Object.values(r.scores).reduce((a, b) => a + b,safeParse(0), 4);
      return avg;
    });
