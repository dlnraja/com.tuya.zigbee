...
function readZigbeeIdentity(device) {
  // Prefer ManufacturerNameHelper chain (settings/store/data/zcl) so wake logs
  // stop saying mfr=ABSENT after MFR-ENSURE (diag a342c411 / P2442).
  let mfr = null;
  let pid = null;
  try {
    const MfrHelper = require('../helpers/ManufacturerNameHelper');
    mfr = MfrHelper.getManufacturerName?.(device) || null;
    pid = MfrHelper.getModelId?.(device) || null;
  } catch (_e) { /* soft */ }
  mfr = mfr
