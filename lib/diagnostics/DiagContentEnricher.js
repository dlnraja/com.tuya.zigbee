...
const MFR_RE = /\b(_TZE?\d{3}_[a-z0-9]{6,12})\b/gi;
const PID_RE = /\b(TS[A-Z0-9]{4,8})\b/g;
...
function extractCouples(text) {
  const couples = [];
  const blockRe = /(?:manufacturerName|Manufacturer Name|zb_manufacturer_name)[:\s"'`]*(_TZE?\d{3}_[a-z0-9]+)[\s\S]{0,120}?(?:modelId|productId|Model ID|zb_model_id)[:\s"'`]*(TS[A-Z0-9]+)/gi;
  let bm;
  while ((bm = blockRe.exec(text))) {
    if (!isPlaceholderMfr(bm[1])) couples.push({ mfr: bm[1], pid: bm[2] });
  }
  const mfrs = unique([...(text.matchAll(MFR_RE))].map((x) => x[1])).filter((m) => !isPlaceholderMfr(m));
  const pids = unique([...(text.matchAll(PID_RE))].map((x) => x[1])).filter((p) => !/^TS0601_rcbo$/i.test(p));
  // Prefer explicit adjacent couples; only cartesian when single mfr + few pids (avoid glue spam)
  if (mfrs.length === 1 && pids.length === 1) couples.push({ mfr: mfrs[0], pid: pids[0] });
  else if (mfrs.length === 1 && pids.length && pids.length <= 2) {
    for (const pid of pids) couples.push({ mfr: mfrs[0], pid });
  }
  const seen = new Set();
