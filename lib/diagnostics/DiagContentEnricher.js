...
const KNOWN_SIGNALS = [
  ...
  // P2323 — Athom tip email / developer tools dashboard (not device RX)
  { id: 'athom_socket_hang', severity: 'high', re: /socket hang up|Timeout after \d+ms/i, fix: 'P139/P2323: wait Athom; no bump-loop; dashboard $timeout 60s; combo budget P2252' },
  { id: 'athom_processing_failed', severity: 'high', re: /processing_failed|failed to process your (?:app|build)|Build #\d+[\s\S]{0,80}processing_failed/i, fix: 'Check Homey developer tools stateMeta; soft-expect if Test healthy' },
  { id: 'foreign_driver_id', severity: 'critical', re: /Invalid Driver ID:\s*\S+/i, fix: 'P2351 safe-get-driver-patch — update Test + re-pair foreign IDs' },
];
