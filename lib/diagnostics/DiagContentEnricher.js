...
const KNOWN_SIGNALS = [
  ...
  { id: 'heap_oom', severity: 'critical', re: /heap|out of memory|allocation failed|OOM/i, fix: 'LiveData caps / buffer JSON load' },
...
function extractCouples(text) {
  ...
  while ((bm = blockRe.exec(text))) {
    couples.push({ mfr: bm[1], pid: bm[2] });
  }
  const mfrs = unique([...(text.matchAll(MFR_RE))].map((x) => x[1]));
  ...
  else if (mfrs.length && pids.length) {
    for (const mfr of mfrs.slice(0, 4)) {
      for (const pid of pids.slice(0, 4)) couples.push({ mfr, pid });
    }
  }
  const seen = new Set();
