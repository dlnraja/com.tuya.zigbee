...
class DiagnosticAPI {
  ...
  generateRecommendations() {
    ...
    // P2537 — soft RF / evolution tips (never throws; complementary to existing advice)
    try {
      const rfTips = this.getRfCoexistenceBrief();
      if (rfTips?.preferredZigbee?.length) {
        recommendations.push({
          priority: 'MEDIUM',
          category: 'RF_COEXISTENCE',
          issue: 'Wi-Fi 2.4 GHz can overlap Zigbee/Thread channels (numbering differs)',
          occurrences: 1,
          affectedDevices: 0,
          suggestedFix: `Prefer Zigbee/Thread ${rfTips.preferredZigbee.join('/')} when Wi-Fi uses ${rfTips.wifiPrimary.join('/')} @ ${rfTips.wifiWidthMhz} MHz. ${rfTips.suziNote || ''}`.trim()
        });
      }
      // P2538 — soft mesh TX flood tip (Homey Tuya MCU / reporting storms)
      if (rfTips?.meshFloodNote?.length || (rfTips?.tips || []).some((t) => /Mesh TX/i.test(t))) {
        recommendations.push({
          priority: 'MEDIUM',
          category: 'MESH_TX_FLOOD',
          issue: 'Aggressive Tuya MCU polls / power reporting can flood Homey Zigbee mesh',
          occurrences: 1,
          affectedDevices: 0,
          suggestedFix: (rfTips.meshFloodNote && rfTips.meshFloodNote.length)
            ? rfTips.meshFloodNote.join('; ')
            : 'Use BootBudget sleepy TX gates; avoid 0x10 version spam; prefer soft power_report modes'
        });
      }
      // P2539 — Zigbee 4.0 / Suzi intelligent awareness (no invent)
      const intel = rfTips?.intelligentIntegration;
      if (intel?.oneLiner) {
        recommendations.push({
          priority: 'LOW',
          category: 'ZIGBEE40_SUZI',
          issue: 'Zigbee 4.0 / Suzi awareness vs Homey Tuya 2.4 GHz path',
          occurrences: 1,
          affectedDevices: 0,
          suggestedFix: intel.oneLiner
        });
      }
    } catch (_) {
      /* never degrade diagnostics */
    }
  ...
  /**
   * Soft RF + Zigbee/Tuya evolution brief for MCP/diagnostics (P2537).
   * Contre quoi: invent Suzi/GP pids or drop 15/20/25 coexistence advice.
   */
  getRfCoexistenceBrief() {
    try {
      const { getDiagnosticsRfBrief } = require('../utils/zigbee-tuya-evolution');
      return getDiagnosticsRfBrief();
    } catch (_) {
      return {
        wifiPrimary: [1, 6, 11],
  ...
  exportForAI() {
    return {
      ...
      architectureReference: {
        baseClasses: {
          BaseUnifiedDevice: 'Ultimate base - handles both Tuya DP (0xEF00) and ZCL protocols with auto-detection',
         SwitchBase: 'Multi-gang switches with physical button detection and power-on/backlight settings',
         SensorBase: 'All sensors - motion, contact, temp/humidity, air quality, soil, rain, smoke',
         PlugBase: 'Smart plugs, water valves, energy monitoring with metering clusters',
         CoverBase: 'Curtain motors, blinds, shutters with position/tilt control',
         LightBase: 'Bulbs, LED strips, dimmers with color temp and RGB support',
         ThermostatBase: 'TRVs, thermostats with schedule and temperature control',
          ButtonDevice: 'Wireless buttons, scene switches with multi-press detection'
        },
        protocols: {
          TUYA_DP: 'Cluster 0xEF00 (61184) - DP types: 0=Raw, 1=Bool, 2=Value(4byte), 3=String, 4=Enum, 5=Bitmap',
