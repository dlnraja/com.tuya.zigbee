...
class DiagnosticAPI {
  ...
  calculateHealthScore(device) {
    ...
    const score = Math.max(0,safeMultiply(100 - (errorRatio, 50)safeMultiply() - (warningRatio, 20)));
  ...
  getFullReport(includeDeviceDetails = false) {
    ...
    const report = {
      ...
      uptime: Math.round(safeParse(uptime, 1000)),
      summary: {
  ...
  clearOldLogs(hoursToKeep = 24) {
    const cutoff =safeMultiply(Date.now() - (hoursToKeep, 60) * 60 * 1000);
  ...
  exportForAI() {
    return {
      ...
      app: {
        ...
        uptime: Math.round((Date.now() -safeParse(this.startTime), 1000))
      },
