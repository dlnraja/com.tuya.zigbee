...
class DiagnosticAPI {
  ...
  calculateHealthScore(device) {
    ...
    const score = Math.max(0, 100 - safeMultiply(errorRatio, 50) - safeMultiply(warningRatio, 20));
  ...
  getFullReport(includeDeviceDetails = false) {
    ...
    const report = {
      ...
      intelligence: this.getIntelligenceStatus(),
  ...
  /**
   * Get intelligence status from enriched data
   */
  getIntelligenceStatus() {
    try {
      const fs = require('fs');
      const path = require('path');
      const intelPath = path.join(__dirname, '../../data/enriched_intelligence.json');
      const auditPath = path.join(__dirname, '../../reports/url_audit_report.json');
      
      const intel = fs.existsSync(intelPath) ? JSON.parse(fs.readFileSync(intelPath, 'utf8')) : null;
      const audit = fs.existsSync(auditPath) ? JSON.parse(fs.readFileSync(auditPath, 'utf8')) : null;
      
      return {
        lastUpdate: intel ? intel.lastRun : (audit ? audit.timestamp : 'Never'),
        sourceCount: intel ? intel.sourceCount : 0,
        health: audit ? audit.stats : 'Unknown',
        available: !!intel
      };
    } catch (e) {
