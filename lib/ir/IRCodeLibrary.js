...
const BUILTIN_CODES={
  ...
  'Xiaomi':{
    'TV':{
      'Power':{proto:'NEC',addr:0xFE,cmd:0x01},
      'Volume Up':{proto:'NEC',addr:0xFE,cmd:0x04},
      'Volume Down':{proto:'NEC',addr:0xFE,cmd:0x05},
      'OK':{proto:'NEC',addr:0xFE,cmd:0x02},
      'Back':{proto:'NEC',addr:0xFE,cmd:0x03}
    },
    'Fan':{
      'Power':{proto:'NEC',addr:0xFD,cmd:0x01},
      'Oscillate':{proto:'NEC',addr:0xFD,cmd:0x02},
      'Speed':{proto:'NEC',addr:0xFD,cmd:0x03}
    }
  },
  'Bose':{
    'Audio':{
      'Power':{proto:'NEC',addr:0x00,cmd:0x0C},
      'Volume Up':{proto:'NEC',addr:0x00,cmd:0x10},
      'Volume Down':{proto:'NEC',addr:0x00,cmd:0x11},
      'Mute':{proto:'NEC',addr:0x00,cmd:0x0D},
      'Source':{proto:'NEC',addr:0x00,cmd:0x12}
    }
  },
  'Dyson':{
    'Fan':{
      'Power':{proto:'NEC',addr:0x01,cmd:0x01},
      'Speed Up':{proto:'NEC',addr:0x01,cmd:0x02},
      'Speed Down':{proto:'NEC',addr:0x01,cmd:0x03},
      'Oscillate':{proto:'NEC',addr:0x01,cmd:0x04},
      'Timer':{proto:'NEC',addr:0x01,cmd:0x05}
    }
  },
...
/**
 * Convert Pronto Hex to Zosung Base64
 * Pronto format: 0000 Freq Burst1 Burst2 [Mark Space...]
 */
function prontoToBase64(prontoStr) {
  try {
    const parts = prontoStr.trim().split(/[ \t\r\n]+/).filter(x => x);
    ...
    buf.writeUInt16BE(totalPairs * 2, 2); // Zosung counts individual bursts
    
    let offset = 4;
    for (let i = 4; i < 4 + totalPairs * 2; i++) {
      const val = parseInt(parts[i], 16);
      const durationUs = val * cycleUs;
      buf.writeUInt16BE(Math.round(durationUs / 10), offset);
      offset += 2;
    }
...
/**
 * Convert Broadlink Base64 to Zosung Base64
 * Broadlink: 26 00 ... -> Decode timing
 */
function broadlinkToBase64(blStr) {
  try {
    const buf = Buffer.from(blStr, 'base64');
    if (buf[0] !== 0x26) {return null;} // 0x26 = IR
...
class IRCodeLibrary{
  ...
  load(){
    ...
    try{
      if(fs.existsSync(IRDB_PATH)){
        // v9.0.35: Buffer-based reads to avoid UTF-16 heap overhead
        this._irdb=JSON.parse(fs.readFileSync(IRDB_PATH));
        this._index=JSON.parse(fs.readFileSync(INDEX_PATH));
      }
    }catch(e){/* IRDB not available, using builtin codes */}
    this._loaded=true;
  }
  ...
  getCode(brand,category,fn){
    ...
    if(builtin){
      ...
      if(builtin.note){return{source:'builtin',protocol:builtin.proto,code:null,note:builtin.note};}
    }
    // Check IRDB
    const irdb=this._irdb?.brands?.[brand]?.categories?.[category];
    if(irdb){
    ...
    // Check for Pronto Hex or Broadlink
    if (fn.startsWith('0000 ')) {
      const zosung = prontoToBase64(fn);
      if (zosung) {return { source: 'pronto', protocol: 'Pronto', code: zosung };}
    }
    if (fn.startsWith('Jg') || fn.startsWith('2600')) { // Broadlink base64
      const zosung = broadlinkToBase64(fn);
      if (zosung) {return { source: 'broadlink', protocol: 'Broadlink', code: zosung };}
    }
  ...
  // Public exposure of converters
  convertProntoToBase64(pronto) { return prontoToBase64(pronto); }
  convertBroadlinkToBase64(bl) { return broadlinkToBase64(bl); }
}
