...
class ZigbeeHelpers {
   ...
   * Handles SDK v3 getNode() and fallback to getData()
   * 
  ...
  static async getIeeeAddress(device) {
    try {
      // Method 1: Official SDK v3 method (Homey v12+)
      if (device.homey?.zigbee?.getNode) {
        try {
          const zigBeeNode = await device.homey.zigbee.getNode(device);
          if (zigBeeNode?.ieeeAddress) {
            device.log('✅ IEEE address retrieved via getNode()');
            return zigBeeNode.ieeeAddress;
          }
        } catch (err) {
          device.log('⚠️  getNode() failed, trying fallback:', err.message);
        }
      }
      
      // Method 2: Fallback to device data
      const deviceData = device.getData();
      if (deviceData?.ieeeAddress) {
        device.log('✅ IEEE address retrieved via getData() (fallback)');
        return deviceData.ieeeAddress;
      }
      
      // Method 3: Try zclNode (if available)
      if (device.zclNode?.ieeeAddress) {
        device.log('✅ IEEE address retrieved via zclNode (fallback)');
        return device.zclNode.ieeeAddress;
      }
      
      device.log('⚠️  IEEE address not available');
      return null;
      
    } catch (err) {
      device.error('IEEE address retrieval failed:', err.message);
      return null;
    }
  }
  ...
  static async configureAttributeReporting(device, configs) {
    if (!device.configureAttributeReporting) {
      device.log('⚠️  configureAttributeReporting not available');
      return false;
    }
    ...
    try {
      ...
      device.log('✅ Attribute reporting configured:', normalizedConfigs.length, 'configs');
      return true;
      
    } catch (err) {
      device.log('⚠️  Attribute reporting configuration failed (non-critical):', err.message);
      return false;
    }
  }
  ...
  static setupClusterListener(endpoint, clusterSpec, attribute, callback, device) {
    try {
      ...
      if (!cluster) {
        device?.log(`⚠️  Cluster not found:`, clusterSpec);
        return false;
      }
      
      // Setup listener
      cluster.on(`attr.${attribute}`, callback);
      device?.log(`✅ Cluster listener configured:`, clusterSpec, attribute);
      return true;
