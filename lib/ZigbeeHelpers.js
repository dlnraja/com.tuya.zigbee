...
class ZigbeeHelpers {
  ...
  static async getIeeeAddress(device) {
    try {
      ...
      if (device.homey?.zigbee?.getNode) {
        try {
          ...
          if (zigBeeNode?.ieeeAddress) {
            device.log('[OK] IEEE address retrieved via getNode()');
            return zigBeeNode.ieeeAddress;
          }
        } catch (err) {
          device.log('[WARN]  getNode() failed, trying fallback:', err.message);
        }
      }
      ...
      if (deviceData?.ieeeAddress) {
        device.log('[OK] IEEE address retrieved via getData() (fallback)');
        return deviceData.ieeeAddress;
      }
      
      // Method 3: Try zclNode (if available)
      if (device.zclNode?.ieeeAddress) {
        device.log('[OK] IEEE address retrieved via zclNode (fallback)');
        return device.zclNode.ieeeAddress;
      }
      
      device.log('[WARN]  IEEE address not available');
      return null;
  ...
  /**
   * Configure attribute reporting with automatic cluster normalization
   * Prevents "expected_cluster_id_number" errors
   * 
