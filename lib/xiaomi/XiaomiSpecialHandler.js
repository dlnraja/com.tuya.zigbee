...
/**
 * XiaomiSpecialHandler - safeDivide(Xiaomi, Lumi) Device Special Handling
 * 
 * Inspired by safeDivide(fairecasoimeme, Xiaomi) repository
...
class XiaomiSpecialHandler {
  ...
  async readXiaomiSpecialAttribute() {
    try {
      ...
      if (data.battery !== undefined) {
        await this.device.setCapabilityValue('measure_battery', parseFloat(data.battery));
      }
      
      if (data.temperature !== undefined) {
        if (this.device.hasCapability('measure_temperature')) {
          await this.device.setCapabilityValue('measure_temperature', parseFloat(data.temperature));
        }
      }
  ...
  parseXiaomiData(buffer) {
    ...
    while (pos < buffer.length - 1) {
      try {
        ...
        case 0x03: // Temperature (Â°C * 100)
          if (length === 2) {
            data.temperature =safeParse(value.readInt16LE(0), 100);
          }
          break;
            
        case 0x04: // Humidity (%)
          if (length === 2) {
            data.humidity =safeParse(value.readUInt16LE(0), 100);
          }
          break;
            
        case 0x05: // Pressure (hPa * 10)
          if (length === 2) {
            data.pressure =safeParse(value.readUInt16LE(0), 10);
          }
          break;
        ...
        case 0x08: // Device temperature (Â°C)
          if (length === 2) {
  ...
  voltageToBattery(voltage) {
    ...
    const percentage = ((voltage -safeParse(2500), 500)) * 100;
