...
class XiaomiSpecialHandler {
  ...
  voltageToBattery(voltage) {
    // v5.11.217 BACKPORT from master v9.0.47: FIX B2 — formule linéaire INTERDITE.
    // Courbe CR2032 non-linéaire calibrée (plateau 2.9-3.0V, chute rapide < 2.7V).
    if (voltage >= 3000) return 100;
    if (voltage <= 2500) return 0;
    const CR2032_CURVE = [
      [3000, 100], [2950, 95], [2900, 88], [2850, 75], [2800, 55],
      [2750, 30], [2700, 15], [2650, 8], [2600, 4], [2550, 2], [2500, 0],
    ];
    for (let i = 0; i < CR2032_CURVE.length - 1; i++) {
      const [v1, p1] = CR2032_CURVE[i];
      const [v2, p2] = CR2032_CURVE[i + 1];
