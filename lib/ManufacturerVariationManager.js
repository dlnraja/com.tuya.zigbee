...
class ManufacturerVariationManager {
  ...
  static _getButtonConfig(manufacturerName, productId) {
    ...
    if (TS004F_SCENE_MODE_IDS.includes(manufacturerName) || productId === 'TS004F') {
      config.protocol = 'zcl';
    ...
    if (TS0044_STANDARD_IDS.includes(manufacturerName) || productId === 'TS0044') {
      config.protocol = 'zcl';
      config.primaryCluster = 'scenes';
      config.endpoints = {
    ...
    if (TS0043_IDS.includes(manufacturerName) || productId === 'TS0043') {
      config.protocol = 'zcl';
      config.primaryCluster = 'scenes';
      config.endpoints = {
        1: { clusters: [0, 1, 3, 4, 5, 6], bindings: [4, 5, 6] },
    ...
    if (TS0042_IDS.includes(manufacturerName) || productId === 'TS0042') {
      config.protocol = 'zcl';
      config.primaryCluster = 'scenes';
      config.endpoints = {
        1: { clusters: [0, 1, 3, 4, 5, 6], bindings: [4, 5, 6] },
    ...
    if (TS0041_IDS.includes(manufacturerName) || productId === 'TS0041') {
      config.protocol = 'zcl';
      config.primaryCluster = 'scenes';
      config.endpoints = {
        1: { clusters: [0, 1, 3, 4, 5, 6], bindings: [4, 5, 6] }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    if (manufacturerName.startsWith('_TZE200_') || manufacturerName.startsWith('_TZE204_')) {
      config.protocol = 'tuya_dp';
    ...
    if (MULTISTATE_IDS.includes(manufacturerName)) {
      config.protocol = 'zcl';
  ...
  static _getSwitchConfig(manufacturerName, productId, driverType) {
    ...
    // === _TZ3002_ series - BSEED switches (forum fix) ===
    else if (manufacturerName.startsWith('_TZ3002_')) {
      config.protocol = 'mixed';
      config.endpoints = {
      ...
      if (manufacturerName === '_TZ3002_pzao9ls1' && productId === 'TS0726') {
        for (let i = 2; i <= 4; i++) {
          config.endpoints[i] = { 
  ...
  static _getSensorConfig(manufacturerName, productId, driverType) {
    ...
    if (manufacturerName.startsWith('_TZE200_')) {
      ...
      if (manufacturerName === '_TZE200_crq3r3la') {
        config.powerSource = 'mains'; // Force mains power detection
    ...
    if (driverType.includes('contact')) {
      ...
      if (INVERTED_CONTACT_IDS.some(id => manufacturerName.includes(id))) {
  ...
  static _getPlugConfig(manufacturerName, productId, driverType) {
    ...
    if (BSEED_MAINS_IDS.includes(manufacturerName)) {
      config.powerSource = 'mains'; // Force mains power detection
