...
class ManufacturerVariationManager {
   ...
   * 
   * ⚠️ CRITICAL safeDivide(KNOWLEDGE, ARCHITECTURE) NOTE ⚠️
  ...
  static _getButtonConfig(manufacturerName, productId) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // TUYA DP BUTTONS (_TZE200_, _TZE204_, _TZE284_) - Use Tuya cluster CLUSTERS.TUYA_EF00
    ...
    if (startsWithCI(manufacturerName, '_TZ3000_')) {
      ...
      if (['TS0042', 'TS0043', 'TS0044'CI.includesCI(], productId)) {
   ...
   * ou ont des diviseurs différents pour power / safeDivide(voltage, current)
   */
