...
class ManufacturerVariationManager {
  ...
  static getManufacturerConfig(manufacturerName, productId, driverType) {
    ...
    // === WATER VALVE VARIATIONS ===
    if (driverType === 'water_valve_garden' || driverType.includes('valve')) {
      return this._getWaterValveConfig(manufacturerName, productId, driverType);
    }

    // === PHYSICAL BUTTON TIMING PROFILES ===
    if (driverType === 'physical_button_profile') {
      return this._getPhysicalButtonProfile(manufacturerName, productId);
    }
  ...
  /**
   * Configuration spécifique pour physical_button_profile
   * v9.7.2: Unification of all historical forum fixes (BSEED, Moes, Zemismart)
   */
  static _getPhysicalButtonProfile(manufacturerName, productId) {
    const profiles = {
      'BSEED': { 
        appCommandWindow: 2000, doubleClickWindow: 500, longPressThreshold: 800,
        protocol: 'zcl_only', brand: 'BSEED', groupIsolation: true, sceneModeDetach: true
      },
      'Zemismart': { 
        appCommandWindow: 1500, doubleClickWindow: 400, longPressThreshold: 700,
        protocol: 'zcl_only', brand: 'Zemismart', perEndpointControl: true, sceneModeDetach: true
      },
      'Moes': { 
        appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
        protocol: 'hybrid', brand: 'Moes', sceneModeDetach: true
      },
      'LoraTap': { 
        appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
        protocol: 'hybrid', brand: 'LoraTap'
      },
      'Sonoff': { 
        appCommandWindow: 800, doubleClickWindow: 400, longPressThreshold: 500,
        protocol: 'zcl_only', brand: 'Sonoff'
      },
      'Lonsonho': { 
    ...
    // Special ID matches (v5.5.999 database)
    const specialIds = {
      '_TZ3000_blhvsaqf': profiles.BSEED,
      '_TZ3000_ysdv91bk': profiles.BSEED,
      '_TZ3000_hafsqare': profiles.BSEED,
      '_TZ3000_e98krvvk': profiles.BSEED,
      '_TZ3000_iedbgyxt': profiles.BSEED,
      '_TZ3000_l9brjwau': profiles.BSEED,
      '_TZ3000_qkixdnon': profiles.BSEED,
      '_TZ3000_a37eix1s': profiles.Zemismart,
      '_TZ3000_empogkya': profiles.Zemismart,
      '_TZ3000_18ejxrzk': profiles.Zemismart,
      '_TZ3000_zgyzgdua': profiles.Moes,
      '_TZ3000_abrsvsou': profiles.Moes,
      '_TZ3000_mh9px7cq': profiles.Moes,
      '_TZ3000_uri7ez8u': profiles.Moes,
      '_TZ3000_rrjr1q0u': profiles.Moes,
      '_TZ3000_vp6clf9d': profiles.Moes,
      '_TZ3000_iszegwpd': profiles.LoraTap,
      '_TZ3000_famkxci2': profiles.LoraTap,
      '_TZ3002_pzao9ls1': profiles.BSEED,
      '_TZ3002_vaq2bfcu': profiles.BSEED,
      'eWeLink': profiles.Sonoff
    };

    if (manufacturerName && specialIds[manufacturerName]) {return specialIds[manufacturerName];}
    
    if (containsCI(manufacturerName, 'BSEED')) {return profiles.BSEED;}
    if (containsCI(manufacturerName, 'Zemismart')) {return profiles.Zemismart;}
    if (containsCI(manufacturerName, 'Moes')) {return profiles.Moes;}
    if (containsCI(manufacturerName, 'LoraTap')) {return profiles.LoraTap;}
    if (containsCI(manufacturerName, 'Sonoff')) {return profiles.Sonoff;}
    if (containsCI(manufacturerName, 'Lonsonho')) {return profiles.Lonsonho;}
    if (containsCI(manufacturerName, 'HOBEIAN')) {return profiles.Moes;}
  ...
  static _getCurtainMotorConfig(manufacturerName, productId) {
    ...
    // WHY(P2335): Moes/ZHA interview is Basic+Groups+Scenes+EF00 ([0,4,5,61184]).
    // Pairing uses compose; runtime MVM must stay EF00-family (never invent 6/258).
    const ef00CurtainClusters = [0, 4, 5, 61184];

    // === _TZE200_ series - Pure Tuya DP ===
    if (startsWithCI(manufacturerName, '_TZE200_')) {
      config.protocol = 'tuya_dp';
      config.endpoints[1].clusters = ef00CurtainClusters.slice();
      config.endpoints[1].bindings = [61184];
      config.dpMappings = {
      ...
      // WHY(P2461 / MIAMO #2229): AM43 solar/battery (`icka1clh` / `zah67ekd`) —
      // cover_4 EF00 only. Never invent tilt / ZCL 258 (UNSUPPORTED_CLUSTER).
      if (/icka1clh|zah67ekd/i.test(String(manufacturerName || ''))) {
        config.specialHandling = 'am43_battery_tubular_ef00';
        config.capabilities = ['windowcoverings_state', 'windowcoverings_set', 'measure_battery'];
        config.dpMappings = {
          1: { capability: 'windowcoverings_state', transform: (v) => v === 0 ? 'up' : v === 2 ? 'down' : 'idle' },
          2: { capability: 'windowcoverings_set', transform: (v) => v / 100 },
          3: { capability: 'windowcoverings_set', transform: (v) => v / 100 },
    ...
    // === _TZE204_ / _TZE284_ — Pure Tuya DP (v5.5.998 + P2335 TZE284 sibling) ===
    ...
    // WHY(P2335): _TZE284_* previously fell through to legacy ZCL 6/258 — wrong for ZTS-EUR-C siblings.
    else if (startsWithCI(manufacturerName, '_TZE204_') || startsWithCI(manufacturerName, '_TZE284_')) {
      config.protocol = 'tuya_dp';
      config.endpoints[1].clusters = ef00CurtainClusters.slice();
      config.endpoints[1].bindings = [61184];
      ...
      config.dpMappings = {
        1: { capability: 'windowcoverings_state', transform: (v) => v === 0 ? 'up' : v === 2 ? 'down' : 'idle' },
        2: { capability: 'windowcoverings_set', transform: (v) => (100 - v) / 100 }, // INVERTED position read
        3: { capability: 'windowcoverings_set', transform: (v) => (100 - v) / 100 }, // INVERTED position state
        5: { capability: null, setting: 'reverse_direction' },
        13: { capability: 'measure_battery', divisor: 1 }
      };
  ...
  static _getButtonConfig(manufacturerName, productId) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // WHY(P2328): `_TZ3000_wkai4ga5` is TS0044 scene remote (Z2M / scene_switch_4),
    // NOT TS004F. Listing it here early-returns with sceneModeAttribute=0x8004 and
    // dead-codes the specific fix below — meter91-class presses go silent.
    const TS004F_SCENE_MODE_IDS = [
      ...
      '_TZ3000_qzjcsmar', '_TZ3000_5tqxpine',
      '_TZ3000_abrsvsou', '_TZ3000_kjfzuycl',
      '_TZ3000_owgcnkrh', '_TZ3000_rrjr1dsk', '_TZ3000_vdfwjopk'
    ];
    // v5.12.5: Removed _TZ3000_ja5osu5g from 4-gang list (it is 1-gang)
    // v5.12.5: TS004F should NOT default to 4-gang without manufacturer verification
    if (includesCI(TS004F_SCENE_MODE_IDS, manufacturerName)) {
      config.protocol = 'zcl';
    ...
    const TS0044_STANDARD_IDS = [
      ...
      '_TZ3000_zgyzgdua', '_TZ3000_ufhtxr59', '_TZ3000_wkai4ga5'
    ];
    // WHY(P2328): mfr-first before bare productId — known TS0044 remotes never
    // inherit TS004F 0x8004 (wkai4ga5 was wrongly in TS004F_SCENE_MODE_IDS).
    if (includesCI(TS0044_STANDARD_IDS, manufacturerName) || equalsCI(productId, 'TS0044')) {
      config.protocol = 'zcl';
      config.primaryCluster = 'scenes';
      config.sceneModeAttribute = null;
      config.endpoints = {
      ...
      config.specialHandling = includesCI(['_TZ3000_wkai4ga5'], manufacturerName)
        ? 'ts0044_scene_switch'
        : 'ts0044_standard';
      if (includesCI(['_TZ3000_wkai4ga5'], manufacturerName)) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    const TS0043_E000_IDS = [
      '_TZ3000_famkxci2'  // LoraTap SS600ZB, SS9300ZB - uses cluster 0xE000
    ];
    if (includesCI(TS0043_E000_IDS, manufacturerName)) {
      config.protocol = 'zcl';
      ...
      config.specialHandling = 'ts0043_loratap_e000';
      return config;
    }
    ...
    const MULTISTATE_IDS = [
      '_TZ3000_gjnozsaz', '_TZ3000_fkvaniuu'
    ];
  ...
  static _getSwitchConfig(manufacturerName, productId, driverType) {
    ...
    // === _TZ3000_ series - Standard ZCL ===
      ...
      // v5.13.7: _TZ3000_h1ipgkwn (2-channel USB switch with power measurement)
      if (equalsCI(manufacturerName, '_TZ3000_h1ipgkwn')) {
        config.protocol = 'mixed';
        config.endpoints[1].clusters.push(2820, 1794); // ElectricalMeasurement + Metering
        config.endpoints[1].bindings.push(2820, 1794);
        config.zclClusters.push(2820, 1794);
        config.specialHandling = 'usb_switch_power';
        config.capabilities.push('measure_power', 'meter_power', 'measure_voltage', 'measure_current');
        config.energyScaling = {
          power_divisor: 10,
          voltage_divisor: 10,
  ...
  static _getSensorConfig(manufacturerName, productId, driverType) {
    ...
    // === HOBEIAN Sensor Fleet (Radar & Multisensors) ===
    if (containsCI(manufacturerName, 'HOBEIAN')) {
      config.protocol = 'mixed';
      config.powerSource = 'battery'; // Default for HOBEIAN sensors

      if (containsCI(productId, 'ZG-204ZM') || containsCI(manufacturerName, '2aaelwxk')) {
        config.specialHandling = 'hobeian_radar_zg204zm';
        config.capabilities = ['alarm_motion', 'measure_luminance.distance', 'measure_luminance', 'measure_battery'];
        config.dpMappings = {
          1: { capability: 'alarm_motion', transform: (v) => Boolean(v) },
          3: { capability: 'measure_luminance.distance', divisor: 100 },
          106: { capability: 'measure_luminance', divisor: 1 },
          121: { capability: 'measure_battery', divisor: 1 }
        };
      } else if (containsCI(productId, 'ZG-204ZV') || containsCI(manufacturerName, 'grgol3xp')) {
        config.specialHandling = 'hobeian_multisensor_zg204zv';
        config.capabilities = ['alarm_motion', 'measure_temperature', 'measure_humidity', 'measure_luminance', 'measure_battery'];
        config.dpMappings = {
          1: { capability: 'alarm_motion', transform: (v) => Boolean(v) },
          3: { capability: 'measure_temperature', divisor: 10 },
          4: { capability: 'measure_humidity', multiplier: 10 },
  ...
  /**
   * Configuration spécifique pour water_valve_garden
   */
  static _getWaterValveConfig(manufacturerName, productId, driverType) {
    const config = {
      protocol: 'mixed',
      endpoints: { 1: { clusters: [0, 1, 6], bindings: [6] } },
      dpMappings: {},
      zclClusters: [6], // OnOff
    ...
    // Detect pure Tuya DP water valves
    if (startsWithCI(manufacturerName, '_TZE200_') || startsWithCI(manufacturerName, '_TZE284_')) {
      config.protocol = 'tuya_dp';
      config.endpoints[1].clusters = [61184]; // 0xEF00
      config.endpoints[1].bindings = [];
      config.zclClusters = [];
      ...
      config.dpMappings = {
        ...
        2: { capability: 'onoff', transform: (v) => Boolean(v) }, // Support dual valves state
        5: { capability: null, internal: 'countdown' },       // countdown timer
        11: { capability: null, internal: 'countdown' },      // alternative countdown timer
        12: { capability: null, internal: 'work_state' },     // work state / status
        104: { capability: null, internal: 'work_state' },    // alternative work state
        7: { capability: 'measure_battery', divisor: 1 },     // battery percentage
        15: { capability: 'measure_battery', divisor: 1 }     // alternative battery percentage
      };

      config.specialHandling = 'tuya_dp_water_valve';
    }
  ...
  static resolveDriverType(manufacturerName, productId) {
    ...
    // WHY(P2328): sacred couples beat bare productId map — TS0044 alone used to
    // always resolve button_wireless_4 and override meter91 / Moes scene locks.
    // Z2M: u3nv1jwk=TM-YKQ004, bgtzm4ny=HOBEIAN ZG-101ZS → button_wireless_4 compose lock
    const SCENE_SWITCH_4_TS0044 = [
      '_TZ3000_zgyzgdua', '_TZ3000_wkai4ga5', '_TZ3000_vp6clf9d',
      '_TZ3000_ufhtxr59', '_TZ3000_abci1hiu', '_TZ3000_a4xycprs',
      '_TZ3000_mh9px7cq', '_TZ3000_nuombroo',
      '_TZ3000_et7afzxz', '_TZ3000_j61x9rxn', '_TZ3000_ee8nrt2l',
      '_TZ3000_w4thianr', '_TZ3000_dziaict4',
      '_TZ3000_b3mgfu0d', '_TZ3000_uaa99arv',
    ];
    if (p === 'TS0044' && includesCI(SCENE_SWITCH_4_TS0044, manufacturerName)) {
      return 'scene_switch_4';
    }
    if (p === 'TS0044' && includesCI(['_TZ3000_kfu8zapd'], manufacturerName)) {
      return 'button_wireless_4';
    }
    // WHY(P2332): Nobø SWS-IZ (Z2M #12768) — TS004F but skip8004; lock wireless_4
    if (p === 'TS004F' && includesCI(['_TZ3000_xffhmvhv'], manufacturerName)) {
      return 'button_wireless_4';
    }
    const map = {
  ...
  static needsSpecialConfig(manufacturerName, productId, driverType) {
    ...
    const specialPatterns = [
      ...
      'HOBEIAN', 'hobeian'                // HOBEIAN sensor fleet
    ];
