...
class ManufacturerVariationManager {
  ...
  static _getCurtainMotorConfig(manufacturerName, productId) {
    ...
    // === _TZE204_ series - Pure Tuya DP (v5.5.997: Longsam Mini M3 etc.) ===
    // Some _TZE204_ curtains use INVERTED control: open=2, close=0 (opposite of standard)
    // GitHub #122: elgato7 Longsam _TZE204_xu4a5rhj - commands sent but motor doesn't respond
    else if (startsWithCI(manufacturerName, '_TZE204_')) {
      config.protocol = 'tuya_dp';
      config.endpoints[1].clusters = [61184]; // 0xEF00 only
      config.endpoints[1].bindings = [];
      
      // v5.5.997: Longsam Mini M3 and similar _TZE204_ curtains use INVERTED DP1 values
      // Standard: open=0, stop=1, close=2
      // Inverted: open=2, stop=1, close=0
      config.specialHandling = 'tze204_curtain_inverted';
      config.dpMappings = {
        1: { capability: 'windowcoverings_state', transform: (v) => v === 2 ? 'up' : v === 0 ? 'down' : 'idle' },
        2: { capability: 'windowcoverings_set', transform: (v) => v / 100 },
        3: { capability: 'dim', transform: (v) => v / 100 }
      };
      // v5.5.997: Flag for inverted control commands
      config.invertedControl = true;
    }
