...
class BaseHybridDevice extends ZigBeeDevice {
   ...
   */
  async onNodeInit() {
    // 🚨 CRITICAL: Call parent initialization FIRST to get zclNode
    try {
      await super.onNodeInit();
      console.log('✅ [SUPER] Parent ZigBeeDevice initialized');
    } catch (err) {
      console.error('❌ [SUPER] Parent init failed:', err.message);
    }
    
    // Wait for zclNode to be available (critical!)
    let retries = 0;
    ...
    if (!this.zclNode) {
      ...
      // Continue anyway with safe defaults
    } else {
  ...
  async _runBackgroundInitialization() {
    try {
      ...
      // Step 4: Setup monitoring and reporting
      this.log('[BACKGROUND] Step 3b/4: Setting up monitoring...');
      await this.setupMonitoring();
      this.log('[BACKGROUND] Monitoring configured');
      
      // Step 5: CRITICAL - Force read ALL initial values (KPIs)
