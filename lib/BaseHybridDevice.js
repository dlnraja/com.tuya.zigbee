...
class BaseHybridDevice extends ZigBeeDevice {
    ...
    try {
      ...
      if (powerCluster) {
        ...
        if (batteryVoltage?.batteryVoltage) {
          ...
          // Store voltage for display if setting enabled
          if (this.getSetting('show_voltage')) {
    ...
    try {
      ...
      this.registerCapability('measure_battery', 1, { // Cluster 1 = powerConfiguration
        ...
        reportParser: (value) => {
          ...
          // Monitor thresholds for flow cards
          this.monitorBatteryThresholds(level).catch(err => 
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('energy_optimization')) {
      this.log('Energy optimization changed:', newSettings.energy_optimization);
