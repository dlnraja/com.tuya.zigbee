...
class BaseHybridDevice extends ZigBeeDevice {
      ...
      if (basicCluster) {
        ...
        if (attributes?.powerSource !== undefined) {
          ...
          if (powerSource === 0x01 || powerSource === 0x02 || powerSource === 0x05 || powerSource === 0x06 ||
            ...
            this.log('⚠️  Unknown power source value, using fallback detection');
  ...
  async setupBatteryMonitoring() {
    ...
    if (await this.setupStandardBatteryMonitoring()) {
      this.log('✅ Standard battery monitoring configured');
      return;
    }
    
    if (await this.setupTuyaBatteryMonitoring()) {
      this.log('✅ Tuya battery monitoring configured');
      return;
    }
    
    if (await this.setupAlternativeEndpointBattery()) {
      this.log('✅ Alternative endpoint battery configured');
      return;
    }
    
    this.log('⚠️  No battery monitoring available (device may not support it)');
  }
  ...
  async setupStandardBatteryMonitoring() {
    try {
      ...
      // SDK3: Configure attribute reporting (numeric cluster ID)
      ...
      this.log('✅ Standard battery monitoring configured (SDK3)');
      return true;
    ...
    try {
        ...
        try {
          ...
          if (data.dp === 15 || data.datapoint === 15) {
            ...
            if (typeof battery === 'number' && battery >= 0 && battery <= 100) {
              this.log(`🔋 Tuya battery update: ${battery}%`);
              if (this.hasCapability('measure_battery')) {
          ...
          if (data.dp === 14 || data.datapoint === 14) {
            this.log(`🔋 Tuya battery state: ${data.value}`);
          }
        } catch (err) {
      ...
      try {
        ...
        if (batteryData?.value) {
          this.log(`🔋 Tuya initial battery: ${batteryData.value}%`);
          if (this.hasCapability('measure_battery')) {
      ...
      for (const epId of [2, 3]) {
        ...
        if (powerCluster) {
          ...
          powerCluster.on('attr.batteryPercentageRemaining', async (value) => {
            ...
            this.log(`🔋 Battery (ep${epId}): ${battery}%`);
            if (this.hasCapability('measure_battery')) {
              await this.setCapabilityValue('measure_battery', battery).catch(this.error);
            }
          });
          ...
          await this.configureAttributeReporting([{
            ...
            maxInterval: 43200,
            minChange: 2
          }]).catch(err => this.log(`Battery reporting config ep${epId} (non-critical):`, err.message));
  ...
  async setupACMonitoring() {
    ...
    try {
      ...
      this.log('⚠️  AC monitoring temporarily disabled - needs SDK3 refactoring');
      this.log('💡 AC devices will work, but power measurements need manual SDK3 implementation');
      
      // const endpoint = this.zclNode.endpoints[1];
      // if (!endpoint?.clusters?.haElectricalMeasurement) {
      //   this.log('⚠️  haElectricalMeasurement cluster not available');
      //   return;
      // }
      //
  ...
  async setupDCMonitoring() {
    ...
    try {
      ...
      this.log('⚠️  DC monitoring temporarily disabled - needs SDK3 refactoring');
      this.log('💡 DC devices will work, but power measurements need manual SDK3 implementation');
  ...
  /**
   * Override configureAttributeReporting to use ZigbeeHelpers auto-converter
   * Automatically normalizes cluster specifications to prevent errors
   */
  async configureAttributeReporting(configs) {
    return ZigbeeHelpers.configureAttributeReporting(this, configs);
  }
