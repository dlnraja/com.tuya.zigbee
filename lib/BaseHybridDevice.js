...
class BaseHybridDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      await this.Promise.resolve(setAvailable()).catch(err => {
    ...
    try {
      ...
      this.Promise.resolve(_runBackgroundInitialization()).catch(err => {
      ...
      if (this.powerType === 'AC' || this.powerType === 'DC') {
        ...
        for (const capability of recommended) {
          if (!this.hasCapability(capability)) {
            await this.Promise.resolve(addCapability(capability)).catch(err => {
        ...
        for (const capability of ['measure_voltage', 'measure_current', 'measure_power', 'meter_power']) {
          if (this.hasCapability(capability)) {
            ...
            if (shouldHide) {
              await this.Promise.resolve(removeCapability(capability)).catch(() => {});
        ...
        if (this.hasCapability('measure_battery')) {
          await this.Promise.resolve(removeCapability('measure_battery')).catch(() => {});
        ...
        if (!this.hasCapability('measure_battery')) {
          await this.Promise.resolve(addCapability('measure_battery')).catch(() => {});
  ...
  async setupStandardBatteryMonitoring() {
    try {
      ...
      try {
        const batteryData = await endpoint.clusters.powerConfiguration.Promise.resolve(readAttributes(['batteryPercentageRemaining', 'batteryVoltage'])).catch(err => {
        ...
        if (batteryData?.batteryPercentageRemaining !== undefined) {
          ...
          if (this.hasCapability('measure_battery')) {
            await this.Promise.resolve(setCapabilityValue('measure_battery', percentage)).catch(this.error);
          }
        }
      ...
      await endpoint.clusters.powerConfiguration.on('attr.batteryPercentageRemaining', value => {
        ...
        this.Promise.resolve(setCapabilityValue('measure_battery', percentage)).catch(this.error);
  ...
  async setupTuyaBatteryMonitoring() {
    try {
        ...
        try {
          ...
          if (data.dp === 15 || data.datapoint === 15) {
            ...
            if (typeof battery === 'number' && battery >= 0 && battery <= 100) {
              ...
              if (this.hasCapability('measure_battery')) {
                await this.Promise.resolve(setCapabilityValue('measure_battery', battery)).catch(this.error);
              }
            }
          }
      ...
      try {
        const batteryData = await tuyaCluster.Promise.resolve(read('dp', 15)).catch(() => null);
        if (batteryData?.value) {
          this.log(`[BATTERY] Tuya initial battery: ${batteryData.value}%`);
          if (this.hasCapability('measure_battery')) {
            await this.Promise.resolve(setCapabilityValue('measure_battery', batteryData.value)).catch(this.error);
          }
        }
      } catch (err) {
    ...
    try {
      ...
      for (const epId of [2, 3]) {
        ...
        if (powerCluster) {
          ...
          try {
            const batteryValue = await powerCluster.Promise.resolve(readAttributes(['batteryPercentageRemaining'])).catch(err => {
  ...
  async _forceReadAllInitialValues() {
    ...
    if (this.hasCapability('measure_temperature')) {
      try {
        ...
        if (tempCluster) {
          const temp = await tempCluster.Promise.resolve(readAttributes(['measuredValue'])).catch(() => null);
          if (temp?.measuredValue !== undefined) {
            const tempC = temp.measuredValue / 100;
            await this.Promise.resolve(setCapabilityValue('measure_temperature', tempC)).catch(this.error);
    ...
    if (this.hasCapability('measure_humidity')) {
      try {
        ...
        if (humCluster) {
          const hum = await humCluster.Promise.resolve(readAttributes(['measuredValue'])).catch(() => null);
          if (hum?.measuredValue !== undefined) {
            const humPercent = hum.measuredValue / 100;
            await this.Promise.resolve(setCapabilityValue('measure_humidity', humPercent)).catch(this.error);
    ...
    if (this.hasCapability('measure_luminance')) {
      try {
        ...
        if (illumCluster) {
          const illum = await illumCluster.Promise.resolve(readAttributes(['measuredValue'])).catch(() => null);
          if (illum?.measuredValue !== undefined) {
            const lux = Math.pow(10, (illum.measuredValue - 1) / 10000);
            await this.Promise.resolve(setCapabilityValue('measure_luminance', lux)).catch(this.error);
    ...
    if (this.hasCapability('measure_power')) {
      try {
        ...
        if (powerCluster) {
          const power = await powerCluster.Promise.resolve(readAttributes(['activePower'])).catch(() => null);
          if (power?.activePower !== undefined) {
            const watts = power.activePower;
            await this.Promise.resolve(setCapabilityValue('measure_power', watts)).catch(this.error);
    ...
    if (this.hasCapability('measure_voltage')) {
      try {
        const powerCluster = endpoint.clusters?.electricalMeasurement || endpoint.clusters?.haElectricalMeasurement;
        if (powerCluster) {
          const voltage = await powerCluster.Promise.resolve(readAttributes(['rmsVoltage'])).catch(() => null);
          if (voltage?.rmsVoltage !== undefined) {
            const volts = voltage.rmsVoltage;
            await this.Promise.resolve(setCapabilityValue('measure_voltage', volts)).catch(this.error);
    ...
    if (this.hasCapability('measure_current')) {
      try {
        const powerCluster = endpoint.clusters?.electricalMeasurement || endpoint.clusters?.haElectricalMeasurement;
        if (powerCluster) {
          const current = await powerCluster.Promise.resolve(readAttributes(['rmsCurrent'])).catch(() => null);
          if (current?.rmsCurrent !== undefined) {
            const amps = current.rmsCurrent / 1000; // mA to A
            await this.Promise.resolve(setCapabilityValue('measure_current', amps)).catch(this.error);
    ...
    if (this.hasCapability('onoff')) {
      try {
        ...
        if (onOffCluster) {
          const state = await onOffCluster.Promise.resolve(readAttributes(['onOff'])).catch(() => null);
          if (state?.onOff !== undefined) {
            await this.Promise.resolve(setCapabilityValue('onoff', state.onOff)).catch(this.error);
    ...
    if (this.hasCapability('alarm_motion')) {
      try {
        ...
        if (occCluster) {
          const occ = await occCluster.Promise.resolve(readAttributes(['occupancy'])).catch(() => null);
          if (occ?.occupancy !== undefined) {
            const occupied = (occ.occupancy & 1) === 1;
            await this.Promise.resolve(setCapabilityValue('alarm_motion', occupied)).catch(this.error);
    ...
    if (this.hasCapability('alarm_contact')) {
      try {
        ...
        if (iasCluster) {
          const zone = await iasCluster.Promise.resolve(readAttributes(['zoneStatus'])).catch(() => null);
          if (zone?.zoneStatus !== undefined) {
            const open = (zone.zoneStatus & 1) === 1;
            await this.Promise.resolve(setCapabilityValue('alarm_contact', open)).catch(this.error);
      ...
      if (clusterName === 'onOff') {
        if (command === 'on' || command === 'off') {
          ...
          if (!this.hasCapability(cap)) {
            await this.Promise.resolve(addCapability(cap)).catch(this.error);
          }
          
          await this.Promise.resolve(setCapabilityValue(cap, state)).catch(this.error);
        ...
        if (command === 'toggle') {
          ...
          const current = await this.Promise.resolve(getCapabilityValue(cap)).catch(() => false);
          await this.Promise.resolve(setCapabilityValue(cap, !current)).catch(this.error);
          return;
        }
      }
  ...
  async setTuyaDpValue(dp, value) {
    ...
    await this.Promise.resolve(setCapabilityValue(cap, value)).catch(this.error);
  }
    ...
    try {
      ...
      if (this.hasCapability('measure_battery') && endpoint.clusters?.powerConfiguration) {
        try {
          ...
          const batteryData = await endpoint.clusters.powerConfiguration.Promise.resolve(readAttributes(['batteryPercentageRemaining'])).catch(() => null);
          ...
          if (batteryData && batteryData.batteryPercentageRemaining != null) {
            ...
            const voltageData = await endpoint.clusters.powerConfiguration.Promise.resolve(readAttributes(['batteryVoltage'])).catch(() => null);
            if (voltageData && voltageData.batteryVoltage) {
              ...
              await this.Promise.resolve(setCapabilityValue('measure_battery', percent)).catch(this.error);
            }
          }
        } catch (err) {
      ...
      if (endpoint.clusters?.temperatureMeasurement || endpoint.clusters?.msTemperatureMeasurement) {
        try {
          const cluster = endpoint.clusters.temperatureMeasurement || endpoint.clusters.msTemperatureMeasurement;
          const tempData = await cluster.Promise.resolve(readAttributes(['measuredValue'])).catch(() => null);
          if (tempData && tempData.measuredValue !== null) {
            this.log(`[INITIAL-READ] ✅ Temperature: ${tempData.measuredValue / 100}°C`);
            if (this.hasCapability('measure_temperature')) {
              await this.Promise.resolve(setCapabilityValue('measure_temperature', tempData.measuredValue / 100)).catch(this.error);
            }
          }
        } catch (err) {
      ...
      if (endpoint.clusters?.relativeHumidity || endpoint.clusters?.msRelativeHumidity) {
        try {
          const cluster = endpoint.clusters.relativeHumidity || endpoint.clusters.msRelativeHumidity;
          const humidityData = await cluster.Promise.resolve(readAttributes(['measuredValue'])).catch(() => null);
          if (humidityData && humidityData.measuredValue !== null) {
            this.log(`[INITIAL-READ] ✅ Humidity: ${humidityData.measuredValue / 100}%`);
            if (this.hasCapability('measure_humidity')) {
              await this.Promise.resolve(setCapabilityValue('measure_humidity', humidityData.measuredValue / 100)).catch(this.error);
            }
          }
        } catch (err) {
