...
class BaseHybridDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      ...
      if (this.zclNode) {
        // CRITICAL FIX: Force read device info if not available
        if (!this.zclNode.manufacturerName || !this.zclNode.modelId) {
          ...
          try {
            ...
            if (basicCluster) {
              const deviceInfo = await basicCluster.readAttributes(['manufacturerName', 'modelId', 'zclVersion']).catch(err => {
                console.error('❌ Failed to read device info:', err.message);
                return null;
              });
              if (deviceInfo) {
                this.log('✅ Device info read:', JSON.stringify(deviceInfo));
                // ✅ ASSIGN to zclNode so they're not "unknown" anymore!
  ...
  async _runBackgroundInitialization() {
    try {
      ...
      */
      ...
      // Step 6: Start monitoring system
      this.log('[BACKGROUND] Step 3e/7: Starting monitoring...');
      try {
        await this.startMonitoring();
        this.log('[BACKGROUND] Monitoring started');
      } catch (err) {
        this.log('[BACKGROUND] ⚠️  Monitoring setup failed:', err.message);
        this.log('[BACKGROUND] Continuing without monitoring...');
      }
      
      // Step 7: Force read all important values NOW (after everything else)
      this.log('[BACKGROUND] Step 3f/7: Force reading initial KPI values...');
      try {
        await this.forceReadKPIs();
        this.log('[BACKGROUND] ✅ Initial KPIs read complete');
      } catch (err) {
        this.log('[BACKGROUND] ⚠️  KPI reading failed:', err.message);
        // Try to read at least battery
        try {
          await this.forceReadBattery();
        } catch (battErr) {
          this.log('[BACKGROUND] ⚠️  Battery reading also failed');
        }
      }
  ...
  async onDeleted() {
    ...
    try {
      ...
      this.error('[CLEANUP] Cleanup failed:', err.message);
    }
  }
}
