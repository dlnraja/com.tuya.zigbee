...
class BaseHybridDevice extends ZigBeeDevice {
      ...
      try {
        ...
        if (tempCluster) {
          ...
          if (temp?.measuredValue !== undefined) {
            ...
            await this.setCapabilityValue('measure_temperature', tempC).catch(this.error);
      ...
      try {
        ...
        if (humCluster) {
          ...
          if (hum?.measuredValue !== undefined) {
            ...
            await this.setCapabilityValue('measure_humidity', humPercent).catch(this.error);
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (power?.activePower !== undefined) {
            ...
            await this.setCapabilityValue('measure_power', watts).catch(this.error);
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (voltage?.rmsVoltage !== undefined) {
            ...
            await this.setCapabilityValue('measure_voltage', volts).catch(this.error);
      ...
      try {
        ...
        if (powerCluster) {
          ...
          if (current?.rmsCurrent !== undefined) {
            ...
            await this.setCapabilityValue('measure_current', amps).catch(this.error);
      ...
      try {
        ...
        if (onOffCluster) {
          ...
          if (state?.onOff !== undefined) {
            await this.setCapabilityValue('onoff', state.onOff).catch(this.error);
      ...
      try {
        ...
        if (occCluster) {
          ...
          if (occ?.occupancy !== undefined) {
            ...
            await this.setCapabilityValue('alarm_motion', occupied).catch(this.error);
    ...
    try {
      ...
      if (clusterName === 'onOff') {
        ...
        if (command === 'toggle') {
          ...
          const current = await this.getCapabilityValue(cap).catch(() => false);
  ...
  async forceInitialRead() {
    ...
    try {
      ...
      if (endpoint.clusters?.powerConfiguration) {
        try {
          const batteryData = await endpoint.clusters.powerConfiguration.readAttributes(['batteryVoltage', 'batteryPercentageRemaining']).catch(() => null);
          if (batteryData) {
            this.log(`[INITIAL-READ] ✅ Battery: ${batteryData.batteryVoltage / 10}V, ${batteryData.batteryPercentageRemaining / 2}%`);
            if (this.hasCapability('measure_battery') && batteryData.batteryPercentageRemaining !== undefined) {
              await this.setCapabilityValue('measure_battery', batteryData.batteryPercentageRemaining / 2).catch(this.error);
            }
          }
        } catch (err) {
          this.log(`[INITIAL-READ] ⚠️ Battery read failed: ${err.message}`);
        }
      }
      ...
      if (endpoint.clusters?.temperatureMeasurement || endpoint.clusters?.msTemperatureMeasurement) {
        try {
          ...
          if (tempData && tempData.measuredValue !== null) {
            ...
            if (this.hasCapability('measure_temperature')) {
              await this.setCapabilityValue('measure_temperature', tempData.measuredValue / 100).catch(this.error);
            }
          }
        } catch (err) {
      ...
      if (endpoint.clusters?.relativeHumidity || endpoint.clusters?.msRelativeHumidity) {
        try {
          ...
          if (humidityData && humidityData.measuredValue !== null) {
            ...
            if (this.hasCapability('measure_humidity')) {
              await this.setCapabilityValue('measure_humidity', humidityData.measuredValue / 100).catch(this.error);
            }
          }
        } catch (err) {
  ...
  async logDeviceIdentity() {
    try {
      ...
      if (this.zclNode?.endpoints?.[1]?.clusters?.basic) {
        try {
          const deviceInfo = await this.zclNode.endpoints[1].clusters.basic.readAttributes(['manufacturerName', 'modelId', 'zclVersion']).catch(() => null);
          if (deviceInfo) {
