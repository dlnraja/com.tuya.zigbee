...
class SwitchDevice extends BaseHybridDevice {
  ...
  async setupSwitchControl() {
    this.log('🔌 Setting up switch control (SDK3)...');
      ...
      try {
        ...
        if (endpoint?.clusters?.onOff) {
          ...
          endpoint.clusters.onOff.on('attr.onOff', async (value) => {
            this.log(`📥 Gang ${ep} cluster update: ${value}`);
          ...
          await this.configureAttributeReporting([{
            ...
            maxInterval: 300,
            minChange: 1
          }]).catch(err => this.log(`Gang ${ep} reporting config (non-critical):`, err.message));
          
          this.log(`✅ Gang ${ep} SDK3 cluster listeners configured`);
        } else {
          this.log(`⚠️  Gang ${ep}: onOff cluster not available`);
        }
      } catch (err) {
    ...
    this.log(`✅ Switch control configured for ${gangCount} gang(s) (SDK3)`);
  }
