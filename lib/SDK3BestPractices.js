...
/**
 * SDK3BestPractices - v5.5.453 (+ P2547 Sdk3ZclSafe re-export)
 *
 ...
 * 7. Defer ZCL in onNodeInit + catch promises (Sdk3ZclSafe)
 */
const Sdk3ZclSafe = require('./utils/Sdk3ZclSafe');

class SDK3BestPractices {
  static get zclSafe() { return Sdk3ZclSafe; }
  static catchZcl(...args) { return Sdk3ZclSafe.catchZcl(...args); }
  static scheduleDeferredInit(...args) { return Sdk3ZclSafe.scheduleDeferredInit(...args); }
  static setCapabilityCaught(...args) { return Sdk3ZclSafe.setCapabilityCaught(...args); }

  /**
   * Ensure device follows SDK3 battery best practices dynamically at runtime
   * Rule: Never keep both alarm_battery AND measure_battery active at runtime to prevent duplicate UI/Flow cards.
   * Adaptability: Developers must declare both in driver.compose.json so the driver can adapt to any hardware variant.
   * At runtime, we dynamically deactivate/remove capabilities:
   * 1. If mains-powered: remove both measure_battery and alarm_battery entirely.
   * 2. If battery-powered and percentage-capable: keep measure_battery and remove alarm_battery (Homey Pro auto-handles alarms).
   * 3. If battery-powered but only alarm-capable: keep alarm_battery and remove measure_battery (prevents empty % UI).
   *
  ...
  static async ensureBatteryBestPractices(device) {
    ...
    if (!hasAlarmBattery && !hasMeasureBattery) {return;}

    // 1. Detect if mains-powered
    const storePowerType = device.getStoreValue ? device.getStoreValue('power_type') : null;
    const isMains = device.powerType === 'AC' || 
                    device.powerType === 'DC' || 
                    device.mainsPowered === true || 
                    storePowerType === 'AC' || 
                    storePowerType === 'DC' ||
                    device.hasCapability('measure_power') || 
                    device.hasCapability('meter_power');

    if (isMains) {
      device.log('[SDK3-BATTERY] Device is mains-powered. Dynamically removing both battery capabilities to keep UI clean.');
      if (hasMeasureBattery) {
        await device.removeCapability('measure_battery').catch(err => {
          device.error('[SDK3-BATTERY] Failed to remove measure_battery:', err.message);
        });
      }
      if (hasAlarmBattery) {
        await device.removeCapability('alarm_battery').catch(err => {
          device.error('[SDK3-BATTERY] Failed to remove alarm_battery:', err.message);
        });
      }
      return;
    }

    // 2. Battery-powered device - resolve Homey SDK3 conflict (duplicate UI indicators)
    // If both are present, we dynamically adapt to the best one based on reported features.
    if (hasAlarmBattery && hasMeasureBattery) {
      // By default, if the device has percentage capabilities, we keep measure_battery and remove alarm_battery.
      // Homey Pro natively handles low-battery alerts/flow triggers from measure_battery percent automatically.
      const hasZclBattery = !!(device.zclNode?.endpoints?.[1]?.clusters?.powerConfiguration || device.zclNode?.endpoints?.[1]?.clusters?.genPowerCfg);
      ...
      if (hasZclBattery || isTuyaDP) {
        device.log('[SDK3-BATTERY] Battery device is percentage-capable. Keeping measure_battery, removing alarm_battery to prevent SDK3 UI conflict.');
        try {
          await device.removeCapability('alarm_battery');
          device.log('[SDK3-BATTERY] Dynamically removed alarm_battery (SDK3 UI conflict resolved)');
        } catch (err) {
          device.error('[SDK3-BATTERY] Failed to remove alarm_battery:', err.message);
        }
      } else {
        // Otherwise, if it only has IAS Zone or no percentage cluster, keep alarm_battery and remove measure_battery
        device.log('[SDK3-BATTERY] Battery device is alarm-only. Keeping alarm_battery, removing measure_battery to prevent empty percentage UI.');
        try {
          await device.removeCapability('measure_battery');
          device.log('[SDK3-BATTERY] Dynamically removed measure_battery (SDK3 UI conflict resolved)');
        } catch (err) {
          device.error('[SDK3-BATTERY] Failed to remove measure_battery:', err.message);
        }
      }
  ...
  static async triggerDeviceFlowCard(device, cardId, tokens = {}) {
    try {
      ...
      if (card) {
        ...
        device.log(`[SDK3-FLOW]  Card not found: ${cardId}`);
      }
    } catch (err) {
   ...
   */
  static async setupIASZoneEnrollment(device, zclNode, endpointId = 1, opts = {}) {
    ...
    try {
      ...
      if (!iasZoneCluster) {
        device.log('[SDK3-IAS]  No IAS Zone cluster on endpoint', endpointId);
        return false;
      }

      device.log('[SDK3-IAS]  Setting up IAS Zone enrollment...');

      // v5.12.3: Always assign handler (SDK3 pattern) - fixes guard bug
      iasZoneCluster.onZoneEnrollRequest = async () => {
        ...
        try {
          if (typeof iasZoneCluster.zoneEnrollResponse === 'function') {
            await iasZoneCluster.zoneEnrollResponse({ enrollResponseCode: 0, zoneId: 10 });
          }
        } catch (_e) { /* already enrolled */ }
        return { enrollResponseCode: 0, zoneId: 10 }; // Success — never zoneId 0
      };
      device.log('[SDK3-IAS]  Zone Enroll Request handler registered');

      // SDK3 Best Practice: Send Zone Enroll Response at init
      // Skip proactive TX for sleepy wireless buttons (f647d35b) — respond on request only.
      if (proactive && typeof iasZoneCluster.zoneEnrollResponse === 'function') {
        try {
          // zoneId 0 leaves IAS devices stuck ("Laatste waarde onbekend" / battery "?")
          // Align with IASZoneManager / SOS Peter pattern: zoneId 10
          await iasZoneCluster.zoneEnrollResponse({ enrollResponseCode: 0, zoneId: 10 });
          device.log('[SDK3-IAS]  Proactive Zone Enroll Response sent');
        } catch (err) {
          // May fail if already enrolled - that's OK
          device.log('[SDK3-IAS]  Zone Enroll Response:', err.message);
        }
      }
  ...
  static async setCapabilityOptions(device, capabilityId, options) {
    if (!device.hasCapability(capabilityId)) {
      device.log(`[SDK3-OPT]  Capability ${capabilityId} not found`);
      return;
    }
  ...
  static async applyAllBestPractices(device, zclNode, options = {}) {
    ...
    // WHY (f647d35b): wireless scene/buttons often advertise iasZone in interview but are
    // NOT SOS/contact — proactive zoneEnrollResponse while sleepy just floods "device offline"
    // and delays 0xFD button capture. Keep enroll for SOS / contact / water / true IAS sensors.
    if (options.hasIASZone !== false) {
      const driverId = String(device.driver?.id || device.driver?.manifest?.id || '');
      const skipProactiveIas = /button_wireless|scene_switch|smart_remote|remote_button|wall_remote|knob/
        .test(driverId)
        && !/sos|emergency|panic|contact|water_leak|smoke|gas/.test(driverId);
      if (skipProactiveIas) {
        device.log('[SDK3-IAS]  Skip proactive IAS enroll for wireless button/remote (listen on wake only)');
        await this.setupIASZoneEnrollment(device, zclNode, options.iasEndpoint || 1, { proactive: false });
      } else {
