 ...
 * 
 *                                                                               
 *   Base de donn̩es LOCALE des profils de batteries par device                  
 *   PAS DE CONNEXION INTERNET - Tout est embarqu̩ dans l'app                    
 *                                                                               
 *   Apr̬s 15 min d'apprentissage, l'app affine automatiquement:                 
 *   - L'algorithme de calcul du pourcentage                                     
 *   - Les seuils de tension (min/max)                                           
 *   - Le type de courbe de d̩charge                                             
 *   - La source de donn̩es batterie (Tuya DP vs ZCL)                            
 *                                                                               
 *   Sources:                                                                    
...
function calculateBatteryPercent(voltage, algorithm, minVoltage = 2.5, maxVoltage = 3.0) {
  ...
  if (algorithm === VOLTAGE_ALGO.MULTIPLY_2) {
    return Math.min(100, Math.max(0,Math.round(safeMultiply(voltage, 2))));
  }

  if (algorithm === VOLTAGE_ALGO.DIVIDE_2) {
    return Math.min(100, Math.max(0, Math.round(safeParse(voltage, 2))));
  }

  // Linear interpolation
  if (algorithm === VOLTAGE_ALGO.LINEAR) {
    const percent = safeDivide(voltage - minVoltage, maxVoltage - minVoltage) * 100;
  ...
  if (curve) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      if (voltage >= points[i + 1].voltage && voltage <= points[i].voltage) {
        ...
        const percent =p2  + safeMultiply(ratio, (p1 - p2));
