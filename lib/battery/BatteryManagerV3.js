...
class BatteryManagerV3 {
  ...
  _resolveTuyaCluster(endpoint) {
    if (!endpoint?.clusters) {return null;}
    try {
      const { findClusterOnEndpoint } = require('../zigbee/ZclClusterLexicon');
      const found = findClusterOnEndpoint(endpoint, 0xEF00);
      if (found) {return found;}
    } catch (_e) { /* fallback */ }
    return endpoint.clusters.tuyaSpecific
  ...
  _resolvePowerCluster(endpoint) {
    if (!endpoint?.clusters) {return null;}
    try {
      const { findClusterOnEndpoint } = require('../zigbee/ZclClusterLexicon');
      const found = findClusterOnEndpoint(endpoint, 0x0001);
      if (found) {return found;}
    } catch (_e) { /* fallback */ }
    return endpoint.clusters.genPowerCfg
  ...
  async tryTuyaDP() {
    try {
      ...
      // Check Tuya cluster
      const tuyaCluster = this._resolveTuyaCluster(endpoint);
  ...
  async tryZCL() {
    try {
      const endpoint = this.device.zclNode?.endpoints?.[1];
      const powerCluster = this._resolvePowerCluster(endpoint);
      if (!powerCluster) {return false;}
      ...
      // Read initial value
      const { batteryPercentageRemaining } = await powerCluster.readAttributes(['batteryPercentageRemaining']).catch(() => ({}));
  ...
  updateBattery(value) {
    ...
    const { safeSetDeviceCapability } = require('../utils/safe-capability');
    if (this.device.hasCapability('measure_battery')) {
      safeSetDeviceCapability(this.device, 'measure_battery', parseFloat(rounded)).catch(err => this.device.error(err));
    }

    // Alarm si <20%
    if (this.device.hasCapability('alarm_battery')) {
      safeSetDeviceCapability(this.device, 'alarm_battery', rounded < 20).catch(err => this.device.error(err));
    }
  }
