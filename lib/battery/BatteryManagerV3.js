...
class BatteryManagerV3 {
  ...
  updateBattery(value) {
    ...
    if (this.device.hasCapability('measure_battery')) {
      this.device.setCapabilityValue('measure_battery', parseFloat(rounded)).catch(err => this.device.error(err));
    }

    // Alarm si <20%
    if (this.device.hasCapability('alarm_battery')) {
      this.device.setCapabilityValue('alarm_battery', rounded < 20).catch(err => this.device.error(err));
    }
  }
