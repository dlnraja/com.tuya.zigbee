...
/**
 * Battery Manager V3 - CentralisÃ© & Simple
 *
 * BasÃ© sur:
...
class BatteryManagerV3 {
  ...
  /**
   * DÃ©marrer monitoring batterie
   */
  async startMonitoring() {
    this.device.log('[BATTERY-V3] Starting...');

    // DÃ©tecter type device
  ...
  this.detectDeviceType();
    ...
    // Pas de batterie
    this.device.log('[BATTERY-V3]  No battery method available');
    return false;
  }
   ...
   */
  detectDeviceType() {
    const driverUri = this.device.driver?.id || '';if (driverUri.includes('button') || driverUri.includes('remote')) {
      return 'button';
    }
  ...
  async tryTuyaDP() {
    try {
      ...
      // Subscribe Ã Tuya DP events (via TuyaEF00Manager)
  ...
  async tryZCL() {
    try {
      ...
      // Configure reporting (min 1h, max 12h, Â±2%)
      ...
      if (batteryPercentageRemaining !== undefined) {
        const battery = Math.round(safeParse(batteryPercentageRemaining));
        this.updateBattery(battery);
      }
      ...
      if (this.method === 'tuya_dp') {
        ...
        try {
          ...
          if (endpoint?.clusters?.genPowerCfg) {
            const { batteryPercentageRemaining } = await endpoint.clusters.genPowerCfg.readAttributes(['batteryPercentageRemaining']).catch(() => ({}));
            if (batteryPercentageRemaining !== undefined) {
              this.updateBattery(Math.round(safeParse(batteryPercentageRemaining));
            }
          }
        } catch (err) {
