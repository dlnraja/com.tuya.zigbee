...
function attach(device, zclNode, options = {}) {
  ...
  // WHY(P2707 / Bastien TS0043 0–84%): voltage curve on coin remotes paints ~84%
  // ceiling and can storm setCapability — skip voltage when ZCL % is authoritative.
  let skipVoltage = !!(options.skipVoltage);
  try {
    const p = typeof device.getDeviceProfile === 'function' ? device.getDeviceProfile() : null;
    if (p?.zcl200IsPercent || p?.skipBatteryReporting) skipVoltage = true;
  } catch (_e) { /* soft */ }
