...
class UniversalBatteryFallback {
  ...
  async _update(percent, source = 'unknown') {
    ...
    // Update capability (L14)
    if (this.device && this.device.hasCapability && this.device.hasCapability('measure_battery')) {
      try {
        await safeSetDeviceCapability(this.device, 'measure_battery', parseFloat(this._smoothedValue));
      } catch (err) {
        if (this.device.log) {this.device.log(`[BATTERY-FB] safeSet failed: ${err.message}`);}
      }
    }
    ...
    if (this.lastVoltage !== null && this.device && this.device.hasCapability && this.device.hasCapability('measure_voltage')) {
      try {
        await safeSetDeviceCapability(this.device, 'measure_voltage', parseFloat(this.lastVoltage));
      } catch { /* ignore */ }
    }
