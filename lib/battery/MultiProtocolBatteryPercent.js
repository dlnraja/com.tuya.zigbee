...
function _deviceContext(device) {
  ...
  try {
    productId = device?.getSetting?.('zb_model_id')
      || device?.getStoreValue?.('modelId')
      || '';
  } catch (_e) { /* soft */ }
  try {
    if (UnifiedBatteryHandler?.lookupBatteryProfile) {
      profile = UnifiedBatteryHandler.lookupBatteryProfile(manufacturer, productId);
    }
  } catch (_e) { /* soft */ }
  try {
    const t = device?.getSetting?.('battery_type')
      || device?.getStoreValue?.('battery_type')
      || (profile?.chemistry && profile.chemistry !== 'MAINS' && profile.chemistry !== 'USB' ? profile.chemistry : null)
  ...
  return { manufacturer, productId, batteryType, temperature, lastValue, profile };
}

const PERCENT_PROTOCOLS = new Set(['zcl', 'tuya-dp', 'wifi', 'mcu', 'cached', 'raw']);

function _shouldAcceptVoltageEstimate(device) {
  try {
    if (device.getStoreValue?.('last_battery_estimated') === true) {return true;}
    const src = normalizeProtocol(device.getStoreValue?.('last_battery_source') || '');
    const t = Number(device.getStoreValue?.('last_battery_time'));
    if (PERCENT_PROTOCOLS.has(src) && Number.isFinite(t) && (Date.now() - t) < 3600000) {
      return false;
    }
  } catch (_e) { /* soft */ }
  return true;
...
function normalizeBatteryPercent(raw, opts = {}) {
  ...
  if (protocol === 'zcl') {
    return normalizeZclBatteryPercent(raw, {
      manufacturer,
      batteryType,
      profile: opts.profile,
      treat200AsSentinel: opts.treat200AsSentinel === true
        || opts.profile?.zcl200IsPercent === false,
    });
  }
...
async function commitBatteryPercent(device, percent, opts = {}) {
  ...
  if (protocol === 'voltage' && !_shouldAcceptVoltageEstimate(device)) {
    return { ok: false, reason: 'prefer-percent', percent: null };
  }
  ...
  try {
    ...
    if (typeof device.safeSetCapabilityValue === 'function') {
      ...
      // WHY(P2461): when Homey UI is `?` (null) but store already has %, skipThrottle
      const cur = device.getCapabilityValue?.('measure_battery');
      const skipThrottle = opts.skipThrottle === true || cur == null || cur === undefined;
