...
/**
 * BatteryMasterEngine — canonical battery % / voltage path for Homey devices.
 *
 * Combines non-linear chemistry curves, ZCL 0–200 normalisation, Tuya DP %,
 * EMA smoothing, anti-flood, anti-halve, and store persistence.
 *
 * Rules:
 * - Never use linear (V - 2.5) / 0.5.
 * - Prefer safeSetCapabilityValue when the device exposes it (L14).
 * - ZCL batteryPercentageRemaining: often 0–200 (200 = 100%).
 * - Tuya DP battery may be 0–100 or 0–200 — use tuyaDpToPercent / feedTuyaDP.
 *
 * This module is a superset of older BatteryManager / UnifiedBatteryHandler
 * behaviours; prefer calling it instead of duplicating formulas in drivers.
 */

// ---------------------------------------------------------------------------
// SECTION 1: BATTERY CHEMISTRY SPECS
// Non-linear discharge curves (CR2032, AA, …)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// --- SECTION 3: MANUFACTURER PROFILES (synth se)
...
const MANUFACTURER_PROFILES = {
  ...
  '_TZE200_0hg3xegf':  { chemistry: 'AA',      source: 'tuya_dp', dpId: 15, algorithm: 'alkaline_curve'  },
  '_TZE200_3p5wskj2':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_4hbh6j9y':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_5y6b4lqo':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_6m3v8arn':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_7n4z9dsp':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_8o5a0etq':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_9p6b1fvu':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_1q7c2gwx':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE200_2r8d3hxz':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE204_3s9e4ivy':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE204_4t0f5jwz':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE204_5u1g6kxa':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE204_6v2h7lyb':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE204_7w3i8mzc':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE204_8x4j9nad':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE284_9y5k0obe':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE284_1z6l1pcf':  { chemistry: 'CR2032',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },
  '_TZE204_q1w2e3r4':  { chemistry: 'MAINS',   source: 'none'                                       },
  '_TZE200_x1c2v3b4':  { chemistry: 'AAA',     source: 'tuya_dp', dpId: 15, algorithm: 'alkaline_curve'  },
  '_TZ3000_a7b8c9d0':  { chemistry: 'CR2430',  source: 'tuya_dp', dpId: 4,  algorithm: 'cr2032_curve'    },'LELLKI':             { chemistry: 'MAINS', source: 'none' },
  ...
  // P2396: removed '_TZE200_r32ctezx' AA map — mains Lerlink fan_switch (GitHub #536)
...
class BatteryMasterEngine {
  constructor(device, options = {}) {
    ...
    try {
      // WHY (P2269): file lives at lib/LowLevelBridge.js — relative ./ was a dead require.
      // LEGACY: prefer BatteryRouter + DeviceIOFacade; LowLevelBridge is thin→ProtocolFallbackChain.
  ...
  update(percent, meta = {}) {
    ...
    if (this.options.antiFlood && this.lastValue !== null) {
      ...
      if (elapsed < this.options.maxChangeWindow && change > this.options.maxChangeRejection && value > 0) {
        this.device.log?.(`[BAT-MASTER] Rejected ${this.lastValue}->${value} (${change}% in ${Math.round(elapsed/1000)}s)`);
        // WHY: battery_anomaly_detected not in compose — avoid FLOW-GUARD spam
    ...
    if (this.options.antiBatteryHalving && this._smoothedValue !== null) {
      ...
      if (value < this._smoothedValue / 2 && this._smoothedValue > 60) {
        this.device.log?.(`[BAT-MASTER] Anti-halve: refused ${this._smoothedValue}->${value}`);
    ...
    // Update capabilities — prefer L14 safeSetCapabilityValue (sanity + anti-flood)
    this._setCap('measure_battery', smoothed);
    if (this.lastVoltage !== null) {
      this._setCap('measure_voltage', this.lastVoltage);
    }
  ...
  // Prefer Homey L14 pipeline when the device exposes it (sanity + anti-flood).
  _setCap(capability, value) {
    ...
    if (typeof this.device.safeSetCapabilityValue === 'function') {
      this.device.safeSetCapabilityValue(capability, num).catch(err => {
        this.device.error?.(`[BAT-MASTER] safeSetCapabilityValue(${capability}) failed: ${err.message}`);
      });
      return;
    }
    this.device.setCapabilityValue(capability, num).catch(err => {
      this.device.error?.(`[BAT-MASTER] setCapabilityValue(${capability}) failed: ${err.message}`);
    });
  }

  async _setCapAsync(capability, value) {
    if (!this.device?.hasCapability?.(capability)) return;
    const num = parseFloat(value);
    try {
      if (typeof this.device.safeSetCapabilityValue === 'function') {
        await this.device.safeSetCapabilityValue(capability, num);
      } else {
        await this.device.setCapabilityValue(capability, num);
      }
    } catch (err) {
      this.device.error?.(`[BAT-MASTER] setCap(${capability}) failed: ${err.message}`);
    }
  }
  ...
  async restoreFromStore() {
    ...
    try {
      ...
      if (Number.isFinite(Number(stored))) {
        ...
        this._smoothedValue = value;
        await this._setCapAsync('measure_battery', value);
