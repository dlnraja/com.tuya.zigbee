...
const BatteryMonitoringMixin = (Base) => class extends Base {
  ...
  async performInitialBatteryRead() {
    ...
    try {
      ...
      if (result?.batteryPercentageRemaining !== undefined) {
        ...
        await this.safeSetCapabilityValue('measure_battery', parseFloat(percentage)).catch(() => {});
   ...
   */
  wait(ms) {
    return new Promise(resolve => (this.homey && typeof this.homey.setTimeout === 'function' ? this.homey : globalThis).setTimeout(resolve, ms));
  }
