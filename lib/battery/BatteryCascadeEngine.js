...
class BatteryCascadeEngine {
  ...
  update(percent, meta = {}) {
    ...
    // Update measure_battery capability (L14)
    if (this.device.hasCapability?.('measure_battery')) {
      safeSetDeviceCapability(this.device, 'measure_battery', parseFloat(smoothed)).catch(err => {
        this.device.error?.(`[BAT-CASCADE] safeSet failed: ${err.message}`);
      });
    }

    // Update measure_voltage capability if present
    if (this.lastVoltage !== null && this.device.hasCapability?.('measure_voltage')) {
      safeSetDeviceCapability(this.device, 'measure_voltage', parseFloat(this.lastVoltage)).catch(() => {});
    }
  ...
  async restoreFromStore() {
    ...
    try {
      ...
      if (Number.isFinite(Number(stored))) {
        ...
        if (this.device.hasCapability?.('measure_battery')) {
          const { safeSetDeviceCapability } = require('../utils/safe-capability');
          await safeSetDeviceCapability(this.device, 'measure_battery', parseFloat(value));
        }
