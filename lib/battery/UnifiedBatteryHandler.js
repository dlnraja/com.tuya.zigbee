...
class UnifiedBatteryHandler {
  ...
  _setupIasZoneBattery(zclNode) {
    try {
      const ep = this._findEndpointByCluster(['iasZone', 'ssIasZone', 0x0500, '0x0500'], zclNode);
      const cluster = ep?.clusters?.iasZone || ep?.clusters?.ssIasZone;
      if (!cluster) {
        // Fallback: Use standard SDK3 registerAttrReportListener when cluster object isn't fully interviewed yet
        if (typeof this.device.registerAttrReportListener === 'function') {
          this.device.log('[BATTERY-UNIFIED] ⚠️ IAS Zone cluster not fully interviewed yet, registering fallback AttrReportListener...');
          this.device.registerAttrReportListener('iasZone', 'zoneStatus', (status) => {
            const lowBat = (status & 0x08) !== 0;
            this._updateBatteryAlarm(lowBat);
          }).catch(err => {
            this.device.log('[BATTERY-UNIFIED] Fallback IAS Zone status registration failed:', err.message);
          });
        }
        return;
      }
  ...
  async _setupZclBattery(zclNode) {
    try {
      ...
      if (!cluster) {
        // Fallback: Use standard SDK3 registerAttrReportListener when cluster object isn't fully interviewed yet
        if (typeof this.device.registerAttrReportListener === 'function') {
          this.device.log('[BATTERY-UNIFIED] ⚠️ ZCL battery cluster not fully interviewed yet, registering fallback AttrReportListeners...');
          
          this.device.registerAttrReportListener('genPowerCfg', 'batteryPercentageRemaining', (value) => {
            const percent = Math.round(value / 2);
            this._updateBattery(percent);
            this.device.log(`[BATTERY-UNIFIED] 📡 ZCL report (fallback): ${percent}%`);
          }).catch(err => {
            this.device.log('[BATTERY-UNIFIED] Fallback batteryPercentageRemaining registration failed:', err.message);
          });

          this.device.registerAttrReportListener('genPowerCfg', 'batteryVoltage', (value) => {
            const voltage = value / 10;
            this._updateVoltage(voltage);
            this.device.log(`[BATTERY-UNIFIED] 📡 ZCL voltage report (fallback): ${voltage}V`);
          }).catch(err => {
            this.device.log('[BATTERY-UNIFIED] Fallback batteryVoltage registration failed:', err.message);
          });
        }
        return;
      }
   ...
   * BUGFIX v5.8.00: Also check IAS Zone and voltage DPs before classifying as mains
   */
