...
class UnifiedBatteryHandler {
    ...
    try {
      ...
      try {
        ...
        if (attrs.batteryPercentageRemaining !== undefined) {
          ...
          const percent = Math.round(safeParse(attrs.batteryPercentageRemaining, 2));
  ...
  _handleTuyaBatteryDP(dp, value) {
    ...
    if (dp === 14 && value > 100) {
      ...
      percent = Math.round(safeParse(value, 2));
    }
  ...
  _voltageToPercent(voltage) {
    ...
    for (let i = 0; i < curve.length - 1; i++) {
      if (mV >= curve[i + 1][0] && mV <= curve[i][0]) {
        return Math.round(curve[i + 1][1] + ((mV - curve[i + 1][0]) /safeMultiply((curve[i][0] - curve[i + 1][0])), (curve)[i][1] - curve[i + 1][1]));
      }
    }
    return null;
  }
