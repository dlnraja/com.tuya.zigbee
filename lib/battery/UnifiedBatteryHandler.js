...
/**
 * UnifiedBatteryHandler v8.2.0 — PHOENIX SOVEREIGN
 *
...
class UnifiedBatteryHandler {
  ...
  /**
   * v5: Normaliser valeur Zigbee (0-200 scale)
   */
  static normalizeZigbeeValue(rawValue) {
    if (rawValue === null || rawValue === undefined || isNaN(rawValue)) return null;
    if (rawValue > 100 && rawValue <= 200) return Math.round(rawValue / 2);
    if (rawValue >= 0 && rawValue <= 100) return Math.round(rawValue / 2);
    // Valeur étrange, laisser passer comme percentage direct
  ...
  async initialize(zclNode) {
    ...
    this.device.log('[BATTERY-UNIFIED] 🔋 Initializing Phoenix Sovereign v8.2.0...');
    ...
    this._storeInterval = this.homey.setInterval(() => {
  ...
  _setupTuyaBattery() {
    ...
    if (!bindManager()) {
      const checkInterval = this.homey.setInterval(() => {
        attempts++;
        if (bindManager()) {
  ...
  static BATTERY_REPORTING_PATTERNS = {
    ...
    // Devices that report voltage as battery percentage
    'voltage_as_percent': {
      detect: (value) => value > 30 && value < 60,
      correct: (value) => {
        // Convert voltage-like values to percentage
        if (value > 30 && value < 50) {
          // Likely millivolts (3000-5000mV range)
          return Math.round(((value - 30) / 20) * 100);
        }
        return value;
      },
      notes: 'Device reports voltage as battery percentage'
    },
  ...
  _validateBatteryValue(value, previousValue = null) {
    ...
    // Sudden drop check (more than 30% in one update)
