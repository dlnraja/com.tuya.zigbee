...
// v5.12.52 (P92.120): negative cache for unsupported ZCL ops
...
class UnifiedBatteryHandler {
        ...
        if (cluster) {
          const attrs = await readAttributesSmart(this.device, cluster, 'genPowerCfg',
            ['batteryPercentageRemaining', 'batteryVoltage'], {
              onFallback: async (device) => {
                // v5.12.52 (P92.120): ZCL attrs unsupported → Tuya DP takes over
  ...
  async _setupZclBattery(zclNode) {
    try {
      ...
      try {
        // v5.12.52 (P92.120): skip attrs already known unsupported on this
