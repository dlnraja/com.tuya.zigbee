...
class UnifiedBatteryHandler {
  ...
  async initialize(zclNode) {
    ...
    this._storeInterval = setInterval(() => {
  ...
  _handleTuyaBatteryDP(dp, value) {
    ...
    // v5: Profile-aware algorithm
    if (this._profile && this._profile.dpId === dp && this._profile.algorithm) {
      percent = UnifiedBatteryHandler.calculateFromTuyaDP(value, this._profile.algorithm);
    } else {
      // Fallback v8
      if (dp === 14 && value > 100) { percent = Math.round(value / 2); }
