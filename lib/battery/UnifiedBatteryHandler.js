...
class UnifiedBatteryHandler {
  ...
  static BATTERY_PROFILES = {
    ...
    '_TZE284_0ints6wl':   { chemistry: 'CR2032', source: 'tuya_dp', dpId: 15, algorithm: 'direct', notes: 'ZG-303Z soil sensor' },
    'HOBEIAN':            { chemistry: '2xAAA', source: 'zcl', algorithm: 'alkaline_curve', zcl200IsPercent: true, notes: 'ZG-303Z soil sensor ZCL battery' },
    ...
    '_TZ3000_b4awzgct':   { chemistry: 'CR2032', source: 'zcl', algorithm: 'cr2032_curve', zcl200IsPercent: true, notes: 'TS0041 4-endpoint button' },
    '_TZ3000_yj6k7vfo':   { chemistry: 'CR2032', source: 'zcl', algorithm: 'cr2032_curve', zcl200IsPercent: true, notes: 'TS0041 4-endpoint button' },
    '_TZ3000_kfu8zapd':   { chemistry: 'CR2032', source: 'zcl', algorithm: 'cr2032_curve', zcl200IsPercent: true, notes: 'TS0044 4-button remote' },
    '_TZ3000_qja6nq5z':   { chemistry: 'CR2032', source: 'zcl', algorithm: 'cr2032_curve', zcl200IsPercent: true, notes: 'TS004F Moes rotary knob' },
  ...
  static lookupBatteryProfile(manufacturerName, productId) {
    if (manufacturerName) {
      const profiles = UnifiedBatteryHandler.BATTERY_PROFILES;
      const exact = profiles[manufacturerName];
      if (exact) {
        return { source: 'manufacturer', ...exact };
      }
      const normalized = String(manufacturerName).toLowerCase();
      const matchedKey = Object.keys(profiles).find(key => key.toLowerCase() === normalized);
      if (matchedKey) {
        return { source: 'manufacturer', ...profiles[matchedKey] };
      }
    }
  ...
  static BATTERY_200_WHITELIST = new Set([
    '_TZ3000_lqmvrwa2',
    '_TZ3000_b4awzgct',
    '_TZ3000_yj6k7vfo',
    '_TZ3000_kfu8zapd',
  ...
  static normalizeZigbeeValue(rawValue, options = {}) {
    ...
    if (rawValue === 255 || rawValue === 0xFFFF) return null;
    if (rawValue === 200) {
  ...
  _getBatteryNormalizeOptions() {
    const settings = this.device.getSettings?.() || {};
    const store = this.device.getStore?.() || {};
    const data = this.device.getData?.() || {};
    const manufacturer = settings.zb_manufacturer_name ||
    ...
    return {
      manufacturer,
      batteryType: this.batteryType || this._profile?.chemistry || 'CR2032',
      ...
      if (this.source?.includes('zcl')) {
        ...
        if (cluster) {
          ...
          if (attrs.batteryPercentageRemaining !== undefined) {
            const percent = UnifiedBatteryHandler.normalizeZigbeeValue(
              attrs.batteryPercentageRemaining,
              this._getBatteryNormalizeOptions()
            );
