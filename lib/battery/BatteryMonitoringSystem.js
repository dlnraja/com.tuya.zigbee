    ...
    try {
      ...
      case 'critical':
        ...
        if (this.device.homey && this.device.homey.flow) {
          try {
            await this.device.homey.flow.getDeviceTriggerCard('low_battery_warning')
              .trigger(this.device, { device_name: this.device.getName(), battery_level: level })
              .catch(() => { /* Flow card may not exist */ });
          } catch (e) { this.device.error('[BATTERY] Flow trigger critical error:', e.message); }
        }
        break;
      ...
      case 'low':
        ...
        // Trigger flow low_battery_warning (app-level card)
        if (this.device.homey && this.device.homey.flow) {
          try {
            await this.device.homey.flow.getDeviceTriggerCard('low_battery_warning')
              .trigger(this.device, {
              ...
              .catch(() => { /* Flow card may not exist */ });
          } catch (e) { this.device.error('[BATTERY] Flow trigger low error:', e.message); }
        }
        break;
      ...
      default:
        ...
        } catch (e) { /* Warning may already be cleared */ }
      ...
      if (previousLevel !== null && previousLevel < 30 && level > 80) {
        ...
        if (this.device.homey && this.device.homey.flow) {
          try {
            // Use app-level flow card that exists
            await this.device.homey.flow.getDeviceTriggerCard('low_battery_warning')
              .trigger(this.device, {
                device_name: this.device.getName(),
                battery_level: level
              ...
              .catch(() => {
                // Flow card may not exist - this is normal
              });
          } catch (e) { this.device.error('[BATTERY] Flow trigger replacement error:', e.message); }
        }
