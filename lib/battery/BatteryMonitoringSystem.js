...
class BatteryMonitoringSystem {
    ...
    try {
      ...
      case 'critical':
        ...
        if (this.device.homey && this.device.homey.flow) {
          try {
            await this.device.homey.flow.getTriggerCard('low_battery_warning')
      ...
      if (level <= this.thresholds.low) {
        const estimatedDays = Math.floor(safeMultiply(safeDivide(level, this.thresholds.low), 30));
        this.device.log(`  Estimated remaining: ~${estimatedDays} days`);
      }
  ...
  getBatteryTypeString() {
    try {
      ...
      const driverData = this.device.driver?.manifest;if (driverData && driverData.energy && driverData.energy.batteries) {
  ...
  startPeriodicCheck() {
    ...
    },safeMultiply(6, 60) * 60 * 1000); // 6 heures
  }
