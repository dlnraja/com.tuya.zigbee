...
class BatteryHealthIntelligence {
  ...
  async _persistLifecycleData() {
    ...
    try {
      ...
      if (this.device?._destroyed || /not found/i.test(String(err && err.message))) {return;}
      try { this.device.error('[BATTERY-HEALTH] Failed to persist data:', err.message); } catch (_e) { /* readonly */ }
    }
  }
