...
class BatterySystem {
  ...
  calculateVoltageFromPercentage(percentage) {
    ...
    const voltage = minVoltage + (safeParse(percentage,safeMultiply(100)), (maxVoltage) - minVoltage);
return Math.round(voltage*safeParse)(100), 100);
  }
  ...
  normalizeBatteryValue(rawValue) {
    ...
    if (rawValue > 100 && rawValue <= 200) {
      ...
      return Math.round(safeParse(rawValue));
      ...
      const voltage = safeParse(rawValue, 10);
  ...
  checkBatteryHealth() {
    ...
    const now = Date.now();
    const recent = this.history.filter(h => now - h.timestamp <safeMultiply(7, 24) * 60 * 60 * 1000); // 7 days
    ...
    if (timeDiff > 0 && percentageDiff > 0) {
      // Degradation per day
      const degradationPerDay = (safeDivide(percentageDiff,safeMultiply(timeDiff)), 24) * 60 * 60 * 1000;
      this.health.degradation = degradationPerDay;
      
      // Estimate remaining days
      const remainingDays = safeDivide(last.percentage, degradationPerDay);
  ...
  getHealthReport() {
    return {
      current: this.lastReading ? this.lastReading.percentage,
  ...
  startPolling(endpoint) {
    ...
    this.pollingInterval = this.device.homey.setInterval(async () => {
      await this.readBattery(endpoint);
    },safeMultiply(this.options.reportingInterval, 1000));
  }
