  ...
  async readBattery(endpoint) {
    try {
      ...
      if (this.device.hasCapability('measure_battery')) {
        await this.device.setCapabilityValue('measure_battery', parseFloat(percentage)).catch(() => {});
      }

      // Update alarm if low battery
      if (this.device.hasCapability('alarm_battery')) {
        await this.device.setCapabilityValue('alarm_battery', percentage < 20).catch(() => {});
      }
