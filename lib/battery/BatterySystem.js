...
class BatterySystem {
  ...
  _resolvePowerCluster(endpoint) {
    ...
    return endpoint.clusters.powerConfiguration
  ...
  async configureBatteryReporting(endpoint) {
    try {
      const cluster = this._resolvePowerCluster(endpoint);
      if (!cluster) {
        this.device.log('[Battery] No powerConfiguration / genPowerCfg cluster');
        return;
      }
  ...
  async readBattery(endpoint) {
    try {
      ...
      if (this.device.hasCapability('measure_battery')) {
        await safeSetDeviceCapability(this.device, 'measure_battery', parseFloat(percentage));
      }

      // Update alarm if low battery
      if (this.device.hasCapability('alarm_battery')) {
        await safeSetDeviceCapability(this.device, 'alarm_battery', percentage < 20);
      }
