 ...
 *    2. MULT2      - Valeur Ã— 2 = pourcentage (ZCL standard)                 
 *    3. VOLTAGE    - Conversion tension  pourcentage (courbes chimie)       
 *    4. ENUM       - Ã‰tats discrets (low/medium/high  10/50/100%)           
 *    
 *                                                                               
 *   COURBES DE DÃ‰CHARGE NON-LINÃ‰AIRES PAR CHIMIE:                               
 *   - CR2032/CR2450: Plateau 3.0V2.7V puis chute rapide                       
 *   - CR123A:        Lithium primaire, trÃ¨s plat jusqu'Ã 20%                   
 *   - Alkaline:      DÃ©clin progressif, non-linÃ©aire                           
 *   - Li-ion:        4.2V3.0V, courbe sigmoÃ¯de                                
 *   - LiFePO4:       3.6V2.5V, trÃ¨s plat (90% capacitÃ© = 3.2-3.3V)            
 *   - NiMH:          1.4V1.0V, plateau Ã 1.2V                                 
 *                                                                               
 * 
 */
...
// 
// COURBES DE DÃ‰CHARGE - DonnÃ©es empiriques par chimie
// 

/**
 * Courbes de dÃ©charge: [voltage]  [capacity %]
 * BasÃ©es sur datasheets et mesures empiriques
 * Format: [[V, %], [V, %], ...]
 */
const DISCHARGE_CURVES = {
  // 
  // CR2032 - Lithium manganese dioxide (Li-MnO2)
  // CaractÃ©ristique: Plateau plat 3.0-2.7V, chute rapide aprÃ¨s
  // 
  cr2032: [
    [3.00, 100],  // Neuf
    [2.95, 95],
    [2.90, 85],   // DÃ©but plateau
    [2.85, 70],
    [2.80, 55],
    [2.75, 40],
    [2.70, 25],   // Fin plateau
  ...
  // 
  // CR2450 - Plus grande capacitÃ©, mÃªme chimie
  // 
  cr2450: [
    [3.00, 100],
    [2.95, 95],
  ...
  // 
  // CR123A - Lithium primaire 3V, trÃ¨s plat
  // 
  cr123a: [
    [3.00, 100],
    [2.95, 90],
  ...
  // 
  // Alkaline AAA/AA - DÃ©clin non-linÃ©aire
  // 2x en sÃ©rie = 3.0V max, 2.0V min
  // 
  aaa_alkaline: [
  ...
  // 
  // Lithium AAA/AA - TrÃ¨s plat (Energizer Lithium)
  // 
  aaa_lithium: [
    [3.00, 100],
    [2.90, 90],
  ...
  // Courbe sigmoÃ¯de caractÃ©ristique
  // 
  lipo: [
  ...
  // 
  // LiFePO4 - TrÃ¨s plat (90% capacitÃ© entre 3.2-3.3V)
  // 
  lifepo4: [
  ...
  // 
  // NiMH - Rechargeable, plateau Ã 1.2V
  // 2x en sÃ©rie = 2.8V max, 2.0V min
  // 
  nimh: [
  ...
  // 
  // Default / Unknown - LinÃ©aire 2.0-3.0V
  // 
  unknown: [
    [3.00, 100],
    [2.75, 75],
...
class BatteryCalculator {
  ...
  /**
   * MÃ©thode principale - Calcul intelligent selon profil
   *
   * @param {number} rawValue - Valeur brute reÃ§ue
   * @param {object} options - Options de calcul
   * @param {string} options.algorithm - Algorithme Ã utiliser
  ...
  static calculate(rawValue, options = {}) {
    ...
    case ALGORITHM.DIRECT:
      ...
      // 
      // MULT2 - ZCL standard (batteryPercentageRemaining = % Ã— 2)
      // 
    case ALGORITHM.MULT2:
      percentage =safeMultiply(rawValue, 2);
      break;
      ...
      // 
    case ALGORITHM.DIV2:
      percentage = safeParse(rawValue, 2);
      break;

      // 
      // VOLTAGE_LINEAR - Interpolation linÃ©aire
      // 
    case ALGORITHM.VOLTAGE_LINEAR:
      ...
      // 
      // VOLTAGE_CURVE - Courbe de dÃ©charge non-linÃ©aire
      // 
    ...
    case ALGORITHM.MILLIVOLT:
      const voltage = safeParse(rawValue, 1000); // mV  V
      percentage = this.voltageToPercentCurve(voltage, chemistry);
      break;

      // 
      // ENUM_3 - Ã‰tats discrets: 0=low, 1=medium, 2=high
      // 
    case ALGORITHM.ENUM_3:
      ...
      // 
      // ENUM_4 - Ã‰tats discrets: 0=critical, 1=low, 2=medium, 3=high
      // 
  ...
  /**
   * Conversion voltage  pourcentage avec courbe de dÃ©charge
   *
   * @param {number} voltage - Voltage mesurÃ© (V)
  ...
  static voltageToPercentCurve(voltage, chemistry = CHEMISTRY.UNKNOWN) {
    ...
    for (let i = 0; i < curve.length - 1; i++) {
      ...
      if (voltage >= v2 && voltage <= v1) {
        // Interpolation linÃ©aire entre les deux points
...
return p2  + safeMultiply(ratio, (p1 - p2));
  ...
  /**
   * Conversion voltage  pourcentage linÃ©aire
   *
   * @param {number} voltage - Voltage mesurÃ© (V)
  ...
  static voltageToPercentLinear(voltage, vMin = 2.0, vMax = 3.0) {
    ...
    return ((voltage - vMin) /safeMultiply((vMax - vMin)), 100);
  }
  ...
  /**
   * DÃ©tecte si la batterie est faible
   *
   * @param {number} percentage - Pourcentage actuel
   * @param {number} threshold - Seuil (dÃ©faut 20%)
   * @returns {boolean}
   */
  ...
  /**
   * DÃ©tecte si la batterie est critique
   *
   * @param {number} percentage - Pourcentage actuel
   * @param {number} threshold - Seuil (dÃ©faut 10%)
   * @returns {boolean}
   */
  ...
  /**
   * Obtenir la courbe de dÃ©charge pour une chimie
   *
   * @param {string} chemistry - Chimie de la batterie
