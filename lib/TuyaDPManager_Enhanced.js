...
class TuyaDPManager_Enhanced {
  ...
  _autoProcessDP(dpId, value, type) {
    ...
    if (this.criticalDPs.battery.includes(dpId)) {
      ...
      if (this.device.hasCapability('measure_battery')) {
        this.device.setCapabilityValue('measure_battery', value).catch(err =>
    ...
    if (this.criticalDPs.humidity.includes(dpId)) {
      ...
      if (this.device.hasCapability('measure_humidity')) {
        this.device.setCapabilityValue('measure_humidity', value).catch(err =>
  ...
  async requestDP(dpId, retryCount = 0) {
    try {
      ...
      // Méthode 1: dataRequest (SDK3 standard)
      if (this.tuyaCluster.dataRequest) {
        await this.tuyaCluster.dataRequest({ dp: dpId });
        this.device.log(`[DP_MGR] ✅ DP${dpId} requested`);
        return true;
      }

      // DISABLED: active DP queries fail on sleepy devices
      this.device.log(`[DP_MGR] ⚠️ Skipping active DP${dpId} query (waiting for passive report)`);
      return false;

      /* COMMENTED OUT - doesn't work for battery devices
      if (this.tuyaCluster.sendData) {
        await this.tuyaCluster.sendData({
          command: 'dataQuery',
          dp: dpId
        });
        this.device.log(`[DP_MGR] ✅ DP${dpId} requested (sendData)`);
        return true;
      }

      if (this.tuyaCluster.command) {
        await this.tuyaCluster.command('dataQuery', { seq: Math.floor(Math.random() * 0xFFFF) });
        this.device.log(`[DP_MGR] ✅ DP${dpId} requested (command)`);
        return true;
      }
      */
