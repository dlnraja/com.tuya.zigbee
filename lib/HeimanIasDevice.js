...
class HeimanIasDevice extends ZigBeeDevice {
   ...
   * v7.4.6: Multi-stage discovery pattern (Direct ID -> Driver-prefixed -> Namespaced)
   */
  _getFlowCard(id, type = 'trigger') {
    try {
      ...
      const methodName = typeMap[type] || 'getTriggerCard';
      const flowMethod = this.homey.flow[methodName];
      if (typeof flowMethod !== 'function') return null;

      const driverId = this.driver?.id || (this.id ? this.id.split(':')[0]);
      ...
      // Legacy variants
      idVariants.push(`tuya_${id}`);
      if (driverId && !id.startsWith(driverId)) {
        idVariants.push(`${driverId}_tuya_${id}`);
      }
      ...
      // v7.4.6: Last resort - try to find it via driver directly
      if (this.driver && typeof this.driver[methodName] === 'function') {
        const card = this.driver[methodName](id);
        if (card) return card;
      }

      this.log?.(`[HeimanIAS] No flow card found for: ${id} (tried variants: ${idVariants.join(', ')})`);
    } catch (err) {
      if (this.error) this.error(`[HeimanIAS] Flow discovery error for ${id}:`, err.message);
    }
    return null;
  }
    ...
    try {
      ...
      if (zoneState.zoneState === 0) {
        ...
        await iasZoneCluster.zoneEnrollResponse({
          ...
          zoneId: 10 // v5.11.130 recall: zoneId 10 is the standard for Tuya SOS/Buttons
        });
  ...
  _triggerAlarmFlow(alarmType) {
    try {
      const triggerCard = this._getFlowCard(`heiman_${alarmType}_alarm`, 'trigger');
      if (triggerCard) {
  ...
  async _initBattery() {
    ...
    try {
      ...
      if (!endpoint?.clusters?.powerConfiguration) {
        this.log('[HeimanIAS] No genPowerCfg cluster, battery via Tuya DP?') : null;
        return;
      }
  ...
  onBatteryReport(rawValue) {
    const percentage = Math.round(safeParse(rawValue));
    const clampedPercentage = Math.min(100, Math.max(0, percentage));

    this.log(`[HeimanIAS] Battery: ${clampedPercentage}%`);

    if (this.hasCapability('measure_battery')) {
      this.setCapabilityValue('measure_battery', parseFloat(clampedPercentage)).catch(this.error);
    }
