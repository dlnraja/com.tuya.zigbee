...
class HeimanIasDevice extends ZigBeeDevice {
  ...
  async onHeimanInit() {
    ...
    this._heimanModel = this.getSetting('zb_model_id') || 'Unknown';
   ...
   */
  async onIasZoneReport(zoneStatus) {
    ...
    if (this.hasCapability('alarm_smoke')) {
      await this.setCapabilityValue('alarm_smoke', alarm1).catch(this.error);
      if (alarm1) {
    ...
    if (this.hasCapability('alarm_co')) {
      await this.setCapabilityValue('alarm_co', alarm1).catch(this.error);
      if (alarm1) {
    ...
    if (this.hasCapability('alarm_gas')) {
      await this.setCapabilityValue('alarm_gas', alarm1).catch(this.error);
      if (alarm1) {
    ...
    if (this.hasCapability('alarm_water')) {
      await this.setCapabilityValue('alarm_water', alarm1).catch(this.error);
      if (alarm1) {
    ...
    if (this.hasCapability('alarm_motion')) {
      await this.setCapabilityValue('alarm_motion', alarm1).catch(this.error);
    }

    // Update contact alarm (for door/window sensors)
    if (this.hasCapability('alarm_contact')) {
      await this.setCapabilityValue('alarm_contact', alarm1).catch(this.error);
    }
    ...
    if (this.hasCapability('alarm_battery')) {
      await this.setCapabilityValue('alarm_battery', batteryLow || batteryDefect).catch(this.error);
    }
   ...
   */
  async onBatteryReport(rawValue) {
    ...
    if (this.hasCapability('measure_battery')) {
      await this.setCapabilityValue('measure_battery', parseFloat(clampedPercentage)).catch(this.error);
    }

    if (this.hasCapability('alarm_battery')) {
      await this.setCapabilityValue('alarm_battery', clampedPercentage < 15).catch(this.error);
    }
   ...
   */
  isHeiman() {
    return CI.containsCI(this._heimanManufacturer, 'heiman') || CI.containsCI(this._heimanManufacturer, 'hs');
  }
