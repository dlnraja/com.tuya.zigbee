...
async function autoMigrateDriver(device, targetDriverId) {
  try {
    device.log(`🔄 AUTO-MIGRATION: ${device.driver.id} → ${targetDriverId}`);
    ...
    if (!targetDriver) {
      device.error(`❌ Target driver not found: ${targetDriverId}`);
      return false;
    }
    
    device.log(`✅ Target driver found: ${targetDriverId}`);
    device.log(`⚠️  Migration will preserve device data and capabilities`);
    ...
    device.log(`📦 Current device data:`, deviceData);
    device.log(`⚙️  Current settings:`, deviceSettings);
    device.log(`🎯 Current capabilities:`, deviceCapabilities);
    
    // Attempt migration via setDriver
    try {
      await device.setDriver(targetDriverId);
      device.log(`✅ MIGRATION SUCCESS: Device now using ${targetDriverId}`);
      return true;
    } catch (setDriverErr) {
      device.error(`❌ setDriver failed:`, setDriverErr.message);
      device.log(`ℹ️  Manual migration required by user`);
      return false;
    }
    
  } catch (err) {
    device.error('❌ autoMigrateDriver error:', err.message);
    return false;
  }
}
