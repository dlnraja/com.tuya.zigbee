...
function getDeviceOverride(modelId, manufacturer) {
  ...
  const overrides = {
    ...
    'TS0002_TZ3000_h1ipgkwn': {
      ...
      recommendedDriver: 'usb_outlet_2port'
    },
...
function isTuyaDP(deviceInfo, device = null) {
  try {
    ...
    // RULE 3: Check for cluster 0xEF00 presence (if zclNode available)
    if (deviceInfo.zclNode) {
      try {
        const endpoints = deviceInfo.zclNode.endpoints || {};
        for (const ep of Object.values(endpoints)) {
          if (ep.clusters?.manuSpecificTuya || ep.clusters?.['0xEF00']) {
            device?.log?.('[isTuyaDP] ✅ Cluster 0xEF00 found → Tuya DP device');
            return true;
          }
        }
      } catch (e) {
    ...
    // RULE 4: Legacy cluster check (if clusters object provided)
    const clusters = deviceInfo.clusters || {};
    if (clusters.manuSpecificTuya || clusters['0xEF00'] || clusters['61184']) {
      device?.log?.('[isTuyaDP] ✅ Cluster in deviceInfo → Tuya DP device');
      return true;
    }
