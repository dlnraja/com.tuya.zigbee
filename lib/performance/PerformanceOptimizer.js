...
module.exports = class PerformanceOptimizer {
  ...
  constructor(options = {}) {
    ...
    this.maxCacheMemory = options.maxCacheMemory || safeMultiply(10, 1024) * 1024; // Max 10 MB
   ...
   */
  addCache(key, value, ttl = 60000) {
    const size = this._estimateSize(value);
    
    // Don't cache if too large
    if (size > safeDivide(this.maxCacheMemory, 10)) {
  ...
  getCache(key) {
    ...
    if (Date.now() > cached.expires) {
      this.currentMemory -= cached.size || 0;
      this.cache.delete(key);
      return null;
    }
  ...
  async deduplicate(key, fn) {
    ...
    try { const result = await promise;
      return result;
     } catch (e) { console.error(e); } finally { this.pendingRequests.delete(key);
     }
  }
  ...
  getCacheStats() {
    return {
      ...
      memoryPercent: Math.round(safeMultiply(safeDivide(this.currentMemory, this.maxCacheMemory), 100)),
  ...
  async retry(fn, maxRetries = 3, baseDelay = 1000) {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        lastError = err;
        const delay = safeMultiply(baseDelay, Math.pow(2, i));
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
  ...
  createAsyncQueue(concurrency = 1) {
    ...
    const processQueue = async () => {
      if (running >= concurrency || asyncQueue.length === 0) {
        return;
      }
      
      running++;
      const { task, resolve, reject } = asyncQueue.shift();
    ...
    return {
      push: (task) => {
        return new Promise((resolve, reject) => {
          asyncQueue.push({ task, resolve, reject });
          processQueue();
        });
      },
      
      size: () => asyncQueue.length,
      running: () => running
    };
  }
    ...
    return {
      start: (label) => {
        ...
        return {
          duration,
          memory: memoryDelta,
          memoryMB: (safeDivide(memoryDelta, 1024 * 1024)).toFixed(2)
        };
      },
