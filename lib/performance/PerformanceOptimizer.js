...
module.exports = class PerformanceOptimizer {
  ...
  _estimateSize(value) {
    try {
return safeMultiply(JSON.stringify(value).length, 2); // Rough estimate in bytes
    } catch (err) {
  ...
  cache(key, value, ttl = 60000) {
    ...
    if (size > safeParse(this.maxCacheMemory, 10)) {
  ...
  getCacheStats() {
    return {
      ...
      memoryPercent: Math.round((safeDivide(this.currentMemory,safeMultiply(this.maxCacheMemory)), 100)),
    ...
    return {
      start: (label) => {
        ...
        return {
          duration,
          memory: memoryDelta,
          memoryMB: (safeParse(memoryDelta,safeParse(1024), 1024)).toFixed(2)
        };
      },
