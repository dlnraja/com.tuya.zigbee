...
/** P2447: compose productId arrays are often polluted (01MINIZB, LCT*, plugs…). */
function isLikelyPollutedPid(pid) {
  const p = String(pid || '').toUpperCase();
  if (!p) return true;
  // Real Tuya Zigbee modelIds almost always TS… / TY… / ZG-… / retail AM43…
  if (/^TS[0-9A-Z]{3,}$/i.test(p)) return false;
  if (/^TY[0-9A-Z]/i.test(p)) return false;
  if (/^ZG-/i.test(p)) return false;
  if (/^AM43/i.test(p)) return false;
  if (/^CK-/i.test(p)) return false;
  return true;
}

function loadMfsModelIds(mfr) {
  try {
    const db = loadJson(resolve('data/mfs_db.json')) || {};
    const mfs = db.manufacturers || db;
    const entry = mfs[mfr] || mfs[String(mfr).toLowerCase()] || mfs[String(mfr).toUpperCase()];
    if (!entry) return null;
    const ids = entry.modelIds || (entry.pid ? [entry.pid] : []);
    return {
      driverId: entry.driverId || entry.driver || null,
      modelIds: ids.filter(Boolean),
    };
  } catch {
    return null;
  }
...
function resolveCoupleCandidates(mfr, model, context) {
  // WHY (P2358): pidCandidates × PRODUCT_ID_DEFAULTS invented _TZ3210_tgvtvdoc+TS0001→switch_1gang
  // while compose already locks rain TS0207/TS0601. Never trust productId_default for MISSING_PID.
  // WHY (P2447): one mfr → many verified pids OK; do NOT rank polluted compose junk
  // (01MINIZB first in button_wireless_4) over mfs_db/registry sacred pids (xabckq1v→TS004F).
  const candidates = [];
  const seen = new Set();
  const add = (pid, source, driver, confidence, extra = {}) => {
    const k = `${norm(mfr)}|${norm(pid)}`;
    if (!pid || seen.has(k)) return;
    seen.add(k);
    candidates.push({ mfr, pid, source, driver, confidence, ...extra });
  };

  const mfsHit = loadMfsModelIds(mfr);
  if (mfsHit?.modelIds?.length) {
    for (const pid of mfsHit.modelIds) {
      add(pid, 'mfs_db', mfsHit.driverId, 99);
    }
  }

  // Registry rows for this mfr (multi-pid doctrine — keep all listed productIds)
  try {
    const regPath = resolve('data/user-misattribution-registry.json');
    const reg = loadJson(regPath);
    for (const row of reg?.cases || []) {
      const mfrs = (row.mfr || []).map((x) => norm(x));
      if (!mfrs.includes(norm(mfr))) continue;
      for (const pid of row.productId || []) {
        add(pid, 'misattribution-registry', row.canonicalDriver, 98);
      }
    }
  } catch { /* optional */ }

  const composeHits = context.composeIndex.get(norm(mfr)) || [];
  const composePidSet = new Set();
  for (const hit of composeHits) {
    for (const pid of hit.pids) {
      composePidSet.add(String(pid).toUpperCase());
      // Polluted non-TS junk gets low score; real TS* stays high but below mfs/registry
      const conf = isLikelyPollutedPid(pid) ? 40 : 96;
      add(pid, 'driver-compose', hit.driver, conf);
    }
  }

  // If mfr is already in a driver compose, only probe those pids — do not invent from pidCandidates.
  // Prefer mfs/registry pids when present (multi-pid OK, invent forbidden).
  const preferredProbe = new Set([
    ...(mfsHit?.modelIds || []).map((p) => String(p).toUpperCase()),
  ]);
  const pidsToProbe = preferredProbe.size
    ? [...preferredProbe]
    : (composePidSet.size
      ? [...composePidSet].filter((p) => !isLikelyPollutedPid(p))
      : (model.pidCandidates || []));
  ...
  for (const pid of pidsToProbe) {
    ...
    // Refuse productId-only defaults (cartesian invent). Exact / CI / heuristic compound only.
    if (fp?.driver && fp.matchType !== 'productId_default') {
      add(pid, 'device-fingerprint-db', fp.driver, 90);
    }
  ...
  for (const c of post.couples || []) {
    ...
    if (c.verdict === 'MISSING_PID' && c.mfr) {
      ...
      if (ranked.preferred) {
        ...
        // P2358/P2447: compose/mfs/registry-known mfr → tip update, not "soft invent"
        const knownSrc = ['driver-compose', 'mfs_db', 'misattribution-registry', 'device-truth']
          .includes(ranked.preferred.source);
        findings.autoResolution = knownSrc
          ? 'alreadyInCatalog'
          : (ranked.preferred.confidence >= 92 ? 'resolvedRegistry' : 'resolvedHeuristic');
        findings.userAction = knownSrc
          ? `Already in ${ranked.preferred.driver} — update Universal Tuya Test + re-pair`
          : ranked.userGuidance;
      } else if (ranked.softHypothesis) {
        inv.softHypothesis = ranked.softHypothesis;
        const knownSoft = ['driver-compose', 'mfs_db', 'misattribution-registry', 'device-truth']
          .includes(ranked.softHypothesis.source);
        findings.autoResolution = knownSoft
          ? 'alreadyInCatalog'
          : 'softHypothesis';
        findings.userAction = knownSoft
          ? `Already in ${ranked.softHypothesis.driver} — update Universal Tuya Test + re-pair`
          : ranked.userGuidance;
