...
function resolveCoupleCandidates(mfr, model, context) {
  ...
  for (const hit of composeHits) {
    for (const pid of hit.pids) {
      ...
      // Compose mfr hit beats cartesian pid defaults (was 85 < productId_default 90)
      add(pid, 'driver-compose', hit.driver, 96);
    }
  }

  // If mfr is already in a driver compose, only probe those pids — do not invent from pidCandidates.
  const pidsToProbe = composePidSet.size
    ? [...composePidSet]
    : (model.pidCandidates || []);
  ...
  for (const c of post.couples || []) {
    ...
    if (c.verdict === 'MISSING_PID' && c.mfr) {
      ...
      if (ranked.preferred) {
        ...
        // P2358: compose-known mfr → tip update, not "soft invent"
        findings.autoResolution = ranked.preferred.source === 'driver-compose'
          ? 'alreadyInCatalog'
          : (ranked.preferred.confidence >= 92 ? 'resolvedRegistry' : 'resolvedHeuristic');
        findings.userAction = ranked.preferred.source === 'driver-compose'
        ...
        findings.autoResolution = ranked.softHypothesis.source === 'driver-compose'
          ? 'alreadyInCatalog'
          : 'softHypothesis';
        findings.userAction = ranked.softHypothesis.source === 'driver-compose'
