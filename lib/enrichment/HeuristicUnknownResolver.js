...
function loadHeuristicModel() {
  // 1) Homey-bundled runtime model (preferred on device) — Buffer parse (P2586)
  try {
    if (fs.existsSync(RUNTIME_MODEL)) {
      try {
        const { loadJsonBuffer } = require('../performance/IntelligentLazyLoad');
        const m = loadJsonBuffer(RUNTIME_MODEL, { maxBytes: 512 * 1024 });
        if (m) return m;
      } catch { /* fall through */ }
      return JSON.parse(fs.readFileSync(RUNTIME_MODEL));
    }
  } catch { /* try CI */ }
...
function resolveUnknownCouple({ mfr, candidates = [], postText = '', issues = [], context = {}, model: modelIn }) {
  ...
  // WHY(P2447): skip demote when top is mfs/registry sacred pid (siblings on same driver are OK)
  const topKnown = top && ['mfs_db', 'misattribution-registry', 'device-truth'].includes(top.source);
  if (list.length > 1 && gap < 8 && tier.name === 'hardLock' && !topKnown) {
