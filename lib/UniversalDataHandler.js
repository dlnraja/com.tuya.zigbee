...
class UniversalDataHandler extends EventEmitter {
  ...
  constructor(device, options = {}) {
    ...
    this._rawHandlers = [];
    // ponytail: MINIMAL_INIT - _unknownDPs was missing; needed by getLearnedPatterns()
    this._unknownDPs = {};

    // Increase max listeners
    this.setMaxListeners(this.options.maxListeners);

    // ponytail: TRIMMED_BANNERS - box drawing was YAGNI; single line is enough
    this._log('[UDH] Created');
  }
...
// console.log(`[UDH] ${message}`, ...args);
  ...
  // ponytail: ONE_LINER_LOG - 7-line box was YAGNI for hot-path; single line is sufficient
  _logData(stage, data, extra = {}) {
    const parts = [`[${stage}] ${this._getDataPreview(data)}`];
    if (extra.dpId !== undefined) {parts.push(`dp=${extra.dpId}`);}
    if (extra.cluster !== undefined) {parts.push(`cluster=${extra.cluster}`);}
    if (extra.endpoint !== undefined) {parts.push(`ep=${extra.endpoint}`);}
    if (extra.event !== undefined) {parts.push(`evt=${extra.event}`);}
    this._log(parts.join(' '));
  }
  ...
  // ponytail: TRIMMED_BANNERS - diagnostic boxes were YAGNI for production; single-line log is enough
  async initialize(zclNode) {
    this._log(`[UDH] Initializing: device=${this.device?.getName?.()} zclNode=${zclNode ? 'present' : 'NULL'}`);
    ...
    this._log('[UDH] Initialization complete');
  ...
  _detectDeviceInfo() {
    ...
    const modelId = settings.zb_model_id || settings.zb_modelId || store.modelId || data.modelId || 'unknown';
    const manufacturer = settings.zb_manufacturer_name || settings.zb_manufacturerName || store.manufacturerName || data.manufacturerName || 'unknown';

    // Protocol detection (case-insensitive)
    const mfrLower = TU.normalize(manufacturer);
  ...
  async _setupTuyaListeners(zclNode) {
    ...
    if (tuyaCluster.on) {
      ...
      for (const attr of attrEvents) {
        ...
        } catch (e) { /* ponytail: attr event may not exist on this cluster version */ }
      }
    }
      ...
      for (const [clusterName, config] of Object.entries(standardClusters)) {
        ...
        if (typeof cluster.on === 'function') {
          ...
          if (config.capability === 'auto_ias') {
            ...
            if (this.device.hasCapability('alarm_motion')) {
              ...
              // Default to alarm_generic or alarm_motion for generic sensors
              if (this.device.hasCapability('alarm_generic')) {
                resolvedConfig.capability = 'alarm_generic';
                this._log('   🔄 IAS Zone mapped to alarm_generic (generic sensor)');
              } else {
                resolvedConfig.capability = 'alarm_motion';
                this._log('   🔄 IAS Zone mapped to alarm_motion (fallback)');
              }
            }
          }
        ...
        for (const evt of reportEvents) {
          if (typeof cluster.on === 'function') {
            ...
            } catch (e) { /* ponytail: report event may not exist on this cluster */ }
          }
        }
      }
    }
  ...
  async _setupEndpointListeners(zclNode) {
    ...
    if (typeof zclNode.on === 'function') {
      ...
      for (const evt of globalEvents) {
        ...
        } catch (e) { /* ponytail: global event may not exist on this node */ }
      }
    }
  }
