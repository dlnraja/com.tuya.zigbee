...
class UniversalDataHandler extends EventEmitter {
  ...
  async initialize(zclNode) {
    this._log('');
    this._log('╔══════════════════════════════════════════════════════════════╗');
    this._log('║       INITIALIZATION STARTING                                ║');
    this._log('╚══════════════════════════════════════════════════════════════╝');

    if (!zclNode) {
      this._log('⚠️ No zclNode provided - limited functionality');
      return false;
    }
  ...
  _detectDeviceInfo() {
    ...
    const isStandardZCL = modelId.startsWith('TS02') || manufacturer === 'SONOFF' || manufacturer === 'eWeLink';
  ...
  _discoverEndpoints(zclNode) {
    ...
    return Object.keys(zclNode.endpoints).map(Number).sort();
  }
  ...
  // Add a handler for the soil moisture DP
  _handleTuyaData(data, eventType) {
    ...
    if (parsed && parsed.length > 0) {
      for (const dp of parsed) {
        ...
        // Handle soil moisture DP
        if (dp.dpId === 105) {
          let moisture = dp.value;
          if (moisture > 100) {
            moisture = moisture / 10;
          }
          this.emit('capability', 'measure_soil_moisture', moisture);
        }
  ...
  async _setupZCLListeners(zclNode) {
    ...
    const standardClusters = {
      ...
      // IAS Zone
      iasZone: { attr: 'zoneStatus', capability: 'alarm_contact', transform: (v) => (v & 1) !== 0 },
      ssIasZone: { attr: 'zoneStatus', capability: 'alarm_contact', transform: (v) => (v & 1) !== 0 }
    };
      ...
      for (const [clusterName, config] of Object.entries(standardClusters)) {
        ...
        if (typeof cluster.on === 'function') {
          cluster.on(`attr.${config.attr}`, (value) => {
            ...
            this._handleZCLAttribute(clusterName, config.attr, value, config);
          });
  ...
  _bumpMaxListeners(zclNode) {
    ...
    // Bump all endpoints and clusters
    for (const epId of Object.keys(zclNode?.endpoints || {})) {
      const endpoint = zclNode.endpoints[epId];
