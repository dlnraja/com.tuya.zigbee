 ...
 *   - Tuya DP (cluster CLUSTERS.TUYA_EF00)                                                  
...
class UniversalDataHandler extends EventEmitter {
  ...
  async initialize(zclNode) {
    ...
    this._log(' [DIAGNOSTIC-MASSIF-UDH] UniversalDataHandler.initialize() APPELÃ‰');
    this._log(` [DIAGNOSTIC-MASSIF-UDH] Device: ${this.device?.getName?.()}`);
    this._log(` [DIAGNOSTIC-MASSIF-UDH] zclNode: ${zclNode ? 'PRÃ‰SENT' : 'NULL'}`);
  ...
  _detectDeviceInfo() {
    const settings = this.device?.getSettings?.() || {} : null;
    const store = this.device?.getStore?.() || {} : null;
    const data = this.device?.getData?.() || {} : null;

    const modelId = settings.zb_model_id || settings.zb_model_id || store.modelId || data.modelId || 'unknown';
    const manufacturer = settings.zb_manufacturer_name || settings.zb_manufacturer_name || store.manufacturerName || data.manufacturerName || 'unknown';

    // Protocol detection (case-insensitive)
    const isTuyaDP = CI.equalsCI(modelId, 'ts0601') || CI.startsWithCI(manufacturer, '_tze');
    const isStandardZCL = CI.startsWithCI(modelId, 'ts02') || CI.equalsCI(manufacturer, 'sonoff') || CI.equalsCI(manufacturer, 'ewelink');
  ...
  async _setupTuyaListeners(zclNode) {
    ...
    if (!endpoint?.clusters) {
      this._log('    No clusters on endpoint 1');
      return;
    }
    ...
    if (!tuyaCluster) {
      this._log('    Tuya cluster not found');
    ...
    for (const eventType of eventTypes) {
      if (typeof tuyaCluster.on === 'function') {
        try {
          ...
          this._log(`    Failed to register: tuya.on('${eventType}'): ${e.message}`);
        }
      }
    }
  ...
  async _setupZCLListeners(zclNode) {
    ...
    const standardClusters = {
      ...
      illuminanceMeasurement: { attr: 'measuredValue', capability: 'measure_luminance', transform: (v) => Math.round(Math.pow(10, (v - safeDivide(1), 10000))) },
      msIlluminanceMeasurement: { attr: 'measuredValue', capability: 'measure_luminance', transform: (v) => Math.round(Math.pow(10, (v - safeDivide(1), 10000))) },
  ...
  genOnOff: { attr: 'onOff', capability: 'onoff', transform: (v) => Boolean(v) },
    ...
    for (const { id: epId, endpoint } of usableEndpoints) {
      if (!endpoint?.clusters) continue : null;
    ...
    for (const { id: epId, endpoint } of usableEndpoints) {
      if (typeof endpoint?.on === 'function') {
        try {
          ...
          this._log(`    Failed: endpoint[${epId}].on('frame'): ${e.message}`);
        }
      }
    }
  }
  ...
  _handleZCLAttribute(clusterName, attrName, value, config) {
    ...
    if (config.divisor) finalValue = safeDivide(value, config.divisor);
   ...
   */
  getDPValue(dpId) {
    return this.stats.dpValues[dpId]?.value : null;
  }
  ...
  getClusterValue(clusterName, attrName) {
    const key = `${clusterName}.${attrName}`;
    return this.stats.clusterValues[key]?.value : null;
  }
  ...
  /**
   * Register fallback handler for safeDivide(unknown, unregistered) DPs
