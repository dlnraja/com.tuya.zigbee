...
class TuyaSpecificClusterDevice extends TuyaZigbeeDevice {
    ...
    async writeBool(dp, value) {
        ...
        try {
            ...
            // WHY(P2336): return false so callers (wall_dimmer soft-fail) retry — undefined looked like success
    ...
    async writeData32(dp, value) {
        ...
        try {
            ...
            // WHY(P2336): same as writeBool — falsy must mean TX failed
    ...
    /**
     * WHY(P2332): must NOT permanently shadow PhysicalButtonMixin._triggerPhysicalFlow.
     * Legacy wall_*_tuya DP path calls `_triggerPhysicalFlow(bool)` (1 arg).
     * Mixin / BoundCluster path calls `_triggerPhysicalFlow(gang, pressType, tokens)`.
     * Contre quoi: raw getDeviceTriggerCard invent + broken gang/press routing.
     */
    async _triggerPhysicalFlow(gangOrValue, pressType, tokens = {}) {
        const legacyBool = arguments.length === 1
          && (typeof gangOrValue === 'boolean' || gangOrValue === 0 || gangOrValue === 1);
        if (legacyBool) {
            if (this._appCommandPending) {return;}
            const state = gangOrValue ? 'on' : 'off';
            const gang = Number(this._gangNumber) || 1;
            const driverId = this.driver?.id || 'switch';
            const ids = [
              `${driverId}_physical_gang${gang}_${state}`,
              `${driverId}_physical_${state}`,
            ];
            for (const tid of ids) {
                if (typeof this._safeTriggerFlow === 'function') {
                    // eslint-disable-next-line no-await-in-loop
                    await this._safeTriggerFlow(tid, { gang, button: String(gang) }, { type: 'tuya-dp-physical' });
                }
            }
            return;
        }
        return TuyaZigbeeDevice.prototype._triggerPhysicalFlow.call(
          this, gangOrValue, pressType, tokens
        );
    }
}
