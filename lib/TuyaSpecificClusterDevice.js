...
class TuyaSpecificClusterDevice extends ZigBeeDevice {
    ...
    async writeBool(dp, value) {
        ...
        // WHY(P2308): Missing Zigbee Node's IEEE Address — skip instead of noisy fail
        data.writeUInt8(value ? 0x01 : 0x00, 0);
        try {
            const token = this.getData?.()?.token || this.zclNode?.ieeeAddr;
            if (!token || !this.zclNode?.endpoints?.[1]?.clusters?.tuya) {
                this.error?.('[P2308] skip writeBool DP' + dp + ': Zigbee IEEE/cluster not ready');
                return false;
            }
    ...
    async safeSetCapabilityValue(capability, value, meta) {
        // WHY(P2301/P2308 Gmail #532): never call this.setCapabilityValue when
        // subclasses override set→safeSet (Maximum call stack).
        ...
        try {
            ...
            try {
                ...
                if (Device?.prototype?.setCapabilityValue) setter = Device.prototype.setCapabilityValue;
            } catch (_e) { /* optional */ }
            if (!setter) {
                const { ZigBeeDevice } = require('homey-zigbeedriver');
                if (ZigBeeDevice?.prototype?.setCapabilityValue) {
                    setter = ZigBeeDevice.prototype.setCapabilityValue;
                }
            }
            if (setter) return await setter.call(this, capability, value);
            return undefined;
        } catch (err) {
