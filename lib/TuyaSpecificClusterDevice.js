...
class TuyaSpecificClusterDevice extends TuyaZigbeeDevice {
     ...
     */
    /**
     * WHY(P2308 / Gmail m1cvyneb 9.0.688): Homey throws
     * "Missing Zigbee Node's IEEE Address" when token/node not ready.
     * Prefer DeviceIO first; never hard-crash the capability listener.
     */
    _hasZigbeeIdentity() {
        const data = (typeof this.getData === 'function' ? this.getData() : null) || {};
        const ieee = data.token
            || data.ieeeAddress
    ...
    async _datapoint(dp, datatype, data) {
        ...
        if (!this._hasZigbeeIdentity()) {
            this.error?.(`[P2308] skip DP${dp}: Zigbee IEEE/token not ready`);
            return false;
        }
        ...
        if (!epTuya?.datapoint) {
            this.error?.(`[P2308] skip DP${dp}: Tuya cluster unavailable`);
            return false;
        }
        return epTuya.datapoint(payload);
    }
