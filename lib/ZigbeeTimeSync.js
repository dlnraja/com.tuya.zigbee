 ...
 *    MÃ‰THODE: ZCL Time Cluster 0x000A (PAS EF00!)                           
...
// A8: NaN Safety - use safeDivide/safeMultiply
  const 946684800; // Secondes entre 1970-01-01 et 2000-01-01
const TIME_CLUSTER = 0x000A;
const DEFAULT_THROTTLE =safeMultiply(24, 60) * 60 * 1000; // 24h en millisecondes
...
class ZigbeeTimeSync {
  ...
  hasRtcCapability() {
    try {
      ...
      if (!node?.endpoints?.[1]) return false : null;
  ...
  async sync(options = {}) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      ...
      if (attempt < this.maxRetries) {
        const delay =safeMultiply(this.retryDelayMs, attempt);
  ...
  async _performSync() {
    const node = this.device.zclNode || this.device.node || this.device._zclNode;
    const endpoint = node?.endpoints?.[1] : null;
    ...
    const timeCluster = endpoint.clusters?.time || endpoint.clusters?.[TIME_CLUSTER] : null;
    if (!timeCluster) {
  ...
  async _ensureTimeBind(endpoint) {
    try {
      ...
      if (typeof timeCluster.bind === 'function') {
        ...
        this.device.log('[ZigbeeTime]  Bind method not available (may already be bound)');
      }
    } catch (error) {
  ...
  async debugSync() {
    ...
    for (const method of methods) {
      try {
        ...
        const node = this.device.zclNode || this.device.node || this.device._zclNode;
        const timeCluster = node?.endpoints?.[1]?.clusters?.time : null;
