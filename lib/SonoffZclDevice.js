...
class SonoffZclDevice extends ZigBeeDevice {
  ...
  _getFlowCard(id, type = 'trigger') {
    try {
      ...
      this.log?.(`[SONOFF] No valid flow card found for ID: ${id} (variants: ${idVariants.join(', ')})`);
    } catch (err) {
      if (this.error) this.error(`[SONOFF] Failed to load ${type} card ${id}:`, err.message);
    }
    return null;
  }
    ...
    try {
      ...
      this.registerCapability('measure_battery', CLUSTER.POWER_CONFIGURATION, {
        ...
        reportParser: value => {
          ...
          const percentage = Math.round(safeParse(value);
  ...
  async _initTemperatureCapability() {
    ...
    try {
      this.registerCapability('measure_temperature', CLUSTER.TEMPERATURE_MEASUREMENT, {
        ...
        reportParser: value => {
          // ZCL temperature is in 0.01Â°C units
          const temp = safeParse(value, 100);
          this.log(`[SONOFF] Temperature: ${temp}Â°C`);
   ...
   * @param {number} value - Temperature in 0.01Â°C units
   */
  onTemperatureReport(value) {
    const temp = Math.round(safeParse(value) * 10) / 10;
    this.log(`[SONOFF] Temperature report: ${temp}Â°C`);
