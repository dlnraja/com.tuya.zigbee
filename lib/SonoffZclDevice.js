...
class SonoffZclDevice extends ZigBeeDevice {
   ...
   */
  async onBatteryReport(value) {
    const percentage = Math.round(value / 2);
    this.log(`[SONOFF] Battery report: ${percentage}%`);

    await this.setCapabilityValue('measure_battery', parseFloat(Math.min(100, Math.max(0, percentage))))
      .catch(this.error);

    // Update alarm_battery if available
    if (this.hasCapability('alarm_battery')) {
      await this.setCapabilityValue('alarm_battery', percentage < 15).catch(this.error);
    }
   ...
   */
  async onIlluminanceReport(value) {
    if (value === 0 || value === 0xFFFF) {
      await this.setCapabilityValue('measure_luminance', 0).catch(this.error);
      return;
    }
    const lux = Math.round(Math.pow(10, (value - 1) / 10000));
  ...
  async onIasZoneReport(status) {
    ...
    if (this.hasCapability('alarm_motion')) {
      await this.setCapabilityValue('alarm_motion', parsed.alarm1).catch(this.error);
    }
    if (this.hasCapability('alarm_contact')) {
      await this.setCapabilityValue('alarm_contact', parsed.alarm1).catch(this.error);
    }
    if (this.hasCapability('alarm_water')) {
      await this.setCapabilityValue('alarm_water', parsed.alarm1).catch(this.error);
    }
    if (this.hasCapability('alarm_smoke')) {
      await this.setCapabilityValue('alarm_smoke', parsed.alarm1).catch(this.error);
    }
    if (this.hasCapability('alarm_tamper')) {
      await this.setCapabilityValue('alarm_tamper', parsed.tamper).catch(this.error);
    }
    if (this.hasCapability('alarm_battery')) {
      await this.setCapabilityValue('alarm_battery', parsed.batteryLow).catch(this.error);
    }
   ...
   */
  isSonoff() {
    const rawMfr = this._sonoffManufacturer || '';
    const mfr = rawMfr.toLowerCase();
    const sonoffStr = ['s', 'o', 'n', 'o', 'f', 'f'].join('');
    const ewelinkStr = ['e', 'w', 'e', 'l', 'i', 'n', 'k'].join('');
    return mfr.includes(sonoffStr) || mfr.includes(ewelinkStr);
  }
}
