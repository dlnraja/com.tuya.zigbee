...
const { isMemoryPressure, trimMapCache, boundedHttpsTextGet } = require('../utils/NetworkResilience');
const TuyaXiaomiOTAProvider = require('./TuyaXiaomiOTAProvider');

const MAX_IMAGE_CACHE_ENTRIES = 200;
const MAX_MANIFEST_BYTES = 4 * 1024 * 1024; // index.json — bounded (was unbounded string concat)
const MANIFEST_TIMEOUT_MS = 60000;
...
class OTARepository {
  ...
  constructor(homey) {
    ...
    // P2544: complementary multi-index cascade — OEM-safe only (never pvvx/doctor64/rococo)
    this.indexUrls = [
      ...
      // zigpy stable Z2M-format mirror (Koenkk-compatible fields) — complementary alternate
      'https://raw.githubusercontent.com/zigpy/zigpy-ota/release/files/stable/z2m_v1_ota.json',
    ];
  ...
  async findImage(manufacturerCode, imageType, currentVersion, modelId = null) {
    if (isMemoryPressure()) {
      this.log('[OTA] Skip findImage — memory pressure (local cache only)');
      const key = `${manufacturerCode}_${imageType}_${modelId || 'any'}`;
      const cached = this.cache.get(key);
      if (cached && Date.now() - cached.timestamp < this.manifestCacheDuration) {
        return cached.image;
      }
      return null;
    }
    ...
    if (TUYA_CODES.includes(manufacturerCode) || XIAOMI_CODES.includes(manufacturerCode)) {
      try {
        ...
        if (image) {
          ...
          trimMapCache(this.cache, MAX_IMAGE_CACHE_ENTRIES);
  ...
  async downloadManifest(url) {
    if (isMemoryPressure()) {
      this._manifestCaches = this._manifestCaches || new Map();
      const cached = this._manifestCaches.get(url);
      if (cached) {
        this.log('[OTA] Memory pressure — serving cached manifest');
        return cached.manifest;
      }
      throw new Error('[OTA] Memory pressure and no cached manifest');
    }

    // v10.6.0 FIX: cache must be keyed PER URL — a single shared cache made
    // the second index (fairecasoimeme) always return Koenkk's manifest.
    this._manifestCaches = this._manifestCaches || new Map();
    ...
    // v5.11.16 SEC: Enforce HTTPS for OTA manifest downloads
    if (!url.startsWith('https://')) {
      throw new Error(`[OTA-SEC] Refusing non-HTTPS manifest URL: ${  url.substring(0, 80)}`);
    }

    // v9.0.40 TITAN: Wrap in circuit breaker for fault tolerance
    const manifest = await this._breaker.exec(() => this._rawDownloadManifest(url));
    this._manifestCaches.set(url, { manifest, timestamp: Date.now() });
    return manifest;
  }

  async _rawDownloadManifest(url) {
    const data = await boundedHttpsTextGet(url, {
  ...
  async downloadImage(imageUrl, expectedSha512 = null) {
    // v5.11.16 SEC: Enforce HTTPS for firmware downloads (MITM prevention)
    if (!imageUrl || !imageUrl.startsWith('https://')) {
      throw new Error(`[OTA-SEC] Refusing non-HTTPS firmware download: ${  (imageUrl || '').substring(0, 80)}`);
    }
    // v9.0.40 TITAN: Wrap in circuit breaker for fault tolerance
    return this._breaker.exec(() => this._rawDownloadImage(imageUrl, expectedSha512));
  }

  async _rawDownloadImage(imageUrl, expectedSha512 = null) {
    // v9.0.385: garde-fous téléchargement
    const MAX_IMAGE_SIZE = 2 * 1024 * 1024; // 2 MB (les images Zigbee font < 500 KB)
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = protocol.get(imageUrl, (res) => {
        ...
        if (contentLength > MAX_IMAGE_SIZE) {
          fail(new Error(`[OTA-SEC] Image too large (${contentLength} bytes > ${MAX_IMAGE_SIZE})`));
          req.destroy();
          return;
        }
        ...
        res.on('data', (chunk) => {
          ...
          if (received > MAX_IMAGE_SIZE) {
            fail(new Error(`[OTA-SEC] Download exceeds max size (${MAX_IMAGE_SIZE} bytes)`));
            req.destroy();
            return;
          }
          chunks.push(chunk);
        ...
        res.on('end', () => {
          ...
          if (expectedSha512) {
            ...
            if (actualHash !== expectedSha512) {
              fail(new Error(`SHA512 mismatch! Expected: ${expectedSha512.substring(0, 16)}..., Got: ${actualHash.substring(0, 16)}...`));
              return;
            }
            this.log('[OTA-SEC] SHA512 verification passed');
          }
      ...
      req.setTimeout(DOWNLOAD_TIMEOUT_MS, () => {
        fail(new Error(`[OTA-SEC] Download timeout after ${DOWNLOAD_TIMEOUT_MS}ms`));
        req.destroy();
      });
    });
  }
