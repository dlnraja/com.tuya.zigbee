...
class OTAUpdateManager {
  ...
  /**
   * v9.0.385: Validate an OTA image header before flashing.
   * Checks the Zigbee OTA magic (0x0BEEF11E), manufacturer code match,
   * imageType match (when the device advertises one) and that the image
   * version is really newer than the device's current version.
   * Throws on any mismatch — never flash an unverified image.
   */
  _validateImageHeader(imageBuffer, updateCheck, device) {
    if (!Buffer.isBuffer(imageBuffer) || imageBuffer.length < 56) {
      throw new Error('[OTA-SEC] Image too small to contain an OTA header');
    }
    const magic = imageBuffer.readUInt32LE(0);
    if (magic !== 0x0BEEF11E) {
      throw new Error(`[OTA-SEC] Invalid OTA magic: 0x${magic.toString(16)} (expected 0x0BEEF11E)`);
    }
    const headerMfr = imageBuffer.readUInt16LE(10);
    const headerType = imageBuffer.readUInt16LE(12);
    const headerVersion = imageBuffer.readUInt32LE(14);

    if (updateCheck.manufacturerCode && headerMfr !== updateCheck.manufacturerCode) {
      throw new Error(`[OTA-SEC] Manufacturer mismatch: image ${headerMfr} vs device ${updateCheck.manufacturerCode}`);
    }
    if (updateCheck.imageType && headerType !== updateCheck.imageType) {
      throw new Error(`[OTA-SEC] imageType mismatch: image ${headerType} vs device ${updateCheck.imageType}`);
    }
    if (headerVersion <= updateCheck.currentVersion) {
      throw new Error(`[OTA-SEC] Image version ${headerVersion} not newer than current ${updateCheck.currentVersion}`);
    }
    this.log(`[OTA-SEC] Header OK: mfr=${headerMfr} type=${headerType} v${headerVersion}`);
  }
        ...
        for (const device of devices) {
          ...
          try {
            ...
            if (result && result.available) {
              updatesFound++;
              const deviceKey = device.getData().id;
              const previous = this.discoveredUpdates.get(deviceKey);
              this.discoveredUpdates.set(deviceKey, {
                deviceName: device.getName(),
              ...
              // v9.0.378: notify + flow trigger only for NEW versions (dedup)
              if (!previous || previous.availableVersion !== result.newVersion) {
                try {
                  // v9.0.379: notification avec guidance (Tuya MCU = flash via
                  // app constructeur ou passerelle, ZCL = flash local possible)
                  const isTuyaMcu = result.isTuyaMcu === true;
                  const guidance = isTuyaMcu
                    ? 'Flash via the manufacturer app or a Zigbee2MQTT setup with the Tuya OTA URL.'
                    : 'Flash from the device page (OTA check) or via Zigbee2MQTT.';
                  await this.homey.notifications.createNotification({
                    excerpt: `OTA update for ${device.getName()}: v${result.currentVersion} → v${result.newVersion}. ${guidance}`,
                  });
                } catch (e) { this.log('[OTA-DISCOVER] notification failed:', e.message); }
                try {
                  const card = this.homey.flow.getDeviceTriggerCard('ota_update_available');
                  await card.trigger(device, {
                    current_version: String(result.currentVersion),
                    new_version: String(result.newVersion),
                  });
                } catch (e) { this.log('[OTA-DISCOVER] trigger failed:', e.message); }
              }
            }
          } catch (e) {
  ...
  async checkUpdate(device) {
    try {
      ...
      // v10.6.0: version UNKNOWN (0 = swBuildId unparsable) — any indexed image
      // would look "newer" and spam false "update available" notifications.
      // With ~31 stale public Tuya images in existence, unknown means "we
      // cannot tell", not "upgrade!".
      if (!fileVersion) {
        return { available: false, reason: 'Current firmware version unknown — cannot determine update status' };
      }

      // Query repository (v9.0.378: device context for per-model matching)
      const availableImage = await this.repository.findImage(
        manufacturerCode,
        imageType,
        fileVersion,
        { modelId: otaInfo.modelId || null, manufacturerName: otaInfo.manufacturerName || null }
      );
      ...
      if (availableImage.fileVersion > fileVersion) {
        return {
          ...
          sha512: availableImage.sha512 || null, // v9.0.385: propagé pour vérif au flash
          changelog: availableImage.changelog || 'No changelog available',
          isTuyaMcu: imageType === 0, // fallback MCU Tuya (pas de cluster OTA ZCL)
        };
      }
  ...
  async performUpdate(device, options = {}) {
    ...
    try {
      ...
      // Download image (v9.0.385: SHA512 propagé depuis l'index)
      this.log(`[OTA] Downloading image for ${device.getName()}...`);
      const imageBuffer = await this.repository.downloadImage(updateCheck.url, updateCheck.sha512 || null);

      // v9.0.385: validation du header OTA avant flash (magic, fabricant,
      // imageType quand connu, version réellement supérieure)
      this._validateImageHeader(imageBuffer, updateCheck, device);
      ...
      // v10.6.0: homey-zigbeedriver exposes NO otaUpdate cluster — flashing
      // from Homey is impossible (verified: zero OTA refs in the driver).
      // Fail honestly instead of crashing on undefined.queryNextImageRequest.
      if (!endpoint?.clusters?.otaUpdate) {
        throw new Error('OTA flashing is not supported on Homey — pair the device to the Tuya app or zigbee2mqtt to flash it. Discovery and notification still work.');
      }
  ...
  getDeviceOTAInfo(device) {
    try {
      ...
      const modelId = device.getSetting?.('zb_model_id') || null;
      const manufacturerName = device.getSetting?.('zb_manufacturer_name') || null;
      if (endpoint && endpoint.clusters.otaUpdate) {
        return {
      ...
      // v9.0.378: Tuya MCU fallback — most Tuya devices have no ZCL OTA
      // cluster (firmware comes via Tuya MCU). Tuya manufacturer code is
      // 0x1141 (4417); imageType stays unknown (0) and the provider matches
      // by manufacturerName from the index restrictions.
      if (manufacturerName && String(manufacturerName).startsWith('_T')) {
        // Try to read the REAL current firmware version from the basic cluster
        // (swBuildId / appVersion) to avoid false "update available" positives
        let fileVersion = 0;
        try {
          const basic = endpoint?.clusters?.basic;
          const attrs = basic?.attributes || {};
          const sw = attrs.swBuildId ?? attrs.appVersion ?? attrs.stackVersion;
          const parsed = parseInt(sw, 10);
          if (Number.isFinite(parsed) && parsed > 0) {fileVersion = parsed;}
        } catch { /* keep 0 */ }
        return {
          manufacturerCode: 4417,
          imageType: 0,
          fileVersion, // dédup via discoveredUpdates quand 0 (inconnu)
          modelId,
          manufacturerName,
        };
      }
