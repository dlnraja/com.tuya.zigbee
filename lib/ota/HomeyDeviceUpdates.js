...
/**
 * Soft fleet hint once per device boot when native Homey Device Updates images exist.
 * Contre quoi: users never find Settings → Device Updates for OEM OTA.
 */
function softAnnounceNativeOta(device) {
  try {
    if (!device || device._p2508OtaHinted) return false;
    if (!driverHasNativeFirmwareUpdates(device)) return false;
    device._p2508OtaHinted = true;
    const name = (typeof device.getName === 'function' && device.getName()) || 'device';
    const ssot = loadSsot();
    const pathHint = ssot.userPaths?.settings || 'Settings → Device Updates';
    device.log?.(`[P2508] OEM Zigbee OTA images shipped — use Homey → ${pathHint} (or Maintenance) for ${name}`);
    return true;
  } catch {
    return false;
  }
