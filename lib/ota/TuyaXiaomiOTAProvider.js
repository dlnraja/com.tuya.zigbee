...
// v5.11.16 SEC: http import removed — all OTA traffic is HTTPS-only
...
const OTA_SOURCES = {
  ...
  // Complementary OEM mirrors (P2544 L99)
  FAIRECASOIMEME: 'https://raw.githubusercontent.com/fairecasoimeme/zigbee-OTA/master/index.json',
  ZIGPY_Z2M_STABLE: 'https://raw.githubusercontent.com/zigpy/zigpy-ota/release/files/stable/z2m_v1_ota.json',
  // Image directories
...
class TuyaXiaomiOTAProvider {
  ...
  async fetchOTAIndex(forceRefresh = false) {
    ...
    try {
      ...
      // v9.0.385: validation de schéma — garder seulement les entrées valides,
      // et ne JAMAIS remplacer le cache par un index vide/invalide
      if (!Array.isArray(parsed)) {
        throw new Error('[OTA-SEC] Index is not an array');
      }
      const valid = parsed.filter(img =>
        img && typeof img === 'object'
        && Number.isFinite(img.manufacturerCode)
        && Number.isFinite(img.imageType)
        && Number.isFinite(img.fileVersion)
        && typeof img.url === 'string' && img.url.startsWith('https://')
      );
      if (valid.length === 0) {
        throw new Error('[OTA-SEC] Index contains no valid entries — keeping previous cache');
      }
      if (valid.length < parsed.length * 0.5) {
        this.log(`[OTA-SEC] WARNING: ${parsed.length - valid.length}/${parsed.length} invalid index entries dropped`);
      }
      this.indexCache = valid;
  ...
  async findImage(manufacturerCode, imageType, currentVersion, deviceInfo = {}) {
    ...
    // v9.0.378: imageType unknown (0) = Tuya MCU devices without ZCL OTA
    // cluster — match by manufacturerName instead (index entries carry
    // manufacturerName restrictions).
    ...
    const candidates = index.filter(img => {
      ...
      // Must match image type (or fallback to manufacturerName when unknown)
      if (!typeUnknown && img.imageType !== imageType) {return false;}
      if (typeUnknown) {
        if (!manufacturerName || !Array.isArray(img.manufacturerName) || img.manufacturerName.length === 0) {return false;}
        if (!TU.includesCI(img.manufacturerName, manufacturerName)) {return false;}
      }
      ...
      if (img.manufacturerName && img.manufacturerName.length > 0 && manufacturerName) {
        if (!TU.includesCI(img.manufacturerName, manufacturerName)) {
          return false;
        }
      }
  ...
  async checkForUpdate(device) {
    try {
      ...
      const manufacturerName = settings.zb_manufacturer_name || settings.zb_manufacturerName;
      ...
      if (tuyaInfo) {
        const image = await this.findImage(
          ...
          tuyaInfo.imageType,
          currentVersion,
          { modelId, manufacturerName } // v10.6.0: was a bare string — destructured away
        );
      ...
      if (xiaomiInfo) {
        const image = await this.findImage(
          ...
          currentVersion,
          { modelId, manufacturerName } // v10.6.0: was a bare string
        );
  ...
  async _httpGet(url) {
    // v5.11.16 SEC: Enforce HTTPS for all OTA network requests
    if (!url || !url.startsWith('https://')) {
      throw new Error(`[OTA-SEC] Refusing non-HTTPS request: ${  (url || '').substring(0, 80)}`);
    }
    // v9.0.40 TITAN: Wrap in circuit breaker for fault tolerance
    return this._breaker.exec(() => this._rawHttpGet(url));
  }

  async _rawHttpGet(url) {
    return new Promise((resolve, reject) => {
      const protocol = https;

      // v10.6.0: 60s timeout + 2MB streaming cap (parity with OTARepository
      // hardening — the provider's raw path had neither).
      ...
      const req = protocol.get(url, (res) => {
        ...
        if (len > MAX_BYTES) {
          req.destroy(new Error(`OTA response too large (${len} bytes)`));
          return;
        }
        ...
        res.on('data', chunk => {
          ...
          if (received > MAX_BYTES) {
            req.destroy(new Error('OTA response exceeded 2MB cap'));
            return;
          }
          data += chunk;
        });
      ...
      req.setTimeout(60000, () => req.destroy(new Error('OTA request timeout (60s)')));
      req.on('error', reject);
    });
  }

  async _httpGetBuffer(url) {
    // v5.11.16 SEC: Enforce HTTPS for firmware image downloads
    if (!url || !url.startsWith('https://')) {
      throw new Error(`[OTA-SEC] Refusing non-HTTPS firmware download: ${  (url || '').substring(0, 80)}`);
    }
    // v9.0.40 TITAN: Wrap in circuit breaker for fault tolerance
    return this._breaker.exec(() => this._rawHttpGetBuffer(url));
  }

  async _rawHttpGetBuffer(url) {
    return new Promise((resolve, reject) => {
      const protocol = https;
      ...
      const req = protocol.get(url, (res) => {
        ...
        const len = Number(res.headers['content-length'] || 0);
        if (len > MAX_BYTES) {
          req.destroy(new Error(`OTA image too large (${len} bytes)`));
          return;
        }
        ...
        res.on('data', chunk => {
          received += chunk.length;
          if (received > MAX_BYTES) {
            req.destroy(new Error('OTA image exceeded 2MB cap'));
            return;
          }
          chunks.push(chunk);
        });
      ...
      req.setTimeout(60000, () => req.destroy(new Error('OTA download timeout (60s)')));
      req.on('error', reject);
    });
  }
