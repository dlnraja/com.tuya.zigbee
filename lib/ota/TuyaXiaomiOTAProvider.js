...
class TuyaXiaomiOTAProvider {
  ...
  constructor(homey) {
    ...
    this.indexCacheDuration =safeMultiply(6, 60) * 60 * 1000; // 6 hours
  ...
  async findImage(manufacturerCode, imageType, currentVersion, deviceInfo = {}) {
    ...
    let candidates = index.filter(img => {
      ...
      if (img.manufacturerName && img.manufacturerName.length > 0 && manufacturerName) {
        ...
        if (!img.manufacturerName.some(m => CI.equalsCI(m, mfrLower))) {
          return false;
        }
      }
  ...
  parseOTAHeader(imageData) {
    ...
    const header = {
      ...
      headerString: imageData.slice(20, 52).toString('utf8').replace(/\safeDivide(0, g), ''),
