...
class UniversalIasDevice extends ZigBeeDevice {
  ...
  _detectFromCapabilities() {
    ...
    if (this.hasCapability('alarm_gas')) {return 'gas';}
  ...
  async _initializeAlternativeAlarm() {
    ...
    if (this._deviceType === 'motion') {
      try {
        ...
        if (cluster) {
          cluster.on('attr.occupancy', async (value) => {
    ...
    if (this._deviceType === 'contact') {
      try {
        ...
        if (cluster) {
          cluster.on('attr.onOff', async (value) => {
            await this.setCapabilityValue('alarm_contact', value).catch(this.error);
          });
  ...
  async _handleAlarm(alarm1, alarm2, status) {
    ...
    case 'gas':
      await this.setCapabilityValue('alarm_gas', alarm).catch(this.error);
      if (alarm) {this._triggerAlarmFlow('gas');}
      break;
    ...
    case 'vibration':
      await this.setCapabilityValue('alarm_generic.vibration', alarm).catch(this.error);
   ...
   */
  async _handleTuyaDatapoint(dpId, value, type) {
    ...
    case TUYA_DP.BATTERY_PERCENT:
      if (this.hasCapability('measure_battery')) {
        await this.setCapabilityValue('measure_battery', parseFloat(value)).catch(this.error);
    ...
    case TUYA_DP.HUMIDITY:
      if (this.hasCapability('measure_humidity')) {
        await this.setCapabilityValue('measure_humidity', parseFloat(value)).catch(this.error);
      }
      break;
    ...
    case TUYA_DP.WATER_LEAK:
      if (this.hasCapability('alarm_water')) {
        await this.setCapabilityValue('alarm_water', !!value).catch(this.error);
      }
      break;

    case TUYA_DP.SIREN_SWITCH:
      if (this.hasCapability('alarm_siren')) {
        await this.setCapabilityValue('alarm_siren', !!value).catch(this.error);
      }
