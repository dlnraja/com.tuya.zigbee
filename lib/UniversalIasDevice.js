...
class UniversalIasDevice extends ZigBeeDevice {
  ...
  async _detectDeviceType() {
    ...
    this._manufacturer = CI.normalize(manufacturerName);
    this._modelId = modelId;
    this._isTuya = CI.startsWithCI(manufacturerName, '_tz') || CI.startsWithCI(manufacturerName, '_ty');
    this._isHeiman = CI.includesCI(manufacturerName, 'heiman');
    this._isSonoff = CI.includesCI(manufacturerName, 'sonoff') || CI.includesCI(manufacturerName, 'ewelink');
    this._isAqara = CI.includesCI(manufacturerName, 'lumi') || CI.includesCI(manufacturerName, 'aqara');
  ...
  _handleTuyaDatapoint(dpId, value, type) {
    ...
    case TUYA_DP.TEMPERATURE:
      if (this.hasCapability('measure_temperature')) {
        // Tuya temps are often in 0.1Â°C or 0.01Â°C
