 ...
 * 
 * SystÃ¨me de logging avancÃ© conforme Ã la documentation Homey SDK v3
 * - Niveaux de log: ERROR, WARN, INFO, DEBUG, TRACE
 * - Logs structurÃ©s avec contexte
 * - Support Zigbee (clusters, endpoints, attributes)
 * - Formatage colorÃ© pour lisibilitÃ©
 * - ContrÃ´le via settings
 */
...
class Logger {
  ...
  constructor(device) {
    ...
    // Niveau de log par dÃ©faut
    this.level = this._getLogLevel();
  }

  /**
   * RÃ©cupÃ¨re le niveau de log depuis les settings
   */
  _getLogLevel() {
    try {
  ...
  settingsChange(changedKeys, oldSettings, newSettings) {
    ...
    // Update log level si debug_logging a changÃ©
  ...
  /**
   * CrÃ©er un timer pour mesurer la performance
   */
  startTimer(label) {
  ...
  // ========== NOUVELLES MÃ‰THODES (SDK v3 Enhanced) ==========
