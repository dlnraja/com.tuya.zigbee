...
// v5.14.0: Per-device-type limits — battery devices get stricter caps
const TX_RX_LIMITS = {
  mains: { txPerMinute: 30, rxPerMinute: 120 },   // Mains-powered switches/plugs
  battery: { txPerMinute: 10, rxPerMinute: 30 },   // Battery sensors — conserve airtime
  router: { txPerMinute: 30, rxPerMinute: 120 },   // Zigbee routers (repeaters)
  // WHY(P2389): Tuya mmWave/presence radars firmware-spam ~150–200 msg/min (Z2M#14742).
  // Higher RX budget avoids false "flooding" Homey alerts while DP coalesce still protects CPU.
  radar: { txPerMinute: 20, rxPerMinute: 250 },
...
const THROTTLE_WINDOWS = {
  ...
  // WHY(P2740 / GH#550): was 5000 — light-off lux + walk-away distance felt laggy
  measure_luminance: 400,
  measure_battery: 60000,  // Battery only every minute
  measure_power: 2000,
  measure_voltage: 5000,
  measure_current: 5000,
  // WHY(P2389): clrdrnya/MTG distance DP spam — coalesce harder than generic sensors
  // WHY(P2740): ceiling gkfbdvyx needs snappy distance (was 5000)
  'measure_luminance.distance': 200,
...
// v9.0.77: Use plain setInterval (module-level, no device `this` available)
if (!globalState.cleanupInterval) {
  globalState.cleanupInterval = globalThis.setInterval(cleanup, 60000); // Every minute
...
/**
 * Get limits for a device type
 * @param {string} deviceType - 'mains', 'battery', 'router', or 'radar'
 * @returns {{ txPerMinute: number, rxPerMinute: number }}
 */
function getLimitsForType(deviceType) {
  return TX_RX_LIMITS[deviceType] || TX_RX_LIMITS.mains;
}

/**
 * WHY(P2389): mmWave/presence radar drivers are a known Tuya firmware flood class.
 * @param {string} driverId
 * @returns {boolean}
 */
function isRadarFloodDriver(driverId) {
  const id = String(driverId || '').toLowerCase();
  return /presence_sensor_radar|sensor_presence_radar|motion_sensor_radar|pir_mmwave|sensor_illuminance_presence/.test(id);
}

/**
 * WHY(P2401 / VicHY #2224–#2226): after app update, driver.id can lag while mmWave
 * already floods (~200 msg/min). Also match mfr / distance cap / floodCalm flag.
 * @param {{ driverId?: string, manufacturerName?: string, hasDistanceCap?: boolean, floodCalm?: boolean }} [hints]
 * @returns {boolean}
 */
function isRadarFloodContext(hints = {}) {
  if (isRadarFloodDriver(hints.driverId)) {return true;}
  if (hints.floodCalm === true) {return true;}
  if (hints.hasDistanceCap === true) {return true;}
  const mfr = String(hints.manufacturerName || '').toLowerCase();
  // Chatty Tuya mmWave families (Z2M#14742 / clrdrnya MTG235)
  return /clrdrnya|sbyx0lm6|gkfbdvyx|laokfqwu|qasjif9e|ijx3oktb|ztq6wn4r|sxm7l9ub/.test(mfr);
}

// v5.14.0: Per-device concurrency semaphore — limits concurrent TX commands
const _semaphoreState = new Map(); // deviceId -> { active: number, queue: Array }

/**
 * v5.14.0: Acquire a TX concurrency slot for a device (max 2 concurrent)
 * @param {string} deviceId
 * @returns {boolean} true if acquired, false if limit reached
 */
function acquireTxSemaphore(deviceId) {
  if (!_semaphoreState.has(deviceId)) {
    _semaphoreState.set(deviceId, { active: 0 });
  }
  const s = _semaphoreState.get(deviceId);
  if (s.active >= 2) {
    return false;
  }
  s.active++;
...
/**
 * v5.14.0: Release a TX concurrency slot for a device
 * @param {string} deviceId
 */
function releaseTxSemaphore(deviceId) {
  const s = _semaphoreState.get(deviceId);
  if (s && s.active > 0) {
    s.active--;
  }
...
/**
 * v5.14.0: Get current concurrency count for a device
 * @param {string} deviceId
 * @returns {number}
 */
function getTxConcurrency(deviceId) {
  const s = _semaphoreState.get(deviceId);
  return s ? s.active : 0;
}
 ...
 */
function trackTx(deviceId, deviceType) {
  const now = Date.now();
  const limits = getLimitsForType(deviceType);
  if (!globalState.txTracker.has(deviceId)) {
    globalState.txTracker.set(deviceId, { count: 0, windowStart: now, blocked: false, blockedUntil: 0, deviceType: deviceType || 'mains' });
  }
  const t = globalState.txTracker.get(deviceId);
  // Update deviceType if provided (may change after first detection)
  if (deviceType) { t.deviceType = deviceType; }
...
function trackRx(deviceId, deviceType) {
  const now = Date.now();
  const limits = getLimitsForType(deviceType);
  if (!globalState.rxTracker.has(deviceId)) {
    globalState.rxTracker.set(deviceId, { count: 0, windowStart: now, deviceType: deviceType || 'mains' });
  }
  const t = globalState.rxTracker.get(deviceId);
  if (deviceType) { t.deviceType = deviceType; }
  if (now - t.windowStart > 60000) { t.count = 0; t.windowStart = now; }
  t.count++;
  return { count: t.count, exceeded: t.count > limits.rxPerMinute };
}
...
function getStats() {
  return {
    ...
    semaphoreActiveCount: _semaphoreState.size,
  };
}
