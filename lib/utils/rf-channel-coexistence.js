...
/**
 * Homey-oriented protocol brief (silent enrich — engineering facts only).
 * WHY (P215): users confuse Zigbee/Wi-Fi/BT/Z-Wave roles; Contre quoi = wrong RF advice.
 * Does not invent device couples; does not recommend cloud Wi-Fi for battery sensors.
 */
function protocolSelectionBrief() {
  return Object.freeze({
    zigbee: Object.freeze({
      bandEu: '2.4 GHz',
      mesh: true,
      typicalIndoorRangeM: Object.freeze({ min: 10, max: 20 }),
      meshExtendsCoverage: true,
      approxThroughputKbps: 250,
      lowPower: true,
      hubRequired: true,
      coexistenceRisk: 'Shares 2.4 GHz with Wi-Fi/BT — plan channels (prefer Zigbee 15/20/25 vs Wi-Fi 1/6/11 @ 20 MHz)',
      bestFor: 'Local sensors, buttons, plugs, bulbs, Tuya Zigbee on Homey',
    }),
    zwave: Object.freeze({
      bandEu: '868 MHz',
      mesh: true,
      typicalIndoorRangeM: Object.freeze({ min: 30, max: 100 }),
      approxThroughputKbps: 100,
      lowPower: true,
      hubRequired: true,
      coexistenceRisk: 'Sub-GHz — less 2.4 GHz Wi-Fi interference; fewer SKUs / higher cost',
      bestFor: 'Locks / security when SKU exists; not this app’s transport',
    }),
    wifi: Object.freeze({
      bandEu: '2.4 / 5 GHz',
      mesh: false,
      lowPower: false,
      hubRequired: false,
      coexistenceRisk: 'Congests 2.4 GHz airtime; many cloud-first SKUs',
      bestFor: 'Powered high-bandwidth (cameras) or local-first firmware — avoid battery Wi-Fi',
    }),
    bluetooth: Object.freeze({
      ...
      lowPower: true,
      hubRequired: false,
      coexistenceRisk: 'Short range; proxies help; not a house-wide mesh',
      bestFor: 'Nearby sensors / remotes; not a Zigbee replacement',
    }),
    thread: Object.freeze({
      bandEu: '2.4 GHz (IEEE 802.15.4)',
      mesh: true,
      lowPower: true,
      hubRequired: true,
      coexistenceRisk: 'Same numbering family as Zigbee — Wi-Fi channel plan still matters',
      bestFor: 'Matter-over-Thread when border routers exist; parallel to Zigbee, not a drop-in',
    }),
    tips: Object.freeze([
      'Zigbee mesh nodes (mains routers) extend coverage beyond ~10–20 m single hop.',
      'EU Zigbee is typically 2.4 GHz; EU Z-Wave is typically 868 MHz — different RF problem spaces.',
      'Prefer Zigbee/Thread for local low-power mesh; prefer Wi-Fi for powered bandwidth; avoid battery Wi-Fi.',
      'Matter is an interoperability layer (often over Thread or Wi-Fi), not a radio by itself.',
      'Zigbee 4.0 (2025) + Suzi (sub-GHz EU ~800 MHz) extend Zigbee — do not replace Homey 2.4 GHz channel plan.',
      'Suzi certification is open (2026-09) but Homey Zigbee today is 2.4 GHz — dual-band bridge hardware required for sub-GHz.',
      'This Homey app targets Tuya Zigbee (ZCL + EF00) — protocol choice ≠ fingerprint invent.',
      'Intelligent path: classifyHomeyRadioCapability (P2539) soft-guides Suzi/GP/Matter; never invent sacred couples.',
    ]),
  });
}
