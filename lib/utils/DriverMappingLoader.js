...
const DP_PARSERS = {
  ...
  // Percentage parsers
  percent_255: (v) => Math.round(safeMultiply(safeParse(v, 255), 100)),
  percent_1000: (v) => Math.round(safeMultiply(safeParse(v, 1000), 100)),
  ...
  parseValue(parserName, value) {
    ...
    try {
      ...
      if (isNaN(v) && typeof value !== 'boolean' && op !== 'enum' && op !== 'boolean') {
        console.warn(`[DRIVER-MAPPING] ⚠️ NaN detected for parser ${parserName} with value:`, value);
