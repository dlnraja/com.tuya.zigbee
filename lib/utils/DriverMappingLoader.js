...
const DP_PARSERS = {
  ...
  /**
   * ⚠️ STRICTEMENT INTERDIT: utiliser des formules linéaires comme (voltage - 2.5) / 0.5
   * ✅ CORRECT: Utiliser UnifiedBatteryHandler.calculateFromVoltage() avec des profils non-linéaires
   * @see lib/battery/UnifiedBatteryHandler.js - calculateFromVoltage(voltage, batteryType, temperature)
   * @example UnifiedBatteryHandler.calculateFromVoltage(2.8, 'CR2450', 25)
   */
  battery_voltage_to_percent: (v) => {
    try {
      const voltage = v / 1000;
      // Déléguer à UnifiedBatteryHandler avec le profil non-linéaire CR2450
      const handler = new UnifiedBatteryHandler();
      const profile = handler.lookupBatteryProfile('_TZ3000_default', 'battery') || '3V_2400';
      return handler.calculateFromVoltage(voltage, profile, 25);
    } catch (err) {
      // Fallback sécurisé si UnifiedBatteryHandler n'est pas disponible
      return Math.min(100, Math.max(0, Math.round((v - 2500) / 10)));
    }
  },
...
function _mappingLoadPressure() {
  try {
    const Lazy = require('../performance/IntelligentLazyLoad');
    if (typeof Lazy.isMemoryPressure === 'function' && Lazy.isMemoryPressure()) return true;
  } catch { /* soft */ }
  try {
    const BootBudget = require('../performance/BootBudget');
    if (typeof BootBudget.isHeapCritical === 'function' && BootBudget.isHeapCritical()) return true;
  } catch { /* soft */ }
  return false;
}
...
class DriverMappingLoader {
  constructor(opts = {}) {
    this.database = null;
    this.loaded = false;
    /** @type {import('../battery/UnifiedBatteryHandler')} */
    this._unifiedBatteryHandler = null;
    // Logger par défaut — injecter via setLogger() pour Homey SDK3
    this._log = console.log.bind(console);
    this._error = console.error.bind(console);
    this._warn = console.warn.bind(console);
    // WHY(P2674): never JSON.parse driver-mapping (~0.4MB) while heap critical —
    // EF00 path during 60+ switch onNodeInit (diag 149bc1a5 SIGABRT JsonParse).
    if (opts.skipLoad || _mappingLoadPressure()) {
      this.database = { devices: {}, parsers: {}, driver_rules: {} };
      this.loaded = false;
      this._deferredDueToPressure = true;
      return;
    }
    this.loadDatabase();
  }
  ...
  /**
   * Injecter un logger Homey SDK3 compatible
   * @param {object} logger - { log, error, warn } functions
   */
  setLogger(logger) {
  ...
  /**
   * Injecter UnifiedBatteryHandler instance pour le parser batterie non-linéaire
   * @param {import('../battery/UnifiedBatteryHandler')} handler
   */
  setBatteryHandler(handler) {
    this._unifiedBatteryHandler = handler;
  }
  ...
  loadDatabase() {
    try {
      // WHY(P2674): soft-skip under pressure; callers may retry after boot
      if (_mappingLoadPressure()) {
        this.database = this.database || { devices: {}, parsers: {}, driver_rules: {} };
        this.loaded = false;
        this._deferredDueToPressure = true;
        this._log('[DRIVER-MAPPING] ⏭️ skip JSON load (heap/RSS pressure)');
        return;
      }

      const dbPath = path.join(__dirname, '../../data/driver-mapping-database.json');
      ...
      // GC trigger prior to database parsing if exposed
      if (typeof global.gc === 'function') {
        try { global.gc(); } catch (e) { /* best-effort GC - not critical */ }
      }

      // Optimization: Read as raw Buffer to save JS heap string allocations
      // WHY(P2586): shared Buffer parse + BootBudget GC under pressure
      let rawBuffer = fs.readFileSync(dbPath);
      try {
        const { parseJsonBuffer } = require('../performance/IntelligentLazyLoad');
        this.database = parseJsonBuffer(rawBuffer) || JSON.parse(rawBuffer);
      } catch {
        this.database = JSON.parse(rawBuffer);
      }
      rawBuffer = null; // Instantly release raw buffer memory reference

      // GC trigger post parsing to immediately release V8 parsing allocations
      try {
        const BootBudget = require('../performance/BootBudget');
        BootBudget.maybeGc();
      } catch (e) {
      ...
      this.database.devices = this.database.devices || this.database.drivers || {};
      this.database.parsers = this.database.parsers || {};
      this.database.driver_rules = this.database.driver_rules || {};
      this.loaded = true;
      this._deferredDueToPressure = false;

      this._log('[DRIVER-MAPPING] ✅ Database loaded successfully and GC-optimized');
      this._log(`[DRIVER-MAPPING]   Version: ${this.database.version}`);
      this._log(`[DRIVER-MAPPING]   Models: ${Object.keys(this.database.devices).length}`);
      
    } catch (err) {
      this._error('[DRIVER-MAPPING] ❌ Failed to load database under memory-optimized parser:', err.message);
      this.database = { devices: {}, parsers: {}, driver_rules: {} };
      if (typeof global.gc === 'function') {
        try { global.gc(); } catch (e) { /* best-effort GC - not critical */ }
      }
  ...
  /**
   * Retry deferred load once heap recovers (P2674).
   */
  ensureLoaded() {
    if (this.loaded) return true;
    if (_mappingLoadPressure()) return false;
    this.loadDatabase();
    return !!this.loaded;
  }
  ...
  getDeviceInfo(model, manufacturer) {
    ...
    // Legacy nested schema: devices[model].manufacturers[mfr]
    const modelData = this.database.devices?.[model];
    if (modelData?.manufacturers) {
      const manufacturerData = modelData.manufacturers[manufacturer]
        || this._lookupCI(modelData.manufacturers, manufacturer);
      if (manufacturerData) {
        return {
          model,
          manufacturer,
    ...
    // Current schema: mfr_index ∩ pid_index → drivers[driverId]
    const mfrDrivers = this._lookupIndexList(this.database.mfr_index, manufacturer);
    const pidDrivers = this._lookupIndexList(this.database.pid_index, model);
    let drivers = [];
    if (mfrDrivers.length && pidDrivers.length) {
      const pidSet = new Set(pidDrivers);
      drivers = mfrDrivers.filter((d) => pidSet.has(d));
    }
    if (!drivers.length) {
      drivers = mfrDrivers.length ? mfrDrivers : pidDrivers;
    }
    if (!drivers.length) {
      return null;
    }
    ...
    const driverMeta = this.database.drivers?.[driverId] || {};
    return {
      model,
      manufacturer,
      ...
      dps: driverMeta.dps || driverMeta.dpMappings || {},
      endpoints: driverMeta.endpoints || {},
      battery: driverMeta.battery || null,
      polling: driverMeta.polling || { initial: '3s', interval: '5m' },
      type: driverMeta.class || driverMeta.type || null,
      cluster: driverMeta.cluster || null,
      description: typeof driverMeta.name === 'string' ? driverMeta.name : driverMeta.name?.en,
      alternateDrivers: drivers.slice(1),
    };
  }
  ...
  _lookupIndexList(index, key) {
    const hit = this._lookupCI(index, key);
  ...
  getManufacturersForModel(model) {
    ...
    const modelData = this.database.devices?.[model];
    if (modelData?.manufacturers) {
      return Object.keys(modelData.manufacturers);
    }

    // Invert pid_index → collect mfrs whose driver lists intersect pid drivers
    const pidDrivers = new Set(this._lookupIndexList(this.database.pid_index, model));
    if (!pidDrivers.size || !this.database.mfr_index) {return [];}
    const out = [];
    for (const [mfr, drivers] of Object.entries(this.database.mfr_index)) {
      const list = Array.isArray(drivers) ? drivers : [drivers];
      if (list.some((d) => pidDrivers.has(d))) {out.push(mfr);}
    }
    return out;
  }
  ...
  parseValue(parserName, value) {
    ...
    if (!parser) {
      this._warn(`[DRIVER-MAPPING] Unknown parser: ${parserName}, returning raw value`);
      return value;
    }

    try {
      // v5.11.16 SEC: Safe parser dispatch (replaces eval — code injection risk)
      const v = typeof value === 'number' ? value : Number(value);
      const op = parser.operation || parserName;
      if (op.startsWith('divide_by_'))   {return v / Number(op.replace('divide_by_', ''));}
      if (op.startsWith('multiply_by_')) {return v * Number(op.replace('multiply_by_', ''));}
      if (op === 'boolean')  {return !!value;}
      if (op === 'invert')   {return !value;}
      if (op === 'negate')   {return -v;}
      if (op === 'round')    {return Math.round(v);}
      if (op === 'celsius_offset') {return v - (parser.offset || 0);}
      if (op === 'clamp')    {return Math.max(parser.min ?? 0, Math.min(parser.max ?? 100, v));}
      if (op === 'enum' && parser.map) {return parser.map[String(value)] ?? value;}
      this._warn(`[DRIVER-MAPPING] Unknown safe parser op: ${op}`);
      return value;
    } catch (err) {
      this._error(`[DRIVER-MAPPING] Parser ${parserName} failed:`, err.message);
      return value;
    }
  }
  ...
  searchDevices(query) {
    const results = [];
    const lowerQuery = TU.normalize(query);
    ...
    for (const [model, modelData] of Object.entries(this.database.devices)) {
      ...
      for (const [manufacturer, device] of Object.entries(manufacturers)) {
        const name = TU.normalize(device.name || '');
        const desc = TU.normalize(modelData.description || '');

        if (name.includes(lowerQuery) || desc.includes(lowerQuery) ||
            TU.normalize(model).includes(lowerQuery) ||
            TU.normalize(manufacturer).includes(lowerQuery)) {
          results.push({
            model,
            manufacturer,
            name: device.name,
  ...
  identifyManufacturer(manufacturerName) {
    ...
    for (const prefix of MANUFACTURER_PREFIXES.tuya) {
      if (TU.normalize(manufacturerName).startsWith(TU.normalize(prefix))) {
    ...
    for (const [brand, prefixes] of Object.entries(MANUFACTURER_PREFIXES.brands)) {
      for (const prefix of prefixes) {
        if (TU.normalize(manufacturerName).includes(TU.normalize(prefix))) {
  ...
  parseWithBuiltinParser(parserName, value) {
    ...
    try {
      ...
      this._error(`[DRIVER-MAPPING] Built-in parser ${parserName} failed:`, err.message);
      return value;
    }
  }
