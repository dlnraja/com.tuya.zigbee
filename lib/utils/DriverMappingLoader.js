...
class DriverMappingLoader {
  ...
  loadDatabase() {
    try {
      ...
      // WHY(P2586/P2678): shared Buffer parse — never utf8 string intermediate
      const Lazy = require('../performance/IntelligentLazyLoad');
      this.database = Lazy.loadJsonBuffer(dbPath, {
        gc: true,
      ...
      if (!this.database) {
        ...
        try {
          this.database = Lazy.parseJsonBuffer(rawBuffer) || JSON.parse(rawBuffer);
        } finally {
          rawBuffer = null;
        }
      }
