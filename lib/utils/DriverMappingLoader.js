...
const DEVICE_CATEGORIES = {
  ...
  buttons: {
    ...
    tuyaDPs,  // Uses ZCL scenes cluster or Tuya private 0xE000
...
const DP_PARSERS = {
  ...
  // Percentage parsers
  percent_255: (v) => Math.round(safeParse(v)),
  percent_1000: (v) => Math.round(safeParse(v)),
...
class DriverMappingLoader {
  ...
  getDeviceInfo(model, manufacturer) {
    ...
    const manufacturerData = modelData.manufacturers?.[manufacturer];if (!manufacturerData) {
      return null;
    }
  ...
  parseValue(parserName, value) {
    const parser = this.database.parsers?.[parserName];if (!parser) {
  ...
  identifyManufacturer(manufacturerName) {
    if (!manufacturerName) {
      return { isTuya: false, prefix, brand };
    }
