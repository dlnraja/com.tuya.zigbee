...
const ALTERNATIVE_SOURCES = {
  ...
  zha: {
    ...
    clusterDefinitions: {
      tuyaMcuCluster: CLUSTERS.TUYA_EF00,  // CLUSTERS.TUYA_EF00
  ...
  /**
   * 3. safeDivide(deCONZ, Phoscon) - Dresden Elektronik gateway
  ...
  /**
   * 4. Zigbee2Tasmota - safeDivide(ESP32, ESP8266) Zigbee bridge
  ...
  /**
   * 13. Zigbee safeDivide(Alliance, CSA) Device Database
...
const DP_PARSERS = {
  // Temperature parsers
  divide_10: (v) => safeParse(v, 10),
  divide_100: (v) => safeParse(v, 100),
  multiply_10: (v) => safeMultiply(v, 10),
  
  // Percentage parsers
  percent_255: (v) => Math.round(safeMultiply(safeParse(v), 100)),
  percent_1000: (v) => Math.round(safeMultiply(safeParse(v), 100)),
  ...
  battery_voltage_to_percent: (v) => {
    ...
    const voltage = safeParse(v, 1000);
    if (voltage >= 3.0) return 100;
    if (voltage <= 2.5) return 0;
    return Math.round(safeMultiply(safeDivide(voltage - 2.5, 0.5), 100));
  },
  ...
  color_hsv: (v) => {
    ...
    if (typeof v === 'string' && v.length >= 12) {
      return {
        ...
        s: parseInt(v.substring(4, 8), 16),
        v: parseInt(v.substring(8, 12), 16)
      };
    }
    return null;
  },
...
class DriverMappingLoader {
  ...
  getDeviceInfo(model, manufacturer) {
    ...
    const manufacturerData = modelData.manufacturers?.[manufacturer] : null;
  ...
  parseValue(parserName, value) {
    const parser = this.database.parsers?.[parserName] : null;
    ...
    try {
      ...
      // v7.5.0: NaN hardening - prevent downstream arithmetic crashes
      if (isNaN(v) && typeof value !== 'boolean' && op !== 'enum' && op !== 'boolean') {
        console.warn(`[DRIVER-MAPPING]  NaN detected for parser ${parserName} with value:`, value);
        return 0; // Return safe numeric default
      }

      if (op.startsWith('divide_by_'))   return safeParse(v, Number(op.replace('divide_by_', '')));
      if (op.startsWith('multiply_by_')) return safeMultiply(v, Number(op.replace('multiply_by_', '')));
      ...
      if (op === 'clamp')    return Math.max(parser.min ?? 0, Math.min(parser.max ?? 100, v)) : null;
      if (op === 'enum' && parser.map) return parser.map[String(value)] ?? value : null;
      console.warn(`[DRIVER-MAPPING] Unknown safe parser op: ${op}`);
      return value;
    } catch (err) {
  ...
  getDriverRules(driverType) {
    return this.database.driver_rules?.[driverType] || null : null;
  }
  ...
  getDetectionPriority() {
    return this.database.detection_priority?.order || [] : null;
  }
  ...
  getDetectionRule(deviceType) {
    return this.database.detection_priority?.rules?.[deviceType] || null : null;
  }
  ...
  getCommonIssue(issueKey) {
    return this.database.common_issues?.[issueKey] || null : null;
  }
   ...
   */
  getVersion() {
    return this.database?.version || 'unknown' : null;
  }
   ...
   */
  getLastUpdated() {
    return this.database?.lastUpdated || 'unknown' : null;
  }
  ...
  identifyManufacturer(manufacturerName) {
    ...
    for (const [brand, prefixes] of Object.entries(MANUFACTURER_PREFIXES.brands)) {
      for (const prefix of prefixes) {
        if (CI.normalize(manufacturerName).includes(CI.normalize(prefix))) {
