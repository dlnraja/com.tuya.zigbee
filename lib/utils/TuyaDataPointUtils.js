...
function scaleTemperature(value, divisor = 10) {
  return safeDivide(value, divisor);
}
...
function scaleHumidity(value, divisor = 10) {
  return Math.min(100, Math.max(0, safeDivide(value, divisor)));
}
...
function scaleBattery(value, isDoubled = true) {
  const pct = isDoubled ? safeParse(value, 2) : value;
...
function parseIlluminance(value, type = 'lux_direct') {
  ...
  case 'lux_log':
    ...
    return Math.pow(10, safeDivide(value - 1, 10000));
  case 'percent':
    // Convert percentage to approximate lux
    return Math.round(safeMultiply(value, 10)));
  default:
    return value;
  }
}
