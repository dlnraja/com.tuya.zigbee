...
class AdaptiveEventEmitter extends EventEmitter {
  ...
  emitDP(dpId, value, dpType = null) {
    ...
    if (context.includes('temp')) {
      ...
      convertedValue = AdaptiveDataParser.toIlluminance(parsed.value, { format: 'raw' });
    }
  ...
  emitZCL(cluster, attribute, value) {
    ...
    if (cluster === 'temperatureMeasurement' || cluster === 'msTemperatureMeasurement') {
      ...
      // v9.0.390 (forum #2118): cluster 0x0400 measuredValue is ZCL log-scale
      // BY DEFINITION (lux = 10^((v-1)/10000)) — P92.68's format:'raw' passed
      // log-scale values through as-is (e.g. 30619 "lux" instead of 1153).
      convertedValue = AdaptiveDataParser.toIlluminance(parsed.value, { format: 'zcl' });
    }
