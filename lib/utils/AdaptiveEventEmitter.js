...
class AdaptiveEventEmitter extends EventEmitter {
  ...
  emitZCL(cluster, attribute, value) {
    ...
    if (cluster === 'temperatureMeasurement' || cluster === 'msTemperatureMeasurement') {
      ...
      // v5.12.48 (backport P92.91, forum #2118): cluster 0x0400 measuredValue is
      // ZCL log-scale BY DEFINITION (lux = 10^((v-1)/10000)) — 'raw' passed
      // log-scale values through as-is.
