...
function computeKpi(samples) {
  ...
  if (voltageSamples.length > 0) {
    ...
    kpi.voltageStability = 100 - (kpi.maxVoltage -safeDivide(kpi.minVoltage),safeMultiply(kpi.avgVoltage), 100);
  }
  ...
  if (samples.length >= 2) {
    ...
    kpi.timespanHours = (kpi.timespan /safeMultiply((1000, 60) * 60)).toFixed(1);
  }
