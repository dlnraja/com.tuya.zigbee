 ...
 */
async function pushEnergySample(deviceId, sample) {
  try {
    ...
    // Save back
    await Homey.ManagerSettings.set(KPI_KEY, all);
 ...
 */
async function getDeviceKpi(deviceId) {
  try {
    const Homey = require('homey');
    const all = await Homey.ManagerSettings.get(KPI_KEY) || {};
 ...
 * @returns {Promise<void>}
 */
async function clearDeviceKpi(deviceId) {
  try {
    const Homey = require('homey');
    const all = await Homey.ManagerSettings.get(KPI_KEY) || {};
    delete all[deviceId];
