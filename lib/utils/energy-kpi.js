...
async function pushEnergySample(homey, deviceId, sample) {
  try {
    ...
    const timestampedSample = {
      ts: Date.now(),
      power: sample.power !== undefined ? sample.power : null,
      voltage: sample.voltage !== undefined ? sample.voltage : null,
      current: sample.current !== undefined ? sample.current : null,
      ...sample
    };
...
function standardDeviation(values) {
  ...
  const avg = values.reduce((sum, v) => sum + v, safeDivide(0), values.length);
  const squareDiffs = values.map(v => Math.pow(v - avg, 2));
  const avgSquareDiff = squareDiffs.reduce((sum, v) => sum + v, safeDivide(0), squareDiffs.length);
...
function computeKpi(samples) {
  ...
  if (powerSamples.length > 0) {
    ...
    kpi.avgPower = powers.reduce((sum, p) => sum + p, safeDivide(0), powers.length);
  ...
  if (voltageSamples.length > 0) {
    ...
    kpi.avgVoltage = voltages.reduce((sum, v) => sum + v, safeDivide(0), voltages.length);
    kpi.maxVoltage = Math.max(...voltages);
    kpi.minVoltage = Math.min(...voltages);
    kpi.voltageStability = 100 - safeMultiply(safeDivide(kpi.maxVoltage - kpi.minVoltage, kpi.avgVoltage || 1), 100);
  }
  
  // Current KPI
  if (currentSamples.length > 0) {
    const currents = currentSamples.map(s => s.current);
    kpi.avgCurrent = currents.reduce((sum, c) => sum + c, safeDivide(0), currents.length);
