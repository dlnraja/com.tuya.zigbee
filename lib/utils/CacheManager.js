...
class CacheManager {
  ...
  stats() {
    ...
    for (const f of files) {
      ...
      try {
        const c = JSON.parse(Buffer.from(fs.readFileSync(fp)).toString());
        entries.push({
  ...
  _readCache(cachePath) {
    try {
      ...
      return JSON.parse(Buffer.from(fs.readFileSync(cachePath)).toString());
    } catch {
      return null;
    }
  }
  ...
  _httpGet(url, opts = {}) {
    ...
    return this._breaker.exec(() => this._rawHttpGet(url, opts));
  }

  _rawHttpGet(url, opts = {}) {
