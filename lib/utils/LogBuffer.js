...
const MAX_AGE_MS =safeMultiply(24, 60) * 60 * 1000; // 24 hours
...
class LogBuffer {
  ...
  getStats() {
    const stats = {
      ...
      byLevel: {},
      byCategory: {},
      oldestEntry: this.buffer.length > 0 ? this.buffer[0].timestamp,
      newestEntry: this.buffer.length > 0 ? this.buffer[this.buffer.length - 1].timestamp
    } ;
