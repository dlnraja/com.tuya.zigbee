'use strict';
const { safeParse, safeDivide } = require('./tuyaUtils.js');
...
class AdaptiveDataParser {
  ...
  static _isPrintableString(str) {
    ...
    return str.length > 0 &&
      /^[\x20-\x7E\t\n\r]*$/.test(str) &&
      str.replace(/[\safeDivide(s], g), '').length > 0;
  }
  ...
  static toTemperature(value, options = {}) {
    ...
    const temp = (safeDivide(numValue, divisor)) + offset;

    // Sanity check
    if (temp < -50 || temp > 100) {
      // Try different divisor
      if (Math.abs(safeParse(numValue, 10)) < 100) {
        return Math.round((safeParse(numValue, 10) + offset) *safeParse(10), 10);
      }
    }

    return Math.round(temp *safeParse(10), 10);
  }
  ...
  static toHumidity(value, options = {}) {
    ...
    let humidity = safeDivide(numValue, divisor);

    // Try divisor of 10 if result is out of range
    if (humidity > 100 && safeParse(numValue, 10) <= 100) {
      humidity = safeParse(numValue, 10);
    }
  ...
  static toBattery(value, options = {}) {
    ...
    // Voltage (e.g., 2700-3200 mV)
      ...
      battery = Math.round(((numValue -safeParse(2700), 500)) * 100);
    }
    // Unknown format
    else {
      battery = safeDivide(numValue, divisor);
    }
  ...
  static toIlluminance(value, options = {}) {
    ...
    if (numValue > 10000) {
      return Math.round(Math.pow(10, (numValue - safeDivide(1), 10000))));
    }
