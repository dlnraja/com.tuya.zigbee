...
class AdaptiveDataParser {
  ...
  static _parseTuyaDPBuffer(buffer) {
    ...
    while (offset < buffer.length - 3) {
      ...
      case 0x02: // Value (number)
        if (length === 4) {value = valueBuffer.readInt32BE(0);}
        else if (length === 2) {value = valueBuffer.readInt16BE(0);}
  ...
  static toTemperature(value, options = {}) {
    // P110: Tuya temp DPs are typically ×10; ÷100 was a double-scale trap when
    // SmartDivisor skips and this fallback runs. Sanity block below still
    // recovers alternate scales.
    const { divisor = 10, offset = 0, signed = true } = options;
    ...
    if (temp < -50 || temp > 100) {
      ...
      // v9.0.369: aucun diviseur ne donne une température plausible — rejeter
      // plutôt que retourner une valeur impossible (ex. 215°C)
      return null;
    }
   ...
   * v9.0.369: magnitude-based heuristic — the old default (÷100 for
   * everything) misread both common encodings: 65 (65% direct) → 1%
   * and 650 (65% ×10) → 7%. Now: ≤100 = percentage, ≤1000 = ×10,
   * else ×100. Explicit options.divisor still wins.
   */
  static toHumidity(value, options = {}) {
    const { divisor = null, min = 0, max = 100 } = options;
    ...
    let humidity;
    if (divisor) {
      humidity = numValue / divisor;
    } else if (numValue <= 100) {
      humidity = numValue;           // déjà un pourcentage (cas le plus courant)
    } else if (numValue <= 1000) {
      humidity = numValue / 10;      // échelle ×10 (ex. 650 = 65.0%)
    } else {
      humidity = numValue / 100;     // échelle ×100 (ex. 6500 = 65%)
    }
  ...
  static toBattery(value, options = {}) {
    ...
    // Voltage (e.g., 2500-3200 mV) — non-linear CR2032 discharge curve
    else if (numValue > 2000 && numValue < 4000) {
      // CR2032 lookup table (mV → %) — linear interpolation between points
      const VBAT_CURVE = [
        [3000, 100], [2950, 90], [2900, 75], [2850, 55],
        [2800, 40], [2750, 25], [2700, 15], [2600, 5], [2500, 0],
      ];
      const mv = Math.round(numValue);
      if (mv >= VBAT_CURVE[0][0]) {battery = 100;}
      else if (mv <= VBAT_CURVE[VBAT_CURVE.length - 1][0]) {battery = 0;}
      else {
        for (let i = 0; i < VBAT_CURVE.length - 1; i++) {
          if (mv >= VBAT_CURVE[i + 1][0] && mv <= VBAT_CURVE[i][0]) {
            const ratio = (mv - VBAT_CURVE[i + 1][0]) / (VBAT_CURVE[i][0] - VBAT_CURVE[i + 1][0]);
            battery = Math.round(VBAT_CURVE[i + 1][1] + ratio * (VBAT_CURVE[i][1] - VBAT_CURVE[i + 1][1]));
            break;
          }
        }
      }
   ...
   * v10.7.0: explicit format option — the magnitude auto-detect (>10000 →
   * ZCL log format) corrupted REAL daylight readings (a genuine 20,000 lux
   * DP report became ~100 lux). Callers that KNOW the source must pass
   * format:'zcl' (ZCL 0x0400 log scale) or format:'raw' (Tuya DP lux).
   * Legacy magnitude heuristic is kept only as the last-resort default.
   */
  static toIlluminance(value, options = {}) {
    const numValue = this._toNumber(value);
    if (numValue === null) {return null;}

    if (options.format === 'zcl') {
    ...
    // Legacy fallback (no format hint): ZCL log only for values that cannot
    // plausibly be raw lux (> 65535 is impossible raw; >10000 stays heuristic
    // for backward compatibility but caps the converted output sanity).
    if (numValue > 65535) {
      return Math.round(Math.pow(10, (numValue - 1) / 10000));
    }
  ...
  /**
   * Convert raw value to voltage (V)
   * Smart divisor detects if value is mV, deciVolts, or Volts
   */
  static toVoltage(value, options = {}) {
    const numValue = this._toNumber(value);
    if (numValue === null) {return null;}

    // Voltage should typically be 100-260V for mains, or 2-4V for battery.
    // If value is huge (e.g. 230000), it's mV (divide by 1000).
    // If value is 2300, it's deciVolts (divide by 10).
    if (numValue > 10000) {return Math.round((numValue / 100) * 10) / 10;} // e.g. 23000 -> 230V
    if (numValue > 1000 && numValue < 3000) {return Math.round((numValue / 10) * 10) / 10;} // 2300 -> 230.0V
    return numValue; // already V
  }

  /**
   * Convert raw value to power (W)
   * Smart divisor detects mW vs W
   */
  static toPower(value, options = {}) {
    const numValue = this._toNumber(value);
    if (numValue === null) {return null;}

    // Power typically 0-5000W
    if (numValue > 500000) {return Math.round((numValue / 100) * 10) / 10;}
    if (numValue > 50000) {return Math.round((numValue / 10) * 10) / 10;}
    return numValue;
  }

  /**
   * Convert raw value to current (A)
   * Smart divisor detects mA vs A
   */
  static toCurrent(value, options = {}) {
    const numValue = this._toNumber(value);
    if (numValue === null) {return null;}

    // Current typically 0-16A
    if (numValue > 16000) {return Math.round((numValue / 10000) * 100) / 100;} // 10000x multiplier
    if (numValue > 160 && numValue <= 16000) {return Math.round((numValue / 1000) * 100) / 100;} // mA
    return numValue;
  }
}
