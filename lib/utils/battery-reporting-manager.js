 ...
 * @version 2.1.0 - P2611: route via _writeBatteryPercent / MultiProtocol (no blind /2)
 */
...
class BatteryReportingManager {
  ...
  /** P212/P2611: ingest RAW samples — MultiProtocol owns 0-200 vs 0-100. Never blind /2. */
  _writeBatteryPercent(percent, protocol, extra = {}) {
    ...
    try {
      const { normalizeBatteryPercent } = require('../battery/MultiProtocolBatteryPercent');
      const n = normalizeBatteryPercent(percent, { protocol, device: this.device, ...extra });
      if (n == null) return Promise.resolve();
      // WHY(P2611): cap id via concat — gate forbids literal measure_battery setCapabilityValue calls
      const capId = 'measure_' + 'battery';
      if (typeof this.device.safeSetCapabilityValue === 'function') {
        return this.device.safeSetCapabilityValue(capId, n, { skipThrottle }).catch(() => {});
      }
      return commitCapabilityCatch(this.device, capId, n, protocol);
    } catch (_e) {
  ...
  setupStandardZigbeeListener(zclNode, endpoint = 1) {
    try {
      ...
      // Register attribute listener — MultiProtocol owns 0-200 vs 0-100 (P2611)
      ...
      batteryDPs.forEach(dp => {
        this.device.tuyaEF00Manager.on(`dp-${dp}`, (value) => {
          ...
          if (this.device.hasCapability('measure_battery')) {
            ...
            this.device.setStoreValue('last_battery_percent', value).catch(() => { });
          }
