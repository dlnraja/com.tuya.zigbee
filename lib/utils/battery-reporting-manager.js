 ...
 * @version 2.1.0 - P212: route writes via commitCapability / MultiProtocol (no L14 bypass)
 */
...
class BatteryReportingManager {
  ...
  /** P212/P218: ingest RAW samples — MultiProtocol owns 0-200 vs 0-100. */
  _writeBatteryPercent(percent, protocol, extra = {}) {
    if (!this.device.hasCapability('measure_battery')) {return Promise.resolve();}
    // WHY(P2470): empty/bogus attr reports (raw undefined) must not wipe UI
    if (percent === null || percent === undefined || percent === '') {return Promise.resolve();}
    const skipThrottle = (() => {
      try {
        const cur = this.device.getCapabilityValue?.('measure_battery');
        return cur == null || cur === undefined;
      } catch (_e) { return true; }
    })();
    if (typeof this.device.ingestBatteryPercent === 'function') {
      return this.device.ingestBatteryPercent(percent, { protocol, skipThrottle, ...extra }).catch(() => {});
    }
    const pct = parseFloat(percent);
    if (!Number.isFinite(pct)) {return Promise.resolve();}
    if (skipThrottle && typeof this.device.safeSetCapabilityValue === 'function') {
      return this.device.safeSetCapabilityValue('measure_battery', pct, { skipThrottle: true }).catch(() => {});
    }
    return commitCapabilityCatch(this.device, 'measure_battery', pct, protocol);
  }
  ...
  async configureStandardZigbee(zclNode, endpoint = 1) {
    try {
      ...
      // P2689 / Z2M: adaptive minChange by SOC + chemistry (coin vs accu)
      let reportCfg = { minInterval: 3600, maxInterval: 43200, minChange: 5 };
      try {
        const { buildAdaptiveReportingConfig } = require('../battery/SmartBatteryAdaptivePrecision');
        const chem = this.device.getEnergy?.()?.batteries?.[0]
          || this.device.getStoreValue?.('battery_type')
          || 'CR2032';
        const pct = this.device.getCapabilityValue?.('measure_battery');
        reportCfg = buildAdaptiveReportingConfig({ chemistry: chem, percent: pct });
      } catch (_e) { /* soft */ }
      ...
      try {
        await powerCfgCluster.configureReporting({
          batteryPercentageRemaining: {
            minInterval: reportCfg.minInterval,
            maxInterval: reportCfg.maxInterval,
            minChange: reportCfg.minChange,
          },
        });
      } catch (configErr) {
  ...
  setupStandardZigbeeListener(zclNode, endpoint = 1) {
    try {
      ...
      powerCfgCluster.on('attr.batteryPercentageRemaining', (value) => {
        this.device.log(`[BATTERY-REPORTING] 📊 Zigbee report raw=${value}`);
        if (this.device.hasCapability('measure_battery')) {
          this._writeBatteryPercent(value, 'zcl');
        }
    ...
    try {
      ...
      batteryDPs.forEach(dp => {
        this.device.tuyaEF00Manager.on(`dp-${dp}`, (value) => {
          ...
          if (this.device.hasCapability('measure_battery')) {
            this._writeBatteryPercent(value, 'tuya-dp', { dp });
          }
          this.device.setStoreValue('battery_last_update', Date.now()).catch(() => { });
        });
      });
  ...
  async readInitial(zclNode, endpoint = 1) {
    try {
      ...
      // Read battery percentage attribute — skip proactive poll on sleepy remotes (0x0001 chatter)
      try {
        const { shouldProactivePowerCfgRead } = require('../zigbee/PowerClusterPolicy');
        if (!shouldProactivePowerCfgRead(this.device)) {
          this.device.log('[BATTERY-REPORTING] ℹ️ skip 0x0001 poll on sleepy remote — wait for reports');
          return null;
        }
      ...
      if (battery && battery.batteryPercentageRemaining !== undefined) {
        this.device.log(`[BATTERY-REPORTING] 📖 Initial read raw=${battery.batteryPercentageRemaining}`);
        if (this.device.hasCapability('measure_battery')) {
          await this._writeBatteryPercent(battery.batteryPercentageRemaining, 'zcl');
        }
        await this.device.setStoreValue('battery_last_update', Date.now()).catch(() => { });
        return battery.batteryPercentageRemaining;
      }
