...
class BatteryReportingManager {
  ...
  detectDeviceType(zclNode, endpoint = 1) {
    try {
      // First check if device is mains-powered (no battery needed)
      const { isMainsPowered, isTuyaDPDevice } = require('../helpers/DeviceDataHelper');

      if (isMainsPowered(this.device)) {
        this.deviceType = 'mains_powered';
        this.isTuyaDevice = false;
        this.device.log('[BATTERY-REPORTING] ⚡ Mains-powered device - no battery reporting needed');
        return;
      }
  ...
  async configure(zclNode, endpoint = 1) {
    try {
      ...
      if (this.deviceType === 'mains_powered') {
        this.device.log('[BATTERY-REPORTING] ⚡ Skipping battery config for mains-powered device');
