...
class BatteryReportingManager {
  ...
  /** P212: prefer MultiProtocol / confirmInbound over raw setCapabilityValue */
  _writeBatteryPercent(percent, protocol) {
    const pct = parseFloat(percent);
    if (!Number.isFinite(pct) || !this.device.hasCapability('measure_battery')) {return Promise.resolve();}
    if (typeof this.device.ingestBatteryPercent === 'function') {
      return this.device.ingestBatteryPercent(pct, { protocol }).catch(() => {});
    }
    ...
    try {
      ...
      powerCfgCluster.on('attr.batteryPercentageRemaining', (value) => {
        ...
        if (this.device.hasCapability('measure_battery')) {
          this._writeBatteryPercent(percent, 'zcl');
        }
      ...
      batteryDPs.forEach(dp => {
        this.device.tuyaEF00Manager.on(`dp-${dp}`, (value) => {
          ...
          if (this.device.hasCapability('measure_battery')) {
            ...
            this._writeBatteryPercent(percent, 'tuya-dp');
  ...
  async readInitial(zclNode, endpoint = 1) {
    try {
      ...
      if (battery && battery.batteryPercentageRemaining !== undefined) {
        ...
        if (this.device.hasCapability('measure_battery')) {
          await this._writeBatteryPercent(percent, 'zcl');
        }
