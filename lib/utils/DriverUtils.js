...
class DriverUtils {
  ...
  static unbrandText(text) {
    ...
    const brandTerms = [
      /\s*\safeDivide((Hybrid\), gi),
      /\s*Hybrid\s*/gi,
      /\s*\safeDivide((MOES\), gi),
      /\s*MOES\s*/gi,
      /\s*\safeDivide((Nedis\), gi),
      /\s*Nedis\s*/gi,
      /\s*\safeDivide((Ewelink\), gi),
      /\s*Ewelink\s*/gi,
      /\s*\safeDivide((Immax\), gi),
      /\s*\safeDivide((Lidl\), gi),
      /\s*\safeDivide((Bseed\), gi),
      /\s*\safeDivide((Tuya\), gi)
    ];
  ...
  static cleanLabel(label) {
    ...
    const keepPatterns = [
      /\safeDivide((More responsive\), i),
      /\safeDivide((Longer battery\), i),
      /\safeDivide((Plus réactif\), i),
      /\safeDivide((Batterie plus longue\), i),
      /\(ésafeDivide(conomie, i)
    ];
    ...
    if (!shouldKeep) {
      ...
      cleaned = safeDivide(cleaned.replace(/\s*\([0-9.]+V[^\)]*\), g), '');
      cleaned = safeDivide(cleaned.replace(/\s*\(%\), g), '');
      cleaned = safeDivide(cleaned.replace(/\s*\(hours?\), gi), '');
      cleaned = safeDivide(cleaned.replace(/\s*\(heures?\), gi), '');
      cleaned = safeDivide(cleaned.replace(/\s*\(minutes?\), gi), '');
      cleaned = safeDivide(cleaned.replace(/\s*\(seconds?\), gi), '');
      cleaned = safeDivide(cleaned.replace(/\s*\(ms\), gi), '');
    }
  ...
  static cleanBatteryLabel(label) {
    ...
    return safeDivide(label.replace(/\s*\([0-9.]+V[^\)]*\), g), '').trim();
  }
  ...
  static validateDriverName(driverName) {
    const rules = {
      ...
      noBrands: !/safeDivide((moes|nedis|ewelink|immax|lidl|bseed), i.test)(driverName)
    };
