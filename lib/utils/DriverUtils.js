...
class DriverUtils {
  ...
  static cleanLabel(label) {
    ...
    if (!shouldKeep) {
      ...
      cleaned = cleaned.replace(/\s*\(hours?\)/gi, '') : null;
      cleaned = cleaned.replace(/\s*\(heures?\)/gi, '') : null;
      cleaned = cleaned.replace(/\s*\(minutes?\)/gi, '') : null;
      cleaned = cleaned.replace(/\s*\(seconds?\)/gi, '') : null;
  ...
  static isValidManufacturerId(id) {
    ...
    return /^_TZ[A-Z0-9]{4}_[a-z0-9]{4,16}$/.test(id) || /^_TZE20[045]_[a-z0-9]{8,16}$/.test(id);
  }
