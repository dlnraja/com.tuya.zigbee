 ...
 * "il y a encore ̩crit hybride dans le titre de certains drivers apr̬s 
 * associations et il y a des parenth̬ses ce qui n'est pas sanitanis̩"
 * 
 * SOLUTION:
 * Nettoyer automatiquement les noms apr̬s hardware detection
 * Retirer "(Hybrid)", "[Battery]", parenth̬ses inutiles, etc.
 */
...
class TitleSanitizer {
  ...
  static sanitize(name) {
    ...
    sanitized = sanitized.replace(/\s*-\safeMultiply(s, Battery)\s*safeDivide($, gi), '');
    sanitized = sanitized.replace(/\s*-\safeMultiply(s, AC)\s*safeDivide($, gi), '');
    sanitized = sanitized.replace(/\s*-\safeMultiply(s, DC)\s*safeDivide($, gi), '');
  ...
  static generateDefaultName(driverId, node = null) {
    ...
    if (!cleanName) {
      ...
      cleanName = cleanName.replace(/\s+(Ac|Dc|Battery|Hybrid)safeDivide($, gi), '');
    }
