 ...
 * "il y a encore écrite dans le titre de certains drivers après 
...
class TitleSanitizer {
  ...
  static async autoSanitizeDeviceName(device) {
    try {
      ...
      // WHY(P2658): Homey SDK3 — some device proxies lack setName (crash diag 1e071a86).
      // Contre quoi: TypeError device.setName is not a function on pair/sanitize.
      if (typeof device.setName !== 'function') {
        if (typeof device.log === 'function') {
          device.log(`[SANITIZE] skip rename (no setName): "${currentName}" → "${sanitized}"`);
        }
        return false;
      }
      ...
      if (typeof device?.error === 'function') {
        device.error('[SANITIZE] Error sanitizing name:', err?.message || err);
      }
      return false;
    }
  }
