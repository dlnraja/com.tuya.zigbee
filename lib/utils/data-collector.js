...
async function registerReportListeners(device, zclNode) {
  ...
  try {
    ...
    if (endpoint.clusters.genPowerCfg) {
      ...
      // P2547/P216: never blind /2 — Tuya often already sends 0–100 (SDK3 + zcl-percent).
      endpoint.clusters.genPowerCfg.on('attr.batteryPercentageRemaining', async (value) => {
        try {
          ...
          const { setCapabilityCaught } = require('./Sdk3ZclSafe');
          const percent = normalizeZclBatteryPercent(value, {
            manufacturer: manufacturerOf(device),
          });
          if (percent == null) return;
          device.log(`[DATA-COLLECTOR] 🔋 Battery report: ${percent}% (raw=${value})`);

          if (device.hasCapability('measure_battery')) {
            await setCapabilityCaught(device, 'measure_battery', parseFloat(percent));
          }
      ...
      endpoint.clusters.genPowerCfg.on('attr.batteryVoltage', async (value) => {
        try {
          ...
          if (device.hasCapability('measure_battery')) {
            await device.safeSetCapabilityValue('measure_battery', parseFloat(percent)).catch(() => { });
          }
    ...
    if (endpoint.clusters.haElectricalMeasurement) {
      ...
      endpoint.clusters.haElectricalMeasurement.on('attr.activePower', async (value) => {
        try {
          ...
          if (device.hasCapability('measure_power')) {
            await device.safeSetCapabilityValue('measure_power', parseFloat(value)).catch(() => { });
          }
      ...
      endpoint.clusters.haElectricalMeasurement.on('attr.rmsvoltage', async (value) => {
        try {
          ...
          if (device.hasCapability('measure_voltage')) {
            await device.safeSetCapabilityValue('measure_voltage', parseFloat(value)).catch(() => { });
          }
      ...
      endpoint.clusters.haElectricalMeasurement.on('attr.rmscurrent', async (value) => {
        try {
          ...
          if (device.hasCapability('measure_current')) {
            await device.safeSetCapabilityValue('measure_current', parseFloat(current)).catch(() => { });
          }
      ...
      endpoint.clusters.msTemperatureMeasurement.on('attr.measuredValue', async (value) => {
        try {
          ...
          if (device.hasCapability('measure_temperature')) {
            await device.safeSetCapabilityValue('measure_temperature', parseFloat(temp)).catch(() => { });
          }
      ...
      endpoint.clusters.msRelativeHumidity.on('attr.measuredValue', async (value) => {
        try {
          ...
          if (device.hasCapability('measure_humidity')) {
            await device.safeSetCapabilityValue('measure_humidity', parseFloat(humidity)).catch(() => { });
          }
      ...
      endpoint.clusters.genOnOff.on('attr.onOff', async (value) => {
        try {
          ...
          if (device.hasCapability('onoff')) {
            await device.safeSetCapabilityValue('onoff', state).catch(() => { });
          }
      ...
      endpoint.clusters.genLevelCtrl.on('attr.currentLevel', async (value) => {
        try {
          ...
          if (device.hasCapability('dim')) {
            await device.safeSetCapabilityValue('dim', parseFloat(level)).catch(() => { });
          }
  ...
  const pollData = async () => {
    try {
      ...
      if (batteryData.percent !== null) {
        if (device.hasCapability('measure_battery')) {
          await device.safeSetCapabilityValue('measure_battery', parseFloat(batteryData.percent)).catch(() => { });
        }
      }
      ...
      if (powerSource === 'mains') {
        ...
        if (energyData.power !== null && device.hasCapability('measure_power')) {
          await device.safeSetCapabilityValue('measure_power', parseFloat(energyData.power)).catch(() => { });
        }

        if (energyData.voltage !== null && device.hasCapability('measure_voltage')) {
          await device.safeSetCapabilityValue('measure_voltage', parseFloat(energyData.voltage)).catch(() => { });
        }

        if (energyData.current !== null && device.hasCapability('measure_current')) {
          await device.safeSetCapabilityValue('measure_current', parseFloat(energyData.current)).catch(() => { });
        }
...
function voltageToPercent(voltage) {
  ...
  // v10.6.0 (R7): delegate to the unified NON-LINEAR curve — the linear
  // vMin/vMax interpolation overstated mid-life charge on coin cells.
  try {
    const UBH = require('../battery/UnifiedBatteryHandler');
    const curved = UBH?.calculateFromVoltage?.(voltage, 'CR2032');
    if (Number.isFinite(curved)) {return curved;}
