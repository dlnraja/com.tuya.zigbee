'use strict';
// TITAN v5: module-level native setTimeout — these run after the Homey
// init returns to handle sleepy EndDevice polling. They survive device
// uninit because the device may be asleep; clearTimeout is not safe here.
...
function fireAndForget(device, promise, options = {}) {
  ...
  let timer = null;
  // module-level native setTimeout: fire-and-forget timeout for sleepy
  // device init — must outlive the calling scope
 ...
 */
function bindClusters(zclNode, clusterNames, log, device) {
  if (!zclNode?.endpoints?.[1] || !Array.isArray(clusterNames)) {return 0;}
  let isPoll = () => false;
  let skipPoll = false;
  try {
    const policy = require('../zigbee/PollControlPolicy');
    isPoll = policy.isPollControlCluster;
    skipPoll = device ? !policy.shouldBindPollControl(device) : true;
  } catch (_e) {
    skipPoll = true;
  }
  ...
  for (const cName of clusterNames) {
    if (skipPoll && isPoll(cName)) {
      log?.(`[SLEEPY-INIT] skip pollControl bind (${cName})`);
      continue;
    }
...
async function deferredRead(device, zclNode, readSpec, delayMs = DEFERRED_BATTERY_DELAY_MS) {
  ...
  // module-level native setTimeout: sleep gap before re-attempting read
  // on a device that just woke up
...
function deferredTuyaDataRequest(device, zclNode, dpIds, delayMs = DEFERRED_BATTERY_DELAY_MS) {
  ...
  // module-level native setTimeout: deferred Tuya DP query for sleepy
  // TS0601 devices that miss the initial broadcast
  setTimeout(() => {
...
function setupIasZoneSleepy(device, iasCluster, zclNode, callbacks = {}) {
  ...
  if (callbacks.onZoneEnrollRequest) {
    iasCluster.onZoneEnrollRequest = async (payload) => {
      ...
      try {
        await iasCluster.zoneEnrollResponse({ enrollResponseCode: 0, zoneId: 10 });
  ...
  fireAndForget(device, (async () => {
    ...
    const hex = homeyIeee ? String(homeyIeee).replace(/[:\-0x]/gi, '') : '';
    if (homeyIeee && hex && !/^0+$/.test(hex)) {
  ...
  // zoneId 10 = Homey / IASZoneManager / forum #2134 Peter pattern (was 23)
  fireAndForget(device, (async () => {
    await iasCluster.zoneEnrollResponse({ enrollResponseCode: 0, zoneId: 10 });
