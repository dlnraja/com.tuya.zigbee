...
function generateCaseVariants(id) {
  ...
  if (m) {
    ...
    // Brand tokens (HOBEIAN, eWeLink, …): keep lower + upper
    canonical = norm;
  }
 ...
 * Returns canonical + lowercase for Tuya IDs; lower+UPPER for brand tokens.
 * @param {string} id
 * @returns {string[]}
 */
function pairingCaseVariants(id) {
  if (!id) {return [];}
  const all = generateCaseVariants(id);
  const norm = normalize(id);
  if (!norm) {return [];}
  ...
  // Brand-style (HOBEIAN, eWeLink): keep original casing + lower + upper
