...
/**
 * WHY(P2677): Athom compose matching is case-sensitive; Homey runtime must stay
 * case-insensitive via normalize(). Brand tokens (HOBEIAN, eWeLink) need more
 * than lower+UPPER — Title case + known OCR/typo aliases that interview/firmware
 * actually emit. Map is keyed by normalize() form.
 *
 * Contre quoi: Bastien/Universal "not recognized" when interview reports `HOBEIAN`
 * but compose only listed `hobeian` (or the reverse).
 */
const BRAND_ATHOM_FORMS = Object.freeze({
  hobeian: Object.freeze(['HOBEIAN', 'Hobeian', 'hobeian', 'heobian', 'Heobian']),
  heobian: Object.freeze(['HOBEIAN', 'Hobeian', 'hobeian', 'heobian', 'Heobian']),
  ewelink: Object.freeze(['eWeLink', 'ewelink', 'EWELINK', 'EWeLink']),
});

/**
 * Intelligent Athom-safe string forms for a brand / mfr token.
 * @param {string} id
 * @returns {string[]|null} frozen alias list, or null if not a known brand family
 */
function brandAthomForms(id) {
  if (!id) return null;
  const key = normalize(id);
  if (!key) return null;
  return BRAND_ATHOM_FORMS[key] || null;
}

/**
 * ProductId Athom forms for ZG- / AY- / WHD- style retail SKUs (case-sensitive pair).
 * @param {string} pid
 * @returns {string[]}
 */
function pairingProductIdVariants(pid) {
  if (!pid) return [];
  const s = String(pid).trim();
  if (!s) return [];
  // Tuya TS* / _TZ* — reuse pairingCaseVariants
  if (/^_?[Tt][SsZzYy]/.test(s) || s.startsWith('_')) {
    return pairingCaseVariants(s);
  }
  // Retail SKU: ZG-301Z-3CH, WHD02, AY-204ZX, 3315-S
  if (/^[A-Za-z0-9]+([-_][A-Za-z0-9]+)*$/.test(s)) {
    return Array.from(new Set([s, s.toLowerCase(), s.toUpperCase()].filter(Boolean)));
  }
  return [s];
}

/**
 * Bounded LRU cache for normalize() results.
 * normalize() is pure and called in hot loops (fingerprint matching over
 * 4000+ entries, per-device init, DP routing). A small LRU removes the
 * repeated NFKD + regex cost for recurring IDs (mfrs/pids are a closed
 * vocabulary of a few thousand strings). Cap keeps memory bounded.
 */
const NORMALIZE_CACHE_MAX = 4096;
const _normalizeCache = new Map();

/** OCR / interview typos → canonical brand key (after lowercasing). */
const NORMALIZE_BRAND_TYPOS = Object.freeze({
  heobian: 'hobeian',
});
 ...
 *  R5. Strip null bytes
 *  R6. Lowercase + trim
 *  R7. Brand OCR typo canonicalize (heobian → hobeian) — P2677
 *
...
function normalize(str) {
  ...
  if (cached !== undefined) {
    ...
    _normalizeCache.delete(key);
    _normalizeCache.set(key, cached);
    return cached;
  }
  let result = key.normalize('NFKD');
  ...
  // WHY(P2677): OCR / forum typos for known brands → same runtime identity
  if (NORMALIZE_BRAND_TYPOS[result]) {
    result = NORMALIZE_BRAND_TYPOS[result];
  }
  if (_normalizeCache.size >= NORMALIZE_CACHE_MAX) {
...
function normalizeDeviceName(name) {
  ...
  const prefixes = ['tuya ', 'moes ', 'lonsonho ', 'avatto ', 'beca ',
                    ...
                    'hobeian ', 'heobian ', 'ewelink ', 'nedis ', 'woox ',
                    'lumi ', 'aqara ', 'ikea ', 'osram ',
                    'philips ', 'signify ', 'innr '];
...
function generateCaseVariants(id) {
  if (!id) {return [];}
  const brand = brandAthomForms(id);
  if (brand) {
    return Array.from(new Set([id, ...brand].filter(Boolean)));
  }
  const norm = normalize(id);
  if (!norm) {return [];}
  const upper = String(id).toUpperCase();
  // Homey pairing + our manifests commonly use prefix UPPER + suffix lower:
  // e.g. _TZ3000_abcdef12 (canonical). Include that even when input is mixed.
  let canonical = norm;
  const m = norm.match(/^(_[a-z0-9]+)_(.+)$/i);
  if (m) {
    canonical = `${m[1].toUpperCase()}_${m[2].toLowerCase()}`;
  } else if (/^[a-z0-9_-]+$/i.test(norm)) {
    // Generic brand-like tokens without alias map: lower + upper only
    return Array.from(new Set([
      id, norm, upper, String(id).toLowerCase(),
    ].filter(Boolean)));
  }
  return Array.from(new Set([
    id,                 // original
    norm,               // fully lowercased (NFKD)
    upper,              // fully uppercased
    canonical,          // prefix UPPER + suffix lower (Tuya sacred form)
    String(id).toLowerCase(),
  ].filter(Boolean)));
}

/**
 * Homey-critical case forms only (keeps manifests smaller than full expand).
 * Returns canonical + lowercase for Tuya IDs; intelligent brand Athom forms
 * for HOBEIAN / eWeLink (Title + typo aliases — P2677).
 * @param {string} id
 * @returns {string[]}
 */
function pairingCaseVariants(id) {
  if (!id) {return [];}
  const brand = brandAthomForms(id);
  if (brand) {
    return Array.from(new Set([id, ...brand].filter(Boolean)));
  ...
  const norm = normalize(id);
  if (!norm) {return [];}
  // Skip synthetic / placeholder IDs
  if (/^_(hybrid|generic)_/i.test(norm) || /needs_device_assignment/i.test(norm)) {
    return [id];
  }
  const m = norm.match(/^(_[a-z0-9]+)_(.+)$/i);
  if (m) {
    const canonical = `${m[1].toUpperCase()}_${m[2].toLowerCase()}`;
    return Array.from(new Set([canonical, norm]));
  }
  // Unknown brand-style (not in BRAND_ATHOM_FORMS): keep original + lower + UPPER only
  // WHY(P2677): do NOT invent Title for every token — that bloated diy/generic compose.
  return Array.from(new Set([id, norm, String(id).toUpperCase()].filter(Boolean)));
}

/**
 * Merge pairing case variants into a manufacturerName list.
 * Homey fingerprint matching is often case-sensitive at compose level, so we
 * keep distinct string forms even when normalize() collapses them.
 * @param {string[]} list
 * @param {string} [mfr] - optional single mfr to expand; if omitted expands all
 * @returns {{ list: string[], added: number }}
 */
function mergeManufacturerCaseVariants(list, mfr) {
  const out = Array.isArray(list) ? [...list] : [];
  const presentExact = new Set(out.map((x) => String(x)));
  let added = 0;
  const seeds = mfr ? [mfr] : out;
  const seenSeed = new Set();
  for (const seed of seeds) {
    const key = normalize(seed);
    if (!key || seenSeed.has(key)) continue;
    seenSeed.add(key);
    for (const v of pairingCaseVariants(seed)) {
      if (!v || presentExact.has(v)) continue;
      out.push(v);
      presentExact.add(v);
      added += 1;
    }
...
/**
 * True if list already contains the Homey-critical string forms for mfr.
 */
function hasPairingCaseCoverage(list, mfr) {
  if (!Array.isArray(list) || !mfr) return false;
  const needed = pairingCaseVariants(mfr);
  return needed.every((n) => list.some((x) => x === n));
}
