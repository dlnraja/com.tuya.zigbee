...
const TU = require('./TuyaNormalizer'); // v10.0.0 ULTIMATE case-insensitive
...
const TUYA_DP_SIGNATURES = [
  ...
  // WHY(P2434): some firmware/interviews put brand in modelId
  { modelId: 'HOBEIAN', manufacturerPrefix: '_TZE200_' },
  { modelId: 'HOBEIAN', manufacturerPrefix: '_TZE204_' },
  { modelId: 'HOBEIAN', manufacturerPrefix: '_TZE284_' },
  { modelId: 'HOBEIAN', manufacturerName: 'HOBEIAN' },
...
function isTuyaDpDevice(device) {
  ...
  for (const sig of TUYA_DP_SIGNATURES) {
    ...
    if (sig.manufacturerPrefix) {
      // v10.0.0: Use TuyaNormalizer (NFKD + accents) for case-insensitive signature matching
      if (manufacturerName && TU.startsWithCI(manufacturerName, sig.manufacturerPrefix)) {
        return true;
      }
    }

    if (sig.manufacturerName) {
      if (manufacturerName && TU.equalsIgnoreCase(manufacturerName, sig.manufacturerName)) {
        return true;
      }
    }
  }
  ...
  if (manufacturerName) {
    for (const prefix of TUYA_DP_PREFIXES) {
      // v10.0.0: Use TuyaNormalizer (NFKD + accents) for case-insensitive prefix matching
      if (TU.startsWithCI(manufacturerName, prefix)) {
        return true;
      }
    }
  }
...
function _getModelId(device) {
  ...
  // ZCL node
  if (device.zclNode && device.zclNode.modelId) {return device.zclNode.modelId;}
  if (device.node && device.node.modelId) {return device.node.modelId;}

  // getData for Homey devices
  if (typeof device.getData === 'function') {
    const data = device.getData();
    if (data && data.modelId) {return data.modelId;}
  }
...
function _getManufacturerName(device) {
  ...
  // ZCL node
  if (device.zclNode && device.zclNode.manufacturerName) {return device.zclNode.manufacturerName;}
  if (device.node && device.node.manufacturerName) {return device.node.manufacturerName;}

  // getData for Homey devices
  if (typeof device.getData === 'function') {
    const data = device.getData();
    if (data && data.manufacturerName) {return data.manufacturerName;}
  }
...
/**
 * Safe multiplication to prevent NaN/null issues
...
/**
 * Safe division to prevent division by zero or NaN issues
 * @param {*} a - Dividend
 * @param {*} b - Divisor
 * @returns {number}
 */
...
/**
 * Safe parsing and division for Tuya DP values
 * @param {*} value - Value to parse
 * @param {number} [divisor=1] - Optional divisor to scale value
 * @returns {number}
 */
function safeParse(value, divisor = 1) {
  if (value === null || value === undefined) {return 0;}
  const parsed = parseFloat(value);
  if (isNaN(parsed)) {return 0;}
  const div = Number(divisor);
  if (isNaN(div) || div === 0) {return parsed;}
  return parsed / div;
}

/**
 * Convert ZCL Illuminance Measurement measuredValue to lux.
 * Zigbee Cluster Library formula: lux = 10^((measuredValue - 1) / 10000).
 * A raw value of 0 means the value is too low to measure, so expose 0 lx.
 *
 * @param {*} value - Raw ZCL measuredValue
 * @returns {number} - Rounded lux value
 */
function zclMeasuredValueToLux(value) {
  const measured = Number(value);
  if (!Number.isFinite(measured) || measured <= 0 || measured >= 0xFFFF) {return 0;}
  // Adaptive (#2134 Peter): many Tuya firmwares put linear lux in measuredValue
  // (<~5000). True ZCL log-encoding is typically ≫5000 for daylight lux.
  if (measured < 5000) {return Math.round(measured);}
  return Math.max(0, Math.round(Math.pow(10, (measured - 1) / 10000)));
}
