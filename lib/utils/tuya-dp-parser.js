...
/**
 * Tuya DP Parser - Parse cluster CLUSTERS.TUYA_EF00 data points
...
/**
 * Parse Tuya DP frame from cluster CLUSTERS.TUYA_EF00
 * 
 * Frame format:
...
function registerTuyaDPListener(device, zclNode, callback) {
  try {
    ...
    // Check if cluster CLUSTERS.TUYA_EF00 exists
    if (!endpoint.clusters || !endpoint.clusters['manuSpecificTuya']) {
      device.log('[TUYA-DP] Cluster CLUSTERS.TUYA_EF00 not found - trying alternative...');
      ...
      if (!tuyaCluster) {
        device.error('[TUYA-DP]  Tuya cluster CLUSTERS.TUYA_EF00 not accessible');
        return false;
      }
...
async function requestTuyaDP(device, zclNode, dpId) {
  try {
    ...
    if (typeof tuyaCluster.dataRequest === 'function') {
      ...
      device.log('[TUYA-DP]  dataRequest command not available');
    }
