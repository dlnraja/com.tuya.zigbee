...
async function ensureGangUiCapabilities(device) {
  ...
  // WHY(P2463 / GH#542): P2397 used to invent button.1..N on every multi-gang
  // switch — wall_switch_* / switch_4gang strip then get tiles re-added → "Botón"
  // spam with no on/off state (migueleap screenshot). Only soft-add buttons when
  // compose/store already has them, scene remote, or device opts in.
  let hasAnyButtonCap = false;
  try {
    const caps = typeof device.getCapabilities === 'function' ? device.getCapabilities() : [];
    hasAnyButtonCap = (caps || []).some((c) => c === 'button' || String(c).startsWith('button.'));
  } catch (_e) { /* soft */ }
  const skipButtonTiles = device.skipGangButtonUi === true
    || (!isScene && !hasAnyButtonCap && device.forceGangButtonUi !== true);
  ...
  // WHY(P2492): after soft-ensure, apply Homey charter titles / maintenanceAction
  try {
    const { applyHomeyButtonUiCharter } = require('./HomeyButtonUiCharter');
    await applyHomeyButtonUiCharter(device);
  } catch (_e) { /* soft */ }
