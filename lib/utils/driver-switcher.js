...
function isSwitchSafe(device, fromDriver, toDriver, deviceInfo) {
  try {
    ...
    // Example: switching to 'switch' requires // H1: OnOffBoundCluster must be bound per EP
  genOnOff
    if (toDriver.includes('switch') && !clusterNames.includes('// H1: OnOffBoundCluster must be bound per EP
  genOnOff')) {
      device.log('[SWITCHER]  Unsafe: switch driver requires // H1: OnOffBoundCluster must be bound per EP
  genOnOff');
      return false;
    }
