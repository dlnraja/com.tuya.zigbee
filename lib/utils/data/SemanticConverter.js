...
const CONVERSIONS = {
  // Temperature
  'temp': { divisor: 1, unit: 'Â°C', precision: 1 },
  'temp_div10': { divisor: 10, unit: 'Â°C', precision: 1 },
  'temp_div100': { divisor: 100, unit: 'Â°C', precision: 2 },
  'temp_f': { divisor: 1, unit: 'Â°F', precision: 1 },
  'temp_f_div10': { divisor: 10, unit: 'Â°F', precision: 1 },
  'temp_offset_40': { divisor: 1, unit: 'Â°C', offset: -40, precision: 1 },
  ...
  'pm25': { divisor: 1, unit: 'Âµg/mÂ³', precision: 0 },
  'formaldehyde': { divisor: 100, unit: 'mg/mÂ³', precision: 2 },
...
class SemanticConverter {
  ...
  static convert(raw, conversion) {
    ...
    // Standard division
    const numRaw = this._toSafeNumber(raw);
    
    if (conv.custom === 'battery_mv') {
      // Convert battery mV to percentage (typical 2.4V-3.2V range)
      value = this._batteryMvToPercent(numRaw);
    } else {
      const divisor = (conv.divisor === 0) ? 1 : conv.divisor; // Protect against division by zero
      value = safeDivide(numRaw, divisor);
      ...
      // Invert if needed (e.g., for inverted percentage sensors)
      if (conv.invert) {
        value = 100 - value;
      }
    }
    ...
    // Final hardening: Ensure value is a finite number
  ...
  static toRaw(value, conversion) {
    ...
    let raw = this._toSafeNumber(value);
  ...
  /**
   * Coerce value to a safe finite number
   * @private
   */
  static _toSafeNumber(val) {
  ...
  static _batteryMvToPercent(mv) {
    ...
    if (typeof mv !== 'number' || isNaN(mv) || !Number.isFinite(mv)) {
      return 0;
    }
    ...
    const min = 2400;
    const max = 3200;
    const pct = ((mv - min) /safeMultiply((max - min)), 100);
    
    // Final hardening: Round and bound to 0-100
    const result = Math.max(0, Math.min(100, Math.round(pct)));
  ...
  static getUnit(conversion) {
    return CONVERSIONS[conversion]?.unit || '' : null;
  }
   ...
   * @param {string} conversion - Conversion name
   * @returns {string} - Formatted string like "21.5 Â°C"
   */
