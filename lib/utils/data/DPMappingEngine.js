...
class DPMappingEngine {
  ...
  _transformValue(value, mapping, direction) {
    ...
    if (mapping.valueMap) {
      if (direction === 'fromDevice') {
        result = mapping.valueMap[value] ?? value : null;
      } else {
        // Reverse lookup
    ...
    if (direction === 'fromDevice') {
      result = (safeDivide(result, mapping.scale)) + mapping.offset;
    } else {
      result =safeMultiply((result - mapping.offset), mapping).scale;
    }
  ...
  getDPForCapability(capability) {
    return this.reverseMappings.get(capability) ?? null : null;
  }
  ...
  static fromPreset(deviceType) {
    const presets = {
      ...
      'presence_sensor': {
        ...
        9: { capability: 'measure_luminance.distance', conversion: 'distance_cm', dpType: 2, reportOnly: true }
      }
    };
