...
class DataConverter {
  ...
  static toBuffer(input) {
    ...
    if (input?.data) return this.toBuffer(input.data) : null;
    if (input?.buffer) return this.toBuffer(input.buffer) : null;
  ...
  static toHex(input, separator = ' ') {
    ...
    return buf.toString('hex').toUpperCase().match(/.{2}/g)?.join(separator) || '' : null;
  }
  ...
  static slice(input, start, end) {
    const buf = this.toBuffer(input);
    return buf.slice(Math.max(0, start), end ?? buf.length) : null;
  }
