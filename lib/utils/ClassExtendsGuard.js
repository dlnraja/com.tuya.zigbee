...
// P24.7 follow-up: this require MUST be guarded too. If homey-zigbeedriver
// itself fails to load (its ZigBeeDriver.js does `class X extends Homey.Driver`
// which throws "Class extends value undefined" when the homey module resolves
// to something without Driver), an unguarded top-level require would crash
// THIS module — and with it every device.js that relies on the guard.
let ZigBeeDevice;
try {
  ZigBeeDevice = require('homey-zigbeedriver').ZigBeeDevice;
} catch (_err) {
  ZigBeeDevice = undefined;
}
...
function safeExtends(className, loader) {
  ...
  // A circular/partial require can resolve to the module namespace object
  // (`{}` or `{ TuyaZigbeeDevice: Class }`) instead of the class itself —
  // extending that object throws "Class extends value #<Object>".
  if (Primary && typeof Primary === 'object' && typeof Primary[className] === 'function') {
    Primary = Primary[className];
  }
  if (Primary && typeof Primary === 'object' && typeof Primary.ZigBeeDevice === 'function') {
