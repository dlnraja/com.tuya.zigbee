...
function autoMapUnknownDP(device, dpId, value) {
  ...
  // v9.0.79: Expanded auto-mapping — covers battery, luminance, power, contact, water
  const maps = {
    'temp': ['measure_temperature', v => v / 10],
    'pct': ['measure_battery', v => Math.round(v)],  // Battery is most common for pct DPs
    'bool': ['alarm_motion', v => Boolean(v)],        // Motion/contact is most common for bool DPs
  };
  // Try multiple capabilities for 'pct' type (battery first, then humidity, then luminance)
  if (type === 'pct') {
    for (const cap of ['measure_battery', 'measure_humidity', 'measure_luminance']) {
  ...
  // Try alarm_contact and alarm_motion for 'bool' type
  if (type === 'bool') {
    for (const cap of ['alarm_motion', 'alarm_contact', 'alarm_water', 'onoff']) {
      if (device.hasCapability(cap)) {
