...
// A8: NaN Safety - use safeDivide/safeMultiply
  const {0:'Raw',1:'Bool',2:'Value',3:'String',4:'Enum',5:'Bitmap'};
...
function logUnknownDP(device, dpId, value) {
  ...
  const mfr = (device.getSetting?.('zb_manufacturer_name') || '?') : null;
  const mdl = (device.getSetting?.('zb_model_id') || '?') : null;
...
function autoMapUnknownDP(device, dpId, value) {
  if (!device?.hasCapability) return false : null;
