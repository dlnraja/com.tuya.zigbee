...
const DataProvenance = require('../telemetry/DataProvenance');
const LocalSmartEnergyLearner = require('../telemetry/LocalSmartEnergyLearner');
...
class DeviceTelemetryEstimator {
  ...
  async recordCapability(capability, value, meta = {}) {
    ...
    // WHY(P2556): normalize origin — measured | estimated | calculated (legacy direct→measured)
    const origin = this._applyingEstimate
      ? (DataProvenance.originFromReason(meta.reason || meta.via) || 'estimated')
      : (DataProvenance.normalizeOrigin(meta.origin || meta.source) || 'measured');
    const source = DataProvenance.toLegacySource(origin);
    const now = Date.now();
    await this._stampProvenance(capability, origin, meta.reason || meta.via || source);
    ...
    if (capability === 'measure_battery') {
      ...
      if (!this._applyingEstimate && isFiniteNumber(value)) {
        ...
        // P2560: local auto-learn when a real power sample arrives while ON
        if (capability === 'measure_power' && DataProvenance.isMeasured(origin)) {
          const isOn = this._getCapability('onoff') === true;
          const learner = this._learner();
          await learner.observeMeasuredPower(value, { isOn, now });
          const lastEst = Number(this._getStore('telemetry_estimated_power_w', NaN));
          if (isOn && isFiniteNumber(lastEst) && lastEst > 0.05) {
            await learner.observeEstimateBias(lastEst, value);
          }
        }
      }
    }
  ...
  /** Lazy local habit/power learner (P2560) — no cloud AI. */
  _learner() {
    if (!this.__smartLearner) {
      this.__smartLearner = new LocalSmartEnergyLearner(this.device);
    }
    return this.__smartLearner;
  }

  async _stampProvenance(capability, origin, reason) {
    try {
      await DataProvenance.stampProvenance(this.device, capability, origin, {
        reason,
        applyUx: true,
        onChanged: (prev, next) => {
          this._emitSourceChanged(capability, prev, next).catch(() => {});
        },
      });
    } catch (_e) {
      await this._setStore(`telemetry_${capability}_source`, DataProvenance.toLegacySource(origin));
      await this._setStore(`telemetry_${capability}_origin`, DataProvenance.normalizeOrigin(origin) || 'measured');
      await this._setStore(`telemetry_${capability}_updated_at`, Date.now());
    }
  }

  async _emitSourceChanged(capability, prev, next) {
    try {
      const card = this.device.homey?.flow?.getDeviceTriggerCard?.('telemetry_source_changed');
      if (!card || typeof card.trigger !== 'function') return;
      const quality = DataProvenance.flowLabelFor
        ? DataProvenance.flowLabelFor(next, 'en')
        : String(next || '');
      await card.trigger(this.device, {
        capability: String(capability),
        previous: String(prev || ''),
        origin: String(next || ''),
        quality_label: String(quality || next || ''),
      }, {}).catch(() => {});
    } catch (_e) { /* soft */ }
  }
  ...
  async _ensureBattery(reason) {
    ...
    // v10.6.0 (cross-platform consensus z2m/ZHA): NEVER fabricate a battery
    // percentage out of thin air. A pure default (no real value ever seen)
    // stays in store for diagnostics only — the capability keeps its real
    // state (null → Homey shows "–" like z2m's sentinel→unknown). Decay
    // estimates anchored on a REAL last-known value remain displayed.
    if (estimate.reason === 'profile-default:no-direct-report') {
      this._log?.(`[TELEMETRY] No real battery ever reported — leaving measure_battery unset (estimate kept in store: ${estimate.percent}%)`);
      return;
    }
  ...
  async _ensureEnergy(reason, usage = null) {
    ...
    // P2560: habit refresh + next-hour prediction (store only — capability fill stays calculated/estimated)
    try {
      const learner = this._learner();
      await learner.refreshHabitsFromUsage({
        onMs: Number(this._getStore('telemetry_usage_on_ms', 0)) || 0,
        offMs: Number(this._getStore('telemetry_usage_off_ms', 0)) || 0,
      });
      const predKwh = learner.predictNextHourKwh({ powerW: power });
      await this._setStore('telemetry_predicted_next_hour_kwh', predKwh);
      await this._setStore('telemetry_smart_learn_snapshot', learner.snapshot());
    } catch (_e) { /* soft */ }

    if (this._shouldFill('measure_power')) {
      const powerOrigin = (this._getStore('telemetry_measure_power_source', null) !== 'estimated'
        && this._isRealNumber(this._getCapability('measure_power')))
        ? 'measured'
        : 'estimated';
      await this._setCapabilityWithOrigin('measure_power', power, powerOrigin, `${reason}:power`);
    }
    if (this._shouldFill('measure_voltage')) {
      const voltOrigin = (this._getStore('telemetry_measure_voltage_source', null) !== 'estimated'
        && this._isRealNumber(this._getCapability('measure_voltage')))
        ? 'measured'
        : 'estimated';
      await this._setCapabilityWithOrigin('measure_voltage', voltage, voltOrigin, `${reason}:voltage`);
    }
    if (this._shouldFill('measure_current')) {
      // WHY(P2556): I = P/V is calculated when not a live meter report
      const curOrigin = (this._getStore('telemetry_measure_current_source', null) === 'direct'
        || this._getStore('telemetry_measure_current_origin', null) === 'measured')
        ? 'measured'
        : 'calculated';
      await this._setCapabilityWithOrigin(
        'measure_current',
        current,
        curOrigin === 'measured' ? 'measured' : 'calculated',
        `${reason}:from_vi`,
      );
    }
    if (this._shouldFill('meter_power')) {
      // Energy integral of estimated power → calculated; profile fill → estimated
      const energyOrigin = /integral|usage|elapsed|accumul/i.test(String(reason))
        ? 'calculated'
        : 'calculated';
      await this._setCapabilityWithOrigin('meter_power', energy, energyOrigin, `${reason}:energy-integral`);
    }
  }
  ...
  _estimatePower() {
    ...
    const specW = this._numberSetting(['nominal_power', 'power_estimate_w', 'estimated_power_w'], this._defaultPowerWatts());
    ...
    const origin = this._getStore('telemetry_measure_power_origin', null)
      || this._getStore('telemetry_measure_power_source', null);

    // Measured-first: live meter wins (never invent over real W)
    if (this._isRealNumber(directPower)
        && this._getStore('telemetry_measure_power_source', null) !== 'estimated'
        && origin !== 'estimated' && origin !== 'predicted'
        && (DataProvenance.isMeasured(origin) || origin === 'direct' || origin == null)) {
      return round(Math.max(0, directPower), 2);
    }

    // P2560: smart blend — manufacturer/driver ref + dim/color + local EMA habits + bias
    try {
      return this._learner().estimateSmartPowerW({
        isOn,
        standbyW: standby,
        specW,
        driverClass: `${this._deviceClass()} ${this._driverId()} ${this._name()}`,
      });
    } catch (_e) {
      return round(Math.max(0, isOn ? specW : standby), 2);
    }
  }
  ...
  _estimateVoltage() {
    ...
    if (this._isRealNumber(directVoltage) && this._getStore('telemetry_measure_voltage_source', null) !== 'estimated'
        && this._getStore('telemetry_measure_voltage_origin', null) !== 'estimated') {
  ...
  _estimateEnergy(power, elapsedMs, now) {
    ...
    const origin = this._getStore('telemetry_meter_power_origin', null);
    let base = Number(this._getStore('telemetry_estimated_meter_power_kwh', NaN));

    if (this._isRealNumber(directEnergy) && source !== 'estimated' && origin !== 'estimated') {
      base = directEnergy;
    }
  ...
  _shouldFill(capability) {
    ...
    // WHY(P2558): measured-first — never fill over a real device report
    const origin = this._getStore(`telemetry_${capability}_origin`, null)
      || this._getStore(`telemetry_${capability}_source`, null);
    if (DataProvenance.isMeasured(origin) || origin === 'direct') {return false;}
    if (this.device._energyRealCaps instanceof Set && this.device._energyRealCaps.has(capability)) {return false;}
    const smart = this.device.smartEnergy;
    if (smart && typeof smart.isAuditComplete === 'function' && !smart.isAuditComplete()) {
      return false;
    }
    if (smart && typeof smart.isRealCapability === 'function' && smart.isRealCapability(capability)) {
      return false;
    }
    const value = this._getCapability(capability);
    if (!this._isRealNumber(value)) {return true;}
    // Refresh soft origins only (estimated / calculated / predicted)
    return DataProvenance.isNonMeasured(origin)
      || origin === 'estimated' || origin === 'calculated' || origin === 'predicted';
  }

  async _setCapabilityEstimated(capability, value, reason) {
    const origin = DataProvenance.originFromReason(reason) || 'estimated';
    return this._setCapabilityWithOrigin(capability, value, origin, reason);
  }

  async _setCapabilityWithOrigin(capability, value, origin, reason) {
    if (!this._hasCapability(capability)) {return false;}
    const current = this._getCapability(capability);
    if (current === value) {
      await this._stampProvenance(capability, origin, reason);
      return true;
    }
    ...
    try {
      ...
      if (typeof this.device.safeSetCapabilityValue === 'function') {
        return await this.device.safeSetCapabilityValue(capability, value, {
          skipThrottle: true,
          forceDedupe: false,
          source: DataProvenance.toLegacySource(origin),
          origin: DataProvenance.normalizeOrigin(origin),
        });
      }
  ...
  _defaultPowerWatts() {
    // v9.0.375: référence produit d'abord (valeurs typiques par driver),
    // heuristiques par classe ensuite.
    const id = this._driverId();
    const ref = this._energyReference();
    if (ref.drivers?.[id]?.nominalW !== undefined && ref.drivers[id].nominalW > 0) {
      return ref.drivers[id].nominalW;
    }
    const cls = this._deviceClass();
    if (ref.classes?.[cls]?.nominalW !== undefined) {return ref.classes[cls].nominalW;}
    const text = `${id} ${cls} ${this._name()}`.toLowerCase();
  ...
  _defaultStandbyWatts() {
    const id = this._driverId();
    const ref = this._energyReference();
    if (ref.drivers?.[id]?.standbyW !== undefined) {return ref.drivers[id].standbyW;}
    const cls = this._deviceClass();
    if (ref.classes?.[cls]?.standbyW !== undefined) {return ref.classes[cls].standbyW;}
    const text = `${id} ${cls} ${this._name()}`.toLowerCase();
  ...
  _energyReference() {
    if (this._energyRefCache) {return this._energyRefCache;}
    try {
      this._energyRefCache = require('../../data/energy-consumption-reference.json');
    } catch {
      this._energyRefCache = { drivers: {}, classes: {} };
    }
    return this._energyRefCache;
  }
  ...
  _isDestroyed() {
    return !!(!this.device || this.device._destroyed || this.device._deleted || this.device.destroyed);
  }
