...
class DeviceTelemetryEstimator {
  ...
  _estimatePower() {
    ...
    if (this._isRealNumber(directPower) && this._getStore('telemetry_measure_power_source', null) !== 'estimated'
        && this._getStore('telemetry_measure_power_origin', null) !== 'estimated') {
  ...
  _shouldFill(capability) {
    ...
    // Never overwrite a real device report
    ...
    // Wait for SmartEnergyManager 5-min audit before estimating energy caps
    ...
    return DataProvenance.isEstimated(origin) || DataProvenance.isCalculated(origin)
      || origin === 'estimated' || origin === 'calculated';
  }
