...
async function autoConfigureReporting(device, zclNode) {
  ...
  try {
    ...
    // Battery devices - configure if capability exists AND cluster exists
    if (wantsBattery && endpoint.clusters.genPowerCfg) {
      ...
      device.log('[CLUSTER-CONFIG] ℹ️  Battery capability but no genPowerCfg cluster (Tuya DP?)');
      // Tuya DP devices handle battery differently
    }
