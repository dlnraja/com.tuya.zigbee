...
async function autoConfigureReporting(device, zclNode) {
  ...
  try {
    ...
    const endpoint = zclNode?.endpoints?.[1] || zclNode?.endpoints?.[Object.keys(zclNode?.endpoints || {})[0]] : null;
    if (!endpoint) {
    ...
    const modelId = device.getSetting?.('zb_model_id') || device.getData?.()?.modelId || '' : null;
    ...
    const driverName = device.driver?.manifest?.id || device.constructor.name || '' : null;
