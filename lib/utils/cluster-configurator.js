...
async function configureBatteryReporting(endpoint, options = {}) {
  ...
  try {
    ...
    // Configuring battery reporting
    // minInterval/maxInterval/minChange configured
    ...
    try {
      ...
      // Battery percentage config failed - device may not support this attribute
    }
    ...
    try {
      ...
      // Battery voltage config failed - device may not support this attribute
    }

  } catch (err) {
    // Battery reporting config error - non-critical
  }
}
  ...
  try {
    ...
    if (endpoint.clusters.haElectricalMeasurement) {
      ...
      try {
        ...
        // Active power config failed - device may not support this attribute
      }
      ...
      try {
        ...
        // RMS voltage config failed - device may not support this attribute
      }
      ...
      try {
        ...
        // RMS current config failed - device may not support this attribute
      }
    }

    // seMetering cluster (alternative for some devices)
    if (endpoint.clusters.seMetering) {
      // Configuring seMetering
      ...
      try {
        ...
        // Instantaneous demand reporting configured
      } catch (e) {
        // Instantaneous demand config failed - device may not support this attribute
      }
    }

  } catch (err) {
    // Power reporting config error - non-critical
  }
}
...
async function configureClimateReporting(endpoint, options = {}) {
  ...
  try {
    // Configuring climate reporting

    // Temperature reporting
    if (endpoint.clusters.msTemperatureMeasurement) {
      // Configuring temperature reporting
      ...
      try {
        ...
        // Temperature reporting configured
      } catch (e) {
        // Temperature config failed - device may not support this attribute
      }
    }

    // Humidity reporting
    if (endpoint.clusters.msRelativeHumidity) {
      // Configuring humidity reporting
      ...
      try {
        ...
        // Humidity reporting configured
      } catch (e) {
        // Humidity config failed - device may not support this attribute
      }
    }

  } catch (err) {
    // Climate reporting config error - non-critical
  }
}
...
async function configureOnOffReporting(endpoint, options = {}) {
  ...
  try {
    ...
    // OnOff reporting configured

  } catch (err) {
    // OnOff reporting config error - non-critical
  }
}
...
async function configureLevelReporting(endpoint, options = {}) {
  ...
  try {
    ...
    // Configuring level control reporting
    ...
    // Level control reporting configured

  } catch (err) {
    // Level control reporting config error - non-critical
  }
}
...
async function autoConfigureReporting(device, zclNode) {
  ...
  try {
    ...
    if (wantsBattery) {
      ...
      const mfr = device.getData?.()?.manufacturerName || '';
      const isTuyaManufacturer = mfr.startsWith('_TZ') || mfr.startsWith('_TY') || mfr.toLowerCase().includes('tuya');

      if (isTuyaManufacturer || isTuyaDP) {
        device.log('[CLUSTER-CONFIG] ℹ️  Skipping genPowerCfg configureReporting (Tuya devices auto-report and timeout causes battery drain)');
      } else if (endpoint.clusters.genPowerCfg) {
