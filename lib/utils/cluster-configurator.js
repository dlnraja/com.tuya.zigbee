...
async function autoConfigureReporting(device, zclNode) {
  ...
  try {
    ...
    // CRITICAL FIX: Detect Tuya DP devices FIRST
    const modelId = device.getData?.()?.modelId || '';
    const manufacturer = device.getData?.()?.manufacturerName || '';
    const isTuyaDP = modelId === 'TS0601' || manufacturer.startsWith('_TZE') || endpoint.clusters?.manuSpecificTuya;

    if (isTuyaDP) {
      device.log('[CLUSTER-CONFIG] 🔶 Tuya DP device detected - using capability-based config');
    }
