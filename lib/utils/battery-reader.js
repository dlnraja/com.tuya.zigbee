...
async function readBattery(device, zclNode) {
  ...
  try {
    ...
    const { isTuyaDP } = require('../device_helpers');
    const deviceInfo = {
      modelId: device.getData()?.zb_product_id || '',
      manufacturer: device.getData()?.zb_manufacturer_name || '',
      zclNode: zclNode
    };

    if (isTuyaDP(deviceInfo, device)) {
