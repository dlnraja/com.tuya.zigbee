...
async function readBattery(device, zclNode) {
  ...
  try {
    ...
    // Use unified helper for reliable detection
    const { isTuyaDPDevice, getModelId, getManufacturer } = require('../helpers/DeviceDataHelper');

    // Get model/manufacturer from ALL possible sources
    ...
    // Check if Tuya DP device
    if (isTuyaDPDevice(device)) {
      device.log('[BATTERY-READER] 🔋 Tuya DP device detected - battery managed by TuyaEF00Manager');
      device.log('[BATTERY-READER] Battery will come via DP 4/14/15 events');
      // TuyaEF00Manager will handle battery via DP events (DP 4/14/15)
      // Return last known value if available, otherwise null
      ...
      if (storedBattery !== null && storedBattery !== undefined) {
        ...
        device.log(`[BATTERY-READER] Using stored Tuya DP battery: ${storedBattery}%`);
      } else {
