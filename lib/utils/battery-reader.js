...
async function readBattery(device, zclNode) {
  ...
  try {
    ...
    if (ep1 && ep1.clusters && ep1.clusters.genPowerCfg) {
      ...
      try {
        ...
        if (attrs && typeof attrs.batteryPercentageRemaining === 'number') {
          ...
          result.percent = normalizeZclBatteryPercent(attrs.batteryPercentageRemaining, { manufacturer });
          if (result.percent === null) {
            device.log('[BATTERY-READER] batteryPercentageRemaining was a sentinel — leaving unknown');
          } else {
      ...
      try {
        ...
        if (attrs && typeof attrs.batteryVoltage === 'number') {
          ...
          result.voltage = UBH.normalizeVoltage?.(attrs.batteryVoltage) ?? null;
          result.percent = normalizeZclBatteryVoltagePercent(attrs.batteryVoltage, { batteryType: 'CR2032' });
          if (result.percent !== null) {
    ...
    // METHOD 4: leave unknown — Homey Energy shows "?" until a real report
    device.log('[BATTERY-READER] ⚠️ All read methods failed — leaving battery unknown');
    result.percent = null;
