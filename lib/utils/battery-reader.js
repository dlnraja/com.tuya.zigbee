...
async function readBattery(device, zclNode) {
  ...
  try {
    ...
    try {
      ...
      // Check if device uses Tuya DP
      const manufacturer = device.getData()?.manufacturerName || device.getStoreValue('manufacturerName') || '';
      const modelId = device.getData()?.productId || device.getData()?.modelId || '';
      
      if (modelId === 'TS0601' || manufacturer.startsWith('_TZE')) {
        device.log('[BATTERY-READER] ⚠️  Tuya DP device detected - cluster 0xEF00 hidden');
        
        // Tuya DP devices report battery via hidden cluster
        // We can't read it directly, but we can listen to reports
        // For now, mark as Tuya DP source and return unknown
        result.source = 'tuya_dp_unavailable';
        device.log('[BATTERY-READER] ⚠️  Battery data requires Tuya DP parsing (not yet implemented)');
        
        // Note: Actual Tuya DP parsing would require:
        // - Listening to cluster 0xEF00 reports
        // - Parsing DP frames (DP ID 15 = battery for most sensors)
        // - Decoding value based on DP type
      }
    } catch (e) {
