...
/**
 * Get a logger bound to homey context, with silent fallback
 */
function _getLogger(homey) {
  const log = (homey && typeof homey.log === 'function') ? homey.log.bind(homey) : () => {};
  const error = (homey && typeof homey.error === 'function') ? homey.error.bind(homey) : () => {};
  return { log, error };
}
...
async function queueMigration(homey, deviceId, targetDriverId, reason = 'auto') {
  try {
    if (!homey || !homey.settings) {
      _getLogger(homey).error('[MIGRATION-QUEUE] Invalid homey instance');
      return;
    }
    ...
    if (existing) {
      _getLogger(homey).log(`[MIGRATION-QUEUE] Device ${deviceId} -> ${targetDriverId} already queued`);
      return;
    }
    ...
    _getLogger(homey).log(`[MIGRATION-QUEUE] Queued migration: ${deviceId} -> ${targetDriverId} (${reason})`);

  } catch (err) {
    _getLogger(homey).error('[MIGRATION-QUEUE] Failed to queue migration:', err.message);
  }
}
...
async function getMigrationQueue(homey) {
  try {
    ...
    _getLogger(homey).error('[MIGRATION-QUEUE] Failed to get queue:', err.message);
    return [];
  }
}
...
async function popMigrations(homey) {
  try {
    ...
    _getLogger(homey).log(`[MIGRATION-QUEUE] Popped ${queue.length} migration(s)`);
    return queue;

  } catch (err) {
    _getLogger(homey).error('[MIGRATION-QUEUE] Failed to pop migrations:', err.message);
    return [];
  }
}
...
async function clearDeviceMigrations(homey, deviceId) {
  try {
    ...
    if (removed > 0) {
      _getLogger(homey).log(`[MIGRATION-QUEUE] Cleared ${removed} migration(s) for device ${deviceId}`);
    }

  } catch (err) {
    _getLogger(homey).error('[MIGRATION-QUEUE] Failed to clear migrations:', err.message);
  }
}
...
async function processMigrationQueue(homey) {
  const { log, error } = _getLogger(homey);
  log('[MIGRATION-QUEUE] Processing migration queue...');

  try {
    const jobs = await popMigrations(homey);

    if (jobs.length === 0) {
      log('[MIGRATION-QUEUE] No migrations to process');
      return;
    }

    log(`[MIGRATION-QUEUE] Processing ${jobs.length} migration(s)...`);
      ...
      try {
        ...
        if (!targetDriver) {
          ...
          if (rawTargetDriverId !== targetDriverId) {
            log(`[MIGRATION-QUEUE] Driver alias: ${rawTargetDriverId} -> ${targetDriverId}`);
          }
          log(`[MIGRATION-QUEUE] Target driver not found: ${targetDriverId} (Device: ${deviceId})`);
          continue;
        }

        // Log alias resolution if it happened
        if (rawTargetDriverId !== targetDriverId) {
          log(`[MIGRATION-QUEUE] Resolved alias: ${rawTargetDriverId} -> ${targetDriverId}`);
        }
        ...
        try {
          let device = null;
          if (homey.devices && typeof homey.devices.getDevice === 'function') {
            device = await homey.devices.getDevice({ id: deviceId });
          } else if (homey.drivers && typeof homey.drivers.getDrivers === 'function') {
            for (const driver of homey.drivers.getDrivers()) {
              try {
                device = driver.getDevice({ id: deviceId });
                if (device) {break;}
              } catch (e) { /* driver does not handle this device */ }
            }
          }
          if (device) {
          ...
          log(`[MIGRATION-QUEUE] Could not get device details: ${deviceErr.message}`);
        }

        // Log migration instructions for user
        log(`[MIGRATION-QUEUE] Migration required: ${deviceId} (${deviceName}) ${actualCurrentDriver} -> ${targetDriverId} (${reason || 'auto'}, confidence: ${confidence || 100}%)`);

      } catch (err) {
        error(`[MIGRATION-QUEUE] Failed to process migration job: ${err.message} (Device: ${deviceId}, Target: ${targetDriverId})`);
      }
    }

    log('[MIGRATION-QUEUE] Queue processing complete');

  } catch (err) {
    error('[MIGRATION-QUEUE] Queue processing error:', err.message);
  }
}
...
async function getMigrationStats(homey) {
  try {
    ...
    _getLogger(homey).error('[MIGRATION-QUEUE] Failed to get stats:', err.message);
