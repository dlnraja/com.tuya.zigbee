...
class TuyaDeviceHelper {
  ...
  static isTuyaDPDevice(meta) {
    ...
    // v10.0.0: Use TuyaNormalizer (NFKD + accents) for case-insensitive device detection
    const mfrLower = TU.normalize(manufacturerName);
    const modelLower = TU.normalize(modelId);

    // Hobeian Tuya DP / hybrid models support (case-insensitive)
    if (mfrLower.includes('hobeian')) {
      const knownDPModels = ['zg-204zm', 'zg-204zv', 'zg-227z', 'zg-303z'];
      if (knownDPModels.some(m => modelLower.includes(m))) {
        return true;
      }
    }
