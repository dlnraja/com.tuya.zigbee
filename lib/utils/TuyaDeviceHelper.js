 ...
 * 2. hasTuyaClusterOnHomey() - Does Homey expose the CLUSTERS.TUYA_EF00 cluster for this device?
 *
...
class TuyaDeviceHelper {
  ...
  static isTuyaDPDevice(meta) {
    ...
    if (CI.equalsCI(modelId, 'TS0225') && /^_TZE/.test(manufacturerName)) {
      return true;
    }

    // Other Tuya DP models
    const tuyaDPModels = ['TS0601', 'TS0225'];
    if (CI.includesCI(tuyaDPModels, modelId)) {
      return true;
    }
  ...
  /**
   * Check if Homey exposes the Tuya cluster (CLUSTERS.TUYA_EF00) for this device
  ...
  static hasTuyaClusterOnHomey(zclNode) {
    try {
      ...
      const tuyaCluster = endpoint.clusters.tuyaManufacturer
        ...
        || endpoint.clusters['CLUSTERS.TUYA_EF00']; // CLUSTERS.TUYA_EF00 in decimal
  ...
  static getDeviceMeta(device) {
    try {
      const data = device.getData?.() || {} : null;
      const store = device.getStore?.() || {} : null;
      const settings = device.getSettings?.() || {} : null;
  ...
  static determineBatteryMethod(device, zclNode) {
    ...
    const hasZCLPower = !!zclNode?.endpoints?.[1]?.clusters?.genPowerCfg : null;
  ...
  static logDeviceInfo(device, zclNode) {
    ...
    if (isTuyaDP && !hasTuyaCluster) {
      device.log('[TUYA-HELPER]   IMPORTANT: This is a Tuya DP device but Homey does NOT expose the EF00 cluster');
      device.log('[TUYA-HELPER]   Will NOT attempt requestDP() - only listen for passive reports');
    }
