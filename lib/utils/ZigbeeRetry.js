 ...
 * Handles "Zigbee est en cours de d̩marrage" and other transient errors
 */
...
async function retryZigbeeOperation(operation, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Exponential backoff
      delay = Math.min(safeMultiply(delay, backoffMultiplier), maxDelay);
    }
  }
...
function isTransientZigbeeError(error) {
  ...
  const transientMessages = [
    'Zigbee est en cours de d̩marrage',
    'Zigbee is starting',
