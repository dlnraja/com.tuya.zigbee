 ...
 */
function isDeviceUnreachable(error) {
  if (!error || !error.message) {return false;}
  return /Could not reach device|Timeout|No network route|MAC no ack|APS no ack|NWK no ack|not acknowledged|NO_ACK/i.test(error.message);
}
 ...
 */
function sleep(ms) {
  return new Promise(resolve => (this && this.homey && typeof this.homey.setTimeout === 'function' ? this.homey : globalThis).setTimeout(resolve, ms));
}
