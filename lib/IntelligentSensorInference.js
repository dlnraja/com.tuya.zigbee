...
class ClimateInference extends IntelligentSensorInference {
  ...
  validateTemperature(rawTemp) {
    ...
    const minTemp = this.options.minTemp ?? -40;
    const maxTemp = this.options.maxTemp ?? 100;
    if (rawTemp < minTemp || rawTemp > maxTemp) {
      this.log(`[CLIMATE-INFER] ⚠️ Temperature out of range: ${rawTemp}°C - REJECTED`);
      this._erraticReadingCount++;
      return this._lastValidTemp;
    }

    // GH #513 / ZT08: MCU may emit 0 until clock DP17 commit. Treat a locked
    // 0°C baseline as uninitialized so the first real reading is not smoothed away.
    if (this._lastValidTemp === 0 && rawTemp !== 0 && Math.abs(rawTemp) >= 1) {
      this._lastValidTemp = null;
    }
  ...
  validateHumidity(rawHumidity) {
    ...
    if (rawHumidity > 100 && rawHumidity <= 1000) {
      rawHumidity = rawHumidity / 10;
    }
