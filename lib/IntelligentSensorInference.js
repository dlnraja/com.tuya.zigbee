...
class ClimateInference extends IntelligentSensorInference {
  ...
  getSensorHealth() {
    return { erraticReadings: this._erraticReadingCount, confidence: this.getConfidence(), isHealthy: this._erraticReadingCount < 5 && this._confidence > 50 };
  }
...
class AirQualityInference extends IntelligentSensorInference {
  ...
  calculateAQI() {
    let aqi = 0, factors = 0;
    if (this._lastCO2 !== null) { aqi += Math.min(500, (this._lastCO2 - 400) / 4); factors++; }
    if (this._lastPM25 !== null) { aqi += Math.min(500, this._lastPM25 * 4); factors++; }
    if (this._lastVOC !== null) { aqi += Math.min(500, this._lastVOC / 4); factors++; }
...
class BatteryInference extends IntelligentSensorInference {
  constructor(device, options = {}) {
    super(device, { lowBatteryThreshold: options.lowBatteryThreshold || 20, criticalBatteryThreshold: options.criticalBatteryThreshold || 10, ...options });
...
class RadioSensingInference extends IntelligentSensorInference {
  constructor(device, options = {}) {
    super(device, { lqiVarianceThreshold: options.lqiVarianceThreshold || 15, activityWindow: options.activityWindow || 15000, presenceHoldTime: options.presenceHoldTime || 60000, ...options });
...
class MaintenancePredictionInference extends IntelligentSensorInference {
  constructor(device, options = {}) { super(device, options); this._errorCount = 0; this._lastCheckIn = Date.now(); }
...
module.exports = { IntelligentSensorInference, MotionLuxInference, ClimateInference, SoilMoistureInference, AirQualityInference, BatteryInference, RadioSensingInference, MaintenancePredictionInference };
