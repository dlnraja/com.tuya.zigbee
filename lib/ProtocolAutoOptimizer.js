 ...
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║                                                                              ║
 * ║  Centralized protocol management for ALL drivers                      ║
 * ║                                                                              ║
 * ║  FEATURES:                                                                   ║
...
class ProtocolAutoOptimizer extends EventEmitter {
  ...
  _trackCapabilityDiscovery(protocol, identifier, capability) {
    ...
    if (!this.discoveredCapabilities.has(key)) {
      ...
      const found = data.sources.find(s => s.protocol === protocol && s.identifier === identifier);
      if (found) {
        found.hits++;
      } else {
  ...
  _scheduleDecision() {
    ...
    this.decisionTimeout = this.device?.homey?.setTimeout?.(() => {
      this._makeDecision();
    }, this.options.decisionDelay) || this.homey.setTimeout(() => {
  ...
  _makeDecision() {
    ...
    if (discoveredCaps.length > 0) {
      ...
      this._log('⚠️ No capabilities discovered yet');
      this._log('');
    }
   ...
   * ponytail: ALL_ACTIVE - switch is kept for future per-protocol query control; all branches set true
   */
  _applyDecision(mode, save = true) {
    ...
    case 'tuya_only':
      ...
      this._log('ℹ️ ZCL/IAS still active for passive data (v5.5.71)');
      break;
    ...
    case 'zcl_only':
      ...
      this._log('ℹ️ Tuya DP still active for passive data (v5.5.71)');
      break;
  ...
  async _loadSavedDecision() {
    try {
      ...
      // ponytail: STORE_READ_FAIL - device store may not exist yet; re-learn from scratch
    }
    return null;
  }
