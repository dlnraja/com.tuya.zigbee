...
const { findDPProfile, getDPInfo } = require('./tuya/TuyaDPDatabase');
...
const DP_CAPABILITY_PATTERNS = {
  ...
  percentage: {
    ...
    soil_moisture: 'measure_humidity.soil',
    dim: 'dim',
...
const COMMON_DP_MAPPINGS = {
  ...
  3: { likely: ['measure_humidity', 'measure_humidity.soil', 'measure_luminance'], context: 'measurement' },
...
class IntelligentDeviceLearner {
   ...
   */
  async initialize() {
    this.log('[LEARNER] 🧠 Initializing Intelligent Device Learner v5.5.336 with Auto-Discovery');

    // 1. PHASE 1: Auto-Discovery via TuyaDPDatabase
    this._loadDPProfileFromDatabase();
  ...
  /**
   * Auto-Discovery: Load precise mapping from TuyaDPDatabase
   */
  _loadDPProfileFromDatabase() {
    try {
      const mfr = this.device._manufacturer || this.device.getData()?.manufacturerName;
      const model = this.device._modelId || this.device.getData()?.modelId;
      if (!mfr) return;
      
      const profile = findDPProfile(mfr, model);
      if (profile && profile.dps) {
        this.log(`[LEARNER] 🌍 Auto-Discovery SUCCESS: Found profile '${profile.name}' for ${mfr}`);
        for (const [dpId, config] of Object.entries(profile.dps)) {
          if (config.capability) {
            this._observedCapabilities.add(config.capability);
            this._observedDPs.set(Number(dpId), {
              values: [],
              rawValues: [],
              timestamps: [],
              inferred: config.capability,
              valueType: null,
              min: Infinity,
              max: -Infinity,
              fromDatabase: true
            });
            this.log(`[LEARNER] 💉 Auto-Injected DP${dpId} → ${config.capability}`);
          }
        }
      }
    } catch (e) {
      this.log(`[LEARNER] ⚠️ Auto-Discovery failed: ${e.message}`);
    }
  }
  ...
  recordDP(dpId, value, rawValue) {
    ...
    if (this._isActiveLearning && !record.fromDatabase && record.values.length >= MIN_SAMPLES_FOR_INFERENCE) {
  ...
  async _addCapabilityIfValid(capability) {
    try {
      ...
      const validCapabilities = [
        ...
        'measure_humidity.soil', 'measure_voc', 'measure_co2', 'measure_pm25',
