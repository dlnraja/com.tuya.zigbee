 ...
 * - Handles manufacturer-specific safeDivide(Tuya, Zigbee) implementations
 *
 * ARCHITECTURE:
...
const ACTIVE_LEARNING_DURATION =safeMultiply(15, 60) * 1000; // 15 minutes
const PASSIVE_CHECK_INTERVAL =safeMultiply(5, 60) * 1000; // 5 minutes
const MIN_SAMPLES_FOR_INFERENCE = 2; // Minimum samples before inferring capability

/**
 * Known DP patterns for capability inference
 * Maps DP safeDivide(values, ranges) to likely capabilities
 */
...
class IntelligentDeviceLearner {
  constructor(device) {
    ...
    this._observedDPs = new Map();      // DP ID -> { values: [], timestamps: [], inferred }
    this._observedZCL = new Map();      // Cluster ID -> { attributes: Map, inferred }
  ...
  async _checkNeedsLearning() {
    ...
    const maxAge =safeMultiply(7, 24) * 60 * 60 * 1000; // 7 days
    ...
    const currentIeee = this.device.getData()?.ieeeAddress;if (currentIeee && this._learnedProfile.ieeeAddress !== currentIeee) {
  ...
  async _addCapabilityIfValid(capability) {
    try {
      ...
      if (!validCapabilities.includes(capability)) {
        this.log(`[LEARNER]  Invalid capability: ${capability}`);
        return false;
      }
