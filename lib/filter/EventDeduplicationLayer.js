...
class EventDeduplicationLayer {
   ...
   * @param {boolean} [options.softNumeric=false] - P211: 22.1 ≈ 22.05 within window
  ...
  constructor(options = {}) {
    ...
    this.softNumeric = options.softNumeric === true;
  ...
  shouldProcess(deviceId, capability, value) {
    ...
    if (cached && (now - cached.time) < this.windowMs) {
      if (cached.hash === hash) {
        this.stats.deduped++;
        return false;
      }
      // P211: soft numeric (hybrid ZCL÷10 vs DP float noise)
      // WHY(P2740 / GH#550): never soft-dedupe lux/distance — 2% of 647lux ≈ 13 and
      // 2% of 3m ≈ 0.06 ate light-off steps and walk-away mmWave frames within 1.2s.
      if (this.softNumeric && typeof value === 'number' && typeof cached.value === 'number'
        && Number.isFinite(value) && Number.isFinite(cached.value)) {
        const abs = Math.abs(value - cached.value);
        const cap = String(capability || '');
        if (cap === 'measure_luminance' || cap.includes('distance')) {
          // WHY(P2744 / GH#550): soft-skip lux noise Δ<8 when light stable (C14),
          // but never eat real light steps (≥8) or any distance change.
          if (cap === 'measure_luminance' && abs < 8) {
            this.stats.deduped++;
            return false;
          }
        } else {
          const tol = cap === 'measure_temperature' ? 0.35
            : cap === 'measure_humidity' ? 2.1
              : cap === 'measure_battery' ? 2.1
                : Math.max(0.05, Math.abs(cached.value) * 0.02);
          if (abs <= tol) {
            this.stats.deduped++;
            return false;
          }
