...
class SanityFilter extends EventEmitter {
   ...
   */
  /**
   * v9.0.49 BUG FIX — _getROCLimit() était appelé mais JAMAIS défini.
   * Retourne la limite de Rate-of-Change (unité/seconde) physiquement plausible.
   */
  _getROCLimit(capability) {
    if (!capability || typeof capability !== 'string') {return Infinity;}
    if (capability.includes('temperature')) {return 0.5;}
    if (capability.includes('humidity')) {return 5;}
    if (capability.includes('distance')) {return 5;}
    if (capability.includes('luminance')) {return 2000;}
    if (capability.includes('pressure')) {return 1;}
    if (capability.includes('co2') || capability.includes('carbon')) {return 100;}
    if (capability.includes('power')) {return 500;}
    if (capability.includes('voltage')) {return 10;}
    if (capability.includes('current')) {return 10;}
    return Infinity;
  }
  ...
  filter(deviceId, capability, rawValue) {
    ...
    let elapsedSec = (now - lastTime) / 1000;

    // v9.0.49 BUG FIX — edge case même-millisecond.
    // Date.now() a une résolution de 1ms. Deux appels synchrones obtiennent le
    // MÊME timestamp → elapsedSec=0 → check ROC skipé → pics fantômes non filtrés.
    // Fix nuancé : on applique un delta minimal (0.001s) SEULEMENT pour les grands
    // écarts (suspects). Une petite variation légitime (21→21.5°C) dans la même ms
    // ne doit PAS être traitée comme un pic — on garde elapsedSec=0 (skip ROC).
    const absDelta = Math.abs(rawValue - lastValue);
    if (elapsedSec === 0 && absDelta > 1.0) {
      elapsedSec = 0.001;
    }
    ...
    if (elapsedSec > 0 && elapsedSec < 3600) { // Only check if last update was within 1 hour
      ...
      // WHY(P2730 / GH#550): 5 m/s was too tight for mmWave DP9 (0.6m @ 100ms → roc=6)
      // Contre quoi: distance UI dipped then rose while walking away (discarded frames).
      if (capability.includes('distance') && roc > 12) {
        this._reportDiscard(deviceId, capability, rawValue, lastValue, 'ROC_LIMIT_DISTANCE');
        return lastValue;
      }

      // Luminance: allow large DOWNWARD jumps (light off) — only block upward spikes.
      // WHY(P2730 / GH#550 HiepSVG): lux lagged several seconds on light-off while
      // light-on felt snappy — ROC discarded 647→50 in <200ms frames.
      // Note: same-ms path forces elapsedSec=0.001 → roc huge; drops must skip ROC.
      if (capability === 'measure_luminance') {
        const isDrop = rawValue < lastValue;
        const abs = Math.abs(rawValue - lastValue);
        // WHY(P2744 / GH#550 @ 9.0.1250): same-ms / sub-ms frames force elapsedSec=0.001
        // → roc explodes → light-ON (and large light-OFF without isDrop path) discarded,
        // while small stable noise (Δ3–5 every 400ms+) still painted. Allow real steps.
        if (isDrop || abs >= 15) {
          if (isDrop) state.ema = rawValue;
        } else if (roc > 2000) {
    ...
    if (history.length >= 3) {
      ...
      if (diffFromMedian > threshold) {
        // WHY(P2730): lux/light-off and distance walk-away are real step changes —
        // do not pin to median (felt "laggy / wrong direction").
        const isLuxOrDistance = capability === 'measure_luminance'
          || capability.includes('distance');
        if (!isLuxOrDistance) {
