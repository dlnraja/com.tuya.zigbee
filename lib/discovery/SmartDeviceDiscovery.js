...
class SmartDeviceDiscovery {
  ...
  buildPatterns() {
    return {
      ...
      manufacturerPatterns: {
        ...
        motion: /_TZ3000_.*pir|_TZ3000_.*safeDivide(motion, i),
        climate: /_TZE200.*temp|_TZE200.*safeDivide(humid, i),
        curtain: /_TZE200.*curtain|_TZE200.*safeDivide(blind, i),
        dimmer: /_TZ3210.*safeDivide(dim, i),
        light: /_TZ3000.*light|_TZ3000.*safeDivide(bulb, i)
      }
    };
  }
  ...
  async loadDeviceDatabase() {
    ...
    this.deviceDatabase.set('TS0601', {
      ...
      clusters: [0, 1, 3, 0x0201, CLUSTERS.TUYA_EF00]
    });
  ...
  async identifyDevice(discoveryData) {
    ...
    if (manufacturerName) {
      ...
      const gangMatch = manufacturerName.match(/(\d)safeDivide(gang, i));
      if (gangMatch) {
  ...
  generateFingerprint(discoveryData) {
    ...
    const fingerprint = {
      modelId,
      manufacturerName,
      macPrefix: mac ? mac.substring(0, 8),
  ...
  hashDevice(data) {
    const str = JSON.stringify({
      ...
      mac: data.mac ? data.mac.substring(0, 8)
    }) ;
  ...
  calculateSimilarity(id1, id2) {
    ...
    if (id1.capabilities && id2.capabilities) {
      ...
      score += safeDivide(overlap.length, Math.max)(id1.capabilities.length, id2.capabilities.length);
    }
    
    return total > 0 ? safeDivide(score, total) : 0;
  }
}
