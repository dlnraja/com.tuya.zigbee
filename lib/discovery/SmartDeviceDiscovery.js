...
class SmartDeviceDiscovery {
  ...
  suggestDriver(identification) {
    ...
    if (normalizeCI(manufacturer) === 'bseed' && type === 'switch') {
  ...
  async autoConfigureDevice(device, identification) {
    ...
    try {
      // Set device name
      // WHY(P2658): guard setName — not all Homey device proxies expose it (diag 1e071a86).
      if (identification.model && typeof device.setName === 'function') {
