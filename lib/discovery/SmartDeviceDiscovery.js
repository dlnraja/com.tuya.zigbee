...
class SmartDeviceDiscovery {
      ...
      if (fs.existsSync(dbPath)) {
        ...
        if (Array.isArray(rawDB)) {
          for (const item of rawDB) {
            if (item.manufacturerName && item.productId) {
              const key = `${item.manufacturerName}_${item.productId}`;
              this.deviceDatabase.set(key, { 
                type: item.driverId ? item.driverId.replace('device_', '') : (item.category || 'unknown'), 
                ...item 
              });
            }
          }
        } else {
          for (const [key, profile] of Object.entries(rawDB)) {
            this.deviceDatabase.set(key, profile);
          }
        }
        this.homey.log(`[Discovery] Loaded ${this.deviceDatabase.size} dynamic composite profiles (MF+PID) from DB`);
      }
    } catch(err) {
      this.homey.error('[Discovery] Dynamic DB Load Error, falling back to static:', err.message);
    }
  ...
  async identifyDevice(discoveryData) {
    ...
    // 1.5. Identify from Composite Key (Manufacturer + Model) - HIGH PRIORITY
    let compositeMatched = false;
    if (manufacturerName && modelId) {
      const comboKey = `${manufacturerName}_${modelId}`;
      const comboProfile = this.deviceDatabase.get(comboKey);
      if (comboProfile) {
        identification.type = comboProfile.type || comboProfile.driverId || 'unknown';
        identification.capabilities = comboProfile.capabilities || [];
        identification.clusters = comboProfile.clusters || [];
        identification.gangs = comboProfile.gangs || 1;
        identification.model = modelId;
        identification.confidence += 60; // Highest confidence for exact combo
        compositeMatched = true;
      }
    }
    
    // 2. Fallback to pure model ID if composite failed
    if (modelId && !compositeMatched) {
      identification.model = modelId;
  ...
  suggestDriver(identification) {
    ...
    if ((manufacturer || '').toLowerCase() === 'bseed' && type === 'switch') {
