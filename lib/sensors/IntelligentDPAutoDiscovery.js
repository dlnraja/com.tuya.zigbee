...
class IntelligentDPAutoDiscovery {
  constructor(device) {
    ...
    // Extract context for smarter inference
    const data = this.device.getData() || {};
    const settings = this.device.getSettings() || {};
    this.productId = data.productId || data.modelId || settings.zb_model_id || settings.product_id || '';
    this.manufacturerName = data.manufacturerName || settings.zb_manufacturer_name || '';
    this.deviceClass = this.device.getClass();
    
    // Special combination flags
    this.isMotionCombo = /motion|pir/i.test(this.manufacturerName) || ['TS0202'].includes(this.productId);
    this.isContactCombo = /contact|door/i.test(this.manufacturerName) || ['TS0203'].includes(this.productId);
    this.isDimmerCombo = /dimmer|dim/i.test(this.manufacturerName) || ['TS110E', 'TS110F', 'TS0601_dimmer'].includes(this.productId);
    this.isClimateCombo = /temp|humid/i.test(this.manufacturerName) || ['TS0201', 'TS0601_climate'].includes(this.productId);
  }
  ...
  _inferDPType(dpId, dpInfo) {
    ...
    if (isBooleanPattern) {
      ...
      // Assign capabilities heuristically based on device class, DP ID, or composite context
      if (this.isContactCombo || (this.deviceClass === 'sensor' && (dpId === 1 || dpId === 101))) {
         dpInfo.inferredCapability = 'alarm_contact'; // Bed sensor, door, etc.
      } else if (this.isMotionCombo) {
         dpInfo.inferredCapability = 'alarm_motion';
      } else if (this.deviceClass === 'light' || this.isDimmerCombo) {
         dpInfo.inferredCapability = 'onoff';
      } else {
         dpInfo.inferredCapability = 'alarm_motion'; // Default heuristic fallback
      }
