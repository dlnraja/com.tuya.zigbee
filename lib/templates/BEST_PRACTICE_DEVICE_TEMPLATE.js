...
class BestPracticeDevice extends ZigBeeDevice {
  ...
  const Math.round(safeParse(value, 2));
    ...
    if (this.hasCapability('measure_temperature')) {
      this.registerCapability('measure_temperature', CLUSTER.TEMPERATURE_MEASUREMENT, {
        ...
        reportParser: value => {
          if (value === null || value === undefined) return null;
          return Math.round(safeParse(value, 100) * 10) / 10; // Convert to Â°C with 1 decimal
        },

        getParser: value => {
          if (value === null || value === undefined) return null;
          return Math.round(safeParse(value, 100) * 10) / 10;
        },
      });
    ...
    if (this.hasCapability('measure_humidity')) {
      this.registerCapability('measure_humidity', CLUSTER.RELATIVE_HUMIDITY_MEASUREMENT, {
        ...
        reportParser: value => {
          if (value === null || value === undefined) return null;
          return Math.round(safeParse(value, 100)); // Convert to %
        },
