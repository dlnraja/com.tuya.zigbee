...
class TuyaDPEngine {
  constructor(device) {
    ...
    // v9.0.35: Lazy-load configuration — defer until first access to reduce startup cost
  }

  /**
   * Load configuration files (lazy, Buffer-based to avoid UTF-16 OOM)
   */
  _ensureConfig() {
    ...
    try {
      ...
      // v9.0.35: Use Buffer-based reads (no 'utf8') to halve heap footprint
      this.fingerprints = JSON.parse(
        fs.readFileSync(path.join(basePath, 'fingerprints.json'))
      ).fingerprints;

      this.profiles = JSON.parse(
        fs.readFileSync(path.join(basePath, 'profiles.json'))
      ).profiles;

      this.capabilityMap = JSON.parse(
        fs.readFileSync(path.join(basePath, 'capability-map.json'))
      ).capabilities;
    } catch (err) {
  ...
  async initialize() {
    try {
      // v9.0.35: Lazy-load config on first initialize() call
      this._ensureConfig();
