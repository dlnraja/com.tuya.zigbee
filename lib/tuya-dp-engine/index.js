...
class TuyaDPEngine {
  ...
  async getManufacturerId() {
    try {
      if (this.device.zclNode && this.device.zclNode.endpoints[1]) {
        ...
        if (basic) {
          const attrs = await basic.readAttributes(['manufacturerName']);
  ...
  async setupCapabilityListener(capability) {
    ...
    this.device.registerCapabilityListener(capability, async (value) => {
      return await this.setCapability(capability, value);
    });
  }
  ...
  async setCapability(capability, value) {
    try {
      ...
      // Send to device
      await this.sendDP(dpConfig.dp, dpValue, dpConfig.type);
  ...
  async handleDPReport(dp, value, type) {
    try {
      ...
      // Update capability
      await this.device.setCapabilityValue(capability, homeyValue);
