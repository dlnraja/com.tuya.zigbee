...
class AutonomousVerificationEngine {
   ...
   */
  _categorize(err) {
    // P2228: never require tools/ci from Homey runtime (tools/ is .homeyignore)
    const msg = String(err?.message || err || '').toLowerCase();
    if (/timeout|econnreset|enetunreach|temporarily/.test(msg)) {
      return { type: 'transient_network', severity: 'WARN', isTransient: true, suggestedFix: 'retry' };
    }
    if (/out of memory|heap|allocation failed/.test(msg)) {
      return { type: 'memory', severity: 'ERROR', isTransient: false, suggestedFix: 'reduce_json_heap' };
    }
    if (/aggregateerror|manufacturername/.test(msg)) {
      return { type: 'aggregate_mfr', severity: 'ERROR', isTransient: false, suggestedFix: 'validate_mfr' };
    }
    try {
      // Optional CI-only enrich when tools/ is present (local agent / GHA — not Homey)
