...
class OTAManager {
  ...
  async initialize() {
    ...
    try {
      ...
      if (!supported) {
        this.device.log('ℹ️  Device does not support OTA updates');
        return;
      }
      ...
      this.device.log('✅ OTAManager initialized successfully');
  ...
  async getCurrentVersion() {
    try {
      ...
      const result = await otaCluster.readAttributes(['currentFileVersion']).catch(() => null);
  ...
  async setupOTAListener() {
    try {
      ...
      this.device.log('✅ OTA cluster listener active');
  ...
  async handleUpgradeEnd(data) {
    ...
    // Notify user
    await this.device.setWarning('Firmware updated successfully! Device may restart.').catch(() => {});
  }
  ...
  async handleImageNotify(data) {
    ...
    // Notify user
    await this.device.setWarning('Firmware update available for this device').catch(() => {});
  }
  ...
  async checkForUpdates() {
    this.device.log('🔍 Checking for OTA updates...');
    ...
    try {
      ...
      if (result && result.status === 0) {
        ...
        this.device.log(`✅ Update available: ${result.fileVersion}`);
