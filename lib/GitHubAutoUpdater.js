...
class GitHubAutoUpdater {
  ...
  async checkForUpdates() {
    console.log('[SYNC] Checking GitHub sources for updates...');
    ...
    for (const [name, url] of Object.entries(this.sources)) {
      try {
        ...
        if (!this.cache.versions[name] || this.cache.versions[name].sha !== latestCommit.sha) {
          console.log(`[OK] New update available from ${name}`);
  ...
  async fetchTuyaDataPoints() {
    console.log('[RECV] Fetching Tuya Data Points from GitHub...');
    ...
    for (const url of sources) {
      try {
        ...
        dataPoints.push(...dps);
        console.log(`[OK] Fetched ${dps.length} DPs from ${url}`);
      } catch (err) {
  ...
  async fetchZigpyQuirks() {
    console.log('[RECV] Fetching zigpy quirks...');
    ...
    try {
      ...
      console.log(`[OK] Found ${quirkFiles.length} quirk files`);
