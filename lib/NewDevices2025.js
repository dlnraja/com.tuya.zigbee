...
const NEW_OTHER_2025 = {
  ...
  ikea: {
    'IKEA of Sweden': {
      ...
      'E2216': { model: 'TRÃ…DFRI LED1837R5 (2025)', type: 'light_dimmable', driver: 'bulb_dimmable' },
      'E2217': { model: 'STOFTMÃ„NGD Air Purifier', type: 'air_purifier', driver: 'air_quality_comprehensive' }
    }
  },
...
function lookupByManufacturer(manufacturerName) {
  ...
  for (const category of Object.values(NEW_TUYA_2025)) {
    for (const [mfr, config] of Object.entries(category)) {
      if (CI.equalsCI(mfr, manufacturerName)) {
        return { type: 'tuya', ...config };
      }
    }
  }
...
function lookupByModel(modelId) {
  ...
  for (const [category, devices] of Object.entries(NEW_TUYA_2025)) {
    for (const [mfr, device] of Object.entries(devices)) {
      if (CI.equalsCI(device.model || '', modelId)) {
  ...
  for (const [brand, manufacturers] of Object.entries(NEW_OTHER_2025)) {
    for (const [mfr, models] of Object.entries(manufacturers)) {
      for (const [model, device] of Object.entries(models)) {
        if (CI.equalsCI(model, modelId) || CI.equalsCI(device.model || '', modelId)) {
...
function isNew2025Device(manufacturerName, modelId) {
  // Check for _TZE284 prefix (2025 Tuya) - case-insensitive
  if (CI.startsWithCI(manufacturerName, '_TZE284')) return true;

  // Check our database
  return !!lookupByManufacturer(manufacturerName) || !!lookupByModel(modelId);
}
