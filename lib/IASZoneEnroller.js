...
class IASZoneEnroller {
    ...
    try {
      ...
      this.endpoint.clusters.iasZone.onZoneEnrollRequest = () => {
        ...
        try {
          ...
          this.log('[OK] Zone Enroll Response sent (zoneId: ' + this.options.zoneId + ')');
          this.enrolled = true;
      ...
      this.log('[SEND] Sending proactive Zone Enroll Response (official fallback)...');
      ...
      this.error('[WARN] Zone enroll listener setup failed:', err.message);
      return false;
    }
  }
  ...
  setupListeners() {
    ...
    try {
      ...
      this.log('[OK] Status listeners configured');
    } catch (err) {
  ...
  handleZoneStatus(zoneStatus) {
    try {
      ...
      this.log(alarmTriggered ? '[ALARM] ALARM TRIGGERED' : '[OK] Alarm cleared');
  ...
  async enroll(zclNode) {
    this.log('[START] Starting IAS Zone enrollment (simple method)...');
    
    // Setup the official Homey method (listener + proactive response)
    if (this.setupZoneEnrollListener()) {
      this.log('[OK] Zone Enroll listener configured (official method)');
      
      // Setup status listeners
      this.setupListeners();
      
      this.log('[OK] Enrollment complete (method: ' + this.enrollmentMethod + ')');
      return this.enrollmentMethod;
    }
    
    this.error('[ERROR] Enrollment failed');
    return null;
  }
