...
/**
 * IAS ZONE ENROLLMENT HANDLER v3.1.18 - PRODUCTION CRASH FIXES
 * 
 ...
 * 
 * CRITICAL FIXES v3.1.18 (2025-10-21):
 * - Prevent "Missing Zigbee Node" crashes during startup
 * - Prevent "Zigbee is aan het opstarten" crashes
 * - Async error handling for zoneEnrollResponse
 * - 500ms stabilization delay before enrollment
...
class IASZoneEnroller {
  ...
  setupZoneEnrollListener() {
    ...
    try {
      ...
      this.endpoint.clusters.iasZone.onZoneEnrollRequest = async () => {
        ...
        try {
          // CRITICAL: Wait a bit for Zigbee to stabilize
          ...
          // CRITICAL: Check if Zigbee is still starting up
          if (!this.device.zclNode.endpoints || Object.keys(this.device.zclNode.endpoints).length === 0) {
            this.error('⚠️ Zigbee still starting, deferring enrollment');
            // Retry after delay
            setTimeout(() => {
              if (this.endpoint.clusters.iasZone && this.endpoint.clusters.iasZone.onZoneEnrollRequest) {
          ...
          await this.endpoint.clusters.iasZone.zoneEnrollResponse({
            ...
            if (err.message.includes('opstarten') || err.message.includes('starting')) {
              this.log('⚠️ Zigbee not ready yet, will retry automatically');
            } else if (err.message.includes('Missing Zigbee Node')) {
              this.error('❌ Zigbee node missing, cannot enroll');
            } else {
          ...
          // CRITICAL: Don't crash the app, just log the error
          this.error('⚠️ Failed to send Zone Enroll Response:', err.message);
          this.log('💡 Enrollment will retry automatically or use fallback methods');
        }
      };
      ...
      try {
        ...
        // CRITICAL: Additional check for endpoints availability
        if (!this.device.zclNode.endpoints || Object.keys(this.device.zclNode.endpoints).length === 0) {
          this.log('⚠️ Zigbee endpoints not ready, skipping proactive response');
          return true;
        }
        
        // Wait a bit before sending to ensure Zigbee is stable
        ...
        await this.endpoint.clusters.iasZone.zoneEnrollResponse({
          ...
          if (err.message.includes('opstarten') || err.message.includes('starting')) {
            this.log('⚠️ Zigbee still starting, enrollment will be retried');
          } else if (err.message.includes('Missing Zigbee Node')) {
            this.log('⚠️ Node not available yet');
          } else {
            throw err;
          }
        });
        ...
        // CRITICAL: Don't crash the app - these errors are recoverable
        if (err.message && (err.message.includes('Missing Zigbee Node') || 
                           err.message.includes('opstarten') || 
                           err.message.includes('starting'))) {
          this.log('⚠️ Zigbee not ready (will retry with fallback methods):', err.message);
        } else {
          this.log('⚠️ Proactive response failed (normal if device not ready):', err.message);
        }
        // Don't re-throw - let fallback methods handle enrollment
      }
  ...
  /**
   * ENHANCED IEEE ADDRESS RETRIEVAL - 7 METHODS
   * Fixes "IEEE address not available" bug reported by users
   */
  async getIEEEAddress(zclNode) {
    this.log('🔍 Attempting IEEE address retrieval (7 methods)...');
    
    // Method 1: Direct from zclNode
    if (zclNode && zclNode.ieeeAddress) {
      this.log('✅ Method 1: zclNode.ieeeAddress');
      return zclNode.ieeeAddress;
    }
    
    // Method 2: From device getData (stored)
    try {
      const deviceData = this.device.getData();
      if (deviceData && (deviceData.ieeeAddress || deviceData.ieee)) {
        const ieee = deviceData.ieeeAddress || deviceData.ieee;
        this.log('✅ Method 2: getData().ieeeAddress');
        return ieee;
      }
    } catch (e) {
      this.log('Method 2 failed:', e.message);
    }
    
    // Method 3: From zclNode object properties
    if (zclNode && (zclNode.ieee || zclNode.ieeeAddr)) {
      const ieee = zclNode.ieee || zclNode.ieeeAddr;
      this.log('✅ Method 3: zclNode.ieee/ieeeAddr');
      return ieee;
    }
    
    // Method 4: From _node.bridgeId (original method)
    if (zclNode && zclNode._node && zclNode._node.bridgeId) {
      this.log('✅ Method 4: _node.bridgeId');
      return zclNode._node.bridgeId;
    }
    
    // Method 5: From device API
    if (typeof this.device.getIeeeAddress === 'function') {
      try {
        const ieee = this.device.getIeeeAddress();
        if (ieee) {
          this.log('✅ Method 5: device.getIeeeAddress()');
          return ieee;
        }
      } catch (e) {
        this.log('Method 5 failed:', e.message);
      }
    }
    
    // Method 6: From store keys
    if (this.device.getStoreKeys && typeof this.device.getStoreKeys === 'function') {
      try {
        const keys = this.device.getStoreKeys();
        if (keys.includes('ieeeAddress') || keys.includes('ieee')) {
          const ieee = this.device.getStoreValue('ieeeAddress') || this.device.getStoreValue('ieee');
          if (ieee) {
            this.log('✅ Method 6: store.ieeeAddress');
            return ieee;
          }
        }
      } catch (e) {
        this.log('Method 6 failed:', e.message);
      }
    }
    
    // Method 7: From network coordinator (last resort)
    if (zclNode && zclNode._node && zclNode._node.networkAddress) {
      try {
        // Some devices expose IEEE via network discovery
        this.log('⚠️ Method 7: Attempting network lookup...');
        // This is a placeholder - actual implementation depends on Homey API
      } catch (e) {
        this.log('Method 7 failed:', e.message);
      }
    }
    
    this.error('❌ All 7 IEEE address methods failed');
    return null;
  }
  ...
  async enrollStandard(zclNode) {
    ...
    try {
      ...
      // Method 2: Get Homey IEEE using ENHANCED 7-method retrieval
      const bridgeId = await this.getIEEEAddress(zclNode);
