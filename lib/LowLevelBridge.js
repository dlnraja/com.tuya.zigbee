...
const ZCL_CLUSTERS = {
  ...
  // Tuya manufacturer clusters — Z2M canonical names (P2267 Google sweep)
  // manuSpecificTuya  = 0xEF00 DP tunnel
  // manuSpecificTuya2 = 0xE002 climate/LCD alarms (+ some OEM radar attrs)
  // manuSpecificTuya3 = 0xE001 power-on / switch mode
  // manuSpecificTuya4 = 0xE000 device options / button frames
  manuSpecificTuya1: 0xEF00,
  manuSpecificTuya2: 0xE002,
  manuSpecificTuya3: 0xE001,
  manuSpecificTuya4: 0xE000,
  // Legacy Homey aliases (pre-P2267 inverted numbering) — keep for old callers
  manuSpecificTuya2LegacyE001: 0xE001,
  manuSpecificTuya3LegacyE002: 0xE002,
...
function buildZclFrame(clusterId, cmd, payload, options = {}) {
  // v10.7.0 FIX: per ZCL spec 07-5123 §2.3.1 the manufacturer code sits
  // BETWEEN frame control and TSN (not after cmd), LITTLE-ENDIAN (not BE).
  ...
  if (frameControl & ZCL_FRAME_CONTROL.MANUFACTURER_SPECIFIC) {
    ...
    header.writeUInt16LE(options.manufacturerCode || 0, 1);
    header.writeUInt8(transactionSeq, 3);
...
function parseZclFrame(buf) {
  ...
  try {
    ...
    // v10.7.0 FIX: mfr code between fc and TSN, little-endian (spec).
    let transactionSeq, cmd, payload, manufacturerCode = null;
    if (manufacturerSpecific) {
      if (buf.length < 5) {return null;}
      manufacturerCode = buf.readUInt16LE(1);
      transactionSeq = buf.readUInt8(3);
      cmd = buf.readUInt8(4);
      payload = buf.slice(5);
    } else {
      transactionSeq = buf.readUInt8(1);
      cmd = buf.readUInt8(2);
      payload = buf.slice(3);
    }
    ...
    try {
      ...
      for (const [epId, ep] of Object.entries(this.zclNode.endpoints)) {
        ...
        if (typeof cluster.readAttributes === 'function') {
          try {
            ...
            if (attrs.batteryPercentageRemaining !== undefined) {
              ...
              const v = Number(attrs.batteryPercentageRemaining);
        ...
        if (cluster.batteryPercentageRemaining !== undefined) {
          const v = Number(cluster.batteryPercentageRemaining);
...
module.exports = {
  ...
  buildZclFrame,
  parseZclFrame,
  // P102: ordered I/O fallback (Homey compensation) — optional soft require
  get ProtocolFallbackChain() {
    try { return require('./io/ProtocolFallbackChain'); } catch (_e) { return null; }
  },
  get HomeyCompensationLayer() {
    try { return require('./io/HomeyCompensationLayer'); } catch (_e) { return null; }
  },
  /** Aligns with data/protocol_quirk_table.json fallbackOrder.tx */
  IO_TX_FALLBACK_ORDER: Object.freeze([
    'wrapper_manager',
    'sdk3_direct',
    'quirk_guided',
    'raw_zcl_frame',
