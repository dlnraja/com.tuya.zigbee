...
class ParallelDetector {
  ...
  async _fingerprintLookup() {
    ...
    try {
      // WHY(P2678): never JSON.parse full fingerprints.json — use DeviceFingerprintDB shards
      const { getFingerprint } = require('../tuya/DeviceFingerprintDB');
      const entry = getFingerprint(mfr, pid);
      if (entry) {
        return {
          driver: entry.driverId || entry.driver || entry.type || null,
