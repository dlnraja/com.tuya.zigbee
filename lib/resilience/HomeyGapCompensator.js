...
function ensureDeviceCompensations(device) {
  ...
  // P2542: note local self-improve stacks (zero cloud AI) — soft only
  try {
    const { listLocalStacks } = require('../features/LocalSelfImproveCatalog');
    if (!device._p2542LocalStacksNoted && typeof device.log === 'function') {
      device._p2542LocalStacksNoted = true;
      device.log(`[P2542] local stacks=${listLocalStacks().length} (no cloud AI)`);
      notes.push('local-self-improve-catalog');
    }
  } catch { /* optional */ }
  ...
  // WHY(P2748): ensure tagged Flow emit on complementary/raw setCapabilityValue
  // paths — wrap only (do NOT fleet softArm raw/EF00 here; memory on Homey).
  try {
    ...
    if (wr?.wrapped) notes.push('p2748-tagged-wrap');
    else if (wr?.ok) notes.push('p2748-tagged-ready');
  } catch { /* optional */ }
