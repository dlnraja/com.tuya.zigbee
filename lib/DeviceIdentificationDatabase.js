...
class DeviceIdentificationDatabase {
  ...
  /**
   * Build complete database by scanning ALL drivers
   */
  async buildDatabase() {
    this.log('🤖 [ID DATABASE] Building intelligent device identification database...');
    ...
    try {
      ...
      if (!fs.existsSync(driversPath)) {
        this.error('❌ [ID DATABASE] Drivers directory not found:', driversPath);
        return;
      }
      
      const driverDirs = fs.readdirSync(driversPath)
        .filter(dir => {
          const fullPath = path.join(driversPath, dir);
          return fs.statSync(fullPath).isDirectory();
        });
      
      this.log(`📂 [ID DATABASE] Found ${driverDirs.length} driver directories`);
      
      // Scan each driver
      for (const driverDir of driverDirs) {
        try {
          const driverData = await this._scanDriver(driverDir, driversPath);
          
          if (driverData) {
            scannedDrivers++;
            
            // Add to database
            const deviceType = this._detectDeviceType(driverDir, driverData);
            ...
            this.database.driverMappings[driverDir] = {
              deviceType,
            ...
            this.database.deviceClasses[driverDir] = driverData.class;
            this.database.capabilities[driverDir] = driverData.capabilities;
          }
        } catch (error) {
          // Skip driver on error, continue with others
          this.error(`⚠️  [ID DATABASE] Error scanning driver ${driverDir}:`, error.message);
        }
      }
      ...
      this.log('✅ [ID DATABASE] Database built successfully!');
      this.log(`   📊 Drivers scanned: ${scannedDrivers}`);
      this.log(`   🏷️  Manufacturer IDs: ${totalManufacturerIds}`);
      this.log(`   📦 Product IDs: ${totalProductIds}`);
      this.log(`   🎯 Device types: ${Object.keys(this.database.manufacturerIds).length}`);
      this.log(`   ⏱️  Duration: ${duration}ms`);
      
      // Log sample data
      this._logDatabaseSample();
  ...
  /**
   * Scan a single driver and extract identification data
   */
  async _scanDriver(driverDir, driversPath) {
    ...
    try {
      ...
      if (composeData.zigbee && composeData.zigbee.manufacturerName) {
        if (Array.isArray(composeData.zigbee.manufacturerName)) {
          manufacturerNames = composeData.zigbee.manufacturerName;
        } else {
          manufacturerNames = [composeData.zigbee.manufacturerName];
        }
      }
      ...
      if (composeData.zigbee && composeData.zigbee.productId) {
        if (Array.isArray(composeData.zigbee.productId)) {
          productIds = composeData.zigbee.productId;
        } else {
          productIds = [composeData.zigbee.productId];
        }
      }
      ...
      return {
        ...
        manufacturerNames,
        productIds,
        name: composeData.name ? composeData.name.en : driverDir
      };
      
    } catch (error) {
      this.error(`❌ [ID DATABASE] Error reading ${driverDir}:`, error.message);
      return null;
    }
  }
  ...
  /**
   * Find matching driver by manufacturer name
   */
  findDriverByManufacturer(manufacturerName) {
    for (const [driverId, mapping] of Object.entries(this.database.driverMappings)) {
      if (mapping.manufacturerNames && mapping.manufacturerNames.includes(manufacturerName)) {
        return {
          driverId,
  ...
  /**
   * Find matching driver by product ID
   */
  findDriverByProductId(productId) {
    for (const [driverId, mapping] of Object.entries(this.database.driverMappings)) {
      if (mapping.productIds && mapping.productIds.includes(productId)) {
        return {
          driverId,
          deviceType: mapping.deviceType,
          confidence: 0.90
  ...
  isManufacturerMatch(manufacturerName, deviceType) {
    ...
    return ids.some(id => manufacturerName.includes(id));
  }
  ...
  /**
   * Log database sample for debugging
   */
  _logDatabaseSample() {
    this.log('');
    this.log('📊 [ID DATABASE] Sample Data:');
    
    // Show USB outlet data as example
    if (this.database.manufacturerIds.usb_outlet) {
      this.log(`   🔌 USB Outlet Manufacturer IDs (${this.database.manufacturerIds.usb_outlet.length}):`);
      this.database.manufacturerIds.usb_outlet.slice(0, 5).forEach(id => {
        this.log(`      • ${id}`);
      });
      if (this.database.manufacturerIds.usb_outlet.length > 5) {
        this.log(`      ... and ${this.database.manufacturerIds.usb_outlet.length - 5} more`);
      }
    }
    
    // Show switch data as example
    if (this.database.manufacturerIds.switch_1gang) {
      this.log(`   🔘 Switch 1-Gang Manufacturer IDs (${this.database.manufacturerIds.switch_1gang.length}):`);
      this.database.manufacturerIds.switch_1gang.slice(0, 3).forEach(id => {
        this.log(`      • ${id}`);
      });
      if (this.database.manufacturerIds.switch_1gang.length > 3) {
        this.log(`      ... and ${this.database.manufacturerIds.switch_1gang.length - 3} more`);
      }
    }
