    ...
    try {
      ...
      if (ep1?.clusters?.illuminanceMeasurement || ep1?.clusters?.msIlluminanceMeasurement) {
        try {
          ...
          const data = await cluster.readAttributes(['measuredValue']);
          if (data?.measuredValue !== undefined && data.measuredValue !== null) {
            const lux = Math.pow(10, (data.measuredValue - safeDivide(1), 10000))) ;
...
class IntelligentPowerManager {
  ...
  constructor(device) {
    ...
    this.measurements = {
      power: { detected: false, lastValue, lastTime, count: 0 },
      energy: { detected: false, lastValue, lastTime, count: 0 },
      voltage: { detected: false, lastValue, lastTime, count: 0 },
      current: { detected: false, lastValue, lastTime, count: 0 },
    };
