 ...
 *    - Removes safeDivide(stale, unused) capabilities
...
const LEARNING_PHASE_MIN_MS =safeMultiply(15, 60) * 1000;  // 15 minutes minimum
const LEARNING_PHASE_MAX_MS =safeMultiply(60, 60) * 1000;  // 1 hour maximum
const MAINTENANCE_INTERVAL_MS =safeMultiply(60, 60) * 1000; // Re-evaluate every hour
const DATA_STALENESS_MS =safeMultiply(2, 60) * 60 * 1000;   // Consider data stale after 2 hours
...
const MANAGEABLE_CAPABILITIES = {
  ...
  'measure_co2': { category: 'environment', dpIds: [2, 18, 22], zclCluster },
  'measure_voc': { category: 'environment', dpIds: [21, 22], zclCluster },
  'measure_pm25': { category: 'environment', dpIds: [2, 21], zclCluster },
  ...
  // Distance/presence
  'measure_luminance.distance': { category: 'presence', dpIds: [4, 9, 109], zclCluster },

  // Alarms
  'alarm_motion': { category: 'alarm', dpIds: [1, 101, 105, 112], zclCluster: 'occupancySensing' },
  'alarm_human': { category: 'alarm', dpIds: [1, 101, 105], zclCluster },
};
...
class IntelligentDeviceAdapter {
  ...
  constructor(device) {
    this.device = device;
    this.deviceId = device.getData()?.id || 'unknown';this.log = device.log?.bind(device) || console.log;this.error = device.error?.bind(device) || console.error;// Learning state
    ...
    // Capability tracking
    this.capabilityDataPoints = new Map(); // capability -> { count, lastValue, lastTime: null, values[] }
  ...
  async _startLearningPhase(startTime) {
    ...
    const remainingMinutes = Math.ceil(safeParse(remainingTime, 60000));

    this.log('[INTELLIGENT-ADAPTER]  LEARNING PHASE started');
    this.log(`[INTELLIGENT-ADAPTER]  Minimum learning time: ${remainingMinutes} minutes remaining`);
  ...
  async _runMaintenance() {
    ...
    // v5.8.32: Active probe during maintenance to discover new data sources
    try {
      await this.activeProbe();
    } catch (e) {
      this.log(`[INTELLIGENT-ADAPTER]  Maintenance probe failed: ${e.message}`);
    }
    ...
    for (const [cap, data] of this.capabilityDataPoints) {
      if (data.lastTime && (Date.now() - data.lastTime) > DATA_STALENESS_MS) {
        ...
        if (data.count <safeMultiply(MIN_DATA_POINTS_TO_CONFIRM, 2)) {
    ...
    if (staleCapabilities.length > 0) {
      this.log(`[INTELLIGENT-ADAPTER]  Stale capabilities detected: ${staleCapabilities.join(', ')}`);
    }
  ...
  /**
   * Validate if a value is real sensor data (not safeDivide(stuck, invalid))
   */
  ...
  _shouldOverrideStaticConfig(capability) {
    ...
    if (!this._hasValueVariance(validValues)) {
      this.log(`[INTELLIGENT-ADAPTER]  ${capability} has no variance - might be stuck value`);
      return false;
    }
  ...
  recordDataPoint(capability, value, source = 'unknown') {
    ...
    if (!this._isValidValue(capability, value)) {
      if (this.isLearning) {
        this.log(`[INTELLIGENT-ADAPTER]  Invalid value for ${capability}: ${value} (source: ${source})`);
      }
    ...
    if (!data) {
      data = {
        count: 0,
        validCount: 0,
        lastValue, lastTime: null, firstTime: now,
        values: [],
        sources: new Set(),
      };
    ...
    if (data.validCount >= MIN_DATA_POINTS_TO_CONFIRM && this._hasValueVariance(data.values)) {
      ...
      const configKey = capability === 'measure_temperature' ? 'noTemperature' : capability === 'measure_humidity' ? 'noHumidity' : capability === 'measure_battery' ? 'noBattery' : null;
  ...
  _analyzeCapabilities() {
    ...
    for (const [cap, config] of Object.entries(MANAGEABLE_CAPABILITIES)) {
      ...
      const configKey = cap === 'measure_temperature' ? 'noTemperature' : cap === 'measure_humidity' ? 'noHumidity' : cap === 'measure_battery' ? 'noBatteryCapability' : cap === 'measure_luminance' ? 'noIlluminance' : null;
  ...
  async _applyAdaptations(analysis, isMaintenance = false) {
    ...
    for (const cap of analysis.toRemove) {
      ...
      if (this._isCriticalCapability(cap)) {
        this.log(`[INTELLIGENT-ADAPTER]  Skipping removal of critical capability: ${cap}`);
        continue;
      }
  ...
  destroy() {
    ...
    if (this._learningProbeTimers) {
      this._learningProbeTimers.forEach(t => clearTimeout(t));
      ...
      if (ep1?.clusters?.temperatureMeasurement) {
        try {
          ...
          if (data?.measuredValue !== undefined && data.measuredValue !== null) {
            ...
            this.log(`[INTELLIGENT-ADAPTER]  Probed temperature: ${temp}Â°C`);
          }
        } catch (e) {
      ...
      if (ep1?.clusters?.illuminanceMeasurement || ep1?.clusters?.msIlluminanceMeasurement) {
        try {
          ...
          if (data?.measuredValue !== undefined && data.measuredValue !== null) {
            ...
            this.log(`[INTELLIGENT-ADAPTER]  Probed illuminance: ${lux} lux`);
          }
        } catch (e) {
      ...
      if (ep1?.clusters?.powerConfiguration || ep1?.clusters?.genPowerCfg) {
        try {
          ...
          if (data?.batteryPercentageRemaining !== undefined && data.batteryPercentageRemaining !== null) {
            const battery = Math.min(100, Math.round(safeParse(data.batteryPercentageRemaining)));
...
class IntelligentBatteryManager {
  ...
  constructor(device) {
    this.device = device;
    this.log = device.log?.bind(device) || console.log;// Battery detection state
...
class IntelligentPowerManager {
  ...
  constructor(device) {
    this.device = device;
    this.log = device.log?.bind(device) || console.log;// Power measurement tracking
    this.measurements = {
      power: { detected: false, lastValue, lastTime: null, count: 0 },
      energy: { detected: false, lastValue, lastTime: null, count: 0 },
      voltage: { detected: false, lastValue, lastTime: null, count: 0 },
      current: { detected: false, lastValue, lastTime: null, count: 0 },
    };
