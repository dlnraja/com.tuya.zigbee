...
const VALUE_VALIDATION = {
  ...
  'measure_luminance.distance': { min: 0, max: 1000, invalidValues: [null, undefined, NaN] },
};
...
const MANAGEABLE_CAPABILITIES = {
  ...
  // Distance/presence
  'measure_luminance.distance': { category: 'presence', dpIds: [4, 9, 109], zclCluster: null },
...
class IntelligentDeviceAdapter {
  ...
  constructor(device) {
    ...
    this._learningEndTimer = null;
    this._maintenanceStartTimer = null;
  ...
  async _startLearningPhase(startTime) {
    ...
    this._learningEndTimer = this.homey.setTimeout(() => {
      if (this._destroyed) {return;}
      this._completeLearningPhase();
    }, learningDuration - (Date.now() - startTime));

    // v5.8.32: Schedule active probes during learning (1min, 5min, 10min)
    this._learningProbeTimers = [];
    for (const delayMs of [60000, 300000, 600000]) {
      const t = this.homey.setTimeout(async () => {
        if (this._destroyed) {return;}
        if (!this.isLearning) {return;}
        try {
          this.log(`[INTELLIGENT-ADAPTER] Active probe (${delayMs / 60000}min)...`);
          if (this._shouldProbeNow()) {
            await this.activeProbe();
            await this._saveLearningData();
          } else {
            this.log('[INTELLIGENT-ADAPTER] Learning probe deferred (sleepy/heap)');
          }
        } catch (e) { /* non-critical */ }
      }, delayMs);
      this._learningProbeTimers.push(t);
    }
  }
  ...
  _startMaintenanceMode() {
    ...
    if (this.maintenanceTimer) {
      this.homey.clearInterval(this.maintenanceTimer);
    }

    // Schedule hourly maintenance
    this.maintenanceTimer = this.homey.setInterval(() => {
      if (this._destroyed) {return;}
      this._runMaintenance();
    }, MAINTENANCE_INTERVAL_MS);

    // Run initial maintenance after 5 minutes
    this._maintenanceStartTimer = this.homey.setTimeout(() => {
      if (this._destroyed) {return;}
      this._runMaintenance();
    }, 5 * 60 * 1000);
  }
  ...
  /**
   * Sleepy CR2032/AAA buttons and sensors must not be ZCL-probed hourly.
   */
  _isSleepyBattery() {
    try {
      return this.device.hasCapability?.('measure_battery') && this.device.mainsPowered !== true;
    } catch (_e) {
      return false;
    }
  }
  ...
  _shouldProbeNow() {
    try {
      ...
      if (BootBudget.isHeapCritical(undefined, this.device)) {return false;}
      if (!this._isSleepyBattery()) {return true;}
      return BootBudget.isRecentlyAwake(this.device);
    } catch (_e) {
      return !this._isSleepyBattery();
    }
  }
  ...
  async _runMaintenance() {
    ...
    // Restore active probe: mains always; sleepy only in the wake window.
    if (this._shouldProbeNow()) {
      try {
        await this.activeProbe();
      } catch (e) {
        this.log(`[INTELLIGENT-ADAPTER] ⚠️ Maintenance probe failed: ${e.message}`);
      }
    } else {
      this.log('[INTELLIGENT-ADAPTER] ⏭️ Defer probe until wake / heap recovers');
    }
  ...
  recordDataPoint(capability, value, source = 'unknown') {
    ...
    if (data.validCount >= MIN_DATA_POINTS_TO_CONFIRM && this._hasValueVariance(data.values)) {
      ...
      // v5.11.16: Static config flags are authoritative - do NOT override them
      // Previous logic tried to override noHumidity/noTemperature from misclassified settings DPs
      ...
      if (configKey && this.staticConfigFlags[configKey]) {
        this.log(`[INTELLIGENT-ADAPTER] 🚫 ${capability} data ignored - static config ${configKey}=true`);
      }
    }
  ...
  recordTuyaDP(dpId, value, dataType) {
    // v5.11.16: Skip DPs with explicit setting/internal mappings in device's static dpMap
    // Prevents DP2=radar_sensitivity / DP3=min_range being misclassified as humidity
    const dpMap = this.device?._getSensorConfig?.()?.dpMap;
    if (dpMap) {
      const entry = dpMap[dpId];
      if (entry && (entry.internal || entry.setting || entry.cap === null)) {
        return;
      }
    }
  ...
  _analyzeCapabilities() {
    ...
    for (const [cap, config] of Object.entries(MANAGEABLE_CAPABILITIES)) {
      ...
      if (data && data.validCount >= MIN_DATA_POINTS_TO_CONFIRM && this._hasValueVariance(data.values)) {
        ...
        // v5.11.16: NEVER override static config flags for sensor-specific configs
        // The override was too permissive - settings DPs were being misclassified as capability data
        if (staticConfigSaysNo) {
          this.log(`[INTELLIGENT-ADAPTER] � ${cap} blocked by static config (${configKey}=true) - NOT overriding`);
          continue;
        }
        ...
        // WHY(P2488): sleepy buttons rarely get battery datapoints in adapter cache —
        // never queue measure_battery for removal when keep-lock applies.
        if (!this.detectedCapabilities.has(cap) && !isOverridden) {
          let keepBattery = false;
          if (cap === 'measure_battery') {
            try {
              const driverId = String(this.device?.driver?.id || '');
              keepBattery = /^button_|remote_|scene_switch_|sos|emergency/i.test(driverId);
              if (!keepBattery && this.batteryManager?._mustKeepBatteryCapability?.()) {
                keepBattery = true;
              }
            } catch (_) { keepBattery = false; }
          }
          if (!keepBattery) analysis.toRemove.push(cap);
        }
    ...
    // WHY(P2490 / Peter a5304ce8 @ 9.0.916): stderr capability_id_not_available_on_device
    // while ZCL still reports batteryPercentageRemaining=76 — cap was stripped earlier.
    // Keep-lock alone cannot paint UI until measure_battery exists again.
    try {
      if (this._mustKeepBatteryCapability?.()
        && !this.device.hasCapability('measure_battery')
        && !analysis.toAdd.includes('measure_battery')) {
        analysis.toAdd.push('measure_battery');
        this.log('[INTELLIGENT-ADAPTER] ➕ Queue rehydrate measure_battery (P2490 keep-lock)');
      }
    } catch (_e) { /* soft */ }
  ...
  _isCriticalCapability(capability) {
    ...
    if (criticalCapabilities.includes(capability)) return true;

    // WHY(P2488 / Peter a5304ce8 @ 9.0.916): hourly maintenance marked measure_battery
    // "unused" on sleepy Smartbutton → strip → ZCL batteryPercentageRemaining throws
    // capability_id_not_available_on_device. Never strip when button/SH-SC07 lock says keep.
    if (capability === 'measure_battery') {
      try {
        if (this.batteryManager && typeof this.batteryManager._mustKeepBatteryCapability === 'function'
          && this.batteryManager._mustKeepBatteryCapability()) {
          return true;
        }
      } catch (_) { /* ignore */ }
      try {
        const driverId = String(this.device?.driver?.id || '');
        if (/^button_|remote_|scene_switch_|sos|emergency/i.test(driverId)) return true;
      } catch (_) { /* ignore */ }
    }
  ...
  destroy() {
    ...
    if (this._learningEndTimer) {
      this.homey.clearTimeout(this._learningEndTimer);
      this._learningEndTimer = null;
    }
    if (this._maintenanceStartTimer) {
      this.homey.clearTimeout(this._maintenanceStartTimer);
      this._maintenanceStartTimer = null;
    }
    // v5.8.32: Clear learning probe timers
    if (this._learningProbeTimers) {
      this._learningProbeTimers.forEach(t => this.homey.clearTimeout(t));
      this._learningProbeTimers = null;
    }
...
class IntelligentBatteryManager {
  ...
  _scheduleBatteryDetection() {
    ...
    intervals.forEach((interval, index) => {
      ...
      this._detectionTimers.push(timer);
    });
  }

  destroy() {
    this._destroyed = true;
    this._detectionTimers.forEach(timer => this.homey.clearTimeout(timer));
    this._detectionTimers = [];
  }

  /**
   * WHY(P2294): Peter diag 4b1a0dc9 — sleepy SH-SC07 never answers battery reads;
   * removing measure_battery flipped HybridProtocolManager out of "sleepy" and
   * protocol-optimize-disabled OnOff 0xFD RX. Never strip battery for known
   * battery buttons / skipBatteryReporting profiles.
   */
  _mustKeepBatteryCapability() {
    try {
      const driverId = String(this.device?.driver?.id || this.device?.driverId || '');
      if (/^button_|remote_|scene_switch_|sos|emergency/i.test(driverId)) return true;
      if (this.device?.mainsPowered === true) return false;
      // WHY(P2296): Zemismart battery covers — never strip measure_battery
      try {
        const { isBatteryCoverMfr } = require('../helpers/batteryPowerSource');
        const mfr = this.device.getManufacturerName?.() || this.device.getSetting?.('zb_manufacturer_name') || '';
        if (isBatteryCoverMfr(mfr)) return true;
      } catch { /* ignore */ }
      // WHY(P2298): settings may arrive after adapter start — lock SH-SC07 / TS0041 by couple
      try {
        const mfr = String(this.device.getSetting?.('zb_manufacturer_name') || this.device._cachedManufacturerName || '');
        const pid = String(this.device.getSetting?.('zb_model_id') || this.device._cachedModelId || '');
        if (/mrpevh8p|5bpeda8u|b4awzgct/i.test(mfr)) return true;
        if (/^TS004[1-4F]/i.test(pid) && /^button_/i.test(driverId)) return true;
      } catch { /* ignore */ }
      const energy = typeof this.device.getEnergy === 'function' ? this.device.getEnergy() : null;
      if (Array.isArray(energy?.batteries) && energy.batteries.length) return true;
      const profile = typeof this.device.getDeviceProfile === 'function'
        ? this.device.getDeviceProfile()
        : this.device?._deviceProfile;
      if (profile?.skipBatteryReporting) return true;
      if (profile?.batteryEpOnly != null) return true;
    } catch {
      /* ignore */
    }
    return false;
  ...
  async _attemptBatteryDetection() {
    if (this._mustKeepBatteryCapability()) {
      this.log('[INTELLIGENT-BATTERY] 🔒 Keep measure_battery (button/sleepy profile) — skip AC strip');
  ...
  _confirmBattery(source, value) {
    ...
    // WHY(P2507 / Peter 77394256): never paint 0% placeholder — Homey UI stays blank/? 
    const pct = parseFloat(value);
    if (!Number.isFinite(pct) || pct <= 0) {
      this.log(`[INTELLIGENT-BATTERY] skip paint non-positive ${value}`);
      return;
    }
    this.device.setCapabilityValue('measure_battery', pct).catch(() => { });
  }
  ...
  async _removeBatteryCapability() {
    if (this._mustKeepBatteryCapability()) {
      this.log('[INTELLIGENT-BATTERY] 🔒 Refusing remove measure_battery (P2294 button/sleepy lock)');
      return;
    }
...
class IntelligentPowerManager {
  ...
  async initialize() {
    ...
    this._adaptationTimer = this.homey.setTimeout(() => {
      if (this._destroyed) {return;}
  ...
  destroy() {
    ...
    if (this._adaptationTimer) {
      this.homey.clearTimeout(this._adaptationTimer);
      this._adaptationTimer = null;
    }
  }
