...
class SmartDriverAdaptation {
  ...
  async analyzeClusters(deviceInfo) {
    ...
    // CRITICAL: Button/Remote detection BEFORE switch detection!
    ...
    // Button indicators:
    // 1. Driver name contains "button" or "remote" or "wireless"
    // 2. Has battery (buttons are battery powered)
    // 3. Has onOff cluster BUT no onoff capability
    // 4. Multiple endpoints (multi-button controllers)
    const isButton = (
    ...
    if (isButton) {
      ...
      this.log('   🔘 BUTTON/REMOTE DETECTED - Controller device, not controllable!');
    }
  ...
  async detectRealCapabilities(clusterAnalysis) {
    ...
    // Convertir features en capabilities
