    ...
    try {
      ...
      if (migrationCheck.shouldMigrate) {
        ...
        if (switchDecision.action === 'recommended') {
          ...
          const migrationSuccess = await autoMigrateDriver(this.device, migrationCheck.recommendedDriver);
      ...
      try {
        ...
        if (this.device.zclNode) {
          ...
          const kpi = await getDeviceKpi(this.device.getData().id);
          if (kpi) {
      ...
      if (multiGangInfo.isMultiGang) {
        ...
        for (let i = 1; i <= multiGangInfo.gangCount; i++) {
          if (i === 1) {
            ...
            analysis.requiredCapabilities.push(`onoff.usb${i}`);
          }
        }
        
        this.log(`   🔌 ✅ USB OUTLET ${multiGangInfo.gangCount}-GANG DETECTED`);
      } else {
