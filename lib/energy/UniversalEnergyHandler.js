  ...
  _registerListeners() {
    ...
    if (meterEp && this.device.hasCapability('meter_power')) {
      meterEp.clusters.metering.on('attr.currentSummDelivered', (value) => {
        ...
        this.device.safeSetCapabilityValue('meter_power', Number(normalized.toFixed(3))).catch(() => {});
      });
    }
    ...
    if (electricalEp) {
      ...
      if (this.device.hasCapability('measure_power')) {
        cluster.on('attr.activePower', (value) => {
          ...
          this.device.safeSetCapabilityValue('measure_power', normalized).catch(() => {});
        });
      }
      ...
      if (this.device.hasCapability('measure_voltage')) {
        cluster.on('attr.rmsVoltage', (value) => {
          ...
          this.device.safeSetCapabilityValue('measure_voltage', Number(normalized.toFixed(1))).catch(() => {});
        });
      }
      ...
      if (this.device.hasCapability('measure_current')) {
        cluster.on('attr.rmsCurrent', (value) => {
          ...
          this.device.safeSetCapabilityValue('measure_current', Number(normalized.toFixed(3))).catch(() => {});
        });
      }
    }
  }
