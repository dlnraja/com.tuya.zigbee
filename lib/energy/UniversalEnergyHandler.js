...
class UniversalEnergyHandler {
  ...
  _applyUniversalQuirks() {
    ...
    if (modelId === 'CK-BL702-SWP-01(7020)' || MfrHelper.getManufacturerName(this.device) === 'eWeLink') {
  ...
  async _setupElectricalMeasurement() {
    ...
    try {
      ...
      try {
        ...
        // Boot mesh congestion is not "offline": still configure reporting /
        // keep polling. Watchdog marks unavailable later if truly silent.
        try {
          const avail = this.device.homey?.app?.availabilityManager;
          const id = this.device.getData?.()?.id || this.device.id;
          if (avail && typeof avail.notePollFailure === 'function') {
            avail.notePollFailure(id);
            const wait = avail.nextPollDelayMs(id);
            if (wait > 0) {
              this.device.log(`[ENERGY-UNIFIED] plug silent — skip hammering for ${wait}ms`);
            }
          }
        } catch (_e) { /* optional */ }
      }
  ...
  _commitMeasured(capability, value) {
    if (value === null || value === undefined || Number.isNaN(Number(value))) {return;}
    const smart = this.device.smartEnergy;
    if (smart && typeof smart.reportMeasured === 'function') {
      smart.reportMeasured(capability, value, 'zcl').catch(() => {});
      return;
    }
    this.device.safeSetCapabilityValue(capability, value).catch(() => {});
    try {
      const DeviceTelemetryEstimator = require('../utils/DeviceTelemetryEstimator');
      DeviceTelemetryEstimator.record(this.device, capability, value, { source: 'direct', via: 'zcl' }).catch(() => {});
    } catch (_e) { /* optional */ }
  }

  _registerListeners() {
    if (!this._boundCaps) {this._boundCaps = new Set();}

    // 1. Metering (kWh)
    const meterEp = this._findEndpoint([0x0702, 'metering']);
    if (meterEp && this.device.hasCapability('meter_power') && !this._boundCaps.has('meter_power')) {
      this._boundCaps.add('meter_power');
      meterEp.clusters.metering.on('attr.currentSummDelivered', (value) => {
        if (value === null || value === undefined) {return;}
        const normalized = value * this.multipliers.meter_power;
        this._commitMeasured('meter_power', Number(normalized.toFixed(3)));
      });
    }
    ...
    if (electricalEp) {
      ...
      if (this.device.hasCapability('measure_power') && !this._boundCaps.has('measure_power')) {
        this._boundCaps.add('measure_power');
        cluster.on('attr.activePower', (value) => {
          if (value === null || value === undefined) {return;}
          const normalized = Math.round(value * this.multipliers.measure_power);
          this._commitMeasured('measure_power', normalized);
        });
      }

      if (this.device.hasCapability('measure_voltage') && !this._boundCaps.has('measure_voltage')) {
        this._boundCaps.add('measure_voltage');
        cluster.on('attr.rmsVoltage', (value) => {
          if (value === null || value === undefined) {return;}
          const normalized = value * this.multipliers.measure_voltage;
          this._commitMeasured('measure_voltage', Number(normalized.toFixed(1)));
        });
      }

      if (this.device.hasCapability('measure_current') && !this._boundCaps.has('measure_current')) {
        this._boundCaps.add('measure_current');
        cluster.on('attr.rmsCurrent', (value) => {
          if (value === null || value === undefined) {return;}
          const normalized = value * this.multipliers.measure_current;
          this._commitMeasured('measure_current', Number(normalized.toFixed(3)));
        });
      }
    }
  }
