...
const TUYA_PROFILES = {
  ...
  'TS0601|_TZE284_vvmbj46n': {
    ...
    dpMap: {
      1: { capability: 'measure_temperature', scale: 10, description: 'Temperature (Â°C x10)' },
  ...
  'TS0601|_TZE284_oitavov2': {
    ...
    dpMap: {
      ...
      3: { capability: 'measure_humidity.soil', description: 'Soil Moisture (%)' },
      5: { capability: 'measure_temperature', scale: 10, description: 'Temperature (Â°C x10)' },
  ...
  'TS0601|_TZE200_rhgsbacq': {
    ...
    dpMap: {
      ...
      101: { capability: 'measure_humidity', description: 'Humidity (Ã·10)' },
      106: { capability: 'measure_luminance', description: 'Illuminance (lux)' },
      111: { capability: 'measure_temperature', scale: 10, description: 'Temperature (Â°C Ã·10)' }
    }
  },
  ...
  'TS0601|_TZE200_ztc6ggyl': {
    ...
    dpMap: {
      ...
      4: { type: 'setting', name: 'detection_distance', description: 'Detection distance (Ã·100)' },
      9: { type: 'internal', name: 'distance', description: 'Target distance (Ã·100)' },
...
function getTuyaProfile(modelId, manufacturer) {
  ...
  for (const [key, profile] of Object.entries(TUYA_PROFILES)) {
    ...
    if (CI.equalsCI(profileModel, modelId)) {
      if (profileManufacturer.endsWith('*')) {
        const prefix = profileManufacturer.slice(0, -1);
        if (CI.startsWithCI(manufacturer, prefix)) {
          return { ...profile, key };
        }
      } else if (CI.equalsCI(profileManufacturer, manufacturer)) {
        return { ...profile, key };
      }
    }
  }
