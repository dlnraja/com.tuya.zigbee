...
function getTuyaProfile(modelId, manufacturer) {
  ...
  // Try wildcard match (e.g. TS0601|_TZE*) - case-insensitive (P91: TuyaNormalizer)
  ...
  for (const [key, profile] of Object.entries(TUYA_PROFILES)) {
    ...
    if (TU.normalize(profileModel) === modelLower && profileManufacturer.endsWith('*')) {
      const prefix = TU.normalize(profileManufacturer.slice(0, -1));
