...
class TuyaTimeSyncHandler {
  ...
  static async handleRequest(device, request = {}) {
    try {
      ...
      const endpoint = node?.endpoints?.[1] || node?.endpoints?.[Object.keys(node.endpoints)[0]] : null;
      const tuyaCluster = endpoint?.clusters?.tuya || endpoint?.clusters?.manuSpecificTuya : null;
