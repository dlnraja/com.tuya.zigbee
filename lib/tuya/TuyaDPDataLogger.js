...
class TuyaDPDataLogger {
  ...
  _autoUpdateCapability(dpId, value) {
    ...
    let standardMappingApplied = false;
    let specialDP1Applied = false;
    ...
    if (mapping && this.device.hasCapability(mapping.cap)) {
      try {
        ...
        standardMappingApplied = Boolean(dynamicResult && mapping.cap === dynamicResult.capability);
    ...
    if (dpId === 1) {
      if (value === true || value === false || value === 0 || value === 1) {
        ...
        if (this.device.hasCapability('alarm_motion')) {
          specialDP1Applied = Boolean(dynamicResult && dynamicResult.capability === 'alarm_motion');
    ...
    if (dynamicResult && !standardMappingApplied && !specialDP1Applied) {
