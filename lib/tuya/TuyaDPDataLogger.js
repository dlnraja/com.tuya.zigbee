...
class TuyaDPDataLogger {
  ...
  _setupZCLTracking() {
    ...
    if (clusters.onOff || clusters.// H1: OnOffBoundCluster must be bound per EP
  genOnOff) {
      const cluster = clusters.onOff || clusters.// H1: OnOffBoundCluster must be bound per EP
  genOnOff;
  ...
  _autoUpdateCapability(dpId, value) {
    const DP_CAPABILITY_MAP = {
      ...
      1: { cap: 'measure_temperature', parser: v => typeof v === 'number' && v > 100 ? safeParse(v, 10) : v },
      3: { cap: 'measure_temperature', parser: v => safeParse(v, 10) },
      18: { cap: 'measure_temperature', parser: v => safeParse(v, 10) },

      // Humidity (divided by 10 or direct)
      2: { cap: 'measure_humidity', parser: v => typeof v === 'number' && v > 100 ? safeParse(v, 10) : v },
      5: { cap: 'measure_humidity', parser: v => typeof v === 'number' && v > 100 ? safeParse(v, 10) : v },
      19: { cap: 'measure_humidity', parser: v => safeParse(v, 10) },
  ...
  getStatus() {
    const now = Date.now();
    const uptime = Math.floor(safeDivide(now - this.startTime, 1000));
    const timeSinceLastDP = this.lastUpdate ? Math.floor(safeDivide(now - this.lastUpdate, 1000)) : null;
  ...
  logStatus() {
    ...
    if (totalData === 0) {
      ...
      this.device.log('    Battery devices may take 4-24 hours to wake up');
      this.device.log('    Try triggering the device (motion, press button, etc.)');
    } else if (status.dpCount === 0 && zclCount > 0) {
      this.device.log('    Device uses standard Zigbee (ZCL), not Tuya DP');
      this.device.log('    This is normal - data is coming via ZCL clusters');
    }
