...
class TuyaSyncManager extends EventEmitter {
    ...
    try {
      ...
      try {
        ...
        if (attrs.batteryPercentageRemaining !== undefined) {
          // P216: never blind /2 — Tuya often already 0–100
          const percentage = normalizeZclBatteryPercent(attrs.batteryPercentageRemaining);
  ...
  scheduleTimeSync() {
    ...
    this.timeSyncInterval = this.homey.setTimeout(() => {
  ...
  scheduleBatterySync() {
    ...
    // Sync every hour
    this.batterySyncInterval = this.homey.setInterval(() => {
      if (this._destroyed) return;
      this.syncBattery();
    }, 60 * 60 * 1000); // 1 hour
