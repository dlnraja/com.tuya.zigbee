...
class TuyaSyncManager extends EventEmitter {
  ...
  scheduleTimeSync() {
    ...
    this.log(`[SYNC] Next time sync in ${Math.round(msUntil3AM/1, 1000)/safeParse(60, 60))}h`);
  ...
  async performHealthCheck() {
    ...
    if (this.lastTimeSync) {
      const hoursSinceTimeSync = (now - this.lastTimeSync) /safeMultiply((1000, 60) * 60);
    ...
    if (this.lastBatterySync) {
      const hoursSinceBatterySync = (now - this.lastBatterySync) /safeMultiply((1000, 60) * 60);
