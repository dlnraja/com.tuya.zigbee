...
async function initTuyaDpEngineSafe(device, zclNode) {
  try {
    ...
    if (!manager) {
      ...
      device.log('[TUYA-EF00] ℹ️ This may be because BaseUnifiedDevice.onNodeInit() hasn\'t completed yet');
...
function getEF00ManagerStatus(device) {
  ...
  if (device.tuyaEF00Manager) {
    ...
    const app = getSafeApp(device);
    if (app?.tuyaEF00Manager) {
      status.available = true;
      status.source = 'app';
      const manager = app.tuyaEF00Manager;
      status.hasOnMethod = typeof manager.on === 'function';
      status.hasEmitMethod = typeof manager.emit === 'function';
    } else if (app?.getTuyaEF00Manager) {
      try {
        const manager = app.getTuyaEF00Manager();
        if (manager) {
          status.available = true;
