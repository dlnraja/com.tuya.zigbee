...
async function initTuyaDpEngineSafe(device, zclNode) {
  try {
    ...
    if (!device.homey || !device.homey.app) {
      device.log('[TUYA-EF00] ⚠️  No homey.app reference available');
      return null;
    }

    // Try to get the global EF00 manager
    const app = device.homey.app;
    let manager = null;

    // Method 1: Via getter function
    ...
    // Method 3: Device already has it (from BaseHybridDevice)
    else if (device.tuyaEF00Manager) {
      manager = device.tuyaEF00Manager;
      device.log('[TUYA-EF00] ✅ Manager found via device.tuyaEF00Manager');
    }

    // If no manager available, disable DP engine gracefully
    if (!manager) {
      device.log('[TUYA-EF00] ⚠️  EF00 manager not available, disabling DP engine for now');
      device.log('[TUYA-EF00] ℹ️  Device will work with standard Zigbee clusters if available');

      // Store the disabled state
      if (typeof device.setStoreValue === 'function') {
        await device.setStoreValue('tuya_dp_engine_disabled', true).catch(err => {
          device.error('[TUYA-EF00] Failed to store DP engine disabled state:', err);
        });
      }
    ...
    // Manager found! Assign to device and enable DP engine
    device.tuyaEF00Manager = manager;
    device.log('[TUYA-EF00] ✅ EF00 manager assigned to device');

    // Store the enabled state
    if (typeof device.setStoreValue === 'function') {
      await device.setStoreValue('tuya_dp_engine_disabled', false).catch(err => {
        device.error('[TUYA-EF00] Failed to store DP engine enabled state:', err);
      });
    }
