...
const MANUFACTURER_FORMAT_DB = {
  ...
  // Floor thermostat (weekday offset quirks → dual 2000; Z2M #32647 / P2472b)
  ...
  '_TZE204_6a4vxfnv': { format: FORMAT.Z2M_2000, confidence: 100, source: 'Z2M tuya.ts / P2472b', device: 'Floor thermostat', notes: 'Weekday+2 quirk; dual 2000' },

  // WHY(P2472b): Joep Insoma dual irrigation — EF00-only; soft time sync
  '_TZE284_fhvpaltk': { format: FORMAT.Z2M_1970, confidence: 90, source: 'P2468/P2472b forum Joep', device: 'Insoma dual irrigation' },
  '_TZE200_fhvpaltk': { format: FORMAT.Z2M_1970, confidence: 85, source: 'P2472b OEM sibling soft', device: 'Insoma dual irrigation' },
  '_TZE204_fhvpaltk': { format: FORMAT.Z2M_1970, confidence: 85, source: 'P2472b OEM sibling soft', device: 'Insoma dual irrigation' },

  // WHY(P2472b): Moes ZTS curtain — forceTimeUpdates / mcuSyncTime (Salvagr #533)
  '_TZE204_5slehgeo': { format: FORMAT.Z2M_1970, confidence: 95, source: 'P2467/P2472b Moes ZTS', device: 'Moes ZTS curtain' },
  '_TZE200_5slehgeo': { format: FORMAT.Z2M_1970, confidence: 90, source: 'P2472b Moes ZTS sibling', device: 'Moes ZTS curtain' },
  '_TZE284_5slehgeo': { format: FORMAT.Z2M_1970, confidence: 90, source: 'P2472b Moes ZTS sibling', device: 'Moes ZTS curtain' },
...
const FIRMWARE_BUGS_DB = {
  ...
  // WHY(P2296): Zemismart battery covers — MCU version poll/respond loop drains pack (Z2M #28655)
  '_TZE200_68nvbio9': {
    bug: 'mcuVersionRequest / respondToMcuVersionResponse floods mesh and drains battery',
    workaround: 'Skip MCU version request; dataQuery only; do not time-sync on every mcuVersionRsp',
    source: 'Z2M issue #28655 #29124',
    severity: 'critical',
    affected_formats: [FORMAT.Z2M_1970],
    fix: { type: 'DISABLE_MCU_VERSION_RESPONSE' },
  },
  '_TZE200_68nvbi09': {
    bug: 'Typo sibling of 68nvbio9 — same MCU version drain',
    workaround: 'Skip MCU version request/response side-effects',
    source: 'Z2M issue #28655',
    severity: 'critical',
    affected_formats: [FORMAT.Z2M_1970],
    fix: { type: 'DISABLE_MCU_VERSION_RESPONSE' },
  },
  '_TZE200_cf1sl3tj': {
    bug: 'ZM85EL-2Z battery cover — MCU version traffic drains battery',
    workaround: 'Skip MCU version request/response side-effects',
    source: 'Z2M ZM85EL-2Z / cover group 148',
    severity: 'critical',
    affected_formats: [FORMAT.Z2M_1970],
    fix: { type: 'DISABLE_MCU_VERSION_RESPONSE' },
  },
  '_TZE200_pw7mji0l': {
    bug: 'Z2M cover sibling — MCU version drain risk',
    workaround: 'Skip MCU version request/response side-effects',
    source: 'Z2M cover fingerprint group',
    severity: 'high',
    affected_formats: [FORMAT.Z2M_1970],
    fix: { type: 'DISABLE_MCU_VERSION_RESPONSE' },
  },
  ...
  '_TZE210_m6lwazh9': {
    bug: 'A-OK AM25 battery blind — MCU version poll drains pack',
    workaround: 'Skip MCU version request/response side-effects',
    source: 'Z2M #30273',
    severity: 'critical',
    affected_formats: [FORMAT.Z2M_1970],
  ...
  '_TZE200_m6lwazh9': {
    bug: 'Forum Zemismart m6lwazh9 cover — treat as battery cover MCU-safe',
    workaround: 'Skip MCU version request/response side-effects',
    source: 'forum-154092 P2297',
    severity: 'high',
    affected_formats: [FORMAT.Z2M_1970],
    fix: { type: 'DISABLE_MCU_VERSION_RESPONSE' },
  },
...
class MCUFormatDatabase {
  ...
  static getByManufacturer(manufacturerName) {
    ...
    const lower = TU.normalize(manufacturerName);
    for (const [key, value] of Object.entries(MANUFACTURER_FORMAT_DB)) {
      if (TU.normalize(key) === lower) {
  ...
  static getFirmwareBug(manufacturerName) {
    if (!manufacturerName) {return null;}
    if (FIRMWARE_BUGS_DB[manufacturerName]) {return FIRMWARE_BUGS_DB[manufacturerName];}
    // Case-insensitive fallback (Homey settings / interview casing varies)
    const needle = String(manufacturerName).toLowerCase();
    for (const [key, val] of Object.entries(FIRMWARE_BUGS_DB)) {
      if (key.toLowerCase() === needle) {return val;}
    }
    return null;
  }
  ...
  static predictFormat(deviceInfo = {}) {
    ...
    if (manufacturerName) {
      for (const rule of MANUFACTURER_PREFIX_RULES) {
        if (TU.startsWithCI(manufacturerName, rule.prefix)) {
