...
/**
 * TUYA TIME SYNCHRONIZATION MANAGER - v5.5.619
 *
 * CRITICAL RULE (from Z2M/HA research):
 * ❌ NEVER push time without device request
 * ✅ ALWAYS respond to device-initiated requests
 *
 * Protocol:
 ...
 *
 * Response format (Europe/Paris):
 * - UTC timestamp (seconds since 1970)
 * - Timezone offset (GMT+1 = +3600, GMT+2 = +7200 DST)
 * - DST flag (0 = winter, 1 = summer)
 *
 * Sources:
...
class TuyaTimeSyncManager {
  ...
  /**
   * Initialize time sync - v5.5.619 PASSIVE MODE
   * ❌ DO NOT push time proactively
   * ✅ Only respond when device requests it
   */
  async initialize(zclNode) {
    if (!zclNode) return false;

    this.device.log('[TIME-SYNC] 🕐 Initializing (PASSIVE MODE - wait for device request)...');
    ...
    // Listen for time sync requests from device (CRITICAL)
    this.setupTimeSyncListener();

    // v5.5.619: Also listen for ZCL Time cluster requests
    this.setupZCLTimeListener(zclNode);

    // v5.5.619: NO initial sync - wait for device to request
    this.device.log('[TIME-SYNC] ✅ Ready - waiting for device time request');

    return true;
  }

  /**
   * Setup listener for device time sync requests - v5.5.619
   * CRITICAL: Device sends 0x24 command when it needs time sync
   * We MUST respond, not push proactively
   */
      ...
      if (this.endpoint && typeof this.endpoint.on === 'function') {
        this.endpoint.on('frame', async (frame) => {
          if (frame.cluster === 0xEF00 && frame.command === 0x24) {
            this.device.log('[TIME-SYNC] 📥 DEVICE REQUESTED TIME SYNC (cmd 0x24)');
            await this.sendTimeSync();
          }
        });
        this.device.log('[TIME-SYNC] ✅ Listening for EF00 cmd 0x24');
      }

      // Listen via Tuya cluster events
      if (this.tuyaCluster && typeof this.tuyaCluster.on === 'function') {
        // Listen for mcuSyncTime command
        this.tuyaCluster.on('mcuSyncTime', async (data) => {
          this.device.log('[TIME-SYNC] 📥 mcuSyncTime request:', data);
          await this.sendTimeSync();
        });

        // Listen for any command that might be time-related
        this.tuyaCluster.on('command', async (commandId, data) => {
          if (commandId === 0x24 || commandId === 36) {
            this.device.log(`[TIME-SYNC] 📥 Command ${commandId} (time request)`);
            await this.sendTimeSync();
          }
        });

        this.device.log('[TIME-SYNC] ✅ Listening for Tuya cluster events');
      }
