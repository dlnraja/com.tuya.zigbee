...
class TuyaTimeSyncManager {
  ...
  async sendTimeSync(sequenceNumber = null, forceFormat = null) {
    ...
    try {
      ...
      // v5.8.74: CRITICAL FIX — Use Tuya 2000 epoch, NOT Unix 1970!
      // Z2M issue #30054 confirms: ALL TS0601 Tuya devices expect seconds since 2000-01-01
      // Previous code used raw Unix timestamps causing time to be ~30 years off on LCD displays
      const TUYA_EPOCH = 946684800; // Seconds from 1970-01-01 to 2000-01-01
      const unixUtc = Math.floor(now.getTime() / 1000);
      const timezoneOffsetSec = -now.getTimezoneOffset() * 60; // GMT+1 = +3600, GMT+2 = +7200
      const utcTimestamp = unixUtc - TUYA_EPOCH;
