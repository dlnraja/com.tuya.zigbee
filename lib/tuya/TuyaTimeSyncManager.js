...
class TuyaTimeSyncManager {
  ...
  /**
   * WHY(P2296): Homey/Z2M — do not fan-out TX on mcuVersionRsp for battery covers.
   */
  _shouldSkipMcuVersionSideEffects() {
    try {
      const mfr = this._getManufacturerName() || this.device.getManufacturerName?.() || '';
  ...
  _scheduleIntelligentSync() {
    ...
    this._intelligentSyncTimer = this.homey.setTimeout(async () => {
  ...
  setupTimeSyncListener() {
    try {
      ...
      if (this.tuyaCluster && typeof this.tuyaCluster.on === 'function') {
        ...
        // WHY(P2296): ZM16EL — responding to every mcuVersionRsp amplifies traffic & drains battery
        this.tuyaCluster.on('mcuVersionRsp', async (data) => {
          if (this._shouldSkipMcuVersionSideEffects()) {
            this.device.log('[TIME-SYNC] ⏭️ Skip time-on-mcuVersionRsp (battery-cover / firmware quirk)');
            return;
          }
  ...
  async sendTimeSync(sequenceNumber = null, forceFormat = null) {
    ...
    try {
      ...
      const formatPayload = TuyaTimeSyncFormats.buildPayload(format, {
        timezone: 'auto',
      ...
      // Prefer Formats payload when it is already 10-byte seq (P2360)
      const payload10 = (formatPayload && formatPayload.length === 10)
        ? Buffer.from(formatPayload)
        : (() => {
          const b = Buffer.alloc(10);
          b.writeUInt16BE(seqNum, 0);
          b.writeUInt32BE(utcTimestamp, 2);
          b.writeUInt32BE(localTimestamp, 6);
          return b;
        })();
