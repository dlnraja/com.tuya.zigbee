 ...
 * v5.5.66: Uses Unix epoch timestamps (seconds since 1970), NOT Zigbee epoch
 *
 * @param {ZigBeeDevice} device - The Homey ZigBee device instance
 * @param {Object} options - Configuration options
 * @returns {Promise<boolean>} - True if sync was successful
...
async function syncDeviceTimeTuya(device, options = {}) {
  ...
  try {
    ...
    // v5.5.66: IMPORTANT - Tuya uses Unix epoch (1970), NOT Zigbee epoch (2000)
    ...
    log(`${logPrefix}   UTC: ${new Date(utcTimestamp * 1000).toISOString()}`);
