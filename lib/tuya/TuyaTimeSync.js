...
async function syncDeviceTimeTuya(device, options = {}) {
  ...
  try {
    ...
    const needsTuyaEpoch = useTuyaEpoch !== null
      ? useTuyaEpoch
      : (mfr.startsWith('_TZE284') || mfr.startsWith('_TZE204') || mfr.includes('vvmbj46n') || device.useTuyaEpoch === true);
    ...
    // v5.5.195: CRITICAL FIX - Tuya devices expect [Local:4][UTC:4] format!
    // Research from Z2M, ZHA, and Tuya documentation shows LCD devices like TH05Z
    // expect LOCAL time first, then UTC time. This is REVERSED from what we had!
    // Format: [Local:4 bytes BE][UTC:4 bytes BE]
    const payload = Buffer.alloc(8);
    payload.writeUInt32BE(localTimestamp, 0);  // Local time FIRST
    payload.writeUInt32BE(utcTimestamp, 4);    // UTC time SECOND

    log(`${logPrefix}   Payload hex: ${payload.toString('hex')} [Local:${localTimestamp}][UTC:${utcTimestamp}]`);
