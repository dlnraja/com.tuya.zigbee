...
async function syncDeviceTimeZCL(device, options = {}) {
  ...
  const _log = createSafeLogger(device, 'TIME-SYNC-ZCL');
...
async function syncDeviceTimeTuya(device, options = {}) {
  ...
  const _log = createSafeLogger(device, 'TIME-SYNC-TUYA');
  const log = silent ? () => { } : (msg) => _log.log(msg);
  const error = (msg, err) => _log.error(msg, err);
  ...
  try {
    ...
    log(`${logPrefix} Syncing (epoch=${ts.epoch}, mfr=${mfr})`);
      ...
      try {
        if (typeof tuyaCluster.mcuSyncTime === 'function') {
          ...
          try {
            ...
            await tuyaCluster.mcuSyncTime({ payloadSize: 8, payload: Buffer.from(payload) });
            sent = true;
          }
...
async function syncDeviceTime(device, options = {}) {
  ...
  const _log = createSafeLogger(device, 'TIME-SYNC');
  const log = (msg) => _log.log(msg);
  log(`${logPrefix} Starting time sync...`);

  // Determine order based on device type
  const settings = device.getSettings?.() || {};
  const modelId = (settings.zb_model_id || settings.zb_modelId || '').toLowerCase();
  const mfrName = TU.normalize(settings.zb_manufacturer_name || settings.zb_manufacturerName);
...
function TuyaTimeSyncMixin(Base) {
  return class extends Base {
    ...
    async setupTimeSync(options = {}) {
      ...
      try {
        const { isSleepyForPollControl } = require('../zigbee/PollControlPolicy');
        if (isSleepyForPollControl(this) && this._deviceRequestedTime !== true) {
          this.log('[TIME-SYNC] skip periodic poll on sleepy node (answer inbound only)');
          this._timeSyncOnWake = options.syncOnWake !== false;
          return;
        }
      ...
      this._timeSyncInterval = scheduler.setInterval(() => {
