...
async function syncDeviceTimeTuya(device, options = {}) {
  ...
  try {
    ...
    // developer.tuya.com: "seconds since January 1, 1970, at 00:00:00 GMT"
    const unixTimestamp = Math.floor(now.getTime() / 1000);
    const timezoneOffsetSec = -now.getTimezoneOffset() * 60;
    const utcTimestamp = unixTimestamp;
    const localTimestamp = unixTimestamp + timezoneOffsetSec;

    log(`${logPrefix} Syncing Tuya time (Unix 1970 epoch)...`);
    log(`${logPrefix}   UTC: ${utcTimestamp} | Local: ${localTimestamp} | TZ: ${timezoneOffsetSec}s`);
