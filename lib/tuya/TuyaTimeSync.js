...
async function syncDeviceTimeTuya(device, options = {}) {
  ...
  try {
    ...
    // v5.5.184: CRITICAL - Detect if device needs Tuya epoch (2000) or Unix epoch (1970)
    // LCD devices like _TZE284_* need Tuya epoch for correct time display
    const mfr = device.getSetting?.('zb_manufacturer_name') || device._protocolInfo?.mfr || '';
    const needsTuyaEpoch = useTuyaEpoch !== null
      ? useTuyaEpoch
      : (mfr.startsWith('_TZE284') || mfr.startsWith('_TZE204') || device.useTuyaEpoch === true);
    ...
    // v5.5.184: CORRECT FORMAT - 8 bytes Big Endian
    // [UTC:4 bytes BE][Local:4 bytes BE]
    const payload = Buffer.alloc(8);
    payload.writeUInt32BE(utcTimestamp, 0);
    payload.writeUInt32BE(localTimestamp, 4);

    log(`${logPrefix}   Payload hex: ${payload.toString('hex')}`);
