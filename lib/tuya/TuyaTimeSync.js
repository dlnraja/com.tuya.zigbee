...
async function syncDeviceTimeTuya(device, options = {}) {
  ...
  try {
    ...
    const mfr = device._manufacturerName
      || device.getSetting?.('zb_manufacturer_name') // Correct key (underscore)
      || device.getSetting?.('zb_manufacturerName')  // Fallback for old pairings (capital N)
      || device._protocolInfo?.mfr
      || '';
    const mfrLower = (mfr || '').toLowerCase();
    // v5.8.74: ALL _TZE* devices use Tuya 2000 epoch, not just _TZE284/_TZE204
    // Z2M issue #30054 confirms: wrong epoch (1970 vs 2000) causes wrong time on ALL TS0601 devices
    const modelId = (device.getSetting?.('zb_model_id') || device.getSetting?.('zb_modelId') || '').toLowerCase();
    const needsTuyaEpoch = useTuyaEpoch !== null
      ? useTuyaEpoch
      : (mfrLower.startsWith('_tze200') || mfrLower.startsWith('_tze204') || mfrLower.startsWith('_tze284') || mfrLower.includes('vvmbj46n') || modelId === 'ts0601' || device.useTuyaEpoch === true);
...
async function syncDeviceTime(device, options = {}) {
  ...
  const mfrName = (settings.zb_manufacturer_name || settings.zb_manufacturerName || '').toLowerCase();
