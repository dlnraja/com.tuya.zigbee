 ...
 *
 * Sources:
 * - github.com/zigbeefordomoticz/wiki (Tuya-CLUSTERS.TUYA_EF00.md)
...
const TUYA_DP_DATABASE = {
  ...
  'TRV_V1': {
    ...
    model: 'TS0601',
    dps: {
      2: { name: 'setpoint', type: 0x02, unit: 'Â°C', divider: 10, capability: 'target_temperature' },
      3: { name: 'temperature', type: 0x02, unit: 'Â°C', divider: 10, capability: 'measure_temperature' },
  ...
  'TRV_V2': {
    ...
    dps: {
      ...
      109: { name: 'dim.valve', type: 0x02, capability: 'dim.valve' },
  ...
  'SOIL_SENSOR': {
    ...
    model: 'TS0601',
    dps: {
      1: { name: 'temperature', type: 0x02, divider: 10, unit: 'Â°C', capability: 'measure_temperature' },
...
function findDPProfile(manufacturerName, modelId = 'TS0601') {
  for (const [profileName, profile] of Object.entries(TUYA_DP_DATABASE)) {
    if (profile.manufacturers && CI.includesCI(profile.manufacturers, manufacturerName)) {
      if (!profile.model || CI.equalsCI(profile.model, modelId)) {
