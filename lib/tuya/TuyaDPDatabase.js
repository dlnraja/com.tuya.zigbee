...
const TUYA_DP_DATABASE = {
  ...
  'RADAR_PIR': {
    ...
    dps: {
      ...
      // ponytail: DP_DISCOVERY - additional radar DPs (distance, etc.) auto-learned via DP Discovery at runtime
    }
  },
...
function findDPProfile(manufacturerName, modelId = 'TS0601') {
  ...
  for (const [profileName, profile] of Object.entries(TUYA_DP_DATABASE)) {
    if (profile.manufacturers && TU.includesCI(profile.manufacturers, manufacturerName)) {
      if (!profile.model || TU.normalize(profile.model) === modelLower) {
