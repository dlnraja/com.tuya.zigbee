...
const TuyaZigbeeDevice = require('./TuyaZigbeeDevice');
 ...
 *
 * P206: extends TuyaZigbeeDevice so EF00 drivers inherit L14 safeSet + button/IO spine.
 *
 * v6.0.0: Antigravity Hardened
 * - Added markAppCommand support for bidirectional sync (optimistic UI)
 * - Enhanced with patterns from JohanBendz PR analysis:
...
class TuyaSpecificClusterDevice extends TuyaZigbeeDevice {
  ...
  constructor(...args) {
    super(...args);
    // P104: elevate DeviceIOFacade onto legacy TuyaSpecificClusterDevice lineage
    try {
  ...
  async onNodeInit({ zclNode }) {
    ...
    try {
      if (this.io) {
        ...
        await this.io.pickProtocol(zclNode || this.zclNode).catch(() => {});
      }
    } catch (_e) { /* non-fatal */ }
    ...
    // P104: deferred interview compensation when IO façade present
    try {
      if (this.io?.runInterviewCompensation) {
        const { safeSetTimeout } = require('../utils/safe-timers');
        safeSetTimeout(this, () => {
          this.io.runInterviewCompensation({
            tryMagic: true,
            ensureIas: false,
            mcu: true,
            pollFallback: true,
          }).catch(() => {});
        }, 1500 + Math.floor(Math.random() * 4000));
      }
    } catch (_e) { /* noop */ }
  }
  ...
  _parseRawTuyaFrame(data) {
    try {
      ...
      // Fuzz-catcher: Verify data has enough bytes
      if (data.length < 6 + len) {
        this.error(`[TUYA-RAW] Fuzz-Catcher: Malformed frame, expected ${6 + len} bytes but got ${data.length}`);
        return;
      }
      let value;
  ...
  async handleTuyaDataReport(data) {
    ...
    try {
      ...
      // Update capability
      await (typeof this.safeSetCapabilityValue === 'function'
        ? this.safeSetCapabilityValue(dpMapping.capability, value, { source: 'tuya-dp', confidence: 0.9 })
  ...
  async sendTuyaCommand(dp, value, type = 'value') {
    try {
      ...
      // P2699: EF00 gap → complementary soft-fail (magic/raw/PFC), not mandatory fatal
      if (!tuyaCluster) {
  ...
  debug(message, data = null) {
    if (this.debugEnabled) {
      ...
      if (data) {
        this.log(`[${deviceName}] DEBUG: ${message}`, data);
      } else {
        this.log(`[${deviceName}] DEBUG: ${message}`);
      }
    }
  }
  ...
  async _sendTuyaDatapoint(dp, datatype, length, data, maxRetries = 2, baseDelay = 300) {
    ...
    // P2314: heal hollow IEEE before Homey cluster.datapoint
    try {
      const { healZigbeeNodeIdentity } = require('../io/healZigbeeNodeIdentity');
      await healZigbeeNodeIdentity(this, { force: false });
    } catch (_e) { /* noop */ }

    // Prefer DeviceIO cascade (manager → raw → magic) — never flood integrations
    if (this.io?.sendDP) {
      try {
        let value = data;
        if (datatype === 1) {value = data[0] === 1;}
        else if (datatype === 2 && data.length >= 4) {value = data.readUInt32BE(0);}
        else if (datatype === 4) {value = data[0];}
        const typeName = ({ 0: 'raw', 1: 'bool', 2: 'value', 3: 'string', 4: 'enum', 5: 'bitmap' })[datatype] || 'value';
        const ok = await this.io.sendDP(dp, value, { dpType: typeName, type: typeName });
        if (ok) {return true;}
      } catch (_e) { /* fall through to cluster retry */ }
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        try {
          const { healZigbeeNodeIdentity, isMissingIeeeError } = require('../io/healZigbeeNodeIdentity');
        ...
        if (attempt < maxRetries) {
          ...
          await new Promise(resolve => this.homey.setTimeout(resolve, waitTime));
          if (this._destroyed) {return;}
        }
      }
    }

    // Last resort: raw sendFrame via IO
    if (this.io?.sendRaw) {
      try {
        const frame = Buffer.alloc(6 + data.length);
        frame.writeUInt16BE(this.transactionID++ & 0xffff, 0);
        frame.writeUInt8(dp & 0xff, 2);
        frame.writeUInt8(datatype & 0xff, 3);
        frame.writeUInt16BE(data.length, 4);
        data.copy(frame, 6);
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      try {
        ...
        if (i < commands.length - 1) {
          await new Promise(resolve => this.homey.setTimeout(resolve, delayBetween));
