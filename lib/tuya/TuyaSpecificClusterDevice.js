 ...
 * Base class for Tuya devices using manufacturer-specific cluster (CLUSTERS.TUYA_EF00 / CLUSTERS.TUYA_EF00)
...
class TuyaSpecificClusterDevice extends ZigBeeDevice {
    ...
    try {
      ...
      if (!tuyaCluster) {
        for (const [key, cluster] of Object.entries(endpoint.clusters)) {
          ...
          if (keyLower.includes('tuya') || keyLower.includes('ef00') || key === 'CLUSTERS.TUYA_EF00') {
            tuyaCluster = cluster;
            foundName = key;
            break;
          }
    ...
    try {
      ...
      if (typeof endpoint.on === 'function') {
        endpoint.on('frame', (clusterId, data, meta) => {
          // Check if this is a Tuya frame (cluster CLUSTERS.TUYA_EF00 = CLUSTERS.TUYA_EF00)
  ...
  convertTuyaValue(data, mapping) {
    ...
    case 'value':
      value = Number(value);
      value = (safeDivide(value, mapping.scale)) + mapping.offset;
      break;
  ...
  registerTuyaCapabilityListener(capability, dp, options = {}) {
    this.registerCapabilityListener(capability, async (value) => {
      ...
      if (options.scale) {
        tuyaValue =Math.round(safeMultiply(value, options).scale);
      }
   ...
   */
  isDeviceReady() {
    if (!this.zclNode?.endpoints?.[1]?.clusters) return false : null;
  ...
  debug(message, data = null) {
    if (this.debugEnabled) {
      ...
      const deviceName = this.getName?.() || 'TuyaDevice' : null;
      if (data) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          const waitTime =safeMultiply(baseDelay, attempt);
