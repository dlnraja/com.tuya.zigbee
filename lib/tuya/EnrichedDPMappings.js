...
const CONVERTERS = {
  ...
  multiplyBy10: (v) => Number(v) * 10, // v5.5.999: Humidity raw 9 → 90% (Peter_van_Werkhoven ZG-204ZV)
  ...
  batteryState: (v) => ({ 0: 10, 1: 50, 2: 100 }[Number(v)] ?? Number(v)),
...
const CLIMATE_SENSOR_DPS = {
  // v6.0.0: Unified Presence/MMWave Radar DPs (from Z2M #2411, #3122)
  'presence_mmwave': {
    1: { capability: 'alarm_motion', converter: CONVERTERS.boolean },
    101: { capability: 'measure_luminance', converter: CONVERTERS.raw },
    102: { capability: 'radar_sensitivity', converter: CONVERTERS.raw, setting: 'sensitivity' },
    104: { capability: 'radar_fading_time', converter: CONVERTERS.raw, setting: 'fading_time' },
    105: { capability: 'radar_detection_delay', converter: CONVERTERS.raw, setting: 'detection_delay' },
    106: { capability: 'target_distance', converter: CONVERTERS.distanceMeters },
  },

  // v6.0.0: Advanced Soil Sensor (Z2M _TZE284_oitavov2)
  'soil_advanced': {
    1: { capability: 'measure_temperature', converter: CONVERTERS.tempCelsius },
    2: { capability: 'measure_humidity', converter: CONVERTERS.raw },
    3: { capability: 'measure_temperature.soil', converter: CONVERTERS.tempCelsius },
    4: { capability: 'measure_moisture', converter: CONVERTERS.raw },
    14: { capability: 'measure_battery', converter: CONVERTERS.batteryPercent },
    15: { capability: 'measure_battery.alt', converter: CONVERTERS.batteryPercent },
  },
  STANDARD: {
  ...
  // ZT08 weather station (_TZE284_hodyryli) — Z2M tuya.ts / GH #513
  // DP3 is battery_state enum 0/1/2 → 10/50/100%, NOT a 0-100 percent and NOT DP4×2.
  ZT08: {
    1: { capability: 'measure_temperature', converter: 'divideBy10', type: 'VALUE', name: 'temperature' },
    2: { capability: 'measure_humidity', converter: 'raw', type: 'VALUE', name: 'humidity' },
    3: { capability: 'measure_battery', converter: 'batteryState', type: 'ENUM', name: 'battery_state' },
    17: { capability: null, converter: 'boolean', type: 'BOOL', name: 'time_format' },
    38: { capability: 'measure_temperature.probe', converter: 'divideBy10', type: 'VALUE', name: 'external_temperature' },
  },
...
const SOIL_SENSOR_DPS = {
  // _TZE284_myd45weu, _TZE284_aao3yzhs, _TZE284_hdml1aav
  STANDARD: {
    ...
    15: { capability: 'measure_battery', converter: 'raw', type: 'VALUE', name: 'battery' },
    20: { capability: 'measure_ec', converter: 'raw', type: 'VALUE', name: 'ec_conductivity' },
    22: { capability: 'measure_ec', converter: 'raw', type: 'VALUE', name: 'fertilizer_ec' },
  },

  // Alternative mapping (some models, e.g. _TZE284_oitavov2)
  ALT: {
    1: { capability: 'measure_temperature', converter: 'divideBy10', type: 'VALUE', name: 'temperature' },
    3: { capability: 'measure_humidity', converter: 'raw', type: 'VALUE', name: 'soil_moisture' },
    5: { capability: 'measure_temperature', converter: 'divideBy10', type: 'VALUE', name: 'temperature' },
  ...
  // EC/Fertilizer variant (e.g. _TZE284_hdml1aav Flower Care)
  EC: {
    3: { capability: 'measure_humidity', converter: 'raw', type: 'VALUE', name: 'soil_moisture' },
    5: { capability: 'measure_temperature', converter: 'divideBy10', type: 'VALUE', name: 'temperature' },
    15: { capability: 'measure_battery', converter: 'multiplyBy2', type: 'VALUE', name: 'battery' },
...
const PRESENCE_SENSOR_DPS = {
  ...
  // Source: Z2M #23853, #23913 - Uses IAS Zone + Tuya DPs
  ...
  // v5.9.0: HOBEIAN radar (_TZE204_laokfqwu)
  // Source: User diagnostics & Z2M converters
  LAOKFQWU: {
    1: { capability: 'alarm_motion', converter: 'boolean', type: 'BOOL', name: 'presence' },
    2: { capability: null, converter: 'raw', type: 'VALUE', name: 'sensitivity' },
    9: { capability: 'measure_luminance.distance', converter: 'divideBy100', type: 'VALUE', name: 'distance' },
    102: { capability: 'measure_luminance', converter: 'raw', type: 'VALUE', name: 'illuminance' },
    103: { capability: null, converter: 'divideBy100', type: 'VALUE', name: 'target_distance' },
  },
};
...
const THERMOSTAT_DPS = {
  ...
  TRV_STANDARD: {
    ...
    14: { capability: null, converter: 'raw', type: 'VALUE', name: 'dim.valve' },
    ...
    104: { capability: null, converter: 'raw', type: 'VALUE', name: 'dim.valve_alt' },
  },
  ...
  ME167: {
    2: { capability: 'target_temperature', converter: 'divideBy10', type: 'VALUE', name: 'target_temp' },
    3: { capability: 'measure_temperature', converter: 'divideBy10', type: 'VALUE', name: 'current_temp' },
    4: { capability: null, converter: 'enum', type: 'ENUM', name: 'mode', values: { 0: 'auto', 1: 'manual', 2: 'off', 3: 'on' } },
    5: { capability: null, converter: 'enum', type: 'ENUM', name: 'system_mode', values: { 0: 'off', 1: 'heat' } },
    7: { capability: null, converter: 'boolean', type: 'BOOL', name: 'child_lock' },
    35: { capability: 'alarm_battery', converter: 'boolean', type: 'BOOL', name: 'battery_low' },
    36: { capability: null, converter: 'boolean', type: 'BOOL', name: 'frost_protection' },
    39: { capability: null, converter: 'divideBy10', type: 'VALUE', name: 'calibration_temp' },
    47: { capability: null, converter: 'raw', type: 'VALUE', name: 'dim.valve' },
    101: { capability: null, converter: 'boolean', type: 'BOOL', name: 'window_detection' },
    102: { capability: null, converter: 'boolean', type: 'BOOL', name: 'window_open' },
    103: { capability: null, converter: 'boolean', type: 'BOOL', name: 'boost' },
  ...
  BRT100: {
    2: { capability: 'target_temperature', converter: 'divideBy10', type: 'VALUE', name: 'target_temp' },
    3: { capability: 'measure_temperature', converter: 'divideBy10', type: 'VALUE', name: 'current_temp' },
    4: { capability: null, converter: 'enum', type: 'ENUM', name: 'mode', values: { 0: 'auto', 1: 'manual', 2: 'temporary_manual', 3: 'holiday' } },
    7: { capability: null, converter: 'boolean', type: 'BOOL', name: 'child_lock' },
    13: { capability: 'measure_battery', converter: 'raw', type: 'VALUE', name: 'battery' },
    14: { capability: null, converter: 'raw', type: 'VALUE', name: 'dim.valve' },
    101: { capability: null, converter: 'boolean', type: 'BOOL', name: 'window_detection' },
    102: { capability: null, converter: 'boolean', type: 'BOOL', name: 'window_open' },
    103: { capability: null, converter: 'boolean', type: 'BOOL', name: 'boost' },
    104: { capability: null, converter: 'enum', type: 'ENUM', name: 'comfort_temp' },
    105: { capability: null, converter: 'enum', type: 'ENUM', name: 'eco_temp' },
    106: { capability: null, converter: 'divideBy10', type: 'VALUE', name: 'max_temp' },
    107: { capability: null, converter: 'divideBy10', type: 'VALUE', name: 'min_temp' },
  },
};
...
const SWITCH_DPS = {
  ...
  /** P2485 Z2M colored backlight 4-gang (7ytnacie / hewlydpz TZE204) */
  COLORED_4GANG: {
    1: { capability: 'onoff', converter: 'boolean', type: 'BOOL', name: 'state_l1' },
    2: { capability: 'onoff.1', converter: 'boolean', type: 'BOOL', name: 'state_l2' },
    3: { capability: 'onoff.2', converter: 'boolean', type: 'BOOL', name: 'state_l3' },
    4: { capability: 'onoff.3', converter: 'boolean', type: 'BOOL', name: 'state_l4' },
    7: { capability: null, converter: 'raw', type: 'VALUE', name: 'countdown_l1' },
    8: { capability: null, converter: 'raw', type: 'VALUE', name: 'countdown_l2' },
    9: { capability: null, converter: 'raw', type: 'VALUE', name: 'countdown_l3' },
    10: { capability: null, converter: 'raw', type: 'VALUE', name: 'countdown_l4' },
    13: { capability: 'onoff', converter: 'boolean', type: 'BOOL', name: 'master' },
    14: { capability: 'power_on_behavior', converter: 'enum', type: 'ENUM', name: 'power_on' },
    15: { capability: null, converter: 'enum', type: 'ENUM', name: 'indicator_mode' },
    16: { capability: null, converter: 'boolean', type: 'BOOL', name: 'backlight_switch' },
    101: { capability: null, converter: 'boolean', type: 'BOOL', name: 'child_lock' },
    102: { capability: null, converter: 'raw', type: 'VALUE', name: 'backlight_pct' },
    103: { capability: null, converter: 'enum', type: 'ENUM', name: 'on_color' },
    104: { capability: null, converter: 'enum', type: 'ENUM', name: 'off_color' },
  },

  /** P2485 Homeetec simple 4-gang (_TZE200_hewlydpz) — backlight bool on DP7 */
  SIMPLE_4GANG_BL7: {
    1: { capability: 'onoff', converter: 'boolean', type: 'BOOL', name: 'state_l1' },
    2: { capability: 'onoff.1', converter: 'boolean', type: 'BOOL', name: 'state_l2' },
    3: { capability: 'onoff.2', converter: 'boolean', type: 'BOOL', name: 'state_l3' },
    4: { capability: 'onoff.3', converter: 'boolean', type: 'BOOL', name: 'state_l4' },
    7: { capability: null, converter: 'boolean', type: 'BOOL', name: 'backlight_mode' },
  },

  /** P2485 Z2M colored 3-gang rkbxtclc */
  COLORED_3GANG: {
    1: { capability: 'onoff', converter: 'boolean', type: 'BOOL', name: 'state_l1' },
    2: { capability: 'onoff.1', converter: 'boolean', type: 'BOOL', name: 'state_l2' },
    3: { capability: 'onoff.2', converter: 'boolean', type: 'BOOL', name: 'state_l3' },
...
// Alias for older call sites (MULTI_GANG typo/name)
SWITCH_DPS.MULTI_GANG = SWITCH_DPS.MULTIGANG;
...
const PLUG_DPS = {
  ...
  THREE_PHASE: {
    ...
    23: { capability: null, converter: 'energyKwh100', type: 'VALUE', name: 'produced_energy' },
    29: { capability: 'measure_power', converter: 'raw', type: 'VALUE', name: 'power' },
    32: { capability: null, converter: 'divideBy100', type: 'VALUE', name: 'ac_frequency' },
    50: { capability: null, converter: 'raw', type: 'VALUE', name: 'power_factor' },
    102: { capability: null, converter: 'raw', type: 'VALUE', name: 'update_frequency' },
    ...
    105: { capability: null, converter: 'raw', type: 'VALUE', name: 'power_a' },
    108: { capability: null, converter: 'raw', type: 'VALUE', name: 'power_factor_a' },
    109: { capability: null, converter: 'energyKwh100', type: 'VALUE', name: 'energy_a' },
    110: { capability: null, converter: 'energyKwh100', type: 'VALUE', name: 'energy_produced_a' },
    112: { capability: null, converter: 'divideBy10', type: 'VALUE', name: 'voltage_b' },
    113: { capability: null, converter: 'divideBy1000', type: 'VALUE', name: 'current_b' },
    114: { capability: null, converter: 'raw', type: 'VALUE', name: 'power_b' },
    117: { capability: null, converter: 'raw', type: 'VALUE', name: 'power_factor_b' },
    118: { capability: null, converter: 'energyKwh100', type: 'VALUE', name: 'energy_b' },
    119: { capability: null, converter: 'energyKwh100', type: 'VALUE', name: 'energy_produced_b' },
    121: { capability: null, converter: 'divideBy10', type: 'VALUE', name: 'voltage_c' },
    122: { capability: null, converter: 'divideBy1000', type: 'VALUE', name: 'current_c' },
    123: { capability: null, converter: 'raw', type: 'VALUE', name: 'power_c' },
  },
  ...
  TWO_CHANNEL: {
    ...
    8: { capability: null, converter: 'raw', type: 'VALUE', name: 'power_b' },
    9: { capability: 'measure_current', converter: 'divideBy1000', type: 'VALUE', name: 'current_a' },
    10: { capability: null, converter: 'divideBy1000', type: 'VALUE', name: 'current_b' },
    101: { capability: 'meter_power', converter: 'energyKwh100', type: 'VALUE', name: 'energy_a' },
    102: { capability: null, converter: 'energyKwh100', type: 'VALUE', name: 'energy_b' },
    111: { capability: null, converter: 'raw', type: 'VALUE', name: 'update_frequency' },
  },
  ...
  DIN_RAIL: {
    1: { capability: 'meter_power', converter: 'energyKwh100', type: 'VALUE', name: 'energy' },
    6: { capability: null, converter: 'raw', type: 'RAW', name: 'phase_data' },
    10: { capability: null, converter: 'enum', type: 'ENUM', name: 'fault', values: { 0: 'clear', 1: 'over_current', 2: 'over_power', 4: 'over_voltage', 8: 'wrong_frequency' } },
    16: { capability: 'onoff', converter: 'boolean', type: 'BOOL', name: 'switch' },
    17: { capability: null, converter: 'raw', type: 'RAW', name: 'threshold' },
    18: { capability: null, converter: 'raw', type: 'STRING', name: 'meter_id' },
    20: { capability: null, converter: 'boolean', type: 'BOOL', name: 'clear_fault' },
  },
};
...
const AIR_QUALITY_DPS = {
  ...
  // WHY(P2538): Z2M TS0601_airbox / PM2.5_airbox — COMPREHENSIVE was a dangling alias
  AIRBOX: {
    2: { capability: 'measure_co2', converter: 'raw', type: 'VALUE', name: 'co2' },
    18: { capability: 'measure_temperature', converter: 'divideBy10', type: 'VALUE', name: 'temperature' },
    19: { capability: 'measure_humidity', converter: 'raw', type: 'VALUE', name: 'humidity' },
    21: { capability: 'measure_voc', converter: 'divideBy100', type: 'VALUE', name: 'voc' },
    22: { capability: 'measure_formaldehyde', converter: 'divideBy100', type: 'VALUE', name: 'formaldehyde' },
  },
};
AIR_QUALITY_DPS.COMPREHENSIVE = AIR_QUALITY_DPS.AIRBOX;
...
const MANUFACTURER_DP_PROFILES = {
  ...
  '_TZE284_hodyryli': { profile: CLIMATE_SENSOR_DPS.ZT08, type: 'climate' },
  ...
  // v5.5.518: LeapMMW 5.8G radar - IAS Zone + Tuya DP (Z2M #23853, #23913)
  '_TZ321C_fkzihax8': { profile: PRESENCE_SENSOR_DPS.LEAPMW_5G8, type: 'radar_ias' },
  '_TZ321C_4slreunp': { profile: PRESENCE_SENSOR_DPS.LEAPMW_5G8, type: 'radar_ias' },
  '_TZE204_laokfqwu': { profile: PRESENCE_SENSOR_DPS.LAOKFQWU, type: 'radar' },
  ...
  // 3-Gang Colored Backlight Switch (Z2M TS0601_3gang_rkbxtclc)
  '_TZE204_rkbxtclc': { profile: SWITCH_DPS.COLORED_3GANG, type: 'switch_3gang_rgb' },
  // P2485 4-gang EF00 family (was misrouted curtain/illuminance/dimmer)
  '_TZE204_7ytnacie': { profile: SWITCH_DPS.COLORED_4GANG, type: 'switch_4gang_rgb' },
  '_TZE204_hewlydpz': { profile: SWITCH_DPS.COLORED_4GANG, type: 'switch_4gang_rgb' },
  '_TZE200_hewlydpz': { profile: SWITCH_DPS.SIMPLE_4GANG_BL7, type: 'switch_4gang' },
  '_TZE200_shkxsgis': { profile: SWITCH_DPS.MULTIGANG, type: 'switch_4gang' },
  '_TZE204_shkxsgis': { profile: SWITCH_DPS.MULTIGANG, type: 'switch_4gang' },
  '_TZE204_aagrxlbd': { profile: SWITCH_DPS.MULTIGANG, type: 'switch_4gang' },
  ...
  '_TZE284_hdml1aav': { profile: SOIL_SENSOR_DPS.EC, type: 'soil' },
...
class EnrichedDPMappings {
  ...
  /**
   * Get DP profile for manufacturer+productId couple.
   * WHY: one mfr ships many pids; refuse mfr-only lock.
   */
  static getProfile(manufacturerName, productId) {
    const mfr = manufacturerName ? String(manufacturerName) : '';
    const pid = productId ? String(productId) : '';
    if (!mfr) {return null;}
    if (pid) {
      const couple = MANUFACTURER_DP_PROFILES[`${mfr}|${pid}`];
      if (couple) {return couple;}
    }
    const profile = MANUFACTURER_DP_PROFILES[mfr] || null;
    if (!profile) {return null;}
    const models = profile.modelIds;
    if (!models || !pid || (Array.isArray(models) && models.includes(pid))) {return profile;}
    return null;
  }
  ...
  static getConverter(converterName) {
    if (typeof converterName === 'function') {return converterName;}
   ...
   */
  static parseDP(manufacturerName, dp, rawValue, productId) {
    const profile = this.getProfile(manufacturerName, productId);
    if (!profile?.profile?.[dp]) {return { value: rawValue, capability: null };}

    const dpConfig = profile.profile[dp];
    const converter = typeof dpConfig.converter === 'function' ? dpConfig.converter : this.getConverter(dpConfig.converter);
   ...
   */
  static getDPsForManufacturer(manufacturerName, productId) {
    const profile = this.getProfile(manufacturerName, productId);
   ...
   */
  static getDeviceType(manufacturerName, productId) {
    const profile = this.getProfile(manufacturerName, productId);
