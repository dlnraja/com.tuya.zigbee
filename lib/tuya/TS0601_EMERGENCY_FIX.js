...
async function applyTS0601EmergencyFix(device, zclNode) {
  try {
    ...
    if (modelId.toUpperCase() !== 'TS0601') {
      device.log('   ℹ️  Not a TS0601 - fix not needed');
      return false;
    }

    const needsFix = TS0601_MANUFACTURERS.some(m => manufacturer.toUpperCase().startsWith(m.toUpperCase()));
    if (!needsFix) {
      device.log('   ℹ️  Manufacturer not in fix list - fix not needed');
      return false;
    }
    ...
    if (!tuyaCluster) {
      device.log('   ❌ Tuya cluster 0xEF00 NOT FOUND!');
      device.log('   Available clusters:', Object.keys(endpoint.clusters || {}));
      return false;
    }

    device.log('   ✅ Tuya cluster 0xEF00 FOUND!');
      ...
      if (mapping) {
        ...
        if (!device.hasCapability(capability)) {
          ...
          try {
            ...
            device.log(`   ⚠️  Could not add ${capability}: ${err.message}`);
          }
        }
        ...
        device.log(`   ℹ️  [EMERGENCY] Unknown DP ${dp}, value: ${JSON.stringify(value)}`);
      }
    });
    ...
    for (const dp of dpList) {
      try {
        ...
        await new Promise(resolve => (device.homey && typeof device.homey.setTimeout === 'function' ? device.homey : globalThis).setTimeout(resolve, 500));
        if (device?._destroyed) {return;}

      } catch (err) {
        device.log(`   ⚠️  DP ${dp} request failed: ${err.message}`);
      }
    }
    ...
    await new Promise(resolve => (device.homey && typeof device.homey.setTimeout === 'function' ? device.homey : globalThis).setTimeout(resolve, 5000));
    if (device?._destroyed) {return false;}

    if (receivedDataReports > 0) {
      device.log(`   🎉 SUCCESS! Received ${receivedDataReports} dataReports!`);
    } else {
      device.log('   ⚠️  WARNING: No dataReports received yet. Device may need time to respond.');
    }
