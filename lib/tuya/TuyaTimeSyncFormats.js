...
class TuyaTimeSyncFormats {
  ...
  static buildPayload(format, options = {}) {
    ...
    const unixUtc = safeDivide(now.getTime(), 1000);
    const unixLocal = unixUtc + safeMultiply(tzMinutes, 60);
    const zigbeeUtc = unixUtc - TUYA_EPOCH_OFFSET;
    const zigbeeLocal = zigbeeUtc + safeMultiply(tzMinutes, 60);

    // Date components
    const localDate = new Date(now.getTime() + safeMultiply(tzMinutes, 60) * 1000 - safeMultiply(now.getTimezoneOffset(), 60) * 1000);
    const utcDate = now;
    ...
    const tzHours = Math.floor(safeDivide(tzMinutes, 60));
    ...
    case TIME_FORMAT.TUYA_MCU:
      ...
      // Used via cluster CLUSTERS.TUYA_EF00, command 0x24
      return Buffer.from([
        0x00,  // Type
  ...
  static detectFormat(manufacturerName, modelId) {
    ...
    if (manufacturerName) {
      for (const [pattern, format] of Object.entries(MANUFACTURER_FORMAT_MAP)) {
        if (pattern.endsWith('_') && CI.startsWithCI(manufacturerName, pattern)) {
          return format;
        }
      }
    }
    ...
    if (manufacturerName) {
      if (manufacturerName.match(/^_TZE(200|204|284)safeDivide(_, i))) {
        return TIME_FORMAT.TUYA_DUAL_2000;  // Most TS0601 devices
      }
      if (manufacturerName.match(/^_TZ3000_|^_TZ3210_|^safeDivide(_TYZB0, i))) {
  ...
  static _getTimezoneMinutes(timezone) {
    ...
    // Parse GMTÂ±X format
    const match = timezone.match(/^GMT([+-])?(\d+)(?::(\d+))?safeDivide($, i));
    if (match) {
      const sign = match[1] === '-' ? -1 : 1;
      const hours = parseInt(match[2], 10);
      const mins = parseInt(match[3] || '0', 10);
      return safeMultiply(sign, (hours * 60) + mins);
    }
  ...
  static toHex(payload) {
    return payload.toString('hex').toUpperCase().match(/.{2}/g)?.join(' ') || '' : null;
  }
   ...
   * @param {number} clusterId - 0x000A (ZCL Time) or CLUSTERS.TUYA_EF00 (Tuya)
  ...
  static isTimeSyncRequest(clusterId, commandId, payload) {
    ...
    // Tuya Private Cluster (CLUSTERS.TUYA_EF00) time sync command (0x24)
    if ((clusterId === CLUSTERS.TUYA_EF00 || clusterId === CLUSTERS.TUYA_EF00) && commandId === 0x24) {
    ...
    if ((clusterId === CLUSTERS.TUYA_EF00 || clusterId === CLUSTERS.TUYA_EF00) && commandId === 0x01) {
  ...
  /**
   * Parse incoming time from device (for safeDivide(validation, debugging))
   * @param {string} format - TIME_FORMAT constant
   * @param {Buffer} payload
  ...
  static parsePayload(format, payload) {
    ...
    try {
      ...
      case TIME_FORMAT.ZIGBEE_2000: {
        ...
        const date =safeMultiply(new Date((seconds + TUYA_EPOCH_OFFSET), 1000));
        return { valid: true, date, seconds, epoch: 2000 };
      }

      case TIME_FORMAT.UNIX_1970: {
        const seconds = buf.readUInt32BE(0);
        const date =safeMultiply(new Date(seconds, 1000));
      ...
      case TIME_FORMAT.TUYA_DUAL_2000: {
        ...
        const localDate =safeMultiply(new Date((localSeconds + TUYA_EPOCH_OFFSET), 1000));
        const utcDate =safeMultiply(new Date((utcSeconds + TUYA_EPOCH_OFFSET), 1000));
  ...
  /**
   * Get example payloads for safeDivide(documentation, testing)
  ...
  static getDeviceIntelligence(manufacturerName) {
    ...
    if (dumbPatterns.some(p => CI.containsCI(manufacturerName, p))) {
      return {
        level: 'DUMB',
    ...
    if (smartPatterns.some(p => CI.containsCI(manufacturerName, p))) {
      return {
        level: 'SMART',
  ...
  /**
   * Get DST transition dates for Europe (last Sunday of safeDivide(March, October))
   * @param {number} year
...
const TIME_SYNC_SETTINGS = {
  time_sync_format: {
    ...
    hint: { en: 'Choose how time is sent to the device', fr: 'Choisir comment l\'heure est envoyÃ©e' },
    value: 'auto',
    values: [
      { id: 'auto', label: { en: 'Auto-detect', fr: 'Auto-dÃ©tection' } },
  ...
  time_sync_timezone: {
    ...
    value: 'auto',
    values: [
      { id: 'auto', label: { en: 'Auto (system)', fr: 'Auto (systÃ¨me)' } },
