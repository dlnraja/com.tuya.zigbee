...
class TuyaTimeSyncFormats {
  ...
  static detectFormat(manufacturerName, modelId, options = {}) {
    // P91: Use TuyaNormalizer for case-insensitive mfr matching (was: raw toLowerCase + exact match)
    const mfrNorm = TU.normalize(manufacturerName || '');
    // Build a normalized map once (lazy)
    if (!this._normalizedMfrMap) {
      this._normalizedMfrMap = new Map();
      for (const [k, v] of Object.entries(MANUFACTURER_FORMAT_MAP)) {
        this._normalizedMfrMap.set(TU.normalize(k), v);
      }
    }
    // Check exact manufacturer match (case-insensitive)
    if (mfrNorm && this._normalizedMfrMap.has(mfrNorm)) {
      return this._normalizedMfrMap.get(mfrNorm);
    }

    // Check manufacturer pattern (prefix)
    if (mfrNorm) {
      for (const [pattern, format] of Object.entries(MANUFACTURER_FORMAT_MAP)) {
        if (pattern.endsWith('_') && mfrNorm.startsWith(TU.normalize(pattern))) {
          return format;
        }
      }
    }
  ...
  static guessFormat(deviceInfo = {}) {
    ...
    const combined = TU.normalize(`${manufacturerName}_${productId}_${modelId}`);
    ...
    if (model === 'TS0601') {
      ...
      // WHY: pid is shared (IAS water leak AND mains repeater). Both are ZCL time, not MCU.
      addScore(TIME_FORMAT.ZIGBEE_2000, 35, 'TS0207 = ZCL water leak or repeater (not MCU)');
    ...
    // ZT08 LCD weather station — Z2M timeStart "1970" + DP17 commit (GH #513)
    if (combined.includes('hodyryli')) {
      addScore(TIME_FORMAT.Z2M_DUAL_1970, 80, 'ZT08 / _TZE284_hodyryli = Z2M dual 1970');
      addScore(TIME_FORMAT.TUYA_DUAL_1970, 20, 'ZT08 alternate dual-1970 endianness');
    }
  ...
  static getDeviceIntelligence(manufacturerName) {
    ...
    if (zt08Patterns.some(p => mfrLower.includes(p.toLowerCase()))) {
      return {
        ...
        action: 'Use Z2M_DUAL_1970 then DP17 commit',
        format: TIME_FORMAT.Z2M_DUAL_1970,
        commitDP: 17,
        commitDelay: 500,
      };
    }
