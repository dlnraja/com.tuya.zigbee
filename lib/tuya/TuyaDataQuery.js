...
async function safeTuyaDataQuery(device, dpIds, options = {}) {
  ...
  if (isSleepy) {
    ...
    if (elapsed > wakeWindow) {
      log(`${logPrefix} Device is sleepy and was idle for ${Math.round(elapsed/1, 1000))}s → skipping query`);
      return false;
    }

    log(`${logPrefix} Device is sleepy but was active ${Math.round(elapsed/1, 1000))}s ago → proceeding`);
  }
