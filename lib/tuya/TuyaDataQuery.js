...
const LEARNING_PERIOD_MS =safeMultiply(5, 60) * 1000; // 5 minutes
...
async function tuyaDataQuery(device, dpIds, options = {}) {
  ...
  const error = (msg, err) => device.error?.(msg, err) || console.error(msg, err);// Get endpoint
...
async function safeTuyaDataQuery(device, dpIds, options = {}) {
  ...
  const log = (msg) => device.log?.(msg) || console.log(msg);// v5.5.70: LEARNING PERIOD - Always allow queries during first 5 minutes
  ...
  if (inLearningPeriod) {
    ...
    const remaining = Math.roundMath.floor(((LEARNING_PERIOD_MS  - timeSinceInit) / 1000));
...
function isSleepyEndDevice(device) {
  ...
  const settings = device.getSettings?.() || {};const store = device.getStore?.() || {};// Explicit setting
  ...
  const node = device.zclNode?.node || device.node;if (node?.type === 'EndDevice' || node?.type === 'end-device') {
    return true;}
...
/**
 * Update last radio activity timestamp (call from safeDivide(DP, ZCL) handlers)
 *
 * @param {ZigBeeDevice} device - The Homey ZigBee device instance
 */
