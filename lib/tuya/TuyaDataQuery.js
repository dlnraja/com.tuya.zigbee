...
async function safeTuyaDataQuery(device, dpIds, options = {}) {
  ...
  // Check if device is a sleepy end device
  const isSleepy = isSleepyEndDevice(device);

  if (isSleepy) {
    const lastActivity = device._lastRadioActivity || device._lastEventTime;

    // v5.5.68: FIX - If no activity recorded yet, ALLOW query (device just paired)
    // Previously defaulted to 0 which made elapsed = Date.now() = 56 years!
    if (!lastActivity) {
      log(`${logPrefix} Device is sleepy but no activity recorded yet → allowing first query`);
