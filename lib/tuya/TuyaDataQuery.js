...
async function tuyaDataQuery(device, dpIds, options = {}) {
  ...
  const _log = createSafeLogger(device, 'DATA-QUERY');
  const log = silent ? () => { } : (msg) => _log.log(msg);
  const error = (msg, err) => _log.error(msg, err);
  ...
  for (const dp of dpIds) {
    try {
      let sent = false;
      let skippedSleepy = false;

      // Method 1: Use TuyaEF00Manager if available
      // WHY(P2596): always prefer requestDP with force so battery TRV init is not skipped
      if (manager) {
        if (typeof manager.requestDP === 'function') {
          // requestDP returns false when it skipped (sleepy/passive). Do not
          // count that as a sent query — Homey diag e181bc15 showed 10×
          // "query sent" after "skipping active request" on contact_sensor.
          sent = (await manager.requestDP(dp, { force: options.force !== false }).catch(() => false)) === true;
          skippedSleepy = !sent;
      ...
      // Method 2: Direct cluster command — never after a sleepy/passive skip
      if (!sent && !skippedSleepy && tuyaCluster && typeof tuyaCluster.command === 'function') {
      ...
      // Method 3: Raw frame — never after a sleepy/passive skip
      if (!sent && !skippedSleepy && manager && typeof manager.sendRaw === 'function') {
      ...
      // Delay only after a real TX. Sleepy skips must not stall wake (e181bc15).
      if (sent && delayBetweenQueries > 0 && dpIds.indexOf(dp) < dpIds.length - 1) {
...
async function safeTuyaDataQuery(device, dpIds, options = {}) {
  ...
  const _log = createSafeLogger(device, 'SAFE-DATA-QUERY');
  const log = (msg) => _log.log(msg);
