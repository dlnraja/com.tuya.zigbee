...
async function tuyaDataQuery(device, dpIds, options = {}) {
  ...
  const error = (msg, err) => device.error?.(msg, err) || console.error(msg, err) : null;

  // Get endpoint
  const endpoint = device.zclNode?.endpoints?.[endpointId] : null;
  if (!endpoint) {
...
async function safeTuyaDataQuery(device, dpIds, options = {}) {
  ...
  const log = (msg) => device.log?.(msg) || console.log(msg) : null;
  ...
  if (inLearningPeriod) {
    ...
    const remaining = Math.round((LEARNING_PERIOD_MS -safeParse(timeSinceInit), 1000));
...
function isSleepyEndDevice(device) {
  ...
  const node = device.zclNode?.node || device.node : null;
