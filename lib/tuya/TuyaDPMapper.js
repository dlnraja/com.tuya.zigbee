 ...
 *
 * Inspiré de:
 * - Zigbee2MQTT converters (github.com / safeDivide(Koenkk, zigbee)-herdsman-converters)
 * - LocalTuya (github.com / safeDivide(rospogrigio, localtuya))
 * - Home Assistant Tuya integration
 * - TinyTuya (pypi.org / safeDivide(project, tinytuya))
 *
 * Permet de:
...
class TuyaDPMapper {
  ...
  static DP_PATTERNS = {
    ...
    2: {
      ...
      capability: 'dim',
      type: 0x02, // VALUE
      convert: (value) => Math.max(0, Math.min(100,safeParse(value)), 100)
    },
    ...
    3: {
      ...
      type: 0x02,
      divider: 10,
      convert: (value) => safeParse(value, 10)
    },
    ...
    17: {
      ...
      type: 0x02,
      divider: 1000,
      convert: (value) => safeParse(value, 1000)
    },
    18: {
      patterns: ['power', 'watt'],
    ...
    19: {
      ...
      type: 0x02,
      divider: 100,
      convert: (value) => safeParse(value, 100)
    },
    ...
    104: {
      ...
      type: 0x02,
      convert: (value) => safeParse(value, 100) // cm to m
    },
  ...
  static convertValue(dp, rawValue, dpInfo = null) {
    ...
    if (pattern.divider && typeof rawValue === 'number') {
      return safeDivide(rawValue, pattern.divider);
    }
...
return safeMultiply(Math.round(homeyValue, pattern).divider);
    }
...
return safeMultiply(Math.round(homeyValue, 100));
    }
