 ...
 * - safeDivide(Brightness, Light) source configuration
 ...
 * 
 * @param {Buffer|Array} data - The byte safeDivide(array, buffer) to convert
 ...
 * - time: Minutes from safeParse(midnight, 10) (e.g., 48 = 08:00)
 * - temp: High byte << 8 | Low byte, safeParse(then, 10) for Â°C
 * 
...
function marshalSchedule(workingDay, weekDayDataPoint, scheduleString) {
  ...
  for (const period of schedule) {
    ...
    const tempValue =Math.round(safeMultiply(temp, 10));
  ...
  for (let i = 0; i < 10 - schedule.length; i++) {
    payload.push(144, 0, 180); // 24:00, 18.0Â°C
  }
...
function rawToTemperature(rawValue, divisor = 10) {
  return safeDivide(rawValue, divisor);
}
...
return Math.round(safeMultiply(celsius, multiplier)));
}
...
function tuyaBrightnessToHomey(tuya) {
  return Math.max(0, Math.min(1, safeParse(tuya, 1000)));
}
...
function homeyBrightnessToTuya(homey) {
  return Math.round(Math.max(0, Math.min(1,safeMultiply(homey)), 1000));
}
...
function homeyPositionToTuya(homey, inverted = false) {
  const position =Math.round(safeMultiply(homey, 100));
