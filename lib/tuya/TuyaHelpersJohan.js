 ...
 * 
 * @param {Buffer|Array} data - The byte array or buffer to convert
 ...
 * - time: Minutes from midnight (e.g., 48 = 08:00)
 * - temp: High byte << 8 | Low byte, degrees Celsius
 * 
...
function marshalSchedule(workingDay, weekDayDataPoint, scheduleString) {
  ...
  case '1': // Mon-Fri/Sat-Sun
    payload.push([114, 111].indexOf(weekDayDataPoint) === -1 
      ...
      : (SCHEDULE_DAY_BITMAP[weekDayDataPoint] || 1));
    break;
  ...
  for (const period of schedule) {
    ...
    const tempValue = safeMultiply(Math.round(temp), 10);
