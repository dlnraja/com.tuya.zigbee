...
class UniversalTimeSync {
  ...
  log(msg) {
    if (this.options.verbose !== false) { // Always log time sync unless explicitly disabled
  ...
  async initialize() {
    ...
    this._recognitionTimer = this.homey.setTimeout(async () => {
  ...
  _startPeriodicSync() {
    ...
    this._syncTimer = this.homey.setInterval(async () => {
      if (this._destroyed) {return;}
