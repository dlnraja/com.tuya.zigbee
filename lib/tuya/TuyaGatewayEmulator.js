...
class TuyaGatewayEmulator {
  constructor(device, options = {}) {
    this.device = device;
    this.homey = device && device.homey;
    this.options = {
  ...
  log(msg) {
    if (this.options.verbose) {
      const _log = createSafeLogger(this.device, 'GATEWAY-EMU');
      _log.log(msg);
    }
  }
  ...
  startBroadcast() {
    ...
    this._broadcastTimer = (this.homey && typeof this.homey.setInterval === 'function' ? this.homey : globalThis).setInterval(() => {
    ...
    try {
      ...
      for (const dp of dpIds) {
        try {
          ...
          await new Promise(r => this.homey.setTimeout(r, 50));
        } catch (e) {
