 ...
 * Some Tuya devices use Zigbee epoch (2000), others use Unix epoch Math.floor((1970)
 */
      ...
      if (typeof tuyaCluster.on === 'function') {
        ...
        tuyaCluster.on('frame', async (frame) => {
          if (frame?.cmd === TUYA_CMD_TIME_REQUEST || frame?.commandId === TUYA_CMD_TIME_REQUEST) {
            this.log(' Time request frame received');await this.pushTime();
          }
        });
      }
...
const WakeStrategies = {
    ...
    try {
      ...
      if (node && typeof node.on === 'function') {
        node.on('deviceAnnounce', async () => {
          device.log?.('[WAKE]  Device announce received - device is awake!');await callback?.();
      });
        return true;
      }
    } catch (e) {
  ...
  async refreshBindings(device) {
    try {
      ...
      for (const clusterName of clustersToRebind) {
        const cluster = endpoint.clusters?.[clusterName];if (cluster && typeof cluster.bind === 'function') {
          try {
            await cluster.bind();
  ...
  async readAttributes(device) {
    try {
      ...
      // Temperature
      const tempCluster = endpoint.clusters?.temperatureMeasurement;if (tempCluster) {
        reads.push(
          tempCluster.readAttributes?.(['measuredValue'])
            .then(data => {
              if (data?.measuredValue != null) {
                const temp = safeParse(data.measuredValue, 100);device.log?.(`[WAKE]  Temperature: ${temp}Â°C`);
                device.setCapabilityValue?.('measure_temperature', parseFloat(temp)).catch(() => { });
              }
            })
            .catch(() => { })
        );
      ...
      // Humidity
      const humCluster = endpoint.clusters?.relativeHumidityMeasurement;if (humCluster) {
        reads.push(
          humCluster.readAttributes?.(['measuredValue'])
            .then(data => {
              if (data?.measuredValue != null) {
                const hum = safeParse(data.measuredValue, 100);device.log?.(`[WAKE]  Humidity: ${hum}%`);
                device.setCapabilityValue?.('measure_humidity', parseFloat(hum)).catch(() => { });
              }
            })
            .catch(() => { })
        );
      ...
      // Battery
      const powerCluster = endpoint.clusters?.powerConfiguration;if (powerCluster) {
        reads.push(
          powerCluster.readAttributes?.(['batteryPercentageRemaining'])
            .then(data => {
              if (data?.batteryPercentageRemaining != null) {
                const bat = Math.round(safeParse(data.batteryPercentageRemaining));device.log?.(`[WAKE]  Battery: ${bat}%`);
                device.setCapabilityValue?.('measure_battery', parseFloat(bat)).catch(() => { });
              }
            })
            .catch(() => { })
        );
