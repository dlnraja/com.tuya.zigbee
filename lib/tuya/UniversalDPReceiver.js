...
class UniversalDPReceiver {
  ...
  setupTuyaListener(zclNode, endpoint = 1) {
    ...
    for (const epNum of eps) {
      ...
      const cluster = ep?.clusters?.tuya || ep?.clusters?.manuSpecificTuya || ep?.clusters?.tuyaSpecific ||
                      ep?.clusters?.[0xEF00] || ep?.clusters?.[61184] || ep?.clusters?.['61184'];
      if (!cluster) {continue;}

      // Fallback 1-4: Standard events
      for (const evt of ['dataReport', 'response', 'report', 'datapoint']) {
        if (typeof cluster.on === 'function') {
          try { cluster.on(evt, (d) => this.handleReport(d)); bound++; } catch { /* event not supported by this cluster impl */ }
        }
      }
      // Fallback 5: addEventListener
      if (typeof cluster.addEventListener === 'function') {
        try { cluster.addEventListener('dataReport', (d) => this.handleReport(d)); bound++; } catch { /* addEventListener not supported */ }
      }
      ...
      if (cluster.reportHandler && !cluster._dpRxWrapped) {
        ...
        } catch { /* reportHandler wrap failed — cluster not compatible */ }
      }
      // Fallback 7: onReport
      if (typeof cluster.onReport === 'function') {
        try { cluster.onReport((d) => this.handleReport(d)); bound++; } catch { /* onReport not supported */ }
      }
    }
    ...
    if (mgr?.on && !mgr._dpRxBound) {
      ...
      } catch { /* TuyaEF00Manager event binding failed */ }
    }
    ...
    this._log(bound > 0 ? `✅ Tuya: ${bound} hooks` : '⚠️ No Tuya cluster');
    return bound > 0;
  }
  ...
  setupZclListener(zclNode, clusterId, attributes, endpoint = 1) {
    ...
    for (const epNum of eps) {
      ...
      for (const attr of attrs) {
        ...
        for (const evt of [`attr.${attr}`, `report.${attr}`, `change.${attr}`]) {
          if (typeof cluster.on === 'function') {
            try { cluster.on(evt, (v) => this._handleZclAttr(clusterId, attr, v)); bound++; } catch { /* event not supported */ }
          }
        }
        // Fallback 4: addEventListener
        if (typeof cluster.addEventListener === 'function') {
          try { cluster.addEventListener(`attr.${attr}`, (v) => this._handleZclAttr(clusterId, attr, v)); bound++; } catch { /* addEventListener not supported */ }
        }
      }
      ...
      if (typeof cluster.onAttributeReport === 'function') {
        ...
        }); bound++; } catch { /* onAttributeReport not supported */ }
      }
      // Fallback 6: reportHandler wrap
    ...
    this._log(bound > 0 ? `✅ ZCL ${clusterId}: ${attrs.join(',')} (${bound})` : `⚠️ No cluster ${clusterId}`);
    return bound > 0;
  }
  ...
  setupFrameListener(zclNode, clusterId, endpoint = 1) {
    ...
    for (const epNum of eps) {
      ...
      if (ep.handleFrame && !ep._frameRxWrapped) {
        ...
        } catch { /* handleFrame wrap failed — endpoint not compatible */ }
      }
      // Fallback 2-4: cluster.on events
      if (cluster?.on) {
        for (const evt of ['frame', 'data', 'raw']) {
          try { cluster.on(evt, (f) => this.handleFrame(f, { cluster: clusterId })); bound++; } catch { /* event not supported */ }
        }
      }
      // Fallback 5: onFrame
      if (typeof cluster?.onFrame === 'function') {
        try { cluster.onFrame((f) => this.handleFrame(f, { cluster: clusterId })); bound++; } catch { /* onFrame not supported */ }
      }
      // Fallback 6: subscribe
      if (typeof cluster?.subscribe === 'function') {
        try { cluster.subscribe((d) => this.handleFrame(d, { cluster: clusterId })); bound++; } catch { /* subscribe not supported */ }
      }
    }

    this._log(bound > 0 ? `✅ Frame ${clusterId}: ${bound} hooks` : '⚠️ No frame hooks');
    return bound > 0;
  }
