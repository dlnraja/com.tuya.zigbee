...
/** Z2M switch_type: toggle=0, state=1, momentary=2 */
function switchTypeToCode(mode) {
  const m = String(mode || 'state').toLowerCase();
  if (m === 'toggle') return 0;
  if (m === 'momentary') return 2;
  return 1; // state (recommended for lighting)
}

/**
 * Force switch type (Z2M exposes toggle|state|momentary).
 * Paths: E001.switchMode, OnOff 0x8004, manuSpecificTuya3.switchType
 */
async function forceSwitchTypeState(device, mode = 'state') {
  const wanted = switchTypeToCode(mode);
  const label = wanted === 0 ? 'toggle' : wanted === 2 ? 'momentary' : 'state';
  let ok = false;
  ...
  try {
    await device.setSettings?.({ switch_mode: label }).catch(() => {});
  } catch (_e) { /* soft */ }

  device.log?.(`[P2632/P2670] force switch_type=${label} (ok=${ok})`);
  return ok;
}

/**
 * WHY(P2670 / Z2M ZG-301Z): countdown is ZCL onWithTimedOff / onTime — NOT EF00.
 * Contre quoi: leftover EF00 setCountdown on noEf00Tx HOBEIAN floods mesh + no effect.
 */
async function setHobeianCountdown(device, seconds) {
  const sec = Math.max(0, Math.min(43200, Math.round(Number(seconds) || 0)));
  const ep = device.zclNode?.endpoints?.[1];
  const onOff = ep?.clusters?.onOff || ep?.clusters?.genOnOff || ep?.clusters?.[6];
  if (!onOff) return false;
  try {
    if (sec <= 0) {
      ...
      device.log?.('[P2670] countdown cleared (0s)');
      return true;
    }
    // Prefer onWithTimedOff (onTime in 1/10s on some stacks; Z2M uses seconds)
    if (typeof onOff.onWithTimedOff === 'function') {
      await onOff.onWithTimedOff({
        ctrlBits: 0,
        onTime: sec * 10,
        offWaitTime: 0,
      }).catch(async () => {
        await onOff.onWithTimedOff({ onTime: sec, offWaitTime: 0 }).catch(() => {});
      });
    } else if (typeof onOff.writeAttributes === 'function') {
      await onOff.writeAttributes({ onTime: sec }).catch(() => {});
    }
    device.log?.(`[P2670] countdown set ${sec}s via ZCL`);
    return true;
  } catch (e) {
    device.log?.(`[P2670] countdown soft-fail: ${e.message}`);
    return false;
  }
}
...
async function healHobeianZg301z(device, zclNode) {
  ...
  // Prefer fleet IntelligentEnergyAdapter (P2672) so all non-metered mains share one path.
  try {
    const IntelligentEnergyAdapter = require('../energy/IntelligentEnergyAdapter');
    await IntelligentEnergyAdapter.apply(device, { force: true });
  } catch (_e) {
    try {
      if (typeof device.setEnergy === 'function') {
        await device.setEnergy({
          approximation: { usageConstant: 0.3 },
        }).catch(() => {});
