...
let _loading = false;
/** WHY(P2674): first load skipped broad catalog — allow merge when heap recovers */
let _broadSkippedDueToPressure = false;
/** WHY(P2678): shard ids already merged into DEVICE_FINGERPRINTS */
const _loadedShards = new Set();
let _shardManifest = null;
...
// WHY(P2674): curated small paths first — broad lib/tuya/fingerprints.json (~0.9MB)
// tips Homey Pro over 64MB when many drivers already occupy ~63MB (diag 149bc1a5).
const CURATED_FINGERPRINT_PATHS = [
  path.resolve(__dirname, '../../data/fingerprints.json'), // Root curated (~16KB)
...
// WHY(P2678): prefer on-demand shards (~max 63KB) over monolith 0.9MB
const FP_SHARDS_DIR = path.resolve(__dirname, './fp-shards');
const BROAD_FINGERPRINT_PATHS = [
  path.resolve(__dirname, './fingerprints.json'),          // Broad bundled catalog (fallback only)
];
const FINGERPRINT_PATHS = [...CURATED_FINGERPRINT_PATHS, ...BROAD_FINGERPRINT_PATHS];

/** Broad catalog soft size gate under memory pressure (Homey Pro ~64MB). */
const BROAD_MAX_BYTES_UNDER_PRESSURE = 64 * 1024;
/** Single shard max — Contre quoi full-file parse */
const SHARD_MAX_BYTES = 200 * 1024;

function _bootBudget() {
  try {
    return require('../performance/BootBudget');
  } catch {
    return null;
  }
}

function _isFpLoadPressure() {
  try {
  ...
  try {
    const bb = _bootBudget();
    if (bb && typeof bb.isHeapCritical === 'function' && bb.isHeapCritical()) return true;
  } catch { /* soft */ }
  return false;
}
...
function _readFingerprintSource(fpath, opts = {}) {
  ...
  try {
    const Lazy = require('../performance/IntelligentLazyLoad');
    if (typeof Lazy.loadJsonBuffer === 'function') {
      const data = Lazy.loadJsonBuffer(fpath, {
        gc: true,
...
function _shardsAvailable() {
  try {
    return fs.existsSync(path.join(FP_SHARDS_DIR, 'manifest.json'));
  } catch {
    return false;
  }
}

function _getShardManifest() {
  if (_shardManifest !== null) return _shardManifest;
  try {
    const p = path.join(FP_SHARDS_DIR, 'manifest.json');
    if (!fs.existsSync(p)) {
      _shardManifest = false;
      return null;
    }
    const Lazy = require('../performance/IntelligentLazyLoad');
    _shardManifest = Lazy.loadJsonBuffer(p, { maxBytes: 512 * 1024 }) || false;
  } catch {
    _shardManifest = false;
  }
  return _shardManifest || null;
}

/**
 * WHY(P2678): load exactly one small shard for this manufacturer (~tens of KB).
 * Merges into DEVICE_FINGERPRINTS and refreshes lowercase index.
 */
function _ensureShardForManufacturer(manufacturerName) {
  if (!manufacturerName || !_shardsAvailable()) return false;
  let shardId;
  try {
    shardId = require('./fingerprintShardId').fingerprintShardId(manufacturerName);
  } catch {
    return false;
  }
  if (!shardId || _loadedShards.has(shardId)) return _loadedShards.has(shardId);

  const shardPath = path.join(FP_SHARDS_DIR, `${shardId}.json`);
  if (!fs.existsSync(shardPath)) {
    _loadedShards.add(shardId); // negative cache — don't retry missing
    return false;
  }

  const data = _readFingerprintSource(shardPath, { maxBytes: SHARD_MAX_BYTES });
  if (!data) {
    _loadedShards.add(shardId);
    return false;
  }
  ...
  Object.assign(DEVICE_FINGERPRINTS, data);
  _loadedShards.add(shardId);
  _buildLowercaseIndex();
  try {
    const bb = _bootBudget();
    if (bb && typeof bb.maybeGc === 'function') bb.maybeGc();
  } catch { /* soft */ }
  return true;
}

/**
 * WHY(P2678): after boot, warm a few shards per tick — never the 0.9MB monolith.
 */
function _tryUpgradeBroadCatalogBatched() {
  if (_loading || _isFpLoadPressure()) return;
  if (!_shardsAvailable()) {
    // Legacy fallback: only if no shards (dev without rebuild)
    _tryUpgradeBroadCatalogMonolith();
    return;
  }
  const man = _getShardManifest();
  if (!man || !man.shards) {
    _broadSkippedDueToPressure = false;
    return;
  }
  const pending = Object.keys(man.shards).filter((id) => !_loadedShards.has(id));
  if (!pending.length) {
    _broadSkippedDueToPressure = false;
    return;
  }
  _loading = true;
  try {
    const batch = pending.slice(0, 8).map((id) => path.join(FP_SHARDS_DIR, `${id}.json`));
    const Lazy = require('../performance/IntelligentLazyLoad');
    const { loaded } = Lazy.loadJsonBuffersBatched(batch, {
      into: DEVICE_FINGERPRINTS || (DEVICE_FINGERPRINTS = {}),
      maxBytes: SHARD_MAX_BYTES,
      maxFiles: 8,
      stopOnPressure: true,
    });
    for (const id of pending.slice(0, loaded)) _loadedShards.add(id);
    if (loaded > 0) _buildLowercaseIndex();
    if (_loadedShards.size >= Object.keys(man.shards).length) {
      _broadSkippedDueToPressure = false;
    }
  } catch { /* soft */ }
  _loading = false;
...
function _tryUpgradeBroadCatalogMonolith() {
  if (!_broadSkippedDueToPressure || _loading || _isFpLoadPressure()) return;
  if (!DEVICE_FINGERPRINTS || typeof DEVICE_FINGERPRINTS !== 'object') return;
  _loading = true;
  try {
    const seen = new Set(
      CURATED_FINGERPRINT_PATHS.map((p) => {
        try { return path.resolve(p); } catch { return p; }
      }),
    );
    const before = Object.keys(DEVICE_FINGERPRINTS).length;
    _mergePathList(BROAD_FINGERPRINT_PATHS, DEVICE_FINGERPRINTS, seen, {
      maxBytes: 2 * 1024 * 1024,
    });
    if (Object.keys(DEVICE_FINGERPRINTS).length > before) {
      _buildLowercaseIndex();
      _broadSkippedDueToPressure = false;
      try {
        const bb = _bootBudget();
  ...
  } catch { /* soft — keep curated */ }
  _loading = false;
}

function _mergePathList(paths, merged, seen, opts) {
  for (const fpath of paths) {
    const resolved = path.resolve(fpath);
    if (seen.has(resolved)) {continue;}
    seen.add(resolved);
    try {
      const data = _readFingerprintSource(resolved, opts);
      if (!data) {continue;}
      Object.assign(merged, data);
    } catch (_err) { /* next path */ }
  }
}

/**
 * WHY(P2674/P2678): Under pressure load curated only.
 * With shards present, never merge the 0.9MB monolith at boot — dynamic shards instead.
 */
function _loadMergedFingerprintSources() {
  const merged = {};
  const seen = new Set();
  const pressure = _isFpLoadPressure();
  _broadSkippedDueToPressure = false;

  _mergePathList(CURATED_FINGERPRINT_PATHS, merged, seen, {
    maxBytes: pressure ? (256 * 1024) : (2 * 1024 * 1024),
  });

  if (_shardsAvailable()) {
    // Dynamic shards cover broad catalog — mark for idle batch warm-up
    _broadSkippedDueToPressure = true;
  } else if (!pressure) {
    _mergePathList(BROAD_FINGERPRINT_PATHS, merged, seen, {
      maxBytes: 2 * 1024 * 1024,
    });
  } else {
    const beforeKeys = Object.keys(merged).length;
    _mergePathList(BROAD_FINGERPRINT_PATHS, merged, seen, {
      maxBytes: BROAD_MAX_BYTES_UNDER_PRESSURE,
    });
    if (Object.keys(merged).length === beforeKeys) {
      try {
        const broadPath = BROAD_FINGERPRINT_PATHS[0];
        if (broadPath && fs.existsSync(broadPath)
            && fs.statSync(broadPath).size > BROAD_MAX_BYTES_UNDER_PRESSURE) {
          _broadSkippedDueToPressure = true;
        }
      } catch { _broadSkippedDueToPressure = true; }
    }
  }
 ...
 * WHY(P2674): if broad was skipped under pressure, retry merge once heap recovers.
 */
function _ensureLoaded() {
  if (DEVICE_FINGERPRINTS !== null) {
    if (_broadSkippedDueToPressure && !_loading && !_isFpLoadPressure()) {
      _tryUpgradeBroadCatalogBatched();
    }
...
function _tryUpgradeBroadCatalog() {
  _tryUpgradeBroadCatalogBatched();
}

/** Fleet scans only — load remaining shards in petits bouts while heap allows */
function _warmAllShardsWhenHealthy() {
  if (_isFpLoadPressure() || !_shardsAvailable()) return;
  let guard = 0;
  while (_broadSkippedDueToPressure && !_isFpLoadPressure() && guard < 64) {
    const before = _loadedShards.size;
    _tryUpgradeBroadCatalogBatched();
    if (_loadedShards.size === before) break;
    guard++;
  }
}
...
function getFingerprint(manufacturerName, modelId) {
  ...
  if (compound) {
    // WHY(P2674 / diag 149bc1a5): exact compound hit must NOT force-load the
    // broad fingerprints JSON during device-storm boot (JsonParse OOM @ ~63MB).
    // Enrich from mfr catalog only when heap is healthy and DB already warm.
    if (DEVICE_FINGERPRINTS !== null && !_isFpLoadPressure()) {
  ...
  // WHY(P2678): on miss / before heuristic, pull only the mfr's shard (~tens of KB)
  if (manufacturerName) {
    _ensureShardForManufacturer(manufacturerName);
  }
  ...
  if (_lowercaseIndex) {
    ...
    if (originalKey) {return db[originalKey] || DEVICE_FINGERPRINTS[originalKey];}
