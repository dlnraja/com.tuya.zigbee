  ...
  for (const fpath of FINGERPRINT_PATHS) {
    try {
      if (fs.existsSync(fpath)) {
        let rawData = fs.readFileSync(fpath, 'utf8');
        ...
        console.log(`[FingerprintDB] ✅ Lazy-loaded from: ${fpath} (${count} devices)`);
        _loaded = true;
        _loading = false;
        return DEVICE_FINGERPRINTS;
      }
    } catch (err) {
      console.error(`[FingerprintDB] ⚠️ Failed at ${fpath}: ${err.message}`);
    }
  }
