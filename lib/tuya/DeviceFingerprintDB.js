...
/**
 * WHY(P2674): Under heap/RSS pressure skip the broad 0.9MB catalog entirely —
 * compound sacred-couple DB + curated 16KB cover boot. Broad merges later when
 * BootBudget recovers (next getFingerprint after pressure clears resets via
 * _softUnloadBroadIfNeeded only on first empty load path).
 */
function _loadMergedFingerprintSources() {
  ...
  if (!pressure) {
    ...
    // Soft: allow tiny broad only if somehow truncated; never 0.9MB mid-storm
    const beforeKeys = Object.keys(merged).length;
    _mergePathList(BROAD_FINGERPRINT_PATHS, merged, seen, {
      maxBytes: BROAD_MAX_BYTES_UNDER_PRESSURE,
    });
    // Broad file exists but was size-gated → mark for later upgrade
