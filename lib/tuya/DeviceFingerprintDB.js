  ...
  for (const fpath of FINGERPRINT_PATHS) {
    try {
      if (fs.existsSync(fpath)) {
        ...
        // Skip empty databases — try next path
        if (count === 0) {
      ...
      console.error(`[FingerprintDB] Failed to load fingerprints at ${fpath}: ${err.message}`);
    }
  }
...
function getDriverId(manufacturerName, modelId) {
  ...
  if (fp) {
    ...
    if (modelId && fp.modelIds && !fp.modelIds.some(m => m.toLowerCase() === modelLower)) {
      // Model ID warning — only in debug to avoid hot-path overhead
    }
    return fp.driverId;
  }
  return null;
