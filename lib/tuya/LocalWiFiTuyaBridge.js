...
class LocalWiFiTuyaBridge extends EventEmitter {
  ...
  async startDiscovery() {
    ...
    this._discovery.on('device-found', (info) => {
      this._refreshSessionIp(info);
      this.emit('device-found', info);
    });
    this._discovery.on('device-updated', (info) => {
      this._refreshSessionIp(info);
  ...
  /**
   * Keep the live session IP in sync with UDP discovery.
   * registerDevice snapshots IP once; DHCP / AP roam otherwise leaves a stale socket.
   */
  _refreshSessionIp(info) {
    const id = info && (info.deviceId || info.id || info.gwId);
    ...
    if (client && client.ip !== ip) {
      client.ip = ip;
      this.homey.log(`[WIFI-BRIDGE] Session IP refreshed for ${id}: ${ip}`);
    }
  }
