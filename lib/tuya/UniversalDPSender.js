...
class UniversalDPSender {
  ...
  _buildBuffer(v, t = 'value', o = {}) {
    ...
    if (v?.hex) try { return Buffer.from(String(v.hex).replace(/^0x/, ''), 'hex'); } catch {}
    if (Array.isArray(v)) return Buffer.from(v);
    if (typeof v === 'string' && t !== 'string') {
      const m = v.match(/^(0x)?([0-9a-fA-F]+)$/); if (m && m[2].length % 2 === 0) return Buffer.from(m[2], 'hex');
    }
  ...
  async sendFrame(cid, fr, o = {}) {
    let buf = Buffer.isBuffer(fr) ? fr : Array.isArray(fr) ? Buffer.from(fr) : null;
    if (!buf && typeof fr === 'string' && /^(0x)?[0-9a-fA-F]+$/.test(fr)) {
        buf = Buffer.from(fr.replace(/^0x/, ''), 'hex');
    }
    if (!buf) buf = Buffer.from([fr]);
    const c = this._getC(cid, o.endpoint || 1);
