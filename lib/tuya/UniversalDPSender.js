'use strict';
/**
 * UniversalDPSender v5.7.28 - Maximum Fallbacks (15+ methods)
 */
...
class UniversalDPSender {
  ...
  _buildBuffer(v, t = 'value', o = {}) {
    ...
    if (t === 'bitmap') {
      const n = Number(v) >>> 0, len = sz || (n <= 0xFF ? 1 : n <= 0xFFFF ? 2 : 4);
    ...
    const n = Number(v) || 0, len = sz || (n >= -128 && n <= 255 ? 1 : n >= -32768 && n <= 65535 ? 2 : 4);
    const b = Buffer.alloc(len);
    if (len === 1) b[0] = n & 0xFF;
    else if (len === 2) le ? b.writeInt16LE(n) : b.writeInt16BE(n);
    else le ? b.writeInt32LE(n) : b.writeInt32BE(n);
    return b;
  }
