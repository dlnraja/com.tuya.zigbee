...
class UniversalDPSender {
  ...
  // WHY(P2380 / Cover stop working 9.0.775): TuyaEF00Manager.sendDP/sendTuyaDP
  // soft-fail with `return false` (cluster missing) instead of throw. Treating
  // that as success made curtain_motor log "✅DP mgr" while stderr said
  // "Tuya cluster not available" and the motor never moved.
  async _try(fn) {
    try {
  ...
  _buildBuffer(v, t = 'value', o = {}) {
    ...
    if (v?.toBuffer) {try { return v.toBuffer(); } catch { /* toBuffer() threw */ }}
    if (v?.buffer) {try { return Buffer.from(v.buffer); } catch { /* invalid buffer property */ }}
    if (v?.data) {try { return Buffer.from(v.data); } catch { /* invalid data property */ }}
    if (v?.bytes) {try { return Buffer.from(v.bytes); } catch { /* invalid bytes property */ }}
    if (v?.hex) {try { return Buffer.from(String(v.hex).replace(/^0x/i,''),'hex'); } catch { /* invalid hex string */ }}
  ...
  async sendTuyaDP(dp, v, t = 'value', o = {}) {
    ...
    const dt = DP_TYPES[t] ?? 2, fr = this._buildFrame(dp, t, buf);
    ...
    // 3b. DeviceIOFacade / ProtocolRxTxChain cascade
    const io = this.device.io;
    if (io?.sendDP && await this._try(() => io.sendDP(dp, v, { ...o, dpType: t }))) { this._log(`✅DP${dp} io`); return true; }
    if (c) {
      // 4. datapoint OLD
    ...
    // 16. Raw stream fallback via IO facade
    if (io?.sendRaw && await this._try(() => io.sendRaw(0xEF00, fr, { endpoint: ep }))) {
      this._log(`✅DP${dp} io.raw`); return true;
    }
