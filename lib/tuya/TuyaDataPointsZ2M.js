...
function hsvToRgb(h, s, v) {
  ...
  return {
    r:Math.round(safeMultiply(r),
    g:Math.round(safeMultiply(g),
    b:Math.round(safeMultiply(b)
  };
}
...
function rgbToHsv(r, g, b) {
  ...
  return {
    h:Math.round(safeMultiply(h),
    s:Math.round(safeMultiply(s),
    v:Math.round(safeMultiply(v)
  };
}
...
function hsvToTuya(h, s, v) {
  ...
  const sHex =Math.round(safeMultiply(s).toString(16).padStart(4, '0');
  const vHex =Math.round(safeMultiply(v).toString(16).padStart(4, '0');
