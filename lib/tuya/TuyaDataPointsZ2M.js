...
const valueConverter = {
  ...
  // Local temperature calibration v1: divideBy10 + unsigned 32-bit wrap for negatives
  // Used by TRV602Z/TRV801Z (_TZE204_qyr2m29i, _TZE204_ltwbm23f)
  // Source: Z2M localTempCalibration1
  localTempCalibration1: {
    from: (value) => value > 0x7FFFFFFF ? (value - 0x100000000) / 10 : value / 10,
    to: (value) => value < 0 ? Math.round((value * 10) + 0x100000000) : Math.round(value * 10)
  },

  // Local temperature calibration v2: divideBy10 + unsigned 16-bit wrap for negatives
  // Source: Z2M localTempCalibration2
  localTempCalibration2: {
    from: (value) => value > 0x7FFF ? (value - 0x10000) / 10 : value / 10,
    to: (value) => value < 0 ? Math.round((value * 10) + 0x10000) : Math.round(value * 10)
  },

  // Local temperature calibration v3: divideBy10 + signed 32-bit (0x7FFFFFFF boundary)
  // Source: Z2M localTempCalibration3
  localTempCalibration3: {
    from: (value) => value > 0x7FFFFFFF ? value - 0x100000000 : value,
    to: (value) => value < 0 ? value + 0x100000000 : value
  },

  // Local temperature calibration v4: divideBy100 + signed 32-bit for 100x scaled int32
  // Used for high-precision calibration values
  // Source: Z2M localTempCalibration4
  localTempCalibration4: {
    from: (value) => value > 0x7FFFFFFF ? (value - 0x100000000) / 100 : value / 100,
    to: (value) => value < 0 ? Math.round((value * 100) + 0x100000000) : Math.round(value * 100)
  },

  // Local temperature calibration v5: raw int32 passthrough (no scaling)
  // Source: Z2M localTempCalibration5
