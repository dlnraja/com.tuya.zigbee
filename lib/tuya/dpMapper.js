 ...
 * dpMapper.js - v5.3.17
 *
 * Tuya Unified DP to Homey Capability mapper
 *
 * Usage:
...
function mapTuyaDPToCapabilities(device, dp, value) {
  ...
  case 1:
    if (device.hasCapability('onoff')) {
      device.safeSetCapabilityValue('onoff', !!value).catch(e => device.error('[DP-MAPPER] onoff:', e.message));
    }
    if (device.hasCapability('alarm_water')) {
      device.safeSetCapabilityValue('alarm_water', !!value).catch(e => device.error('[DP-MAPPER] alarm_water:', e.message));
    }
    if (device.hasCapability('alarm_contact')) {
      device.safeSetCapabilityValue('alarm_contact', !!value).catch(e => device.error('[DP-MAPPER] alarm_contact:', e.message));
    }
    if (device.hasCapability('alarm_motion')) {
      device.safeSetCapabilityValue('alarm_motion', !!value).catch(e => device.error('[DP-MAPPER] alarm_motion:', e.message));
    }
    break;
  ...
  case 2:
    if (device.hasCapability('windowcoverings_set')) {
      ...
      device.safeSetCapabilityValue('windowcoverings_set', pos).catch(e => device.error('[DP-MAPPER] windowcoverings_set:', e.message));
    }
    if (device.hasCapability('dim')) {
      const dim = Math.max(0, Math.min(1, value / 1000));
      device.safeSetCapabilityValue('dim', parseFloat(dim)).catch(e => device.error('[DP-MAPPER] dim:', e.message));
    }
    break;
  ...
  case 15:
    if (device.hasCapability('measure_battery')) {
      ...
      device.safeSetCapabilityValue('measure_battery', parseFloat(battery)).catch(() => {});
  ...
  case 5:
    if (device.hasCapability('alarm_curtain_motor')) {
      device.safeSetCapabilityValue('alarm_curtain_motor', !!value).catch(e => device.error('[DP-MAPPER] alarm_curtain_motor:', e.message));
    }
    break;
  ...
  case 7:
    if (device.hasCapability('onoff.2')) {
      device.safeSetCapabilityValue('onoff.2', !!value).catch(e => device.error('[DP-MAPPER] onoff.2:', e.message));
    }
    break;
  case 8:
    if (device.hasCapability('onoff.3')) {
      device.safeSetCapabilityValue('onoff.3', !!value).catch(e => device.error('[DP-MAPPER] onoff.3:', e.message));
    }
    break;
  case 9:
    if (device.hasCapability('onoff.4')) {
      device.safeSetCapabilityValue('onoff.4', !!value).catch(e => device.error('[DP-MAPPER] onoff.4:', e.message));
    }
    break;
  ...
  case 18:
    if (device.hasCapability('measure_temperature')) {
      const temp = value / 10;
      if (temp >= -40 && temp <= 80) {
        device.safeSetCapabilityValue('measure_temperature', parseFloat(temp)).catch(() => {});
  ...
  case 19:
    if (device.hasCapability('measure_humidity')) {
      ...
      if (humidity >= 0 && humidity <= 100) {
        device.safeSetCapabilityValue('measure_humidity', parseFloat(humidity)).catch(e => device.error('[DP-MAPPER] measure_humidity:', e.message));
  ...
  case 103:
    if (device.hasCapability('measure_humidity')) {
      ...
      if (moisture >= 0 && moisture <= 100) {
        device.safeSetCapabilityValue('measure_humidity', parseFloat(moisture)).catch(e => device.error('[DP-MAPPER] measure_humidity:', e.message));
  ...
  case 104:
    if (device.hasCapability('measure_luminance')) {
      device.safeSetCapabilityValue('measure_luminance', parseFloat(value)).catch(e => device.error('[DP-MAPPER] measure_luminance:', e.message));
  ...
  case 35:
    if (device.hasCapability('measure_voltage')) {
      ...
      if (voltage > 0 && voltage < 50) {
        device.safeSetCapabilityValue('measure_voltage', parseFloat(voltage)).catch(e => device.error('[DP-MAPPER] measure_voltage:', e.message));
