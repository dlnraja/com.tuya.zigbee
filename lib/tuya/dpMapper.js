  ...
  switch (dp) {
    ...
    case 2:
      ...
      if (device.hasCapability('dim')) {
        ...
        device.setCapabilityValue('dim', dim).catch(e => device.error('[DP-MAPPER] dim:', e.message));
      }
      break;
    ...
    case 15:
      if (device.hasCapability('measure_battery')) {
        ...
        device.setCapabilityValue('measure_battery', battery).catch(e => device.error('[DP-MAPPER] measure_battery:', e.message));
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // DP 5: Alarm motor stall (curtains)
    // ══════════════════════════════════════════════════════════════════════════
    case 5:
      if (device.hasCapability('alarm_motor_stall')) {
        device.setCapabilityValue('alarm_motor_stall', !!value).catch(e => device.error('[DP-MAPPER] alarm_motor_stall:', e.message));
      }
      break;
    ...
    case 18:
      if (device.hasCapability('measure_temperature')) {
        const temp = value / 10;
        if (temp >= -40 && temp <= 80) {
          device.setCapabilityValue('measure_temperature', temp).catch(e => device.error('[DP-MAPPER] measure_temperature:', e.message));
    ...
    case 19:
      if (device.hasCapability('measure_humidity')) {
        ...
        if (humidity >= 0 && humidity <= 100) {
          device.setCapabilityValue('measure_humidity', humidity).catch(e => device.error('[DP-MAPPER] measure_humidity:', e.message));
    ...
    case 103:
      if (device.hasCapability('measure_humidity')) {
        ...
        if (moisture >= 0 && moisture <= 100) {
          device.setCapabilityValue('measure_humidity', moisture).catch(e => device.error('[DP-MAPPER] measure_humidity:', e.message));
    ...
    case 104:
      if (device.hasCapability('measure_luminance')) {
        device.setCapabilityValue('measure_luminance', value).catch(e => device.error('[DP-MAPPER] measure_luminance:', e.message));
    ...
    case 35:
      if (device.hasCapability('measure_voltage')) {
        ...
        if (voltage > 0 && voltage < 50) {
          device.setCapabilityValue('measure_voltage', voltage).catch(e => device.error('[DP-MAPPER] measure_voltage:', e.message));
