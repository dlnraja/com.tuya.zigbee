...
class TuyaEF00Manager extends EventEmitter {
  ...
  async handleDatapoint(data) {
    ...
    try {
      const card = this.device?.homey?.flow?.getTriggerCard?.('tuya_dp_received');
      if (card) {
    ...
    if (deviceDpMappings[dp]) {
      ...
      if (mapping.capability) {
        ...
        if (typeof this.device._safeSetCapability === 'function') {
          this._log(`[EF00] Using _safeSetCapability for ${mapping.capability}`);
          this.device._safeSetCapability(mapping.capability, parsedValue)
            .then(() => {
          ...
          this.device.setCapabilityValue(mapping.capability, parsedValue)
            .then(() => {
              this._log(`[TUYA] ✅ ${mapping.capability} = ${parsedValue} (DP ${dp}, mapped via device)`);
            })
            .catch(err => {
