...
class TuyaEF00Manager extends EventEmitter {
  ...
  /**
   * v5.5.206: Enhanced time sync for LCD climate sensors (_TZE284_vvmbj46n and similar)
  ...
  async sendTimeSync(zclNode, options = {}) {
    ...
    try {
      ...
      // v5.5.206: Support both standard (8 bytes) and extended (12 bytes) payloads
      const useExtended = options.useExtended || this._needsExtendedPayload();
      let payload;
      ...
      if (useExtended) {
        ...
        this.device.log(`[TUYA] 🕒 Extended time sync: UTC=${utcTimestamp}, offset=${localOffset}s`);
      } else {
        ...
        this.device.log(`[TUYA] 🕒 Standard time sync: ${now.toISOString()}`);
      }
      ...
      const sendFrame = (attempt = 1) => {
        try {
          ...
          this.device.log(`[TUYA] 📤 Sending time sync attempt ${attempt} on EP${targetEndpoint}:`, frame.toString('hex'));
          ...
          if (typeof tuyaCluster.setData === 'function') {
            ...
            this.device.log(`[TUYA] ✅ Time sync attempt ${attempt} sent via setData`);
            return true;
          }
          ...
          if (typeof tuyaCluster.command === 'function') {
            ...
            this.device.log(`[TUYA] ✅ Time sync attempt ${attempt} sent via command(setData)`);
            return true;
          }
      ...
      setImmediate(() => {
        ...
        if (options.doubleSync !== false) {
          setTimeout(() => {
            this.device.log('[TUYA] 🔄 Sending second time sync for LCD climate sensor');
            sendFrame(2);

            // v5.5.206: Send time_valid DP after second sync
            setTimeout(() => {
      ...
      this.device.log(`[TUYA] Time sync error: ${err.message}`);
      return false;
    }
  }
