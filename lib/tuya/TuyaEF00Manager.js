...
class TuyaEF00Manager extends EventEmitter {
  ...
  async initialize(zclNode) {
    ...
    if (!tuyaCluster) {
      ...
      if (modelUpper === 'TS0601' || modelUpper.startsWith('TS02') || modelUpper.startsWith('TS004') || modelUpper.startsWith('TS130')) {
        this._log(`[TUYA] ⚠️ ${modelUpper} device but cluster CLUSTERS.TUYA_EF00 not accessible`);
    ...
    try {
      ...
      const sendFrame = async (attempt = 1) => {
        try {
          ...
          this._log(`${logPrefix} ⚠️No setData/command available, skipping time sync`);
  ...
  async handleDatapoint(data) {
    ...
    // v7.0.22: 🛡️ MULTI-GANG SYNCHRONIZATION SHIELD
    ...
    if (typeof this.device.handleGangReport === 'function') {
      ...
      if (handledByShield) {
        this._log(`[EF00] 🛡️ DP${dp} handled by Multi-Gang Shield`);
