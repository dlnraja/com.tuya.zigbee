...
class TuyaEF00Manager extends EventEmitter {
  ...
  _applyDPValue(dp, value) {
    ...
    if (mapping) {
      ...
      if (this.device) logUnknownDP(this.device, dp, value);
    }
  }
  ...
  async handleDatapoint(data) {
    ...
    if (AdaptiveDataParser && value !== undefined) {
      ...
      // v5.11.15: Skip auto-conversion if device has explicit dpMappings divisor for this DP
      // Prevents double-division (e.g. raw 206 /100 here then /10 in dpMappings = 0.2 instead of 20.6)
      const localDpMap = this.device?.dpMappings?.[dp];
      if (localDpMap && localDpMap.divisor && localDpMap.divisor !== 1) {
        value = parsed.value;
        this._log(`[TUYA-DP] 📦 DP${dp} raw=${parsed.value} (skipping auto-convert, dpMappings divisor=${localDpMap.divisor})`);
