...
const { safeSetTimeout, safeClearTimeout, safeClearInterval } = require('../utils/safe-timers');
...
// Historical cross-ref (stable + Phoenix rules): SmartDivisor prevents
// AdaptiveDataParser /100 then dpMappings /10 double-division.
let smartParse;
try {
  ({ smartParse } = require('../managers/SmartDivisorManager'));
} catch (_e) {
  smartParse = null;
}
...
const TIME_FORMATS = {
  ...
  TUYA_SEQ_10: 'tuya_seq_10',             // 10 bytes: [Seq:2BE][UTC:4BE][Local:4BE] epoch 1970 (MCU v3.3+)
  TUYA_SEQ_10_E2K: 'tuya_seq_10_e2k',     // 10 bytes: [Seq:2BE][UTC:4BE][Local:4BE] epoch 2000
...
class TuyaEF00Manager extends EventEmitter {
  ...
  async sendDP(dpId, value, dpType = 'value', options = {}) {
    ...
    // WHY(P2475 / Z2M forceTimeUpdates): EF00 TX without initialize → hollow cluster / no mcuSyncTime.
    try {
      await this._ensureEf00ReadyForTx();
    } catch (_e) { /* soft */ }
  ...
  /**
   * P2475 — arm EF00 listeners / BoundCluster before any TX (DP or mcuSyncTime).
   * @returns {Promise<boolean>}
   */
  async _ensureEf00ReadyForTx() {
    if (this._ef00ReadyArmed) return true;
    const zcl = this.zclNode || this.device?.zclNode;
    if (!zcl) return false;
    if (!this.zclNode) this.zclNode = zcl;
    // WHY(P2484): never re-enter initialize while it is already running (sendTimeSync
    // inside initialize used to nest → MaxListeners → heap OOM on Homey 64MB).
    if (
      ...
      try {
        // Prefer device-wide launchOnce (peer instance may already be live)
        if (typeof this.constructor.launchOnce === 'function') {
          await this.constructor.launchOnce(this.device, zcl);
        } else {
          await this.initialize(zcl);
        }
        this._p2475InitOnce = true;
        this._log?.('[P2475] EF00 initialize before TX');
      } catch (e) {
        this._log?.(`[P2475] EF00 initialize soft: ${e.message}`);
      }
    }
    this._ef00ReadyArmed = true;
    return true;
  }
   ...
   */
  _tryFragmentReassembly(dpId, value, dpType) {
    if (!Buffer.isBuffer(value)) { return false; }

    // WHY(P2302 / #532 diag f84180b7): datatype=2 value DPs are 4-byte BE ints
    // (e.g. DP24 temp [0,0,0,220]=22.0°C). Old heuristic read UInt16BE(2)=220 as
    // "totalLen" and swallowed the payload → FRAG timeout → 0B → dead temp path.
    // Only reassemble RAW (0) or when totalLen clearly exceeds this buffer.
    const dt = Number(dpType);
    if (dt === 1 || dt === 2 || dt === 4 || dt === 5) { return false; }
    if (value.length <= 4 && dt !== 0) { return false; }
    ...
    if (value.length >= 4) {
      ...
      // Real multi-frame only: declared total must exceed this buffer
      if (totalLen > 0 && totalLen < 65536 && fragIndex < 256 && totalLen > value.length) {
  ...
  async _sendDPRaw(dpId, value, dpType) {
    try {
      // WHY(P2467): UnifiedCoverBase used to create manager without initialize() —
      // fall back to device.zclNode so TX still uses structured datapoint path.
      const zclNode = this.zclNode || this.device?.zclNode;
      if (!zclNode) {return false;}
      if (!this.zclNode) {this.zclNode = zclNode;}

      // P2314: heal hollow IEEE before cluster.datapoint (PresentSky dead controls)
      try {
        const { healZigbeeNodeIdentity } = require('../io/healZigbeeNodeIdentity');
        await healZigbeeNodeIdentity(this.device, { force: false });
      } catch (_e) { /* noop */ }

      const endpoint = zclNode.endpoints?.[1] || zclNode.endpoints?.[2];
      if (!endpoint) {return false;}

      if (this.device._universalDPSender && !this._inSendDPRaw) {
        // WHY(P2467): avoid recursion — UniversalDPSender.sendTuyaDP → mgr.sendDP → here
        this._inSendDPRaw = true;
        try {
          return await this.device._universalDPSender.sendTuyaDP(dpId, value, dpType);
        } finally {
          this._inSendDPRaw = false;
        }
      }

      const typeName = this._normalizeDPTypeName(dpType);
      const typeId = this._normalizeDPTypeId(typeName);

      let buffer;
      if (typeName === 'bool') {
        buffer = Buffer.from([value ? 1 : 0]);
      } else if (typeName === 'enum' || typeName === 'bitmap') {
        buffer = Buffer.from([Number(value) & 0xFF]);
      } else if (typeName === 'string') {
        buffer = Buffer.from(String(value || ''), 'latin1');
      } else if (typeName === 'raw' && Buffer.isBuffer(value)) {
        buffer = value;
      } else {
      ...
      // L1 — structured datapoint args (matches TuyaSpecificCluster COMMANDS)
      if (cluster?.datapoint) {
        try {
          ...
          this._log?.(`[DP-RAW] datapoint failed: ${err?.message || err}`);
        }
      }

      // L2 — endpoint.sendFrame (bypass Homey datapoint IEEE path when possible)
      // WHY(P2467): Tuya TY_DATA_REQUEST payload is seq16 + dp + type + len16 + data
      const frame = Buffer.alloc(6 + buffer.length);
      frame.writeUInt16BE(transid & 0xffff, 0);
      frame[2] = Number(dpId) & 0xff;
      frame[3] = typeId;
      ...
      if (typeof endpoint.sendFrame === 'function') {
        try {
          ...
          this._log?.(`[DP-RAW] sendFrame failed: ${err?.message || err}`);
        }
      }

      if (this.device.io?.sendRaw) {
        try {
          const ok = await this.device.io.sendRaw(0xEF00, frame, { endpoint: 1, command: 0x00 });
          if (ok) {return true;}
        } catch (_e) { /* noop */ }
      }
  ...
  async initialize(zclNode) {
    // WHY(P2484 / GH#548 Wuma diag 97413373 @ 9.0.908): re-entrant initialize() stacked
    // dozens of .on('dp'|'response'|…) listeners → MaxListenersExceeded → V8 heap OOM.
    if (this._initialized) return true;
    if (this._initializing) return false;

    // WHY(P2486b): multiple instances OK — launch ONLY if none already launched on device.
    const peer = TuyaEF00Manager.findLaunched(this.device);
    if (peer && peer !== this) {
      this._initialized = true;
      this._p2475InitOnce = true;
      this._skippedAsPeerLaunch = true;
      this._log('[TUYA] P2486 skip launch — peer EF00 manager already running');
      return true;
    }

    this._initializing = true;

    const verbose = process.env.TUYA_EF00_VERBOSE === '1';
    const massif = (msg) => { if (verbose) this._log(msg); };

    try {
    // DIAGNOSTIC MASSIF - gated (default off on Homey — flood = OOM)
    massif('🔍 [DIAGNOSTIC-MASSIF-TUYA] ═══════════════════════════════════════');
    massif('🔍 [DIAGNOSTIC-MASSIF-TUYA] TuyaEF00Manager.initialize() APPELÉ');
    massif(`🔍 [DIAGNOSTIC-MASSIF-TUYA] Device: ${this.device.getName()}`);
    massif(`🔍 [DIAGNOSTIC-MASSIF-TUYA] zclNode: ${zclNode ? 'PRÉSENT' : 'NULL'}`);
    massif(`🔍 [DIAGNOSTIC-MASSIF-TUYA] zclNode endpoints: ${JSON.stringify(Object.keys(zclNode?.endpoints || {}))}`);
    ...
    massif(`🔍 [DIAGNOSTIC-MASSIF-TUYA] Model: ${model}, Manufacturer: ${manufacturer}`);
    this._log(`[TUYA] Initializing TuyaEF00Manager for ${model}/${manufacturer}`);

    // WHY(P2486): early exits must mark done — else every TX/_ensureEf00Ready
    // re-enters initialize and floods Homey logs (ed0de063 / 97413373 @ 9.0.908).
    if (!zclNode) {
      this._initialized = true;
      this._p2475InitOnce = true;
      return false;
    ...
    if (!tuyaCluster) {
      ...
      if (modelUpper === 'TS0601' || modelUpper.startsWith('TS02') || modelUpper.startsWith('TS004') || modelUpper.startsWith('TS130')) {
        ...
        const passiveOk = await this._setupPassiveMode(endpoint, manufacturer);
        if (passiveOk !== false) {
          this._initialized = true;
          this._p2475InitOnce = true;
        }
      ...
      // Mark done so we do not re-enter forever on ZCL-only sensors (temphumidsensor)
      this._initialized = true;
      this._p2475InitOnce = true;
      return false;
    }
    ...
    this._dpRequestTimer = safeSetTimeout(this.device, async () => {
      if (!this.device || this.device._destroyed) {return;}
      ...
      if (this.deviceInfo && this.deviceInfo.dps) {
        ...
        for (const dpId of dpIds) {
          ...
          await new Promise(resolve => safeSetTimeout(this.device, resolve, 200)); // Space out requests
        }
      } else {
        ...
        for (const dp of [1, 2, 4, 15, 3, 5, 9, 101, 102, 14]) {
          ...
          await new Promise(resolve => safeSetTimeout(this.device, resolve, 200));
        }
      }
      ...
      this._dpRetryTimer = safeSetTimeout(this.device, () => {
        if (!this.device || this.device._destroyed) {return;}
    ...
    this._p2475InitOnce = true;
    try {
      if (this.device) this.device.__ef00LaunchedManager = this;
    } catch (_) { /* noop */ }
    return true;
    } finally {
  ...
  async _setupPassiveMode(endpoint, manufacturer) {
    ...
    const PASSIVE_DP_MAPPINGS = {
      ...
      // Generic fallback for unknown TS0601 — P122: do NOT invent alarm_motion
      // (misclassifies soil/climate/contact as PIR). Only soft sensor scalars.
      '_generic': {
        2: { capability: 'measure_humidity', parser: (v) => v },
        3: { capability: 'measure_temperature', parser: (v) => v / 10 },
        4: { capability: 'measure_battery', parser: (v) => v },
  ...
  _handlePassiveFrame(frame) {
    try {
      ...
      // v10.6.0 FIX: manufacturer-specific bit (0x04) — header is then 5 bytes
      // (fc + mfrCode LE + tsn + cmdId), cmdId at data[4], so the Tuya
      // status/trans pair starts at 5 and DPs at 7. The old code only knew
      // the 3-byte header form and parsed DPs from inside the header.
      if (data.length >= 6) {
        const mfrBit = (data[0] & 0x04) !== 0;
        const cmdId = mfrBit ? data[4] : data[2];
        const fcBits = data[0] & 0x03;
        const knownCmd = cmdId === 0x01 || cmdId === 0x02 || cmdId === 0x24;
        // Si fcBits correspond à une commande spécifique au cluster et cmdId est connu
        if (fcBits === 0x01 && knownCmd) {
          offset = mfrBit ? 7 : 5; // header ZCL (3 ou 5 octets) + Tuya status/trans (2 octets)
        } else {
  ...
  _applyDPValue(dp, value) {
    ...
    // WHY(P2715 / GH#550 HiepSVG): ceiling/MTG radar DP4=range, DP101=find_switch,
    // DP10≠SOC — never invent measure_battery=7% from detection_range.
    const batteryDPs = [4, 14, 15, 33, 35, 10, 101];
    if (batteryDPs.includes(dp) && !this._isPresenceRadarBatteryForbiddenDp(dp)) {
    ...
    if (dp === 247 && this.device.hasCapability?.('measure_voltage')) {
      try {
        ...
        if (voltage > 0 && voltage < 300) { // Sanity check
          ...
          commitCapabilityCatch(this.device, 'measure_voltage', parseFloat(voltage), 'tuya-dp');
        }
      } catch (e) {
    ...
    if (mapping) {
      ...
      // P212: confirmInbound funnel (fusion + SmartCap + L14)
      commitCapabilityCatch(this.device, capability, parsedValue, 'tuya-dp').then((r) => {
        if (r?.ok !== false) {return;}
        const errMsg = r?.error || '';
        if (!errMsg.includes('Invalid type') && !errMsg.includes('Invalid Capability Value')) {
          if (errMsg) {this._error(`[TUYA-PASSIVE] Failed to set ${capability}:`, errMsg);}
          return;
        }
        ...
        commitCapabilityCatch(this.device, capability, coercedValue, 'tuya-dp');
      });
    } else {
  ...
  _buildTimePayload(format, options = {}) {
    ...
    // WHY(P2360): MCU UART v3.3+ sequence-echo — 10 bytes, not TUYA_MCU 9-byte
    case TIME_FORMATS.TUYA_SEQ_10: {
      const seq = Number(options.sequenceNumber) || 0;
      const buf = Buffer.alloc(10);
      buf.writeUInt16BE(seq & 0xFFFF, 0);
      buf.writeUInt32BE(unix1970Utc, 2);
      buf.writeUInt32BE(unix1970Local, 6);
      return buf;
    }

    case TIME_FORMATS.TUYA_SEQ_10_E2K: {
      const seqE2k = Number(options.sequenceNumber) || 0;
      const bufE2k = Buffer.alloc(10);
      bufE2k.writeUInt16BE(seqE2k & 0xFFFF, 0);
      bufE2k.writeUInt32BE(Math.max(0, tuya2000Utc), 2);
      bufE2k.writeUInt32BE(Math.max(0, tuya2000Local), 6);
      return bufE2k;
    }
  ...
  _mapFormatName(formatName) {
    ...
    const mapping = {
      ...
      // WHY(P2360): MCU v3.3+ needs 10-byte seq echo — never collapse to 9-byte TUYA_MCU
      'tuya_seq_10': TIME_FORMATS.TUYA_SEQ_10,
      'tuya_seq_10_e2k': TIME_FORMATS.TUYA_SEQ_10_E2K,
  ...
  async sendTimeSync(zclNode, options = {}) {
    ...
    // WHY(P2475): never attempt mcuSyncTime / DP24 before EF00 initialize
    try {
    ...
    try {
      ...
      const sendWithFallback = async () => {
        for (let attempt = 0; attempt < Math.min(maxAttempts, fallbackChain.length); attempt++) {
          ...
          try {
            ...
            // WHY(P2467/P2475 / #533 Moes ZTS): Z2M forceTimeUpdates uses mcuSyncTime
            // with epoch matching the guessed format (1970 vs 2000 dual).
            if (!sent && typeof tuyaCluster.command === 'function') {
              try {
                const useEpoch2000 = /2000|tuya_timestamp|zigbee_2000/i.test(String(format || ''));
                const unixUtc = Math.floor(Date.now() / 1000);
                const unixLocal = unixUtc - (new Date().getTimezoneOffset() * 60);
                const EPOCH_2000 = 946684800;
                const utc = useEpoch2000 ? Math.max(0, unixUtc - EPOCH_2000) : unixUtc;
                const local = useEpoch2000 ? Math.max(0, unixLocal - EPOCH_2000) : unixLocal;
                await tuyaCluster.command('mcuSyncTime', { seq: 8, utc, local });
                this._log(`${logPrefix} Time sync attempt ${attempt + 1} sent via mcuSyncTime (${useEpoch2000 ? 'epoch2000' : 'epoch1970'})`);
                sent = true;
              } catch (cmdErr) {
                this._log(`${logPrefix} mcuSyncTime failed: ${cmdErr.message}`);
              }
            }

            // Method 1: setData (SDK3 standard)
            if (!sent && typeof tuyaCluster.setData === 'function') {
            ...
            // Method 3: commandMcuSyncTime (legacy name)
            if (!sent && typeof tuyaCluster.command === 'function') {
              try {
      ...
      if (result) {
        // WHY(P2472b / Z2M #29627): ZT08 hodyryli needs DP17=false ~500ms after mcuSyncTime
        // or MCU silently ignores the clock. Same path as GlobalTimeSyncEngine.
        this._scheduleDp17CommitAfterTimeSync(zclNode, logPrefix).catch(() => {});
  ...
  /**
   * WHY(P2472b / Z2M #29627): After successful mcuSyncTime, ZT08-class firmwares need
   * DP17=false ~500ms later or the clock stays at 00:xx. Lookup via MCUFormatDatabase.
   */
  async _scheduleDp17CommitAfterTimeSync(zclNode, logPrefix = '[TUYA]') {
    try {
      ...
      if (!bug?.fix || bug.fix.type !== 'DP17_COMMIT') {return false;}

      const delayMs = Number(bug.fix.delay_ms) || 500;
      const dp = Number(bug.fix.dp) || 17;
      const value = bug.fix.value === true;

      // WHY(P2472b CI): delay via device-safe timer only (mirror-ci-grep bans bare setTimeout)
      ...
      if (!this.device || this.device._destroyed) {return false;}

      if (typeof this.sendDP === 'function') {
        await this.sendDP(dp, value, 'bool');
      } else if (typeof this.writeDataPointBool === 'function') {
        await this.writeDataPointBool(dp, value);
      } else {
        return false;
      }
      this._log(`${logPrefix} P2472b DP${dp}=${value} commit after time sync (${bug.bug || 'DP17'})`);
      return true;
    } catch (err) {
      this._log(`${logPrefix} P2472b DP17 commit soft-fail: ${err?.message || err}`);
      return false;
    }
  }
  ...
  setupDatapointListener(tuyaCluster) {
    try {
      // WHY(P2484): idempotent — Homey 64MB dies if initialize re-binds 50+ listeners
      if (this._ef00ListenersBound) {
        this._log('[TUYA] 🎧 Datapoint listeners already bound (skip)');
        return;
      }
      ...
      // Soft-raise EventEmitter cap once (still prefer single bind above)
      try {
        if (typeof tuyaCluster.setMaxListeners === 'function') {
          tuyaCluster.setMaxListeners(Math.max(tuyaCluster.getMaxListeners?.() || 10, 32));
        }
      } catch (_e) { /* noop */ }
  ...
  async requestDP(dp, options = {}) {
    const { retry = false, maxRetries = 2, force = false } = options;

    try {
      this._log(`[TUYA] 🔍 Requesting DP ${dp}...`);

      // v5.5.54: Check if device forces active mode (climate/soil sensors)
      const forceActive = this.device.forceActiveTuyaMode === true || force === true;

      // WHY(P2596 / Michaelp diag 6eabd9c4): battery TRV + passiveMode skipped ALL
      // requestDP → DATA-QUERY 0/10 → caps stay null after pair. Allow force/init.
      const learningMs = 5 * 60 * 1000;
      const initAt = this.device._deviceInitTime || this.device._trvInitAt || 0;
      const inLearning = initAt > 0 && (Date.now() - initAt) < learningMs;
      const forceLearn = inLearning && (
        this.device._isPureTuyaDP
        || this.device.forceActiveTuyaMode === true
        || /radiator|thermostat|trv/i.test(String(this.device.driver?.id || ''))
      );
      ...
      if (this.passiveMode && !forceActive && !forceLearn) {
      ...
      } else if (this.passiveMode && (forceActive || forceLearn)) {
        this._log(`[TUYA] 🔥 Passive override (force/learn) — sending DP${dp} request (P2596)`);
      }
          ...
          if (this.device.hasCapability?.('measure_battery')) {
            ...
            if ([4, 10, 14, 15, 101].includes(dp)) {
              ...
              if (currentBattery === null || currentBattery === undefined) {
                this._log('[TUYA] Battery unknown until wake-up report (DP timeout is normal)');
              }
            }
          } else {
  ...
  /**
   * Request all datapoints (query_all / dataQuery).
   * P102: canonical name used by Unified* bases and DeviceIOFacade.
   * Alias: queryAllDatapoints ≡ requestAllDPs
   */
  async requestAllDPs(options = {}) {
    try {
      const endpoint = this.device.zclNode?.endpoints?.[options.endpoint ?? 1];
      if (!endpoint) {
        this._log('[TUYA] ℹ️  requestAllDPs: endpoint not available');
        return false;
      }
      ...
      if (!tuyaCluster) {
        if (!this._clusterMissingLogged) {
          this._log('[TUYA] ℹ️  requestAllDPs: Tuya cluster not available');
          this._clusterMissingLogged = true;
        }
        return false;
      }
      ...
      if (!sent && typeof tuyaCluster.dataQuery === 'function') {
        try {
          await Promise.race([
            tuyaCluster.dataQuery({ seq }),
            new Promise((_, reject) => this.device.homey.setTimeout(() => reject(new Error('Timeout')), timeoutMs)),
          ]);
          this._log('[TUYA] ✅ requestAllDPs via dataQuery');
          sent = true;
        } catch (_e) { /* next */ }
      }
      ...
      if (!sent && typeof tuyaCluster.command === 'function') {
        try {
          await Promise.race([
            tuyaCluster.command('dataQuery', { seq }, { disableDefaultResponse: true }),
            new Promise((_, reject) => this.device.homey.setTimeout(() => reject(new Error('Timeout')), timeoutMs)),
          ]);
          this._log('[TUYA] ✅ requestAllDPs via command(dataQuery)');
          sent = true;
        } catch (_e) { /* next */ }
      }
      ...
      if (!sent && typeof tuyaCluster.getData === 'function') {
        try {
          await Promise.race([
            tuyaCluster.getData({ seq, datapoints: Buffer.alloc(0) }),
            new Promise((_, reject) => this.device.homey.setTimeout(() => reject(new Error('Timeout')), timeoutMs)),
          ]);
          this._log('[TUYA] ✅ requestAllDPs via getData(empty)');
          sent = true;
        } catch (_e) { /* next */ }
      }

      // Fallback: DP1 request often wakes MCU and triggers a full report burst
      if (!sent) {
        this._log('[TUYA] requestAllDPs falling back to requestDP(1)');
        return !!(await this.requestDP(1, options));
      }

      return true;
    } catch (err) {
      this._log('[TUYA] ℹ️  requestAllDPs:', err?.message || err);
      return false;
    }
  }

  /** @deprecated Prefer requestAllDPs — kept for TuyaZigbeeDevice / CORE_RULES callers */
  async queryAllDatapoints(options = {}) {
    return this.requestAllDPs(options);
  }
  ...
  async sendTuyaDP(dp, dpType, value) {
    try {
      ...
      // WHY(P2380): scan EP1/2/3 — some covers expose 0xEF00 only on EP2
      let endpointUsed = endpoint;
      let tuyaCluster = endpoint.clusters.tuyaManufacturer
        || endpoint.clusters.tuya
        || endpoint.clusters.tuyaSpecific
        || endpoint.clusters.manuSpecificTuya
        || endpoint.clusters[0xEF00]
      ...
      if (!tuyaCluster) {
        for (const epId of [1, 2, 3, 11]) {
          ...
          if (typeof ep.sendFrame === 'function') {
            endpointUsed = ep;
          }
        }
      }

      // Cluster object may be absent while endpoint.sendFrame still works (hollow interview)
      if (!tuyaCluster && typeof endpointUsed?.sendFrame !== 'function') {
        throw new Error('Tuya cluster not available');
      }

      const typeId = typeof dpType === 'number' ? dpType : this._normalizeDPTypeId(dpType);
      let dataBuf;
      if (typeId === 1) {
      ...
      } else if (typeId === 0 && Buffer.isBuffer(value)) {
        dataBuf = value;
      } else {
        dataBuf = Buffer.alloc(4);
        dataBuf.writeInt32BE(Math.round(Number(value) || 0), 0);
      }

      const discovery = this.device?._dpAutoDiscovery || this.device?._discovery;
      if (discovery?.recordFrame) {
        discovery.recordFrame({
          endpointId: endpointUsed?.ID || endpointUsed?.endpointId || 1,
          clusterId: 0xEF00,
          commandId: 0x00,
          payload: dataBuf,
          datapoints: [{ dpId: dp, dpType: typeId, value }]
        }, 'tx', { source: 'tuya_ef00_sendTuyaDP' });
      }

      const transid = Math.floor(Math.random() * 255);

      // Prefer structured datapoint (Homey builds correct ZCL) over raw sendFrame
      if (tuyaCluster?.datapoint) {
        try {
          ...
          this._log(`[TUYA] ✅ DP${dp} sent successfully (datapoint)`);
          return true;
        } catch (dpErr) {
          this._log(`[TUYA] datapoint path failed: ${dpErr.message}`);
        }
      }

      // WHY(P2467): TY_DATA_REQUEST = seq16 + dp + type + len16 + data (not status+transid)
      const frame = Buffer.alloc(6 + dataBuf.length);
      frame.writeUInt16BE(transid & 0xffff, 0);
      frame[2] = Number(dp) & 0xff;
      frame[3] = typeId;
      frame.writeUInt16BE(dataBuf.length, 4);
      dataBuf.copy(frame, 6);

      await endpointUsed.sendFrame(0xEF00, frame, 0x00);
  ...
  async handleDatapoint(data) {
    ...
    // v10.2.0 / P2302: Try DP fragmentation reassembly before normal processing
    // Pass datatype so value/bool/enum buffers are never mistaken for frag headers
    if (Buffer.isBuffer(value) && this._tryFragmentReassembly(dp, value, dpType)) {
    ...
    if (AdaptiveDataParser && value !== undefined) {
      ...
      // WHY(P2420 / VicHY #2227 clrdrnya): radar DP2/3/102 use `cap: null` + `setting` —
      // falsy `cap` used to fall through to generic DP2→humidity SmartDivisor.
      if (localDpMap && (
        localDpMap.divisor !== undefined
        || localDpMap.transform
        || localDpMap.capability
        || localDpMap.cap
        || localDpMap.setting
        || localDpMap.internal
        || localDpMap.skip
        || Object.prototype.hasOwnProperty.call(localDpMap, 'cap')
        || typeof localDpMap.parse === 'function'
      )) {
        value = parsed.value;
        this._log(`[TUYA-DP] 📦 DP${dp} raw=${parsed.value} (skipping auto-convert, dpMappings has explicit handler)`);
      } else {
        // Prefer SmartDivisorManager for scaled sensors (P107 cross-era)
        let smartApplied = false;
        if (smartParse && typeof parsed.value === 'number' && Number.isFinite(parsed.value)) {
          try {
            let capability = null;
            if (context.includes('temp')) {capability = 'measure_temperature';}
            else if (context.includes('humid')) {capability = 'measure_humidity';}
            else if (context.includes('co2')) {capability = 'measure_co2';}
            else if (context.includes('illumin') || context.includes('lux')) {capability = 'measure_luminance';}
            else if (context.includes('formaldehyde')) {capability = 'measure_formaldehyde';}
            else if (context.includes('voltage')) {capability = 'measure_voltage';}
            else if (context.includes('current')) {capability = 'measure_current';}
            else if (context.includes('power') && !context.includes('meter')) {capability = 'measure_power';}
            else if (context.includes('energy') || context.includes('meter')) {capability = 'meter_power';}

            if (capability) {
              const mfr = (typeof getManufacturer === 'function' ? getManufacturer(this.device) : null)
                || this.device.getSetting?.('zb_manufacturer_name')
                || this.device.getData?.()?.manufacturerName
                || '';
              value = smartParse(parsed.value, dp, {
                manufacturerName: mfr,
                capability,
                deviceId: this.device.getData?.()?.id,
                protocol: 'zigbee',
                defaultDivisor: capability === 'measure_temperature' ? 10 : undefined,
              });
              smartApplied = true;
              this._log(`[TUYA-DP] 🧮 SmartDivisor ${capability}: raw=${parsed.value} → ${value}`);
            }
          } catch (e) {
            this._log(`[TUYA-DP] SmartDivisor fallback: ${e?.message || e}`);
          }
        }
        ...
        if (!smartApplied) {
          if (context.includes('temp')) {
            ...
            value = AdaptiveDataParser.toIlluminance(parsed.value, { format: 'raw' });
    ...
    // WHY(P2715 / GH#550): do this AFTER we know DP ownership — but still skip radar
    // range/find_switch DPs that collide with battery DP ids (OCR Battery 7%).
    ...
    // WHY(P2333): also honor internal/skip reservations (FCU valve DP36, dimmer DP2)
    // WHY(P2528 / VicHY 74e5cae7): radar configs use `cap` (not `capability`).
    // Without honoring `cap`, DP104 lux logged "Unmapped" after dpReport already
    // handled it, and value-type DP1 could fall through to measure_temperature.
    const deviceDpMappings = this.device.dpMappings || {};
    const ownedMap = deviceDpMappings[dp] || deviceDpMappings[Number(dp)] || deviceDpMappings[String(dp)];
    if (ownedMap && (
      ownedMap.capability
      || ownedMap.cap
      || ownedMap.internal
      || ownedMap.skip
      || ownedMap.setting
      || Object.prototype.hasOwnProperty.call(ownedMap, 'cap')
    )) {
      // Device owns this DP (dpReport → device._handleDP already ran above)
      return;
    }

    // Common DP mappings (override in device-specific code)
    // NOTE: DP 1 can be BOTH temperature AND motion depending on device type!
    // WHY(P2302 / #532): bool DP1 on wall_thermostat was mapped to measure_temperature=0
    // and also tried onoff=0 as number — kill FCU off feedback. Gate by datatype + caps.
    const dpTypeNum = Number(dpType);
    const driverId = String(this.device?.driver?.id || '');
    const isThermoDriver = /thermostat|trv|climate|heater|radiator/i.test(driverId);
    // WHY(P2350 / PresentSky 60959c24): driver.id can be empty early in boot —
    // also treat as dimmer when device dpMappings already claim DP2 → dim.
    const map2 = this.device?.dpMappings?.[2] || this.device?.dpMappings?.['2'];
    const isDimmerDriver = /wall_dimmer|light_dimmer|_dimmer|dimmer_tuya/i.test(driverId)
      || map2?.capability === 'dim';
    // WHY(P2414 / diag c5165a37): presence/PIR mmWave — DP2 is often sensitivity /
    // distance / clutter, NEVER humidity. Generic map invented measure_humidity=4.
    const radarMfr = String(
      this.device?.getSetting?.('zb_manufacturer_name')
      || this.device?._radarConfigName
      || '',
    );
    const isPresenceRadarDriver = /presence_sensor|radar|mmwave|pir_radar|motion_sensor/i.test(driverId)
      || (this.device?.hasCapability?.('alarm_motion') && /presence|radar/i.test(driverId))
      // WHY(P2713 / VicHY #2254): driver.id can be empty early — still treat as radar
      || this.device?.mainsPowered === true && this.device?.hasCapability?.('measure_luminance.distance')
      || /clrdrnya|gkfbdvyx|sbyx0lm6|mtg075|ya4ft0w4|laokfqwu/i.test(radarMfr);
    // WHY(P2715 / GH#550): MTG lux=DP104 (DP103=cline). Ceiling V3 lux=DP103 (Z2M).
    // Never blanket-skip DP103 on all radars — that killed gkfbdvyx lux before dpMap seed.
    const isMtgLuxFamily = /clrdrnya|sbyx0lm6|dtzziy1e|mtoaryre|mtg075|iaqz94ek/i.test(radarMfr);
    // Strict datatype==bool only — never steal value-type DP1 temperatures
    if (Number(dp) === 1 && dpTypeNum === 1 && this.device.hasCapability?.('onoff')) {
      const onVal = value === true || value === 1 || value === '1' || value === 'ON' || value === 'on';
      commitCapabilityCatch(this.device, 'onoff', !!onVal, 'tuya-dp-bool1')
        .then((r) => {
          if (r?.ok !== false && !r?.skipped) {
            this._log(`[TUYA] ✅ onoff = ${!!onVal} (DP 1 bool)`);
          }
        });
      return;
    }
    if (Number(dp) === 1 && isThermoDriver && dpTypeNum === 1) {
      return; // driver processResponse owns FCU/BHT on/off
    }
    // WHY(P2333 / PresentSky diag 60959c24): DP2 on EF00 dimmers is brightness 0–1000,
    // not humidity — generic map + SmartDivisor invented measure_humidity=21.
    if (isDimmerDriver && Number(dp) === 2) {
      return;
    }
    // WHY(P2414 / Gmail diag c5165a37 @ 9.0.797): presence radar DP2→humidity skip
    if (isPresenceRadarDriver && Number(dp) === 2) {
      return;
    }
    // WHY(P2713 / VicHY #2254 diag af98752d @ 9.0.1145): when MTG dpMap not yet
    // seeded, generic fallback painted measure_battery=100 from DP4 (detection_range)
    // and measure_luminance from DP103 (cline). Lux is DP104 on MTG075/clrdrnya.
    // WHY(P2715 / GH#550 HiepSVG): ceiling gkfbdvyx lux IS DP103 — allow generic lux.
    if (isPresenceRadarDriver && Number(dp) === 4) {
      this._log(`[TUYA] P2713/P2715 skip generic DP4 on presence radar (not battery)`);
      return;
    }
    if (isPresenceRadarDriver && Number(dp) === 103 && isMtgLuxFamily) {
      this._log(`[TUYA] P2713 skip generic DP103 on MTG radar (cline, lux=DP104)`);
      return;
    }

    const dpMappings = {
      // Soil Sensor DPs — DP1 temp ONLY when value-type (not bool on/off)
      1: (dpTypeNum === 2 || dpTypeNum === 0) ? 'measure_temperature' : null,
      // 1: 'alarm_motion',       // Motion (bool) - for PIR sensors - handled by driver override
      // WHY(P2414): never invent humidity on presence/dimmer DP2
      2: (isDimmerDriver || isPresenceRadarDriver) ? null : 'measure_humidity',
      ...
      // WHY(P2713+P2715): MTG DP103=cline; ceiling V3 DP103=lux (allow generic)
      103: (isPresenceRadarDriver && isMtgLuxFamily) ? null : 'measure_luminance',
      106: 'measure_luminance',    // Illuminance (lux) - ZG-204ZM radar

      // Battery — WHY(P2713): MTG DP4 = detection_range meters, never SOC
      4: isPresenceRadarDriver ? null : 'measure_battery',
      14: isPresenceRadarDriver ? null : 'alarm_battery',
      15: isPresenceRadarDriver ? null : 'measure_battery',
    ...
    if (capability) {
      ...
      commitCapabilityCatch(this.device, capability, parsedValue, 'tuya-dp')
        .then((r) => {
          if (r?.ok !== false && !r?.skipped) {
            this._log(`[TUYA] ✅ ${capability} = ${parsedValue} (DP ${dp})`);
          } else if (r?.skipped) {
            this._log(`[TUYA] ⏭️ ${capability} skipped (${r.reason || 'fusion'}) DP ${dp}`);
          } else if (r?.error) {
            this._error(`[TUYA] ❌ Failed to set ${capability}:`, r.error);
          }
        });
    } else {
  ...
  /**
   * WHY(P2715 / GH#550 HiepSVG OCR Battery 7%): presence radars reuse battery DP ids
   * for range / find_switch / illuminance-compat. Never forward those to SmartBattery.
   * Contre quoi: DynCap invents measure_battery + zones after Repair.
   */
  _isPresenceRadarBatteryForbiddenDp(dp) {
    try {
      const dpN = Number(dp);
      if (![4, 10, 14, 15, 101].includes(dpN)) return false;
      const driverId = String(this.device?.driver?.id || '');
      const mfr = String(
        this.device?.getSetting?.('zb_manufacturer_name')
        || this.device?._radarConfigName
      ...
      const isRadar = /presence_sensor|radar|mmwave|pir_radar/i.test(driverId)
        || (this.device?.mainsPowered === true
          && this.device?.hasCapability?.('measure_luminance.distance'))
        || /clrdrnya|gkfbdvyx|sbyx0lm6|mtg075|ya4ft0w4|laokfqwu|dtzziy1e|mtoaryre/i.test(mfr);
      return !!isRadar;
    } catch (_e) {
      return false;
    }
  ...
  _forwardToBatteryManager(dpId, value) {
    try {
      // P110: prefer SmartBatteryManager (devices expose smartBattery, not batteryManagerV4)
      const smart = this.device.smartBattery;
      if (smart && typeof smart.handleDP === 'function') {
        this._log(`[TUYA] 🔋 Forwarding DP${dpId} to SmartBatteryManager`);
        Promise.resolve(smart.handleDP(dpId, value)).catch((e) => {
          this._error('[TUYA] ❌ SmartBattery forward error:', e?.message || e);
        });
        return;
      }
      ...
      } else if (batteryManager && typeof batteryManager.handleDP === 'function') {
        Promise.resolve(batteryManager.handleDP(dpId, value)).catch(() => {});
      } else {
  ...
  _getDPContext(dpId) {
    ...
    const maps = this.device?.dpMappings || {};
    const localMapping = maps[dpId] || maps[String(dpId)] || maps[Number(dpId)];
    if (localMapping) {
      const capName = localMapping.capability || localMapping.cap;
      if (capName) {
        // Return capability name as context (e.g., 'alarm_contact', 'measure_temperature')
        return capName;
      }
      // WHY(P2420): `cap: null` + setting/internal = driver-owned — never generic humidity/battery
      if (
        localMapping.internal
        || localMapping.skip
        || localMapping.setting
        || Object.prototype.hasOwnProperty.call(localMapping, 'cap')
      ) {
        return `owned_dp_${dpId}`;
      }
    }

    const driverId = String(this.device?.driver?.id || '');
    // WHY(P2333/P2350): EF00 dimmer DP2 = brightness — never "humidity" SmartDivisor path
    const map2 = maps[2] || maps['2'];
    if ((/wall_dimmer|light_dimmer|_dimmer/i.test(driverId) || map2?.capability === 'dim' || map2?.cap === 'dim')
      && Number(dpId) === 2) {
      return 'dim';
    }
    // WHY(P2420 / VicHY): presence radars — never generic DP2 humidity (MTG sensitivity)
    if (/presence_sensor_radar|sensor_illuminance_presence|sensor_presence_radar/i.test(driverId)
      && Number(dpId) === 2) {
      return 'owned_dp_2';
    }
    // WHY(P2713 / VicHY #2254 af98752d): DP4=range, DP103=cline on MTG — never battery/lux context
    if (/presence_sensor_radar|sensor_presence_radar|mmwave/i.test(driverId)
      && (Number(dpId) === 4 || Number(dpId) === 103 || Number(dpId) === 14 || Number(dpId) === 15)) {
      return `owned_dp_${dpId}`;
    }
   ...
   */
  cleanup() {
    const _t = ['dailySyncTimer', '_dpRequestTimer', '_dpRetryTimer', '_stubbornTimer', '_doubleSyncTimer', '_timeValidTimer'];
    for (const t of _t) {
      if (this[t]) { safeClearTimeout(this.device, this[t]); this[t] = null; }
    }
    if (this._watchdogTimer) {
      safeClearInterval(this.device || this.homey, this._watchdogTimer);
