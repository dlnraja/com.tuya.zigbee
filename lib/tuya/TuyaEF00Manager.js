      ...
      const sendWithFallback = async () => {
        for (let attempt = 0; attempt < Math.min(maxAttempts, fallbackChain.length); attempt++) {
          ...
          try {
            ...
            // WHY(P2467 / #533 Moes ZTS): Z2M forceTimeUpdates uses mcuSyncTime command
            // FIRST — setData(DP 0x24) is not the same as cluster cmd 0x24.
            if (!sent && typeof tuyaCluster.command === 'function') {
              try {
                ...
                this._log(`${logPrefix} Time sync attempt ${attempt + 1} sent via mcuSyncTime`);
                sent = true;
              } catch (cmdErr) {
