...
class TuyaEF00Manager extends EventEmitter {
  ...
  async initialize(zclNode) {
    ...
    if (!tuyaCluster) {
      // v5.2.9: For TS0601, and now TS0203/TS0202, still setup even without visible cluster
      // Many battery devices hide EF00 cluster during quick pairing but send DPs later!
      if (model.toUpperCase() === 'TS0601' || model.toUpperCase() === 'TS0203' || model.toUpperCase() === 'TS0202') {
        this._log('[TUYA] ⚠️ TS0601/TS020x device but cluster 0xEF00 not accessible');
