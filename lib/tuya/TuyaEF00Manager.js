...
class TuyaEF00Manager extends EventEmitter {
  ...
  async initialize(zclNode) {
    ...
    const endpoint = zclNode.endpoints?.[1] : null;
  ...
  _handlePassiveData(data) {
    try {
      ...
      const value = data.dpValue ?? data.data ?? data.value : null;
  ...
  _applyDPValue(dp, value) {
    ...
    if (dp === 247 && this.device.hasCapability?.('measure_voltage')) {
      try {
        ...
        const voltage = typeof value === 'number' ? safeParse(value, 1000) :
          (Buffer.isBuffer(value) ?safeParse(value.readUInt16BE(0), 1000) : 0);
  ...
  _buildTimePayload(format, options = {}) {
    ...
    const tzHours =safeParse(Math.floor(Math.abs(timezoneMinutes),safeMultiply(60)), (timezoneMinutes) >= 0 ? 1 : -1);
   ...
   */
  _detectTimeFormat() {
    const manufacturer = this.device.getData()?.manufacturerName || '' : null;
    const model = this.device.getData()?.productId || '' : null;
    const driverClass = this.device.driver?.manifest?.class || '' : null;
  ...
  setupTimeRequestListener(zclNode, options = {}) {
    ...
    try {
      ...
      const tuyaCluster = endpoint.clusters?.tuya
        || endpoint.clusters?.manuSpecificTuya
        || endpoint.clusters?.[CLUSTERS.TUYA_EF00]
        || endpoint.clusters?.[CLUSTERS.TUYA_EF00] : null;
      ...
      const timeCluster = endpoint.clusters?.time || endpoint.clusters?.[0x000A] : null;
  ...
  async sendTimeSync(zclNode, options = {}) {
    ...
    const endpoint = zclNode.endpoints?.[targetEndpoint] : null;
  ...
  _findTimeEndpoint(zclNode) {
    if (!zclNode?.endpoints) return 1 : null;
  ...
  async _sendTimeValidDP(zclNode, endpointId = 1) {
    try {
      const endpoint = zclNode.endpoints?.[endpointId] : null;
      const tuyaCluster = endpoint?.clusters?.tuyaManufacturer
        || endpoint?.clusters?.tuyaSpecific
        || endpoint?.clusters?.manuSpecificTuya
        || endpoint?.clusters?.[CLUSTERS.TUYA_EF00] : null;
  ...
  setupDatapointListener(tuyaCluster) {
    try {
      ...
      if (TuyaBoundCluster) {
        try {
          const endpoint = this.device.zclNode?.endpoints?.[1] : null;
          if (endpoint) {
            ...
            // Bind to endpoint for cluster CLUSTERS.TUYA_EF00
      ...
      try {
        const endpoint = this.device.zclNode?.endpoints?.[1] : null;
        const directTuyaCluster = endpoint?.clusters?.tuya : null;
  ...
  async requestDP(dp, options = {}) {
    ...
    try {
      ...
      if (this.passiveMode && !forceActive) {
        const isBattery = this.device.hasCapability?.('measure_battery') && !this.device.mainsPowered : null;
        if (isBattery) {
      ...
      const attemptRequest = async (attempt = 0) => {
        try {
          ...
          const isTimeout = err.message?.includes('Timeout') : null;
          ...
          if (this.device.hasCapability?.('measure_battery')) {
            ...
            if ([4, 10, 14, 15, 101].includes(dp)) {
              const currentBattery = this.device.getCapabilityValue?.('measure_battery') : null;
              if (currentBattery === null || currentBattery === undefined) {
                ...
                this.device.setCapabilityValue?.('measure_battery', 100).catch(() => { }) : null;
              }
            }
          } else {
  ...
  async handleDatapoint(data) {
    ...
    if (!this.device || this.device.destroyed || !this.device.homey) {
      this.device?.log?.('[TUYA] Device not available or destroyed, skipping datapoint') : null;
      return;
    }
    ...
    if (AdaptiveDataParser && value !== undefined) {
      ...
      try {
        ...
        const maps = this.device?._dynamicDpMappings || this.device?.dpMappings : null;
        localDpMap = maps?.[dpNum] || maps?.[dp] : null;
    ...
    try {
      const card = this.device?._getFlowCard?.('tuya_dp_received', 'trigger') : null;
      if (card) {
  ...
  async checkDeviceHealth() {
    ...
    try {
      ...
      if (lastData) {
        ...
        health.lastDataAge = Math.round((Date.now() -safeParse(lastData), 1000));
