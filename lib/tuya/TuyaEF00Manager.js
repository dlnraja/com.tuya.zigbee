...
class TuyaEF00Manager extends EventEmitter {
  ...
  async handleDatapoint(data) {
    ...
    const isPresenceRadarDriver = /presence_sensor|radar|mmwave|pir_radar|motion_sensor/i.test(driverId)
      ...
      || /clrdrnya|gkfbdvyx|sbyx0lm6|mtg075/i.test(String(
        this.device?.getSetting?.('zb_manufacturer_name')
    ...
    // and measure_luminance=null from DP103 (cline). Lux is DP104 on MTG075/clrdrnya.
    if (isPresenceRadarDriver && (Number(dp) === 4 || Number(dp) === 103)) {
      this._log(`[TUYA] P2713 skip generic DP${dp} on presence radar (not battery/lux)`);
      return;
    }
    ...
    const dpMappings = {
      ...
      // WHY(P2713 / VicHY #2254): MTG075 lux = DP104; DP103 is cline — never generic lux
      103: isPresenceRadarDriver ? null : 'measure_luminance',
