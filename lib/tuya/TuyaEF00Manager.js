...
class TuyaEF00Manager extends EventEmitter {
  ...
  handleDatapoint(data) {
    ...
    try {
      this.device.homey.flow.getDeviceTriggerCard('tuya_dp_received')
        .trigger(this.device, {
          dp_id: dp,
    ...
    try {
      ...
      if (dpType === TuyaDPParser.DP_TYPE.BOOL) {
        this.device.homey.flow.getDeviceTriggerCard('receive_status_boolean')
          .trigger(this.device, { dp_id: dp, value: Boolean(value) });
      } else if (dpType === TuyaDPParser.DP_TYPE.VALUE || dpType === TuyaDPParser.DP_TYPE.ENUM) {
        const n = typeof value === 'number' ? value : Number(value);
        this.device.homey.flow.getDeviceTriggerCard('receive_status_number')
          .trigger(this.device, { dp_id: dp, value: Number.isFinite(n) ? n : 0 });
      } else if (dpType === TuyaDPParser.DP_TYPE.STRING) {
        this.device.homey.flow.getDeviceTriggerCard('receive_status_string')
          .trigger(this.device, { dp_id: dp, value: typeof value === 'string' ? value : JSON.stringify(value) });
      } else {
        this.device.homey.flow.getDeviceTriggerCard('receive_status_json')
          .trigger(this.device, { dp_id: dp, value: stringifyRaw(value) });
      }
    } catch (err) { }
