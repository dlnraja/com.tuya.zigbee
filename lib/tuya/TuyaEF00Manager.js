...
class TuyaEF00Manager extends EventEmitter {
  ...
  _tryFragmentReassembly(dpId, value, dpType) {
    ...
    // WHY(P2302 / #532): datatype=2 value DPs are 4-byte BE ints
    // (e.g. DP24 temp [0,0,0,220]=22.0°C). Old heuristic read UInt16BE(2)=220 as
    // "totalLen" and swallowed the payload → FRAG timeout → 0B.
          ...
          if (this.device.hasCapability?.('measure_battery')) {
            ...
            if ([4, 10, 14, 15, 101].includes(dp)) {
              ...
              if (currentBattery === null || currentBattery === undefined) {
                this._log('[TUYA] 📊 Battery unknown after DP timeout — wait for wake-up report');
              }
            }
          } else {
  ...
  async handleDatapoint(data) {
    ...
    if (Number(dp) === 1 && dpTypeNum === 1 && this.device.hasCapability?.('onoff')) {
      ...
      const setFn = typeof this.device.safeSetCapabilityValue === 'function'
        ? this.device.safeSetCapabilityValue.bind(this.device)
        : this.device.setCapabilityValue.bind(this.device);
      Promise.resolve(setFn('onoff', !!onVal))
        .then(() => this._log(`[TUYA] ✅ onoff = ${!!onVal} (DP 1 bool)`))
        .catch((e) => this._log(`[TUYA] onoff DP1 set failed: ${e?.message || e}`));
      return;
    }
