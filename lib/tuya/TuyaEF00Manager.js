...
class TuyaEF00Manager extends EventEmitter {
  ...
  async requestDP(dp) {
    try {
      ...
      // DISABLED: dataQuery doesn't work for sleepy enddevices
      // They must report passively, we just listen
      this.device.log(`[TUYA] ⚠️  Skipping active DP${dp} query (sleepy device - waiting for passive report)`);
      return false;

      /* COMMENTED OUT - causes "dp is an unexpected property" error
      try {
        if (typeof tuyaCluster.dataQuery === 'function') {
          await tuyaCluster.dataQuery({ seq: Math.floor(Math.random() * 0xFFFF) });
