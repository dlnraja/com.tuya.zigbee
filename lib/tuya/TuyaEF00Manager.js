...
class TuyaEF00Manager extends EventEmitter {
  ...
  async initialize(zclNode) {
    ...
    this.device.log('🔍 [DIAGNOSTIC-MASSIF-TUYA] ═══════════════════════════════════════');
    this.device.log('🔍 [DIAGNOSTIC-MASSIF-TUYA] TuyaEF00Manager.initialize() APPELÉ');
    this.device.log(`🔍 [DIAGNOSTIC-MASSIF-TUYA] Device: ${this.device.getName()}`);
    this.device.log(`🔍 [DIAGNOSTIC-MASSIF-TUYA] zclNode: ${zclNode ? 'PRÉSENT' : 'NULL'}`);
    this.device.log(`🔍 [DIAGNOSTIC-MASSIF-TUYA] zclNode endpoints: ${JSON.stringify(Object.keys(zclNode?.endpoints || {}))}`);
    ...
    this.device.log(`🔍 [DIAGNOSTIC-MASSIF-TUYA] Model: ${model}, Manufacturer: ${manufacturer}`);
    this.device.log(`[TUYA] Initializing TuyaEF00Manager for ${model}/${manufacturer}`);
  ...
  /**
   * v5.5.207: Enhanced time sync for LCD climate sensors (_TZE284_vvmbj46n and similar)
   * Implements multi-endpoint discovery, immediate response, double sync, and extended payload
   * NEW v5.5.207: Support for FORCED sync (bypass all conditions for passive LCD devices)
   */
  async sendTimeSync(zclNode, options = {}) {
    if (!zclNode) return false;

    // v5.5.207: Extract new forced sync parameters
    const {
      forceSync = false,           // BYPASS all conditions - FORCE sync for passive devices
      useExtendedPayload = false,  // Use 12-byte payload for LCD climate sensors
    ...
    try {
      ...
      const localOffset = now.getTimezoneOffset() * -60; // Convert to seconds, flip sign

      // v5.5.207: Support both standard (8 bytes) and extended (12 bytes) payloads
      const useExtended = useExtendedPayload || this._needsExtendedPayload();
      let payload;

      if (useExtended) {
        // Extended 12-byte payload for LCD climate sensors
        payload = Buffer.alloc(12);
        payload.writeUInt32BE(utcTimestamp, 0);     // UTC timestamp (4 bytes)
        payload.writeInt32BE(localOffset, 4);       // Local offset (4 bytes)
        payload.writeUInt32BE(0x00000000, 8);       // Reserved/DST/padding (4 bytes)
        this.device.log(`${logPrefix} 🕒 Extended time sync: UTC=${utcTimestamp}, offset=${localOffset}s`);
      } else {
        // Standard 7-byte payload
        payload = Buffer.from([
        ...
        this.device.log(`${logPrefix} 🕒 Standard time sync: ${now.toISOString()}`);
      }

      this.device.log(`${logPrefix} Sending time sync:`, {
      ...
      // v5.5.206: Enhanced frame building for LCD climate sensors
      const sendFrame = async (attempt = 1) => {
        try {
          if (!endpoint) {
            this.device.log(`[TUYA] No endpoint ${targetEndpoint}`);
            return false;
          }

          // Build Tuya frame with proper header for extended payloads
          const dp = 0x24; // Time sync DP
          const datatype = useExtended ? 0x0C : 0x00; // Extended vs RAW
          ...
          this.device.log(`${logPrefix} 📤 Sending time sync attempt ${attempt} on EP${targetEndpoint}:`, frame.toString('hex'));
          ...
          if (typeof tuyaCluster.setData === 'function') {
            ...
            this.device.log(`${logPrefix} ✅ Time sync attempt ${attempt} sent via setData`);
            return true;
          }
          ...
          if (typeof tuyaCluster.command === 'function') {
            ...
            this.device.log(`${logPrefix} ✅ Time sync attempt ${attempt} sent via command(setData)`);
            return true;
          }

          // Method 3: Fallback - write raw attribute (least likely to work but worth trying)
          this.device.log(`${logPrefix} ⚠️ No setData/command available, skipping time sync`);
          this.device.log(`${logPrefix} Available cluster methods:`, Object.keys(tuyaCluster).filter(k => typeof tuyaCluster[k] === 'function').join(', '));
          return false;
        } catch (err1) {
          // Silent fail - time sync is not critical for device operation
          this.device.log(`${logPrefix} ℹ️ Time sync not supported by this device:`, err1.message);
          return false;
        }
      };
      ...
      setImmediate(() => {
        ...
        if (options.doubleSync !== false) {
          setTimeout(() => {
            this.device.log(`${logPrefix} 🔄 Sending second time sync for LCD climate sensor`);
            sendFrame(2);

            // v5.5.207: Send time_valid DP after second sync (if requested)
            if (sendTimeValidDP) {
              setTimeout(() => {
                this.device.log(`${logPrefix} 📡 Sending time_valid DPs to enable LCD display`);
      ...
      this.device.log(`${logPrefix} Time sync error: ${err.message}`);
      return false;
    }
  }
  ...
  _findTimeEndpoint(zclNode) {
    ...
    Object.keys(zclNode.endpoints).forEach(epId => {
      ...
      if (endpoint?.clusters) {
        this.device.log(`[TUYA] 📍 EP${epId} clusters:`, Object.keys(endpoint.clusters));
      }
    });
    ...
    for (const epId of Object.keys(zclNode.endpoints)) {
      ...
      if (hasEF00) {
        this.device.log(`[TUYA] 🎯 Found Tuya cluster on endpoint ${epId}`);
      ...
      for (const dpId of timeValidDPs) {
        try {
          ...
          if (typeof tuyaCluster.setData === 'function') {
            ...
            this.device.log(`[TUYA] 📡 Sent time_valid DP${dpId}=true on EP${endpointId}`);
          }
        } catch (err) {
          this.device.log(`[TUYA] ⚠️ Could not send DP${dpId}:`, err.message);
        }
      }

      return true;
    } catch (err) {
      this.device.log('[TUYA] ❌ time_valid DP send failed:', err.message);
      return false;
    }
  }
  ...
  setupDatapointListener(tuyaCluster) {
    try {
        ...
        try {
          ...
          if (endpoint) {
            ...
            const tuyaBound = new TuyaBoundCluster({
              device: this.device,
              onDataReport: (parsed) => {
                this.device.log('[TUYA-BOUND] 📥 dataReport callback:', JSON.stringify(parsed));
              ...
              onDataResponse: (parsed) => {
                this.device.log('[TUYA-BOUND] 📥 dataResponse callback:', JSON.stringify(parsed));
                if (parsed.datapoints) {
                  for (const dp of parsed.datapoints) {
                    // v5.5.204: CRITICAL - Add device safety check
                    if (this.device && !this.device.destroyed) {
              ...
              onMcuSyncTime: () => {
                this.device.log('[TUYA-BOUND] ⏰ Time sync request received');
            ...
            for (const clusterName of clusterNames) {
              try {
                ...
                this.device.log(`[TUYA] ✅ TuyaBoundCluster bound with name: ${clusterName}`);
                bound = true;
                break;
              } catch (e) {
                // Try next name
            ...
            if (!bound) {
              this.device.log('[TUYA] ⚠️ Could not bind TuyaBoundCluster, trying direct method...');
            }
            this._tuyaBoundCluster = tuyaBound;
          }
        } catch (boundErr) {
          this.device.log('[TUYA] ⚠️ BoundCluster setup failed:', boundErr.message);
          // Continue with fallback methods
        }
      } else {
        this.device.log('[TUYA] ⚠️ TuyaBoundCluster not available, using fallback listeners');
      }
      ...
      if (typeof tuyaCluster.on === 'function') {
        ...
        this.device.log('[TUYA] ✅ response listener registered on passed cluster');
      }
      ...
      try {
        ...
        if (directTuyaCluster && typeof directTuyaCluster.on === 'function') {
          ...
          directTuyaCluster.on('response', (value) => {
            this.device.log('[TUYA-DIRECT] 📥 response received!', JSON.stringify(value));
            // v5.5.202: Safe handleDatapoint call
            if (this.device && !this.device.destroyed) {
          ...
          directTuyaCluster.on('reporting', (value) => {
            this.device.log('[TUYA-DIRECT] 📥 reporting received!', JSON.stringify(value));
            // v5.5.202: Safe handleDatapoint call
            if (this.device && !this.device.destroyed) {
              this.handleDatapoint(value);
            }
          ...
          directTuyaCluster.on('dp', (dpId, value, dpType) => {
            this.device.log(`[TUYA-DIRECT] 📥 DP${dpId} = ${value}`);
          ...
          this.device.log('[TUYA] ✅ Direct cluster listeners registered (zclNode.endpoints[1].clusters.tuya)');
        } else {
          this.device.log('[TUYA] ⚠️ Direct tuya cluster not available or no .on() method');
          this.device.log('[TUYA] Available clusters:', Object.keys(endpoint?.clusters || {}).join(', '));
        }
      } catch (directErr) {
        this.device.log('[TUYA] ⚠️ Direct cluster listener failed:', directErr.message);
      }
  ...
  async requestDP(dp, options = {}) {
    ...
    try {
      ...
      if (this.passiveMode && !forceActive) {
        ...
        if (isBattery) {
          this.device.log(`[TUYA] ℹ️  Passive mode + battery device - skipping active request for DP${dp}`);
          return false;
        }
        this.device.log(`[TUYA] ⚡ Passive mode but mains-powered - attempting request for DP${dp}`);
      } else if (this.passiveMode && forceActive) {
        this.device.log(`[TUYA] 🔥 Passive mode but forceActiveTuyaMode=true - sending DP${dp} request`);
      }
        ...
        try {
          ...
          if (!sent && typeof tuyaCluster.getData === 'function') {
            try {
              ...
              this.device.log(`[TUYA] ✅ DP${dp} query sent via getData`);
              sent = true;
          ...
          if (!sent && typeof tuyaCluster.command === 'function') {
            try {
              ...
              this.device.log(`[TUYA] ✅ DP${dp} query sent via command('dataQuery')`);
              sent = true;
            } catch (e) { /* try next method */ }
          }
          ...
          if (!sent && typeof tuyaCluster.writeRaw === 'function') {
            try {
              ...
              this.device.log(`[TUYA] ✅ DP${dp} query sent via writeRaw`);
              sent = true;
            } catch (e) { /* try next method */ }
          }
          ...
          if (!sent && typeof tuyaCluster.mcuVersionRequest === 'function' && dp === 1) {
            try {
              ...
              this.device.log(`[TUYA] ✅ mcuVersionRequest sent (triggers DP reports)`);
              sent = true;
            } catch (e) { /* ignore */ }
          }
          ...
          if (!sent) {
            ...
            if (!this._getDataNotAvailableLogged) {
              this.device.log('[TUYA] ℹ️ No active query method available - using passive reporting only');
  ...
  async handleDatapoint(data) {
    ...
    if (!this.device || this.device.destroyed || !this.device.homey) {
      this.log('[TUYA] Device not available or destroyed, skipping datapoint');
      return;
    }
    ...
    try {
      ...
      if (!this.device || !this.device.homey) {
        this.log('[FLOW] Device or homey instance not available, skipping flow trigger');
        return;
      }
      ...
      if (flowCard) {
        flowCard.trigger(this.device, {
          ...
          this.log('[FLOW] tuya_dp_received trigger failed:', err.message);
        });
      }
    } catch (err) {
      this.log('[FLOW] Failed to get tuya_dp_received flow card:', err.message);
    }
    ...
    try {
      ...
      const triggerFlowCard = async (cardId, data) => {
        try {
          ...
          this.log(`[FLOW] ${cardId} trigger failed:`, err.message);
        }
      };
      ...
      this.log('[FLOW] Flow card trigger error:', err.message);
    }
