...
class TuyaEF00Manager extends EventEmitter {
  ...
  async _setupPassiveMode(endpoint, manufacturer) {
    ...
    // DP mappings for common presence radars
  ...
  _applyDPValue(dp, value) {
    ...
    if (mapping) {
      ...
      this.emit(`dp-${dp}`, parsedValue);
      this.emit('dataReport', { dpId: dp, dpValue: value, parsedValue });
    } else {
      this.device.log(`[TUYA-PASSIVE] ℹ️ Unknown DP ${dp}, value: ${JSON.stringify(value)}`);
    ...
    try {
      ...
      const attemptRequest = async (attempt = 0) => {
        try {
          ...
          // Battery devices timeout is normal
