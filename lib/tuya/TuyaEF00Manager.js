...
class TuyaEF00Manager extends EventEmitter {
  ...
  async _sendDPRaw(dpId, value, dpType) {
    try {
      ...
      const endpoint = this.zclNode.endpoints?.[1] || this.zclNode.endpoints?.[2];
      ...
      const sendWithFallback = async () => {
        for (let attempt = 0; attempt < Math.min(maxAttempts, fallbackChain.length); attempt++) {
          ...
          try {
            ...
            // WHY(P2475 / #533 Moes): Z2M-style mcuSyncTime first (epoch 1970/2000 by format)
            if (!sent && typeof tuyaCluster.command === 'function') {
              try {
            ...
            // Method 3: commandMcuSyncTime (legacy alias)
            if (!sent && typeof tuyaCluster.command === 'function') {
              try {
  ...
  async sendTuyaDP(dp, dpType, value) {
    try {
      ...
      // Send via endpoint frame (cluster object is optional for hollow nodes)
      await endpointUsed.sendFrame(0xEF00, buffer, 0x00);
