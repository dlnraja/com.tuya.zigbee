...
class TuyaEF00Manager extends EventEmitter {
    ...
    if (mapping) {
      ...
      this.device.setCapabilityValue(capability, parsedValue).catch(err => {
        ...
        if (err.message.includes('Invalid type') || err.message.includes('Invalid Capability Value')) {
          ...
          this.device.setCapabilityValue(capability, coercedValue).catch(e2 => {
    ...
    try {
      ...
      const sendFrame = async (attempt = 1) => {
        try {
          ...
          // Method 3: Try command('commandMcuSyncTime') pattern directly
          if (typeof tuyaCluster.command === 'function') {
            try {
              await tuyaCluster.command('commandMcuSyncTime', { payload: payload });
              this._log(`${logPrefix} ✅ Time sync attempt ${attempt} sent via commandMcuSyncTime`);
              return true;
            } catch (cmdErr) {
              this._log(`${logPrefix} ℹ️ commandMcuSyncTime failed, trying fallback:`, cmdErr.message);
            }
          }

          // Method 4: Raw sendFrame fallback (Magic Packet for incomplete interviews like Avatto WT198)
          if (typeof endpoint.sendFrame === 'function') {
            await endpoint.sendFrame(0xEF00, 0x11, 0x24, payload);
            this._log(`${logPrefix} ✅ Time sync attempt ${attempt} sent via raw sendFrame (0x24) Magic Packet`);
            return true;
          }

          this._log(`${logPrefix} ⚠️ No methods available to send time sync`);
          return false;
        } catch (err1) {
          // Attempt raw fallback even if an outer try-catch threw (e.g. cluster locked)
          try {
            if (endpoint && typeof endpoint.sendFrame === 'function') {
              await endpoint.sendFrame(0xEF00, 0x11, 0x24, payload);
              this._log(`${logPrefix} ✅ Time sync attempt ${attempt} sent via outer raw sendFrame (0x24) Magic Packet`);
              return true;
            }
          } catch (e2) {
             // Silent fail
  ...
  async requestDP(dp, options = {}) {
    ...
    // Destruction guard — prevent sending to destroyed device
    if (!this.device || this.device.destroyed || this.device._destroyed) {
      return false;
    }
    ...
    try {
      ...
      // v8.2.0: REORDERED — raw frame first (most reliable), then cluster methods
      const attemptRequest = async (attempt = 0) => {
        try {
          const timeoutMs = this.device.hasCapability?.('measure_battery') ? 3000 : 8000;
          let sent = false;

          // Method 1: Raw frame (MOST RELIABLE — bypasses cluster abstraction)
          // Tuya dataQuery = command 0x03, payload = [seqHi, seqLo]
          // Some devices also need command 0x10 (mcuVersionRequest) as magic packet
          try {
            const endpoint = this.device.zclNode?.endpoints?.[1];
            if (endpoint && typeof endpoint.sendFrame === 'function') {
              // Send dataQuery (0x03) with seq number
              const dataQueryFrame = Buffer.from([
                0x00, // tuya command id for cluster
                (seq >> 8) & 0xFF, seq & 0xFF, // sequence number
                0x03, // dataQuery command
                0x00, 0x02, // length (2 bytes)
                (seq >> 8) & 0xFF, seq & 0xFF // seq echo
              ]);
              await Promise.race([
                endpoint.sendFrame(0xEF00, dataQueryFrame, 0x01),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), timeoutMs))
              ]);
              this._log(`[TUYA] ✅ DP${dp} dataQuery sent via raw frame`);
              sent = true;
            }
          } catch (e) { /* try next method */ }

          // Method 2: dataQuery command (cluster method — no args per TuyaSpecificCluster)
          if (!sent && typeof tuyaCluster.dataQuery === 'function') {
            try {
              ...
              this._log(`[TUYA] ✅ DP${dp} query sent via cluster.dataQuery`);
              sent = true;
            } catch (e) { /* try next method */ }
          }

          // Method 3: Generic command('dataQuery')
          if (!sent && typeof tuyaCluster.command === 'function') {
            try {
              await Promise.race([
                tuyaCluster.command('dataQuery', {}, { disableDefaultResponse: true }),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), timeoutMs))
              ]);
          ...
          // Method 4: mcuVersionRequest (magic packet — triggers some devices to report)
          ...
          // Method 5: Raw mcuVersionRequest (0x10) — triggers ALL devices to report
          if (!sent && dp === 1) {
            try {
              const endpoint = this.device.zclNode?.endpoints?.[1];
              if (endpoint && typeof endpoint.sendFrame === 'function') {
                const mcuFrame = Buffer.from([
                  0x00, // tuya command id
                  (seq >> 8) & 0xFF, seq & 0xFF, // seq
                  0x10, // mcuVersionRequest command
                  0x00, 0x02, // length
                  0x00, 0x01 // payload
                ]);
                await Promise.race([
                  endpoint.sendFrame(0xEF00, mcuFrame, 0x01),
                  new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), timeoutMs))
                ]);
                this._log('[TUYA] ✅ Raw mcuVersionRequest sent');
                sent = true;
              }
            } catch (e) { /* ignore */ }
          }
    ...
    try {
      ...
      while (offset < buffer.length) {
        ...
        try {
          ...
          // v8.2.0: Skip bad DP and continue to next (don't break entire frame)
          this._error('[TUYA] DP parse error, skipping:', parseErr.message);
  ...
  /**
   * Compatibility wrapper for sendDP(dp, value, type) calls throughout the codebase.
   * Maps string type names to numeric dpType values expected by sendTuyaDP.
   * @param {number} dp - Data Point ID
   * @param {any} value - Value to send
   * @param {string|number} type - DP type as string ('bool','value','enum','raw') or numeric
   */
  async sendDP(dp, value, type) {
    const TYPE_MAP = { bool: 1, boolean: 1, value: 2, number: 2, string: 3, enum: 4, raw: 0, bitmap: 5 };
    const dpType = typeof type === 'number' ? type : (TYPE_MAP[String(type).toLowerCase()] ?? 2);
  ...
  async sendTuyaDP(dp, dpType, value) {
    ...
    try {
      ...
      try {
        ...
        this._log(`[TUYA] ✅ DP${dp} sent via raw frame`);
        return true;
      } catch (e) { /* try fallbacks */ }

      // Method 2: Cluster setData command
      if (tuyaCluster && typeof tuyaCluster.setData === 'function') {
        try {
          await tuyaCluster.setData({
            status: 0, transid: 0, dp, datatype: dpType,
            length: buffer.length - 4, data: buffer.slice(4)
          });
          this._log(`[TUYA] ✅ DP${dp} sent via cluster.setData`);
          return true;
        } catch (e) { /* try next */ }
      }

      // Method 3: Generic command('datapoint')
      if (tuyaCluster && typeof tuyaCluster.command === 'function') {
        try {
          await tuyaCluster.command('datapoint', {
            status: 0, transid: 0, dp, datatype: dpType,
            length: buffer.length - 4, data: buffer.slice(4)
          }, { disableDefaultResponse: true });
          this._log(`[TUYA] ✅ DP${dp} sent via command('datapoint')`);
          return true;
        } catch (e) { /* last resort failed */ }
      }

      this._error(`[TUYA] All send methods failed for DP${dp}`);
      return false;
    } catch (err) {
  ...
  async handleStubbornDevice() {
    ...
    if (this._stubbornRetryCount < 5) {
      ...
      this._stubbornTimer = setTimeout(() => this.handleStubbornDevice(), backoffMs * 2);
    }
  ...
  cleanup() {
    ...
    // _watchdogTimer uses setInterval, so clear with clearInterval
