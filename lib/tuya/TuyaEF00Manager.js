...
class TuyaEF00Manager extends EventEmitter {
  ...
  /**
   * v5.5.378: CRITICAL FIX - Time sync for LCD climate sensors
   * Reference: https://github.com/Koenkk/zigbee2mqtt/issues/30054
   *
   * KEY FINDINGS FROM Z2M RESEARCH:
   * - Tuya LCD devices expect timestamps in TUYA EPOCH (2000-01-01), NOT Unix (1970)
   * - Payload format: [Local:4 bytes BE][UTC:4 bytes BE] - LOCAL FIRST!
   * - Unix epoch timestamp for Dec 2025: ~1,766,000,000 → shows year 2055 on LCD (WRONG)
   * - Tuya epoch timestamp for Dec 2025: ~820,000,000 → shows correct year 2025
   *
   * TUYA_EPOCH_OFFSET = 946684800 seconds (from 1970-01-01 to 2000-01-01)
   */
  async sendTimeSync(zclNode, options = {}) {
    if (!zclNode) return false;

    // v5.5.378: Extract all sync parameters including useTuyaEpoch
    const {
      forceSync = false,           // BYPASS all conditions - FORCE sync for passive devices
      useExtendedPayload = false,  // Use 8-byte timestamp payload for LCD climate sensors
      useTuyaEpoch = true,         // v5.5.378: CRITICAL - Use Tuya epoch (2000) for LCD devices
    ...
    // v5.5.378: TUYA EPOCH OFFSET - difference between Unix (1970) and Tuya (2000) epochs
    const TUYA_EPOCH_OFFSET = 946684800; // seconds from 1970-01-01 to 2000-01-01

    try {
      const now = new Date();
      const unixTimestamp = Math.floor(now.getTime() / 1000);
      const timezoneOffsetSec = now.getTimezoneOffset() * -60; // Convert to seconds, flip sign (GMT+1 = +3600)

      // v5.5.378: CRITICAL - Apply Tuya epoch offset for LCD devices
      // LCD devices interpret timestamps as "seconds since 2000-01-01"
      // If we send Unix timestamp, they show year 2055 instead of 2025!
      const utcTimestamp = useTuyaEpoch ? (unixTimestamp - TUYA_EPOCH_OFFSET) : unixTimestamp;
      const localTimestamp = utcTimestamp + timezoneOffsetSec;

      // v5.5.378: Support both standard (7 bytes date) and extended (8 bytes timestamp) payloads
      const useExtended = useExtendedPayload || this._needsExtendedPayload();
      let payload;

      if (useExtended) {
        // v5.5.378: FIXED 8-byte payload format: [Local:4 bytes BE][UTC:4 bytes BE]
        // Research from Z2M #30054 and TuyaTimeSync.js confirms LOCAL time FIRST!
        payload = Buffer.alloc(8);
        payload.writeUInt32BE(localTimestamp, 0);  // LOCAL time FIRST (4 bytes BE)
        payload.writeUInt32BE(utcTimestamp, 4);    // UTC time SECOND (4 bytes BE)
        this._log(`${logPrefix} 🕒 Time sync payload: [Local:${localTimestamp}][UTC:${utcTimestamp}]`);
        this._log(`${logPrefix} 🕒 Epoch: ${useTuyaEpoch ? 'TUYA (2000)' : 'UNIX (1970)'}`);
        this._log(`${logPrefix} 🕒 Hex: ${payload.toString('hex')}`);
      } else {
        // Standard 7-byte payload (year-2000, month, day, hour, min, sec, weekday)
        payload = Buffer.from([
        ...
        this._log(`${logPrefix} 🕒 Standard time sync: ${now.toISOString()}`);
      }

      this._log(`${logPrefix} Sending time sync:`, {
        date: now.toISOString(),
        localTime: new Date((useTuyaEpoch ? localTimestamp + TUYA_EPOCH_OFFSET : localTimestamp) * 1000).toISOString(),
        payload: payload.toString('hex'),
        epoch: useTuyaEpoch ? 'TUYA_2000' : 'UNIX_1970',
        forceSync: forceSync,
