...
class TuyaEF00Manager extends EventEmitter {
  ...
  async handleDatapoint(data) {
    ...
    // reporting, dp). Chaque DP était traité 2-4× → double update capability + spam logs.
    // Fix : Map _processedDPs (dp → timestamp) avec fenêtre 500ms. Un même DP avec la
    // MÊME valeur dans la fenêtre est ignoré (un changement de valeur passe toujours).
    if (!this._processedDPs) this._processedDPs = new Map();
    const dpKey = `${dp}:${JSON.stringify(value)}`;
    const now = Date.now();
    const lastSeen = this._processedDPs.get(dpKey);
    if (lastSeen && (now - lastSeen) < 500) {
      return; // Doublon dans la fenêtre 500ms → skip silencieux
    }
    this._processedDPs.set(dpKey, now);
    // Nettoyage périodique (évit croissance mémoire)
    if (this._processedDPs.size > 200) {
      for (const [k, t] of this._processedDPs) {
        if (now - t > 5000) this._processedDPs.delete(k);
      }
    }
