...
class TuyaZigbeeDevice extends ZigBeeDeviceWithDiagnostics {
  ...
  async triggerFlowCard(cardId, tokens = {}, state = {}) {
    // 🛡️ v8.5.0: Guard against destroyed device
    ...
    try {
      ...
      if (flow && typeof flow.getDeviceTriggerCard === 'function') {
        try {
          ...
          // If not found as device trigger, fallback
        }
      }

      // 2. Deprecated fallback removed in v7.5.31 to follow SDK3 strictly
      if (!card) {
          this.log(`[FLOW-MANAGER]  Card NOT found in driver or app: ${cardId}`);
      }
  ...
  /**
   * 🛡️ v8.5.0: Safe setCapabilityValue with destruction guard
  ...
  async _safeSetCapability(capability, value) {
    ...
    try {
      ...
      if (!this._destroyed) {
        this.error(`[_safeSetCapability] Error setting ${capability}:`, err.message);
      }
    }
    return this;
  }
  ...
  /**
   * 🛡️ v8.5.0: Cleanup method for onDeleted/onUninit
  ...
  async _destroyDevice() {
    ...
    // Clean up universal bridge
    if (this._universalBridge && typeof this._universalBridge.destroy === 'function') {
      try { this._universalBridge.destroy(); } catch(e) {}
      this._universalBridge = null;
    }

    // Clean up smart battery handler
    if (this.smartBattery && typeof this.smartBattery.destroy === 'function') {
      try { this.smartBattery.destroy(); } catch(e) {}
      this.smartBattery = null;
    }

    // Clean up smart energy handler
    if (this.smartEnergy && typeof this.smartEnergy.destroy === 'function') {
      try { this.smartEnergy.destroy(); } catch(e) {}
      this.smartEnergy = null;
    }

    // Clean up smart adaptation
    if (this.smartAdaptation && typeof this.smartAdaptation.destroy === 'function') {
      try { this.smartAdaptation.destroy(); } catch(e) {}
    }
    
    this.log('🛡️ [DESTROY] Device resources cleaned up');
  }
