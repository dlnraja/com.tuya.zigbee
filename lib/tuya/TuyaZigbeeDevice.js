...
class TuyaZigbeeDevice extends ZigBeeDeviceWithDiagnostics {
  ...
  async runIntelligentAdaptation() {
    ...
    try {
      ...
      const identificationDatabase = this.homey.app?.identificationDatabase || null : null;
  ...
  async _checkOtaRoutine() {
    ...
    try {
      const update = await this.homey.app.otaManager?.checkUpdate(this) : null;
  ...
  async retryIASEnrollment(maxRetries = 3) {
    const iasCluster = this.zclNode?.endpoints?.[1]?.clusters?.iasZone : null;
