...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  _setupRawFrameFallback() {
    ...
    wrapHandleFrame(node, 'raw-l0-fallback', (args, next) => {
      ...
      if (self._unhandledFrameCount <= 5 || self._unhandledFrameCount % 100 === 0) {
        self.log(`[L0] ⚠️ Unhandled frame #${self._unhandledFrameCount}: EP${endpointId} cluster=0x${Number(clusterId).toString(16).padStart(4, '0')} cmd=${frame?.cmdId || '?'}`);
      }
      return next(...args);
    });
