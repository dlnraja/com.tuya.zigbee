...
const { trackTx, trackRx, isRadarFloodDriver } = require('../utils/UniversalThrottleManager');
...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  /**
   * Soft Homey timeline alert / radar log for RX flood shed (P2389).
   * Kept out of the L0 hot path body so P2284 chain-forward stays adjacent to [RX-SHED].
   * @param {{ count?: number }} rxResult
   */
  _softRxShedHomeyAlert(rxResult) {
    ...
    if (!isRadarFloodDriver(driverId)) {
      ...
      this.log(`[RX-SHED][P2389] Radar firmware chatty (${count}/min) — coalescing telemetry; no Homey alert`);
    }
  }
  ...
  trackIncomingReport() {
    ...
    // WHY(P2389): classify mmWave/presence as radar RX budget (firmware chatty)
    const driverId = this.driver?.id || '';
    const rxType = isRadarFloodDriver(driverId) ? 'radar' : undefined;
