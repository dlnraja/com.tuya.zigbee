  ...
  _setupRawFrameFallback() {
    ...
    wrapHandleFrame(node, 'raw-l0-fallback', (args, next) => {
      ...
      if (rxResult.exceeded && !keepAlways && clusterId !== 0x0500 && clusterId !== 0x0501) {
        ...
        const driverId = self.driver?.id || '';
        if (!isRadarFloodDriver(driverId)) {
          const lastAlert = self.getStoreValue?.('last_flood_alert_at') || 0;
          if (Date.now() - lastAlert > 86400000) {
            Promise.resolve(self.setStoreValue?.('last_flood_alert_at', Date.now())).catch(() => {});
            self.homey.notifications.createNotification({
              excerpt: `⚠️ ${self.getName()} is flooding the Zigbee network (${rxResult.count} msg/min) — known Tuya firmware bug class. Reports are being rate-limited automatically.`
            }).catch(() => {});
          }
        } else if (self._rxShedLogCount === 1 || self._rxShedLogCount % 120 === 0) {
          self.log(`[RX-SHED][P2389] Radar firmware chatty (${rxResult.count}/min) — coalescing telemetry; no Homey alert`);
        }
