...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  async runIntelligentAdaptation() {
    ...
    // P2502 BOTH (Peter OOM complementary): defer SMART ADAPT under heap pressure
    try {
  ...
  async _registerButtonCapabilityListeners() {
    ...
    // WHY(P2397): soft-ensure button.N / onoff.gangN BEFORE registering UI listeners
    try {
    ...
    for (const cap of caps) {
      ...
      try {
        this.registerCapabilityListener(cap, async () => {
          ...
          // WHY(P2492/P2497): Homey charter virtual path (dedup + role-aware)
          try {
