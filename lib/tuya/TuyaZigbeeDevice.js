...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  constructor(...args) {
    ...
    // 🛡️ Bind log/error — empêche "Cannot read properties of undefined (reading '_destroyed')"
    // quand this.error/this.log est passé non bindé en callback (.catch(this.error), etc.)
    // Guarded: on newer SDK3 runtimes log/error are read-only accessors — the
    // assignment itself throws "Cannot assign to read only property 'error'"
    // and kills device init (crash reports: ContactSensor/SosEmergency/WaterLeak).
    try { this.error = this.error.bind(this); } catch (_e) { /* read-only on this SDK: already callable */ }
    try { this.log = this.log.bind(this); } catch (_e) { /* read-only on this SDK: already callable */ }
  ...
  async onNodeInit({ zclNode } = {}) {
    ...
    await this.telemetryEstimator?.refresh?.('tuya-node-init').catch(err => this.log(`[TELEMETRY] init refresh failed: ${err.message}`));
    ...
    this.node.handleFrame = (endpointId, clusterId, frame, meta) => {
      ...
      if (rxResult.exceeded && clusterId !== 0x0500 && clusterId !== 0x0501) {
        ...
        if (Date.now() - lastAlert > 86400000) {
          ...
          this.setStoreValue?.('last_flood_alert_at', Date.now()).catch(() => {});
  ...
  async _generateIssueReport() {
    ...
    await this.setStoreValue?.('last_issue_report_md', md).catch(() => {});
    ...
    if (endpoint && /^https:\/\/[\w.-]+\.workers\.dev\/?$/.test(endpoint)) {
      try {
        ...
        posted = await new Promise((resolve) => {
  ...
  async retryIASEnrollment(maxRetries = 3) {
    ...
    for (let i = 1; i <= maxRetries; i++) {
      try {
        ...
        if (cie) {
          try {
            await iasCluster.writeAttributes({ iasCieAddr: cie });
  ...
  canSendCommand(commandType = 'command') {
    ...
    if (this._txTotal % 25 === 0) {
      this.setStoreValue?.('stats_tx_total', this._txTotal).catch(() => {});
    }
  ...
  trackIncomingReport() {
    ...
    if (this._rxTotal % 25 === 0) {
      this.setStoreValue?.('stats_rx_total', this._rxTotal).catch(() => {});
    }
    return result;
  }
      ...
      try {
        this.registerCapabilityListener(cap, async () => {
          ...
          try {
            ...
            // Fire the same flow a physical press would fire (bidirectional UX)
