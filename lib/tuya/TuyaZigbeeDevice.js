...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  async initSmartManagers() {
    try {
      // WHY(P2673 Bastien-first / P2701): profile energy before SmartEnergy audit
      // Non-metered / interview gaps stay complementary soft — never invent phantom power.
      try {
        const IntelligentEnergyAdapter = require('../energy/IntelligentEnergyAdapter');
        await IntelligentEnergyAdapter.apply(this);
      } catch (e) {
        this.log(`[P2673] IntelligentEnergyAdapter soft-fail: ${e.message}`);
      }
  ...
  async _triggerCustomFlowsIfNeeded(capability, value, previousValue) {
    ...
    try {
      ...
      // WHY(P2747): Homey tag icon = tokens — fire declared *_changed cards
      try {
