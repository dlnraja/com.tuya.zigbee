...
const { shouldWaitForDefaultResponse } = require('../zigbee/ZclDefaultResponsePolicy');
...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  async onUninit() {
    ...
    CapabilityMapCache.invalidate(this);
    // App restart/update uses onUninit, not onDeleted — timers must tear down here.
    await this._destroyDevice();
  ...
  async registerOnOffCapability() {
    try {
      ...
      if (shouldWaitForDefaultResponse(this) === false) {
        opts.waitForResponse = false;
      }
      await this.registerCapability('onoff', 'genOnOff', opts);
  ...
  async safeSetCapabilityValue(capability, value) {
    ...
    try {
      ...
      // Guard: computed caps (meter_*) are Homey-integrated — device must NOT write these.
      // If device reports meter_power, write measure_power instead (native measurement).
      if (!this.hasCapability(capability)) {
        const COMPUTED_CAPS = new Set(['meter_gas', 'meter_power', 'meter_rain', 'meter_water']);
        const COMPUTED_REMAP = { meter_power: 'measure_power' };
        if (COMPUTED_CAPS.has(capability)) {
          const remapped = COMPUTED_REMAP[capability];
          if (remapped) { capability = remapped; }
          else { /* Homey-computed, skip addCapability */ }
        }
        if (!COMPUTED_CAPS.has(capability)) {
          await this.addCapability(capability).catch(() => {});
        }
