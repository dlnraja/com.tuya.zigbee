...
class TuyaZigbeeDevice extends ZigBeeDeviceWithDiagnostics {
  ...
  get sessionManager() { return this.homey.app.sessionManager; }
  get healthMonitor() { return this.homey.app.healthMonitor; }
  get sanityFilter() { return this.homey.app.sanityFilter; }
  ...
  async _checkOtaRoutine() {
    ...
    try {
      const update = await this.homey.app.otaManager?.checkUpdate(this);
  ...
  trackIncomingReport() {
    ...
    if (this.homey.app.healthMonitor) {
      this.homey.app.healthMonitor.recordCheckIn(deviceId, modelId);
    }
