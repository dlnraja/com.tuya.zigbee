...
class TuyaZigbeeDevice extends VirtualButtonMixin(PhysicalButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  async onNodeInit({ zclNode } = {}) {
    ...
    if (this._protocolInfo?.isTuyaDP || this._protocolInfo?.protocol === 'HYBRID') {
      setTimeout(() => this.tuyaEF00Manager?.queryAllDatapoints?.(), 2000);
    }
  }
  ...
  _setupRawFrameFallback() {
    ...
    this.node.handleFrame = (endpointId, clusterId, frame, meta) => {
      ...
      // Cleanup old entries every 100 frames
      if (this._frameDedupCount++ % 100 === 0) {
  ...
  async _attemptReconnection() {
    ...
    try {
      ...
      if (state.consecutiveFailures <= state.maxRetries) {
        ...
        setTimeout(() => this._attemptReconnection(), backoffMs);
      } else {
        await this._notifyPairingFailure();
      }
    }
