...
class TuyaZigbeeDevice extends ZigBeeDeviceWithDiagnostics {
   ...
   * Falls back to deprecated getTriggerCard if getDeviceTriggerCard fails or is missing.
   *
  ...
  async triggerFlowCard(cardId, tokens = {}, state = {}) {
    ...
    try {
      ...
      // 2. Try deprecated getTriggerCard as fallback
      if (!card && this.homey.flow && typeof this.homey.flow.getTriggerCard === 'function') {
        try {
