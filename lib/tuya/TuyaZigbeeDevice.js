...
const { trackTx, trackRx, isRadarFloodDriver, isRadarFloodContext } = require('../utils/UniversalThrottleManager');
...
const { shouldSkipEf00Query } = require('../zigbee/MeshFloodCalm');
...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
   ...
   *
   * WHY(P2282): Never use isForbiddenPlacement when pid is known — that mfr-wide
   * helper poisoned HOBEIAN water (3315-S / ZG-222*) via rain ZG-223Z forbid
   * (forum #2202 Peter diag 95a7c6e5 → false "expect soil_sensor").
   * Placement stays enricher-only (unknown pid).
   */
  async _warnIfMisattributedDriver() {
    ...
    // Couple-first: known pid → only exact forbiddenDrivers for that couple.
    // Unknown pid → placement helper (enricher-class bans), never invent target.
    const forbidden = pid
      ? registry.isForbiddenDriver(mfr, pid, driverId)
      : registry.isForbiddenPlacement(mfr, driverId);
    if (!forbidden) {
      // WHY(P2289): Peter #2203 OCR — pre-P2282 setUnavailable("expect soil_sensor")
      // persists across app updates until we explicitly clear MISATTR state.
      try {
        const reason = typeof this.getUnavailableReason === 'function'
          ? String(this.getUnavailableReason() || '')
          : '';
        if (/Wrong driver|expect\s+\w+_sensor|MISATTR/i.test(reason)) {
          await this.setAvailable?.();
          this.log(`[MISATTR] cleared stale unavailable for ${mfr}+${pid || '?'}`);
        }
      } catch { /* optional */ }
      try {
        const warn = typeof this.getWarning === 'function' ? String(this.getWarning() || '') : '';
        if (/Wrong driver|expect\s+\w+_sensor/i.test(warn)) {
          await this.unsetWarning?.();
        }
      } catch { /* optional */ }
      return;
    }
    const c = registry.lookup(mfr, pid);
  ...
  async onNodeInit({ zclNode } = {}) {
    ...
    // WHY(P2500 / T43287 Athom): fleet heal — sensor measure_*/meter_*/alarm_* must
    // stay getable (tiles + Insights). Runs after button charter so button.N can
    // keep getable:false while measure_battery stays true (P2499 Peter).
    try {
      const { healSensorCapabilityGetable } = require('../utils/HomeyCapabilityUx');
      await healSensorCapabilityGetable(this);
    } catch (_e) { /* soft */ }

    // WHY(P2506): Homey forbids measure_battery + alarm_battery — own tuya_battery_low.
    try {
      if (this.hasCapability?.('measure_battery')) {
        const SmartBatteryLowAlarm = require('../battery/SmartBatteryLowAlarm');
        await SmartBatteryLowAlarm.ensureCapability(this);
        const pct = this.getCapabilityValue('measure_battery');
        if (typeof pct === 'number') {
          await SmartBatteryLowAlarm.applyFromPercent(this, pct, { emitFlows: false });
        }
      }
    } catch (_e) { /* soft */ }

    // WHY(P2508): soft hint when this driver ships Homey Device Updates OEM images
    try {
      const { softAnnounceNativeOta } = require('../ota/HomeyDeviceUpdates');
      softAnnounceNativeOta(this);
    } catch (_e) { /* soft */ }

    // 🛡️ v5.13.0: UNIVERSAL TX/RX FALLBACK HANDLER
    this._setupRawFrameFallback();

    // P2321: quiet-mains avail poll (HomeSuite ZBMINIR2 idea — MIT reimpl)
    this._maybeStartQuietMainsAvailPoll();
    ...
    safeSetTimeout(this, async () => {
      ...
      try {
        ...
        // WHY(P2665): HOBEIAN ZG-301Z has unused 0xEF00 — queryAll flooded Bastien mesh
        if ((this._protocolInfo?.isTuyaDP || this._protocolInfo?.protocol === 'HYBRID')
          && !shouldSkipIasOnlyEf00Tx(this)
          && !shouldSkipEf00Query(this)) {
  ...
  _setupRawFrameFallback() {
    ...
    // WHY(P2284): register as chain tag — never blind-overwrite Physical 0xFD / IO EF00
    const { wrapHandleFrame } = require('../utils/BidirectionalButtonState');
    const self = this;
    const node = this.node;
    wrapHandleFrame(node, 'raw-l0-fallback', (args, next) => {
      ...
      try {
        const availMgr = self.homey.app && self.homey.app.availabilityManager;
        if (availMgr) {
          const devId = (typeof self.getData === 'function' && self.getData() && self.getData().id) || self.id;
          if (devId) { availMgr.markSeen(devId); }
        }
      } catch (_e) { /* non-critical */ }

      // WHY(P2284): never drop genOnOff / IAS / EF00 on L0 dedup — button 0xFD + SOS
      const keepAlways = clusterId === 6 || clusterId === 0x0006
        || clusterId === 0x0500 || clusterId === 0x0501
        || clusterId === 0xEF00 || clusterId === 61184;

      const duplicate = self._rawFrameDeduplicator.shouldSuppress(endpointId, clusterId, frame, meta);
      if (duplicate.suppress && !keepAlways) {
        self.trackIncomingReport();
        if (!self._rawFrameDuplicateCount) {self._rawFrameDuplicateCount = 0;}
        self._rawFrameDuplicateCount += 1;
        if (self._rawFrameDuplicateCount <= 5 || self._rawFrameDuplicateCount % 100 === 0) {
          self.log(`[L0] Suppressed exact duplicate raw frame #${self._rawFrameDuplicateCount}: EP${endpointId} cluster=0x${Number(clusterId).toString(16).padStart(4, '0')} age=${duplicate.age}ms`);
        }
        // WHY(P2284): never orphan Physical 0xFD / IO / BoundCluster — forward only
        return next(...args);
      }

      let handled = false;

      self._diagLogger.frameReceived({
        endpointId,
        clusterId,
      ...
      if (typeof self.onZigBeeMessage === 'function') {
        try {
          if (self.onZigBeeMessage(endpointId, clusterId, frame, meta) === true) {
            handled = true;
          }
        } catch (e) {
          self._diagLogger.operationFailed('onZigBeeMessage handler', e, {
            endpointId,
            clusterId
          });
        }
      ...
      if (!handled && typeof self.onZigbeeMessage === 'function') {
        try {
          if (self.onZigbeeMessage(endpointId, clusterId, frame, meta) === true) {
            handled = true;
          }
        } catch (e) {
          self._diagLogger.operationFailed('onZigbeeMessage handler (legacy)', e, {
            endpointId,
            clusterId
          });
        }
      ...
      // WHY(P2661 / Bastien be119f76+52ef684a): Time(0x000A)/OTA(0x0019) chatter must NOT
      // inflate RX flood counters — diags looked like flood while 3ch was on Homey Virtual.
      const cidForRx = Number(clusterId);
      const quietRx = cidForRx === 0x000a || cidForRx === 0x0019 || cidForRx === 10 || cidForRx === 25;
      const rxResult = quietRx
        ? { exceeded: false, count: 0 }
        : self.trackIncomingReport();
      try {
        self.protocolRxTx?.noteRx?.(clusterId, { endpointId });
      } catch (_e) { /* soft */ }
      if (rxResult.exceeded && !keepAlways && clusterId !== 0x0500 && clusterId !== 0x0501) {
        if (!self._rxShedLogCount) {self._rxShedLogCount = 0;}
        self._rxShedLogCount++;
        if (self._rxShedLogCount <= 3 || self._rxShedLogCount % 60 === 0) {
          self.log(`⚠️ [RX-SHED] Flood protection: shedding frame (${rxResult.count}/min, cluster 0x${Number(clusterId).toString(16)})`);
        }
        // WHY(P2284): shed heavy L0 work only — still forward chain (buttons/EF00 siblings)
        try { self._softRxShedHomeyAlert(rxResult); } catch (_shedSoft) { /* soft */ }
        return next(...args);
      }

      // Driver handled: still continue chain so BoundCluster/0xFD siblings see frame
      if (handled) {
        return next(...args);
      }

      if (!self._unhandledFrameCount) {self._unhandledFrameCount = 0;}
      self._unhandledFrameCount++;
      // WHY(P2645 / Bastien 4d4e1684): Time (0x000A) + OTA (0x0019) chatter is normal —
      // do not spam diagnostic logs / user diags with L0 warnings.
      const cidNum = Number(clusterId);
      const quietCluster = cidNum === 0x000a || cidNum === 0x0019 || cidNum === 10 || cidNum === 25;
      if (!quietCluster && (self._unhandledFrameCount <= 5 || self._unhandledFrameCount % 100 === 0)) {
        self.log(`[L0] ⚠️ Unhandled frame #${self._unhandledFrameCount}: EP${endpointId} cluster=0x${cidNum.toString(16).padStart(4, '0')} cmd=${frame?.cmdId || '?'}`);
      }
      return next(...args);
    });
  ...
  /**
   * WHY(P2401 / VicHY): radar flood calm even when driver.id is empty mid-update.
   * Soft Homey alert / radar log for RX flood shed stays adjacent to [RX-SHED] (P2284/P2389).
   * @returns {boolean}
   */
  _isRadarFloodCalmDevice() {
    let hasDistanceCap = false;
    try { hasDistanceCap = !!this.hasCapability?.('measure_luminance.distance'); } catch (_e) { /* soft */ }
    let mfr = '';
    try {
      mfr = this.getSetting?.('zb_manufacturer_name')
        || this.getStoreValue?.('zb_manufacturer_name')
        || this.getData?.()?.manufacturerName
        || '';
    } catch (_e2) { /* soft */ }
    return isRadarFloodContext({
      driverId: this.driver?.id || '',
      manufacturerName: mfr,
      hasDistanceCap,
      floodCalm: this._radarFloodCalm === true,
    });
  }

  _softRxShedHomeyAlert(rxResult) {
    // WHY(P2389): mmWave/presence radars firmware-spam ~200 msg/min (Z2M#14742).
    // App already coalesces DP distance/lux — Homey timeline alerts scare users for expected behaviour.
    // WHY(P2401): also suppress when mfr/distance cap known but driver.id lags (VicHY update timeline).
    const count = rxResult?.count || 0;
    if (!this._isRadarFloodCalmDevice()) {
      ...
      if (Date.now() - lastAlert > 86400000) {
        ...
        this.homey.notifications.createNotification({
          excerpt: `⚠️ ${this.getName()} is flooding the Zigbee network (${count} msg/min) — known Tuya firmware bug class. Reports are being rate-limited automatically.`
        }).catch(() => {});
      }
    } else if (this._rxShedLogCount === 1 || this._rxShedLogCount % 120 === 0) {
      this.log(`[RX-SHED][P2389/P2401] Radar firmware chatty (${count}/min) — coalescing telemetry; no Homey alert`);
    }
  }
  ...
  async onEndDeviceAnnounce() {
    ...
    // WHY(P2461): noEf00Tx buttons (mrpevh8p) are HYBRID but have no 0xEF00 — skip blast
    // WHY(P2665): HOBEIAN ZG-301Z unused EF00 — skip queryAll (Bastien ~5k TX/node)
    let skipEf00Query = false;
    try {
      const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
      skipEf00Query = !!(profile?.noEf00Tx) || shouldSkipEf00Query(this);
    } catch (_e) { /* soft */ }
    if ((this._protocolInfo?.isTuyaDP || this._protocolInfo?.protocol === 'HYBRID')
      && !shouldSkipIasOnlyEf00Tx(this)
      && !skipEf00Query) {
  ...
  async _reconfigureAttributeReporting() {
    ...
    // WHY(P2663): HOBEIAN / mesh-calm devices must not re-arm electrical or tight onOff
    // reporting on announce (Bastien 8× modules flooded mesh after tip 1.0.34).
    if (this._skipElectricalReporting || this._hobeianZg301z) {
      this.log('[PAIRING] P2663 skip attribute reconfigure (mesh calm / HOBEIAN)');
      return;
    }
    ...
    // Battery
    // WHY(P2461 / Peter #2230 diag 048cff91 + Z2M #8072): SH-SC07 / TS0041 family —
    // configuring batteryPercentageRemaining reporting drops the device hourly,
    // flashes LED, shortens battery life, and needs 2 presses (disco). Profile
    // skipBatteryReporting must skip this reconfigure path on announce/wake.
    if (this.hasCapability('measure_battery')) {
      let skipBattReport = false;
      try {
        const profile = typeof this.getDeviceProfile === 'function'
          ? this.getDeviceProfile()
          : (this._deviceProfile || null);
        skipBattReport = !!(profile?.skipBatteryReporting);
      } catch (_e) { /* soft */ }
      if (!skipBattReport) {
        const ep = this.zclNode.endpoints[1];
        ...
        this.log('[PAIRING] P2461 skip powerConfiguration reporting (profile.skipBatteryReporting / Z2M#8072)');
      }
    }
    ...
    if (this.hasCapability('onoff')) {
      ...
      if (cluster) {
        // WHY(P2682 / Bastien f37e8a91): minInterval 0 → 90ms onOff storms (count 241+)
        reportingConfigs.push({
          cluster: 'onOff',
          attributeName: 'onOff',
          minInterval: 1,
  ...
  async runIntelligentAdaptation() {
    ...
    // P2502 complementary (Peter 375def7f @ 9.0.895): SMART ADAPT storm under heap
    // pressure amplifies MaxListeners/OOM — defer until BootBudget allows heavy work.
    try {
      const BootBudget = require('../performance/BootBudget');
      if (BootBudget.isHeapCritical() || !BootBudget.shouldStartHeavyFeatures()) {
        this.log(`⏩ [SMART ADAPT] Deferred (heap ${BootBudget.heapUsedMb()}MB) — P2502`);
        return;
      }
    } catch (_e) { /* BootBudget optional in unit tests */ }
  ...
  async _tuyaReviveRoutine() {
    ...
    await run('Battery read', async () => {
      // WHY(P2665): mains HOBEIAN — skip genPowerCfg read storm
      if (shouldSkipEf00Query(this) || this._hobeianZg301z || this.mainsPowered) {
        this.log('[REVIVE] skip battery read (mains / mesh-calm)');
        return;
      }
  ...
  async _checkOtaRoutine() {
    // WHY(P2357/P2359): Homey Device Updates installs native Zigbee OTA from
    // firmwareUpdates / assets/firmware. Prefer Homey UI over external flash.
    this.log('🛡️ [OTA] Device Updates / maintenance check from Device View');
    let HDU = null;
    try { HDU = require('../ota/HomeyDeviceUpdates'); } catch { /* optional */ }
    try {
      const platform = HDU?.isDeviceUpdatesPlatformReady?.(this.homey);
      const hasNative = HDU?.driverHasNativeFirmwareUpdates?.(this) === true;
      ...
      const excerpt = HDU?.notificationExcerpt?.(this.getName(), payload)
        || (available
          ? `[${this.getName()}] Firmware v${update.newVersion} — open Homey → Settings → Device Updates.`
          : `${this.getName()} is on the latest firmware known to this app.`);
      await this.homey.notifications.createNotification({ excerpt }).catch(() => {});
      return HDU?.userGuidance?.(payload)
        || (available
          ? `Firmware v${update.newVersion} available via Homey Device Updates.`
          : 'Device is on the latest firmware known to this app.');
    } catch (err) {
  ...
  trackIncomingReport() {
    ...
    // WHY(P2389/P2401): classify mmWave/presence as radar RX budget (firmware chatty)
    const rxType = this._isRadarFloodCalmDevice() ? 'radar' : undefined;
    const result = trackRx(deviceId, rxType);
    if (result.exceeded) {
  ...
  async setCapabilityValue(capability, value) {
    // WHY(P2403 / diag 0e28d470): refuse DIY ghost caps BEFORE depth accounting —
    // leftover tuya_dp_* from older tips caused 90× "[P2308] recursion aborted" noise on radars.
    try {
      const cap = String(capability || '');
      if (/^tuya_dp_(raw|value|string|bitmap)$/.test(cap) || cap === 'tuya_cluster_event') {
        const diy = /universal|generic_diy|device_generic|fallback|catch.?all/i.test(String(this.driver?.id || ''));
        if (!diy) {return false;}
      }
    } catch (_e) { /* soft */ }

    // WHY(P2308 / Gmail #532): hard depth cap — SafeCapabilityMixin / child
    // overrides must never recurse set→safeSet→set into RangeError.
    this.__capSetDepth = (this.__capSetDepth || 0) + 1;
    try {
      if (this.__capSetDepth > 3) {
        // DIY aborts stay soft-once; real caps keep error visibility
        const cap = String(capability || '');
        if (/^tuya_dp_/.test(cap) || cap === 'tuya_cluster_event') {
          if (!this._p2308DiyAbortLogged) {
            this._p2308DiyAbortLogged = true;
            this.log?.(`[P2308/P2403] DIY recursion aborted once: ${cap}`);
          }
        } else {
          this.error?.(`[P2308] setCapabilityValue recursion aborted: ${capability}`);
        }
        return false;
      }
      return await this.safeSetCapabilityValue(capability, value);
    } finally {
      this.__capSetDepth -= 1;
    }
  }
  ...
  async safeSetCapabilityValue(capability, value, meta) {
    ...
    // WHY(P2392): refuse firmware-ghost DIY caps + phantom mains battery fleet-wide
    try {
      const cap = String(capability || '');
      if (/^tuya_dp_(raw|value|string|bitmap)$/.test(cap) || cap === 'tuya_cluster_event') {
        const diy = /universal|generic_diy|device_generic|fallback|catch.?all/i.test(String(this.driver?.id || ''));
      ...
      if ((cap === 'measure_battery' || cap === 'alarm_battery') && this.mainsPowered === true) {
        return false;
      }
      // WHY(P2506 / Homey SDK): never keep native alarm_battery beside measure_battery —
      // we own tuya_battery_low instead (smart hysteresis).
      if (cap === 'alarm_battery' && this.hasCapability?.('measure_battery')) {
        try {
          const SmartBatteryLowAlarm = require('../battery/SmartBatteryLowAlarm');
          await SmartBatteryLowAlarm.stripNativeAlarmBattery(this);
          const pct = this.getCapabilityValue('measure_battery');
          if (typeof pct === 'number') {
            await SmartBatteryLowAlarm.applyFromPercent(this, pct, { emitFlows: false });
          }
      ...
      if (Array.isArray(this._forbiddenCapabilities) && this._forbiddenCapabilities.includes(cap)) {
        return false;
      }
    } catch (_e) { /* fail-open */ }
    ...
    try {
      // WHY(P2302): Homey rejects onoff/alarms when DP parsers hand number 0/1
      if (typeof capability === 'string'
          && (capability === 'onoff' || capability.startsWith('onoff.')
            || capability.startsWith('alarm_') || capability === 'child_lock'
            || capability === 'locked')
          && typeof value === 'number') {
        value = value !== 0;
      }
      ...
      if (this.hasCapability(capability)) {
        ...
        // WHY(P2740 / GH#550 @ 9.0.1243): lux 10s + distance default 5s made light-off
        // lag several seconds and walk-away distance dip then jump (frames discarded).
        // WHY(P2744 / GH#550 @ 9.0.1250): keep snappy lux but EventDedup eats Δ<8 noise.
        const THROTTLE = {
          'measure_battery': 300000, 'alarm_battery': 300000,
          'measure_temperature': 30000, 'measure_humidity': 30000,
          'measure_luminance': 400, 'measure_luminance.distance': 200,
          'alarm_motion': 2000,
        ...
        // WHY(P2740): lux sig 50 missed stepwise light-off; distance needs sub-meter bypass.
        // WHY(P2744): lux sig 8 — real light steps bypass; noise Δ<8 handled by EventDedup.
        const SIGNIFICANT = {
          'measure_battery': 2, 'measure_temperature': 0.3,
          'measure_humidity': 2, 'measure_luminance': 8,
          'measure_luminance.distance': 0.08
        };
        ...
        if (tracker) {
          ...
          // WHY(P2740): any lux DROP bypasses (light-off must feel snappy)
          const isBooleanChange = typeof calibratedValue === 'boolean' && valueChanged;
          const isLuxDrop = capability === 'measure_luminance'
            && typeof calibratedValue === 'number' && typeof previousValue === 'number'
            && calibratedValue < previousValue;
          if (!valueChanged) {return true;} // No change, skip
          if (elapsed < throttleMs && !sigChange && !isBooleanChange && !isLuxDrop) {return true;} // Throttled
        }
        ...
        // 7b. P2506: app-owned battery-low (never Homey alarm_battery + measure_battery)
        if (capability === 'measure_battery' && typeof filteredValue === 'number') {
          try {
            const SmartBatteryLowAlarm = require('../battery/SmartBatteryLowAlarm');
            await SmartBatteryLowAlarm.applyFromPercent(this, filteredValue);
          } catch (_e) { /* soft */ }
        }

        await DeviceTelemetryEstimator.record(this, capability, filteredValue, {
          previousValue,
          // WHY(P2557): forward origin/source so estimated/calculated never become silent "measured"
  ...
  async _triggerCustomFlowsIfNeeded(capability, value, previousValue) {
    ...
    try {
      ...
      // WHY(P2449): declared *_brightness_changed cards for dim updates (fleet)
      if (capability === 'dim' || /^dim\.\d+$/.test(String(capability))) {
        try {
          const { emitBrightnessChanged } = require('../flow/DeclaredFlowCardAutoWire');
          const n = Number(value);
          const pct = !Number.isFinite(n) ? 0 : (n > 1 ? Math.round(n) : Math.round(n * 100));
          await emitBrightnessChanged(this, pct);
        } catch (_e) { /* optional */ }
      }

      // WHY(P2747/P2748): Homey tag icon = tokens — fire declared *_changed cards
      try {
  ...
  async _registerButtonCapabilityListeners() {
    ...
    // WHY(P2397/P2492): soft-ensure gang UI + Homey charter titles/maintenanceAction
    try {
      const { ensureGangUiCapabilities } = require('../utils/ensureGangUiCapabilities');
      await ensureGangUiCapabilities(this);
    } catch (_e) { /* soft */ }
    try {
      const { applyHomeyButtonUiCharter } = require('../utils/HomeyButtonUiCharter');
    ...
    for (const cap of caps) {
      ...
      try {
        this.registerCapabilityListener(cap, async () => {
          ...
          try {
            const { handleVirtualUiPress } = require('../utils/HomeyButtonUiCharter');
            const r = await handleVirtualUiPress(this, gang);
            this.log(`[BUTTON-UI] path=${r.path} role=${r.role || '?'}`);
          } catch (e) {
            this.log('[BUTTON-UI] action failed:', e.message);
          }
          return true;
        });
      } catch (e) {
  ...
  async _destroyDevice() {
    ...
    // P2321: stop quiet avail poll + unregister availability (sync, before store races)
    try { this._stopQuietMainsAvailPoll(); } catch (_e) { /* noop */ }
    try {
      const availMgr = this.homey?.app?.availabilityManager;
      if (availMgr && typeof availMgr.unregisterDevice === 'function') {
        const devId = (typeof this.getData === 'function' && this.getData()?.id) || this.id;
        if (devId) { availMgr.unregisterDevice(devId); }
      }
    } catch (_e) { /* app may already be tearing down */ }
  ...
  /**
   * P2321 — Quiet mains availability poll (HomeSuite idea, MIT reimplementation).
   * ZBMINIR2 / MINI-ZBD / BASICZBR3 often stay silent for hours; a cheap
   * genBasic.zclVersion read every 5 min feeds DeviceAvailabilityManager.
   * Mains-only; never for battery / measure_battery devices.
   */
  _isQuietMainsAvailCandidate() {
    const { isQuietMainsAvailCandidate } = require('../helpers/QuietMainsAvailPoll');
    return isQuietMainsAvailCandidate(this);
  }

  _maybeStartQuietMainsAvailPoll() {
    if (this._quietAvailPollTimer || this._destroyed) { return; }
    if (!this._isQuietMainsAvailCandidate()) { return; }
    const { safeSetInterval } = require('../utils/safe-timers');
    const INTERVAL_MS = 5 * 60 * 1000;
    this._quietAvailPollTimer = safeSetInterval(this, () => {
      this._quietMainsAvailTick().catch(() => {});
    }, INTERVAL_MS);
    this.log('[AVAIL] quiet-mains poll started (5 min zclVersion)');
  }

  _stopQuietMainsAvailPoll() {
    if (!this._quietAvailPollTimer) { return; }
    const { safeClearInterval } = require('../utils/safe-timers');
    safeClearInterval(this, this._quietAvailPollTimer);
    this._quietAvailPollTimer = null;
  }

  async _quietMainsAvailTick() {
    if (this._destroyed) { return; }
    try {
      const ep = this.zclNode?.endpoints?.[1];
      const basic = ep?.clusters?.basic || ep?.clusters?.genBasic;
      if (!basic || typeof basic.readAttributes !== 'function') { return; }
      await basic.readAttributes(['zclVersion']);
      const availMgr = this.homey?.app?.availabilityManager;
      if (availMgr) {
        const devId = (typeof this.getData === 'function' && this.getData()?.id) || this.id;
        if (devId) { availMgr.markSeen(devId); }
      ...
      try {
        const availMgr = this.homey?.app?.availabilityManager;
        const devId = (typeof this.getData === 'function' && this.getData()?.id) || this.id;
        if (availMgr && typeof availMgr.notePollFailure === 'function' && devId) {
          availMgr.notePollFailure(devId);
        }
      } catch (_e2) { /* noop */ }
    }
  }
