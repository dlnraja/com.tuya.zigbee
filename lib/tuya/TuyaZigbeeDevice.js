...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  async _checkOtaRoutine() {
    // WHY(P2357): Homey Device Updates (Pro/Cloud ≥13.2 + Mobile ≥9.10) installs
    // native Zigbee OTA from this app's firmwareUpdates / assets/firmware bins.
    // Prefer directing users to Homey UI over external Tuya/Z2M flash paths.
    ...
    try {
      ...
      if (update && update.available) {
        await this.homey.notifications.createNotification({
          excerpt: `[${this.getName()}] Firmware v${update.newVersion} available — open Homey → More → Settings → Device Updates (or this device → Maintenance) to install.`,
        }).catch(() => {});
        return `Firmware v${update.newVersion} available. Use Homey Device Updates (Settings → Device Updates) or Maintenance on this device. Requires Homey Pro ≥13.2 and Mobile App ≥9.10.`;
      }
      return 'Device is on the latest firmware known to this app. You can also check Homey → Settings → Device Updates.';
    } catch (err) {
