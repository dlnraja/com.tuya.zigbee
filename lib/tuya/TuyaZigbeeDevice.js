...
class TuyaZigbeeDevice extends PhysicalButtonMixin(VirtualButtonMixin(ZigBeeDeviceWithDiagnostics)) {
  ...
  async onDeleted() {
    ...
    // Backport (issue #513): Call parent onDeleted so ZigBeeDevice removes
    // node/zclNode listeners — otherwise late frames hit a disposed device and
    // Homey core crashes with "Cannot read properties of null (reading '_onDeleted')"
    if (super.onDeleted) {
   ...
   * v5.12.33: coordinator IEEE via IEEEAdvancedEnrollment (NEVER write the
   ...
   * v5.12.35: replaces the duplicated ad-hoc "smartDivisorDetect" heuristic —
