  ...
  if (impossible) {
    for (const f of ALT_FACTORS) {
      ...
      if (candJump >= -0.001 && candJump <= MAX_PLAUSIBLE_JUMP_KWH && cand <= ABSURD_ABSOLUTE_KWH) {
        ...
        v = cand;
        // Sync sticky correction into SmartDivisor SSOT when parse meta is present
        _syncStickyToSmartDivisor(device, st.factor);
        break;
      }
    }
...
/**
 * Drivers set device._energyParseMeta = { mfr, dpId, divisor } when parsing meter_power.
 * Sticky factor f means parsed kWh was too high → true divisor = baseDivisor / f.
 */
function _syncStickyToSmartDivisor(device, stickyFactor) {
  const meta = device && device._energyParseMeta;
  if (!meta || !meta.divisor || !stickyFactor || stickyFactor === 1) {return;}
  const learned = meta.divisor / stickyFactor;
  try {
    const { rememberLearnedDivisor } = require('../managers/SmartDivisorManager');
    const ok = rememberLearnedDivisor(meta.mfr || '', meta.dpId, meta.capability || 'meter_power', learned);
    if (ok && typeof device.log === 'function') {
      device.log(`[ENERGY-GUARD] SmartDivisor learned divisor=${learned} for dp=${meta.dpId} (sticky×${stickyFactor})`);
    }
  } catch (_) { /* optional — keep guard standalone */ }
}
