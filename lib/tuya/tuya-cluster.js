...
class TuyaCluster {
   ...
   * Fixes upstream issue #1263 - missing writeInteger implementation
   */
  async writeInteger(endpoint, dp, value) {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        // Write to Tuya manufacturer-specific cluster (0xEF00)
        return await endpoint.write('manuSpecificTuya', 'datapoint', payload);
      } catch (e) {
        if (this.isTimeout(e) && i < maxRetries - 1) {
          // Retry with jitter (50-120ms)
          const jitter = 50 + Math.random() * 70;
          await this.delay(jitter);
          this.log?.(`Retry ${i + 1}/${maxRetries} after ${Math.round(jitter)}ms`);
        } else {
