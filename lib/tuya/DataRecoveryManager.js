...
const TUYA_CLUSTER_ID = CLUSTERS.TUYA_EF00;
...
class DataRecoveryManager {
  ...
  _scheduleRecoveryCheck(delayMs, reason) {
    ...
    this.log(` Recovery check scheduled in ${delayMs/1000}s (${reason})`);
  }
  ...
  /**
   * Get list of capabilities with safeDivide(null, undefined) values
   * v5.5.318: Skip capabilities that don't apply to device type
   */
  _getMissingCapabilities() {
    const missing = [];
    const caps = this.device.getCapabilities?.() || [];// v5.5.318: Check if this is a button device (buttons don't need sensor data recovery)
  ...
  async _executeRecoveryStrategies(missingCaps) {
    ...
    if (stillMissing.length > 0) {
      this.log(` Still missing after recovery: ${stillMissing.join(', ')}`);
  ...
  async _readTemperature(endpoint) {
    const cluster = endpoint.clusters?.temperatureMeasurement;if (!cluster?.readAttributes) return;
    ...
    if (data?.measuredValue != null) {
      ...
      this.log(` Temperature: ${temp}Â°C`);
      await this.device.setCapabilityValue('measure_temperature', parseFloat(temp)).catch(() => { });
    }
  }

  async _readHumidity(endpoint) {
    const cluster = endpoint.clusters?.relativeHumidityMeasurement;if (!cluster?.readAttributes) return;
  ...
  async _readBattery(endpoint) {
    const cluster = endpoint.clusters?.powerConfiguration;if (!cluster?.readAttributes) return;
    ...
    if (data?.batteryPercentageRemaining != null) {
      ...
      const bat = Math.max(0, Math.min(100, Math.round(safeMultiply(safeDivide(voltage - 2.4, 0.6), 100))));
  ...
  async _readIlluminance(endpoint) {
    const cluster = endpoint.clusters?.illuminanceMeasurement;if (!cluster?.readAttributes) return;

    const data = await cluster.readAttributes(['measuredValue']);
    if (data?.measuredValue != null) {
      // Convert from log scale: lux = 10^((measuredValue-1)/10000)
      const lux = data.measuredValue > 0 ? Math.round(Math.pow(10, safeDivide(data.measuredValue - 1, 10000))) : 0;
  ...
  async _strategyConfigureReporting() {
    ...
    // Humidity reporting
    try {
      const humCluster = endpoint.clusters?.relativeHumidityMeasurement;if (humCluster?.configureReporting) {
    ...
    // Battery reporting
    try {
      const powerCluster = endpoint.clusters?.powerConfiguration;if (powerCluster?.configureReporting) {
  ...
  async _strategyIASEnroll() {
    this.log(' Strategy 5: IAS Zone Enrollment');
