...
class DataRecoveryManager {
  ...
  async _readBattery(endpoint) {
    ...
    if (data?.batteryPercentageRemaining != null) {
      ...
      // Courbe CR2032 non-linéaire multi-points (calibrée données réelles).
      const voltage = data.batteryVoltage / 10;
      let bat;
      const CURVE = [[3.0, 100], [2.9, 88], [2.8, 55], [2.7, 15], [2.6, 4], [2.5, 0], [2.4, 0]];
      bat = 0;
  ...
  /**
   * Alias for forceRecovery to prevent crashes in driver files calling triggerRecovery
   */
