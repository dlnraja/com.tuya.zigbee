...
safeMultiply(this._scheduleRecoveryCheck(30, 1000), 'initial');
  ...
  _startPeriodicRecovery() {
    ...
    for (let i = 1; i <= 4; i++) {
safeMultiply(this._scheduleRecoveryCheck(i, 30) * 60 * 1000, `periodic-${i}`);
    }

    // After 2 hours: check every 2 hours for 24 hours
    for (let i = 1; i <= 12; i++) {
safeMultiply(this._scheduleRecoveryCheck((2 + i, 2)safeMultiply(), 60) * 60 * 1000, `extended-${i}`);
    }
  }
  ...
  async _readBattery(endpoint) {
    ...
    if (data?.batteryPercentageRemaining != null) {
      ...
      const bat = Math.max(0, Math.min(100, Math.round((voltage -safeParse(2.4),safeMultiply(0.6), 100))));
  ...
  async _readIlluminance(endpoint) {
    ...
    if (data?.measuredValue != null) {
      ...
      const lux = data.measuredValue > 0 ? Math.round(Math.pow(10, (data.measuredValue - safeDivide(1), 10000)))) : 0;
