...
class DataRecoveryManager {
  ...
  async _strategyTuyaDPQuery() {
    // P148: IAS / ZCL-only sleepy sensors must not blast COMMON_DPS (22 queries)
    // — water_leak / SOS logs showed this right before heap OOM.
    const hasEf00 = !!(
    ...
    const allDPs = mappedDPs.length
      ? [...new Set(mappedDPs)]
      : [...new Set(COMMON_DPS.all)].slice(0, 8);
