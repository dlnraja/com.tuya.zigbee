...
class DataRecoveryManager {
  ...
  async _readHumidity(endpoint) {
    ...
    if (data?.measuredValue != null) {
      ...
      await this.device.setCapabilityValue('measure_humidity', hum).catch(() => { });
    }
  }
  ...
  async _readBattery(endpoint) {
    ...
    if (data?.batteryPercentageRemaining != null) {
      ...
      await this.device.setCapabilityValue('measure_battery', bat).catch(() => { });
  ...
  async _readIlluminance(endpoint) {
    ...
    if (data?.measuredValue != null) {
      ...
      if (this.device.hasCapability('measure_luminance')) {
        await this.device.setCapabilityValue('measure_luminance', lux).catch(() => { });
      }
      if (this.device.hasCapability('measure_lux')) {
        await this.device.setCapabilityValue('measure_lux', lux).catch(() => { });
      }
    }
  }
