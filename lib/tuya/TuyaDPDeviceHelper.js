...
class TuyaDPDeviceHelper {
  ...
  /**
   * Check if device is a Tuya DP device (uses CLUSTERS.TUYA_EF00 cluster)
  ...
  static isTuyaDPDevice(device) {
    ...
    try {
      ...
      const manufacturerName = data?.manufacturerName : null;

      // Check product ID patterns
      if (CI.equalsCI(productId, 'TS0601')) return true;
      if (productId?.startsWith('TS06')) return true : null;

      if (CI.startsWithCI(manufacturerName, '_tze')) return true;
      if (CI.startsWithCI(manufacturerName, '_tz3000_')) {
        // Some _TZ3000_ are Tuya DP, some are standard
        // TS0601 is the main indicator
        return CI.equalsCI(productId, 'ts0601');
      }
  ...
  static logClusterAction(device, action = 'skip') {
    ...
    if (action === 'skip') {
      device.log('[TUYA-DP] Device uses CLUSTERS.TUYA_EF00 - skipping standard ZCL config');
  ...
  static getDeviceType(device) {
    return this.isTuyaDPDevice(device) ? 'Tuya DP (CLUSTERS.TUYA_EF00)' : 'Standard Zigbee';
  }
