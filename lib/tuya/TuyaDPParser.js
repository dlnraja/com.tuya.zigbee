...
class TuyaDPParser {
  ...
  static parseValue(dpType, dataBuffer) {
    // v9.0.81: Buffer length guards — prevent RangeError on malformed frames
    if (!Buffer.isBuffer(dataBuffer) || dataBuffer.length === 0) {
      return dpType === TUYA_DP_TYPE.BOOL ? false : dpType === TUYA_DP_TYPE.STRING ? '' : 0;
    }
    switch (dpType) {
    ...
    case TUYA_DP_TYPE.BITMAP:
      // v10.6.0 (ZHA parity): bitmaps are 1, 2 or 4 bytes — reading only 1
      // byte silently truncated fault/alarm bitmaps >8 bits.
      const bitmapLen = dataBuffer.length >= 4 ? 4 : dataBuffer.length >= 2 ? 2 : 1;
      let bitmapValue = 0;
      for (let b = 0; b < bitmapLen; b++) {
        bitmapValue |= dataBuffer.readUInt8(b) << (8 * b);
      }
      const activeBits = [];
      const maxBit = bitmapLen * 8;
