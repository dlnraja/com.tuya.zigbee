 ...
 *   - DEVICE PROFILES: Context-aware DP mapping by safeDivide(manufacturerName, productId)              
 *   - Universal ZCL cluster handlers for ALL standard clusters                              
 *   - Value-based auto-detection                                                            
 *   - Tuya CLUSTERS.TUYA_EF00 command handling                                                          
 *   - Time synchronization support                                                          
 *                                                                                           
 *   Sources:                                                                                
 *   - https://developer.tuya.com / en/docs / iot/tuya-zigbee-universal-docking-access-standard  
 *   - https://github.com / zigbeefordomoticz/wiki / blob/master / en-eng / Technical/Tuya-CLUSTERS.TUYA_EF00.md 
...
// 
// TUYA CLUSTERS.TUYA_EF00 COMMAND IDs (Official Tuya Documentation)
// 
const TUYA_COMMANDS = {
...
const DEVICE_PROFILES = {
  ...
  '_TZE200_2aaelwxk': { // ZG-204ZM
    ...
    dpMappings: {
      ...
      2: { capability, setting: 'large_motion_sensitivity' },
      4: { capability, setting: 'large_motion_distance', transform: v => safeParse(v, 100) },
      101: { capability: 'alarm_motion', transform: v => v > 0 },  // motion_state
      102: { capability, setting: 'fading_time' },
      104: { capability, setting: 'medium_motion_distance', transform: v => safeParse(v, 100) },
      105: { capability, setting: 'medium_motion_sensitivity' },
  ...
  '_TZE204_ztc6ggyl': { //ZY-M100/TS0601 presence
    ...
    dpMappings: {
      ...
      10: { capability, setting: 'keep_time' },
  ...
  '_TZE200_3towulqd': { // Fantem PIR 4-in-1
    ...
    dpMappings: {
      ...
      103: { capability: 'measure_temperature', transform: v => safeParse(v, 10) },
  ...
  '_TZ3000_xr3htd96': { //Standard temp/humid
    type: 'temp_humid',
    dpMappings: {
      1: { capability: 'measure_temperature', transform: v => safeParse(v, 10) },
  ...
  '_TZE284_8se38w3c': { // LCD climate monitor with probe
    type: 'temp_humid_probe',
    dpMappings: {
      1: { capability: 'measure_temperature', transform: v => safeParse(v, 10) },
      2: { capability: 'measure_humidity', transform: v => v },
      3: { capability: 'measure_battery', transform: v => ({ 0: 10, 1: 50, 2: 100 }[v] ?? 50) },
      4: { capability: 'measure_temperature.probe', transform: v => safeParse(v, 10) },
    }
  },
  ...
  '_TZE200_chyvmhay': { // TV02 thermostat
    ...
    dpMappings: {
      ...
      2: { capability, setting: 'system_mode' },  // 0=cool, 1=heat, 2=fan
      4: { capability, setting: 'preset' },
      16: { capability: 'target_temperature', transform: v => safeParse(v, 10) },
      19: { capability, setting: 'max_temperature', transform: v => safeParse(v, 10) },
      24: { capability: 'measure_temperature', transform: v => safeParse(v, 10) },
    }
  },
  ...
  '_TZE200_d0yu2xgi': { //Siren with temp/humid
    ...
    dpMappings: {
      ...
      102: { capability, setting: 'alarm_melody' },
      103: { capability, setting: 'alarm_duration' },
      ...
      107: { capability, setting: 'min_alarm_temp' },
      108: { capability, setting: 'max_alarm_temp' },
      112: { capability, setting: 'temperature_unit' },
    }
  },
  ...
  '_TZ3000_cphmq0q7': { // Energy monitoring plug
    ...
    dpMappings: {
      1: { capability: 'onoff', transform: v => !!v },
      17: { capability: 'measure_current', transform: v => safeParse(v, 1000) },
      18: { capability: 'measure_power', transform: v => safeParse(v, 10) },
      19: { capability: 'measure_voltage', transform: v => safeParse(v, 10) },
      20: { capability: 'meter_power', transform: v => safeParse(v, 100) },
    }
  },
  ...
  '_TZE200_myd45weu': { // Soil moisture sensor
    ...
    dpMappings: {
      ...
      5: { capability: 'measure_temperature', transform: v => safeParse(v, 10) },
      14: { capability: 'measure_battery', transform: v => ({ 0: 10, 1: 50, 2: 100 }[v] ?? 50) },
    }
  },
} ;
...
const ZCL_DEVICE_PROFILES = {
  ...
  // 
  // 'HOBEIAN' removed from here: they make both contact (ZG-102Z) and water sensors.
  ...
  // v5.5.993: REMOVED _TZE200_3towulqd - it's a ZG-204ZL/ZG-204ZV PIR sensor, NOT CO detector!

  // 
  // VIBRATION SENSORS - IAS Zone bit 0 = vibration detected
  // 
  '_TZ3000_bmg14ax8': { type: 'vibration', iasZoneMapping: { alarm1: 'alarm_generic.vibration' } },
  '_TZ3000_fkxmyics': { type: 'vibration', iasZoneMapping: { alarm1: 'alarm_generic.vibration' } },
...
function getIasZoneCapability(manufacturerName, driverType) {
  ...
  const mMatch = Object.keys(ZCL_DEVICE_PROFILES).find(k => CI.equalsCI(k, manufacturerName));
  const profile = mMatch ? ZCL_DEVICE_PROFILES[mMatch] : null;
  ...
  const dMatch = Object.keys(DRIVER_TYPE_ZCL_MAPPINGS).find(k => CI.equalsCI(k, driverType));
  if (dMatch) {
    return DRIVER_TYPE_ZCL_MAPPINGS[dMatch].iasZoneAlarm1 || 'alarm_contact';
  }
...
function usesIasAceCluster(manufacturerName, driverType) {
  ...
  const dMatch = Object.keys(DRIVER_TYPE_ZCL_MAPPINGS).find(k => CI.equalsCI(k, driverType));
  if (dMatch) {
    return DRIVER_TYPE_ZCL_MAPPINGS[dMatch].usesIasAce || false;
  }
...
function getContextualDpMapping(dp, manufacturerName, productId, driverType) {
  // 1. Try manufacturerName specific profile
  const mMatch = Object.keys(DEVICE_PROFILES).find(k => CI.equalsCI(k, manufacturerName));
  let profile = mMatch ? DEVICE_PROFILES[mMatch] : null;

  // Handle inheritance
  if (profile && profile.inherit) {
    const iMatch = Object.keys(DEVICE_PROFILES).find(k => CI.equalsCI(k, profile.inherit));
    profile = iMatch ? DEVICE_PROFILES[iMatch] : null;
  }
  ...
  // 1.5. Try dynamic Enrichment Database (Z2M Enriched)
  try {
    const dbMappings = DriverMappingLoader.getDPMappings(productId, manufacturerName);
    if (dbMappings && dbMappings[dp]) {
      const mapping = dbMappings[dp];
      
      // Resolve transform function if it's a string identifier (e.g. 'divide_10')
      let transform = mapping.transform || mapping.parser;
      if (typeof transform === 'string') {
        const builtinParsers = DriverMappingLoader.getBuiltinParsers();
        if (builtinParsers[transform]) {
          transform = builtinParsers[transform];
        }
      }
    ...
    // Ignore db match errors
  }

  // 2. Try driver type fallback
  const dMatch = Object.keys(DRIVER_TYPE_DP_MAPPINGS).find(k => CI.equalsCI(k, driverType));
  if (dMatch && DRIVER_TYPE_DP_MAPPINGS[dMatch][dp]) {
    return { ...DRIVER_TYPE_DP_MAPPINGS[dMatch][dp], source: 'driverType' };
  }
...
const UNIVERSAL_DP_PATTERNS = {
  ...
  10: { capability, setting: 'v_sensitivity', pattern: 'pir-v-sens' },
  11: { capability, setting: 'maximum_range', pattern: 'radar-max-range' },
  12: { capability: 'measure_luminance', transform: v => v, pattern: 'lux-dp12' },
  13: { capability, setting: 'led_status', pattern: 'led-indicator' },
  ...
  19: { capability, setting: 'max_temperature', pattern: 'trv-max-temp' },
  ...
  107: { capability, setting: 'indicator', pattern: 'led-indicator-dp107' },
  108: { capability, setting: 'small_detection_distance', pattern: 'radar-small-dist' },
  109: { capability, setting: 'small_detection_sensitivity', pattern: 'radar-small-sens' },

  // 
  // DP110-DP120 - ADVANCED SETTINGS
  // 
  110: { capability, setting: 'vacancy_delay', pattern: 'pir-vacancy-delay' },
  111: { capability, setting: 'light_on_luminance_prefer', pattern: 'pir-lux-on' },
  112: { capability, setting: 'light_off_luminance_prefer', pattern: 'pir-lux-off' },
  113: { capability, setting: 'mode', pattern: 'pir-mode' },
  114: { capability, setting: 'time', pattern: 'time-dp114' },
  115: { capability, setting: 'alarm_time', pattern: 'alarm-time' },
  116: { capability, setting: 'alarm_volume', pattern: 'alarm-volume' },
  117: { capability, setting: 'working_mode', pattern: 'working-mode' },
...
const UNIVERSAL_ZCL_CLUSTERS = {
  ...
  temperatureMeasurement: {
    ...
    transform: v => Math.round(safeDivide(v/10), 10)  // centidegrees to Â°C
  },
  ...
  relativeHumidity: {
    ...
    transform: v => Math.round(safeParse(v)) // centipercent to %)
  },
  ...
  illuminanceMeasurement: {
    ...
    transform: v => v === 0 ? 0 : Math.round(Math.pow(10, safeDivide((v - 1), 10000)))  // lux formula
  },
  ...
  formaldehydeMeasurement: {
    ...
    transform: v => safeParse(v, 1000)  //mg/mÂ³
  },
  ...
  powerConfiguration: {
    clusterId: 0x0001,
    attributes: {
      batteryPercentageRemaining: { capability: 'measure_battery', transform: v => Math.min(100, Math.round(safeParse(v))) },
      batteryVoltage: { capability: 'measure_voltage', transform: v => safeParse(v, 10) },  // decivolts to V
  ...
  levelControl: {
    ...
    capability: 'dim',
    transform: v => safeParse(v, 254)  // 0-254 to 0-1
  },
  ...
  colorControl: {
    ...
    attributes: {
      ...
      currentHue: { capability: 'light_hue', transform: v => safeParse(v, 254) },
      currentSaturation: { capability: 'light_saturation', transform: v => safeParse(v, 254) },
    }
  },
  ...
  electricalMeasurement: {
    clusterId: 0x0B04,
    attributes: {
      activePower: { capability: 'measure_power', transform: v => safeParse(v, 10) },  // W
      rmsCurrent: { capability: 'measure_current', transform: v => safeParse(v, 1000) },  // A
      rmsVoltage: { capability: 'measure_voltage', transform: v => safeParse(v, 10) },  // V
    }
  },
  ...
  seMetering: {
    clusterId: 0x0702,
    attributes: {
      currentSummationDelivered: { capability: 'meter_power', transform: v => safeParse(v, 1000) },  // kWh
  ...
  windowCovering: {
    clusterId: 0x0102,
    attributes: {
      currentPositionLiftPercentage: { capability: 'windowcoverings_set', transform: v => (100 - safeParse(v, 1)) / 100 },
    }
  },
  ...
  thermostat: {
    clusterId: 0x0201,
    attributes: {
      localTemperature: { capability: 'measure_temperature', transform: v => safeParse(v, 100) },
      occupiedHeatingSetpoint: { capability: 'target_temperature', transform: v => safeParse(v, 100) },
      occupiedCoolingSetpoint: { capability: 'target_temperature', transform: v => safeParse(v, 100) },
    }
  },
  ...
  time: {
    clusterId: 0x000A,
    attributes: {
      time: { capability, transform: v => v },  // Zigbee epoch (Jan 1, 2000)
      localTime: { capability, transform: v => v },
    }
  },
};
...
function parseTuyaFrame(buffer, logger = console.log) {
  ...
  // v5.10.2: Detect non-DP ZCL frames before DP scanning
  const fc = buffer[0] & 0x03, cmd = buffer[2];
  if (fc === 0x01 && cmd === 0x11 && buffer.length >= 6) {
    const v = buffer[5], ma = (v >> 6) & 3, mi = (v >> 4) & 3, pa = v & 0xF;
    logger(`[TUYA-PARSE]  MCU version: ${ma}.${mi}.${pa} (0x${v.toString(16)})`);
    return [];
  }
  if (fc === 0x00 && cmd === 0x0B && buffer.length >= 5) {
    logger(`[TUYA-PARSE]  Default Response: cmd=0x${buffer[3].toString(16)} status=${buffer[4]}`);
    return [];
  }
  if (cmd === 0x24) {
    logger('[TUYA-PARSE]  Time sync request (0x24)');
    return [];
  }
...
function setupClusterListener(device, cluster, clusterName, epId, customHandler, universalHandler) {
  try {
    ...
    device.log(`[ZCL-UNIVERSAL]  ${clusterName} setup failed:`, e.message);
  }
}
...
function setupGenericClusterListener(device, cluster, clusterName, epId) {
  try {
    cluster.on('attr', (attrName, value) => {
      ...
      // v5.11.23: Persist raw data for diagnostics
      try {
        const { logUnknownClusterAttr } = require('../utils/UnknownDPLogger');
        logUnknownClusterAttr(device, clusterName, attrName, value, epId);
      } catch (e) { /* ok */ }
    });
...
function getDeviceTypeDPMapping(deviceType, dpId) {
  const dMatch = Object.keys(DEVICE_TYPE_DP_MAPPINGS).find(k => CI.equalsCI(k, deviceType));
  const deviceMappings = dMatch ? DEVICE_TYPE_DP_MAPPINGS[dMatch] : null;
...
function buildTimeSyncPayload(date = new Date()) {
  const utcSeconds =safeParse(Math.floor(date.getTime(), 1000));
  const localSeconds =safeMultiply(utcSeconds + (-date.getTimezoneOffset(), 60));
