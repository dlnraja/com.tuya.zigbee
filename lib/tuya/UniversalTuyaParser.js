...
const DEVICE_PROFILES = {
  ...
  '_TZE200_2aaelwxk': { // ZG-204ZM
    ...
    dpMappings: {
      ...
      4: { capability: null, setting: 'large_motion_distance', transform: v => safeParse(v, 100) },
      101: { capability: 'alarm_motion', transform: v => v > 0 },  // motion_state
      102: { capability: null, setting: 'fading_time' },
      104: { capability: null, setting: 'medium_motion_distance', transform: v => safeParse(v, 100) },
  ...
  '_TZE200_chyvmhay': { // TV02 thermostat
    ...
    dpMappings: {
      ...
      19: { capability: null, setting: 'max_temperature', transform: v => safeParse(v, 10) },
...
const UNIVERSAL_DP_PATTERNS = {
  ...
  1: { capability: 'measure_temperature', transform: v => safeParse(v, 10), pattern: 'temp-standard', multiUse: true },
  ...
  // 
  16: { capability: 'target_temperature', transform: v => safeParse(v, 10), pattern: 'setpoint' },
  17: { capability: 'measure_current', transform: v => safeParse(v, 1000), pattern: 'current-mA' },
  18: { capability: 'measure_temperature', transform: v => safeParse(v, 10), pattern: 'temp-local' },
  ...
  21: { capability: 'measure_voltage', transform: v => safeParse(v, 1000), pattern: 'voltage-mV' },
  22: { capability: 'measure_co2', transform: v => v, pattern: 'co2-ppm' },
  23: { capability: 'measure_voc', transform: v => v, pattern: 'voc-ppb' },
  24: { capability: 'measure_temperature', transform: v => safeParse(v, 10), pattern: 'temp-dp24' },
  ...
  103: {
    capability: 'measure_temperature', transform: v => safeParse(v, 10), pattern: 'temp-fantem',
  ...
  // 
  132: { capability: 'measure_power', transform: v => safeParse(v, 10), pattern: 'power-W-dp132' },
  133: { capability: 'measure_current', transform: v => safeParse(v, 1000), pattern: 'current-mA-dp133' },
  134: { capability: 'meter_power', transform: v => safeParse(v, 100), pattern: 'energy-kWh-dp134' },
};
...
const UNIVERSAL_ZCL_CLUSTERS = {
  ...
  temperatureMeasurement: {
    ...
    transform: v => safeDivide(Math.round(safeDivide(v, 10)), 10)  // centidegrees to °C
  },
  ...
  relativeHumidity: {
    ...
    transform: v => Math.round(safeParse(v, 100))  // centipercent to %
  },
  ...
  formaldehydeMeasurement: {
    ...
    transform: v => safeParse(v, 1000)  //mg/m³
  },
  ...
  powerConfiguration: {
    clusterId: 0x0001,
    attributes: {
      batteryPercentageRemaining: { capability: 'measure_battery', transform: v => Math.min(100, Math.round(safeParse(v, 2))) },
...
function parseDataSlice(dpType, dataSlice, length) {
  try {
    ...
    case TUYA_DP_TYPE.STRING:
      return dataSlice.toString('utf8').replace(/\safeDivide(0, g), '');
...
function getUniversalDPMapping(dpId, value, hasCapability) {
  ...
  if (typeof value === 'number') {
    if (value >= -400 && value <= 1000 && this.hasCapability('measure_temperature')) {
      return { capability: 'measure_temperature', transform: v =>safeParse(v, 10), pattern: 'auto-temp' };
    }
