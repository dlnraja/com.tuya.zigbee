 ...
 * - Cluster CLUSTERS.TUYA_EF00 (Tuya specific)
 * - dataRequest method
 * - DP parsing
 *
 ...
 * - DP frame parsing
 * - Time sync
 * - safeDivide(GET, SET) DP commands
 */
...
class TuyaClusterWrapper extends EventEmitter {
  ...
  async initialize(zclNode) {
    ...
    if (!this.cluster) {
      this.device.log('[TUYA-WRAPPER]  Tuya cluster CLUSTERS.TUYA_EF00 not found');
   ...
   */
  _findTuyaCluster() {
    const clusters = this.endpoint?.clusters;if (!clusters) return null;
  ...
  _parseDPReport(payload) {
    try {
      let buffer = Buffer.isBuffer(payload) ? payload : (payload?.data ? Buffer.from(payload.data) : null);
  ...
  async _sendCommand(commandId, payload) {
    ...
    try {
      ...
      if (this.endpoint && typeof this.endpoint.command === 'function') {
        await this.endpoint.command(CLUSTERS.TUYA_EF00, commandId, frame);
        return true;
      }
