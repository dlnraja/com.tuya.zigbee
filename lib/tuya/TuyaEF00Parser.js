  ...
  parseFrame(commandId, payload) {
    try {
      ...
      case DP_TYPE.BOOL:
        ...
        break;
      case DP_TYPE.VALUE:
        value = data.length >= 4 ? data.readInt32BE(0) : data.length >= 2 ? data.readInt16BE(0) : data.length > 0 ? data[0] : 0;
        break;
      case DP_TYPE.STRING:
      ...
      case DP_TYPE.ENUM:
        ...
        break;
      case DP_TYPE.BITMAP:
        value = data.length >= 4 ? data.readUInt32BE(0) : data.length >= 2 ? data.readUInt16BE(0) : data.length > 0 ? data[0] : 0;
        break;
      case DP_TYPE.RAW:
      default:
        value = data;
