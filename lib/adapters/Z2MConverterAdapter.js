...
function generateDriverId(deviceType, caps, z2mDev) {
  ...
  const rawId = prefix.replace(/[^a-z0-9_]/g, '_');
  return sanitizeDriverId(rawId);
}

function sanitizeDriverId(driverId) {
  ...
  let clean = uniqueParts.join('_');
  
  // Avoid common hybrid duplicate patterns
  clean = clean
    .replace('switch_wall_switch', 'switch_wall')
    .replace('dimmer_wall_dimmer', 'dimmer_wall')
    .replace('sensor_climate_sensor', 'climate_sensor')
    .replace('presence_sensor_presence', 'presence_sensor')
    .replace('motion_sensor_motion', 'motion_sensor');
...
function generateDeviceJS(config) {
  ...
  // Generate functional working capability registrations instead of placeholders
  const registrations = config.capabilities.map(c => {
    if (c === 'onoff') {return '    await this.registerOnOffCapability();';}
    if (c === 'measure_temperature') {return '    await this.registerTemperatureCapability();';}
    if (c === 'measure_humidity') {return '    await this.registerHumidityCapability();';}
    if (c === 'measure_luminance') {return '    await this.registerLuminanceCapability();';}
    if (c === 'measure_battery') {return '    await this.registerBatteryCapability();';}
    return `    // Auto-scaffolded listener fallback for ${c}\n    await this.safeRegisterCapability('${c}');`;
  }).join('\n');
...
function loadExistingDrivers(driversDir) {
  ...
  for (const d of fs.readdirSync(driversDir)) {
    ...
    try {
      const config = JSON.parse(fs.readFileSync(cf));
      drivers.push({ id: d, ...config });
    } catch { /* malformed compose JSON — skip driver */ }
  }
  return drivers;
}
