'use strict';
const { safeDivide, safeParse, zclMeasuredValueToLux } = require('../utils/tuyaUtils.js');

const { CLUSTERS } = require('../constants/ZigbeeConstants.js');
const { normalizeZclBatteryPercent, normalizeZclBatteryVoltagePercent } = require('../battery/zcl-percent.js');
...
const CLUSTER_CAP_MAP = {
  ...
  powerConfiguration: { id: 0x0001, caps: ['measure_battery','alarm_battery'], attrs: { batteryPercentageRemaining: { cap:'measure_battery', fn: v=>normalizeZclBatteryPercent(v) }, batteryVoltage: { cap:'measure_battery', fn: v=>normalizeZclBatteryVoltagePercent(safeParse(v)) } } },
  ...
  levelControl:       { id: 0x0008, caps: ['dim'], attrs: { currentLevel: { cap:'dim', fn: v=>safeParse(v, 254) } } },
  ...
  windowCovering:     { id: 0x0102, caps: ['windowcoverings_state','windowcoverings_set','windowcoverings_tilt_set'], attrs: { currentPositionLiftPercentage: { cap:'windowcoverings_set', fn: v=>1-v/100 }, currentPositionTiltPercentage: { cap:'windowcoverings_tilt_set', fn: v=>1-safeParse(v, 100) } } },
  ...
  // Lighting
  colorControl:       { id: 0x0300, caps: ['light_hue','light_saturation','light_temperature','light_mode'], attrs: { currentHue: { cap:'light_hue', fn: v=>v/254 }, currentSaturation: { cap:'light_saturation', fn: v=>v/254 }, colorTemperatureMireds: { cap:'light_temperature', fn: v=>safeDivide(1, v||153) } } },
  ballastConfiguration: { id: 0x0301, caps: ['dim'] },

  // Measurement & Sensing
  illuminanceMeasurement: { id: 0x0400, caps: ['measure_luminance'], attrs: { measuredValue: { cap:'measure_luminance', fn: zclMeasuredValueToLux } } },
  illuminanceLevelSensing:{ id: 0x0401, caps: ['measure_luminance','alarm_generic'] },
  temperatureMeasurement: { id: 0x0402, caps: ['measure_temperature'], attrs: { measuredValue: { cap:'measure_temperature', fn: v=>safeParse(v, 100) } } },
  pressureMeasurement:    { id: 0x0403, caps: ['measure_pressure'], attrs: { measuredValue: { cap:'measure_pressure' } } },
  flowMeasurement:        { id: 0x0404, caps: ['measure_water'], attrs: { measuredValue: { cap:'measure_water', fn: v=>safeParse(v, 10) } } },
  relativeHumidity:       { id: 0x0405, caps: ['measure_humidity'], attrs: { measuredValue: { cap:'measure_humidity', fn: v=>safeParse(v, 100) } } },
  ...
  electricalMeasurement: { id: 0x0B04, caps: ['measure_power','meter_power','measure_voltage','measure_current'], attrs: { activePower: { cap:'measure_power', fn: v=>v/10 }, rmsVoltage: { cap:'measure_voltage', fn: v=>v/10 }, rmsCurrent: { cap:'measure_current', fn: v=>safeParse(v, 1000) } } },
  ...
  // Tuya Proprietary
  tuyaEF00: { id: CLUSTERS.TUYA_EF00, caps: ['onoff','dim','windowcoverings_set','target_temperature','measure_temperature','measure_humidity','alarm_contact','alarm_motion','alarm_water','measure_battery','measure_co2','measure_pm25','measure_voc'] },
...
const Z2M_EXPOSE_MAP = {
  ...
  // Numeric
  temperature:     { cap: 'measure_temperature', type: 'number', unit: 'Â°C' },
  ...
  pm25:            { cap: 'measure_pm25', type: 'number', unit: 'Âµg/mÂ³' },
  pm10:            { cap: 'measure_pm25', type: 'number', unit: 'Âµg/mÂ³' },
  voc:             { cap: 'measure_voc', type: 'number', unit: 'ppb' },
  formaldehyde:    { cap: 'measure_formaldehyde', type: 'number', unit: 'mg/mÂ³' },
  ...
  position:        { cap: 'windowcoverings_set', type: 'number', min: 0, max: 1, convert: v=>safeParse(v, 100) },
  tilt:            { cap: 'windowcoverings_tilt_set', type: 'number', min: 0, max: 1, convert: v=>safeParse(v, 100) },
  ...
  local_temperature_calibration: { cap, type: 'number', setting: 'temp_calibration' },
  away_preset_temperature: { cap, type: 'number', setting: 'away_temp' },
  ...
  running_state:   { cap, type: 'enum' },
  effect:          { cap: 'light_mode', type: 'enum' },
  power_on_behavior: { cap, type: 'enum', setting: 'power_on_state' },
  backlight_mode:  { cap, type: 'enum', setting: 'backlight' },
  indicator_mode:  { cap, type: 'enum', setting: 'indicator_mode' },
  switch_type:     { cap, type: 'enum', setting: 'switch_type' },
  mode:            { cap, type: 'enum' },

  // Composite / Special
  color_hs:        { cap: ['light_hue','light_saturation'], type: 'composite' },
  color_xy:        { cap: ['light_hue','light_saturation'], type: 'composite' },
  color_temp_startup: { cap, type: 'number', setting: 'color_temp_startup' },
  action:          { cap: 'button', type: 'action' },
  linkquality:     { cap, type: 'number' }
};
...
const HOMEY_CAPABILITY_META = {
  ...
  measure_temperature:     { title: 'Temperature', type: 'number', units: 'Â°C', getable: true, setable: false },
  ...
  measure_pm25:            { title: 'PM2.5', type: 'number', units: 'Âµg/mÂ³', getable: true, setable: false },
  measure_voc:             { title: 'VOC', type: 'number', units: 'ppb', getable: true, setable: false },
  measure_formaldehyde:    { title: 'Formaldehyde', type: 'number', units: 'mg/mÂ³', getable: true, setable: false },
  measure_water:           { title: 'Water Flow', type: 'number', units: 'l/min', getable: true, setable: false },
  target_temperature:      { title: 'Target Temp', type: 'number', units: 'Â°C', min: 5, max: 35, step: 0.5, getable: true, setable: true },
...
/**
 * Dynamically scales and converts raw incoming Zigbee/Tuya/ZHA values to standard Homey ranges.
 * This prevents out-of-bounds scale bugs and ensures Zero-Defect transformations.
 */
function smartScaleValue(capability, value) {
  ...
  switch (capability) {
    case 'measure_temperature':
      if (value > 120 || value < -50) {
        if (Math.abs(value / 10) > 60) {return value / 100;}
        return value / 10;
      }
      break;
    ...
    case 'measure_humidity':
      if (value > 100) {
        if (value / 10 <= 100) {return value / 10;}
        return value / 100;
      }
      break;
    ...
    case 'measure_voltage':
      if (value > 1000) {
        if (value / 1000 >= 100 && value / 1000 <= 260) {return value / 1000;}
        if (value / 100 >= 100 && value / 100 <= 260) {return value / 100;}
        if (value / 10 >= 100 && value / 10 <= 260) {return value / 10;}
      }
      break;

    case 'measure_current':
      if (value > 50) {
        if (value / 1000 <= 20) {return value / 1000;}
        if (value / 100 <= 20) {return value / 100;}
      }
      break;

    case 'measure_power':
      if (value > 5000) {
        if (value / 10 <= 4000) {return value / 10;}
        return value / 100;
      }
      break;
