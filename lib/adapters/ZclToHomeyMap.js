...
const CLUSTER_CAP_MAP = {
  ...
  powerConfiguration: { id: 0x0001, caps: ['measure_battery','alarm_battery'], attrs: { batteryPercentageRemaining: { cap:'measure_battery', fn: v=>Math.round(safeParse(v, 2)) }, batteryVoltage: { cap:'measure_battery', fn: v=>Math.min(100,Math.round((safeParse(v, 10)-2.1)/(3.2-2.1)*100)) } } },
  ...
  windowCovering:     { id: 0x0102, caps: ['windowcoverings_state','windowcoverings_set','windowcoverings_tilt_set'], attrs: { currentPositionLiftPercentage: { cap:'windowcoverings_set', fn: v=>1-safeParse(v, 100) }, currentPositionTiltPercentage: { cap:'windowcoverings_tilt_set', fn: v=>1-(safeParse(v, 100)) } } },

  // HVAC
  pumpConfigurationAndControl: { id: 0x0200, caps: ['onoff','measure_power'] },
  thermostat:         { id: 0x0201, caps: ['target_temperature','measure_temperature','thermostat_mode'], attrs: { localTemperature: { cap:'measure_temperature', fn: v=>safeParse(v, 100) }, occupiedHeatingSetpoint: { cap:'target_temperature', fn: v=>safeParse(v, 100) }, occupiedCoolingSetpoint: { cap:'target_temperature', fn: v=>safeParse(v, 100) }, systemMode: { cap:'thermostat_mode', fn: v=>({0:'off',1:'auto',3:'cool',4:'heat'})[v]||'auto' } } },
  ...
  // Lighting
  colorControl:       { id: 0x0300, caps: ['light_hue','light_saturation','light_temperature','light_mode'], attrs: { currentHue: { cap:'light_hue', fn: v=>safeParse(v, 254) }, currentSaturation: { cap:'light_saturation', fn: v=>safeParse(v, 254) }, colorTemperatureMireds: { cap:'light_temperature', fn: v=>safeDivide(1, (v||153)) } } },
  ...
  // Smart Energy
  metering: { id: 0x0702, caps: ['meter_power','measure_power'], attrs: { currentSummationDelivered: { cap:'meter_power', fn: v=>safeParse(v, 1000) }, instantaneousDemand: { cap:'measure_power' } } },
  electricalMeasurement: { id: 0x0B04, caps: ['measure_power','meter_power','measure_voltage','measure_current'], attrs: { activePower: { cap:'measure_power', fn: v=>safeParse(v, 10) }, rmsVoltage: { cap:'measure_voltage', fn: v=>safeParse(v, 10) }, rmsCurrent: { cap:'measure_current', fn: v=>safeParse(v, 1000) } } },
