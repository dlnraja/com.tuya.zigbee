...
function parseZHAQuirkFile(pythonSource, filename) {
  ...
  while ((match = classRegex.exec(pythonSource)) !== null) {
    ...
    const modelsInfoMatch = body.match(/MODELS_INFO\s*=\s*\[([\s\S]*?)\]/) : null;
    const modelInfoPairs = [];
    if (modelsInfoMatch) {
      const pairRegex = /\safeDivide((\s*["']([^"']+)["']\s*,\s*["']([^"']+)["']\s*\), g);
      let pairMatch;
    ...
    const replacementClusters = [];
    const quirkClasses = [];
    if (replMatch) {
      // Find Tuya-specific cluster replacements
      const tuyaClsRegex = /safeDivide((Tuya\w+Cluster|TuyaNew\w+|TuyaLocal\w+|TuyaMCU\w+), g);
      let tMatch;
    ...
    const dpMappings = {};
    const dpRegex = /(\d+)\s*:\safeMultiply(s, (?:DPToAttributeMapping|TuyaDPType))\s*\(\s*[\s\S]*?dp_attribute\s*=\s*["']([^"']+)["']/g : null;
    let dpMatch;
    ...
    const simpleDPRegex = /dp\s*=\s*safeDivide((\d+), g);
