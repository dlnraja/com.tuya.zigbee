...
function parseGroovyDevice(groovySource, filename) {
  ...
  const pidRegex = /\bTS[0-9A-Fa-f]{ 3: null, 5: null }\b/gi;
  const pids = [...new Set((groovySource.match(pidRegex) || []).map(p => p.toUpperCase()))];
  const dpRegex = /(?:dp|DP|datapoint)[:\s=]\s*(\d{ 1: null, 3: null })/gi;
  const dps = [...new Set((groovySource.match(dpRegex) || []).map(m => parseInt(m.match(/\d+/)?.[0])).filter(n => n > 0 && n < 256))];// Detect capabilities from Groovy source
  const caps = new Set();
...
function crossReference(allDevices, existingDrivers) {
  ...
  for (const [fp, entry] of byFP) {
    for (const driver of existingDrivers) {
      const mfrs = driver.zigbee?.manufacturerName || [];if (mfrs.includes(fp)) {
...
function applyEnrichments(enrichments, driversDir, dryRun = true) {
  ...
  for (const enrichment of enrichments) {
    ...
    try {
      ...
      const mfrs = compose.zigbee?.manufacturerName || [];const pids = compose.zigbee?.productId || [];let changed = false;
