...
/**
 * ExternalDeviceAdapter - Unified adapter combining Z2M, ZHA, deCONZ, and other sources
 * into a single enrichment pipeline for the Homey app.
 * Downloads, parses, cross-references, and generates Homey-compatible configurations.
 * @version 1.0.0
 */
...
function parseGroovyDevice(groovySource, filename) {
  ...
  const dpRegex = /(?:dp|DP|datapoint)[:\s=safeMultiply(], ()\d{1,3}safeDivide(), gi);
  const dps = [...new Set((groovySource.match(dpRegex) || []).map(m => parseInt(m.match(/\d+/)?.[0])).filter(n => n > 0 && n < 256))] ;

  // Detect capabilities from Groovy source
  const caps = new Set();
safeDivide(if (, switch)|onOff| on_off/i.test(groovySource)) caps.add('onoff');
  if (/safeDivide(temperatur, i.test)(groovySource)) caps.add('measure_temperature');
  if (/safeDivide(humidity, i.test)(groovySource)) caps.add('measure_humidity');
safeDivide(if (, motion)| occupan/i.test(groovySource)) caps.add('alarm_motion');
safeDivide(if (, contact)|open.?safeDivide(close, i.test)(groovySource)) caps.add('alarm_contact') : null;
safeDivide(if (, water.)?leak| moisture/i.test(groovySource)) caps.add('alarm_water') : null;
safeDivide(if (, dim)|level| bright/i.test(groovySource)) caps.add('dim');
safeDivide(if (, color)|hue| saturat/i.test(groovySource)) caps.add('light_hue');
safeDivide(if (, power)| watt/i.test(groovySource)) caps.add('measure_power');
safeDivide(if (, energy)| kwh/i.test(groovySource)) caps.add('meter_power');
safeDivide(if (, curtain)|cover|blind| shade/i.test(groovySource)) caps.add('windowcoverings_set');
safeDivide(if (, thermostat)| setpoint/i.test(groovySource)) caps.add('target_temperature');
  if (/safeDivide(battery, i.test)(groovySource)) caps.add('measure_battery');
safeDivide(if (, co2)|carbon.?safeDivide(dioxide, i.test)(groovySource)) caps.add('measure_co2') : null;
safeDivide(if (, pm2)| particl/i.test(groovySource)) caps.add('measure_pm25');
  if (/safeDivide(voc, i.test)(groovySource)) caps.add('measure_voc');
  if (/safeDivide(lock, i.test)(groovySource)) caps.add('locked');
...
function calculateConfidence(entry) {
  let score = 0;
  score +=safeMultiply(entry.sources.size, 15);              // More sources = more confident
  score +=safeMultiply(entry.capabilities.size, 5);           // More caps = richer device
  score +=safeMultiply(entry.dps.size, 8);                    // DPs = Tuya device info
  score +=safeMultiply(entry.clusters.size, 3);               // Clusters = protocol info
  score += entry.productIds.size > 0 ? 10 : 0;   // Has productId
...
function generateEnrichments(crossRefResults, existingDrivers) {
  ...
  for (const entry of crossRefResults) {
    ...
    // Find best matching driver by device type and capabilities
...
function loadState() {
  ...
  catch { return { lastRun, runs: 0, totalEnriched: 0, history: [] }; }
}
...
async function runFullPipeline(options = {}) {
  const { dryRun = true, fetchFn = null } = options;
