...
class EnhancedDPHandler {
  ...
  async registerDatapointListener() {
    ...
    this.device.log('[OK] Datapoint listener registered for push updates (no polling)');
  }
  ...
  convertValue4byte(value, config) {
    ...
    // Apply conditional scale
    if (config.conditionalScale && numValue > config.conditionalScale.threshold) {
      numValue = numValue / config.conditionalScale.divisor;
    }
