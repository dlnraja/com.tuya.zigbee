...
class AutonomousIntelligenceGate {
  ...
  process(dp, value) {
    ...
    if (isStuck && dp === 1) {
      this.device.log(`[INTEL-GATE] ⚠️ DP1 STUCK PATTERN DETECTED - Increasing heuristic weight for distance DPs`);
      this._stuckMode = true;
    }

    // If confidence is significantly high for a different mapping, flag it
    const currentConfidence = this.confidenceMap[dp]?.[analysis.capability] || 0;
    if (currentConfidence > 0.85 && analysis.capability !== this._getCurrentMapping(dp)) {
  ...
  _updateConfidence(dp, analysis) {
    ...
    const current = this.confidenceMap[dp][analysis.capability] || 0.5; // Start at midpoint
    // Exponential moving average for score
    this.confidenceMap[dp][analysis.capability] = (current * 0.7) + (analysis.score * 0.3);
  }
  ...
  setStuckMode(stuck) {
    ...
    if (stuck) {
      this.device.log('[INTEL-GATE] ⚠️ Manual Stuck Mode enabled - Inference reliability prioritized');
    }
  }
