...
// CLUSTERS.TUYA_EF00-0xEFFF (Tuya) on ALL EPs + 0xFC00-0xFFFF (manufacturer-specific)
...
class CustomClusterManager {
  ...
  _resolveId(name, cluster) {
    ...
    const NAMED = {
      tuya: CLUSTERS.TUYA_EF00, tuyaSpecific: CLUSTERS.TUYA_EF00,
      manuSpecificTuya: CLUSTERS.TUYA_EF00, tuyaManufacturer: CLUSTERS.TUYA_EF00
    };
    return NAMED[name] || null;
  }

  _classify(id) {
    if (id === CLUSTERS.TUYA_EF00) return 'tuya_dp';
  ...
  _smartMap(attr,val){
    ...
    if(!match)return;
    const cap=match.cap;
    const v=match.div?safeDivide(val, match.div):val;
