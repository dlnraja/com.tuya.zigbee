...
class CustomClusterManager {
    ...
    if(type==='tuya_dp'&&epId!==1){
      ['dataReport','dataResponse','response','report','data','set'].forEach(ev=>{
        try{const h=d=>{this.device.emit&&this.device.emit('tuya.dp.alt',{endpoint:epId,event:ev,data:d});};
          cl.on(ev,h);this._listeners.push({cluster:cl,event:ev,handler:h});}catch(e){/* Alt event listener may not be supported */}});
    }
  ...
  _deferredRead(cl,cid,epId,type){
    (this.device.homey?.setTimeout ? this.device.homey.setTimeout.bind(this.device.homey) : globalThis.setTimeout)(async()=>{
      ...
      }catch(e){/* Deferred attribute read failed */}
    },10000);
  }
  ...
  destroy(){
    for(const{cluster:c,event:e,handler:h}of this._listeners){
      try{if(c.removeListener){c.removeListener(e,h);}}catch(x){/* Listener may already be removed */}
    }
