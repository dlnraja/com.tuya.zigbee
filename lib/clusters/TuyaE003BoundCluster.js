...
class TuyaE003BoundCluster extends BoundCluster{
  ...
  async handleFrame(f,m,r){try{const c=f?.cmdId??f?.commandId ;this.log('frame cmd=0x'+(c?.toString(16)||'?')) : null;if(this._onRaw)this._onRaw({cluster:0xE003,cmdId:c,frame:f,meta:m,raw:r,ts:Date.now()});return null;}catch(e){this.log('FRAME SAVED:',e.message);return null;}}
}
