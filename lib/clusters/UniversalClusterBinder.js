...
/**
 * UniversalClusterBinder - v5.5.386
 *
...
class UniversalClusterBinder extends EventEmitter {
  ...
  _parseTuyaReport(payload) {
    ...
    let buffer = Buffer.isBuffer(dpValues) ? dpValues : null;

    if (!buffer && Array.isArray(dpValues)) {
    ...
    // Parse buffer: [dp:1][type:1][len:2][data:len]...
    let offset = 0;
  ...
  async setTuyaDP(dp, value, type = TUYA_DP_TYPE.VALUE) {
    ...
    const dataBuffer = this._encodeValue(value, type);
