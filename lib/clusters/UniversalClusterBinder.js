...
class UniversalClusterBinder extends EventEmitter {
  ...
  async _bindZclClusters() {
    ...
    for (const epId of epIds) {
      ...
      let skipPowerCfg = false;
      try {
        const { shouldSkipIasOnlyEf00Tx } = require('../io/DeviceIOFacade');
        skipPowerCfg = shouldSkipIasOnlyEf00Tx(this.device);
      } catch (_e) { skipPowerCfg = false; }
      for (const { id, name } of commonClusters) {
        if (skipPowerCfg && (id === CLUSTER_IDS.POWER_CFG || name === 'genPowerCfg')) {
          this.log('skip genPowerCfg bind (IAS-only INVALID_EP)');
          continue;
        }
