...
const CLUSTER_IDS = {
  ...
  // v5.11.30: Tuya Manufacturer-Specific
  TUYA_E000: 0xE000,
  TUYA_E001: 0xE001,
  TUYA_E002: 0xE002,
  TUYA_E003: 0xE003,
...
class UniversalClusterBinder extends EventEmitter {
  ...
  async initialize(zclNode) {
    ...
    // v5.11.30: Bind manufacturer-specific clusters (E001/E002) for ALL device types
    await this._bindManufacturerClusters();
  ...
  async setTuyaDP(dp, value, type = TUYA_DP_TYPE.VALUE) {
    // P2699: non-native EF00 TX is complementary — soft-fail so PFC/raw can continue
    if (!this.tuyaCluster) {
      this.log?.('Tuya cluster not available — complementary soft-fail (P2699)');
      try {
        const { softArmComplementaryIo } = require('../io/NonNativeComplementary');
        softArmComplementaryIo(this.device || this, {});
      } catch (_e) { /* optional */ }
      return false;
    }
    ...
    const dataBuffer = Buffer.isBuffer(payload) ? payload : Buffer.from([payload]);
  ...
  async _bindZclClusters() {
    ...
    // v5.8.36: Scan ALL endpoints, not just EP1
    const endpoints = this.zclNode?.endpoints || {};
    const epIds = Object.keys(endpoints);
    if (epIds.length === 0) {
      this.log('⚠️ No endpoints available');
      return;
    }
    ...
    for (const epId of epIds) {
      ...
      for (const { id, name } of commonClusters) {
        const cluster = this._findCluster(ep, id) || ep.clusters[name];
        if (cluster && !this.zclClusters.has(id)) {
          this.zclClusters.set(id, cluster);
          await this._bindAndConfigure(cluster, id, name);
          if (parseInt(epId) !== 1) {
            this.log(`📋 Found ${name} on non-standard EP${epId}`);
          }
        }
      }
    }

    this.log(`✅ Bound ${this.zclClusters.size} ZCL clusters across ${epIds.length} endpoints`);
  }
  ...
  // v5.11.30: Bind E001/E002/E003 BoundClusters + Zosung IR
  // v5.12.x: consult protocol quirk table for expected exotic clusters (interview miss)
  async _bindManufacturerClusters() {
    const eps = this.zclNode?.endpoints || {};

    // Soft interview-miss aliasing before bind attempts
    try {
      const hcl = this.device?.homeyCompensation;
      if (hcl && typeof hcl.compensateInterviewMiss === 'function') {
        hcl.compensateInterviewMiss(this.zclNode);
      } else {
        const { loadProtocolQuirkTable } = require('../io/HomeyCompensationLayer');
        const table = loadProtocolQuirkTable();
        const pid = String(
          this.device?.getSetting?.('zb_model_id')
          || this.device?.getData?.()?.modelId
          || '',
        ).toUpperCase();
        const expected = table?.modelExpectedClusters?.[pid] || [];
        if (expected.length && this.device?.log) {
          this.device.log(`[UCB] quirk expected clusters for ${pid}: ${expected.map((c) => `0x${Number(c).toString(16)}`).join(',')}`);
        }
      }
    } catch (_e) { /* non-fatal */ }
    ...
    for (const epId of Object.keys(eps)) {
      ...
      const e001 = ep.clusters.tuyaE001 || ep.clusters[0xE001];
      if (e001) {
        try {
          const BC = require('./TuyaE001BoundCluster');
          ep.bind(CLUSTER_IDS.TUYA_E001, new BC({ device: this.device, onAttributeReport: (a) => this.emit('e001Attr', a) }));
          this.log(`✅ E001 BoundCluster EP${  epId}`);
        } catch (e) { this.log('⚠️ E001:', e.message); }
      }

      // 0xE002: Tuya Sensor Alarms
      const e002 = ep.clusters.tuyaE002 || ep.clusters[0xE002];
      if (e002) {
        try {
          const BC = require('./TuyaE002BoundCluster');
          ep.bind(CLUSTER_IDS.TUYA_E002, new BC({ device: this.device, onAttributeReport: (a) => this.emit('e002Attr', a) }));
          this.log(`✅ E002 BoundCluster EP${  epId}`);
        } catch (e) { this.log('⚠️ E002:', e.message); }
      }

      // 0xE003: Tuya Scene/Button Events
      const e003 = ep.clusters.tuyaE003 || ep.clusters[0xE003];
      if (e003) {
        try {
          const BC = require('./TuyaE003BoundCluster');
          ep.bind(CLUSTER_IDS.TUYA_E003, new BC({
            device: this.device,
            onSceneEvent: (e) => this.emit('e003Scene', e),
            onRawFrame: (f) => this.emit('e003Raw', f)
          }));
          this.log(`✅ E003 BoundCluster EP${  epId}`);
        } catch (e) { this.log('⚠️ E003:', e.message); }
      }

      // 0xE004: Zosung IR Control
      const e004 = ep.clusters.zosungIRControl || ep.clusters[0xE004];
      if (e004) {
        try {
          const BC = require('./ZosungIRControlBoundCluster');
          ep.bind(CLUSTER_IDS.ZOSUNG_IR_CONTROL, new BC({ device: this.device }));
          this.log(`✅ E004 (Zosung IR Ctrl) BoundCluster EP${  epId}`);
        } catch (e) { this.log('⚠️ E004:', e.message); }
      }

      // 0xED00: Zosung IR Transmit
      const ed00 = ep.clusters.zosungIRTransmit || ep.clusters[0xED00];
      if (ed00) {
        try {
          const BC = require('./ZosungIRTransmitBoundCluster');
          ep.bind(CLUSTER_IDS.ZOSUNG_IR_TRANSMIT, new BC({ device: this.device }));
          this.log(`✅ ED00 (Zosung IR Tx) BoundCluster EP${  epId}`);
        } catch (e) { this.log('⚠️ ED00:', e.message); }
      }
    }
  }
