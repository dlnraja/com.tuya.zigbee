...
class IasAceBoundCluster extends BoundCluster {
  ...
  arm(payload) {
    ...
    } catch (err) { /* Frame error handled silently */ }
  }
  ...
  // Homey/zigbee-clusters may dispatch by command name instead of id.
  commandEmergency(payload) { return this.emergency(payload); }
  ...
  commandFire(payload) { return this.fire(payload); }
  ...
  commandPanic(payload) { return this.panic(payload); }
  ...
  // WHY: the previous override returned early for cmd 0–9 and never called
  // super.handleFrame, so commandEmergency/panic never reached emergency().
  // Peter SOS tiles stayed silent while IAS Zone keep-alives were ignored.
  async handleFrame(f, m, r) {
    if (typeof super.handleFrame === 'function') {
      return super.handleFrame(f, m, r);
    }
    const c = f?.cmdId ?? f?.commandId;
    if (c === 2) {return this.emergency(m);}
    if (c === 3) {return this.fire(m);}
    if (c === 4) {return this.panic(m);}
    return undefined;
  }
}
...
try {
  ...
  // Cluster may already exist - silently ignore
}
