...
// v9.1.0: Removed cascading require('./TuyaE000Cluster')
// TuyaE000Cluster is now registered centrally by registerClusters.js at app startup
// The old require triggered double registration and noisy "already exists" errors
...
class TuyaE000BoundCluster extends BoundCluster {
  ...
  async _hc(id,data) { try { this.log(`cmd${id}`,data?.toString?.('hex')||'none'); await this.handleFrame({cmdId:id,data},null,data); } catch(err) { this.log(`⚠️ FRAME SAVED — cmd${id} error:`, err.message); } }
