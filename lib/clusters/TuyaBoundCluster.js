...
class TuyaBoundCluster extends BoundCluster {
  ...
  constructor({
    ...
    this._maxFrameLog = 100; // v5.14.0: Increased from 20 to 100 for better debugging
    // v5.14.0: Sequence number validation
    this._outgoingSequences = new Map(); // seq -> { timestamp, dpId, resolved }
    this._nextSeq = 1; // Next sequence number we will use
    this._maxOutstandingSeqs = 64; // Max tracked outstanding sequences
  }
  ...
  dataReport(payload) {
    try {
      ...
      // v5.14.0: Validate sequence number (spontaneous reports use seq=0)
      const seq = payload?.seq;
      if (typeof seq === 'number') {
        this.validateIncomingSequence(seq);
      }
  ...
  dataResponse(payload) {
    try {
      ...
      // v5.14.0: Validate sequence number against outstanding requests
      const seq = payload?.seq;
      if (typeof seq === 'number') {
        this.validateIncomingSequence(seq);
        this._cleanupSequences();
      }
  ...
  _rawDP(id,p){try{this.log(`📥 cmd0x${id.toString(16)}`);const d=p?.data||p?.dpValues||p;if(d&&this._onDataReport){try{this._onDataReport(this._parsePayload({dpValues:d}));}catch(e2){/* DP payload parse failed */}}if(this._onRawFrame){this._onRawFrame({cmdId:id,frame:p,timestamp:Date.now()});}}catch(e){this.log(`⚠️ FRAME SAVED — cmd0x${id.toString(16)}:`,e.message);}}
  ...
  async _handleFrameInner(frame, meta, rawFrame) {
    ...
    const knownCommands = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x20, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x41, 0x42, 0x43];
  ...
  /**
   * v5.14.0: Register an outgoing sequence number (called before sending a command)
   * @param {number} seq - Sequence number being sent
   * @param {string} context - Description of the request (e.g. 'getData DP1')
   */
  trackOutgoingSequence(seq, context = '') {
    this._outgoingSequences.set(seq, {
      timestamp: Date.now(),
      context,
      resolved: false,
    });
    // Evict oldest if too many outstanding
    if (this._outgoingSequences.size > this._maxOutstandingSeqs) {
      const oldestKey = this._outgoingSequences.keys().next().value;
      this._outgoingSequences.delete(oldestKey);
    }
  }

  /**
   * v5.14.0: Validate an incoming sequence number against outstanding requests
   * @param {number} seq - Sequence number from device response
   * @returns {{ valid: boolean, matched: boolean, context: string|null, age: number|null }}
   */
  validateIncomingSequence(seq) {
    if (typeof seq !== 'number' || seq === 0) {
      // Sequence 0 or non-numeric is typically a spontaneous report — allow it
      return { valid: true, matched: false, context: null, age: null };
    }

    const entry = this._outgoingSequences.get(seq);
    if (entry) {
      entry.resolved = true;
      const age = Date.now() - entry.timestamp;
      this.log(`✅ Sequence 0x${seq.toString(16).padStart(4, '0')} matched (${entry.context}, ${age}ms ago)`);
      return { valid: true, matched: true, context: entry.context, age };
    }

    // Not matched to an outgoing request — could be a spontaneous report or stale
    // Allow but warn if it looks like it should have matched
    this.log(`⚠️ Sequence 0x${seq.toString(16).padStart(4, '0')} not matched to outgoing request (spontaneous report?)`);
    return { valid: true, matched: false, context: null, age: null };
  }

  /**
   * v5.14.0: Generate the next sequence number for outgoing commands
   * @returns {number}
   */
  getNextSequence() {
    const seq = this._nextSeq;
    this._nextSeq = (this._nextSeq + 1) & 0xFFFF; // Wrap at 16-bit
    if (this._nextSeq === 0) {this._nextSeq = 1;} // Skip 0
    return seq;
  }

  /**
   * v5.14.0: Clean up stale outstanding sequences (older than 30s)
   */
  _cleanupSequences() {
    const now = Date.now();
    const maxAge = 30000;
    for (const [seq, entry] of this._outgoingSequences) {
      if (now - entry.timestamp > maxAge || entry.resolved) {
        this._outgoingSequences.delete(seq);
      }
    }
  }
