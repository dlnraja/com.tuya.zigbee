...
class TuyaBoundCluster extends BoundCluster {
  ...
  static get COMMANDS() {
    return {
      ...
      // v5.5.960: CRITICAL FIX - Capture the 2-byte sequence number from device request
      // Device sends: [seqHi:1][seqLo:1] (2 bytes)
      // Host must respond: [seqHi:1][seqLo:1][UTC:4 BE][Local:4 BE] (10 bytes)
      mcuSyncTime: {
        id: 0x24,
        args: {
          payloadSize: ZCLDataTypes.uint16,  // Payload size (usually 0 for request)
          payload: ZCLDataTypes.buffer,      // Raw payload with sequence number
        },
      },
    };
  }
