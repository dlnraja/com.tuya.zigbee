...
class TuyaSpecificCluster extends Cluster {
  ...
  static get COMMANDS() {
     ...
     *
     * CRITICAL: Command names MUST be 'response' and 'reporting'
     * NOT 'dataReport' and 'dataResponse' !
     *
     * The cluster.on('response') and cluster.on('reporting') events
     * are emitted based on these command names.
     *
     * Sources:
     * - Working version v5.5.46 of this app
    ...
    return {
       ...
       * Used to request current value of DPs
       */
      dataQuery: {
        id: 0x03,
        args: {
      ...
      // v5.5.46: These MUST have args to emit events when received
      // ═══════════════════════════════════════════════════════════════════

      /**
       * Response to getData (0x01) - device responds to our query
       * CRITICAL: Name MUST be 'response' for cluster.on('response') to work!
       */
      response: {
        id: 0x01,
        args: {
       ...
       * CRITICAL: Name MUST be 'reporting' for cluster.on('reporting') to work!
       */
      reporting: {
        id: 0x02,
        args: {
