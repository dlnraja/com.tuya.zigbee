...
const ZCL_AUTO_MAP = {
  ...
  // WHY(P2540): complementary ZCL air-quality alternates (ZHA/Z2M 0x040d / VOC / HCHO / PM2.5)
  'msCO2.measuredValue': { cap: 'measure_co2', div: 1 },
  'carbonDioxideMeasurement.measuredValue': { cap: 'measure_co2', div: 1 },
  'msCarbonDioxide.measuredValue': { cap: 'measure_co2', div: 1 },
  'pm25Measurement.measuredValue': { cap: 'measure_pm25', div: 1 },
  'msFineParticle.measuredValue': { cap: 'measure_pm25', div: 1 },
  'formaldehydeMeasurement.measuredValue': { cap: 'measure_formaldehyde', div: 100 },
  'msFormaldehyde.measuredValue': { cap: 'measure_formaldehyde', div: 100 },
  'vocMeasurement.measuredValue': { cap: 'measure_voc', div: 1 },
  'msVOC.measuredValue': { cap: 'measure_voc', div: 1 },
  'msVoc.measuredValue': { cap: 'measure_voc', div: 1 },
};
...
const CLUSTER_ATTRS = {
  ...
  // P2540 — listen+report for AQ ZCL alternates (was map-only → never read)
  'msCO2': ['measuredValue'],
  'carbonDioxideMeasurement': ['measuredValue'],
  'msCarbonDioxide': ['measuredValue'],
  'pm25Measurement': ['measuredValue'],
  'msFineParticle': ['measuredValue'],
  'formaldehydeMeasurement': ['measuredValue'],
  'msFormaldehyde': ['measuredValue'],
  'vocMeasurement': ['measuredValue'],
  'msVOC': ['measuredValue'],
  'msVoc': ['measuredValue'],
};
...
class RawClusterFallback {
  constructor(device) {
    ...
    this.discovered = [];
    this.timer = null;
    this._quirkExotic = null;
    this.device.log('[RAW-FALLBACK] Initialized (v5.12.1 - quirk-table aware)');
  }

  _loadExoticClusterHints() {
    if (this._quirkExotic) {return this._quirkExotic;}
    try {
      const { loadProtocolQuirkTable } = require('../io/HomeyCompensationLayer');
      const table = loadProtocolQuirkTable();
      this._quirkExotic = table?.exoticClusters || {};
    } catch (_e) {
      this._quirkExotic = {};
    }
    return this._quirkExotic;
  }

  async initialize(zclNode) {
    if (!zclNode || !zclNode.endpoints) {return;}

    // Soft: ask compensation layer to alias interview-miss keys first
    try {
      const hcl = this.device.homeyCompensation;
      if (hcl && typeof hcl.compensateInterviewMiss === 'function') {
        hcl.compensateInterviewMiss(zclNode);
      }
    } catch (_e) { /* non-fatal */ }
  ...
  _shouldSkip(name, cluster) {
    ...
    if (containsCI(name, 'tuya') || name === '0xef00') {return true;}

    // Quirk-table exotic mfr clusters — still attach, but mark for diagnostics
    try {
      const exotic = this._loadExoticClusterHints();
      const upper = String(name).toUpperCase();
      if (exotic[upper] || exotic[`0x${upper.replace(/^0X/, '')}`]) {
        this.device.log?.(`[RAW-FALLBACK] exotic quirk cluster: ${name}`);
      }
    } catch (_e) { /* noop */ }
  ...
  _deferClusterRead(cluster, name, epId) {
    (this.device.homey?.setTimeout ? this.device.homey.setTimeout.bind(this.device.homey) : globalThis.setTimeout)(async () => {
  ...
  _tryAutoMap(lookupKey, value) {
    ...
    // v9.0.79 / P2540: L14 + fusion source tag when available
    if (typeof this.device.confirmInbound === 'function') {
      this.device.confirmInbound(mapping.cap, finalValue, 'raw_cluster').catch((e) => {
        this.device.log(`[RAW-FALLBACK] ❌ confirmInbound failed: ${e.message}`);
      });
      return;
    }
    this.device.safeSetCapabilityValue(mapping.cap, finalValue, { source: 'raw_cluster' }).catch((e) => {
  ...
  _scheduleSummary() {
    ...
    this.timer = (this.device.homey?.setTimeout ? this.device.homey.setTimeout.bind(this.device.homey) : globalThis.setTimeout)(() => this._writeSummary(), 10000);
  }
...
RawClusterFallback.ZCL_AUTO_MAP = ZCL_AUTO_MAP;
RawClusterFallback.CLUSTER_ATTRS = CLUSTER_ATTRS;
