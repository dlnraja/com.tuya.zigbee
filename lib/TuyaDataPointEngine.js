  ...
  async readAllDataPoints(dpMapping) {
    ...
    for (const dpId of dpIds) {
      try {
        ...
        try {
          const result = await this.tuyaCluster.Promise.resolve(readAttributes([dpId])).catch(() => null);
          if (result && result[dpId] !== undefined) {
            value = result[dpId];
            this.device.log(`[TUYA-DP] ✅ DP ${dpId} read via readAttributes: ${value}`);
          }
        } catch (readErr) {
          this.device.log(`[TUYA-DP] ⚠️  readAttributes failed for DP ${dpId}`);
        }
        
        // Method 2: read specific DP command
        if (value === undefined && typeof this.tuyaCluster.readDP === 'function') {
          try {
            value = await this.tuyaCluster.Promise.resolve(readDP(dpId)).catch(() => null);
            if (value !== undefined && value !== null) {
              this.device.log(`[TUYA-DP] ✅ DP ${dpId} read via readDP: ${value}`);
            }
          } catch (cmdErr) {
            this.device.log(`[TUYA-DP] ⚠️  readDP failed for DP ${dpId}`);
          }
        }
