...
function buildCalmElectricalReportingConfigs({
  withBattery = false,
  withJitter = true,
  chemistry = null,
  percent = null,
  ...
  if (withBattery) {
    let batt = { minInterval: 3600, maxInterval: 43200, minChange: 2 };
    try {
      const { buildAdaptiveReportingConfig } = require('../battery/SmartBatteryAdaptivePrecision');
      batt = buildAdaptiveReportingConfig({ chemistry: chemistry || 'CR2032', percent });
    } catch (_e) { /* soft */ }
    configs.unshift(wrap({
      cluster: 'genPowerCfg',
      attributeName: 'batteryPercentageRemaining',
      minInterval: batt.minInterval,
      maxInterval: batt.maxInterval,
      minChange: batt.minChange,
    }));
  }
  return configs;
}
