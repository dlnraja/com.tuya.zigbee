 ...
 */
function shouldSkipSleepyRemoteBatteryTx(profile, opts = {}) {
  // WHY(P2727): Bastien boot sets bastien_skip_battery_tx before profiles resolve —
  // tip-lag Homey ≤1.0.93 was still TX-storming remotes while overlay purged late.
  try {
    if (opts?.homey?.settings?.get?.('bastien_skip_battery_tx') === true) {
      if (!profile || profile.mainsPowered !== true) return true;
    }
  } catch (_e) { /* soft */ }
...
function shouldProactivePowerCfgRead(device) {
  ...
  try {
    ...
    if (shouldSkipSleepyRemoteBatteryTx(profile, { homey: device.homey })) {return false;}
  } catch (_e) { /* soft */ }
  return true;
}
