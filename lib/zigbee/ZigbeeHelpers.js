...
class ZigbeeHelpers {
  ...
  static normalizeClusterSpec(clusterSpec) {
    ...
    if (typeof clusterSpec === 'string') {
      ...
      const normalized = normalizeCI(clusterSpec);
      ...
      const clusterKey = Object.keys(CLUSTER).find(key =>
        normalizeCI(key) === normalized ||
        normalizeCI(CLUSTER[key]?.name) === normalized
      );
   ...
   * 
   * REASON: Caused infinite recursion loop with BaseUnifiedDevice override
   * 
  ...
  static async configureAttributeReportingLegacy(device, configs) {
    ...
    // v5.12.7: Recursion guard - prevent infinite loop if device overrides configureAttributeReporting
    if (device._inConfigureAttrReportingLegacy) {
      device.log('[WARN]  configureAttributeReportingLegacy: recursion detected, aborting');
      return false;
    }
    device._inConfigureAttrReportingLegacy = true;
    try {
      ...
      device._inConfigureAttrReportingLegacy = false;
    }
  }
...
/**
 * v10.6.0: Shared ZCL header parser for RAW frames (Buffer, full header
 * included — what `node.handleFrame` hands to app-level interceptors).
 *
 * Layout (ZCL spec 07-5123 §2.3.1):
 *   byte 0      : frame control (bit 0x04 = manufacturer-specific,
 *                                 bit 0x01 = cluster-specific,
 *                                 bit 0x08 = direction server→client)
 *   if mfr bit  : bytes 1-2 = manufacturer code (uint16 LITTLE-ENDIAN)
 *   next byte   : transaction sequence number (TSN)
 *   next byte   : command id
 *   rest        : payload
 *
 * Every `data[2]` literal in the codebase silently breaks when the mfr bit
 * is set (cmd id lands at data[4]) — this helper is the single correct way.
 *
 * @param {Buffer|Array} buf - Raw frame bytes
 * @returns {{fc:number, mfrCode:number|null, tsn:number, cmdId:number,
 *            payloadOffset:number, clusterSpecific:boolean,
 *            serverToClient:boolean}|null}
 */
function parseZclHeader(buf) {
  if (!buf || buf.length < 3) {return null;}
  const bytes = Buffer.isBuffer(buf) ? buf : Buffer.from(buf);
  const fc = bytes[0];
  const manufacturerSpecific = (fc & 0x04) !== 0;
  let offset = 1;
  let mfrCode = null;
  if (manufacturerSpecific) {
    if (bytes.length < 5) {return null;}
    mfrCode = bytes.readUInt16LE(1); // spec: little-endian
    offset = 3;
  }
  ...
  return {
    ...
    mfrCode,
    tsn: bytes[offset],
    cmdId: bytes[offset + 1],
    payloadOffset: offset + 2,
    clusterSpecific: (fc & 0x01) !== 0,
    serverToClient: (fc & 0x08) !== 0
  };
}

module.exports = ZigbeeHelpers;
module.exports.parseZclHeader = parseZclHeader;
