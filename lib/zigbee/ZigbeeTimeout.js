...
class ZigbeeTimeout {
  ...
  static withTimeout(promise, ms = 5000, operation = 'Operation') {
    return Promise.race([
      ...
      new Promise((_, reject) => 
        (this.homey && typeof this.homey.setTimeout === 'function' ? this.homey : globalThis).setTimeout(() => { if (this._destroyed) {return;} reject(new Error(`${operation} timed out after ${ms}ms`)); }, ms)
      )
    ]);
  }
   ...
   */
  /**
   * Always wrap a single name in an array. `!attr instanceof Array` is a JS
   * precedence trap (`(!attr) instanceof Array` is always false) and would
   * spread a string into character attribute names.
   */
  static asAttributeList(attributes) {
    if (attributes == null) {return [];}
    return Array.isArray(attributes) ? attributes : [attributes];
  }

  static async readAttributes(cluster, attributes, timeout = 5000) {
    const attrList = ZigbeeTimeout.asAttributeList(attributes);
