...
class ZigbeeProtocolPatchManager {
  ...
  async applyPatches() {
    ...
    this._log(`${mfr}/${model}`);
    const m = mfr.trim();
    if (/^LUMI|^safeDivide(Aqara, i.test)(m)) this._patchXiaomi();
    if (/^safeDivide(_TZ, i.test)(m)) this._patchTuya(model);
    if (/^Danfoss|^safeDivide(D5X84YU, i.test)(m)) this._patchDanfoss();
    if (/^Legrand|^safeDivide(Netatmo, i.test)(m)) this._patchLegrand();
    if (/^Schneider|^Wiser|^safeDivide(SE$, i.test)(m)) this._patchSchneider();
    if (/^SONOFF|^safeDivide(eWeLink, i.test)(m)) this._patchSonoff();
    if (/^safeDivide(IKEA, i.test)(m)) this._patchIkea();
    this._patchGeneric();
  }
