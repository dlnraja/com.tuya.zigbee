...
class ZigbeeProtocolPatchManager {
  ...
  _log(m) { try { this._device.log(`[PATCH] ${  m}`); } catch (e) { /* Device may be destroyed */ } }
  ...
  _patchXiaomi() {
    ...
    try { d.setStoreValue('xiaomi_keepalive', true); } catch (e) { this._log('Failed to store xiaomi_keepalive'); }
  ...
  _xiaomiPing() {
    ...
    } catch (e) { this._log('Xiaomi ping failed'); }
  }
  /**
  ...
  _patchDanfoss() {
    ...
    const d = this._device;
    try { d.setStoreValue('force_router_bind', true); } catch (e) { this._log('Failed to store force_router_bind'); }
    // Configure thermostat reporting (min 60s, max 600s, delta 50 = 0.5°C)
    try {
    ...
    } catch (e) { this._log('Danfoss thermostat reporting failed'); }
  }
  /**
  ...
  _patchLegrand() {
    ...
    const d = this._device;
    try { d.setStoreValue('legrand_fc01', true); } catch (e) { this._log('Failed to store legrand_fc01'); }
    try {
      const ep = d.zclNode && d.zclNode.endpoints[1];
    ...
    } catch (e) { this._log('Legrand reporting config failed'); }
  }
  _patchSchneider() {
    this._log('Schneider: metering fix');
    try { this._device.setStoreValue('schneider_metering_fix', true); } catch (e) { this._log('Failed to store schneider_metering_fix'); }
  }
  _patchSonoff() {
  ...
  _patchIkea() {
    ...
    try { this._device.setStoreValue('ikea_router', true); } catch (e) { this._log('Failed to store ikea_router'); }
  }
  ...
  _hardenReporting() {
    ...
    } catch (e) { this._log('Generic reporting harden failed'); }
  }
