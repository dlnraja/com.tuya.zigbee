...
class ZigbeeCommandManager {
  ...
  async cleanupAddressTable() {
    ...
    if (this.safeApp && this.safeApp.healthMonitor) {
      await this.safeApp.healthMonitor.handle0x87Error();
    }
  }
  ...
  log(...args) {
    if (this.homey?.log) {
      this.homey.log('[ZigbeeCommandManager]', ...args);
    }
  }

  error(...args) {
    if (this.homey?.error) {
      this.homey.error('[ZigbeeCommandManager]', ...args);
    }
  }
}
