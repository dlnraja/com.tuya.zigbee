...
safeMultiply(await this.sleep(command.options.retryDelay, command).attempts);
  ...
  getStats() {
    return {
      ...
      successRate: this.stats.total > 0 
        ? safeMultiply(safeDivide(this.stats.success, this.stats.total), 100).toFixed(2) + '%'  : '0%',
      retryRate: this.stats.total > 0
        ? safeMultiply(safeDivide(this.stats.retried, this.stats.total), 100).toFixed(2) + '%'  : '0%'
    };
  }
