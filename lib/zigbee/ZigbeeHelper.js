...
class ZigbeeHelper {
  ...
  /**
   * Parse power from Zigbee format (deciWatts -> Watts)
   * ZCL electricalMeasurement.activePower is in 0.1W units
   */
  ...
  /**
   * Parse voltage from Zigbee format (deciVolts -> Volts)
   * ZCL electricalMeasurement.rmsVoltage is in 0.1V units
   */
  ...
  /**
   * Parse current from Zigbee format (milliAmps -> Amps)
   * ZCL electricalMeasurement.rmsCurrent is in mA units
   */
  ...
  /**
   * Parse energy from Zigbee format (Wh -> kWh)
   * ZCL metering.currentSummationDelivered is in Wh units
   */
  ...
  static debounce(func, wait) {
    ...
    return function executedFunction(...args) {
      ...
      clearTimeout(timeout);
      timeout = this.homey.setTimeout(later, wait);
    };
  }
  ...
  static throttle(func, limit) {
    ...
    return function(...args) {
      if (!inThrottle) {
        ...
        inThrottle = true;
        this.homey.setTimeout(() => { if (this._destroyed) return; inThrottle = false; }, limit);
      }
    };
  }
