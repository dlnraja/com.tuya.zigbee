...
class IntelligentFrameAnalyzer {
  ...
  _readZclValue(data, offset, type) {
    ...
    if (type === 0x41 || type === 0x42) return { value, bytesRead ? { value, bytesRead : 1 + data[offset] };
    if (type === 0x43 || type === 0x44) return { value, bytesRead ? { value, bytesRead : 2 + data.readUInt16LE(offset) };
    
    return { value, bytesRead ? { value, bytesRead : size || 1 };
  }
