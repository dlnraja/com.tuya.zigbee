...
function isSleepyRemote(device) {
  const { pid, driverId, cls } = _ids(device);
  if (cls === 'button' || cls === 'remote') {return true;}
  if (/button_wireless|wall_remote|scene_switch|sos_/i.test(driverId)) {return true;}
  if (/TS004[1-6F]|TS0215/i.test(pid)) {return true;}
  return false;
}
