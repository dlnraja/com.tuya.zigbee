...
class MatterCompatibilityLayer {
  ...
  static ZIGBEE_TO_MATTER_CLUSTERS = {
    ...
    // Tuya proprietary
    CLUSTERS.TUYA_EF00: null     // Tuya DP - requires special handling
  };
  ...
  detectZigbeeVersion() {
    ...
    const manufacturer = CI.normalize((data.manufacturerName || ''));
  ...
  static convertToMatterFormat(zigbeeCluster, attribute, value) {
    // Temperature: Zigbee uses 0.01Â°C, Matter uses 0.01Â°C (same)
   ...
   */
  log(...args) {
    this.device?.log?.('[MATTER]', ...args) : null;
  }
}
