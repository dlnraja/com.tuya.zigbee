...
function findRawCluster(device, clusterLabel, endpointId = 1) {
  let id = CLUSTER_NAME_TO_ID[clusterLabel];
  if (!id && clusterLabel) {
    try {
      const { normalizeClusterId } = require('./ZclClusterLexicon');
      id = normalizeClusterId(clusterLabel);
    } catch (_e) { /* fallback */ }
  }
  if (!id) { return null; }
  ...
  return ep.clusters[id] || ep.clusters[String(id)] || ep.clusters[`0x${id.toString(16)}`] || null;
}
