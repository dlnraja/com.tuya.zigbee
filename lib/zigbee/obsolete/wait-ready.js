...
async function waitForZigbeeReady(device, { maxAttempts = 15, delayMs = 300 } = {}) {
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      if (endpoint && endpoint.clusters) {
        ...
        device.log(`✅ Zigbee ready (attempt ${i + 1}/${maxAttempts})`);
        return endpoint;
      }
  ...
  device.error(`❌ Zigbee not ready after ${maxAttempts} attempts (${maxAttempts * delayMs}ms)`);
  return null;
}
...
async function waitForCluster(device, clusterName, { maxAttempts = 15, delayMs = 300 } = {}) {
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      if (cluster) {
        device.log(`✅ Cluster ${clusterName} ready (attempt ${i + 1}/${maxAttempts})`);
        return cluster;
      }
  ...
  device.error(`❌ Cluster ${clusterName} not ready after ${maxAttempts} attempts`);
  return null;
}
