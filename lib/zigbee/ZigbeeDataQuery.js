...
class ZigbeeDataQuery {
  ...
  _isMainsPowered() {
    ...
    try {
      ...
      const ps = this.device.getStoreValue?.('powerSource');if (ps === 'Mains' || ps === 'mains') { this._isMains = true; return true; }
    } catch (e) { /* ignore */ }
    this._isMains = false;
    return false;
  }
  ...
  async queryAllAttributes(options = {}) {
    ...
    for (const [clusterName, mapping] of Object.entries(CLUSTER_CAPABILITY_MAP)) {
      const cluster = ep.clusters?.[clusterName];if (!cluster || !cluster.readAttributes) continue;
  ...
  async bindAllClusters(options = {}) {
    ...
    for (const clusterName of clustersToBind) {
      const cluster = ep.clusters?.[clusterName];if (!cluster?.bind) continue;
