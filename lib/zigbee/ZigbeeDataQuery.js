...
class ZigbeeDataQuery {
  ...
  async _processPendingQueries() {
    ...
    for (const queryFn of queries) {
      try {
        await queryFn();
        await new Promise(r => this.homey.setTimeout(r, 100)); // Small delay between queries
      } catch (err) {
      ...
      try {
        ...
        const data = await Promise.race([
          ...
          new Promise((_, reject) =>
            this.homey.setTimeout(() => { if (this._destroyed) return; reject(new Error('Timeout')); }, timeout)
          )
        ]);
