...
class ZigbeeDataQuery {
  ...
  /** WHY(P2470): SH-SC07 / sleepy buttons must not ZCL-storm battery (Z2M#8072). */
  _skipBatteryReporting() {
    try {
      const profile = typeof this.device.getDeviceProfile === 'function'
        ? this.device.getDeviceProfile()
        : this.device?._deviceProfile;
      if (profile?.skipBatteryReporting || profile?.noEf00Tx) return true;
      const driverId = String(this.device?.driver?.id || '');
      if (/^button_wireless_|^button_emergency_/i.test(driverId) && profile?.batteryEpOnly != null) {
        return true;
      }
    } catch (_e) { /* soft */ }
    return false;
  ...
  markAwake() {
    ...
    // v5.10.1: Skip sleep tracking for mains-powered devices (prevents log spam)
    if (this._isMainsPowered()) {return;}
    ...
    this._sleepTimer = this.device.homey.setTimeout(() => {
  ...
  /**
   * v5.10.1: Check if device is mains-powered (cached)
   */
  _isMainsPowered() {
    if (this._isMains !== null) {return this._isMains;}
    try {
      const cfg = this.device._sensorConfig || {};
      if (cfg.battery === false) { this._isMains = true; return true; }
      const ps = this.device.getStoreValue?.('powerSource');
      if (ps === 'Mains' || ps === 'mains') { this._isMains = true; return true; }
    } catch (e) { /* ignore */ }
    this._isMains = false;
    return false;
  }
  ...
  async _processPendingQueries() {
    ...
    for (const queryFn of queries) {
      try {
        await queryFn();
        await new Promise(r => this.device.homey.setTimeout(r, 100)); // Small delay between queries
      } catch (err) {
  ...
  async queryAllAttributes(options = {}) {
    ...
    for (const [clusterName, mapping] of Object.entries(CLUSTER_CAPABILITY_MAP)) {
      ...
      // WHY(P2470 / Peter 8afffc76): active battery read times out 5s and burns wake
      if (clusterName === 'powerConfiguration' && this._skipBatteryReporting()) {
        this.log('⊘ Skip powerConfiguration read (profile.skipBatteryReporting / Z2M#8072)');
        continue;
      }
      ...
      try {
        ...
        const data = await Promise.race([
          ...
          new Promise((_, reject) =>
            this.device.homey.setTimeout(() => { if (this._destroyed) {return;} reject(new Error('Timeout')); }, timeout)
          )
        ]);
  ...
  async configureAllReporting(options = {}) {
    ...
    // WHY(P2470 / Z2M#8072): never configureAttributeReporting on SH-SC07 / skipBatteryReporting
    if (ep.clusters?.powerConfiguration?.configureReporting && !this._skipBatteryReporting()) {
      try {
    ...
    } else if (this._skipBatteryReporting()) {
      this.log('⊘ Skip battery configureReporting (profile.skipBatteryReporting / Z2M#8072)');
    }
...
class DataQuery {
  ...
  async _queryWhileAwake() {
    ...
    BootBudget.markRadioActivity(this.device);

    // WHY: restore hybrid query, but never dump 10 unknown DPs on a sleepy
    // contact that only reported lux (e181bc15). Known DPs only, heap-gated.
    const sleepyBattery = this.device.hasCapability?.('measure_battery')
      && this.device.mainsPowered !== true
      && this.device.forceActiveTuyaMode !== true;
    if (sleepyBattery) {
      if (!BootBudget.shouldTxSleepy(this.device)) {
        this.log('⚡ Device awake — skip active query (heap/not in wake window)');
        return;
      }
      const seen = BootBudget.seenDpList(this.device, 3);
      if (seen.length > 0 && this.device.tuyaDataQuery) {
        this.log(`⚡ Sleepy wake — query seen DPs [${seen.join(', ')}]`);
        await this.device.tuyaDataQuery(seen, {
          logPrefix: '[HYBRID-TUYA]',
          delayBetweenQueries: 0,
        }).catch(() => { });
      } else {
        this.log('⚡ Sleepy wake — no seen DPs yet, listen only');
      }
      return;
    }
   ...
   */
  async initialize() {
    this.log('🚀 Initializing data query...');

    // WHY(P2470 / Peter 8afffc76 @9.0.882): sleepy SH-SC07 — bind+configure+queryAll
    // on boot burns the wake window (battery Timeout) and leaves UI `?` / mute presses.
    // Listen-only: still log cluster analysis; skip active ZCL storms.
    const sleepyButton = this.zigbeeQuery._skipBatteryReporting();
    if (sleepyButton) {
      this.log('P2470 sleepy-button mode — skip bind/configure/queryAll (listen on press)');
      await this.zigbeeQuery.logClusterInfo().catch(() => {});
      this.log('✅ data query initialized (passive)');
      return;
    }
    ...
    this.log('✅ data query initialized');
  }
  ...
  async forceQuery() {
    ...
    const skipTuya = this.device._skipTuyaDataQuery === true
      || this.device._iasOnlyProfile === true
      || this.device._deviceProfile?.type === 'ias_zone';

    // Tuya DPs
    if (!skipTuya && this.device.tuyaDataQuery) {
      const dpMappings = this.device.dpMappings || {};
      const dpIds = Object.keys(dpMappings).map(Number).filter(n => !isNaN(n));
      if (dpIds.length > 0) {
