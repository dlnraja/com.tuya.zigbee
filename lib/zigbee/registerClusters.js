...
function registerCustomClusters(logger = null) {
  ...
  try {
    ...
    // v9.0.146: 0x0021 is a standard Green Power cluster in the SDK stack.
    // Registering our broad custom definition caused noisy startup failures in
    // diagnostics while the native cluster path was already available.
    registrationState.skipped++;
    if (logger) {
      logger.log('[INFO] GreenPowerCluster (0x0021) handled by Homey SDK; custom registration skipped');
    }
