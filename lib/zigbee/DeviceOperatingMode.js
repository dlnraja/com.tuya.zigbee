...
function classifyOperatingFamily(device) {
  ...
  // WHY: Nobø SWS-IZ diag 9cbf9eb6 — user labels TS004F but firmware rejects
  // genOnOff 0x8004 ("32772 is not a valid attribute"). Z2M lists mfr as TS0044
  // wireless 4-button; physical presses arrive as TS0044-RAW multi-EP.
  if (/xffhmvhv/i.test(mfr)) {
  ...
  if (!model) {
    ...
    if (/button_wireless_4|scene_controller/.test(driver)) {
      return { family: 'ts004f_or_4', writeSceneAttr: true, defaultMode: 'scene', tryOnce: true };
    }
