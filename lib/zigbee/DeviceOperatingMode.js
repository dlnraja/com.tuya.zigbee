...
// WHY(P2439/P2448/P2453): rotary knobs (ERS-10 / ZG-101ZD) — NOT 1-btn scene remotes
// (kaflzta4/ja5osu5g + TS004F). Diag a342c411: kaflzta4 skipped 0x8004 → dead press.
// P2453: an5rjiwd is TS0041 → button_wireless_1 (no 0x8004); do NOT treat as TS004F scene.
// P2448: uri7ongn/ixla93vd/g9g2xnch were missing → forced scene → levelControl rotation dead.
// Do NOT list abrsvsou|4fjiwweb here — locked to button_wireless_4 (4-endpoint scene).
const KNOB_MFR = /402vrq2i|qja6nq5z|gwkzibhs|ugi8ky6u|uri7ongn|ixla93vd|g9g2xnch|csflgqj2|pzui3skt|pcqjmcud|uri7ber7/i;
const TS004F_ONE_BUTTON_SCENE_MFR = /kaflzta4|ja5osu5g/i;
...
function classifyOperatingFamily(device) {
  ...
  // WHY(P2439 / Johan #1365 / diag a342c411): 1-btn TS004F scene remotes need
  // genOnOff 0x8004=event so Homey gets 0xFD / multi-press. Command mode alone
  // is silent unless the remote is group-bound to a light — Homey flows stay dead.
  if (TS004F_ONE_BUTTON_SCENE_MFR.test(mfr)) {
    return { family: 'ts004f', writeSceneAttr: true, defaultMode: 'scene' };
  }
  // WHY(P2448): rotary driver listens to levelControl step/move — needs command mode.
  // Old path matched /smart_knob/ → scene + device.js wrote 0x8004=1 → rotation dead.
  if (/smart_knob_rotary/i.test(driver)) {
    return { family: 'knob', writeSceneAttr: true, defaultMode: 'dimmer' };
  }
  // WHY(P2448): smart_knob_switch is the dimmer-oriented sibling (uri7ber7) — same command default.
  if (/smart_knob_switch/i.test(driver)) {
    return { family: 'knob', writeSceneAttr: true, defaultMode: 'dimmer' };
  }
  // Known rotary knobs: actively restore command/dimmer (undo prior scene force)
  if (KNOB_MFR.test(mfr)) {
    return { family: 'knob', writeSceneAttr: true, defaultMode: 'dimmer' };
  }
  // WHY(P2442 / diag a342c411): early wake often has mfr=ABSENT + pid=ABSENT
  // before MFR-ENSURE. Old path: smart_knob → family=knob → skip 0x8004 → dead press.
  // Default smart_knob (NOT _rotary) to scene/event unless a known rotary mfr matched.
  if (/^smart_knob$|smart_knob_switch/i.test(driver)) {
    if (/TS004F/i.test(model) || !model || TS004F_ONE_BUTTON_SCENE_MFR.test(mfr) || !mfr) {
      return { family: 'ts004f', writeSceneAttr: true, defaultMode: 'scene' };
    }
    // Non-TS004F productId on smart_knob without rotary mfr — still prefer scene
    // (Homey flows need 0xFD); rotary rotation still works via levelControl RX.
    return { family: 'ts004f', writeSceneAttr: true, defaultMode: 'scene' };
  }
  // WHY(P2354 / T150690 Steampunk + Z2M Moes TS004F_1): `_TZ3000_xabckq1v`+TS004F
  // needs genOnOff 0x8004=event for multi-click. Was wrongly in the TS0044 skip list
  // so Homey never forced event mode → users saw "generic" / dead multi-press.
  if (/xabckq1v/i.test(mfr)) {
    return { family: 'ts004f', writeSceneAttr: true, defaultMode: 'scene' };
  }
  ...
  // WHY(P2290 / P2461): mrpevh8p / SH-SC07 siblings reject 0x8004 even if modelId lags
  // — family label is endpoint_remote/ts0041 (NOT ts0044) so wake logs match productId.
  // WHY(P2317): also Moes/Tuya TS0044_1 white-labels (mh9px7cq/dziaict4/…)
  // Note: do NOT list xabckq1v here — it is a real TS004F event-mode remote (P2354).
  if (/mrpevh8p|5bpeda8u|b4awzgct/i.test(mfr)) {
    return { family: 'ts0041', writeSceneAttr: false, defaultMode: 'scene' };
  }
  if (/xffhmvhv|kfu8zapd|zgyzgdua|wkai4ga5|a7ouggvs|key8kk7r|bczr4e10|mh9px7cq|j61x9rxn|dziaict4|u3nv1jwk|a4xycprs/i.test(mfr)) {
...
async function applyDesiredMode(device, zclNode) {
  ...
  // WHY(P2448): compose default "scene" or user "auto" must not override knob family
  // defaultMode=dimmer — otherwise rotary stays stuck writing 0x8004=1.
  let setting = String(device.getSetting?.('button_mode') || '').toLowerCase();

  // WHY(P2450 / diag a342c411 + 8adfe4ce): 1-btn scene remotes (kaflzta4) often keep
  // button_mode=dimmer from the era when smart_knob was family=knob (skip/write 0x8004=0).
  // That permanently kills Homey 0xFD presses. Also covers ABSENT@smart_knob wake
  // (family already ts004f/scene via P2442) before MFR-ENSURE names kaflzta4.
  const mfr = _mfr(device);
  const driver = _driverId(device);
  const stuckDimmerSceneRemote = (
    TS004F_ONE_BUTTON_SCENE_MFR.test(mfr)
    || (family.family === 'ts004f' && family.defaultMode === 'scene'
      && /^smart_knob$/i.test(driver) && !KNOB_MFR.test(mfr))
  );
  if (stuckDimmerSceneRemote
    && (setting === 'dimmer' || setting === 'command')
    && device.getStoreValue?.('p2450_user_force_dimmer') !== true) {
    setting = 'scene';
    try {
      await device.setSettings?.({ button_mode: 'auto' });
      device.log?.('[OP-MODE] P2450 reset stuck dimmer → auto/scene (1-btn TS004F)');
    } catch (_e) { /* settings may be locked */ }
    // WHY(P2450): prior failed command-mode writes may have marked 0x8004 unsupported
    // or scene_mode_switch_failed — clear so wake can recover presses.
    try {
      await device.setStoreValue?.(STORE_UNSUPPORTED, false);
      await device.setStoreValue?.('scene_mode_switch_failed', false);
    } catch (_e) { /* optional */ }
  }

  if (!setting || setting === 'auto') {
    setting = String(family.defaultMode || 'scene').toLowerCase();
  }
