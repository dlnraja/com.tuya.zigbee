...
function classifyOperatingFamily(device) {
  ...
  // smart_knob + TS004F without a known rotary mfr (mfr may be ABSENT at early wake)
  // → scene/event so button presses reach Homey after re-pair / wake.
  if (/smart_knob/.test(driver) && /TS004F/i.test(model)) {
    return { family: 'ts004f', writeSceneAttr: true, defaultMode: 'scene' };
  }
