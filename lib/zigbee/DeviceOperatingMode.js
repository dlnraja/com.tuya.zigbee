...
// WHY(P2439): rotary knobs only — NOT one-button scene remotes (kaflzta4/ja5osu5g/an5rjiwd).
// Diag a342c411: kaflzta4 on smart_knob skipped 0x8004 → "button press not working".
const KNOB_MFR = /402vrq2i|qja6nq5z|gwkzibhs|ugi8ky6u/i;
...
function classifyOperatingFamily(device) {
  ...
  // Known rotary knobs: keep command/dimmer (rotation via levelControl)
  ...
  // Default smart_knob to scene/event unless a known rotary mfr matched above.
  if (/smart_knob/.test(driver)) {
