...
class ZigbeeDebug {
  ...
  const nodeInfo = { ieeeAddress: zclNode.ieeeAddress || 'N/A', manufacturerName: zclNode.manufacturerName || 'N/A', productId: zclNode.productId || 'N/A', endpoints: Object.keys(zclNode.endpoints || {}) };
  ...
  setupReportMonitoring(endpoint, clusterName) {
    try {
      ...
      const originalOn = cluster.on?.bind(cluster);if (originalOn) {
