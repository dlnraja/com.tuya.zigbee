...
class FallbackSystem {
  ...
  async executeWithFallback(name, strategies, options = {}) {
    ...
    this.stats.attempts++;
    this.debug(`🔄 [${name}] Starting with ${strategies.length} strategies`);
    ...
    for (let i = 0; i < strategies.length; i++) {
      ...
      try {
        ...
        this.log(`✅ [${name}] ${strategyName} succeeded (${duration}ms)`);
        return result;
        
      } catch (err) {
        this.debug(`❌ [${name}] ${strategyName} failed: ${err.message}`);
  ...
  async configureReportWithFallback(config, options = {}) {
    ...
    const strategies = [
      ...
      async () => {
        ...
        // Bind first
        await endpoint.clusters[cluster].bind();
      ...
      async () => {
        ...
        this.debug(`⚠️  Report config failed, will use manual polling for ${cluster}.${attributeName}`);
