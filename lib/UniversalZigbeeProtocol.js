...
class UniversalZigbeeProtocol {
  ...
  async analyzeDevice() {
    ...
    const result = {
      ...
      model: this.device.getSetting?.('zb_model_id') || 'Unknown',
      endpoints: {},
      capabilities: []
    };
  ...
  safeFlowTrigger(cardId, tokens = {}) {
    try {
      ...
      this.log?.(`[FLOW] Trigger card '${cardId}' not found:`, e.message);
    }
    return false;
  }
  ...
  debouncedFlow(cardId, tokens = {}, delayMs = 300) {
    ...
    this.homey.setTimeout(() => { if (this._destroyed) return; delete this._flowDebounce[key]; }, delayMs);
    return this.safeFlowTrigger(cardId, tokens);
  }

  // v5.5.658: data handler (ZCL + DP + private)
    ...
    for (const cap of fb.capabilities) {
      if (!this.device.hasCapability?.(cap)) {
        try { await this.device.addCapability?.(cap); this.log(`[AUTO-CAP] Added ${cap}`); }
          ...
          this.log?.(`[AUTO-CAP] Could not add ${cap}:`, e.message);
        }
      }
    }
    return true;
