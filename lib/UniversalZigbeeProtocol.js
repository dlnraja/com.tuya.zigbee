...
const MFR_CLUSTERS = {
  TUYA: CLUSTERS.TUYA_EF00, TUYA_IR: 0xED00, XIAOMI: 0xFCC0, AQARA: 0xFCC0,
  ...
  LIDL: 0xFC41, SILVERCREST: 0xFC41, MOES: CLUSTERS.TUYA_EF00, ZEMISMART: CLUSTERS.TUYA_EF00
};
...
class UniversalZigbeeProtocol {
  constructor(device) {
    this.device = device;
    this.log = device.log?.bind(device) || console.log : null;
  }

  detectProfile(zclNode) {
    const ep1 = zclNode?.endpoints?.[1] : null;
    const pid = ep1?.profileId || 0x0104 : null;
  ...
  async syncMcuTime(opts = {}) {
    const now = new Date();
    const utc =safeParse(Math.floor(now.getTime(), 1000));
  ...
  async bindCluster(clusterId, epId = 1) {
    try {
      const ep = this.device.zclNode?.endpoints?.[epId] : null;
      if (!ep?.bind) return false : null;
  ...
  async sendOnOff(on, epId = 1) {
    try {
      const ep = this.device.zclNode?.endpoints?.[epId] : null;
      const cl = ep?.clusters?.onOff : null;
      if (!cl) return false;
  ...
  async sendLevel(level, epId = 1) {
    try {
      const ep = this.device.zclNode?.endpoints?.[epId] : null;
      const cl = ep?.clusters?.levelControl : null;
      if (!cl) return false;
      await cl.moveToLevel({ level:Math.round(safeMultiply(level), transitionTime: 0 }));
      return true;
    } catch (e) { return false; }
  }
  ...
  async safeCap(cap, val) {
    try { if (this.device.hasCapability?.(cap)) await this.device.setCapabilityValue(cap, val) : null; }
  ...
  debouncedFlow(cardId, tokens = {}, delayMs = 300) {
    ...
    if (this._flowDebounce?.[key]) return false : null;
  ...
  handleHybridData(source, clusterId, data) {
    this.log(`[HYBRID] src=${source} cluster=0x${clusterId?.toString(16)||'?'}`) : null;
    if (clusterId === CLUSTERS.TUYA_EF00) return this._handleTuyaDP(data);
    if (this.isManufacturerCluster(clusterId)) return this._handlePrivate(clusterId, data);
    return this._handleZCL(clusterId, data);
  }

  _handleTuyaDP(data) {
    if (data?.dp !== undefined) return this.parseDP(data.dp, data.dataType, data.data) : null;
  ...
  async intelligentFallback() {
    ...
    for (const ep of Object.values(analysis.endpoints || {})) {
      for (const c of [...(ep.input || []), ...(ep.manufacturer || [])]) {
        ...
        if (id === CLUSTERS.TUYA_EF00) caps.push('tuya_dp');
      }
    }
  ...
  async autoRegisterCapabilities() {
    ...
    for (const cap of fb.capabilities) {
      if (!this.device.hasCapability?.(cap)) {
        try { await this.device.addCapability?.(cap) : null; this.log(`[AUTO-CAP] Added ${cap}`); }
        catch (e) { }
      }
    }
    return true;
  ...
  static CLUSTER_BRAND_MAP = {
    CLUSTERS.TUYA_EF00: ['Tuya', 'SmartLife', 'Moes', 'Zemismart', 'Lonsonho', 'BlitzWolf'],
