...
class MotionAwarePresenceDetector extends EventEmitter {
  ...
  async enable() {
    ...
    this.log(`✅ Motion Aware enabled (sensitivity: ${this.sensitivity})`);
  }
  ...
  disable() {
    ...
    this.log('✅ Motion Aware disabled');
  }
  ...
  async initializeBaseline() {
    ...
    if (samples.length > 0) {
      ...
      this.log(`✅ Baseline established: ${this.baseline.toFixed(1)} dBm (${samples.length} samples)`);
    } else {
      this.error('❌ Failed to establish baseline (no samples)');
    ...
    try {
      ...
      if (deviation > threshold) {
        ...
        if (!this.presenceDetected) {
          this.log(`🎯 Presence DETECTED (RSSI: ${rssi} dBm, baseline: ${this.baseline.toFixed(1)} dBm, deviation: ${deviation.toFixed(1)} dBm)`);
