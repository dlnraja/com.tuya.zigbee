...
async function _toggleGang(device, gang, sourceCap) {
  const now = Date.now();
  try {
    const { ensureDedup, stampVirtual } = require('../utils/BidirectionalButtonState');
    const d = ensureDedup(device);
    const lastPhysical = d.lastPhysicalPress[gang] || 0;
    if (now - lastPhysical < (d.dedupWindow || 2000)) {
      _log(device, `[DEDUP] Skipping ${sourceCap} (physical ${now - lastPhysical}ms ago)`);
      return true;
    }
    stampVirtual(device, gang, now);
  } catch (_e) {
  ...
  if (gangCap) {
    ...
    const next = !current;
    // WHY(P2283): markAppCommand + L14 safeSet — never raw setCapabilityValue
    if (typeof device.markAppCommand === 'function') {
      device.markAppCommand(gang, next);
    }
    if (typeof device._setGangOnOff === 'function') {
      await device._setGangOnOff(gang, next);
    } else if (typeof device.safeSetCapabilityValue === 'function') {
      await device.safeSetCapabilityValue(gangCap, next).catch(() => {});
    } else if (typeof device.setCapabilityValue === 'function') {
      await device.setCapabilityValue(gangCap, next).catch(() => {});
    }
  } else if (typeof device._setGangOnOff === 'function') {
    // WHY(P2397): subdevice parents — TX EP without local onoff.gangN cap
    if (typeof device.markAppCommand === 'function') {
      device.markAppCommand(gang, true);
    }
    await device._setGangOnOff(gang, true).catch(() => {});
  } else {
...
async function _feed(device) {
  try {
    ...
    if (tuyaCluster) {
      // WHY(P2711): never pass `value` — Homey rejects unexpected property on datapoint
