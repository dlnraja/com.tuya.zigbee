...
const SafeCapabilityMixin = {
  ...
  async safeSetCapabilityValue(capability, value) {
    try {
      ...
      // 4) WHY(P2308 / Gmail #532): NEVER call this.setCapabilityValue here.
      // TuyaZigbeeDevice overrides setCapabilityValue → safeSetCapabilityValue,
      // so this.setCapabilityValue would recurse until "Maximum call stack".
      // Prefer Homey Device.prototype (SDK) when available.
      let setter = null;
      try {
        const { Device } = require('homey');
        if (Device?.prototype?.setCapabilityValue) {
          setter = Device.prototype.setCapabilityValue;
        }
      } catch (_e) { /* optional */ }
      if (setter) {
        await setter.call(this, capability, value);
      } else {
        // WHY(P2308): no independent SDK setter — skip rather than risk
        // setCapabilityValue ↔ safeSetCapabilityValue recursion.
        return false;
      }
      return true;
    } catch (_err) {
