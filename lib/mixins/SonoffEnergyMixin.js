'use strict';

const CID = 0xFC11;
const MFR = { manufacturerCode: 0x1286, disableDefaultResponse: false };
const EA = {
  current: 0x7004,
  voltage: 0x7005,
  power: 0x7006,
  energyToday: 0x7009,
  energyMonth: 0x700a,
  energyYesterday: 0x700b,
};

const { equalsCI, includesCI } = require('../utils/CaseInsensitiveMatcher');

module.exports = { setupSonoffEnergy: _setup };

async function _setup(device, zclNode) {
  const m = device.getSetting('zb_manufacturer_name') || '';
  if (!equalsCI(m, 'sonoff') && !equalsCI(m, 'ewelink')) {return false;}
  const pid = device.getSetting('zb_model_id') || '';
  if (!includesCI(['S60ZB', 'S31ZB', 'SPM', 'SNZB-06P'], pid)) {return false;}
  const ep = zclNode && zclNode.endpoints && zclNode.endpoints[1];
  if (!ep) {return false;}
  const cl = ep.clusters && ep.clusters[CID];
  if (!cl) {return false;}
  device.log('[SONOFF-NRG] Init 0xFC11 energy');
  let vals = {};
  try { vals = await cl.readAttributes(Object.values(EA), MFR) || {}; }
  catch (e) { device.log('[SONOFF-NRG] Read:', e.message); }
  if (vals[EA.current] !== undefined && device.hasCapability('measure_current'))
    {device.setCapabilityValue('measure_current', vals[EA.current] / 1000).catch(() => device.log('[SONOFF-NRG] setCapabilityValue measure_current failed')); }
  if (vals[EA.voltage] !== undefined && device.hasCapability('measure_voltage'))
    {device.setCapabilityValue('measure_voltage', vals[EA.voltage] / 1000).catch(() => device.log('[SONOFF-NRG] setCapabilityValue measure_voltage failed')); }
  if (vals[EA.power] !== undefined && device.hasCapability('measure_power'))
    {device.setCapabilityValue('measure_power', vals[EA.power] / 1000).catch(() => device.log('[SONOFF-NRG] setCapabilityValue measure_power failed')); }
  device._sonoffNrgCluster = cl;
  device.log('[SONOFF-NRG] Ready');
  return true;
}
