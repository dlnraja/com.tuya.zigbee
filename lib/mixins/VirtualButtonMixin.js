      ...
      try {
        ...
        if (this._isPureTuyaDP) {
          try {
            ...
            this.log(`[VIRTUAL-BTN] ⚠️ Tuya DP listener failed: ${e.message}`);
          }
        }
        ...
        if (!success && isZclOnly) {
          try {
            ...
            this.log(`[VIRTUAL-BTN] ⚠️ ZCL-only listener failed: ${e.message}`);
          }
        }
        ...
        if (!success && node) {
          ...
          if (onOff && typeof onOff[newValue ? 'setOn' : 'setOff'] === 'function') {
            try {
              ...
              this.log(`[VIRTUAL-BTN] ⚠️ ZCL failed: ${zclErr.message}`);
            }
          }
        }
