...
const VirtualButtonMixin = (Base) => {
  ...
  async _handleVirtualToggle(gang) {
    ...
    try {
      ...
      // v5.5.991: Try listener first, fallback to direct ZCL for ZCL-only devices
      try {
        ...
        const epNum = gang || 1;
        const onOff = this.zclNode?.endpoints?.[epNum]?.clusters?.onOff || 
                      this.zclNode?.endpoints?.[epNum]?.clusters?.genOnOff;
        if (onOff) {
          await onOff[newValue ? 'setOn' : 'setOff']();
          await this.setCapabilityValue(targetCap, newValue);
