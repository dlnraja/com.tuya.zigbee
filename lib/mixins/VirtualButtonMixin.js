...
const VirtualButtonMixin = (Base) => {
  ...
  async _handleVirtualToggle(gang) {
    ...
    try {
      ...
      // v5.5.999: Improved ZCL fallback for multi-gang ZCL-only devices (BSEED diagnostic fix)
      // For ZCL-only devices, directly use ZCL commands instead of triggerCapabilityListener
      const node = this._zclNode || this.zclNode;
      const epNum = gang || 1;
      
      // v5.5.999: Check if device has registered capability listener for this capability
      ...
      // v5.5.999: Helper to find onOff cluster with multiple lookup strategies
      ...
      if (isZclOnly && node) {
        ...
        if (onOff && typeof onOff[newValue ? 'setOn' : 'setOff'] === 'function') {
          ...
          // v5.5.999: Cluster not ready - try to send via capability listener as fallback
          this.log(`[VIRTUAL-BTN] ⚠️ No onOff cluster on EP${epNum}, trying capability listener`);
          try {
            await this.triggerCapabilityListener(targetCap, newValue);
            this.log(`[VIRTUAL-BTN] ✅ Capability listener succeeded for ${targetCap}`);
          } catch (e) {
            // v5.5.999: Last resort - try setting value and let base class handle it
            this.log(`[VIRTUAL-BTN] ⚠️ Listener failed (${e.message}), setting value directly`);
            await this.setCapabilityValue(targetCap, newValue).catch(() => {});
          }
        }
      } else {
        ...
        try {
          ...
          if (onOff && typeof onOff[newValue ? 'setOn' : 'setOff'] === 'function') {
            ...
            // v5.5.999: Even if ZCL fails, try setting capability value
            this.log(`[VIRTUAL-BTN] ⚠️ No ZCL cluster for EP${epNum}, setting value directly`);
            await this.setCapabilityValue(targetCap, newValue).catch(() => {});
          }
        }
      }
