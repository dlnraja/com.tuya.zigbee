...
const VirtualButtonMixin = (Base) => {
  ...
  return class extends Base {
    ...
    markAppCommand(gang, value) {
      ...
      this._appCommandTimeout[key] = this.homey.setTimeout(() => { 
    ...
    async _handleVirtualToggle(gang = 1) {
      ...
      if (nowTap - (this._virtualToggleLastTap[gang] || 0) < 300) {
        this.log(`[VIRTUAL-BTN] 🛡️ [G${gang}] Tap dropped (anti-spam 300ms)`);
        return;
      }
    ...
    async _tryExecuteZCL(gang, value) {
      ...
      try {
        ...
        if (typeof this.canSendCommand === 'function' && !this.canSendCommand('virtual_button')) {
          this.log(`[ZCL-EXEC] 🛡️ [G${gang}] TX blocked by rate limiter (sleepy-device flood protection)`);
          return false;
        }
    ...
    async _tryExecuteTuyaDP(gang, value) {
      ...
      try {
        ...
        if (typeof this.canSendCommand === 'function' && !this.canSendCommand('virtual_button')) {
          this.log(`[DP-EXEC] 🛡️ [G${gang}] TX blocked by rate limiter (sleepy-device flood protection)`);
          return false;
        }
        // Mark as App command
        if (typeof this.markAppCommand === 'function') {
        ...
        if (this.hasCapability('onoff')) {
          ...
          if (onOff) {
            for (let i = 0; i < 3; i++) {
              ...
              await new Promise(r => this.homey.setTimeout(r, 400));
            }
