...
const VirtualButtonMixin = (Base) => {
  ...
  return class extends Base {
    ...
    async initVirtualButtons() {
      ...
      // WHY(P2397): resolve from gangCount/buttonCount + button.N caps (legacy thin)
      let gangCount = 1;
      try {
        const { resolveGangCount } = require('../utils/BidirectionalButtonState');
        gangCount = resolveGangCount(this);
      } catch (_e) {
    ...
    async _handleVirtualToggle(gang = 1) {
      ...
      let finalCap = this.hasCapability(targetCap) ? targetCap : this.hasCapability(asymmetricCap) ? asymmetricCap : null;

      // WHY(P2397): Homey zigbee.devices parents often lack onoff.gangN (child tile
      // owns onoff) but button.N must still TX the matching endpoint.
      if (!finalCap && gang === 1 && this.hasCapability('onoff')) {
        finalCap = 'onoff';
      }

      try {
        const currentValue = finalCap ? (this.getCapabilityValue(finalCap) === true) : false;
        const newValue = !currentValue;
        
        this.log(`[VIRTUAL-BTN] [G${gang}] Toggle Request: ${currentValue} → ${newValue}${finalCap ? ` (${finalCap})` : ' (EP-only)'}`);

        // Record event
        this._recordVirtualButtonEvent(gang, 'toggle', { cap: finalCap || `ep${gang}`, from: currentValue, to: newValue });
        ...
        // Bidirectional Sync: Optimistically update UI when a gang cap exists
        if (finalCap) {
        ...
        // Prefer UnifiedSwitchBase cascade when present (multi-EP + EF00 aware)
        if (!this._virtualButtonState) {
          this._virtualButtonState = { gangs: {} };
        }
        if (!this._virtualButtonState.gangs[gang]) {
          this._virtualButtonState.gangs[gang] = { consecutiveFailures: 0, totalPresses: 0 };
        }
        if (typeof this._setGangOnOff === 'function') {
          try {
            await this._setGangOnOff(gang, newValue);
            this._virtualButtonState.gangs[gang].consecutiveFailures = 0;
            return;
          } catch (e) {
            this.log(`[VIRTUAL-BTN] [G${gang}] _setGangOnOff failed: ${e.message}`);
          }
        }
    ...
    _recordVirtualButtonEvent(gang, type, data = {}) {
      ...
      // WHY(P2332): declared-only — prefer _safeTriggerFlow over raw getter
      try {
        if (typeof this._safeTriggerFlow === 'function') {
          this._safeTriggerFlow('virtual_button_pressed', {
            type, gang, correlationId: event.correlationId, button: String(gang),
          }, { type: 'virtual' });
        } else {
          ...
          const resolved = findDeclaredCI(declared, 'virtual_button_pressed') || 'virtual_button_pressed';
          if (declared.size && !findDeclaredCI(declared, 'virtual_button_pressed')) { /* skip invent */ }
          else {
            const card = safeGetFlowCard(this.homey, resolved, 'trigger', declared.size ? declared : null);
            if (!isNoopFlowCard(card) && typeof card?.trigger === 'function') {
              card.trigger(this, { type, gang, correlationId: event.correlationId }, {}).catch(() => {});
            }
          }
        }
