...
const VirtualButtonMixin = (Base) => class extends Base {
  ...
  async initVirtualButtons() {
    ...
    // Dim up button
    if (this.hasCapability('button.dim_up')) {
      this.registerCapabilityListener('button.dim_up', async () => {
    ...
    // Dim down button
    if (this.hasCapability('button.dim_down')) {
      this.registerCapabilityListener('button.dim_down', async () => {
