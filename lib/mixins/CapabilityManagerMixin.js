...
const BVB_CONSTRAINTS = {
  measure_temperature: { min: -40, max: 80, maxJump: 10 },        // v7.0.0: Reduced Max 80Â°C (standard sensors)
...
const CapabilityManagerMixin = {
  ...
  _blockBizarreValue(capability, value) {
    ...
    if (bounds?.maxJump && typeof value === 'number' && typeof previous === 'number') {
      ...
      if (jump > bounds.maxJump && !isRecovery) {
        ...
        hold = hold || { count: 0, lastValue, time: now };
  ...
  _getFlowCard(id, type = 'trigger') {
    try {
      ...
      const driverId = this.driver?.id || this.manifest?.id || (this.id ? this.id.split(':')[0]);
      
      // Stage 0: Build ID variants (Direct, Prefixed, Namespaced)
      const idVariants = [id];
      if (driverId) {
      ...
      // Stage 1: Try SDK 3 Flow Manager methods with variants
      ...
      if (this.driver && typeof this.driver[type] === 'function') {
        for (const variantId of idVariants) {
          try {
            const card = this.driver[type](variantId);
            if (card) return card;
          } catch (e) { /* continue */ }
        }
      }

      this.log?.(`[FLOW-SAFE] No valid flow card found for ID: ${id} (variants: ${idVariants.join(', ')})`);
    } catch (err) {
