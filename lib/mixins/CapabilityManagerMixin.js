...
const CapabilityManagerMixin = {
    ...
    try {
      ...
      if (typeof this.hasCapability === 'function' && !this.hasCapability(capability)) {
        ...
        try {
          ...
          this.log?.(`[CAP-MANAGER] Could not add capability ${capability}:`, err.message);
        }
      }
      ...
      const THROTTLE = {
        ...
        // WHY(P2740 / GH#550): was 10000 — light-off lux lagged several seconds
        'measure_luminance': 400,
        'measure_luminance.distance': 200,
        'measure_power': 4000,
        'measure_voltage': 8000,
      ...
      try {
        ...
        await commitCapability(this, capability, calibratedValue, options.source || 'unknown');
      } catch (_e) {
        const sdkSetter = (typeof this.safeSetCapabilityValue === 'function'
          ? this.safeSetCapabilityValue.bind(this)
          : (this._setCapabilityValueDirect
            ? this._setCapabilityValueDirect.bind(this)
            : (this.setCapabilityValue ? this.setCapabilityValue.bind(this) : null)));
        if (sdkSetter) {
          await sdkSetter(capability, calibratedValue, { source: options.source }).catch(() => {});
        }
      }
  ...
  _updateLastSeen() {
    ...
    if (typeof this.setLastSeenAt === 'function') {
      try { this.setLastSeenAt(now); } catch(e) {
        this.log?.('[CAP-MANAGER] setLastSeenAt skipped:', e.message);
      }
    }
  ...
  _getFlowCard(id, type = 'trigger') {
    try {
      ...
      this.log?.(`[CAP-MANAGER] Flow card '${id}' not found:`, err.message);
    }
    return null;
  },
