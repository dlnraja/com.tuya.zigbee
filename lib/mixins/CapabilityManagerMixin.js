...
const CapabilityManagerMixin = {
  ...
  _blockBizarreValue(capability, value) {
    ...
    // v6.1.1: 255 filtering (CRITICAL: exclude lighting to avoid 100% dimming lock)
    const isLighting = ['dim', 'light_hue', 'light_saturation', 'light_temperature', 'light_mode'].includes(capability);
    if (value === 255 && !isLighting && !['measure_voltage', 'measure_power', 'meter_power', 'measure_current'].includes(capability)) {
