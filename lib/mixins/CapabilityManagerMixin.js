    ...
    try {
      ...
      if (tracker && !options.skipThrottle && !isBooleanChange) {
        ...
        if (elapsed < throttleMs && !sigChange && !forceTrigger) {
          // v7.0.18: DROPPED !valueChanged check to fix sensor flickering
          // This ensures that minor changes (<0.3°C, <2% hum) are ignored 
          // during the throttle window even if the value changed slightly.
          return false;
        }
      }
