  ...
  _blockBizarreValue(capability, value) {
    ...
    if (bounds?.maxJump && typeof value === 'number' && typeof previous === 'number') {
      ...
      if (jump > bounds.maxJump && !isRecovery) {
        ...
        if (hold.count < 2) {
          this.log(`[BVB] 🚫 Blocked jump from ${previous} to ${value} (Δ ${jump.toFixed(1)} > Max ${bounds.maxJump}) for ${capability}`);
          return true;
        }
      ...
      for (const method of candidates) {
        ...
        if (typeof flowMethod === 'function') {
          try {
            ...
            if (this.error) this.error(`[FLOW-SAFE] ⚠️ method ${method} failed for ${id}:`, e.message);
          }
        }
      }
