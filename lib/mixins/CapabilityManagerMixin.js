...
const CapabilityManagerMixin = {
  ...
  _blockBizarreValue(capability, value) {
    ...
    // v6.1.0: Syndrome du 65535 (0xFFFF)
    if (value === 65535) {
      this.log(`[BVB] 🚫 Blocked Tuya garbage data (65535/0xFFFF) for ${capability}`);
      return true;
    }
    
    // v6.1.0: 255 filtering (except for capabilities where 255 is valid like voltage or power)
    if (value === 255 && !['measure_voltage', 'measure_power', 'meter_power', 'measure_current'].includes(capability)) {
    ...
    if (bounds?.maxJump && typeof value === 'number' && typeof previous === 'number') {
      ...
      if (jump > bounds.maxJump && !isRecovery) {
        ...
        const hold = this._jumpHold[capability] || { count: 0, lastValue: null };
