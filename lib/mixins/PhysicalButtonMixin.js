...
const DEVICE_PROFILES = {
  ...
  '_TZ3000_l9brjwau': {
    // WHY(P2455 / GH#544): wired BSEED TS0002 → wall_switch_2gang_1way (not TS0012 invent).
    appCommandWindow: 2000, doubleClickWindow: 500, longPressThreshold: 800,
    protocol: 'zcl_only', brand: 'BSEED', productId: 'TS0002'
  },
  // v5.8.1: BSEED 3-gang
  '_TZ3000_qkixdnon': { 
    appCommandWindow: 2000, doubleClickWindow: 500, longPressThreshold: 800,
  ...
  // WHY(P2554): Moes/SunBeech dfgbtub0 — HA/Z2M TS0042 + sacred TS0044 sibling.
  // Same 0xFD hybrid; never invent pid — profile is mfr-scoped, pid from interview.
  '_TZ3000_dfgbtub0': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Moes', usesE000: true,
    skip8004: true, writeSceneAttr: false, sceneSwitch: true,
    source: 'P2554_z2m_ha_dfgbtub0'
  },
  '_tz3000_dfgbtub0': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Moes', usesE000: true,
  ...
  // WHY(P2317): Z2M TS0044_1 white-labels — same 0xFD/E000 path; never 0x8004
  '_TZ3000_mh9px7cq': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Moes', usesE000: true, productId: 'TS0044',
    buttonCount: 4, skip8004: true, writeSceneAttr: false, sceneSwitch: true,
    source: 'P2317_z2m_ts0044_1'
  },
  '_TZ3000_j61x9rxn': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Moes', usesE000: true, productId: 'TS0044',
    buttonCount: 4, skip8004: true, writeSceneAttr: false, sceneSwitch: true,
    source: 'P2317_z2m_ts0044'
  },
  '_TZ3000_dziaict4': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
  ...
  // WHY(P2440 / Peter #2230 diag 048cff91): multi-path (0xFD + onOff + level) → flicker.
  // WHY(P2461 / #2230 @9.0.836): firmware retransmit ~3.4s later still outside 1100ms
  // window → second flow burst (disco). Raise gang-only window to 4s.
  '_TZ3000_mrpevh8p': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    ...
    debounceMs: 4000, crossPathDedupMs: 4000, crossPathGangOnly: true,
    whiteLabel: ['SH-SC07', 'RSH-SC021'],
    source: 'P2285_z2m_hubitat_johan1120_forum2202_P2440_P2461',
  },
  '_TZ3000_5bpeda8u': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya/SH-SC07', usesE000: true, productId: 'TS0041',
    ...
    noEf00Tx: true, mapAllEndpointsToButton1: true,
    debounceMs: 4000, crossPathDedupMs: 4000, crossPathGangOnly: true,
    source: 'P2285_z2m_whitelabel_SH-SC07_P2461',
  },
  '_TZ3000_b4awzgct': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya/SH-SC07', usesE000: true, productId: 'TS0041',
    ...
    noEf00Tx: true, mapAllEndpointsToButton1: true,
    debounceMs: 4000, crossPathDedupMs: 4000, crossPathGangOnly: true,
    source: 'P2285_z2m_whitelabel_SH-SC07_was_4ts0041_misroute_P2461',
  },
  ...
  // WHY(P2680 / GH#551): TS0043 sticky is hybrid OnOff 0xFD + E000 — never zcl_only /
  // never write 0x8004. Contre quoi: "aucun bouton ne fonctionne" after Generic re-pair.
  // WHY(P2735): snappy parity with vsxvaj9i (Homey Flow queue + bi-dir soft UI).
  '_TZ3000_famkxci2': {
    appCommandWindow: 180, doubleClickWindow: 120, longPressThreshold: 550,
    protocol: 'hybrid', brand: 'LoraTap', usesE000: true, productId: 'TS0043',
    buttonCount: 3, debounceMs: 25, crossPathDedupMs: 40,
    skip8004: true, writeSceneAttr: false, sceneSwitch: true,
    collapsePhantomEndpoints: true,
    // WHY(P2691): coin remote — never configureReporting / press powerCfg TX
    noEf00Tx: true, skipBatteryReporting: true, batteryEpOnly: 1,
    skipSoftwareHoldRelease: true,
    disableLevelControlComplement: true,
    snappyRelayFlow: true,
    skipUiPulse: true,
    zcl200IsPercent: true,
    source: 'P2735_gh551_famkxci2_snappy_parity',
  },
  // WHY(P2683 / Bastien): _TZ3000_vsxvaj9i+TS0043 — Developer Tools "canaux" are NOT
  // Homey Flows. Presses = 0xFD/E000 → Flow "Button pressed". skip8004 forever.
  // WHY(P2693/P2702): snappy debounce + reject phantom gang + no LevelControl invent.
  '_TZ3000_vsxvaj9i': {
    appCommandWindow: 180, doubleClickWindow: 120, longPressThreshold: 550,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS0043',
    buttonCount: 3, debounceMs: 25, crossPathDedupMs: 40,
    skip8004: true, writeSceneAttr: false, sceneSwitch: true,
    collapsePhantomEndpoints: true, noEf00Tx: true, skipBatteryReporting: true,
    // WHY(P2686 / diag 885a9901): 0xFD long is discrete — never invent HOLD-RELEASE
    skipSoftwareHoldRelease: true,
    disableLevelControlComplement: true,
    snappyRelayFlow: true,
    skipUiPulse: true,
    zcl200IsPercent: true,
    batteryEpOnly: 1,
    source: 'P2707_bastien_vsxvaj9i_ultimate',
  },
  '_TZ3000_nrfkrgf4': { 
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
  ...
  '_TZ3000_ja5osu5g': { 
    ...
    buttonCount: 1, writeSceneAttr: true, source: 'P2439_one_button_ts004f'
  },
  // WHY(P2453): ZHA/Z2M + Johan #1128 — an5rjiwd is TS0041 1-btn (not TS004F / not 4-gang)
  '_TZ3000_an5rjiwd': { 
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS0041',
    buttonCount: 1, writeSceneAttr: false, source: 'P2453_ts0041_button_wireless_1'
  },
  '_tz3000_an5rjiwd': { 
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS0041',
  ...
  // WHY(P2638): Bastien node3 + Z2M#25720 + ZHA TuyaSmartRemote0041TO + SmartHomeScene Moes Star Ring
  // Interview EP1 [0,1,6] out ota/time — NO E000/EF00. ZHA uses TuyaNoBindPowerConfiguration.
  // Z2M: tuya.fz.on_off_action → single/double/hold; configureMagicPacket; no battery reporting (#8072).
  // HA gist (compujunk): zha_event remote_button_{short,double,long}_press — Homey = Flow triggers (not sticky action cap).
  // UI name "3ch" is WRONG — physical is 1 button (TS0041).
  // WHY(P2684 / Bastien): 1500ms debounce felt "aucune fonctionnalité" — snappy like P2702
  '_TZ3000_axpdxqgu': {
    appCommandWindow: 180, doubleClickWindow: 120, longPressThreshold: 550,
    protocol: 'zcl_0xfd', brand: 'Moes Star Ring', usesE000: false, productId: 'TS0041',
    buttonCount: 1, skip8004: true, writeSceneAttr: false,
    collapsePhantomEndpoints: true, batteryEpOnly: 1, skipBatteryReporting: true,
    noEf00Tx: true, mapAllEndpointsToButton1: true,
    debounceMs: 25, crossPathDedupMs: 40, crossPathGangOnly: true,
    snappyRelayFlow: true,
    zcl200IsPercent: true,
    whiteLabel: ['Moes Star Ring', 'ZT-YK01-class'],
    source: 'P2707_bastien_ts0041_ultimate',
  },
  '_tz3000_axpdxqgu': {
    appCommandWindow: 180, doubleClickWindow: 120, longPressThreshold: 550,
    protocol: 'zcl_0xfd', brand: 'Moes Star Ring', usesE000: false, productId: 'TS0041',
  ...
  // WHY(P2639 / HA T455202): Chinese TS0041 ships TS0044 4-EP firmware — ZHA shows 4 dead switches.
  // Medium @edwarddubilyer + ZHA#1557: use TS0044-shaped signature but keep MODEL=TS0041; Homey collapses EP2–4→btn1.
  // Sibling class: adndolvx, itb0omhv, x7mej5oc — never button_wireless_4 / never invent 4 physical gangs.
  '_TZ3000_adndolvx': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'zcl_0xfd', brand: 'Tuya/Chinese-TS0041', usesE000: true, productId: 'TS0041',
    buttonCount: 1, skip8004: true, writeSceneAttr: false,
    collapsePhantomEndpoints: true, batteryEpOnly: 1, skipBatteryReporting: true,
    noEf00Tx: true, mapAllEndpointsToButton1: true, phantomEpFirmware: true,
    debounceMs: 1500, crossPathDedupMs: 1500, crossPathGangOnly: true,
    source: 'P2639_HA_T455202_ZHA1557_Medium_chinese_TS0041',
  },
  '_tz3000_adndolvx': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'zcl_0xfd', brand: 'Tuya/Chinese-TS0041', usesE000: true, productId: 'TS0041',
  ...
  '_TZ3000_itb0omhv': {
    ...
    collapsePhantomEndpoints: true, batteryEpOnly: 1, skipBatteryReporting: true,
    noEf00Tx: true, mapAllEndpointsToButton1: true, phantomEpFirmware: true,
    source: 'P2639_HA_T455202_sibling_itb0omhv',
  },
  '_tz3000_itb0omhv': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'zcl_0xfd', brand: 'Tuya/Chinese-TS0041', usesE000: true, productId: 'TS0041',
  ...
  // WHY(P2439): Homey diag a342c411 — button press dead while classified as knob (skip 0x8004)
  '_TZ3000_kaflzta4': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
    buttonCount: 1, writeSceneAttr: true, source: 'P2439_diag_a342c411'
  },
  '_tz3000_kaflzta4': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
  ...
  // WHY(P2448): ERS-10 / ZG-101ZD rotaries — allow 0x8004 write (command/dimmer), not skip
  '_TZ3000_uri7ongn': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
    buttonCount: 1, writeSceneAttr: true, rotary: true, source: 'P2448_ERS10'
  },
  '_tz3000_uri7ongn': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
  ...
  '_TZ3000_g9g2xnch': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
    buttonCount: 1, writeSceneAttr: true, rotary: true, source: 'P2448_ZG101ZD'
  },
  '_tz3000_g9g2xnch': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
  ...
  '_TZ3000_qja6nq5z': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
    buttonCount: 1, writeSceneAttr: true, rotary: true, source: 'P2448_smart_knob_rotary'
  },
  '_tz3000_qja6nq5z': {
    appCommandWindow: 1000, doubleClickWindow: 350, longPressThreshold: 600,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS004F',
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // HOBEIAN Switches — Bastien house lights (ZG-301Z / WHD02)
  // WHY(P2704): snappy Homey↔relay; 1s appCommandWindow made physical feel laggy
  // after UI toggle. Contre quoi: kitchen/salon lights slow after Flow/UI.
  // ════════════════════════════════════════════════════════════════════════════
  'HOBEIAN': {
    appCommandWindow: 350, doubleClickWindow: 280, longPressThreshold: 550,
    debounceMs: 80, crossPathDedupMs: 120,
    protocol: 'zcl_onoff', brand: 'HOBEIAN', productId: 'ZG-301Z',
    snappyTx: true, snappyRelayFlow: true,
    noEf00Tx: true, mainsPowered: true, skipBatteryReporting: true,
    source: 'P2704_bastien_hobeian_fleet_snappy',
  },
  // WHY(P2704 / Bastien Node20): salon 4-gang module — serialize TX without 15–50ms
  // pace + long retry; physical↔UI stays reactive.
  '_TZ3000_ltt60asa': {
    appCommandWindow: 350, doubleClickWindow: 280, longPressThreshold: 550,
    debounceMs: 80, crossPathDedupMs: 120, crossPathGangOnly: false,
    protocol: 'zcl_onoff', brand: 'Tuya', productId: 'TS0004', gangCount: 4,
    snappyTx: true, snappyRelayFlow: true, mainsPowered: true,
  ...
  '_TZ3000_mmkbptmx': {
    ...
    protocol: 'zcl_onoff', brand: 'Tuya', productId: 'TS0004', gangCount: 4,
    snappyTx: true, snappyRelayFlow: true, mainsPowered: true,
    source: 'P2704_bastien_ts0004_module_snappy',
  },
  '_TZ3000_liygxtcq': {
    appCommandWindow: 350, doubleClickWindow: 280, longPressThreshold: 550,
    debounceMs: 80, crossPathDedupMs: 120,
  ...
  // WHY(P2681 / Bastien diag 8f0915fa): 2-btn sticky — hybrid 0xFD; phantom EP3/4 FW
  // WHY(P2702 / diag 1f4dcf2e @1.0.76): 200ms still "latence enorme" button→relay — 80/120
  '_TZ3000_dzwgk7e2': {
    appCommandWindow: 180, doubleClickWindow: 120, longPressThreshold: 550,
    protocol: 'hybrid', brand: 'Tuya', usesE000: true, productId: 'TS0042',
    buttonCount: 2, debounceMs: 25, crossPathDedupMs: 40,
    skip8004: true, writeSceneAttr: false, sceneSwitch: true,
    collapsePhantomEndpoints: true, noEf00Tx: true, skipBatteryReporting: true,
    // WHY(P2686 / Bastien 885a9901): single press was firing button_2_release ghost
    // (leftover invent timer) + powerCfg TX lag ~6s — discrete 0xFD, no soft release
    skipSoftwareHoldRelease: true,
    // WHY(P2693): LevelControl stop→release invent toggled the other lamp
    disableLevelControlComplement: true,
    snappyRelayFlow: true,
    skipUiPulse: true,
    zcl200IsPercent: true,
    batteryEpOnly: 1,
    source: 'P2707_bastien_dzwgk7e2_crash_safe',
  },
...
const PhysicalButtonMixin = (SuperClass) => {
   ...
   */
  async _sendTuyaMagicPacket(zclNode, opts = {}) {
    try {
      const { sendTuyaMagicPacket } = require('../zigbee/TuyaMagicPacket');
      // WHY(P2554 / Z2M configureMagicPacket / HA first-action-ignored):
      // force on init so 0xFFDE=0x13 always lands — idempotent skip drops first press.
      await sendTuyaMagicPacket(this, zclNode, 1, { force: true, ...opts });
    } catch (e) {
      // best-effort only
    }
  }
   ...
   */
  _shouldSkipCoverMotorPhysicalDpDetection() {
    if (!this.hasCapability?.('windowcoverings_state') || !this.hasCapability?.('windowcoverings_set')) {
      return false;
    }
    if (this.hasCapability?.('onoff') || this.hasCapability?.('onoff.1')) {
      return false;
    }
    const driverId = String(this.driver?.id || '');
    if (driverId === 'curtain_motor_wall') { return false; }
    return /^(curtain_motor|curtain_motor_shutter|smart_screen_switch)$/.test(driverId)
      || (driverId.startsWith('curtain_motor') && driverId !== 'curtain_motor_wall');
  }

  async initPhysicalButtonDetection(zclNode) {
    // WHY(P2395): UnifiedSwitchBase._setupZCLMode skips duplicate ZCL command
    // listeners when this flag is set — must stamp on the real mixin path
    // (HybridSwitchBase alone was insufficient → double physical RX risk).
    this._hasPhysicalButtonMixin = true;
    ...
    // WHY(P2374 / diag 05867379): curtain_motor DP1=state enum, DP2=position — not bool
    // button gangs. PhysicalButtonMixin was spamming [PHYSICAL-DP] and starving hybrid RX.
    if (this._shouldSkipCoverMotorPhysicalDpDetection()) {
      this._buttonDiagLogger.info('Skip Tuya DP button detection (cover motor DP semantics)');
    ...
    // WHY(P2683): any profile.buttonCount (TS0042/43 stickies) must clamp gangCount —
    // Contre quoi: resolveGangCount Math.max + phantom EP4 → bind EP3/4 → ghost lamp Flows
    ...
    if (earlyProfile?.buttonCount && Number(earlyProfile.buttonCount) > 0) {
      const n = Number(earlyProfile.buttonCount);
      this.buttonCount = n;
      if (earlyProfile.collapsePhantomEndpoints || earlyProfile.mapAllEndpointsToButton1
        || earlyProfile.sceneSwitch || /^TS004[12346F]$/i.test(String(earlyProfile.productId || ''))) {
        this.gangCount = n;
      }
    }
    ...
    // WHY(P2290/P2683): clamp after Math.max so phantom EP4 never wins over sticky buttonCount
    if (earlyProfile?.buttonCount && Number(earlyProfile.buttonCount) > 0
      && (earlyProfile.collapsePhantomEndpoints || earlyProfile.mapAllEndpointsToButton1
        || earlyProfile.sceneSwitch || /^TS004[12346F]$/i.test(String(earlyProfile.productId || '')))) {
    ...
    for (let gang = 1; gang <= gangCount; gang++) {
      // WHY(P2681): seed from Homey capability so first ZCL report is not null→value PHYSICAL
      let seedState = null;
      try {
        const caps = [
          gang === 1 ? 'onoff' : null,
        ...
        for (const cap of caps) {
          if (this.hasCapability?.(cap)) {
            const v = this.getCapabilityValue(cap);
            if (typeof v === 'boolean') { seedState = v; break; }
          }
        }
      } catch (_e) { /* soft */ }
    ...
    // WHY(P2395): apply preferredLevels gates BEFORE gang DP/ZCL wiring so L7
    // EF00 skip is known for TS004x remotes (residual interview noise).
    try {
  ...
  _isDebounced(gang = 1) {
    ...
    // WHY(P2721): snappyRelayFlow must NOT inherit a fat Hubitat-class debounce —
    // report path uses snappy floor so rapid toggle isn't blocked for seconds.
    const profile = this.getDeviceProfile?.() || {};
    let debounceMs = this._reportDebounceMs || 200;
    if (profile.snappyRelayFlow) {
      debounceMs = Math.min(Number(profile.debounceMs) || 25, 80);
    } else if (Number(profile.debounceMs) > 0) {
      debounceMs = Number(profile.debounceMs);
    }
  ...
  getDeviceProfile(overrides = null) {
    ...
    // WHY(P2378): accept late-identity overrides (P2298) when getSetting is still stale.
    const ov = overrides || this._identityOverrides || null;
    let manufacturerName = '';
    let modelId = '';
    try {
      const MfrHelper = require('../helpers/ManufacturerNameHelper');
      manufacturerName = (ov && ov.zb_manufacturer_name)
        || MfrHelper.getManufacturerName(this) || '';
      modelId = (ov && ov.zb_model_id)
        || MfrHelper.getModelId(this) || '';
    } catch (_e) {
      manufacturerName = (ov && ov.zb_manufacturer_name)
    ...
    // WHY(P2718 Bastien 27b0bd04 @1.0.82 OOM): getDeviceProfile was called 8+ times
    // per press, each time spreading DEVICE_PROFILES + logging → heap/CPU storm.
    // Cache by mfr|pid until identity changes. Contre quoi: blank remotes after crash.
    const cacheKey = `${String(manufacturerName || '').toLowerCase()}|${String(modelId || '').toUpperCase()}|${String(this.driver?.id || this.driverId || '')}`;
    if (!overrides && this._deviceProfileCache && this._deviceProfileCacheKey === cacheKey) {
      return this._deviceProfileCache;
    }

    // Exact case-insensitive mfr match (avoid substring false positives)
    // WHY(P2736 / Bastien L99): exact mfr hit used to RETURN raw profile and skip
    // P2714 snappy floor — lagging siblings (pre-P2735 famkxci2, TS0043 OEM) kept
    // appWindow=1000 / no snappyRelayFlow while vsxvaj9i was fast. Contre quoi:
    // Homey Flow queue lag + black remotes "still slow" after tip update.
    const driverId = String(this.driver?.id || this.driverId || '');
    const modelUp = String(modelId || '').toUpperCase();

    const applyTs004xSnappyParity = (profile) => {
      if (!profile || typeof profile !== 'object') return profile;
      const pid = String(profile.productId || modelUp || '').toUpperCase();
      // Never force snappy on knobs / scene-only TS004F rotary
      if (/TS004F/i.test(pid) || /knob|rotary/i.test(String(profile.brand || '')) || /knob|rotary/i.test(driverId)) {
        return profile;
      }
      const bc = Number(profile.buttonCount) || 0;
      const isTs004Scene = /^TS004[123]$/i.test(pid)
        || (profile.sceneSwitch === true && bc >= 1 && /^TS004/i.test(pid));
      if (!isTs004Scene) return profile;
      if (profile.snappyRelayFlow === true && (bc <= 1 || profile.skipUiPulse === true)
          && Number(profile.debounceMs || 999) <= 40) {
        return profile;
      }
      const multi = bc > 1;
      return {
        ...profile,
        appCommandWindow: Math.min(Number(profile.appCommandWindow) || 1000, 180),
        doubleClickWindow: Math.min(Number(profile.doubleClickWindow) || 400, 120),
        longPressThreshold: Math.min(Number(profile.longPressThreshold) || 600, 550),
        debounceMs: Math.min(Number(profile.debounceMs) || 999, 25),
        crossPathDedupMs: Math.min(Number(profile.crossPathDedupMs) || 200, 40),
        skip8004: true,
        ...
        disableLevelControlComplement: true,
        snappyRelayFlow: true,
        skipUiPulse: multi ? true : !!profile.skipUiPulse,
        zcl200IsPercent: profile.zcl200IsPercent !== false,
        source: `${profile.source || 'profile'}+P2736_snappy_parity`,
      };
    };

    const finish = (resolved) => {
      const out = applyTs004xSnappyParity(resolved);
      if (!overrides && out) {
        this._deviceProfileCache = out;
        this._deviceProfileCacheKey = cacheKey;
      }
      return out;
    };
    ...
    for (const [mfr, profile] of Object.entries(DEVICE_PROFILES)) {
      if (mfr !== 'default' && TU.equalsIgnoreCase(manufacturerName, mfr)) {
        ...
        return finish({ ...profile, manufacturerName: mfr, detectedName: manufacturerName });
      }
    }
    ...
    // WHY(P2714 / Bastien diag e1654535+1f4dcf2e @1.0.76/80): TS0042/43 felt
    // much slower than TS0041 because (1) empty mfr used default appWindow=1000
    // until late MFR-ENSURE re-arm, (2) class fallback debounce was still 80ms
    // without snappyRelayFlow. Match axpdxqgu / vsxvaj9i floor immediately.
    const ts004RemoteFallback = (productId, buttonCount, source) => {
      const pid = String(productId || '').toUpperCase();
      if (pid && !/^TS004[12346F]$/i.test(pid)) { return null; }
      const multi = Number(buttonCount) > 1;
      return {
        ...DEVICE_PROFILES.default,
        protocol: 'hybrid',
        brand: 'Tuya remote',
        productId: pid || (buttonCount === 1 ? 'TS0041' : buttonCount === 2 ? 'TS0042' : buttonCount === 3 ? 'TS0043' : 'TS0044'),
        buttonCount,
        appCommandWindow: 180,
        doubleClickWindow: 120,
        longPressThreshold: 550,
        debounceMs: 25,
        crossPathDedupMs: 40,
        skip8004: true,
        writeSceneAttr: false,
    ...
    for (const [prefix, count] of Object.entries(driverButtonCount)) {
      if (!driverId.includes(prefix)) continue;
      // Prefer known pid; else still snappy from driver class (empty zb_model_id at boot)
      const pidHint = modelUp.startsWith('TS004') ? modelUp : '';
      const fb = ts004RemoteFallback(pidHint, count, pidHint
        ? `P2714_${prefix}_${pidHint}`
        : `P2714_${prefix}_driver_snappy`);
      if (fb) {
        if (!this._profileFallbackLogged) {
          this.log?.(`[PROFILE] P2714 ${prefix}${pidHint ? `+${pidHint}` : ''} snappy fallback`);
          this._profileFallbackLogged = true;
        }
        return finish(fb);
      }
    }
    ...
    // WHY(P2395): SOS remotes — never write 0x8004; cascade skips L7 EF00 residual
    if (/button_emergency_sos|emergency_sos/i.test(driverId)) {
      if (!this._profileFallbackLogged) {
        this.log?.(`[PROFILE] P2395 SOS skip8004 fallback for "${manufacturerName || 'mfr?'}"`);
        this._profileFallbackLogged = true;
      }
      return finish({
        ...DEVICE_PROFILES.default,
        ...
        manufacturerName: manufacturerName || 'unknown',
        detectedName: manufacturerName || 'unknown',
        source: 'P2395_button_emergency_sos',
      });
    }

    return finish({ ...DEVICE_PROFILES.default, manufacturerName: 'unknown', detectedName: manufacturerName,
      // WHY(P2645): unknown button remotes still skip battery configureReporting
      skipBatteryReporting: true, skip8004: true, writeSceneAttr: false, batteryEpOnly: 1,
    });
  }
  ...
  _getTimingProfile() {
    ...
    // WHY(P2681): never crash / log "undefined" when BaseUnifiedDevice returns null
    const raw = (typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null) || {};
    const deviceProfile = {
      ...(DEVICE_PROFILES.default || {}),
      ...raw,
      brand: raw.brand || DEVICE_PROFILES.default?.brand || 'Generic',
      manufacturerName: raw.manufacturerName || raw.detectedName || 'unknown',
      appCommandWindow: raw.appCommandWindow || DEVICE_PROFILES.default?.appCommandWindow || 500,
      doubleClickWindow: raw.doubleClickWindow || DEVICE_PROFILES.default?.doubleClickWindow || 400,
      longPressThreshold: raw.longPressThreshold || DEVICE_PROFILES.default?.longPressThreshold || 600,
    };
  ...
  _safeTriggerFlow(triggerId, tokens = {}, debug = {}) {
    try {
      ...
      let flowState = {};
      try {
        const { stateFromFlowTokens } = require('../flow/FlowCardDeviceResolve');
        flowState = stateFromFlowTokens(tokens, {
          button: debug.gang != null ? String(debug.gang) : undefined,
          gang: debug.gang != null ? String(debug.gang) : undefined,
          pressType: debug.pressType,
        });
      } catch (_e) {
        flowState = {
          button: tokens.button != null ? String(tokens.button) : (debug.gang != null ? String(debug.gang) : undefined),
          gang: tokens.gang != null ? String(tokens.gang) : (debug.gang != null ? String(debug.gang) : undefined),
        };
      }
      ...
      try {
        ...
        return triggerFlowCardHeuristic(this, candidates, tokens, 'trigger', flowState);
      } catch (_e) {
      ...
      const declared = collectDeclaredFlowIds(this.homey, this);
      const target = findDeclaredCI(declared, candidates[0]) || (declared.size ? null : candidates[0]);
      // WHY(P2381): allow driver-scoped ID when not yet in merged declared set
      const driverId = String(this.driver?.id || '');
      const fallback = (!target && driverId
        && String(candidates[0] || '').toLowerCase().startsWith(`${driverId.toLowerCase()}_`))
        ? candidates[0]
        : null;
      const useId = target || fallback;
      if (!useId) {return Promise.resolve(false);}
      const triggerCard = safeGetFlowCard(
        this.homey,
        useId,
        'trigger',
        target ? declared : null,
      );
      if (isNoopFlowCard(triggerCard) || typeof triggerCard?.trigger !== 'function') {
        this.log(` Flow '${useId}' trigger method unavailable`);
        return Promise.resolve(false);
      }
      return triggerCard.trigger(this, tokens, flowState)
        .then(() => true)
        .catch((err) => {
          this.error(`[PHYSICAL] Flow '${useId}' failed: ${err.message}`);
          return false;
        });
    } catch (err) {
  ...
  _triggerPhysicalFlow(gang, pressType, tokens = {}) {
    // WHY(P2693 / Bastien vsxvaj9i+dzwgk7e2): phantom EP3/4 must never fire Flow
    // Contre quoi: btn1 press toggles "the other" lamp via ghost gang 3/4
    try {
      const profileGate = this.getDeviceProfile?.() || {};
      const maxBtn = Math.max(1, Number(
        profileGate.buttonCount || this.buttonCount || this.gangCount || 1,
      ) || 1);
      const g = Number(gang) || 1;
      if ((profileGate.collapsePhantomEndpoints || profileGate.sceneSwitch
        || /^TS004[12346]$/i.test(String(profileGate.productId || '')))
        && g > maxBtn) {
        this.log(`[PHYSICAL] P2693 reject phantom gang ${g} > buttonCount ${maxBtn}`);
        return Promise.resolve(false);
      }
    } catch (_gate) { /* soft */ }

    // WHY(P2440 / Peter #2230 diag 048cff91): parallel RX (0xFD + onOff + levelControl)
    // fires multiple flow cards for one finger press → lights disco/flicker.
    // 1-btn remotes: gang-only window (different clusters invent different pressTypes).
    // Multi-gang: keep type-aware key so true double/long still land after the window.
    if (tokens?.source !== 'virtual' && !tokens?._internalTrigger) {
      try {
        const profile = this.getDeviceProfile?.() || {};
        const win = Number(profile.crossPathDedupMs || profile.debounceMs || this._crossPathDedupMs || 900);
        const norm = String(pressType || 'single')
          .replace(/^long_press$/i, 'long')
          .replace(/^(on|toggle|commandOn|commandToggle)$/i, 'single')
          .replace(/^(off|commandOff)$/i, 'double')
          .toLowerCase();
        const oneBtn = profile.crossPathGangOnly === true
          || Number(profile.buttonCount) === 1
          || !!profile.mapAllEndpointsToButton1
          || /button_wireless_1/i.test(String(this.driver?.id || ''));
        this._crossPathPress = this._crossPathPress || {};
        const key = oneBtn ? `g${gang}` : `g${gang}:${norm}`;
        const now = Date.now();
        const last = this._crossPathPress[key] || 0;
        if (win > 0 && now - last < win) {
          this.log(`[PHYSICAL] P2440 cross-path dedup ${key} skip ${norm} (${now - last}ms < ${win}ms)`);
          return Promise.resolve(false);
        }
        this._crossPathPress[key] = now;
        // WHY(P2440): Peter "no battery" — read ZCL % on first accepted press (awake window)
        // WHY(P2733 / Bastien IG): snappy/skipBatt remotes — never schedule press-path
        // battery work (even store restore) — stalls TS0041 when TS0042/43 fire.
        if (oneBtn && typeof this._readBatteryWhileAwake === 'function'
          && !(profile.snappyRelayFlow || profile.skipBatteryReporting || profile.noEf00Tx)) {
          try { this._readBatteryWhileAwake().catch(() => {}); } catch (_b) { /* soft */ }
        }
      } catch (_e) { /* soft */ }
    }
    ...
    // WHY(P2492): Homey charter — physical → UI sync (onoff tiles + elegant pulse)
    // WHY(P2707 / Bastien): full charter on multi-gang snappy OOMed Homey.
    // WHY(P2734): skipUiPulse → soft complementary pulse (bi-dir tiles) — never silent.
    if (tokens?.source !== 'virtual') {
      let softOnly = false;
      try {
        const p = this.getDeviceProfile?.() || {};
        softOnly = !!(p.skipUiPulse || (p.snappyRelayFlow && (Number(p.buttonCount) || Number(this.buttonCount) || 0) > 1));
      } catch (_e) { /* soft */ }
      if (softOnly) {
        try {
          const { softPulsePhysicalUi } = require('../utils/HomeyButtonUiCharter');
          softPulsePhysicalUi(this, gang, { source: 'physical', snappy: true, pulseMs: 50 });
        } catch (_e) {
          try {
            const { pulseButtonCapability } = require('../utils/ensureGangUiCapabilities');
            pulseButtonCapability(this, gang, 50);
          } catch (_e2) { /* soft */ }
        }
      } else {
        try {
          const { syncPhysicalToHomeyUi } = require('../utils/HomeyButtonUiCharter');
          const syncOpts = { source: 'physical' };
          if (tokens && typeof tokens.value === 'boolean') syncOpts.value = tokens.value;
          else if (tokens && typeof tokens.onoff === 'boolean') syncOpts.value = tokens.onoff;
          Promise.resolve(syncPhysicalToHomeyUi(this, gang, syncOpts)).catch(() => {});
        } catch (_e) {
          try {
            const { pulseButtonCapability } = require('../utils/ensureGangUiCapabilities');
            pulseButtonCapability(this, gang);
          } catch (_e2) { /* soft */ }
        }
      }
    }
    ...
    if (typeof this.triggerButtonPress === 'function' && !tokens._internalTrigger) {
      ...
      // WHY(P2706): snappy remotes must NOT await card cascade — isolates TS0041
      // from TS0042/43 Flow storms (app-wide Homey Flow queue).
      try {
        if (this.getDeviceProfile?.()?.snappyRelayFlow) {
          Promise.resolve(this.triggerButtonPress(gang, normType, clicks, extra)).catch(() => {});
    ...
    // WHY(P2331): on/off presses are switch physical cards — never map them to
    // `*_button_pressed` for button_wireless_switch / fingerbot (those invent IDs).
    const isOnOffPress = pressType === 'on' || pressType === 'off';
    const useRemoteButtonCards = isButtonDevice && !isOnOffPress
      && !/button_wireless_switch|fingerbot|_sensor_switch$|_purifier_switch$/.test(driverId);
    ...
    if (useRemoteButtonCards) {
      ...
      // WHY(P2331): also try `${driverId}_1gang_physical_*` / `_switch_1gang_physical_*` below
      if (gangCount === 1) {
    ...
    // WHY(P2330): many *_switch compose files use `${driverId}_${gangCount}gang_gangN_scene`
    // WHY(P2659/Bastien 4c0d232b): never invent undeclared switch_1gang_gang1_scene /
    // switch_1gang_1gang_gang1_scene — P2381 driver-prefix fallback still probes Homey → FLOW-GUARD spam.
    ...
    if ((pressType === 'on' || pressType === 'off') && (sceneMode === 'magic' || sceneMode === 'both' || sceneMode === 'auto')) {
      const sceneCandidates = gangCount === 1
        ? [`${driverId}_gang1_scene`, `${driverId}_1gang_gang1_scene`]
        : [`${driverId}_gang${gang}_scene`, `${driverId}_${gangCount}gang_gang${gang}_scene`];
      let declaredScene = null;
      try {
        const { collectDeclaredFlowIds, findDeclaredCI } = require('../flow/FlowCardHeuristics');
        declaredScene = collectDeclaredFlowIds(this.homey, this);
        for (const sceneCardId of sceneCandidates) {
          if (!declaredScene?.size || !findDeclaredCI(declaredScene, sceneCardId)) {continue;}
          this._safeTriggerFlow(sceneCardId, { action: pressType, gang, button: String(gang) }, { type: 'scene' });
        }
      } catch (_e) {
        // Soft: skip invent when declared set unavailable
      }
    }
    if (sceneMode === 'magic' && (pressType === 'on' || pressType === 'off')) {return;}

    // WHY(P2330): Homey dropdown args on scene_switch_4_button_pressed need `button`, not only `gang`
    const flowTokens = { ...tokens, gang, button: String(tokens.button || gang) };
    ...
    this._safeTriggerFlow(flowCardId, flowTokens, { pressType, gang }).then(success => {
      // WHY(P2330/P2331): Nganged + 1gang physical IDs used by *_switch compose files
      if (!success && !useRemoteButtonCards) {
        const alts = [];
        if (gangCount > 1) {
          alts.push(`${driverId}_${gangCount}gang_physical_gang${gang}_${pressType}`);
        } else {
          alts.push(`${driverId}_1gang_physical_${pressType}`);
          alts.push(`${driverId}_switch_1gang_physical_${pressType}`);
        }
        for (const alt of alts) {
          if (alt !== flowCardId) {
            this._safeTriggerFlow(alt, flowTokens, { pressType, gang, type: 'compose-alt' });
          }
        }
      }
  ...
  async _setupOnOffFdBoundCluster(zclNode) {
    ...
    const profileBtn = Number(this.getDeviceProfile?.()?.buttonCount) || 0;
    // WHY(P2290): SH-SC07 phantom EP2–4 still emit 0xFD — bind them, route to btn1
    // WHY(P2683): never bind past sticky buttonCount (TS0042 phantom EP3/4 → ghost Flows)
    let bindMax = map1
      ? Math.max(gangCount, ...Object.keys(zclNode?.endpoints || {}).map(Number).filter((n) => n >= 1 && n <= 4), 1)
      : gangCount;
    if (!map1 && profileBtn > 0) {
      bindMax = Math.min(bindMax, profileBtn);
    }
    try {
  ...
  async _setupGangPhysicalDetection(zclNode, gang) {
    ...
    // WHY(P2682 / Bastien 8f0915fa+f37e8a91): sleepy TS0042 fails group join
    // ("Impossible de joindre…") with TX retries — wastes airtime on busy mesh.
    const isMultiGangRelay = (this.gangCount > 1) && this.hasCapability?.('onoff');
    const isSleepyRemote = !!(
      this.hasCapability?.('measure_battery')
      || this.getEnergy?.()?.batteries?.length
      || /button_wireless|scene_switch|remote_button|smart_knob/i.test(String(this.driver?.id || ''))
    );
    const groupsCluster = endpoint?.clusters?.groups || endpoint?.clusters?.genGroups || endpoint?.clusters?.[4] || endpoint?.clusters?.['groups'];
    if (groupsCluster && !isMultiGangRelay && !isSleepyRemote) {
      try {
    ...
    } else if (groupsCluster && isSleepyRemote) {
      this.log(`[PHYSICAL] P2682 skip group join on sleepy remote EP${gang} (mesh calm)`);
    }
    ...
    // WHY(P2395): cascade preferredLevels without L7 → skip residual EF00 on
    // pure ZCL remotes (TS004x) even if interview shows a dead 0xEF00.
    if (this._skipCascadeL7Ef00) {
      if (gang === 1) {
        this._buttonDiagLogger?.info?.('Skip Tuya DP physical (cascade L7 not preferred)');
      }
    } else if ((profile.protocol === 'tuya_dp' || this._isPureTuyaDP === true)
  ...
  _handleTuyaDPReport(gang, value) {
    ...
    // WHY(P2681 / Bastien 8f0915fa): null→on/off after init is attribute bootstrap, not a press
    // WHY(P2714): seed before debounce so first report does not burn the press window
    const previousState = state.lastState;
    if (previousState === null || previousState === undefined) {
      state.lastState = value;
      this.log(`[PHYSICAL-DP] Gang ${gang}: bootstrap seed ${value} (no flow)`);
      return;
    }
    ...
    this.log(`[PHYSICAL-DP] Gang ${gang}: ${previousState} → ${value} (${isPhysical ? 'PHYSICAL' : 'APP'})`);
  ...
  _handleAttributeReport(gang, value, data) {
    ...
    // WHY(P2721 Bastien IG 2026-09-24): snappy remotes felt ~7s dead between presses —
    // 5s TSN window + powerCfg lag ≈ “attendre sept secondes avant d'éteindre”.
    // Contre quoi: legitimate rapid toggles dropped as TSN “doublon”.
    // WHY(P2733): button_wireless_* default to 400ms even before profile resolves.
    if (data && data.transactionSequenceNumber !== undefined) {
      ...
      try {
        const p = this.getDeviceProfile?.() || {};
        const driverId = String(this.driver?.id || this.driverId || '');
        if (p.snappyRelayFlow || (Number(p.debounceMs) || 0) > 0 && Number(p.debounceMs) <= 80
          || /button_wireless_|scene_switch_/.test(driverId)) {
          tsnWin = 400;
        }
      } catch (_e) { /* soft */ }
      const isDup = ring.some(e => e.tsn === tsn && (nowT - e.ts) < tsnWin);
      if (isDup) {
    ...
    // WHY(P2681 / Bastien diag 8f0915fa): first ZCL sample after init has lastState=null.
    // Treating null→false/true as PHYSICAL floods Homey Flows across every switch_1gang
    // that reports ("boutons marchent au hasard" / random automations).
    // WHY(P2714 / e1654535): bootstrap must NOT call _isDebounced first — that armed
    // the 80ms window and ate the next real 0xFD press (TS0043 "très lentement").
    if (previousState === null || previousState === undefined) {
      state.lastState = value;
      this.log(`[PHYSICAL] Gang ${gang}: bootstrap seed ${value} (no flow)`);
      return;
    }
  ...
  async _onZigbeeIdentityResolved(updates = {}) {
    ...
    // WHY(P2381): cache overrides BEFORE profile lookup / re-arm so _getTimingProfile
    // never logs "undefined device profile" after late MFR-ENSURE (cfbf687f @ 9.0.779).
    this._identityOverrides = {
      zb_manufacturer_name: mfr || undefined,
      zb_model_id: pid || undefined,
    };
    // WHY(P2718): drop stale profile cache so late MFR-ENSURE gets snappy house couples
    this._deviceProfileCache = null;
    this._deviceProfileCacheKey = null;
    this._profileMatchLogged = false;
    this._profileFallbackLogged = false;
    // WHY(P2378): pass updates so profile matches before Homey settings cache refreshes
    const profile = typeof this.getDeviceProfile === 'function'
      ? this.getDeviceProfile(updates)
      : null;
    const driverId = String(this.driver?.id || '');
    // WHY(P2450 / diag a342c411+8adfe4ce): late MFR-ENSURE on smart_knob often lands
    // AFTER wake already ran with mfr=ABSENT. Must re-apply 0x8004 scene for kaflzta4
    // (and siblings) — otherwise stuck dimmer/command mode → dead press forever.
    const isOneBtnSceneKnob = /kaflzta4|ja5osu5g/i.test(mfr)
      || (/^smart_knob$/i.test(driverId) && /^TS004F$/i.test(pid)
        && !/402vrq2i|qja6nq5z|gwkzibhs|ugi8ky6u|uri7ongn|ixla93vd|g9g2xnch|csflgqj2|pzui3skt|pcqjmcud|uri7ber7/i.test(mfr));
    // WHY(P2316): also re-arm meter91/Nobø late identity (0xFD BoundCluster + magic)
    const needsRearm = !!(profile?.mapAllEndpointsToButton1
      || profile?.collapsePhantomEndpoints
      || isOneBtnSceneKnob
      || /mrpevh8p|5bpeda8u|b4awzgct|zgyzgdua|wkai4ga5|xffhmvhv|kfu8zapd|mh9px7cq|j61x9rxn|dziaict4|u3nv1jwk|a4xycprs|etufnltx|dzwgk7e2|famkxci2|vsxvaj9i|axpdxqgu|adndolvx|itb0omhv|x7mej5oc/i.test(mfr)
      || (/button_wireless_1/.test(driverId) && /^TS0041$/i.test(pid))
      || (/button_wireless_[23468]|scene_switch_4|remote_button_wireless/.test(driverId)
        && /^TS004[12346F]$/i.test(pid)));
    if (!needsRearm) return;
    this.log?.(`[PHYSICAL] P2298/P2450 identity resolved (${mfr}|${pid}) — re-arm + mode`);
    // WHY(P2681): refresh timing AFTER overrides so profile is not "undefined" / default
    try {
      this._timingProfile = this._getTimingProfile();
    } catch (_e) { /* soft */ }
    const zclNode = this.zclNode;
    if (zclNode && isOneBtnSceneKnob) {
      try {
        const DOM = require('../zigbee/DeviceOperatingMode');
        const r = await DOM.applyDesiredMode(this, zclNode);
        this.log?.(`[PHYSICAL] P2450 mode after identity: ${r.desired || r.skipped} via=${r.via || r.ok}`);
      } catch (modeErr) {
        this.log?.(`[PHYSICAL] P2450 mode apply failed: ${modeErr.message}`);
      }
      if (typeof this._universalSceneModeSwitch === 'function') {
        await this._universalSceneModeSwitch(zclNode).catch(() => {});
      }
    }
    this._cleanupPhysicalButtonDetection();
    // WHY(P2683): identity re-arm must reinstall hybrid — flag stayed true after first
    // init (often before zb_manufacturer_name) → dead 0xFD path on vsxvaj9i/dzwgk7e2
    try {
      this._wallSceneRemoteHybridInstalled = false;
    } catch (_e) { /* soft */ }
    ...
    if (zclNode && /button_wireless|scene_switch|remote_button/i.test(driverId)) {
      try {
        const { installWallSceneRemoteHybrid } = require('../devices/WallSceneRemoteHybridInit');
        const maxB = Number(this.buttonCount || this.gangCount || profile?.buttonCount || 2) || 2;
        ...
        this.log?.(`[PHYSICAL] P2683 hybrid re-arm soft: ${e.message}`);
      }
    }
  }
  ...
  async onEndDeviceAnnounce() {
    ...
    const wakeProfile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
    // WHY(P2733 / Bastien mesh TX 40–59%): snappy remotes — listen-only on wake.
    // Contre quoi: fire-and-forget bind×EP×cluster still floods Homey Zigbee queue
    // and makes the next press wait ~7s (Bastien IG).
    const snappySleepy = !!(wakeProfile?.snappyRelayFlow || wakeProfile?.skipBatteryReporting || wakeProfile?.noEf00Tx);
    if (snappySleepy) {
      this.log('[PHYSICAL] P2733 snappy wake — re-arm 0xFD only (no mass bind)');
      // WHY(P2739): do NOT force TuyaMagicPacket on snappy wake — burns CR2032 +
      // Homey TX queue (Bastien diag WAKE-UP / mcuVersionRequest class).
      try {
        this._onOffFdBoundClusterInitialized = false;
        this._onOffFdRawCatcherHooked = false;
        void this._setupOnOffFdBoundCluster(zclNode).catch(() => {});
    ...
    // WHY(P2312): phantom-EP 1-btn remotes (Peter mrpevh8p+TS0041) still fire 0xFD on EP2–4
    const map1Wake = !!(wakeProfile?.mapAllEndpointsToButton1 || wakeProfile?.collapsePhantomEndpoints);
    const gangWake = Math.max(Number(this.gangCount) || 1, Number(this.buttonCount) || 1);
    const epWakeKeys = Object.keys(zclNode.endpoints || {}).map(Number).filter((n) => n >= 1 && n <= 8);
    const endpoints = map1Wake
      ? Math.max(gangWake, ...epWakeKeys, 4)
      : gangWake;
    if (map1Wake) {
      this.log(`[PHYSICAL] P2312 wake bind EP1–${endpoints} (map1 / phantom collapse)`);
    }
    ...
    // P116 / P2685: on wake, re-probe battery ONLY if profile allows TX.
    // WHY(P2685 / Bastien + Z2M#8072): skipBatteryReporting remotes (TS0041 axpdxqgu /
    // mrpevh8p) — never readAttributes powerCfg on announce (burns CR2032/CR2450).
    // Prefer store restore; ZclBatteryMonitor listeners stay passive.
    try {
      if (this.hasCapability?.('measure_battery') && !this.mainsPowered) {
        const profile = typeof this.getDeviceProfile === 'function' ? this.getDeviceProfile() : null;
        let skipBattTx = !!(profile?.skipBatteryReporting || profile?.noEf00Tx);
        try {
          const { shouldSkipSleepyRemoteBatteryTx } = require('../zigbee/PowerClusterPolicy');
          // WHY(P2727): Bastien early boot gate — skip powerCfg even if profile slow
          skipBattTx = shouldSkipSleepyRemoteBatteryTx(profile, { homey: this.homey });
        } catch (_e) { /* soft */ }
        ...
        if (typeof stored === 'number' && Number.isFinite(stored)
          ...
          // WHY(P2461): skipThrottle — UI `?` with store 100% was blocked as duplicate_value
          if (typeof this.safeSetCapabilityValue === 'function') {
            await this.safeSetCapabilityValue('measure_battery', stored, { skipThrottle: true }).catch(() => {});
          }
        }
        if (skipBattTx) {
          this.log?.('[PHYSICAL] P2685 skip announce powerCfg read (skipBatteryReporting / pile)');
        } else {
        ...
        // WHY(P2461): skip EF00 DP query for noEf00 / skipBatteryReporting profiles
        try {
          if (!skipBattTx) {
