...
const PhysicalButtonMixin = (SuperClass) => {
  ...
  async _sendTuyaMagicPacket(zclNode) {
    try {
      ...
      await basic.readAttributes([
        ...
        this.log(' Tuya magic packet sent (device stays paired/reporting)');
      }).catch(() => {});
    } catch (e) {
      // best-effort only
    }
  ...
  async initSceneModeManagement(zclNode) {
    ...
    if (mode === 'dimmer') {
      this.log('[PHYSICAL-MODE] ℹ Manual Dimmer mode selected, skipping switch');
      return;
    }
    ...
    if (needsSwitch) {
      this.log(' Device needs scene mode switching');
