...
const PhysicalButtonMixin = (SuperClass) => {
  ...
  async _sendTuyaMagicPacket(zclNode) {
    try {
      if (this._magicPacketSent) {return;}
      this._magicPacketSent = true;
      const mfr = this.getSetting?.('zb_manufacturer_name') ||
                  this.getStoreValue?.('zb_manufacturer_name') || '';
      if (!/^_(T[ZY]|TY)/i.test(mfr) && !/tuya|moes|hobeian|bseed|zemismart|lonsonho/i.test(mfr)) {return;}
      const basic = zclNode?.endpoints?.[1]?.clusters?.basic;
      if (!basic || typeof basic.readAttributes !== 'function') {return;}
      // Attrs: manufacturerName(0x0004), zclVersion(0x0000), appVersion(0x0001),
      //        modelId(0x0005), powerSource(0x0007), attrReportingStatus(0xfffe)
      await basic.readAttributes([
        'manufacturerName', 'zclVersion', 'appVersion',
        'modelId', 'powerSource', 0xfffe
      ]).then(() => {
        this.log('[PHYSICAL] ✨ Tuya magic packet sent (device stays paired/reporting)');
      }).catch(() => {});
    } catch (e) {
      // best-effort only
    }
  ...
  async initPhysicalButtonDetection(zclNode) {
    ...
    if (this._physicalButtonDetectionInitialized) {
      this.log('[PHYSICAL] ⚠️ Already initialized, skipping duplicate setup');
      return;
    }
  ...
  _isDebounced(gang = 1) {
    ...
    if (now - lastForGang < debounceMs) {
      this.log(`[PHYSICAL] 🛡️ Debounce actif gang ${gang}: skip (${now - lastForGang}ms < ${debounceMs}ms)`);
      return true;
    }
  ...
  _safeTriggerFlow(triggerId, tokens = {}, debug = {}) {
    try {
      ...
      if (typeof triggerCard?.trigger !== 'function') {
        this.log(`[PHYSICAL] ⚠️ Flow '${candidates[0]}' trigger method unavailable`);
        return Promise.resolve(false);
      }
    ...
    if (clusterId === 5 || clusterId === 6 || clusterId === 8 || clusterId === 0xE000 || clusterId === 0xE001) {
      ...
      if (clusterId === 0xE001 && cmd === 0) {
        ...
        if (action === undefined || action > 2) {
          this.log(`[RX-RAW L1] ❓ E001 trame non reconnue (len=${frameData.length}): ${JSON.stringify(frameData).slice(0, 80)}`);
          return false;
        }
        const pressType = action === 0 ? 'single' : action === 1 ? 'double' : 'long';
        ...
        if (nowE1 - (this._sceneDedup[dedupKeyE1] || 0) < 350) {
            this.log(`[RX-RAW L1] 🚫 E001 Button ${button} DEBOUNCED`);
            return true;
        }
        this._sceneDedup[dedupKeyE1] = nowE1;
        this.log(`[RX-RAW L1] 🔘 E001 Button ${button} ${pressType.toUpperCase()} PRESS (HOBEIAN)`);
        if (typeof this._triggerPhysicalFlow === 'function') {
          this._triggerPhysicalFlow(button, pressType);
        }
        return true;
