...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  async initPhysicalButtonDetection(zclNode) {
    ...
    // v5.11.218 BACKPORT master v9.0.49: fallback robuste (this.homey?.setInterval || setInterval)
    this._metricsSyncInterval = (this.homey?.setInterval || setInterval)(() => {
  ...
  _scheduleSceneModeRecovery(zclNode) {
    ...
    // v5.11.218 BACKPORT master v9.0.49: fallback robuste setInterval
    this._sceneRecoveryTimer = (this.homey?.setInterval || setInterval)(async () => {
   ...
   * v5.5.996: Enhanced to support ZCL, Tuya DP, and modes
   */
  async _setupGangPhysicalDetection(zclNode, gang) {
    ...
    // v5.5.996: Support Tuya DP devices (packetninja support)
    // ════════════════════════════════════════════════════════════════════════
  ...
  _triggerPhysicalFlow(gang, pressType, tokens = {}) {
    ...
    if ((pressType === 'on' || pressType === 'off') && (sceneMode === 'magic' || sceneMode === 'both' || sceneMode === 'auto')) {
      ...
      try {
        this.homey.flow.getDeviceTriggerCard(sceneCardId).trigger(this, { action: pressType, gang }, {}).catch(() => {});
