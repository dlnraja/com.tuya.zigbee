...
const PhysicalButtonMixin = (SuperClass) => {
  ...
  _triggerPhysicalFlow(gang, pressType, tokens = {}) {
    ...
    // WHY(P2707 / Bastien): snappy skipUiPulse remotes — charter sync storms crash
    // Homey and lag TS0041 when TS0042/43 fire. Flow-only path is enough.
    if (tokens?.source !== 'virtual') {
      let skipCharter = false;
      try {
        const p = this.getDeviceProfile?.() || {};
        skipCharter = !!(p.skipUiPulse || (p.snappyRelayFlow && (Number(p.buttonCount) || Number(this.buttonCount) || 0) > 1));
      } catch (_e) { /* soft */ }
      if (!skipCharter) {
        try {
