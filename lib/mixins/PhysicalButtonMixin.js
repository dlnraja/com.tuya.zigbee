...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  async initPhysicalButtonDetection(zclNode) {
    ...
    if (this._metricsSyncInterval) {try { (this.homey?.clearInterval || clearInterval)(this._metricsSyncInterval); } catch (_e) {}}
  ...
  _handleAttributeReport(gang, value, data) {
    ...
    if (value === true) {
      ...
      if (state.clickTimeout) {
        try { (this.homey?.clearTimeout || clearTimeout)(state.clickTimeout); } catch (_e) {}
      }

      // Set timeout to finalize click detection
      // v9.0.49: Fallback robuste (this.homey?.setTimeout || setTimeout)
      state.clickTimeout = (this.homey?.setTimeout || setTimeout)(() => {
        if (this._destroyed) return;
        this._finalizeClickDetection(gang);
      }, this._timingProfile.doubleClickWindow);
