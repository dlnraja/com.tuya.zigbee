...
const PhysicalButtonMixin = (SuperClass) => {
  ...
  _triggerPhysicalFlow(gang, pressType, tokens = {}) {
    ...
    this._safeTriggerFlow(flowCardId, flowTokens, { pressType, gang }).then(success => {
      // WHY(P2330): also try Nganged physical IDs used by button_wireless_switch / gas_sensor_switch compose
      if (!success && !isButtonDevice && gangCount > 1) {
        const nganged = `${driverId}_${gangCount}gang_physical_gang${gang}_${pressType}`;
        if (nganged !== flowCardId) {
          this._safeTriggerFlow(nganged, flowTokens, { pressType, gang, type: 'nganged' });
        }
      }
