...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  async initPhysicalButtonDetection(zclNode) {
    ...
    if (this._metricsSyncInterval) {clearInterval(this._metricsSyncInterval);}
  ...
  async _setupGangPhysicalDetection(zclNode, gang) {
    ...
    if (onOffCluster && typeof onOffCluster.on === 'function') {
      ...
      try {
        onOffCluster.on('toggle', () => this._triggerPhysicalFlow(gang, 'single'));
        onOffCluster.on('on', () => this._triggerPhysicalFlow(gang, 'single'));
        onOffCluster.on('off', () => this._triggerPhysicalFlow(gang, 'single'));
        ...
        onOffCluster.on('command', (commandName, commandPayload) => {
          if (commandName === '0xFD' || commandName === 'tuyaAction') {
            const payload = commandPayload?.data?.[0] ?? commandPayload;
            const PRESS_MAP = require('../utils/TuyaPressTypeMap').PRESS_MAP;
            const pressType = typeof payload === 'number'
              ? (PRESS_MAP[payload] || 'single')
              : 'single';
            this.log(`[PHYSICAL] Gang ${gang} Tuya 0xFD command: payload=${payload} -> ${pressType}`);
  ...
  _handleAttributeReport(gang, value, data) {
    ...
    // v9.0.40: TSN-level deduplication - filter duplicate Zigbee frames by transactionSequenceNumber
    if (data && data.transactionSequenceNumber !== undefined) {
      const tsn = data.transactionSequenceNumber;
      const lastTSN = this._lastTSN?.get(gang);
      if (lastTSN === tsn) {
        this.log(`[PHYSICAL] Gang ${gang}: Duplicate TSN ${tsn} - skipping`);
        return;
      }
      if (this._lastTSN) {this._lastTSN.set(gang, tsn);}
    }
  ...
  onZigBeeMessage(endpointId, clusterId, frame, meta) {
    ...
    if (clusterId === 5 || clusterId === 6 || clusterId === 8 || clusterId === 0xE000) {
      ...
      if (clusterId === 6 && cmd <= 2) {
        this.log(`[RX-RAW L1] 🔘 Button ${endpointId} OnOff command: ${cmd}`);
        // Only dedup
        const now = Date.now();
