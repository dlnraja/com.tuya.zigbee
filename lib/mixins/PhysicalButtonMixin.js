...
const PhysicalButtonMixin = (SuperClass) => {
  ...
  getDeviceProfile(overrides = null) {
    ...
    const ts004RemoteFallback = (productId, buttonCount, source) => {
      if (!/^TS004[12346F]$/i.test(productId)) { return null; }
      // WHY(P2702): TS004x class floor snappy (was 400 — Bastien button→relay lag)
      const debounceMs = /^TS004[12346F]$/i.test(productId) ? 80 : 120;
      return {
        ...
        buttonCount,
        debounceMs,
        crossPathDedupMs: debounceMs,
        skip8004: true,
        writeSceneAttr: false,
        sceneSwitch: /^TS004[34]$/i.test(productId),
        usesE000: true,
        // WHY(P2691 / Bastien pile 885a9901): class-wide skip powerCfg TX on coin remotes
    ...
    for (const [prefix, count] of Object.entries(driverButtonCount)) {
      if (driverId.includes(prefix) && modelUp.startsWith('TS004')) {
        const fb = ts004RemoteFallback(modelUp, count, `P2387_${prefix}_${modelUp}`);
        if (fb) {
          this.log?.(`[PROFILE] P2387 ${prefix}+${modelUp} fallback`);
          return fb;
        }
      }
    }
