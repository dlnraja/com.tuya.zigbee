...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  _triggerPhysicalFlow(gang, pressType, tokens = {}) {
    ...
    // We also trigger a universal 'action_button_pressed' card if available.
    // This allows ONE flow to handle ANY button from ANY driver.
    try {
    ...
    // v7.0.0: Safe flow card lookup using app centralized getter
    try {
      ...
      if (flowCard) {
        ...
        this.log(`[PHYSICAL] ℹ️ Flow card not available: ${flowCardId}`);
      }
    } catch (err) {
      // Flow card not defined for this driver - this is normal for drivers without physical flow cards
      this.log(`[PHYSICAL] ℹ️ Flow card lookup error: ${flowCardId} (${err.message})`);
      ...
      if (pressType === 'single' || pressType === 'double' || pressType === 'triple') {
        try {
          ...
          const fallbackCard = this.homey.app?._safeGetTriggerCard?.(fallbackId);
          if (fallbackCard) {
            fallbackCard.trigger(this, flowTokens, {}).catch(() => {});
          }
        } catch (e) {
