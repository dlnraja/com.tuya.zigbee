...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  _triggerPhysicalFlow(gang, pressType, tokens = {}) {
    ...
    if ((pressType === 'on' || pressType === 'off') && (sceneMode === 'magic' || sceneMode === 'both' || sceneMode === 'auto')) {
      ...
      try {
        this.homey.flow.getTriggerCard().trigger(this { action: pressType, gang }, {}).catch(() => {});
    ...
    // v5.5.910: Wrap in try-catch - getTriggerCard throws synchronously if card doesn't exist
    try {
      const flowCard = this.homey.flow.getTriggerCard(flowCardId);
      ...
      if (pressType === 'single' || pressType === 'double' || pressType === 'triple') {
        try {
          ...
          const fallbackCard = this.homey.flow.getTriggerCard(fallbackId);
