...
const PhysicalButtonMixin = (SuperClass) => {
   ...
   * TS0041/42/43 are per-endpoint remotes — do NOT write 0x8004.
  ...
  async initSceneModeManagement(zclNode) {
    ...
    if (!family.writeSceneAttr) {
      this.log(`[PHYSICAL-MODE] skip 0x8004 (${family.family})`);
      return;
    }
  ...
  /**
   * Apply Homey button_mode onto TS004F/TS0044. No-op for endpoint remotes / relays.
   */
    ...
    try {
      ...
      if (Array.isArray(declaredTriggers)) {
        const isDeclared = declaredTriggers.some((entry) =>
          typeof entry === 'string' ? entry === candidates[0] : entry?.id === candidates[0]
        );
        if (!isDeclared) {return Promise.resolve(false);}
      }
      const { safeGetFlowCard } = require('../flow/FlowCardGetters');
      const triggerCard = safeGetFlowCard(this.homey, candidates[0], 'trigger')
        || (typeof this.homey.flow.getDeviceTriggerCard === 'function'
          ? this.homey.flow.getDeviceTriggerCard(candidates[0])
          : null);
      if (typeof triggerCard?.trigger !== 'function') {
        this.log(` Flow '${candidates[0]}' trigger method unavailable`);
        return Promise.resolve(false);
      }
      return triggerCard.trigger(this, tokens, {})
        .then(() => true)
        .catch((err) => {
          this.error(`[PHYSICAL] Flow '${candidates[0]}' failed: ${err.message}`);
          return false;
        });
    } catch (err) {
  ...
  async _setupOnOffFdBoundCluster(zclNode) {
      ...
      for (let gang = 1; gang <= gangCount; gang++) {
        ...
        const bc = new OnOffBoundCluster({
          onSetOn: (payload) => {
            ...
            this.log(`[PHYSICAL] EP${gang} OnOffBound 0xFD scene=${payload?.scene} -> ${press}`);
            this._triggerPhysicalFlow(gang, press);
          },
          onSetOff: () => this._triggerPhysicalFlow(gang, 'double'),
          ...
          onToggle: (payload) => {
            if (payload && payload.cmdId === 0xFC) {
              ...
              this.log(`[PHYSICAL] EP${gang} OnOffBound 0xFC -> ${press}`);
              this._triggerPhysicalFlow(gang, press, { rotate: true, direction: dir });
              return;
            }
            this._triggerPhysicalFlow(gang, 'long');
          },
        });
        bc._device = this;
        if (!endpoint.bindings) {endpoint.bindings = {};}
        endpoint.bindings.onOff = bc;
        this.log(`[PHYSICAL] OnOffBoundCluster EP${gang} ready (0xFD)`);
      }
    } catch (e) {
    ...
    try {
      ...
      if (this._onOffFdRawCatcherHooked) {return;}
      const orig = zclNode.handleFrame.bind(zclNode);
      const self = this;
      zclNode.handleFrame = function onOffFdRawCatcher(epId, cId, f, m) {
        try {
          if (cId === 6 || cId === 0x0006) {
            ...
            const gang = (epId >= 1 && epId <= gangCount) ? epId : 1;
            const PRESS_MAP = require('../utils/TuyaPressTypeMap').PRESS_MAP;

            // Z2M/ZHA: 0xFD = tuyaAction (single/double/long); Homey SDK drops mfr cmds natively
            ...
            if (looksFd) {
              ...
              const dk = `raw6_fd_${gang}_${scene}`;
              if (now - (self._sceneDedup[dk] || 0) >= 350) {
                self._sceneDedup[dk] = now;
                self.log(`[PHYSICAL-RAW] EP${gang} cluster6 0xFD scene=${scene} -> ${pt}`);
                self._triggerPhysicalFlow(gang, pt);
              }
            }

            // Z2M/ZHA: 0xFC = rotate (knobs / TS004F) — complementary path Homey misses
            ...
            if (looksFc) {
              ...
              if (now - (self._sceneDedup[dk] || 0) >= 200) {
                self._sceneDedup[dk] = now;
                self.log(`[PHYSICAL-RAW] EP${gang} cluster6 0xFC dir=${dir} -> ${pt}`);
      ...
      this.log('[PHYSICAL] raw cluster-6 0xFD catcher ready');
    } catch (e) {
  ...
  onZigBeeMessage(endpointId, clusterId, frame, meta) {
    ...
    if (clusterId === 5 || clusterId === 6 || clusterId === 8 || clusterId === 0xE000 || clusterId === 0xE001) {
      ...
      if (clusterId === 6 && cmd === 0xFC) {
        ...
        if (typeof this._triggerPhysicalFlow === 'function') {
          const press = dir === 1 ? 'rotate_left' : dir === 2 ? 'rotate_stop' : 'rotate_right';
          this._triggerPhysicalFlow(endpointId, press, { rotate: true, direction: dir });
        }
        return true;
      }
