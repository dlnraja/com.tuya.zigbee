...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  _triggerPhysicalFlow(gang, pressType, tokens = {}) {
      ...
      try {
        ...
        if (flowCard) {
          flowCard.trigger(this, flowTokens, {})
            .catch(err => this.log(`[PHYSICAL] ⚠️ Trigger error for ${flowCardId}: ${err.message}`));
    ...
    if (!triggered) {
      this.log(`[PHYSICAL] ℹ️ No matching flow card found in candidates: ${Array.from(candidates).join(', ')}`);
