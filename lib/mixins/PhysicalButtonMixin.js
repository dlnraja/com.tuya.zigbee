...
const MANUFACTURER_TIMING_PROFILES = {
...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  async initPhysicalButtonDetection(zclNode) {
    // Get timing profile based on manufacturer
    this._timingProfile = this._getTimingProfile();
    this.log(`[PHYSICAL] Timing profile: appWindow=${this._timingProfile.appCommandWindow}ms`);
  ...
  /**
   * Get timing profile based on device manufacturer
   */
  _getTimingProfile() {
    const manufacturerName = this.getSetting?.('zb_manufacturer_name') || 
                             this.getStoreValue?.('manufacturerName') || '';
    
    // Check for specific manufacturer profiles
    for (const [mfr, profile] of Object.entries(MANUFACTURER_TIMING_PROFILES)) {
      if (mfr !== 'default' && manufacturerName.toLowerCase().includes(mfr.toLowerCase())) {
        this.log(`[PHYSICAL] Using ${mfr} timing profile`);
        return profile;
      }
    }
    
    return MANUFACTURER_TIMING_PROFILES.default;
  }
