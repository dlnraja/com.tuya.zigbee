...
const PhysicalButtonMixin = (SuperClass) => class extends SuperClass {
  ...
  getDeviceProfile() {
    ...
    for (const [mfr, profile] of Object.entries(DEVICE_PROFILES)) {
      if (mfr !== 'default' && TU.containsCI(manufacturerName, mfr)) {
    ...
    try {
      ...
      if (Array.isArray(declaredTriggers)) {
        const isDeclared = declaredTriggers.some((entry) => 
          typeof entry === 'string' ? entry === triggerId : entry?.id === triggerId
        );
  ...
  onZigBeeMessage(endpointId, clusterId, frame, meta) {
    ...
    if (clusterId === 5 || clusterId === 6 || clusterId === 8 || clusterId === 0xE000 || clusterId === 0xE001) {
      ...
      if (clusterId === 0xE000 && cmd === 0) {
        // v5.12.53 (P92.121): 0-based vs 1-based — frameData[3]===0 is a
      ...
      if (clusterId === 0xE001 && cmd === 0) {
        // v5.12.53 (P92.121): same 0-based fix as E000 (see above).
        const rawButton = frameData[3];
