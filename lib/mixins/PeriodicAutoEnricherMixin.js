...
const PeriodicAutoEnricherMixin = {
  ...
  initPeriodicAutoEnricher() {
    ...
    this._initialScanTimeout = this.homey.setTimeout(async () => { if (this._destroyed) return; await this._refreshExistingValues(); }, 600000);

    // Then every 4 hours
    this._hourlyScanInterval = this.homey.setInterval(async () => { if (this._destroyed) return; await this._refreshExistingValues(); }, 14400000);

    this.log('[ENRICH] Value refresh scheduled (10min + 4h)');
  },

  async _refreshExistingValues() {
    if (!this.zclNode || this._destroyed) {return;}
  ...
  cleanupPeriodicEnricher() {
    if (this._initialScanTimeout) {this.homey.clearTimeout(this._initialScanTimeout);}
    if (this._hourlyScanInterval) {this.homey.clearInterval(this._hourlyScanInterval);}
  },
};
