...
const REFRESH_MAP = {
  ...
  measure_luminance: {
    ...
    attr: 'measuredValue',
    xform: v => (v > 0 ? Math.round(Math.pow(10, safeDivide(v - 1, 10000))) : 0),
    validate: v => v >= 0 && v <= 100000,
  },
  measure_battery: {
    clusters: ['powerConfiguration'],
    attr: 'batteryPercentageRemaining',
    xform: v => Math.min(100, Math.round(safeParse(v))),
...
const PeriodicAutoEnricherMixin = {
  ...
  async _refreshExistingValues() {
    ...
    for (const [cap, cfg] of Object.entries(REFRESH_MAP)) {
      ...
      try {
        ...
        const raw = result?.[cfg.attr];if (raw === undefined || raw === null) continue;
  ...
  _findRefreshCluster(clusterNames) {
    const endpoints = this.zclNode?.endpoints || {};for (const ep of Object.values(endpoints)) {
