  ...
  async _initSmartEnrichment() {
    ...
    try {
      ...
      if (!modelId || !manufacturer) {
        this.log('⚠️ [SMART-ENRICH] Missing modelId or manufacturer, skipping');
        return;
      }
      ...
      if (!dbInfo) {
        this.log('ℹ️ [SMART-ENRICH] No database record found for this model/mfr');
        return;
      }
      ...
      // 3. Check for specific driver recommendation (Self-Healing)
      const currentDriver = this.driver?.id;
      if (dbInfo.driver && dbInfo.driver !== currentDriver && dbInfo.driver !== 'universal_zigbee') {
        this.log(`⚠️ [SMART-ENRICH] Current driver (${currentDriver}) differs from recommended (${dbInfo.driver})`);
        // Trigger migration assistant if not already in specialized driver
        if (currentDriver === 'universal_zigbee') {
          this.log('   💡 This device should migrate to its specialized driver for full support.');
          // Migration logic is handled by SmartDriverAdaptation, but we highlight it here
        }
      }
