...
async function handleSonoffEwlSettings(device, key, value) {
  const cl = device._sonoffEwlCluster;
  if (!cl) {return false;}
  try {
    if (key === 'sonoff_network_led')
      ...
      const m2 = { edge: 0, pulse: 1, following: 2 };
      ...
      const t = Math.round((parseFloat(value) || 1) * 2);
...
async function setupSonoffEwelink(device, zclNode) {
  ...
  if (vals[A.networkLed] !== undefined)
    {device.setSettings({ sonoff_network_led: vals[A.networkLed] ? 'on' : 'off' }).catch(() => device.log('[SONOFF-EWL] setSettings network_led failed')); }
  if (vals[A.radioPower] !== undefined)
    {device.setSettings({ sonoff_turbo_mode: vals[A.radioPower] === 0x14 ? 'on' : 'off' }).catch(() => device.log('[SONOFF-EWL] setSettings turbo_mode failed')); }
  if (vals[A.detachRelayMode] !== undefined)
    {device.setSettings({ sonoff_detach_relay: vals[A.detachRelayMode] ? 'on' : 'off' }).catch(() => device.log('[SONOFF-EWL] setSettings detach_relay failed')); }
  if (vals[A.externalTriggerMode] !== undefined) {
    const tmap = { 0: 'edge', 1: 'pulse', 2: 'following' };
    device.setSettings({ sonoff_external_trigger: tmap[vals[A.externalTriggerMode]] || 'edge' }).catch(() => device.log('[SONOFF-EWL] setSettings external_trigger failed'));
  }
