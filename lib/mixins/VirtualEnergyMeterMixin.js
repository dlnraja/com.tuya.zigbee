...
const VirtualEnergyMeterMixin = {
  ...
  async _updateVirtualEnergy() {
    ...
    if (allowEstimate) {
      if (this.hasCapability('measure_power') && !(smart?.isRealCapability?.('measure_power'))) {
        await this._setEstimatedCap('measure_power', Math.round(currentPower * 10) / 10, 'estimated');
      }
      if (this.hasCapability('meter_power') && !(smart?.isRealCapability?.('meter_power'))) {
        await this._setEstimatedCap('meter_power', Math.round(totalEnergy * 1000) / 1000, 'calculated');
      }
      if (this.hasCapability('measure_voltage') && !(smart?.isRealCapability?.('measure_voltage'))) {
        await this._setEstimatedCap('measure_voltage', this._virtualVoltage, 'estimated');
      }
      if (this.hasCapability('measure_current') && !(smart?.isRealCapability?.('measure_current'))) {
        await this._setEstimatedCap('measure_current', Math.round(currentA * 100) / 100, 'calculated');
      }
    }
  ...
  /**
   * Internal capability setter — stamps provenance estimated|calculated (P2557)
   */
  async _setEstimatedCap(capabilityId, value, origin = 'estimated') {
    if (this._destroyed) {return;}
    const o = origin === 'calculated' ? 'calculated' : 'estimated';
    await this._safeSetCapability(capabilityId, value, { source: o, origin: o });
    try {
      const DeviceTelemetryEstimator = require('../utils/DeviceTelemetryEstimator');
      await DeviceTelemetryEstimator.record(this, capabilityId, value, {
        source: o,
        origin: o,
        via: `virtual-meter-mixin:${o}`,
      });
    } catch (_e) { /* optional */ }
  },
  ...
  async _safeSetCapability(capabilityId, value, options = {}) {
    if (this._destroyed) {return;}
    const source = options.origin || options.source || 'estimated';
    try {
      const { commitCapability } = require('../layers/commitCapability');
      return commitCapability(this, capabilityId, value, source, 0.4);
    } catch (_e) {
