...
const VirtualEnergyMeterMixin = {
  ...
  async _initVirtualEnergy() {
    ...
    this._virtualEnergyTimer = this.homey.setInterval(() => { if (this._destroyed) {return;} this._updateVirtualEnergy().catch(err => this.error(err)); }, 300000);
  ...
  async _setEstimatedCap(capabilityId, value) {
    ...
    await this._safeSetCapability(capabilityId, value);
    try {
  ...
  async _safeSetCapability(capabilityId, value) {
    ...
    if (typeof this.safeSetCapabilityValue === 'function') {
      return this.safeSetCapabilityValue(capabilityId, value).catch(() => {});
    }
