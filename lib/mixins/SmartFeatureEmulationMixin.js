...
const SmartFeatureEmulationMixin = {
  ...
  _setupStateRecovery() {
    this.on('capability:onoff', (value) => {
      ...
      if (this.getSetting('enable_state_recovery') === true) {
        this.setStoreValue('last_onoff_state', value).catch(() => {});
      }
    });

    // On Init, check if we should restore
    const lastValue = this.getStoreValue('last_onoff_state');
    if (this.getSetting('enable_state_recovery') === true && lastValue !== undefined) {
      safeSetTimeout(this, async () => {
        ...
        if (current !== lastValue) {
          ...
          await this.safeSetCapabilityValue('onoff', lastValue).catch(() => {});
        }
  ...
  _setupMotionCooldown() {
    ...
    this.on('capability:alarm_motion', (value) => {
      ...
      if (value === false && holdTime > 0) {
        ...
        if (this._motionCooldownTimeout) safeClearTimeout(this, this._motionCooldownTimeout);
        this._motionCooldownTimeout = safeSetTimeout(this, () => {
          if (this._destroyed) return;
  ...
  _setupDoubleClickEmulation() {
    ...
    this.on('capability:button', (value) => {
      ...
      if (this._clickTimer) safeClearTimeout(this, this._clickTimer);

      this._clickTimer = safeSetTimeout(this, () => {
        if (this._destroyed) return;
  ...
  _setupAutoOffTimer() {
    this.on('capability:onoff', (value) => {
      ...
      if (value === true && duration > 0) {
        ...
        if (this._autoOffTimeout) safeClearTimeout(this, this._autoOffTimeout);
        this._autoOffTimeout = safeSetTimeout(this, async () => {
          if (this._destroyed) return;
          this.log('[SMART-TIMER] Reached duration, turning off...');
          await this.safeSetCapabilityValue('onoff', false).catch(() => {});
        }, safeMultiply(duration, 60000));
      } else if (this._autoOffTimeout) {
        safeClearTimeout(this, this._autoOffTimeout);
      }
    });
  },
  ...
  _handleChildLock(capability, value, source = 'tuya') {
    ...
    if (isLocked && source === 'tuya') {
       ...
       safeSetTimeout(this, () => {
         if (this._destroyed) return;
         this.safeSetCapabilityValue(capability, prevValue).catch(() => {});
       }, 500);
       return true; // Handled
    }
    return false;
  ...
  async _applyNightMode() {
    ...
    if (isNight) {
      const nightDim = safeDivide(parseInt(this.getSetting('night_mode_dim')) || 10, 100);
      ...
      if (this.getSetting('enable_soft_transitions') === true) {
        ...
        await this.setCapabilityValue('dim', nightDim).catch(() => {});
      }
    }
  },
  ...
  async _softTransition(capability, targetValue, durationMs = 1500) {
    ...
    const steps = 8;
    const stepValue = safeDivide(targetValue - startValue, steps);
    ...
    for (let i = 1; i <= steps; i++) {
      const next = Math.max(0, Math.min(1, startValue + safeMultiply(stepValue, i)));
  ...
  _setupVacationMode() {
    ...
    // Check every hour
    if (this._vacationTimer) this.homey.clearInterval(this._vacationTimer);
    this._vacationTimer = this.homey.setInterval(() => {
      if (this._destroyed) return;
      ...
      if (hour >= 18 && hour <= 23 && Math.random() < 0.15) {
        ...
        this.safeSetCapabilityValue('onoff', target).catch(() => {});
      }
    }, 3600000);
  },
  ...
  _setupInchingMode() {
    this.on('capability:onoff', (value) => {
      ...
      if (value === true && inchingTime > 0) {
        ...
        if (this._inchingTimeout) safeClearTimeout(this, this._inchingTimeout);
        this._inchingTimeout = safeSetTimeout(this, async () => {
          if (this._destroyed) return;
          await this.safeSetCapabilityValue('onoff', false).catch(() => {});
  ...
  _setupCircadianLighting() {
    ...
    if (this._circadianTimer) this.homey.clearInterval(this._circadianTimer);
    this._circadianTimer = this.homey.setInterval(() => {
      if (this._destroyed) return;
  ...
  async _applyCircadianCT() {
    ...
    // 06:00 - 09:00: Warm to Cool (0.8 -> 0.3)
    // 09:00 - 17:00: Cool (0.3)
    // 17:00 - 21:00: Cool to Warm (0.3 -> 0.9)
    // 21:00 - 06:00: Warm (1.0)

    if (hour >= 6 && hour < 9) {
      // Morning: Warm (0.8) -> Cool (0.3) over 3 hours
      const progress = safeDivide(hour - 6, 3);
      targetCT = 0.8 - safeMultiply(progress, 0.5);
    } else if (hour >= 9 && hour < 17) {
      targetCT = 0.3;
    } else if (hour >= 17 && hour < 21) {
      // Evening: Cool (0.3) -> Warm (0.9) over 4 hours
      const progress = safeDivide(hour - 17, 4);
      targetCT = 0.3 + safeMultiply(progress, 0.6);
    } else {
      targetCT = 1.0;
    }

    this.log(`[SMART-CIRCADIAN] Applying Adaptive CT for hour ${hour}: ${targetCT.toFixed(2)}`);
    await this.safeSetCapabilityValue('light_temperature', targetCT).catch(() => {});
  },
