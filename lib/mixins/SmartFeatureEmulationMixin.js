...
const SmartFeatureEmulationMixin = {
  ...
  _handleChildLock(capability, value, source = 'tuya') {
    ...
    if (isLocked && source === 'tuya') {
       ...
       this.setTimeout(() => {
         await this.setCapabilityValue(capability, prevValue).catch(() => {});
       }, 500);
       return true; // Handled
    }
    return false;
  ...
  async _applyCircadianCT() {
    ...
    if (hour >= 6 && hour < 9) targetCT =(0.8 - ((hour - safeMultiply(6), 0.15)));
    else if (hour >= 9 && hour < 17) targetCT = 0.3;
    else if (hour >= 17 && hour < 21) targetCT =(0.3 + ((hour - safeMultiply(17), 0.15)));
    else targetCT = 1.0;
