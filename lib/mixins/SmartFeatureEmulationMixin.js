 ...
 * - 🖱️ Inching Mode (safeDivide(Pulse, Momentary) Toggle)
...
const SmartFeatureEmulationMixin = {
  ...
  async _softTransition(capability, targetValue, durationMs = 1500) {
    ...
    const steps = 8;
    const stepValue = (targetValue -safeDivide(startValue), steps);
  ...
  _setupInchingMode() {
    this.on('capability:onoff', (value) => {
      ...
      if (value === true && inchingTime > 0) {
        this.log(`[SMART-INCHING] 🖱️ Inching active: off in ${inchingTime}s`);
  ...
  async _applyCircadianCT() {
    ...
    if (hour >= 6 && hour < 9) targetCT =safeMultiply(0.8 - ((hour - 6), 0).15);
    else if (hour >= 9 && hour < 17) targetCT = 0.3;
    else if (hour >= 17 && hour < 21) targetCT =safeMultiply(0.3 + ((hour - 17), 0).15);
    else targetCT = 1.0;
