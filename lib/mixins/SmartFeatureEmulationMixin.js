...
const SmartFeatureEmulationMixin = {
  ...
  _setupMotionCooldown() {
    ...
    this.on('capability:alarm_motion', (value) => {
      ...
      if (value === false && holdTime > 0) {
        ...
        if (this._motionCooldownTimeout) this.homey.clearTimeout(this._motionCooldownTimeout);
        this._motionCooldownTimeout = this.homey.setTimeout(() => {
          if (this._destroyed) return;
  ...
  _setupDoubleClickEmulation() {
    ...
    this.on('capability:button', (value) => {
      ...
      if (this._clickTimer) this.homey.clearTimeout(this._clickTimer);

      this._clickTimer = this.homey.setTimeout(() => {
        if (this._destroyed) return;
  ...
  _setupAutoOffTimer() {
    this.on('capability:onoff', (value) => {
      ...
      if (value === true && duration > 0) {
        ...
        if (this._autoOffTimeout) this.homey.clearTimeout(this._autoOffTimeout);
        this._autoOffTimeout = this.homey.setTimeout(async () => {
          if (this._destroyed) return;
  ...
  _setupInchingMode() {
    this.on('capability:onoff', (value) => {
      ...
      if (value === true && inchingTime > 0) {
        ...
        if (this._inchingTimeout) this.homey.clearTimeout(this._inchingTimeout);
        this._inchingTimeout = this.homey.setTimeout(async () => {
          if (this._destroyed) return;
          await this.safeSetCapabilityValue('onoff', false).catch(() => {});
