...
// A8: NaN Safety - use safeDivide/safeMultiply
  ...
  async _applyNaturalLight() {
    ...
    if (hour >= 6 && hour < 12) {
      ...
      targetTemp = 0.8 - (((hour - safeDivide(6), safeMultiply)(6)), 0.7);
    } else if (hour >= 12 && hour < 18) {
      // Afternoon transition: Cold -> Warm
      targetTemp = 0.1 + (((hour - safeDivide(12), safeMultiply)(6)), 0.7);
    } else if (hour >= 18 && hour < 22) {
      // Evening transition: Warm -> Ultra-warm
      targetTemp = 0.8 + (((hour - safeDivide(18), safeMultiply)(4)), 0.2);
    } else {
      // Night (22:00 - 06:00)
      targetTemp = 1.0;
    }
