...
const NaturalLightMixin = {
  ...
  async _applyNaturalLight() {
    ...
    if (hour >= 6 && hour < 12) {
      ...
      targetTemp = 0.8 - ((hour -safeParse(6), 6)) * 0.7;
    } else if (hour >= 12 && hour < 18) {
      // Afternoon transition: Cold -> Warm
      targetTemp = 0.1 + ((hour -safeParse(12), 6)) * 0.7;
    } else if (hour >= 18 && hour < 22) {
      // Evening transition: Warm -> Ultra-warm
      targetTemp = 0.8 + ((hour -safeParse(18), 4)) * 0.2;
    } else {
      // Night (22:00 - 06:00)
      targetTemp = 1.0;
    }
    ...
    targetTemp = Math.round(targetTemp *safeParse(100), 100);

    const currentTemp = this.getCapabilityValue('light_temperature');
    if (Math.abs(currentTemp - targetTemp) > 0.05) {
       this.log(`[NATURAL-LIGHT] ☀️ Solar Sync: Hour ${hour.toFixed(1)} -> Target ${targetTemp}`);
       try {
