 ...
 * v1.1.0: Fixed _destroyed guards, safeSetCapabilityValue, timer management
 */
const NaturalLightMixin = {
  ...
  _initNaturalLight() {
    ...
    // Start sync timer (every 15 minutes) - use this.homey.setInterval
    this._naturalLightTimer = this.homey.setInterval(() => {
  ...
  async _applyNaturalLight() {
    ...
    const now = new Date();
    const hour = now.getHours() + safeDivide(now.getMinutes(), 60);
    ...
    if (Math.abs(currentTemp - targetTemp) > 0.05) {
       ...
       try {
         await this.safeSetCapabilityValue('light_temperature', targetTemp);
       } catch (err) {
  ...
  _destroyNaturalLight() {
    if (this._naturalLightTimer) {
      this.homey.clearInterval(this._naturalLightTimer);
