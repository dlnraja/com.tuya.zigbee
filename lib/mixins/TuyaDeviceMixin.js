...
function TuyaDeviceMixin(Base) {
  return class extends Base {
    ...
    async onNodeInit({ zclNode }) {
      ...
      // ═══════════════════════════════════════════════════════════════
      if (!this._isBatteryDevice()) {
        this.homey.setTimeout(() => { if (this._destroyed) {return;} this._requestAllDPs(); }, 3000);
      }
      ...
      // ═══════════════════════════════════════════════════════════════
      // STEP 7: Initialize Virtual Energy Estimator (if lacking hardware metering)
      // ═══════════════════════════════════════════════════════════════
      if (!this.hasCapability('measure_power') || !this.hasCapability('meter_power')) {
        try {
          ...
          this.error('[TUYA-MIXIN] Failed to initialize Virtual Energy Estimator:', e.message);
        }
      }
    ...
    async _setupBattery(zclNode) {
      ...
      // P212: do NOT invent 100% phantom battery — wait for real ZCL/DP/IAS samples
    }
    ...
    async handleDP(dp, value) {
      ...
      // v9.0.383: contrôles indépendants par capability (le switch avait 3x case 1,
      // rendant les blocs température/eau inaccessibles)
      const { commitCapability } = require('../layers/commitCapability');

      if (dp === 1 && this.hasCapability('onoff')) {
        await commitCapability(this, 'onoff', !!value, 'tuya-dp');
      }

      // Battery (various DPs)
      if ([4, 10, 14, 15, 101].includes(dp) && this.hasCapability('measure_battery')) {
        let percent = Math.max(0, Math.min(100, value));
        if (this.batteryHandler && typeof this.batteryHandler.trackTuyaDP === 'function') {
          const calculatedPercent = this.batteryHandler.trackTuyaDP(dp, value);
          if (calculatedPercent !== null) {
            percent = calculatedPercent;
          }
        }
        if (typeof this.ingestBatteryPercent === 'function') {
          await this.ingestBatteryPercent(percent, { protocol: 'tuya-dp', dp });
        } else {
          await commitCapability(this, 'measure_battery', parseFloat(percent), 'tuya-dp');
        }
      }

      // Temperature (usually DP 101 or 1, scaled by 10)
      if ((dp === 1 || dp === 101) && this.hasCapability('measure_temperature') && typeof value === 'number') {
        const temp = value / 10;
        if (temp > -50 && temp < 100) {
          await commitCapability(this, 'measure_temperature', parseFloat(temp), 'tuya-dp');
        }
      }

      // Humidity (usually DP 102 or 2)
      if ((dp === 2 || dp === 102) && this.hasCapability('measure_humidity') && typeof value === 'number') {
        const hum = value > 100 ? value / 10 : value;
        if (hum >= 0 && hum <= 100) {
          await commitCapability(this, 'measure_humidity', parseFloat(hum), 'tuya-dp');
        }
      }

      // Voltage (DP 247 in mV)
      if (dp === 247 && this.hasCapability('measure_voltage')) {
        const voltage = value / 1000;
        if (voltage > 0 && voltage < 300) {
          await commitCapability(this, 'measure_voltage', parseFloat(voltage), 'tuya-dp');
        }
      }

      // Water alarm (DP 1 for water sensors)
      if (dp === 1 && this.hasCapability('alarm_water')) {
        await commitCapability(this, 'alarm_water', !!value, 'tuya-dp');
      }

      const handled = (dp === 1 && (this.hasCapability('onoff') || this.hasCapability('alarm_water') || this.hasCapability('measure_temperature')))
        || ([4, 10, 14, 15, 101].includes(dp) && this.hasCapability('measure_battery'))
        || ((dp === 2 || dp === 102) && this.hasCapability('measure_humidity'))
        || (dp === 247 && this.hasCapability('measure_voltage'));
      if (!handled) {
    ...
    async sendDP(dp, type, value) {
      // P2699: EF00 missing → complementary soft-fail (raw/PFC), never mandatory throw
      if (!this._tuyaCluster || !this.tuya) {
        this.log?.('[TUYA-MIXIN] Tuya cluster not available — complementary soft-fail (P2699)');
        try {
          const { softArmComplementaryIo } = require('../io/NonNativeComplementary');
          softArmComplementaryIo(this, {});
        } catch (_e) { /* optional */ }
        return false;
      }
    ...
    async setCapability(capability, value) {
      if (this.hasCapability(capability)) {
        return await this.setCapabilityValue(capability, value).catch(err => {
    ...
    async onDeleted() {
      ...
      // Clean up Virtual Energy Estimator
      if (this.virtualEnergyEstimator) {
        try {
          this.virtualEnergyEstimator.destroy();
        } catch (e) {
          this.error('[TUYA-MIXIN] Error destroying Virtual Energy Estimator:', e.message);
        }
      }
