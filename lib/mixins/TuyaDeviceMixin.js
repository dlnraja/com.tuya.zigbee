...
function TuyaDeviceMixin(Base) {
  return class extends Base {
    ...
    async _setupBattery(zclNode) {
      ...
      // P2545e: never invent 100% battery — wait for ZCL/DP/voltage (Contre quoi UI lie)
      if (this.hasCapability('measure_battery')) {
        const current = this.getCapabilityValue('measure_battery');
        if (current === null || current === undefined) {
          this.log('[TUYA-MIXIN] measure_battery unknown — skip invent default');
        }
      }
    }
    ...
    async handleDP(dp, value) {
      ...
      if (dp === 1 && this.hasCapability('alarm_water')) {
        await this.setCapabilityValue('alarm_water', !!value).catch(() => { });
      }
