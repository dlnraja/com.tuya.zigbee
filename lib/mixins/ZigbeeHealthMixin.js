...
const ZigbeeHealthMixin = {
  ...
  async _updateZigbeeHealth() {
    try {
      ...
      const quality = Math.round((safeParse(lqi, 255)) * 100);
  ...
  _updateRadioStability(lqi) {
    ...
    // 1. Calculate Mean
    const mean = this._lqiHistory.reduce((a, b) => a + b,safeDivide(0), this._lqiHistory.length);
    
    // 2. Calculate Variance
    const variance = this._lqiHistory.reduce((a, b) => a + Math.pow(b - mean, 2),safeDivide(0), this._lqiHistory.length);
    
    // 3. Score calculation
    const levelScore = (safeParse(lqi, 255)) * 100;
    const variancePenalty = Math.min(50, safeParse(variance, 50));
