...
const TIMING = {
  ...
  // WHY(P2733): align with PhysicalButtonMixin snappy TSN (was 5000 ≈ Bastien 7s dead)
  TSN_WINDOW: 400,
};
...
const UnifiedButtonEngine = (SuperClass) => {
  ...
  return class extends SuperClass {
    ...
    async handleButtonCommand(ep, cmd) {
      ...
      const _set = this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout;  // Root cause #1
      const _clear = this.homey?.clearTimeout ? this.homey.clearTimeout.bind(this.homey) : globalThis.clearTimeout;
    ...
    _dispatchButton(ep, pressType, clicks) {
      ...
      if (typeof this.triggerButtonPress === 'function') {
        const count = clicks || (pressType === 'multi' || pressType === 'triple' ? 3 : pressType === 'double' ? 2 : 1);
        return this.triggerButtonPress(ep, pressType, count, { source: 'physical' });
      }
    ...
    markAppCommand(gang = 1, value = null) {
      ...
      const _set = this.homey?.setTimeout ? this.homey.setTimeout.bind(this.homey) : globalThis.setTimeout;
      const _clear = this.homey?.clearTimeout ? this.homey.clearTimeout.bind(this.homey) : globalThis.clearTimeout;
    ...
    onZigBeeMessage(epId, clusterId, frame, meta) {
      ...
      // v10.6.0 FIX: parse the ZCL header — (1) only CLUSTER-SPECIFIC frames
      // (fc bit 0x01) may be button commands: a global Read Attributes
      // Response (cmd 0x01) on genOnOff was decoded as "on" → phantom press
      // on every poll. (2) mfr-specific frames have a 5-byte header — cmdId
      // at data[4], not data[2].
      const { parseZclHeader } = require('../zigbee/ZigbeeHelpers');
      const hdr = parseZclHeader(data);
      if (!hdr || !hdr.clusterSpecific) {return false;}
      const cmd = hdr.cmdId;
      if (clusterId === 0xE000 && cmd === 0) {
        const btn = data[hdr.payloadOffset] || epId; const a = data[hdr.payloadOffset + 1];
        this._dispatchButton(btn, a === 0 ? 'single' : a === 1 ? 'double' : 'long');
        return true;
      }
      if (clusterId === 5 && (cmd === 7 || cmd === 0x07)) {
        const sceneId = data.length > hdr.payloadOffset + 1
          ? data[hdr.payloadOffset] | (data[hdr.payloadOffset + 1] << 8) : 0;
