...
async function healZigbeeNodeIdentity(device, opts = {}) {
  ...
  // WHY(P2322): Homey ZigBeeDevice.node often keeps ieee after zclNode went hollow
  try {
    const n = device.node;
    if (n?.ieeeAddress) {candidates.push({ via: 'node.ieeeAddress', v: n.ieeeAddress });}
    if (n?.ieeeAddr) {candidates.push({ via: 'node.ieeeAddr', v: n.ieeeAddr });}
  } catch (_e) { /* noop */ }
