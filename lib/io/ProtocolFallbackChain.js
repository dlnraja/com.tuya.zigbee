  ...
  async transmit(intent = {}) {
    ...
    return this.run('tx', {
      ...
      sdk3_direct: async () => {
        ...
        const ep = this.device?.zclNode?.endpoints?.[intent.ep ?? 1];
        const cluster = ep?.clusters?.[intent.cluster]
          || ep?.clusters?.[String(intent.cluster)];
  ...
  async receive(intent = {}) {
    ...
    return this.run('rx', {
      ...
      raw_frame_parse: async () => {
        ...
        if (!ef00 || typeof ef00.parseIncomingFrame !== 'function' || !intent.frame) {
          return false;
        }
        const parsed = ef00.parseIncomingFrame(intent.frame);
        return parsed ? { source: 'raw_frame', value: parsed } : false;
      },
