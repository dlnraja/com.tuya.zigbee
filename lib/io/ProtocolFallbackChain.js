 ...
 *   TX: wrapper_manager → sdk3_direct → … → capability_command_router → … → flow/ota → poll
 *   RX: capability_listener → zcl/dp/bound/ias → raw → … → flow/energy → poll → antispam
 * P2544: complementary wrappers beyond Homey native zigbee/tuya libs.
 */
...
class ProtocolFallbackChain {
  ...
  async transmit(intent = {}) {
    ...
    return this.run('tx', {
      ...
      // P2544 complementary TX soft hooks (beyond Homey native libs)
      capability_command_router: async () => {
        if (intent.capability == null || intent.value === undefined) {return false;}
        try {
          const { writeCapabilityWithFallbacks } = require('../zigbee/CapabilityCommandRouter');
          if (typeof writeCapabilityWithFallbacks !== 'function') {return false;}
          const r = await writeCapabilityWithFallbacks(
            this.device, intent.capability, intent.value, intent.opts || {}
          );
          return r ? { via: 'capability_command_router', result: r } : false;
        } catch (_e) {
          return false;
        }
      },
      flow_action: async () => {
        if (typeof intent.flowAction !== 'function') {return false;}
        try {
          const r = await intent.flowAction(this.device, intent);
          return r != null ? { via: 'flow_action', result: r } : false;
        } catch (_e) {
          return false;
        }
      },
      ota_firmware: async () => {
        if (!intent.ota) {return false;}
        const ota = this.device?.otaManager || this.device?._ota;
        if (!ota || typeof ota.checkForUpdate !== 'function') {return false;}
        try {
          return await ota.checkForUpdate(intent.opts || {});
        } catch (_e) {
          return false;
        }
      },
  ...
  async receive(intent = {}) {
    ...
    return this.run('rx', {
      ...
      // P2544 complementary RX — soft hooks (skip if no device helper)
      flow_trigger: async () => {
        if (intent.flowValue == null) {return false;}
        return { source: 'flow_trigger', value: intent.flowValue };
      },
      energy_report: async () => {
        const energy = this.device?.energyHandler || this.device?._energyHandler;
        if (!energy || typeof energy.readLatest !== 'function') {return false;}
        try {
          const v = await energy.readLatest(intent);
          return v != null ? { source: 'energy_report', value: v } : false;
        } catch (_e) {
          return false;
        }
      },
      ...
      antispam_filter: async () => {
        // Marker step: real antiflood lives in safeSetCapabilityValue (L14).
        // Returning false keeps cascade non-fatal; presence in order documents coverage.
        if (intent.capability == null || intent.value == null) {return false;}
        if (typeof this.device?.safeSetCapabilityValue !== 'function') {return false;}
        return false;
      },
    }, intent);
  }
