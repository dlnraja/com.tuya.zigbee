...
class DeviceIOFacade {
  ...
  async sendDP(dp, value, opts = {}) {
    try {
      if (shouldSkipIasOnlyEf00Tx(this.device)) {
        this._log(`sendDP skipped (IAS-only / no 61184) dp=${dp}`);
        return false;
      }
  ...
  async bindCluster(ep, cluster) {
    try {
      const isPowerCfg = cluster === 'genPowerCfg' || cluster === 'powerConfiguration'
        || cluster === 1 || cluster === 0x0001 || cluster === '1';
      if (isPowerCfg && shouldSkipIasOnlyEf00Tx(this.device)) {
        this._log(`bindCluster: skip genPowerCfg on IAS-only (INVALID_EP) ep=${ep}`);
        return false;
      }
  ...
  async sendRaw(clusterId, payload, opts = {}) {
    try {
      ...
      if ((cid === 0xEF00 || cid === 61184) && shouldSkipIasOnlyEf00Tx(this.device)) {
        this._log('sendRaw skipped (IAS-only / no 61184)');
        return false;
      }
  ...
  async magicHandshake(opts = {}) {
    try {
      if (shouldSkipIasOnlyEf00Tx(this.device)) {
        this._log('magicHandshake skipped (IAS-only / no 61184)');
        return false;
      }
