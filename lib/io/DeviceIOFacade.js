...
class DeviceIOFacade {
  ...
  _resolveCluster(endpointId, cluster) {
    ...
    // 1. Direct match on ep.clusters (fast path)
    if (ep.clusters[cluster]) {return ep.clusters[cluster];}
    const strKey = String(cluster);
    if (ep.clusters[strKey]) {return ep.clusters[strKey];}

    // 2. Lexicon lookup (maps numeric IDs to SDK names or names to numeric IDs)
    try {
      const { lookupCluster, normalizeClusterId } = require('../zigbee/ZclClusterLexicon');
      const normId = normalizeClusterId(cluster);
      if (normId != null) {
        if (ep.clusters[normId]) {return ep.clusters[normId];}
        if (ep.clusters[String(normId)]) {return ep.clusters[String(normId)];}
        const hex = `0x${normId.toString(16).padStart(4, '0')}`;
        if (ep.clusters[hex]) {return ep.clusters[hex];}
        const meta = lookupCluster(normId);
        if (meta?.names) {
          for (const n of meta.names) {
            if (ep.clusters[n]) {return ep.clusters[n];}
    ...
    // 3. Fallback: case-insensitive scan of ep.clusters keys
