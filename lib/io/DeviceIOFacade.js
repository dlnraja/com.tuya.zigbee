...
class DeviceIOFacade {
  ...
  _attachCompensationLayer(zclNode) {
    try {
      ...
      // P2700: soft-arm raw + RX/TX + PFC for non-native Homey gaps (never mandatory)
      try {
        const { softArmComplementaryIo } = require('./NonNativeComplementary');
        softArmComplementaryIo(this.device, {
          io: this,
          compensation: this._compensation,
          zclNode: zclNode || this.device.zclNode,
        });
    ...
    try {
      ...
      if (!iasOnlySkip && opts.mcu !== false && typeof this.device.initTuyaMcu === 'function') {
        ...
        try {
          ...
          // WHY(P2472b): prefer attach() (negotiate + soft resync) over bare negotiate
          if (typeof this.device.tuyaMcuManager.attach === 'function') {
            await this.device.tuyaMcuManager.attach().catch(() => {});
          } else if (typeof this.device.tuyaMcuManager.negotiate === 'function') {
  ...
  async resolveWifi(intent = {}) {
    try {
      ...
      const opts = {
        ...
        deviceId: intent.deviceId || intent.device_id || intent.id || settings.device_id || settings.deviceId || settings.id || settings.device_id_wifi,
        localKey: intent.localKey || intent.local_key || intent.key || settings.local_key || settings.localKey || settings.key || settings.device_key,
        ip: intent.ip || intent.ip_address || settings.ip || settings.ip_address || settings.device_ip || settings.address,
