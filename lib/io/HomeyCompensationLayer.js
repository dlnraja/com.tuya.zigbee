...
/**
 * HomeyCompensationLayer (P102 Phase 2 / P2250 / P2699)
 *
 * Compensates incomplete Homey interview / SDK cluster coverage.
 * Homey native does NOT expose every ZCL/EF00 bind, report, or TX path —
 * keep parallel low-level wrappers active for ALL inbound/outbound traffic:
 ...
 * - RX cascade: capability_listener → zcl_attr_report → tuya_dp_report →
 *   raw_frame_parse → raw_cluster_fallback → poll_heuristic
 * - TX cascade: wrapper_manager → sdk3_direct → quirk_guided → raw_zcl_frame →
 *   magic_handshake_retry → poll_heuristic
 *
 * P2699: proprietary / non-native DP+clusters are complementary ONLY —
 * never mandatory compose/bind; soft-arm via NonNativeComplementary.
 *
 * Never invents manufacturerNames into compose. Never throws fatal.
 * Pairing identity stays case-insensitive (mfr+pid) via TuyaNormalizer.
 */

const fs = require('fs');
const path = require('path');
const { safeSetInterval, safeClearInterval, safeSetTimeout } = require('../utils/safe-timers');
...
const RUN_LISTENER_REGISTRY = Symbol.for('tuya.flow.runListenerRegistry');

function getRunListenerRegistry() {
  if (!global[RUN_LISTENER_REGISTRY]) {global[RUN_LISTENER_REGISTRY] = new Set();}
  return global[RUN_LISTENER_REGISTRY];
}
...
/**
 * Historical restore (codex-diag-timeouts DriverFlowCardSupport):
 * guard duplicate registerRunListener to avoid Homey "already registered" crashes.
 */
function wrapFlowCardRunListener(card, key) {
  if (!card || typeof card.registerRunListener !== 'function') {return card;}
  if (card.__tuyaRunListenerGuardInstalled) {return card;}

  const original = card.registerRunListener.bind(card);
  ...
  card.registerRunListener = function guardedRegisterRunListener(listener) {
    const registry = getRunListenerRegistry();
    if (registry.has(key)) {return card;}
    try {
      const result = original(listener);
      registry.add(key);
      return result;
    } catch (err) {
...
function safeGetFlowCard(homey, id, type = 'action', declaredIds = null) {
  try {
    const flow = homey?.flow;
    if (!flow || !id) {return NOOP_CARD;}
    // WHY (P2247): never call Homey getters for undeclared IDs — FLOW-GUARD spam
    // Inline CI match (avoid circular require with FlowCardHeuristics).
    if (declaredIds && declaredIds.size) {
      let hit = null;
      if (declaredIds.has(id)) {hit = id;}
      else {
        const needle = String(id).toLowerCase();
        for (const d of declaredIds) {
          if (String(d).toLowerCase() === needle) {hit = d; break;}
        }
      }
      if (!hit) {return NOOP_CARD;}
      id = hit;
    }
    ...
    for (const m of methods) {
      ...
      try {
        ...
        if (card) {return wrapFlowCardRunListener(card, `${type}:${id}`);}
      } catch (_e) { /* try next */ }
    }
    return NOOP_CARD;
  } catch (_e) {
...
function isNoopFlowCard(card) {
  return !card || card.__tuyaFlowNoop === true;
}
...
class HomeyCompensationLayer {
  ...
  async attach(zclNode, opts = {}) {
    try {
      ...
      // P2699: non-native gaps → complementary soft-arm (never mandatory / never throw)
      let complementary = { ok: false, armed: [] };
      try {
        const { softArmComplementaryIo } = require('./NonNativeComplementary');
        complementary = softArmComplementaryIo(this.device, { ...opts, io: this.io, compensation: this });
      ...
      return { ok: true, ...miss, mcu, complementary };
    } catch (err) {
  ...
  async ensureTuyaClusterCompensated(opts = {}) {
    try {
      ...
      await new Promise((r) => {
        const ms = opts.rescanDelayMs != null ? Number(opts.rescanDelayMs) : 1500;
        try {
        ...
        // Prefer destroyed-safe helper over bare setTimeout (syntax-check gate)
        if (this.device) {
          safeSetTimeout(this.device, r, ms);
        } else {
          // module-level native setTimeout — no device / no Homey instance
          setTimeout(r, ms);
        }
      });
  ...
  async negotiateMcu(opts = {}) {
    try {
      ...
      // Prefer curated MCUFormatDatabase (cross-era Z2M/ZHA map) then guessFormat
      let format = null;
      try {
        const MCUFormatDatabase = require('../tuya/MCUFormatDatabase');
        const looked = MCUFormatDatabase.lookup({
          manufacturerName: mfr,
          productId: pid,
          mcuVersion: version || this._mcu.mcuVersion,
        });
        if (looked?.format) {
          format = looked.format;
          this._log('MCUFormatDatabase:', looked.matchType, looked.confidence);
        }
      } catch (_e) { /* optional */ }

      if (!format && TuyaTimeSyncFormats?.guessFormat) {
        format = TuyaTimeSyncFormats.guessFormat({
          manufacturerName: mfr,
          productId: pid,
          mcuVersion: version || this._mcu.mcuVersion,
        });
  ...
  async applyInitSequence(seqId, intent = {}) {
    try {
      ...
      if (seqId === 'mcu_negotiate') {
        return this.negotiateMcu(intent.opts || {});
      }
      if (seqId === 'tuya_time_sync') {
        // P110: time sync must call sendTimeSync — not MCU negotiate
        const ef00 = this.device?.tuyaEF00Manager;
        const zclNode = this.device?.zclNode;
        if (ef00 && typeof ef00.sendTimeSync === 'function' && zclNode) {
          await ef00.sendTimeSync(zclNode, intent.opts || {}).catch(() => null);
          return true;
        }
        if (this.device?.io?.syncTime) {
          return !!(await this.device.io.syncTime(intent.opts || {}).catch(() => false));
        }
        // Soft fallback: negotiate may still wake MCU + guess format
        return this.negotiateMcu(intent.opts || {});
      }
      ...
      if ((seqId === 'metering_divisor_fix' || seqId === 'electrical_current_divisor') && io?.readZcl) {
        ...
        const primary = await io.readZcl(intent.endpoint ?? 1, cluster, attrs);
        if (primary) {return true;}
        // Alternate cluster id forms
        const altCluster = seqId === 'metering_divisor_fix' ? 0x0702 : 0x0B04;
        return !!(await io.readZcl(intent.endpoint ?? 1, altCluster, attrs));
      }
      if (seqId === 'e001_external_switch_read' && io?.readZcl) {
        const a = await io.readZcl(1, 0xE001, [0xD030]);
        if (a) {return true;}
        return !!(await io.readZcl(1, 'tuyaE001', [0xD030]));
      }
      if (seqId === 'ts004f_scene_mode') {
        // Best-effort: ensure onOff present + magic wake; scene cmds are RX-only
        if (io?.magicHandshake) {
          await io.magicHandshake(intent.opts || {}).catch(() => false);
        }
        if (io?.bindCluster) {
          await io.bindCluster(1, 'onOff').catch(() => false);
        }
        return true;
      }
      return false;
  ...
  async applyQuirkGuidedInit(opts = {}) {
    ...
    try {
      ...
      // Case-insensitive mfr quirk lookup (Sacred Couple specificity)
      const quirks = table.mfrQuirks || {};
      let mfrEntry = quirks[mfr] || quirks[mfr.toUpperCase()] || quirks[mfr.toLowerCase()];
      if (!mfrEntry) {
        const mfrLower = mfr.toLowerCase();
        for (const [key, val] of Object.entries(quirks)) {
          if (String(key).toLowerCase() === mfrLower) {
            mfrEntry = val;
            break;
          }
        }
      ...
      const modelExpect = table.modelExpectedClusters?.[pid]
        || table.modelExpectedClusters?.[pid.toUpperCase()];
      ...
      if (/^TS0601$/i.test(pid) || /^_TZE/i.test(mfr)) {
        ...
        seqIds.add('tuya_time_sync');
      }
      if (/^TS020[235]/i.test(pid) || /^TS021[056]/i.test(pid)) {
        seqIds.add('ias_cie_enroll');
      }
      if (/^TS004F$/i.test(pid)) {
        seqIds.add('tuya_magic_packet');
        seqIds.add('ts004f_scene_mode');
        seqIds.add('e001_external_switch_read');
      }
      // WHY(P2253/P2554): TS0041–46 hybrid remotes — magic wake only (Z2M/ZHA/HA).
      // Never queue ts004f_scene_mode (0x8004) — that family rejects attr 32772.
      if (/^TS004[1-6]$/i.test(pid) || /^TS0215A?$/i.test(pid)) {
        seqIds.add('tuya_magic_packet');
      }
      if (/^TS000[1-4]$/i.test(pid)) {
        seqIds.add('tuya_magic_packet');
        seqIds.add('e001_external_switch_read');
      }
      if (/^(TS011F|TS0121)$/i.test(pid)) {
        seqIds.add('tuya_magic_packet');
        seqIds.add('metering_divisor_fix');
        seqIds.add('electrical_current_divisor');
      }
      if (/^TS130F$/i.test(pid)) {
        seqIds.add('tuya_magic_packet');
        seqIds.add('tuya_query_all_dps');
      }
...
module.exports.isNoopFlowCard = isNoopFlowCard;
module.exports.NOOP_CARD = NOOP_CARD;
