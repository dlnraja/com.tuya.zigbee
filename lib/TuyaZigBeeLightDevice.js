...
class TuyaZigBeeLightDevice extends TuyaZigbeeDevice {

    constructor(...args) {
        super(...args);
        // P104: elevate DeviceIOFacade onto light lineage (super already installs; keep idempotent)
        try {
    ...
    async onNodeInit({ zclNode }) {
        // P206: inherit L14 + universal protocol layers from TuyaZigbeeDevice
    ...
    async changeOnOff(onoff) {
        ...
        return this.onOffCluster[onoff ? 'setOn' : 'setOff']()
            .then(async result => {
                if (onoff === false) {
                    await this.setCapabilityValue('dim', 0).catch(err => this.error(err)); // Set dim to zero when turned off
                } else if (onoff) {
                    ...
                    await wait(1000)
                        .then(async () => {
                            ...
                            await this.setCapabilityValue('dim', Math.max(0.01, currentLevel / MAX_DIM)).catch(err => this.error(err));
                        })
                        .catch(err => {
            ...
            return this.levelControlCluster.moveToLevel(moveToLevelCommand)
                .then(async result => {
                    ...
                    if (dim === 0) {
                        ...
                        await this.setCapabilityValue('onoff', true).catch(err => this.error(err));
                    }
                    return result;
                });
        }
    ...
    async changeColorTemperature(temperature, opts = {}) {
        ...
        if (this.hasCapability('light_mode')
            ...
            await this.setCapabilityValue('light_mode', 'temperature').catch(err => this.error(err));
        }
    ...
    async changeColor({ hue, saturation, value }, opts = {}) {
        ...
        if (this.hasCapability('light_mode'
            ...
            await this.setCapabilityValue('light_mode', 'color').catch(err => this.error(err));
        }
    ...
    async onEndDeviceAnnounce() {
        ...
        if (typeof currentHue === 'number' && typeof currentSaturation === 'number') {
            await this.setCapabilityValue('light_hue', currentHue / MAX_HUE).catch(err => this.error(err));
            await this.setCapabilityValue('light_saturation', currentSaturation / MAX_SATURATION).catch(err => this.error(err));
        }

        // Determine the light_mode
        if (this.hasCapability('light_mode')) {
            await this.setCapabilityValue('light_mode', colorMode === 'colorTemperatureMireds' ? 'temperature' : 'color').catch(err => this.error(err));
        }
        ...
        if (typeof colorTemperatureMireds === 'number') {
            await this.setCapabilityValue('light_temperature',1-(colorTemperatureMireds / MAX_COLORTEMPERATURE)).catch(err => this.error(err));
        }
    }
