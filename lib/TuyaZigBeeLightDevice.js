...
const lightModeCapabilityDefinition = {
    ...
    userOpts: {
        ...
        setParser(lightMode, opts = {}) {
            ...
            switch (lightMode) {
                case 'temperature': {
                    // ponytail: BUGFIX - was using undefined `value`; read current temperature from device
                    const currentTemp = this.getCapabilityValue('light_temperature') || 0.5;
                    const colorTemperature = Math.round(currentTemp * MAX_COLORTEMPERATURE);
...
class TuyaZigBeeLightDevice extends ZigBeeDevice {
    ...
    async onNodeInit({ zclNode }) {
        // ponytail: DEBUG_PERMANENT - debug kept intentionally for Tuya light troubleshooting
        this.enableDebug();
    ...
    async changeOnOff(onoff) {
        ...
        return this.onOffCluster[onoff ? 'setOn' : 'setOff']()
            .then(async result => {
                if (onoff === false) {
                    await this.setCapabilityValue('dim', 0).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} })); // Set dim to zero when turned off
                } else if (onoff) {
                    ...
                    await wait(1000)
                        .then(async () => {
                            ...
                            await this.setCapabilityValue('dim', Math.max(0.01, currentLevel / MAX_DIM)).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
                        })
                        .catch(err => {
    ...
    async changeDimLevel(dim, opts = {}) {
        ...
        if (!this.hasCapability('light_mode') && this.hasCapability('light_temperature')) {
            ...
            return this.changeOnOff(false)
                .then(async result => {
                    await this.setCapabilityValue('onoff', false).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
                    return result;
                });
            ...
            return this.levelControlCluster.moveToLevel(moveToLevelCommand)
                .then(async result => {
                    ...
                    if (dim === 0) {
                        ...
                        await this.setCapabilityValue('onoff', true).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
                    }
                    return result;
                });
        }
    ...
    async changeColorTemperature(temperature, opts = {}) {
        ...
        if (this.hasCapability('light_mode')
            ...
            await this.setCapabilityValue('light_mode', 'temperature').catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
        }
    ...
    async changeColor({ hue, saturation, value }, opts = {}) {
        ...
        if (this.hasCapability('light_mode'
            ...
            await this.setCapabilityValue('light_mode', 'color').catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
        }
    ...
    async onEndDeviceAnnounce() {
        ...
        if (typeof currentHue === 'number' && typeof currentSaturation === 'number') {
            await this.setCapabilityValue('light_hue', currentHue / MAX_HUE).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
            await this.setCapabilityValue('light_saturation', currentSaturation / MAX_SATURATION).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
        }

        // Determine the light_mode
        if (this.hasCapability('light_mode')) {
            await this.setCapabilityValue('light_mode', colorMode === 'colorTemperatureMireds' ? 'temperature' : 'color').catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
        }
        ...
        if (typeof colorTemperatureMireds === 'number') {
            await this.setCapabilityValue('light_temperature',1-(colorTemperatureMireds / MAX_COLORTEMPERATURE)).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
        }
    }
