...
const mixin = (BaseClass) => class extends BaseClass {
  ...
  async onButtonPressed(btn = 1) {
    this._lastPressTime = Date.now();
    this._lastPressType = 'short';
    const capsToPulse = [];
    if (this.hasCapability?.(`button.${btn}`)) capsToPulse.push(`button.${btn}`);
    if (btn === 1 && this.hasCapability?.('button')) capsToPulse.push('button');
    if (capsToPulse.length === 0) return;
    const setCap = typeof this.safeSetCapabilityValue === 'function'
      ? (cap, val) => this.safeSetCapabilityValue(cap, val)
      : (cap, val) => this.setCapabilityValue(cap, val);
    for (const cap of capsToPulse) {
      setCap(cap, true).catch(() => {});
      safeSetTimeout(this, () => {
        setCap(cap, false).catch(() => {});
      }, 500);
    }
  }
