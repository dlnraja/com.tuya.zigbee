...
class ZigpyIntegration {
   ...
   */
  async initialize() {
    this.device.log('🔧 ZigpyIntegration initializing...');
    ...
    try {
      ...
      this.device.log('✅ ZigpyIntegration initialized successfully');
      this.device.log(`   OTA Support: ${this.zigpyFeatures.otaSupport ? '✅' : '❌'}`);
  ...
  async checkOTASupport() {
    ...
    if (endpoint.clusters[25] || endpoint.clusters[0x0019]) {
      ...
      this.device.log('✅ OTA Update support detected (Cluster 0x0019)');
      
      // Try to read OTA attributes
      try {
        const otaCluster = endpoint.clusters[25];
        if (otaCluster) {
          const currentVersion = await otaCluster.readAttributes(['currentFileVersion']).catch(() => null);
          if (currentVersion) {
  ...
  async applyKnownQuirks() {
    ...
    // Tuya Magic Spell (zigpy Discussion #823)
    ...
    if (this.device.hasCapability('alarm_contact')) {
      ...
      this.device.log('✅ Quirk applied: Contact sensor inversion');
    }
  }
  ...
  async applyTuyaMagicSpell() {
    try {
      ...
      await basicCluster.readAttributes(magicAttributes).catch(err => {
      ...
      this.device.log('✅ Quirk applied: Tuya Magic Spell (unlock endpoints)');
    ...
    for (const [clusterId, config] of Object.entries(reportingConfigs)) {
      ...
      if (endpoint.clusters[id]) {
        try {
          ...
          this.device.log(`✅ Enhanced reporting configured for cluster ${id}`);
