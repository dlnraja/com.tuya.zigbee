...
class TuyaUniversalBridge extends EventEmitter {
  ...
  async init() {
    ...
    // WHY(P2392 / VicHY #2224): NEVER add DIY tuya_dp_* capabilities on normal drivers.
    // Adding them caused Homey Energy/UI noise + P2308 recursion (tuya_dp_value) on radars.
    // Only DIY / universal / generic drivers that already declare these caps may use them.
    const driverId = String(this.device?.driver?.id || '').toLowerCase();
    const diyDriver = /universal|generic_diy|device_generic|fallback|catch.?all/.test(driverId);
    ...
    if (diyDriver) {
      for (const cap of caps) {
        if (!this.device.hasCapability(cap)) {
          try { await this.device.addCapability(cap); } catch { /* already exists */ }
        }
      }
    } else {
      // Soft-strip if a previous tip wrongly injected them
      for (const cap of caps) {
        if (this.device.hasCapability?.(cap)) {
          try { await this.device.removeCapability(cap); } catch { /* soft */ }
        }
      }
      this._diyCapsEnabled = false;
      this.log('Universal bridge init (flow-only; DIY caps disabled for this driver)');
    }
  ...
  onClusterEvent(clusterId, cmdId, data) {
    ...
    if (typeof this.device.hasCapability === 'function' && this.device.hasCapability('tuya_cluster_event')) {
  ...
  _updateCapability(dp, value, dpType, typeName) {
    // WHY(P2392/P2403): only write DIY caps on DIY drivers that still have them
    if (this._diyCapsEnabled === false) {return;}
    const has = (cap) => typeof this.device.hasCapability === 'function' && this.device.hasCapability(cap);
    const setCap = (cap, val) => {
      try {
        if (typeof this.device.safeSetCapabilityValue === 'function') {
          Promise.resolve(this.device.safeSetCapabilityValue(cap, val)).catch(() => {});
        } else {
          this.device.setCapabilityValue(cap, val).catch(() => {});
        }
      } catch (_e) { /* soft */ }
    };
    try {
      switch (dpType) {
      case 0: // raw
        if (!has('tuya_dp_raw')) {break;}
        if (Buffer.isBuffer(value)) {
          setCap('tuya_dp_raw', `DP${dp}:${value.toString('hex')}`);
        } else {
          setCap('tuya_dp_raw', `DP${dp}:${JSON.stringify(value)}`);
        }
        break;
      case 1: // bool
      case 2: // value
      case 4: // enum
        if (!has('tuya_dp_value')) {break;}
        if (typeof value === 'number' || typeof value === 'boolean') {
          setCap('tuya_dp_value', Number(value));
        }
        break;
      case 3: // string
        if (!has('tuya_dp_string')) {break;}
        setCap('tuya_dp_string', String(value));
        break;
      case 5: // bitmap
        if (!has('tuya_dp_bitmap')) {break;}
        setCap('tuya_dp_bitmap', Number(value));
        break;
      }
    } catch (err) { this.log('[BRIDGE] DP handler error:', err.message); }
  }
  ...
  _triggerDPFlow(dp, value, dpType, typeName) {
    try {
      const card = this.device.homey.flow.getDeviceTriggerCard('tuya_dp_changed');
      if (card) {
        const tokens = {
          dp_number: dp,
    ...
    } catch (err) { this.log('[BRIDGE] DP flow trigger error:', err.message); }

    // Also trigger type-specific cards
    try {
      if (dpType === 5) {
        const card = this.device.homey.flow.getDeviceTriggerCard('tuya_bitmap_changed');
        if (card) {
          card.trigger(this.device, { dp_number: dp, bitmap: Number(value) }, {}).catch(() => {});
        }
      }
      if (dpType === 0) {
        const card = this.device.homey.flow.getDeviceTriggerCard('tuya_raw_received');
        if (card) {
          const hex = Buffer.isBuffer(value) ? value.toString('hex') : String(value);
          card.trigger(this.device, { dp_number: dp, raw_hex: hex }, {}).catch(() => {});
        }
      }
    } catch (err) { this.log('[BRIDGE] Type-specific flow trigger error:', err.message); }
  }
  ...
  _triggerClusterFlow(event) {
    try {
      const card = this.device.homey.flow.getDeviceTriggerCard('tuya_cluster_received');
      if (card) {
        const tokens = {
    ...
    } catch (err) { this.log('[BRIDGE] Cluster flow trigger error:', err.message); }
  }
  ...
  async sendDP(dp, value, type) {
    ...
    const cluster = ep?.clusters?.tuya || ep?.clusters?.[0xEF00] || ep?.clusters?.[61184];
