...
class TuyaUniversalBridge extends EventEmitter {
  ...
  _triggerDPFlow(dp, value, dpType, typeName) {
    try {
      const card = this.device.homey.flow.getTriggerCard('tuya_dp_changed');
      if (card) {
        const tokens = {
          dp_number: dp,
    ...
    try {
      if (dpType === 5) {
        const card = this.device.homey.flow.getTriggerCard('tuya_bitmap_changed');
        if (card) {
          card.trigger(this.device, { dp_number: dp, bitmap: Number(value) }, {}).catch(() => {});
        }
      }
      if (dpType === 0) {
        const card = this.device.homey.flow.getTriggerCard('tuya_raw_received');
        if (card) {
  ...
  _triggerClusterFlow(event) {
    try {
      const card = this.device.homey.flow.getTriggerCard('tuya_cluster_received');
      if (card) {
        const tokens = {
