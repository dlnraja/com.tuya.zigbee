 ...
 * - Tuya CLUSTERS.TUYA_EF00 protocol documentation
...
const KNOWN_CLUSTERS = {
  ...
  TUYA_SPECIFIC: CLUSTERS.TUYA_EF00,
  TUYA_E000: 0xE000,
  TUYA_E001: 0xE001,
};
...
class UniversalRawInterpreter {
  ...
  interpretFrame(frame, meta = {}) {
    ...
    if (!buffer || buffer.length === 0) {
      this.log(` Frame #${frameId}: Empty or invalid`);
  ...
  _interpretTuyaButton(buffer, meta) {
    ...
    if (buffer.length >= 2) {
      ...
      if (value <= 2) {
        ...
        button = Math.floor(safeParse(value, 3)) + 1;
