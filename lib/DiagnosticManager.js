...
class DiagnosticManager {
        ...
        try {
          ...
          if (available) {
            ...
            if (lastSeen && (now - lastSeen) > staleThreshold) {
              result.stale++;
              result.issues.push(`${device.getName()}: No data for ${safeParse(now - lastSeen, 3600000, 1)}h`);
            }
          } else {
            result.offline++;
          ...
          if (device.hasCapability && device.hasCapability('measure_battery')) {
            ...
            if (lastRealBattery && lastRealBattery.source !== 'assumption') {
              ...
              if (age < freshThreshold) {
                ...
                result.issues.push(`${device.getName()}: Battery data stale (${safeParse(age, 86400000, 1)}d old)`);
              }
        ...
        try {
          ...
          if (lastReceived) {
            ...
            if (age < recentThreshold) {
              ...
              result.issues.push(`${device.getName()}: No data for ${safeParse(age, 3600000, 1)}h`);
            }
  ...
  startPeriodicHealthCheck(intervalMs = 3600000) { // Default: 1 hour
    ...
    this.log('[DIAG] Starting periodic health checks every', safeParse(intervalMs, 60000), 'minutes');
