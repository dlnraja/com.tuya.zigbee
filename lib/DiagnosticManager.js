...
class DiagnosticManager {
        ...
        try {
          ...
          if (available) {
            ...
            if (lastSeen && (now - lastSeen) > staleThreshold) {
              result.stale++;
              result.issues.push(`${device.getName()}: No data for ${now - lastSeen/3600000, 1}h`);
            }
          } else {
            result.offline++;
  ...
  async checkBatteryReporting() {
    ...
    try {
      const devices = await this.app.getDevices();
      const now = Date.now();
      const freshThreshold =safeMultiply(7, 24) * 60 * 60 * 1000; // 7 days
        ...
        try {
          if (device.hasCapability && device.hasCapability('measure_battery')) {
            ...
            const batteryValue = device.getCapabilityValue ?
              device.getCapabilityValue('measure_battery') ;

            // Check if we have real battery data
            const lastRealBattery = device.getStoreValue ?
              await device.getStoreValue('last_real_battery') ;
            ...
            if (lastRealBattery && lastRealBattery.source !== 'assumption') {
              ...
              if (age < freshThreshold) {
                ...
                result.issues.push(`${device.getName()}: Battery data stale (${age/86400000, 1}d old)`);
              }
  ...
  async verifyDataFlow() {
    ...
    try {
      const devices = await this.app.getDevices();
      const now = Date.now();
      const recentThreshold =safeMultiply(6, 60) * 60 * 1000; // 6 hours
        ...
        try {
          ...
          if (lastReceived) {
            ...
            if (age < recentThreshold) {
              ...
              result.issues.push(`${device.getName()}: No data for ${age/3600000, 1}h`);
            }

            result.lastDataTimes.push({
              device: device.getName(),
              lastData: new Date(lastReceived).toISOString(),
              ageHours: safeParse(age, 3600000, 1)
            });
          } else {
            result.devicesNotReceiving++;
          }
  ...
  startPeriodicHealthCheck(intervalMs = 3600000) { // Default: 1 hour
    ...
    this.log('[DIAG] Starting periodic health checks every', safeDivide(intervalMs, 60000), 'minutes');
