...
/**
 * Wire DPAdaptationEngine + DynamicCapabilityManager into the live dpReport
 * stream emitted by TuyaEF00Manager. Safe to call multiple times — idempotent.
...
async function _bootDynamicAdaptation(device) {
  ...
  if (!device.dynamicCapabilityManager) {
    try {
      ...
      device.dynamicCapabilityManager = new DynamicCapabilityManager(device);
      await device.dynamicCapabilityManager.initialize().catch(() => {});
    } catch (e) {
      try { device.log?.(`[LAYERS] DynamicCapabilityManager skip: ${e.message}`); } catch { /* noop */ }
    }
  }

  // --- IntelligentDriverHotSwap ---
  if (!device.hotSwap) {
    try {
      ...
      await device.hotSwap.restoreProfileOverride().catch(() => {});
    } catch (e) {
      try { device.log?.(`[LAYERS] IntelligentDriverHotSwap skip: ${e.message}`); } catch { /* noop */ }
    }
  }

  // Wire all three engines to the live dpReport event from TuyaEF00Manager
  ef00.on('dpReport', ({ dpId, value, dpType }) => {
    if (device._destroyed) return;
    // Strategy 1+2: DP adaptation + dynamic capability discovery
    ...
    // Strategy 2: feed observations into hot-swap routing
    try {
      if (device.hotSwap && device.dynamicCapabilityManager) {
        const mapping = device.dynamicCapabilityManager._discoveredDPs?.get(dpId);
        if (mapping?.capability && mapping?.confidence) {
          device.hotSwap.observeDP(dpId, value, mapping.capability, mapping.confidence).catch(() => {});
        }
      }
    } catch { /* non-blocking */ }
    // Strategy 3: protocol frame counting
    try { device.hotSwap?.observeFrame?.('ef00'); } catch { /* non-blocking */ }
  });

  // Also count ZCL frames for protocol renegotiation (strategy 3)
  if (typeof ef00.on === 'function') {
    ef00.on('zclReport', () => {
  ...
  try {
    device.log?.('[LAYERS] DynamicAdaptation + IntelligentDriverHotSwap wired to dpReport stream');
  } catch { /* noop */ }
}
