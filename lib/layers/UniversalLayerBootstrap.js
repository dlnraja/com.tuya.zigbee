...
async function bootstrapUniversalLayers(device, zclNode) {
  ...
  if (!device.tuyaEF00Manager) {
    ...
    // P2314: PresentSky / BSEED MCU dimmers (_TZE* + TS0601) always need EF00 manager
    const mcuDimmerHeuristic = /^_TZE/i.test(mfr) && /^TS0601/i.test(pid);
    // WHY(P2574): incomplete / unsupported interview — still soft-attach EF00 + DP cascade
    const incompleteOrForced = !!(
      info.interviewIncomplete
      || info.parallelDiscover
      || device._ef00OnlyInterview
      || device._isPureTuyaDP
      || device._incompleteInterviewCompensated
    );
    const wantEf00 = !!(
      info.hasTuyaCluster
    ...
    if (wantEf00 && !zclOnly) {
      try {
        // WHY(P2486b): several TuyaEF00Manager instances allowed; launchOnce only if none launched.
        const TuyaEF00Manager = require('../tuya/TuyaEF00Manager');
        await TuyaEF00Manager.launchOnce(device, node);
        out.ef00 = true;
      } catch (e) {
  ...
  // WHY(P2574): DP / cluster / raw RX-TX even when Homey interview miss or non-native
  // (runs even if EF00 manager already attached from a prior path).
  try {
    const info = device._protocolInfo || {};
    const zclOnly = String(info.protocol || '').toLowerCase() === 'zcl_only'
      || String(device._manufacturerConfig?.protocol || '').toLowerCase() === 'zcl_only';
    ...
    if (!zclOnly && (incompleteOrForced || info.protocol === 'TUYA_DP' || info.preferDpTx)) {
      const mfr = String(
        device.getSetting?.('zb_manufacturer_name')
        || device.getStoreValue?.('manufacturerName')
        || device.getData?.()?.manufacturerName
      ...
      const { armIncompleteInterviewCompensation } = require('../zigbee/Ef00OnlyInterview');
      const arm = await armIncompleteInterviewCompensation(device, node, {
        force: !!(device._ef00OnlyInterview || device._forcePureTuyaDp),
        mfr,
      });
      out.incompleteArm = !!arm?.ok;
      out.compensated = !!arm?.compensated;
    }
  } catch (e) {
    try { device.log?.(`[LAYERS] IncompleteInterview arm skip: ${e.message}`); } catch { /* noop */ }
  }
