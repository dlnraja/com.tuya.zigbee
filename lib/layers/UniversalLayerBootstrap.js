...
async function _bootDynamicAdaptation(device) {
  ...
  // Wire both engines to the live dpReport event from TuyaEF00Manager
  ef00.on('dpReport', ({ dpId, value, dpType }) => {
    if (device._destroyed) return;
    try {
      device.dpAdaptationEngine?.analyzeDP?.(dpId, value, dpType);
    } catch { /* non-blocking */ }
    try {
      device.dynamicCapabilityManager?.processDP?.(dpId, value, dpType);
    } catch { /* non-blocking */ }
  });

  try {
    device.log?.('[LAYERS] DynamicAdaptation wired to dpReport stream');
  } catch { /* noop */ }
}
