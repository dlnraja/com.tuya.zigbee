 ...
 *  - LayerSignalFusion (P211: cross-layer echo / phantom / priority-hold)
...
const DEFAULT_SMART_CAPS = {
  measure_battery: {
    zcl: { priority: 1, weight: 0.35, ttl: 600000 },
    'tuya-dp': { priority: 2, weight: 0.3, ttl: 300000 },
    wifi: { priority: 2, weight: 0.25, ttl: 300000 },
    voltage: { priority: 3, weight: 0.2, ttl: 300000 },
    ias: { priority: 4, weight: 0.1, ttl: 120000 },
    raw: { priority: 5, weight: 0.1, ttl: 180000 },
    mcu: { priority: 5, weight: 0.1, ttl: 180000 },
...
async function attachCrossLayerRedundancy(device, zclNode) {
  ...
  /**
   * Confirm inbound sample through ReceptionManager + LayerSignalFusion + SmartCap + L14.
   * Prefer this over raw setCapabilityValue when the value came from ZCL/DP/IAS/raw.
   * P211: dual-layer echoes / phantoms / spam → single Homey write.
   */
  device.confirmInbound = async function confirmInbound(capability, value, source = 'unknown', confidence = 0.85) {
    ...
    try {
      let fusion;
      try {
        fusion = require('./LayerSignalFusion');
      } catch (_e) {
        fusion = null;
      }
      const normSource = fusion?.normalizeSource?.(source) || source;

      const rx = this.receptionManager?.receive?.(capability, value, normSource);

      // Learn optimizer hits when present
      try {
        if (this.protocolOptimizer?.registerHit) {
          const proto = normSource === 'tuya-dp' || normSource === 'dp' ? 'tuya'
            : normSource === 'ias' ? 'ias'
              : normSource === 'raw' || normSource === 'raw_frame' ? 'raw' : 'zcl';
      ...
      if (sc) {
        ...
        if (!sc.validator.sources.has(normSource)) {
          sc.validator.addSource(normSource, { priority: 5, weight: 0.25, ttl: 120000 });
        }
        sc.record(normSource, value, confidence);
      }

      // P211: cross-layer echo / phantom / priority-hold BEFORE SmartCap commit
      // so hysteresis state is not advanced on suppressed duplicates.
      if (fusion) {
        const gate = fusion.decide(this, capability, value, normSource, { confidence });
        if (gate.commit === false) {
          return {
            ...
            agree: !!gate.agree || !!rx?.crossChannel,
            winner: gate.winner,
            via: normSource,
          };
        }
        ...
        return {
          ok: true,
          skipped: true,
          reason: rx.crossChannel ? 'cross-layer-echo' : 'rx-dedup',
          agree: !!rx.crossChannel,
          via: normSource,
        };
      }
      ...
      if (sc) {
        ...
        // No meta.source here — fusion already committed; avoids double-gate + double bookkeeping
        ...
        return {
          ...
          sources: decision.sources || [normSource],
          value: toSet,
        };
      }
      ...
      return { ok: true, via: normSource, agree: false };
    } catch (err) {
  ...
  // P209: multi-protocol battery % (ZCL / DP / WiFi / IAS / voltage / raw)
  try {
    const { attachMultiProtocolBattery } = require('../battery/MultiProtocolBatteryPercent');
    attachMultiProtocolBattery(device);
    out.batteryMulti = true;
  } catch (_e) { /* soft */ }
