 ...
 * Also: hybrid contact with BOTH IAS + EF00 (Peter #2190 / 0cea6870) — DP1 and
 * IAS fire within ~1s and pulse alarm_contact open→closed. Prefer IAS for
 * alarm_* when an IAS zone cluster is present.
...
function isSleepyIasNoEf00(device) {
  ...
  // Hybrid contact/water: IAS present → still prefer IAS over leftover DP1 noise
  if (hasIas && (pid === 'TS0203' || /contact|water_leak|sos|door/i.test(String(device.driver && device.driver.id || '')))) {
    return true;
  }
