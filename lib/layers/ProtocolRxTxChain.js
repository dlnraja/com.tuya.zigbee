...
const PROTOCOL_PATHS = Object.freeze({
  ...
  zcl: {
    ...
    // P2540 complementary AQ hints (listen via RawClusterFallback + driver HYBRID — not invent pid)
    complementaryClusters: Object.freeze([
      0x0402, 0x0405, 0x040d, 0x042a, 0x042b, 0x042e,
    ]),
  },
  tuya_bound: {
  ...
  // P2544 complementary paths beyond Homey native zigbee/tuya libs
  flow: {
    id: 'flow', label: 'Flow cards / DeclaredFlowCardAutoWire', tx: true, rx: true,
    clusters: null, sources: ['flow', 'flow_trigger', 'flow_action'],
    sleepySafe: true, cost: 1, needsMagic: false, homeyGap: 'declared_only_unwired',
  },
  energy: {
    id: 'energy', label: 'UniversalEnergyHandler / electrical measure', tx: false, rx: true,
    clusters: [0x0B04, 0x0702], sources: ['energy', 'electrical'],
    sleepySafe: true, cost: 2, needsMagic: false, homeyGap: 'phantom_battery_mains',
  },
  poll: {
    id: 'poll', label: 'Heuristic poll / DataQuery / PollControl', tx: true, rx: true,
    clusters: [0x0020], sources: ['poll', 'poll_heuristic', 'data_query'],
    sleepySafe: false, cost: 4, needsMagic: false, homeyGap: 'sleepy_poll_storm',
  },
  capability_router: {
    id: 'capability_router', label: 'CapabilityCommandRouter ZCL↔DP cascade', tx: true, rx: false,
    clusters: null, sources: ['capability_router', 'parallel_discover'],
    sleepySafe: true, cost: 2, needsMagic: false, homeyGap: 'unsupported_cluster_0x81',
  },
  antispam: {
    id: 'antispam', label: 'L14 safeSet + antiflood / EMA-ROC', tx: false, rx: true,
    clusters: null, sources: ['antispam', 'safe_set', 'throttle'],
    sleepySafe: true, cost: 0, needsMagic: false, homeyGap: 'capability_flood',
  },
  ota: {
    id: 'ota', label: 'OTA / Homey Device Updates (OEM indexes)', tx: true, rx: true,
    clusters: [0x0019], sources: ['ota', 'firmware_update'],
    sleepySafe: false, cost: 5, needsMagic: false, homeyGap: 'sleepy_ota_wake',
  },
});
...
function classifyCluster(clusterId) {
  ...
  let id = null;
  try {
    const { normalizeClusterId } = require('../zigbee/ZclClusterLexicon');
    id = normalizeClusterId(clusterId);
  } catch (_e) {
    id = typeof clusterId === 'string' && clusterId.startsWith('0x')
...
class ProtocolRxTxChain {
  ...
  _resolveIntentTargets(intent) {
    ...
    const cap = String(intent.capability).toLowerCase().trim();
    ...
    for (const map of dpMaps) {
      ...
      for (const [dpStr, def] of Object.entries(map)) {
        ...
        const defCap = (typeof def === 'string' ? def : (def?.capability || def?.name || '')).toLowerCase().trim();
        if (defCap === cap) {
          intent.dp = dp;
