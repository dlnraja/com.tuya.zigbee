...
class ZigbeeDebug {
  ...
  async fullDiagnostic(zclNode) {
    ...
    try {
      ...
      this.logger.exception('❌ Diagnostic failed', err);
    }
  }
  ...
  async discoverAttributes(zclNode) {
    this.logger.debug('🔍 Discovering supported attributes...');
  ...
  async testCluster(cluster, clusterName) {
    this.logger.debug(`📊 Testing cluster: ${clusterName}`);
  ...
  setupReportMonitoring(endpoint, clusterName) {
    try {
      ...
      if (originalOn) {
        ...
        this.logger.debug(`✅ Report monitoring enabled for ${clusterName}`);
      }
    } catch (err) {
