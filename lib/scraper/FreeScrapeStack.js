...
const {
  ...
  isMemoryPressure,
} = require('../utils/NetworkResilience');

/** P2372: free-tier daily caps — never saturate Jina/Microlink/AllOrigins/Firecrawl */
let _budget = null;
function budgetApi() {
  if (_budget) return _budget;
  try {
    _budget = require('../../tools/ci/free-scrape-budget');
  } catch {
    _budget = {
      allow: () => ({ ok: true }),
      consume: () => ({ ok: true }),
...
function assertBudget(tier) {
  const a = budgetApi().allow(tier);
  if (!a.ok) throw new Error(`budget_${a.reason || 'denied'}:${tier}`);
}

const _freeScrapeBreaker = createNetworkBreaker('FreeScrapeStack', {
  failureThreshold: 5,
  resetTimeout: 60000,
});
...
function request(targetUrl, options = {}) {
  return _freeScrapeBreaker.exec(() => boundedHttpRequest(targetUrl, {
    ...
    maxBytes: options.maxBytes || MAX_SCRAPE_BYTES,
    body: options.body,
  }));
}
...
function readCache(url) {
  try {
    ...
    // Buffer parse avoids UTF-16 intermediate string (TITAN / Homey RAM gate)
...
async function viaMicrolink(url, opts = {}) {
  ...
  if (res.statusCode === 429 || res.statusCode === 403) {
    budgetApi().markRateLimited('microlink', res.statusCode);
    throw new Error(`microlink HTTP ${res.statusCode}`);
  }
...
async function viaWayback(url, opts = {}) {
  ...
  if (res.statusCode === 429 || res.statusCode === 403) {
    budgetApi().markRateLimited('wayback', res.statusCode);
    throw new Error(`wayback HTTP ${res.statusCode}`);
  }
  if (res.statusCode !== 200) {throw new Error(`wayback HTTP ${res.statusCode}`);}
  budgetApi().consume('wayback');
...
async function viaJina(url, opts = {}) {
  ...
  const tier = via === 'firecrawl' ? 'firecrawl' : 'jina';
  budgetApi().consume(tier);
...
function structuredExtract(text, schemaHints = {}) {
  ...
  const s = String(text || '');
  // Sacred-couple mfr only — never treat TS#### productIds as manufacturers.
  const mfrRe = /_TZE(?:200|204|284|28[0-9A-Z]*)_[A-Za-z0-9]+|_TZ(?:3000|3210|3002|3040|1800)_[A-Za-z0-9]+|_TYZB\d+_[A-Za-z0-9]+|_TYST11_[A-Za-z0-9]+|\bHOBEIAN\b/gi;
  const mfrs = s.match(mfrRe) || [];
  out.manufacturers = [...new Set(mfrs.map((m) => m.trim()))].slice(0, 50);
  const pidRe = /\b(TS\d{4}[A-Z]?|ZG-[\w-]+|SNZB-\d+)\b/gi;
...
async function freeScrape(url, opts = {}) {
  ...
  if (isMemoryPressure({ rssLimit: 512 * 1024 * 1024, heapLimit: 400 * 1024 * 1024 })) {
    ...
    if (hit?.text) {
      return {
        text: hit.text,
        via: hit.via || 'cache-stale',
        extracted: hit.extracted || structuredExtract(hit.text, opts.schemaHints),
        cached: true,
        memoryPressure: true,
      };
    }
    throw new Error('FREE_SCRAPE memory pressure and no cache');
  }

  if (!_freeScrapeBreaker.isAvailable) {
    const hit = readCache(url);
    if (hit?.text) {
      return {
        text: hit.text,
    ...
    throw new Error('FREE_SCRAPE circuit open and no cache');
  }
...
async function freeScrapeMany(urls, opts = {}) {
  // WHY(P2372): cap concurrency so free readers never saturate / get IP-blocked
