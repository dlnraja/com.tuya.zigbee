...
const _readerBreaker = createNetworkBreaker('ReaderFallback', {
  failureThreshold: 4,
  resetTimeout: 90000,
});
...
function firecrawlBudgetLeft() {
  try {
    ...
    const state = JSON.parse(fs.readFileSync(BUDGET_FILE));
...
function firecrawlBudgetConsume() {
  try {
    ...
    try {
      const prev = JSON.parse(fs.readFileSync(BUDGET_FILE));
...
function request(targetUrl, options = {}) {
  return _readerBreaker.exec(() => boundedHttpRequest(targetUrl, {
    ...
    maxBytes: options.maxBytes || MAX_READER_BYTES,
    body: options.body,
  }));
}
...
async function readerFetch(url, opts = {}) {
  if (isMemoryPressure({ rssLimit: 512 * 1024 * 1024, heapLimit: 400 * 1024 * 1024 })) {
    throw new Error('reader fallback skipped — memory pressure');
  }
  if (!_readerBreaker.isAvailable) {
    throw new Error('reader fallback skipped — circuit breaker open');
  }
...
module.exports = { readerFetch, viaJina, viaFirecrawl, firecrawlBudgetLeft };
