...
const { isMemoryPressure } = require('../utils/NetworkResilience');

const _breakerBySource = new Map();
function breakerForSource(source, log) {
  if (!_breakerBySource.has(source)) {
    _breakerBySource.set(source, new CircuitBreaker({
      name: `SmartFetch:${source}`,
      failureThreshold: 5,
      resetTimeout: 60000,
      successThreshold: 2,
      maxBackoff: 300000,
  ...
  return _breakerBySource.get(source);
}
...
class SmartFetcher {
  ...
  constructor(opts = {}) {
    ...
    // Free reader fallback chain (Jina Reader → Firecrawl) as last resort
    // when the origin blocks or keeps failing. Opt out per-instance with
    // readerFallback: false, globally with SMART_FETCH_READER_FALLBACK=0.
    this.readerFallback = opts.readerFallback ?? (process.env.SMART_FETCH_READER_FALLBACK !== '0');
  ...
  async fetch(url, opts = {}) {
    if (isMemoryPressure({ rssLimit: 512 * 1024 * 1024, heapLimit: 400 * 1024 * 1024 })) {
      if (this.cache) {
        ...
        if (cached) {
          return { body: cached.body, fromCache: true, stale: true, memoryPressure: true, attempts: 0, durationMs: 0 };
        }
      }
    }
    ...
    let lastErr = null;
    const breaker = breakerForSource(this.source);
    if (!breaker.isAvailable && this.cache) {
      const cached = this.cache.get(url);
      if (cached) {
        return { body: cached.body, fromCache: true, stale: true, circuitOpen: true, attempts: 0, durationMs: Date.now() - t0 };
      }
    }

    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        const res = await breaker.exec(() => rawRequest(url, { headers, timeout: this.timeout }));
    ...
    // Free reader fallback chain (Agent Reach / firecrawl style): when the
    // origin keeps blocking us (403/429 exhausted, repeated 5xx), try
    // Jina Reader first (free, keyless), then Firecrawl if FIRECRAWL_API_KEY
    // is set. Single attempt per reader — free-tier quotas stay intact.
    if (this.readerFallback) {
      try {
        const r = await readerFetch(url, { timeout: Math.max(this.timeout, 45000) });
        let body = r.body;
        // WHY(P2527): reader markdown is untrusted external content — neutralize injection
        try {
          const { sanitizeUntrusted } = require('../security/UntrustedContentGuard');
          const g = sanitizeUntrusted(String(body || ''), { source: 'scrape', maxChars: 500000 });
          body = g.text;
        } catch (_e) { /* soft */ }
        if (this.cache) {this.cache.store(url, body, {});}
        if (this.persistMetrics) {recordMetric(this.source, { success: true, durationMs: Date.now() - t0 });}
        log(C.Y, `[${this.source}] reader fallback via ${r.via} for ${url.slice(0, 60)} (origin error: ${lastErr?.message})`);
        return { body, fromCache: false, viaReader: r.via, statusCode: 200, attempts: this.maxRetries + 1, durationMs: Date.now() - t0, untrustedSanitized: true };
      } catch (readerErr) {
        log(C.Y, `[${this.source}] reader fallback failed for ${url.slice(0, 60)}: ${readerErr.message}`);
      }
    }
