...
function registerCustomClusters(logger = null) {
  try {
    ...
    if (logger) {
      logger.log('✅ Tuya Manufacturer Cluster (0xEF00) registered successfully');
    }
    return true;
  } catch (err) {
    ...
    if (err.message && err.message.includes('already exists')) {
      if (logger) {
        logger.log('⚠️ Tuya Manufacturer Cluster already registered (skipping)');
      }
      return true;
    }
    
    if (logger) {
      logger.error('❌ Failed to register Tuya Manufacturer Cluster:', err);
    }
    return false;
  }
}
