...
function buildSnapshot(homey, options = {}) {
  const now = Date.now();
  const availability = options.availabilityManager || safe(() => homey.__tuyaApp && homey.__tuyaApp.availabilityManager, null);
  ...
  for (const { device, driverId } of collectDevices(homey)) {
    ...
    const availEntry = availability && availability._devices && availability._devices.get(id);
