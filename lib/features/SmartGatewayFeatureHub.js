...
/**
 * SmartGatewayFeatureHub (P2563–P2565) — branding-free gateway smart features.
...
class SmartGatewayFeatureHub {
  ...
  start() {
    ...
    try { this.ambient = new SoftAmbientSync(this.app, { maxLights: 4, minIntervalMs: 2000 }); } catch (e) {
      this.app.error?.('[SmartGatewayFeatureHub] ambient failed:', e.message);
    }
    try { this.deviceLink = new SoftDeviceLink(this.app); this.deviceLink.start(); } catch (e) {
      this.app.error?.('[SmartGatewayFeatureHub] deviceLink failed:', e.message);
    }
    try { this.luxDim = new LuxAdaptiveDim(this.app); } catch (e) {
      this.app.error?.('[SmartGatewayFeatureHub] luxDim failed:', e.message);
    }
    try { this.mirror = new MirrorLightSync(this.app); } catch (e) {
      this.app.error?.('[SmartGatewayFeatureHub] mirror failed:', e.message);
    }
    try { this.welcome = new WelcomeHomeSoft(this.app); } catch (e) {
      this.app.error?.('[SmartGatewayFeatureHub] welcome failed:', e.message);
    }
    try { this.absence = new AbsenceEnergySoft(this.app); } catch (e) {
      this.app.error?.('[SmartGatewayFeatureHub] absence failed:', e.message);
    }
    return this;
  }

  stop() {
    for (const k of ['mesh', 'ambient', 'deviceLink', 'luxDim', 'mirror']) {
