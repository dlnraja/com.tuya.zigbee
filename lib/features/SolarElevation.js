...
class SolarElevation extends EventEmitter {
  ...
  _setInterval(callback, intervalMs) {
    const scheduler = typeof this.homey?.setInterval === 'function'
  ...
  _clearInterval(timer) {
    const clearer = typeof this.homey?.clearInterval === 'function'
      ? this.homey.clearInterval.bind(this.homey)
      : clearInterval;
    return clearer(timer);
  }
