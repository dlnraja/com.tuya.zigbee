...
class BatteryManagerV4 {
  ...
  /**
   * Helper to resolve power configuration cluster with caseless/alias support
   */
  _getPowerCluster(endpoint) {
  ...
  async tryZCL() {
    try {
      ...
      const powerCluster = this._getPowerCluster(endpoint);
      ...
      const { batteryVoltage } = await powerCluster.readAttributes(['batteryVoltage']).catch(() => ({}));
    ...
    this.interval = this.device.homey.setInterval(async () => {
      ...
      if (this.method === 'tuya_dp' && this.device.tuyaEF00Manager) {
        ...
        try {
          ...
          const powerCluster = this._getPowerCluster(endpoint);
          if (powerCluster) {
            const { batteryPercentageRemaining, batteryVoltage } = await powerCluster.readAttributes(['batteryPercentageRemaining', 'batteryVoltage']).catch(() => ({}));
