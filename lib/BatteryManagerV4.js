...
class BatteryManagerV4 {
  ...
  /**
   * Try Tuya DP
   * v5.0.6: Check if Tuya cluster is actually available before registering
   */
  async tryTuyaDP() {
    try {
      if (!this.device.tuyaEF00Manager) {
        this.device.log('[BATTERY-V4] ℹ️  No tuyaEF00Manager available');
        return false;
      }

      // v5.0.6: Check if Tuya cluster is actually available
      // This flag is set by _initTuyaDpEngine in device.js
      if (this.device._tuyaClusterAvailable === false) {
        this.device.log('[BATTERY-V4] ℹ️  Tuya cluster not available, skipping DP battery');
        return false;
      }
      ...
      // Request initial (don't spam if cluster not available)
      const result4 = await this.device.tuyaEF00Manager.requestDP(4).catch(() => false);
      const result15 = await this.device.tuyaEF00Manager.requestDP(15).catch(() => false);

      // If both failed, mark as not available for future polls
      if (!result4 && !result15) {
        this.device.log('[BATTERY-V4] ⚠️  Initial DP requests failed - device may report passively');
        this._dpRequestsFailed = true;
      }
  ...
  async updateBattery(value, source) {
    ...
    // AUDIT V2: Enhanced logging for visibility
    this.device.log(`[BATTERY-V4] 🔋 Updating battery: ${rounded}% (source: ${source}, type: ${this.batteryType})`);
    ...
    if (this.device.hasCapability('alarm_battery')) {
      ...
      this.device.log(`[BATTERY-V4] ℹ️  alarm_battery capability not available (optional)`);
    }
  }

  /**
   * Start polling
   * v5.0.6: Don't poll if DP requests failed on init
   */
  startPolling(intervalMs) {
    ...
    this.interval = this.device.homey.setInterval(async () => {
      if (this.method === 'tuya_dp' && this.device.tuyaEF00Manager) {
        // v5.0.6: Skip polling if we know DP requests fail
        if (this._dpRequestsFailed || this.device._tuyaClusterAvailable === false) {
          // Don't spam - battery devices report passively on wake
          return;
        }
