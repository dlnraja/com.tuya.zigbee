...
class BatteryManagerV4 {
  ...
  /**
   * v5.0.9: Register DP listeners for passive battery reports
   */
  _registerTuyaDPListeners() {
    if (!this.device.tuyaEF00Manager) return;

    // Common battery DPs
    this.device.tuyaEF00Manager.on('dp-4', (value) => {
      this.updateBattery(this._convertTuyaDPToBattery(value), 'DP4');
    });
    this.device.tuyaEF00Manager.on('dp-15', (value) => {
      this.updateBattery(this._convertTuyaDPToBattery(value), 'DP15');
    });
    this.device.tuyaEF00Manager.on('dp-33', (value) => {
      this.updateBattery(this._convertTuyaDPToBattery(value), 'DP33');
    });
    this.device.tuyaEF00Manager.on('dp-35', (value) => {
      this.updateBattery(this._convertTuyaDPToBattery(value), 'DP35');
    });
  }
