'use strict';
const { safeParse, safeDivide } = require('./utils/MathUtils.js');
...
class ClusterMapper {
  ...
  mapToCapability(cluster, attr, value) {
    const map = {
      ...
      'levelControl.currentLevel': { cap: 'dim', transform: v => safeDivide(v, 254) },
      'temperatureMeasurement.measuredValue': { cap: 'measure_temperature', transform: v => safeDivide(v, 100) },
      'relativeHumidity.measuredValue': { cap: 'measure_humidity', transform: v => safeDivide(v, 100) },
      'powerConfiguration.batteryPercentageRemaining': { cap: 'measure_battery', transform: v => safeDivide(v, 2) }
    };
...
class DpInterpreter {
  ...
  interpret(dpId, value) {
    ...
    else if (map.divisor) v = safeDivide(value, map.divisor);
