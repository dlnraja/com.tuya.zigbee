...
class ClusterMapper {
  ...
  mapToCapability(cluster, attr, value) {
    const map = {
      ...
      'levelControl.currentLevel': { cap: 'dim', transform: v => safeParse(v, 254) },
      'temperatureMeasurement.measuredValue': { cap: 'measure_temperature', transform: v => safeParse(v, 100) },
      'relativeHumidity.measuredValue': { cap: 'measure_humidity', transform: v => safeParse(v, 100) },
      'powerConfiguration.batteryPercentageRemaining': { cap: 'measure_battery', transform: v => safeParse(v, 2) }
    };
    const key = `${cluster}.${attr}`;
    const m = map[key];
    return m ? { capability: m.cap, value: m.transform(value) };
  }
}
