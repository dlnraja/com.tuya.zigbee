...
class UniversalIasDevice extends ZigBeeDevice {
  ...
  async _detectDeviceType() {
    ...
    this._isTuya = manufacturerName.startsWith('_TZ') || manufacturerName.startsWith('_TY');
    this._isHeiman = manufacturerName.includes('HEIMAN');
    this._isSonoff = manufacturerName.includes('SONOFF') || manufacturerName.includes('eWeLink');
    this._isAqara = manufacturerName.includes('LUMI') || manufacturerName.includes('Aqara');
