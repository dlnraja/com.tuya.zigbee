...
class TuyaQuirk {
  ...
  /** Whether this quirk matches a device (Zigbee zb_* or WiFi product_id/product_key) */
  static matches(deviceSettings) {
    const mfr = TU.normalize(deviceSettings.zb_manufacturer_name || deviceSettings.manufacturer_name || '');
    const pid = TU.normalize(
      deviceSettings.zb_model_id
      || deviceSettings.product_id
      || deviceSettings.product_key
      || '',
    );
    return this.manufacturers.some((m) => mfr.includes(TU.normalize(m)))
      || this.productIds.some((p) => pid === TU.normalize(p));
  }
...
class SmartPlugPowerQuirk extends TuyaQuirk {
  ...
  static get productIds() { return ['okaz9tbl']; }
