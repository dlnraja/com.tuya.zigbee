'use strict';
// v5.12.9: TuyaLocalDevice - Enhanced base class for Tuya WiFi local LAN control
// Uses the highly robust, rate-limited and queued TuyaLocalClient wrapper.
// Pattern from: tuya-local (HA), heszegi, rebtor, TinyTuya, Drenso/com.tuya2
const Homey = require('homey');
const TuyaLocalClient = require('./TuyaLocalClient');
const CapabilityMapCache = require('../utils/CapabilityMapCache');
const { getDeviceCache, removeDeviceCache } = require('../managers/DPCache');
// v9.0.40: SmartDivisorManager for WiFi devices with smartDivisor: true
const { smartParse } = require('../managers/SmartDivisorManager');
const { safeSetTimeout, safeClearTimeout, safeSetInterval, safeClearInterval } = require('../utils/safe-timers');
const { allowsCloudFallback, normalizeWiFiConnectionPolicy } = require('../wifi/WiFiConnectionPolicy');
const { resolveWiFiTransport, resolveLanFailureAction } = require('../wifi/LocalFirstResolver');
const {
  resolveCommandGapMs,
  resolveOfflineGraceMs,
  resolveFireAndForget,
} = require('./wifiLanReliabilitySettings');

let UnifiedBatteryHandler;
try {
  UnifiedBatteryHandler = require('../battery/UnifiedBatteryHandler');
} catch (_e) {
  UnifiedBatteryHandler = null;
}

class TuyaLocalDevice extends Homey.Device {
  /** Override: capability-to-DP mappings [{capability, dp, toDevice, fromDevice}] */
  get capabilityMap() {
    // v9.0.40: Use centralized CapabilityMapCache (WeakMap) for GC-friendly caching
    const cached = CapabilityMapCache.get(this);
    if (cached) {return cached;}

    if (this.dpMappings) {
      const map = [];
      for (const [dp, cfg] of Object.entries(this.dpMappings)) {
        if (cfg && cfg.capability) {
          const dpNum = isNaN(dp) ? dp : parseInt(dp, 10);
          map.push({
            capability: cfg.capability,
            dp: dpNum,
            toDevice: cfg.reverseTransform || ((v) => {
              if (cfg.divisor) {return v * cfg.divisor;}
              if (cfg.multiplier) {return v / cfg.multiplier;}
              return v;
            }),
            fromDevice: (v) => this._fromDeviceValue(cfg.capability, dpNum, cfg, v)
          });
        }
      }
      CapabilityMapCache.set(this, map);
      return map;
    }
    CapabilityMapCache.set(this, []);
    return [];
  }
  /** Override: protocol version (default auto-detect) */
  get protocolVersion() { return 'auto'; }

  
  /** Safe app getter to prevent proxy crash */
  get safeApp() {
    try { if (!this.homey || this.homey.isDestroyed) {return null;} return this.homey.app; } catch(e) { return null; }
  }

  /**
   * safeSetCapabilityValue - Hardened setter that checks _destroyed flag
   * Required by CORE_RULES R1: prevents crashes after device destroy
   */
  async safeSetCapabilityValue(capability, value) {
    if (this._destroyed) {return;}
    try {
      if (!this.hasCapability(capability)) {return;}
      const previousValue = this.getCapabilityValue(capability);
      const result = await this.setCapabilityValue(capability, value);

      // v9.2.0: Generic capability change trigger (Device Capabilities inspired)
      if (value !== previousValue) {
        try {
          const app = this.homey?.app;
          if (app?.featureFlowCards?.triggerCapabilityChanged) {
            const deviceId = this.getData?.()?.id || this.getId?.();
            app.featureFlowCards.triggerCapabilityChanged(deviceId, capability, value, previousValue);
          }
        } catch (_e) { /* non-critical */ }
      }

      return result;
    } catch (err) {
      // Only log if not destroyed (avoid post-cleanup noise)
      if (!this._destroyed) {
        this.error(`[safeSet] ${capability} failed:`, err.message || err);
      }
    }
  }

  /**
   * v9.0.40: SDK3 isFirstInit() pattern - returns true only on first pairing
   * Use this for one-time setup actions (adding capabilities, initial configuration)
   * that should not repeat on every app restart.
   * @returns {boolean}
   */
  isFirstInit() {
    if (this.getStoreValue('_isFirstInitDone')) {return false;}
    this.setStoreValue('_isFirstInitDone', true).catch(() => {});
    return true;
  }

  getWiFiConnectionPolicy() {
    const stored = this.getStoreValue?.('wifi_connection_policy') || this.getStoreValue?.('wifiConnectionPolicy') || {};
    return normalizeWiFiConnectionPolicy(stored);
  }

  _allowsCloudKeyRecovery() {
    return allowsCloudFallback(this.getWiFiConnectionPolicy());
  }

  async onInit() {
    this.log('TuyaLocalDevice init:', this.getName());
    this._destroyed = false; // v9.0.40: Lifecycle guard
    this._connected = false;
    this._client = null;
    this._lastKnownDps = null; // v9.0.40: Cached DP values for offline mode
    // P2619: offline grace (com.tuyalocal) — brief TCP drops must not spam unavailable/flows
    this._offlineGraceTimer = null;
    this._unavailableGraceTimer = null;

    // v9.0.40: Initialize DP cache for offline fallback
    this._dpCache = getDeviceCache(this.getId());

    // v9.0.40: Warm up capability map cache
    CapabilityMapCache.warmup(this);

    // WHY(P2525 / T146735): Homey reboot must not force QR re-login for LAN WiFi —
    // restore device_id+local_key from store, then mirror settings→store, pin local_first.
    try {
      const { hydrateLocalCredentialsOnBoot } = require('../wifi/LocalCredentialPersist');
      const hyd = await hydrateLocalCredentialsOnBoot(this);
      if (hyd?.restore?.restored) {
        this.log('[P2525] LAN credentials restored after missing settings (reboot Contre quoi)');
      }
    } catch (e) {
      this.log('[P2525] credential hydrate skipped:', e.message);
    }

    // P2367: WiFi quirks + DP enrichment (tuya-local / ZHA-quirk pattern)
    try {
      const { QuirkRegistry } = require('./TuyaQuirk');
      await QuirkRegistry.applyAll(this);
    } catch (e) {
      this.log('[WIFI-INIT] Quirk apply skipped:', e.message);
    }
    try {
      const { enrichWiFiDpMappings } = require('./WiFiDPRegistry');
      enrichWiFiDpMappings(this);
    } catch (e) {
      this.log('[WIFI-INIT] DP enrich skipped:', e.message);
    }

    for (const cap of this.capabilityMap) {
      if (!this.hasCapability(cap.capability)) {continue;}
      this.registerCapabilityListener(cap.capability, async (value) => {
        const dpValue = cap.toDevice ? cap.toDevice(value) : value;
        await this._setDP(cap.dp, dpValue);
      });
    }
    // v9.0.412 (P92.117): UNIVERSAL button.N UI listeners for WiFi devices —
    // same coverage as the Zigbee base (P92.114): press → toggle gang DP.
    await Promise.resolve(this._registerButtonCapabilityListeners()).catch(err => this.log(`[BUTTON-INIT] ⚠️ UI listeners error: ${err.message}`));
    await this._createDevice();
  }

  /**
   * v9.0.412 (P92.117): button.N UI listeners (WiFi/Tuya local).
   * Press → toggle the matching gang DP (DP1..DP4 for gang 1..4).
   */
  async _registerButtonCapabilityListeners() {
    if (this._buttonCapListenersRegistered) { return; }
    this._buttonCapListenersRegistered = true;
    const caps = (typeof this.getCapabilities === 'function' && this.getCapabilities()) || [];
    for (const cap of caps) {
      const m = /^button\.(\d+)$/.exec(cap);
      if (!m) { continue; }
      const gang = parseInt(m[1], 10);
      try {
        this.registerCapabilityListener(cap, async () => {
          this.log(`[BUTTON-UI] ${cap} pressed (virtual)`);
          try {
            const gangCap = this.hasCapability(`onoff.gang${gang}`) ? `onoff.gang${gang}`
              : (gang === 1 && this.hasCapability('onoff') ? 'onoff' : null);
            if (gangCap) {
              const current = this.getCapabilityValue(gangCap) === true;
              if (typeof this._setGangOnOff === 'function') {
                await this._setGangOnOff(gang, !current);
              } else if (typeof this._setDP === 'function') {
                await this._setDP(gang, !current);
              }
            }
          } catch (e) { this.log('[BUTTON-UI] action failed:', e.message); }
          return true;
        });
      } catch (e) {
        this.log(`[BUTTON-UI] could not register ${cap}: ${e.message}`);
      }
    }
  }

  async _createDevice() {
    const settings = this.getSettings();
    const deviceId = settings.device_id;
    const { normalizeLocalKey } = require('./UdpDiscoveryKeys');
    const localKey = normalizeLocalKey(settings.local_key || settings.device_key)
      || settings.local_key || settings.device_key;
    const ipAddress = settings.ip || settings.device_ip || settings.ip_address;

    // v9.1.0: Local-first transport resolution with verbose decision logging (tuya-local pattern)
    const discovered = this.safeApp?._tuyaUDPDiscovery?.getDevice?.(deviceId);
    const cloudHealth = (() => {
      try { return require('../wifi/CloudHealthState').getCloudHealthSnapshot(); }
      catch (_) { return {}; }
    })();
    const decision = resolveWiFiTransport({
      policy: this.getWiFiConnectionPolicy(),
      deviceId,
      localKey,
      ip: ipAddress,
      discoveredIp: discovered ? discovered.ip : null,
      hasCloudCredentials: this._hasCloudCredentials(),
      cloudRateLimited: cloudHealth.cloudRateLimited === true,
      cloudUnhealthy: cloudHealth.cloudUnhealthy === true,
    });
    this.log(`[LOCAL-FIRST] Transport decision: ${decision.transport.toUpperCase()} — ${decision.reason}`);

    if (decision.transport === 'none') {
      this.log('[LOCAL-FIRST] Device cannot be reached: no local credentials and no usable cloud fallback.');
      await this.setUnavailable('Missing device credentials (device_id/local_key). Re-pair or use Repair.');
      return;
    }
    if (decision.transport === 'cloud') {
      this.log('[LOCAL-FIRST] Cloud-only mode: local credentials missing but cloud fallback opted in. Control will be limited until local credentials are provided.');
      await this.setUnavailable('Local credentials missing. Add device_id/local_key via Repair for full local control.');
      return;
    }
    // P2408: solicit silent 3.5 devices before TCP connect
    try { await this.safeApp?._tuyaUDPDiscovery?.probeNow?.(); } catch (_e) { /* non-fatal */ }
    try {
      const version = settings.protocol_version || this.protocolVersion;
      const preferredFromUdp = discovered?.version || null;
      const isAuto = !version || version === 'auto';
      if (preferredFromUdp) {
        this.log(`[LOCAL-FIRST] UDP hinted protocol v${preferredFromUdp} — cascade starts there`);
      }
      this._client = new TuyaLocalClient({
        id: deviceId,
        key: localKey,
        ip: decision.ip || ipAddress || undefined,
        version: isAuto ? 'auto' : version,
        preferredVersion: preferredFromUdp || (isAuto ? null : version),
        autoDetectProtocol: isAuto || version === 'auto',
        resolveIP: (id) => {
          if (this.homey && this.safeApp && this.safeApp?._tuyaUDPDiscovery) {
            const d = this.safeApp?._tuyaUDPDiscovery.getDevice(id);
            return d ? d.ip : null;
          }
          return null;
        },
        log: (...args) => this.log('[TUYA-TCP]', ...args),
        minCommandInterval: resolveCommandGapMs(settings),
        fireAndForget: resolveFireAndForget(settings),
      });

      // P2619: live command pacing from settings
      if (typeof this._client.setCommandGap === 'function') {
        this._client.setCommandGap(resolveCommandGapMs(settings));
      }
      // P2642 BOTH: fire-and-forget from settings
      if (typeof this._client.setFireAndForget === 'function') {
        this._client.setFireAndForget(resolveFireAndForget(settings));
      }
      // P2641 BOTH: stale-data watchdog — 60s UPDATEDPS poll → max(90s, 3×poll)
      if (typeof this._client.setDataTimeout === 'function') {
        const pollSec = Number(settings.polling_interval);
        const pollMs = Number.isFinite(pollSec) && pollSec >= 0
          ? Math.floor(pollSec) * 1000
          : 60000;
        this._client.setDataTimeout(pollMs);
      }

      this._client.on('connected', () => this._onConnected());
      this._client.on('disconnected', () => this._onDisconnected());
      this._client.on('error', (err) => this._onError(err));
      this._client.on('dp-update', (dps) => this._onData({ dps }));
      // v9.0.405: WiFi devices count for availability too (they never pass
      // through TuyaZigbeeDevice.handleFrame) — any local event marks them seen.
      const _markSeen = () => {
        try {
          const availMgr = this.homey.app && this.homey.app.availabilityManager;
          if (availMgr) {
            const devId = (typeof this.getData === 'function' && this.getData() && this.getData().id) || this.id;
            if (devId) { availMgr.markSeen(devId); }
          }
        } catch (_e) { /* non-critical */ }
      };
      this._client.on('dp-update', _markSeen);
      this._client.on('connected', _markSeen);
      this._client.on('heartbeat', _markSeen);
      this._client.on('auth-error', (err) => {
        if (this._allowsCloudKeyRecovery()) {
          this.setWarning('Authentication failed: Local key might have changed. Attempting opted-in cloud recovery...');
          this._attemptLocalKeyRecovery();
          return;
        }
        this.setWarning('Authentication failed: Local key might have changed. Local-first mode keeps cloud fallback disabled; update the key from Repair.');
      });

      // v9.0.40: Connection state tracking for offline mode
      this._client.on('connection-state', (state) => {
        if (this._destroyed) {return;}
        if (state === 'disconnected') {
          this._lastKnownDps = { ...this._client.lastDps };
          this.log(`[OFFLINE] Device offline. Cached ${Object.keys(this._lastKnownDps).length} DP values.`);
        } else if (state === 'connected' && this._lastKnownDps) {
          this.log('[OFFLINE] Device back online. Resuming normal operation.');
          this._lastKnownDps = null;
        }
      });

      // v9.1.0: LAN connection window exhausted -> local-first fallback decision
      this._client.on('connection-timeout', () => this._onConnectionTimeout());

      // v7.5.0: Dynamic IP self-healing cache
      this._client.on('ip-resolved', (resolvedIp) => {
        const settings = this.getSettings();
        const newSettings = {};
        if (settings.ip !== undefined && settings.ip !== resolvedIp) {newSettings.ip = resolvedIp;}
        if (settings.device_ip !== undefined && settings.device_ip !== resolvedIp) {newSettings.device_ip = resolvedIp;}
        if (settings.ip_address !== undefined && settings.ip_address !== resolvedIp) {newSettings.ip_address = resolvedIp;}

        if (Object.keys(newSettings).length > 0) {
          this.log(`[TUYA-TCP] Dynamic IP change detected, storing resolved IP ${resolvedIp} in settings`);
          this.setSettings(newSettings).catch(err => this.error(err));
        }
      });

      // v7.5.0: Dynamic Protocol Version self-healing cache
      this._client.on('version-resolved', (resolvedVersion) => {
        const settings = this.getSettings();
        if (settings.protocol_version !== undefined && settings.protocol_version !== resolvedVersion) {
          this.log(`[TUYA-TCP] Auto-detected protocol version v${resolvedVersion}, storing in settings`);
          this.setSettings({ protocol_version: resolvedVersion }).catch(err => this.error(err));
        }
      });

      await this._client.connect();
    } catch (err) {
      this.error('TuyaLocalClient create failed:', err.message);
      await this.setUnavailable(`Client: ${  err.message}`);
    }
  }

  _onConnected() {
    if (this._destroyed) {return;} // v9.0.40: Guard
    this.log('Connected');
    this._connected = true;
    // P2619: cancel offline grace — device came back before window expired
    this._clearOfflineGraceTimers();
    this.setAvailable().catch(err => this.error(err));
    this.unsetWarning();

    // v9.0.40: Periodic UPDATEDPS command to force WiFi devices to refresh data points
    // Prevents stale data when device doesn't proactively report changes
    this._startUpdatedpsPolling();
  }

  /**
   * v9.0.40: Start periodic UPDATEDPS polling for WiFi devices
   * Sends an empty set command to trigger the device to report all current DP values
   */
  _startUpdatedpsPolling() {
    this._stopUpdatedpsPolling();
    const intervalMs = 60000; // Every 60 seconds
    this._updatedpsInterval = safeSetInterval(this, async () => {
      if (this._destroyed || !this._connected || !this._client) {return;}
      try {
        // Send UPDATEDPS command (empty dps object forces device to refresh all DPs)
        await this._client.refresh();
      } catch (err) {
        // Non-critical - just log and continue
        if (!this._destroyed) {
          this.log('[UPDATEDPS] Refresh failed (non-critical):', err.message);
        }
      }
    }, intervalMs);
  }

  /**
   * v9.0.40: Stop UPDATEDPS polling
   */
  _stopUpdatedpsPolling() {
    if (this._updatedpsInterval) {
      safeClearInterval(this, this._updatedpsInterval);
      this._updatedpsInterval = null;
    }
  }

  _onDisconnected() {
    if (this._destroyed) {return;} // v9.0.40: Guard
    this.log('Disconnected - entering offline mode');
    this._connected = false;
    // v9.0.40: Don't mark unavailable immediately - let client retry with backoff
    this.setWarning('Device disconnected. Reconnecting automatically...');

    // P2619: delay unavailable + optional disconnect flow (com.tuyalocal offline grace)
    // Contre quoi: pet feeder / dehumidifier brief TCP drops → spurious offline notifications
    const graceMs = resolveOfflineGraceMs(this.getSettings?.() || {});
    this._clearOfflineGraceTimers();
    // Short floor (5s) even when grace=0 for tile flash; grace covers flow/unavailable
    const unavailableMs = Math.max(5000, graceMs);
    this._unavailableGraceTimer = safeSetTimeout(this, () => {
      this._unavailableGraceTimer = null;
      if (this._destroyed || this._connected) return;
      this.setUnavailable('Device disconnected').catch(() => {});
    }, unavailableMs);
    this._offlineGraceTimer = safeSetTimeout(this, () => {
      this._offlineGraceTimer = null;
      if (this._destroyed || this._connected) return;
      // Best-effort: drivers that declare *_device_disconnected get a late fire
      const driverId = this.driver?.id || this.driver?.manifest?.id;
      if (driverId) {
        this.triggerFlowCard(`${driverId}_device_disconnected`).catch(() => {});
      }
    }, graceMs);
  }

  _clearOfflineGraceTimers() {
    if (this._offlineGraceTimer) {
      safeClearTimeout(this, this._offlineGraceTimer);
      this._offlineGraceTimer = null;
    }
    if (this._unavailableGraceTimer) {
      safeClearTimeout(this, this._unavailableGraceTimer);
      this._unavailableGraceTimer = null;
    }
  }

  _onError(err) {
    if (this._destroyed) {return;} // v9.0.40: Guard
    const msg = err.message || '';
    this.error('Device error:', msg);
    if (msg.includes('find()') || msg.includes('timeout')) {
      this.setWarning('Discovery timeout: Please assign a Static IP in settings if your router blocks UDP broadcasts.');
    }
  }

  /**
   * v9.1.0: True when app-level Tuya cloud credentials are configured.
   */
  _hasCloudCredentials() {
    try {
      return Boolean(
        this.homey.settings.get('tuya_cloud_access_id') &&
        this.homey.settings.get('tuya_cloud_access_secret')
      );
    } catch (_e) {
      return false;
    }
  }

  /**
   * v9.1.0: LAN connection window exhausted (device unreachable locally).
   * Local-first fallback: log the decision verbosely, optionally take a
   * cloud status snapshot (policy opt-in only), then schedule a LAN retry.
   */
  async _onConnectionTimeout() {
    if (this._destroyed) {return;}
    const cloudHealth = (() => {
      try { return require('../wifi/CloudHealthState').getCloudHealthSnapshot(); }
      catch (_) { return {}; }
    })();
    const action = resolveLanFailureAction({
      policy: this.getWiFiConnectionPolicy(),
      hasCloudCredentials: this._hasCloudCredentials(),
      cloudRateLimited: cloudHealth.cloudRateLimited === true,
      cloudUnhealthy: cloudHealth.cloudUnhealthy === true,
    });
    this.log(`[LOCAL-FIRST] Fallback decision: ${action.action.toUpperCase()} — ${action.reason}`);

    if (action.action === 'cloud_status') {
      await this._cloudStatusFallback();
    }

    await this.setUnavailable('Device unreachable on local network. Retrying automatically...').catch(() => {});
    this._scheduleLocalRetry();
  }

  /**
   * v9.1.0: One-shot cloud status snapshot for diagnostics when LAN is down.
   * Control stays local; the snapshot is logged, never applied blindly.
   * Rate-limited to 1 call per 10 minutes.
   */
  async _cloudStatusFallback() {
    if (this._destroyed) {return;}
    if (this._lastCloudStatusAt && Date.now() - this._lastCloudStatusAt < 600000) {
      this.log('[LOCAL-FIRST] Cloud status snapshot skipped (rate-limited, 10min cooldown).');
      return;
    }
    this._lastCloudStatusAt = Date.now();
    try {
      const TuyaCloudAPI = require('./TuyaCloudAPI');
      const api = new TuyaCloudAPI({
        accessId: this.homey.settings.get('tuya_cloud_access_id'),
        accessKey: this.homey.settings.get('tuya_cloud_access_secret'),
        region: this.homey.settings.get('tuya_cloud_region') || 'eu',
        log: this.log.bind(this),
      });
      const deviceId = this.getSettings().device_id;
      const res = await api.getDeviceStatus(deviceId);
      if (this._destroyed) {return;}
      if (res && res.success) {
        this.log('[LOCAL-FIRST] Cloud status snapshot (diagnostic only):', JSON.stringify(res.result).slice(0, 500));
        this.setWarning('Device unreachable locally. Cloud reachable: check device power/network. LAN retry in progress...');
      } else {
        try {
          const { looksLikeCloudRateLimit, markCloudRateLimited } = require('../wifi/CloudHealthState');
          if (looksLikeCloudRateLimit(res)) {
            markCloudRateLimited({ reason: 'cloud_status_' + String(res && res.code) });
            this.log('[LOCAL-FIRST] Cloud rate-limited during status snapshot — preferring LAN retries.');
          }
        } catch (_) { /* optional */ }
        this.log('[LOCAL-FIRST] Cloud status snapshot failed:', JSON.stringify(res).slice(0, 200));
      }
    } catch (err) {
      if (!this._destroyed) {
        try {
          const { looksLikeCloudRateLimit, markCloudRateLimited } = require('../wifi/CloudHealthState');
          if (looksLikeCloudRateLimit(err)) {
            markCloudRateLimited({ reason: err.message || 'cloud_status_error' });
          }
        } catch (_) { /* optional */ }
        this.error('[LOCAL-FIRST] Cloud status snapshot error:', err.message || err);
      }
    }
  }

  /**
   * v9.1.0: Retry the LAN connection after a cooldown once the client's
   * total connection window is exhausted (client stops retrying by design).
   */
  _scheduleLocalRetry() {
    if (this._destroyed || this._localRetryTimer) {return;}
    const delayMs = 5 * 60 * 1000;
    this.log(`[LOCAL-FIRST] LAN retry scheduled in ${delayMs / 60000}min.`);
    this._localRetryTimer = safeSetTimeout(this, async () => {
      this._localRetryTimer = null;
      if (this._destroyed || this._connected || !this._client) {return;}
      this.log('[LOCAL-FIRST] Retrying LAN connection after cooldown...');
      try {
        await this._client.connect();
      } catch (err) {
        if (!this._destroyed) {
          this.error('[LOCAL-FIRST] LAN retry failed:', err.message || err);
        }
      }
    }, delayMs);
  }

  async _onData(data) {
    if (this._destroyed) {return;} // v9.0.40: Guard against post-destroy data
    if (!data || !data.dps) {return;}
    this.log('DPs:', JSON.stringify(data.dps));

    // v9.0.40: Cache DP values for offline fallback
    if (this._dpCache) {
      this._dpCache.updateFromDps(data.dps);
    }

    // T15811 soft-learn: multisock countdown / extras (DP7/9/17…) — store, never invent caps
    try {
      this._softLearnWifiDps(data.dps);
    } catch (_) { /* soft */ }

    try {
      await this._processDPUpdate(data.dps);
    } catch (err) {
      this.error('[TUYA-TCP] Raw DP hook failed:', err.message || err);
    }

    for (const cap of this.capabilityMap) {
      if (data.dps[cap.dp] !== undefined) {
        const value = cap.fromDevice ? cap.fromDevice(data.dps[cap.dp]) : data.dps[cap.dp];
        if (cap.capability === 'measure_battery' && (value === null || value === undefined)) {
          this.log(`[WIFI-BATTERY] Ignored invalid DP${cap.dp} battery value: ${data.dps[cap.dp]}`);
          continue;
        }
        await this._setLocalCapabilityValue(cap.capability, value).catch(err => this.error(err));
        if (cap.capability === 'measure_battery') {
          await this.setStoreValue?.('last_battery_percentage', Math.round(Number(value))).catch(() => {});
          await this.setStoreValue?.('last_battery_time', Date.now()).catch(() => {});
          await this.setStoreValue?.('last_battery_source', `wifi-dp-${cap.dp}`).catch(() => {});
          await this.setStoreValue?.('last_battery_estimated', false).catch(() => {});
        }
      }
    }
  }

  /**
   * T15811 / P2485 — soft-learn unknown LAN DPs (countdown etc.) without inventing Homey caps.
   * Forum: NEW DATA ATTRIBUTE dps 9 / 17 on Neo Coolcam multisock.
   */
  _softLearnWifiDps(dps) {
    if (!dps || typeof dps !== 'object') return;
    const known = new Set((this.capabilityMap || []).map((c) => String(c.dp)));
    const learned = this._wifiSoftLearnedDps || (this._wifiSoftLearnedDps = {});
    for (const [k, v] of Object.entries(dps)) {
      if (known.has(String(k))) continue;
      learned[k] = { value: v, at: Date.now() };
      // Heuristic: small ints on 7/8/9/10/17 often countdown / extras
      const id = Number(k);
      if ([7, 8, 9, 10, 17].includes(id) && Number.isFinite(Number(v))) {
        this.log(`[WIFI-SOFT] T15811 soft-learn DP${id}=${v} (countdown/extra — stored, no invent cap)`);
      }
    }
    try {
      this.setStoreValue?.('wifi_soft_learned_dps', learned).catch(() => {});
    } catch (_) { /* ignore */ }
  }

  async _processDPUpdate(_dps) {
    // Optional subclass hook for generic/raw DP drivers.
  }

  _fromDeviceValue(capability, dp, cfg, rawValue) {
    if (capability === 'measure_battery') {
      return this._normalizeLocalBatteryValue(dp, rawValue, cfg);
    }

    if (cfg.transform) {return cfg.transform(rawValue);}
    if (cfg.smartDivisor === true) {
      return smartParse(rawValue, dp, {
        manufacturerName: this.getSetting?.('zb_manufacturer_name') || '',
        capability: cfg.capability,
        deviceId: this.getData?.()?.id || '',
        protocol: 'wifi',
      });
    }
    if (cfg.divisor) {return rawValue / cfg.divisor;}
    if (cfg.multiplier) {return rawValue * cfg.multiplier;}
    return rawValue;
  }

  _normalizeLocalBatteryValue(dp, rawValue, cfg = {}) {
    if (!UnifiedBatteryHandler) {
      const numeric = Number(rawValue);
      if (!Number.isFinite(numeric) || numeric < 0 || numeric > 200 || numeric === 255 || numeric === 0xFFFF) {
        return null;
      }
      return numeric > 100 ? Math.round(numeric / 2) : Math.round(numeric);
    }

    const transformed = cfg.transform ? cfg.transform(rawValue) : rawValue;
    const settings = this.getSettings?.() || {};
    const store = this.getStore?.() || {};
    const data = this.getData?.() || {};
    const manufacturer = cfg.manufacturerName ||
      settings.zb_manufacturer_name ||
      settings.manufacturerName ||
      store.manufacturerName ||
      data.manufacturerName ||
      '';
    const modelId = cfg.modelId ||
      settings.zb_model_id ||
      settings.modelId ||
      store.modelId ||
      data.modelId ||
      '';
    const lookedUpProfile = UnifiedBatteryHandler.lookupBatteryProfile?.(manufacturer, modelId);
    const hasExplicitProfile = Boolean(
      cfg.batteryProfile ||
      lookedUpProfile ||
      cfg.batteryAlgorithm ||
      cfg.algorithm
    );
    const profile = hasExplicitProfile
      ? cfg.batteryProfile || lookedUpProfile || {
        dpId: Number(dp),
        algorithm: cfg.batteryAlgorithm || cfg.algorithm || 'direct',
        chemistry: cfg.batteryType || cfg.chemistry || 'CR2032',
      }
      : null;

    const normalized = UnifiedBatteryHandler.normalizeTuyaBatteryValue(Number(dp), transformed, {
      profile,
      profileMatch: hasExplicitProfile,
      batteryType: cfg.batteryType || profile?.chemistry || 'CR2032',
      lastValue: this.getCapabilityValue?.('measure_battery'),
      temperature: this.getCapabilityValue?.('measure_temperature') || settings.temperature,
      stateMode: cfg.stateMode,
    });

    if (normalized !== null) {return normalized;}

    const raw = UnifiedBatteryHandler.coerceNumericValue(transformed);
    const voltage = UnifiedBatteryHandler.normalizeVoltage(transformed);
    if (Number.isFinite(raw) && raw >= 800 && voltage !== null) {
      return UnifiedBatteryHandler.calculateFromVoltage(voltage, cfg.batteryType || profile?.chemistry || 'CR2032');
    }

    return null;
  }

  async _setLocalCapabilityValue(capability, value) {
    if (typeof this.safeSetCapabilityValue === 'function') {
      return this.safeSetCapabilityValue(capability, value);
    }
    return this.setCapabilityValue(capability, value);
  }

  async _setDP(dp, value) {
    if (!this._client) {
      throw new Error('Device client not initialized');
    }
    // v9.0.40: Let TuyaLocalClient handle offline queuing
    const dpId = isNaN(dp) ? dp : parseInt(dp, 10);
    await this._client.setDP(dpId, value);
  }

  async _setMultipleDPs(dpsObj) {
    if (!this._client) {
      throw new Error('Device client not initialized');
    }
    await this._client.setDPs(dpsObj);
  }

  async onSettings({ oldSettings, newSettings, changedKeys }) {
    let needsRecreation = false;
    for (const key of changedKeys) {
      if (['device_id', 'local_key', 'device_key'].includes(key)) {
        needsRecreation = true;
      }
      if (['ip', 'device_ip', 'ip_address'].includes(key)) {
        const val = newSettings[key];
        if (this._client && val !== this._client.ip) {
          needsRecreation = true;
        }
      }
      if (key === 'protocol_version') {
        const val = newSettings[key];
        if (this._client && val !== this._client.version) {
          needsRecreation = true;
        }
      }
      // P2619: live apply — no reconnect
      if (key === 'command_gap_ms' && this._client && typeof this._client.setCommandGap === 'function') {
        this._client.setCommandGap(resolveCommandGapMs(newSettings));
      }
      // P2642 BOTH: live fire-and-forget
      if (key === 'fire_and_forget' && this._client && typeof this._client.setFireAndForget === 'function') {
        this._client.setFireAndForget(resolveFireAndForget(newSettings));
      }
      // P2641 BOTH: live stale-data window (0 = push-only, disable watchdog)
      if (key === 'polling_interval' && this._client && typeof this._client.setDataTimeout === 'function') {
        const pollSec = Number(newSettings.polling_interval);
        const pollMs = Number.isFinite(pollSec) && pollSec >= 0
          ? Math.floor(pollSec) * 1000
          : 60000;
        this._client.setDataTimeout(pollMs);
      }
    }

    if (needsRecreation) {
      this.log('[TUYA-TCP] Setting change requires client recreation, restarting connection...');
      this._destroyDevice();
      await this._createDevice();
    }
    // WHY(P2525): keep store mirror in sync when user edits LAN keys (survive next reboot)
    try {
      const { persistLocalCredentials } = require('../wifi/LocalCredentialPersist');
      await persistLocalCredentials(this);
    } catch (_e) { /* soft */ }
  }


  _destroyDevice() {
    this._stopUpdatedpsPolling(); // v9.0.40: Stop UPDATEDPS polling
    this._clearOfflineGraceTimers(); // P2619
    if (this._localRetryTimer) { // v9.1.0: Stop scheduled LAN retry
      safeClearTimeout(this, this._localRetryTimer);
      this._localRetryTimer = null;
    }
    if (this._client) {
      try {
        this._client.destroy();
      } catch (e) { /* ignore */ }
      this._client = null;
    }
    this._connected = false;
  }

  async _attemptLocalKeyRecovery() {
    if (!this._allowsCloudKeyRecovery()) {
      this.log('[TUYA-TCP] Cloud key recovery skipped by local-first policy.');
      return;
    }
    if (this._isRecoveringKey) {return;}
    // v9.0.40: Max 3 retry attempts, then cool down for 1 hour
    if (!this._keyRecoveryAttempts) {this._keyRecoveryAttempts = 0;}
    if (this._keyRecoveryAttempts >= 3) {
      if (!this._keyRecoveryCooldown || Date.now() - this._keyRecoveryCooldown < 3600000) {return;}
      this._keyRecoveryAttempts = 0; // Reset after cooldown
    }
    this._isRecoveringKey = true;
    
    this.log('[TUYA-TCP] 🔄 Attempting intelligent Local Key auto-recovery...');
    
    try {
      const accessId = this.homey.settings.get('tuya_cloud_access_id');
      const accessKey = this.homey.settings.get('tuya_cloud_access_secret');
      const region = this.homey.settings.get('tuya_cloud_region') || 'eu';
      
      if (!accessId || !accessKey) {
        this.log('[TUYA-TCP] ❌ Cannot auto-recover key: Cloud API credentials missing in app settings.');
        this._isRecoveringKey = false;
        return;
      }
      
      const deviceId = this.getSettings().device_id;
      if (!deviceId) {
        this._isRecoveringKey = false;
        return;
      }

      const TuyaCloudAPI = require('./TuyaCloudAPI');
      const api = new TuyaCloudAPI({ accessId, accessKey, region, log: this.log.bind(this) });
      
      this.log(`[TUYA-TCP] ☁️ Fetching fresh local_key for device ${deviceId}...`);
      const res = await api.getDeviceInfo(deviceId);
      
      if (res && res.success && res.result && res.result.local_key) {
        const { normalizeLocalKey } = require('./UdpDiscoveryKeys');
        const newLocalKey = normalizeLocalKey(res.result.local_key) || res.result.local_key;
        this.log(`[TUYA-TCP] ✅ New local_key fetched successfully!`);
        
        // Update settings without triggering onSettings reload (or handle it correctly)
        await this.setSettings({ local_key: newLocalKey }).catch(err => this.error(err));
        
        if (this._client) {
          this._client.updateKey(newLocalKey);
          // The client will use the new key on its next auto-reconnect attempt
        }
        this.setWarning('Local key auto-recovered successfully. Reconnecting...');
      } else {
        this.log('[TUYA-TCP] ❌ Cloud API returned failure or no local_key:', res);
        this.setWarning('Auto-recovery failed. Please check your Cloud API credentials or re-pair.');
      }
    } catch (err) {
      this.error('[TUYA-TCP] ❌ Auto-recovery error:', err.message);
    }

    this._keyRecoveryAttempts++; // v9.0.40: Track attempts
    if (this._keyRecoveryAttempts >= 3) {
      this._keyRecoveryCooldown = Date.now(); // v9.0.40: Start cooldown
      this.log('[TUYA-TCP] ⏸️ Key recovery max attempts reached, cooling down for 1 hour');
    }
    this._isRecoveringKey = false;
  }

  async onUninit() {
    this._destroyed = true; // v9.0.40: Set before cleanup
    CapabilityMapCache.invalidate(this); // v9.0.40: Release cache on uninit
    if (this._dpCache) { this._dpCache.destroy(); this._dpCache = null; }
    this._destroyDevice();
    if (super.onUninit) {
      await super.onUninit();
    }
  }

  async onDeleted() {
    this._destroyed = true; // v9.0.40: Set before cleanup
    CapabilityMapCache.invalidate(this); // v9.0.40: Release cache on deletion
    removeDeviceCache(this.getId()); // v9.0.40: Remove DP cache on deletion
    this._dpCache = null;
    this._destroyDevice();
    if (super.onDeleted) {
      await super.onDeleted();
    }
  }

  // ─── Homey mDNS discovery (P2410) — _tuya._tcp when firmware advertises ───
  _discoveryDeviceId() {
    const settings = (typeof this.getSettings === 'function' && this.getSettings()) || {};
    const data = (typeof this.getData === 'function' && this.getData()) || {};
    return settings.device_id || data.id || data.deviceId || null;
  }

  onDiscoveryResult(discoveryResult) {
    const id = this._discoveryDeviceId();
    if (!id || !discoveryResult) return false;
    const rid = discoveryResult.id || discoveryResult.txt?.id;
    return String(rid) === String(id);
  }

  async onDiscoveryAvailable(discoveryResult) {
    this._applyDiscoveryAddress(discoveryResult, 'available');
  }

  onDiscoveryAddressChanged(discoveryResult) {
    this._applyDiscoveryAddress(discoveryResult, 'address-changed');
  }

  onDiscoveryLastSeenChanged(discoveryResult) {
    // Device advertising again — nudge reconnect if client idle
    if (this._destroyed) return;
    if (this._client && typeof this._client.connect === 'function' && !this._client.connected) {
      this._client.connect().catch(() => {});
    }
  }

  _applyDiscoveryAddress(discoveryResult, reason) {
    if (this._destroyed || !discoveryResult) return;
    const address = discoveryResult.address || discoveryResult.host;
    if (!address) return;
    const settings = (typeof this.getSettings === 'function' && this.getSettings()) || {};
    const current = settings.ip || settings.ip_address || '';
    if (current && current === address) return;
    this.log(`[DISC-MDNS] ${reason}: IP ${current || '(none)'} → ${address}`);
    this.setSettings({ ip: address }).catch(() => {});
    if (this._client && typeof this._client.updateIP === 'function') {
      this._client.updateIP(address);
      this._client.connect().catch(() => {});
    }
  }

  /**
   * triggerFlowCard - Safe helper to trigger a device flow card using modern SDKv3 API
   * Follows strict SDK3 patterns for device-specific triggers.
   *
   * @param {string} cardId - Flow trigger card ID
   * @param {Object} [tokens={}] - Dynamic flow tokens
   * @param {Object} [state={}] - Flow card state
   * @returns {Promise<boolean>} - True if triggered successfully
   */
  async triggerFlowCard(cardId, tokens = {}, state = {}) {
    this.log(`[FLOW] 🎬 Triggering card: ${cardId}`);
    try {
      let card = null;

      // 1. Try modern SDKv3 getDeviceTriggerCard
      if (this.homey.flow && typeof this.homey.flow.getDeviceTriggerCard === 'function') {
        try {
          card = this.homey.flow.getDeviceTriggerCard(cardId);
        } catch (e) {
          // If not found as device trigger
        }
      }

      if (card && typeof card.trigger === 'function') {
        await card.trigger(this, tokens, state);
        this.log(`[FLOW] ✅ Card triggered successfully: ${cardId}`);
        return true;
      } else {
        this.log(`⚠️ [FLOW] Card not found or trigger is not a function: ${cardId}`);
      }
    } catch (err) {
      this.error(`❌ [FLOW] Error triggering card ${cardId}:`, err.message);
    }
    return false;
  }

}

module.exports = TuyaLocalDevice;
