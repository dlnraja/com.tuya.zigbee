...
class TuyaLocalDevice extends Homey.Device {
    ...
    try {
      ...
      this._client.on('ip-resolved', (resolvedIp) => {
        ...
        if (Object.keys(newSettings).length > 0) {
          ...
          this.setSettings(newSettings).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
        }
      });
      ...
      this._client.on('version-resolved', (resolvedVersion) => {
        ...
        if (settings.protocol_version !== undefined && settings.protocol_version !== resolvedVersion) {
          ...
          this.setSettings({ protocol_version: resolvedVersion }).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
        }
      });
  ...
  _onConnected() {
    ...
    this.setAvailable().catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
    this.unsetWarning();
  ...
  async _onData(data) {
    ...
    for (const cap of this.capabilityMap) {
      if (data.dps[cap.dp] !== undefined) {
        ...
        await this._setLocalCapabilityValue(cap.capability, value).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
  ...
  async _attemptLocalKeyRecovery() {
    ...
    try {
      ...
      if (res && res.success && res.result && res.result.local_key) {
        ...
        await this.setSettings({ local_key: newLocalKey }).catch(this._boundError || ((e) => { try { this.error(e); } catch (_) {} }));
