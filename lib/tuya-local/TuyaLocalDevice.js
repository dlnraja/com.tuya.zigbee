...
class TuyaLocalDevice extends Homey.Device {
  ...
  _scheduleLocalRetry() {
    ...
    this._localRetryTimer = this.homey.setTimeout(async () => {
  ...
  _destroyDevice() {
    ...
    if (this._localRetryTimer) { // v9.1.0: Stop scheduled LAN retry
      try {
        if (this.homey && typeof this.homey.clearTimeout === 'function') {
          this.homey.clearTimeout(this._localRetryTimer);
        } else {
          clearTimeout(this._localRetryTimer);
        }
      } catch (_e) { /* ignore */ }
      this._localRetryTimer = null;
    }
