...
class TuyaWiFiHybridManager extends EventEmitter {
  ...
  _initCloud() {
    ...
    if (accessId && accessSecret) {
      ...
      this.log('[WiFi-Hybrid] ☁️ Cloud Shadowing active');
        ...
        if (data && data.devId === this.device.getSettings().device_id) {
          ...
          if (dps) {
            ...
            dps.forEach(item => {
              ...
              // 🛡️ THE ANTI-ECHO FILTER (entrance shield)
  ...
  async _initLocal(forceNewKey = null) {
    ...
    if (!deviceId || !localKey) {
      this.log('[WiFi-Hybrid] ⚠️ Missing local credentials');
    ...
    await this._localClient.connect().catch(e => {
      this.log(`[WiFi-Hybrid] ⚠️ Local connect failed: ${e.message}`);
      this._fallbackToCloud = true;
    });
  }
  ...
  async syncMappings(providedAuth = null) {
    try {
      ...
      this.log(`[WiFi-Hybrid] ⚠️ Mapping sync failed: ${e.message}`);
    }
  }
  ...
  async setDP(dp, value) {
    ...
    if (this._localClient && this._localClient.connected && !this._fallbackToCloud) {
      try {
        ...
        this.log(`[WiFi-Hybrid] ⚠️ Local command failed: ${err.message}`);
      }
    }
    ...
    if (this._cloudEnabled && this._cloudApi) {
      ...
      if (!successLocally) {
        this.log(`[WiFi-Hybrid] ☁️ FALLBACK: Sending Code ${code}=${value} via Cloud`);
