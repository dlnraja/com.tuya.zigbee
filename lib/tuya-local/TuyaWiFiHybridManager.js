'use strict';
// v9.0.40: DEPRECATED - This module is currently unused (dead code).
// No driver imports or instantiates it. Kept for potential future cloud fallback integration.
// If you want to integrate it, pass `homey` to TuyaLocalClient and TuyaShadowPulsar.init(homey).
...
class TuyaWiFiHybridManager extends EventEmitter {
  ...
  async initialize() {
    ...
    if (this._cloudEnabled) {
      this.syncMappings().catch(err => this.error(err));
  ...
  _initCloud() {
    ...
    if (accessId && accessSecret) {
      ...
      // v9.0.405: start the outbound drain — without init() the leaky-bucket
      // queue was configured but NEVER drained (cloud shadowing was receive-only).
      TuyaShadowPulsar.init(this.homey);
    }
  }
