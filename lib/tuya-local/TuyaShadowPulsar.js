...
class TuyaShadowPulsar {
    constructor() {
        ...
        // v9.0.40: Use native setInterval (no homey dependency at import time)
        this.pulsarTimer = null; // Started lazily via init()
    ...
    /**
     * Initialize the pulsar timer. Must be called after app is ready.
     * @param {object} [homey] - Optional homey reference for logging
     */
    init(homey) {
        if (this.pulsarTimer) return; // Already started
        if (homey && homey.setInterval) {
            this.pulsarTimer = homey.setInterval(() => { if (this._destroyed) return; this._drainBucket(); }, this.RATE_LIMIT_MS);
        } else {
            // v5.12.48 (backport): this.homey n'existe pas dans cette classe — fallback global
            this.pulsarTimer = globalThis.setInterval(() => { if (this._destroyed) return; this._drainBucket(); }, this.RATE_LIMIT_MS);
        }
    }
