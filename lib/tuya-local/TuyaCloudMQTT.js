...
class TuyaCloudMQTT {
  ...
  async _connect() {
    ...
    try {
      ...
      this.client.on('connect', () => {
        ...
        this._consecutiveFailures = 0; // v9.0.40: Reset on success
        this._reconnectDelay = 30000; // v9.0.40: Reset backoff
      ...
      this._consecutiveFailures++; // v9.0.40: Track failures for backoff
      const backoff = Math.min(this._reconnectDelay * Math.pow(2, this._consecutiveFailures - 1), this._maxReconnectDelay);
      this._scheduleReconnect(backoff);
    }
  }
  ...
  _scheduleReconnect(ms) {
    if (this._reconnectTimer) {clearTimeout(this._reconnectTimer);}
    this._reconnectTimer = (this.homey && typeof this.homey.setTimeout === 'function' ? this.homey : globalThis).setTimeout(() => { if (this._destroyed) {return;} this._connect(); }, ms);
  }
