...
class TuyaLocalDriver extends Homey.Driver {
  ...
  async onPair(session) {
    ...
    session.setHandler('manual_add', async (data) => {
      ...
      // v10.10.0 (L99): TCP probe BEFORE creating the device — connecting
      // with an invalid local_key would otherwise create a device that
      // loops on auth errors forever. 8s timeout; a clear error is thrown
      // so the user fixes the key instead of getting a zombie device.
      const probeResult = await this._probeLocalKey({
        id: device_id,
        key: local_key,
      ...
      if (!probeResult.ok) {
        throw new Error(`Cannot reach device with these credentials: ${probeResult.reason}. Check the local_key (it changes on every re-pair) and the IP.`);
      }
    ...
    session.setHandler('one_click_auto_pair', async () => {
      try {
        ...
        pairMethod = 'cloud_key_lookup';
        // Step 1: login + devices with local keys
        this._auth = new TuyaSmartLifeAuth({ region, log: this });
        const loginRes = await this._auth.loginWithApiKey(accessId, accessSecret);
        if (!loginRes.success) {throw new Error(loginRes.error || 'API login failed');}
        const devRes = await this._auth.getDevicesWithLocalKeys();
        ...
        // Step 2: LAN discovery (global UDP cache first, on-demand scan fallback)
        let lanDevices = this.safeApp?._tuyaUDPDiscovery ? this.safeApp?._tuyaUDPDiscovery.devices : [];
        if (!lanDevices.length) {
          try {
            this._discovery = new TuyaDeviceDiscovery({ log: this, timeout: 8000 });
            lanDevices = await this._discovery.scan();
          } catch (e) { this.log('[TuyaLocal] one-click LAN scan failed:', e.message); }
        }

        // Step 3: match cloud ↔ LAN, keep only devices with a local key
        discoveredDevices = TuyaDeviceDiscovery.matchDevices(devRes.devices, lanDevices);
        const pairable = discoveredDevices.filter(d => d.local_key);
        this.log(`[TuyaLocal] one-click auto-pair: ${pairable.length} device(s) ready (cloud: ${devRes.devices.length}, LAN: ${lanDevices.length})`);
        return {
          success: true,
          count: pairable.length,
          cloud_count: devRes.devices.length,
   ...
   * a Tuya local connection before the device is created. 8s hard timeout.
  ...
  async _probeLocalKey(cfg) {
    ...
    try {
      ...
      probe = new TuyAPI({ id: cfg.id, key: cfg.key, version: cfg.version });
      const result = await Promise.race([
        (async () => {
          if (cfg.ip) {
            await probe.find({ timeout: 5000 }).catch(() => {});
          } else {
            await probe.find({ timeout: 6000 });
          }
          await probe.connect();
          return { ok: true, ip: probe.device?.ip || cfg.ip };
        })(),
        // v9.0.422 (P92.129): module-level native setTimeout — one-shot
        // watchdog resolving the probe race after 8s (no Homey device
        // context exists at this point in the local-connect probe).
        new Promise((resolve) => setTimeout(() => resolve({ ok: false, reason: 'timeout (8s)' }), 8000))
      ]);
      return result;
    } catch (err) {
      return { ok: false, reason: err.message || 'connection failed' };
    } finally {
      try { if (probe) {await probe.disconnect();} } catch { /* ignore */ }
    }
  }
}
