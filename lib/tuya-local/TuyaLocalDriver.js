...
class TuyaLocalDriver extends Homey.Driver {
  ...
  /**
   * P2621: Dedicated wifi_* drivers override with Tuya category Set.
   * Empty / unset = accept all (wifi_generic / wifi_heater / …).
  ...
  /** Soft-filter cloud/LAN list for dedicated category drivers. */
  _filterByExpectedCategory(devices) {
    const cats = this.getExpectedTuyaCategories?.();
    if (!cats || !(cats instanceof Set) || cats.size === 0) return devices;
    const matched = (devices || []).filter((d) => {
      const c = String(d.category || d.tuya_category || '').toLowerCase();
      if (!c) return true; // unknown category — still show (user can pick)
      return cats.has(c);
    });
    // If filter emptied the list, fall back to full list (better UX than empty pair)
    return matched.length ? matched : devices;
  }
  ...
  async onPair(session) {
    ...
    session.setHandler('login', async (data) => {
      try {
        if (data.mode === 'simple') {
          ...
          const accessId = this.homey.settings.get('tuya_cloud_access_id');
          const accessSecret = this.homey.settings.get('tuya_cloud_access_secret');
          const settingsRegion = this.homey.settings.get('tuya_cloud_region');
          const autoRegionSetting = this.homey.settings.get('tuya_cloud_auto_region');
          const autoRegion = data.autoRegion !== false && autoRegionSetting !== false && autoRegionSetting !== '0';
          const region = normalizeRegion(data.region || settingsRegion || 'eu');
          ...
          // WHY(P2471/P2476): email OR phone (+ country code); authorized-login + auto-region/schema.
          const username = data.email || data.username || data.phone || '';
          const loginRes = await cloudApi.login(username, data.password, {
            countryCode: data.countryCode,
            schema: data.schema || 'tuyaSmart',
            region,
            autoRegion,
            email: data.email,
            phone: data.phone,
          ...
          if (!loginRes.success) {
            throw new Error(loginRes.msg || 'Easy login failed. Check email/phone, password, Region.');
          }

          // Persist winning region so next pair / settings stay aligned
          const usedRegion = normalizeRegion(loginRes.region || cloudApi.region || region);
          try {
            await this.homey.settings.set('tuya_cloud_region', usedRegion);
          } catch (_e) { /* soft */ }
          ...
          this._auth = new TuyaSmartLifeAuth({ region: usedRegion, log: this, autoRegion: true });
          ...
          const matched = await this._matchCloudAndLan(devRes.devices);
          discoveredDevices = matched.devices;
          return { count: discoveredDevices.length, region: usedRegion, schema: loginRes.schema, ...matched };
    ...
    session.setHandler('list_devices', async () => {
      ...
      const filtered = this._filterByExpectedCategory(discoveredDevices);
      return filtered
