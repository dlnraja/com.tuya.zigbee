...
class TuyaDiscoveryEngine extends EventEmitter {
  ...
  getDevice(id) {
    ...
    if (entry && (Date.now() - entry.lastSeen < this._deviceTTL)) {
      return { ...entry.info, lastSeen: entry.lastSeen };
    }
    return null;
  }
  ...
  _handleUDPMessage(msg, rinfo, encrypted, gcm) {
    ...
    let info;
    try { info = JSON.parse(payload); } catch (e) { return; }
    if (!info || !info.gwId) return;
