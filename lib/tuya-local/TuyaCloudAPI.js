...
class TuyaCloudAPI {
  ...
  async login(username, password, options = {}) {
    // WHY(P2476): Smart Life accounts may be phone-only — normalize before IoT login.
    const { normalizeTuyaUsername } = require('./normalizeTuyaUsername');
    const norm = normalizeTuyaUsername(username, options.countryCode);
