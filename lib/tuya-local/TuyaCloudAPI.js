...
const {
  REGIONS: AUTH_REGIONS,
  normalizeRegion,
  getRegionEndpoint,
} = require('./TuyaAuthCatalog');

const ENDPOINTS = Object.fromEntries(
  Object.entries(AUTH_REGIONS).map(([id, meta]) => [id, meta.endpoint])
);
// legacy aliases kept for callers
ENDPOINTS.cn = ENDPOINTS.cn || getRegionEndpoint('cn');
...
class TuyaCloudAPI {
  ...
  async login(username, password, options = {}) {
    // WHY(P2471): Easy Login must use associated-users authorized-login (not iot-03/users/login),
    // with country_code + schema, and auto-region × schema fallback when the account DC differs.
    const {
      buildLoginIdentity,
      resolveLoginRegions,
      resolveLoginSchemas,
      buildAuthorizedLoginBody,
    } = require('./TuyaUserLogin');
    const { normalizeAppSchema } = require('./TuyaAuthCatalog');

    const identity = buildLoginIdentity({
      username,
      email: options.email,
      phone: options.phone,
    ...
    const preferredSchema = normalizeAppSchema(options.schema || 'tuyaSmart');
    const autoRegion = options.autoRegion !== false;
    const regions = resolveLoginRegions(options.region || this.region, { autoRegion });
    const schemas = resolveLoginSchemas(preferredSchema);

    let lastRes = { success: false, msg: 'Login failed' };

    for (const region of regions) {
      this.region = region;
      this.endpoint = getRegionEndpoint(region);
      ...
      for (const schema of schemas) {
        ...
        this.log(`[P2471] authorized-login region=${region} schema=${schema} kind=${identity.kind}`);
        let res = await this._post('/v1.0/iot-01/associated-users/actions/authorized-login', body);

        // Soft fallback for projects still linked via legacy iot-03 login
        if (!res?.success && (res?.code === 1106 || res?.code === 1100 || /not exist|invalid/i.test(String(res?.msg || '')))) {
          res = await this._post('/v1.0/iot-03/users/login', {
        ...
        if (res?.success && res.result) {
          const { access_token, refresh_token, uid, expire, expire_time } = res.result;
          const ttlSec = Number(expire_time != null ? expire_time : expire) || 7200;
          this.tokenInfo = {
            access_token,
            refresh_token,
            uid,
            expire: (ttlSec * 1000) + Date.now(),
          };
          return {
            ...res,
            region,
            schema,
            identity: { kind: identity.kind, countryCode: identity.countryCode },
          };
        }
        lastRes = res || lastRes;
      }
        ...
        res.on('end', () => {
          ...
          try {
            ...
            try {
              ...
              if (res.statusCode === 429 || looksLikeCloudRateLimit(parsed) || looksLikeCloudRateLimit({ code: res.statusCode })) {
                markCloudRateLimited({ reason: 'http_' + res.statusCode + '_code_' + String(parsed && parsed.code) });
              }
            } catch (_) { /* optional local-first signal */ }
            resolve(parsed);
          } catch (err) {
