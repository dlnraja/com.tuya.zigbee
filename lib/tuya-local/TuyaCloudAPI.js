...
const COUNTRY_TO_REGION = {
  1: 'us', 33: 'eu', 34: 'eu', 39: 'eu', 44: 'eu', 46: 'eu', 47: 'eu',
  49: 'eu', 55: 'us', 61: 'eu', 65: 'eu', 81: 'eu', 82: 'eu', 86: 'cn',
  91: 'in', 351: 'eu', 352: 'eu', 353: 'eu', 358: 'eu', 372: 'eu',
  380: 'eu', 420: 'eu', 421: 'eu', 43: 'eu', 48: 'eu', 7: 'eu',
};
...
class TuyaCloudAPI {
        ...
        res.on('end', () => {
          try {
            ...
            if (p.success === false) {
              ...
              if (code === 1010 || msg.includes('token')) msg = 'Token expired or invalid — re-authenticate';
  ...
  async loginWithSmartLife(email, password, countryCode, schema) {
    ...
    this._log('[CLOUD] Smart Life login: ' + email + ' (country ' + countryCode + ', schema ' + schema + ')');
  ...
  async _getDevicesForUser(uid) {
    const resp = await this._request('GET', '/v1.0/users/' + uid + '/devices');
  ...
  async getDeviceById(deviceId) {
    await this.getToken();
    const resp = await this._request('GET', '/v1.0/devices/' + deviceId);
    const d = resp.result;
    return {
      deviceId: d.id,
      name: d.name || '',
