...
/**
 * LAN TCP protocol versions (tuyapi / TinyTuya).
 * P2619: Keep a complete set; auto cascade prefers real-world frequency (com.tuyalocal).
 */
const PROTOCOL_VERSIONS = Object.freeze(['3.5', '3.4', '3.3', '3.2', '3.1']);

/**
 * Auto-detect / pairing try order — frequency-first (andiwirz/com.tuyalocal 1.0.234).
 * WHY: 3.3 is by far the most common; newest-first wastes timeouts on silent 3.5 probes.
 * Contre quoi: pairing that only succeeds on 3.3 after four failed attempts.
 */
const PAIRING_PROTOCOL_ORDER = Object.freeze(['3.3', '3.4', '3.1', '3.5', '3.2']);
 ...
 * Auto / empty → frequency-first PAIRING_PROTOCOL_ORDER (P2619 complementary).
 * Explicit preferred → that version first, then remaining in pairing order.
 * @param {string} [preferred]
 * @returns {string[]}
 */
function buildProtocolFallbackChain(preferred) {
  const pref = String(preferred || '').trim();
  const order = [...PAIRING_PROTOCOL_ORDER];
  if (!pref || pref === 'auto') return order;
  if (!PROTOCOL_VERSIONS.includes(pref) && !order.includes(pref)) return order;
  const rest = order.filter((v) => v !== pref);
  return [pref, ...rest];
}
