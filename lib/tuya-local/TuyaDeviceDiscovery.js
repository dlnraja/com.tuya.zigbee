...
class TuyaDeviceDiscovery extends EventEmitter {
  ...
  _decryptUDPGCM(msg) {
    ...
    return decryptUdpGcm(raw);
  }

  _decryptUDP(msg) {
    const raw = stripUdpFrame(msg);
    return decryptUdpEcb(raw);
  }
  ...
  /** TinyTuya active probe during pairing scan */
  _sendActiveProbe() {
    const payload = Buffer.from(buildActiveProbePayload('255.255.255.255'));
    for (const sock of this._sockets) {
      try {
        sock.send(payload, PORT_UNENCRYPTED, '255.255.255.255');
        sock.send(payload, PORT_ENCRYPTED, '255.255.255.255');
      } catch { /* non-fatal */ }
    }
  }
