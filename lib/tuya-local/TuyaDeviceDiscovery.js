...
class TuyaDeviceDiscovery extends EventEmitter {
  ...
  stop() {
    this._running = false;
    if (this._scanTimer) {
      safeClearTimeout(this._scanTimer);
      this._scanTimer = null;
    }
  ...
  static classifyDeviceType(category, productKey, productName) {
    ...
    if (cat === 'wk' || name.includes('thermostat') || name.includes('trv')) {
      return 'Thermostat';
    }
    // P2621 UNION — dedicated wifi_heat_pump / wifi_kettle / wifi_ev_charger
    if (cat === 'rs' || cat === 'rsx' || cat === 'rsq' || name.includes('heat pump') || name.includes('heatpump') || name.includes('pompe à chaleur')) {
      return 'Heat Pump';
    }
    if (cat === 'bh' || cat === 'bhj' || name.includes('kettle') || name.includes('bouilloire') || name.includes('water cooker')) {
      return 'Smart Kettle';
    }
    if (cat === 'qccdz' || cat === 'qccd' || name.includes('ev charger') || name.includes('wallbox') || name.includes('borne') || pk.includes('evcharger') || pk.includes('wallbox')) {
      return 'EV Charger';
    }
    if (cat === 'qn' || name.includes('heater') || name.includes('radiator') || name.includes('convector')) {
      return 'Heater';
    }
