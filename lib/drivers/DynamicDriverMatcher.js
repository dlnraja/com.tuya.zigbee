...
class DynamicDriverMatcher {
  constructor() {
    ...
    this.ambiguousWildcards = new Set(); // Wildcards revendiqués par plusieurs drivers
  ...
  _normalizeDeviceId(deviceId) {
    ...
    // v10.0.0: Use TuyaNormalizer (NFKD + accents + lowercase) for case-insensitive
    return TU.normalizeTuyaID(deviceId).replace(/[-_\s]+/g, '').replace(/^0x/i, '');
  }
  ...
  buildIndex(drivers) {
    ...
    for (const driver of drivers) {
      ...
      for (const fp of driver.fingerprints || []) {
        ...
        // v10.0.0: Use TuyaNormalizer for case-insensitive key construction
        const key = TU.normalize(`${mfr}:${model}`);
        this.fingerprintIndex.set(key, driver);
        
        // Index alternatif avec wildcards pour matching flexible.
        // Un wildcard revendiqué par plusieurs drivers est ambigu : le
        // retirer plutôt que laisser le dernier écrivain gagner, sinon un
        // device sans match exact est routé vers un driver arbitraire
        // (ex. soil sensor TS0601 appairé en Curtain Module, forum #2091).
        this._setWildcard(`*:${model}`, driver);
        if (mfr !== '*') {
          this._setWildcard(`${mfr}:*`, driver);
        }
      }
    }
  }
  ...
  /**
   * Enregistre une clé wildcard sans écraser un driver concurrent.
   * @private
   */
  _setWildcard(key, driver) {
    if (this.ambiguousWildcards.has(key)) {return;}
    const existing = this.fingerprintIndex.get(key);
    if (existing && existing !== driver) {
      this.fingerprintIndex.delete(key);
      this.ambiguousWildcards.add(key);
      return;
    }
    this.fingerprintIndex.set(key, driver);
  }
  ...
  // Log helper - no-op without device context
  log() {}
}
