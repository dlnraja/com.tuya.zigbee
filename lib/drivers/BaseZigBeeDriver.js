...
class BaseZigBeeDriver extends ZigBeeDriver {
  ...
  async onInit() {
    await super.onInit();
    const driverId = this.id || this.manifest?.id || 'unknown';

    // Normalize manufacturer-related settings and store values
    try {
      ...
      for (const device of devices) {
        if (typeof device.getSettings === 'function') {
          ...
          if (settings.tuya_manufacturer_name && typeof settings.tuya_manufacturer_name === 'string') {
            const normalized = settings.tuya_manufacturer_name.toLowerCase();
  ...
  async onPairListDevices(devices) {
    ...
    for (const device of devices) {
      ...
      if (device.data?.manufacturerName) {
        device.data.manufacturerName = device.data.manufacturerName.toLowerCase();
      }
      if (device.settings?.tuya_manufacturer_name) {
        device.settings.tuya_manufacturer_name = device.settings.tuya_manufacturer_name.toLowerCase();
      }
