...
class BaseZigBeeDriver extends ZigBeeDriver {
  ...
  async onPairListDevices(devices) {
    try {
      const { logPairingIdentityBatch } = require('../pairing/logPairingIdentity');
  ...
  getDeviceById(id) {
    try {
      ...
      // Attempt manual fallback search from initialized devices
      try {
        const devices = this.getDevices();
        const found = devices.find(d => {
          const data = d.getData();
          return data && (data.id === id || data.ieeeAddress === id);
        });
        if (found) {
           this.log(`[RECOVERY] Found device manually: ${id}`);
           return found;
        }
      } catch (e) { /* Manual device lookup failed */ }

      // To prevent `Cannot read properties of null (reading '_onSetCapabilityValue')` when UI interacts with an uninitialized device
      // we throw a standard error which Homey handles gracefully.
      throw new Error(`Device not initialized or found: ${id}`);
    }
  }

  /**
   * Safe Flow Card Getter.
   * WHY (P2398): Prefer getConditionCard / getActionCard first — Homey SDK3
   * often has no getDeviceConditionCard / getDeviceActionCard. Preferring the
   * Device* getters hit App FLOW-GUARD noops and skipped real cards
   * (water_valve_garden_is_open on 2b0b4e4f / 9.0.743). Skip __flowGuardNoop.
   */
  _getFlowCard(id, type = 'trigger') {
    ...
    // Triggers: Device* is the SDK3 canonical API. Conditions/actions: non-Device first.
      ...
      try {
        ...
        if (typeof card.registerRunListener === 'function') {
          ...
          card.registerRunListener = (handler) => {
            if (global._registeredFlowCardListeners.has(key) || global._registeredFlowCardListeners.has(id)) {return card;}
