...
phase('dual-claim-compose-gate', () => run('dual-claim', 'tools/ci/dual-claim-compose-gate.js'));
phase('align-mfs-db-intelligent --check', () =>
  run('align-mfs', 'tools/ci/align-mfs-db-intelligent.js', ['--check']));
...
// Recommendations from dual-claim tail
...
if (dual && /Found \d+ dual-claim/i.test(dual.tail || '')) {
  ...
  if (m && Number(m[1]) > 0) {
    summary.recommendations.push(
      'Dual-claim conflicts remain — triage with registry + Z2M/ZHA then surgical compose edit (no codegen).'
    );
  }
}
summary.recommendations.push(
