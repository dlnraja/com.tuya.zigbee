...
>
> **MANDATORY DEVICE TRUTH STEP**: Read [docs/knowledge/DEVICE_TRUTH.md](docs/knowledge/DEVICE_TRUTH.md) then look up the relevant driver in [docs/knowledge/device-truth.json](docs/knowledge/device-truth.json) (431 drivers, 1-by-1). For DP/cluster/source notes per couple, read [docs/knowledge/PECULIARITIES.md](docs/knowledge/PECULIARITIES.md). Lock **manufacturerName + productId** (app is couple-native; one mfs mfr may list many pids — NORMAL). Machine SSOT: [config/architecture/sacred-couple-ssot.json](config/architecture/sacred-couple-ssot.json) · [docs/architecture/SACRED_COUPLE_SSOT.md](docs/architecture/SACRED_COUPLE_SSOT.md) · `npm run check:p2494`. Never invent a pid. **Publish** = Homey App Store; **do not post** = no Homey Community / PM replies.
>
