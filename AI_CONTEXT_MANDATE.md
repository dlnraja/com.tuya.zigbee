...
>
> **MANDATORY DEVICE TRUTH STEP**: Read [docs/knowledge/DEVICE_TRUTH.md](docs/knowledge/DEVICE_TRUTH.md) then look up the relevant driver in [docs/knowledge/device-truth.json](docs/knowledge/device-truth.json) (431 drivers, 1-by-1). For DP/cluster/source notes per couple, read [docs/knowledge/PECULIARITIES.md](docs/knowledge/PECULIARITIES.md). Lock **manufacturerName + productId** (Homey compose `productId` = Zigbee Model ID / `modelId` = internal `pid`; **productName** is catalog-only — see [docs/architecture/IDENTITY_FIELDS_SSOT.md](docs/architecture/IDENTITY_FIELDS_SSOT.md)). One mfs mfr may list many pids — NORMAL. Machine SSOT: [config/architecture/sacred-couple-ssot.json](config/architecture/sacred-couple-ssot.json) · [docs/architecture/SACRED_COUPLE_SSOT.md](docs/architecture/SACRED_COUPLE_SSOT.md) · `npm run check:p2494` / `check:p2496`. Never invent a pid. **Publish** = Homey App Store; **do not post** = no Homey Community / PM replies.
>
> **P2469 UNIT-TEST ANTI-RÉGRESSION (every prompt):** After any behavior fix, **add or extend** a smart `test/critical/pNNNN-*.test.js` that fails if the bug class returns (Contre quoi / P215). Wire `npm run check:pNNNN`. Prefer focused filesystem / pure-logic locks — no invent pid, no mega brittle snapshots. Cursor rule: [`.cursor/rules/unit-test-anti-regression-always.mdc`](.cursor/rules/unit-test-anti-regression-always.mdc) · doctrine: [`docs/rules/UNIT_TEST_ANTI_REGRESSION.md`](docs/rules/UNIT_TEST_ANTI_REGRESSION.md) · gate: `npm run check:p2469` / `npm run check:p246x`.
>
> **P2520 COMPLEMENTARY VARIANT ENRICH (every prompt / every enrich):** Enrichments are **variants and additions**, never degradations. Union `manufacturerName`/`productId`/capabilities; append settings by id; never wipe compose arrays. Helper: [`lib/enrichment/ComplementaryMerge.js`](lib/enrichment/ComplementaryMerge.js). Cursor rule: [`.cursor/rules/complementary-variant-enrich-always.mdc`](.cursor/rules/complementary-variant-enrich-always.mdc) · doctrine: [`docs/rules/COMPLEMENTARY_VARIANT_ENRICH.md`](docs/rules/COMPLEMENTARY_VARIANT_ENRICH.md) · SSOT: [`config/architecture/complementary-variant-enrich-ssot.json`](config/architecture/complementary-variant-enrich-ssot.json) · gates: `npm run check:p2520` · `npm run check:p2519`.
>
> **P2269 ANTI-SPAGHETTI SSOT**: Prefer [SPAGHETTI_MAP.md](docs/architecture/SPAGHETTI_MAP.md), [PROTOCOL_TX_RX_SSOT.md](docs/architecture/PROTOCOL_TX_RX_SSOT.md), [BATTERY_SSOT.md](docs/architecture/BATTERY_SSOT.md), [TIME_SYNC_SSOT.md](docs/architecture/TIME_SYNC_SSOT.md), [PARSER_SSOT.md](docs/architecture/PARSER_SSOT.md), [PUBLISH_SSOT.md](docs/architecture/PUBLISH_SSOT.md), [COMM_PATHFINDING.md](docs/architecture/COMM_PATHFINDING.md), [KNOB_FLOW_WIRING_SSOT.md](docs/architecture/KNOB_FLOW_WIRING_SSOT.md). Machine publish SSOT: [config/architecture/publish-ssot.json](config/architecture/publish-ssot.json). Harvest ≥50: `reports/discussion-harvest-2026-08-26/`. Gates: `npm run check:p2269` · `npm run check:p2270` · `npm run check:p2278` · `npm run check:p2279` · `npm run check:p2284`–`p2288` · `npm run check:p244x`. Dual track SSOT: [config/architecture/dual-app-tracks.json](config/architecture/dual-app-tracks.json). L99 apply log: `reports/l99-reco-apply-2026-08-26/`.
>
