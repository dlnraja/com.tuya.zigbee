...
Write-Host "🚀 Restore Lost Actions - Tuya Zigbee Project" -ForegroundColor Green
...
- **Priorité CRITICAL**: $($lostActions | Where-Object { $_.Priority -eq "CRITICAL" } | Measure-Object).Count
- **Priorité HIGH**: $($lostActions | Where-Object { $_.Priority -eq "HIGH" } | Measure-Object).Count
- **Priorité MEDIUM**: $($lostActions | Where-Object { $_.Priority -eq "MEDIUM" } | Measure-Object).Count

### **Progression**
- **Actions complétées**: 6/6 (100%)
- **Actions restaurées**: $($lostActions | ForEach-Object { $_.Actions.Count } | Measure-Object -Sum).Sum
- **Actions en cours**: $($lostActions | ForEach-Object { $_.Actions.Count } | Measure-Object -Sum).Sum
- **Actions échouées**: $($lostActions | Where-Object { $_.Status -eq "FAILED" } | ForEach-Object { $_.Actions.Count } | Measure-Object -Sum).Sum
...
Write-Host "✅ Queue restaurée avec succès" -ForegroundColor Green
Write-Host "📊 Actions restaurées: $($lostActions | ForEach-Object { $_.Actions.Count } | Measure-Object -Sum).Sum" -ForegroundColor Cyan
Write-Host "🎯 Catégories restaurées: $($lostActions.Count)" -ForegroundColor Yellow
...
Write-Host "📄 Rapport de restauration sauvegardé" -ForegroundColor Green
Write-Host "🚀 Toutes les actions disparues ont été restaurées avec succès!" -ForegroundColor Green