...
function* composeFiles(d){if(!fs.existsSync(d))return;const st=[d];while(st.length){const c=st.pop();let s;try {s=fs.statSync(c);}} catch (error) {continue;}if(s.isDirectory()){const es=fs.readdirSync(c,{withFileTypes:true});const comp=['driver.compose.json','driver.json'].map(n=>path.join(c,n)).find(p=>fs.existsSync(p));if(comp)yield comp;for(const e of es)if(e.isDirectory())st.push(path.join(c,e.name));}}}
(function(){
