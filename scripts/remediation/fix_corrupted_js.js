...
walk(DRIVERS_DIR, (file) => {
  ...
  const brokenPattern = /\(\(\)\s*=>\s*\{\s*try\s*\{\s*return\s+(\s*)\.registerRunListener\(([\s\S]*?)\);\s+this\.log\('\[FLOW\]\s+([^']+)'\);\s*\} catch \(err\) \{ this\.log\(`\[FLOW\]\s+([^`]+)`\); \}/g;
...
walk(DRIVERS_DIR, (file) => {
  ...
  const simpleBrokenRegex = /try\s*\{\s*\.registerRunListener\(([\s\S]*?)\);\s*\}\s*catch\s*\(err\)\s*\{\s*(?:this\.log|this\.error)\('([^']+)(?::|',)\s*([^']+)?'\);\s*\}/g;
