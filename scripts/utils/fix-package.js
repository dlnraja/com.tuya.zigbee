...
// Helper toArray - règle d'or sécurité
function toArray(v) {
  if (v == null) return [];
...
function repoUses(mods){
  ...
  const rx=new RegExp(toArray(mods).map(m=>`(['"])${m}\\1`).join('|'));
...
(function(){
  ...
  pkg.scripts.diag    = pkg.scripts.diag    || 'node scripts/diagnose-drivers.js --strict';
  pkg.scripts.assets  = pkg.scripts.assets  || 'node scripts/assets-generate.js';
  pkg.scripts.small   = pkg.scripts.small   || 'node scripts/create-small-png.js';
  ...
  pkg.scripts.restore = pkg.scripts.restore || 'node scripts/sources/local/restore-tmp-sources.js';
  pkg.scripts.sources = pkg.scripts.sources || 'node scripts/sources/sources-orchestrator.js --now';
  pkg.scripts.z2mseed = pkg.scripts.z2mseed || 'node scripts/sources/z2m-seed.js';
  pkg.scripts.mega    = pkg.scripts.mega    || 'node scripts/mega-sources-complete.js';
