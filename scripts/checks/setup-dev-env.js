 ...
 * Original: PS1
 * Converti le: 2025-09-03T20:43:34.458Z
 */
...
# Début du script
console.log "=== Configuration de l'environnement de développement ===" -ForegroundColor Green
console.log "Ce script va configurer votre environnement pour exécuter les scripts de scan."
console.log "------------------------------------------------------"
...
if (Test-Command "node") {
    ...
    if ($majorVersion -lt 14) {
        console.errorMsg "Node.js version $nodeVersion est trop ancienne. Version 14 ou supérieure requise."
        console.log "Veuillez mettre à jour Node.js depuis https://nodejs.org/" -ForegroundColor Yellow
        exit 1
    }
} else {
    console.errorMsg "Node.js n'est pas installé ou n'est pas dans le PATH"
    console.log "Veuillez installer Node.js depuis https://nodejs.org/ et réessayer" -ForegroundColor Yellow
    exit 1
}
...
foreach ($dep in $globalDeps) {
    if (-not (Test-Command $dep)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            console.errorMsg "Échec de l'installation de $dep"
        } else {
...
if (fs.existsSync "package.json") {
    ...
    if (fs.existsSync "package-lock.json") {
        $backupName = "package-lock.json.backup.$(new Date() -Format 'yyyyMMddHHmmss')"
    ...
    console.errorMsg "Le fichier package.json est introuvable"
    exit 1
}
...
if (-not (fs.existsSync "drivers")) {
    console.errorMsg "Le répertoire 'drivers' est introuvable"
    exit 1
}

# Vérifier si le fichier app.json existe
if (-not (fs.existsSync "app.json")) {
    console.errorMsg "Le fichier 'app.json' est introuvable"
    exit 1
}
...
console.log "Vous pouvez maintenant exécuter les scripts de scan avec les commandes suivantes:"
console.log "- npm run scan:scout     # Pour l'analyse des datapoints"
console.log "- npm run scan:architect # Pour l'analyse de l'architecture"
console.log "- npm run scan:full      # Pour une analyse complète"
