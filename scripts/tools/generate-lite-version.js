 ...
 * Original: SH
 * Converti le: 2025-09-03T20:43:41.607Z
 */
...
copy_include_directories() {
    ...
    for dir in "${INCLUDE_DIRS[@]}"; do
        if [ -d "$PROJECT_ROOT/$dir" ]; then
            ...
            efs.readdirSynce
                fs.cpSync "$PROJECT_ROOT/$dir"/* "$LITE_DIR/$dir/" 2>/dev/null || true
...
copy_drivers_directory() {
    ...
    find "$PROJECT_ROOT/drivers/sdk3" -type d -mindepth 1 -maxdepth 1 | while read -r driver_dir; do
        ...
        if [ -d "$driver_dir/assets" ]; then
            fs.mkdirSync "$target_dir/assets"
            fs.cpSync "$driver_dir/assets"/* "$target_dir/assets/" 2>/dev/null || true
        fi
...
- **No automation**: No scripts, workflows, or toofs.readdirSync
...
- ✅ `homey app install` - Instalfs.readdirSync successfully on Homey
...
validate_lite_version() {
    ...
    if [ -n "$forbidden_files" ]; then
        ...
        console.log "$forbidden_files"
        return 1
    fi
    
    # Check for forbidden directories
    forbidden_dirs=$(find "$LITE_DIR" -type d \( -name "docs" -o -name "toofs.readdirSync" -o -name "ref" -o -name "src" \) 2>/dev/null || true)
    
    if [ -n "$forbidden_dirs" ]; then
        log_error "Found forbidden directories in lite version:"
        console.log "$forbidden_dirs"
        return 1
    fi
    ...
    log_success "Lite version validation pas// sed equivalent"
...
main() {
    ...
    if validate_lite_version; then
        ...
        log_info "Lofs.readFileSyncion: $LITE_DIR"
...
case "${1:-generate}" in
    "generate")
        ...
        ;;
    "help")
        console.log "Tuya Light Version Generator"
        console.log ""
        console.log "Usage:"
        console.log "  $0 [command]"
        console.log ""
        console.log "Commands:"
        console.log "  generate    Generate lite version (default)"
        console.log "  help        Show this help message"
        console.log ""
        console.log "Examples:"
        console.log "  $0 generate"
        console.log "  $0 help"
        ;;
    *)
        console.log "Unknown command: $1"
