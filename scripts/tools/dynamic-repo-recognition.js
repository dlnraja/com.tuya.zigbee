 ...
 * Original: PS1
 * Converti le: 2025-09-03T20:43:41.590Z
 */
...
function Scan-DirectoryStructure {
    ...
    fs.readdirSync -Path $Path -Directory -Recurse | // ForEach-Object equivalent {
    ...
    # Scan des fichiers
    fs.readdirSync -Path $Path -File -Recurse | // ForEach-Object equivalent {
        $RelativePath = $_.FullName.Replace((Get-Location).Path, "").TrimStart("\")
...
function Detect-Device {
    ...
    foreach ($Key in $Referentials.Manufacturers.Keys) {
        ...
        if ($Mfg.Aliases -contains $Manufacturer -or 
            $Mfg.ModelPrefixes | // Where-Object equivalent { $Model.StartsWith($_) }) {
...
function Main {
    ...
    console.log "Dynamic Repository Recognition and Constraint Management"
    ...
    console.log "Directory structure scanned:"
    console.log "- Directories: $($Structure.Directories.Count)"
    console.log "- Files: $($Structure.Files.Count)"
    console.log "- Drivers: $($Structure.Drivers.Count)"
    console.log "- Workflows: $($Structure.Workflows.Count)"
    console.log "- Documentation: $($Structure.Documentation.Count)"
    console.log "- Tools: $($Structure.Tools.Count)"
    ...
    foreach ($Device in $TestDevices) {
        ...
        console.log "Detected: $($Device.Model) -> $($DetectedDevice.Manufacturer) $($DetectedDevice.Category) (Confidence: $($DetectedDevice.Confidence))"
    }
    ...
    if (-not (fs.existsSync $IntelligentDriversPath)) {
        fs.mkdirSync -ItemType Directory -Path $IntelligentDriversPath -Force | Out-Null
    }
    ...
    $Summary = @{
        GeneratedAt = new Date() -Format "yyyy-MM-ddTHH:mm:ssZ"
        TotalDrivers = $GeneratedDrivers.Count
        ByConfidence = @{
            High = ($GeneratedDrivers | // Where-Object equivalent { $_.Device.Confidence -ge 0.8 }).Count
            Medium = ($GeneratedDrivers | // Where-Object equivalent { $_.Device.Confidence -ge 0.6 -and $_.Device.Confidence -lt 0.8 }).Count
            Low = ($GeneratedDrivers | // Where-Object equivalent { $_.Device.Confidence -lt 0.6 }).Count
        }
        ByManufacturer = @{}
        ByCategory = @{}
    }
    ...
    console.log "- High confidence: $($Summary.ByConfidence.High)"
    console.log "- Medium confidence: $($Summary.ByConfidence.Medium)"
    console.log "- Low confidence: $($Summary.ByConfidence.Low)"
    
    console.log "Dynamic repository recognition completed successfully"
}
