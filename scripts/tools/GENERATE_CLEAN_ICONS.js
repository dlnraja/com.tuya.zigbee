...
class SmartImageGenerator {
    ...
    async generateDriverImage(driverName, size) {
        ...
        // Power badge (bottom right - small)
        const powerType = driverName.toLowerCase().includes('battery') || driverName.toLowerCase().includes('cr2032') ? '🔋' : '⚡';
        const badgeSize = width * 0.12;
        const badgeX = width - badgeSize;
        const badgeY = height - badgeSize * 1.2;
        
        ctx.fillStyle = 'rgba(255, 255, 255, 0.95)';
        ctx.beginPath();
        ctx.arc(badgeX, badgeY, badgeSize * 0.5, 0, 2 * Math.PI);
        ctx.fill();
        
        ctx.font = `${badgeSize * 0.7}px Arial`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(powerType, badgeX, badgeY);

        // Category badge (bottom center - white like Build 20-45)
        const labelY = height - 20;
        let label = colors.name.toUpperCase();
        if (gangCount > 1 && category === 'switches') label = `${gangCount}G ${label}`;
        ...
        ctx.fillStyle = 'rgba(255, 255, 255, 0.95)';
        const roundRadius = badgeHeight / 2;
        ctx.beginPath();
        ctx.moveTo((width - badgeWidth) / 2 + roundRadius, labelY - badgeHeight / 2);
        ctx.lineTo((width + badgeWidth) / 2 - roundRadius, labelY - badgeHeight / 2);
        ctx.quadraticCurveTo((width + badgeWidth) / 2, labelY - badgeHeight / 2, (width + badgeWidth) / 2, labelY - badgeHeight / 2 + roundRadius);
        ctx.lineTo((width + badgeWidth) / 2, labelY + badgeHeight / 2 - roundRadius);
        ctx.quadraticCurveTo((width + badgeWidth) / 2, labelY + badgeHeight / 2, (width + badgeWidth) / 2 - roundRadius, labelY + badgeHeight / 2);
        ctx.lineTo((width - badgeWidth) / 2 + roundRadius, labelY + badgeHeight / 2);
        ctx.quadraticCurveTo((width - badgeWidth) / 2, labelY + badgeHeight / 2, (width - badgeWidth) / 2, labelY + badgeHeight / 2 - roundRadius);
        ctx.lineTo((width - badgeWidth) / 2, labelY - badgeHeight / 2 + roundRadius);
        ctx.quadraticCurveTo((width - badgeWidth) / 2, labelY - badgeHeight / 2, (width - badgeWidth) / 2 + roundRadius, labelY - badgeHeight / 2);
