...
/**
 * 🚀 SILENT BACKGROUND PROCESSOR - BRIEF "BÉTON"
 * 
 * Processus silencieux en arrière-plan avec 7zip
...
class SilentBackgroundProcessor {
    constructor() {
    ...
    async startSilentExtraction() {
        try {
            ...
            // Lancer l'extraction silencieuse avec 7zip
    ...
    async extractZipSilently(zipName) {
        ...
        try {
            ...
            // Lancer 7zip en mode silencieux
            const process = this.spawn7zipSilently(zipName, downloadPath, extractPath);
            ...
            await this.logSilently(`🔄 Extraction lancée: ${zipName}`);
...
process.on('SIGINT', () => {
    if (global.silentProcessor) {
        global.silentProcessor.stopAllProcesses();
    }
    process.exit(0);
});
...
if (require.main === module) {
    const processor = new SilentBackgroundProcessor();
    global.silentProcessor = processor;
...
module.exports = SilentBackgroundProcessor;
