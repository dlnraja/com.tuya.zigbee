...
function readProgress(){try {return JSON.parse(fs.readFileSync(path.join(QUEUE,'z2m-progress.json'),'utf8'));}} catch (error) {return{cursor:0,processed:[]};}}
function writeProgress(p){fs.mkdirSync(QUEUE,{recursive:true});fs.writeFileSync(path.join(QUEUE,'z2m-progress.json'),JSON.stringify(p,null,2));}
function gh(pathname){return new Promise((resolve)=>{const t=process.env.GITHUB_TOKEN;if(!t)return resolve({ok:false,reason:'NO_TOKEN'});
  const opt={hostname:'api.github.com',path:pathname,headers:{'User-Agent':'homey-tuya-bot','Authorization':`Bearer ${t}`,'Accept':'application/vnd.github+json'}};https.get(opt,res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>{try {resolve({ok:true,json:JSON.parse(d)});}} catch (error) {resolve({ok:false});}});}).on('error',()=>resolve({ok:false}));})}
function raw(pathname){return new Promise((resolve)=>{const t=process.env.GITHUB_TOKEN;if(!t)return resolve({ok:false});
  const opt={hostname:'raw.githubusercontent.com',path:pathname,headers:{'User-Agent':'homey-tuya-bot','Authorization':`Bearer ${t}`}};https.get(opt,res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>resolve({ok:true,text:d}));}).on('error',()=>resolve({ok:false}));})}
function slug(s){return String(s||'').toLowerCase().replace(/[^a-z0-9._-]+/g,'-').replace(/^-+|-+$/g,'');}
function A(v){return Array.isArray(v)?v:(v?[v]:[]);}
function domainOf(vendor){const v=String(vendor||'').toLowerCase();return (/tuya|^_tz|^_ty/.test(v)?'tuya':'zigbee');}
function vendorHeur(vendor){const v=String(vendor||'').toLowerCase();if(/tuya|^_tz|^_ty/.test(v))return'tuya';if(/aqara|lumi/.test(v))return'aqara';if(/ikea|tradfri/.test(v))return'ikea';if(/philips|signify|hue/.test(v))return'philips';if(/sonoff|itead/.test(v))return'sonoff';if(/ledvance|osram/.test(v))return'ledvance';return v||'generic';}
