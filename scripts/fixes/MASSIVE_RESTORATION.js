...
targets.forEach(relPath => {
    ...
    content = content.replace(/Math\.round\(safeParse\(([^)]+)\);/g, 'Math.round(safeParse($1));');
    content = content.replace(/Math\.min\(100,\s*Math\.round\(safeParse\(([^)]+)\)\);/g, 'Math.min(100, Math.round(safeParse($1)));');
    content = content.replace(/Math\.min\(100,\s*Math\.round\(safeParse\(([^)]+)\);\s*\)/g, 'Math.min(100, Math.round(safeParse($1)));');
    content = content.replace(/Math\.min\(100,\s*Math\.round\(safeParse\(([^));]+)\));/g, 'Math.min(100, Math.round(safeParse($1))));'));
