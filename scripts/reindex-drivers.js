...
function* walk(d){const st=[d];while(st.length){const c=st.pop();let s;try {s=fs.statSync(c);}} catch (error) {continue;}if(s.isDirectory()){const es=fs.readdirSync(c);for(const e of es)st.push(path.join(c,e));const comp=['driver.compose.json','driver.json'].map(n=>path.join(c,n)).find(p=>fs.existsSync(p));if(comp)yield comp;}}}
