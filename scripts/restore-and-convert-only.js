...
console.log('🔄 RESTAURATION ET CONVERSION SEULEMENT v3.4.1...');

const fs = require('fs-extra');
const path = require('path');

class RestoreAndConvertOnly {
  constructor() {
  ...
  async run() {
    try {
      console.log('📁 Phase 1: Restauration des fichiers .json supprimés par erreur...');
      await this.restoreJsonFiles();
      
      console.log('📁 Phase 2: Restauration des dossiers supprimés par erreur...');
      await this.restoreDirectories();
      
      console.log('📁 Phase 3: Conversion des scripts PowerShell en JavaScript...');
      await this.convertPowerShellToJS();
      
      console.log('📁 Phase 4: Suppression des scripts PowerShell originaux...');
      await this.removePowerShellScripts();
      
      console.log('📁 Phase 5: Validation de la structure finale...');
      await this.validateFinalStructure();
      
      console.log('✅ RESTAURATION ET CONVERSION TERMINÉES !');
  ...
  async restoreJsonFiles() {
    // Restaurer les fichiers .json de configuration
    const jsonFiles = [
      { source: 'backups/json/config/app.json', target: 'app.json' },
      { source: 'backups/json/config/package.json', target: 'package.json' },
      { source: 'backups/json/config/package-lock.json', target: 'package-lock.json' },
      { source: 'backups/json/linting/.prettierrc', target: '.prettierrc' },
      { source: 'backups/json/linting/.eslintrc.json', target: '.eslintrc.json' },
      { source: 'backups/json/testing/.nycrc.json', target: '.nycrc.json' },
      { source: 'backups/json/testing/.mocharc.json', target: '.mocharc.json' },
      { source: 'backups/json/git/.gitmessage', target: '.gitmessage' },
      { source: 'backups/json/git/CODEOWNERS', target: 'CODEOWNERS' }
    ];

    for (const fileInfo of jsonFiles) {
      const sourcePath = path.join(this.projectRoot, fileInfo.source);
      const targetPath = path.join(this.projectRoot, fileInfo.target);
      
      if (await fs.pathExists(sourcePath)) {
        await fs.move(sourcePath, targetPath);
        console.log(`📁 Restauré: ${fileInfo.target}`);
      }
    }
  }

  async restoreDirectories() {
    // Restaurer les dossiers supprimés par erreur
    const dirsToRestore = [
      'tools',
      'lib', 
      'tests',
    ...
    for (const dirName of dirsToRestore) {
      const sourcePath = path.join(this.backupsPath, dirName);
      const targetPath = path.join(this.projectRoot, dirName);
      
      if (await fs.pathExists(sourcePath)) {
        await fs.move(sourcePath, targetPath);
        console.log(`📁 Restauré: ${dirName}`);
      }
    }

    // Restaurer les fichiers markdown importants
    const markdownFiles = [
      { source: 'backups/markdown/README.md', target: 'README.md' },
      { source: 'backups/markdown/CHANGELOG.md', target: 'CHANGELOG.md' },
      { source: 'backups/markdown/FINAL_REPORT_3.3.0.md', target: 'FINAL_REPORT_3.3.0.md' }
    ];
    ...
    // Restaurer app.js
    const appJsSource = path.join(this.backupsPath, 'markdown', 'app.js');
    const appJsTarget = path.join(this.projectRoot, 'app.js');
    
    if (await fs.pathExists(appJsSource)) {
      await fs.move(appJsSource, appJsTarget);
      console.log('📁 Restauré: app.js');
    }
  }
  ...
  generateJSContent(psScriptName) {
    ...
    switch (baseName) {
      ...
      case 'final-reorganization-push':
        return this.generateFinalReorganizationPushJS();
      default:
...
async function cleanupDirect() {
  try {
    ...
    console.log('📁 Déplacement des fichiers temporaires uniquement...');

    // Fichiers temporaires à déplacer (PAS les fichiers .json, .png, .img)
    const filesToMove = [
    ...
    console.log('⚠️ ATTENTION: Fichiers .json, .png, .img préservés !');
...
async function restoreTuya() {
  try {
    ...
    if (await fs.pathExists(backupPath)) {
      ...
      await createBasicDrivers(tuyaPath);
    }
...
async function createBasicDrivers(tuyaPath) {
  ...
  for (const driver of basicDrivers) {
    ...
    await createDriverFiles(driverPath, driver);
  }
}
...
restoreTuya();`;
  ...
  generateFinalReorganizationPushJS() {
    return `#!/usr/bin/env node
...
class FinalReorganizationPush {
  ...
  async run() {
    try {
      console.log('📁 Phase 1: Validation de la structure finale...');
      await this.validateFinalStructure();
      
      console.log('📁 Phase 2: Push vers GitHub...');
...
// Exécuter la restauration et conversion
const restoration = new RestoreAndConvertOnly();
restoration.run();
