...
# Fonction de nettoyage des références YOLO
function Remove-YoloReferences {
    Write-Host "Nettoyage des références YOLO..." -ForegroundColor Yellow
    ...
    foreach ($file in $filesToClean) {
        if (Test-Path $file) {
            ...
            # Remplacer les références YOLO
            $cleanedContent = $content -replace "YOLO mode|YOLO MODE|yolo mode", "Mode enrichissement"
            $cleanedContent = $cleanedContent -replace "YOLO|yolo", "Enrichissement"
...
# 3. Nettoyage des références YOLO
Remove-YoloReferences
Write-Host "Références YOLO nettoyées" -ForegroundColor Green
