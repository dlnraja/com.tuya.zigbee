...
console.log('🔧 PRÉFIXER FLOW CARDS AVEC NOM DU DRIVER\n');
...
drivers.forEach(driverName => {
  ...
  try {
    ...
    // Fonction pour préfixer avec le driver
    function prefixWithDriver(id) {
      if (!id) return id;
      
      // Si déjà préfixé avec le driver, ne rien faire
      if (id.startsWith(driverName + '_')) {
        return id;
      }
      
      // Sinon, préfixer
      return driverName + '_' + id;
    }
    ...
    if (flowCompose.triggers) {
      flowCompose.triggers.forEach(trigger => {
        ...
        trigger.id = prefixWithDriver(trigger.id);
    ...
    if (flowCompose.conditions) {
      flowCompose.conditions.forEach(condition => {
        ...
        condition.id = prefixWithDriver(condition.id);
    ...
    if (flowCompose.actions) {
      flowCompose.actions.forEach(action => {
        ...
        action.id = prefixWithDriver(action.id);
    ...
    if (modified) {
      // Backup
      const backupPath = flowComposePath + '.backup-prefix.' + Date.now();
      ...
      console.log(`✅ ${driverName}: ${updatedCount} flow cards préfixées`);
    }
...
console.log(`   Flow cards préfixées: ${stats.flowCardsUpdated}`);
...
if (stats.filesModified > 0) {
  ...
  console.log(`   4. git commit -m "fix: prefix all flow cards with driver name instead of brand"`);
}
