  ...
  while(st.length){
    ...
    if(s.isDirectory()){
      for(const e of fs.readdirSync(cur,{withFileTypes:true})){
        ...
        else if(e.isFile() && /\.m?js$/i.test(e.name)) yield p;
      }
    }
  }
}
function migrateFile(file) {
  ...
  if (hasOnMesh && !hasOnNode) {
    src = src.replace(/class\s+([A-Za-z0-9_]+)\s+extends\s+([A-Za-z0-9_]+)/,
  ...
  if (src !== before) { fs.writeFileSync(file, src); console.log('[migrate] updated', path.relative(process.cwd(),file)); return true; }
  return false;
}
(function(){
  const root = path.join(process.cwd(), 'drivers');
  if (!fs.existsSync(root)) { console.log('[migrate] drivers/ not found'); return; }
  let changed=0; for (const f of jsFiles(root)) { try { const txt = fs.readFileSync(f,'utf8'); if (/homey-meshdriver/.test(txt)) { if (migrateFile(f)) changed++; } } catch {} }
