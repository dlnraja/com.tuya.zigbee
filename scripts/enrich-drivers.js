...
(async function(){
  ...
  const repo=[]; if(fs.existsSync(DRV)){const st=[DRV];while(st.length){const cur=st.pop();let s;try{s=fs.statSync(cur);}catch{continue;}if(s.isDirectory()){const comp=['driver.compose.json','driver.json'].map(n=>path.join(cur,n)).find(p=>fs.existsSync(p));if(comp)repo.push({dir:cur,compose:comp,obj:j(comp)});for(const e of fs.readdirSync(cur))st.push(path.join(cur,e));}}}
