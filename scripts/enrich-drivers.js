...
const BK = [path.join(ROOT, '.backup')].filter(fs.existsSync);

function listCompose(root) {
  const out = [];
...
function M(a, b) { return Array.from(new Set([...(A(a)), ...(A(b))].filter(Boolean))); }

(function() {
  const src = [];
  
  if (fs.existsSync(TMP)) {
    for (const d of fs.readdirSync(TMP, { withFileTypes: true })) {
      if (!d.isDirectory()) continue;
      const dir = path.join(TMP, d.name);
      for (const f of listCompose(dir)) {
        src.push({ file: f, obj: j(f) });
      }
    }
  }
  
  for (const b of BK) {
    for (const f of listCompose(b)) {
      src.push({ file: f, obj: j(f) });
    }
  }
  ...
  if (fs.existsSync(DRV)) {
    ...
    while (st.length) {
      ...
      if (s.isDirectory()) {
        const comp = ['driver.compose.json', 'driver.json'].map(n => path.join(cur, n)).find(p => fs.existsSync(p));
        if (comp) repo.push({ dir: cur, compose: comp, obj: j(comp) });
  ...
  if (!src.length) {
    console.log('[enrich] nothing to merge');
    return;
  }
  ...
  for (const s of src) {
    const o = s.obj || {};
    const z = o.zigbee || {};
    const mans = A(o.manufacturerName || z.manufacturerName);
    const mods = A(o.modelId || z.modelId || o.productId);
    const caps = A(o.capabilities);

    // cible approximative → reorg placera correctement ensuite (domain/cat/vendor/model plus tard)
    let target = null, score = -1;
    for (const r of repo) {
      const R = A(r.obj?.capabilities), S = caps;
      const c = [...new Set(R)].filter(x => new Set(S).has(x)).length;
      ...
      const sc = c * 2 + m1 + m2;
      if (sc > score) { score = sc; target = r; }
    }
    
    if (!target) {
      const id = (mods[0] || mans[0] || 'device').toLowerCase().replace(/[^a-z0-9._-]+/g, '-');
      const dir = path.join(DRV, id);
...
module.exports = Device;`);
      ...
      target = { dir, compose: comp, obj: j(comp) };
      repo.push(target);
      created++;
    }
    ...
    if (JSON.stringify(obj) !== before) {
      fs.writeFileSync(target.compose, JSON.stringify(obj, null, 2));
      merges++;
    }
    
    const assets = path.join(target.dir, 'assets');
