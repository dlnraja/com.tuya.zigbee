...
function injectMFS() {
  ...
  for (const [driverId, mfrs] of Object.entries(driverMfrs)) {
    ...
    const existingMfrsNorm = new Set(compose.zigbee.manufacturerName.map(m => CI.normalize(m)));
    let added = 0;

    for (const mfr of mfrs) {
      if (!existingMfrsNorm.has(CI.normalize(mfr))) {
        compose.zigbee.manufacturerName.push(mfr);
        existingMfrsNorm.add(CI.normalize(mfr));
        added++;
      }
    }
