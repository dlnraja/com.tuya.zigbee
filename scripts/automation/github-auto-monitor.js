...
class GitHubAutoMonitor {
  ...
  /**
   * 🌐 Fetch issues from Johan's repository (PUBLIC API - NO TOKEN NEEDED)
   */
  async fetchJohanIssues() {
    const url = `https://api.github.com/repos/JohanBendz/com.tuya.zigbee/issues?state=open&per_page=50&sort=created&direction=desc`;

    try {
      this.log('INFO', `🔍 Fetching issues from Johan's public repository...`);
      ...
      // Filter device request issues that haven't been processed
      const deviceRequestIssues = issuesData.filter(issue => {
        const isDeviceRequest =
          issue.title.toLowerCase().includes('device') ||
          issue.title.toLowerCase().includes('support') ||
          issue.body?.toLowerCase().includes('manufacturername') ||
          issue.body?.toLowerCase().includes('_tz');
