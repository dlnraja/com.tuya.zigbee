...
function repoUses(mods) {
  ...
  const rx = new RegExp(mods.map(m => `(['"])${m}\\1`).join('|'));
  const files = patterns.flatMap(p => glob.sync(p, { ignore: ['node_modules/**', 'dist/**', '.homeybuild/**', '.git/**'] }));
  return files.some(f => {
    try { return rx.test(fs.readFileSync(f, 'utf8')); } catch { return false; }
  });
}
...
(function() {
  ...
  if (usesZigbeeDriver || DO_MIGRATE) {
    ...
    // si on n'a pas migré, on garde meshdriver mais on ajoute zigbee-clusters si utilisé
    ...
    // défaut (sécurisé) : zigbeedriver moderne
    pkg.dependencies['homey-zigbeedriver'] = pkg.dependencies['homey-zigbeedriver'] || '^2.0.0';
    pkg.dependencies['zigbee-clusters'] = pkg.dependencies['zigbee-clusters'] || '^2.0.0';
  }
  ...
  pkg.scripts.mega = pkg.scripts.mega || 'node scripts/mega-tuya-ultimate.js';
