...
- **P2484** GH#548 Wuma diag `97413373` @ **9.0.908**: crash = **heap OOM** (not motionsensor). Root: `TuyaEF00Manager.initialize()` re-entrant via `sendTimeSync` → stacked `.on()` listeners (`MaxListenersExceeded` 51). Idempotent init + listener bind-once + gate DIAGNOSTIC-MASSIF. GH#547 HiepSVG `_TZE204_gkfbdvyx`+`TS0601`: compose had FP but **Athom compact dropped mfr** (not sacred-kept) → Unknown Zigbee; pin sacred-keep. BOTH. Tip after publish.
