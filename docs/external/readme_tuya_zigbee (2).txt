...
- ⚙️ Generate **intelligent and complete support** for unknown, legacy, hybrid, or exotic Tuya Zigbee devices
...
- 🎯 Ensure **all drivers in all branches** are as complete, optimized, and functional as possible per use case
...
5. **Ensure `tuya-light` follows its lightweight philosophy**:
   - ✅ Drivers must remain fully complete, optimized, debugged and per-device functional
   - 🚫 Do not include automation, CI, extra tooling, or markdown/docs in `tuya-light`
   - ✅ It may contain all known and tested `.driver.compose.json` (even complex ones)
   - Add GitHub Action that **fails** if non-driver or non-relevant files are committed
   - README must clearly explain this minimal fallback yet functional role
...
- Create or restore `.github/workflows/*.yml` with correct jobs:
  ...
  - Enforce tuya-light content rule (no automation/config tooling)
...
- Enrich legacy, unknown or partial drivers to make them fully functional
- Ensure drivers cover **the widest range of devices intelligently and reliably**, even in minimal repos
...
- With **fully complete, optimized drivers** in both branches
- `tuya-light`: lightweight but full driver functionality, no automation
- `master`: all features, CI, dashboard, translations, tooling
- Automatically maintainable, cleanly versioned, and fallback-capable
