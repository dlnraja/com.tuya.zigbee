...
2. **PhysicalButtonMixin** (`lib/mixins/PhysicalButtonMixin.js`)
   ...
   - v8.3.0: Memory leak fix — `_metricsSyncInterval` saved and cleared on deletion
...
## v8.3.0 Bug Fixes (07/06/2026)

### Fix 1: Memory Leak — `_metricsSyncInterval` Not Cleared
- **Root cause:** `PhysicalButtonMixin.js` called `this.homey.setInterval()` for periodic metrics sync but never saved the interval ID. On device deletion, the interval continued firing against a destroyed device, causing memory leaks and potential crashes.
- **Fix:** Saved interval to `this._metricsSyncInterval` and added `.clear()` in `_cleanupPhysicalButtonDetection()`.
- **Affected drivers:** All drivers using `PhysicalButtonMixin` (plug_smart, device_plug_smart, device_plug_smart_water, sensor_contact_plug, wall_dimmer_1gang_1way, etc.)

### Fix 2: Flow Card `.trigger()` Never Called
- **Root cause:** `getDeviceTriggerCard(flowId)` was called and stored, but `.trigger(tokens)` was never invoked. Physical button presses produced no flow card triggers.
- **Fix:** Changed to `getDeviceTriggerCard(flowId).trigger(tokens).catch(() => {})`.
- **Affected drivers:** plug_smart, device_plug_smart, device_plug_smart_water, sensor_contact_plug

### Fix 3: `await` Without `async` in ZCL Callbacks
- **Root cause:** ZCL cluster `attrReport` and `cmd` callbacks used `await` inside non-async functions. This passes Node.js syntax validation but causes `SyntaxError` on Node.js v22+.
- **Fix:** Added `async` keyword to all ZCL callback functions containing `await`.
- **Affected drivers:** wall_dimmer_1gang_1way, plug_smart, device_plug_smart, device_plug_smart_water, sensor_contact_plug

### Fix 4: Missing `onDeleted()` Cleanup
- **Root cause:** `wall_dimmer_1gang_1way/device.js` had no `onDeleted()` override. Timer leaks on device removal.
- **Fix:** Added `onDeleted()` with `this._destroyed = true` and `super.onDeleted()`.

### Fix 5: Duplicate `initPhysicalButtonDetection()` Calls
- **Root cause:** Some drivers called `initPhysicalButtonDetection()` twice — once with proper arguments, once without. The second call silently failed but wasted resources.
...
**Cause**: Missing cluster bindings, scene mode not activated, or `.trigger()` never called
**Solution**: Verify `setupButtonDetection()`, `_universalSceneModeSwitch()`, and that `getDeviceTriggerCard().trigger().catch()` is used

### Double Triggers
**Cause**: Deduplication window too short or `_virtualPhysicalDedup` not initialized
**Solution**: Ensure `_virtualPhysicalDedup` initialized in `onNodeInit()` before any listener

### Memory Leak After Device Removal
**Cause**: Intervals/timers not cleared in `onDeleted()`/`onUninit()`
**Solution**: Save all `setInterval()`/`setTimeout()` IDs to instance properties, clear them in cleanup methods
