...
## MCU UART Debugging Guide

### Detecting MCU Devices
MCU devices bridge through a serial UART connection. Detection methods:

1. **Header Detection**: TuyaDPParser detects `0x55 0xAA` at start of DP payload
2. **Manufacturer Prefix**: `_TZE200/204/284` typically indicates MCU-based device
3. **Product ID**: `TS0601` is almost always MCU-bridged
4. **Endpoint Clusters**: 0xEF00 present without standard ZCL clusters

### MCU Protocol Debugging
```
L4 [DPParser] MCU UART header detected: 0x55 0xAA
L4 [DPParser] Command: 0x07, DataLen: 9
L4 [DPParser] Payload: [0x00, 0x07, YY, MM, DD, HH, MM, SS, Weekday]
L5 [TimeSync] Format detected: TUYA_MCU (confidence: 90%)
```

### Common MCU Issues
| Symptom | Debug Steps |
|---------|-------------|
| Device not responding | Check if MCU header present; verify protocol version |
| Wrong time format | Run guessFormat() to see ranked candidates |
| Payload rejected | Check payload size matches MCU version (8 vs 10 bytes) |
| Sequence mismatch | Ensure sequence number echoed from device request |

## Cluster Binding Debugging

### Binding Verification
```bash
# Check if device is bound to correct cluster
# In Homey Developer Tools: View device interview
# Verify clusters: 0xEF00 (Tuya DP), 0x0006 (OnOff), 0x0008 (Level)

# Debug binding issues:
# 1. Check TuyaBoundCluster.js for binding state
# 2. Verify endpoint matches device expectation
# 3. Check for cluster conflicts (0xEF00 vs 0x0006)
```

### Cluster Conflict Resolution
When a device supports both ZCL and Tuya DP:
1. **IntelligentProtocolRouter** decides which protocol to use
2. **HybridProtocolManager** manages switching between protocols
3. **TuyaBoundCluster** captures commands for Tuya DP routing
4. Check logs for `L2 [ProtocolRouter]` messages to see routing decisions
...
*Updated: MCU UART Debugging Guide, Cluster Binding Debugging*
